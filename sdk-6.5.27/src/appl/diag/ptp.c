...
STATIC char *bcm_ptp_bmca_reason_code_str[] = {
    "Announce Timeout",
    "Better Priority1",
    "Better Class",
    "Better Accuracy",
    "Better Scale_variance",
    "Better Priority2",
    "Better Identity",
    "Better StepsRemoved+1",
    "Better SenderIdentity",
    "Better Topology",
    "Better Source PortNumber",
    "Better Rx PortNumber",
    "Preferred Master",
    "Better SignalFail",
    "Better LocalPriority",
    "Local Grand Master",
    "Unknown",
};
...
#define PACKET_DUMP_COUNT_DEFAULT        10
STATIC uint32 diag_packet_dump_mask[BCM_MAX_NUM_UNITS][PTP_MAX_STACKS_PER_UNIT] = {{0}};
uint32 packet_dump_count = PACKET_DUMP_COUNT_DEFAULT;
#endif
...
#define PTP_CLI_RESULT_RETURN(__cmd_result__)    \
    do {                                         \
      switch ((cmd_result_t)__cmd_result__) {    \
      case CMD_OK:                               \
          PTP_VERBOSE("PASSED\n");               \
          break;                                 \
      case CMD_FAIL:                             \
      ...
      default:                                   \
          PTP_VERBOSE("UNKNOWN\n");              \
          return (cmd_result_t)CMD_FAIL;         \
      }                                          \
      return (cmd_result_t)__cmd_result__;       \
  } while (0)
...
const char* ptp_print_bmca_reason_code(unsigned int reason_code)
{
   if(reason_code < _bcm_ptp_bmca_reason_code_last) {
      return bcm_ptp_bmca_reason_code_str[reason_code];
   } else {
      return bcm_ptp_bmca_reason_code_str[_bcm_ptp_bmca_reason_code_last];
   }
}
...
STATIC char *ptp_clock_accuracy_get(bcm_ptp_clock_accuracy_t *clock_accuracy, char *clock_accuracy_str)
{
    if (*clock_accuracy >= bcmPTPClockAccuracy25ns &&
        *clock_accuracy <= bcmPTPClockAccuracyL10s) {
            sal_strlcpy(clock_accuracy_str, 
                    bcm_ptp_clock_accuracy_str[(int)(*clock_accuracy 
                    - bcmPTPClockAccuracy25ns)], 64);
    }
    else {
        sal_strncpy(clock_accuracy_str, 
                "bcmPTPClockAccuracyUnknown/bcmPTPClockAccuracyReserved", 64);
    }
...
{
#if defined(BCM_DNX_SUPPORT)
    if(PTP_CONDITIONAL_IS_JERICHO2(unit)) {
        /* ptp_parse_sysport_to_modport :
         * Parse DNX sys_port_aggregate to find out mod id, port id & trunk id
         */
        int is_spa = FALSE;
        ...
        *port = 0;
        ...
        SHR_IF_ERR_EXIT(dnx_trunk_system_port_is_spa(unit, sys_port_aggr, &is_spa));
        if (is_spa) {
            /* lag case */
            uint32 flags = 0;
            int pool = 0;
    ...
    for (index = 0; index < dataset.num_foreign_masters; index++) {
...

        {
            ...
            if (SOC_IS_QUX(unit) || SOC_IS_QAX(unit) || SOC_IS_QMX(unit) ||
                PTP_CONDITIONAL_IS_JERICHO2(unit)) {
...
#define PTP_STACK_USAGE \
    ...
    "\t PTP Stack Performance           Prints ToP CPU and Memory Info to debug\n"
...
cmd_ptp_stack(int unit, args_t *a)
{
    ...
    if (parse_cmp("Create", arg, 0)) {
        ...
        if (rv == BCM_E_NONE) {
            ...
            cli_out("\nbcm_ptp_stack_create() Error... (FCN RETURN= %d: %s)\n", rv, _SHR_ERRMSG(rv));
            PTP_CLI_RESULT_RETURN(CMD_FAIL);
        }
...
#define PTP_CLOCK_USAGE \
    ...
    "\t                  NtpTodDisable=<0/1> PtpTodDisable=<0/1>\n "    \
...
cmd_ptp_clock(int unit, args_t *a)
{
    ...
    int arg_holdover_inspec_time = 3600;
    int priority2 = -1;
    int arg_sync_receipt_timeout = 1000;
    int arg_delay_resp_receipt_timeout = 1000;
    ...
    int arg_ntp_tod_disable = 0;
    int arg_ptp_tod_disable = 0;
    ...
    if (parse_cmp("Create", arg, 0)) {
        ...
        parse_table_add(&pt, "HoldoverInspecTime", PQ_INT, (void*)3600, &arg_holdover_inspec_time, NULL);
        ...
        parse_table_add(&pt, "SyncReceiptTo", PQ_INT, (void*)1000, &arg_sync_receipt_timeout, NULL);
        parse_table_add(&pt, "DelayRespReceiptTo", PQ_INT, (void*)1000, &arg_delay_resp_receipt_timeout, NULL);
        parse_table_add(&pt, "NtpTodDisable", PQ_INT, (void*)0, &arg_ntp_tod_disable, NULL);
        parse_table_add(&pt, "PtpTodDisable", PQ_INT, (void*)0, &arg_ptp_tod_disable, NULL);
        ...
        sal_memcpy(clock_info.clock_identity, clock_id, sizeof(clock_id));
        ...
        if (arg_sync_receipt_timeout >= 0){
            clock_info.sync_receipt_timeout_msecs = arg_sync_receipt_timeout;
        }

        if (arg_delay_resp_receipt_timeout >= 0){
            clock_info.delay_resp_receipt_timeout_msecs = arg_delay_resp_receipt_timeout;
        }

        if (PTP_UC_FEATURE_CHECK(PTP_NTP_PTP_TOD_DISABLE)) {
            if (arg_ntp_tod_disable) {
                clock_info.flags |= (1u << BCM_PTP_CLOCK_FLAGS_NTP_TOD_UPDATE_DISABLE);
            }

            if (arg_ptp_tod_disable) {
                clock_info.flags |= (1u << BCM_PTP_CLOCK_FLAGS_PTP_TOD_UPDATE_DISABLE);
            }
        }
...
cmd_ptp_port(int unit, args_t *a)
{
    ...
    if (parse_cmp("Configure", arg, 0)) {
        ...
        parse_table_add(&pt, "InVlan", PQ_INT, (void*)1, &vlan_inner, NULL);
        parse_table_add(&pt, "TpidOUT", PQ_INT, (void*)0x8100, &tpid_out, NULL);
        parse_table_add(&pt, "TpidIN", PQ_INT, (void*)0x88a8, &tpid_in, NULL);
        ...
        if (ip != 0) {
            ...
            sal_memcpy(port_info.multicast_l2, ipv4_multicast_mac, 6);
            sal_memcpy(port_info.multicast_l2 + 6, port_info.mac, 6);
            port_info.multicast_l2[12] = (uint8)(tpid_out >> 8);
            port_info.multicast_l2[13] = (uint8)(tpid_out);
            port_info.multicast_l2[14] = (uint8)(vlantag >> 8);
            port_info.multicast_l2[15] = (uint8)(vlantag);
            if (vlan_inner != 1) {
                port_info.multicast_l2[16] = (uint8)(tpid_in >> 8);
                port_info.multicast_l2[17] = (uint8)(tpid_in);
                port_info.multicast_l2[18] = (uint8)(vlan_inner >> 8);
                port_info.multicast_l2[19] = (uint8)(vlan_inner);
                port_info.multicast_l2[20] = 0x08;
                port_info.multicast_l2[21] = 0x00;
                port_info.multicast_l2_size = 22;
                cli_out("\n Outer[%04x %04x] Inner[%04x %04x] EthType[0x0800]\n", tpid_out, vlantag, tpid_in, vlan_inner);
            } else {
            ...
            sal_memcpy(port_info.multicast_pdelay_l2, ipv4_multicast_pdelay_mac, 6);
            sal_memcpy(port_info.multicast_pdelay_l2 + 6, port_info.mac, 6);
            ...
            sal_memcpy(port_info.multicast_l2, ipv6_multicast_mac, 6);
            sal_memcpy(port_info.multicast_l2 + 6, port_info.mac, 6);
            port_info.multicast_l2[12] = (uint8)(tpid_out >> 8);
            port_info.multicast_l2[13] = (uint8)(tpid_out);
            port_info.multicast_l2[14] = (uint8)(vlantag >> 8);
            ...
            if (vlan_inner != 1) {
                ...
                port_info.multicast_l2[18] = (uint8)(vlan_inner >> 8);
                port_info.multicast_l2[19] = (uint8)(vlan_inner);
                port_info.multicast_l2[20] = 0x86;
                port_info.multicast_l2[21] = 0xdd;
                port_info.multicast_l2_size = 22;
                cli_out("\n Outer[%04x %04x] Inner[%04x %04x] EthType[0x86dd]\n", tpid_out, vlantag, tpid_in, vlan_inner);
            } else {
            ...
            sal_memcpy(port_info.multicast_pdelay_l2, ipv6_multicast_pdelay_mac, 6);
            sal_memcpy(port_info.multicast_pdelay_l2 + 6, port_info.mac, 6);
            port_info.multicast_pdelay_l2[12] = 0x81;
            port_info.multicast_pdelay_l2[13] = 0x00;
            port_info.multicast_pdelay_l2[14] = (uint8)(vlantag >> 8);
            ...
            sal_memcpy(port_info.multicast_l2, l2_multicast_mac, 6);
            sal_memcpy(port_info.multicast_l2 + 6, port_info.mac, 6);
            port_info.multicast_l2[12] = (uint8)(tpid_out >> 8);
            port_info.multicast_l2[13] = (uint8)(tpid_out);
            port_info.multicast_l2[14] = (uint8)(vlantag >> 8);
            ...
            if (vlan_inner != 1) {
                ...
                port_info.multicast_l2[18] = (uint8)(vlan_inner >> 8);
                port_info.multicast_l2[19] = (uint8)(vlan_inner);
                port_info.multicast_l2[20] = 0x88;
                port_info.multicast_l2[21] = 0xf7;
                port_info.multicast_l2_size = 22;
                cli_out("\n Outer[%04x %04x] Inner[%04x %04x] EthType[0x88f7]\n", tpid_out, vlantag, tpid_in, vlan_inner);
            } else {
            ...
            sal_memcpy(port_info.multicast_pdelay_l2, l2_multicast_pdelay_mac, 6);
            sal_memcpy(port_info.multicast_pdelay_l2 + 6, port_info.mac, 6);
            port_info.multicast_pdelay_l2[12] = 0x81;
            port_info.multicast_pdelay_l2[13] = 0x00;
            port_info.multicast_pdelay_l2[14] = (uint8)(vlantag >> 8);
        ...
        if (port_type == bcmPTPPortTypeVirtual ||
            ...
            if (fmt_str) {
                ...
                port_info.vport_info.format = bcmPTPTODFormatChinaTcom; /* CM/CT use same enumerator. */
        ...
        if (parse_cmp("Show", arg, 0)) {
            ...
            sal_memset(&per_port_packet_drop_counters, 0x0, sizeof(per_port_packet_drop_counters));
            BCM_IF_ERROR_RETURN(
...
cmd_ptp_tod_in(int unit, args_t *a)
{
    ...
    if (tod.source != bcmPTPTODSourceNone) {
    ...
    if (format_str) {
        ...
        tod.format = bcmPTPTODFormatChinaTcom; /* CM/CT use same enumerator. */
    } else if (parse_cmp("ChinaTelecom", format, 0)) {
        tod.format = bcmPTPTODFormatChinaTcom; /* CM/CT use same enumerator. */
    } else if (parse_cmp("BCMTS", format, 0)) {
        tod.format = bcmPTPTODFormatBCMTS;
...
#ifdef COMPILER_HAS_LONGLONG
STATIC void _bcm_ptp_diag_apts_get_op_mode_string(bcm_ptp_clock_apts_mode_t op_mode, char *ret_str)
{
    switch (op_mode){
...
STATIC int
_bcm_ptp_is_ext_servo_type_idt_servo(int unit, bcm_ptp_stack_id_t stack_id, int clock_num)
{
    int is_ext_servo_type_idt_servo = 1;
    ...
    {
#ifdef BCM_PTP_EXT_SERVO_SUPPORT
        is_ext_servo_type_idt_servo = 1;

        if (PTP_CONDITIONAL_IS_MONTEREY(unit) ||
            PTP_CONDITIONAL_IS_QAX(unit) ||
            PTP_CONDITIONAL_IS_FIRELIGHT(unit)) {
            /* IDT servo is currently supported on QAX/QUX/MO/FL. */
        } else if (PTP_CONDITIONAL_IS_KATANA(unit) || PTP_CONDITIONAL_IS_TRIUMPH3(unit) || PTP_CONDITIONAL_IS_TRIDENT2(unit) ||
            PTP_CONDITIONAL_IS_ARAD(unit) || PTP_CONDITIONAL_IS_KATANA2(unit) || PTP_CONDITIONAL_IS_HURRICANE2(unit) ||
            PTP_CONDITIONAL_IS_GREYHOUND(unit) || PTP_CONDITIONAL_IS_SABER2(unit) || PTP_CONDITIONAL_IS_TOMAHAWK(unit) ||
            PTP_CONDITIONAL_IS_APACHE(unit) || PTP_CONDITIONAL_IS_HURRICANE3(unit)) {
            /* list of devices where IDT servo not integrated */
            is_ext_servo_type_idt_servo = 0;
        }
#else
        is_ext_servo_type_idt_servo = 0;
#endif
    ...
    return (is_ext_servo_type_idt_servo);
}

/* Displaying legacy Symmtricom Servo Information */

STATIC cmd_result_t
_bcm_ptp_display_ext_servo_type_non_idt_servo_info(int unit, bcm_ptp_stack_id_t stack_id, int clock_num)
{
    bcm_ptp_servo_config_t config;
    ...
    PTP_IF_ERROR_RETURN(bcm_ptp_servo_configuration_get(unit, stack_id, clock_num, &config));
    PTP_IF_ERROR_RETURN(_bcm_ptp_servo_performance_get(unit, stack_id, clock_num, &perf));
...
cmd_ptp_servo_perf_get(int unit, args_t *a)
{
    ...
    if (config.servo == bcmPTPServoTypeExt  &&
        !_bcm_ptp_is_ext_servo_type_idt_servo(unit, arg_stack_id, arg_clock_num)) {

        _bcm_ptp_display_ext_servo_type_non_idt_servo_info(unit, arg_stack_id, arg_clock_num);

        PTP_CLI_RESULT_RETURN(CMD_OK);
    }

    PTP_IF_ERROR_RETURN(bcm_ptp_servo_performance_get(unit, arg_stack_id, arg_clock_num, &perf));
...
#ifdef COMPILER_HAS_LONGLONG
    {
        ...
        /* Display APTS information */
        {
            int apts_enabled;
            char curr_op_mode_str[30] = {0};
            bcm_ptp_clock_apts_mode_t op_mode;

            bcm_ptp_clock_apts_enable_get(unit, arg_stack_id, arg_clock_num, &apts_enabled);
            bcm_ptp_clock_apts_mode_get(unit, arg_stack_id, arg_clock_num, &op_mode);
            _bcm_ptp_diag_apts_get_op_mode_string(op_mode, curr_op_mode_str);
            if (apts_enabled)
            {
                cli_out("\n");
                cli_out("\t Current APTS Mode              : %s\n", curr_op_mode_str);
                cli_out("\n");
            }
        }
    }
...
cmd_ptp_servo_freq_corr_get(int unit, args_t *a)
{
    ...
    PTP_IF_ERROR_RETURN(_bcm_ptp_freq_corr_get(unit, PTP_STACK_ID_DEFAULT, PTP_CLOCK_NUMBER_DEFAULT, &corr, &offset, NULL, NULL));
...
cmd_ptp_servo_config(int unit, args_t *a)
{
    ...
    parse_table_add(&pt, "FilterProfile", PQ_INT, (void*)(size_t)invalid_value, &arg_filter_profile, NULL);
    ...
    if (got_set_data) {
        ...
        if (arg_filter_profile != invalid_value) {
            config.filter_profile = arg_filter_profile;
        } else {
...
cmd_result_t _bcm_ptp_find_slave_port (int unit, bcm_ptp_stack_id_t stack_id, int clock_num, int *ptp_slave_port) {
...
cmd_ptp_master(int unit, args_t *a)
{
    ...
    if (!arg) {
        ...
        i = 0;
        sal_memcpy(master.address.raw_l2_header, port_data.multicast_l2, port_data.multicast_l2_size);
        /* update destination mac with that of Master's */
        sal_memcpy(master.address.raw_l2_header, arg_master_mac, sizeof(bcm_mac_t));
        master.address.raw_l2_header_length = port_data.multicast_l2_size;
        LOG_DEBUG(BSL_LS_BCM_PTP,
                            (BSL_META_U(unit, "L2 Hdr %d bytes: %02x%02x%02x%02x%02x%02x %02x%02x%02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x%02x%02x %02x%02x\n"),
            master.address.raw_l2_header_length,
            master.address.raw_l2_header[0], master.address.raw_l2_header[1],master.address.raw_l2_header[2],master.address.raw_l2_header[3],
            master.address.raw_l2_header[4], master.address.raw_l2_header[5],master.address.raw_l2_header[6],master.address.raw_l2_header[7],
            master.address.raw_l2_header[8], master.address.raw_l2_header[9],master.address.raw_l2_header[10],master.address.raw_l2_header[11],
            master.address.raw_l2_header[12], master.address.raw_l2_header[13],master.address.raw_l2_header[14],master.address.raw_l2_header[15],
            master.address.raw_l2_header[16],master.address.raw_l2_header[17],master.address.raw_l2_header[18],master.address.raw_l2_header[19],
            master.address.raw_l2_header[20],master.address.raw_l2_header[21],master.address.raw_l2_header[22],master.address.raw_l2_header[23],
            master.address.raw_l2_header[24],master.address.raw_l2_header[25]));
...
cmd_ptp_slave(int unit, args_t *a)
{
    ...
    if (!arg) {
        ...
        i = 0;
        sal_memcpy(slave.peer_address.raw_l2_header, arg_slave_mac, sizeof(bcm_mac_t));
        i += sizeof(bcm_mac_t);
        sal_memcpy(slave.peer_address.raw_l2_header + i, port_data.mac, sizeof(bcm_mac_t));
        i += sizeof(bcm_mac_t);
...
cmd_ptp_peer_dataset_get(int unit, args_t *a)
{
    ...
    for (index = 0; index < num_peers; index++) {
        ...
        if (PTP_UC_FEATURE_CHECK(PTP_REMOTE_PORT_NUMBER)) {
            cli_out("\t Port ID     : %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%04x\n",
                    ...
                    peers[index].remote_port_number);
        } else {
...
#if (defined(BCM_CMICM_SUPPORT) || defined(BCM_CMICX_SUPPORT)) && defined(BCM_PTP_INTERNAL_STACK_SUPPORT)
extern int _bcm_ptp_tag_send(int unit, uint16 *tags, int num_tags, int uC);
#endif
STATIC cmd_result_t
cmd_ptp_register(int unit, args_t *a)
{
    ...
    while (arg) {
        if (parse_cmp("ARP", arg, 0)) {
...
#if (defined(BCM_CMICM_SUPPORT) || defined(BCM_CMICX_SUPPORT)) && defined(BCM_PTP_INTERNAL_STACK_SUPPORT)
        } else if (parse_cmp("Tags", arg, 0)) {
            uint16 tags[MOS_MSG_SUBCLASS_PTP_TAG_COUNT];
            uint16 num = 0;
            arg = ARG_GET(a);
            while (arg) {
                if (!isint((char *)arg)) {
                ...
                tags[num] = parse_integer((char *)arg);
                num++;

                if (num >= MOS_MSG_SUBCLASS_PTP_TAG_COUNT)
                    break;
            ...
            PTP_IF_ERROR_RETURN(_bcm_ptp_tag_send(unit, tags, num, 1));
            PTP_CLI_RESULT_RETURN(CMD_OK);
#endif
        } else {
            PTP_CLI_RESULT_RETURN(CMD_USAGE);
...
#define PTP_TELECOM_USAGE_2 \
    ...
    "\t PTP TELecom SLaVe ClkClass Add [Class=<value>] [Enable=y/n]\n"               \
    "\t PTP TELecom SLaVe ClkClass Remove [Class=<value>]\n"                         \
    "\t PTP TELecom SLaVe ClkClass SHow\n"                                           \
...
cmd_ptp_telecom(int unit, args_t *a)
{
    ...
    int arg_port_vlan = 1;
    int arg_port_prio = 0;
    int arg_port_vlan_in = 1;
    int arg_port_tpid_out = 0x8100;
    int arg_port_tpid_in = 0x88a8;
    char *arg_timestamp_mechanism = 0;
    ...
    int arg_ClkClassEnable = 1, arg_ClkClass = 0;
    ...
    char *tokstr =NULL;
    char uint64_decimal_string[27];
    _bcm_ptp_telecom_g8265_custom_clock_class_t custom_clock_class[PTP_TELECOM_MAX_CUSTOM_CLKCLASS_ENTRIES];
    int arg_ntp_tod_disable = 0;
    int arg_ptp_tod_disable = 0;
    ...
    if (!arg) {
        ...
        if (!arg) {
            ...
            if (PTP_UC_FEATURE_CHECK(PTP_NTP_PTP_TOD_DISABLE)) {
                if (arg_ntp_tod_disable) {
                    clockInfo.flags |= (1u << BCM_PTP_CLOCK_FLAGS_NTP_TOD_UPDATE_DISABLE);
                }

                if (arg_ptp_tod_disable) {
                    clockInfo.flags |= (1u << BCM_PTP_CLOCK_FLAGS_PTP_TOD_UPDATE_DISABLE);
                }
            }
        ...
        if (!arg) {
            ...
            parse_table_add(&pt, "MAC", PQ_MAC, (void*)arg_port_mac,
                            &arg_port_mac, NULL);
            parse_table_add(&pt, "TpidOUT", PQ_INT, (void*)0x8100, &arg_port_tpid_out, NULL);
            parse_table_add(&pt, "TpidIN", PQ_INT, (void*)0x88a8, &arg_port_tpid_in, NULL);
            parse_table_add(&pt, "VLAN", PQ_INT, (void*)1,
                            &arg_port_vlan, NULL);
            parse_table_add(&pt, "InVlan", PQ_INT, (void*)1,
                            &arg_port_vlan_in, NULL);
            ...
            sal_memcpy(portInfo.multicast_l2 + 6, portInfo.mac, 6);
            portInfo.multicast_l2[12] = (uint8)(arg_port_tpid_out >> 8);
            portInfo.multicast_l2[13] = (uint8)(arg_port_tpid_out);
            portInfo.multicast_l2[14] = (uint8)(portInfo.rx_packets_vlan >> 8);
            portInfo.multicast_l2[15] = (uint8)(portInfo.rx_packets_vlan);
            if (arg_port_vlan_in != 1) {
                portInfo.multicast_l2[16] = (uint8)(arg_port_tpid_in >> 8);
                portInfo.multicast_l2[17] = (uint8)(arg_port_tpid_in);
                portInfo.multicast_l2[18] = (uint8)(arg_port_vlan_in >> 8);
                portInfo.multicast_l2[19] = (uint8)(arg_port_vlan_in);
                portInfo.multicast_l2[20] = 0x08;
                portInfo.multicast_l2[21] = 0x00;
                portInfo.multicast_l2_size = 22;
                cli_out("\n Outer[%04x %04x] Inner[%04x %04x] EthType[0x0800]\n", arg_port_tpid_out, portInfo.rx_packets_vlan, arg_port_tpid_in, arg_port_vlan_in);
            } else {
                portInfo.multicast_l2[16] = 0x08;
                portInfo.multicast_l2[17] = 0x00;
                portInfo.multicast_l2_size = 18;
            }
        ...
        if (!arg) {
        ...
        } else if (parse_cmp("ClkClass", arg, 0)) {
            ...
            if (!arg) {
                ...
                parse_table_init(unit, &pt);
                parse_table_add(&pt, "Class", PQ_INT, (void*)1,
                                &arg_ClkClass, NULL);
                parse_table_add(&pt, "Enable", PQ_BOOL, (void*)1,
                                &arg_ClkClassEnable, NULL);
                if (parse_arg_eq(a, &pt) < 0) {
                    cli_out("%s: Invalid option %s\n", ARG_CMD(a), ARG_CUR(a));
                }
                parse_arg_eq_done(&pt);

                _bcm_ptp_telecom_g8265_custom_clock_class_add(unit, arg_stack_id,
                    arg_clock_num, arg_ClkClass, arg_ClkClassEnable);
            } else if (parse_cmp("Remove", arg, 0)) {
                parse_table_init(unit, &pt);
                parse_table_add(&pt, "Class", PQ_INT, (void*)1,
                                &arg_ClkClass, NULL);
                ...
                _bcm_ptp_telecom_g8265_custom_clock_class_remove(unit, arg_stack_id,
                    arg_clock_num, arg_ClkClass);
            } else if (parse_cmp("SHow", arg, 0)) {
                int tmp = 0;

                _bcm_ptp_telecom_g8265_custom_clock_class_list_get(unit, arg_stack_id,
                    arg_clock_num, custom_clock_class);
                for (tmp=0; tmp < PTP_TELECOM_MAX_CUSTOM_CLKCLASS_ENTRIES; tmp++) {
                     cli_out("Entry No: %d Clock Class: %d Enable: %d\n",
                        tmp, custom_clock_class[tmp].clock_class, custom_clock_class[tmp].enable);
                }
...
int ts_offset_cb(int unit, void *user_data, uint32 *cb_data_ptr, bcm_time_capture_cb_type_t status)
{
    ...
    if (*cb_data_ptr == 0){
        BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_TS0_FREQ_CTRL_UPDATE_UPPERr, 0, 0, &regval_upper);
        regval_upper &= 0xFFFF;
        BCM_TIME_WRITE_NS_REGr(unit, NS_TIMESYNC_TS0_FREQ_CTRL_UPDATE_UPPERr, 0, 0, regval_upper);
        BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_TS0_TIMESTAMP_UPPER_STATUSr, 0, 0, &regval_upper);
        BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_TS0_TIMESTAMP_LOWER_STATUSr, 0, 0, &regval_lower);
    } else {
        BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_TS1_FREQ_CTRL_UPDATE_UPPERr, 0, 0, &regval_upper);
        regval_upper &= 0xFFFF;
        BCM_TIME_WRITE_NS_REGr(unit, NS_TIMESYNC_TS1_FREQ_CTRL_UPDATE_UPPERr, 0, 0, 0);
        BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_TS1_TIMESTAMP_UPPER_STATUSr, 0, 0, &regval_upper);
        BCM_TIME_READ_NS_REGr(unit, NS_TIMESYNC_TS1_TIMESTAMP_LOWER_STATUSr, 0, 0, &regval_lower);
    }
...
/* PTP Packet ... */
#define PTP_PACKETDUMP_USAGE    \
"PTP PacKeTDump [[+/-]<option> ...] [Count=<value>]\n\t"    \
   " Set or clear debug options, +option adds to list of output, \n\t"    \
   " -option removes from list, no +/- toggles current state. \n\t"    \
   " If no options given, a current list is printed out.\n\t"    \
   " If no count given, default or previous set is value is used.\n"

STATIC cmd_result_t
cmd_ptp_packet_dump(int unit, args_t *a)
{
    parse_table_t pt;
    int count = 0;
    ...
    parse_pm_t opt_list[] = {
        { "@PacKeTDump_All",          0x00000003 },
        { "PacKeTDump_Tx",            0x00000001 },
        { "PacKeTDump_Rx",            0x00000002 },
        { 0, 0}
    };

    parse_table_init(unit, &pt);
    parse_table_add(&pt, "Count", PQ_INT, (void*)0, &count, NULL);

    if (parse_arg_eq(a, &pt) < 0) {
        cli_out("%s: Invalid option %s\n", ARG_CMD(a), ARG_CUR(a));
        cli_out(PTP_PACKETDUMP_USAGE);
        PTP_CLI_RESULT_RETURN(CMD_FAIL);
    }
    ...
    if (ARG_CNT(a) == 0) {
        cli_out("Packet Dump is enabled for:\n\n");
        parse_mask_format(55, opt_list, diag_packet_dump_mask[unit][stack_id]);
        cli_out("Current Count set %u\n", packet_dump_count);
        cli_out("Packet Dump is disabled for:\n\n");
        parse_mask_format(55, opt_list, ~diag_packet_dump_mask[unit][stack_id]);
        cli_out("\n");
        cli_out("Other permissible options:  PacKeTDump_All\n");
        PTP_CLI_RESULT_RETURN(CMD_OK);
    }

    arg = ARG_CUR(a);
    while (arg) {
        /* extract packet dump mask */
        if (arg[0] == '0' && arg[1] == 'x') {
            diag_packet_dump_mask[unit][stack_id] = parse_integer((char*)arg);
        } else if (parse_mask(arg, opt_list, &diag_packet_dump_mask[unit][stack_id])) {
            /* Not a mask arg.  Break from 'while' so other args are parsed */
            break;
        }
        ARG_NEXT(a);
    ...
    if (ARG_CNT(a)) {
        ...
        if (parse_arg_eq(a, &pt) < 0) {
            ...
            if (count) {
                packet_dump_count = count;
            }

        }
    }

    PTP_IF_ERROR_RETURN(_bcm_ptp_packet_dump_configure(unit, stack_id, clock_num,
                diag_packet_dump_mask[unit][stack_id],
                packet_dump_count));
    PTP_CLI_RESULT_RETURN(CMD_OK);
}
...
tdpll_instance_show(int unit, int low_idx, int high_idx)
{
...
#if defined (BCM_APACHE_SUPPORT) ||  defined (BCM_MONTEREY_SUPPORT) ||  defined (BCM_FIRELIGHT_SUPPORT)
    ...
    int output_idx[15] = {5, 6, 7, 8, 9, 10, 1000, 1000, 4, 0, 1, 2, 3, 11, 12};
    ...
    if (SOC_IS_APACHE(unit) && !SOC_IS_MONTEREY(unit)) {
        ...
        cli_out("#  GP0  GP1  GP2  GP3  GP4  GP5  SE1  SE2  1588  BS0  BS1  Synce1  Synce2  Synce3  Synce4  BW   PBO\n");
        /* GPIO0 - GPIO 5 */
        output_idx[0] = 7; output_idx[1] = 8; output_idx[2] = 9; output_idx[3] = 10; output_idx[4] = 11; output_idx[5] = 12;
        /* SE1, SE2  */
        output_idx[6] = 1000; output_idx[7] = 1000;
        /* 1588  */
        output_idx[8] = 6;
        /* BS0, BS1  */
        output_idx[9] = 0; output_idx[10] = 1;
        /* Synce1 -> Synce4  */
        output_idx[11] = 2; output_idx[12] = 3; output_idx[13] = 4; output_idx[14] = 5;
    } else if (SOC_IS_FIRELIGHT(unit)){
        cli_out("#  GP0  GP1  GP2  GP3  GP4  GP5  SE1  SE2  1588  BS0  BS1    -       -    BW   PBO\n");
        /* GPIO0 - GPIO 5 */
        output_idx[0] = 5; output_idx[1] = 6; output_idx[2] = 7; output_idx[3] = 8; output_idx[4] = 9; output_idx[5] = 10;
        /* SE1, SE2  */
        output_idx[6] = 1000; output_idx[7] = 1000;
        /* 1588  */
        output_idx[8] = 4;
        ...
        output_idx[9] = 0; output_idx[10] = 1;
        /* Synce1 -> Synce4  */
        output_idx[11] = 1000; output_idx[12] = 1000; output_idx[13] = 1000; output_idx[14] = 1000;
    } else {
...
cmd_tdpll_instance(int unit, args_t *a)
{
    ...
    char *output_str_fl[TDPLL_OUTPUT_CLOCK_NUM_MAX] =
        {"BroadSync0    ", "BroadSync1    ", "SyncE1        ", "SyncE2        ", "1588-Hybrid   ",
        "GPIO0", "GPIO1", "GPIO2", "GPIO3", "GPIO4", "GPIO5"};
#endif
#if defined (BCM_QAX_SUPPORT)
    ...
    if (!arg || parse_cmp("show", arg, 0)) {
        ...
        if (SOC_IS_MONTEREY(unit)) {
            ...
            cli_out("             | %s [%s] | %s [%s] |\n"
                    ...
                    "             | %s [%s] | \n"
                    "             | %s [%s] | %s [%s] | %s [%s] | %s [%s] | %s [%s] |\n",
                    output_str_fl[0], output_mask & (1 << 0) ? "x":" ",
                    output_str_fl[1], output_mask & (1 << 1) ? "x":" ",
                    output_str_fl[2], output_mask & (1 << 2) ? "x":" ",
                    output_str_fl[3], output_mask & (1 << 3) ? "x":" ",
                    output_str_fl[4], output_mask & (1 << 4) ? "x":" ",
                    output_str_fl[5], output_mask & (1 << 5) ? "x":" ",
                    output_str_fl[6], output_mask & (1 << 6) ? "x":" ",
                    output_str_fl[7], output_mask & (1 << 7) ? "x":" ",
                    output_str_fl[8], output_mask & (1 << 8) ? "x":" ",
                    output_str_fl[9], output_mask & (1 << 9) ? "x":" ");
        } else
#endif
#if defined(BCM_QAX_SUPPORT)
        if (SOC_IS_QUX(unit)) {
...
int get_input_clock_index(int arg_clock_index, int arg_l1mux_port, int arg_l1mux_index)
{
    ...
    if ((clock_index == 6) && (arg_l1mux_port > 0) && (arg_l1mux_port <= 64)) {
        ...
        cli_out("### get_input_clock_index: skipped clock_index or port modification!!\n");
    }
...
cmd_tdpll_inputclk(int unit, args_t *a)
{
    ...
    char *arg_index_str_fl[TDPLL_INPUT_CLOCK_NUM_MAX+1] =
        {"GPIO0","GPIO1","GPIO2","GPIO3","GPIO4","GPIO5",
         "SyncE1","SyncE2","1588-Freq", "1588-Phase", 0};
#endif
...
#if defined(BCM_FIRELIGHT_SUPPORT)
        if (clock_index_str && SOC_IS_FIRELIGHT(unit)) {
                arg_clock_index = 0;
                while (arg_index_str_fl[arg_clock_index]) {
                    if (parse_cmp(arg_index_str_fl[arg_clock_index], clock_index_str, 0)) {
                        break;
                    }
                    ++arg_clock_index;
                }
                if (!arg_index_str_fl[arg_clock_index]) {
    ...
    if (!arg || parse_cmp("show", arg, 0)) {
        ...
        if (SOC_IS_MONTEREY(unit)) {
            if (arg_clock_index == 6) {
                ...
                if (!isint(l1Mux)) {
                    cli_out("Error!! Invalid L1Mux e.g. 'tdpll in get synce <l1Mux>'\n");
                    return CMD_FAIL;
                }
                arg_l1mux_index = parse_integer(l1Mux);
                if (arg_l1mux_index != 0 && arg_l1mux_index != 1) {
                    cli_out("Error!! Unsupported syncE e.g. 'tdpll in get synce <l1Mux>'\n");
                    return CMD_FAIL;
                }
            }
        ...
        if (SOC_IS_FIRELIGHT(unit)) {
            cli_out("Index    : %d (%s)   L1 MUX: %d.%d\n", arg_clock_index,
                    ...
                    arg_index_str_fl[arg_clock_index] : "Unknown", arg_l1mux.index, arg_l1mux.port);
        } else
#endif
        {
            cli_out("Index    : %d (%s)   L1 MUX: %d.%d\n", arg_clock_index,
...
#if defined (BCM_FIRELIGHT_SUPPORT) || defined(BCM_MONTEREY_SUPPORT)
#define TDPLL_OUTPUTCLK_USAGE_FL\
    "TDPLL OUTputclk <subcommand> [...]\n"                                      \
    "\t TDPLL OUTputclk Get <index>\n"                                          \
    "\t                 where <index> = | BroadSync0    | BroadSync1    |\n"    \
    "\t                                 | N/A           | N/A           |\n"    \
...
#endif
#if (defined(BCM_MONTEREY_SUPPORT) && defined(INCLUDE_GDPLL)) || defined(BCM_FIRELIGHT_SUPPORT)
    char *arg_index_str_fl[TDPLL_OUTPUT_CLOCK_NUM_MAX] =
        {"BroadSync0", "BroadSync1", "N/A", "N/A", "1588-Hybrid",
        "GPIO0", "GPIO1", "GPIO2", "GPIO3", "GPIO4", "GPIO5", 0};
#endif
...
/*    int arg_l1mux_port = 0; */
    ...
    if (arg) {
...
#if (defined(BCM_MONTEREY_SUPPORT) && defined(INCLUDE_GDPLL)) || defined(BCM_FIRELIGHT_SUPPORT)
#if (defined(BCM_MONTEREY_SUPPORT) && defined(INCLUDE_GDPLL))
        if (clock_index_str && SOC_IS_MONTEREY(unit)) {
            arg_clock_index = 0;
        ...
        if (clock_index_str && SOC_IS_FIRELIGHT(unit)) {
            ...
            if (!arg_index_str_fl[arg_clock_index]) {
                cli_out("%s OUTputclk Get/Set: Invalid Clock index '%s'\n", ARG_CMD(a), clock_index_str);
                cli_out(TDPLL_OUTPUTCLK_USAGE_FL);
                return CMD_FAIL;
           }
...
/* PTP ToD Offset */
#define PTP_TOD_OFFSET_USAGE \
    "PTP ToDOffSet                         Set/Get PTP and NTP offset\n" \
    "\t PTP ToDOffSet Set [NtpSec=<seconds>] [NtpNSec=<nanoseconds>]\n" \
    "\t                   [PtpSec=<seconds>] [PtpNSec=<nanoseconds>]\n" \
    "\t PTP ToDOffSet Get\n"

STATIC cmd_result_t
cmd_ptp_tod_offset(int unit, args_t *a)
{
    int rv;
    const char * arg;
    parse_table_t pt;
    bcm_ptp_time_spec_t ptp_offset;
    bcm_ptp_time_spec_t ntp_offset;
    uint32 flags;
    ...
    if (parse_cmp("Set", arg, 0)) {
        ...
        parse_table_add(&pt, "PtpSec", PQ_DFL | PQ_INT64, NULL, &ptp_sec, NULL);
        parse_table_add(&pt, "PtpNSec", PQ_DFL | PQ_INT, NULL, &ptp_nsec, NULL);
        parse_table_add(&pt, "NtpSec", PQ_DFL | PQ_INT64, NULL, &ntp_sec, NULL);
        parse_table_add(&pt, "NtpNSec", PQ_DFL | PQ_INT, NULL, &ntp_nsec, NULL);
        ...
        flags = 0;
        if (ptp_sec != 0 || ptp_nsec != 0) {
            flags |= BCM_PTP_TOD_PTP_OFFSET;

            ptp_offset.isnegative = 0;
            if (ptp_sec < 0) {
                ptp_offset.isnegative = 1;
                ptp_sec = -ptp_sec;
            }

            if ((int32)ptp_nsec < 0) {
                ptp_offset.isnegative = 1;
                ptp_nsec = -((int32)ptp_nsec);
            }

            while (ptp_nsec >= NANOSECONDS_PER_SECOND) {
                ptp_nsec -= NANOSECONDS_PER_SECOND;
            }

            COMPILER_64_SET(ptp_offset.seconds, 0, ptp_sec);
            ptp_offset.nanoseconds = ptp_nsec;
        }

        if (ntp_sec != 0 || ntp_nsec != 0) {
            flags |= BCM_PTP_TOD_NTP_OFFSET;

            ntp_offset.isnegative = 0;
            if (ntp_sec < 0) {
                ntp_offset.isnegative = 1;
                ntp_sec = -ntp_sec;
            }

            if ((int32)ntp_nsec < 0) {
                ntp_offset.isnegative = 1;
                ntp_nsec = -((int32)ntp_nsec);
            }

            while (ntp_nsec >= NANOSECONDS_PER_SECOND) {
                ntp_nsec -= NANOSECONDS_PER_SECOND;
            }

            COMPILER_64_SET(ntp_offset.seconds, 0, ntp_sec);
            ntp_offset.nanoseconds = ntp_nsec;
        }

        rv = bcm_ptp_ntp_tod_offset_set(unit, cur_stack_id, cur_clock_num, flags,
                    &ptp_offset, &ntp_offset);
        if (rv == BCM_E_NONE) {
            PTP_CLI_RESULT_RETURN(CMD_OK);
        } else {
            cli_out("\ncmd_ptp_tod_offset() Error... (FCN RETURN= %d: %s)\n", rv, _SHR_ERRMSG(rv));
            PTP_CLI_RESULT_RETURN(CMD_FAIL);
        }
    } else if (parse_cmp("Get", arg, 0)) {

        flags = BCM_PTP_TOD_PTP_OFFSET | BCM_PTP_TOD_NTP_OFFSET;
        rv = bcm_ptp_ntp_tod_offset_get(unit, cur_stack_id, cur_clock_num, flags,
                    &ptp_offset, &ntp_offset);
        if (rv == BCM_E_NONE) {
            cli_out("PTP ToD Offset %s%usec.%unsec\n",
                        (ptp_offset.isnegative ? "-":""), COMPILER_64_LO(ptp_offset.seconds),
                        ptp_offset.nanoseconds);
            cli_out("NTP ToD Offset %s%usec.%unsec\n",
                        (ntp_offset.isnegative ? "-":""), COMPILER_64_LO(ntp_offset.seconds),
                        ntp_offset.nanoseconds);
            PTP_CLI_RESULT_RETURN(CMD_OK);
        } else {
            cli_out("\ncmd_ptp_tod_offset() Error... (FCN RETURN= %d)\n", rv);
            PTP_CLI_RESULT_RETURN(CMD_FAIL);
        }
    } else {
        PTP_CLI_RESULT_RETURN(CMD_USAGE);
...
char cmd_ptp_usage[] =
    ...
    "  " PTP_MODULAR_USAGE "\n"
    "  " PTP_DIAGSHELL_USAGE "\n"
    "  " PTP_PACKETDUMP_USAGE "\n"
    "  " PTP_TOD_OFFSET_USAGE "\n"
#endif /* __KERNEL__ */
    "  " PTP_HELP_USAGE "\n"
...
static subcommand_t subcommands[] = {
    ...
    {"PacKeTDump", cmd_ptp_packet_dump, PTP_PACKETDUMP_USAGE, 0},
    {"ToDOffSet", cmd_ptp_tod_offset, PTP_TOD_OFFSET_USAGE, 0},
#endif /* __KERNEL__ */
    ...
    void *user_data)
{
    ...
    case _bcm_ptp_master_change_event:
        if (verboseEvents == 2) {
...

            {
                ...
                ptp_cb_printf("\n");
                ptp_cb_printf("   Master Change Reason Code: %s\n", ptp_print_bmca_reason_code((unsigned int)(msg->data[81])));
            }
    ...
    case _bcm_ptp_slave_unavail_event:
        if (verboseEvents == 2) {
             ...
             *    Octet 44...45: Module number of card on which slave is seen.
             *    Octet 46...47: Physical port number of card on which slave is seen.
             *    Octet 48...49: Peer port number
             */
            ...
            if (PTP_UC_FEATURE_CHECK(PTP_REMOTE_PORT_NUMBER)) {
                ptp_cb_printf("   Port Identity: "
                        ...
                        msg->data[3], msg->data[4], msg->data[5], msg->data[6],
                        msg->data[7], msg->data[8], msg->data[9], msg->data[10],
                        _bcm_ptp_uint16_read(msg->data + 48));
            } else {
    ...
    case _bcm_ptp_pdv_thres_exceed_event:
        {
            bcm_ptp_clock_port_address_t address;
            int offset=0, set=0;
            address.addr_type = msg->data[0];
            ...
            ptp_cb_printf("Event %s: PDV_THRESHOLD_EXCEED\n", time_str);
            ptp_cb_printf("   Master Protocol: %u", msg->data[0]);
            ...
            set = msg->data[offset];
            ptp_cb_printf("   PDV Threshold Exceed : %s\n", (set) ? "SET" : "CLEAR");
        }
        break;
