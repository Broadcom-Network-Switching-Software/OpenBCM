...
shr_error_e
sh_cmd_is_fec_ber_proj_supported(
    int unit,
    rhlist_t * list)
{
    SHR_FUNC_INIT_VARS(unit);
...
shr_error_e
sh_cmd_is_fec_ber_proj_not_supported(
    int unit,
    rhlist_t * list)
{
    SHR_FUNC_INIT_VARS(unit);
...
sh_sand_enum_t sh_enum_table_flow_agg_id[] = {
    { /** No need to define value if plugin_str was defined */
     .string = "max",
     .desc = "Number of flow aggregates supported",
     .plugin_str = "DNX_DATA.trunk.flow_agg.nof_flow_aggs-1"},
    {NULL}
};
...
#ifndef NO_FILEIO
#define SH_SAND_ERR_COUNT_EXIT  (10)
    ...
    char *filename)
{
     ...
     */
    sal_strncat_s(filename, command, SH_SAND_MAX_TOKEN_SIZE);
    /*
    ...
    char *gold_path)
{
    ...
    if (utilex_is_prime_flag_set(flags, SH_CMD_CUSTOM_XML_VERIFY))
    {
    ...
    int flags)
{
    ...
    if (!dbx_file_exists(local_filename) && !dbx_file_exists(gold_filepath))
    {
        SHR_CLI_EXIT(_SHR_E_EMPTY, "Both the Local and Gold files don't exist\n");
    }
    ...
    while (NULL != sal_fgets(local_line, SH_SAND_MAX_TOKEN_SIZE - 1, local_result))
    {
        ...
        if (sal_strcasecmp(local_line, gold_line))
        {
            ...
            {
                if ((sal_strstr(gold_line, RHNAME(token_entry)) != NULL) &&
                    (sal_strstr(local_line, RHNAME(token_entry)) != NULL))
                    break;
            }
            /*
            ...
            if (err_count >= SH_SAND_ERR_COUNT_EXIT)
            {
                break;
            }
        }
...
exit:
    ...
    if (token_list != NULL)
    {
        utilex_rhlist_free_all(token_list);
    }
...
static sh_sand_option_t compare_arguments[] = {
    ...
    {"file", SAL_FIELD_TYPE_STR, "File name to compare", "", NULL, NULL, SH_SAND_ARG_QUIET},
    {NULL}
};
...
static sh_sand_man_t resource_man = {
    ...
    .examples = "name=gold\n" "name=FRameWork count=1-5 full\n" "path=\"thread status\" full",
};
...
sh_sand_cmd_t sh_sand_shell_cmds[] = {
    ...
    ,
    {"custom", custom_compare_cmd, NULL, compare_arguments, &compare_man, NULL, NULL,
     SH_CMD_SKIP_EXEC * SH_CMD_NO_XML_VERIFY}
    ,
