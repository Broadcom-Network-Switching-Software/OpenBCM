    ...
    int max_line_width)
{
    ...
    if (!ISEMPTY(prt_ctr->title))
    {
        ...
        /*
         * Allocate the title format string.
         */
        title_format = sal_alloc(total_width, "diag title format string");
        ...
        if (title_width < (total_width - 4))
        {
            ...
            sal_snprintf(title_format, total_width - 1, "| %s%ds ", "%", front_title_padding + title_width);
            ...
            sal_strncat_s(title_format, title_padding, total_width);
            sal_strncat_s(title_format, " |\n", total_width);
        }
        else
        {
            sal_snprintf(title_format, total_width - 1, "| %s-%ds |\n", "%", total_width - 4);
        }
        ...
        if (prt_ctr->list != NULL)
        {
            sal_snprintf(title_format, total_width - 1, "| %s%ds | %s-%ds | %s-%ds |\n",
        ...
        do
        {
            ...
            PRT_COLUMN_ITERATOR(k, prt_column, prt_ctr)
            {
                ...
                {   /**
                    ...
                    sal_memcpy(tmp_str, column[k].next_ptr, token_size);
                    cells_to_print++;
                    /*
    ...
    RHITERATOR(prt_row_start, prt_ctr->list)
    {
        ...
        do
        {
            ...
            PRT_COLUMN_ITERATOR(k, prt_column, prt_ctr)
            {
                token_size = sal_strnlen(column[k].next_ptr, SH_SAND_MAX_TOKEN_SIZE);
                if ((token_size > column[k].width) || prt_token_has_eol(column[k].next_ptr))
                {   /**
                     * Split the string to print portions fitting into the width
                     * Adjust token_size
                    ...
                    switch (prt_column->flex_type)
                    {
                        ...
                        case PRT_FLEX_ASCII_ORIG:
                            token_size = prt_get_token_size(column[k].next_ptr, column[k].width, &next_shift);
                            /*
                             * Check if the current cell is too long and needs to be spread across multiple rows.
                             */
                            if (next_shift == 0)
                            {
                                split_cell = TRUE;
                    ...
                    /*
                     * Update cells_to_print counter only if column width and token_size are different than 0.
                     */
                    if (column[k].width != 0 && token_size != 0)
                    {
                        cells_to_print++;
                    }
                    /*
            ...
            /*
             * If a cell from the previous line was too long and thus split, print empty lines (except for that cell's column)
             * until the cell's contents have been fully added to the table.
             */
            while (split_cell == TRUE)
            {
                /*
                ...
                /*
                 * Iterate through the columns preceding (to the left of) the column whose cell was split (split_col), printing them as empty.
                 */
                k = 0;
                while (k < split_col)
                {
                    prt_printf(column[k].format, "");
                    k++;
                }

                /*
                 * Attempt to split what remains of the cell to fit into the column size.
                 * Note - the cell may be so long that it needs to be split multiple times.
                 */
                token_size = prt_get_token_size(column[k].next_ptr, column[k].width, &next_shift);
                sal_memcpy(tmp_str, column[k].next_ptr, token_size);
                tmp_str[token_size] = 0;
                ...
                /*
                 * Print the next part of the split cell and adjust counters and pointers.
                 */
                prt_printf(column[k].format, to_ptr);
                column[k].next_ptr += (token_size + next_shift);

                /*
                 * Check if the cell needs to be split again.
                 */
                if ((next_shift > 0) || (sal_strnlen(column[k].next_ptr, SH_SAND_MAX_TOKEN_SIZE) <= column[k].width))
                {
                    split_cell = FALSE;
                }
                ...
                k++;
                /*
                 * Iterate through the columns succeeding (to the right of) the split column (split_col), printing them as empty.
                 */
                while (k < prt_ctr->col_num)
                {
                    prt_printf(column[k].format, "");
                    k++;
                }
