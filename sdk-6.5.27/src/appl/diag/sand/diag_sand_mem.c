    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        switch (mem)
        {
            ...
            case FLEXMAC_TINY_MAC_REGISTERSm:
                /*
                 * Skipped memories for the testing because SW shouldn't and can't access the same instance of HW
                 * At each time HW accesses only one instance.
                 */
            ...
            case MSW_PORTID_MAPPING_RX_32_TO_384_LOOKUP_TABLEm:
            case MSW_PORTID_MAPPING_TX_384_TO_32_TAG_LOOKUP_TABLEm:
                /*
                 * BRINGUP_FIX Under investigation memories 
                 */
            case ARB_RX_QPM_0_CAL_MEMm:
            case ARB_RX_QPM_1_CAL_MEMm:
            case ARB_TX_PPM_CAL_MEM_Am:
                return 1;
            default:
                break;
        }
    ...
    char *substr_match)
{
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++)
    {
        ...
        if (substr_match != NULL
            && sal_strncaseindex(SOC_MEM_NAME(unit, mem), substr_match, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                                 SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == NULL
            && sal_strncaseindex(SOC_MEM_UFNAME(unit, mem), substr_match, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                                 SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == NULL)
        {
            continue;
        }
