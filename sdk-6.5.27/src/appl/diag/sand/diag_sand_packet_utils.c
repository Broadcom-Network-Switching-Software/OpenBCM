    ...
    int *length_p)
{
    ...
    SHR_NULL_CHECK(p, _SHR_E_PARAM, "tx_string_packet");
    ...
    if (pkt_len > SAND_PACKET_MAX_SIZE_BYTES)
    {
        SHR_CLI_EXIT(_SHR_E_PARAM, "The packet length (%d) is too large\n", pkt_len);
    }
    ...
    bcm_pkt_t * bcm_pkt)
{
    ...
    if ((pkt_data = sal_alloc(SAND_PACKET_MAX_SIZE_BYTES, "tx_string_packet")) == NULL)
    {
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(bcm_pkt_alloc(unit, packet_length + DNX_MODULE_HEADER_SIZE, 0, &pkt_info));
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        SHR_IF_ERR_EXIT(bcm_port_get(unit, src_port, &port_flags, &interface_info, &mapping_info));
        /*
         * Check if channelized port
         */
        if (port_flags & BCM_PORT_ADD_CONFIG_CHANNELIZED)
        {
        ...
        else
        {
            ...
            bcm_core_t src_port_core = mapping_info.core;
            ...
            /*
             * Traverse CPU ports to find the first CPU port which is on the same core of src_port
             */
            BCM_PBMP_ITER(config.cpu, port_ndx)
            {
                SHR_IF_ERR_EXIT(bcm_port_get(unit, port_ndx, &port_flags, &interface_info, &mapping_info));
                if (src_port_core == mapping_info.core)
                {
                    cpu_channel = mapping_info.channel;
                    has_cpu_port = TRUE;
                    break;
            ...
            if (!has_cpu_port)
            {
                SHR_CLI_EXIT(_SHR_E_PARAM, "Failed to find a channelized port for input Port %d\n", src_port);
            }
        }
    }
#endif
...
sh_pkt_sat_identifier_info_t pkt_sat_identifier_info = { 0 /* Color */ , 0 /*TC*/ };
    ...
    rhhandle_t packet_h)
{
    ...
    identifier.trap_id = pkt_sat_rx_info.sat_trap_id;
    /*
     * Need to configure the same TC/DP values as the ones configured in the packet 
     */
    identifier.color = pkt_sat_identifier_info.color;
    identifier.tc = pkt_sat_identifier_info.tc;
...
/* *INDENT-OFF* */
/* Default field qualifiers used for the SAT trap.
 * These values can be changed in order to configure different SAT collector criteria*/
sh_pkt_qualifier_t pkt_qualifiers[] = {
/*    name,           qial_id,                qual_type                      qual_arg, offset, mask */
    ...
    {"ETH1.VLAN.PCP", bcmFieldQualifyVlanPri, bcmFieldInputTypeLayerAbsolute,  0,      96,     0x7},
    {"ETH1.VLAN.DEI", bcmFieldQualifyVlanCfi, bcmFieldInputTypeLayerAbsolute,  0,      96,     0x1},
    {NULL},
};
/* *INDENT-ON* */

/*Returns the packet qualifier information*/
static sh_pkt_qualifier_t *
diag_sand_packet_get_qual_info(
    char *qualifier_name,
    sh_pkt_qualifier_t * pkt_qualifier)
{
    sh_pkt_qualifier_t *pkt_qual;

    /*
     * Check if the pkt qualifiers have been overwritten
     */
    if (pkt_qualifier == NULL)
    {
        /*
         * Use the default values
         */
        pkt_qual = &pkt_qualifiers[0];
    }
    else
    {
        /*
         ...
         */
        pkt_qual = &pkt_qualifier[0];
    }

    for (; pkt_qual->name != NULL; pkt_qual++)
    {
        if (!sal_strcasecmp(pkt_qual->name, qualifier_name))
        {
            return pkt_qual;
        }
    }
...
/* Create a presel entry/field group.
 * The SAT uses the PMF configuration to capture matched packets.*/
shr_error_e
    ...
    sh_pkt_qualifier_t * pkt_qualifier)
{
    ...
    RHITERATOR(sand_packet, rx_sat_list)
    {
        ...
        RHITERATOR(proto, sand_packet->proto_list)
        {
            ...
            RHITERATOR(field_in, proto->field_list)
            {
                 ...
                 */
                if ((pkt_qual_info = diag_sand_packet_get_qual_info((RHNAME(field_in)), pkt_qualifier)) == NULL)
                {
                    /*
    ...
    SHR_IF_ERR_EXIT(bcm_field_group_add(unit, 0, &fg_info, &pkt_sat_rx_info.fg_id));
    /*
    ...
    int unit)
{
    ...
    pkt_sat_identifier_info.color = 0;
    pkt_sat_identifier_info.tc = 0;
    /*
    ...
    rhhandle_t packet_h)
{
     ...
     * 3. Set Rate Pattern
     */
    if (sand_packet->stream.rate_pattern.rate_pattern_mode == 0)
    {
    ...
    {
        rate_pattern.rate_pattern_mode = sand_packet->stream.rate_pattern.rate_pattern_mode;
        rate_pattern.high_threshold = sand_packet->stream.rate_pattern.high_threshold;
        rate_pattern.low_threshold = sand_packet->stream.rate_pattern.low_threshold;
        rate_pattern.stop_iter_count = sand_packet->stream.rate_pattern.stop_iter_count;
        rate_pattern.stop_burst_count = sand_packet->stream.rate_pattern.stop_burst_count;
        rate_pattern.stop_interval_count = sand_packet->stream.rate_pattern.stop_interval_count;
        rate_pattern.burst_packet_weight = sand_packet->stream.rate_pattern.burst_packet_weight;
        rate_pattern.interval_packet_weight = sand_packet->stream.rate_pattern.interval_packet_weight;
        rate_pattern.flags = sand_packet->stream.rate_pattern.flags;
    }
    ...
    if (sand_packet->flags & SAND_PACKET_STREAM_SA_INCREMENTAL_FIELD)
    {
        ...
        packet_config.packet_edit[0].stamps[0].offset = 80;
    }
    ...
    packet_config.offsets.packet_length_compensation = sand_packet->stream.compensation;
...
shr_error_e
diag_sand_packet_stream_destroy(
    int unit,
    rhhandle_t packet_h_p)
{
    int gtf_id;
    ...
    sand_packet = (sand_packet_t *) packet_h_p;

    /** Destroy the stream */
    gtf_id = sand_packet->stream.gtf_id;
    if (gtf_id != -1)
    {
        SHR_IF_ERR_EXIT(bcm_sat_gtf_destroy(unit, gtf_id));
    }

    /** Clear the packet headers list */
    proto_list = sand_packet->proto_list;
    if (proto_list != NULL)
    {
        signal_output_t *header;
        RHITERATOR(header, proto_list)
        {
            if (header->field_list)
            {
                utilex_rhlist_clean(header->field_list);
            }
        }
        utilex_rhlist_clean(proto_list);
    }

    /** Reset the fields in the handle to default values */
    sal_memset(&sand_packet->stream, 0, sizeof(sand_packet_stream_t));
    sal_memset(&sand_packet->collector, 0, sizeof(sand_packet_collector_t));
    sand_packet->stream.gtf_id = -1;
    sand_packet->core_id = -1;
    sand_packet->packet_size = SAND_PACKET_DEFAULT_SIZE_BYTES;
    sand_packet->flags = 0;
    ...
    uint32 flags)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        SHR_IF_ERR_EXIT(bcm_port_get(unit, src_port, &port_flags, &interface_info, &mapping_info));
        sand_packet->core_id = mapping_info.core;
        /*
         * Check if LAG member
         */
        if (mapping_info.pp_port == 0xFFFFFFFF)
        {
            ...
            /*
             * Switch logical to sysport
             */
            SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_phy_info_get
                            (unit, src_port, DNX_ALGO_GPM_GPORT_TO_PHY_OP_RETRIVE_SYS_PORT, &gport_info));
            BCM_GPORT_SYSTEM_PORT_ID_SET(system_gport, gport_info.sys_port);
            /*
             * Get trunk id from sysport Gport
             */
            SHR_IF_ERR_EXIT(bcm_trunk_find(unit, -1, system_gport, &tid_find));
            /*
             ...
             */
            if ((SAL_BIT(sand_packet->core_id) & allocation_info.core_bitmap) == 0)
            {
                SHR_CLI_EXIT(_SHR_E_PARAM, "Port %d is not a valid member of Trunk %d\n", src_port, tid_find);
            }
            else
            {
                pp_port = allocation_info.pp_port_per_core_array[sand_packet->core_id];
            }
        }
        else
        {
            /*
             * Get pp port directly if src_port is not LAG member
             */
    ...
    int stream_nof)
{
    ...
    for (i_stream = 0; i_stream < stream_nof; i_stream++)
    {
        ...
        if (i_stream != 0)
        {
            PACKET_GRANULARITY(packet_hs[i_stream]) = PACKET_GRANULARITY(packet_hs[0]);
        }
    ...
    int unit)
{
    ...
    {
        BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NON_NATIVE_SET(vlan_port.vlan_port_id, vlan_port_id);
    }
    else
    {
    ...
    int unit)
{
    ...
    if (dnx_data_flow.general.feature_get(unit, dnx_data_flow_general_is_flow_enabled_for_legacy_applications))
    {
        BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NON_NATIVE_SET(vlan_port_id, vlan_port_id);
    }
    else
    {
    ...
    int unit)
{
    ...
    else
    {
        ...
        route.l3a_subnet = host_addr;
        route.l3a_ip_mask = 0xFFFFFFFF;
        route.l3a_vrf = vrf;
        route.l3a_intf = kaps_result;
...
shr_error_e
diag_sand_packet_stream_pattern_create(
    int unit,
    int core_id,
    rhhandle_t packet_h,
    int pkt_length,
    ...
    int granularity)
{
    ...
    if (rate_pattern.rate_pattern_mode != 0)
    {
        sand_packet->stream.rate_pattern = rate_pattern;
    }
...
shr_error_e
diag_sand_packet_stream_with_compensation_create(
    int unit,
    int core_id,
    rhhandle_t packet_h,
    int pkt_length,
    ...
    int compensation)
{
    ...
    if (compensation != 0)
    {
        sand_packet->stream.compensation = compensation;
    }

    SHR_IF_ERR_EXIT(diag_sand_packet_stream_create
                    (unit, core_id, packet_h, pkt_length, time, rate_unit, rate, burst, granularity));
