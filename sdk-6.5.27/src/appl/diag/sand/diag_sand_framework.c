    ...
    uint32 flags)
{
    ...
    {   /** If not found in system list look in local one - not found - error   */
        ...
        if (SHR_FUNC_VAL_IS(_SHR_E_UNAVAIL) && (sh_sand_cmd != NULL) && (!ISEMPTY(sh_sand_cmd->man->compatibility)))
        {
             ...
             * UNAVAIL error
             */
            SHR_CLI_EXIT(_SHR_E_UNAVAIL, "NO_SUPPORT Command:\"%s\" - %s\n", sh_sand_cmd->cmd_only,
                         sh_sand_cmd->man->compatibility);
        }
        else if (SHR_FUNC_VAL_IS(_SHR_E_NOT_FOUND)
                 && (sh_sand_cmd_get(unit, cmd_name, sh_sand_cmd_a, &sh_sand_cmd, 1, SH_CMD_TRAVERSE_NO_CB) !=
                     _SHR_E_NOT_FOUND))
        {
            /*
             * Command exists in tree but cannot be found without skipping callbacks, print it and and set UNAVAIL error.
             */
            SHR_CLI_EXIT(_SHR_E_UNAVAIL,
                         "NO_SUPPORT Command:\"%s\" is not supported for current device and/or configuration\n",
                         cmd_name);
        }
    ...
    else if (sh_sand_cmd->action)
    {
...
#endif
        {
            for (current_core = 0; current_core < nof_cores; current_core++)
            {
                ...
                else if ((args->a_cmd != NULL)
                         && !sal_strncasecmp(args->a_cmd, "ctest", sal_strnlen("ctest", SH_SAND_MAX_TOKEN_SIZE)))
                {
                    /*
                     * If command is to run a CTest, include error message on failure.
                     */
                    SHR_CLI_EXIT_IF_ERR(sh_sand_cmd->action(unit, args, sand_control), "CTEST_FAILED\n");
                }
                else
                {
                    SHR_CLI_EXIT_IF_ERR(sh_sand_cmd->action(unit, args, sand_control), "");
    ...
    int flags)
{
    ...
    {   /* Verify that every command is either leaf or branch */
        ...
        /*
         * Add SH_CMD_SKIP_EXEC flag if it is set for its parent.
         */
        if (sh_sand_cmd->parent_cmd != NULL
            && utilex_is_prime_flag_set(sh_sand_cmd->parent_cmd->flags, SH_CMD_SKIP_EXEC))
        {
            utilex_prime_flag_set(&sh_sand_cmd->flags, SH_CMD_SKIP_EXEC);
        }
    ...
    char *command)
{
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(exec_args, sizeof(*exec_args), "leafs", "%s%s%s\n", EMPTY, EMPTY, EMPTY);
...
#if defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT)
void
sh_sand_traverse_add_init_required_flag(
    int unit,
    sh_sand_cmd_t * sh_sand_cmd)
{
    sh_sand_cmd_t *sh_sand_cmd_curr;
     ...
     */
    for (sh_sand_cmd_curr = sh_sand_cmd; sh_sand_cmd_curr != NULL && sh_sand_cmd_curr->keyword != NULL;
         sh_sand_cmd_curr++)
    {
        /*
        ...
        {
            sh_sand_traverse_add_init_required_flag(unit, sh_sand_cmd_curr->child_cmd_a);
        }
...
#if defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT)
    /** lock the command calibration to prevent calibrations occurring in parallel */
    if (dnx_calibrate_lock != NULL)
    {
        /** take lock */
        if (0 != sal_mutex_take(dnx_calibrate_lock, 2000000))
        {
            SHR_ERR_EXIT(BCM_E_TIMEOUT, "unable to take dnx_calibrate_lock mutex");
        }
    }

    /*
     * If BCM init sequence was not completed successfully, recursively add the SH_CMD_INIT_REQUIRED flag to all of the system commands and the
     * DNX/F commands and their respective descendants. The is necessary to enabling the activation of these commands and their inclusion in autocomplete.
     */
    if (!dnxc_init_is_init_done_get(unit))
    {
        sh_sand_traverse_add_init_required_flag(unit, sh_sand_sys_cmds);
        sh_sand_traverse_add_init_required_flag(unit, sh_root_commands);
    }
#endif
...
exit:
    ...
    if (dnx_calibrate_lock != NULL)
    {
        if (0 != sal_mutex_give(dnx_calibrate_lock))
        {
            LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "unable to give dnx_calibrate_lock mutex \n")));
        }
    }
#endif
