...
static sh_sand_man_t packet_tx_man = {
    ...
    .examples = "PTCH_2 ETH1 IPv4 show resume=NO\n"
        "packet=simple rx\n"
        "packet=default sat port=13\n"
        "packet=default port=218 size=256 rate=10000 burst=10000 dur=1 unit=kbits\n"
        "pkt=sat\n"
        "packet=default sat port=13 duration=0 grnl=1000\n"
        ...
        "raw DATA=0x80c90000000000010000000000028100000108004500002e0000000040ff79d20000000000000000000102030405060708090a0b0c0d0e0f1011 resume=Yes SignalDumpValid=Yes SignalDumpFile=./dump.xml",
...
static sh_sand_option_t packet_tx_arguments[] = {
   /* Name | Type               | Description                            | Default      */
    ...
    {"SignalDumpValid", SAL_FIELD_TYPE_BOOL, "Indicate to dump signal", "No"},
    {"SignalDumpFile",    SAL_FIELD_TYPE_STR,    "File to store the result of signal dump", ""},
    {NULL}      /* End of options list - must be last. */
};
/* *INDENT-ON* */
    ...
    sh_sand_control_t * sand_control)
{
    ...
    RHITERATOR(obj, pit)
    {
        ...
        /** obj->print_value is a hex data string without 0x */
        sal_snprintf(data_str, RHNAME_MAX_SIZE - 1, "0x%s", obj->print_value);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char *packet_n = NULL;
    int sig_dump_valid = FALSE;
    char *sig_dump_file = NULL;
...
#if !defined(NO_FILEIO)
    SH_SAND_GET_BOOL("SignalDumpValid", sig_dump_valid);
    SH_SAND_GET_STR("SignalDumpFile", sig_dump_file);
    if (sig_dump_valid)
    {
        if (ISEMPTY(sig_dump_file))
        {
            SHR_CLI_EXIT(_SHR_E_PARAM, "File to store the result of signal dump is not specified\n");
        }
    }
    else
    {
        if (!ISEMPTY(sig_dump_file))
        {
            SHR_CLI_EXIT(_SHR_E_PARAM, "Option SignalDumpValid is required\n");
        }
    }
#endif
...
exit:
#if !defined(NO_FILEIO)
    if (sig_dump_valid && !ISEMPTY(sig_dump_file))
    {
        args_t *sig_args = NULL;
        char file_argv[256] = "file=";
        int max_file_name_len = 256;

        sig_args = sal_alloc(sizeof(args_t), "sig_args");
        if (sig_args)
        {
            /*
             * Set argvs
             */
            sal_memset(sig_args, 0, sizeof(args_t));
            sig_args->a_cmd = "SIGnal";
            sig_args->a_argv[0] = "sig";
            sig_args->a_argv[1] = "get";
            sig_args->a_argv[2] = "all";
            sal_strncat_s(file_argv, sig_dump_file, max_file_name_len - 1);
            sig_args->a_argv[3] = file_argv;
            sal_strncpy(sig_args->a_buffer, "sig\0get\0all\0file=", ARGS_BUFFER - 1);
            sal_strncat_s(sig_args->a_buffer, sig_dump_file, ARGS_BUFFER - 1);
            sig_args->a_argc = 4;
            sig_args->a_arg = 1;

            cmd_sand_signal(unit, sig_args);
            sal_free(sig_args);
        }
    }
#endif
    ...
    args_t * args)
{
    ...
    sh_sand_act(unit, args, sh_sand_packet_cmds, sh_sand_sys_cmds, NULL, SH_CMD_FLAGS_NONE);
    ARG_DISCARD(args);
    /*
