    ...
    else
    {
        ...
        if (all_regs || parse_symbolic_reference(unit, &alist, str) < 0)
        {
            ...
            for (reg = 0; reg < NUM_SOC_REG; reg++)
            {
                ...
                if (sal_strncaseindex(SOC_REG_NAME(unit, reg), str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
                {
                    if (!found && !all_regs)
                    {
    ...
    int wrap)
{
    ...
    int nprint;
    int rlen;
    soc_reg_above_64_val_t resval={0}, fieldval, resfld;
...
#endif
#if defined(BCM_PETRA_SUPPORT) || defined(BCM_ESW_SUPPORT)
    ...
    char *name)
{
    ...
    for (reg = 0; reg < NUM_SOC_REG; reg++)
    {
        ...
        if (diag_sand_filter_regs(unit, reg) || diag_sand_filter_regs_2(unit, reg) || (name != NULL && (*regblktype != block_type)))
        {
            continue;
        }
        if (soc_genreg == SOC_REG_INFO(unit, reg).regtype || soc_cpureg == SOC_REG_INFO(unit, reg).regtype || (soc_iprocreg == SOC_REG_INFO(unit, reg).regtype && !SOC_IS_RAMON(unit)))
        {
            if (is_block_instance)
            {
        ...
        if (soc_portreg == SOC_REG_INFO(unit, reg).regtype)
        {
            PBMP_ITER(PBMP_PORT_ALL(unit), port)
            {
                ...
                {
                    int is_master;
                    rc = dnx_algo_port_is_master_get(unit, port, 0, &is_master);
                    if (rc < 0)
                    {
                        goto exit;
                    }
...
char cmd_sand_i2c_smbus_usage[] =
#ifdef COMPILER_STRING_CONST_LIMIT
    "Full documentation cannot be displayed with -pendantic compiler\n";
#else
    "\n\tPerform I2C smbus read/write from the CPU\n\t"
    "Usages:\n\t"
    "i2c_smbus write [ACCESS_TYPE] <bus_id> <device_address> <internal_address> {<value>}\n\t"
    "i2c_smbus read [ACCESS_TYPE] <bus_id> <device_address> {<internal_address>}\n\t"
    "\n\t"
    "internal_address must range of 0 - 0xff\n\t"
    "ACCESS_TYPE can be:\n\t"
    "\t0x1  - not specify address, data length is byte\n\t"
    "\t0x10 - address length is byte, no data specified, only for write\n\t"
    "\t0x11 - address length is byte, data length is byte\n\t"
    "\t0x12 - address length is byte, data length is word(2 bytes)\n\t"
    "\tOther option supported.\n\t"
    "\n\t" "Examples:\n\t" "i2c_smbus write 0x12 1 0x41 0x4 0xaa\n\t" "i2c_smbus read 0x11 0 0x4b 0x58\n\t"
    "i2c_smbus read 0x01 1 0x73\n\t" "i2c_smbus write 0x01 1 0x73 2\n\t" "i2c_smbus write 0x10 1 0x72 0xff\n\t";
#endif /* COMPILER_STRING_CONST_LIMIT */
...
{
#ifndef __KERNEL__
    char *function, *bus_addr, *chip_addr, *data_addr, *alen, *value;
    int int_bus_addr, int_chip_addr, int_data_addr;
    uint32 int_value = 0;
    uint8 alen_dlen;
    int rv;
    int strnlen_function;
    ...
    if (!sal_strncasecmp(function, "write", strnlen_function))
    {
        ...
        int_bus_addr = sal_ctoi(bus_addr, 0);
        /** chip address */
        chip_addr = ARG_GET(a);
        if (!chip_addr)
        {
        ...
        int_data_addr = sal_ctoi(data_addr, 0);
        if (int_data_addr < 0 || int_data_addr > 0xff)
        {
            return CMD_USAGE;
        }
        /** value */
        value = ARG_GET(a);
        if (alen_dlen != 0x10) /* 0x10 only specify data address, data can be ignored */
        {
            /** I2C_SMBUS_ALEN_ZERO_DLEN_BYTE: transfer address, no data */
            if (!value)
            {
                return CMD_USAGE;
            }
        ...
        rv = sal_i2c_smbus_write(int_bus_addr, (uint16)int_chip_addr, (uint8)int_data_addr, alen_dlen, int_value);
        if (BCM_FAILURE(rv))
        {
            bsl_log_add(BSL_APPL_SHELL, bslSeverityError, bslSinkIgnore, unit, "I2C access fails");
            return CMD_FAIL;
    ...
    else if (!sal_strncasecmp(function, "read", strnlen_function))
    {
        ...
        /** internal address */
        data_addr = ARG_GET(a);
        if (alen_dlen == 0x01) /* not specify address for read */
        {
            /** no need internal address */
            int_data_addr = 0;
        }
        else
        {
        ...
        rv = sal_i2c_smbus_read(int_bus_addr, (uint16)int_chip_addr, (uint8)int_data_addr, alen_dlen, &int_value);
        if (BCM_FAILURE(rv))
        {
            bsl_log_add(BSL_APPL_SHELL, bslSeverityError, bslSinkIgnore, unit, "I2C access fails");
            return CMD_FAIL;
