...
/*
 * Used in access_modify_cmd
 */
#define DIAG_SAND_ACCESS_MODIFY_CMD_SIZE 1024
/*
 * Used in access_write_cmd
 */
#define DIAG_SAND_ACCESS_WRITE_CMD_SIZE 4095
    ...
    uint32 *addr_p)
{
    ...
    sal_memset(value, 0, sizeof(uint32) * SOC_REG_ABOVE_64_MAX_SIZE_U32);
...
shr_error_e
access_read_autocomplete_cb(
    int unit,
    char *name_substring,
    char *name_full_string,
    int *num_matches_found)
{
    int reg_mem_index, ii;
    int num_matches_shown = 0;
    char *reg_mem_names[2];

    SHR_FUNC_INIT_VARS(unit);

    for (reg_mem_index = 0; reg_mem_index < NUM_SOC_REG && reg_mem_index < NUM_SOC_MEM; reg_mem_index++)
    {
        reg_mem_names[0] = reg_mem_names[1] = "";
        if (SOC_REG_IS_VALID(unit, reg_mem_index))
        {
            reg_mem_names[0] = SOC_REG_NAME(unit, reg_mem_index);
        }
        if (SOC_MEM_IS_VALID(unit, reg_mem_index))
        {
            reg_mem_names[1] = SOC_MEM_NAME(unit, reg_mem_index);
        }
        for (ii = 0; ii < 2; ii++)
        {
            if (sal_strncaseindex
                (reg_mem_names[ii], name_substring, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                 SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            {
                (*num_matches_found)++;
                if (sal_strlen(name_full_string) + sal_strlen(reg_mem_names[ii]) >= AUTOCOMPLETE_MAX_STRING_LEN)
                {
                    continue;
                }
                num_matches_shown++;
                sal_strncat_s(name_full_string, reg_mem_names[ii], AUTOCOMPLETE_MAX_STRING_LEN);
                sal_strncat_s(name_full_string, " ", AUTOCOMPLETE_MAX_STRING_LEN);
            }
        }
    }

    if (num_matches_shown < (*num_matches_found))
    {
        cli_out("\nToo many names match '%s'. Showing only %d matches out of %d:\n", name_substring, num_matches_shown,
                *num_matches_found);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    sal_memset(reg_num, 0, sizeof(int) * soc_invalidreg);
...
static sh_sand_man_t access_list_man = {
    ...
    .examples = "prp=reg.*iproc field=no\n" "name=RIP block=SCH\n" "prp=mem address=0x00800000 desc\n" "all",
};
...
static sh_sand_option_t access_read_options[] = {
    {"name", SAL_FIELD_TYPE_STR, "Regular expression used to filter regs/mems and their fields", "", NULL, NULL,
     SH_SAND_ARG_FREE,.ac_option_cb = access_read_autocomplete_cb},
    {"block", SAL_FIELD_TYPE_STR, "Block name or any substring of it to filter regs/mems", ""},
    ...
    for (reg = 0; reg < NUM_SOC_REG; reg++)
    {
        ...
        if (separate_flag == TRUE && print_flag == TRUE)
        {
            ...
            if (sal_strcmp(folder_name, ""))
            {
                ...
                sal_strncat_s(help_filename, folder_name, 512);
                sal_strncat_s(help_filename, SOC_REG_NAME(unit, reg), 512);
                sal_strncat_s(help_filename, ".xml", 512);
            }
        }

        regtype = reginfo->regtype;
        if (soc_is_reg_flexe_core(unit, reg) || soc_is_reg_framer(unit, reg))
        {
            regtype = soc_genreg;
        }
        switch (regtype)
        ...
        SOC_MEM_BLOCK_ITER(unit, mem, i_bl)
        {
            ...
            if (separate_flag == TRUE && print_flag == TRUE)
            {
                ...
                if (sal_strcmp(folder_name, ""))
                {
                    sal_memset(help_filename, 0, sizeof(char) * 512);
                    sal_strncat_s(help_filename, folder_name, 512);
                    sal_strncat_s(help_filename, SOC_MEM_NAME(unit, mem), 512);
                    sal_strncat_s(help_filename, ".xml", 512);
                }
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char *h_value;
    char command[DIAG_SAND_ACCESS_WRITE_CMD_SIZE];
    uint8 reg, mem;
    int value_index;
    ...
    sal_memset(command, 0, DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
    ...
    if (reg == TRUE)
    {
        ...
        sal_strncat_s(command, "s ", DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        sal_strncat_s(command, name, DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        if (ISEMPTY(offset))
        {
            sal_strncat_s(command, offset, DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
            sal_strncat_s(command, " ", DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        }
        if (ISEMPTY(symbol))
        {
            sal_strncat_s(command, symbol, DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
            sal_strncat_s(command, " ", DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        }
        ...
        if (!ISEMPTY(value))
        {
            ...
            while (h_value != NULL)
            {
                ...
                sal_strncat_s(command, h_value, DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
    ...
    if (mem == TRUE)
    {
        ...
        sal_strncat_s(command, "w ", DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        sal_strncat_s(command, name, DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        if (!ISEMPTY(block))
        {
            sal_strncat_s(command, ".", DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
            sal_strncat_s(command, block, DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        }
        else if (!ISEMPTY(index))
        {
            sal_strncat_s(command, "[", DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
            sal_strncat_s(command, index, DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
            sal_strncat_s(command, "]", DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        }
        sal_strncat_s(command, " ", DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        sal_strncat_s(command, entry, DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        sal_strncat_s(command, " ", DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        sal_strncat_s(command, entrycount, DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
        sal_strncat_s(command, " ", DIAG_SAND_ACCESS_WRITE_CMD_SIZE);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char *h_value;
    char command[DIAG_SAND_ACCESS_MODIFY_CMD_SIZE];
    uint8 reg, mem;
    int value_index;
    ...
    if (reg == TRUE)
    {
        sal_memset(command, 0, DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        if (ISEMPTY(name))
        {
            SHR_CLI_EXIT(_SHR_E_PARAM, "Set parameter \"name\" of the memory.\n");
        }

        sal_strncat_s(command, "m ", DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        sal_strncat_s(command, name, DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        sal_strncat_s(command, " ", DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        ...
        if (!ISEMPTY(value))
        {
            ...
            while (h_value != NULL)
            {
                ...
                sal_strncat_s(command, h_value, DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
    ...
    if (mem == TRUE)
    {
        ...
        sal_strncat_s(command, "mod ", DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        sal_strncat_s(command, name, DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);

        if (!ISEMPTY(index))
        {
            sal_strncat_s(command, "[", DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
            sal_strncat_s(command, index, DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
            sal_strncat_s(command, "]", DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        }
        if (!ISEMPTY(block))
        {
            sal_strncat_s(command, ".", DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
            sal_strncat_s(command, block, DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        }
        sal_strncat_s(command, " ", DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        sal_strncat_s(command, entry, DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        sal_strncat_s(command, " ", DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        sal_strncat_s(command, entrycount, DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
        sal_strncat_s(command, " ", DIAG_SAND_ACCESS_MODIFY_CMD_SIZE);
    ...
    while (hit_data_length >= ADAPTER_HIT_INFO_SIZE)
    {
        ...
        {       /* Opcode was for memory */
            ...
            for (mem = 0; mem < NUM_SOC_MEM; mem++)
            {
                ...
                {
                    sal_memset(mem_value, 0, SOC_MAX_MEM_WORDS * 4);
                    soc_error = 0;
                }
                if (soc_error < 0)
                {
    ...
    args_t * args)
{
    ...
    sh_sand_act(unit, args, sh_sand_access_cmds, sh_sand_sys_cmds, NULL, SH_CMD_FLAGS_NONE);
    ARG_DISCARD(args);
    /*
