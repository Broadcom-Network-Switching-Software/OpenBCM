...
STATIC cmd_result_t
bcmi_diag_ft_state_transition_show(int unit, args_t *a)
{
    bcmi_ft_state_transition_show(unit);
...
cmd_ft(int unit, args_t *a)
{
    ...
    if (soc_feature(unit, soc_feature_flex_flowtracker_ver_1)) {
        ...
        if (!ft_initialized[unit]) {
            cli_out("%s: Module not initialized\n", ARG_CMD(a));
            return CMD_FAIL;
        }
        ...
        if (group_dump_enable) {
            ...
            if ((sal_strcasecmp(subcmd_s, "state_trans") == 0) ||
                (sal_strcasecmp(subcmd_s, "st") == 0)) {
                return bcmi_diag_ft_state_transition_show(unit, a);
            }
        }
