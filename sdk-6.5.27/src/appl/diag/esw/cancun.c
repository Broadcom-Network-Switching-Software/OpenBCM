...
uint8 * _cancun_file_buffer_load(char *filename, long *file_len);
        ...
        if (dir) {
            ...
            while ((d = sal_readdir(dir)) != NULL) {
                ...
                if (dir2) {
                    ...
                    file_buf = _cancun_file_buffer_load(filename, &file_len);
                    if (file_buf) {
                        rv = soc_cancun_file_info_get(unit, &ccf, filename, file_buf, file_len);
                        if (rv == SOC_E_NONE) {
...
int _cmd_cancun_file_load(int unit, char* filename, uint32 file_type,
    ...
    if (file_buf) {
        ...
        if(soc_cancun_file_load(unit, file_buf, file_len, &file_type,
                                &file_format) == SOC_E_NONE) {
            rv = CMD_OK;
        }
        sal_free(file_buf);
    }
 ...
 * Parameters:
 *             filename    - (IN) Pointer to filename string
 *             file_len     - (IN) Required buffer length in byte
 * Returns:
 *             uint8 pointer of allocated and loaded buffer
 */
uint8 * _cancun_file_buffer_load(char *filename, long *file_len) {
    ...
    *file_len = ftell(fp);
    ...
    file_buf = (uint8*)sal_alloc(*file_len, "soc_cancun_file_buf");
    if(file_buf) {
