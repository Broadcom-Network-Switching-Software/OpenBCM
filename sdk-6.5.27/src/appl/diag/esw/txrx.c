...
#define ENCAP_HIGIG3    4 /* Note: no BCM_PORT_ENCAP_ equivalent */
#define ENCAP_RAW       5 /* Note: no BCM_PORT_ENCAP_ equivalent */
#define ENCAP_COUNT     6 /* Number of CLI encapsulation modes */
...
typedef struct xd_s {                   /* TX/RX description */
    ...
    uint32      xd_hg3_cid;             /* Higig3 Class ID */
    uint32      xd_hg3_l3;              /* Higig3 L3 Routed */
    uint32      xd_hg3_mirror;          /* Higig3 Mirror */
    uint32      xd_hg3_mp;              /* Higig3 MultiPoint */
    uint32      xd_hg3_dvp;             /* Higig3 Dest VP */
    uint32      xd_hg3_svp;             /* Higig3 Src VP */
    uint32      xd_hg3_don_learn;       /* Higig3 Donnot Learn */
    uint32      xd_hg3_lag_fo;          /* Higig3 Lag Failover */
 ...
 */
{
    ...
    if (xd->hdr_mode == ENCAP_IEEE) {
...
#ifdef BCM_HIGIG2_SUPPORT
#if defined(BCM_TRIDENT3_SUPPORT)
        if (xd->hdr_mode == ENCAP_HIGIG3 && soc_feature(u, soc_feature_higig3)) {
            /* Map traffic class to internal priority */
            parse_table_add(&pt, "TrafficClass",     PQ_INT,     0,
                            &xd->xd_prio_int, NULL);
            parse_table_add(&pt, "McastGroupID",     PQ_INT,     0,
                            &xd->xd_hg_mgid, NULL);
            parse_table_add(&pt, "LoadBalID",        PQ_INT,     0,
                            &xd->xd_hg_lbid, NULL);
            parse_table_add(&pt, "DropPrecedence",   PQ_MULTI,   0,
                            &xd->xd_hg_dp, drop_precedence);
            parse_table_add(&pt, "ClassID",          PQ_INT,     0,
                            &xd->xd_hg3_cid, NULL);
            parse_table_add(&pt, "L3Routed",         PQ_INT,     0,
                            &xd->xd_hg3_l3, NULL);
            parse_table_add(&pt, "Mirror",           PQ_INT,     0,
                            &xd->xd_hg3_mirror, NULL);
            parse_table_add(&pt, "MultiPoint",       PQ_INT,     0,
                            &xd->xd_hg3_mp, NULL);
            parse_table_add(&pt, "DestVP",           PQ_INT,     0,
                            &xd->xd_hg3_dvp, NULL);
            parse_table_add(&pt, "SrcVP",            PQ_INT,     0,
                            &xd->xd_hg3_svp, NULL);
            parse_table_add(&pt, "DonotLearn",       PQ_INT,     0,
                            &xd->xd_hg3_don_learn, NULL);
            parse_table_add(&pt, "LagFailover",      PQ_INT,     0,
                            &xd->xd_hg3_lag_fo, NULL);
        ...
        if (soc_feature(u, soc_feature_higig2) ||
            soc_feature(u, soc_feature_higig3) ||
            SOC_IS_TOMAHAWK3(u)) {
...
#if defined(BCM_TRIDENT3_SUPPORT)
STATIC int
_tx_hg3hdr_setup(bcm_pkt_t *pkt, xd_t *xd)
{
    ...
    ether_type = bcm_td3_tx_hg3_ethertype_get(unit);

    sal_memset(xgh, 0, sizeof(pkt->_higig));
    soc_higig3_field_set(unit, xgh, HG_ether_type, ether_type);
    soc_higig3_field_set(unit, xgh, HG_ehv, 2);
    soc_higig3_field_set(unit, xgh, HG_opcode, pkt->opcode);

    vlan_val = BCM_PKT_VLAN_CONTROL(pkt);
    soc_higig3_field_set(unit, xgh, HG_vlan_tag, vlan_val);
    soc_higig3_field_set(unit, xgh, HG_dst_mod, pkt->dest_mod);
    soc_higig3_field_set(unit, xgh, HG_dst_port, pkt->dest_port);
    soc_higig3_field_set(unit, xgh, HG_src_mod, pkt->src_mod);
    soc_higig3_field_set(unit, xgh, HG_src_port, pkt->src_port);
    soc_higig3_field_set(unit, xgh, HG_cos, pkt->prio_int);
    ...
    if (multicast) {
        ...
        mc_index_offset = 0;
        mc_index = _BCM_MULTICAST_ID_GET(xd->xd_hg_mgid);
        soc_higig3_field_set(unit, xgh, HG_mgid,
                             mc_index + mc_index_offset);

    }

    soc_higig3_field_set(unit, xgh, HG_lbid,        xd->xd_hg_lbid);
    soc_higig3_field_set(unit, xgh, HG_dp,          xd->xd_hg_dp);
    soc_higig3_field_set(unit, xgh, HG_class_id,    xd->xd_hg3_cid);
    soc_higig3_field_set(unit, xgh, HG_donot_learn, xd->xd_hg3_don_learn);
    soc_higig3_field_set(unit, xgh, HG_dst_vp,      xd->xd_hg3_dvp);
    soc_higig3_field_set(unit, xgh, HG_src_vp,      xd->xd_hg3_svp);
    soc_higig3_field_set(unit, xgh, HG_l3,          xd->xd_hg3_l3);
    soc_higig3_field_set(unit, xgh, HG_lag_failover,xd->xd_hg3_lag_fo);
    soc_higig3_field_set(unit, xgh, HG_mirror,      xd->xd_hg3_mirror);
    soc_higig3_field_set(unit, xgh, HG_multipoint,  xd->xd_hg3_mp);
...
add_higig_hdr(xd_t *xd, bcm_pkt_t *pkt_info)
{
    ...
    case ENCAP_HIGIG2:      /* HiGig2 header */
        ...
        break;
    case ENCAP_HIGIG3:      /* HiGig3 header */
#if defined(BCM_TRIDENT3_SUPPORT)
        if (soc_feature(xd->xd_unit, soc_feature_higig3)) {
            _tx_hg3hdr_setup(pkt_info, xd);
        } else
#endif
        {
            cli_out("Warning:  Higig3 not supported by device\n");
        }
        break;
#endif /* BCM_HIGIG2_SUPPORT */
...
char cmd_hdr_mode_usage[] =
    ...
    "      Higig3 --  Use parsed HiGig3 header information\n"
#endif /* BCM_HIGIG2_SUPPORT */
