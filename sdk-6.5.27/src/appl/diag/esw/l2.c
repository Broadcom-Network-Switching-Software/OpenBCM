...
if_esw_l2(int unit, args_t *a)
{
    ...
    int		arg_static = 0, arg_trunk = 0, arg_l3if = 0,
        ...
        arg_group = 0, arg_pending = 0, arg_ignore_pending = 0, arg_limitexempt = 0, arg_mirror = 0;
    int arg_mesh = 0;
    ...
    if (!sal_strcasecmp(subcmd, "replace")) {
        ...
        parse_table_add(&pt, "PENDing",	        PQ_DFL|PQ_BOOL,
                0, &arg_pending, NULL);
        parse_table_add(&pt, "IgnorePENDing",	PQ_DFL|PQ_BOOL,
                0, &arg_ignore_pending, NULL);
