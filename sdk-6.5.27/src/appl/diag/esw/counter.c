...
cmd_esw_counter(int unit, args_t *a)
{
    ...
    if (soc_feature(unit, soc_feature_subtag_counters)) {
        SOC_PBMP_OR(temp_pbmp, SOC_INFO(unit).subtag_pp_port_pbm);
    }
...
trig_name_t _trig_names[bcmDbgCntNum] = {
...
termination errors"},
    {"RxEcmpProtectionMemberCollisionDrop", "Enable to count packets dropped \
due to ECMP member table bank collision is detected while resolving primary \
and secondary next hops"},
    {"RxIntMtuError", "Enable to count In-band Network Telemetry (INT) packets \
experiencing L3 MTU check failure due to metadata insertion"},
    {"RxEcmpMemberNotResolvedDrop", "Enable to count packets dropped due to \
primary, secondary and tertiary next hop selections are down or \
programmatically disabled and a valid next hop could not be resolved"},
    {"RxNonFirstPass", "Enable to count packets marked as non-first pass \
packets"},
    {"RxEcmpProtectionMemberActive", "Enable to count packets due to secondary \
or tertiary next hop is picked during next hop resolution because \
primary/secondary next hop is either down or programmatically disabled"},
    {"RxStatefulModSecondPassDrop", "Enable to count switch copies dropped due \
to 2nd pass processing of stateful MOD"},
    {"RxHigig3Drop", "Enable to count packets dropped due to the drop \
configuration on HIGIG3 packets"},
    {"RxNonHigig3Drop", "Enable to count packets dropped due to the drop \
configuration on non-HIGIG3 packets"},
    {"RxL2TunnelSvpAssignFailDrop", "Enable to indicate packets dropped due to \
source virtual port assignment failure. Applicable for VxLAN and SRv6"},
    {"RxSrv6ProcessErrorDrop", "Enable to count packets dropped due to SRV6 \
process error"},
    {"RxSrv6ValidationErrorDrop", "Enable to count packets dropped due to SRV6 \
validation error"},
    {"RxSrv6UnknownRoutingTypeWithNonZeroSlDrop", "Enable to count packets \
dropped due to unknown routing type in SRV6 packets with nonzero segments left \
field"},
    {"TxDcnPktRoutingPlaneMapDrop", "Enable to count packets IFA action not \
applied"},
    {"TxIfaActionNotApplied", "packets dropped due to VXLAN tunnel \
termination errors"},
    {"TxNonFirstPass", "Enable to count packets marked as non-first pass \
packets"},
    {"TxEpRecircDrop", "Enable to count packets dropped due to EP \
recirculation drop"},
    {"TxIfaTermMirrorCopySuppress", "Enable to count IFA terminated packets \
experiencing mirror copy suppressing"},
    {"TxEpRecircSupress", "Enable to count packets experiencing EP \
recirculation suppressing"}
};
