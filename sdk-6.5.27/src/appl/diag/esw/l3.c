...
_l3_cmd_info_show(int unit)
{
    ...
    if (single_level_ecmp) {
        rv = bcm_switch_object_count_get(unit,
                      bcmSwitchObjectEcmpMemberDefragStat, &i);
        if (BCM_FAILURE(rv)) {
            cli_out("Error L3 info: %s\n", bcm_errmsg(rv));
            return CMD_FAIL;
        }
        ...
        rv = bcm_switch_object_count_get(unit,
                      bcmSwitchObjectOverlayEcmpMemberDefragStat, &i);
        if (BCM_FAILURE(rv)) {
            cli_out("Error L3 info: %s\n", bcm_errmsg(rv));
            return CMD_FAIL;
        }
        cli_out("%-20s : %6d\n", "overlay_ecmp_defrag", i);

        rv = bcm_switch_object_count_get(unit,
                      bcmSwitchObjectUnderlayEcmpMemberDefragStat, &i);
        if (BCM_FAILURE(rv)) {
            cli_out("Error L3 info: %s\n", bcm_errmsg(rv));
            return CMD_FAIL;
        }
        cli_out("%-20s : %6d\n", "underlay_ecmp_defrag", i);
    }
...
_l3_cmd_egress_intf_show(int unit, args_t *a) 
{
    ...
    for (interface = min_idx; interface < max_idx; interface++) {
        /* Skip interface id reserved for Copy To Cpu purposes. */
        if (interface == BCM_XGS3_L3_L2CPU_INTF_IDX(unit)) {
            continue;
        }
        sal_memset(&intf, 0, sizeof (bcm_l3_intf_t));
        intf.l3a_intf_id = interface;
...
_l3_cmd_route_perf_test(int unit, args_t *a)
{
    ...
    */
    ...
    parse_table_add(&pt, "OPs", PQ_DFL | PQ_INT, (void *)0xf,
                    (void *)&op_bitmap, 0);
    parse_table_add(&pt, "GblPri", PQ_DFL | PQ_INT, (void *)3,
                    (void *)&global_type, 0);
...
cmd_esw_alpm(int unit, args_t *a)
{
    ...
    if (sal_strcasecmp(table, "defrag") == 0) {
        ...
        int max_time = -1, maxl2cnt = -1, maxl3cnt = -1, l2cnt = 0, l3cnt = 0, l2shk = 0, l3shk = 0;
        ...
        if (SOC_IS_TOMAHAWK3(unit)) {
            ...
            rv = bcm_esw_alpm_defrag(unit, vrf, ipt, max_time, maxl2cnt, maxl3cnt,
                                     &l2cnt, &l3cnt, &l2shk, &l3shk);

            cli_out("  L2 bucket merge cnt: %d (shrink %d)\n", l2cnt, l2shk);
            cli_out("  L3 bucket merge cnt: %d (shrink %d)\n", l3cnt, l3shk);
...
if_l3(int unit, args_t *a)
{
...
#endif
    void *user_data = NULL;
    uint8 l3_switch_object_counts_valid = 0;

    COMPILER_REFERENCE(l3_switch_object_counts_valid);
    sal_memset(mac, 0, sizeof(bcm_mac_t));
    ...
    case L3_SHOW:
...
#if defined(BCM_TRIDENT3_SUPPORT) || defined(BCM_TRIDENT2_SUPPORT)
        if (soc_property_get(unit, spn_L3_ALPM_ENABLE, 0) &&
              soc_feature(unit, soc_feature_alpm)) {
            l3_switch_object_counts_valid = 1;
        }
#endif        
        if ((SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT2X(unit) || SOC_IS_TRIUMPH3(unit) ||
            ...
            ((!(soc_property_get(unit, spn_L3_ALPM_ENABLE, 0) &&
              soc_feature(unit, soc_feature_alpm))) || l3_switch_object_counts_valid) &&
            !ext_tcam_flag) {
            if (SOC_ALPM_128B_ENABLE(unit)) {
