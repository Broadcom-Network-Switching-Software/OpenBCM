...
fp_action_add(int unit, args_t *args)
{
    ...
    switch (action) {
        ...
        case bcmFieldActionSRNetIdSource:
            FP_GET_NUMB(p0, subcmd, args);
            break;
...
fp_range_get(int unit, args_t *args)
{
    ...
    if(SOC_IS_TOMAHAWK3(unit)) {
        retval = bcm_range_get(unit, &range_config);
    } else
#endif
    {
...
#ifdef BCM_TOMAHAWK3_SUPPORT
    if (SOC_IS_TOMAHAWK3(unit)) {
        cli_out("FP range get: rid=%d, min=%d max=%d ", range_config.rid, range_config.min, range_config.max);
        cli_out("flags=0x%x%s%s%s%s\n",
                range_config.rtype,
                range_config.rtype == bcmRangeTypeL4SrcPort ? " SRCPORT" : "",
                range_config.rtype == bcmRangeTypeL4DstPort ? " DSTPORT" : "",
                range_config.rtype == bcmRangeTypeOuterVlan ? " OUTERVLAN" : "",
                range_config.rtype == bcmRangeTypePacketLength ? " PACKET LEN" : "");
    } else
#endif
    {
...
fp_list_actions(int unit, args_t *args)
{
    ...
    fp_action_table[bcmFieldActionSRNetIdSource].action_0 = "BCM_FIELD_SR_NET_ID_SOURCE_xxx";
...
fp_lookup_control(const char *control_str, bcm_field_control_t *control)
{
    ...
    char                   lng_str[FP_STAT_DST_STR_SZ];
    ...
    for (*control = 0; *control < bcmFieldControlCount; (*control)++) {
        ...
        sal_memset(lng_str, 0, sizeof(char) * FP_STAT_DST_STR_SZ);
        /* Test for the suffix only */
        sal_strlcpy(tbl_str, _fp_control_name(*control), FP_STAT_STR_SZ);
        if (!sal_strcasecmp(tbl_str, control_str)) {
            break;
        }
        /* Test for whole name of the control */
        sal_strlcpy(lng_str, "bcmFieldControl", FP_STAT_DST_STR_SZ);
        sal_strncat(lng_str, tbl_str,
                FP_STAT_DST_STR_SZ - 1 - sal_strlen(lng_str));
...
fp_lookup_L2Format(const char *type_str)
{
    ...
    for (type = 0; type < bcmFieldL2FormatCount; (type)++) {
        /* Test for the suffix only */
        sal_memset(tbl_str, 0, sizeof(char) * FP_STAT_STR_SZ);
        sal_strlcpy(tbl_str, _fp_qual_L2Format_name(type), FP_STAT_STR_SZ);
        if (!sal_strcasecmp(tbl_str, type_str)) {
            break;
        }
        /* Test for whole name of the L2Format*/
        sal_memset(lng_str, 0, sizeof(char) * FP_STAT_DST_STR_SZ);
        sal_strlcpy(lng_str, prefix, FP_STAT_DST_STR_SZ);
        sal_strncat(lng_str, tbl_str, FP_STAT_DST_STR_SZ - 1 - sal_strlen(lng_str));
...
fp_lookup_IpType(const char *type_str)
{
    ...
    for (type = 0; type < bcmFieldIpTypeCount; (type)++) {
        /* Test for the suffix only */
        sal_memset(tbl_str, 0, sizeof(char) * FP_STAT_STR_SZ);
        sal_strlcpy(tbl_str, _fp_qual_IpType_name(type), FP_STAT_STR_SZ);
        if (!sal_strcasecmp(tbl_str, type_str)) {
            break;
        }
...
fp_lookup_IpProtocolCommon(const char *type_str)
{
    ...
    for (type = 0; type < bcmFieldIpProtocolCommonCount; (type)++) {
        /* Test for the suffix only */
        sal_memset(tbl_str, 0, sizeof(char) * FP_STAT_STR_SZ);
        sal_strlcpy(tbl_str, _fp_qual_IpProtocolCommon_name(type),
                FP_STAT_STR_SZ);
        if (!sal_strcasecmp(tbl_str, type_str)) {
            break;
        }
        ...
        sal_memset(lng_str, 0, sizeof(char) * FP_STAT_DST_STR_SZ);
        sal_strlcpy(lng_str, prefix, FP_STAT_DST_STR_SZ);
        strncat(lng_str, tbl_str, FP_STAT_DST_STR_SZ - 1 - sal_strlen(lng_str));
        if (!sal_strcasecmp(lng_str, type_str)) {
            break;
        }
    }
...
fp_lookup_stage(const char *stage_str) 
{
    ...
    for (stage = 0; stage < bcmFieldStageCount; stage++) {
        ...
        sal_memset(lng_str, 0, sizeof(char) * FP_STAT_DST_STR_SZ);
        /* Test for the suffix only */
        sal_strlcpy(tbl_str, _fp_qual_stage_name(stage), FP_STAT_STR_SZ);
        if (!sal_strcasecmp(tbl_str, stage_str)) {
            break;
        }
        /* Test for whole name of the Stage */
        sal_strlcpy(lng_str, "bcmFieldStage", FP_STAT_DST_STR_SZ);
        sal_strncat(lng_str, tbl_str,
                FP_STAT_DST_STR_SZ - 1 - sal_strlen(lng_str));
...
fp_lookup_OamDrop(const char *type_str)
{
    ...
    for (type = 0; type < bcmFieldOamDropCount; (type)++) {
        /* Test for the suffix only */
        sal_memset(tbl_str, 0, sizeof(char) * FP_STAT_STR_SZ);
        sal_strlcpy(tbl_str, _fp_qual_OamDrop_name(type), FP_STAT_STR_SZ);
        if (!sal_strcasecmp(tbl_str, type_str)) {
            break;
        }
            ...
            char *qual_str)
{
    ...
    /* BCM.0> fp qual 'eid' Qual 'data' 'mask' */
    retval = func(unit, eid, data, mask);
    sal_strlcpy(str, "bcm_field_qualify_", FP_STAT_STR_SZ);
    sal_strncat(str, qual_str, FP_STAT_STR_SZ - 1 - strlen(str));
    FP_CHECK_RETURN(unit, retval, str);
            ...
            char *qual_str)
{
    ...
    /* BCM.0> fp qual 'eid' Qual 'data' 'mask'  */
    retval = func(unit, eid, data,mask);
    sal_strlcpy(str,src_str,FP_STAT_STR_SZ);
    sal_strncat(str, qual_str, FP_STAT_STR_SZ - 1 - strlen(str));
    FP_CHECK_RETURN(unit, retval, str);
...
{
    int                 statid = -1;
    bcm_field_stat_t    type = (bcm_field_stat_t) _bcmFieldStatCount;
    cmd_result_t        retval;
    uint64              val64;
    parse_table_t       pt;
