...
extern cint_data_t sand_signals_cint_data;
#endif
...
#if (defined(BCM_PETRA_SUPPORT) || defined(BCM_DNX_SUPPORT))
    cint_interpreter_add_data(&sand_signals_cint_data, NULL);
#endif
...
{ 
    ...
    if(init == 0) {
        ...
        /*
         * API logging depends on definitions of BCM APIs, datastructures etc
         * (PAPI) to be pushed into CINT. That has just been completed above,
         * call the the API logging init also here.
         */
...
cmd_cint(int unit, args_t* a)
{
    ...
    if(s) {
        ...
        if(!cmp) {
            ...
            cmp = sal_strcmp(s, "not_allow_file_info");
            if(!cmp) {
                s = ARG_GET(a);
            } else {
                argv[argc] = "allow_file_info";
                argc++; 
            }
        }
#endif
    ...
    if(ARG_CUR(a)) {
        /* Load file */
        s = ARG_GET(a);
        file_name = original_file_name = s;
        ...
        if((fp = sal_fopen(s, "r")) != NULL) {
            ...
            if ( argc <= CMD_CINT_ARG_MAX ) {
                cint_interpreter_parse(fp, NULL, argc, argv, file_name);
            }
            else {
...
/* { */
            /*
             * For adapter, if CINT file was not found and the prefix of the CINT file name is
             * as used on a real unit (../../../..) then replace it by the standard adapter prefix
             * (../../..) and try again.
             */
#define REAL_UNIT_CINT_PREFIX             "../../../.."
#define REAL_UNIT_CINT_PREFIX_EXTRA_CHARS 3
            if (sal_memcmp(REAL_UNIT_CINT_PREFIX,file_name,sizeof(REAL_UNIT_CINT_PREFIX) - 1) == 0) {
                file_name = &file_name[REAL_UNIT_CINT_PREFIX_EXTRA_CHARS];
                if((fp = sal_fopen((char *)file_name, "r")) != NULL) {
                    cli_out("Success: Fallback file '%s' was found and opened.\n", file_name);
                    sal_memset(&argv[argc], 0,
                               (sizeof(argv)-(argc*sizeof(char *))));
                    while( (s = ARG_GET(a)) ) {
                        if (argc < CMD_CINT_ARG_MAX) {
...
/* } */
        ...
        /*
         * Just 'cint' without arguments.
         */
        cli_out("Entering C Interpreter. Type 'exit;' to quit.\n\n");
        cint_interpreter_parse(NULL, "cint> ", argc, argv, NULL);
    }
    return 0;
}
