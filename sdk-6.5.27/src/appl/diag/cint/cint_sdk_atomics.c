...
__BCM_PBMP_ITER_HANDLER(const char* name, cint_ast_t* arguments, cint_ast_t* statements)
{
    ...
    _if = cint_ast_if(cint_ast_string("if"), _condition, statements, 0); 
...
__BCM_IF_ERROR_RETURN_HANDLER(const char* name, cint_ast_t* arguments)
{
    ...
    */
    ...
    cint_ast_t* _int = cint_ast_integer(0);
    _int->line = arguments->line;
    ...
    _if = cint_ast_if(cint_ast_string("if"), cint_ast_operator(cintOpLessThan, 
                                        __rv__, 
                                        cint_ast_integer(0)), 
...
extern char cint_ast_last_declared_function_name[];

static cint_ast_t*
__BCM_IF_ERROR_RETURN_MSG(const char* name, cint_ast_t* arguments)
{
    /* This macro do as the following in C code:

    do {
        int __rv__;
        if ((__rv__ = (arguments)) < 0) {
            printf("%s[%d]:%s Error \'%s\' indicated, %s\n", __file__, __line__, __func__, bcm_errmsg(__rv__), error);
            return(__rv__);
    } while(0)
    ...
    */
    ...
    cint_ast_t* _if;
    cint_ast_t* _statements;
    cint_ast_t* _fail_statements;
    cint_ast_t* _fail_msg_args;
    char file_name[200] = {0,};
    char function_name[200] = {0,};
    cint_ast_t* _bcm_errmsg;
    ...
    _fail_msg_args = cint_ast_string("%s[%d]:%s Error \'%s\' indicated, %s\n ");
    if(arguments->file == NULL){
        cint_ast_append(_fail_msg_args, cint_ast_string(" "));
    } else {
        CINT_STRNCPY(file_name, arguments->file, 198);
        CINT_STRCAT(file_name, " ");
        cint_ast_append(_fail_msg_args, cint_ast_string(file_name));
    }
    cint_ast_append(_fail_msg_args, cint_ast_integer(arguments->line));
    CINT_STRNCPY(function_name, cint_ast_last_declared_function_name, 198);
    CINT_STRCAT(function_name, " ");
    cint_ast_append(_fail_msg_args, cint_ast_string(function_name));
    _bcm_errmsg = cint_ast_function(cint_ast_identifier("bcm_errmsg"), __rv__);
    cint_ast_append(_fail_msg_args, _bcm_errmsg);
    cint_ast_append(_fail_msg_args, arguments->next);
    _fail_statements = cint_ast_function(cint_ast_identifier("printf"), _fail_msg_args);
    cint_ast_append(_fail_statements, cint_ast_return(__rv__));
...
#ifdef BCM_SAND_SUPPORT
        { "BCM_IF_ERROR_RETURN_MSG", __BCM_IF_ERROR_RETURN_MSG },
#endif
        { NULL }
    }; 
