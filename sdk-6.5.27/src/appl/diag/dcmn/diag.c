...
int qax_counters_graphic_print(int unit, uint32 print_none_zero, uint32 hex_base, uint32 core_disp) {
    ...
    for (i = 0; i < QAX_NOF_GRAPHIC_COUNTERS; ++i)
    {
       ...
       if (qax_counters[i].block == SOC_BLK_NBIL) {
    ...
    /*COMPILER_64_ADD_64(sum_counters ,dual_counters_data[NBIL_RX_NUM_TOTAL_DROPPED_EOPS]);*/
    for (i = QAX_NBIH_PRD_PKT_DROP_CNT_PORT_0; i <= QAX_NBIH_PRD_PKT_DROP_CNT_PORT_15; i++) {
        COMPILER_64_ADD_64(sum_counters, counters_data[i]);
    }
    for (i = QAX_NBIL_PRD_PKT_DROP_CNT_PORT_0; i <= QAX_NBIL_PRD_PKT_DROP_CNT_PORT_15; i++) {
        COMPILER_64_ADD_64(sum_counters, counters_data[i]);
        if (core_disp) {
            COMPILER_64_ADD_64(sum_counters, dual_counters_data[i]);
        }
...
int qux_counters_graphic_print(int unit, uint32 print_none_zero, uint32 hex_base, uint32 core_disp)
{
    ...
    COMPILER_64_ZERO(sum_counters);
    COMPILER_64_ADD_64(sum_counters ,counters_data[QUX_NIF_RX_NUM_TOTAL_DROPPED_EOPS]);
    for (i = QUX_NIF_PRD_PKT_DROP_CNT_PORT0; i <= QUX_NIF_PRD_PKT_DROP_CNT_PORT35; i++) {
        COMPILER_64_ADD_64(sum_counters, counters_data[i]);
    }
    compute_counter_val_into_string_2(unit, sum_counters, NIF_RX_NUM_TOTAL_DROPPED_EOPSr,
                                      RX_NUM_TOTAL_DROPPED_EOPSf, 31, print_none_zero, hex_base, cntbuf[0]);
