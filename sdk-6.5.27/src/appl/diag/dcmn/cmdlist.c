...
#if defined(BCM_PETRA_SUPPORT)
    {"DmaRomTest",  cmd_dpp_drt,              cmd_dpp_drt_usage,
     "Simple test of the SOC DMA ROM API" },
...
#endif
#if defined(BCM_PETRA_SUPPORT)
    {"KBP", cmd_dpp_kbp, cmd_dpp_kbp_usage, "Perform Access to KBP/KAPS data"},
#endif
#ifdef INCLUDE_KNET
...
#if (defined(BCM_DNX_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined (BCM_DPP_SUPPORT)) && defined(BCM_WARM_BOOT_SUPPORT)
    {"STATE",       sh_state,                 sh_state_usage,
    "state diagnostic tool"},
#endif /* (BCM_DNX_SUPPORT || BCM_PETRA_SUPPORT || BCM_DPP_SUPPORT) && BCM_WARM_BOOT_SUPPORT */
#ifdef BCM_PETRA_SUPPORT
