...
#if defined(BCM_JERICHO_SUPPORT) && !defined(BCM_88030)
    ...
    cmd_ppd_api_diag_pp_mc_info_print(int unit, int core, uint32 mc_id)
{
    ...
    if(SOC_IS_QAX(unit)){
        ...
        if (DPP_MCDS_TYPE_IS_INGRESS(QAX_MCDS_ENTRY_GET_TYPE(mcdb_entry))) { /* ingress multicast */
            ...
            uint32 format;
            uint32 destination, destination_1 = 0;
            uint32 cud, cud_1 = 0;
            ...
            while ((entry_index > 0) && (entry_index < QAX_MC_INGRESS_LINK_PTR_END)) {
                ...
                switch (format) {
                    ...
                    case 2: /* TAR_MCDB_DOUBLE_REPLICATIONm */
                        entry_index++;
                        destination = soc_mem_field32_get(unit, TAR_MCDB_DOUBLE_REPLICATIONm, mcdb_entry, DESTINATION_0f);
                        cud = soc_mem_field32_get(unit, TAR_MCDB_DOUBLE_REPLICATIONm, mcdb_entry, CUD_0f);
                        destination_1 = soc_mem_field32_get(unit, TAR_MCDB_DOUBLE_REPLICATIONm, mcdb_entry, DESTINATION_1f);
                        cud_1 = soc_mem_field32_get(unit, TAR_MCDB_DOUBLE_REPLICATIONm, mcdb_entry, CUD_1f);
                        break;
                ...
                {
                    LOG_CLI((BSL_META_U(unit, "    |            ")));
                    cmd_ppd_api_diag_pp_destination_19bits_print(unit, destination_1);
                    LOG_CLI((BSL_META_U(unit, "           |            0x%05X            |\n\r"), cud_1));
                    cud_1 = 0;
                }
            }
