...
static int dcmn_appl_bde_dev_used_by_destroyed_unit[SOC_MAX_NUM_DEVICES];
...
appl_dcmn_init(int unit, appl_dcmn_init_param_t* init_param) {
    ...
    if(!init_param->no_attach) {
        int bde_dev = dcmn_appl_bde_dev_used_by_destroyed_unit[unit];
    ...
    /* call to sysconf_chip_override cause kernel build error */
       ...
       sysconf_chip_override(unit, &devid, &revid); 
        ...
        LOG_INFO(BSL_LS_BCM_INIT,
                 (BSL_META_U(unit,
                             "%d: Attach unit.\n"), unit));
        rv = soc_cm_device_create(devid, revid, NULL, bde_dev);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_APPL_SHELL,
                      (BSL_META_U(unit,
    ...
    if (SOC_DPP_IS_ELK_ENABLE(unit)) {
        ...
        if (ARAD_KBP_IS_OP_OR_OP2) {
            ...
            /*
             * check with KBP if we can use it with PCIE destroy
             * system("/sbin/rmmod /projects/NTSW_SW_REGRESSION/regression/bin/op/kbp_driver.ko");
             */
            /* First check if the module is already load */
            FILE *fp;
            static uint8 is_kbp_driver_loaded = FALSE;
            char output[RHNAME_MAX_SIZE];
            if (!is_kbp_driver_loaded) {
                fp = popen("lsmod | grep kbp_driver", "r");
                if (fp == NULL) {
                    LOG_ERROR(BSL_LS_APPL_SHELL,
                              (BSL_META_U(unit,
                                          "Failed to open a pipe to read lsmod. \n")));
                    rv = 1;
                    goto exit;
                }
                sal_fgets(output, sizeof(output), fp);
                pclose(fp);
                if (sal_strlen(output) == 0 ) {
                    /* There is no print so the module is not loaded */
                    system("/sbin/insmod kbp_driver.ko");
                }
                is_kbp_driver_loaded = TRUE;
            }
        }
#endif
        if (!init_param->warmboot) {
    ...
    if( (!init_param->no_rx_los) && ((SOC_IS_ARAD(unit) && !SOC_IS_JERICHO(unit))) ) {
    ...
    SOC_FLAGS_SET(unit,SOC_FLAGS_GET(unit) | SOC_F_ALL_MODULES_INITED);
...
int appl_dcmn_deinit(int unit, appl_dcmn_init_param_t* init_param) 
{
    ...
    /* soc_cm_device_destroy() will destroy the unit to bde_dev mapping, so keep the bde dev fore recreating the unit by appl_dcmn_init() */
    dcmn_appl_bde_dev_used_by_destroyed_unit[unit] = soc_cm_unit_2_bde_dev(unit);
    ...
    if (!init_param->no_attach) {
        ...
        if (cint_logger_tr141_get() &&
            (soc_property_get(SOC_UNIT_NONE_SAL_CONFIG, spn_API_LOGGER_RESET_ON_TR141, 0) != 0)) {

            cli_out("%d: Resetting the Logger\n", unit);
            cint_logger_reset();
        }
#endif
