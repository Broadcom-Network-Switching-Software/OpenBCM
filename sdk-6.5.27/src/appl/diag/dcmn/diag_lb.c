  ...
  )
{
    ...
    if (SOC_E_NONE != soc_dpp_lb_enhancement_support_get(unit, &is_lb_enhance_support)) {
    ...
    for (lbg_tmp = lbg_start; lbg_tmp <= lbg_end; lbg_tmp++) {
        ...
        sub_item_cnt = 0;
        if (lbg_tmp != SOC_TMC_LB_CONTROL) {
            DPP_LB_SUB_ITEM_DEC(lbg_tmp);
        } else {
            DPP_LB_SUB_ITEM_STR("Ctl");
        }
        ...
        if (SOC_IS_QUX(unit)) {
            ...
            DPP_LB_SUB_ITEM_DEC(tx_lbg_info_tmp->speed);
        }
