    ...
    int meter_id)
{
    ...
    if (dnx_data_meter.diag.feature_get(unit, dnx_data_meter_diag_multi_dbs_rate_measurements_support))
    {
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_DATABASE_ID,
                ...
                else
                {
...
#endif
                    {
                        ...
                        SHR_IF_ERR_EXIT(sh_dnx_meter_rate_counters_get
                                        (unit, core_idx, is_egress, is_global, database_id, &cir_token_counter,
                                         &eir_token_counter));
