...
/*
 * vlan show command functions for trunk gport
 */
STATIC shr_error_e
sh_dnx_vlan_show_trunk_gport(
    int unit,
    bcm_vlan_data_t ** listp,
    int *countp)
{
    bcm_gport_t gport[BCM_PBMP_PORT_MAX] = { 0 };
    int is_untagged[BCM_PBMP_PORT_MAX] = { 0 };
    int array_size = 0;
    bcm_trunk_info_t trunk_info;
    bcm_trunk_member_t *trunk_member_array = NULL;
    int trunk_member_array_size = 256;
    int trunk_member_count, trunk_member_idx, vid_idx = 0;
    bcm_vlan_t vid;
    int trunk_id = 0;
    ...
    int j = 0;
    int count = *countp;
    bcm_vlan_data_t *list = *listp;

    SHR_FUNC_INIT_VARS(unit);

    SHR_ALLOC_SET_ZERO(trunk_member_array, (sizeof(bcm_trunk_member_t) * trunk_member_array_size), "trunk_member_array",
                       "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

    for (vid = (BCM_VLAN_MIN + 1); vid < BCM_VLAN_COUNT; vid++)
    {
        is_vsi_allocated = 0;
        SHR_IF_ERR_EXIT(vlan_db.vsi.is_allocated(unit, vid, &is_vsi_allocated));
        if (!is_vsi_allocated)
        {
            continue;
        }
        sal_memset(gport, 0, sizeof(gport));
        sal_memset(is_untagged, 0, sizeof(is_untagged));
        sal_memset(trunk_member_array, 0, (sizeof(bcm_trunk_member_t) * trunk_member_array_size));
        array_size = trunk_member_count = 0;
        SHR_IF_ERR_EXIT(bcm_vlan_gport_get_all(unit, vid, BCM_PBMP_PORT_MAX, gport, is_untagged, &array_size));
        if (array_size == 0)
        {
            continue;
        }
        ...
        {
            if (vid == list[vid_idx].vlan_tag)
            {
                break;
            }
        }

        /** The vid membership only has trunk gport */
        if (vid_idx == count)
        {
            list[count].vlan_tag = vid;
            BCM_PBMP_CLEAR(list[count].port_bitmap);
            BCM_PBMP_CLEAR(list[count].ut_port_bitmap);
            count++;
        }

        for (j = 0; j < array_size; j++)
        {
            if (!BCM_GPORT_IS_TRUNK(gport[j]))
            {
                continue;
            }
            trunk_id = BCM_GPORT_TRUNK_GET(gport[j]);
            SHR_IF_ERR_EXIT(bcm_trunk_get(unit, trunk_id, &trunk_info, 256, trunk_member_array, &trunk_member_count));
            for (trunk_member_idx = 0; trunk_member_idx < trunk_member_count; trunk_member_idx++)
            {
                local_port = BCM_GPORT_SYSTEM_PORT_ID_GET(trunk_member_array[trunk_member_idx].gport);
                if (local_port == BCM_GPORT_INVALID)
                {
                    continue;
                }

                BCM_PBMP_PORT_ADD(list[vid_idx].port_bitmap, local_port);
                if (is_untagged[j])
                {
                    BCM_PBMP_PORT_ADD(list[vid_idx].ut_port_bitmap, local_port);
                }
            }
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** Find trunk port member per vlan, and update related vid port bitmap */
    SHR_IF_ERR_EXIT(sh_dnx_vlan_show_trunk_gport(unit, &list, &count));
