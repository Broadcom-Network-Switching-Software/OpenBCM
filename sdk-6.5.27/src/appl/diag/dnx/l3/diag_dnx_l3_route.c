    ...
    int unit)
{
    ...
    route_entry.l3a_flags2 = fwd_only_flag;
    ...
    if (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group))
    {
        ...
        sal_memcpy(config.mc_ip6_addr, mc_ip6_addr, sizeof(config.mc_ip6_addr));
        sal_memcpy(config.mc_ip6_mask, mc_ip6_mask, sizeof(config.mc_ip6_mask));
        config.vrf = vrf;
        config.mc_ip_addr = 123; /* VRF + Group compressed value */ ;
        config.mc_ip_mask = 0xFFFF;
        SHR_IF_ERR_EXIT(bcm_ipmc_config_add(unit, &config));

        bcm_ipmc_addr_t_init(&data);
        data.flags |= (BCM_IPMC_IP6 | BCM_IPMC_RAW_ENTRY);
        data.mc_ip_addr = 123;  /* VRF + Group compressed value */
    ...
    if (dnx_data_elk.application.feature_get(unit, dnx_data_elk_application_ipv4)
        || dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_separate_fwd_rpf_dbs))
    {
        route_entry.l3a_flags2 |= fwd_only_flag;
    }
    SHR_IF_ERR_EXIT(bcm_l3_route_add(unit, &route_entry));
    ...
    int unit)
{
    ...
    if (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group))
    {
        ...
        sal_memcpy(config.mc_ip6_mask, mc_ip6_mask, sizeof(config.mc_ip6_mask));
        config.vrf = vrf;
        SHR_IF_ERR_EXIT(bcm_ipmc_config_remove(unit, &config));
    ...
    char *action)
{
    ...
    {
        sal_snprintf(buffer, DIAG_DNX_L3_IPV6_ADDRESS_PRINTABLE_STRING_SIZE,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else if ((ip6_present && !ip_present) || (v6_flag == TRUE))
    {
        ...
        if (dnx_data_l3.fwd.feature_get(unit, dnx_data_l3_fwd_ipmc_config_cmprs_vrf_group))
        {
            ...
            sal_memcpy(config.mc_ip6_mask, mc_ip6_mask, sizeof(config.mc_ip6_mask));
            config.vrf = vrf;
            config.mc_ip_addr = mc_ip_addr;
            config.mc_ip_mask = mc_ip_mask;
            SHR_IF_ERR_EXIT(bcm_ipmc_config_add(unit, &config));
