...
#define DNX_TRAP_LIST_LAST_PKT_SYNOPSIS "[core=0/1/all block=<IRPP | ERPP | ETPP>]"
#define DNX_TRAP_LIST_LAST_PKT_ADD_EXAMPLE "block=ETPP"
...
static sh_sand_man_t dnx_trap_last_pkt_info_man = {
    ...
    .synopsis = DNX_TRAP_LIST_LAST_PKT_SYNOPSIS,
    .examples = "block=IRPP core=0\n" DNX_TRAP_LIST_LAST_PKT_ADD_EXAMPLE,
...
const sh_dnx_trap_erpp_hit_indication_t erpp_hit_sig_list[] = {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (block_i = 0; block_i < SH_DNX_TRAP_INGRESS_NOF_BLOCKS; block_i++)
    {
            ...
            (cur_trap_code < DBAL_ENUM_FVAL_INGRESS_TRAP_ID_RESERVED_ITMH_SNIF_CODE))
        {
            ...
            PRT_CELL_SET("%s(0x%x)", dbal_enum_id_to_string(unit, DBAL_FIELD_INGRESS_TRAP_ID, cur_trap_code),
                         cur_trap_code);
    ...
    sh_dnx_trap_erpp_info_t * resolved_snp_info)
{
    ...
    uint32 hit_sig_val;
    int size_of_erpp_array = (sizeof(erpp_hit_sig_list) / sizeof(sh_dnx_trap_erpp_hit_indication_t));
    SHR_FUNC_INIT_VARS(unit);
    ...
    for (trap_ind = 0; trap_ind < size_of_erpp_array; trap_ind++)
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int trap_ind = 0, trap_id = 0, fwd_act_profile = 0;
    bcm_gport_t trap_gport_get;
    char trap_name[DBAL_MAX_STRING_LENGTH];
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_rx_trap_t fwd_considered_traps_list[bcmRxTrapCount] = { 0 };
    bcm_rx_trap_t snp_considered_traps_list[bcmRxTrapCount] = { 0 };
    ...
    bcm_rx_trap_t * trap_type_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_etpp_fwd_act_profile_map_to_oam_trap_type_dispatch
                    (unit, oam_sub_type, fwd_act_profile, &oam_trap_type));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_in, core_id)
    {
        sal_printf("\nCore %d:", core_id);
        sal_printf("\n-------\n");
