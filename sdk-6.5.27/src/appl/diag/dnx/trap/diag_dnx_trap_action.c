    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint8 ud_trap_name[BCM_RX_TRAP_MAX_NAME_LEN];
    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (snp_code < DBAL_ENUM_FVAL_INGRESS_TRAP_ID_RESERVED_ITMH_SNIF_CODE)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_ud_name_get(unit, snp_code, ud_trap_name));

        if (ud_trap_name[0] != '\0')
        {
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
            PRT_CELL_SET("Trap name");
            PRT_CELL_SET("%s", ud_trap_name);
        }
    }
    else
    {
    ...
    PRT_CELL_SET("%s(0x%x)", dbal_enum_id_to_string(unit, DBAL_FIELD_INGRESS_TRAP_ID, snp_code), snp_code);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    dnx_field_context_sys_hdr_profile_e dnx_fabric_header;
    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_ud_name_get(unit, trap_id, ud_trap_name));
    ...
    PRT_CELL_SET("%s(0x%x)", dbal_enum_id_to_string(unit, DBAL_FIELD_INGRESS_TRAP_ID, trap_id), trap_id);

    /** In case this is not last resolved trap, show fwd and snoop action in the same table */
    if (sal_strncmp(trap_title, "IRPP Trap Action", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        if (trap_config_p->snoop_strength)
        {
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
    ...
    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_UPDATE_ACL_PROFILE)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Update ACL Profile");
        PRT_CELL_SET("Enabled");
    }

    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_REPLICATION_FAILOVER_DISABLE)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Replication Failover");
        PRT_CELL_SET("Disabled");
    }

    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_FALLBACK_TO_BRIDGE_DISABLE)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Fallback to Bridge");
        PRT_CELL_SET("Disabled");
    }

    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_NETWORK_GROUP_DISABLE)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Network Group");
        PRT_CELL_SET("Disabled");
    }

    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_SAME_INTERFACE_DISABLE)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Same Interface");
        PRT_CELL_SET("Disabled");
    }

    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_UPDATE_TRUNK_MEMBERS_LOCAL_ONLY)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Trunk Members Local Only");
        PRT_CELL_SET("Enabled");
    }

    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_UPDATE_EXEM_LEARN)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Is EXEM Learn");
        if (trap_config_p->is_exem_learn == TRUE)
        {
            PRT_CELL_SET("TRUE");
        }
        else
    ...
    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_FABRIC_HEADER_SET)
    {
        SHR_IF_ERR_EXIT(dnx_field_map_system_header_profile_bcm_to_dnx
                        (unit, trap_config_p->fabric_header, &dnx_fabric_header));
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Fabric Header");
        PRT_CELL_SET("%s", dnx_data_field.system_headers.system_header_profiles_get(unit, dnx_fabric_header)->name);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    dnx_rx_trap_class_e trap_class = DNX_RX_TRAP_ID_CLASS_GET(unit, trap_id);
    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        PRT_CELL_SET("%s(0x%x)", dbal_enum_id_to_string(unit, DBAL_FIELD_ETPP_TRAP_ACT_PROFILE, fwd_act_profile),
                     fwd_act_profile);
    }
    else
    {
    ...
    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_KEEP_FABRIC_HEADER)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Keep Fabric Header");
        PRT_CELL_SET("TRUE");
    }

    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_MIRROR_CODE_CLEAR)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Discard Mirror Copy");
        PRT_CELL_SET("TRUE");
    }

    if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_SNOOP_CODE_CLEAR)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
        PRT_CELL_SET("Discard Snoop Copy");
        PRT_CELL_SET("TRUE");
    }
    ...
    else
    {
        ...
        if (trap_config_p->flags2 & BCM_RX_TRAP_FLAGS2_INT_STAT_DISABLE)
        {
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
            PRT_CELL_SET("Disable Statistics");
            PRT_CELL_SET("TRUE");
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if ((trap_block == DNX_RX_TRAP_BLOCK_ERPP) && !dnx_data_pp.stages.is_erpp_supported_get(unit))
    {
    ...
    else
    {
        ...
        switch (trap_block)
        {
            ...
            case DNX_RX_TRAP_BLOCK_ERPP:
                if (DNX_RX_TRAP_ID_CLASS_GET(unit, trap_id) != DNX_RX_TRAP_CLASS_USER_DEFINED)
                {
