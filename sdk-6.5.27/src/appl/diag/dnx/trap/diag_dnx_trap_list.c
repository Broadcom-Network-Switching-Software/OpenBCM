...
#define DNX_TRAP_LIST_UD_MAN_SYNOPSIS "[block=<IRPP | ERPP | ETPP> state=<USED | FREE | ALL>]"
#define DNX_TRAP_LIST_APPL_MAN_SYNOPSIS "[block=<ERPP | ETPP>]"
...
static sh_sand_man_t dnx_trap_list_ud_man = {
    ...
    .synopsis = DNX_TRAP_LIST_UD_MAN_SYNOPSIS,
...
static sh_sand_man_t dnx_trap_list_appl_man = {
    ...
    .synopsis = DNX_TRAP_LIST_APPL_MAN_SYNOPSIS,
    .examples = "block=ETPP"
};
...
sh_dnx_trap_list_ud_irpp_print(
    ...
    sh_sand_control_t * sand_control)
{
    uint8 is_allocated, trap_name[BCM_RX_TRAP_MAX_NAME_LEN];
    ...
    for (alloc_index = 0; alloc_index < dnx_data_trap.ingress.nof_user_defined_traps_get(unit); alloc_index++)
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_map_ingress_ud_alloc_get(unit, alloc_index, &trap_id));
        SHR_IF_ERR_EXIT(dnx_rx_trap_is_trap_id_allocated(unit, trap_id, &is_allocated));
        SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_ud_name_get(unit, trap_id, trap_name));
    ...
    switch (state)
    {
        ...
        case 1:
            ...
            while (trap_id != -1)
            {
                ...
                PRT_CELL_SET("0x%x", DNX_RX_TRAP_ID_TYPE_GET(trap_id));
    ...
    for (trap_id = ud_block_info.lower_bound_trap_id; trap_id <= ud_block_info.upper_bound_trap_id; trap_id++)
    {
        ...
        if ((state == 2) || (is_allocated == state))
        {
            ...
            if (trap_block == DNX_RX_TRAP_BLOCK_ETPP)
            {
                if (dnx_data_pp.stages.is_erpp_supported_get(unit))
                {
                    PRT_CELL_SET("%s(0x%x)", dbal_enum_id_to_string(unit, DBAL_FIELD_ETPP_TRAP_ACT_PROFILE, trap_id),
                                 trap_id);
                }
                else
                {
    ...
    sh_sand_control_t * sand_control)
{
    dnx_rx_trap_block_e trap_block;
    int state;
    uint8 egress_divided_to_erpp_and_etpp = dnx_data_pp.stages.is_erpp_supported_get(unit);
    SHR_FUNC_INIT_VARS(unit);
    ...
    switch (trap_block)
    {
        ...
        case DNX_RX_TRAP_BLOCK_ERPP:
            ...
            {
                LOG_CLI_EX("No ERPP block in device%s%s%s%s\n", EMPTY, EMPTY, EMPTY, EMPTY);
            }
            break;
        case DNX_RX_TRAP_BLOCK_ETPP:
            SHR_IF_ERR_EXIT(sh_dnx_trap_list_ud_egress_print(unit, state, trap_block, sand_control));
...
{
    dnx_rx_trap_map_type_t *trap_map_list;
    int nof_traps, list_ind, trap_id, default_trap_id, fwd_act_profile = 0;
    ...
    {
        default_trap_id = BCM_RX_TRAP_EG_TRAP_ID_DEFAULT;
    }
    else
    {
        default_trap_id = BCM_RX_TRAP_EG_TX_TRAP_ID_DEFAULT;
    }

    BCM_GPORT_TRAP_SET(trap_gport_empty, default_trap_id, 0, 0);
    ...
    for (list_ind = 0; list_ind < nof_traps; list_ind++)
    {
        ...
        if (trap_gport != trap_gport_empty)
        {
            ...
            fwd_act_profile = DNX_RX_TRAP_EGRESS_ACTION_PROFILE_GET(trap_id);
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
            PRT_CELL_SET("%s", trap_map_list[list_ind].trap_name);
            if (trap_block == DNX_RX_TRAP_BLOCK_ETPP)
            {
                ...
                {
                    PRT_CELL_SET("%s(0x%x)",
                                 dbal_enum_id_to_string(unit, DBAL_FIELD_ETPP_TRAP_ACT_PROFILE, fwd_act_profile),
                                 fwd_act_profile);
                }
                else
                {
