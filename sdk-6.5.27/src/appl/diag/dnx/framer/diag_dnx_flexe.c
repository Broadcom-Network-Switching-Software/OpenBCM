...
#include <framer_sal_j2x_info_define.h>
#endif
    ...
    else
    {
        ...
        BCM_PBMP_ITER(flexe_clients, logical_port)
        {
            ...
            if (stat_channel != -1)
            {
                ...
                for (i = flexe_core_mac_Count - 1; i >= 0; --i)
                {
    ...
    */
    ...
    for (ii=0; ii<COUNTOF(row_desc); ++ii)
    {
        ...
        SHR_BIT_ITER(&grp_active_bmp, DNX_DIAG_FLEXE_CORE_NOF_PORTS, group)
        {
            ...
            if (grp_instances != 0)
            {
                for(instance=0; instance<dnx_data_nif.flexe.nof_flexe_instances_get(unit); ++instance)
                {
                    /* coverity[ptr_arith:FALSE] */
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (lphy=0; lphy<DNX_DIAG_FLEXE_CORE_NOF_LPHYS; ++lphy)
    {
        ...
        if (reorder_sel < dnx_data_nif.flexe.nof_flexe_instances_get(unit))
        {
            PRT_CELL_SET("%d.0", reorder_sel);
        }
        else if (reorder_sel < (dnx_data_nif.flexe.nof_flexe_instances_get(unit) << 1))
        {
            PRT_CELL_SET("%d.1", reorder_sel-dnx_data_nif.flexe.nof_flexe_instances_get(unit));
        }
        else
        {
            PRT_CELL_SET("");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (ii=0; ii<dnx_data_nif.flexe.nof_flexe_instances_get(unit); ++ii)
    {
        PRT_COLUMN_ADD("%d", ii);
    }
...
static diag_flexe_dump_reg_row_t flexe_gen1_oh_rows[] =
    {
        {
            "RX Mode",
        ...
        {
            "SC inserted",
            FSCL_RESERVED_INS_CFGr,
            SC_INSf,
            "synchronization configuration inserted",
        },
        {
            "Actual SC",
            FSCL_CC_RX_VALUEr,
...
#ifndef NO_FRAMER_LIB_BUILD
static diag_flexe_dump_reg_row_t flexe_gen2_oh_rows[] =
    {
        {
            "RX Mode",
            FLEXE1_RX_MODE_CFG,
            flexe1_rx_oh_lc_flexe_rx_mode,
            "0: 50G; 1: 100G; 2: 200G; 3: 400G",
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
            "TX Mode",
            FLEXE1_TX_MODE_CFG,
            flexe1_tx_oh_lc_flexe_tx_mode,
            "0: 50G; 1: 100G; 2: 200G; 3: 400G",
            MODULE_J2X_FLEXE1_TX_OH_M0
        },
        {
            "RX multi-frame mode",
            FLEXE1_RX_MODE_CFG,
            flexe1_rx_oh_lc_mf16t32_rx_cfg,
            "0: each MF has 16 frames; 1: each MF has 32 frames",
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
            "TX multi-frame mode",
            FLEXE1_TX_MODE_CFG,
            flexe1_tx_oh_lc_mf16t32_tx_cfg,
            "0: each MF has 16 frames; 1: each MF has 32 frames",
            MODULE_J2X_FLEXE1_TX_OH_M0
        },
        {
        ...
        },
        {
            "Exp SC",
            FLEXE1_EXCC_OH_CFG,
            flexe1_rx_oh_lc_exsc,
            "Expected synchronization configuration",
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
            "SC inserted",
            FLEXE1_RESERVED_INS_CFG_W0,
            flexe1_tx_oh_lc_sc_ins,
            "synchronization configuration inserted",
            MODULE_J2X_FLEXE1_TX_OH_M0
        },
        {
...
{
    uint32 reg_val = 0;
    uint32 val = 0, group_index;
    uint32 instances_bmp[1] = { 0 };
    int ii, row, group_is_present;
    int group_valid;
    ...
    SH_SAND_IS_PRESENT("group", group_is_present);

    if (group_is_present)
    {
        SH_SAND_GET_UINT32("group", group_index);
        SHR_IF_ERR_EXIT(dnx_algo_flexe_group_valid_get(unit, group_index, &group_valid));
        if (!group_valid)
        {
            SHR_EXIT();
        }
         ...
         * Get FlexE instances
         */
        SHR_IF_ERR_EXIT(dnx_algo_flexe_group_instances_get(unit, group_index, instances_bmp));
    }
    else
    {
        SHR_BITSET_RANGE(instances_bmp, 0, dnx_data_nif.flexe.nof_flexe_instances_get(unit));
    }

    SHR_BIT_ITER(instances_bmp, dnx_data_nif.flexe.nof_flexe_instances_get(unit), ii)
    {
        PRT_COLUMN_ADD("%d", ii);
    }
    ...
    if (dnx_data_nif.framer.gen_get(unit) == DNX_ALGO_FLEXE_GEN_1)
    {
        for (row = 0; row < COUNTOF(flexe_gen1_oh_rows); ++row)
        {
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE_BEFORE);
            PRT_CELL_SET("%s", flexe_gen1_oh_rows[row].row_desc);

            SHR_BIT_ITER(instances_bmp, dnx_data_nif.flexe.nof_flexe_instances_get(unit), ii)
            {
                SHR_IF_ERR_EXIT(soc_custom_reg32_get(unit, flexe_gen1_oh_rows[row].reg, REG_PORT_ANY, ii, &reg_val));
                val = soc_reg_field_get(unit, flexe_gen1_oh_rows[row].reg, reg_val, flexe_gen1_oh_rows[row].field);

                PRT_CELL_SET("%d", val);
            }

            PRT_CELL_SET("%s", flexe_gen1_oh_rows[row].row_comments);
        }
    }
    else
    {
 #ifndef NO_FRAMER_LIB_BUILD
        for (row = 0; row < COUNTOF(flexe_gen2_oh_rows); ++row)
        {
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE_BEFORE);
            PRT_CELL_SET("%s", flexe_gen2_oh_rows[row].row_desc);

            SHR_BIT_ITER(instances_bmp, dnx_data_nif.flexe.nof_flexe_instances_get(unit), ii)
            {
                SHR_IF_ERR_EXIT(flexe_core_register_get(unit, flexe_gen2_oh_rows[row].module_id + ii,
                                flexe_gen2_oh_rows[row].reg, flexe_gen2_oh_rows[row].field, &val));
                PRT_CELL_SET("%d", val);
            }
            PRT_CELL_SET("%s", flexe_gen2_oh_rows[row].row_comments);
        }
#endif
    }
    PRT_COMMITX;
...
static sh_sand_option_t dnx_diag_flexe_dump_oh_info_options[] = {
    {"group",   SAL_FIELD_TYPE_UINT32, "FlexE group ID",  "0",  NULL},
    {NULL}
};
...
static diag_flexe_dump_reg_row_t flexe_gen1_oh_alarm_rows[] =
    {
        {
            "OOF",
            FSCL_OH_RX_ALMr,
        ...
        },
        {
            ...
            FSCL_B_66_CODE_ERRr,
            B_66_CODE_ERRf,
            "66b code error in FlexE OH Tx",
        },
    };
#ifndef NO_FRAMER_LIB_BUILD
static diag_flexe_dump_reg_row_t flexe_gen2_oh_alarm_rows[] =
    {
        {
            "OOF",
            FLEXE1_OH_RX_ALM,
            flexe1_rx_oh_lc_OOF_ALM,
            "Out of frame",
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
        ...
        },
        {
            "OOP",
            FLEXE1_OH_RX_ALM,
            flexe1_rx_oh_lc_OOP_ALM,
            "Out of PAD. Not relevant for 100G FlexE PHY",
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
            "LOP",
            FLEXE1_OH_RX_ALM,
            flexe1_rx_oh_lc_LOP_ALM,
            "Loss of PAD. Not relevant for 100G FlexE PHY",
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
            "OH1 block",
            FLEXE1_OH_RX_ALM,
            flexe1_rx_oh_lc_OH1_BLOCK_ALM,
            "OH1 block error (!= {2'b10, 8'b4B})",
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
        ...
        },
        {
            "OMF2 Vote alarm",
            FLEXE1_OH_RX_ALM,
            flexe1_rx_oh_lc_OMF2_VOTE_ALM,
            "3 OMF2 indications are not equal. 1G/1.25 slot mode Only",
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
            "OMF2 OOM",
            FLEXE1_OH_RX_ALM,
            flexe1_rx_oh_lc_OMF2_OOM_ALM,
            "OMF2 multi-frames are not locked. 1G/1.25 slot mode Only",
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
            "66B code error",
            FLEXE1_B66CODE_ERR,
            flexe1_tx_oh_lc_b66code_err,
            "66b code error in FlexE OH Tx",
            MODULE_J2X_FLEXE1_TX_OH_M0
        }
    };
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (dnx_data_nif.framer.gen_get(unit) == DNX_ALGO_FLEXE_GEN_1)
    {
        for (row = 0; row < COUNTOF(flexe_gen1_oh_alarm_rows); ++row)
        {
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE_BEFORE);
            PRT_CELL_SET("%s", flexe_gen1_oh_alarm_rows[row].row_desc);

            SHR_BIT_ITER(instances_bmp, dnx_data_nif.flexe.nof_flexe_instances_get(unit), ii)
            {
                SHR_IF_ERR_EXIT(soc_custom_reg32_get(unit, flexe_gen1_oh_alarm_rows[row].reg, REG_PORT_ANY, ii, &reg_val));
                val = soc_reg_field_get(unit, flexe_gen1_oh_alarm_rows[row].reg, reg_val, flexe_gen1_oh_alarm_rows[row].field);

                PRT_CELL_SET("%d", val);
            }

            PRT_CELL_SET("%s", flexe_gen1_oh_alarm_rows[row].row_comments);
        }
    }
    else
    {
 #ifndef NO_FRAMER_LIB_BUILD
        for (row = 0; row < COUNTOF(flexe_gen2_oh_alarm_rows); ++row)
        {
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE_BEFORE);
            PRT_CELL_SET("%s", flexe_gen2_oh_alarm_rows[row].row_desc);

            SHR_BIT_ITER(instances_bmp, dnx_data_nif.flexe.nof_flexe_instances_get(unit), ii)
            {
                SHR_IF_ERR_EXIT(flexe_core_register_get(unit, flexe_gen2_oh_alarm_rows[row].module_id + ii,
                                flexe_gen2_oh_alarm_rows[row].reg, flexe_gen2_oh_alarm_rows[row].field, &val));
                PRT_CELL_SET("%d", val);
            }
            PRT_CELL_SET("%s", flexe_gen2_oh_alarm_rows[row].row_comments);
        }
#endif
    }
    PRT_COMMITX;
...
static diag_flexe_dump_mem_row_t flexe_gen1_phymap_rows[] =
    {
        {
            "PHYMAP received",
            FSCL_PHYMAP_RXm,
...
#ifndef NO_FRAMER_LIB_BUILD
static diag_flexe_dump_mem_row_t flexe_gen2_phymap_rows[] =
    {
        {
            "PHYMAP received",
            FLEXE1_PHYMAP_RX,
            0,
            flexe1_rx_oh_lc_rx_acphymap,
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
            "PHYMAP RX expected",
            FLEXE1_PHYMAP_RX_EXP,
            0,
            flexe1_rx_oh_lc_rx_exphymap,
            MODULE_J2X_FLEXE1_RX_OH_M0
        },
        {
            "PHYMAP inserted",
            FLEXE1_PHYMAP_INS_CFG,
            0,
            flexe1_tx_oh_lc_phymap_ins,
            MODULE_J2X_FLEXE1_TX_OH_M0
        },
    };
#endif
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (row = 0; row < COUNTOF(flexe_gen1_phymap_rows); ++row)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE_BEFORE);
        PRT_CELL_SET("%s", flexe_gen1_phymap_rows[row].row_desc);
        ...
        for (ii = 0; ii < 32; ++ii)
        {
            if (dnx_data_nif.framer.gen_get(unit) == DNX_ALGO_FLEXE_GEN_1)
            {
                SHR_IF_ERR_EXIT(soc_custom_mem_array_read
                                (unit, flexe_gen1_phymap_rows[row].mem, instance, FSCL_BLOCK(unit), ii, &data));
            }
            else
            {
#ifndef NO_FRAMER_LIB_BUILD
                SHR_IF_ERR_EXIT(flexe_core_memory_get(unit, flexe_gen2_phymap_rows[row].module_id + instance,
                                                      flexe_gen2_phymap_rows[row].mem, flexe_gen2_phymap_rows[row].field,
                                                      ii, &data));
#endif
            }
            ...
            if (data != 0)
            {
                ...
                {
                    if (col < dnx_data_nif.flexe.nof_flexe_instances_get(unit))
                    {
...
sh_sand_cmd_t dnx_diag_flexe_dump_cmds[] = {
     ...
     * Name | Leaf Action | Junction Array | Options for Leaf | Usage
     */
    {"GRouP",    dnx_diag_flexe_dump_group_cmd,      NULL,                              dnx_diag_flexe_dump_group_options,     &dnx_diag_flexe_dump_group_man,   NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NOT_RELEASE, sh_cmd_is_flexe_gen1},
    {"DeMuX",    dnx_diag_flexe_dump_demux_cmd,      NULL,                              dnx_diag_flexe_dump_demux_options,     &dnx_diag_flexe_dump_demux_man,   NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NOT_RELEASE, sh_cmd_is_flexe_gen1},
    {"MUX",      dnx_diag_flexe_dump_mux_cmd,        NULL,                              dnx_diag_flexe_dump_mux_options,       &dnx_diag_flexe_dump_mux_man,     NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NOT_RELEASE, sh_cmd_is_flexe_gen1},
    {"Port",     dnx_diag_flexe_dump_port_cmd,       NULL,                              dnx_diag_flexe_dump_port_options,      &dnx_diag_flexe_dump_port_man,    NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NOT_RELEASE, sh_cmd_is_flexe_gen1},
    {"OverHead", NULL,                               dnx_diag_flexe_dump_oh_cmds,       NULL,                                  &dnx_diag_flexe_dump_oh_man,      NULL, NULL, SH_CMD_NOT_RELEASE},
    {"CHaNnel",  NULL,                               dnx_diag_flexe_dump_channel_cmds,  NULL,                                  &dnx_diag_flexe_dump_channel_man, NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NOT_RELEASE, sh_cmd_is_flexe_gen1},
    {"SW",       dnx_diag_flexe_dump_sw_cmd,         NULL,                              dnx_diag_flexe_dump_sw_options,        &dnx_diag_flexe_dump_sw_man,      NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NOT_RELEASE, sh_cmd_is_flexe_gen1},
