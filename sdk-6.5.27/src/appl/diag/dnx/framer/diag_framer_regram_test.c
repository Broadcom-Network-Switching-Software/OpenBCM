...
#include "framer_uas_j2x_reg_operate.h"
...
_diag_frame_regram_need_skip(
    FRAMER_MODULE_INST_S * module_inst,
    REG_INFO_S * reg_info)
{
    int rc = 0;
    ...
    void *user_data)
{
    ...
    rv = j2x_regp_read(unit, module_inst->module_addr, reg_info->offset_addr, &reg_data);
    ...
    void *user_data)
{
    ...
    rv = j2x_regp_write(unit, module_inst->module_addr, reg_info->offset_addr, reg_data);
    if (rv != 0)
    {
    ...
    void *user_data)
{
    ...
    {
        sal_memset(ram_data, 0, sizeof(uint32) * width32);
        rv = (j2x_ramp_read(unit, module_inst->module_addr, reg_info->offset_addr, ii, width, ram_data));
    ...
    void *user_data)
{
    ...
    for (ii = 0; ii < depth; ii++)
    {
        rv = j2x_ramp_write(unit, module_inst->module_addr, reg_info->offset_addr, ii, width, ram_data);
        if (rv != 0)
        {
