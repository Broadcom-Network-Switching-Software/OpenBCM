...
char cmd_dump_usage[] =
    ...
    "dbginfo - Display the command debug information and field debug info.\n\t"
    "pfx - Display with prefix format.\n\t"
    "brief - Display in brief format.\n\t"
    "cfg - Display configuration memories.\n\t"
    "\n"
    "Examples:\n\t"
    "Dump * 0 1\n\t"
    "Dump *.FBSW\n\t"
...
char cmd_write_usage[] =
    ...
    "dbginfo - Write memory and print debug info.\n\t"
    "\n"
    "Examples:\n\t"
...
char cmd_modify_usage[] =
    ...
    "raw - Write in raw format.\n\t"
    "pfx - Write in prefix format.\n\t"
    "dbginfo - Write memory and print field debug info.\n\t"
    "\n"
    "Examples:\n\t"
...
char cmd_list_usage[] =
    ...
    "dbginfo - LIST regisers/memories in dbg format and print field debug info.\n\t"
    ...
    void *user_data)
{
    ...
    if (mflags & FRAMER_SYMBOL_DISPLAY_F_CFG_SYM)
    {
    ...
    if (mflags & FRAMER_SYMBOL_DISPLAY_F_DBG_INFO)
    {
    ...
    for (k = start; k < start + count; k++)
    {
        ...
        {
            cli_out("%08x", data[i]);
            if ((mflags & FRAMER_SYMBOL_DISPLAY_F_DATA_NO_SPACE) == 0)
            {
                cli_out(" ");
            }
        }
        cli_out("\n");
        if (count == 1 && (mflags & FRAMER_SYMBOL_DISPLAY_F_BRIEF) == 0)
        {
            if (mflags & FRAMER_SYMBOL_DISPLAY_F_DBG_INFO)
            {
...
{
    int rv = _SHR_E_NONE;
    char *name, *arg2 = NULL, *arg3 = NULL, *extra_args = NULL;
    uint32 flags = 0, mflags = 0;
    uint32 dflen;
    diag_mem_t dm;
    ...
    for (;;)
    {
        ...
        {
            flags |= FRAMER_REGSFILE_F_DBG;
            mflags |= FRAMER_SYMBOL_DISPLAY_F_DBG_INFO;
            name = ARG_GET(a);
        }
        ...
        {
            mflags |= FRAMER_SYMBOL_DISPLAY_F_BRIEF;
            mflags |= FRAMER_SYMBOL_DISPLAY_F_DATA_NO_SPACE;
            name = ARG_GET(a);
        }
        else if (name && !sal_strncasecmp(name, "cfg", sal_strnlen(name, dflen)))
        {
            mflags |= FRAMER_SYMBOL_DISPLAY_F_CFG_SYM;
            name = ARG_GET(a);
        }
        else
        {
    ...
    void *user_data)
{
    ...
    {
        if (dl->flags & FRAMER_SYMBOL_DISPLAY_F_DBG_INFO)
        {
            fflags |= FRAMER_SYMBOL_FIELD_F_DBG;
        }
    ...
    args_t * a)
{
    ...
    for (;;)
    {
        ...
        {
            flags |= FRAMER_REGSFILE_F_DBG;
            dl.flags |= FRAMER_SYMBOL_DISPLAY_F_DBG_INFO;
            name = ARG_GET(a);
        }
