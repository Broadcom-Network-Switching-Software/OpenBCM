    ...
    FRAMER_U32 * data)
{
    ...
    for (i = 0; i < dw; i++)
    {
        SHR_IF_ERR_EXIT_NO_MSG(j2x_regp_read(unit, base_addr, offset + i, &data[i]));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    FRAMER_U32 * data)
{
    ...
    for (i = 0; i < dw; i++)
    {
        SHR_IF_ERR_EXIT_NO_MSG(j2x_regp_write(unit, base_addr, offset + i, data[i]));
    }
exit:
    SHR_FUNC_EXIT;
}
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT_NO_MSG(j2x_ramp_read(unit, base_addr, offset, index, width, data));
exit:
    SHR_FUNC_EXIT;
}
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT_NO_MSG(j2x_ramp_write(unit, base_addr, offset, index, width, data));
exit:
    SHR_FUNC_EXIT;
}
    ...
    FRAMER_U32 * value)
{
    ...
    FRAMER_U32 fvalue, fwidth, fmask, prev_value;
    ...
    FRAMER_U32 * value)
{
    ...
    FRAMER_U32 msb_mask, lsb_mask, lsb_mask_revert, lsb_mask_revert_low;
    FRAMER_U32 fvalue, fwidth, prev_data, curr_data;
    ...
    lsb_mask_revert = (32 - lsbm) >= 32 ? 0 : 0xffffffff & ~lsb_mask_revert_low;
    prev_data = 0;
    i = 0;
    for (j = wmsb; j >= wlsb; j--)
    {
