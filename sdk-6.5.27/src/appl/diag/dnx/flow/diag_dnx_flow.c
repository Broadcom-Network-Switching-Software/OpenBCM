    ...
    int *is_exact_match)
{
    ...
    *is_exact_match = FALSE;
    ...
    for (; handle_idx < nof_flow_apps; handle_idx++)
    {
        ...
        if (sal_strncaseindex(flow_app_name_str, str_match, FLOW_STR_MAX, FLOW_STR_MAX))
        {
            break;
        }
    }
    ...
    uint8 *found_field)
{
    ...
    for (field_idx = 0; field_idx < nof_common_fields; field_idx++)
    {
         ...
         */
        if ((common_fields_desc != NULL)
            && sal_strncaseindex(common_fields_desc->field_name, field_name_str_match, FLOW_STR_MAX, FLOW_STR_MAX))
        {
            *found_field = TRUE;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (field_idx = FLOW_S_F_EMPTY + 1; field_idx < NOF_FLOW_SPECIAL_FIELDS; field_idx++)
    {
        ...
        if (sal_strncaseindex(special_field->name, field_name_str_match, FLOW_STR_MAX, FLOW_STR_MAX))
        {
