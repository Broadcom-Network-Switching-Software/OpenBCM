...
/**
 * \brief -
 * oam ClassiFier command - Read inputs from egress signals
 * Used when LAST option is on.
 * \param [in] unit -     Relevant unit.
 * \param [in] core -     Relevant core.
 * \param [in] *oam_cf_inputs - pointer to inputs to oam cf
 ...
 */
static shr_error_e
diag_oam_cf_read_egress_signals(
    int unit,
    int core,
    oam_cf_inputs_t * oam_cf_inputs)
{

    uint32 lif_entry_handle_id;
    uint32 global_lif;
    char global_out_lif_sig_name[RHNAME_MAX_SIZE];
    char sig_name[RHNAME_MAX_SIZE];
    int rv = 0;
    uint32 packet_header[32];
    uint32 EPP_Layer_Offsets[2];
    uint32 PPH_Fwd_Layer_Index;
    uint8 temp_byte;
    int fwd_plus1_offset;
    uint16 oam_inject_map;
    uint32 Fwd_Action_Strength;
    uint32 handle_id;
    uint32 Out_PP_Port;
    uint8 mac_msb[5];
    int my_cfm_mac;
    uint8 da_lsb;
    uint32 mac_lsb_range[8];
    int nof_lifs = 3;
    int i;
    ...
    for (i = 0; i < nof_lifs; i++)
    {
        sal_snprintf(global_out_lif_sig_name, RHNAME_MAX_SIZE, "Global_Out_LIF_%d", i);
        SHR_IF_ERR_EXIT_NO_MSG(dpp_dsig_read(unit, core, NULL, "PRP", NULL, global_out_lif_sig_name, &global_lif, 4));
        if (global_lif > 0)
        {
            ...
            if (rv == _SHR_E_NONE)
            {
                if (i == 0)
                {
                    SHR_IF_ERR_EXIT_NO_MSG(dbal_entry_handle_value_field32_get
                                           (unit, lif_entry_handle_id, DBAL_FIELD_OUT_LIF, INST_SINGLE,
                                            &oam_cf_inputs->lif));
                    oam_cf_inputs->lif_1 = oam_cf_inputs->lif;
                }
                if (i == 1)
     ...
     * Read Packet and PDU 
     */
    sal_snprintf(sig_name, RHNAME_MAX_SIZE, "Pkt_Header");
    SHR_IF_ERR_EXIT_NO_MSG(dpp_dsig_read(unit, core, NULL, NULL, "Term", sig_name, packet_header, 1024));

    sal_snprintf(sig_name, RHNAME_MAX_SIZE, "PPH_Fwd_Layer_Index");
    SHR_IF_ERR_EXIT_NO_MSG(dpp_dsig_read(unit, core, NULL, NULL, "Term", sig_name, &PPH_Fwd_Layer_Index, 3));

    sal_snprintf(sig_name, RHNAME_MAX_SIZE, "EPP_Layer_Offsets");
    SHR_IF_ERR_EXIT_NO_MSG(dpp_dsig_read(unit, core, NULL, NULL, "Term", sig_name, EPP_Layer_Offsets, 45));

    fwd_plus1_offset = (EPP_Layer_Offsets[0] >> ((PPH_Fwd_Layer_Index + 1) * 9)) & 0x1ff;
    oam_cf_inputs->mdl = packet_header[31 - (fwd_plus1_offset / 4)];

    /*
     * MDL 
     */
    temp_byte = (packet_header[31 - (fwd_plus1_offset / 4)]) >> ((3 - (fwd_plus1_offset % 4)) * 8);
    oam_cf_inputs->mdl = (temp_byte >> 5) & 0x7;

    /*
     * OPCODE 
     */
    temp_byte = (packet_header[31 - (fwd_plus1_offset / 4)]) >> ((3 - (fwd_plus1_offset % 4) - 1) * 8);
    oam_cf_inputs->opcode = temp_byte;

    /*
     * INJECT 
     */
    sal_snprintf(sig_name, RHNAME_MAX_SIZE, "Fwd_Action_Strength");
    SHR_IF_ERR_EXIT_NO_MSG(dpp_dsig_read(unit, core, NULL, NULL, "Term", sig_name, &Fwd_Action_Strength, 4));

    SHR_IF_ERR_EXIT_NO_MSG(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_OAM_GENERAL_CONFIGURATION, &handle_id));
    rv = dbal_entry_get(unit, handle_id, DBAL_GET_ALL_FIELDS);
    if (rv == _SHR_E_NONE)
    {
        SHR_IF_ERR_EXIT_NO_MSG(dbal_entry_handle_value_field16_get
                               (unit, handle_id, DBAL_FIELD_EGRESS_MAP_STRENGTH_TO_OAM_INJECTION, INST_SINGLE,
                                &oam_inject_map));
    }

    SHR_IF_ERR_EXIT_NO_MSG(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_OAM_GENERAL_CONFIGURATION, handle_id));

    oam_cf_inputs->inj = oam_inject_map & (1 << Fwd_Action_Strength);

    /*
     * packet_is_bfd 
     */
    oam_cf_inputs->pkt_is_bfd = 0;

    /*
     * My CFM MAC 
     */

    sal_snprintf(sig_name, RHNAME_MAX_SIZE, "Out_PP_Port");
    SHR_IF_ERR_EXIT_NO_MSG(dpp_dsig_read(unit, core, NULL, NULL, "Term", sig_name, &Out_PP_Port, 4));

    SHR_IF_ERR_EXIT_NO_MSG(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EGRESS_PP_PORT, &handle_id));
    /** Set key */
    dbal_entry_key_field32_set(unit, handle_id, DBAL_FIELD_PP_PORT, Out_PP_Port);
    dbal_entry_key_field32_set(unit, handle_id, DBAL_FIELD_CORE_ID, core);

    /** Get values */
    dbal_value_field_arr8_request(unit, handle_id, DBAL_FIELD_MY_CFM_MAC_MSB, INST_SINGLE, mac_msb);
    dbal_value_field_arr32_request(unit, handle_id, DBAL_FIELD_MY_CFM_MAC_LSB_MAP, INST_SINGLE, mac_lsb_range);
    /** Access the table */
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, handle_id, DBAL_COMMIT));

    SHR_IF_ERR_EXIT_NO_MSG(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_EGRESS_PP_PORT, handle_id));

    my_cfm_mac = 1;
    for (i = 0; i < 5; i++)
    {
        if (packet_header[31 - i] != mac_msb[i])
        {
            my_cfm_mac = 0;
        }
    }

    da_lsb = packet_header[26];
    if (((mac_lsb_range[da_lsb / 32] >> (da_lsb % 32)) & 1) == 0)
    {
        my_cfm_mac = 0;
    }

    oam_cf_inputs->mymac = my_cfm_mac;

    /*
     * da_is_mc 
     */
    oam_cf_inputs->da_is_mc = packet_header[31] & 0x1;  /* Bit 40 */
    ...
    oam_cf_results_t * oam_cf_results)
{
    ...
    if (oam_cf_results->acc_entry_found)
    {
        ...
        if (oam_cf_inputs->ing)
        {
            sal_snprintf(disp_line[47], sizeof(disp_line[47]), "    |  ACC  | --- trap_code------(0x%08x)--->",
                         oam_cf_results->trap_code);
        }
        else
        {
            sal_snprintf(disp_line[47], sizeof(disp_line[47]), "    |  ACC  | --- action-profile-(0x%08x)--->",
                         oam_cf_results->trap_code);
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (oam_cf_inputs.last_run == 1)
    {
        ...
        {
            SHR_IF_ERR_EXIT(diag_oam_cf_read_egress_signals(unit, oam_cf_inputs.core_id, &oam_cf_inputs));
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (oam_bfdep_results->endpoint_info.flags & BCM_BFD_ENDPOINT_IN_HW)
    {
        ...
        switch (oam_bfdep_results->endpoint_info.remote_diag)
        {
            ...
            default:
            {
                PRT_CELL_SET("0x%x", oam_bfdep_results->endpoint_info.remote_diag);
                oam_bfdep_results->no_errs++;
                break;
            }
        }
    ...
    void *user_data)
{
    ...
    while (DNX_SW_STATE_LL_IS_NODE_VALID(node))
    {
        ...
        oam_sw_db_info.oam_group_sw_db_info.oam_group_array_of_linked_lists.next_node(unit, node, &node);
...
{                                                                                                    \
     ...
     */
    {
        ...
        SHR_IF_ERR_EXIT(soc_mem_array_read
                        (unit, counter_bank, eg_array_index, block, eg_entry_index, &eg_entry_above_64));
        /*
        ...
        for (pcp_index = 1; pcp_index < 8; pcp_index++)
        {
            index = (pcp_index + inner_hw_index) % nof_counters_per_entry;
            {
                if ((pcp_index + inner_hw_index) == nof_counters_per_entry)
                {
                    block = MCP_BLOCK(unit, core);
                    /*
                     * coverity explanation: coverity has found that we get here for NEW_ACCESS
                     * and oamp_mep_db_mem = mOAMP_MEP_DB, which causes an out-of-bounds access.
                     * This is a false positive.
                     */
                     /* coverity[overrun-call:FALSE]  */
                    SHR_IF_ERR_EXIT(soc_mem_array_read
                                    (unit, counter_bank, ing_array_index, block, ing_entry_index + 1,
                                     &ing_entry_above_64));
                    SHR_IF_ERR_EXIT(soc_mem_array_read
                                    (unit, counter_bank, eg_array_index, block, eg_entry_index + 1,
                                     &eg_entry_above_64));
                }
