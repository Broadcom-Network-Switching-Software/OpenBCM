    ...
    SH_DNX_CORES_ITER(unit, core_in, core_id)
    {
        ...
        SH_DNX_CORES_ITER(unit, core_in, ing_core_id)
        {
                            ...
                            (unit, DBAL_FIELD_IVE_OUTER_VID_SRC, vid_src_out, &vid_src_out));
            {
                ...
                uint32 sizeof_copy;
                if (sizeof(vid_parse_out) < sizeof(str))
                {
                    /*
                     * Container is too small!
                     */
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "Sizeof vid_parse_out (%d) is too small to contain output (%d is required)\n",
                                 (int) sizeof(vid_parse_out), (int) sizeof(str));
                }
                else
                {
                    /*
                     * Only DBAL_MAX_STRING_LENGTH bytes are copied to the destination!
                     */
                    sizeof_copy = sizeof(str);
                }
                ...
                SHR_IF_ERR_EXIT(dnx_dbal_fields_string_form_hw_value_get
                                (unit, (char *) dbal_field_to_string(unit, DBAL_FIELD_IVE_OUTER_VID_SRC), vid_src_out,
                                 str));
                sal_memcpy(vid_parse_out, str, sizeof_copy);
            }
            SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                            (unit, DBAL_FIELD_IVE_OUTER_PCP_DEI_SRC, pcp_dei_src_out, &pcp_dei_src_out));
            {
                char str[DBAL_MAX_STRING_LENGTH];
                uint32 sizeof_copy;
                if (sizeof(pcp_dei_parse_out) < sizeof(str))
                {
                    /*
                     * Container is too small!
                     */
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "Sizeof pcp_dei_parse_out (%d) is too small to contain output (%d is required)\n",
                                 (int) sizeof(pcp_dei_parse_out), (int) sizeof(str));
                }
                else
                {
                    /*
                ...
                SHR_IF_ERR_EXIT(dnx_dbal_fields_string_form_hw_value_get
                                (unit, (char *) dbal_field_to_string(unit, DBAL_FIELD_IVE_OUTER_PCP_DEI_SRC),
                                 pcp_dei_src_out, str));
                sal_memcpy(pcp_dei_parse_out, str, sizeof_copy);
            }
            SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_IVE_INNER_VID_SRC, vid_src_in, &vid_src_in));
            {
                char str[DBAL_MAX_STRING_LENGTH];
                uint32 sizeof_copy;
                if (sizeof(vid_parse_in) < sizeof(str))
                {
                    /*
                     * Container is too small!
                     */
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "Sizeof vid_parse_in (%d) is too small to contain output (%d is required)\n",
                                 (int) sizeof(vid_parse_in), (int) sizeof(str));
                }
                else
                {
                    /*
                ...
                sal_memset(str, 0, sizeof(str));
                SHR_IF_ERR_EXIT(dnx_dbal_fields_string_form_hw_value_get
                                (unit, (char *) dbal_field_to_string(unit, DBAL_FIELD_IVE_INNER_VID_SRC),
                                 vid_src_in, str));
                sal_memcpy(vid_parse_in, str, sizeof_copy);
            }
            SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
                            (unit, DBAL_FIELD_IVE_INNER_PCP_DEI_SRC, pcp_dei_src_in, &pcp_dei_src_in));
            {
                char str[DBAL_MAX_STRING_LENGTH];
                uint32 sizeof_copy;
                if (sizeof(pcp_dei_parse_in) < sizeof(str))
                {
                    /*
                     * Container is too small!
                     */
                    SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                 "Sizeof pcp_dei_parse_in (%d) is too small to contain output (%d is required)\n",
                                 (int) sizeof(pcp_dei_parse_in), (int) sizeof(str));
                }
                else
                {
                    /*
                ...
                SHR_IF_ERR_EXIT(dnx_dbal_fields_string_form_hw_value_get
                                (unit, (char *) dbal_field_to_string(unit, DBAL_FIELD_IVE_INNER_PCP_DEI_SRC),
                                 pcp_dei_src_in, str));
                sal_memcpy(pcp_dei_parse_in, str, sizeof_copy);
            }
    ...
    int *nof_tags)
{
    ...
    if (field_incoming_tag_structure <= DBAL_ENUM_FVAL_INCOMING_TAG_STRUCTURE_UNTAGGED_1)
    {
        *nof_tags = 0;
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core_id, "ETPP", "Term", "FWD", "context", &fwd_context, 1));
    /** convert the fwd_context HW value (from signal) to ENUM value of this field in DBAL */
    SHR_IF_ERR_EXIT(dbal_fields_enum_value_get(unit, DBAL_FIELD_FWD_CONTEXT_ID, fwd_context, &fwd_context));
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_EGRESS_ETPP_FWD_CONTEXT_PROPERTIES, &entry_handle_id));
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FWD_CONTEXT_ID, fwd_context);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_FORWARDING_CANDIDATE_HEADER, INST_SINGLE,
                     &fwd_candidate_header));

    /** AC is found in ETPS_1 and fwd_candidate_header is ethernet, means using in FWD stage, otherwise ENCAP stage*/
    if ((ac_etps_id == 1) && (fwd_candidate_header == DBAL_ENUM_FVAL_ETPP_FORWARDING_CANDIDATE_HEADER_ETHERNET))
    {
        ...
        SHR_IF_ERR_EXIT(dpp_dsig_read
                        (unit, core_id, "ETPP", "Term", "FWD", "Incoming_Tag_Structure", &incoming_tag_structure, 1));

        {
            char str[DBAL_MAX_STRING_LENGTH];
            uint32 sizeof_copy;
            if (sizeof(incoming_tag_structure_parse) < sizeof(str))
            {
                /*
                 * Container is too small!
                 */
                SHR_ERR_EXIT(_SHR_E_INTERNAL,
                             "Sizeof incoming_tag_structure_parse (%d) is too small to contain output (%d is required)\n",
                             (int) sizeof(incoming_tag_structure_parse), (int) sizeof(str));
            }
            else
            {
                /*
            ...
            sal_memset(str, 0, sizeof(str));
            SHR_IF_ERR_EXIT(dnx_dbal_fields_string_form_hw_value_get
                            (unit, (char *) dbal_field_to_string(unit, DBAL_FIELD_INCOMING_TAG_STRUCTURE),
                             incoming_tag_structure, str));
            sal_memcpy(incoming_tag_structure_parse, str, sizeof_copy);
        }
        /** convert the in_tag_s HW value (from signal) to ENUM value of this field in DBAL */
        SHR_IF_ERR_EXIT(dbal_fields_enum_value_get(unit, DBAL_FIELD_INCOMING_TAG_STRUCTURE, incoming_tag_structure,
                                                   &incoming_tag_structure_sw));

        /** Read EVE map table (FWD stage only)*/
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_EGRESS_VLAN_EDIT_COMMAND_MAPPING, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VLAN_EDIT_PROFILE, vlan_edit_profile);
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_INCOMING_TAG_STRUCTURE, incoming_tag_structure_sw);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    ...
    else
    {
        ...
        /** Read EVE command table*/
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_EGRESS_VLAN_EDIT_COMMAND_ENCAP, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_VLAN_EDIT_PROFILE, vlan_edit_profile);
        SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_EVE_OUTER_VID_SRC, vid_src_out, &vid_src_out));
    {
        ...
        sal_memset(str, 0, sizeof(str));
        SHR_IF_ERR_EXIT(dnx_dbal_fields_string_form_hw_value_get
                        (unit, (char *) dbal_field_to_string(unit, DBAL_FIELD_EVE_OUTER_VID_SRC), vid_src_out, str));
        sal_memcpy(vid_parse_out, str, sizeof_copy);
    }
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
    ...
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_EVE_INNER_VID_SRC, vid_src_in, &vid_src_in));
    {
        ...
        sal_memset(str, 0, sizeof(str));
        SHR_IF_ERR_EXIT(dnx_dbal_fields_string_form_hw_value_get
                        (unit, (char *) dbal_field_to_string(unit, DBAL_FIELD_EVE_INNER_VID_SRC), vid_src_in, str));
        sal_memcpy(vid_parse_in, str, sizeof_copy);
    }
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_DNX_CORES_ITER(unit, core_in, core_id)
    {
        ...
        /** Get etps info */
        SHR_IF_ERR_EXIT(dpp_dsig_read(unit, core_id, "ETPP", "Term", "", "ETPS_Valid", &etps_valid, 1));
         ...
         */
        if (sal_strncaseindex(eve_stage, "all", SH_SAND_MAX_TOKEN_SIZE, sizeof("all"))
            || sal_strncaseindex(eve_stage, "NaTiVe", SH_SAND_MAX_TOKEN_SIZE, sizeof("NaTiVe"))
            || sal_strncaseindex(eve_stage, "ntv", SH_SAND_MAX_TOKEN_SIZE, sizeof("ntv")))
        {
         ...
         */
        if (sal_strncaseindex(eve_stage, "all", SH_SAND_MAX_TOKEN_SIZE, sizeof("all"))
            || sal_strncaseindex(eve_stage, "out", SH_SAND_MAX_TOKEN_SIZE, sizeof("out")))
        {
