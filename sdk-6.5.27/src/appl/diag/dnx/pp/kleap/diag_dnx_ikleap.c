...
{                                                               \
    if (kbr2phy_mapping[kbr_idx] == DBAL_PHYSICAL_TABLE_FLP_LARGE_TCAM) \
    {                                                           \
        continue;                                               \
    }                                                           \
}
...
{                                                                               \
    ...
    {                                                                           \
        ...
        else if (kbr2phy_mapping[kbr_idx] == DBAL_PHYSICAL_TABLE_FLP_LARGE_TCAM) \
        {                                                                       \
            SHR_IF_ERR_EXIT(diag_dnx_ikleap_flp_large_tcam_opcode_print(unit, context_hw_val, acl_context, prt_ctr)); \
        }                                                                       \
        else                                                                    \
        {                                                                       \
            SHR_ERR_EXIT(_SHR_E_CONFIG,                                         \
                         "not a PMF stage & non FLP LARGE TCAM KBR & app_db_id is non present - non legal state\n"\
                         "sub-stage: %s, context: %d, kbr: %d\n", stage_name, context_enum_val, kbr_idx);\
        }                                                                       \
    }                                                                           \
}

#define DIAG_IKLEAP_VIS_SET_CONTEXT_AND_NASID_FOR_FLP_LARGE_TCAM                                            \
do {                                                                                                        \
...
{                                                                               \
    ...
    {                                                                           \
        ...
        DIAG_IKLEAP_VIS_SET_CONTEXT_AND_NASID_FOR_FLP_LARGE_TCAM;               \
    }                                                                           \
    else                                                                        \
    {                                                                           \
        continue;                                                               \
...
#define IKLEAP_KAPS_STRENGTH_WIDTH           (3)
#define IKLEAP_LPM_LART_AND_DEFAULT_WIDTH    (4+1)
...
static shr_error_e
diag_dnx_ikleap_flp_large_tcam_opcode_print(
    int unit,
    uint8 context_hw_val,
    uint8 acl_context,
    ...
    int core_in)
{
    ...
    {
        if (core_in == (dnx_data_adapter.reg_mem_access.swap_core_index_zero_with_core_index_get(unit) +
    ...
    int *is_default_lif)
{
    ...
    dbal_physical_tables_e dbal_phy_db = DBAL_NOF_PHYSICAL_TABLES;
    ...
    char **updated_res_string)
{
    ...
    {
        int nof_bits_to_clear = IKLEAP_KAPS_STRENGTH_WIDTH;

        if (dnx_data_mdb.kaps.max_nof_result_types_get(unit) != 1)
        {
            nof_bits_to_clear = IKLEAP_LPM_LART_AND_DEFAULT_WIDTH;
        }
        SHR_IF_ERR_EXIT(utilex_bitstream_reset_bit_range(res_sig_value,
                                                         res_sig_size - nof_bits_to_clear, res_sig_size - 1));
        res_sig_size -= nof_bits_to_clear;
    ...
    /** Start key-result parsing using DBAL */
    {
        ...
        if (phyDb != DBAL_PHYSICAL_TABLE_FLP_LARGE_TCAM)
        {
            SHR_IF_ERR_EXIT_EXCEPT_IF(dbal_table_printable_entry_get(unit, logical_table_for_kbr,
                                                                     key_sig_value, DBAL_RESULT_TYPE_NOT_INITIALIZED,
                                                                     core_id, res_sig_size, res_sig_value,
        ...
        else
        {
            if (dnx_kbp_device_enabled(unit))
            {
                SHR_IF_ERR_EXIT_EXCEPT_IF(kbp_mngr_opcode_printable_entry_get(unit, 0, context_hw_val, acl_context,
                                                                              key_sig_value, key_sig_size,
                                                                              res_sig_value, res_sig_size, nof_results,
...
sh_sand_man_t dnx_ikleap_visib_man = {.brief = "Show last packet key and payload per lookup",
    ...
    .examples = "stage=fwd1\n" "stage=vtt1 silence=Yes phyDB=ISEM1\n" "stage=fwd1 SHORT",
    ...
    sh_sand_control_t * sand_control)
{
    ...
    kbr_value_e *key_sig_fwd12_2flp_large_tcam_kbr = NULL;
             ...
             stage_index++)
        {
            ...
            for (sub_stage = 0; sub_stage <= STAGE_INFO.stage_type; sub_stage++)
            {
                ...
                for (kbr_idx = 2; kbr_idx < nof_kbr; kbr_idx++)
                {
                    ...
                    uint32 context_val = context_hw_val;
                     ...
                     */
                    dbal_entry_key_field32_set(unit, kbr_info_handle_id, context_profile_field, context_val);
                    ...
                    if (!fixed_lookup_key_indication)
                    {
                        ...
                        else if (kbr2phy_mapping[kbr_idx] == DBAL_PHYSICAL_TABLE_FLP_LARGE_TCAM)
                        {
                            ...
                            key_sig_fwd12_2flp_large_tcam_kbr = key_sig_fwd12_2kbp_kbr;
                    ...
                    if (kbr_has_app_db_id)
                    {
                        ...
                        {
                            LOG_CLI((BSL_META
                                     ("\nFailed to find signal: %s from: %s to %s\n\n"), app_sig_name, stage_name,
                                     phy_db_name));
                            continue;
                        }
                    ...
                    for (ii = 0; ii < nof_key_signals_to_concatanate; ii++)
                    {
                        ...
                        if ((sal_strncmp(STAGE_INFO.kleap_stage_name, "FWD12", KLEAP_STAGE_NAME_LEN) == 0) &&
                            (kbr2phy_mapping[kbr_idx] == DBAL_PHYSICAL_TABLE_FLP_LARGE_TCAM))
                        {
                            uint32 flp_large_tcam_context = context_hw_val;
                            /** Check if the key is ACL or FWD (key is mapped to KBR) */
                            dbal_entry_key_field32_set(unit, kbr_info_handle_id, context_profile_field,
                                                       flp_large_tcam_context);
                            dbal_entry_key_field32_set(unit, kbr_info_handle_id, kbr_idx_field,
                                                       key_sig_fwd12_2flp_large_tcam_kbr[ii]);
                    ...
                    if (kbr2phy_mapping[kbr_idx] == DBAL_PHYSICAL_TABLE_FLP_LARGE_TCAM)
                    {
                        uint8 ext_tcam_lkp_index;

                        for (ext_tcam_lkp_index = 0; ext_tcam_lkp_index <= nof_results; ext_tcam_lkp_index++)
                        {
                            PRT_CELL_SET("%s", key_signal_str[ext_tcam_lkp_index]);
                            PRT_CELL_SET("%s", res_signal_str[ext_tcam_lkp_index]);
                            if ((ext_tcam_lkp_index + 1) <= nof_results)
                            {
                                PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
                                PRT_CELL_SET("%s", "");
                            }
                    ...
                    if (is_vt_stage)
                    {
                        ...
                        if (bare_metal_disabled && lif_enabler)
                        {
                            ...
                            if (local_in_lif != 0)
                            {
                                ...
                                dbal_physical_tables_e dbal_physical_table = DBAL_NOF_PHYSICAL_TABLES;
                                ...
                                SHR_IF_ERR_EXIT(diag_ikleap_vis_build_key_and_result_strings
                                                (unit, context_hw_val, 0, entry_handle->phy_entry.key,
                                                 ...
                                                 dbal_physical_table, lif_dbal_table, core_id, &nof_results,
                                                 &res_string));
