    ...
    prt_control_t * prt_ctr)
{
     ...
     * Bytes to strip
     */
    if (dnx_data_debug.general.etpp_eop_and_sop_dec_above_threshold_get(unit))
    {
        SHR_IF_ERR_EXIT_NO_MSG(dpp_dsig_read(unit, core, "ETPP", "BTC", NULL, "Bytes_to_Strip_EOP", &bts, 9));
        PRT_ROW_ADD(EKLEAP_PRT_STAGE_SEP);
        PRT_CELL_SET("Btc");
