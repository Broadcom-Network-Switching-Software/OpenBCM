 ...
 *   \param [in] qos_type_len - length of qos_type string
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** print the table */
    if (sal_strncaseindex(qos_type, "remark", qos_type_len, sizeof("remark"))
        || sal_strncaseindex(qos_type, "rm", qos_type_len, sizeof("rm")))
    {
        SHR_IF_ERR_EXIT(sh_dnx_pp_cos_ingress_qos_type_prepare(unit, core, DNX_QOS_DIAG_INGRESS_REMARK, prt_ctr));
    }

    if (sal_strncaseindex(qos_type, "phb", qos_type_len, sizeof("phb")))
    {
    ...
    uint32 *comp_strength)
{
    ...
    uint32 global_in_lif, local_in_lif;
    ...
    else if (!sal_strncasecmp(qos_type, "phb", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        if (dnx_data_qos.feature.feature_get(unit, dnx_data_qos_feature_seperate_tc_dp))
        {
            sig_qos_layer_index = "tc_layer_index";
            sig_qos_profile = "tc_mapping_profile";
            sig_port_strength = "tc_Strength";
        }
        else
        {
        ...
        else
        {
            ...
            if (local_in_lif != DNX_ALGO_GPM_LIF_INVALID)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                                (unit, local_in_lif, core, local_lif_payload.phy_table,
                                 &inlif_dbal_table_id, &inlif_dbal_result_type));

                SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, inlif_dbal_table_id, &entry_handle_id));
                SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, inlif_dbal_table_id, &core_mode));
...
sh_dnx_pp_cos_cmds(
    ...
    sh_sand_control_t * sand_control)
{
    int core_in, core_id, qos_type_len;
    ...
    if (!(sal_strncaseindex(pipe, "ing", SH_SAND_MAX_TOKEN_SIZE, sizeof("ing")) ||
          sal_strncaseindex(pipe, "egr", SH_SAND_MAX_TOKEN_SIZE, sizeof("egr")) ||
          sal_strncaseindex(pipe, "both", SH_SAND_MAX_TOKEN_SIZE, sizeof("both"))) ||
        !(sal_strncaseindex(print_content, "map", SH_SAND_MAX_TOKEN_SIZE, sizeof("map")) ||
          sal_strncaseindex(print_content, "prop", SH_SAND_MAX_TOKEN_SIZE, sizeof("prop"))
          || sal_strncaseindex(print_content, "all", SH_SAND_MAX_TOKEN_SIZE, sizeof("all"))))
    {
    ...
    qos_type = "ReMark|PHB";
    qos_type_len = sal_strnlen(qos_type, 11);
        ...
        || sal_strncaseindex(pipe, "both", SH_SAND_MAX_TOKEN_SIZE, sizeof("both")))
    {
        SH_DNX_CORES_ITER(unit, core_in, core_id)
        {
            ...
            if (sal_strncaseindex(print_content, "prop", SH_SAND_MAX_TOKEN_SIZE, sizeof("prop"))
                || sal_strncaseindex(print_content, "all", SH_SAND_MAX_TOKEN_SIZE, sizeof("all")))
            {
                SHR_IF_ERR_EXIT(sh_dnx_pp_cos_ingress_qos_propag_print(unit, core_id, sand_control));
            }

            if (sal_strncaseindex(print_content, "map", SH_SAND_MAX_TOKEN_SIZE, sizeof("map"))
                || sal_strncaseindex(print_content, "all", SH_SAND_MAX_TOKEN_SIZE, sizeof("all")))
            {
                SHR_IF_ERR_EXIT(sh_dnx_pp_cos_ingress_qos_info_print
                                (unit, core_id, qos_type, qos_type_len, sand_control));
            }
        }
    }

    /** Print egress qos mapping info if required.*/
    if (sal_strncaseindex(pipe, "egr", SH_SAND_MAX_TOKEN_SIZE, sizeof("egr"))
        || sal_strncaseindex(pipe, "both", SH_SAND_MAX_TOKEN_SIZE, sizeof("both")))
    {
        SH_DNX_CORES_ITER(unit, core_in, core_id)
        {
