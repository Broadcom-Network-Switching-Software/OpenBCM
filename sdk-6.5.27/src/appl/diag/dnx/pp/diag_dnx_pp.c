...
int
dnx_pp_is_signal_available(
    int unit,
    int core_id,
    dnx_pp_block_e pp_block)
{
    signal_output_t *sig_context_element = NULL;
    int rv = _SHR_E_NONE;
    int is_sig_available = TRUE;

    rv = dnx_debug_block_is_ready(unit, core_id, pp_block);
    if (rv != _SHR_E_NONE)
    {
        is_sig_available = FALSE;
    }

    /** We use "context" to indicate if the signal in the block is compelete.*/
    if (pp_block == DNX_PP_BLOCK_IRPP || pp_block == DNX_PP_BLOCK_ETPP)
    {
        rv = sand_signal_output_find(unit, core_id, 0, (pp_block == DNX_PP_BLOCK_IRPP ? "IRPP" : "ETPP"), "", "",
                                     "context", &sig_context_element);
    }
    else
    {
        rv = _SHR_E_NONE;
    ...
    sand_signal_output_free(sig_context_element);

    return is_sig_available;
}
...
#if 1
    {"Cross_Connect_Traverse", sh_dnx_pp_cc_cmds, NULL,                      dnx_pp_cc_options,           &dnx_pp_cc_man,              NULL, NULL, SH_CMD_CONDITIONAL, sh_dnx_pp_info_fec_fwd_rpf_not_separated},
#else
