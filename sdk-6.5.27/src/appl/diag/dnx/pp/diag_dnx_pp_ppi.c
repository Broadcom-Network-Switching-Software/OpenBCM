    ...
    sh_sand_control_t * sand_control)
{
    ...
    signal_output_t *protocols_list_element_head = NULL;

    /** tag structure */
    signal_output_t *tag_s_list_element = NULL;
    /** layer offsets */
    signal_output_t *offsets_list_element = NULL;
    signal_output_t *offsets_list_element_head = NULL;
...
#define MAX_PORT_SIGNALS        5
        ...
        DIAG_DNX_PP_BLOCK_SIGNAL_IS_AVAILABLE_CONT(core_id, DNX_PP_BLOCK_IRPP, !short_mode);
        ...
        SHR_CLI_EXIT_IF_ERR(sand_signal_output_find
                            (unit, core_id, 0, "IRPP", dnx_data_pp.debug.from_signal_str_get(unit)->ingress_layers, "",
                             "layer_qualifiers", &lq_list_element), "no signal record for layer_qualifiers\n");

        /** get the layer protocols */
        SHR_CLI_EXIT_IF_ERR(sand_signal_output_find
                            (unit, core_id, 0, "IRPP", dnx_data_pp.debug.from_signal_str_get(unit)->ingress_layers, "",
                             "layer_protocols", &protocols_list_element_head),
                            "no signal record for layer_protocols\n");

        /** get the layer offsets */
        SHR_CLI_EXIT_IF_ERR(sand_signal_output_find
                            (unit, core_id, 0, "IRPP", dnx_data_pp.debug.from_signal_str_get(unit)->ingress_layers, "",
                             "layer_offsets", &offsets_list_element_head), "no signal record for layer_offsets\n");

        /** get the header and its parsing */
        ...
        sal_strncpy(signal_key[i_sig].from, dnx_data_pp.debug.from_signal_str_get(unit)->in_port,
                    BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
        sal_strncpy(signal_key[i_sig].signal, "In_Port", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);
        ...
        sal_memset(&signal_key[i_sig], 0, sizeof(bcm_instru_vis_signal_key_t));
        sal_strncpy(signal_key[i_sig].block, "IRPP", BCM_INSTRU_VIS_BLOCK_NAME_MAX_SIZE - 1);
        sal_strncpy(signal_key[i_sig].from, dnx_data_pp.debug.from_signal_str_get(unit)->ptc,
                    BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
        ...
        protocols_list_element = utilex_rhlist_entry_get_first(protocols_list_element_head->field_list);

        /** set the layer offset list to first sub field (first layer offset) */
        offsets_list_element = utilex_rhlist_entry_get_first(offsets_list_element_head->field_list);
        ...
        if (!short_mode)
        {
            ...
            RHITERATOR(field_list_element, sig_list_element->field_list)
            {
                ...
                signal_output_t *next_next_field_list_element = NULL;
                char next_next_layer_name[RHNAME_MAX_SIZE] = "";
                ...
                if (next_field_list_element != NULL)
                {
                    sal_strncpy(next_layer_name, SH_DNX_PP_GET_SIG_EXP_NAME(next_field_list_element),
                                RHNAME_MAX_SIZE - 1);
                    next_next_field_list_element = utilex_rhlist_entry_get_next(next_field_list_element);
                    if (next_next_field_list_element != NULL)
                    {
                        sal_strncpy(next_next_layer_name, SH_DNX_PP_GET_SIG_EXP_NAME(next_next_field_list_element),
                                    RHNAME_MAX_SIZE - 1);
                    }
                }
                ...
                if (sub_layer_idx != -1)
                {
                    /** UDP is considered as a sublayer in VXLAN cases.*/
                    if (!sal_strnstr(name, "UDP", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
                        || sal_strnstr(next_layer_name, "VXLAN", sizeof(next_layer_name)))
                    {
                        sub_layer_flag = 1;
                ...
                sal_strncat_s(print_value, field_list_element->print_value, sizeof(print_value));
                ...
                if (next_field_list_element != NULL)
                {
                    ...
                    {
                        /** UDP is considered as a sublayer in VXLAN cases.*/
                        if (!sal_strnstr(next_layer_name, "UDP", sizeof(next_layer_name))
                            || sal_strnstr(next_next_layer_name, "VXLAN", sizeof(next_next_layer_name)))
                        {
                    ...
                    if (!system_header_flag)
                    {
                        ...
                        if (sal_strcasestr(top_layer_name, "ETH"))
                        {
                            ...
                            SHR_IF_ERR_EXIT(dbal_fields_enum_value_get
                                            (unit, DBAL_FIELD_INCOMING_TAG_STRUCTURE, tag_s_ing_value_hw,
                                             &tag_s_ing_value_hw_sw));

                            /** translate hw id to sw id */
                            dnx_port_tpid_class_hw_id_get_info(unit, TRUE, (int) tag_s_ing_value_hw_sw,
                                                               &tag_s_ing_value_sw);

                            PRT_INFO_ADD("%s hw value 0x%s (%s) / sw value (0x%d)", tag_s_ing_name,
                                         tag_s_ing_print_value, tag_s_ing_parse, tag_s_ing_value_sw);
                            PRT_INFO_SET_MODE(PRT_ROW_SEP_NONE);
                        ...
                        if (protocols_list_element->value[0] != 0)
                        {
                            ...
                            if (!skip_qualifier_layer_flag)
                            {
                                ...
                                offsets_list_element = utilex_rhlist_entry_get_next(offsets_list_element);
                                     ...
                                     (next_layer_name, skip_qualifier_layers[skip_ql_idx],
                                      sizeof(next_layer_name)) != -0)
                            {
                                /*
                    ...
                    else
                    {
                        ...
                        if ((sal_strncmp
                             (system_header_qualifiers_commands[system_hdr_idx], "",
                              SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != -0) && (!skip_qualifier_layer_flag))
                        {
                            SHR_IF_ERR_EXIT(sand_signal_output_find
                                            (unit, core_id, 0, "IRPP",
                                             dnx_data_pp.debug.from_signal_str_get(unit)->ingress_layers, "",
...
exit:
    ...
    sand_signal_output_free(protocols_list_element_head);
    sand_signal_output_free(tag_s_list_element);
    sand_signal_output_free(offsets_list_element_head);
