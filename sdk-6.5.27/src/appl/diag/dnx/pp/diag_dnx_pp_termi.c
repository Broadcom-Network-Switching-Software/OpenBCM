    ...
    diag_pp_termi_output_t * outputs)
{
    ...
    for (stage_idx = 1; stage_idx < DNX_PP_TERMI_INGRESS_STAGES - 1; stage_idx++)
    {
        ...
        {
            if (sal_strncmp
                (next_protocol_array[next_protocol_idx], "MPLS", sizeof(next_protocol_array[next_protocol_idx])) == 0)
            {
                /** This is actually an MPLS case, do nothing here but wait for the fwd_layer_index update with loop. */
                continue;
            }
    ...
    diag_pp_termi_output_t * outputs)
{
    ...
    SH_DNX_CORES_ITER(unit, inputs.core_id, core_id)
    {
        ...
        DIAG_DNX_PP_BLOCK_SIGNAL_IS_AVAILABLE_CONT(core_id, DNX_PP_BLOCK_IRPP, TRUE);
