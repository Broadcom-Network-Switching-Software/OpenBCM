 ...
 *   \param [in] is_fwd - forwarding or RPF indication
    ...
    char *packet_struct_str)
{
    ...
    uint32 attr_type =
        DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY | DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY | DBAL_ENTRY_ATTR_FLAG_HIT_GET;
    ...
    if (dnx_data_debug.general.fer_debug_signals_get(unit))
    {
        ...
        {
            sal_strncpy(signal_key[0].signal, "If_1_Key", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);
            sal_strncpy(signal_key[1].signal, "If_1_Key", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);
            sal_strncpy(signal_key[2].signal, "If_1_Key", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);
        }
        SHR_IF_ERR_EXIT(bcm_dnx_instru_vis_signal_get(unit, core_id, 0, nof_signals, signal_key, signal_result));
        ...
        for (hier_id = 0; hier_id < DNX_DATA_MAX_MDB_DIRECT_NOF_FEC_ECMP_HIERARCHIES; hier_id++)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_l3_fec_is_allocated
                            (unit, fec_sig_values[hier_id] * dnx_data_mdb.direct.nof_fecs_in_super_fec_get(unit),
                             &super_fec_is_allocated));
            ...
            if (super_fec_is_allocated)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_entry_attribute_get
                                (unit, entry_handle_id, DBAL_ENTRY_ATTR_HITBIT, &fec_hit[hier_id]));
                sal_snprintf(fec_hier_str[hier_id], RHNAME_MAX_SIZE, "Hierarchy %d: 0x%x (%s)\n", hier_id + 1,
                             fec_sig_values[hier_id], dbal_mdb_hitbit_to_string(unit,
                                                                                (dbal_physical_tables_e)
                                                                                FEC_TABLE_FROM_HIER_GET(hier_id),
                                                                                fec_hit[hier_id]));
            }
            else
            {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    const uint32 nof_ecmp = dnx_data_l3.ecmp.nof_ecmp_get(unit);
    ...
    uint32 selected_ecmp[DNX_DATA_MAX_MDB_DIRECT_NOF_FEC_ECMP_HIERARCHIES] = { nof_ecmp, nof_ecmp, nof_ecmp };
    ...
    SH_DNX_CORES_ITER(unit, core_in, core_id)
    {
        ...
        /** Add ECMP entries */
        if ((selected_ecmp[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_1] != nof_ecmp)
            || (selected_ecmp[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_2] != nof_ecmp)
            || (selected_ecmp[DBAL_ENUM_FVAL_HIERARCHY_LEVEL_HIERARCHY_LEVEL_3] != nof_ecmp))
        {
            ...
            for (hier_id = 0; hier_id < nof_hierarchies; hier_id++)
            {
                if (selected_ecmp[hier_id] == nof_ecmp)
                {
                    continue;
                }
