    ...
    sh_sand_control_t * sand_control)
{
    ...
    int is_end, egress_msb_bit, nof_ingress_bits, nof_egress_bits;
    char *buffer = NULL;
    ...
    nof_egress_bits = sal_strnlen(egress_bits_p, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    ...
    else
    {
        sal_snprintf(egress_profile_bits, DBAL_MAX_STRING_LENGTH, "%s%d %s %d%s", "Egress profile bits [",
                     ...
                     IN_LIF_PROFILE_PMF_RESERVED_START_BIT - nof_egress_bits, "]");
    }
        ...
        else if (IN_LIF_PROFILE_EGRESS_RESERVED_START_BIT > 0)
        {
            PRT_INFO_ADD
                ("Profile bits separation: %s, %s, Ingress profile bits = [%d : %d]",
                 acl_profile_bits, egress_profile_bits, IN_LIF_PROFILE_PMF_RESERVED_START_BIT - nof_egress_bits - 1, 0);
        }
    }
    PRT_COMMITX;
    if (IN_LIF_PROFILE_EGRESS_RESERVED_NOF_BITS != 0)
    ...
    while (!is_end)
    {
        ...
        if ((saved_index < in_lif_profile) || ((default_entry == 0) && (in_lif_profile == 0)))
        {
            ...
            sh_dnx_pp_occ_masked_bits_print(nof_egress_bits,
                                            nof_egress_bits, prefix_value_binary_buffer,
    ...
    sh_sand_control_t * sand_control)
{
    uint32 entry_handle_id, rpf_mode, in_lif_profile, icmpv4_trap_handling_profile, entry_handle_id_vsi_profile,
        ...
        0, lif_same_interface_mode = 0, acl_bits_value = 0, in_lif_profile_ingress_bits = 0, msb_bit =
        0, mapped_in_lif_profile = 0;
    ...
    /** rpf_buffer_ipv4 holds the rpf mode value when in unified RPF mode config and the IPV4 rpf mode when using separate config. */
    char *rpf_buffer_ipv4 = NULL;
    char *rpf_buffer_ipv6 = NULL;
    char *buffer = NULL;
    char *acl_bits_p = NULL;
    char *ingress_bits_p = NULL;
    char *egress_bits_p = NULL;
    ...
    SHR_ALLOC(native_indexed_mode_bits, sizeof(*native_indexed_mode_bits) * (NATIVE_INDEXED_MODE_NOF_BITS + 1),
              "native_indexed_mode_bits", "%s%s%s", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO(rpf_buffer_ipv4, sizeof(*rpf_buffer_ipv4) * (DBAL_MAX_PRINTABLE_BUFFER_SIZE + 1),
                       "rpf_buffer_ipv4", "%s%s%s", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO(rpf_buffer_ipv6, sizeof(*rpf_buffer_ipv6) * (DBAL_MAX_PRINTABLE_BUFFER_SIZE + 1),
                       "rpf_buffer_ipv6", "%s%s%s", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO(buffer, sizeof(*buffer) * (DBAL_MAX_PRINTABLE_BUFFER_SIZE + 1), "buffer", "%s%s%s", EMPTY, EMPTY,
                       EMPTY);
    ...
    {
        PRT_TITLE_SET("ACL and Ingress bits");
    }
    else
    {
    ...
    {
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Field class CS ID");
    }
    ...
    {
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "RPF mode IPv4");
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "RPF mode IPv6");
    }
    else
    {
    ...
    while (!is_end)
    {
        ...
        if ((saved_index < in_lif_profile) || ((default_entry == 0) && (in_lif_profile == 0)))
        {
            ...
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                (unit, entry_handle_id, DBAL_FIELD_RPF_MODE_IPV4, INST_SINGLE, &rpf_mode));
                SHR_IF_ERR_EXIT(dbal_table_field32_printable_string_get
                                (unit, DBAL_TABLE_IN_LIF_PROFILE_TABLE, DBAL_FIELD_RPF_MODE_IPV4, 0, rpf_mode, NULL, 0,
                                 FALSE, rpf_buffer_ipv4));
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                (unit, entry_handle_id, DBAL_FIELD_RPF_MODE_IPV6, INST_SINGLE, &rpf_mode));
                SHR_IF_ERR_EXIT(dbal_table_field32_printable_string_get
                                (unit, DBAL_TABLE_IN_LIF_PROFILE_TABLE, DBAL_FIELD_RPF_MODE_IPV6, 0, rpf_mode, NULL, 0,
                                 FALSE, rpf_buffer_ipv6));
            }
            else
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
            ...
            if ((!dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_use_mapped_in_lif_profile)) &&
                (dnx_data_lif.
                 in_lif.feature_get(unit, dnx_data_lif_in_lif_use_in_lif_profile_range_allocation_per_pmf_profiles)))
            {
                if ((routing_enablers_profile == 0)
                    && (rpf_mode == 0) && (l2tp_trap_handling_profile == 0) && (icmpv4_trap_handling_profile == 0)
                    && (icmpv6_trap_handling_profile == 0) && (arp_trap_handling_profile == 0)
                    && (igmp_trap_handling_profile == 0) && (dhcp_trap_handling_profile == 0)
                    && (non_auth_8021x_trap_handling_profile == 0) && (cs_in_lif_vtt_profile == 0)
                    && (cs_in_lif_fwd_profile == 0) && (da_not_found_destination_profile == 0)
                    && (oam_default_profile == 0))
                {
                    default_entry = 1;
                }
            ...
            if (default_entry != 1)
            {
                ...
                if (nof_ingress_bits == 0)
                {
                    if (dnx_data_lif.in_lif.feature_get(unit,
                ...
                PRT_CELL_SET("%s", rpf_buffer_ipv4);
                if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_separate_ipv4_ipv6_urpf_mode_config))
                {
                    PRT_CELL_SET("%s", rpf_buffer_ipv6);
                }
                ...
                {
                    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                    (unit, entry_handle_id, DBAL_FIELD_MAPPED_IN_LIF_PROFILE, INST_SINGLE,
                                     &mapped_in_lif_profile));
                    dbal_entry_key_field32_set(unit, entry_handle_id_mapped_in_lif_profile,
                                               DBAL_FIELD_MAPPED_IN_LIF_PROFILE, mapped_in_lif_profile);
                    ...
                    PRT_CELL_SET("%d", in_lif_orientation);
                    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                    (unit, entry_handle_id_mapped_in_lif_profile, DBAL_FIELD_INNER_DP_PROFILE,
                                     INST_SINGLE, &ive_outer_qos_dp_profile));
                    PRT_CELL_SET("%d", ive_outer_qos_dp_profile);
                    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                    (unit, entry_handle_id_mapped_in_lif_profile, DBAL_FIELD_OUTER_DP_PROFILE,
                                     INST_SINGLE, &ive_inner_qos_dp_profile));
...
exit:
    ...
    SHR_FREE(native_indexed_mode_bits);
    SHR_FREE(buffer);
    SHR_FREE(rpf_buffer_ipv4);
    SHR_FREE(rpf_buffer_ipv6);
    SHR_FREE(prefix_value_binary_buffer);
    SHR_FREE(prefix_value_binary_buffer_full);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int is_end, saved_profile, default_entry, ingress_bits_mask, nof_ingress_bits, nof_egress_bits, profile_bits_mask;
    int eth_rif_profile_used_bits_mask = 0, egress_reserved_nof_bits = 0, eth_rif_profile_egress_reserved_start_bit = 0;
    char *prefix_value_binary_buffer = NULL;
    char *prefix_value_binary_buffer_full = NULL;
    uint32 rpf_mode, icmpv4_trap_handling_profile, icmpv6_trap_handling_profile, arp_trap_handling_profile,
        ...
        l2tp_trap_handling_profile, acl_bits_value = 0;
    char *acl_bits_p = NULL;
    char *ingress_bits_p = NULL;
    char *egress_bits_p = NULL;
    char *buffer = NULL;
    char *acl_profile_bits = NULL, *acl_value_binary_buffer = NULL, *acl_value_binary_buffer_full = NULL;
    in_lif_template_data_t in_lif_profile_template_data;
    ...
    SHR_ALLOC(acl_profile_bits, sizeof(*acl_profile_bits) * (DBAL_MAX_STRING_LENGTH + 1), "acl_profile_bits", "%s%s%s",
              EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO(acl_value_binary_buffer,
                       sizeof(*acl_value_binary_buffer) * (ETH_RIF_PROFILE_PMF_RESERVED_NOF_BITS + 1),
                       "acl_value_binary_buffer", "%s%s%s", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO(acl_value_binary_buffer_full,
                       sizeof(*acl_value_binary_buffer_full) * (ETH_RIF_PROFILE_PMF_RESERVED_NOF_BITS + 1),
                       "acl_value_binary_buffer_full", "%s%s%s", EMPTY, EMPTY, EMPTY);
    is_end = 0;
    ...
    {
        profile_bits_mask = (1 << NOF_ETH_RIF_PROFILE_BITS) - 1;
    }
    else
    {
        profile_bits_mask = ingress_bits_mask;
    }
    SHR_ALLOC(ingress_bits_p, sizeof(*ingress_bits_p) * (nof_ingress_bits + 1),
              "ingress_bits_p", "%s%s%s", EMPTY, EMPTY, EMPTY);
    SHR_ALLOC_SET_ZERO(prefix_value_binary_buffer, sizeof(*prefix_value_binary_buffer) * (nof_ingress_bits + 1),
    ...
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_in_lif_profile_mc_bridge_fallback_support))
    {
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "MC bridge fallback");
    }
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_separate_ipv4_ipv6_urpf_mode_config))
    {
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "RPF mode IPv4");
    ...
    while (!is_end)
    {
        ...
        if ((((saved_profile & profile_bits_mask) < (vsi_profile & profile_bits_mask))
                ...
                && (vsi_profile == CALCULATE_VSI_PROFILE_FROM_ETH_RIF_PROFILE(VSI_MSB_PREFIX, 1))))
        {
                ...
                && (IN_LIF_PROFILE_PMF_RESERVED_NOF_BITS > 0))
            {
                ...
                /** Get the value of the ACL bits */
                utilex_bitstream_get_field(&vsi_profile, ETH_RIF_PROFILE_PMF_RESERVED_START_BIT,
                                           ETH_RIF_PROFILE_PMF_RESERVED_NOF_BITS, &acl_bits_value);
                /** Convert acl_bits_value to binary value */
                sal_itoa(acl_value_binary_buffer, acl_bits_value, 2, 0, 0);
                sh_dnx_pp_occ_masked_bits_print(ETH_RIF_PROFILE_PMF_RESERVED_NOF_BITS,
                                                ETH_RIF_PROFILE_PMF_RESERVED_NOF_BITS, acl_value_binary_buffer,
                                                acl_value_binary_buffer_full);
            }
            ...
            if (dnx_data_lif.
                in_lif.feature_get(unit, dnx_data_lif_in_lif_use_eth_rif_profile_range_allocation_per_pmf_profiles)
                && (ETH_RIF_PROFILE_PMF_RESERVED_NOF_BITS > 0))
            {
                PRT_CELL_SET("%s%s%s", acl_value_binary_buffer_full, egress_bits_p, prefix_value_binary_buffer_full);
            }
            else
            {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_in_lif_profile_template_data_get
                                (unit, vsi_profile & eth_rif_profile_used_bits_mask, ETH_RIF,
                                 &in_lif_profile_template_data));
                PRT_CELL_SET("%d", in_lif_profile_template_data.field_class_id);
            }
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                            (unit, entry_handle_id_vsi_profile, DBAL_FIELD_ROUTING_ENABLERS_PROFILE, INST_SINGLE,
            ...
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                (unit, entry_handle_id_lif_profile, DBAL_FIELD_RPF_MODE_IPV4, INST_SINGLE, &rpf_mode));
                SHR_IF_ERR_EXIT(dbal_table_field32_printable_string_get
                                (unit, DBAL_TABLE_IN_LIF_PROFILE_TABLE, DBAL_FIELD_RPF_MODE_IPV4, 0, rpf_mode, NULL, 0,
                                 FALSE, buffer));
                PRT_CELL_SET("%s", buffer);
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                (unit, entry_handle_id_lif_profile, DBAL_FIELD_RPF_MODE_IPV6, INST_SINGLE, &rpf_mode));
                SHR_IF_ERR_EXIT(dbal_table_field32_printable_string_get
                                (unit, DBAL_TABLE_IN_LIF_PROFILE_TABLE, DBAL_FIELD_RPF_MODE_IPV6, 0, rpf_mode, NULL, 0,
                                 FALSE, buffer));
                PRT_CELL_SET("%s", buffer);
...
/**
 * \brief - Return no_support in case ETPP and ERPP are not supported
 *
 */
static shr_error_e
sh_cmd_is_etpp_and_erpp_supported(
    int unit,
    rhlist_t * test_list)
{
    SHR_FUNC_INIT_VARS(unit);
...
sh_sand_cmd_t sh_dnx_pp_occ_outlif_profile_sub_cmd[] = {
    ...
    {"Etpp", sh_dnx_pp_occ_etpp_profile_print_cmd, NULL, NULL, &sh_dnx_pp_occ_etpp_profile_sub_man,
     NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_etpp_and_erpp_supported},
