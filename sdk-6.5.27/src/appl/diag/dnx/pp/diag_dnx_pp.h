...
#define DIAG_DNX_PP_BLOCK_SIGNAL_IS_AVAILABLE_CONT(core_mcr, block_mcr, log_mcr)                              \
    if (dnx_pp_is_signal_available(unit, core_mcr, block_mcr) == FALSE)                                        \
    {                                                                                                           \
        if(log_mcr)                                                                                             \
        {                                                                                                       \
            LOG_CLI(("The command depends on signals on %s(%s) for core %d which are not unabridged!\n",                   \
...
/**
 * \brief
 *    A function helps to check if the signal in the block is complete for diagnostics commands.
 * \param [in] unit - unit ID
 * \param [in] core_id - relevant core_id
 * \param [in] pp_block - block id
 * \return
 *   \retval Zero if no error was detected
 *   \retval Negative if error was detected. See \ref shr_error_e
 */
int dnx_pp_is_signal_available(
    int unit,
    int core_id,
