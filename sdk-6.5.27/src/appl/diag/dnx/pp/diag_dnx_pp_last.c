...
/** reads the FWD layer lookup terms signal, and also extra lookup terms (PP_LAST_FODO, Global LIF) and copies to a structure */
...
/** reads the FWD layer lookup terms signal regarding the RPF lookup, and also extra lookup terms (PP_LAST_FODO) and copies to a structure */
#define __PP_LAST_GET_FWD_LAYER_RPF_LOOKUP_TERMS(__fwd_layer)                                                                                                                           \
    current_rpf_layer_lookup_terms.nof_lookup_terms_signals = pp_last_rpf_layers_lookup_terms[__fwd_layer].nof_lookup_terms_signals;                                                \
    current_rpf_layer_lookup_terms.nof_lookup_terms_extra = pp_last_rpf_layers_lookup_terms[__fwd_layer].nof_lookup_terms_extra;                                                    \
                                                                                                                                                                            \
    for (i_sig_fwd_lookup_term = 0; i_sig_fwd_lookup_term < pp_last_rpf_layers_lookup_terms[__fwd_layer].nof_lookup_terms_signals; i_sig_fwd_lookup_term++)                         \
    {                                                                                                                                                                       \
        if (pkt_header_sig && pkt_header_sig->field_list)                                                                                                                                              \
        {                                                                                                                                                                            \
            field_header_sig = utilex_rhlist_entry_get_by_name(pkt_header_sig->field_list, pp_last_rpf_layers_lookup_terms[__fwd_layer].lookup_term_signals[i_sig_fwd_lookup_term]); \
        }                                                                                                                                                                            \
        else                                                                                                                                                                         \
        {                                                                                                                                                                            \
            break; \
        ...
        if (field_header_sig)                                                                                                                                               \
        {                                                                                                                                                                   \
            sal_strncpy_s(current_rpf_layer_lookup_terms.lookup_term_signals[i_sig_fwd_lookup_term], field_header_sig->print_value, RHSTRING_MAX_SIZE-1);                   \
        }                                                                                                                                                                   \
        else                                                                                                                                                                \
        {                                                                                                                                                                   \
            sal_strncpy_s(current_rpf_layer_lookup_terms.lookup_term_signals[i_sig_fwd_lookup_term], "Not Found", RHSTRING_MAX_SIZE-1);                                     \
        }                                                                                                                                                                   \
    }                                                                                                                                                                       \
                                                                                                                                                                            \
    for (i_extra_fwd_lookup_term = 0; i_extra_fwd_lookup_term < pp_last_rpf_layers_lookup_terms[__fwd_layer].nof_lookup_terms_extra; i_extra_fwd_lookup_term++)                     \
    {                                                                                                                                                                       \
        if (pp_last_rpf_layers_lookup_terms[__fwd_layer].extra_lookup_term[i_extra_fwd_lookup_term] == PP_LAST_FODO)                                                                        \
        {                                                                                                                                                                   \
            sal_strncpy_s(current_rpf_layer_lookup_terms.extra_lookup_term_str[i_extra_fwd_lookup_term], fodo_str, RHSTRING_MAX_SIZE-1);                                    \
        }                                                                                                                                                                  \
    }
...
typedef enum
{
    ...
    PP_LAST_RPF_LOOKUP_TABLE_ID,
    PP_LAST_FEC_FWD_TABLE_ID,
    PP_LAST_FEC_RPF_TABLE_ID,
...
/** Init RPF lookup terms for the forward layer */
pp_last_layer_lookup_terms_t pp_last_rpf_layers_lookup_terms[PP_LAST_NOF_FWD_LAYERS] = {
    /** ETH FWD LAYER lookup terms - PP_LAST_FODO (VSI), SA, DA */
    {
     {"SA"},
     1,
     {PP_LAST_FODO},
     1},
    /** IPv4 FWD LAYER lookup terms - PP_LAST_FODO (VRF), SIP */
    {
     {"SIP"},
     1,
     {PP_LAST_FODO},
     1},
    /** IPv6 FWD LAYER lookup terms - PP_LAST_FODO (VRF), SIP */
    {
     {"SIP"},
     1,
     {PP_LAST_FODO},
    ...
    int core)
{
    ...
    PRT_TITLE_SET_PL(PP_LAST_RPF_LOOKUP_TABLE_ID, "<RPF Lookup>");
    PRT_COLUMN_ADD_FLEX_PL(PP_LAST_RPF_LOOKUP_TABLE_ID, PRT_FLEX_ASCII, "Key Components");
    PRT_COLUMN_ADD_FLEX_PL(PP_LAST_RPF_LOOKUP_TABLE_ID, PRT_FLEX_ASCII, "Result Destination");
    PRT_ROW_ADD_PL(PP_LAST_RPF_LOOKUP_TABLE_ID, PRT_ROW_SEP_NONE);

    PRT_TITLE_SET_PL(PP_LAST_FEC_FWD_TABLE_ID, "<FEC FWD>");
    PRT_COLUMN_ADD_FLEX_PL(PP_LAST_FEC_FWD_TABLE_ID, PRT_FLEX_ASCII, "Destination");
    PRT_COLUMN_ADD_FLEX_PL(PP_LAST_FEC_FWD_TABLE_ID, PRT_FLEX_ASCII, "Outlifs");
    PRT_COLUMN_ADD_FLEX_PL(PP_LAST_FEC_FWD_TABLE_ID, PRT_FLEX_ASCII, "Hiearchies Hit");
    PRT_ROW_ADD_PL(PP_LAST_FEC_FWD_TABLE_ID, PRT_ROW_SEP_NONE);

    PRT_TITLE_SET_PL(PP_LAST_FEC_RPF_TABLE_ID, "<FEC RPF>");
    PRT_COLUMN_ADD_FLEX_PL(PP_LAST_FEC_RPF_TABLE_ID, PRT_FLEX_ASCII, "Destination");
    PRT_COLUMN_ADD_FLEX_PL(PP_LAST_FEC_RPF_TABLE_ID, PRT_FLEX_ASCII, "Outlifs");
    PRT_COLUMN_ADD_FLEX_PL(PP_LAST_FEC_RPF_TABLE_ID, PRT_FLEX_ASCII, "Hiearchies Hit");
    PRT_ROW_ADD_PL(PP_LAST_FEC_RPF_TABLE_ID, PRT_ROW_SEP_NONE);
    ...
    int core_id)
{
    ...
    pp_last_current_layer_lookup_terms_t current_rpf_layer_lookup_terms = {
        {"N/A"},
        0,
        {"N/A"},
        0
    ...
    RHITERATOR(protocols_sig, protocols_sig_head->field_list)
    {
        ...
        if (i_sig == fwd_layer_index)
        {
            ...
            if (sal_strnstr(fwd_layer_string, "ETH", sizeof(fwd_layer_string)))
            {
                ...
                __PP_LAST_GET_FWD_LAYER_RPF_LOOKUP_TERMS(PP_LAST_ETH_FWD_LAYER);
            ...
            else if (sal_strnstr(fwd_layer_string, "IPV4", sizeof(fwd_layer_string)))
            {
                ...
                __PP_LAST_GET_FWD_LAYER_RPF_LOOKUP_TERMS(PP_LAST_IPV4_FWD_LAYER);
            ...
            else if (sal_strnstr(fwd_layer_string, "IPV6", sizeof(fwd_layer_string)))
            {
                ...
                __PP_LAST_GET_FWD_LAYER_RPF_LOOKUP_TERMS(PP_LAST_IPV6_FWD_LAYER);
    ...
    /*************************
     * fill RPF Lookup table *
     *************************/
    if (fwd_layer_type != PP_LAST_MPLS_FWD_LAYER)
    {
        /*
         * print the RPF Layer lookup terms
         */

        val_string[0] = '\0';
        for (i_sig_fwd_lookup_term = 0; i_sig_fwd_lookup_term < current_rpf_layer_lookup_terms.nof_lookup_terms_signals;
             i_sig_fwd_lookup_term++)
        {
            sal_strncat_s(val_string,
                          pp_last_rpf_layers_lookup_terms[fwd_layer_type].lookup_term_signals[i_sig_fwd_lookup_term],
                          sizeof(val_string));
            sal_strncat_s(val_string, " = ", sizeof(val_string));
            sal_strncat_s(val_string, current_rpf_layer_lookup_terms.lookup_term_signals[i_sig_fwd_lookup_term],
                          sizeof(val_string));
            sal_strncat_s(val_string, "\n", sizeof(val_string));
        }

        for (i_extra_fwd_lookup_term = 0;
             i_extra_fwd_lookup_term < current_rpf_layer_lookup_terms.nof_lookup_terms_extra; i_extra_fwd_lookup_term++)
        {
            sal_strncat_s(val_string, current_rpf_layer_lookup_terms.extra_lookup_term_str[i_extra_fwd_lookup_term],
                          sizeof(val_string));
            if (i_extra_fwd_lookup_term < current_rpf_layer_lookup_terms.nof_lookup_terms_extra - 1)
            {
                sal_strncat_s(val_string, "\n", sizeof(val_string));
            }
        }

        PRT_CELL_SET_PL(PP_LAST_RPF_LOOKUP_TABLE_ID, "%s", val_string);

        /*
         * print the RPF destination
         */
        ...
        sal_strncpy_s(signal_key[i_sig].block, "IRPP", BCM_INSTRU_VIS_BLOCK_NAME_MAX_SIZE - 1);
        sal_strncpy_s(signal_key[i_sig].from, "FWD2", BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
        sal_strncpy_s(signal_key[i_sig].signal, "rpf_dst.value", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);

        i_sig++;
        sal_memset(&signal_key[i_sig], 0, sizeof(bcm_instru_vis_signal_key_t));
        sal_strncpy_s(signal_key[i_sig].block, "IRPP", BCM_INSTRU_VIS_BLOCK_NAME_MAX_SIZE - 1);
        sal_strncpy_s(signal_key[i_sig].from, "FWD2", BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
        sal_strncpy_s(signal_key[i_sig].signal, "rpf_dst.type", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);
        ...
        if (signal_result[0].status == BCM_E_NONE)
        {
            PRT_CELL_SET_PL(PP_LAST_RPF_LOOKUP_TABLE_ID, "0x%x (%s)", signal_result[0].value[0],
                            signal_result[1].status == BCM_E_NONE ? signal_result[1].print_value : "N/A");
        }
        else
        {
            PRT_CELL_SET_PL(PP_LAST_RPF_LOOKUP_TABLE_ID, "Not Found");
        }

        pp_last_occupied_tables[PP_LAST_RPF_LOOKUP_TABLE_ID] = TRUE;
    }
    ...
    if (sal_strncasecmp(signal_result[0].print_value, "FEC", sizeof(signal_result[0].print_value)) == 0)
    {
        ...
        {
            PRT_CELL_SET_PL(PP_LAST_FEC_FWD_TABLE_ID, "0x%x (%s)", signal_result[1].value[0],
                            signal_result[2].status == BCM_E_NONE ? signal_result[2].print_value : "N/A");
        }
        else
        {
            PRT_CELL_SET_PL(PP_LAST_FEC_FWD_TABLE_ID, "Not Found");
        }
        ...
        PRT_CELL_SET_PL(PP_LAST_FEC_FWD_TABLE_ID, "%s", fec_outlifs_values_str);
        /*
         * Hierachies Hit 
         */
        sal_memset(packet_structure_str, 0, sizeof(packet_structure_str));
        SHR_IF_ERR_EXIT(dnx_pp_vis_fer_info(unit, core_id, 1, packet_structure_str));
        PRT_CELL_SET_PL(PP_LAST_FEC_FWD_TABLE_ID, "%s", packet_structure_str);

        pp_last_occupied_tables[PP_LAST_FEC_FWD_TABLE_ID] = TRUE;
    ...
    sal_strncpy_s(signal_key[i_sig].block, "IRPP", BCM_INSTRU_VIS_BLOCK_NAME_MAX_SIZE - 1);
    sal_strncpy_s(signal_key[i_sig].from, "IPMF1", BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
    sal_strncpy_s(signal_key[i_sig].signal, "RPF_Out_LIF", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);

    i_sig++;
    sal_memset(&signal_key[i_sig], 0, sizeof(bcm_instru_vis_signal_key_t));
    sal_strncpy_s(signal_key[i_sig].block, "IRPP", BCM_INSTRU_VIS_BLOCK_NAME_MAX_SIZE - 1);
    sal_strncpy_s(signal_key[i_sig].from, "FWD2", BCM_INSTRU_VIS_STAGE_NAME_MAX_SIZE - 1);
    sal_strncpy_s(signal_key[i_sig].signal, "RPF_Dst_Valid", BCM_INSTRU_VIS_SIGNAL_NAME_MAX_SIZE - 1);

    nof_signals = i_sig + 1;
    SHR_IF_ERR_EXIT(bcm_dnx_instru_vis_signal_get(unit, core_id, 0, nof_signals, signal_key, signal_result));

    /** allow FEC table print only if PMF1 destination was FEC and RPF destination is valid */
    if ((sal_strncasecmp(signal_result[0].print_value, "FEC", sizeof(signal_result[0].print_value)) == 0)
        && (sal_strncasecmp(signal_result[4].print_value, "01", sizeof(signal_result[4].print_value)) == 0))
    {
        char fec_outlifs_values_str[DSIG_MAX_SIZE_STR] = " ";
        /** build the FEC outlifs values string */
        int result_str_length = sal_strnlen(fec_outlifs_values_str, sizeof(fec_outlifs_values_str));
        ...
        if (signal_result[1].status == BCM_E_NONE)
        {
            PRT_CELL_SET_PL(PP_LAST_FEC_RPF_TABLE_ID, "0x%x (%s)", signal_result[1].value[0],
                            signal_result[2].status == BCM_E_NONE ? signal_result[2].print_value : "N/A");
        }
        else
        {
            PRT_CELL_SET_PL(PP_LAST_FEC_RPF_TABLE_ID, "Not Found");
        }

        fec_outlifs_values_str[result_str_length] = '0';
        fec_outlifs_values_str[result_str_length + 1] = '\0';
        sal_strncat_s(fec_outlifs_values_str, ": ", sizeof(fec_outlifs_values_str));

        if (signal_result[3].status == BCM_E_NONE)
        {
            sal_strncat_s(fec_outlifs_values_str, signal_result[3].print_value, sizeof(fec_outlifs_values_str));
        }
        else
        {
            sal_strncat_s(fec_outlifs_values_str, "N/A", sizeof(fec_outlifs_values_str));
        ...
        PRT_CELL_SET_PL(PP_LAST_FEC_RPF_TABLE_ID, "%s", fec_outlifs_values_str);
        /*
         * Hiearchies Hit 
         */
        sal_memset(packet_structure_str, 0, sizeof(packet_structure_str));
        SHR_IF_ERR_EXIT(dnx_pp_vis_fer_info(unit, core_id, 0, packet_structure_str));
        PRT_CELL_SET_PL(PP_LAST_FEC_RPF_TABLE_ID, "%s", packet_structure_str);
        pp_last_occupied_tables[PP_LAST_FEC_RPF_TABLE_ID] = TRUE;
    } /** of if IPMF1 destination is FEC and RPF destination is valid */
