    ...
    for (bcm_qual = 0; bcm_qual < bcmFieldQualifyCount; bcm_qual++)
    {
        ...
        if (cs_qual_map_p->nof == 1)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_dnx_to_dbal
                            (unit, stage, cs_dnx_qual, &is_supported, &dbal_field_id, &is_special, &special_size,
                             &special_offset));
        ...
        if (is_special && is_lr_qual)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Qual \"%s\" stage \"%s\", both special and LR.\n",
                         dnx_field_bcm_qual_text(unit, cs_bcm_qual), dnx_field_stage_text(unit, stage));
        }

        SHR_IF_ERR_EXIT(dbal_tables_field_size_get(unit, dnx_field_map_stage_info[stage].cs_table_id, dbal_field_id,
        ...
        else
        {
            if (is_special)
            {
                if ((special_size <= 0) || (special_offset < 0))
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Qual \"%s\" stage \"%s\", special but offset %d size %d.\n",
                                 dnx_field_bcm_qual_text(unit, cs_bcm_qual), dnx_field_stage_text(unit, stage),
                                 special_offset, special_size);
                }
                PRT_CELL_SET("%d (%d:%d)", dnx_cs_qual_size, special_offset, (special_offset + special_size - 1));
            }
            else
            {
        ...
        if (cs_qual_map_p->nof > 1)
        {
            ...
            while (cs_qual_iter < cs_qual_map_p->nof)
            {
                for (cs_qual_iter++; cs_qual_iter < cs_qual_map_p->nof; cs_qual_iter++)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_field_map_cs_qual_dnx_to_dbal
                                    (unit, stage, cs_dnx_qual, &is_supported, &dbal_field_id, &is_special,
                ...
                SHR_IF_ERR_EXIT(dbal_tables_field_size_get(unit, dnx_field_map_stage_info[stage].cs_table_id,
                                                           dbal_field_id, TRUE, 0, 0, &dnx_cs_qual_size));
                ...
                else
                {
                    if (is_special)
                    {
                        ...
                        PRT_CELL_SET("%d (%d:%d)", dnx_cs_qual_size, special_offset,
                                     (special_offset + special_size - 1));
                    }
                    else
                    {
                        PRT_CELL_SET("%d", dnx_cs_qual_size);
