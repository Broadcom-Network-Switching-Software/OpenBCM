...
static shr_error_e
diag_dnx_field_attach_info_example_generate(
    int unit,
    int flags,
    ...
    rhlist_t * examples_list)
{
    ...
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, "group=70 context=15"), "");
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, "group=72 context=12"), "");
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, "group=73 context=12"), "");
    if (dnx_data_field.state_table.supported_get(unit))
    {
        SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, "group=74 context=11"), "");
    }
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, "group=75 context=0"), "");
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, "group=76 context=12"), "");
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, "group=77 context=15"), "");
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, "group=78 context=15"), "");
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, "group=79 context=12"), "");
...
static sh_sand_man_t Field_attach_info_man = {
    ...
    .example_generate_cb = diag_dnx_field_attach_info_example_generate,
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** Prepare and print FFCs and qualifiers which are using them. */
    SHR_IF_ERR_EXIT(dnx_field_kbp_opcode_to_contexts(unit, opcode_id, &fwd_nof_contexts, fwd_contexts, acl_contexts));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n"
                         "The given context %d is not within the valid range 0-%d, for stage \"%s\".\r\n\n",
                         context_id,
                         dnx_data_field.stage.stage_info_get(unit, fg_info->group_basic_info.field_stage)->nof_contexts,
                         dnx_field_stage_text(unit, fg_info->group_basic_info.field_stage));
        }
    }
    ...
    if (fg_info->group_basic_info.field_stage == DNX_FIELD_STAGE_EXTERNAL)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
                         "\r\n" "Attach Info, for External stage, will be presented only after device lock.\r\n\n");
        }
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n" "The given Apptype %d does not exist.\r\n\n", app_type);
        }
    ...
    else
    {
        ...
        if (!is_allocated)
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n" "The given context Id %d is not allocated.\r\n\n", context_id);
        }
    }
    ...
    if (!is_attached)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n"
                         "The given FG Id %d is not attached to Apptype %d (opcode ID %d).\r\n\n",
                         fg_id, app_type, context_id);
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n"
                         "The given FG Id %d is not attached to context %d.\r\n\n", fg_id, context_id);
        }
    }
