 ...
 */
shr_error_e
diag_dnx_field_ace_is_available(
    int unit,
    rhlist_t * test_list)
{
    SHR_FUNC_INIT_VARS(unit);

    if (FALSE == dnx_data_field.ace.supported_get(unit))
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (!is_ace_allocated)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "\r\n" "ACE Format ID %d is not allocated\r\n\n", ace_format_id);
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    rv = dnx_field_ace_entry_get(unit, entry_handle, &entry_payload, &ace_format_id_retrieved);
    ...
    if (input_ace_format_id == -1)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "\r\n" "ACE Entry 0x%X belongs to an ACE Format ID %d that doesn't exist.\r\n\n", entry_handle,
                         ace_format_id);
        }
    }
    else
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n" "ACE entry 0x%X is a member of ACE Format ID %d and not %d.\r\n\n",
                         entry_handle, ace_format_id_retrieved, ace_format_id);
        }
    }
