    ...
    sh_sand_control_t * sand_control)
{
    ...
    switch (fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_DIRECT_TABLE_MDB:
        {
            ...
            SHR_IF_ERR_EXIT(diag_dnx_field_utils_group_type_conversion(unit, fg_type, converted_fg_type));
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n"
                         "Please use DBAL diagnostics instead to query on entries of Field groups of type %s, DBAL table name: %s\r\n\n",
                         converted_fg_type, fg_name);
            break;
        }
        default:
        {
            ...
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n"
                         "Field Group Type %s is not supported by Entry Info diagnostic.\r\n\n", converted_fg_type);
            break;
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    /** Get information about the given Entry. */
    SHR_IF_ERR_EXIT(dnx_field_entry_t_init(unit, &entry_info));
    rv = dnx_field_entry_external_tcam_get(unit, fg_id, entry_id, &is_entry_cached, &entry_info);
    if (rv == _SHR_E_NOT_FOUND)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "\r\n" "The given entry %d (0x%08X) is invalid.\r\n", entry_id, entry_id);
    }
    else if (rv == _SHR_E_CONFIG)
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    switch (fg_info->group_basic_info.fg_type)
    {
        ...
        case DNX_FIELD_GROUP_TYPE_TCAM:
        {
            if (entry_id == DNX_FIELD_ENTRY_ACCESS_ID_INVALID)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n" "Presence of option:\"ENTry\" is requested.\r\n\n");
            }
            /*
            ...
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n"
                             "The given Entry Id %d (0x%08X) doesn't belong to the given FG Id %d.\r\n\n",
                             entry_id, entry_id, fg_id);
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    switch (fg_type)
    {
        ...
        default:
        {
            ...
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n" "Field Group Type %s is not supported by Entry Dump diagnostic.\r\n\n",
                         converted_fg_type);
            break;
        }
    }
