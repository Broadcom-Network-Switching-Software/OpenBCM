 ...
 * \param [out] presel_qual_arg - Output string of the presel qualifier argument.
 *
 * \return
    ...
    char presel_qual_arg[DIAG_DNX_FIELD_UTILS_STR_SIZE])
{
    ...
    {
        if (qual_data.qual_type == bcmFieldQualifyCascadedKeyValue)
        {
            sal_snprintf(presel_qual_arg, DIAG_DNX_FIELD_UTILS_STR_SIZE, "%d", qual_data.qual_arg);
        }
        else
        {
    ...
    {
        sal_strncpy_s(presel_qual_arg, "N/A", DIAG_DNX_FIELD_UTILS_STR_SIZE);
    }
    else
    {
            ...
            qual_data.qual_type == bcmFieldQualifyForwardingLayerQualifier)
        {
            ...
            {
                sal_strncpy_s(presel_qual_arg, "0", DIAG_DNX_FIELD_UTILS_STR_SIZE);
            }
            else if (qual_data.qual_arg > 0)
            {
                sal_snprintf(presel_qual_arg, DIAG_DNX_FIELD_UTILS_STR_SIZE, "%d/%d",
        ...
        else if (qual_data.qual_type == bcmFieldQualifyVPortRangeCheck ||
                 qual_data.qual_type == bcmFieldQualifyInVportClass ||
                 qual_data.qual_type == bcmFieldQualifyOutVportClass)
        {
            sal_snprintf(presel_qual_arg, DIAG_DNX_FIELD_UTILS_STR_SIZE, "%d", qual_data.qual_arg);
        }
        else
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n" "Value:\"%d\" for option:\"presel\" is out of range:%d-%d!\r\n\n",
                         presel_id_lower_in, 0, (DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CS_LINES - 1));
        }
        if ((presel_id_upper_in < 0) || (presel_id_upper_in >= DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CS_LINES))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n" "Value:\"%d\" for option:\"presel\" is out of range:%d-%d!\r\n\n",
                         presel_id_upper_in, 0, (DNX_DATA_MAX_FIELD_COMMON_MAX_VAL_NOF_CS_LINES - 1));
        }
