...
static shr_error_e
diag_dnx_field_range_extended_condition(
    int unit,
    rhlist_t * list)
{
    SHR_FUNC_INIT_VARS(unit);

    if (dnx_data_field.features.extended_l4_ops_get(unit) == FALSE)
    {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (!dnx_data_field.features.extended_l4_ops_get(unit))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "\r\n" "Range Extended diagnostic is not available for current device."
                     " Please use 'field range list' diagnostic.\r\n\n");
    }
...
/**
 * \brief
 *   List of shell options for 'range extended' shell commands
 */
...
sh_sand_cmd_t Sh_dnx_field_range_cmds[] = {
    ...
    {"extended", NULL, Sh_dnx_field_range_extended_cmds, NULL, &Field_range_extended_man,
     NULL, NULL, SH_CMD_CONDITIONAL, diag_dnx_field_range_extended_condition}
    ,
    {NULL}
};
