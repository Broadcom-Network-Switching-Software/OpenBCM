...
static sh_sand_man_t Sh_dnx_field_presel_base_man = {
    .brief = "Presenting operations related to Field Presel (Currently display of configuration and allocation)",
    .full = NULL
};
...
sh_sand_cmd_t Sh_dnx_field_cmds[] = {
    ...
    {"presel",        NULL,                    Sh_dnx_field_presel_cmds,        NULL, &Sh_dnx_field_presel_base_man     , NULL, NULL, SH_CMD_NO_XML_VERIFY},
    ...
    {"ace",           NULL,                    Sh_dnx_field_ace_cmds,           NULL, &Sh_dnx_field_ace_base_man        , NULL, NULL, SH_CMD_NO_XML_VERIFY * SH_CMD_CONDITIONAL, diag_dnx_field_ace_is_available},
 ...
 */
sh_sand_enum_t Field_presel_diag_stage_enum_table[] = {
    {"IPMF1", 0, "ingress PMF1 stage"},
    {"IPMF2", 1, "ingress PMF2 stage"},
    {"IPMF3", 2, "ingress PMF3 stage"},
    {"EPMF", 3, "Egress PMF stage"},
    ...
    int add_entries)
{
    ...
    bcm_field_action_t uda_epmf_dp;
    ...
    bcm_field_action_info_t_init(&action_info);
    action_info.action_type = bcmFieldActionDropPrecedence;
    action_info.prefix_size = 2;
    action_info.prefix_value = 0x2;
    action_info.size = 0;
    action_info.stage = bcmFieldStageEgress;
    sal_strncpy_s((char *) action_info.name, "uda_epmf_dp", sizeof(action_info.name));
    SHR_IF_ERR_EXIT(bcm_field_action_create(unit, 0, &action_info, &uda_epmf_dp));
    ...
    fg_info.action_types[1] = uda_epmf_dp;
    ...
    /*
     * Add TCAM direct field group, if supported
     */
    bcm_field_group_info_t_init(&fg_info);
    ...
    if (dnx_data_field.features.exem_vmv_removable_from_payload_get(unit) &&
        (dnx_data_field.exem.feature_get(unit, dnx_data_field_exem_stage_selection_includes_ipmf1) == FALSE))
    {
        bcm_field_presel_entry_id_info_init(&presel_entry_id);
        bcm_field_presel_entry_data_info_init(&presel_entry_data);
        presel_entry_id.presel_id = 16;
        presel_entry_id.stage = bcmFieldStageIngressPMF2;
        presel_entry_data.context_id = Sh_dnx_field_init_context_ipmf2_based_default_1;
        presel_entry_data.entry_valid = TRUE;
        presel_entry_data.nof_qualifiers = 1;
        presel_entry_data.qual_data[0].qual_type = bcmFieldQualifyCascadedKeyValue;
        presel_entry_data.qual_data[0].qual_arg = Sh_dnx_field_init_fg_id_exem;
        presel_entry_data.qual_data[0].qual_value = 0xe;
        presel_entry_data.qual_data[0].qual_mask = -1;
        SHR_IF_ERR_EXIT(bcm_field_presel_set(unit, 0, &presel_entry_id, &presel_entry_data));
    }
    bcm_field_presel_entry_id_info_init(&presel_entry_id);
