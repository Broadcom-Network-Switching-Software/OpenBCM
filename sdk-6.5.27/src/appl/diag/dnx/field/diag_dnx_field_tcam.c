...
/**
 * \brief
 *   This function returns the  table, which owns the given handler id.
 * \param [in] unit - The unit number.
 * \param [in] handler_id - Handler Id, which owner should be retrieved.
 * \param [out] dbal_table_p - dbal table which owns the given handler id.
 *
 * \return
 *   Error code (as per 'shr_error_e').
 * \see
 ...
 */
static shr_error_e
diag_dnx_field_tcam_handler_id_dbal_table_get(
    int unit,
    uint32 handler_id,
    dbal_tables_e * dbal_table_p)
{
    uint8 table_found;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (handler_id = 0; handler_id < nof_tcam_handlers; handler_id++)
    {
        ...
        if (print_handler_table)
        {
            ...
            PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "BCM APP DB");
        ...
        SHR_IF_ERR_EXIT(diag_dnx_field_tcam_handler_id_dbal_table_get(unit, handler_id, &dbal_table));
        if ((fg_params.stage == DNX_FIELD_TCAM_STAGE_IPMF1) || (fg_params.stage == DNX_FIELD_TCAM_STAGE_IPMF2) ||
            (fg_params.stage == DNX_FIELD_TCAM_STAGE_IPMF3) || (fg_params.stage == DNX_FIELD_TCAM_STAGE_EPMF))
        {
            /*
             * In case of PMF, there is no BCM APP DB.
             */
            /** BCM APP DB*/
            PRT_CELL_SET("N/A");
        }
        else
        {
            ...
            int pp_app_found = FALSE;
            for (pp_app = bcmFieldAppDbFirst; pp_app < bcmFieldAppDbCount; pp_app++)
            {
                if (dnx_data_field_map.tcam.tcam_pp_app_get(unit, pp_app)->dbal_table == dbal_table)
                {
                    pp_app_found = TRUE;
                    break;
                }
            }
            if (pp_app_found == FALSE)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "handler_id %d has DBAL table %d, is in TCAM stage %d, "
                             "and no pp_app associated with it.\n", handler_id, dbal_table, fg_params.stage);
            }
            /** BCM APP DB*/
            PRT_CELL_SET("%s", dnx_field_bcm_app_db_text(pp_app));
        }
        /** Sanity check.*/
        SHR_IF_ERR_EXIT(dbal_tables_is_table_active(unit, dbal_table, &table_is_active));
        if (!table_is_active)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "handler_id %d has DBAL table %d, which is not active.\n",
                         handler_id, dbal_table);
        }
        PRT_CELL_SET("%s", dbal_logical_table_to_string(unit, dbal_table));
        DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_index)
        {
