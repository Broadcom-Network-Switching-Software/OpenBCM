...
static const diag_dnx_crps_expansion_type_t exp_type_enum_to_char_binding_table[] = {
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeTmDropReason, "TmDropReason"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeDispositionIsDrop, "DispositionIsDrop"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeDropPrecedenceMeter0Valid, "DropPrecedenceMeter0Valid"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeDropPrecedenceMeter0Value, "DropPrecedenceMeter0Value"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeDropPrecedenceMeter1Valid, "DropPrecedenceMeter1Valid"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypDropPrecedenceMeter1Value, "DropPrecedenceMeter1Value"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeDropPrecedenceMeter2Valid, "DropPrecedenceMeter2Valid"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeDropPrecedenceMeter2Value, "DropPrecedenceMeter2Value"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeTrafficClass, "TrafficClass"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeDropPrecedenceMeterResolved, "DropPrecedenceMeterResolved"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeDropPrecedenceInput, "DropPrecedenceInput"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeSystemMultiCast, "SystemMultiCast"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeEcnEligibleAndCni, "EcnEligibleAndCni"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypePortMetaData, "PortMetaData"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypePpDropReason, "PpDropReason"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeMetaData, "MetaData"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeDiscardPp, "DiscardPp"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeDiscardTm, "DiscardTm"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeEgressTmActionType, "EgressTmActionType"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeLatencyBin, "LatencyBin"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeLatencyFlowProfile, "LatencyFlowProfile"},
    {(bcm_stat_counter_format_type_t) bcmStatExpansionTypeTmLastCopy, "TmLastCopy"},
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int base_engine_id;
    uint32 is_rate_calculation_enable = FALSE;
    char *counter_units;
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        ...
        else
        {
            ...
            if (dnx_data_crps.general.feature_get(unit,
                                                  dnx_data_crps.general.feature_get(unit,
                                                                                    dnx_data_crps_general_crps_rate_calculation_support)))
            {
                SHR_IF_ERR_EXIT(bcm_stat_counter_database_control_get
                                (unit, flags, &database, bcmStatCounterDatabaseRateCalculationEnable,
                                 &is_rate_calculation_enable));
                if (is_rate_calculation_enable)
                {
                    PRT_COLUMN_ADD("Last Period Value");
                    PRT_COLUMN_ADD("Sequence Number");
                }
            }
            ...
            for (offset_id = 0; offset_id < nstat; offset_id++)
            {
                ...
                /** print rate calculation parameters*/
                if (is_rate_calculation_enable)
                {
                    PRT_CELL_SET("0x%08x%08x", COMPILER_64_HI(output_data.value_arr[offset_id].last_period_change),
                                 COMPILER_64_LO(output_data.value_arr[offset_id].last_period_change));
                    PRT_CELL_SET("%d", COMPILER_64_LO(output_data.value_arr[offset_id].sequence_number));
                }

                /** print format counter id and engine id */
                PRT_CELL_SET("%s", counter_units);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int core_id, core_idx, nof_counters;
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine
                            (unit, engine_id, interface.format_type, interface.counter_size, &nof_counters));
            /** interface*/
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
            PRT_CELL_SET("engine size");
            PRT_CELL_SET("%d", nof_counters);
            PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
            PRT_CELL_SET("source");
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        ...
        if (interface.source != bcmStatCounterInterfaceOamp)
        {
            ...
            if (dnx_data_crps.expansion.source_expansion_info_get(unit,
                                                                  interface_key.interface_source)->expansion_size <=
                dnx_data_crps.expansion.expansion_size_get(unit))
            {
                type_index = 0;
                for (expansion_type = 0; expansion_type < bcmStatExpansionTypeMax; expansion_type++)
                {
                    if (dnx_data_crps.expansion.source_metadata_types_get(unit, interface_key.interface_source,
                                                                          expansion_type)->nof_bits > 0)
                    {
                        expansion.expansion_elements[type_index].type = expansion_type;
                        type_index++;
                    }
                }
            }
            ...
            for (ind = 0; ind < expansion.nof_elements; ind++)
            {
                ...
                size =
                    dnx_data_crps.expansion.source_metadata_types_get(unit, interface.source,
                                                                      expansion.expansion_elements[ind].type)->nof_bits;
                SHR_IF_ERR_EXIT(dnx_crps_src_interface_expansion_type_offset_get
                                (unit, core_id, interface.source, interface.command_id,
                                 expansion.expansion_elements[ind].type, &offset));
        ...
        for (offset_id = 0; offset_id < interface.counter_set_size; offset_id++)
        {
            ...
            for (mem_idx = 0; mem_idx < DNX_DATA_MAX_CRPS_ENGINE_DATA_MAPPING_TABLE_SIZE; mem_idx++)
            {
                ...
                if (value.valid == 1 && value.counter_set_offset == offset_id)
                {
                    ...
                    sal_strncat_s(data_mapping_str, data_mapping_buf, sizeof(data_mapping_str));
                }
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int core_id, core_idx, type_id, nof_counters;
    uint32 flags = 0;
    int total_size = 0;
    bcm_stat_counter_database_t database;
    bcm_stat_counter_interface_t interface;
    ...
    DNXCMN_CORES_ITER(unit, core_id, core_idx)
    {
        ...
        {
            /**
             * Reset total size for next database
             */
            total_size = 0;
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, engine_id_arr[0], format_type, 0        /* state
                                                                                                         * counter size 
                                                                                                         */ , &nof_counters));
    end_object_stat_id_arr[0] = nof_counters / counter_set_size - 1;
    type_offset[0] = 0;

    /** configure type 1 */
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, engine_id_arr[1], format_type, 0        /* state
                                                                                                         * counter size 
                                                                                                         */ , &nof_counters));
    end_object_stat_id_arr[1] = nof_counters / counter_set_size - 1;
    ...
    /** configure type 2 */
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, engine_id_arr[2], format_type, 0        /* state
                                                                                                         * counter size 
                                                                                                         */ , &nof_counters));
    end_object_stat_id_arr[2] = nof_counters / counter_set_size - 1;
    type_offset[2] = (end_object_stat_id_arr[1] - start_object_stat_id_arr[1] + 1) + type_offset[1];

    /** configure type 3 */
    SHR_IF_ERR_EXIT(dnx_crps_mgmt_nof_counters_per_engine(unit, engine_id_arr[3], format_type, 0        /* state
                                                                                                         * counter size 
                                                                                                         */ , &nof_counters));
    end_object_stat_id_arr[3] = nof_counters / counter_set_size - 1;
    ...
    cnt_interface.format_type = format_type;
    ...
    for (color = bcmColorGreen; color <= bcmColorBlack; color++)
    {
        for (reject = 0; reject < 2; reject++)
        {
            ...
            ctr_set_map.expansion_data_mapping[ctr_set_map.nof_entries].value.set_id = 0;
