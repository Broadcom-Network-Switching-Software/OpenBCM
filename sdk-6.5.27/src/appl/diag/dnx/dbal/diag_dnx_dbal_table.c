        ...
        SH_SAND_GET_ITERATOR(sand_arg)
        {
            ...
            if (field_id == DBAL_FIELD_EMPTY)
            {
                ...
                if (sal_strncasecmp(rule_name_str, "HITBIT", strnlen_rule_name_str) == 0)
                {
                    ...
                    SHR_IF_ERR_EXIT(dbal_iterator_attribute_rule_add(unit, entry_handle_id, attr_type, attr_val));
                    num_of_rules++;
                    hit_bit_rule = TRUE;
                }
                else if (sal_strncasecmp(rule_name_str, "core_group", strnlen_rule_name_str) == 0)
                {
                    /*
                     * if rule is CORE_GROUP_ALL, it is like default behavior, no need to define a rule 
                     */
                    if (attr_val != (uint32) DBAL_CORE_GROUP_ALL)
                    {
                        SHR_IF_ERR_EXIT(dbal_iterator_attribute_rule_add
                                        (unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, (int) attr_val));
                        num_of_rules++;
                    }
                }
                else
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (res_type_index = 0; res_type_index < table->nof_result_types; res_type_index++)
    {
        ...
        if ((table->core_mode == DBAL_CORE_MODE_DPCG) && (DBAL_NOF_CORE_GROUPS > 1))
        {
            PRT_COLUMN_ADD_FLEX_PL(res_type_index, PRT_FLEX_ASCII, "Core Group");
        }
    ...
    while (!is_end)
    {
        ...
        for (iter = 0; iter < entry_handle->nof_key_fields; iter++)
        {
            ...
            if (entry_handle->key_field_ids[iter] == DBAL_FIELD_CORE_ID)
            {
                ...
                if ((entry_handle->phy_entry.core_id == DBAL_CORE_ALL) || (table->core_mode == DBAL_CORE_MODE_SBC))
                {
                    sal_snprintf(buffer, sizeof(buffer), "ALL");
                }
                else
                {
                    sal_snprintf(buffer, sizeof(buffer), " %d ", entry_handle->phy_entry.core_id);
                }
            }
            else
            {
        ...
        {
            uint32 core_group;
            SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, &core_group));
            PRT_CELL_SET_PL(curr_res_type_index, "%d", core_group);
        }
        /*
         * For TCAM tables - add IS_VALID indication
         */
