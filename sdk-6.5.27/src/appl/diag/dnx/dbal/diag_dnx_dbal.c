...
extern shr_error_e dbal_hl_access_memreg_size_get_per_access(
    int unit,
    int global_regmem,
    int is_new_access,
    uint8 is_mem,
...
extern const char *dbal_hl_access_regmem_name_get(
    int unit,
    CONST dbal_logical_table_t * table,
    int regmem,
    uint8 is_mem);

extern shr_error_e dbal_hl_access_field_info_get_per_access(
    int unit,
    CONST dbal_logical_table_t * table,
    int regmem,
    int hw_field,
    ...
    *is_full_match = 0;
    ...
    for (iter = curr_field_type; iter < DBAL_NOF_FIELD_TYPES_DEF; iter++)
    {
        ...
        if (sal_strncaseindex
            (field_type_info->name, substr_match, DBAL_MAX_STRING_LENGTH, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
            *field_type = iter;
            break;
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (ii = DBAL_FIELD_TYPE_DEF_EMPTY + 1; ii < DBAL_NOF_FIELD_TYPES_DEF; ii++)
    {
        ...
        {
            if (sal_strncaseindex
                (field_type_info->name, field_type_name, DBAL_MAX_STRING_LENGTH,
    ...
    if (handle_id != -1)
    {
        ...
        if (entry_handle->handle_status != DBAL_HANDLE_STATUS_AVAILABLE)
        {
            LOG_CLI((BSL_META("handle id=%d, status=%d, core=%d, error indication=%d, table:%s\n"),
                     handle_id, entry_handle->handle_status, entry_handle->phy_entry.core_id,
    ...
    int unit)
{
    ...
    while (table_id != DBAL_TABLE_EMPTY)
    {
        ...
        {
            regmem = l2p_hl_info->l2p_fields_info[0].reg[0];
            is_mem = 0;
            for (ii = 1; ii < nof_fields; ii++)
            {
                if (regmem != l2p_hl_info->l2p_fields_info[ii].reg[0])
                {
                    break;
                }
            }
        ...
        {
            regmem = l2p_hl_info->l2p_fields_info[0].memory[0];
            is_mem = 1;
            for (ii = 1; ii < nof_fields; ii++)
            {
                if (regmem != l2p_hl_info->l2p_fields_info[ii].memory[0])
                {
                    break;
                }
            }
        ...
        if (ii == nof_fields)
        {
            uint32 regmem_size_bits = 0;

            LOG_CLI((BSL_META("Table %s mapped to unique regmem %s"), table->table_name,
                     dbal_hl_access_regmem_name_get(unit, table, regmem, is_mem)));
            table_count++;
            ...
            SHR_IF_ERR_EXIT(dbal_hl_access_memreg_size_get_per_access(unit, regmem,
                                                                      SHR_IS_BITSET(table->indications_bm,
                                                                                    DBAL_TABLE_IND_IS_NEW_ACCESS),
                                                                      is_mem, &regmem_size_bits));
            if (regmem_size_bits == table->max_payload_size)
            {
                    ...
                    switch (iter)
                    {
                        case DBAL_HL_ACCESS_MEMORY:
                            for (hl_iter = 0; hl_iter < l2p_hl_info->num_of_access_fields; hl_iter++)
                            {
                                ...
                                {
                                    if (l2p_hl_info->l2p_fields_info[hl_iter].memory[index] !=
                                        DBAL_HL_ACCESS_HW_ENTITY_INVALID)
                                    {
                                        if (sal_strncaseindex
                                            (dbal_hl_access_regmem_name_get
                                             (unit, table, l2p_hl_info->l2p_fields_info[hl_iter].memory[index], 1),
                                             hw_element_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                                             SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                                        {
                                            PRT_CELL_SET("%s", table->table_name);
                                            PRT_CELL_SET("%s",
                                                         dbal_hl_access_regmem_name_get(unit, table,
                                                                                        l2p_hl_info->l2p_fields_info
                                                                                        [hl_iter].memory[index], 1));
                                        ...
                                        if (l2p_hl_info->l2p_fields_info[hl_iter].alias_memory !=
                                            DBAL_HL_ACCESS_HW_ENTITY_INVALID)
                                        {
                                            if (sal_strncaseindex
                                                (dbal_hl_access_regmem_name_get
                                                 (unit, table, l2p_hl_info->l2p_fields_info[hl_iter].alias_memory, 1),
                                                 hw_element_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                                                 SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                                            {
                                                PRT_CELL_SET("%s", table->table_name);
                                        ...
                                        if (l2p_hl_info->l2p_fields_info[hl_iter].hw_field !=
                                            DBAL_HL_ACCESS_HW_FIELD_INVALID)
                                        {
                                            char field_name[DBAL_MAX_SHORT_STRING_LENGTH];
                                            SHR_IF_ERR_EXIT(dbal_hl_access_field_info_get_per_access(unit,
                                                                                                     table,
                                                                                                     l2p_hl_info->l2p_fields_info
                                                                                                     [hl_iter].memory
                                                                                                     [0],
                                                                                                     ...
                                                                                                     field_name));
                                            if (sal_strncaseindex
                                                (field_name, hw_element_name, DBAL_MAX_SHORT_STRING_LENGTH,
                                                 SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                                            {
                                                PRT_CELL_SET("%s", table->table_name);
                        ...
                        case DBAL_HL_ACCESS_REGISTER:
                            for (hl_iter = 0; hl_iter < l2p_hl_info->num_of_access_fields; hl_iter++)
                            {
                                ...
                                for (index = 0; index < DBAL_MAX_NUMBER_OF_HW_ELEMENTS; index++)
                                {
                                    if (l2p_hl_info->l2p_fields_info[hl_iter].reg[index] !=
                                        DBAL_HL_ACCESS_HW_ENTITY_INVALID)
                                    {
                                        if (sal_strncaseindex
                                            (dbal_hl_access_regmem_name_get
                                             (unit, table, l2p_hl_info->l2p_fields_info[hl_iter].reg[index], 0),
                                             hw_element_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                                             SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                                        {
                                            PRT_CELL_SET("%s", table->table_name);
                                ...
                                if (l2p_hl_info->l2p_fields_info[hl_iter].hw_field != DBAL_HL_ACCESS_HW_FIELD_INVALID)
                                {
                                    char field_name[DBAL_MAX_SHORT_STRING_LENGTH];
                                    SHR_IF_ERR_EXIT(dbal_hl_access_field_info_get_per_access(unit,
                                                                                             table,
    ...
    int *is_full_match)
{
    ...
    *is_full_match = 0;
    ...
    for (group_index = curr_group_id; group_index < DBAL_NOF_HW_ENTITY_GROUPS; group_index++)
    {
        ...
        if (sal_strncaseindex
            (dbal_hw_entity_group_to_string(unit, group_index), substr_match,
             SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
            *group_id = group_index;
            break;
        }
    ...
    uint8 *is_full_match)
{
    ...
    *is_full_match = 0;
    ...
    for (iter = curr_phy_table; iter < DBAL_NOF_PHYSICAL_TABLES; iter++)
    {
        ...
        if (sal_strncaseindex
            (curr_str, substr_match, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
             SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
            *phy_table = iter;
            break;
...
static sh_sand_option_t dnx_dbal_sbc_core_group_options[] = {
    {"ID", SAL_FIELD_TYPE_INT32, "Core Group ID", "-1"},
    {NULL}
};

static shr_error_e
cmd_dbal_access_sbc_core_group(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    uint8 core_group_id = 0;

    SHR_FUNC_INIT_VARS(unit);

    SH_SAND_GET_INT32("ID", core_group_id);

    if ((core_group_id == 0) || (core_group_id == 1))
    {
        SHR_IF_ERR_EXIT(dbal_sbc_group_id_set(unit, core_group_id));
        LOG_CLI((BSL_META("Core Group Id was set to %d. Dump and get operations will be"
                          " performed from this core group only.\n"), core_group_id));
    }
    else
    {
        uint8 cg_id;
        SHR_IF_ERR_EXIT(dbal_sbc_group_id_get(unit, &cg_id));
        LOG_CLI((BSL_META("sbc_group_id: %d\n"), cg_id));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (table_iter = 0; table_iter < dnx_data_dbal.table.nof_dynamic_and_static_tables_get(unit); table_iter++)
    {
        ...
        if (sal_strncaseindex(table->table_name, "example", DBAL_MAX_STRING_LENGTH, sizeof("example")))
        {
...
static sh_sand_man_t dnx_dbal_access_core_group_man = {
    .brief = "Set the core group to dump entries from when doing table dump",
    .full = "Set the core group to dump entries from when doing table dump.\n"
        "Relevant for SBC tables in a multiple core groups device.",
    .synopsis = "ID=0",
    .examples = "ID=0\n"
};
...
sh_sand_cmd_t diag_dbal_access_pemla_cmds[] = {
   /******************************************************************************************************************
...
sh_sand_cmd_t sh_dnx_dbal_access_cmds[] = {
   ...
   *******************************************************************************************************************/
    ...
    {"SW", cmd_dbal_access_sw_info_dump, NULL, dnx_dbal_access_sw_status_options, &dnx_dbal_access_sw_status_man, NULL,
     NULL, SH_CMD_NO_XML_VERIFY},
    {"PEMLA", NULL, diag_dbal_access_pemla_cmds, dnx_dbal_access_pemla_status_options,
     &dnx_dbal_access_pemla_status_man, NULL, NULL, SH_CMD_NO_XML_VERIFY},
    {"core_group", cmd_dbal_access_sbc_core_group, NULL, dnx_dbal_sbc_core_group_options,
     &dnx_dbal_access_core_group_man, NULL, NULL, SH_CMD_NO_XML_VERIFY},
    {NULL}
};
/**********************************************************************************************************************
 *  DBAL DIAGNOSTIC PACK:
    ...
    while (!is_end)
    {
        for (iter = 0; iter < l2p_hl_info->num_of_access_fields; iter++)
        {
            ...
            if (is_condition_pass)
            {
                ...
                if (sal_strncaseindex
                    (SOC_REG_NAME(unit, curr_reg), hw_element_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                     SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                {
                     /** related entry found */
                    (*is_found) = 1;
    ...
    while (!is_end)
    {
        for (iter = 0; iter < l2p_hl_info->num_of_access_fields; iter++)
        {
            ...
            if (is_condition_pass)
            {
                ...
                if (sal_strncaseindex
                    (SOC_MEM_NAME(unit, curr_mem), hw_element_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                     SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                {
