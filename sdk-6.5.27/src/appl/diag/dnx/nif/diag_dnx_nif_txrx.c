    ...
    sh_sand_control_t * sand_control)
{
    ...
    /*
     * OFR does not support max occupancy
     */
    if (!is_ofr)
    {
        PRT_COLUMN_ADD("Max Occupancy");
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_port_ofr_rx_fifo_status_counter_map(unit, port));
    SHR_IF_ERR_EXIT(dnx_algo_port_db.port_rx_fifo_counter.port.set(unit, port));
...
sh_sand_man_t sh_dnx_nif_rx_mapping_man = {
    .brief    = "Maps a counter to a port",
