    ...
    char *lane_str)
{
    ...
    BCM_PBMP_ITER(*pbmp, lane)
    {
        ...
        if (last_added == -1)
        {
            ...
            if (is_over_fabric)
            {
                sal_snprintf(tmp_str1, sizeof(tmp_str1), "fabric:%02u", last_added);
            }
            else
            {
                sal_snprintf(tmp_str1, sizeof(tmp_str1), "%02u", last_added);
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    char ohif_str[DNX_DIAG_NIF_STATUS_MAX_STRING_LEN];
    int port_has_speed;
    ...
    BCM_PBMP_ITER(nif_pbmp, port)
    {
        ...
        {
            sal_snprintf(port_info_str, sizeof(port_info_str), "ETH %s", if_fmt_speed(speed_str, port_resource.speed));
        }
        else if (DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
        {
            sal_snprintf(port_info_str, sizeof(port_info_str), "OTN %s", if_fmt_speed(speed_str, port_resource.speed));
        }
        else
        {
            sal_snprintf(port_info_str, sizeof(port_info_str), "ILKN %s", if_fmt_speed(speed_str, port_resource.speed));
        }
        /*
         * Get NIF phys
         */
        ...
        if (!no_rates && port_enable && !synce_enable && !SAL_BOOT_PLISIM)
        {
            ...
            sal_snprintf(serdes_freq_str, sizeof(serdes_freq_str), "%d.%03d", serdes_freq_arr[master_port],
                         serdes_freq_remain_arr[master_port]);
        }
        else
        {
            sal_snprintf(serdes_freq_str, sizeof(serdes_freq_str), "N/A");
        }
        ...
        {
            utilex_str_append(comment_str, "MGMT");
        }
        ...
        {
            utilex_str_append(comment_str, "FLEXE_UNAWARE");
        }
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH_FEEDBACK(unit, port_info))
        {
            utilex_str_append(comment_str, "FEEDBACK");
        }
        ...
        if (DNX_ALGO_PORT_TYPE_IS_FRAMER_MGMT(unit, port_info))
        {
            ...
            {
                utilex_str_append(comment_str, "FRAMER_PTP");
            }
            else
            {
                sal_snprintf(ohif_str, sizeof(ohif_str), "FRAMER_OHIF%d", mgmt_channel);
                utilex_str_append(comment_str, ohif_str);
            }
        }
