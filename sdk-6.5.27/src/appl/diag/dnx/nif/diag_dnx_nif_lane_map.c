    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_PBMP_ITER(ilkn_id_bmp, ilkn_id_tmp)
    {
        ...
        if (is_over_fabric)
        {
            ...
            sal_snprintf(str_base, sizeof(str_base), "fabric:");
        }
        else
        {
...
shr_error_e
sh_dnx_nif_otn_lane_map_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    bcm_pbmp_t pbmp;
    int logical_port_tmp, actual_size;
    int nof_lanes;
    int lane, nof_phy_lanes, i, count;
    bcm_pbmp_t phys;
    bcm_pbmp_t logical_port_bmp;
    ...
    BCM_PBMP_CLEAR(pbmp);
    SH_SAND_GET_PORT("Port", pbmp);
    /*
     * Get the ILKN id to port mapping
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_OTN_PHY, 0, &logical_port_bmp));
    /*
     * Print table header
     */
    PRT_TITLE_SET("OTN Lane Map");

    PRT_COLUMN_ADD("Port#");
    PRT_COLUMN_ADD("Lane#");
    PRT_COLUMN_ADD("SRD_RX#");
    PRT_COLUMN_ADD("SRD_TX#");
    PRT_COLUMN_ADD("Logical OTN lane#");
    PRT_COLUMN_ADD("OTN lanes#");

    BCM_PBMP_ITER(logical_port_bmp, logical_port_tmp)
    {
        if (!BCM_PBMP_MEMBER(pbmp, logical_port_tmp))
        {
            continue;
        }
        bcm_port_resource_t_init(&resource);
        SHR_IF_ERR_EXIT(bcm_port_resource_get(unit, logical_port_tmp, &resource));
        /*
         * Get the lane map info from Lane Map DB
         */
        type = DNX_ALGO_LANE_MAP_NIF_SIDE;
        ...
        SHR_ALLOC(lane2serdes, nof_lanes * sizeof(soc_dnxc_lane_map_db_map_t), "Lane to serdes map", "%s%s%s\r\n",
                  EMPTY, EMPTY, EMPTY);
        SHR_IF_ERR_EXIT(dnx_algo_lane_map_lane_to_serdes_map_get(unit, type, nof_lanes, lane2serdes));
        SHR_IF_ERR_EXIT(dnx_algo_port_nif_phys_get(unit, logical_port_tmp, 0, &phys));
        BCM_PBMP_COUNT(phys, nof_phy_lanes);
        /*
         * Get the OTN lane order
         */
        if (resource.logical_otn_lane != BCM_PORT_RESOURCE_DEFAULT_REQUEST)
        {
            SHR_IF_ERR_EXIT(imb_port_lane_map_get
                            (unit, logical_port_tmp, 0, DNX_DATA_MAX_NIF_OTN_NOF_LANES, lane_order, &actual_size));
        }
        /*
         * Print the table
         */
        ...
        {
            /** Only print the otn for the first lane */
            if (count == 0)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE_BEFORE);
                PRT_CELL_SET("%d", logical_port_tmp);
            }
            else
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            ...
            if (count == 0)
            {
                if (resource.logical_otn_lane != BCM_PORT_RESOURCE_DEFAULT_REQUEST)
                {
                    PRT_CELL_SET("%d", resource.logical_otn_lane);
                }
                else
                {
                    PRT_CELL_SET("%s", "N/A");
            ...
            if (resource.logical_otn_lane != BCM_PORT_RESOURCE_DEFAULT_REQUEST)
            {
                ...
                {
                    PRT_CELL_SET("%02d,%02d", lane_order[count * 2], lane_order[count * 2 + 1]);
                }
                else
                {
                    PRT_CELL_SET("%02d", lane_order[count]);
                }
            }
            else
            {
...
sh_sand_man_t sh_dnx_nif_otn_lane_map_man = {
    .brief    = "Diagnostic for OTN lane map for given OTN port",
    .full     = "Include the OTN lane order INFO and logical to physical mapping",
    .synopsis = "[Port=<logical_port>]",
    .examples = "\n"
                "port=otn"
};

sh_sand_option_t sh_dnx_nif_otn_lane_map_options[] = {
    /*name           type                  desc          default    ext*/
    {"Port",      SAL_FIELD_TYPE_PORT, "Logical Port ID",    "otn",       NULL},
    {NULL}
};
