    ...
    int nof_valid_clusters)
{
    ...
    PRT_CELL_SET("%s",
                 dbal_core_mode_to_string(unit,
                                          dnx_data_mdb.pdbs.dbal_pdb_info_get(unit,
                                                                              dbal_physical_table_id)->core_mode));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else if (ISEMPTY(table_name))
    {
        ...
        for (ii = 1; ii < DBAL_NOF_PHYSICAL_TABLES; ii++)
        {
            ...
            PRT_CELL_SET("%s",
                         dbal_core_mode_to_string(unit, dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, ii)->core_mode));
...
static sh_sand_option_t dnx_mdb_direct_options[] = {
    ...
    {MDB_DIAG_STR_HIT_CLR, SAL_FIELD_TYPE_BOOL, "Perform a hitbit clear operation", "no"},
#endif
    {MDB_DIAG_STR_INSTANCE, SAL_FIELD_TYPE_INT32, "The instance (core/core_group) ID", "0"},
    {NULL}
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    sal_memset(&physical_entry_iterator, 0, sizeof(physical_entry_iterator));
    physical_entry_iterator.core_id = instance_id;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_INT32(MDB_DIAG_STR_INSTANCE, instance_id);
    entry.core_id = instance_id;
    ...
    else
    {
        ...
        else if (dump_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_diag_direct_dump
                            (unit, instance_id, dbal_physical_table_id, app_id, hw_cmd, sand_control));
        }
        else if (clear_cmd != 0)
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        {
            SHR_IF_ERR_EXIT(mdb_diag_eedb_dump
                            (unit, instance_id, dbal_physical_table_id, app_id, hw_cmd, sand_control));
        }
        else if (clear_cmd != 0)
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        if (dump_cmd != 0)
        {
            SHR_IF_ERR_EXIT(mdb_diag_em_dump(unit, instance_id, dbal_physical_table_id, app_id, hw_cmd, sand_control));
        }
        else if (clear_cmd != 0)
        {
