...
typedef struct xd_s
{
    ...
    uint32 sig_dump_valid;
    char *sig_dump_file;        /* XML File to store the result of sig get all */
#endif
} xd_t;
...
sh_sand_option_t sh_dnx_tx_options[] = {
    ...
    {"SignalDumpValid", SAL_FIELD_TYPE_BOOL, "Indicate to dump signal", "0"},
    {"SignalDumpFile", SAL_FIELD_TYPE_STR, "File to store signal dump", ""},
    {NULL}
};
...
sh_sand_man_t sh_dnx_tx_man = {
    ...
    .examples = "1 PtchSRCport=200 DaTa=0x11223344abcd000000000001810000010899111111111111111111112222222222222222222233333333333333333333444444444444444444445555555555555555555566666666666666666666aabbccddeeffaabbccdd\n"
                "2 PORT=200 PtchSRCport=202\n"
                "3 TXUnit=0 PortBitMap=eth13 UntagBitMap=eth13 LENgth=80\n"
                "4 PortBitMap=eth13 UnTagged=No TagProtocolIdentifier=0x8100 VLan=2 VlanPrio=1 PatTeRN=0x12345678 PatternINC=1 PatternRandom=No\n"
                "5 PortBitMap=cpu200 SourceMac=00:00:00:07:00:00 SourceMacInc=0x1 DestMac=00:00:00:e3:00:00 DestMacInc=0x1\n"
                "6 PtchSRCport=200 PtchHeader=0x8f17 CRC=Append VISibility=No\n"
                "7 PtchSRCport=200 CRC=Append VISibility=No\n"
                "8 PortBitMap=eth13 FILENaMe=pkt\n"
                "9 PtchSRCport=200 ItmhPRESent=Yes FwdActStrength3LSB=0x7 AsExtPresent=No PphType=0x0 InMirrDisable=Yes DropPrecedence=0x3 ForWarD=SPA DeSTination=13 FEC=0 ACTion=0 FlowId=0 MultiCast=0 SnoopProfile =0x1f TrafficClass=0x7 ItmhBaseExtPresent=Yes ItmhBaseExtType=0x1 ItmhBaseExtValue=0x3fffff\n"
                "10 PtchSRCport=200 SignalDumpValid=1 SignalDumpFile=./dump.xml\n",
};
/* *INDENT-ON* */
    ...
    xd_t * xd)
{
     ...
     */
#if !defined(NO_FILEIO)
    SH_SAND_GET_BOOL("SignalDumpValid", xd->sig_dump_valid);
    SH_SAND_GET_STR("SignalDumpFile", xd->sig_dump_file);

    if (xd->sig_dump_valid)
    {
        if (ISEMPTY(xd->sig_dump_file))
        {
    ...
    {
        xfile = sal_strndup(xd->xd_file, SH_SAND_MAX_TOKEN_SIZE);
    }
    else
    {
        xfile = NULL;
    ...
    {
        xdata = sal_strndup(xd->xd_data, SH_SAND_MAX_TOKEN_SIZE);
    }
    else
    {
        xdata = NULL;
    ...
    sh_sand_control_t * sand_control)
{
     ...
     */
#if !defined(NO_FILEIO)
    if (xd->sig_dump_valid && !ISEMPTY(xd->sig_dump_file))
    {
        ...
        if (sig_args == NULL)
        {
            SHR_CLI_EXIT(_SHR_E_MEMORY, "No memory for sig_args\n");
        }
        /*
         * Set argvs
         */
        ...
        sal_strncat_s(file_argv, xd->sig_dump_file, max_file_name_len - 1);
        sig_args->a_argv[3] = file_argv;
        sal_strncpy(sig_args->a_buffer, "sig\0get\0all\0file=", ARGS_BUFFER - 1);
        sal_strncat_s(sig_args->a_buffer, xd->sig_dump_file, ARGS_BUFFER - 1);
        sig_args->a_argc = 4;
        sig_args->a_arg = 1;

        cmd_sand_signal(xd->xd_tx_unit, sig_args);
        sal_free(sig_args);
    }
...
/*
 *  Simplest test command for the SOC DMA ROM interfaces
 */
static sh_sand_man_t drt_init_man = {
    .brief = "Reconfigure TX/RX channels in polled mode",
    .full = "Reconfigure TX/RX channels in polled mode\n"
        "    RX channel is specified by Macro SOC_DMA_ROM_RX_CHANNEL, channel 7 by default\n"
        "    RX is started up and ready for packets to arrive\n"
        "    TX channel is specified by Macro SOC_DMA_ROM_TX_CHANNEL, channel 6 by default\n"
...
/**
 * \brief - DmaRomTest init command implementation
 * \param [in] unit - Unit #
 * \param [in] args - Command line arguments, may be NULL if called from outside the shell
 * \param [in] sand_control - Shell command control structure
 * \return
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(soc_dma_rom_init(unit, 1600, 1, 1));

exit:
    SHR_FUNC_EXIT;
}

static sh_sand_man_t drt_detach_man = {
    .brief = "Finalize the polled TX/RX module",
    .full = "Finalize the polled TX/RX module\n"
        "    Abort DMA active on the all DMA channels\n" "    Free internal memory associated with DMA.",
    .synopsis = NULL,
    .examples = NULL    /* NULL will result in option less example */
};

/**
 * \brief - DmaRomTest detach command implementation
 * \param [in] unit - Unit #
 * \param [in] args - Command line arguments, may be NULL if called from outside the shell
 * \param [in] sand_control - Shell command control structure
 * \return
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(soc_dma_rom_detach(unit));

exit:
    SHR_FUNC_EXIT;
}

static sh_sand_man_t drt_rx_man = {
    .brief = "Check whether the packet is received, and if so, dump it",
    .full = "Check whether the packet is received, and if so, dump it\n"
...
/**
 * \brief - DmaRomTest rx command implementation
 * \param [in] unit - Unit #
 * \param [in] args - Command line arguments, may be NULL if called from outside the shell
 * \param [in] sand_control - Shell command control structure
 * \return
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(soc_dma_rom_rx_poll(unit, &dcb));
    ...
    if (dcb)
    {
        ...
        pkt = (uint8 *) dnx_drv_soc_dcb_addr_get(unit, dcb);
        len = dnx_drv_soc_dcb_xfercount_get(unit, dcb);
...
static sh_sand_man_t drt_tx_man = {
    .brief = "Simple packet transmit using soc_dma_rom_tx_start()",
    .full = "Simple packet transmit using soc_dma_rom_tx_start()\n"
        "    Packet in fixed format of ETH1oPTCH2oModuleHeader is injected from fixed CPU port 200\n"
        "        ModuleHeader delivers cpu channel of CPU port 200\n"
        "        PTCH2 delivers PP port of CPU port 200\n"
        "        ETH1 is boardcasted with VID=1\n"
        "    TX is started via soc_dma_rom_tx_start()\n" "    Check if TX is completed.",
    .synopsis = NULL,
    .examples = NULL    /* NULL will result in option less example */
};

/**
 * \brief - DmaRomTest tx command implementation
 * \param [in] unit - Unit #
 * \param [in] args - Command line arguments, may be NULL if called from outside the shell
 * \param [in] sand_control - Shell command control structure
 * \return
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint8 pkt_data[100] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,     /* Module Header */
    ...
    if (dcb == NULL)
    {
        SHR_CLI_EXIT(_SHR_E_MEMORY, "No memory for soc_dma_rom_dcb_alloc\n");
    }
     ...
     * Set PTCH2.SSP
     */
    pkt_data[DNX_MODULE_HEADER_SIZE + 1] = mapping_info.pp_port & 0xff;
    /*
     * c_hg is set to indicate Module Header is present
     */
    dnx_drv_soc_dcb_hg_set(unit, dcb, 1);
    /*
     * get CPU channel of CPU port 200
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_in_channel_get(unit, cpu_port, &channel));
    /*
     * set CPU channel to first byte of Module header
     */
    channel_value = (uint8) (channel & 0xff);
    pkt_data[0] = channel_value;

    dcb_addr = (uint8 *) dnx_drv_soc_dcb_addr_get(unit, dcb);
    /*
    ...
    SHR_IF_ERR_EXIT(soc_dma_rom_tx_start(unit, dcb));
...
sh_sand_cmd_t sh_dnx_drt_cmds[] = {
    {"init", drt_init_cmd, NULL, NULL, &drt_init_man}
    ,
    {"detach", drt_detach_cmd, NULL, NULL, &drt_detach_man}
    ,
    {"rx", drt_rx_cmd, NULL, NULL, &drt_rx_man}
    ,
    {"tx", drt_tx_cmd, NULL, NULL, &drt_tx_man}
    ,
    {NULL}
};

sh_sand_man_t sh_dnx_drt_man = {
    .brief = "Simplest test command for the SOC DMA ROM interfaces"
};
