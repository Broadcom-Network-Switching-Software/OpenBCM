    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        PRT_COLUMN_ADD("EEDB banks used for data (free entries/total entries)");
    }
        ...
        if (!dnx_data_lif.out_lif.feature_get(unit, dnx_data_lif_out_lif_per_phase_algo))
        {
            if (assigned == DNX_ALGO_OUTLIF_BANK_ASSIGNED_TO_EEDB)
            {
                     ...
                     current_eedb_bank_array_index++)
                {
                    ...
                    {
                        int num_free_eedb_bank_entries = 0;

                        num_free_eedb_bank_entries =
                            eedb_bank_size * 4 - nof_allocated_elements * (address_granularity / 30);
                        sal_snprintf(eedb_banks, strn_size, "%s%d (%d/%d) ", eedb_banks, current_eedb_bank,
                                     num_free_eedb_bank_entries, eedb_bank_size * 4);
                    }
                }
...
static shr_error_e
dnx_outlif_allocation_phase_dump_all_cmd(
    int unit,
    sh_sand_control_t * sand_control)
{
    uint8 address_granularity, first_eedb_bank, last_eedb_bank;
    uint32 phase_id, first_bank_for_phase = 0, last_bank_for_phase = 0, first_bank_for_phase_no_ll =
        0, last_bank_for_phase_no_ll = 0;
    int outlif_bank_size;

    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);

    outlif_bank_size = dnx_data_lif.out_lif.allocation_bank_size_get(unit);

    PRT_TITLE_SET("Phases - outlif banks summary");
    PRT_COLUMN_ADD("Phase id");
    PRT_COLUMN_ADD("Outlif banks ll range (banks, IDs)");
    PRT_COLUMN_ADD("Outlif banks no ll range (banks, IDs)");
    PRT_COLUMN_ADD("Possible EEDB banks for ll");
    PRT_COLUMN_ADD("granularity");

    for (phase_id = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_1; phase_id < DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT;
         phase_id++)
    {
        PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);

        PRT_CELL_SET("%d", phase_id);
        ...
        /*
         * Outlif banks ll range (banks, IDs)
         */
        if (last_bank_for_phase > first_bank_for_phase)
        {
            PRT_CELL_SET("{%d-%d , %d-%d}", first_bank_for_phase, last_bank_for_phase,
                         first_bank_for_phase * outlif_bank_size, last_bank_for_phase * outlif_bank_size);
        }
        else
        {
            PRT_CELL_SET("NA");
        ...
        /*
         * Outlif banks no ll range (banks, IDs)
         */
        if (last_bank_for_phase_no_ll > first_bank_for_phase_no_ll)
        {
            PRT_CELL_SET("{%d-%d , %d-%d}", first_bank_for_phase_no_ll, last_bank_for_phase_no_ll,
                         first_bank_for_phase_no_ll * outlif_bank_size, last_bank_for_phase_no_ll * outlif_bank_size);
        }
        else
        {
            PRT_CELL_SET("NA");
        ...
        /*
         * Possible EEDB banks for ll
         */
        PRT_CELL_SET("{%d-%d}", first_eedb_bank, last_eedb_bank);
...
static shr_error_e
dnx_outlif_allocation_phase_dump_single_cmd(
    int unit,
    sh_sand_control_t * sand_control,
    int phase_id)
{
    ...
    uint32 first_bank_for_phase = 0, last_bank_for_phase = 0, first_bank_for_phase_no_ll =
        0, last_bank_for_phase_no_ll = 0;
    ...
    for (current_bank = first_bank_for_phase; current_bank <= last_bank_for_phase; current_bank++)
    {
        ...
        if ((!in_use) && (!dnx_data_lif.out_lif.feature_get(unit, dnx_data_lif_out_lif_per_phase_algo)))
        {
            continue;
        }
    ...
    if (!dnx_data_lif.out_lif.feature_get(unit, dnx_data_lif_out_lif_per_phase_algo))
    {
        while (DNX_SW_STATE_LL_IS_NODE_VALID(current_node))
        {
            ...
            if (bank_logical_phase == phase_id)
            {
                     ...
                     current_eedb_bank_array_index++)
                {
                    ...
                    if (current_eedb_bank != (uint8) DNX_ALGO_LOCAL_OUTLIF_INVALID_BANK)
                    {
                        ...
                        SHR_IF_ERR_EXIT(lif_mngr_db.
                                        egress_local_lif.nof_allocated_elements_in_range_get(unit, current_bank,
                                                                                             OUTLIF_BANK_FIRST_INDEX
                                                                                             (unit,
                                                                                              current_bank) +
                                                                                             nof_outlifs_per_eedb_bank *
            ...
            SHR_IF_ERR_EXIT(local_outlif_info.eedb_banks_info.
                            used_data_bank_per_logical_phase.next_node(unit, current_node, &current_node));
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_ENUM("phase", phase_id);

    if (phase_id == DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_COUNT)
    {
        SHR_IF_ERR_EXIT(dnx_outlif_allocation_phase_dump_all_cmd(unit, sand_control));
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_outlif_allocation_phase_dump_single_cmd(unit, sand_control, phase_id));
    }
    ...
    int *is_full_match)
{
    ...
    *is_full_match = 0;
    ...
    for (iter = curr_table_id; iter < dnx_data_dbal.table.nof_dynamic_and_static_tables_get(unit); iter++)
    {
        ...
        if (sal_strncaseindex(table->table_name, substr_match, DBAL_MAX_STRING_LENGTH, DBAL_MAX_STRING_LENGTH))
        {
            *table_id = iter;
            break;
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_CELL_SET("%d - %d", 0, dnx_data_l3.rif.nof_rifs_get(unit) - 1);
    PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);

    PRT_CELL_SET("%s", "L2_GPORT_GLOBAL_LIF");
    PRT_CELL_SET("%d - %d", dnx_data_l3.rif.nof_rifs_get(unit),
                 dnx_data_lif.global_lif.nof_global_l2_gport_lifs_get(unit) - 1);
    PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
    ...
    if ((dnx_data_lif.global_lif.nof_global_in_lifs_get(unit) - dnx_data_mpls.general.nof_php_gport_lif_get(unit) > 0)
        && ((dnx_data_lif.global_lif.nof_global_in_lifs_get(unit) - dnx_data_mpls.general.nof_php_gport_lif_get(unit)) <
    ...
    PRT_CELL_SET("%d - %d", range_start,
                 dnx_data_lif.global_lif.nof_global_in_lifs_get(unit) -
                 dnx_data_mpls.general.nof_php_gport_lif_get(unit) - 1);
...
static shr_error_e
dnx_global_lif_single_direction_tag_params_cmd(
    int unit,
    int global_lif,
    int direction)
{
    ...
    /** get the alloc_info from the global_lif */
    rv = dnx_algo_global_out_lif_alloc_info_get(unit, global_lif, direction, &alloc_info);
    if (rv != _SHR_E_NONE)
    {
        LOG_CLI((BSL_META_U
                 (unit, "\nSomething went wrong, dnx_algo_global_out_lif_alloc_info_get returned status %d\n"), rv));
        SHR_EXIT();
    }

    LOG_CLI((BSL_META_U(unit, " has the tag 0x%x with the parameters:\n"), tag));
    /** get the tag */
    dnx_global_lif_tag_set(unit, &alloc_info, &tag);

    if (alloc_info.outlif_intf != -1)
    {
        uint32 outlif_intf_size = dnx_data_lif.global_lif.global_lif_tag_nof_intf_bits_get(unit);
        LOG_CLI((BSL_META_U(unit, "\tOutlif Intf = %d, at bits %d to %d\n"), alloc_info.outlif_intf, offset,
                 (offset + outlif_intf_size)));
        offset += outlif_intf_size;
    }
    if (alloc_info.outlif_group != -1)
    {
        uint32 outlif_group_size = dnx_data_lif.global_lif.global_lif_tag_nof_group_bits_get(unit);
        LOG_CLI((BSL_META_U(unit, "\tOutlif Group = %d, at bits %d to %d\n"), alloc_info.outlif_group, offset,
                 (offset + outlif_group_size)));
        offset += outlif_group_size;
    }
    if (alloc_info.bta != -1)
    {
        uint32 bta_size = dnx_data_lif.global_lif.global_lif_tag_nof_bta_bits_get(unit);
        LOG_CLI((BSL_META_U(unit, "\tBTA = %d, at bits %d to %d\n"), alloc_info.bta, offset, (offset + bta_size)));
        offset += bta_size;
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_UINT32("ID", global_lif);
    SH_SAND_GET_ENUM("direction", dir);

    LOG_CLI((BSL_META_U(unit, "Global lif 0x%x\n"), global_lif));
    if ((dir == DNX_ALGO_LIF_INGRESS) || (dir == 0))
    {
        LOG_CLI((BSL_META_U(unit, "\tGlobal lif ingress info")));
        SHR_IF_ERR_EXIT(dnx_global_lif_single_direction_tag_params_cmd(unit, global_lif, DNX_ALGO_LIF_INGRESS));
    }

    if ((dir == DNX_ALGO_LIF_EGRESS) || (dir == 0))
    {
        LOG_CLI((BSL_META_U(unit, "\tGlobal lif egress info")));
        SHR_IF_ERR_EXIT(dnx_global_lif_single_direction_tag_params_cmd(unit, global_lif, DNX_ALGO_LIF_EGRESS));
    }

    if ((dir == (DNX_ALGO_LIF_INGRESS | DNX_ALGO_LIF_EGRESS)) || (dir == 0))
    {
        LOG_CLI((BSL_META_U(unit, "\tGlobal lif symmetric info")));
        SHR_IF_ERR_EXIT(dnx_global_lif_single_direction_tag_params_cmd(unit, global_lif,
...
static sh_sand_option_t sh_dnx_global_lif_tag_params_options[] = {
    {"ID", SAL_FIELD_TYPE_UINT32, "Global Lif", "-1"},
    {"Direction", SAL_FIELD_TYPE_ENUM, "Global Lif direction", "all", (void *) sh_dnx_global_lif_direction_enum_table},
    {NULL}
};
...
static sh_sand_man_t sh_dnx_global_lif_tag_params_man = {
    .brief = "Display global LIF allocation info parameters.",
    .examples = "ID=425543531\n" "Direction=ingress\n" "ID=0x1000 Direction=all"
};
...
static sh_sand_cmd_t sh_dnx_global_lif_cmds[] = {
    ...
    {"GlobalLIF", dnx_global_lif_tag_params_cmd, NULL, sh_dnx_global_lif_tag_params_options,
     &sh_dnx_global_lif_tag_params_man},
    {NULL}
};
