    ...
    bcm_gport_t * flow_gport)
{
    ...
    else
    {
        ...
        /** Check if it is Shared shaper*/
        SHR_IF_ERR_EXIT(dnx_sch_shared_shaper_mode_get(unit, core, flow_id, &mode));
        if (mode != DBAL_ENUM_FVAL_SHARED_SHAPER_MODE_NONE)
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "TM gport");
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "NOF COS");
        PRT_COLUMN_ADD("Type");
        PRT_COLUMN_ADD("Modid");
        PRT_COLUMN_ADD("FTMH PP DSP");
    ...
    uint32 *credit_rate_kbps)
{
    ...
    gtimer_time.time_units = DNXC_TIME_UNIT_SEC;
    ...
    uint32 *fc_percent)
{
    ...
    *fc_percent = 0;
    if (*fc_count)
    {
        ...
        timer_info.time_units = DNXC_TIME_UNIT_USEC;
        COMPILER_64_SET(timer_info.time, 0, 1000);
        SHR_IF_ERR_EXIT(dnxc_time_time_to_clock_cycles_get(unit, &timer_info, &nof_clock_cycles));
    ...
    int flow[2])
{
    ...
    int flow_region, base_flow_id, num_cos_levels, dummy_core, group_index;
    ...
    if (*is_composite)
    {
        ...
        group_index = DNX_SCH_QUARTET_GROUP_INDEX_FROM_REGION_INDEX_GET(flow_region);
...
sh_dnx_scheduler_voq_con_info_print(
    ...
    sh_sand_control_t * sand_control)
{
    int is_slow_enabled, is_enabled, dummy_core, is_low_rate;
    int cos = 0, flow_id;
    ...
    PRT_CELL_SET("%s", is_low_rate ? "TRUE" : "FALSE");
...
sh_dnx_scheduler_se_info_print(
    ...
    sh_sand_control_t * sand_control)
{
    int is_enabled = 0, is_composite = FALSE, dummy_core, flow_id, is_low_rate = FALSE;
    uint32 flags = 0;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (hr_idx = 0; hr_idx < num_priorities; hr_idx++)
    {
        ...
        SHR_IF_ERR_EXIT(bcm_cosq_control_get(unit, e2e_tc_gport, hr_idx, bcmCosqControlBandwidthBurstMax, &max_burst));
        PRT_CELL_SET("%u", kbits_sec_max);
        PRT_CELL_SET("%d", max_burst);
        SHR_IF_ERR_EXIT(bcm_cosq_control_get(unit, e2e_tc_gport, hr_idx, bcmCosqControlGroup, &group));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (fmq_class = 0; fmq_class < dnx_data_sch.general.nof_fmq_class_get(unit); fmq_class++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_sch_convert_flow_to_se_id(unit, core, fmq_flow_id, &se_id));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (soc_sand_is_emulation_system(unit) && print_status_provided)
    {
        cli_out("print_status cannot be performed on emulation");
        print_status_provided = 0;
    }
...
#ifdef INCLUDE_CTEST
/** Used in 8 priorities callbacks to store soc properties to be restored */
rhhandle_t diag_dnx_8_priorities_soc_h[SOC_MAX_NUM_DEVICES] = { NULL };
shr_error_e
cmd_dnx_8_priorities_init_cb(
    int unit)
{
     ...
     * Set SOC properties
     */
    SHR_IF_ERR_EXIT(ctest_dnxc_set_soc_properties(unit, ctest_soc_property, &diag_dnx_8_priorities_soc_h[unit]));
...
shr_error_e
cmd_dnx_8_priorities_deinit_cb(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);

    ctest_dnxc_restore_soc_properties(unit, diag_dnx_8_priorities_soc_h[unit]);
...
#ifdef INCLUDE_CTEST
        ,
    .init_cb = cmd_dnx_8_priorities_init_cb,
    .deinit_cb = cmd_dnx_8_priorities_deinit_cb
#endif
};
/**
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (priority_propagation_enable == FALSE && num_priorities >= dnx_data_sch.ps.min_priority_for_tcg_get(unit)
        && !dnx_data_sch.general.feature_get(unit, dnx_data_sch_general_tcg_mapping_hardcoded))
    {
        ...
        for (tcg_idx = 0; tcg_idx < num_priorities; tcg_idx++)
        {
            ...
            SHR_IF_ERR_EXIT(bcm_cosq_control_get
                            (unit, gport_info.out_gport, tcg_idx, bcmCosqControlBandwidthBurstMax, &max_burst));
            PRT_CELL_SET("%d", max_burst);
            ...
            for (tc_idx = 0; tc_idx < num_priorities; tc_idx++)
            {
                if (tc_to_tcg[tc_idx] == tcg_idx)
                {
                    ...
                    sal_strncat_s(tcg_to_tc_str, tcg_to_tc_str_buf, sizeof(tcg_to_tc_str));
                }
            }
    ...
    for (tc_idx = 0; tc_idx < num_priorities; tc_idx++)
    {
        ...
        SHR_IF_ERR_EXIT(bcm_cosq_control_get
                        (unit, gport_info.out_gport, tc_idx, bcmCosqControlBandwidthBurstMax, &max_burst));
...
/* *INDENT-OFF* */
    ...
    {"FABRIC_NORMAL",               DBAL_ENUM_FVAL_COUNT_TYPE_FABRIC_NORMAL,    "Count type fabric normal"},
    {NULL}
/* *INDENT-ON* */
};
...
/* *INDENT-OFF* */
    ...
    {"TYpe",            SAL_FIELD_TYPE_ENUM,    "msg to count",                                NULL,             (void *) sand_diag_fsm_cnt_type_enum},
    {NULL}
/* *INDENT-ON* */
};
...
sh_sand_man_t sh_dnx_scheduler_fsm_config_man = {
    ...
    .synopsis = "TYpe=fsm_type [id=flow_id]",
    .examples = "type=FABRIC_SLOW\n" "type=FABRIC_NORMAL id=136"
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_core_t usr_core;
    uint32 fabric_type;
    int flow_id, id_provided;
    int filter_by_flow_id = 0;
    int filter_by_type = 1;
...
sh_sand_man_t sh_dnx_scheduler_fsm_get_man = {
    .brief = "Get fms counter fabric type and flow ID",
    .full = "Get the fms counter fabric type and flow ID and if it uses filter",
    .synopsis = "",
    .examples = ""
};

shr_error_e
sh_dnx_scheduler_fsm_get_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
...
sh_sand_cmd_t sh_dnx_scheduler_fsm_count_cmds[] = {
    /*
     * keyword, action, command, options, man 
     */
    {"ConFiG", sh_dnx_scheduler_fsm_config_cmd, NULL, sh_dnx_scheduler_fsm_config_options,
     &sh_dnx_scheduler_fsm_config_man}
    ,
    {"Get", sh_dnx_scheduler_fsm_get_cmd, NULL, NULL, &sh_dnx_scheduler_fsm_get_man}
    ,
    {NULL}
};

sh_sand_man_t sh_dnx_scheduler_fsm_count_man = {
    .brief = "TM Scheduler FSM Count commands"
};
