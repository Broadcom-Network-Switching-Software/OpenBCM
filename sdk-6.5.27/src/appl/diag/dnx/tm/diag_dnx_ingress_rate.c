    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        SHR_IF_ERR_CONT(dnx_gtimer_clear(unit, SOC_BLK_IRE, core));
        ...
        if (dnx_data_device.general.feature_get(unit, dnx_data_device_general_ipc_exists))
        {
            DNX_ING_COUNTER_RATE_PRINT_64(counter_idx, "IPC Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                          counter_info.ipc_pkt_cnt, counter_info.ipc_pkt_ovf);
        }
        else
        {
...
/* *INDENT-OFF* */
    ...
    .full     = "Calculate and display IRE counter rate per interface type per core \n"
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        if (dnx_data_ingr_congestion.
            dbal.feature_get(unit, dnx_data_ingr_congestion_dbal_fabric_and_dram_separate_dequeue_pkt_counters))
        {
            DNX_ING_COUNTER_RATE_PRINT_64(counter_idx, "DE-QUEUE to DRAM Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                          counter_info.pkt_deq_dram_cnt, counter_info.deq_dram_pkt_ovf);

            DNX_ING_COUNTER_RATE_PRINT_64(counter_idx, "DE-QUEUE Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
                                          counter_info.pkt_deq_fabric_cnt, counter_info.deq_fabric_pkt_ovf);
        }
        else
        {
            DNX_ING_COUNTER_RATE_PRINT_64(counter_idx, "DE-QUEUE Packet Count", DNX_INGRESS_COUTNER_TYPE_PACKET,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 default_base_queue_id = 0, default_nof_cos = 0;
    dnxc_time_t gtimer_time;
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        /** Get the default counter parameters to restore */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_voq_programmable_counter_configuration_get
                        (unit, core, &default_base_queue_id, &default_nof_cos));
        ...
        /** Restore the programmable counter to track */
        SHR_IF_ERR_EXIT(dnx_ingress_congestion_voq_programmable_counter_configuration_set
                        (unit, core, default_base_queue_id, default_nof_cos));
