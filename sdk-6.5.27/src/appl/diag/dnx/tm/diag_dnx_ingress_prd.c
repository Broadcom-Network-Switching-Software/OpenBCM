...
#define DNX_DIAG_PORT_PRD_PRIORITY_STR_ASSEMBLE(prd_prio_str, prio_bmp, priority)  \
    do {                                                                           \
        ...
        sal_snprintf(tmp_str, sizeof(tmp_str), (*prio_bmp == 0) ? "%d" : ",%d", priority);\
    ...
    prt_control_t * prt_ctr)
{
     ...
     */
    sal_snprintf(priority_str, sizeof(priority_str), "%d", priority);
    ...
    int counter_id)
{
    ...
    *prio_bmp = 0;
    ...
    {
        sal_snprintf(counter_id_str, sizeof(counter_id_str), "AUTOMAPPED");

    }
    else if (counter_id >= 0)
    {
        sal_snprintf(counter_id_str, sizeof(counter_id_str), "%d", counter_id);
    }
    else
    {
        sal_snprintf(counter_id_str, sizeof(counter_id_str), "UNMAPPED");
    }
    ...
    if ((src_prio & BCM_PORT_F_PRIORITY_TDM) == src_prio)
    {
        ...
        sal_strncpy_s(prd_prio_str, "TDM", DNX_DIAG_PORT_PRD_MAX_STR_LEN);
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, 0))
        {
            sal_strncpy_s(prd_prio_str, "-", DNX_DIAG_PORT_PRD_MAX_STR_LEN);
        }

        /*
         * IF not printing the first priority group, skip port and header type columns.
         */
        if (prio_group_idx != 0)
        {
            sal_strncpy_s(port_str, "", DNX_DIAG_PORT_PRD_MAX_STR_LEN);
            sal_strncpy_s(header_str, "", DNX_DIAG_PORT_PRD_MAX_STR_LEN);
        }

        PRD_ROW_PRINT(port_str, header_str, prd_prio_str, sch_prio_str[sch_prio], fifo_size_str, "unlimited", drop_str);
        if (is_ofr)
        {
    ...
    else
    {
        SHR_BIT_ITER(prio_bmp, dnx_data_nif.prd.nof_priorities_get(unit), priority)
        {
            ...
            {
                sal_snprintf(threshold_str, sizeof(threshold_str), "%d - unlimited", priority);
            }
            else
            {
                sal_snprintf(threshold_str, sizeof(threshold_str), "%d - %u", priority, threshold);
            }
    ...
    prt_control_t * prt_ctr)
{
    ...
    else
    {
         ...
         * It is irrelevant which one of bcmPortNifSchedulerHigh or bcmPortNifSchedulerLow
         * will be used since the there is a single counter.
         */
    ...
    sh_sand_control_t * sand_control)
{
    ...
    BCM_PBMP_ITER(all_pbmp, port)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, 0)
            ...
            || (DNX_ALGO_PORT_TYPE_IS_ING_TM(unit, port_info) && DNX_ALGO_PORT_TYPE_IS_NIF_ETH_STIF(unit, port_info)))
        {
