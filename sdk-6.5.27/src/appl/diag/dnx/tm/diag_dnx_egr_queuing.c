    ...
    dnx_egq_counter_rate_info_t * rate_info)
{
    ...
    else if (EGQ_NOT_INCLUDED_VAL != port)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_egr_queuing_pqp_counter_configuration_set
                        (unit, core_used, DNX_EGQ_COUTNER_FILTER_BY_OTM, port));
    }
    else
    {
        core_used = core;
    ...
    int *nof_counter)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_out_tm_port_get(unit, port, &core_used, &otm));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_out_tm_port_get(unit, logical_port, &core, &otm));
