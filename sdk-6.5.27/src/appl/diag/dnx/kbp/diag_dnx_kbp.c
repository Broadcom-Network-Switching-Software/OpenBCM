...
#include "src/bcm/dnx/kbp/kbp_mngr_internal.h"
...
{                                                                     \
int iter = 0, iter_num = sal_strnlen(header,SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)+sal_strnlen(name,SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);       \
LOG_CLI((BSL_META("\n%s %s\n"), header, name));                     \
    ...
    rhlist_t * test_list)
{
    ...
    {   /** one of supported KBP devices must be defined */
        ...
        if ((kbp_device_type == NULL) ||
            (sal_strncasecmp(kbp_device_type, "BCM52311", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
             && sal_strncasecmp(kbp_device_type, "BCM52321", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
        {
            SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
        }
        else
        {   /** if kbp_blackhole defined disable command */
            ...
            if ((kbp_blackhole != NULL)
                && sal_strncasecmp(kbp_blackhole, "0", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            {
                SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
            }
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (sal_strncmp(file_path, "stdout", sal_strnlen(file_path, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
    {
...
dnx_kbp_diag_sdk_ver_cmd(
    ...
    sh_sand_control_t * sand_control)
{
    const char *ver_str_supported[] = { "KBP SDK 1.5.18_EA1", "KBP SDK 1.5.18" };
    const char *ver_str = NULL;
    int nof_supported_versions = sizeof(ver_str_supported) / sizeof(ver_str_supported[0]);
    ...
    {
        int ver_iter;
        uint8 version_supported = FALSE;
        int longest_length = 0;
        int version_end = 0;
        /** Expecting "KBP SDK " to be always present in the version string. */
        int ver_str_start_offset = sal_strnlen("KBP SDK ", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
        int ver_str_len = sal_strnlen(ver_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
        /** Read the version string until invalid whitespace or end of string is found */
        for (version_end = ver_str_start_offset; version_end < ver_str_len; version_end++)
        {
            if ((ver_str[version_end] == ' ') || (ver_str[version_end] == '\0'))
            {
                break;
            }
        }
        for (ver_iter = 0; ver_iter < nof_supported_versions; ver_iter++)
        {
            /** Take the longest of the two version strings */
            /** Use version_end to dismiss all symbols appended to the version (e.g. date of version delivery) */
            longest_length =
                sal_strnlen(ver_str, version_end) > sal_strnlen(ver_str_supported[ver_iter],
                                                                SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ?
                sal_strnlen(ver_str, version_end) : sal_strnlen(ver_str_supported[ver_iter],
                                                                SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
            if (sal_strncmp(ver_str, ver_str_supported[ver_iter], longest_length) == 0)
            {
        ...
        {
            char ver_str_all[SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH + 1] = { 0 };
            for (ver_iter = 0; ver_iter < nof_supported_versions; ver_iter++)
            {
                sal_strncat_s(ver_str_all, " \"", 3);
                sal_strncat_s(ver_str_all, ver_str_supported[ver_iter],
                              sal_strnlen(ver_str_supported[ver_iter], SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH));
                sal_strncat_s(ver_str_all, "\"", 2);
            }
    ...
    for (index = 0; index < nof_segments; index++)
    {
        ...
        if (!build_overlay_fields && !DNX_KBP_KEY_IS_OVERLAY_FIELD(overlay_offset_bytes))
        {
            sal_snprintf(master_key_segment, (DNX_KBP_DIAG_PRINTABLE_KEY_SEGMENT_SIZE - 1), "<%s %d %s> ", segment_name,
                         segment_bytes, segment_type_string);

            sal_strncat_s(master_key, master_key_segment,
                          (DNX_KBP_MAX_NOF_SEGMENTS_PER_MASTER_KEY * DNX_KBP_DIAG_PRINTABLE_KEY_SEGMENT_SIZE));
        }
        else if (build_overlay_fields && DNX_KBP_KEY_IS_OVERLAY_FIELD(overlay_offset_bytes))
        {
            sal_snprintf(master_key_segment, (DNX_KBP_DIAG_PRINTABLE_KEY_SEGMENT_SIZE - 1), "<%s %d %s %d> ",
                         segment_name, segment_bytes, segment_type_string, overlay_offset_bytes);
    ...
    char db_key[DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP * DNX_KBP_DIAG_PRINTABLE_KEY_SEGMENT_SIZE])
{
    ...
    for (index = 0; index < nof_segments; index++)
    {
        ...
        sal_strncat_s(db_key, db_key_segment,
                      (DNX_KBP_MAX_NOF_SEGMENTS_PER_LOOKUP * DNX_KBP_DIAG_PRINTABLE_KEY_SEGMENT_SIZE));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    dbal_enum_value_field_kbp_device_status_e kbp_mngr_status;
    uint8 db_id[DNX_DATA_MAX_ELK_GENERAL_NOF_DBS_PER_CORE] = { 0 };
    ...
    while (!is_end)
    {
        ...
        for (index = 0; index < DNX_KBP_MAX_NOF_LOOKUPS; index++)
        {
            ...
            for (db_index = 0; db_index < DNX_KBP_NOF_DBS_PER_CORE; db_index++)
            {
                SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                                (unit, lkp_entry_handle_id, DBAL_FIELD_LOOKUP_DB, db_index, &db_id[db_index]));
                /** Check if there are more than one DBs per core and they are different */
                if ((db_index > 0) && (db_id[db_index] != db_id[db_index - 1]))
                {
                    print_per_core_db = TRUE;
                }
            }
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
            ...
            {
                dbal_entry_key_field8_set(unit, db_entry_handle_id, DBAL_FIELD_DB_ID, db_id[0]);
                ...
                if (!print_per_core_db)
                {
                    PRT_CELL_SET("%d", db_id[0]); /** DB ID */
                }
                else
                {
                    PRT_CELL_SET("%d/%d", db_id[0], db_id[1]); /** DB ID + DB ID on second core */
                }
                PRT_CELL_SET("%s", dbal_logical_table_to_string(unit, table_id)); /** DB Name */
                PRT_CELL_SET("%s", lookup_type_string); /** Type */
    ...
    sh_sand_control_t * sand_control)
{
    ...
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Result Width (large optimized)");
    ...
    PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Range Expansion");
    ...
    while (!is_end)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                        (unit, db_entry_handle_id, DBAL_FIELD_LARGE_OPTIMIZED_WIDTH, INST_SINGLE,
        ...
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, db_entry_handle_id, DBAL_FIELD_DB_IS_DPC, INST_SINGLE, &is_dpc));
        SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                        (unit, db_entry_handle_id, DBAL_FIELD_ACL_DB_AS_LPM, INST_SINGLE, &acl_db_as_lpm));
        ...
        {
            SHR_IF_ERR_EXIT(dbal_entry_handle_value_field8_get
                            (unit, db_entry_handle_id, DBAL_FIELD_DB_CORE_ID, INST_SINGLE, &core_id));
        ...
        if (large_optimized_width)
        {
            PRT_CELL_SET("%d", large_optimized_width);
        }
        else
        {
            PRT_CELL_SET("%s", "N/A");
        ...
        PRT_CELL_SET("%s%s", dbal_table_type_to_string(unit, type), acl_db_as_lpm ? " as LPM" : "");
        PRT_CELL_SET("%s", dbal_logical_table_to_string(unit, table_id));
        ...
        if (kbp_mngr_status == DBAL_ENUM_FVAL_KBP_DEVICE_STATUS_LOCKED)
        {
            SHR_IF_ERR_EXIT(kbp_mngr_db_capacity_get
                            (unit, db_id, &num_of_entries, &estimated_capacity, &range_expansion));
            PRT_CELL_SET("%d", num_of_entries);
            PRT_CELL_SET("%d", estimated_capacity);
            PRT_CELL_SET("%d", range_expansion);
        }
        else
        {
            PRT_CELL_SET("%s", "N/A");
    ...
    char payload_string[DBAL_FIELD_ARRAY_MAX_SIZE_IN_BYTES * 2 + 1])
{
    ...
    for (index = 0; index < rop_packet->payload_len; index++)
    {
        ...
        sal_strncat_s(payload_string, tmp_byte, (DBAL_FIELD_ARRAY_MAX_SIZE_IN_BYTES * 2 + 1));
    }
}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    DNXCMN_CORES_ITER(unit, core_id, core)
    {
        ...
        for (offset = 0; offset < DNX_KBP_ROP_RECORDS_PER_CORE(unit); offset++)
        {
            if (request)
            {
                SHR_IF_ERR_EXIT(dnx_kbp_rop_last_request_get
                                (unit, core, offset, DNX_KBP_ROP_GET_DNX_SIDE, &rop_packet));
            ...
            if (response)
            {
                SHR_IF_ERR_EXIT(dnx_kbp_rop_last_response_get
                                (unit, core, offset, DNX_KBP_ROP_GET_DNX_SIDE, &rop_packet));
                dnx_kbp_diag_rop_last_payload_to_string(&rop_packet, payload_string);
                SHR_IF_ERR_EXIT(dnx_kbp_diag_opcode_string(unit, rop_packet.opcode, opcode_str));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_STR("PATH", file_path);
    SH_SAND_GET_UINT32("DeViCe", device_id);

    if (device_id != 0)
    {
        if (!DNX_KBP_IS_SECOND_KBP_EXIST || (DNX_KBP_IS_SECOND_KBP_EXIST && (device_id != 1)))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Device %d is not available on the current configuration\n", device_id);
        }
    }

    if (sal_strncmp(file_path, "stdout", sal_strnlen(file_path, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) != 0)
    {
        kbp_file = fopen(file_path, "w");
    }
    else
    ...
    DNX_KBP_TRY(kbp_device_interface_print_regs
                (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER], Kbp_app_data[unit]->flags,
                 &Kbp_app_data[unit]->device_config[device_id], kbp_file));
    if (sal_strncmp(file_path, "stdout", sal_strnlen(file_path, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) != 0)
    {
        fclose(kbp_file);
    }
    ...
    char master_key[DNX_KBP_MAX_NOF_SEGMENTS_PER_MASTER_KEY * DNX_KBP_DIAG_PRINTABLE_KEY_SEGMENT_SIZE])
{
    ...
    for (index = 0; index < nof_segments; index++)
    {
        ...
        char master_key_segment[DNX_KBP_DIAG_PRINTABLE_KEY_SEGMENT_SIZE] = {
            0
        };
        SHR_IF_ERR_EXIT(kbp_mngr_master_key_segment_get
                        (unit, opcode_id, index, &segment_type, &segment_bytes, segment_name, &overlay_offset_bytes));
        if ((sal_strncmp
             (segment_name, "ZERO_PADDING",
              sal_strnlen("ZERO_PADDING", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) == 0)
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SH_SAND_GET_ITERATOR(sand_arg)
    {
        ...
        for (index = 0; index < nof_master_key_segments; index++)
        {
            ...
            if (DNX_KBP_KEY_IS_OVERLAY_FIELD(overlay_offset_bytes) ||
                (sal_strncmp
                 (segment_name, arg_name, sal_strnlen(arg_name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) == 0))
            {
    ...
    LOG_CLI((BSL_META("kbp search result for opcode %s is:\n"), opcode_name));

    for (index = 0; index < DNX_KBP_MAX_NOF_RESULTS; index++)
    {
        if (search_rslt.result_valid[index])
        {
            if (search_rslt.hit_or_miss[index])
            {
                int iter;
                uint8 result_size[1];
                ...
                LOG_CLI((BSL_META("Result %d HIT: "), index));
                for (iter = 0; iter < BITS2BYTES(result_size[0]); iter++)
                {
                    LOG_CLI((BSL_META("%02X"), search_rslt.assoc_data[index][iter]));
                }
                LOG_CLI((BSL_META(" / %db\n"), result_size[0]));
            }
            else
            {
                LOG_CLI((BSL_META("Result %d MISS\n"), index));
            }
        }
    }
exit:
    ...
    sh_sand_control_t * sand_control)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_access_info_get(unit, table_id, DBAL_ACCESS_METHOD_KBP, (void *) kbp_db_handles));
    ...
    SHR_IF_ERR_EXIT(dbal_iterator_init(unit, db_entry_handle_id, DBAL_ITER_MODE_GET_ALL_EXCEPT_DEFAULT));
    SHR_IF_ERR_EXIT(dbal_iterator_value_field_arr8_rule_add
                    (unit, db_entry_handle_id, DBAL_FIELD_VALID, INST_SINGLE, DBAL_CONDITION_EQUAL_TO,
                     &is_valid, &is_valid));
    SHR_IF_ERR_EXIT(dbal_iterator_get_next(unit, db_entry_handle_id, &is_end));
    while (!is_end)
    {
    ...
    if (nof_dbs_found == 0)
    {
        SHR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "DB [%s] not found in KBP DBs %s%s", db_name, EMPTY, EMPTY);
    }
    if (nof_dbs_found > DNX_KBP_NOF_DBS_PER_CORE)
    {
        SHR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "Found %d KBP DBs associated to DB %s Cannot be more than %d",
                          nof_dbs_found, db_name, DNX_KBP_NOF_DBS_PER_CORE);
    }
    SHR_IF_ERR_EXIT(dbal_tables_core_mode_get(unit, table_id, &core_mode));
    if ((core_mode == DBAL_CORE_MODE_SBC) && (nof_dbs_found != 1))
    {
        SHR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "Expected to find 1 KBP DB associated to DB %s but found %d %s",
                          db_name, nof_dbs_found, EMPTY);
    }
    if ((core_mode == DBAL_CORE_MODE_DPC) && (nof_dbs_found != DNX_KBP_NOF_DBS_PER_CORE))
    {
        SHR_EXIT_WITH_LOG(_SHR_E_INTERNAL, "Expected to find %d KBP DB associated to DB %s but found %d",
                          DNX_KBP_NOF_DBS_PER_CORE, db_name, nof_dbs_found);
    }
    for (db_iter = 0; db_iter < nof_dbs_found; db_iter++)
    {
        /** Initiate counters read */
        DNX_KBP_TRY(kbp_db_counter_read_initiate(kbp_db_handles[db_iter].db_p));
        do
        {
            DNX_KBP_TRY(kbp_db_is_counter_read_complete(kbp_db_handles[db_iter].db_p, &is_complete));
        }
        while (is_complete == 0);
    }
...
/* { */
    {
        ...
        while (!is_end)
        {
            ...
            DNX_KBP_TRY(kbp_entry_get_counter_value(kbp_db_handles[core_id].db_p, kbp_entry, &value));
            if (value > 0)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
                if (nof_dbs_found > 1)
                ...
                PRT_CELL_SET(COMPILER_64_PRINTF_FORMAT, COMPILER_64_PRINTF_VALUE(value));
            }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (lane_is_present)
    {
        ...
        DNX_KBP_TRY(kbp_device_interface_per_lane_prbs
                    (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER], Kbp_app_data[unit]->flags,
                     &Kbp_app_data[unit]->device_config[device_id], poly, 1, lane_bmp));
        cli_out("Clearing PRBS counters, please ignore!\n");
        DNX_KBP_TRY(kbp_device_interface_per_lane_prbs_print
                    (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER], Kbp_app_data[unit]->flags,
                     &Kbp_app_data[unit]->device_config[device_id], lane_bmp));
    }
    else
    {
        cli_out("Enabling PRBS, PRBS poly %d\n", poly);
        DNX_KBP_TRY(kbp_device_interface_prbs
                    (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER], Kbp_app_data[unit]->flags,
                     &Kbp_app_data[unit]->device_config[device_id], poly, 1));
        cli_out("Clearing PRBS counters, please ignore!\n");
        DNX_KBP_TRY(kbp_device_interface_prbs_print
                    (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER], Kbp_app_data[unit]->flags,
                     &Kbp_app_data[unit]->device_config[device_id]));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (lane_is_present)
    {
        ...
        DNX_KBP_TRY(kbp_device_interface_per_lane_prbs
                    (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER], Kbp_app_data[unit]->flags,
                     &Kbp_app_data[unit]->device_config[device_id], poly, 0, lane_bmp));
    }
    else
    {
        DNX_KBP_TRY(kbp_device_interface_prbs
                    (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER], Kbp_app_data[unit]->flags,
                     &Kbp_app_data[unit]->device_config[device_id], poly, 0));
    }
exit:
    SHR_FUNC_EXIT;
}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (lane_is_present)
    {
        ...
        DNX_KBP_TRY(kbp_device_interface_per_lane_diag_print
                    (Kbp_app_data[unit]->device_type[DNX_KBP_INST_MASTER], Kbp_app_data[unit]->flags,
                     &Kbp_app_data[unit]->device_config[0], lane_bmp));
    }
    else
    {
...
static sh_sand_man_t dnx_kbp_diag_sdk_ver_man = {
    .brief = "Prints and verifies the KBPSDK version"
};
...
static sh_sand_option_t dnx_kbp_diag_status_options[] = {
    ...
    {"DeViCe", SAL_FIELD_TYPE_UINT32, "Device ID", "0"},
    {NULL}
};
...
sh_sand_cmd_t sh_dnx_kbp_cmds[] = {
    ...
    {"SYNC",       dnx_kbp_diag_sync_cmd,      NULL,                     NULL,                            &dnx_kbp_diag_sync_man, NULL, NULL, SH_CMD_SKIP_MEMORY_CHECK},
    {NULL}
};
