...
#define DNX_DRAM_FIRMWARE_AAPL_COMMON_OPTION_LIST \
    ...
    {"debug",                   SAL_FIELD_TYPE_BOOL,     "Toggles debug level prints",                              "FALSE",        NULL,                                                   NULL,    SH_SAND_ARG_QUIET}
/* *INDENT-ON* */
    ...
    dnx_hbmc_firmware_aapl_params_t * aapl_params)
{
    ...
    SH_SAND_GET_BOOL("debug", aapl_params->debug);
...
{
    uint32 dram_start = 0;
    uint32 dram_end = DNX_DATA_MAX_DRAM_GENERAL_INFO_MAX_NOF_DRAMS - 1;
    ...
    if (!SAL_BOOT_PLISIM)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_hbmc_cattrip_indication_disable(unit, dram_start, dram_end, should_reenable_cattrip));
        SHR_IF_ERR_EXIT(dnx_hbmc_firmware_full_aapl_diagnostics_run
                        (unit, dram_start, dram_end, &aapl_params, cmd_return_code_array));
    }
