...
#ifdef INCLUDE_AAPL
#include "diag_dnx_dram_firmware_aapl.h"
#endif
...
static sh_sand_enum_t sand_diag_dram_firmware_operation_enum[] = {
    ...
    {"hbm_read_samsung_device_sn",       DNX_HBMC_FIRMWARE_OPERATION_HBM_READ_SAMSUNG_DEVICE_SN,      "#"},
    ...
    {"hbm_begin_collection_lane_failures",  DNX_HBMC_FIRMWARE_OPERATION_HBM_BEGIN_COLLECTING_LANE_FAILURES, "#"},
    {"hbm_convert_lane_failures_to_soft_repairs",  DNX_HBMC_FIRMWARE_OPERATION_HBM_CONVERT_LANE_FAILURES_TO_SOFT_REPAIRS, "#"},
    {"hbm_comp_lane_repair",            DNX_HBMC_FIRMWARE_OPERATION_HBM_COMP_LANE_REPAIR,             "#"},
    {"hbm_dword_write",                 DNX_HBMC_FIRMWARE_OPERATION_HBM_DWORD_WRITE,                  "#"},
    {"hbm_dword_read",                  DNX_HBMC_FIRMWARE_OPERATION_HBM_DWORD_READ,                   "#"},
    {"hbm_convert_repairs_to_mode2",    DNX_HBMC_FIRMWARE_OPERATION_HBM_CONVERT_REPAIRS_TO_MODE2,     "#"},
    {NULL}
};
...
static sh_sand_man_t sh_dnx_firmware_operation_man = {
    ...
    .examples = "operation=0x0\n"
                "dram=0 operation=reset_phy_and_hbm\n"
                "channel=0 operation=bypass_test"
};
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int should_reenable_cattrip[DNX_DATA_MAX_DRAM_GENERAL_INFO_MAX_NOF_DRAMS] = { 0 };
    uint32 dram_end = DNX_DATA_MAX_DRAM_GENERAL_INFO_MAX_NOF_DRAMS;
    uint32 dram_start = 0;
    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (operation == DNX_HBMC_FIRMWARE_OPERATION_LANE_REPAIR)
    {
        SHR_IF_ERR_EXIT(dnx_hbmc_cattrip_indication_disable
                        (unit, dram_start, dram_end, should_reenable_cattrip));
    }
    ...
    for (dram_index = dram_start; dram_index <= dram_end; ++dram_index)
    {
        ...
        switch (operation)
        {
            ...
            /** read Samsung a 34-bit unique identifier for an HBM */
            case DNX_HBMC_FIRMWARE_OPERATION_HBM_READ_SAMSUNG_DEVICE_SN:
            {
                uint32 sn_low;
                if (dram_index == dram_start)
                {
                    PRT_TITLE_SET("Samsung Serial Number");
                    PRT_COLUMN_ADD("Dram");
                    PRT_COLUMN_ADD("SN");
                }
                PRT_ROW_ADD(PRT_ROW_SEP_UNDERSCORE);
                PRT_CELL_SET("0x%x", dram_index);
                sn_low = (hbm_spare.hbm_spare_data[1] << 16) | hbm_spare.hbm_spare_data[0];
                PRT_CELL_SET("0x%x%x", hbm_spare.hbm_spare_data[2], sn_low);

                if (dram_index == dram_end)
                {
                    PRT_COMMITX;
                }
                break;
...
exit:
    SHR_IF_ERR_CONT(dnx_hbmc_cattrip_indication_restore(unit, dram_start, dram_end, should_reenable_cattrip));
    PRT_FREE;
    SHR_FUNC_EXIT;
}
...
static sh_sand_enum_t sand_diag_dram_firmware_parameter_enum[] = {
    ...
    {"hbm_lane_repair_options",                  DNX_HBMC_FIRMWARE_PARAM_HBM_LANE_REPAIR_OPTIONS,                "#"},
    ...
    {"hbm_comp_lane_repair_options",             DNX_HBMC_FIRMWARE_PARAM_HBM_COMP_LANE_REPAIR_OPTIONS,           "#"},
    {"hbm_mmt_rddata_en",                        DNX_HBMC_FIRMWARE_PARAM_HBM_MMT_RDDATA_EN,                      "#"},
    {"hbm_mmt_row_range",                        DNX_HBMC_FIRMWARE_PARAM_HBM_MMT_ROW_RANGE,                      "#"},
    {NULL}
};
...
static sh_sand_man_t sh_dnx_firmware_parameter_set_man = {
    ...
    .examples = "parameter=0x0 value=0x0\n"
                "dram=0 parameter=hbm_reserved value=0x0"
};
...
#ifdef INCLUDE_AAPL
    {"AAPL",        NULL,                                 sh_dnx_dram_firmware_aapl_cmds,        NULL,                                  &sh_dnx_dram_firmware_aapl_man},
#endif
    {NULL}
};
