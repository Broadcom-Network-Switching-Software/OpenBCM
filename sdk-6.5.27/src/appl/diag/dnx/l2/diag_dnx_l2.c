    ...
    void *user_data)
{
    ...
    uint32 result_type;
    uint8 is_valid_dest;
    uint8 separate_fwd_lrn_mact = dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_separate_fwd_learn_mact);
...
sh_dnx_l2_show_cmd(
    ...
    sh_sand_control_t * sand_control)
{
    int svl_flag = FALSE, ivl_flag = FALSE;
    int traverse_flags;
    int nof_entries;
    PRT_INIT_VARS;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    uint32 fwd_action = 1;
    uint32 cores_group = BCM_CORES_GROUP_ALL;
    uint8 separate_fwd_lrn_mact = dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_separate_fwd_learn_mact);
    uint8 is_ivl_supported = dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_ivl_feature_support);
    ...
    if ((cores_group != BCM_CORES_GROUP_ALL) && (cores_group >= DBAL_NOF_CORE_GROUPS))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    if (separate_fwd_lrn_mact)
    {
        l2addr.flags2 |= fwd_action ? BCM_L2_FLAGS2_FWD_ONLY : BCM_L2_FLAGS2_LEARN_ONLY;
    }
    if (nof_core_groups > 1)
    {
        l2addr.cores_group = cores_group;
    }
 ...
 * If the MACT is separated, the entry can be deleted from MACT_DA or MACT_SA.
 */
static shr_error_e
    ...
    uint32 *l2_entry_counter_p)
{
    ...
    uint32 core_group_id = 0;
    dbal_tables_e table_id = DBAL_TABLE_MACT_MANAGEMENT_GENERAL_CONFIGURATION;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_set(unit, entry_handle_id, DBAL_ENTRY_ATTR_CORE_GROUP, core_group_id));
    }
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
...
sh_sand_man_t sh_dnx_l2_show_man_w_ivl = {
...
sh_sand_man_t sh_dnx_l2_show_man_wo_ivl = {
    .brief = "Show l2 address table",
    .full = "Show the entries from SVL and IVL MACT tables",
    .examples = "",
    .init_cb = diag_sand_packet_send_simple,
...
sh_sand_cmd_t sh_dnx_l2_cmds[] = {
    ...
    ,
    {"show", sh_dnx_l2_show_cmd, NULL, dnx_l2_show_options, &sh_dnx_l2_show_man_w_ivl, NULL, NULL, SH_CMD_CONDITIONAL,
     sh_cmd_ivl_svl_supported}
    ,
    {"show", sh_dnx_l2_show_cmd, NULL, NULL, &sh_dnx_l2_show_man_wo_ivl, NULL, NULL, SH_CMD_CONDITIONAL,
     sh_cmd_ivl_svl_not_supported}
    ,
