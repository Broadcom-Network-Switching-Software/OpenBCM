    ...
    uint32 *lif_flags)
{
    ...
    if (gport != BCM_GPORT_INVALID)
    {
        ...
        flags = (BCM_GPORT_IS_TUNNEL(gport) && BCM_GPORT_SUB_TYPE_IS_VIRTUAL_EGRESS_POINTED(gport)) ?
            DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF : DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_AND_GLOBAL_LIF;
        flags |= DNX_ALGO_GPM_GPORT_HW_RESOURCES_NON_STRICT | DNX_ALGO_GPM_GPORT_HW_RESOURCES_VIRTUAL_EGRESS;
        SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_to_hw_resources(unit, gport, flags, &lif_hw_resources));

        is_egress_virtual = (BCM_GPORT_IS_VLAN_PORT(gport) || BCM_GPORT_IS_FLOW_LIF(gport)) &&
    ...
    else
    {
        ...
        if (is_ingress)
        {
            ...
            {
                dbal_physical_tables_e dbal_physical_table_inner;
                {
                    dbal_physical_table_inner = DBAL_PHYSICAL_TABLE_INLIF_2;
                }
                rv = dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get(unit, local_lif, _SHR_CORE_ALL,
                                                                             dbal_physical_table_inner,
                                                                             &(lif_hw_resources.inlif_dbal_table_id),
                                                                             &
                                                                             (lif_hw_resources.inlif_dbal_result_type));
            }
        ...
        if (is_egress)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get(unit, local_lif, &(lif_hw_resources.outlif_dbal_table_id),
                                                            ...
                                                            &(lif_hw_resources.logical_phase), NULL, NULL, NULL));
        }
    }
    ...
    if (fec_hw_resources.fec_id == DNX_ALGO_GPM_FEC_INVALID
        && lif_hw_resources.local_in_lif == DNX_ALGO_GPM_LIF_INVALID
        && lif_hw_resources.local_out_lif == DNX_ALGO_GPM_LIF_INVALID && !is_egress_virtual)
    {
    ...
    if (is_ingress)
    {
        if (lif_hw_resources.local_in_lif != DNX_ALGO_GPM_LIF_INVALID)
        {
            ...
            if (BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS_NATIVE(gport) || BCM_GPORT_SUB_TYPE_IS_VIRTUAL_INGRESS(gport))
            {
                LOG_CLI((BSL_META("Virtual id:0x%08X -> "), lif_hw_resources.virtual_id));
            }
            for (int idx = 0; idx < lif_hw_resources.local_in_lif_per_core.nof_in_lifs; idx++)
            {
                char core_str[8] = "ALL";
                if (lif_hw_resources.local_in_lif_per_core.core_id[0] != _SHR_CORE_ALL)
                {
                    sal_itoa(core_str, lif_hw_resources.local_in_lif_per_core.core_id[idx], 10, 0, 0);
                }
                LOG_CLI((BSL_META("\nLocal inlif:0x%08X, core_idx=%s"),
                         lif_hw_resources.local_in_lif_per_core.local_in_lif[idx], core_str));
                /*
                 * For core all - no need to print the info for all cores 
                 */
                if (lif_hw_resources.local_in_lif_per_core.core_id[0] == _SHR_CORE_ALL)
                {
                    break;
            ...
            for (int idx = 0; idx < lif_hw_resources.local_in_lif_per_core.nof_in_lifs; idx++)
            {
                sal_snprintf(shell_commands_buffer, CHAR_BUFF_SIZE, "%s%s core_id=%d in_lif=%d result_type=%s",
                             dbal_entry_get_command, logical_table_str,
                             (lif_hw_resources.local_in_lif_per_core.core_id[0] == BCM_CORE_ALL ?
                              0 : lif_hw_resources.local_in_lif_per_core.core_id[idx]),
                             lif_hw_resources.local_in_lif, result_type_str);
                SHR_IF_ERR_EXIT(sh_process_command(unit, shell_commands_buffer));
                if (lif_hw_resources.local_in_lif_per_core.core_id[0] == BCM_CORE_ALL)
                {
                    /*
                     * No need to print for all cores when it's symmetric 
                     */
                    break;
                }
            }
    ...
    if (is_egress)
    {
        ...
        if (lif_hw_resources.local_out_lif != DNX_ALGO_GPM_LIF_INVALID)
        {
            ...
            LOG_CLI((BSL_META("Logical phase:0x%08X -> phase:%d\n"), lif_hw_resources.local_out_lif,
                     lif_hw_resources.logical_phase + 1));
            LOG_CLI((BSL_META("Table: %s"), logical_table_str));
            /*
        ...
        else if (is_egress_virtual)
        {
            ...
            {
                /** key gport information is not supported when working with flow user should use flow diag for entry get  */
                SHR_IF_ERR_EXIT(sh_process_command(unit, shell_commands_buffer));
                SHR_EXIT();
            }
            ...
            switch (res_type)
            {
                ...
                case DBAL_RESULT_TYPE_EGRESS_VIRTUAL_GPORT_TO_MATCH_INFO_SW_ESEM_NAMESPACE_VSI:
                {
                    ...
                    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                                    (unit, sw_table_handle_id, DBAL_FIELD_NETWORK_DOMAIN, INST_SINGLE,
                                     &match_class_id));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    is_ingress = sal_strncaseindex(direction, "i", SH_SAND_MAX_TOKEN_SIZE, sizeof("i"))
        || sal_strncaseindex(direction, "b", SH_SAND_MAX_TOKEN_SIZE, sizeof("b"));
    is_egress = sal_strncaseindex(direction, "o", SH_SAND_MAX_TOKEN_SIZE, sizeof("o"))
        || sal_strncaseindex(direction, "b", SH_SAND_MAX_TOKEN_SIZE, sizeof("b"));
    ...
    if (is_ingress)
    {
        if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_phy_db_dpc) && is_dpc == -1)
        {
            /**In case no DPC input and the device support DPC inLIF only, force DPC.*/
            is_dpc = 1;
        }
        if (is_global_lif)
        {
