...
sal_mutex_t dnx_calibrate_lock = NULL;
#endif
...
#if defined(BCM_EDK_SUPPORT) && defined(EDKHLIB) && !defined(BCM_LTSW_SUPPORT)
...
char *env_last_entry=NULL;
char **environ=&env_last_entry;
...
_display_current_shell_mode(int unit)
{
    ...
    switch (command_mode_get(unit)) {
        ...
        case DNXF_CMD_MODE:
            ...
            break;
        case LTSW_CMD_MODE:
            sal_strncpy(mode ,"LTSW", sizeof(mode));
            break;
        case CTSW_CMD_MODE:
            sal_strncpy(mode ,"CTSW", sizeof(mode));
            break;
        default:
...
char shell_mode_usage[] =
  "\n    mode <m> where m is ESW, LTSW or CTSW\n";
...
cmd_mode(int unit, args_t *a)
{
...
#if defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
    /* Try to get the NPL version from flexcode if loaded. */
    (void)soc_ltsw_is_cna(unit, &is_cna);
#endif
    ...
    if ((subcmd = ARG_GET(a)) == NULL) { /* Swap modes. */
        ...
        for (new_mode = curr_mode+1; new_mode != curr_mode; new_mode++) {
            if (new_mode > CTSW_CMD_MODE) {
                /*
                 * If the loop has gone beyond CTSW_CMD_MODE, which is
            ...
            if (!((ESW_CMD_MODE == new_mode) && SOC_IS_ESW(unit)) ||
                !((LTSW_CMD_MODE == new_mode) && SOC_IS_LTSW(unit) && !is_cna)||
                !((CTSW_CMD_MODE == new_mode) && SOC_IS_LTSW(unit) && is_cna)) {
                /*
        ...
        } else if ((sal_strcasecmp(subcmd, "LTSW") == 0)) {
            if (!SOC_IS_LTSW(unit)) {
                sal_printf("LTSW Mode is not supported on non-LTSW devices \n");
                return CMD_USAGE;
            }
            command_mode_set(unit, LTSW_CMD_MODE);
        } else if ((sal_strcasecmp(subcmd, "CTSW") == 0)) {
            if (!SOC_IS_LTSW(unit)) {
                sal_printf("CTSW Mode is not supported on non-LTSW devices \n");
                return CMD_USAGE;
            }
...
/*
 * Orderly stop things that must not stop in the middle.
 * This would currently be CPU2TAP.
 */
static void diag_mandatory_cleanup(void)
{
#ifdef BCM_DNX_SUPPORT
    int d, unit;

    /* loop over all devices/units, stopping CPU2TAP if needed, not in the middle of a script */
    for (d = 0; d < soc_ndev; d++) {
        unit = SOC_NDEV_IDX2DEV(d);
        if (BCM_SUCCESS(bcm_attach_check(unit)) && SOC_IS_DNX(unit))
        {
            /* Set global CPU2TAP disable, and wait for CPU2TAP for unit to be disabled. */
            /* This assures the unit will not be in the middle of a CPU2TAP sequence when exiting */
            int rv = appl_dnxc_clean_up(unit);
            if (BCM_FAILURE(rv))
            {
                LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "appl_dnxc_clean_up() failed: %s\n"), soc_errmsg(rv)));
            }
        }
    }
...
#if defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT)
    if (dnx_calibrate_lock != NULL)
    {
        sal_mutex_destroy(dnx_calibrate_lock);
        dnx_calibrate_lock = NULL;
    }
#endif
...
#if !defined(BCM_DNXF_SUPPORT) && !defined(BCM_DNX_SUPPORT) && !defined(BCM_PETRA_SUPPORT) && !defined (BCM_DPP_SUPPORT)
#ifdef BCM_WARM_BOOT_SUPPORT
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SAND_SUPPORT) || defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
    int     warm_boot = FALSE;
#endif /* BCM_ESW_SUPPORT  || BCM_SAND_SUPPORT || BCM_LTSW_SUPPORT || BCM_CTSW_SUPPORT */
#endif /* BCM_WARM_BOOT_SUPPORT */
#endif /*!defined(BCM_DNXF_SUPPORT) && !defined(BCM_DNX_SUPPORT) && !defined(BCM_PETRA_SUPPORT) && !defined (BCM_DPP_SUPPORT)*/
    ...
    if (!(flags & BOOT_F_NO_PROBE)) {
        SHR_BPROF_STATS_TIME(SHR_BPROF_SOC_PROBE) {
            ...
            if ((sysconf_ltsw_probe()) < 0) {
                cli_out("Failed to probe LTSW device\n");
            }
#endif /* BCM_LTSW_SUPPORT || BCM_CTSW_SUPPORT */
            if ((sysconf_probe()) < 0 && !soc_ndev) {
        ...
        if (!(flags & BOOT_F_NO_ATTACH)) {
            /* loop over the units and attach them */
...
/* For now only TH3 supports Fast Reboot, so only allow TH3 to set flags */
...
#if defined(BCM_DNX_SUPPORT) || defined(BCM_DNXF_SUPPORT)
    dnx_calibrate_lock = sal_mutex_create("dnx_calibrate_lock");
    if (!dnx_calibrate_lock)
    {
        cli_out("unable to create dnx_calibrate_lock\n");
    }
#endif
    ...
    if (!(flags & BOOT_F_NO_RC)) {
        /* loop over the units and execute the init script */
        for (i = 0; i < soc_ndev; i++) {
    ...
    {
        /*
         * Calibrate (initialize and adjust) the command tree after running the init sequence.
         * This function removes all dynamic commands determined to be unavailable in the current device/configuration.
         */
        cmdlist_calibrate();
    }
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_SAND_SUPPORT) || defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
#if defined(BCM_LTSW_SUPPORT)
    /* For FFB, user MUST execute 'ffb apply' and 'ffb done' explicitly to
     * sync the install the configuration to the device. ISSU_DONE is moved to
     * ffb done stage. */
    if (!(flags & BOOT_F_FAST_REBOOT)) {
        if ((sysconf_ltsw_ha_issu_done()) < 0) {
...
void diag_sdk_cleanup(void)
{
#if defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
    int i;
#endif

    diag_mandatory_cleanup();

#if defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
    for (i = 0; i < soc_ndev; i++) {
#if defined(BCM_LTSW_SUPPORT)
        if (ltsw_pw_running(SOC_NDEV_IDX2DEV(i))) {
            (void) ltsw_pw_clean(SOC_NDEV_IDX2DEV(i));
        }
#endif
#if defined(BCM_CTSW_SUPPORT)
        if (ctsw_pw_running(SOC_NDEV_IDX2DEV(i))) {
            (void) ctsw_pw_clean(SOC_NDEV_IDX2DEV(i));
        }
#endif
    }
    if (sysconf_ltsw_cleanup() < 0) {
        cli_out("Failed to do the SDK cleanup.\n");
    }
    if (sysconf_ltsw_ha_deinit() < 0) {
        cli_out("Failed to do the HA cleanup.\n");
    }
#endif
...
#if defined(BCM_CTSW_SUPPORT)
    if (ctsw_pw_running(unit)) {
        SYSTEM_INIT_CHECK(ctsw_pw_clean(unit), "PacketWatcher clean");
    }
#endif /* BCM_CTSW_SUPPORT */

    SYSTEM_INIT_CHECK(sysconf_ltsw_dev_init(unit), "Device init");

    if ((rv = soc_ltsw_is_cna(unit, &is_cna)) < 0) {
        cli_out("Unable to get the NPL version on unit %d: %s\n",
                unit, bcm_errmsg(rv));
    }
    if (is_cna) {
        command_mode_set(unit, CTSW_CMD_MODE);
...
#if defined(BCM_CTSW_SUPPORT)
    if ((!SAL_BOOT_XGSSIM) && (is_cna)) {
        SYSTEM_INIT_CHECK(ctsw_do_resync_counters(unit, pbmp),
                          "Resync counters");
    }
#endif /* BCM_CTSW_SUPPORT */
...
#if defined(BCM_EDK_SUPPORT) && defined(EDKHLIB) && !defined(BCM_LTSW_SUPPORT)
    {
        char* argv_list[2] = {"edk_main", "shutdown"};

        /* Shutdown EDK-Host */
        if (edk_main(2, argv_list) != BCM_E_NONE) {
            cli_out("EDK: ERROR in edk_main() while shutdown\n");
        }
    }
#endif
