...
static void
appl_warmboot_storage_alloc(int unit, int module_id, int scache_part_no, int alloc_size)
{
    ...
    if (module_id < 0 || module_id > (BCM_MODULE__COUNT + 3) ) {
       cli_out("Invalid module Id.\n");
        return;
    }
    if ( alloc_size <= 0 ) {
       cli_out("Invalid alloc size.\n");
        return;
    }

    part_count = scache_max_partitions[unit][module_id];
    if ((scache_part_no < 0) || (scache_part_no >= part_count)) {
        cli_out("Invalid scache part no.\n");
        return;
    }

    SOC_SCACHE_HANDLE_SET(handle, unit, module_id, scache_part_no);
    rv = soc_scache_ptr_get(unit, handle, &scache_ptr,
                            &size);
    if (BCM_FAILURE(rv)) {
        cli_out("unable to allocate scache.\n");
        return;
    }
    rv = soc_scache_realloc(unit, handle, alloc_size);
    if (BCM_FAILURE(rv)) {
        cli_out("unable to allocate scache.\n");
        return;
    }
...
appl_warmboot_storage_requirements(int unit)
{
    ...
    for (module = 0; module < BCM_MODULE__COUNT; module++) {
        ...
        while(0 <= part_count) { 
            ...
            if (storage_per_module[module] > 0) {
                cli_out("%s module (%d):  %d bytes handle: %#x\n",
                        bcm_module_name(unit, module), module,
    ...
    for(module = BCM_MODULE__COUNT; module < BCM_MODULE__COUNT + 3; module++) {
        ...
        if (SOC_SUCCESS(rv)) {
                cli_out("%s module (%d): %d bytes handle: %#x\n",
                        soc_module_name(unit, module), module,
                        storage_per_module[module], handle);
        } else {
            storage_per_module[module] = 0;
        }        
...
#else
#if (defined(BCM_XGS3_SWITCH_SUPPORT))
    "Parameters: [on | off | show | shutdown | storage [alloc <module_id> <scache_part_no> <alloc size>]| sync | usage]\n"
#else
...
#if (defined(BCM_XGS3_SWITCH_SUPPORT))
    "or allocate additional scache for given module and part no.\n"
#endif
#if defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)
...
#if (defined(BCM_XGS3_SWITCH_SUPPORT))
            int scache_module_id = 0;
            int scache_part_no = 0;
            int scache_alloc_size = 0;
            c = ARG_GET(a);
            if (c == NULL) {
                appl_warmboot_storage_requirements(u);
            } else {
                if (!sal_strcasecmp(c, "alloc")) {
                   c = ARG_GET(a);
                   if (c == NULL) {
                       return(CMD_USAGE);
                   }
                   ...
                   scache_module_id = parse_integer(c);
                   c = ARG_GET(a);
                   if (c == NULL) {
                       return(CMD_USAGE);
                   }
                   ...
                   scache_part_no = parse_integer(c);
                   c = ARG_GET(a);
                   if (c == NULL) {
                       return(CMD_USAGE);
                   }
                   if (!isint(c)) {
                       cli_out("Invalid scache alloc size.\n");
                       return (CMD_FAIL);
                   }
                   scache_alloc_size = parse_integer(c);
                   appl_warmboot_storage_alloc(unit,
                                               scache_module_id,
                                               scache_part_no,
                                               scache_alloc_size);
