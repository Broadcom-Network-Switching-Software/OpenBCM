...
char *loopback_mode[] = {
    "NONE", "MAC", "PHY", "RMT", "MAC_RMT", "EDB", "NIF", NULL
};
...
#if defined(BCM_ESW_SUPPORT)  || defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)
/* { */
void
...
esw_brief_port_info(int unit, int port, bcm_port_info_t *info, uint32 flags)
{
    ...
    _CHECK_PRINT(BSL_UNIT_UNKNOWN, flags, BCM_PORT_ATTR_LOOPBACK_MASK,
                 ...
                 LOOPBACK_MODE(info->loopback) : "     ");
    if (SOC_IS_TRIDENT3X(unit)) {
        _CHECK_PRINT(BSL_UNIT_UNKNOWN, flags, BCM_PORT_ATTR_ENCAP_MASK,
             " %7s",
             info->encap_mode != BCM_PORT_ENCAP_COUNT ?
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
/* { */
/*
 * Function:
 *    if_port_stat
...
char if_port_samp_rate_usage[] =
    ...
    "IngressRate/EgressRate:\n\t"
    "\tOn average, every 1/(ingress_rate|egress_rate) packets will be sampled.\n\t"
 ...
 *      if_port_samp
 * Purpose:
 *      Set/display of sflow port sampling characteristics.
 * Parameters:
 *      unit - SOC unit #
 *      args - pointer to comand line arguments
 * Returns:
 *      CMD_OK/CMD_FAIL
 */
char if_port_samp_usage[] =
    "Set/Display port sampling characteristics.\n"
    "Parameters: <pbm> [IngressRate=<value>] [EgressRate=<value>]\n\t"
    "[IngressDest=None|Mirror|CPU|MirrorAndCPU]\n\t"
    "IngressRate/EgressRate:\n\t"
    "\tOn average, every 1/(ingress_rate|egress_rate) packets will be sampled.\n\t"
    "\tA rate of 0 indicates no sampling.\n\t"
    "\tA rate of 1 indicates all packets sampled.\n";
...
cmd_result_t
if_port_samp(int unit, args_t *args)
{
    pbmp_t pbm;
    bcm_port_config_t pcfg;
    char *ch;
    ...
    if(soc_feature(unit, soc_feature_sflow_ing_mem)) {
        if ((ch =  ARG_CUR(args)) != NULL) {
            parse_table_init(unit, &pt);
            parse_table_add(&pt, "IngressRate", PQ_DFL | PQ_INT,
                            0, &ingress_rate, 0);
            parse_table_add(&pt, "EgressRate", PQ_DFL | PQ_INT,
                            0, &egress_rate, 0);
            parse_table_add(&pt, "IngressDest", PQ_DFL | PQ_STRING,
                            0, &dest, 0);
            ...
            DPORT_SOC_PBMP_ITER(unit, pbm, dport, port) {
                ...
                if (flags & 0x1) {
                    rv = bcm_port_control_set(unit, port,
                                              ...
                                              ingress_rate);
                    if (rv == BCM_E_NONE) {
                        cli_out("IngressRate set to %d\n",
                                ingress_rate);
                    } else {
                        if (rv == BCM_E_UNAVAIL) {
                            cli_out("IngressRate %d is not available\n",
                                    ingress_rate);
                        } else {
                            cli_out("IngressRate %d is unsuccessful\n",
                                    ingress_rate);
                        }
                    }
                }
               ...
               if (flags & 0x2) {
                    rv = bcm_port_control_set(unit, port,
                                              ...
                                              egress_rate);
                    if (rv == BCM_E_NONE) {
                        cli_out("EgressRate set to %d\n",
                                egress_rate);
                    } else {
                        if (rv == BCM_E_UNAVAIL) {
                            cli_out("EgressRate %d is not available\n",
                                    egress_rate);
                        } else {
                            cli_out("EgressRate %d is unsuccessful\n",
                                    egress_rate);
                        }
                    }
                }
                ...
                if (flags & 0x4) {
                    value = 0;
                    if (sal_strcasecmp(dest, "None") == 0) {
                        value = 0;
                    }
                    if (sal_strcasecmp(dest, "CPU") == 0) {
                        value = BCM_PORT_CONTROL_SAMPLE_DEST_CPU;
                    }
                    if (sal_strcasecmp(dest, "Mirror") == 0) {
                        value = BCM_PORT_CONTROL_SAMPLE_DEST_MIRROR;
                    }
                    if (sal_strcasecmp(dest, "MirrorAndCPU") == 0) {
                        value = BCM_PORT_CONTROL_SAMPLE_DEST_CPU
                    ...
                    if (rv == BCM_E_NONE) {
                        cli_out("IngressDest set to %s\n",
                                dest);
                    } else {
                        if (rv == BCM_E_UNAVAIL) {
                            cli_out("IngressDest %s is not available\n",
                                    dest);
                        } else {
                            cli_out("IngressDest %s is unsuccessful\n",
                                    dest);
                        }
                    }
                }
            ...
            cli_out("Port Sampling Details:\n");
            cli_out("%10s %14s %14s %16s\n",
                    "port", "IngressRate", "EgressRate", "IngressDest");
            DPORT_BCM_PBMP_ITER(unit, pbm, dport, port) {
                rv = bcm_port_control_get(unit, port, bcmPortControlSampleIngressRate,
                                          &ingress_rate);
                if (BCM_FAILURE(rv)) {
                    cli_out("Port control get:bcmPortControlSampleIngressDest "
                            "failed with %d\n", rv);
                    return CMD_FAIL;
                }

                rv = bcm_port_control_get(unit, port, bcmPortControlSampleEgressRate,
                                          &egress_rate);
                if (BCM_FAILURE(rv)) {
                    cli_out("Port control get:bcmPortControlSampleIngressDest "
                            "failed with %d\n", rv);
                ...
                rv = bcm_port_control_get(unit, port, bcmPortControlSampleIngressDest,
                                          &value);
                if (BCM_FAILURE(rv)) {
                    cli_out("Port control get:bcmPortControlSampleIngressDest "
                            "failed with %d\n", rv);
                ...
                switch (value) {
                    case 0:
                        dest = "None";
                        break;
                    case BCM_PORT_CONTROL_SAMPLE_DEST_MIRROR:
                        dest = "Mirror";
                        break;
                    case BCM_PORT_CONTROL_SAMPLE_DEST_CPU:
                        dest = "CPU";
                        break;
                    case BCM_PORT_CONTROL_SAMPLE_DEST_CPU
                         | BCM_PORT_CONTROL_SAMPLE_DEST_MIRROR:
                ...
                cli_out("%5s(%3d) %14d %14d %16s\n", SOC_PORT_NAME(unit, port), port,
                        ingress_rate, egress_rate, dest);
            }
        }
    }
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT) || defined(BCM_DFE_SUPPORT) || defined(BCM_PETRA_SUPPORT)
/* { */
/*
 * Function:
 *    disp_port_info
               ...
               int st_port, uint32 flags)
{
    ...
    if ((flags & BCM_PORT_ATTR_MEDIUM_MASK) &&
        (0 <= (int)(info->medium)) && (info->medium < BCM_PORT_MEDIUM_COUNT)) {
...
/* } */
#endif
#if defined(BCM_PETRA_SUPPORT) || defined(BCM_ESW_SUPPORT) || defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT) || defined(BCM_DFE_SUPPORT)
/* { */
...
if_esw_port(int u, args_t *a)
{
    ...
    if (ARG_CNT(a) == 0) {
        ...
        if (info_given.encap_mode >= 0
              && info_given.encap_mode < (sizeof(encap_mode) / sizeof(encap_mode[0]))
              &&!sal_strcmp(encap_mode[info_given.encap_mode], "HIGIG3")) {
            info_given.encap_mode = BCM_PORT_ENCAP_HIGIG3;
        }
