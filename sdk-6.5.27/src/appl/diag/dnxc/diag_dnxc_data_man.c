    ...
    int release)
{
    ...
    *add_soc_prop = FALSE;
    ...
    if (sal_strnstr(label, "|", SH_SAND_MAX_TOKEN_SIZE) != NULL)
    {
    ...
    for (int i = 0; i < num_of_labels; i++)
    {
        ...
        if (!sal_strncmp(label_list[i], "flexe", 5))
        {
    ...
    *num_of_prop += 1;
    ...
    if (property->method != dnxc_data_property_method_custom)
    {
        ...
        if (is_suffix)
        {
            if (key_name == NULL)
            {
                sal_snprintf(synopsis_str, dnx_drv_soc_property_name_max() * 2, "%s[_%s]=%s", property->name,
                             property->suffix, val_str);
            }
            else
            {
                sal_snprintf(synopsis_str, dnx_drv_soc_property_name_max() * 2, "%s[_%s<%s>]=%s", property->name,
                             property->suffix, key_name, val_str);
            }
        }
        else
        {
            sal_snprintf(synopsis_str, dnx_drv_soc_property_name_max() * 2, "%s=%s", property->name, val_str);
        }
    ...
    int *module_nof_prop)
{
    ...
    for (table_index = 0; table_index < submodule->nof_tables; table_index++)
    {
         ...
         */
        rv = dnxc_data_utils_dump_verify(unit, state, table->flags, table->labels, 0, NULL, &dump);
        SHR_IF_ERR_EXIT(rv);
    ...
    for (feature_index = 0; feature_index < submodule->nof_features; feature_index++)
    {
         ...
         * Check if data is supported
         */
        rv = dnxc_data_utils_dump_verify(unit, state, feature->flags, feature->labels, 0, NULL, &dump);
        SHR_IF_ERR_EXIT(rv);
        /** filter internal properties */
    ...
    for (define_index = 0; define_index < submodule->nof_defines; define_index++)
    {
         ...
         * Check if data is supported
         */
        rv = dnxc_data_utils_dump_verify(unit, state, define->flags, define->labels, 0, NULL, &dump);
        SHR_IF_ERR_EXIT(rv);
        /** filter internal properties */
