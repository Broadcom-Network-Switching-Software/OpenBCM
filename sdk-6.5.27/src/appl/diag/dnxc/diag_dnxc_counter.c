...
static shr_error_e update_dnxc_algo_counter_pbmp(
    int unit,
    bcm_pbmp_t counter_pbmp)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_mib_counter_pbmp_set(unit, counter_pbmp));
    }
#endif
#ifdef BCM_DNXF_SUPPORT
    if (SOC_IS_DNXF(unit))
...
static shr_error_e update_dnxc_algo_counter_interval(
    int unit,
    int interval)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    int is_counter_on)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        BCM_PBMP_ITER(*counter_pbmp, p)
        {
            ...
            if ((DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(STIF, MGMT))
                 && is_master_port)
                || DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK))
    ...
    if(is_counter_on)
    {
        SHR_IF_ERR_EXIT(update_dnxc_algo_counter_pbmp(unit, *counter_pbmp));
    }
    else
    {
        SHR_IF_ERR_EXIT(update_dnxc_algo_counter_pbmp(unit, soc->counter_pbmp));
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else if (off_state)/*stop counter */
    {
        ...
        SHR_IF_ERR_EXIT_WITH_LOG(rv, "%s: Error: Could not stop counter: %s%s\n",
                                ARG_CMD(args), bcm_errmsg(rv), EMPTY);
        rv = update_dnxc_algo_counter_interval(unit, 0);
        SHR_IF_ERR_EXIT_WITH_LOG(rv, "%s: Error: Could not update algo counter interval: %s%s\n",
                                ARG_CMD(args), bcm_errmsg(rv), EMPTY);
        if (is_pbmp_present)
        {
    ...
    else if (on_state)/*start counter */
    {
        ...
        SHR_IF_ERR_EXIT_WITH_LOG(rv, "%s: Error: Could not start counter: %s%s\n",
                                ARG_CMD(args), bcm_errmsg(rv), EMPTY);
        rv = update_dnxc_algo_counter_interval(unit, us);
        SHR_IF_ERR_EXIT_WITH_LOG(rv, "%s: Error: Could not update algo counter interval: %s%s\n",
                                ARG_CMD(args), bcm_errmsg(rv), EMPTY);
    }
