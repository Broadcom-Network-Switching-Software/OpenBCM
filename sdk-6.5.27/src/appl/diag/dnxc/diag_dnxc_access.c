    ...
    char *field_name)
{
    ...
    const access_block_instance_t *block_instances;
    ...
    for (mem_index = memory_first_index; mem_index <= memory_last_index; mem_index++)
    {
        ...
        {
            block_instances = device_info->block_instances + local_block->instances_start_index;
            ...
            bsl_log_add(BSL_APPL_SHELL, bslSeverityNormal, bslSinkIgnore, unit, "0x%02X 0x%04X: %s\n",
                        block_instances[inst_i == ACCESS_ALL_BLOCK_INSTANCES ? 0 : inst_i].sbus_info.sbus_block_id,
    ...
    int is_mem)
{
    ...
    if (is_mem)
    {
        ...
        {
            /** Filter by flags */
            if (cached_flag & ACCESS_REGMEM_STATE_FLAG_CACHEABLE) {
                /** Printing only the cached memories */
                if (!access_local_regmem_cache_get(runtime_info, regmem_i))
                {
                    continue;
                }
            }
            ...
            if (substr_match != NULL && sal_strncaseindex(access_global_names[global_regmem_name_indices[rm_info->global_regmem_id]], substr_match, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == NULL)
            {
                continue;
            }
    ...
    args_t *a)
{
    ...
    uint32 flags_to_be_set = 0, flags_to_be_clear = 0, cached_flag = 0;
    int console_was_on = 0, console_disabled = 0, pushed_ctrl_c = 0;
    parse_table_t pt;
    jmp_buf ctrl_c;
    char *fname = "", *tab = NULL, *mem_name = NULL, *data = NULL;
    access_regmem_state_t *regmem_state = runtime_info->regmem_state;
    ...
    for (tab = ARG_GET(a); tab != NULL; tab = ARG_GET(a))
    {
        if (sal_strncasecmp(tab, "cache-table", sizeof("cache-table")) == 0)
        {
            ...
            {
                bsl_log_add(BSL_APPL_SHELL, bslSeverityNormal, bslSinkIgnore, unit, "This cache does not support on this device.\n");
                return CMD_FAIL;
            }
            cached_flag |= ACCESS_REGMEM_STATE_FLAG_CACHEABLE;
        }
        else if (sal_strncasecmp(tab, "dynamic-table", sizeof("dynamic-table")) == 0)
        {
    ...
    {
        access_dnxc_regmem_list_summary(runtime_info, NULL, print_block_names, flags_to_be_set, flags_to_be_clear, cached_flag, 1);
        rv = CMD_OK;
        goto done;
    }
    ...
    if (parse_memory_name_rv == 1)
    {
        ...
        access_dnxc_regmem_list_summary(runtime_info, mem_name, print_block_names, flags_to_be_set, flags_to_be_clear, cached_flag, 1);
        rv = CMD_OK;
        goto done;
    }
    ...
    args_t *a)
{
    ...
    {
        access_dnxc_regmem_list_summary(runtime_info, NULL, print_block_names, flags_to_be_set, flags_to_be_clear, 0, 0);
        rv = CMD_OK;
        goto done;
    }
    ...
    if (parse_register_name_rv == 1)
    {
        ...
        access_dnxc_regmem_list_summary(runtime_info, reg_name, print_block_names, flags_to_be_set, flags_to_be_clear, 0, 0);
        rv = CMD_OK;
        goto done;
    }
    ...
    args_t *a)
{
    ...
    {   /** Numerical address given */
        ...
        if (access_find_regmem_by_address_and_by_sbus_block_id(runtime_info, ACCESS_REGMEM_FLAG_MEMORY, 0, block_inst_sbus_id, regaddr, &regmem_i, &inst_i, &array_index, &memory_index) != SOC_E_NONE)
        {
