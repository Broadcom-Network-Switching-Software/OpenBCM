...
#define SER_INTR_MAX_INSTANCE_NUM       256
#define SER_INTR_INSTANCE_BMP_WIDTH32   (SER_INTR_MAX_INSTANCE_NUM/32)
#define SER_INTR_INSTANCE_BMP_WIDTH8    (SER_INTR_MAX_INSTANCE_NUM/8)
typedef enum
{
...
/** see header file */
shr_error_e
sh_cmd_is_access_not_available(
    int unit,
    rhlist_t * test_list)
{
...
static sh_sand_option_t access_ser_list_reg_options[] = {
...
static sh_sand_man_t access_ser_list_reg_man = {
    .brief = "SER LIST REGister commands\n",
    .full = "List Register based on input condition\n",
    .examples = "BLock=sch DYNAMIC=no\n" "ACCess=RW STatiC=no"
};

static shr_error_e
access_ser_get_single_reg_info(
    ...
    char *pRegInfo)
{
    ...
    SHR_NULL_CHECK(pRegInfo, _SHR_E_PARAM, "pRegInfo");

    /** Convert global to local regmem ID */
    ...
    {
        sal_snprintf(str_mem_info + sal_strnlen(str_mem_info, sizeof(str_mem_info)),
                     sizeof(str_mem_info) - sal_strnlen(str_mem_info, sizeof(str_mem_info)), "RO/");
    }
    /*
     * WriteOnly 
     */
    ...
    {
        sal_snprintf(str_mem_info + sal_strnlen(str_mem_info, sizeof(str_mem_info)),
                     sizeof(str_mem_info) - sal_strnlen(str_mem_info, sizeof(str_mem_info)), "WO/");
    }
    ...
    {
        sal_snprintf(str_mem_info + sal_strnlen(str_mem_info, sizeof(str_mem_info)),
                     sizeof(str_mem_info) - sal_strnlen(str_mem_info, sizeof(str_mem_info)),
                     "Array[%d]/", rm_info->nof_array_indices);
    }

    sal_strncpy_s(pRegInfo, str_mem_info, DIAG_DNX_SER_INFO_MAX_LEN);
...
static shr_error_e
access_ser_list_register_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control)
{
    int ii, dynamic_flag, static_flag;
    char *str_block, *access_type;
    int number;
    ...
    for (local_blk_id = start_id; local_blk_id < end_id; local_blk_id++)
    {
        ...
        for (ii = 0; ii < block->nof_registers; ii++)
        {
            local_regmem = block->registers_start + ii;
            rm_info = device_info->local_regs + local_regmem;
            ...
            SHR_IF_ERR_EXIT(access_ser_get_single_reg_info(runtime_info, local_regmem, str_mem_info));
            ...
            PRT_CELL_SET("%X", rm_info->width_in_bits);
    ...
    char *pMemInfo)
{
    ...
    {
        rm_info = device_info->local_regs + rm_info->u.mem.alias_regmem;
    ...
    {
        sal_snprintf(str_mem_info + sal_strnlen(str_mem_info, sizeof(str_mem_info)),
                     sizeof(str_mem_info) - sal_strnlen(str_mem_info, sizeof(str_mem_info)), "Dyn/");
    }
    /*
     * ReadOnly 
     */
...
dnxc_ser_intr_info_print_access(
    ...
    sh_sand_control_t * sand_control)
{
    int intr, nof_interrupts, start_id, end_id;
    ...
    if ((intr_id >= 0) && (intr_id < nof_interrupts))
    {
        start_id = intr_id;
        end_id = intr_id;
    }
    ...
    {
        start_id = 0;
        end_id = nof_interrupts - 1;
    }

    for (intr = start_id; intr <= end_id; intr++)
    {
        ...
        {
            sal_snprintf(str_flags + sal_strlen(str_flags), sizeof(str_flags) - sal_strlen(str_flags),
                         " ACT_BCM_AND_USR_CB");
        }
        else if (SHR_BITGET(&flags, SOC_INTERRUPT_DB_FLAGS_CORR_ACT_OVERRIDE_ENABLE))
        {
            sal_snprintf(str_flags + sal_strlen(str_flags), sizeof(str_flags) - sal_strlen(str_flags), " ACT_OVERRIDE");
        }
        else
        {
            sal_snprintf(str_flags + sal_strlen(str_flags), sizeof(str_flags) - sal_strlen(str_flags), " ACT_BCM_ONLY");
        }
        if (SHR_BITGET(&flags, SOC_INTERRUPT_DB_FLAGS_FORCE_UNMASK_BITS))
        {
            sal_snprintf(str_flags + sal_strlen(str_flags), sizeof(str_flags) - sal_strlen(str_flags), " FORCE_UNMASK");
        }
        if (SHR_BITGET(&flags, SOC_INTERRUPT_DB_FLAGS_UNMASK_AND_CLEAR_DISABLE_BITS))
        {
            sal_snprintf(str_flags + sal_strlen(str_flags), sizeof(str_flags) - sal_strlen(str_flags),
                         " CLEAR_DISABLE");
        }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (dump_opt == 0)
    {
        ...
        SHR_IF_ERR_EXIT(mdb_dh_macro_cluster_entry_get
                        (unit, core_die_id, macro_type, macro_index, cluster_index, entry_index, entry_data, &ecc));
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (dump_opt == 0)
    {
        ...
        SHR_IF_ERR_EXIT(mdb_dh_macro_cluster_entry_set
                        (unit, core_die_id, macro_type, macro_index, cluster_index, entry_index, array_uint32, ecc));
    }
    else if (dump_opt == 1)
    {
        /*
    ...
    char *pMemInfo)
{
    ...
    is_mem_alias = SOC_MEM_IS_ALIAS(unit, mem);
    mem_alias = mem;
...
static sh_sand_man_t dnxc_ser_memory_list_man = {
    .brief = "SER LIST MEMory commands\n",
...
static sh_sand_man_t dnxc_ser_list_man = {
    .brief = "SER LIST commands \n",
    .full = "SER LIST commands\n",
};

sh_sand_cmd_t dnxc_ser_list_cmds[] = {
    /*
     * Name | Leaf Action | Junction Array | Options for Leaf | Usage 
     */
    {"MEMory", dnxc_ser_memory_list_cmd, NULL, dnxc_ser_memory_list_options, &dnxc_ser_memory_list_man, NULL, NULL,
     SH_CMD_NO_XML_VERIFY},

#ifdef BCM_ACCESS_SUPPORT
    {"REGister", access_ser_list_register_cmd, NULL, access_ser_list_reg_options, &access_ser_list_reg_man,
     NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY * SH_CMD_NOT_RELEASE, sh_cmd_is_access_available},
#endif
...
sh_sand_cmd_t sh_dnxf_diag_ser_cmds[] = {
     ...
     * Name | Leaf Action | Junction Array | Options for Leaf | Usage 
     */
    {"LIST", NULL, dnxc_ser_list_cmds, NULL, &dnxc_ser_list_man},
...
sh_sand_cmd_t sh_dnx_diag_ser_cmds[] = {
    ...
    {"TCam", NULL, dnxc_diag_ser_tcam_cmds, NULL, &dnxc_diag_ser_tcam_man,
     NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NOT_RELEASE, sh_cmd_is_access_not_available},
    {"LIST", NULL, dnxc_ser_list_cmds, NULL, &dnxc_ser_list_man},
    {"INTeRrupt", NULL, dnxc_ser_intr_cmds, NULL, &dnxc_ser_intr_man},
