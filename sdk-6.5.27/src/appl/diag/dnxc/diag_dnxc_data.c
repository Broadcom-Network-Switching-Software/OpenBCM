...
static sh_sand_option_t dnxc_data_dump_options[] = {
    ...
    {"name",     SAL_FIELD_TYPE_STR,  "[<module_name>[.<submodule_name>[.<data_name>]]] (auto-completion supported)", "", NULL, NULL, SH_SAND_ARG_FREE, .ac_option_cb=diag_dnxc_dump_autocomplete_cb},
    {NULL}
};
/* *INDENT-ON* */
...
static sh_sand_man_t dnxc_data_dump_man = {
    ...
    .examples = "name=fabric.pipes.map\n"
                "name=fabric.pipes.*\n"
                "name=fabric.pipes.\n"
                "name=fabric.\n"
                "chg *\n"
...
static sh_sand_man_t dnxc_data_info_man = {
    ...
    .full     = "if '*' is specified will display info about the all branch\nNote that getting info of modules, submodules and data can be done with or without prefacing with 'name=',\n but autocomplete is supported only if prefaced with 'name='",
    .synopsis = "[changed] [property] [define/numeric/feature/table] [label=<label>] [<module_name>[.<submodule_name>[.<data_name>]]]",
    .examples = "name=fabric.pipes.map\n"
                "name=fabric.pipes.*\n"
                "name=fabric.\n"
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (define_index = 0; define_index < submodule->nof_defines; define_index++)
    {
        ...
        rv = dnxc_data_utils_dump_verify(unit, state, submodule->defines[define_index].flags,
    ...
    for (feature_index = 0; feature_index < submodule->nof_features; feature_index++)
    {
         ...
         */
        state = dnxc_data_mgmt_state_get(unit);
        rv = dnxc_data_utils_dump_verify(unit, state, submodule->features[feature_index].flags,
    ...
    for (table_index = 0; table_index < submodule->nof_tables; table_index++)
    {
         ...
         */
        state = dnxc_data_mgmt_state_get(unit);
        rv = dnxc_data_utils_dump_verify(unit, state, submodule->tables[table_index].flags,
    ...
    int *num_matches_found)
{
    ...
    if (nof_tokens == 1)
    {
        ...
        for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
        {
            ...
            if ((module == NULL)
                || !sal_strncasecmp("*", module, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
                || sal_strncaseindex(_dnxc_data[unit].modules[module_index].name, module,
                                     ...
                                     SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            {
                 ...
                 */
                if (sal_strnlen(dump_full_list, AUTOCOMPLETE_MAX_STRING_LEN) +
                    sal_strnlen(_dnxc_data[unit].modules[module_index].name,
    ...
    else if (nof_tokens == 2)
    {
        ...
        else
        {
            ...
            for (submodule_index = 0; submodule_index < matching_module->nof_submodules; submodule_index++)
            {
                ...
                if ((submodule == NULL)
                    || !sal_strncasecmp("*", submodule, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
                    || sal_strncaseindex(matching_module->submodules[submodule_index].name, submodule,
                                         SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                                         SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                {
                    (*num_matches_found)++;
                     ...
                     */
                    if (sal_strnlen(dump_full_list, AUTOCOMPLETE_MAX_STRING_LEN) +
                        sal_strnlen(matching_module->name,
                                    AUTOCOMPLETE_MAX_STRING_LEN) +
                        sal_strnlen(matching_module->submodules[submodule_index].name,
                                    AUTOCOMPLETE_MAX_STRING_LEN) + 3 >= AUTOCOMPLETE_MAX_STRING_LEN)
                    {
                        (*num_matches_found) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
                        dump_full_list[0] = 0;
                        cli_out("\n");
            ...
            else
            {
                ...
                for (data_index = 0; data_index < matching_submodule->nof_defines; data_index++)
                {
                    ...
                    if ((data == NULL) || !sal_strncasecmp("*", data, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ||
                        sal_strncaseindex(matching_submodule->defines[data_index].name, data,
                                          SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                                          SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                    {
                        (*num_matches_found)++;
                        ...
                        if (sal_strnlen(dump_full_list, AUTOCOMPLETE_MAX_STRING_LEN) +
                            sal_strnlen(matching_module->name,
                                        AUTOCOMPLETE_MAX_STRING_LEN) + sal_strnlen(matching_submodule->name,
                                                                                   AUTOCOMPLETE_MAX_STRING_LEN) +
                            sal_strnlen(matching_submodule->defines[data_index].name,
                                        AUTOCOMPLETE_MAX_STRING_LEN) + 3 >= AUTOCOMPLETE_MAX_STRING_LEN)
                        {
                            (*num_matches_found) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
                            dump_full_list[0] = 0;
                ...
                for (data_index = 0; data_index < matching_submodule->nof_features; data_index++)
                {
                     ...
                     */
                    if ((data == NULL) || !sal_strncasecmp("*", data, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ||
                        sal_strncaseindex(matching_submodule->features[data_index].name, data,
                                          SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                                          SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                    {
                        (*num_matches_found)++;
                        ...
                        if (sal_strnlen(dump_full_list, AUTOCOMPLETE_MAX_STRING_LEN) +
                            sal_strnlen(matching_module->name,
                                        AUTOCOMPLETE_MAX_STRING_LEN) + sal_strnlen(matching_submodule->name,
                                                                                   AUTOCOMPLETE_MAX_STRING_LEN) +
                            sal_strnlen(matching_submodule->features[data_index].name,
                                        AUTOCOMPLETE_MAX_STRING_LEN) + 3 >= AUTOCOMPLETE_MAX_STRING_LEN)
                        {
                            (*num_matches_found) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
                            dump_full_list[0] = 0;
                ...
                for (data_index = 0; data_index < matching_submodule->nof_tables; data_index++)
                {
                     ...
                     */
                    if ((data == NULL) || !sal_strncasecmp("*", data, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ||
                        sal_strncaseindex(matching_submodule->tables[data_index].name, data,
                                          SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH,
                                          SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
                    {
                        (*num_matches_found)++;
                        ...
                        if (sal_strnlen(dump_full_list, AUTOCOMPLETE_MAX_STRING_LEN) +
                            sal_strnlen(matching_module->name,
                                        AUTOCOMPLETE_MAX_STRING_LEN) + sal_strnlen(matching_submodule->name,
                                                                                   AUTOCOMPLETE_MAX_STRING_LEN) +
                            sal_strnlen(matching_submodule->tables[data_index].name,
                                        AUTOCOMPLETE_MAX_STRING_LEN) + 3 >= AUTOCOMPLETE_MAX_STRING_LEN)
                        {
                            (*num_matches_found) = AUTOCOMPLETE_PRINT_OUT_OF_SCOPE;
                            dump_full_list[0] = 0;
    ...
    for (module_index = 0; module_index < _dnxc_data[unit].nof_modules; module_index++)
    {
        ...
        for (submodule_index = 0; submodule_index < module->nof_submodules; submodule_index++)
        {
            ...
            for (table_index = 0; table_index < submodule->nof_tables; table_index++)
            {
                 ...
                 */
                state = dnxc_data_mgmt_state_get(unit);
                rv = dnxc_data_utils_dump_verify(unit, state, table->flags, table->labels, 0, label, &dump);
                SHR_IF_ERR_EXIT(rv);
                ...
                if (dump)
                {
                    ...
                    for (value_index = 0; value_index < table->nof_values; value_index++)
                    {
                        if (table->values[value_index].property.name != NULL)
                        {
                             ...
                             * check if substring
                             */
                            else if (sal_strncaseindex
                                     (property_name, property, SOC_PROPERTY_NAME_MAX, SOC_PROPERTY_NAME_MAX) != NULL
            ...
            for (feature_index = 0; feature_index < submodule->nof_features; feature_index++)
            {
                 ...
                 */
                state = dnxc_data_mgmt_state_get(unit);
                rv = dnxc_data_utils_dump_verify(unit, state, feature->flags, feature->labels, 0, label, &dump);
                SHR_IF_ERR_EXIT(rv);
            ...
            for (define_index = 0; define_index < submodule->nof_defines; define_index++)
            {
                 ...
                 */
                state = dnxc_data_mgmt_state_get(unit);
                rv = dnxc_data_utils_dump_verify(unit, state, define->flags, define->labels, 0, label, &dump);
                SHR_IF_ERR_EXIT(rv);
    ...
    if (submodule->nof_features > 0)
    {
        ...
        for (feature_index = 0; feature_index < submodule->nof_features; feature_index++)
        {
            ...
            SHR_IF_ERR_EXIT(dnxc_data_utils_dump_verify
                            (unit, state, features[feature_index].flags,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (submodule->nof_defines > 0)
    {
        ...
        for (define_index = 0; define_index < submodule->nof_defines; define_index++)
        {
            ...
            state = dnxc_data_mgmt_state_get(unit);
            SHR_IF_ERR_EXIT(dnxc_data_utils_dump_verify
                            (unit, state, defines[define_index].flags, defines[define_index].labels,
    ...
    sh_sand_control_t * sand_control)
{
    ...
    for (table_index = 0; table_index < submodule->nof_tables; table_index++)
    {
         ...
         * Check if data is supported / match flags
         */
        SHR_IF_ERR_EXIT(dnxc_data_utils_dump_verify(unit, state, table->flags, table->labels, flags, NULL, &do_export));
        if (do_export)
        {
            /*
...
static sh_sand_man_t dnxc_data_export_man = {
    ...
    .full     = "Prints all the changed data in a format that is friendly to exporting to xml files. To perform the actual export,"
                ...
                "to export all changed data to a file in the current directory, use: data export changed file=dnx_data.xml",
