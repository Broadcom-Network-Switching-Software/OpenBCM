...
#define DIAG_DNXC_PORT_LOOPBACK_CMD_MAX_LENGTH    40
...
#define DIAG_DNXC_PORT_BER_PROJ_INTEGER_PART_GET(_proj_ber)       ((_proj_ber & 0xFF000000) >> 24)
#define DIAG_DNXC_PORT_BER_PROJ_DECIMAL_PART_GET(_proj_ber)       ((_proj_ber & 0x00FF0000) >> 16)
#define DIAG_DNXC_PORT_BER_PROJ_EXPONENT_PART_GET(_proj_ber)      (_proj_ber & 0x0000FFFF)
...
/*
 * \brief
 * Generate dynamically "port loopback" examples depends if NIF loopback supported
 */
static shr_error_e
dnxc_port_loopback_example_generate(
    int unit,
    int flags,
...
{
    int port = 1;
    char loopback_mode_str[DIAG_DNXC_PORT_LOOPBACK_CMD_MAX_LENGTH];
#ifdef BCM_DNX_SUPPORT
    int imb_type = 0;
    ...
    sal_memset(&loopback_mode_str, 0, sizeof(loopback_mode_str));

    sal_snprintf(loopback_mode_str, DIAG_DNXC_PORT_LOOPBACK_CMD_MAX_LENGTH, "%d mode=%s \n", port, "mac");
#ifdef BCM_DNX_SUPPORT
    if (SOC_IS_DNX(unit))
    {
        ...
        if (TRUE == dnx_data_nif.eth.is_nif_loopback_supported_get(unit, imb_type)->is_supported)
        {
            sal_snprintf(loopback_mode_str, DIAG_DNXC_PORT_LOOPBACK_CMD_MAX_LENGTH, "%d mode=%s \n", port, "nif");
        }
    }
#endif
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, loopback_mode_str), "");

    sal_memset(&loopback_mode_str, 0, sizeof(loopback_mode_str));
    sal_snprintf(loopback_mode_str, DIAG_DNXC_PORT_LOOPBACK_CMD_MAX_LENGTH, "%d mode=none", port);
    SHR_CLI_EXIT_IF_ERR(sh_sand_example_add(unit, flags, sh_sand_cmd, examples_list, loopback_mode_str), "");
    ...
    sh_sand_control_t * sand_control)
{
    bcm_port_t port;
    bcm_pbmp_t pbm;
    phymod_core_access_t phy_core_acc[DIAG_DNXC_MAX_PORT_CORES];
    phymod_core_info_t phy_core_info;
    int nof_cores = 0;
    int first_phy = 0;
    ...
    BCM_PBMP_ITER(pbm, port)
    {
        ...
        {
            SHR_IF_ERR_EXIT(phymod_core_access_t_init(&phy_core_acc[i]));
        }

        SHR_IF_ERR_EXIT(phymod_core_info_t_init(&phy_core_info));

        sal_memset(&diag_info, 0, sizeof(portmod_port_diag_info_t));

        SHR_IF_ERR_EXIT(portmod_port_core_access_get
                        (unit, port, -1, DIAG_DNXC_MAX_PORT_CORES, phy_core_acc, &nof_cores, NULL));
        ...
        SHR_IF_ERR_EXIT(phymod_core_info_get(phy_core_acc, &phy_core_info));

        pname = phymod_core_version_t_mapping[phy_core_info.core_version].key;
        ...
        if (IS_IL_PORT(unit, port))
        {
            ...
            for (i = 0; i < nof_aggregated_pms; i++)
            {
                sal_snprintf(pname, sizeof(lnstr) - sal_strnlen(lnstr, sizeof(lnstr)) + 1, "/%d",
                             controlled_pms[i].phy);
                ...
                SHR_IF_ERR_EXIT(phymod_core_info_get(&phy_core_acc[i], &phy_core_info));

                PRT_CELL_SET("0x%x", phy_core_info.phy_id0);
                PRT_CELL_SET("0x%x", phy_core_info.phy_id1);
                PRT_CELL_SET("0x%x", phy_core_acc[i].access.addr);
        ...
        PRT_CELL_SET("0x%x", phy_core_info.phy_id0);
        PRT_CELL_SET("0x%x", phy_core_info.phy_id1);
        PRT_CELL_SET("0x%x", phy_core_acc[0].access.addr);
        PRT_CELL_SET("%s", lnstr);
    }
    PRT_COMMITX;
exit:
    ...
    uint32 *serdes_rate)
{
    ...
    if (IS_IL_PORT(unit, port) || IS_SFI_PORT(unit, port) || IS_OTN_PORT(unit, port))
    {
    ...
    *serdes_rate = 0;
    switch (lane_speed)
    {
        ...
        case 11095:
            /*
             * OTU25u-RS-KP4, OTL50u.2-RS
             */
        case 26562:
            /*
             * FOIC1.4-RS, OTU25-RS, OTL50.2-RS, OTL4.4
             */
        case 27952:
            /*
             * OTLC.4
        ...
        case 28076:
            /*
             * FOIC1.2-RS, FOIC2.4-RS, FOIC4.8-RS, OTL50.1-RS, OTL4.2
             */
        case 55904:
            /*
             * OTLC.2
    ...
    BCM_PBMP_ITER(pbmp, port)
    {
        ...
        if (SOC_IS_DNX(unit))
        {
            if (IS_SFI_PORT(unit, port))
            {
                ...
                if (!is_dt_present &&
                    !dnx_data_fabric.general.feature_get(unit,
                                                         dnx_data_fabric_general_measure_down_port_with_rx_supported))
                {
                    is_rx = 0;
                }
            }
    ...
    if (!SAL_BOOT_PLISIM)
    {
        ...
        if (SOC_IS_DNX(unit))
        {
            ...
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, DNX_ALGO_PORT_TYPE_INCLUDE(ELK, L1))
                || DNX_ALGO_PORT_TYPE_IS_FABRIC(unit, port_info) || DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
            {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (!SAL_BOOT_PLISIM)
    {
        if (is_write)
        {
            SHR_IF_ERR_EXIT(soc_dnxc_cmic_miim_write(unit, clause, phyid, SOC_PHY_CLAUSE45_ADDR(devad, regad),
                                                     wr_data));
        }
        else
        {
            SHR_IF_ERR_EXIT(soc_dnxc_cmic_miim_read(unit, clause, phyid, SOC_PHY_CLAUSE45_ADDR(devad, regad),
                                                    &rd_data));
        }
        if (!is_write)
        {
...
sh_sand_cmd_t sh_dnxc_phy_cmds[] = {
    /*keyword,  action,   command,  options,  man */
#ifdef BCM_DNX_SUPPORT
    {"info",      cmd_dnxc_phy_info,      NULL,                   NULL,                        &dnxc_phy_info_man, NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_dnx},
#endif
#ifdef BCM_DNXF_SUPPORT
    {"info",      cmd_dnxc_phy_info,      NULL,                   NULL,                        &dnxc_phy_info_man, NULL, NULL, SH_CMD_CONDITIONAL * SH_CMD_NO_XML_VERIFY, sh_cmd_is_dnxf_single_core},
#endif
...
static sh_sand_enum_t dnxc_port_loopback_mode_enum_table[] = {
    ...
    {"nif",      BCM_PORT_LOOPBACK_NIF, "Loopback mode: NIF"},
    {NULL}
};
...
sh_sand_man_t sh_dnxc_port_loopback_man = {
    ...
    .example_generate_cb = dnxc_port_loopback_example_generate
};
