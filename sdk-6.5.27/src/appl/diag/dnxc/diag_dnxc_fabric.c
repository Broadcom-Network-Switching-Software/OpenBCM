...
#define DIAG_DNXC_LINK_STATUS_FMAC_SYMB_ERR_BUF_SIZE    64
...
/* *INDENT-ON* */
static shr_error_e
cmd_dnxc_fabric_link_status_fmac_symb_error_get(
    int unit,
    bcm_port_t port,
    uint32 *is_uc_err,
    char *range_format_buf)
{
    uint32 symbol_err = 0;
    _shr_pbmp_t symbol_err_pbmp;
    SHR_FUNC_INIT_VARS(unit);

#ifdef BCM_DNX_SUPPORT
    if (soc_is(unit, DNX_DEVICE))
    {
        SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_symb_error_get(unit, port, &symbol_err));
    }
#endif
    ...
    if (soc_is(unit, DNXF_DEVICE))
    {
        SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                        (unit, mbcm_dnxf_fabric_links_fmac_symb_error_get, (unit, port, &symbol_err)));
    }
#endif

    /*
     * The most significant bit means there is an uncorrectable error
     */
    *is_uc_err = SHR_BITGET(&symbol_err, DNXC_FABRIC_LINK_UNCORRECTABLE_ERROR_INDICATION_BIT);
    SHR_BITCLR(&symbol_err, DNXC_FABRIC_LINK_UNCORRECTABLE_ERROR_INDICATION_BIT);
    /*
     * Trim the LSB 0 which means no correctable errors passed through the FEC
     */
    SHR_BITCLR(&symbol_err, 0);
    _SHR_PBMP_CLEAR(symbol_err_pbmp);
    _SHR_PBMP_WORD_SET(symbol_err_pbmp, 0, symbol_err);
    shr_pbmp_range_format(symbol_err_pbmp, range_format_buf, DIAG_DNXC_LINK_STATUS_FMAC_SYMB_ERR_BUF_SIZE);
exit:
    SHR_FUNC_EXIT;
}
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int is_remote_local_fault_supported = FALSE;
    int is_symb_errors_per_frame_supported = FALSE;
    uint32 is_uc_err = FALSE;
    char range_format_buf[DIAG_DNXC_LINK_STATUS_FMAC_SYMB_ERR_BUF_SIZE];
    ...
    if (soc_is(unit, DNXF_DEVICE))
    {
        ...
        {
            is_remote_local_fault_supported = TRUE;
        }
        if (dnxf_data_fabric.general.feature_get(unit, dnxf_data_fabric_general_is_symb_errors_per_frame_supported))
        {
            is_symb_errors_per_frame_supported = TRUE;
        }
    }
#endif
#ifdef BCM_DNX_SUPPORT
    if (soc_is(unit, DNX_DEVICE))
    {
        ...
        if (dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_is_symb_errors_per_frame_supported))
        {
            is_symb_errors_per_frame_supported = TRUE;
        }
    }
    ...
    if (is_symb_errors_per_frame_supported)
    {
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Nof Symbol Errors Per Frame");
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Uncorrectable Error");
    }
    if (is_remote_local_fault_supported)
    {
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Local Fault");
        PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "Remote Fault");
    }
    ...
    BCM_PBMP_ITER(diag_info_bmp, logical_port)
    {
        ...
        if (is_symb_errors_per_frame_supported)
        {
            SHR_IF_ERR_EXIT(cmd_dnxc_fabric_link_status_fmac_symb_error_get
                            (unit, logical_port, &is_uc_err, range_format_buf));
            PRT_CELL_SET("%s", range_format_buf[0] ? range_format_buf : " - ");
            PRT_CELL_SET("%s", is_uc_err ? "UC" : " - ");
        }
        if (is_remote_local_fault_supported)
        {
            PRT_CELL_SET("%s", links_status & BCM_FABRIC_LINK_STATUS_LOCAL_FAULT ? "***" : " - ");
            PRT_CELL_SET("%s", links_status & BCM_FABRIC_LINK_STATUS_REMOTE_FAULT ? "***" : " - ");
        }
    }
    ...
    sh_sand_control_t * sand_control)
{
    ...
    int nof_links = 0;
    bcm_port_control_t fec_controls[] =
        { bcmPortControlFecErrorDetectEnable, bcmPortControlLlfcCellsCongestionIndEnable,
        bcmPortControlLowLatencyLLFCEnable, bcmPortControlLLFCAfterFecEnable,
        bcmPortControlControlCellsFecBypassEnable, -1
    };
    int is_combination_valid, is_control_type_valid, i = 0;
    int ref_clk_dnxc = DNXC_SERDES_REF_CLOCK_NOF;
    PRT_INIT_VARS;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (SOC_IS_DNXF(unit))
    {
        SHR_IF_ERR_EXIT(soc_dnxf_port_max_port_get(unit, &nof_links));
    }
#endif
#ifdef BCM_DNX_SUPPORT
    if (SOC_IS_DNX(unit))
    ...
    BCM_PBMP_ITER(diag_info_bmp, logical_port)
    {
        ...
        for (i = 0; ((int) fec_controls[i]) != -1; i++)
        {
            SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fec_control_verify_params_get
                            (unit, link_config.fec_type, fec_controls[i], &is_combination_valid,
                             &is_control_type_valid));

            /*
             * If valid, get the selected control
             */
            if (is_combination_valid)
            {
                switch (fec_controls[i])
                {
                    case bcmPortControlFecErrorDetectEnable:
                        SHR_IF_ERR_EXIT(bcm_port_control_get(unit, logical_port, bcmPortControlFecErrorDetectEnable,
                                                             &link_config.fec_error_detect));
                        break;
                    ...
                    case bcmPortControlLowLatencyLLFCEnable:
                        SHR_IF_ERR_EXIT(bcm_port_control_get(unit, logical_port, bcmPortControlLowLatencyLLFCEnable,
                                                             &link_config.fec_llfc_low_latency));
                        break;
                    case bcmPortControlLLFCAfterFecEnable:
                        SHR_IF_ERR_EXIT(bcm_port_control_get(unit, logical_port, bcmPortControlLLFCAfterFecEnable,
                                                             &link_config.fec_llfc_after_fec));
                        break;
        ...
        else
        {
            ...
            if (!dnxf_data_fabric.cell.feature_get(unit, dnxf_data_fabric_cell_format_always_with_pcp))
            {
                SHR_IF_ERR_EXIT(bcm_fabric_link_control_get
                                (unit, logical_port, bcmFabricLinkPcpEnable, &link_config.pcp));
            }
#endif
        }
        /*
        ...
        {
#ifdef BCM_DNX_SUPPORT
            SHR_IF_ERR_EXIT(dnx_algo_port_soc_ref_clk_get(unit, logical_port, &ref_clk_dnxc));
        ...
        {
#ifdef BCM_DNXF_SUPPORT
            ref_clk_dnxc = SOC_INFO(unit).port_refclk_int[logical_port];
#endif
        }
        switch (ref_clk_dnxc)
        {
        ...
        sal_snprintf(fec_str, sizeof(fec_str), "%s%s%s%s%s%s",
                     fec_type,
        ...
        {
            sal_snprintf(pipe_mapping_buf, sizeof(pipe_mapping_buf), "   0->%d   ",
                         link_config.pipe_mapping.remote_pipe_mapping[0]);
            pipe_mapping_str = pipe_mapping_buf;
        }
        else if (link_config.pipe_mapping.num_of_remote_pipes == 2)
        {
            sal_snprintf(pipe_mapping_buf, sizeof(pipe_mapping_buf), "0->%d, 1->%d",
                         link_config.pipe_mapping.remote_pipe_mapping[0],
...
cmd_dnxc_fabric_mesh_topology(
    ...
    sh_sand_control_t * sand_control)
{
    int link_idx, port_idx, col_idx, diag_idx, printed_link = 0;
    int is_flexible_link_mapping_supported;
    bcm_pbmp_t type2_links;
    ...
    if (SOC_IS_DNXF(unit))
    {
        is_flexible_link_mapping_supported =
            dnxf_data_port.lane_map.feature_get(unit, dnxf_data_port_lane_map_is_flexible_link_mapping_supported);
    }
    else
#endif
    {
        is_flexible_link_mapping_supported = 0;
    }

    /*
     * Get traffic info
     */
    SHR_IF_ERR_EXIT(soc_dnxc_fabric_mesh_topology_diag_get(unit, &mesh_topology_info));

    for (diag_idx = 0; diag_idx < mesh_topology_info.nof_diag; diag_idx++)
    {
        /*
         * Print mesh topology diag topic
         */
        if (mesh_topology_info.nof_diag == 1)
        {
            LOG_CLI(("MESH TOPOLOGY DIAG \n"));
        }
        else
        {
            LOG_CLI(("MESH TOPOLOGY DIAG VIRTUAL DEVICE %d:\n", diag_idx));
        }
        ...
        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("1");
        PRT_CELL_SET("%u", mesh_topology_info.mesh_topology_diag[diag_idx].indication_1);

        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("2");
        PRT_CELL_SET("%u", mesh_topology_info.mesh_topology_diag[diag_idx].indication_2);

        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("3");
        PRT_CELL_SET("%u", mesh_topology_info.mesh_topology_diag[diag_idx].indication_3);

        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("4");
        PRT_CELL_SET("%u", mesh_topology_info.mesh_topology_diag[diag_idx].indication_4);
        ...
        PRT_CELL_SET("%u", mesh_topology_info.mesh_topology_diag[diag_idx].rx_control_cells_type1);

        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("rx_control_cells_type2");
        PRT_CELL_SET("%u", mesh_topology_info.mesh_topology_diag[diag_idx].rx_control_cells_type2);

        PRT_ROW_ADD(PRT_ROW_SEP_NONE);
        PRT_CELL_SET("control_cells_type3");
        PRT_CELL_SET("%u", mesh_topology_info.mesh_topology_diag[diag_idx].control_cells_type3);
        ...
        BCM_PBMP_CLEAR(type2_links);

        if (is_flexible_link_mapping_supported)
        {
            BCM_PBMP_ITER(PBMP_SFI_ALL(unit), port_idx)
            {
                if (mesh_topology_info.mesh_topology_diag[diag_idx].type2_list[port_idx] !=
                    SOC_DNXC_FABRIC_MESH_TOPOLOGY_INVALID_LINK)
                {
                    BCM_PBMP_PORT_ADD(type2_links, port_idx);
                }
            }
        }
        else
        {
            _SHR_PBMP_PORTS_RANGE_ADD(type2_links, 0, mesh_topology_info.mesh_topology_diag[diag_idx].link_list_count);
        }

        printed_link = 0;
        BCM_PBMP_ITER(type2_links, link_idx)
        {
            if ((printed_link % DIAG_DNXC_MAX_NOF_MESH_TOPOLOGY_CLMNS) == 0)
            {
                PRT_ROW_ADD(PRT_ROW_SEP_NONE);
            }
            printed_link++;
            ...
            if (mesh_topology_info.mesh_topology_diag[diag_idx].type2_list[link_idx] !=
                SOC_DNXC_FABRIC_MESH_TOPOLOGY_TYPE_2_LINK_NOT_AVAILABLE)
            {
                PRT_CELL_SET("0x%x", mesh_topology_info.mesh_topology_diag[diag_idx].type2_list[link_idx]);
            }
            else
            {
                PRT_CELL_SET("N/A");
         ...
         */
        LOG_CLI(("status 1:\n"));
        LOG_CLI(("status_1 %u \n", mesh_topology_info.mesh_topology_diag[diag_idx].status_1));
        if (mesh_topology_info.mesh_topology_diag[diag_idx].status_1_id1 == SOC_DNXC_FABRIC_MESH_TOPOLOGY_INVALID_LINK)
        {
            LOG_CLI(("status_1_id1 N/A \n"));
        }
        else
        {
            LOG_CLI(("status_1_id1 %u \n", mesh_topology_info.mesh_topology_diag[diag_idx].status_1_id1));
        }
        LOG_CLI(("status_1_id2 %u \n", mesh_topology_info.mesh_topology_diag[diag_idx].status_1_id2));
        /*
         * Print mesh topology Status table 1 }
         */
         ...
         */
        LOG_CLI(("status 2:\n"));
        LOG_CLI(("status_2 %u \n", mesh_topology_info.mesh_topology_diag[diag_idx].status_2));
        if (mesh_topology_info.mesh_topology_diag[diag_idx].status_1_id1 == SOC_DNXC_FABRIC_MESH_TOPOLOGY_INVALID_LINK)
        {
            LOG_CLI(("status_2_id1 N/A \n"));
        }
        else
        {
            LOG_CLI(("status_2_id1 %u \n", mesh_topology_info.mesh_topology_diag[diag_idx].status_2_id1));
        }
        LOG_CLI(("status_2_id2 %u \n\n", mesh_topology_info.mesh_topology_diag[diag_idx].status_2_id2));
        /*
         * Print mesh topology Status table 2 }
         */
    }
...
sh_sand_cmd_t sh_dnxc_fabric_cmds[] = {
    ...
    {"cgm",             NULL,                          sh_dnx_fabric_cgm_cmds,   NULL,                              &dnx_fabric_cgm_man, NULL, NULL, SH_CMD_CONDITIONAL, sh_cmd_is_dnx_cgm},
