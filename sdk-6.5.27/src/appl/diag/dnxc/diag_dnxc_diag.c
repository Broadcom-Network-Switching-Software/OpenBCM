...
extern shr_error_e access_dnxc_diag_counter_cmd(
    int unit,
    args_t * args,
    sh_sand_control_t * sand_control);
#endif
    ...
    sh_sand_control_t * sand_control)
{
    uint32 avs_val[2];
#ifdef BCM_DNX_SUPPORT
    uint32 core_grp_idx, core_grp_num;
#endif
    ...
    if (SOC_IS_DNX(unit))
    {
        core_grp_num = dnx_data_device.general.nof_core_groups_get(unit);
        if (core_grp_num == 1)
        {
        ...
        {
            for (core_grp_idx = 0; core_grp_idx < core_grp_num; core_grp_idx++)
            {
                LOG_CLI(("AVS Status for core group %d : 0x%x\n", core_grp_idx, avs_val[core_grp_idx]));
            }
        }
    }
    else
