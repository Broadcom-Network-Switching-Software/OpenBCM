    ...
    sh_sand_control_t * sand_control)
{
    ...
    if (SOC_IS_DNXF(unit))
    {
        ...
        SHR_IF_ERR_EXIT(soc_dnxf_port_max_port_get(unit, &map_size));
    }
    else
#endif
    {
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        {
            PRT_COLUMN_ADD_FLEX(PRT_FLEX_ASCII, "OTN Lane");
        }
    }
#endif
    /*
    ...
    SHR_ALLOC(serdes_map, nof_lanes * sizeof(bcm_port_lane_to_serdes_map_t), "Lane to serdes mapping struct",
              "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    SHR_IF_ERR_EXIT(bcm_port_lane_to_serdes_map_get(unit, flags, map_size, serdes_map));
    ...
    if (print_lane != DIAG_DNXC_OPTION_ALL)
    {
        ...
        if (SOC_IS_DNXF(unit))
        {
            SHR_IF_ERR_EXIT(soc_dnxf_drv_fmac_lane_real_get(unit, print_lane, &print_lane_temp));
            print_lane = print_lane_temp;
        }
#endif
    ...
    for (lane_id = start_lane; lane_id <= end_lane; ++lane_id)
    {
        ...
        if (SOC_IS_DNXF(unit))
        {
            ...
            SHR_IF_ERR_EXIT(soc_dnxf_drv_fmac_lane_aligned_get(unit, lane_id, &aligned_lane_id));
        }
        else
#endif
        {
                ...
                if (is_active)
                {
                        ...
                        && dnx_data_nif.otn.feature_get(unit, dnx_data_nif_otn_is_supported))
                    {
                        if (DNX_ALGO_PORT_TYPE_IS_OTN_PHY(unit, port_info))
                        {
                            ...
                            {
                                BCM_PBMP_COUNT(phys, nof_phy_lanes);
                                SHR_IF_ERR_EXIT(imb_port_lane_map_get
                                                (unit, logical_port, 0, nof_logical_lanes, lane_order, &actual_size));
                                logical_lane =
                                    nof_phy_lanes ==
                                    actual_size ? lane_order[internal_lane] : lane_order[internal_lane * 2];
                            }
                        }
                        else
                        {
        ...
        if (SOC_IS_DNXF(unit))
        {
            PRT_CELL_SET("%d", aligned_lane_id);
        }
        else
#endif
        {
        ...
        if (SOC_IS_DNX(unit))
        {
            ...
            if (dnx_data_nif.global.otn_supported_get(unit)
                && dnx_data_nif.otn.feature_get(unit, dnx_data_nif_otn_is_supported))
            {
                if (is_active && (logical_lane != -1))
                {
                    if (nof_phy_lanes == actual_size)
                    {
                        PRT_CELL_SET("%d", logical_lane);
                    }
                    else
                    {
                        PRT_CELL_SET("%d,%d", logical_lane, logical_lane + 1);
                    }
                }
                else
                {
