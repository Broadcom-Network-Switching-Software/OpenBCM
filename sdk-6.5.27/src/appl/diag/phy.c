...
_phy_diag_linkcat(int unit, bcm_pbmp_t pbmp, args_t *args)
{
    ...
    if ( mode != NULL ) {
        ...
        } else if ( !sal_strcasecmp(mode, "pre_rx") ) {
            mode_val = phymodlinkCAT_PRE_RX;
        } else if ( !sal_strcasecmp(mode, "rx_measure") ) {
            mode_val = phymodlinkCAT_RX_MEASURE;
        } else {
    ...
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        ...
        linkcat_config.mode = mode_val;
        linkcat_config.donot_change_osr = 0;/* always set to 0 in SDK6 */
        rv = phymod_phy_linkcat(&access, linkcat_config);
        if (rv == BCM_E_UNAVAIL) {
...
_phy_diag_berproj(int unit, bcm_pbmp_t pbmp, args_t *args)
{
    ...
    bcm_error_t rv = BCM_E_NONE;
    char *if_str;
    int phy_unit = 0, phy_unit_value = 0, phy_unit_if;
    uint32 inst;
    soc_port_phy_ber_proj_params_t params;
    soc_phy_prbs_errcnt_t** prbs_errcnt = NULL;
    uint32 time_remaining, sample_time;
#ifdef PORTMOD_SUPPORT
    ...
    /* set max sample time per accumulation, default is measurement time */
    sample_time = params.ber_proj_timeout_s;
#ifdef PORTMOD_SUPPORT
    DPORT_BCM_PBMP_ITER(unit, pbmp, dport, port) {
        phymod_core_access_t_init(&core);
        phymod_core_info_t_init(&core_info);
        portmod_port_main_core_access_get(unit, port, -1, &core, &nof_cores);
        if(nof_cores == 0) {
             continue;
        }
        rv = phymod_core_info_get(&core, &core_info);
        if (rv != BCM_E_NONE) {
            res = CMD_FAIL;
            goto exit;
        }
        if ((core_info.core_version == phymodCoreVersionBlackhawk16) ||
            (core_info.core_version == phymodCoreVersionTscbh16) ||
            (core_info.core_version == phymodCoreVersionTscbhFlexE)) {
            sample_time = 5;
            break;
        } else {
            sample_time = params.ber_proj_timeout_s;
    ...
    while (time_remaining > 0) {
        if (time_remaining > sample_time) {
            sal_sleep(sample_time);
            time_remaining = time_remaining - sample_time;
            cli_out(".");
        } else {
            sal_sleep(time_remaining);
            time_remaining = 0;
...
_print_timesync_config(bcm_port_phy_timesync_config_t *conf)
{
    ...
    cli_out("OriginalTimecodeSeconds = 0x%08x_%08x\n",
...
_print_heartbeat_ts(int unit, bcm_port_t port)
{
    ...
    cli_out("Heartbeat TS = 0x%08x_%08x\n",
...
{
    int rv;
    bcm_port_phy_timesync_enhanced_capture_t  capt;

    rv = bcm_port_phy_timesync_enhanced_capture_get(unit, port, &capt);
    if (rv != BCM_E_NONE) {
        cli_out("bcm_port_control_phy_timesync_get "
                "failed with error  u=%d p=%d %s\n",
                unit, port, bcm_errmsg(rv));
    ...
    cli_out("Capture   TS = 0x%08x_%08x  SPCI/MAC = 0x%08x_%08x\n",
            COMPILER_64_HI(capt.timestamp), COMPILER_64_LO(capt.timestamp),
            COMPILER_64_HI(capt.source_port_identity),
            COMPILER_64_LO(capt.source_port_identity) );
    cli_out("\t  Dir=%cx MsgType=%x Protocol=0x%x"
            " Domain=0x%x SeqID=0x%x SrcPort=0x%x Vlan=0x%x\n",
            ((bcmPortPhyTimesyncDirIngress == capt.direction) ? 'R' : 'T'),
            (uint32) capt.msg_type, (uint32) capt.protocol,
             capt.domain, capt.seq_id, (uint32) capt.source_port, (uint32) capt.vlan_id);
}
 ...
 * corrected and uncorrected FEC codewords, bits error and symbol error for a given port.
 * The command then uses bits error count to compute the pre-FEC bit error rate for the
 * port, and uses sybmol error count to compute symbol error rate for each lane of the port.
 * FECStat can be used to collect FEC status counters anytime there is FEC traffic
 * so there is no need to stop real traffic and dedicate the port for diagnostics purposes.
 */
...
typedef struct fec_stat_pinfo_s {
    ...
    double ser[PM_MAX_LANES];   /*  updated every interval */
} fec_stat_pinfo_t;
...
fec_stat_fec_type_get(int unit, bcm_port_t port, bcm_port_phy_fec_t *fec_type)
{
    ...
    *fec_type = fspi->fec_type;
    ...
    if ((fspi->fec_type == bcmPortPhyFecRs544) ||
        ...
        (fspi->fec_type == bcmPortPhyFecRsFec) ||
...
STATIC int
fec_stat_ser_compute(int unit, bcm_port_t port)
{
    int idx, num_symbols_per_cw;
    double rate;
    double total_symbols_received_per_second = 1.0;
    double time_to_receive_one_cw = 1.0;
    fec_stat_pinfo_t *fspi;
    fec_stat_cb_t *fscb;
    ...
    rate =  (fspi->speed / 1000) * 1000.0 * 1000 * 1000;
    if (fspi->fec_type == bcmPortPhyFecRsFec) {
        time_to_receive_one_cw = 5280 / ((rate * 66)/64);
        num_symbols_per_cw = 528;
    } else if ((fspi->fec_type == bcmPortPhyFecRs272) ||
              (fspi->fec_type == bcmPortPhyFecRs272_2xN)) {
        time_to_receive_one_cw = 2720 / ((((rate * 66) / 64) * 272) / 264);
        num_symbols_per_cw = 272;
    } else if ((fspi->fec_type == bcmPortPhyFecRs544) ||
              (fspi->fec_type == bcmPortPhyFecRs544_2xN)) {
        time_to_receive_one_cw = 5440/ ((((rate * 66) / 64) * 544) / 528);
        num_symbols_per_cw = 544;
    } else {
        cli_out("Error: Do not support SER for FEC type: %d\n", fspi->fec_type);
        return BCM_E_UNAVAIL;
    }

    for (idx = 0; idx < fspi->lanes; idx++) {
        total_symbols_received_per_second = (1 / time_to_receive_one_cw) * num_symbols_per_cw;
        fspi->ser[idx] = (fsco->cur.symbol_error[idx] / fscb->secs) /
                         (total_symbols_received_per_second /fspi->lanes);

        LOG_DEBUG(BSL_LS_APPL_PHY,
                  (BSL_META_U(unit,
                     "port %d: time %d time_to_receive_one_cw = %8.2e "
                     "num_symbols_per_cw = %d  total_symbols_received = %8.2e "
                     "SER = %8.2e \n"), port, fscb->secs, time_to_receive_one_cw,
                     num_symbols_per_cw, total_symbols_received_per_second, fspi->ser[idx]));
    }
...
STATIC int
fec_stat_ber_compute(int unit, bcm_port_t port, uint32 bits_error_count)
{
    double rate;
    double total_bits_received = 1.0;
    fec_stat_pinfo_t *fspi;
    fec_stat_cb_t *fscb;
    ...
    rate =  (fspi->speed / 1000) * 1000.0 * 1000 * 1000;
    if (fspi->fec_type == bcmPortPhyFecRsFec) {
        total_bits_received = (rate * 66) / 64;
    } else if ((fspi->fec_type == bcmPortPhyFecRs272) ||
              (fspi->fec_type == bcmPortPhyFecRs272_2xN)) {
        total_bits_received = (((rate * 66) / 64) * 272) / 264;
    } else if ((fspi->fec_type == bcmPortPhyFecRs544) ||
              (fspi->fec_type == bcmPortPhyFecRs544_2xN)) {
        total_bits_received = (((rate * 66) / 64) * 544) / 528;
    } else {
        cli_out("Error: Do not support BER for FEC type: %d\n", fspi->fec_type);
        return BCM_E_UNAVAIL;
    }
    fspi->ber[0] = (bits_error_count/fscb->secs)/ total_bits_received;

    LOG_DEBUG(BSL_LS_APPL_PHY,
                  (BSL_META_U(unit,
                             "Updated BER for port %d: bits_error_count: %u "
                              "total_bits_received: %8.2e time: %d secs ber:%8.2e\n"),
                   port, bits_error_count, total_bits_received, fscb->secs, fspi->ber[0]));
                      ...
                      uint64 acc_counter, uint64 cur_counter, sal_time_t secs)
{
    ...
    if (lane != NULL) {
     ...
     /* Show counters since last show */
     format_uint64_decimal(buf, cur_counter, ',');
     LOG_CLI(("  %20s",buf));

     /* Show counters per second */
     COMPILER_64_UDIV_32(acc_counter, (uint32)secs);
     format_uint64_decimal(buf, acc_counter, ',');
     LOG_CLI(("  %12s/s\n",buf));
}
...
fec_stat_port_counter(int unit, bcm_port_t port)
{
    ...
    fec_stat_counter_show(uncorrected_name, BCM_PORT_NAME(unit, port), NULL,
                          ...
                          secs);
    if (symb_err_valid) {
        fec_stat_counter_show(bits_err_name, BCM_PORT_NAME(unit, port), NULL,
                              fsco->acc.bits_error, fsco->cur.bits_error,
                              secs);
...
fec_stat_pinfo_update(int unit, bcm_port_t port)
{
    ...
    sal_memset(fspi->ser, 0, PM_MAX_LANES * sizeof(double));
...
{
    bcm_error_t rv = BCM_E_NONE;
    uint32 count, corrected_count, error_count[PM_MAX_LANES] = {0}, bits_err_count;
    soc_port_phy_rsfec_symb_errcnt_t symbol_error_count;
    uint32 inst;
    int i, corrected_ctrl, uncorrected_ctrl, bits_err_ctrl, rsfec_err_valid = 0;
    bcm_port_phy_fec_t fec_type;
    fec_stat_pinfo_t *fspi;
    fec_stat_counter_t *fsco;
    ...
    if (fec_type == bcmPortPhyFecRsFec) {
        ...
        bits_err_ctrl = BCM_PORT_PHY_CONTROL_RS_FEC_BIT_ERROR_COUNT;
        rsfec_err_valid = 1;
    ...
    if (rsfec_err_valid) {
        ...
        for (i = 0; i < fspi->lanes; i++) {
            ...
            fsco->cur.symbol_error[i] = error_count[i];
            LOG_DEBUG(BSL_LS_APPL_PHY,
                      (BSL_META_U(unit, "lane %d: sym_error_count %d\n"),
                               i, error_count[i]));
        }

        /* Calculate SER */
        rv = fec_stat_ser_compute(unit, port);
        if (rv != BCM_E_NONE) {
            FEC_STAT_UNLOCK(unit);
            return CMD_FAIL;
        }
        ...
        /* Get bit errors */
        fec_stat_accumulate(unit, port, bits_err_ctrl, &(fsco->acc.bits_error),
                            &bits_err_count);
        if ((fspi->fec_type == bcmPortPhyFecRs544) ||
            (fspi->fec_type == bcmPortPhyFecRs544_2xN)) {
            /* Subtract bits_err_count which is added in fec_stat_accumulate. */
            COMPILER_64_SUB_32(fsco->acc.bits_error, bits_err_count);
            /* Normalize bits_error_count. */
            bits_err_count = (uint32)(bits_err_count * 544 / 514);
            COMPILER_64_ADD_32(fsco->acc.bits_error, bits_err_count);
        }
        fsco->cur.bits_error = bits_err_count;
        /* Calculate BER */
        rv = fec_stat_ber_compute(unit, port, bits_err_count);
        if (rv != BCM_E_NONE) {
            FEC_STAT_UNLOCK(unit);
            return CMD_FAIL;
        }
...
fec_stat_ber(int unit, bcm_pbmp_t pbmp, args_t *a)
{
    ...
    FEC_STAT_LOCK(unit);
    LOG_CLI(("%-10s   %s", "port", "PRE-FEC BER\n"));
    LOG_CLI(("====\n"));
    BCM_PBMP_ITER(pbmp, port) {
        fspi = &(fec_stat_cb[unit].pinfo[port]);
        ...
        if ((fspi->fec_type  == bcmPortPhyFecRsFec)  ||
            (fspi->fec_type == bcmPortPhyFecRs272)   ||
            (fspi->fec_type == bcmPortPhyFecRs544)   ||
            (fspi->fec_type == bcmPortPhyFecRs272_2xN)||
            (fspi->fec_type == bcmPortPhyFecRs544_2xN))  {
            if (fspi->ber[0]) {
    ...
    BCM_PBMP_ITER(pbmp, port) {
        ...
        if ((fspi->fec_type  == bcmPortPhyFecRsFec)  ||
            ...
            for (lane = 0; lane < fspi->lanes; lane++) {
                if (fspi->ser[lane]) {
                    LOG_CLI(("%-s[%d]     :  %4.2e\n", BCM_PORT_NAME(unit, port), lane, fspi->ser[lane]));
                } else {
                    LOG_CLI(("%-s[%d]     :  N/A\n", BCM_PORT_NAME(unit, port), lane));
...
_if_esw_phy_control(int u, args_t *a)
{
    ...
    DPORT_BCM_PBMP_ITER(u, pbm, dport, p) {
        ...
        cmd_rv = port_phy_control_update(u, p,
                        (bcm_port_phy_control_t) SOC_PHY_CONTROL_INTERFACE_MAX,
