    ...
    utilex_step_time_dnx_data * step_id_dnx_data)
{
    ...
    if (soc_is(unit, DNXF_DEVICE))
    {
        SHR_IF_ERR_EXIT(appl_dnxf_per_dev_step_time_threshold_get(unit, step_id, step_id_dnx_data));
    }
    else
#endif
#ifdef BCM_DNX_SUPPORT
/* { */
    if (soc_is(unit, DNX_DEVICE))
    {
        SHR_IF_ERR_EXIT(appl_dnx_per_dev_step_time_threshold_get(unit, step_id, step_id_dnx_data));
    }
    else
/* } */
#endif
    ...
    utilex_step_time_dnx_data * step_id_dnx_data)
{
    ...
    if (soc_is(unit, DNXF_DEVICE))
    {
        SHR_IF_ERR_EXIT(appl_dnxf_per_dev_full_time_threshold_get(unit, step_id_dnx_data));
    }
    else
#endif
#ifdef BCM_DNX_SUPPORT
/* { */
    if (soc_is(unit, DNX_DEVICE))
    {
        SHR_IF_ERR_EXIT(appl_dnx_per_dev_full_time_threshold_get(unit, step_id_dnx_data));
    }
    else
/* } */
#endif
...
appl_dnxc_clean_up(
    int unit)
{
#if defined __SANITIZE_ADDRESS__ || THREAD_SANITIZER_IS_ACTIVE == 1
    appl_dnxc_init_param_t params;
#endif /* __SANITIZE_ADDRESS__ || THREAD_SANITIZER_IS_ACTIVE == 1 */
...
#else /* __SANITIZE_ADDRESS__ || THREAD_SANITIZER_IS_ACTIVE == 1 */
