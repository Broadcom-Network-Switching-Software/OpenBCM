 ...
 *          for the APPL init from SOC properties.
 *
    ...
    utilex_step_time_dnx_data * step_id_dnx_data)
{
    ...
    if (step_id == UTILEX_SEQ_STEP_INVALID)
    {
        ...
        SHR_IF_ERR_EXIT(appl_dnxc_per_dev_full_time_threshold_get(unit, step_id_dnx_data));
    }
    else
    {
        /** The thresholds are device-dependent */
        SHR_IF_ERR_EXIT(appl_dnxc_per_dev_step_time_threshold_get(unit, step_id, step_id_dnx_data));
    }
    ...
    utilex_seq_t * seq)
{
    ...
    /** Get the application init flags */
    ...
    seq->multithread_en = (flags & APPL_INIT_ADVANCED_F_MULTITHREAD) ? 1 : 0;
    ...
    int forward)
{
    ...
    /*
     * If the seq_ptr_arr, for the device, is NULL simply exit from the function and do not perform anything(deinit).
     * This case is relevant only for forward=0, otherwise the function will exit error _SHR_E_MEMORY
     */
    if (seq_ptr_arr[unit] == NULL)
    {
        SHR_EXIT();
    }
