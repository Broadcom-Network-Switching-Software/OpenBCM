...
typedef enum dnx_pp_vlan_tag_format_default_e
{
    ...
    /**
     * Frame has S-priority VLAN TAG
     */
    DNX_PP_ETHERNET_FRAME_VLAN_FORMAT_S_PRIORITY_TAG = 29,
    ...
    int unit)
{
    ...
    /*
     * Set mapping:
     * Frame has C-VLAN TAG with priority (VID=0) - set to S-Priority!
     */
    if (!is_ext_vcp_enable)
    {
        tpid_class.flags = BCM_PORT_TPID_CLASS_INGRESS_ONLY | BCM_PORT_TPID_CLASS_OUTER_IS_PRIO;
        tpid_class.tag_format_class_id = DNX_PP_ETHERNET_FRAME_VLAN_FORMAT_S_PRIORITY_TAG;
        tpid_class.tpid1 = DNX_TPID_VALUE_C_8100_TAG;
        tpid_class.tpid2 = BCM_PORT_TPID_CLASS_TPID_ANY;
    ...
    /*
     * Set mapping:
     * Frame has S-VLAN TAG with priority (VID=0) - set to S-Priority!
     */
    if (!is_ext_vcp_enable)
    {
        tpid_class.flags = BCM_PORT_TPID_CLASS_INGRESS_ONLY | BCM_PORT_TPID_CLASS_OUTER_IS_PRIO;
    ...
    int unit)
{
    ...
    BCM_PBMP_ITER(pbmp_pp_eth_candidates, logical_port)
    {
        ...
        if ((flags & APPL_DNX_FILTERED_PORTS_FLAGS)
            || (port_interface_info.flags & APPL_DNX_FILTERED_PORT_INTERFACE_FLAGS))
        {
            continue;
        }
        if ((port_interface_info.interface == BCM_PORT_IF_ERP)
            ...
            || (port_interface_info.interface == BCM_PORT_IF_OTN)
            || ((port_interface_info.interface == BCM_PORT_IF_NIF_ETH) && (flags & BCM_PORT_ADD_IF_FRAMER_MGMT)))
        {
            continue;
        }
    ...
    int unit)
{
    ...
    /*
     * Create Tag-Struct for S-Priority TAG
     */
    tag_format_class_id = DNX_PP_ETHERNET_FRAME_VLAN_FORMAT_S_PRIORITY_TAG;
    SHR_IF_ERR_EXIT(bcm_port_tpid_class_create
                    (unit, BCM_PORT_TPID_CLASS_WITH_ID, bcmTagStructTypeSPrioTag, &tag_format_class_id));
    ...
    int unit)
{
    ...
    /*
     * Second, set back entry 0 (vlan_edit_profile=0, UNTAG) to action ID 0 (Add VSI as 1 tag), 
     * and enable POST-EVE proccessing to remove 1 tag. 
     */
    ...
    int unit)
{
    ...
    bcm_gport_t gport;
    int system_port_id;
    int my_modid, actual_modid_count = 0;
    ...
    if (nof_port)
    {
        ...
        BCM_PBMP_ITER(pbmp_eth, logical_port)
        {
            if (BCM_PBMP_MEMBER(port_config.oamp, logical_port))
                continue;
            appl_dnx_logical_port_to_sysport_get(unit, my_modid, logical_port, &system_port_id);
            BCM_GPORT_SYSTEM_PORT_ID_SET(gport, system_port_id);
            rep_array[port_idx].port = gport;
            port_idx++;
        }
