...
/**
 * \brief - Check if the given OLP port is DA OLP port
 *          We assume that all the OLP DAs in the system has local port ID as follows:
 *          <OLP DA core_group 0> + (2*cores_group ID)
 *          when the value of <OLP DA cores_group 0> is the same in all the system
 *
 * \param [in] unit - unit id
 * \param [in] local_olp_da_port - the local OLP DA ID of the first cores group
 * \param [in] logical_port - an OLP logical port that we want to check for beeing DA OLP
 * \param [in] cores_group - the cores group of the logical port that we need for the formula above
 * \param [out] is_olp_da - boolean, True if the port is OLP DA False otherwise
 *
 * \return
 *   shr_error_e
 *
 ...
 */
static shr_error_e
appl_dnx_ref_l2_port_is_olp_da(
    int unit,
    bcm_port_t local_olp_da_port,
    ...
    uint8 *is_olp_da)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(is_olp_da, _SHR_E_PARAM, "is_olp_da");

    *is_olp_da = FALSE;
    if (logical_port == (local_olp_da_port + 2 * cores_group))
    {
        *is_olp_da = TRUE;
    }
    ...
    bcm_gport_t * olp_port)
{
    ...
    /** Default value in case OLP port wasn't configured */
    *olp_port = -1;

    /** Get the OLP port bitmap */
    ...
    if (num_of_ports > 0)
    {
        int nof_cores_in_group = dnx_data_device.general.nof_cores_per_core_group_get(unit);

        /** Iterate over the OLP ports and return the port that matches the arguments */
        BCM_PBMP_ITER(pbmp, port)
        {
            ...
            /** Check the block offset and the core group */
            if ((interface_info.interface_id == olp_blk_offset) &&
                ((mapping_info.core >= (cores_group * nof_cores_in_group)) &&
                 (mapping_info.core < ((cores_group + 1) * nof_cores_in_group))))
            {
                *olp_port = port;
                break;
            }
    ...
    bcm_multicast_t mc_group_id)
{
    ...
    bcm_gport_t gport;
    bcm_multicast_replication_t rep_array[DNX_DATA_MAX_DEVICE_GENERAL_NOF_CORE_GROUPS *
                                          DNX_DATA_MAX_L2_GENERAL_LEM_NOF_DBS * SOC_MAX_NUM_DEVICES];
    int rep_array_size = 0;
    int my_modid = 0, actual_modid_count = 0;
    bcm_stk_modid_config_t modid_array[MAX_DEVICE_NOF_FAP_ID];

    SHR_FUNC_INIT_VARS(unit);

    sal_memset(rep_array, 0, SOC_MAX_NUM_DEVICES * sizeof(bcm_multicast_replication_t));

    if (dnx_data_l2.general.lem_nof_dbs_get(unit) > 1)
    {
        SHR_IF_ERR_EXIT(bcm_stk_modid_config_get_all(unit, MAX_DEVICE_NOF_FAP_ID, modid_array, &actual_modid_count));
    ...
    {
        /** For V2 MCDB Formats, need to also specify the Element type */
    ...
    else
    {
        ...
        {
            uint8 is_olp_da = FALSE;
            int cores_group_device_in_system =
                sys_port_map_array[index_in_array].core / DNX_DATA_MAX_DEVICE_GENERAL_NOF_CORES_PER_CORE_GROUP;
            /** only a single OLP port per device/core group */
            SHR_IF_ERR_EXIT(appl_dnx_ref_l2_port_is_olp_da
                            (unit, olp_port, sys_port_map_array[index_in_array].local_port,
                             cores_group_device_in_system, &is_olp_da));
            if (is_olp_da)
            {
    ...
    {
        int system_port_id;
        bcm_gport_t olp_sa_port;
        /** Get OLP SA */
        SHR_IF_ERR_EXIT(appl_dnx_l2_olp_port_get
                        (unit, cores_group, dnx_data_l2.olp.olp_learning_block_id_get(unit), &olp_sa_port));
        appl_dnx_logical_port_to_sysport_get(unit, my_modid, olp_sa_port, &system_port_id);
        BCM_GPORT_SYSTEM_PORT_ID_SET(gport, system_port_id);
        rep_array[rep_array_size].port = gport;
        ++rep_array_size;
    }
    ...
    SHR_IF_ERR_EXIT(bcm_multicast_add(unit, mc_group_id, flags, rep_array_size, rep_array));
...
/**
 * \brief - Configure learn message for OLP
 *
 * \param [in] unit - unit ID
 * \param [in] olp_port - logical port of the OLP
 * \param [in] cores_group - cores group ID
 * \param [in] mc_group_id - mc group ID
 *
 * \return
 *   shr_error_e
 *
 ...
 */
static shr_error_e
appl_dnx_init_config_learn_msg_for_olp(
    int unit,
    int cores_group,
    bcm_multicast_t mc_group_id)
{
    ...
    learn_msgs.cores_group = cores_group;
    ...
    int unit)
{
    ...
    int nof_core_groups = dnx_data_device.general.nof_core_groups_get(unit);
    int core_groups_iter = 0;
    SHR_FUNC_INIT_VARS(unit);

    /** Get the first OLP port number */
    SHR_IF_ERR_EXIT(appl_dnx_l2_olp_port_get
                    (unit, 0 /* first cores group */ , dnx_data_l2.olp.olp_fwd_block_id_get(unit), &olp_port));

    for (core_groups_iter = 0; core_groups_iter < nof_core_groups; core_groups_iter++)
    {
        mc_group_id = dnx_data_multicast.params.olp_ing_multicast_id_get(unit, core_groups_iter)->multicast_id;

        /** Make sure that an OLP port is configured */
        if (olp_port != -1)
        {
            /** Create a mc group with all the OLP ports */
            SHR_IF_ERR_EXIT(appl_dnx_open_ingress_mc_group_for_olp(unit, olp_port, core_groups_iter, mc_group_id));

            /** Set the destination of the queue to be multicast group with all the OLP ports in the fabric */
            SHR_IF_ERR_EXIT(appl_dnx_init_config_learn_msg_for_olp(unit, core_groups_iter, mc_group_id));
        }
    }
exit:
    SHR_FUNC_EXIT;
