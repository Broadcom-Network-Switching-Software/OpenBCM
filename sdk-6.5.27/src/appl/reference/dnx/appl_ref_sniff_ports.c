    ...
    bcm_port_t * mirror_port)
{
    ...
    *mirror_port = BCM_PORT_INVALID;
    ...
    /** find a free (not existent) rcy_mirror port within the required range */
    {
        /** find an unused rcy_mirror port out of the exisitng ports */
    ...
    bcm_port_t port)
{
    ...
    {
        /** create rcy_mirror port and map it to forward port */
        {
            /** map forward port to rcy_mirror port */
    ...
    {
        LOG_WARN(BSL_LS_APPL_COMMON, (BSL_META_U(unit, "No free mirror recycle ports are left for port:%d \n"), port));
    }
