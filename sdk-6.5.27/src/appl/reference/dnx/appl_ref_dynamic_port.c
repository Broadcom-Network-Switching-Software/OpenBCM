    ...
    int port)
{
    ...
    uint32 port_flags_tmp, intf_flags_tmp;
    ...
    if (in_flags & APPL_DNX_DYN_PORT_USE_SYS_PORT_MAP)
    {
        ...
        port_flags_tmp = port_flags;
        intf_flags_tmp = intf_flags;
    }
    else
    {
        ...
        SHR_IF_ERR_EXIT(bcm_port_get(unit, port, &port_flags_tmp, &interface_info, &mapping_info));
        intf_flags_tmp = interface_info.flags;
    ...
    if (!(port_flags_tmp & APPL_DNX_FILTERED_PORTS_FLAGS) && !(intf_flags_tmp & APPL_DNX_FILTERED_PORT_INTERFACE_FLAGS))
    {
        ...
        in the system. */
        rv = appl_dnx_system_port_db_get_internal(unit, sys_params->system_port_db, system_port_id, &old_sys_port_map);
    ...
    {
        {
            ...
            if (!_SHR_IS_FLAG_SET(in_flags, APPL_DNX_DYN_PORT_SKIP_COMPENSATION_PER_PORT))
            {
                /** set ingress compensation per port (used by crps and stif) */
                SHR_IF_ERR_EXIT(appl_dnx_compensation_port_delta_set
                                (unit, port, APPL_DNX_REF_COMPENSATION_ING_PORT_DELTA));
            }
        }
    ...
    int port)
{
    ...
    sal_memset(&sys_port_map, 0, sizeof(system_port_mapping_t));
    ...
    /** update voq profiles */
    SHR_IF_ERR_EXIT(appl_dnx_sys_port_voq_modify(unit, system_port_id, &sys_port_map));
