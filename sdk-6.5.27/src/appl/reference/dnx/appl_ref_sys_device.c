    ...
    bcm_port_t logical_port)
{
    ...
    if ((!(flags & APPL_DNX_FILTERED_PORTS_FLAGS))
        ...
        && (port_mapping_info.pp_dsp != 0xffffffff))
    {
    ...
    int unit)
{
     ...
     */
    dnx_rollback_journal_appl_init_state_change(unit, TRUE);
    ...
    int unit)
{
    ...
    dnx_rollback_journal_appl_init_state_change(unit, FALSE);
    ...
    bcm_pbmp_t * pbmp)
{
    ...
    BCM_PBMP_ITER(*pbmp, logical_port)
    {
        ...
        /** filter out OTN phy */
        if (port_interface_info.interface == BCM_PORT_IF_OTN)
        {
            BCM_PBMP_PORT_REMOVE(*pbmp, logical_port);
        }
        /** filter out Framer mgmt port */
        if ((port_interface_info.interface == BCM_PORT_IF_NIF_ETH) && (flags & BCM_PORT_ADD_IF_FRAMER_MGMT))
        {
            BCM_PBMP_PORT_REMOVE(*pbmp, logical_port);
        }
    }
