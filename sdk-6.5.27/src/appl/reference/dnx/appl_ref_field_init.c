    ...
    int unit)
{
    ...
    sal_strncpy_s((char *) (trap_config.name), "L4_TRAP_Do_Nothing", sizeof(trap_config.name));
    ...
    int unit)
{
    ...
    uint32 olp_mc = dnx_data_multicast.params.olp_ing_multicast_id_get(unit, 0)->multicast_id;
    ...
    int unit)
{
    ...
    uint32 olp_mc = dnx_data_multicast.params.olp_ing_multicast_id_get(unit, 0)->multicast_id;
    uint32 rcy_port = LEARN_RCY_PORT1;
    ...
    bcm_field_action_info_t action_info_get;
    int const_action_index;
    int my_modid, rcy_sys_port, actual_modid_count = 0;
    ...
    appl_dnx_logical_port_to_sysport_get(unit, my_modid, rcy_port, &rcy_sys_port);
    BCM_GPORT_SYSTEM_PORT_ID_SET(rcy_sys_port, rcy_sys_port);
    entry_info.entry_qual[0].type = fg_info.qual_types[0];
    entry_info.entry_qual[0].value[0] = cmd_type_delete;        /* Value of CMD type "refresh or delete" */
    entry_info.entry_qual[0].mask[0] = 5;
    entry_info.entry_action[0].type = fg_info.action_types[0];
    entry_info.entry_action[0].value[0] = rcy_sys_port;
         ...
         dnx_data_headers.system_headers.system_headers_mode_jericho_get(unit)))
    {
        if (!BCM_PBMP_MEMBER(port_config.rcy, LEARN_RCY_PORT1))
        {
            LOG_INFO(BSL_LOG_MODULE,
                     (BSL_META_U(unit, "RCY port should be configured in the device for the field in J1 mode\n")));
    ...
    bcm_field_action_priority_t efes_action_position)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    if (dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_egress_estimated_bta_btr_hw_config) &&
        !dnx_data_l3.feature.feature_get(unit, dnx_data_l3_feature_mc_bridge_fallback_outlifs_set))
    {
        int ebtr_value = 26;
    ...
    int unit)
{
    ...
    p_data.qual_data[1].qual_type = bcmFieldQualifyForwardingTypeRaw;
    p_data.qual_data[1].qual_arg = 1;
    
    p_data.qual_data[1].qual_value = 17;
    p_data.qual_data[1].qual_mask = 0x1F;
    ...
    p_data.qual_data[0].qual_type = bcmFieldQualifyForwardingTypeRaw;
    p_data.qual_data[0].qual_arg = 0;
    
    p_data.qual_data[0].qual_value = 17;
    p_data.qual_data[0].qual_mask = 0x1F;
