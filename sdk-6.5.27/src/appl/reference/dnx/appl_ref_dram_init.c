...
/**
* \brief
*   This function executes dram power up.
...
/**
* \brief
*   This function executes dram power up for J2P board.
*   The function should be called part of dram temperature procedure.
*   After power down was made and all condition fulfilled,
*   user may power up the DRAM and after that call API bcm_switch_dram_init.
* \param [in] unit - The unit number.
...
*/
static shr_error_e
appl_dnx_dram_power_up_handle_j2p(
    int unit)
{
#if defined(CPU_I2C_BUS_NUM_DEFAULT)
    int pok_signal;
    ...
    /**********************************************************************************************************/
    /*
     * IMPORTANT!!! This routine belong only to DNX-J2P boards. Each board should has its own sequence of I2C
     */
    /**********************************************************************************************************/
...
#if defined(CPU_I2C_BUS_NUM_DEFAULT)
    /** Open i2c mux #4 */
    SHR_IF_ERR_EXIT(cpu_i2c_write(0x70, 0x0, 0x1, 0x10));

    /** Enable HBM power source */
    SHR_IF_ERR_EXIT(cpu_i2c_write(0x21, 0xFA, 0x12, 0xF3FF));

    /** wait for powers to stabilize */
    sal_sleep(1);

    /** Disable force Power OK signal */
    SHR_IF_ERR_EXIT(cpu_i2c_read(0x40, 0x32, 0x44, &pok_signal));
    SHR_BITCLR(&pok_signal, 0);
    SHR_IF_ERR_EXIT(cpu_i2c_write(0x40, 0x32, 0x44, pok_signal));

    /** Close mux channel */
    SHR_IF_ERR_EXIT(cpu_i2c_write(0x70, 0x0, 0x1, 0x0));
...
/**
* \brief
*   This function executes dram power up for J2X board.
*   The function should be called part of dram temperature procedure.
*   After power down was made and all condition fulfilled,
*   user may power up the DRAM and after that call API bcm_switch_dram_init.
* \param [in] unit - The unit number.
...
*/
static shr_error_e
appl_dnx_dram_power_up_handle_j2x(
    int unit)
{
#if defined(CPU_I2C_BUS_NUM_DEFAULT)
    int pok_signal;
    ...
    /**********************************************************************************************************/
    /*
     * IMPORTANT!!! This routine belong only to DNX-J2X boards. Each board should has its own sequence of I2C
     */
    /**********************************************************************************************************/
    ...
    /** Enable HBM power source */
    SHR_IF_ERR_EXIT(cpu_i2c_write(0x21, 0xFA, 0x12, 0x0000));
    ...
    int unit)
{
    ...
    else if (soc_is(unit, J2P_DEVICE))
    {
        SHR_IF_ERR_EXIT(appl_dnx_dram_power_up_handle_j2p(unit));
    }
    else if (soc_is(unit, J2X_DEVICE))
    {
        SHR_IF_ERR_EXIT(appl_dnx_dram_power_up_handle_j2x(unit));
    }
    else
    {
...
/**
* \brief
*   This function executes dram power down
* \param [in] unit - The unit number.
*
* \return
*   Error code (as per 'shr_error_e').
...
/**
* \brief
*   This function executes dram power down for J2P board.
* \param [in] unit - The unit number.
*
* \return
*   Error code (as per 'shr_error_e').
...
*/
static void
appl_dnx_dram_power_down_handle_j2p(
    int unit,
    uint32 flags,
    void *userdata)
{
    ...
    /** Force Power OK signal */
    ret = cpu_i2c_read(0x40, 0x32, 0x44, &pok_signal);
    if (ret != BCM_E_NONE)
    {
        LOG_ERROR(BSL_LS_BCMDNX_DRAM,
                  (BSL_META("Force Power OK signal: Error in %s: cpu_i2c_read() returned with error.\n"),
                   FUNCTION_NAME()));
        return;
    }
    SHR_BITSET(&pok_signal, 0);
    ret = cpu_i2c_write(0x40, 0x32, 0x44, pok_signal);
    if (ret != BCM_E_NONE)
    {
        LOG_ERROR(BSL_LS_BCMDNX_DRAM,
                  (BSL_META("Force Power OK signal: Error in %s: cpu_i2c_write() returned with error.\n"),
                   FUNCTION_NAME()));
        return;
    }
    /** Open i2c mux #4 */
    ret = cpu_i2c_write(0x70, 0x0, 0x1, 0x10);
    if (ret != BCM_E_NONE)
    {
        LOG_ERROR(BSL_LS_BCMDNX_DRAM,
                  (BSL_META("Open i2c mux #4: Error in %s: cpu_i2c_write() returned with error.\n"), FUNCTION_NAME()));
        return;
    }
    /** Disable HBM power source */
    ret = cpu_i2c_write(0x21, 0xFA, 0x12, 0xF7FF);
    if (ret != BCM_E_NONE)
    {
        LOG_ERROR(BSL_LS_BCMDNX_DRAM,
                  (BSL_META("Disable HBM power source: Error in %s: cpu_i2c_write() returned with error.\n"),
                   FUNCTION_NAME()));
        return;
    }
    /** Close mux channel */
    ret = cpu_i2c_write(0x70, 0x0, 0x1, 0x0);
    if (ret != BCM_E_NONE)
    {
        LOG_ERROR(BSL_LS_BCMDNX_DRAM,
                  (BSL_META("Close mux channel: Error in %s: cpu_i2c_write() returned with error.\n"),
                   FUNCTION_NAME()));
        return;
    }
#endif
...
/**
* \brief
*   This function executes dram power down for J2X board.
* \param [in] unit - The unit number.
*
* \return
*   Error code (as per 'shr_error_e').
...
*/
static void
appl_dnx_dram_power_down_handle_j2x(
    int unit,
    uint32 flags,
    void *userdata)
{
    ...
    /** Disable HBM power source */
    ret = cpu_i2c_write(0x21, 0xFA, 0x12, 0x0100);
    if (ret != BCM_E_NONE)
    {
        LOG_ERROR(BSL_LS_BCMDNX_DRAM,
                  (BSL_META("Disable HBM power source: Error in %s: cpu_i2c_write() returned with error.\n"),
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(bcm_switch_dram_power_down_cb_register(unit, 0, appl_dnx_dram_power_down_handle_j2p, NULL));
    }
    else if (soc_is(unit, J2X_DEVICE))
    {
        SHR_IF_ERR_EXIT(bcm_switch_dram_power_down_cb_register(unit, 0, appl_dnx_dram_power_down_handle_j2x, NULL));
    }
    else
    {
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(bcm_switch_dram_power_down_cb_unregister(unit, appl_dnx_dram_power_down_handle_j2p, NULL));
    }
    else
    {
