...
#ifdef BCM_TRIUMPH_SUPPORT
    if (soc_mem_is_valid(unit, EXT_L2_ENTRYm)) {
        SOP_MEM_STATE_MAX_INDEX_GET(unit, EXT_L2_ENTRYm, ext_index_max);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EXT_L2_ENTRYm, -1);
    }
#endif /* BCM_TRIUMPH_SUPPORT */
...
#ifdef BCM_TRIUMPH_SUPPORT
    if (soc_mem_is_valid(unit, EXT_L2_ENTRYm)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EXT_L2_ENTRYm, ext_index_max);
    }
#endif /* BCM_TRIUMPH_SUPPORT */
}
...
int
lb_port_eligible(int unit, int port)
{
    int rv;

    /* In TD3, HG3 OPCODE doesn't support CPU (0) */
    if (SOC_IS_TRIDENT3X(unit) &&
        soc_feature(unit, soc_feature_higig3)) {
        int pmode = BCM_PORT_ENCAP_IEEE;
        rv = bcm_port_encap_get(unit, port, &pmode);
        if (rv == 0 && pmode == BCM_PORT_ENCAP_HIGIG3) {
            LOG_INFO(BSL_LS_APPL_TESTS,
                     (BSL_META_U(unit,
                                 "Not eligible port(%s) type (HIGIG3)\n"),
                      SOC_PORT_NAME(unit, port)));
            return 0;
        }
    }
