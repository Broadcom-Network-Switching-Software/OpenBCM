    ...
    if (soc_feature(u, soc_feature_cmicx)) {
      if (sal_strcasecmp(mem_name,"QSPI_FLASH") == 0) {
          *p = mw;
          ...
          rv = qspi_flash_test_init(u,a,&mw->parm);
          goto done;
      }
    }
#endif
...
#if defined(BCM_RPC_SUPPORT) || defined(BCM_CTSW_SUPPORT) || defined(BCM_ESW_SUPPORT) || defined(BCM_LOOP_SUPPORT) || defined(BCM_PETRA_SUPPORT)
/* { */
    /*
     * Turn off FP.
...
#define ENABLE_DYNAMIC_MEMORY_ACCESS_OFFSET 0x0084
STATIC int
direct_enable_dynamic_memories_access_cb(int unit, soc_regaddrinfo_t *ainfo, void *data)
{
    int blk,inst=0;
    SOC_INIT_FUNC_DEFS;
    ...
    if (!SOC_REG_IS_VALID(unit,ainfo->reg) ||
        (SOC_REG_BASE(unit, ainfo->reg) != ENABLE_DYNAMIC_MEMORY_ACCESS_OFFSET)) {
        SOC_EXIT;
    }
    SOC_BLOCK_ITER(unit, blk, SOC_REG_FIRST_BLK_TYPE(SOC_REG_INFO(unit, ainfo->reg).block)) {
        _SOC_IF_ERR_EXIT(soc_reg_above_64_set(unit, ainfo->reg, inst, 0, data));
        inst++;
    }
...
enable_dynamic_memories_access(int unit)
{
  ...
  soc_reg_above_64_val_t write_val_above64;
    ...
    if (SOC_IS_J2P(unit) || SOC_IS_J2X(unit) || SOC_IS_Q2A(unit)) {
        SOC_REG_ABOVE_64_ALLONES(write_val_above64);
        _SOC_IF_ERR_EXIT(soc_reg_iterate(unit, direct_enable_dynamic_memories_access_cb, write_val_above64));
    } else {
...
int ser_test(int unit, args_t *arg, void *p) {
    ...
    int mem_scan_rate = 0, mem_scan_running = 0;
#endif
#ifdef BCM_SRAM_SCAN_SUPPORT
    sal_usecs_t sram_scan_interval = 0;
    int sram_scan_rate = 0, sram_scan_running = 0;
#endif
...
memories_clear_test(int unit, args_t *a, void *pa)
{
    ...
    {
        soc_sand_control_dynamic_mem_writes(unit, j2p_dynamic_mem_enable_regs, 1);
    }
    else if (SOC_IS_J2X(unit))
    {
        soc_sand_control_dynamic_mem_writes(unit, j2x_dynamic_mem_enable_regs, 1);
    }
    /* Write to memories */
    ...
    {
        soc_sand_control_dynamic_mem_writes(unit, j2p_dynamic_mem_disable_regs, 0);
    }
    else if (SOC_IS_J2X(unit))
    {
        soc_sand_control_dynamic_mem_writes(unit, j2x_dynamic_mem_disable_regs, 0);
    }
