    ...
    PBMP_ITER(pkt_intg->rx_pbmp, port) {
        if (port < SOC_MAX_NUM_PORTS) {
            ...
            for (j = 0; j < port_exp_pkt_cnt; j++) {
                ...
                if (SOC_IS_TOMAHAWK3(unit)) {
                    ep_to_cpu_hdr38_t *ep2cpu;
                    ep2cpu = (ep_to_cpu_hdr38_t *)rx_pkt;
                    rx_ts = ep2cpu->timestamp;
                }
                ...
                if (pkt_intg->type == PKT_TYPE_L2_TS) {
                    ...
                    sum += latency;
                    LOG_INFO(BSL_LS_APPL_TESTS,
                             (BSL_META_U(unit, "-- Packet %0d rx_ts %0x TS2 %0x TS1 %0x Latency %0d\n"),
                                         j, rx_ts, ts2, ts1, latency));
                }
            }
 ...
 *
 */
void start_cmicx_timesync(int unit)
{
    uint32 rval;

    /* Enable Timestamp Generation */
    READ_NS_TIMESYNC_TS0_FREQ_CTRL_FRAC_LOWERr(unit, &rval);
    /* 250 Mhz - 4.000 ns */
    soc_reg_field_set(unit, NS_TIMESYNC_TS0_FREQ_CTRL_FRAC_LOWERr, &rval, LINCf,
                      0x40000000);
    WRITE_NS_TIMESYNC_TS0_FREQ_CTRL_FRAC_LOWERr(unit, rval);

    READ_NS_MISC_CLK_EVENT_CTRLr(unit, &rval);
    soc_reg_field_set(unit, NS_MISC_CLK_EVENT_CTRLr, &rval,
                      TIME_CAPTURE_ENABLEf, 1);
    WRITE_NS_MISC_CLK_EVENT_CTRLr(unit, rval);

    READ_NS_TIMESYNC_TS0_COUNTER_ENABLEr(unit, &rval);
    soc_reg_field_set(unit, NS_TIMESYNC_TS0_COUNTER_ENABLEr, &rval,
                      ENABLEf, 1);
    WRITE_NS_TIMESYNC_TS0_COUNTER_ENABLEr(unit, rval);
}
