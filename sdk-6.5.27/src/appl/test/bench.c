...
/* Use 256MB to prevent CPU memory caching from affecting results */
#define SAND_BENCH_BUFMEM_SIZE   0x10000000
...
benchmark_init(int u, args_t *a, void **pa)
{
    ...
    *pa = (void *) b;
    ...
    if (SOC_IS_SAND(u))
    {
        b->sizeMem = SAND_BENCH_BUFMEM_SIZE;
    } else {
#endif
...
benchmark_test(int unit, args_t *a, void *pa)
{
    ...
    soc_reg_above_64_val_t  wide_reg = { 0 };
#endif
	...
	for (i = 0; i < iter_mem; i++) {
	    sal_memset(b->bufMem, 0, b->sizeMem);
        }
    }
    benchmark_end(b);
    ...
    if (benchmark_begin(b, "memcpy sys mem", "MB",
        ...
        for (i = 0; i < iter_mem; i++) {
            sal_memcpy(b->bufMem + b->sizeMem / 2, b->bufMem, b->sizeMem / 2);
        }
    }
    benchmark_end(b);
	...
	for (i = 0; i < iter_mem; i++) {
	    sal_memset(b->bufDMA, 0, b->sizeDMA);
        }
    }
    benchmark_end(b);
    ...
    if (benchmark_begin(b, "memcpy dma mem", "MB",
        ...
        for (i = 0; i < iter_mem; i++) {
            sal_memcpy(b->bufDMA + b->sizeDMA / 2, b->bufDMA, b->sizeDMA / 2);
        }
    }
    benchmark_end(b);
...
#elif defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT)
            test_register = CMIC_DEV_REV_IDr;
#else
            test_register = 0;
#endif
        }
    ...
    if (SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)
        ...
        if (benchmark_begin(b, "read wide reg", "read",
                iter_cmic)) {
            for (i = 0; i < iter_cmic; i++) {
                SOC_IF_ERROR_RETURN(READ_ECI_INDIRECT_COMMAND_WR_DATAr(unit, wide_reg));
            }
        }
        benchmark_end(b);
    }
    ...
    if (SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)
            ...
            if (benchmark_begin(b, "write wide reg", "write",
                iter_cmic)) {
            for (i = 0; i < iter_cmic; i++) {
                SOC_IF_ERROR_RETURN(WRITE_ECI_INDIRECT_COMMAND_WR_DATAr(unit, wide_reg));
            }
        }
        benchmark_end(b);
    }
    ...
    if (SOC_IS_XGS_SWITCH(unit)) {
        ...
        sal_memset(&l2xent, 0, sizeof(l2xent));
