    ...
    test_dnxf_fabric_snake_params_t * test_params)
{
    ...
    /*print interrupts*/
    if (!SHR_BITNULL_RANGE(results.interrupts_status, 0, 32 * SOC_DNXF_SNAKE_TEST_INTERRUPT_RESULT_SIZE_IN_WORDS))
    {
...
shr_error_e test_dnxf_fabric_snake_test_lane_swap_port_clear_all(int unit)
{
    ...
    /** get nof_lanes */
    SHR_IF_ERR_EXIT(soc_dnxf_port_max_port_get(unit, &nof_lanes));

    /* configure mapping on all existing fabric ports - both attached and detached */
    SHR_IF_ERR_EXIT(soc_dnxf_port_lane_map_simple_get(unit, lane_to_serdes_map));
...
test_dnxf_fabric_snake_test(int unit, args_t *a, void *p)
{
        ...
        !(test_params->snake_test_run_flags & SOC_DNXF_SNAKE_STOP_TEST))
    {
        ...
        rv = soc_dnxf_cell_snake_test_prepare(unit, test_params->loopback_flag, &test_params->res);
        if (rv != BCM_E_NONE)
        {
            test_params->res.test_failed = 1; /*make sure test marked as failed*/
            return -1;
