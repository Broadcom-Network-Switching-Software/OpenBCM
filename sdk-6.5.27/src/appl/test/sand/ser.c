...
typedef struct ser_test_params_s {
    ...
    char* block_name_parse;         /* block name to parse */
 ...
 */
STATIC
int access_memory_ser_test_run_mem_single_instance(int unit)
{
    int rv = BCM_E_NONE, test_failed = 0, i, succeed=0;
    uint32 sleep_for_a_bit = 2500000;
    bcm_switch_event_control_t get_interrupt_count, type;
    ...
    uint32 int_mask;
    int entry_length, bits_found[] = {-1, -1};
    int nof_needed_bits, nof_bits_found = 0, random_bit, attampt_counter, toggle_counter;
    uint32 mem_buff[SOC_MAX_MEM_WORDS], mask[SOC_MAX_MEM_WORDS];
    uint32 mem_buff_restored_from_cache[SOC_MAX_MEM_WORDS],orig_mem_buff[SOC_MAX_MEM_WORDS];
    uint32 interrupt_count, new_interrupt_count;
    ...
    /* Assume the copyno is for single instance */
    if (ser_test_params->copyno == -1)
    {
        LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "Invalid copyno(instance num)\n")));
        test_failed = 1;
        rv = BCM_E_UNAVAIL;
        _SOC_IF_ERR_EXIT(rv);
    }
    inst = ser_test_params->copyno;
    interrupts = SOC_CONTROL(unit)->interrupts_info->interrupt_db_info;

    regmem = (access_regmem_id_t)ser_test_params->mem;
    rm_info = ACCESS_GET_REGMEM_INFO_BY_GLOBAL_ID(runtime_info, regmem);
    ser_test_params->interrupt = dnxc_access_ser_interrupt_get(runtime_info, rm_info, (ser_test_params->error == ECC2));
    if (ser_test_params->interrupt == -1)
    {
        LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(unit, "Can't get interrupt regmem for Mem:%s\n"),
                                    ACCESS_REGMEM_NAME(ser_test_params->mem)));
        test_failed = 1;
        rv = BCM_E_UNAVAIL;
        _SOC_IF_ERR_EXIT(rv);
    }
    ...
    LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "\nRecieved SER Arguments for memory %s(%d) are:\n"),
                                 ACCESS_REGMEM_NAME(regmem), ser_test_params->mem));
    LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "    Array=%-2d,    Entry=%-6d,    CopyNo=%-2d,    Error Type=%d\n"),
                                 ser_test_params->array_index, ser_test_params->index,
                                 ser_test_params->copyno, ser_test_params->error));
#if !defined(SOC_NO_NAMES)
    LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "    Interrupt=%s(%d)\n"),
                                 interrupts[ser_test_params->interrupt].name, ser_test_params->interrupt));
#else
    LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit, "    Interrupt=%d\n"),  ser_test_params->interrupt));
#endif

    type.event_id = ser_test_params->interrupt; 
    type.action = bcmSwitchEventMask;
    type.index = ser_test_params->copyno;
    bcm_switch_event_control_get(unit, BCM_SWITCH_EVENT_DEVICE_INTERRUPT, type, &int_mask);
    if (int_mask) 
    {
        LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(unit, "Interrupt %d Masked so skipping the test for Mem:%s\n"),
                                    ser_test_params->interrupt, ACCESS_REGMEM_NAME(regmem))); 
        _SOC_IF_ERR_EXIT(bcm_switch_event_control_set(unit, BCM_SWITCH_EVENT_DEVICE_INTERRUPT, type, 0));
    }

    nof_needed_bits = 1;
    if ((ser_test_params->error == ECC2) && (ACCESS_REGMEM_IS_ECC(rm_info)))
    {
        nof_needed_bits = 2;
    }
    ...
    sal_srand(sal_time());
    entry_length = rm_info->width_in_bits;
    _SOC_IF_ERR_EXIT(access_regmem_datamask_rw_get(unit, regmem, mask));
    /* get random bit, check if not r/o or w/o, and save it, repeat untill got all needed bits, or timed out. */
    for (attampt_counter = 0; (attampt_counter < MAX_NOF_ATTAMPTS_TO_GET_A_RW_BIT); ++attampt_counter)
    {
        ...
        if ( mask[random_bit / BIT_IN_UINT32] & (1u << random_bit % BIT_IN_UINT32) )
        {
            ...
            nof_bits_found++;
            if (nof_bits_found >= nof_needed_bits)
            {
                break;
            }
        }
        else
        {
            LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(unit, "bit %d/%d of Memory %s is not r/w bit and is being skipped, skip count = %d\n"), 
                                                   random_bit, entry_length - 1, ACCESS_REGMEM_NAME(regmem), attampt_counter ));
        }
    }
    ...
    /* Read entry */
    _SOC_IF_ERR_EXIT(access_regmem(unit, FLAG_ACCESS_DO_NOT_USE_CACHE, regmem, inst,
                                      ser_test_params->array_index, ser_test_params->index, mem_buff));
    ...
    /** Enable Bypass PARITY and ECC **/
    _SOC_IF_ERR_EXIT(dnxc_access_ser_gmoreg_bypass_ecc_set(runtime_info, rm_info->local_block_id, inst, 1));

    /** Write modified entry **/ 
    _SOC_IF_ERR_EXIT(access_regmem(unit, FLAG_ACCESS_IS_WRITE | FLAG_ACCESS_DO_NOT_USE_CACHE, regmem, inst,
                                   ser_test_params->array_index, ser_test_params->index, mem_buff));

    /** Re-activate PARITY and ECC and Read memory --> Should Trigger the Interrupt **/
    _SOC_IF_ERR_EXIT(dnxc_access_ser_gmoreg_bypass_ecc_set(runtime_info, rm_info->local_block_id, inst, 0));
    _SOC_IF_ERR_EXIT(access_regmem(unit, FLAG_ACCESS_DO_NOT_USE_CACHE, regmem, inst,
                                          ser_test_params->array_index, ser_test_params->index, mem_buff_restored_from_cache));
    ...
    if (!succeed)
    {
        LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(unit, "Memory %s Interrupt doesnt trigger(counter=%d) check to see if interrupt id %d is correct\n"),
                                    ACCESS_REGMEM_NAME(regmem), new_interrupt_count, ser_test_params->interrupt));
        test_failed = 1;
        rv = BCM_E_FAIL;
        _SOC_IF_ERR_EXIT(rv);
    }
...
STATIC int access_memory_ser_test_run_mem(access_runtime_info_t *runtime_info, access_regmem_id_t regmem)
{
    int rv = 0;
    ser_test_params_t *ser_test_params;
    const access_device_type_info_t *device_info;
    const access_device_block_t *local_block;
    const access_device_regmem_t *rm_info;
    uint32 instances_start_index, nof_instances, ii;
    const uint8 *block_instances_flags;

    ser_test_params = ser_parameters[runtime_info->unit];
    ...
    if (ser_test_params->copyno == -1)
    {
        ...
        {
            if ((block_instances_flags[ii] & ACCESS_RUNTIME_BLOCK_INSTANCE_FLAG_DISABLED) == 0)
            {
                ser_test_params->copyno = ii;
                rv = access_memory_ser_test_run_mem_single_instance(runtime_info->unit);
                if (rv != BCM_E_NONE)
                {
                    break;
                }
    ...
    {
        if ((ser_test_params->copyno) < nof_instances)
        {
            rv = access_memory_ser_test_run_mem_single_instance(runtime_info->unit);
        }
    }
...
STATIC int access_mem_ser_test_need_skip(access_runtime_info_t *runtime_info, access_regmem_id_t regmem)
{
    int rc = 0;
    if (SOC_IS_DNX(runtime_info->unit))
    {
        /* Jericho2 */
        switch (regmem)
        {
            /* Once SW enabled shapers these memories are not accessible */
            case mEPS_HEADR_CMPNS_CAL_HP_CBM:
            case mEPS_HEADR_CMPNS_CAL_LP_CBM:
            case mEPS_OTM_HIGH_CAL_CRDT_BALANCE:
            case mEPS_OTM_LOW_CAL_CRDT_BALANCE:
            /* ecc event of RTP_UNICAST_DISTRIBUTION_MEMORY can't be cleared  */
            case mRTP_UNICAST_DISTRIBUTION_MEMORY:
            /* need hardware reset device, skip it. */
            case mSCH_TOKEN_MEMORY_CONTROLLER_TMC:
            {
                rc = 1;
            }
        }

        if (SOC_IS_J2P(runtime_info->unit))
        {
            switch (regmem)
            {
                /* those memories are not accessible */
                case mIPS_MEM_740000:
                case mIPS_MEM_D60000:
                case mOAMP_RXP_R_MEP_INDEX_FIFO:
                {
                    rc = 1;
                }
            }
...
STATIC int access_mem_ser_test_cb(access_runtime_info_t *runtime_info, access_regmem_id_t regmem)
{
    int rv = 0;
    int random_index;
    ser_test_params_t *ser_test_params;
    ...
    ser_test_params = ser_parameters[runtime_info->unit];
    if (access_mem_ser_test_need_skip(runtime_info, regmem))
    {
        LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(runtime_info->unit, "Skip unjustified on Mem:%s\n"),
                                     ACCESS_REGMEM_NAME(regmem)));
    ...
    ser_test_params->mem = regmem;
    rm_info = ACCESS_GET_REGMEM_INFO_BY_GLOBAL_ID(runtime_info, regmem);

    random_index = sal_rand() % rm_info->u.mem.mem_nof_elements;
    ser_test_params->index = random_index;

    random_index = sal_rand() % rm_info->nof_array_indices;
    ser_test_params->array_index = rm_info->first_array_index + random_index;

    rv = access_memory_ser_test_run_mem(runtime_info, regmem);

    switch (rv)
    {
        case BCM_E_PARAM:
            LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(runtime_info->unit, "second skip list(param) on Mem:%s\n"),
                                                    ACCESS_REGMEM_NAME(regmem)));
            ser_test_params->param_counter++;
            break;
        case BCM_E_TIMEOUT:
            LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(runtime_info->unit, "second skip list(timeout) on Mem:%s\n"),
                                                    ACCESS_REGMEM_NAME(regmem)));
            ser_test_params->timeout_counter++;
            ser_test_params->error_mem_list[ser_test_params->error_counter++] = regmem;
            break;

        case BCM_E_UNAVAIL:
        case BCM_E_FAIL:
        case BCM_E_INTERNAL:
            LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(runtime_info->unit, "Found Error  on Mem:%s\n"),
                                                    ACCESS_REGMEM_NAME(regmem)));
            ser_test_params->error_mem_list[ser_test_params->error_counter] = regmem;
...
/*
 * Function:
 *      access_memory_ser_test_init
 * Purpose:
 *      takes care of all of init process for the ser test with new access,
 *      get arguments and make a perliminery sanity check for given args
 * Parameters:
 *      unit    - Device Number
 ...
 *      BCM_E_XXX
 */
static int access_memory_ser_test_init(int unit, ser_test_params_t *ser_test_params)
{
    access_regmem_id_t regmem;
    ...
    regmem = ACCESS_INVALID_REGMEM_ID;
    if (sal_strnlen(ser_test_params->mem_name_parse, 1) != 0)
    {
        regmem = access_parse_regmem_by_name(unit, ser_test_params->mem_name_parse);
    }
    ser_test_params->mem = regmem;
    if (ser_test_params->mem != ACCESS_INVALID_REGMEM_ID)
    {
        ser_test_params->index = parse_integer(ser_test_params->index_parse);
        ser_test_params->array_index = parse_integer(ser_test_params->array_index_parse);
    }

    blk_type = BLOCK_NOF_BLOCKS;
    if (sal_strnlen(ser_test_params->block_name_parse, 1) != 0)
    {
        blk_type = access_block_type_name_match(unit, ser_test_params->block_name_parse);
    }
    ser_test_params->block_type = blk_type;
    ...
    if (!ser_test_params->run_all)
    {
        ...
        if (regmem != ACCESS_INVALID_REGMEM_ID)
        {
            ...
            if (!ACCESS_REGMEM_IS_ECC(rm_info) &&
                !ACCESS_REGMEM_IS_PARITY(rm_info))
            {
            ...
            {
                LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(unit,"Memory \"%s\" is readonly\n"),
                                            ser_test_params->mem_name_parse));
                goto done;
            }

            if (ser_test_params->index > rm_info->u.mem.mem_nof_elements)
            {
            ...
            if ((ser_test_params->array_index < rm_info->first_array_index) || 
                (ser_test_params->array_index >= (rm_info->first_array_index + rm_info->nof_array_indices)))
            {
                LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(unit,"array index %d is invalid for memory \"%s\"\n"),
                                            ser_test_params->array_index, ser_test_params->mem_name_parse));
                goto done;
            }
            if (ser_test_params->copyno != -1)
            {
...
static int ser_access_regmem_mem_iter(
    access_runtime_info_t *runtime_info,
    uint32 in_block_type,
    int dynamic_memory)
{
    int ii, rv;
    ...
    for (local_blk_id = 0; local_blk_id < device_info->nof_blocks; local_blk_id++)
    {
        block = ACCESS_DEV_BLK_INFO(runtime_info, local_blk_id);
        block_type = block->block_type;
        if (sal_strncasecmp(access_global_block_types[block_type].block_name, "BRDC", 4) == 0)
        {
            continue;
        }
        if ((block_type == BLOCK_CMIC) || (block_type == BLOCK_IPROC))
        {
            continue;
        }

        if (in_block_type < BLOCK_NOF_BLOCKS)
        {
            if (in_block_type != block_type)
            {
                continue;
            }
        }
        ...
        {
            local_regmem = block->memories_start + ii;
            ...
            {
                LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(runtime_info->unit, "Memory %s is WriteOnly!! -- skip\n"),
                                            ACCESS_REGMEM_NAME(regmem)));
                continue;
            }
            if (ACCESS_REGMEM_IS_RO(rm_info))
            {
                LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(runtime_info->unit, "Memory %s is ReadOnly!! -- skip\n"),
                                            ACCESS_REGMEM_NAME(regmem)));
                continue;
            }
            if (ACCESS_REGMEM_IS_DYNAMIC(rm_info))
            {
                if (dynamic_memory == 0)
                {
                    LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(runtime_info->unit, "Memory %s is Dynamic!! -- skip\n"),
                                                ACCESS_REGMEM_NAME(regmem)));
                    continue;
                }
            } else if (dynamic_memory != 0)
            {
                LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(runtime_info->unit, "Memory %s is Static!! -- skip\n"),
                                            ACCESS_REGMEM_NAME(regmem)));
                continue;
            }
            ...
            {
                LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(runtime_info->unit, "Memory %s is Alias!! -- skip\n"),
                                            ACCESS_REGMEM_NAME(regmem)));
                continue;
            }

            rv = access_mem_ser_test_cb(runtime_info, regmem);
            if (rv < 0)
            {
                LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(runtime_info->unit, "ser_access_regmem_mem_iter fail for Mem:%s\n"),
                                            ACCESS_REGMEM_NAME(regmem)));
            }
 ...
 *      BCM_E_XXX
 */
int access_memory_ser_test_run(int unit, ser_test_params_t *ser_test_params)
{
    access_regmem_id_t regmem;
    access_runtime_info_t *runtime_info;
    uint32      ii;
    ...
    /** enable dynamic **/
    dnxc_access_enable_dynamic_memory_access(unit, 1);
    ...
    regmem = ser_test_params->mem;
#ifdef BCM_DNX_SUPPORT
    if (SOC_IS_DNX(unit))
    {
        if (ser_test_params->mdb)
        {
            rv = dnx_mdb_phymem_ser_verify(unit, regmem, ser_test_params->mdb);
            return rv;
        }
    }
#endif
    ...
    {
        /*rv = xor_mem_ser_test(unit, regmem);*/
        return rv;
    }
    ...
    if (ser_access_regmem_mem_iter(runtime_info,
                                   ser_test_params->block_type,
                                   ser_test_params->test_dynamic)
        < 0)
    {
    ...
    for (ii=0; ii<ser_test_params->error_counter; ii++)
    {
        LOG_INFO(BSL_LS_APPL_TESTS,(BSL_META_U(unit, "%-6d %s\n"),
                                    ii, ACCESS_REGMEM_NAME(ser_test_params->error_mem_list[ii])));
    }
...
" \n"
  ...
  "TestDynamic=<1/0>               -  Specifies if the test run on dynamic memories\n"
  "XOR=<1/0>                       -  Specifies if the test run on XOR memories\n"
...
extern int memory_ser_test_init(int unit, args_t *a, void **p)
{
    ...
    parse_table_add(&parse_table,  "Block",         PQ_STRING, "",       &(ser_test_params->block_name_parse),  NULL);
    ...
    {
        rv = access_memory_ser_test_init(unit, ser_test_params);
        goto done;
    }
    ...
    ser_test_params->skip_schan_error = 1;
    ...
    else
    {
        ...
        ser_test_params->skip_schan_error = SOC_FEATURE_GET(unit, soc_feature_ignore_schan_ecc_error);
        SOC_FEATURE_SET(unit, soc_feature_ignore_schan_ecc_error);
    }
    return rv; 
}
...
int mem_ser_test_cb(int unit, soc_mem_t mem,void *p)
{
    ...
    /* Need to review carefully the skip memory list and skip block list */
...
/*     unjustified skip list*/
       ...
       /*  jericho and above*/
        ...
        switch (mem) {
            ...
            /* SKIP JER memories, when revert 1B data, ASIC 2B ECC error interrupt generated */
            case EDB_EEDB_BANKm:
    ...
    if (SOC_IS_QUX(unit)) {
       /*  Qux */
        ...
        switch (mem) {
            ...
            /**
             *  no int or need to special handle
             */
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        switch (mem)
        {
            ...
            /* Dynamic memory, because ECC interrupt wrong */
            case DDP_BDBC_UCm:
            {
                LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit,"Skip unjustified on Mem:%s\n"),SOC_MEM_NAME(unit, mem)));
                ser_test_params->unjustified_skipped_counter++;
    ...
    if (ser_test_params->test_dynamic)
    {
        ...
        {
            LOG_INFO(BSL_LS_APPL_TESTS, (BSL_META_U(unit,"Skip static memory Mem:%s\n"),SOC_MEM_NAME(unit, mem)));
            ser_test_params->unjustified_skipped_counter++;
            return rv;
        }
        switch (mem)
...
int memory_ser_test_run(int unit, args_t *a, void *p)
{
    ...
    if (access_new_interrupt_available(unit))
    {
        rv = access_memory_ser_test_run(unit, ser_test_params);
        return rv;
    }
#endif /* BCM_ACCESS_SUPPORT */
...
extern int memory_ser_test_done(int unit, void *p)
{
    ...
    if (!ser_parameters[unit]->skip_schan_error)
    {
        SOC_FEATURE_CLEAR(unit, soc_feature_ignore_schan_ecc_error);
    }
