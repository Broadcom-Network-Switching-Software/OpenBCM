...
tpacket_setup(int unit, p_t *p)
{
    ...
    if (p->ppc > 1) { /* Multiple packets per chain */
        if (p->p_array) {
            ...
            for (i = 0; i < p->ppc; i++) {
                err = bcm_pkt_alloc(unit, PACKET_MAX_BUFFER,
                              BCM_TX_CRC_REGEN, &pkt);
                if ((!pkt) || (BCM_FAILURE(err))) {
        ...
        err = bcm_pkt_alloc(unit, PACKET_MAX_BUFFER, BCM_TX_CRC_REGEN, &(p->p_pkt));
        if ((!p->p_pkt) || (BCM_FAILURE(err))) {
...
tpacket_tx_test(int unit, args_t *a, void *pa)
{
    ...
    for (len = p->len_start; len <= p->len_end; len += p->p_l_inc) {
        FOREACH_P_PKT(p, i, pkt) {
            if (pkt == NULL)
            {
                test_error(unit, "pkt is NULL. Illegal.\n");
                return(-1);
            }
