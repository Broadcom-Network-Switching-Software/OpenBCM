...
typedef struct cint_logger_rtime_s {
    ...
    /*
     * total of all run-times measured so far
     */
    COMPILER_UINT64 total;
...
/*
 * Update the cint_logger_rtime_t members - min, max and total - from the last
 * measured value stored in the member last.  sp is variable of type
 * cint_logger_api_params_t, and item one of its members - arg_ptrs, preload,
 * call1, dispatch, call2, or total
 */
#define UPDATE_AVG_MIN_MAX(sp, item)                        \
{                                                           \
    COMPILER_64_ADD_32(sp.item.total, sp.item.last);       \
                                                            \
