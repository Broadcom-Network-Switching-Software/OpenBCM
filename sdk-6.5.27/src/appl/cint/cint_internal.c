                            ...
                            void* addr)
{
    ...
    /*
     * propagate CONST attribute from desc of this new variable, even if the
     * caller did not specify CONST in the flags argument. This helps to keep
     * structure members marked CONST really CONST. E.g. struct s may have a
     * member m which is marked CONST. Without the change below, s.m could still
     * be assigned in the CINT interpreter.
     */
    v->flags |= (desc->flags & CINT_VARIABLE_F_CONST);
    ...
    /*
     * Above, desc is being used to populate v->dt.desc. Flags was missed
     * hence propagating that as well while restricting to CONST for now.
     */
    v->dt.desc.flags |= (desc->flags & CINT_VARIABLE_F_CONST);
...
{
    /*
     * It is allowed to dereference void** and higher order pointers but not
     * void*. Also, it is allowed to assign without typecasting void* ptr to a
     * non-void* ptr and vice-versa, but not void** and higher pointers to a
     * non-void* ptr and vice-versa. Therefore, return TRUE only for variables
     * that are pointers of basetype void and pcount == 1.
     */
    return cint_is_pointer(v) && (v->dt.desc.pcount == 1) && !CINT_STRCMP(v->dt.desc.basetype, "void"); 
}
