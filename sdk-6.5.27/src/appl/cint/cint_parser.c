...
extern int cint_c_scanner_start(void*, void*, const char*); 
...
int 
cint_cparser_start_handle(cint_cparser_t* cp, void* handle, const char* file_name)
{
    if(cp == NULL) {
        return -1; 
    }

    cint_c_scanner_start(handle, cp->scanner, file_name); 
