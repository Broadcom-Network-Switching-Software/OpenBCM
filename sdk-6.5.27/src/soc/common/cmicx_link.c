...
void _soc_iproc_l2p_pbmp_update(int unit, soc_pbmp_t logical_pbmp, ls_phy_pbmp_t *phy_pbmp)
{
    ...
    int phy_port = 0;
    soc_info_t *si;
    soc_pbmp_t logical_otn_pbmp;

    si = &SOC_INFO(unit);

    SOC_PBMP_ASSIGN(logical_otn_pbmp, PBMP_OTN_ALL(unit));
    SOC_PBMP_AND(logical_otn_pbmp, logical_pbmp);
    SOC_PBMP_REMOVE(logical_pbmp, logical_otn_pbmp);
    ...
    /*
     * Assign all phy port of OTN to HW linkscan
     */
    if (SOC_PBMP_NOT_NULL(logical_otn_pbmp))
    {
        for (phy_port=0; phy_port<SOC_MAX_NUM_PORTS; phy_port++)
        {
            ...
            {
                if (SOC_PBMP_MEMBER(logical_otn_pbmp, port))
                {
                    LS_PHY_PBMP_PORT_ADD(*phy_pbmp, phy_port);
                }
            }
...
void _soc_iproc_p2l_pbmp_update(int unit, ls_phy_pbmp_t phy_pbmp, soc_pbmp_t *logical_pbmp)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        {
            if (IS_OTN_PORT(unit, port))
            {
                otn_link = 0;
                rv = dnx_algo_port_nif_phys_get(unit, port, 0, &otn_phys_pbmp);
                if (rv == 0)
                {
                    otn_link = 1;
                    BCM_PBMP_ITER(otn_phys_pbmp, phy_port)
                    {
 ...
 * Purpose:
 *      Get the current HW link status and cache it.
 * Parameters:
 *      unit number
 * Returns:
 *      SOC_E_NONE
...
int soc_cmicx_linkscan_hw_link_cache_get(int unit, soc_pbmp_t *hw_link)
{
    ...
    soc_pbmp_t link_force_down;
    char buf[SOC_PBMP_FMT_LEN];

    CMICX_LINK_LOCK(unit);

    /* Cache link status and link down indicators from queue. */
    SOC_PBMP_ASSIGN(soc->cmicx_link_stat_cache, soc->cmicx_link_stat);
    SOC_PBMP_ASSIGN(soc->cmicx_link_latch_down_cache, soc->cmicx_link_latch_down);

    /* Clear link down indicator after cache. */
    SOC_PBMP_CLEAR(soc->cmicx_link_latch_down);

    /*
     * If there was ever link down event happened and final link is up.
     * Then force the corresponding port to down status and send a interrupt
     * to inform linkscan thread to restore the link status to up.
     */
    SOC_PBMP_ASSIGN(link_force_down, soc->cmicx_link_latch_down_cache);
    SOC_PBMP_AND(link_force_down, soc->cmicx_link_stat_cache);
    if (SOC_PBMP_NOT_NULL(link_force_down)) {

        /* Alter the link up status to be down. */
        SOC_PBMP_REMOVE(soc->cmicx_link_stat_cache, link_force_down);

        /*
         * Inform linkscan thread to restore the link status
         * from soc->cmicx_link_stat in next cache_get.
         */
    ...
    SOC_PBMP_ASSIGN(*hw_link, soc->cmicx_link_stat_cache);

    LOG_DEBUG(BSL_LS_SOC_M0,
             (BSL_META_U(unit, "soc->cmicx_link_stat_cache:%s\n"),
              SOC_PBMP_FMT(soc->cmicx_link_stat_cache, buf)));
    LOG_DEBUG(BSL_LS_SOC_M0,
             (BSL_META_U(unit, "soc->cmicx_link_stat:%s\n"),
              SOC_PBMP_FMT(soc->cmicx_link_stat, buf)));
    LOG_DEBUG(BSL_LS_SOC_M0,
             (BSL_META_U(unit, "soc->cmicx_link_latch_down_cache:%s\n"),
              SOC_PBMP_FMT(soc->cmicx_link_latch_down_cache, buf)));
    LOG_DEBUG(BSL_LS_SOC_M0,
             (BSL_META_U(unit, "soc->cmicx_link_latch_down:%s\n"),
              SOC_PBMP_FMT(soc->cmicx_link_latch_down, buf)));
    LOG_DEBUG(BSL_LS_SOC_M0,
             (BSL_META_U(unit, "link_force_down:%s\n"),
              SOC_PBMP_FMT(link_force_down, buf)));
 ...
 * Parameters:
 *      unit number
 *      msg Linkscan message from cortex-M0
 *      interrupt 1: in interrupt context.
 * Returns:
 *      SOC_E_NONE
 */
int
soc_iproc_link_status_update(int unit, soc_iproc_mbox_msg_t *msg, int interrupt)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    ...
    if (interrupt) {
        /* Compare with previous link status and mark ports with link down.*/
        SOC_PBMP_ASSIGN(tmp, soc->cmicx_link_stat);
        SOC_PBMP_XOR(tmp, soc->cmicx_link_stat_cache);
        SOC_PBMP_AND(tmp, soc->cmicx_link_stat_cache);
        SOC_PBMP_OR(soc->cmicx_link_latch_down, tmp);
    } else {
        /* For XGS device only, in soc_cmicx_linkscan_config, cmicx linkscan
         * is paused. The link status should be synced at this point.
         * The latched down should be cleared.
         */
        SOC_PBMP_CLEAR(soc->cmicx_link_latch_down);
        SOC_PBMP_CLEAR(soc->cmicx_link_latch_down_cache);
        SOC_PBMP_ASSIGN(soc->cmicx_link_stat_cache, soc->cmicx_link_stat);
    }
    CMICX_LINK_UNLOCK(unit);
...
soc_iproc_flr_link_up(int unit, soc_iproc_mbox_msg_t *msg)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        SOC_PBMP_CLEAR(flr_linkup);
        LS_PHY_PBMP_ITER(flr_linkup_ppbmp, idx)
        {
            if (idx < SOC_MAX_NUM_PORTS)
            {
                 SOC_PBMP_PORT_ADD(flr_linkup, idx);
            }
        }
    }
    else
...
int soc_iproc_linkscan_process_message(soc_iproc_mbox_info_t *chan, soc_iproc_mbox_msg_t *msg)
{
    ...
    switch(msg->id) {
        case LS_LINK_STATUS_CHANGE:
            LOG_DEBUG(BSL_LS_SOC_M0, (BSL_META_U(chan->unit, "HW link change\n")));
            return soc_iproc_link_status_update(chan->unit, msg, 1);
        case LS_HW_CONFIG:
            LOG_DEBUG(BSL_LS_SOC_M0, (BSL_META_U(chan->unit, "HW link config\n")));
            if (IS_IPROC_RESP_SUCCESS(msg)) {
                ...
                if ((!SOC_IS_DNXF(chan->unit)) && (!SOC_IS_DNX(chan->unit))) {
                    ...
                    soc_iproc_link_status_update(chan->unit, msg, 1);
                }
            ...
            break;
        case LS_FLR_LINKUP:
            LOG_DEBUG(BSL_LS_SOC_M0, (BSL_META_U(chan->unit, "HW FLR link up\n")));
...
/*!
 * \brief configure loopback info to hw linkscan m0 firmware.
 *
 ...
 */
int
soc_cmicx_linkscan_loopback_info_set(int unit, int pport, int enable)
{
    uint32 value;
    FWLINKSCAN_LOOPBACKr_t loopback;
    ...
    SOC_IF_ERROR_RETURN
        (READ_FWLINKSCAN_LOOPBACKr(LS_SHMEM(unit), pport / 32, &loopback));
    value = FWLINKSCAN_LOOPBACKr_GET(loopback);
    value &= ~(1 << (pport % 32));
    value |= (enable << (pport % 32));
    FWLINKSCAN_LOOPBACKr_SET(loopback, value);
    SOC_IF_ERROR_RETURN
        (WRITE_FWLINKSCAN_LOOPBACKr(LS_SHMEM(unit), pport / 32, loopback));
...
static soc_linkscan_drv_t cmicx_linkscan_drv  = {
     ...
     .loopback_info_set = soc_cmicx_linkscan_loopback_info_set
};
...
int soc_cmicx_linkscan_dump(int unit)
{
    ...
    cli_out("Loopback info:\n    ");
    for (ii=0; ii<BYTES2WORDS(FWLINKSCAN_LOOPBACKr_SIZE); ii++)
    {
        SOC_IF_ERROR_RETURN(READ_FWLINKSCAN_LOOPBACKr(LS_SHMEM(unit), ii, &loopback));
        for (kk=0; kk<32; kk++)
        {
            if (FWLINKSCAN_LOOPBACKr_GET(loopback) & (1 << kk))
            {
                pport = ii * 32 + kk;
                cli_out("%d ", pport);
            }
