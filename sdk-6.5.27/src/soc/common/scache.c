...
soc_scache_commit_specific_data(int unit, soc_scache_handle_t handle, uint32 data_size, uint8 *data, int offset)
{
    ...
    /* Writing to scache during deinit is not allowed*/
    if (SOC_IS_DETACHING(unit)) {
        /* Ideally should return an error. Currently we ignore the write because we know there
           is a write to scache during de-init, and we do not want it to invalidate the scache */
        LOG_DEBUG(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                           ...
                           soc_free_func_t free_f)
{
    ...
    stable->rf = rf;
    stable->wf = wf;
    stable->alloc_f = alloc_f ? alloc_f : ((SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) ? soc_scache_defl_dnx_alloc : soc_scache_defl_alloc);
    stable->free_f  = free_f ? free_f : ((SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) ? soc_scache_defl_dnx_free : soc_scache_defl_free);
