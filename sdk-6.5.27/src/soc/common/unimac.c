...
#if defined(PHYMOD_SUPPORT) && defined(PHYMOD_QTCE_SUPPORT)
    if (SOC_IS_GREYHOUND2(unit) && (pm_phy->type == phymodDispatchTypeQtce)) {
        /* Notify phy driver to override SD and RX_LOCK for MAC loopback. */
        SOC_IF_ERROR_RETURN
            (soc_phyctrl_notify(unit, port, phyEventMacLoopback, loopback));
    }
...
static int
mac_uni_timesync_tx_info_get (int unit, soc_port_t port,
                               soc_port_timesync_tx_info_t *tx_info)
{
    ...
    if (SOC_REG_IS_VALID(unit,TX_TS_SEQ_IDr)) {
        ...
        tx_info->sequence_id = soc_reg_field_get(unit,
                    TX_TS_SEQ_IDr, rval, TSTS_SEQ_IDf);
    }

    if (SOC_REG_IS_VALID(unit,TX_TS_DATAr)) {
        SOC_IF_ERROR_RETURN(READ_TX_TS_DATAr(unit, port, &rval));
        tx_info->timestamps_in_fifo = soc_reg_field_get(unit, TX_TS_DATAr, rval, TX_TS_DATAf);
...
mac_driver_t soc_mac_uni = {
    ...
    mac_uni_timesync_tx_info_get     /* md_timesync_tx_info_get */
};
