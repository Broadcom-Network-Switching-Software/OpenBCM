    ...
    )
{
    ...
    /* 2. caculate granularity if necessary */
    if (clocks_per_sec && clocks_per_cycle)
    {
    ...
    if (granularity == 0)
    {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "granularity can't be zero \n")));
        return SOC_E_PARAM;
    }

    /* 3. caculate clocks_per_cycle/bytes_per_cycle */
    ...
    {
        /*
         * caculate bytes per cycle, given rate(Units: kbps)
    ...
    /* 4. caculate interval value for bytes per cycle */
...
soc_error_t soc_sat_gtf_bandwidth_set (
    ...
    SOC_SAND_IN uint32 gtf_rate_factor
    )
{
    int rv = SOC_E_NONE;
    soc_reg_above_64_val_t data;
    ...
    if (!(bw->flags & SOC_SAT_GTF_RATE_IN_PACKETS) && gtf_rate_factor)
    {
        /*
         * Compensate for IPG and SAT headers
         */
        max_rate = bw->rate * (gtf_rate_factor / SOC_SAT_GTF_RATE_CMP_CONVERSION);
    }
    else
    {
        max_rate = bw->rate;
    ...
    )
{
    ...
    if ((priority == SOC_SAT_GTF_OBJ_COMMON)) {
        ...
        if (clocks_per_cycle == 0) {
            LOG_ERROR(BSL_LS_SOC_SAT,
                      (BSL_META_U(unit,
                                  "Invalid value for RATE_NUM_CLKS_CYCLEf\n")));
    ...
    )
{
     ...
     */
    /* lost_packet_cnt  FlowStatAccumEntry1     RcvdPktCntrAccum bits[41:0] */
    offset_base = (80);
    len = 42;
    fld_64[0] = fld_64[1] = 0;
    SHR_BITCOPY_RANGE(fld_64, 0, reg_above64_val_3, offset_base, len);
    COMPILER_64_SET(stat->lost_packet_cnt, fld_64[1], fld_64[0]);
