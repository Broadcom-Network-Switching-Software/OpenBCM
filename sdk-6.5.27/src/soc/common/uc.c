...
static int soc_m7_in_reset(int unit, int uC, int *prv)
{
#if defined(BCM_DNX_SUPPORT)
    uint32      rst_control;
    if (SOC_IS_J2X(unit)) {
        rst_control = soc_pci_read(unit, soc_reg_addr(unit, M_7_SS_CONFIGr, REG_PORT_ANY, 0));
        *prv = !soc_reg_field_get(
                        unit, M_7_SS_CONFIGr, rst_control, CPUWAITf);
        return SOC_E_NONE;
    }
#endif
    return (SOC_E_FAIL);
...
#if defined(INCLUDE_GDPLL)
    if (SOC_INFO(unit).num_ucs == uC) {
        int halt;
        if (SOC_E_NONE == soc_m7_in_reset(unit, uC, &halt)) {
            return halt;
        }
    }
#endif
...
#endif /* BCM_IPROC_SUPPORT */
#if defined(BCM_CMICM_SUPPORT) && (defined(BCM_ESW_SUPPORT) || defined(BCM_PETRA_SUPPORT))
    if (soc_feature(unit, soc_feature_mcs)) {
 ...
 */
{
    ...
    for (i = 0; i < len; i += 4, addr += 4) {
        ...
        if (SOC_INFO(unit).num_ucs == uC) {
            if (SOC_IS_J2X(unit)) {
                rc = soc_pci_write(unit, addr, wdata);
                if (rc != SOC_E_NONE) {
                    SOC_CMIC_UCDBG_LOG_ADD((unit, soc_pci_write,
                        "%s(): soc_pci_write failed uC:%d addr:0x%x len:%d\n",
...
#if defined(INCLUDE_GDPLL)
static int soc_uc_m7_reset(int unit, int uC)
{
#if defined(BCM_DNX_SUPPORT)
    uint32      rst_control;
    if (SOC_IS_J2X(unit)) {
        /* J2X, with uc:4 is ARM-M7 */
        rst_control = soc_pci_read(unit, soc_reg_addr(unit, M_7_SS_CONFIGr, REG_PORT_ANY, 0));
        soc_reg_field_set(unit, M_7_SS_CONFIGr, &rst_control,
                      CPUWAITf, 1);
        soc_pci_write(unit, soc_reg_addr(unit, M_7_SS_CONFIGr, REG_PORT_ANY, 0), rst_control);

        rst_control = soc_pci_read(unit, soc_reg_addr(unit, M_7_SS_CTRLr, REG_PORT_ANY, 0));
        soc_reg_field_set(unit, M_7_SS_CTRLr, &rst_control,
                      POWERUP_RESETf, 1);
        soc_reg_field_set(unit, M_7_SS_CTRLr, &rst_control,
                      SYS_RESETf, 1);
        soc_pci_write(unit, soc_reg_addr(unit, M_7_SS_CTRLr,  REG_PORT_ANY, 0), rst_control);
...
#if defined(INCLUDE_GDPLL)
    if (SOC_INFO(unit).num_ucs == uC) {
        if (SOC_E_NONE == soc_uc_m7_reset(unit, uC)) {
            return SOC_E_NONE;
        }
    }
#endif /* INCLUDE_GDPLL */
...
#if defined(INCLUDE_GDPLL)
static int soc_uc_m7_start(int unit, int uC)
{
#if defined(BCM_DNX_SUPPORT)
    uint32  rst_control;
    if (SOC_IS_J2X(unit)) {
...
#if defined(INCLUDE_GDPLL)
    if (SOC_INFO(unit).num_ucs == uC) {
        if (SOC_E_NONE == soc_uc_m7_start(unit, uC)) {
            return SOC_E_NONE;
        }
    }
#endif
