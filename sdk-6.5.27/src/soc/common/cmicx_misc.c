...
soc_cmicx_handle_hotswap_and_dma_abort(int unit)
{
    ...
    int is_warm_boot = SAL_BOOT_WARM_BOOT || SOC_WARM_BOOT(unit);
    int dma_abort_in_cold_boot = soc_property_get(unit, spn_CMIC_DMA_ABORT_IN_COLD_BOOT, DEFAULT_CMIC_DMA_ABORT_IN_COLD_BOOT);

    /*
     * Always abort DMA operations on warmboot.
     * In case of cold boot, decide to abort according to 'cmic_dmx_abort_in_cold_boot' SOC property.
     */
    if (is_warm_boot || dma_abort_in_cold_boot)
    {
        /*
         * In case of warmboot, abort only CMC0.
         */
        rv = soc_cmicx_dma_abort(unit, is_warm_boot ? SOC_DMA_ABORT_CMC0 : 0);
    ...
    rv2 = soc_pcie_hot_swap_handling(unit, SOC_PCIE_HOTSWAP_HANDLING_FLAG_DONOT_ABORT | SOC_PCIE_HOTSWAP_HANDLING_FLAG_DONOT_DISABLE_INTERRUPTS);
