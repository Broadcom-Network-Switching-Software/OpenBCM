...
int             soc_ndev = 0;
/* soc_ndev_idx2dev_map is used to loop over the available units.
 * It maps integers 0..ndev-1 to the units=logical device IDs.
 ...
 *      soc_state_get
 * Purpose:
 *      Read value of 'soc_state' for specified unit.
 * Parameters:
 *      unit - Device unit #
 * Returns:
 *      uint32 - Value of soc_state. If 'unit' is illegal then
 *      return '-1'
 */
uint32
soc_state_get(int unit)
{
    uint32 ret;
 ...
 *      soc_state_set
 * Purpose:
 *      Write 'value' into 'soc_state' of specified unit.
 * Parameters:
 *      unit -
 *          Device unit #
 *      value -
            Value to load into soc_state[unit]
 * Returns:
 *      int - If 'unit' is illegal then
 *      no action is taken (no memory is written to) and a negative
 *      value is returned. Otherwise, zero is returned.
 */
int
soc_state_set(int unit, uint32 value)
{
    int ret;
 ...
 */
int
soc_flags_set(int unit, uint32 value)
{
    soc_control_t *soc;
    int ret;
...
soc_init(int unit)
{
    ...
    if(SOC_IS_DNXF(unit)) {
        return (soc_dnxf_soft_reset_init(unit));
    }
#endif /*  defined(BCM_DNXF_SUPPORT)  */
#if defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
    if(SOC_IS_LTSW(unit)) {
        return (soc_ltsw_init(unit));
    }
#endif /* BCM_LTSW_SUPPORT | BCM_CTSW_SUPPORT */
    {
...
soc_device_reset(int unit, int mode, int action)
{
    ...
    if (SOC_IS_DNX(unit)) {
        if ((SOC_IS_J2P(unit) || SOC_IS_J2X(unit)) && ((mode == SOC_DNX_RESET_MODE_BLOCKS_SOFT_RESET) || (mode == SOC_DNX_RESET_MODE_BLOCKS_AND_FABRIC_SOFT_RESET)))
        {
            LOG_ERROR(BSL_LS_SOC_COMMON,
...
soc_attached(int unit)
{
    ...
    return (SOC_CONTROL(unit) && ((SOC_FLAGS_GET(unit) & SOC_F_ATTACHED) != 0));
}
...
#if defined(BCM_IPROC_SUPPORT) && (defined(BCM_ESW_SUPPORT) || defined(BCM_PETRA_SUPPORT) || defined(BCM_DFE_SUPPORT))
        if (soc_feature(unit, soc_feature_iproc)) {
...
soc_xgxs_reset(int unit, soc_port_t port)
{
    soc_reg_t           reg;
    uint64              rval64;
#if defined(BCM_IPROC_SUPPORT) && defined(BCM_ESW_SUPPORT)
    uint32              rval;
#endif
    ...
    if (
#ifdef BCM_ESW_SUPPORT
        SOC_REG_FIELD_VALID(unit, reg, LCREFENf) ||
#endif
...
#endif
    if (soc_reg_field_valid(unit,reg, rstb_hw)) {
...
#endif
    if (soc_reg_field_valid(unit, reg, txd1_g_fifo_restb)) {
...
soc_tsc_xgxs_reset(int unit, soc_port_t port, int reg_idx)
{
    ...
    if (0 == SOC_PORT_VALID_RANGE(unit, port) && !SOC_IS_BLKID_PORT(port)) {
        return SOC_E_PORT;
    }
...
#ifdef BCM_DNXF_SUPPORT
    if(SOC_IS_DNXF(unit)) {
        int rv = soc_dnxf_nof_block_instances(unit, block_types[0], &nof_block_instances);
        SOC_IF_ERROR_RETURN(rv);
    }
#endif
#ifdef BCM_PETRA_SUPPORT
...
#ifdef BCM_CMICX_SUPPORT
    if (soc_feature(unit, soc_feature_cmicx)) {
        soc_led_driver_attach(unit, &cmicx_led_driver);
    }
#endif
    return SOC_E_NONE;
}
...
#define SOC_MEMSTATE_SHARED_SPACE 1

#define SOC_MEMSTATE_SEPARATE_SPACE 2

typedef struct soc_memstate_alloc_method_s {
    /*
     * 0: not allocated;
     *
     * SOC_MEMSTATE_SHARED_SPACE:
     * allocated a memState entry from a reserved space shared with other mems.
     *
     * SOC_MEMSTATE_SEPARATE_SPACE:
     * allocated a space separately.
     */
    uint8 method[2][NUM_SOC_MEM];

    /* Extra soc_memstate_t entries. */
    soc_memstate_t *extra_soc_memstate;
    /* Extra sop_memstate_t entries. */
    sop_memstate_t *extra_sop_memstate;
    /* Index the first free soc_memstate_t entry. */
    uint16 soc_idx_free;
    /* Index the first free sop_memstate_t entry. */
    uint16 sop_idx_free;
} soc_memstate_alloc_method_t;

static soc_memstate_alloc_method_t *memstate_alloc_method[SOC_MAX_NUM_DEVICES];

/* Extra memstate entries used for invalid memory tables. */
#define SOC_MEMSTATE_EXTRA_SPACE 64

#define SOC_MEMSTATE_SOC_ALLOC_METHOD(unit, mem) \
    memstate_alloc_method[unit]->method[0][mem]

#define SOC_EXTRA_SOC_MEMSTATE_BASE(unit) \
    memstate_alloc_method[unit]->extra_soc_memstate

#define SOC_EXTRA_SOC_MEMSTATE_FREE_ENTRY(unit, index) \
    memstate_alloc_method[unit]->extra_soc_memstate[index]

#define SOC_EXTRA_SOC_MEMSTATE_FREE_IDX(unit) \
    memstate_alloc_method[unit]->soc_idx_free

#define SOC_MEMSTATE_SOP_ALLOC_METHOD(unit, mem) \
    memstate_alloc_method[unit]->method[1][mem]

#define SOC_EXTRA_SOP_MEMSTATE_BASE(unit) \
    memstate_alloc_method[unit]->extra_sop_memstate

#define SOC_EXTRA_SOP_MEMSTATE_FREE_ENTRY(unit, index) \
    memstate_alloc_method[unit]->extra_sop_memstate[index]

#define SOC_EXTRA_SOP_MEMSTATE_FREE_IDX(unit) \
    memstate_alloc_method[unit]->sop_idx_free

/*
 * Function:
 *      soc_memstate_free
 * Purpose:
 *      free all memory space allocated by soc_memstate_alloc.
 * Parameters:
 *      unit - StrataSwitch unit #
 */
void
...
{
    soc_mem_t mem;
    int sop_valid_mem_free = FALSE;
    int soc_valid_mem_free = FALSE;

    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        if (SOC_PERSIST(unit) && SOP_MEM_STATE(unit, mem)) {
            if (SOC_MEMSTATE_SOP_ALLOC_METHOD(unit, mem) ==
                SOC_MEMSTATE_SEPARATE_SPACE) {
                sal_free(SOP_MEM_STATE(unit, mem));
            } else if (!sop_valid_mem_free) {
                sal_free(SOP_MEM_STATE(unit, mem));
                sop_valid_mem_free = TRUE;
            }
            SOP_MEM_STATE(unit, mem) = NULL;
        }
        if (SOC_CONTROL(unit) && SOC_MEM_STATE(unit, mem)) {
            if (SOC_MEMSTATE_SOC_ALLOC_METHOD(unit, mem) ==
                SOC_MEMSTATE_SEPARATE_SPACE) {
                sal_free(SOC_MEM_STATE(unit, mem));
            } else if (!soc_valid_mem_free) {
                sal_free(SOC_MEM_STATE(unit, mem));
                soc_valid_mem_free = TRUE;
            }
            SOC_MEM_STATE(unit, mem) = NULL;
        }
    }
    if (memstate_alloc_method[unit]) {
        sal_free(memstate_alloc_method[unit]);
        memstate_alloc_method[unit] = NULL;
    }
}
 ...
 *      soc_memstate_alloc
 * Purpose:
 *      Allocate memstate for every valid memory table.
 * Parameters:
 *      unit - StrataSwitch unit #
 */
int
...
{
    soc_mem_t mem, *valid_mem_ptr = NULL;
    uint32 sz, num_valid_mems, valid_mem_idx;
    soc_memstate_t *memstate_soc;
    sop_memstate_t *memstate_sop;

    soc_memstate_free(unit);

    sz = sizeof(soc_memstate_alloc_method_t);
    memstate_alloc_method[unit] = sal_alloc(sz, "memStateAllocMethod");
    if (memstate_alloc_method[unit] == NULL) {
        return SOC_E_MEMORY;
    }
    sal_memset(memstate_alloc_method[unit], 0, sz);

    sz = sizeof(soc_mem_t) * NUM_SOC_MEM;
    valid_mem_ptr = sal_alloc(sz, "validMemArray");
    if (valid_mem_ptr == NULL) {
        sal_free(memstate_alloc_method[unit]);
        return SOC_E_MEMORY;
    }
    sal_memset(valid_mem_ptr, 0, sz);

    valid_mem_idx = 0;
    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        if (SOC_MEM_IS_VALID(unit, mem)) {
            valid_mem_ptr[valid_mem_idx++] = mem;
        }
    }
    num_valid_mems = valid_mem_idx;

    if (SOC_PERSIST(unit)) {
        sz = sizeof(sop_memstate_t) *
            (num_valid_mems + SOC_MEMSTATE_EXTRA_SPACE);
        memstate_sop = sal_alloc(sz, "memStateSop");
        if (memstate_sop) {
            sal_memset(memstate_sop, 0, sz);
        } else {
            sal_free(valid_mem_ptr);
            sal_free(memstate_alloc_method[unit]);
            return SOC_E_MEMORY;
        }
        valid_mem_idx = 0;
        for (; valid_mem_idx < num_valid_mems; valid_mem_idx++) {
            mem = valid_mem_ptr[valid_mem_idx];
            SOP_MEM_STATE(unit, mem) = &memstate_sop[valid_mem_idx];
            SOC_MEMSTATE_SOP_ALLOC_METHOD(unit, mem) =
                SOC_MEMSTATE_SHARED_SPACE;
        }
        SOC_EXTRA_SOP_MEMSTATE_BASE(unit) = memstate_sop + num_valid_mems;
    }
    if (SOC_CONTROL(unit)) {
        sz = sizeof(soc_memstate_t) *
            (num_valid_mems + SOC_MEMSTATE_EXTRA_SPACE);
        memstate_soc = sal_alloc(sz, "memStateSoc");
        if (memstate_soc) {
            sal_memset(memstate_soc, 0, sz);
        } else {
            soc_memstate_free(unit);
            sal_free(valid_mem_ptr);
            return SOC_E_MEMORY;
        ...
        for (; valid_mem_idx < num_valid_mems; valid_mem_idx++) {
            mem = valid_mem_ptr[valid_mem_idx];
            SOC_MEM_STATE(unit, mem) = &memstate_soc[valid_mem_idx];
            SOC_MEMSTATE_SOC_ALLOC_METHOD(unit, mem) =
                SOC_MEMSTATE_SHARED_SPACE;
        }
        SOC_EXTRA_SOC_MEMSTATE_BASE(unit) = memstate_soc + num_valid_mems;
    }
    sal_free(valid_mem_ptr);
    return SOC_E_NONE;
}
...
{
    uint32 sz = sizeof(sop_memstate_t);
    uint16 free_index;

    if (SOP_MEM_STATE(unit, mem)) {
        return;
    }
    free_index = SOC_EXTRA_SOP_MEMSTATE_FREE_IDX(unit);
    if (free_index < SOC_MEMSTATE_EXTRA_SPACE) {
        SOP_MEM_STATE(unit, mem) =
            &SOC_EXTRA_SOP_MEMSTATE_FREE_ENTRY(unit, free_index);
        SOC_EXTRA_SOP_MEMSTATE_FREE_IDX(unit)++;
        SOP_MEM_STATE(unit, mem)->index_max = -1;
        SOC_MEMSTATE_SOP_ALLOC_METHOD(unit, mem) =
             SOC_MEMSTATE_SHARED_SPACE;
        return;
    }
    SOP_MEM_STATE(unit, mem) = sal_alloc(sz, "memStateSop");
    if (SOP_MEM_STATE(unit, mem)) {
        sal_memset(SOP_MEM_STATE(unit, mem), 0, sz);
        SOP_MEM_STATE(unit, mem)->index_max = -1;
        SOC_MEMSTATE_SOP_ALLOC_METHOD(unit, mem) =
            SOC_MEMSTATE_SEPARATE_SPACE;
    }
...
{
    uint32 sz = sizeof(soc_memstate_t);
    uint16 free_index;

    if (SOC_MEM_STATE(unit, mem)) {
        return;
    }
    free_index = SOC_EXTRA_SOC_MEMSTATE_FREE_IDX(unit);
    if (free_index < SOC_MEMSTATE_EXTRA_SPACE) {
        SOC_MEM_STATE(unit, mem) =
            &SOC_EXTRA_SOC_MEMSTATE_FREE_ENTRY(unit, free_index);
        SOC_EXTRA_SOC_MEMSTATE_FREE_IDX(unit)++;
        SOC_MEMSTATE_SOC_ALLOC_METHOD(unit, mem) =
             SOC_MEMSTATE_SHARED_SPACE;
        return;
    }
    SOC_MEM_STATE(unit, mem) = sal_alloc(sz, "memStateSoc");
    if (SOC_MEM_STATE(unit, mem)) {
        sal_memset(SOC_MEM_STATE(unit, mem), 0, sz);
        SOC_MEMSTATE_SOC_ALLOC_METHOD(unit, mem) =
            SOC_MEMSTATE_SEPARATE_SPACE;
    }
}
