...
soc_pci_getreg(int unit, uint32 addr, uint32 *datap)
{
    ...
    if (SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) {
        assert(addr < 0x50000 || ((addr<7*1024*1024+128*1024) && (addr >= 7*1024*1024)) || (addr >= 0x608000 && addr < 0x646000) ||
               (addr >= 0x600000 && addr < 0x604000)); /* check that the address is in the currently supported address ranges of the 2nd BAR in DNX devices */
    }
