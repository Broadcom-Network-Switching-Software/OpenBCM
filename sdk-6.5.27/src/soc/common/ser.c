...
#define _SOC_ACC_TYPE_FOR_PIPE(pipe) \
                              ...
                              int err_type)
{
    ...
    if (SOC_IS_FIRELIGHT(unit)) {
        ...
        SOC_MEM_STATE_MEMBER_GET(unit, cur_ser_info->mem, corrupt[copyno], corrupt);
        if (corrupt != NULL) {
    ...
    defined(BCM_GREYHOUND_SUPPORT)
            ...
            SOC_MEM_STATE_MEMBER_GET(unit, cur_mem, corrupt[copyno], corrupt);
            ...
            if ((corrupt == NULL) || (TCAM_CORRUPT_MAP_TST(corrupt, mem_index))) {
                ...
                spci.reg = INVALIDr;
                spci.mem = cur_mem;
                spci.blk_type = SOC_BLOCK_TYPE(unit, copyno);
                spci.index = mem_index;
                          ...
                          uint8 *hard_fault)
{
    ...
    *hard_fault = 0;
    ...
    } else if (at == -1) { /* Use s/w mem cache */
...
#endif
                    {
                        ...
                        LOG_ERROR(BSL_LS_SOC_SER,
                                  (BSL_META_U(unit,
                                              ...
                                              "at: %s.%s[%d] data[%d]: 0x%x 0x%x !!\n"),
                                   SOC_MEM_NAME(unit, mem),
                                   SOC_BLOCK_NAME(unit, copyno),
                                   index, dw, entry[dw], cptr[dw]));
#else
                        LOG_ERROR(BSL_LS_SOC_SER,
                                  (BSL_META_U(unit,
                                              "Hard fault detected (pipe compare) "
    ...
    if ((SOC_IS_TD2P_TT2P(unit) ||
        ...
        case L3_DEFIP_DATA_ONLYm:
...
#endif
                acc_type = _SOC_ACC_TYPE_FOR_PIPE(pipe);
            } else
#endif /* BCM_TOMAHAWK_SUPPORT */
            {
                if (pipe == 0) {
            ...
            if ((SOC_IS_TRIDENT2(unit) || SOC_IS_TITAN2(unit)) &&
                ...
                for (check_pipe = 0; check_pipe < NUM_PIPE(unit); check_pipe++) {
                    restore_pipe = check_pipe;
#ifdef BCM_TOMAHAWK3_SUPPORT
                    if (SOC_IS_TOMAHAWK3(unit)) {
                        if (mem == L3_DEFIP_ALPM_LEVEL3_ECCm) {
                            restore_pipe = (check_pipe == 1 || check_pipe == 6 || check_pipe == 7) ? 0 :
                                           (check_pipe == 2 || check_pipe == 3 || check_pipe == 5) ? 4 :
                                            check_pipe;
                        } else if (mem == L3_DEFIP_ALPM_LEVEL2_ECCm) {
                            restore_pipe = (check_pipe == 1) ? 0 :
                                   (check_pipe == 2) ? 3 :
                                   (check_pipe == 5) ? 4 :
                                   (check_pipe == 6) ? 7 :
                                    check_pipe;
...
#endif
                    if (restore_pipe == pipe) {
                        continue;
                    }

#if defined(BCM_TOMAHAWK_SUPPORT)
                    if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit)) {
                        cache_acc_type = _SOC_ACC_TYPE_FOR_PIPE(restore_pipe);
                    } else
#endif
                    {
                        if (restore_pipe == 0) {
                            cache_acc_type = _SOC_MEM_ADDR_ACC_TYPE_PIPE_X;
                        } else if (restore_pipe == 1) {
            ...
            if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit)) {
                if (has_multi_error) {
                    LOG_WARN(BSL_LS_SOC_SER,
                              (BSL_META_U(unit,
                                          "CLEAR RESTORE for pipe %0d: %s[%d] "
                                          "blk: %s index: %d\n"),
                               pipe, SOC_MEM_NAME(unit, report_mem), report_mem,
                               SOC_BLOCK_NAME(unit, copyno), report_index));
                } else {
                    LOG_WARN(BSL_LS_SOC_SER,
                              (BSL_META_U(unit,
    ...
    if (fix == 0) {
        ...
        if (SOC_IS_TRX(unit)) {
            ...
            if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit) ||
                ...
                case IFP_POLICY_TABLE_WIDEm:
                    ...
                    vindex = index;
                    break;
                case IP_PARSER0_HME_STAGE_TCAM_ONLY_0m:
        ...
        if (EGR_IP_TUNNELm == reported_mem) {
            ...
            if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TOMAHAWK(unit) ||
                SOC_IS_TOMAHAWKPLUS(unit) || SOC_IS_TRIDENT3X(unit)) {
                int neighbour_index;
                ...
                */
                for (i = 0; i < 2; i++) {
                    SOC_MEM_STATE_MEMBER_GET(unit, check_mem_list[i], cache[copyno], cache);
                    SOC_MEM_STATE_MEMBER_GET(unit, check_mem_list[i], vmap[copyno], vmap);
            ...
            if (SOC_IS_TOMAHAWK2(unit) || SOC_IS_TOMAHAWK3(unit)) {
                int neighbour_index;
                soc_mem_t check_mem_list = EGR_IP_TUNNELm;

                neighbour_index = index % 2? index -1 : index + 1;

                /* Check if the neighbour is valid or not for EGR_IP_TUNNEL.
                * If yes, do not clear EGR_IP_TUNNEL_IPV6 view.
                * If no, clear EGR_IP_TUNNEL_IPV6 view.
                */
                SOC_MEM_STATE_MEMBER_GET(unit, check_mem_list, cache[copyno], cache);
                SOC_MEM_STATE_MEMBER_GET(unit, check_mem_list, vmap[copyno], vmap);
...
#define _SOC_IF_ERR_L2X_THAW(rv, mem)                                          \
...
#endif
    SOC_MEM_STATE_MEMBER_GET(unit, mem_for_cache_read, cache[copyno], cache);
    SOC_MEM_STATE_MEMBER_GET(unit, mem_for_cache_read, vmap[copyno], vmap);
        ...
        if (SOC_IS_TOMAHAWKX(unit) || SOC_IS_TRIDENT3X(unit)) {
            ...
            if ((mem == FPEM_ECCm) || (mem == FPEM_LPm)) {
                ...
                if (fpem_mem_mode == _SOC_SER_MEM_MODE_PIPE_UNIQUE) {
                   at = _SOC_ACC_TYPE_FOR_PIPE(pipe);
                }
            }
        }
#endif /* BCM_TOMAHAWK_SUPPORT */
    ...
    for (i = 0; i < sram_info->ram_count; i++) {
        ...
        if ((mem == L3_ENTRY_IPV4_UNICASTm) || use_overlay_mem_correction) {
            ...
            SOC_MEM_STATE_MEMBER_GET(unit, sram_info->view[i], cache[copyno], cache);
            SOC_MEM_STATE_MEMBER_GET(unit, sram_info->view[i], vmap[copyno], vmap);
...
for (i = 0; hash_mem[i] != INVALIDm; i++) {
    ...
    if (copyno != -1) {
        SOC_MEM_STATE_MEMBER_GET(unit, hash_mem[i], cache[copyno], cache);
        SOC_MEM_STATE_MEMBER_GET(unit, hash_mem[i], vmap[copyno], vmap);
    }
...
#if defined(BCM_TRIDENT2_SUPPORT)
int
_soc_td2x_tdm_calendar_ser_correction(int unit, soc_mem_t mem, int index, int copyno, _soc_ser_correct_info_t *si)
{
    int calendar_id, i, index_min, index_max, idx, entry_dw, pipe;
    int calendar_end, calendar_single_end;
    int found = 0;
    uint32 rval = 0;
    soc_mem_t switch_mem = INVALIDm;
    uint32 *cache = NULL;
    uint8 *vmap;
    uint32 entry[SOC_MAX_MEM_WORDS];
    soc_field_t end_field = INVALIDf, single_end_field = INVALIDf;
    void *null_entry = NULL;
    static const soc_mem_t tdm[][2] = {
        {ES_PIPE0_TDM_TABLE_0m, ES_PIPE0_TDM_TABLE_1m},
        {ES_PIPE1_TDM_TABLE_0m, ES_PIPE1_TDM_TABLE_1m},
        {ES_PIPE0_TDM_TABLE_1m, ES_PIPE0_TDM_TABLE_0m},
        {ES_PIPE1_TDM_TABLE_1m, ES_PIPE1_TDM_TABLE_0m}
    };
    static const soc_reg_t tdm_cfg[2] = {
        ES_PIPE0_TDM_CONFIGr, ES_PIPE1_TDM_CONFIGr
    };

    for (i = 0; i < COUNTOF(tdm); i++) {
        if (mem == tdm[i][0]) {
            found = 1;
            break;
        }
    ...
    if (found) {
        entry_dw = soc_mem_entry_words(unit, mem);
        pipe = i % NUM_PIPE(unit);
        SOC_IF_ERROR_RETURN(soc_reg32_get(unit, tdm_cfg[pipe], REG_PORT_ANY, 0, &rval));
        calendar_id = soc_reg_field_get(unit, tdm_cfg[pipe], rval, CURR_CALf);
        calendar_end = calendar_id ? soc_reg_field_get(unit, tdm_cfg[pipe], rval, CAL1_ENDf) :
                                     soc_reg_field_get(unit, tdm_cfg[pipe], rval, CAL0_ENDf);
        calendar_single_end = calendar_id ? soc_reg_field_get(unit, tdm_cfg[pipe], rval, CAL1_END_SINGLEf) :
                                            soc_reg_field_get(unit, tdm_cfg[pipe], rval, CAL0_END_SINGLEf);
        if (((calendar_id == 0) && (i >= 0 && i <=1)) ||
            ((calendar_id == 1) && (i >= 2 && i <=3))) {
            /* reported memory is active, need to switch to backup */
            calendar_id ^= 1;
            switch_mem = tdm[i][1];
            SOC_MEM_STATE_MEMBER_GET(unit, mem, cache[copyno], cache);
            SOC_MEM_STATE_MEMBER_GET(unit, mem, vmap[copyno], vmap);
        } else {
            /* reported memory is backup, need to get active memory's cache */
            switch_mem = INVALIDm;
            SOC_MEM_STATE_MEMBER_GET(unit, tdm[i][1], cache[copyno], cache);
            SOC_MEM_STATE_MEMBER_GET(unit, tdm[i][1], vmap[copyno], vmap);
        }

        if (switch_mem != INVALIDm) {
            index_min = SOC_MEM_INFO(unit, mem).index_min;
            index_max = SOC_MEM_INFO(unit, mem).index_max;
            if (cache != NULL) {
                for (idx = index_min; idx <= index_max; idx++) {
                    if (CACHE_VMAP_TST(vmap, idx)) {
                        sal_memset(entry, 0, SOC_MAX_MEM_WORDS * 4);
                        sal_memcpy(entry, cache + idx * entry_dw, entry_dw * 4);
                        SOC_IF_ERROR_RETURN
                            (soc_mem_write(unit, switch_mem, MEM_BLOCK_ALL, idx, entry));
                    }
                }
            }
            soc_reg_field_set(unit, tdm_cfg[pipe], &rval, CURR_CALf, calendar_id);
            end_field = calendar_id ? CAL1_ENDf : CAL0_ENDf;
            single_end_field = calendar_id ? CAL1_END_SINGLEf : CAL0_END_SINGLEf;
            soc_reg_field_set(unit, tdm_cfg[pipe], &rval, ENABLEf, 1);
            soc_reg_field_set(unit, tdm_cfg[pipe], &rval, end_field, calendar_end);
            soc_reg_field_set(unit, tdm_cfg[pipe], &rval, single_end_field, calendar_single_end);
            SOC_IF_ERROR_RETURN(soc_reg32_set(unit, tdm_cfg[pipe], REG_PORT_ANY, 0, rval));
        }
        ...
        if (cache != NULL && CACHE_VMAP_TST(vmap, index)) {
            ...
            LOG_WARN(BSL_LS_SOC_SER,
                      (BSL_META_U(unit,
                                  "CACHE_RESTORE: %s[%d] blk: %s index: %d\n"),
                       SOC_MEM_NAME(unit, mem), mem,
                       SOC_BLOCK_NAME(unit, copyno),
                       index));
        } else {
            null_entry = soc_mem_entry_null(unit, mem);
            SOC_IF_ERROR_RETURN
                (soc_mem_write(unit, mem, MEM_BLOCK_ALL, index, null_entry));
            si->ctype = SocSerCorrectTypeEntryClear;
            LOG_WARN(BSL_LS_SOC_SER,
                      (BSL_META_U(unit,
                                  "CLEAR_RESTORE: %s[%d] blk: %s index: %d\n"),
                       SOC_MEM_NAME(unit, mem), mem,
                       SOC_BLOCK_NAME(unit, copyno),
                       index));
        }

        /* Switch back to the previous calendar id */
        if (switch_mem != INVALIDm) {
            calendar_id ^= 1;
            soc_reg_field_set(unit, tdm_cfg[pipe], &rval, CURR_CALf, calendar_id);
            soc_reg_field_set(unit, tdm_cfg[pipe], &rval, ENABLEf, 1);
...
#if defined(BCM_TOMAHAWK_SUPPORT)
int
_soc_tdm_calendar_ser_correction(int unit, soc_mem_t mem, int index, int copyno, _soc_ser_correct_info_t *si)
{
    int inst, calendar_id, i, index_min, index_max, idx, entry_dw, pipe, calendar_end;
    int found = 0;
    uint32 rval = 0;
    soc_mem_t switch_mem = INVALIDm;
    uint32 *cache = NULL;
    uint8 *vmap;
    uint32 entry[SOC_MAX_MEM_WORDS];
    soc_field_t end_field = INVALIDf;
    void *null_entry = NULL;
    static const soc_mem_t tdm[][2] = {
        {TDM_CALENDAR0_PIPE0m, TDM_CALENDAR1_PIPE0m},
        {TDM_CALENDAR0_PIPE1m, TDM_CALENDAR1_PIPE1m},
        {TDM_CALENDAR0_PIPE2m, TDM_CALENDAR1_PIPE2m},
        {TDM_CALENDAR0_PIPE3m, TDM_CALENDAR1_PIPE3m},
        {TDM_CALENDAR1_PIPE0m, TDM_CALENDAR0_PIPE0m},
        {TDM_CALENDAR1_PIPE1m, TDM_CALENDAR0_PIPE1m},
        {TDM_CALENDAR1_PIPE2m, TDM_CALENDAR0_PIPE2m},
        {TDM_CALENDAR1_PIPE3m, TDM_CALENDAR0_PIPE3m},
    };
    ...
    if (found) {
        ...
        SOC_IF_ERROR_RETURN(soc_reg32_get(unit, TDM_CONFIGr, inst, 0, &rval));
        calendar_id = soc_reg_field_get(unit, TDM_CONFIGr, rval, CURR_CALf);
        calendar_end = calendar_id ? soc_reg_field_get(unit, TDM_CONFIGr, rval, CAL1_ENDf) :
                                     soc_reg_field_get(unit, TDM_CONFIGr, rval, CAL0_ENDf);
        if (((calendar_id == 0) && (i >= 0 && i <=3)) ||
            ((calendar_id == 1) && (i >= 4 && i <=7))) {
            /* reported memory is active, need to switch to backup */
            calendar_id ^= 1;
            switch_mem = tdm[i][1];
            SOC_MEM_STATE_MEMBER_GET(unit, mem, cache[copyno], cache);
        ...
        if (switch_mem != INVALIDm) {
            ...
            soc_reg_field_set(unit, TDM_CONFIGr, &rval, CURR_CALf, calendar_id);
            end_field = calendar_id ? CAL1_ENDf : CAL0_ENDf;
            soc_reg_field_set(unit, TDM_CONFIGr, &rval, ENABLEf, 1);
            soc_reg_field_set(unit, TDM_CONFIGr, &rval, end_field, calendar_end);
            SOC_IF_ERROR_RETURN(soc_reg32_set(unit, TDM_CONFIGr, inst, 0, rval));
        }
...
#if defined(BCM_TOMAHAWK_SUPPORT)
int
_soc_is_tdm_calendar_ser_correction(int unit, soc_mem_t mem, int index, int copyno, _soc_ser_correct_info_t *si)
{
    int calendar_id, i, index_min, index_max, idx, entry_dw, pipe, calendar_end;
    int found = 0;
    uint32 rval = 0;
    soc_mem_t switch_mem = INVALIDm;
    ...
    static const soc_mem_t tdm[][2] = {
        ...
        {IS_TDM_CALENDAR1_PIPE0m, IS_TDM_CALENDAR0_PIPE0m},
        {IS_TDM_CALENDAR1_PIPE1m, IS_TDM_CALENDAR0_PIPE1m},
        {IS_TDM_CALENDAR1_PIPE2m, IS_TDM_CALENDAR0_PIPE2m},
        {IS_TDM_CALENDAR1_PIPE3m, IS_TDM_CALENDAR0_PIPE3m},
    };
    ...
    if (found) {
        ...
        calendar_id = soc_reg_field_get(unit, reg, rval, CURR_CALf);
        calendar_end = calendar_id ? soc_reg_field_get(unit, reg, rval, CAL1_ENDf) :
                                     soc_reg_field_get(unit, reg, rval, CAL0_ENDf);
        if (((calendar_id == 0) && (i >= 0 && i <=3)) ||
            ((calendar_id == 1) && (i >= 4 && i <=7))) {
            /* reported memory is active, need to switch to backup */
            calendar_id ^= 1;
        ...
        if (switch_mem != INVALIDm) {
            ...
            soc_reg_field_set(unit, reg, &rval, end_field, calendar_end);
                                ...
                                int index, uint32 *entry_x)
{
    ...
    if (bulk_mem[0] == mem) {
        SOC_MEM_STATE_MEMBER_GET(unit, bulk_mem[1], cache[copyno], cache);
        if (cache == NULL) {
            rv = SOC_E_INTERNAL;
            goto exit;
        }
        ...
        SOC_MEM_STATE_MEMBER_GET(unit, bulk_mem[0], cache[copyno], cache);
        if (cache == NULL) {
            rv = SOC_E_INTERNAL;
            goto exit;
        }
                           ...
                           uint32 *entry)
{
...
#if defined(BCM_TOMAHAWK_SUPPORT)
    if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit) || SOC_IS_TRIDENT3X(unit)) {
        if (rv == SOC_E_UNAVAIL) {
            rv = _soc_tdm_calendar_ser_correction(unit, mem, index, copyno, si);
        }
    }
#endif
    ...
    if (SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit) || SOC_IS_TRIDENT3X(unit)) {
        if (rv == SOC_E_UNAVAIL) {
            rv = _soc_is_tdm_calendar_ser_correction(unit, mem, index, copyno, si);
        }
    }
#endif
    ...
    if (SOC_IS_TRIDENT2X(unit)) {
        if (rv == SOC_E_UNAVAIL) {
            rv = _soc_td2x_tdm_calendar_ser_correction(unit, mem, index, copyno, si);
        }
    }
#endif
    if (rv == SOC_E_NONE || rv != SOC_E_UNAVAIL) {
        if (si->log_id != 0) {
            ...
            log_generic->hard_fault = si->hard_fault;
            soc_ser_log_add_tlv(unit, si->log_id, SOC_SER_LOG_TLV_GENERIC,
                sizeof(soc_ser_log_tlv_generic_t), log_generic);
        }
        if (rv == SOC_E_NONE) {
...
#ifdef BCM_TOMAHAWK_SUPPORT
    if (SOC_IS_TOMAHAWK3(unit) && (mem == IFP_TCAMm)) {
        mem_cache = SOC_MEM_UNIQUE_ACC(unit, IFP_TCAMm)[recover_pipe];
        SOC_MEM_STATE_MEMBER_GET(unit, mem_cache, cache[copyno], cache);
        SOC_MEM_STATE_MEMBER_GET(unit, mem_cache, vmap[copyno], vmap);
    } else if ((SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit)) &&
        (mem == IFP_TCAM_WIDEm)) {
        mem_cache = SOC_MEM_UNIQUE_ACC(unit, IFP_TCAM_WIDEm)[recover_pipe];
        SOC_MEM_STATE_MEMBER_GET(unit, mem_cache, cache[copyno], cache);
        SOC_MEM_STATE_MEMBER_GET(unit, mem_cache, vmap[copyno], vmap);
    } else
#endif
    ...
    if (cache != NULL && CACHE_VMAP_TST(vmap, index)) {
        ...
        if ((SOC_IS_TOMAHAWK3(unit) && (mem == IFP_TCAMm)) ||
            ((SOC_IS_TOMAHAWK(unit) || SOC_IS_TOMAHAWK2(unit)) &&
             (mem == IFP_TCAM_WIDEm))) {
            rv = soc_mem_pipe_select_write(unit, SOC_MEM_NO_FLAGS, mem,
                                           copyno, recover_pipe, index, null_entry);
        } else
#endif
        {
            rv = soc_mem_write(unit, mem, copyno, index, null_entry);
                           ...
                           soc_stat_t *stat)
{
    ...
    sal_memset(&log_mem, 0, sizeof(soc_ser_log_tlv_memory_t));
    log_mem.index = index;
...
soc_ser_correction(int unit, _soc_ser_correct_info_t *si)
{
    ...
    /* Init acc_type for single pipe devices */
    if (!soc_feature(unit, soc_feature_two_ingress_pipes)) {
        si->acc_type = _SOC_ACC_TYPE_PIPE_ANY;
    }
    ...
    if (si->flags & SOC_SER_SRC_MEM) {
        /* Handle mems */
        rv = _soc_ser_mem_correction(unit, si, &log_generic, stat);
    } else {
        /* Handle regs */
        rv = _soc_ser_reg_correction(unit, si, &log_generic, stat);
    }
    ...
    if (SOC_FAILURE(rv)) {
        soc_event_generate(unit,
                           SOC_SWITCH_EVENT_PARITY_ERROR,
                           SOC_SWITCH_EVENT_DATA_ERROR_FAILEDTOCORRECT,
                           si->sblk | (si->pipe_num << SOC_SER_ERROR_PIPE_BP) |
                           SOC_SER_ERROR_DATA_BLK_ADDR_SET,
                           si->addr);
    }
    return rv;
...
#ifdef BCM_TOMAHAWK_SUPPORT
    if (SOC_IS_TOMAHAWKX(unit)) {
        rv = soc_th_ser_single_bit_report_for_bus_buf_get(unit, enable);
    }
#endif
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3X(unit)) {
        rv = soc_td3_ser_single_bit_report_for_bus_buf_get(unit, enable);
    }
#endif
#ifdef BCM_TRIDENT2_SUPPORT
    if (SOC_IS_TRIDENT2X(unit) || SOC_IS_TITAN2X(unit)) {
        rv = soc_td2x_ser_single_bit_report_for_bus_buf_get(unit, enable);
    }
#endif
...
#define REG_SER_INFO_COUNT 2
/*
 * Function:
 *      soc_ser_single_bit_error_enable_set
 * Purpose:
 *      Enable/Disable single bit eror reporting for the memories which are
 *      protected by ECC logic and whoes SER_RESPONSE is SER_NONE on TH/TH2/TH3/TD3 chips
 * Parameters:
 ...
 */
int soc_ser_single_bit_error_enable_set(int unit, int enable)
{
    int i, k;
    int at, bt, flen;
    uint32 value = 0;
    ...
    if (NULL == mem_ser_info_all[0] ||
        NULL == mem_ser_info_all[1] ||
        NULL == mem_ser_info_all[2]) {
        return SOC_E_UNAVAIL;
    }
    ...
    for (k = 0; k < MEM_SER_INFO_COUNT; k++) {
        ...
        for (i = 0; mem_ser_info[i].mem != INVALIDm; i++) {
...
#ifdef BCM_HURRICANE4_SUPPORT
            if (_soc_hr4_reg_skip(unit, ecc_reg)) {
                continue;
            }
#endif
            ...
            if (enable) {
                flen = soc_reg_field_length(unit, ecc_reg, ecc_fld);
                value = (1 << flen) - 1;
            }

            at = SOC_REG_ACC_TYPE(unit, ecc_reg);
            bt = SOC_REG_BASE_TYPE(unit, ecc_reg);
            if (at == 20) { /* For single acctype */
                int inst = 0, pipe = 0, max_pipe = 1;
                switch (bt) {
                    case 2: /* IPIPE */
                    case 3: /* EPIPE */
                ...
                for (pipe = 0; pipe < max_pipe; pipe++) {
                    inst = pipe | SOC_REG_ADDR_INSTANCE_MASK;
                    SOC_IF_ERROR_RETURN
                        (soc_reg_field32_modify(unit, ecc_reg, inst,
                                                ecc_fld, value));
                }
            } else {
                SOC_IF_ERROR_RETURN
 ...
 */
int soc_ser_single_bit_error_enable_get(int unit, int *enable)
{
    soc_reg_t ecc_reg;
    soc_field_t ecc_fld;
    _soc_mem_ser_en_info_t *mem_ser_info = NULL;
    uint32 fld_value = 0, i;
    uint64 rval64;
    ...
    for (i = 0; mem_ser_info[i].mem != INVALIDm; i++) {
        ...
        if (soc_reg_field_valid(unit, ecc_reg, ecc_fld)) {
            COMPILER_64_ZERO(rval64);
            SOC_IF_ERROR_RETURN
                (soc_reg_get(unit, ecc_reg, REG_PORT_ANY, 0, &rval64));
            fld_value = soc_reg64_field32_get(unit, ecc_reg, rval64, ecc_fld);
            break;
        }
    }
...
int soc_ser_single_bit_sw_correction_skip(int unit, soc_mem_t mem)
{
#ifdef BCM_TRIDENT3_SUPPORT
    if (SOC_IS_TRIDENT3X(unit)) {
        return soc_td3_ser_single_bit_sw_correction_skip(unit, mem);
    }
#endif
...
/*
 * Function:
 *      soc_ser_single_bit_sw_correction_enable_set
 * Purpose:
 *      Enable/Disable single bit eror reporting for the memories which
 *      SER_RESPONSE is not SER_NONE on TH/TH2/TH3/TD3 chips
 * Parameters:
 *      unit        - (IN) Device Number
 *      enable         - (IN) Enable/Disable
 * Returns:
 *      SOC_E_NONE if set successfully, an error otherwise
 */
int soc_ser_single_bit_sw_correction_enable_set(int unit, int enable)
{
    int i, k;
    int at, bt, flen;
    uint32 value = 0;
    ...
    _soc_mem_ser_en_info_t *mem_ser_info;
    _soc_mem_ser_en_info_t *mem_ser_info_all[MEM_SER_INFO_COUNT] = {NULL};
    _soc_reg_ser_en_info_t *reg_ser_info;
    _soc_reg_ser_en_info_t *reg_ser_info_all[REG_SER_INFO_COUNT] = {NULL};
#ifdef BCM_TOMAHAWK3_SUPPORT
    uint16 dev_id;
    uint8 rev_id;
    ...
    mem_ser_info_all[1] = SOC_EP_MEM_SER_INFO(unit);
    mem_ser_info_all[2] = SOC_MMU_MEM_SER_INFO(unit);
    reg_ser_info_all[0] = SOC_IP_REG_SER_INFO(unit);
    reg_ser_info_all[1] = SOC_EP_REG_SER_INFO(unit);
...
#ifdef BCM_HURRICANE4_SUPPORT
            if (SOC_IS_HURRICANE4(unit) &&
                _soc_hr4_mem_skip(unit, mem_ser_info[i].mem)) {
                continue;
            }
#endif /* BCM_HURRICANE4_SUPPORT */
#ifdef BCM_FIREBOLT6_SUPPORT
            if (SOC_IS_FIREBOLT6(unit) &&
                _soc_fb6_mem_skip(unit, mem_ser_info[i].mem)) {
                continue;
            }
#endif /* BCM_FIREBOLT6_SUPPORT */
            if (soc_ser_single_bit_sw_correction_skip(unit, mem_ser_info[i].mem)) {
                continue;
            }

            /* Skip the memory which SER_RESPONSE is SER_NONE*/
            if (SOC_MEM_SER_CORRECTION_TYPE(unit, mem_ser_info[i].mem) == 0) {
                continue;
            }
            ...
            if (at == 20) { /* For single acctype */
                ...
                for (pipe = 0; pipe < max_pipe; pipe++) {
                    ...
                    if (dev_id == BCM56982_DEVICE_ID) {
                        int rv = 0;
                        if (SOC_REG_IS_64(unit, ecc_reg)) {
                            uint64 rval;
                            COMPILER_64_ZERO(rval);
                            soc_reg64_field32_set(unit, ecc_reg, &rval,
                                    ecc_fld, value);
                            rv = (soc_reg64_set(unit, ecc_reg,
                                                REG_PORT_ANY, 0, rval));
                            if (rv < 0) {
                                return rv;
                            }
                            ...
                            uint32 rval = 0;
                            soc_reg_field_set(unit, ecc_reg, &rval,
                                ecc_fld, value);
                            rv = (soc_reg32_set(unit, ecc_reg,
                                    REG_PORT_ANY, 0, rval));
                            if (rv < 0) {
    ...
    if (NULL == reg_ser_info_all[0] ||
        NULL == reg_ser_info_all[1]) {
        return SOC_E_UNAVAIL;
    }

    for (k = 0; k < REG_SER_INFO_COUNT; k++) {
        reg_ser_info = reg_ser_info_all[k];
        for (i = 0; reg_ser_info[i].reg != INVALIDr; i++) {
            if (!SOC_REG_IS_VALID(unit, reg_ser_info[i].reg)) {
                continue;
            }

            ecc_reg = reg_ser_info[i].ecc1b_ctrl.ctrl_type.en_reg;
            ecc_fld = reg_ser_info[i].ecc1b_ctrl.en_fld;
            ...
            LOG_VERBOSE(BSL_LS_SOC_SER,
                        (BSL_META_U(unit,
                                    "unit %d, MEM %50s REG %30s FIELD %40s\n"),
                         unit, SOC_REG_NAME(unit, reg_ser_info[i].reg),
                         SOC_REG_NAME(unit, ecc_reg),
                         SOC_FIELD_NAME(unit, ecc_fld)));
...
/*
 * Function:
 *      soc_ser_single_bit_sw_correction_enable_get
 * Purpose:
 *      Get the status of single bit eror reporting for the memories which
 *      SER_RESPONSE is not SER_NONE on TH/TH2/TH3/TD3 chips
 * Parameters:
 *      unit        - (IN) Device Number
 *      enable         - (OUT) Enable/Disable
 ...
 *      SOC_E_NONE if get successfully, an error otherwise
 */
int soc_ser_single_bit_sw_correction_enable_get(int unit, int *enable)
{
    soc_reg_t ecc_reg;
    soc_field_t ecc_fld;
    _soc_mem_ser_en_info_t *mem_ser_info = NULL;
...
#ifdef BCM_TOMAHAWK3_SUPPORT
            if (dev_id == BCM56982_DEVICE_ID) {
                /* Iterate through all for 56982, since some PM are inactive */
                if (fld_value) {
                    break;
                }
            } else
...
int
soc_add_log_generic(int unit, _soc_ser_correct_info_t *spci)
{
