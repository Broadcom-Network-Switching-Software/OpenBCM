...
#define LOG_SBUSDMA(err, ls_, stuff_)         \
    if (err) {                                \
...
********************************************/
void
cmicx_sbusdma_curr_op_details(int unit, int cmc, int ch, int err)
{
    uint32 rval;
    ...
    LOG_SBUSDMA(err, BSL_LS_SOC_COMMON,
             (BSL_META_U(unit,
...
int cmicx_sbusdma_ch_get(int unit, int *cmc, int *ch)
{
    ...
    if (SOC_SUCCESS(rv)) {
        ...
        for (i = 0; i < sbusdma_chan_max; i++) {
            cmicx_sbusdma_curr_op_details(unit, *cmc, i, 1);
        }
    }
    return rv;
}
...
#if defined(SRC_SOC_COMMON_ACTIVATE_UNUSED) || defined(BCM_CMICX_SUPPORT) && defined(BCM_SBUSDMA_SUPPORT)
/* { */
/*******************************************
...

{
    ...
    if (SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) {
        if (_cmicx_sbusdma_ch.lock[unit] != NULL) {
            sal_spinlock_destroy(_cmicx_sbusdma_ch.lock[unit]);
            _cmicx_sbusdma_ch.lock[unit] = NULL;
        }

        /* Cleanup channel semaphore */
        if (_cmicx_sbusdma_ch.sem[unit] != NULL) {
            sal_sem_destroy(_cmicx_sbusdma_ch.sem[unit]);
            _cmicx_sbusdma_ch.sem[unit] = NULL;
        }
    } else
#endif
    {
...

{
    ...
    if (SOC_IS_DNX(unit) || SOC_IS_DNXF(unit)) {
        /* channel lock */
        _cmicx_sbusdma_ch.lock[unit] = sal_spinlock_create("sbusdma Lock");
        if (_cmicx_sbusdma_ch.lock[unit] == NULL) {
            return SOC_E_MEMORY;
        }
        /* Initialize channel semaphore */
        _cmicx_sbusdma_ch.sem[unit] = sal_sem_create("SBUSDMA Channel",
            sal_sem_COUNTING,
...
/*******************************************
* @function cmicx_sbusdma_event_dpc
...
********************************************/
void
cmicx_sbusdma_event_dpc(void *p_unit, void *event_id, void *cmc, void *chan,
                        void *p5)
{
    ...
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META_U(unit,
                          "Generate I/O error event for ID%d, cmc = %d, chan =%d\n"),
                          PTR_TO_INT(event_id), PTR_TO_INT(cmc), PTR_TO_INT(chan)));
