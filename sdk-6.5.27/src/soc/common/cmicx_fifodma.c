...
cmicx_dnxf_fifodma_done(int unit, void *data)
{
    ...
    if (dnxf_data_fabric.fifo_dma.enable_get(unit) &&
        ((soc_dnxf_control_t *)SOC_CONTROL(unit)->drv)->rx_reception_tmngr_hndlr != NULL) {
        SOC_CONTROL(unit)->stat.intr_fifo_dma[in_d->ch]++;
        (void)shr_thread_manager_trigger(((soc_dnxf_control_t *)SOC_CONTROL(unit)->drv)->rx_reception_tmngr_hndlr);
    }
}
#endif
