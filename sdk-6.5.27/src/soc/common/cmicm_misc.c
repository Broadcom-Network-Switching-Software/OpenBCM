 ...
 *      cmicm_dma_active
 * Purpose:
        To log the actives DMA's in given cmc
 * Parameters:
 *      unit - SOC unit #
 * Returns:
 *      SOC_E_NONE - success
...
int
soc_cmicm_dma_active(int unit, int cmc)
{
    int chan;
    uint32 val, cmic_address;
    chans_group_t packet_channels, sbus_channels, fifo_channels, ccm_channels, schan_modules;

    LOG_ERROR(BSL_LS_SOC_DMA, (BSL_META_U(unit, "Log Active DMA\n")));

    /* Active packet DMA in all channels */
    chans_group_init(&packet_channels);
    for (chan = 0; chan < N_DMA_CHAN; ++chan) {
        cmic_address = CMIC_CMCx_CHy_DMA_CTRL_OFFSET(cmc, chan);
        /* if PKTDMA_ENABLE == 1 */
        val = soc_pci_read(unit, cmic_address);
        if (val & PKTDMA_ENABLE) {
            LOG_ERROR(BSL_LS_SOC_DMA, (BSL_META_U(unit, "Active packet DMA CMC %d channel %d\n"), cmc, chan));
        }
    }

    /* Active s-bus DMA in all channels */
    chans_group_init(&sbus_channels);
    for (chan = 0; chan < CMIC_CMCx_SBUSDMA_CHAN_MAX; ++chan) {
        cmic_address = CMIC_CMCx_SBUSDMA_CHy_CONTROL(cmc, chan);
        /* if SBUSDMA_START == 1 */
        val = soc_pci_read(unit, cmic_address);
        if (val & SBUSDMA_START) {
            LOG_ERROR(BSL_LS_SOC_DMA, (BSL_META_U(unit, "Active s-bus DMA CMC %d channel %d\n"), cmc, chan));
        }
    }

    /* Active FIFO DMA in all channels, FIFO DMA is not per CMC */
    chans_group_init(&fifo_channels);
    for (chan = 0; chan < N_DMA_CHAN; ++chan) {
        cmic_address = CMIC_CMCx_FIFO_CHy_RD_DMA_CFG_OFFSET(cmc, chan);
        /* if FIFODMA_ENABLE == 1 */
        val = soc_pci_read(unit, cmic_address);
        if (val & FIFODMA_ENABLE) {
            LOG_ERROR(BSL_LS_SOC_DMA, (BSL_META_U(unit, "Aborting FIFO DMA CMC %d channel %d\n"), cmc, chan));
        }
    }


    /* Active CCM DMA in all channels */
    chans_group_init(&ccm_channels);
    cmic_address = CMIC_CMCx_CCM_DMA_CFG_OFFSET(cmc);
    /* if CCMDMA_ENABLE == 1 */
    val = soc_pci_read(unit, cmic_address);
    if (val & CCMDMA_ENABLE) {
        LOG_ERROR(BSL_LS_SOC_DMA, (BSL_META_U(unit, "Active CCM DMA CMC %d\n"), cmc));
    }

    /* Active SCHAN for all modules */
    chans_group_init(&schan_modules);
    cmic_address = CMIC_COMMON_CMCx_SCHAN_CTRL_OFFSET(cmc);
    /* if CMIC_COMMON_POOL_SCHAN_FIFO_0_CHy_CTRL.MSG_START == 1 */
    val = soc_pci_read(unit, cmic_address);
    if (val & SC_CMCx_MSG_START) {
        LOG_ERROR(BSL_LS_SOC_DMA, (BSL_META_U(unit, "Active s-chan modules %d\n"), cmc));
    }
