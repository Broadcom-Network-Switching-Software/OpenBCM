...
soc_features_bcm56870_a0(int unit, soc_feature_t feature)
{
    ...
    case soc_feature_l3mc_dnapt_snat:
    case soc_feature_retain_ing_pkt_otpid:
...
/*
 * Function:    soc_access_feature_init
 * Purpose:     initialize features into the new access cache
 * Parameters:  unit    - the device, device_type
 * Returns:     void
 */
void
soc_access_feature_init(int unit)
{
    soc_feature_t       f;

    assert(COUNTOF(soc_feature_name) == (soc_feature_count + 1));
    sal_memset(SOC_CONTROL(0)->features, 0,
            sizeof(SOC_CONTROL(0)->features));
    for (f = 0; f < soc_feature_count; f++) {
        SOC_FEATURE_DEBUG_PRINT(("%s : ", soc_feature_name[f]));
    }
}
