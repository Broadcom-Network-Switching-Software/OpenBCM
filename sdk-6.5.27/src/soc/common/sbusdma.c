    ...
    if ((handle > 0) && (handle <= SOC_SBUSDMA_MAX_DESC)
        ...
        if (!(swd->ctrl.flags & SOC_SBUSDMA_CFG_USE_SUPPLIED_DESC)) {
            ...
            if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war) &&
                ...
                soc_iproc_m0ssq_sram_free(unit, PTR_TO_INT(swd->desc));
            } else
#endif
            {
                soc_cm_sfree(unit, swd->desc);
...
/*******************************************
* @function soc_sbusdma_deinit
* purpose API to De-Initialize SBUS DMA
*
* @param unit [in] unit
*
* @returns SOC_E_NONE
...
* @end
********************************************/
int soc_sbusdma_deinit(int unit)
{
    int rv;

    rv = soc_sbusdma_desc_detach(unit);
    if (rv != SOC_E_NONE) {
        return rv;
    }
...
#ifdef BCM_CMICX_SUPPORT
    if (soc_feature(unit, soc_feature_cmicx)) {
        /* de-Initialize sbusdma channel */
        SOC_IF_ERROR_RETURN(cmicx_sbusdma_ch_deinit(unit));
    }
#endif
