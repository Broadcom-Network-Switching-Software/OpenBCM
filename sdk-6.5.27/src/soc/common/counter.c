...
#if defined(BCM_DNX_SUPPORT)
static int _soc_counter_dnx_sbusdma_setup(int unit);
#endif
...
typedef struct soc_counter_collection_preempt_s {
    int enable;                             /* preempt enable/disable */
    int call_cnt;                           /* count of active preempt calls */
    int pause;                              /* pause preempt due to max period overdue */
    sal_usecs_t max_period_us;              /* max preempt period in us. 0 means no max period */
    sal_usecs_t ts_col_start;               /* timestamp for the the latest counter collection start */
    sal_usecs_t ts_col_complete;            /* timestamp for the latest counter collection completion */
    volatile sal_spinlock_t counter_lock;   /* spinlock for this data structure */
} soc_counter_collection_preempt_t;

enum {
    /* Periodic counter collection start */
    SOC_COUNTER_TS_EVENT_COL_START,
    /* Periodic counter collection complete */
    SOC_COUNTER_TS_EVENT_COL_COMPLETE,
    /* Number of counter timestamp events. Stay as the last enum */
    SOC_COUNTER_TS_EVENT_NUM,
};

static soc_counter_collection_preempt_t
soc_counter_collection_preempt[SOC_MAX_NUM_DEVICES] = {{0}};

int
soc_counter_collection_preempt_active(int unit, int active) {

    if (soc_feature(unit, soc_feature_ctr_collection_preempt)) {
        soc_counter_collection_preempt_t *preempt =
            &soc_counter_collection_preempt[unit];
        if (preempt->enable) {
            if (active == 1) {
                preempt->call_cnt++;
            } else {
                /* The following operations need to be atomic so this function
                   can be reentrant */
                COUNTER_ATOMIC_BEGIN(preempt);
                preempt->call_cnt =
                    (preempt->call_cnt <= 0)? 0: preempt->call_cnt - 1;
                COUNTER_ATOMIC_END(preempt);
            }
        }
    }
    return SOC_E_NONE;
...
STATIC void
soc_counter_collection_preempt_init(int unit) {

    if (soc_feature(unit, soc_feature_ctr_collection_preempt)) {
        soc_control_t *soc = SOC_CONTROL(unit);
        soc_counter_collection_preempt_t *preempt =
            &soc_counter_collection_preempt[unit];
        preempt->enable = soc_property_get(unit, "counter_collection_preempt_enable", 0);
        preempt->pause = 0;
        preempt->call_cnt = 0;
        preempt->ts_col_start = preempt->ts_col_complete = sal_time_usecs();
        preempt->max_period_us =
            soc_property_get(unit, "counter_collection_preempt_max_period_usec", 5000000);

        /* Unless user setting it to 0 (infinite),
           max_period_us should be greater or equal to counter_interval */
        if (preempt->max_period_us != 0 && preempt->max_period_us < soc->counter_interval) {
            preempt->max_period_us = soc->counter_interval;
        }

        /* Create spinlock for preempt structure */
        if (preempt->counter_lock != NULL) {
            sal_spinlock_destroy(preempt->counter_lock);
            preempt->counter_lock = NULL;
        }
        preempt->counter_lock = sal_spinlock_create("counter collection preempt spinlock");
        if (preempt->counter_lock == NULL) {
            LOG_ERROR(BSL_LS_SOC_COUNTER,
                (BSL_META_U(unit,
                 "counter collection preempt lock create failed\n")));
        }

        LOG_INFO(BSL_LS_SOC_COUNTER,
            (BSL_META_U(unit,
             "Counter collection preempt init. (Enable: %d, Max period: %d us)\n"),
             preempt->enable, preempt->max_period_us));
    }
}

STATIC int
soc_counter_collection_preempt_check(int unit) {

    if (soc_feature(unit, soc_feature_ctr_collection_preempt)) {
        soc_counter_collection_preempt_t *preempt =
            &soc_counter_collection_preempt[unit];

        /* Counter collection is preempted only when all the following are true:
            1. Counter collection preempt is enabled
            2. Counter collection preempt time doesn't exceed the max period
            3. Active preempt calls are detected
        */
        if (preempt->enable == 1 &&
            preempt->pause == 0 &&
            preempt->call_cnt > 0) {
            ...
            LOG_INFO(BSL_LS_SOC_COUNTER,
                (BSL_META_U(unit,
                 "Preempt call detected (call cnt = %d)!\n"),
                 preempt->call_cnt));
...
STATIC int
soc_counter_collection_preempt_event(int unit, int event) {

    if (soc_feature(unit, soc_feature_ctr_collection_preempt)) {
        soc_counter_collection_preempt_t *preempt =
            &soc_counter_collection_preempt[unit];
        if (event == SOC_COUNTER_TS_EVENT_COL_START) {
            if (preempt->max_period_us != 0 && preempt->pause == 0) {
                int elapsed;
                preempt->ts_col_start = sal_time_usecs();
                elapsed = SAL_USECS_SUB (preempt->ts_col_start, preempt->ts_col_complete);

                if (elapsed >= preempt->max_period_us) {
                    preempt->pause = 1;

                    LOG_INFO(BSL_LS_SOC_COUNTER,
                        (BSL_META_U(unit,
                         "Pause counter collection preemption. Continuous preempt: %d us\n"),
                         SAL_USECS_SUB (preempt->ts_col_start, preempt->ts_col_complete)));
                }
            }
        } else if (event == SOC_COUNTER_TS_EVENT_COL_COMPLETE) {
            if (preempt->pause == 1) {
                LOG_INFO(BSL_LS_SOC_COUNTER,
                    (BSL_META_U(unit,
                     "Resume counter collection preemption while full counter collection completed. (previously complete is %d us ago)\n"),
                     SAL_USECS_SUB (sal_time_usecs(), preempt->ts_col_complete)));
            }
            preempt->ts_col_complete = sal_time_usecs();
            preempt->pause = 0;
        } else {
            return SOC_E_PARAM;
        }
...
#if defined(BCM_XGS5_SWITCH_PORT_SUPPORT) || defined(SRC_SOC_COMMON_ACTIVATE_UNUSED) || defined(BCM_ESW_SUPPORT)
/*******************************************
...
********************************************/
static void
_soc_counter_event_dpc(void *p_unit, void *event_id, void *reason, void *action,
                        void *p5)
{
    ...
    /* Restart counter */
    soc_event_generate(unit, PTR_TO_INT(event_id), PTR_TO_INT(reason),
                       PTR_TO_INT(action), 0);
}
#endif
                             ...
                             void *data)
{
    ...
    if (status == SOC_E_NONE) {
        ...
        COUNTER_ATOMIC_BEGIN(soc);
        soc->counter_blk_dma_err_count = 0;
        COUNTER_ATOMIC_END(soc);
        LOG_DEBUG(BSL_LS_SOC_COUNTER,
                    (BSL_META_U(unit,
        ...
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                   ...
                   ctr_process->blk, ctr_process->bindex, ctr_process->entries));
        if (status == SOC_E_FAIL) {
            COUNTER_ATOMIC_BEGIN(soc);
            soc->counter_blk_dma_err_count++;
            COUNTER_ATOMIC_END(soc);
        }
        /* Check for SW timeout or consecutive DMA error */
        if ((status == SOC_E_TIMEOUT) ||
            (soc->counter_blk_dma_err_count >= soc->counter_max_dma_err_count)) {
             /* Reset the count */
             /* coverity[double_lock:FALSE] */
             COUNTER_ATOMIC_BEGIN(soc);
             soc->counter_blk_dma_err_count = 0;
             ...
             LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Fatal error in blk counter DESC SBUSDMA \n")));
            ...
            /*
             * Generate switch event to restart counter if consecutive data error.
             * SOC_E_TIMEOUT condition is handled by SBUSDMA driver
             */
            if (status == SOC_E_FAIL) {
                sal_dpc(_soc_counter_event_dpc, INT_TO_PTR(unit),
                            INT_TO_PTR(SOC_SWITCH_EVENT_IO_ERROR),
                            INT_TO_PTR(SOC_SWITCH_EVENT_IO_ERROR_SBUSDMA_NO_DATA),
                            INT_TO_PTR(SOC_SWITCH_COUNTER_RESTART), NULL);
            }
        }
    }
}
...
soc_counter_attach(int unit)
{
    ...
    if (soc_feature(unit, soc_feature_access)) {
        /*Ignore this, will be assign value in the next*/
        port = 0;
    } else if (soc_feature(unit, soc_feature_subtag_counters)) {
            port = SOC_MAX_NUM_PP_PORTS;
#if defined(BCM_METROLITE_SUPPORT)
            if (SOC_IS_METROLITE(unit)) {
                port = ML_MAX_PP_PORTS;
            }
#endif
    } else {
...
#if defined(BCM_DNX_SUPPORT)
            if (SOC_IS_DNX(unit)) {
                n_bytes = portsize * dnx_data_nif.phys.nof_phys_get(unit);
                assert(n_bytes > 0);
            }
#endif
...
int
soc_controlled_counter_get_delta_by_counter_idx(int unit, soc_port_t port, int counter_id,uint64 *delta_cnt)
{
    int port_base = 0;
    soc_control_t *soc = SOC_CONTROL(unit);
    ...
    LOG_DEBUG(BSL_LS_SOC_COUNTER,
                (BSL_META_U(unit,
                            "soc_controlled_counter_get_delta: unit=%d "
                            "port=%d, counter id=%d\n"), unit, port, counter_id));
    ...
    ctr = &soc->controlled_counters[counter_id];
    ...
    /* The following section updates 64-bit values and must be atomic */
    {
        COUNTER_ATOMIC_DEF s = SOC_CONTROL(unit);
        COUNTER_ATOMIC_BEGIN(s);
        COMPILER_64_COPY(*delta_cnt, soc->counter_delta[port_base + ctr->counter_idx]);
        COUNTER_ATOMIC_END(s);
                         ...
                         int ar_idx, uint64 val)
{
    ...
    soc_port_t          base_port;
#if defined (BCM_METROLITE_SUPPORT)
    soc_port_t          port_iter;
    soc_info_t          *si = &SOC_INFO(unit);
#endif
    ...
    PBMP_ITER(pbmp, port) {
        base_port = port;
#if defined (BCM_METROLITE_SUPPORT)
        if (soc_feature(unit, soc_feature_subtag_counters)) {
            /* Use counter map of base port for subtag ports */
            if (SOC_PBMP_MEMBER(SOC_INFO(unit).subtag_pp_port_pbm, port)) {
                /* Get base port for PP port */
                SOC_PBMP_ITER(si->subtag_allowed_pbm, port_iter)  {
                    if (SOC_PBMP_MEMBER(SOC_PORT_PP_BITMAP(unit, port_iter), port)) {
                        base_port = port_iter;
                        break;
                    }
                }
            }
            if (base_port >= sizeof(SOC_CONTROL(unit)->counter_map)/
                            sizeof(SOC_CONTROL(unit)->counter_map[0])) {
                continue;
            }
        }
#endif
         ...
         * longer exist.
         */
        if (!SOC_CONTROL(unit)->counter_map[base_port]) {
            continue;
        }
    ...
    continue;
        ...
        if (ctr_reg != INVALIDr) {
            ...
            if (IS_LB_PORT(unit, port) ||
                ...
                min_index = 0;
                max_ctr =  PORT_CTR_NUM(unit, base_port);
            }
        }
        ...
        )
        {
            ...
            if (dma_counters ) {
                ...
                for (i = min_index; i < max_ctr; i++) {
                    ctr_ref = PORT_CTR_REG(unit, base_port, i);
...
int soc_controlled_counter_length_get(int unit, soc_controlled_counter_t* ctr, int port, int *length)
{
    if (ctr && ctr->controlled_counter_length_f != NULL) {
        ctr->controlled_counter_length_f(unit, ctr->counter_id, port, length);
    } else {
        ...
        for (index = 0; soc->controlled_counters[index].controlled_counter_f != NULL; index++) {
            ...
            if(clear_on_read) {
                ...
                if (COMPILER_64_LT(ctr_diff, ctr_prev)) {
                     ...
                     */
                    SOC_IF_ERROR_RETURN(soc_controlled_counter_length_get(unit, ctr, port, &width));
...
soc_counter_collect64(int unit, int discard, soc_port_t tmp_port, soc_reg_t hw_ctr_reg)
{
    ...
    PBMP_ITER(counter_pbmp, port) {
        ...
        if (hw_ctr_reg == INVALIDr) {
            index = 0;
            max_index = PORT_CTR_NUM(unit, base_port);
        } else {
        ...
        for ( ; index < max_index; index++) {
            ...
            ctr_reg = PORT_CTR_REG(unit, base_port, index)->reg;
            ar_idx = PORT_CTR_REG(unit, base_port, index)->index;
               ...
               )
            {
                ...
                if (
#ifdef BCM_ESW_SUPPORT
                    soc_reg_field_valid(unit, ctr_reg, EVEN_PARITYf) ||
#endif
                    soc_reg_field_valid(unit, ctr_reg, PARITYf)) {
                                    ...
                                    soc_reg_t ctr_reg)
{
    ...
    for (; id < non_dma_counter_end; id++) {
        ...
        if ((ctr_reg == INVALIDr) &&
            soc_counter_collection_preempt_check(unit) == SOC_E_BUSY) {
            return;
        }
            ...
            do {
                ...
                if (ctr_reg == INVALIDr) {
                    for (i = 0; i < max_pipe; i++) {
                        ...
                        if (soc_counter_collection_preempt_check(unit) == SOC_E_BUSY) {
                            return;
                        }
...
#ifdef BCM_SBUSDMA_SUPPORT
#if defined(BCM_XGS5_SWITCH_PORT_SUPPORT) || defined(BCM_DNX_SUPPORT) || defined(BCM_ESW_SUPPORT)
/* { */
STATIC void _soc_sbusdma_port_ctr_cb(int unit, int status, sbusdma_desc_handle_t handle,
                              void *data)
{
    int  phy_port;
    COUNTER_ATOMIC_DEF  s = SOC_CONTROL(unit);
    ...
    if (status == SOC_E_NONE) {
        ...
        /* coverity[double_lock:FALSE] */
        COUNTER_ATOMIC_BEGIN(s);
        s->counter_port_dma_err_count = 0;
        COUNTER_ATOMIC_END(s);
    } else {
        uint8 i;
        int blk;
        ...
        if (status == SOC_E_FAIL) {
            /* coverity[double_lock:FALSE] */
            COUNTER_ATOMIC_BEGIN(s);
            s->counter_port_dma_err_count++;
            COUNTER_ATOMIC_END(s);
        }
        /* Check for SW timeout or consecutive DMA error */
        if ((status == SOC_E_TIMEOUT) ||
            (s->counter_port_dma_err_count >= s->counter_max_dma_err_count)) {
             /* Reset the count */
             COUNTER_ATOMIC_BEGIN(s);
             s->counter_port_dma_err_count = 0;
             COUNTER_ATOMIC_END(s);
             LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "Fatal error in port counter DESC SBUSDMA \n")));
            ...
            for (i=0; i<=(COUNTER_SBUSDMA_DESC_SIZE-1); i++) {
                ...
                if (!(soc_feature(unit, soc_feature_subtag_counters) &&
                    (phy_port > SOC_INFO(unit).lb_port))) {
                    ...
                    /* Only IPIPE and EPIPE counters valid for subports */
                    if (i == 2) {
                        break;
                    }
                }
...
#endif
        if (soc_feature(unit, soc_feature_subtag_counters)) {
            SOC_PBMP_OR(all_pbmp, SOC_INFO(unit).subtag_pp_port_pbm);
        }
...
#define _MIB_TABLE_COUNTERS "MIB TBL CTRS"
        ...
        /* MAC counters not applicable for subports */
        if (soc_feature(unit, soc_feature_subtag_counters)) {
            if (phy_port > SOC_INFO(unit).lb_port) {
                continue;
            }
        }
        /* MAC */
...
/*
 * Function:
 *      soc_counter_port_dnx_sbusdma_desc_alloc
 * Purpose:
...
soc_counter_port_dma_stat_get(int unit, soc_port_t port, int dma_offset, uint64 *val)
{
    ...
    port_base_dma = phy_port * soc->counter_perport;

    if (dma_offset >= soc->counter_perport || dma_offset < 0 )
    {
        LOG_ERROR(BSL_LS_SOC_COUNTER,
                 (BSL_META_U(unit,
                             "port %d dma offset %d exceeds max counter %d per port\n"),
                  port, dma_offset, soc->counter_perport));
    }
    ptr = (uint32 *)&soc->counter_buf64[port_base_dma + dma_offset];

    COMPILER_64_SET(*val, ptr[1], ptr[0]);
...
soc_counter_port_dma_stat_set(int unit, soc_port_t port, int dma_offset, uint64 val)
{
    ...
    COMPILER_64_TO_32_LO(ptr[0], val);
    COMPILER_64_TO_32_HI(ptr[1], val);

    return SOC_E_NONE;
}

static int
_soc_counter_dnx_sbusdma_setup(int unit)
{
    soc_port_t port;
    soc_pbmp_t nif_eth_ports_bitmap;

    sal_memset(_soc_port_counter_handles[unit], 0, sizeof(_soc_port_counter_handles[unit]));

    SOC_IF_ERROR_RETURN(dnx_algo_port_logicals_get
                    (unit, BCM_CORE_ALL, DNX_ALGO_PORT_LOGICALS_TYPE_NIF_ETH, DNX_ALGO_PORT_LOGICALS_F_MASTER_ONLY,
                     &nif_eth_ports_bitmap));
    /*
     * sbusdma desc alloc for all eth port even if it is not added in counter bitmap
     */
    if (dnx_data_mib.general.feature_get(unit, dnx_data_mib_general_mib_counters_statdma_support))
    {
        SOC_PBMP_ITER(nif_eth_ports_bitmap, port)
        {
            SOC_IF_ERROR_RETURN(soc_counter_port_sbusdma_desc_alloc(unit, port));
        }
    }
...
static int
_soc_counter_dnx_sbudma_desc_free_all(int unit)
{
    soc_port_t port;
    soc_pbmp_t nif_eth_ports_bitmap;
    ...
    if (dnx_data_mib.general.feature_get(unit, dnx_data_mib_general_mib_counters_statdma_support))
    {
        SOC_PBMP_ITER(nif_eth_ports_bitmap, port)
        {
            SOC_IF_ERROR_RETURN(soc_counter_port_sbusdma_desc_free(unit, port));
        }
    }
...
int
soc_counter_port_dnx_sbusdma_desc_alloc(int unit, soc_port_t port)
{
    soc_control_t       *soc;
    int                 csize;
    soc_ctr_type_t      ctype;
    soc_reg_t           reg;
    soc_reg_t           mac_clmib_reg;
    int                 mac_clmib_cnt, mac_clmib_first, mac_clmib_last;
    uint32 mib_tbl_addr = 0, mib_tbl_cnt, nof_rows_mib_tbl_per_port;
    uint8  mib_tbl_acc_type = 0;

    uint32  mac_addr = 0;
    int     mac_blkoff = 0;
    uint8   mac_acc_type = 0;
    int                 blk, phy_port, i;
    int                 port_num_blktype;
    soc_sbusdma_desc_ctrl_t   ctrl = {0};
    soc_sbusdma_desc_cfg_t    cfg;
    ...
    mac_clmib_cnt = mac_clmib_first != -1 ? mac_clmib_last - mac_clmib_first + 1 : 0;
    mib_tbl_cnt = dnx_data_mib.general.nof_express_rows_per_cdmib_port_get(unit) +
                  dnx_data_mib.general.nof_preempt_rows_per_cdmib_port_get(unit);
    nof_rows_mib_tbl_per_port = dnx_data_mib.general.nof_rows_per_cdmib_port_get(unit);
    buff = &soc->counter_buf64[phy_port * soc->counter_perport];

    if (SOC_BLOCK_IS_CMP(unit, SOC_PORT_BLOCK(unit, phy_port), SOC_BLK_CDPORT)) {
        /* CDMIB_TABLE */
        mib_tbl_addr = soc_mem_addr_get(unit, CDMIB_MEMm, 0, 0, 0, &mib_tbl_acc_type);
        ctrl.flags = SOC_SBUSDMA_MEMORY_CMD_MSG;
        ctrl.cfg_count = 1;
        ...
        ctrl.cb = _soc_sbusdma_port_ctr_cb;
        ctrl.data = INT_TO_PTR(phy_port);
#define _CDMIB_COUNTERS "CDMIB CTRS"
        sal_strncpy_s(ctrl.name, _CDMIB_COUNTERS, sizeof(ctrl.name));
        cfg.acc_type = mib_tbl_acc_type;
        cfg.blk = SOC_PORT_BLOCK(unit, phy_port);
        cfg.blk = SOC_BLOCK2SCH(unit, cfg.blk);
        cfg.addr = mib_tbl_addr + SOC_PORT_BINDEX(unit, phy_port) * nof_rows_mib_tbl_per_port;
        cfg.width = 16; /* (8 counters) * 2, 64bit = 2*int */
        cfg.count = mib_tbl_cnt; /* 8 counters per row in table */
        cfg.addr_shift = 0;
        SOC_IF_ERROR_RETURN
            (soc_sbusdma_desc_create(unit, &ctrl, &cfg, TRUE,
                                     &_soc_port_counter_handles[unit][phy_port][0]));
        LOG_VERBOSE(BSL_LS_SOC_COUNTER,
                 (BSL_META_U(unit,
                             "port %d phy_port %d handle mib_table: %d\n"),
                  port, phy_port, _soc_port_counter_handles[unit][phy_port][0]));
    } else if (SOC_BLOCK_IS_CMP(unit, SOC_PORT_BLOCK(unit, phy_port), SOC_BLK_CLPORT)) {
        ...
        ctrl.cb = _soc_sbusdma_port_ctr_cb;
        ctrl.data = INT_TO_PTR(phy_port);
#define _CLMIBS_COUNTERS "CLMIB CTRS"
        sal_strncpy_s(ctrl.name, _CLMIBS_COUNTERS, sizeof(ctrl.name));
        cfg.width = 2;
        cfg.count = mac_clmib_cnt;
        cfg.acc_type = mac_acc_type;
        cfg.blk = mac_blkoff;
        cfg.addr = mac_addr;
        cfg.addr_shift = 8;
        ...
        LOG_ERROR(BSL_LS_SOC_COMMON,
                (BSL_META_U(unit,
                    "Unknown mac type port:%d phy_port:%d in counter thread\n"),
                    port, phy_port));
    }
...
soc_counter_port_sbusdma_desc_alloc(int unit, soc_port_t port)
{
    ...
    if (SOC_IS_DNX(unit)) {
        ...
        return soc_counter_port_dnx_sbusdma_desc_alloc(unit, port);
    }
#endif /*BCM_DNX_SUPPORT*/
...
#endif /* BCM_XGS5_SWITCH_PORT_SUPPORT || BCM_DNX_SUPPORT*/
...
#endif /*BCM_XGS_SUPPORT || BCM_DNX_SUPPORT*/
/*
 * Function:
 *      soc_counter_thread
 * Purpose:
...
soc_counter_thread(void *unit_vp)
{
    ...
    /* Set Maximum DMA error count */
    soc->counter_max_dma_err_count =
         soc_property_get(unit, spn_SOC_COUNTER_MAX_DMA_ERR_COUNT, 3);
    ...
    if (dma) {
        ...
        /* For dnx, dma setup is in soc_counter_start since
         * it has memory symmetric check. 
         */ 
        if (!SOC_IS_DNX(unit)) {
    ...
    soc_counter_collection_preempt_init(unit);
    ...
    while ((interval = soc->counter_interval) != 0) {
        ...
        soc_counter_collection_preempt_event(unit, SOC_COUNTER_TS_EVENT_COL_START);
        if (soc_counter_collection_preempt_check(unit) == SOC_E_BUSY)
            continue;
...
#ifdef BCM_DNX_SUPPORT
                if (SOC_IS_DNX(unit)) {
                    /*flexe mac doesn't support statdma*/
                    if (IS_FLEXE_MAC_PORT(unit, port) || IS_IL_PORT(unit, port)
        ...
        soc_counter_collection_preempt_event(unit, SOC_COUNTER_TS_EVENT_COL_COMPLETE);
...
skip_collection:
        ...
        LOG_DEBUG(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "Iteration time: %u usec, start time: %u usec\n"),
                     SAL_USECS_SUB(sal_time_usecs(), start_time), start_time));
#endif
...
soc_counter_start(int unit, uint32 flags, int interval, pbmp_t pbmp)
{
    ...
    if (interval != 0) {
         ...
         * sbus dma setup
         */
        if (SOC_IS_DNX(unit) && (soc->counter_flags & SOC_COUNTER_F_DMA))
        {
            rv = _soc_counter_dnx_sbusdma_setup(unit);
            if (rv) {
                (void)_soc_counter_dnx_sbudma_desc_free_all(unit);
                LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
                                      "soc_counter_start: sbus dma setup failed\n")));
                return rv;
            }
...
soc_counter_stop(int unit)
{
    ...
    if (SOC_IS_DNX(unit)) {
        if ((soc->counter_flags & SOC_COUNTER_F_DMA) &&
            soc_feature(unit, soc_feature_sbusdma)) {
            int err;
            if ((err = _soc_counter_dnx_sbudma_desc_free_all(unit)) != 0) {
                LOG_ERROR(BSL_LS_SOC_COMMON,
                          (BSL_META_U(unit,
            ...
            do {
                ...
                LOG_VERBOSE(BSL_LS_SOC_COUNTER,
                         (BSL_META_U(unit,
                          ...
                          id, SOC_COUNTER_NON_DMA_START, NUM_PIPE(unit), non_dma->mem));
