...
int soc_cmic_uc_msg_send(int unit, int uC, mos_msg_data_t *msg,
    ...
    while (1) {                 /* Retry loop */
        ...
        if (sal_sem_take(soc->uc_msg_send_queue_sems[uC], MILLISECOND_USEC) == -1) {
            /* No point in waiting of if the core is in exception */
            if ((soc_uc_status(unit, uC, &stat) != SOC_E_NONE) || (stat == 0)) {
                rc = SOC_E_TIMEOUT;
                break;
            }
