...
#define IRQ_MASK_SET(u, i, m)  { \
                               ...
                               if (i == IRQ_MASK_INDEX(CMC0_CH0_DESC_DONE)) { \
                                   WRITE_INTC_INTR4(u, _irq_reg_map[i].enable, m); \
                               } else { \
...
#ifdef INCLUDE_KNET
#include <soc/knet.h>
#define WRITE_INTC_INTR4(unit, reg, rv) \
        soc_knet_irq_mask_set(unit, soc_reg_addr(unit, reg, REG_PORT_ANY, 0), rv)
#else
#define WRITE_INTC_INTR4(unit, reg, rv) \
        WRITE_INTC_INTR(unit, reg, rv)
#endif
...
/* Instrumentation to keep the interrupt count per source */
typedef struct cmicx_intc_intr_raw_status_s {
    uint64  intr_count[CMICX_INTR_REG_NUM][32];
} cmicx_intc_intr_raw_status_t;

/* local variables */

STATIC cmicx_intc_intr_raw_status_t  intc_intr_raw_status[SOC_MAX_NUM_DEVICES];
...
soc_cmicx_intr_init(int unit, soc_cmic_intr_op_t *intr_op)
{
    ...
    /* Adaptable Polling */
    soc->irq_adaptive_poll_mode =
         soc_property_get(unit, spn_IRQ_ADAPTIVE_POLL_MODE, 1);
...
#ifdef __DUNE_SLK_BCM_CPU__
        soc->msi_x_intr_clear_auto_mode_enable =
             soc_property_get(unit, spn_MSI_X_INTR_CLEAR_AUTO_MODE_ENABLE, 0);
#else
        soc->msi_x_intr_clear_auto_mode_enable =
    ...
    sal_memset(&intc_intr_raw_status[unit], 0, sizeof(cmicx_intc_intr_raw_status_t));
    ...
    do {
        ...
        for (i = 0; i < CMICX_INTR_REG_NUM; i++) {
            ...
            if (SOC_KNET_MODE(unit) && i == IRQ_MASK_INDEX(CMC0_CH0_DESC_DONE)) {
                for (chan = 0; chan < CMICX_N_DMA_CHAN; chan++) {
                    if (SOC_UCHAN(unit, chan)) {
                        break;
                    }
                }
                if (chan == CMICX_N_DMA_CHAN) {
                    continue;
                }
            }
#endif
            ...
            while (irqStat) {
                uint32 bit;
                bit = _irq_least_bit_set(irqStat);
                mask = 0x01 << bit;
                intr = bit;
                ...
                if ((irqMask & mask) && IS_IRQ_VALID(intr)) {
                    if ((CMIC_INTR_HANDLE(unit, intr)->num == intr) &&
                        ...
                        LOG_VERBOSE(BSL_LS_SOC_INTR, (BSL_META_U(unit,
                                                    "%s:unit %d, intr %u\n"),
                                                    __func__, unit, intr));
                        COMPILER_64_ADD_32(intc_intr_raw_status[unit].intr_count[i][bit], 1);
                        CMIC_INTR_HANDLE(unit, intr)->intr_fn(unit,
                            CMIC_INTR_HANDLE(unit, intr)->intr_data);
                        if (soc->irq_adaptive_poll_mode) {
                            poll_counter++;
                        }
                    }
                }
