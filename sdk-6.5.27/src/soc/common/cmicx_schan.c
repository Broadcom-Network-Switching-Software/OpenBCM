...
#define CMICX_SCHAN_OP_RETRY_AVAIL(opcode) \
...
#define CMICX_SCHAN_OP_RETRY_TIMES  3
...
/*******************************************
* @function _soc_cmicx_schan_op_event_dpc
...
********************************************/
STATIC void
_soc_cmicx_schan_op_event_dpc(void *p_unit, void *event_id, void *cmc, void *p4,
                              void *p5)
{
...
STATIC int _soc_cmicx_schan_get(int unit, int *ch)
{
    ...
    int rv = SOC_E_BUSY;
    soc_timeout_t to;
    int ch_count = _soc_cmicx_schan[unit].ch_num_cfg_count;
        ...
        if (schan_err & SC_CHx_SCHAN_ERR_ERR_BIT) {
            switch(err_code)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if (!soc_feature(unit, soc_feature_ignore_schan_ecc_error)) {
                                 ...
                                 int ch)
{
    ...
    if ((rv == SOC_E_TIMEOUT || rv == SOC_E_INTERNAL) &&
       ((flags & SCHAN_OP_FLAG_DO_NOT_LOG_TIMEOUTS) == 0)) {
        if (LOG_CHECK(BSL_LS_SOC_SCHAN | BSL_ERROR)) {
            LOG_ERROR(BSL_LS_SOC_SCHAN,
                      (BSL_META_U(unit,
                       "soc_cmicx_schan_op operation timed out\n")));
            soc_schan_dump(unit, msg, dwc_read, BSL_ERROR);
        }
    }
...
/*******************************************
* @function _soc_cmicx_schan_timeout_stats
...
********************************************/
STATIC int
_soc_cmicx_schan_timeout_stats(int unit, uint32 opcode)
{
    switch (opcode) {
    case READ_MEMORY_CMD_MSG:
...
retry:
    ...
    if (rv == SOC_E_TIMEOUT || rv == SOC_E_INTERNAL) {
        _soc_cmicx_schan_timeout_stats(unit, opcode);

        if (retry_enable && CMICX_SCHAN_OP_RETRY_AVAIL(opcode)) {
            if (++retry_times <= CMICX_SCHAN_OP_RETRY_TIMES) {
                ...
                /* Retry */
                if ((soc_pci_read(unit, CMIC_COMMON_POOL_SCHAN_CHx_CTRL(ch)) &
                              SC_CHx_MSG_DONE)) {
                   goto retry;
                }
            }
            if ((soc_pci_read(unit, CMIC_COMMON_POOL_SCHAN_CHx_CTRL(ch)) &
                              SC_CHx_MSG_DONE) == 0) {
                /* Not able to reset, channels may have lockedup */
                /* Generate switch event */
                sal_dpc(_soc_cmicx_schan_op_event_dpc, INT_TO_PTR(unit),
                        INT_TO_PTR(SOC_SWITCH_EVENT_IO_ERROR_SCHANPIO_TIMEOUT),
                        NULL, NULL, NULL);
            }
        }
        /* SW timeout error due to internal error */
        if (rv == SOC_E_INTERNAL) {
            rv = SOC_E_TIMEOUT;
        }
        if (LOG_CHECK(BSL_LS_SOC_SCHAN | BSL_ERROR)) {
            LOG_ERROR(BSL_LS_SOC_SCHAN,
                      (BSL_META_U(unit,
                       "operation (0x%x) timed out channel=%d\n"),
                       opcode, ch));
        }
...
_cmicx_schan_intr_init(int unit)
{
    ...
    /* Register interrupts */
    handle = sal_alloc(ch_count * sizeof(soc_cmic_intr_handler_t),
                        "schan_interrupt");
    if (handle == NULL) {
        return SOC_E_MEMORY;
    }
    ...
    rv = soc_cmic_intr_register(unit, handle, ch_count);
error:
    sal_free(handle);
...

{
   ...
   _soc_cmicx_schan[unit].ch_num_cfg_count =
         soc_property_get(unit, spn_CMICX_SCHAN_NUM_CFG_COUNT, CMIC_SCHAN_NUM_MAX);
   if (( _soc_cmicx_schan[unit].ch_num_cfg_count < 1) ||
         (_soc_cmicx_schan[unit].ch_num_cfg_count > CMIC_SCHAN_NUM_MAX)) {
       return SOC_E_PARAM;
   }


   _soc_cmicx_schan[unit].ch_map = (0x01 << _soc_cmicx_schan[unit].ch_num_cfg_count) -1;
