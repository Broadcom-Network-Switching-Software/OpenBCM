...
/*
 * Function:
 *     soc_phyctrl_signal_detect_status_get
 * Purpose:
 *     get phy signal detect and signal detect change status
 * Parameters:
 *      unit - (IN) Unit number.
 *      port - (IN) Port.
 *      sd_status - (OUT) signal detect status.
 * Returns:
 *      SOC_E_xxx
 * Notes:
 */
int
soc_phyctrl_signal_detect_status_get(int unit, soc_port_t port,
                                     soc_phy_signal_detect_status_t* sd_status)

{
    ...
    SOC_NULL_PARAM_CHECK(sd_status);
    ...
    rv = (PHY_SIGNAL_DETECT_STATUS_GET(pd, unit, port, sd_status));

    INT_MCU_UNLOCK(unit);

    if (SOC_FAILURE(rv)) {
        PHYCTRL_WARN(("soc_phyctrl_signal_detect_status_get failed %d\n", rv));
    }
    return rv;
}
