...
retry:
        if (!param->flags || (param->flags & (SOC_MEM_WRITE_SET_ONLY | SOC_MEM_WRITE_INCR_NOADD))) {
            ...
            if (param->flags & SOC_MEM_WRITE_INCR_NOADD) {
                /* Do not add to the address for each increment */
                soc_reg_field_set(unit, CMIC_CMC0_SBUSDMA_CH0_REQUESTr, &rval, INCR_NOADDf, 1);
            }
        ...
        SOC_MEM_BLOCK_ITER(unit, param->mem, blk) {
            ...
            SOC_MEM_STATE_MEMBER_GET(unit, param->mem, cache[blk], cache);
            SOC_MEM_STATE_MEMBER_GET(unit, param->mem, vmap[blk], vmap);
