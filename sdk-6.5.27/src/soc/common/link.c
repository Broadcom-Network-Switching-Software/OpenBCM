...
soc_linkscan_loopback_info_set(int unit, int port, int enable)
{
    ...
    /* Configure loopback info to hw linkscan firmware. */
    SOC_IF_ERROR_RETURN(SOC_LINKSCAN_LOOPBACK_INFO_SET(unit, pport, enable));
                            ...
                            soc_linkscan_flr_timeout_t timeout)
{
    ...
    if (enable) {
        SOC_IF_ERROR_RETURN(soc_linkscan_flr_timeout_value_get(unit, timeout, &value));
    }

#ifdef BCM_DNX_SUPPORT
    if ((SOC_IS_DNX(unit)) && (IS_OTN_PORT(unit, port)))
    {
        for (pport=0; pport<SOC_MAX_NUM_PORTS; pport++)
        {
            if (port == si->port_p2l_mapping[pport])
            {
                if (enable)
                {
                    SOC_IF_ERROR_RETURN(SOC_LINKSCAN_FLR_TIMEOUT_SET(unit, pport, value));
                }
                /* Enable or Disbale FLR. */
                SOC_IF_ERROR_RETURN(SOC_LINKSCAN_FLR_ENABLE_SET(unit, pport, enable));
            }
        }
