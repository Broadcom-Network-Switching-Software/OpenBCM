...
)
{
    ...
    if (port == REG_PORT_ANY || port == SOC_CORE_ALL) {
        ...
        inst = actual_port;
        if (port == actual_port) {
            soc_block_types_t reg_block = SOC_REG_INFO(unit, reg).block;
            if (*reg_block == SOC_BLK_CDMAC || *reg_block == SOC_BLK_CDPORT) {
                /* used to mark that the instance is a port number */
...
soc_reg_above_64_get(int unit, soc_reg_t reg, int port, int index, soc_reg_above_64_val_t data)
{
    ...
    if (SOC_REG_IS_ABOVE_64(unit, reg)) 
    {
        ...
        if (addr == (uint32)(-1) || block == -1) {
            LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
              "soc_reg_above_64_get failed to get the address of %s\n"), SOC_REG_NAME(unit, reg)));
            return SOC_E_FAIL;
        }
...
/*
 * Read an internal SOC register through S-Channel messaging buffer.
 * Handle register at any size, write only the size of the register.
 */
 
int
soc_reg_above_64_get_actual_size(int unit, soc_reg_t reg, int port, int index, uint32 *data)
{
    uint32 addr;
    int block = -1;
    uint8 at; 
...
/*
 * Write an internal SOC register through S-Channel messaging buffer.
 * Handle register at any size; do not require data size above the actual register size in uint32s.
 */

int
soc_reg_above_64_set_actual_size(int unit, soc_reg_t reg, int port, int index, uint32 *data)
{
    return soc_reg_above_64_set(unit, reg, port, index, data);
}
...
soc_reg32_set(int unit, soc_reg_t reg, int port, int index, uint32 data)
{
    ...
    for (i = 0; i < access_info.num_blks && rv == SOC_E_NONE; ++i) {
#ifdef BCM_TOMAHAWK3_SUPPORT
        /* For 56983 Lower core_group SKU, we make sure DATA_SPLIT and DUPL writes get
...
/*
 * Function: soc_reg_above_64_field_get_actual_size
 * Purpose:  Get the value of a field from a register, does not clear uint32s outside the size of the field
 */
int
soc_reg_above_64_field_get_actual_size(int unit, soc_reg_t reg, uint32 *regval, 
                           soc_field_t field, uint32 *field_val)
{
    soc_field_info_t *finfop;

    if (!SOC_REG_IS_VALID(unit, reg)) {
#if !defined(SOC_NO_NAMES)
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "reg %s is invalid\n"), soc_reg_name[reg]));
#endif
        return SOC_E_PARAM;
    }
    ...
    if (finfop == NULL) {
#if !defined(SOC_NO_NAMES)
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "Did not find field %s in register %s\n"), soc_fieldnames[field], soc_reg_name[reg]));
#endif
        return SOC_E_PARAM;
    }
    
    if (finfop->len > 0) {
        field_val[(finfop->len - 1) / SHR_BITWID] = 0;
        /* COVERITY  assert validates the input for NULL */
        /* coverity[var_deref_op : FALSE] */
        SHR_BITCOPY_RANGE(field_val, 0, regval, finfop->bp, finfop->len);
    } else {
        field_val[0] = 0;
...
/*
 Set the value of a register's field, does not require values wider than that of the register and field.
 */
int
soc_reg_above_64_field_set_actual_size(int unit, soc_reg_t reg, uint32 *regval,
                            soc_field_t field, CONST uint32 *value)
{
    soc_field_info_t *finfop;
    unsigned msb_bits;
    ...
    /* Check if the field value will fit into the field: Verify that value's msb word bits that do not fit in the field are all zeroes. */
    /* COVERITY  assert validates the input for NULL */
    /* coverity[var_deref_op : FALSE] */
    msb_bits = finfop->len % 32; /* Bits in msb word of field value, verify the left over bits are zeros */
    if ((value[finfop->len / 32] & ~(((uint32)0xffffffff) >> (32 - msb_bits))) != 0) {
#if !defined(SOC_NO_NAMES)
        LOG_ERROR(BSL_LS_SOC_COMMON, (BSL_META_U(unit, "reg %s field %s has %u bits and a larger value was given\n"), soc_reg_name[reg], soc_fieldnames[field], (unsigned)finfop->len));
#endif
        return SOC_E_PARAM;
    }
