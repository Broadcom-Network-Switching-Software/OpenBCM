...
kaps_jr1_status jerp_kaps_xpt_translate_blk(int unit, uint32 blk_nr, uint32 row_nr, uint32 *blk_hw_nr, uint32 *row_hw_nr) {
...
kaps_jr1_status jerp_kaps_xpt_hb_read(void *xpt, uint32_t blk_nr, uint32_t row_nr, uint8_t *bytes) {
    int rv = KAPS_JR1_OK, unit;
    uint32 data;
    uint32 blk_hw_nr;
    uint32 row_hw_nr;
...
kaps_jr1_status jerp_kaps_xpt_hb_write(void *xpt, uint32_t blk_nr, uint32_t row_nr, uint8_t *bytes) {
    int rv = KAPS_JR1_OK, unit;
    uint32 blk_hw_nr;
    uint32 row_hw_nr;
...
kaps_jr1_status jerp_kaps_xpt_hb_dump_reply(int unit, uint32 start_hw_blk_num, uint32 end_hw_blk_num, 
        ...
        for(;;){
            ...
            if (soc_timeout_check(&to)) {
                LOG_CLI((BSL_META_U(0, "%s() State machine control done signal timeout!\n"),
                                        FUNCTION_NAME()));
                rv = KAPS_JR1_FATAL_TRANSPORT_ERROR;
                return rv;
            }
        }
    }
...
kaps_jr1_status jerp_kaps_xpt_hb_dump(void *xpt, uint32_t start_blk_num, uint32_t start_row_num, uint32_t end_blk_num,
...
kaps_jr1_status jerp_kaps_xpt_hb_copy_write(int unit, uint32 src_hw_blk_num, uint32 dest_hw_blk_num, uint32 src_hw_row_num, uint32 dest_hw_row_num,
...
kaps_jr1_status jerp_kaps_xpt_hb_copy(void *xpt, uint32_t src_blk_num, uint32_t src_row_num, uint32_t dest_blk_num,
...
kaps_jr1_status jerp_kaps_xpt_translate_blk_func_offset_to_mem_reg(int unit,
                                                          uint8     blk_id,
                                                          uint32     func,
                                                          uint32     offset,
                                                          soc_mem_t *mem,
