...
int
soc_dpp_str_prop_ext_ram_type(int unit, SOC_TMC_DRAM_TYPE *soc_dram_type)
{
    char *propkey, *propval;
    ...
    if ((!propval) || (sal_strcmp(propval, "NONE") == 0)) {
        *soc_dram_type = SOC_TMC_DRAM_NOF_TYPES;
    } else if (sal_strcmp(propval, "GDDR3") == 0) {
        *soc_dram_type = SOC_TMC_DRAM_TYPE_GDDR3;
    } else if (sal_strcmp(propval, "DDR2") == 0) {
        *soc_dram_type = SOC_TMC_DRAM_TYPE_DDR2;
    } else if (sal_strcmp(propval, "DDR3") == 0) {
        *soc_dram_type = SOC_TMC_DRAM_TYPE_DDR3;        
    } else {
        SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("Unexpected property value")));
    }
...
soc_dpp_attach(int unit)
{
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        if (SOC_MEM_IS_VALID(unit, mem)) {
            SOP_MEM_STATE_MAX_INDEX_SET(unit, mem,
    ...
    rv = soc_sand_indirect_mem_access_init(unit);
    if (rv != SOC_E_NONE) {
       SOCDNX_EXIT_WITH_ERR(rv, (_BSL_SOCDNX_MSG("failed to Initialize indirect memory access")));
    }
exit: 
...
{    
    ...
    {  
        soc_sand_rv = chip_sim_em_init(unit, SOC_SAND_DEV_ARAD, SOC_DPP_IS_LEM_SIM_ENABLE(unit));
        SOCDNX_IF_ERR_EXIT(soc_sand_rv);
    }
