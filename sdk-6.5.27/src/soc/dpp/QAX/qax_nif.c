...
int
soc_qax_nif_sif_set(int unit, int core, uint32 first_phy, int enable)
{
    ...
    if (SOC_IS_QUX(unit)) {
        ...
        soc_reg_field_set(unit, NIF_SIF_CFGr, &reg_val, SIF_PORT_ENf, enable);
        SOCDNX_IF_ERR_EXIT(WRITE_NIF_SIF_CFGr(unit, reg_val));

        SOCDNX_IF_ERR_EXIT(READ_NIF_ADDITIONAL_RESETSr(unit, &reg_val));
        soc_reg_field_set(unit, NIF_ADDITIONAL_RESETSr, &reg_val, SIF_RSTNf, enable);
