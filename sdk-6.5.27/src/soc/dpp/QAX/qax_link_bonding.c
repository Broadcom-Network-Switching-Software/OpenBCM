  ...
  )
{
    ...
    SOC_TMC_LB_VALUE_MAX_CHECK(lbg_id, SOC_TMC_LB_LBG_MAX_WITH_CONTROL, "lbg_id");
  ...
  )
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        lb_nof_entries =  soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "lb_num_of_entry", 2);
        if (lb_nof_entries < 1 || lb_nof_entries > 2) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG(
                "Unexpected property value (\"%d\") for %s"), lb_nof_entries, "lb_num_of_entry"));
        }
        ...
        rv = LB_INFO_ACCESS.is_allocated(unit, &is_sw_allocated);
        SOCDNX_IF_ERR_EXIT(rv);

        if(!is_sw_allocated) {
            if (LB_INFO_ACCESS.alloc(unit) != SOC_E_NONE) {
                LOG_ERROR(BSL_LS_SOC_LB,
                          (BSL_META_U(unit,
        ...
        SOC_PBMP_ITER(pbmp, port_i) {
            ...
            else if (SOC_DPP_CONFIG(unit)->qax->lbi_en) {
        ...
        if (lb_ctrl_port != -1) {
            SOCDNX_IF_ERR_EXIT(qax_lb_port_speed_set(unit, lb_ctrl_port, speed));
        }
    }
  ...
  )
{
    ...
    if (is_control) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_tm_port_to_base_q_pair_get(unit, core, tm_port, &base_queue_pair));

        *fld_val32_tmp = modem_id;
        fld_index = (base_queue_pair % 16);
        SOCDNX_IF_ERR_EXIT(soc_reg64_get(unit, reg, REG_PORT_ANY, 0, &reg_val64));
        val_64_tmp[0] = COMPILER_64_LO(reg_val64);
        val_64_tmp[1] = COMPILER_64_HI(reg_val64);
        SHR_BITCOPY_RANGE(val_64_tmp, fld_index*4, fld_val32_tmp, 0, 4);
        COMPILER_64_SET(reg_val64, val_64_tmp[1], val_64_tmp[0]);
        SOCDNX_IF_ERR_EXIT(soc_reg64_set(unit, reg, REG_PORT_ANY, 0, reg_val64));
    } else {
  ...
  )
{
    ...
    LB_INFO_ACCESS.lbg_speed.get(unit, lbg_id, &speed);
    ...
    if ((lbg_id == SOC_TMC_LB_CONTROL) || (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "lb_num_of_entry", 2) == 1)) {
  ...
  )
{
    ...
    if (shaper_config->op_flags == 0) {
        SOCDNX_IF_ERR_EXIT(rv);
    }
    SOC_TMC_LB_VALUE_MAX_CHECK(modem_id, SOC_TMC_LB_MODEM_MAX, "modem_id");
    if (shaper_config->op_flags & SOC_TMC_LB_MODEM_SHARPER_CONFIG_OP_FLAG_HDR_COMPENSATION) {
    ...
    if (shaper_config->op_flags & SOC_TMC_LB_MODEM_SHARPER_CONFIG_OP_FLAG_SHAPER) {
    ...
    if (shaper_config->op_flags & SOC_TMC_LB_MODEM_SHARPER_CONFIG_OP_FLAG_SHAPER) {
        ...
        rate_granularity = SOC_TMC_LB_EGR_MODEM_SHAPER_RATE_GRANULARITY(unit);
        freq_mhz = arad_chip_kilo_ticks_per_sec_get(unit)/1000;
        if (freq_mhz != 250) {
             
            rate_granularity = rate_granularity*freq_mhz/250;
        }

        soc_sand_u64_multiply_longs(shaper_config->rate, 1000, &u64_1);
        soc_sand_u64_devide_u64_long(&u64_1, rate_granularity, &u64_2);
        hw_rate = u64_2.arr[0];
  ...
  )
{
    ...
    *hw_hdr_compensation = soc_mem_field32_get(unit, modem_table, modem_data, HDR_COMPENSATIONf);
    ...
    soc_sand_u64_multiply_longs(hw_rate, rate_granularity, &u64_1);
   ...
   )
{
    ...
    SOCDNX_NULL_CHECK(is_ctrl_port);

    if (is_on_modem)
    {
        SOCDNX_IF_ERR_EXIT(qax_lb_modem_ports_on_same_interface_get(unit, port, &modem_pbmp));
        if (SOC_PBMP_NOT_NULL(modem_pbmp)) {
            SOC_PBMP_COUNT(modem_pbmp, modem_cnt);
            *is_ctrl_port = 1;
        }
    }
    else
    {
        rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_tm_port_get, (unit, port, &tm_port, &core));
        SOCDNX_IF_ERR_EXIT(rv);
        rv = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_port2egress_offset, (unit, core, tm_port, &egress_offset));
        SOCDNX_IF_ERR_EXIT(rv);
        if (egress_offset == SOC_QAX_EGR_IF_LBG_RESERVE) {
            *is_ctrl_port = 1;
        }
...
soc_error_t
  qax_lb_ing_control_packet_identification_config_set (
    SOC_SAND_IN  int unit,
    SOC_SAND_IN  uint32 flags,
    SOC_SAND_IN  uint16 global_tpid
   )
{
    soc_error_t
        rv = SOC_E_NONE;
    ...
    reg = ILB_REG_019Cr;
    rv = soc_reg32_get(unit, reg, REG_PORT_ANY, 0, &reg_val32);
    SOCDNX_IF_ERR_EXIT(rv);

    soc_reg_field_set(unit, reg, &reg_val32, FIELD_0_15f, global_tpid);

    rv = soc_reg32_set(unit, reg, REG_PORT_ANY, 0, reg_val32);
    SOCDNX_IF_ERR_EXIT(rv);
...
soc_error_t
  qax_lb_ing_control_packet_identification_config_get (
    SOC_SAND_IN  int unit,
    SOC_SAND_IN  uint32 flags,
    SOC_SAND_OUT  uint16 *global_tpid
   )
{
    soc_error_t
        rv = SOC_E_NONE;
    ...
    SOCDNX_NULL_CHECK(global_tpid);

    reg = ILB_REG_019Cr;
    rv = soc_reg32_get(unit, reg, REG_PORT_ANY, 0, &reg_val32);
    SOCDNX_IF_ERR_EXIT(rv);

    *global_tpid = soc_reg_field_get(unit, reg, reg_val32, FIELD_0_15f);
  ...
  )
{
    ...
    if (is_check_by_port) {
        ...
        if (IS_LBG_PORT(unit,port)) {
            ...
            lb_egr_fifo_range = (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "lb_num_of_entry", 2) == 1) ? 1 : LB_EGR_FIFO_RANGE_PER_LBG;
        }
  ...
  )
{
    ...
    if (IS_LBG_PORT(unit,port)) {
        ...
        SOC_TMC_LB_VALUE_MAX_CHECK(lbg_id, SOC_TMC_LB_LBG_MAX, "lbg_id");
        lb_egr_fifo_range = (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "lb_num_of_entry", 2) == 1) ? 1 : LB_EGR_FIFO_RANGE_PER_LBG;
        lb_egr_credit_entry = (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "lb_num_of_entry", 2) == 1) ? 1: LB_EGR_CREDIT_ENTRY_PER_LBG;
    }
    else if (SOC_DPP_CONFIG(unit)->qax->lbi_en && (pcnt != 0)) {
    ...
    SOCDNX_IF_ERR_EXIT(qax_lb_egr_lbg_enable_get(unit, lbg_id, &is_lb_enable_ori));
    ...
    SOCDNX_IF_ERR_EXIT(qax_lb_port_speed_sw_set(unit, port, 0, speed));
  ...
  )
{
    ...
    if (port != -1) {
        ...
        SOC_PBMP_COUNT(modem_pbmp, pcnt);
        if (IS_LBG_PORT(unit,port)) {
            SOCDNX_IF_ERR_EXIT(sw_state_access[unit].dpp.bcm.linkbonding.port_to_lbg.get(unit, port, &lbg_id_l));
            SOC_TMC_LB_VALUE_MAX_CHECK(lbg_id_l, SOC_TMC_LB_LBG_MAX, "lbg_id_l");
        }
        else if (SOC_DPP_CONFIG(unit)->qax->lbi_en && (pcnt != 0)) {
            lbg_id_l = SOC_TMC_LB_LBG_MAX+1;
        }
        else {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PORT, (_BSL_SOCDNX_MSG("Port %d is not lb port"),port));
        }
   ...
   LB_INFO_ACCESS.lbg_speed.set(unit, lbg_id_l, speed);
  ...
  )
{
    ...
    soc_lb_lbg_packet_config_t
        lbg_pkt;
    uint32
        is_lb_enhance_support = 0;

    SOCDNX_INIT_FUNC_DEFS;

    SOCDNX_NULL_CHECK(tx_lbg_info);

    if (lbg_id == SOC_TMC_LB_CONTROL) {
        rv = qax_lb_enhancement_support_get(unit, &is_lb_enhance_support);
        SOCDNX_IF_ERR_EXIT(rv);
        if (is_lb_enhance_support == 0) {
            SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM,
                 (_BSL_SOCDNX_MSG("%s: control traffic is not supported in this devide\n"), unit));
        }
    }
    ...
    if (SOC_IS_QUX(unit))
    {
        ...
        LB_INFO_ACCESS.lbg_speed.get(unit, lbg_id, &tx_lbg_info->speed);
    }
  ...
  )
{
    ...
    uint32 is_lb_enhance_support = 0;
    ...
    if ((socLbDirectionTx == lb_key->direction) || (socLbDirectionBoth == lb_key->direction)) {
        ...
        if ((socLbObjLbg == lb_key->obj_type) || (socLbObjAll == lb_key->obj_type)) {
            ...
            if (lb_key->lbg_id == -1) {
                lbg_start = 0;
                lbg_end = is_lb_enhance_support ? SOC_TMC_LB_LBG_MAX_WITH_CONTROL : SOC_TMC_LB_LBG_MAX;
            }
            else {
                lbg_start = lbg_end = lb_key->lbg_id;
            }
...
soc_error_t
  qax_lb_enhancement_support_get(
    SOC_SAND_IN  int                          unit,
    SOC_SAND_OUT uint32                       *is_support
  )
{
    soc_error_t
        rv = SOC_E_NONE;
    ...
    SOCDNX_NULL_CHECK(is_support);

    SOC_REG_ABOVE_64_CLEAR(reg_val_abv64);
    rv = soc_reg_above_64_get(unit, ECI_OGER_1003r, 0, 0, reg_val_abv64);
    *is_support = SHR_BITGET(reg_val_abv64, support_bit) ? 1 : 0;
