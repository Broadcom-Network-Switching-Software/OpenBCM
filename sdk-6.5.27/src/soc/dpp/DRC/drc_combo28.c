...
{
    soc_reg_above_64_val_t voq_dram_bound_prms_max_thresholds;
    soc_reg_above_64_val_t* voq_dram_bound_prms_copy = 0;
    soc_reg_above_64_val_t* cgm_voq_copy_runner;
    int i;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    SOCDNX_ALLOC(voq_dram_bound_prms_copy, soc_reg_above_64_val_t , ARAD_ITM_QT_RT_CLS_MAX + 1, "voq_dram_bound_prms_copy");
    cgm_voq_copy_runner = voq_dram_bound_prms_copy;
    for (i = 0; i <= ARAD_ITM_QT_RT_CLS_MAX; i++, cgm_voq_copy_runner++)
    {
        SOCDNX_IF_ERR_EXIT(soc_mem_read(unit, CGM_VOQ_DRAM_BOUND_PRMSm, MEM_BLOCK_ALL, i, cgm_voq_copy_runner));
    }

    
    LOG_INFO(BSL_LS_SOC_DRAM, (BSL_META_U(unit, "%s() forcing traffic into OCB\n"), FUNCTION_NAME()));
    sal_memset(voq_dram_bound_prms_max_thresholds, 0xFF, sizeof(soc_reg_above_64_val_t));
    for (i = 0; i <= ARAD_ITM_QT_RT_CLS_MAX; i++ )
    {
        SOCDNX_IF_ERR_EXIT(soc_mem_write(unit, CGM_VOQ_DRAM_BOUND_PRMSm, MEM_BLOCK_ALL, i, voq_dram_bound_prms_max_thresholds));
    }
    ...
    for (i = 0; i <= ARAD_ITM_QT_RT_CLS_MAX; i++, cgm_voq_copy_runner++)
    {
        SOCDNX_IF_ERR_EXIT(soc_mem_write(unit, CGM_VOQ_DRAM_BOUND_PRMSm, MEM_BLOCK_ALL, i, cgm_voq_copy_runner));
    }


exit:
    SOCDNX_FREE(voq_dram_bound_prms_copy);
    SOCDNX_FUNC_RETURN;
}
