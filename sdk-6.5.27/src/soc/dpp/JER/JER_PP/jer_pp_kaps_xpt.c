...
#define JER_KAPS_XPT_EXTENDED_FUNC_ENUM     KAPS_JR1_FUNC14
#define JER_KAPS_XPT_RPB_FUNC_KAPS_SEARCH   KAPS_JR1_FUNC3
...
STATIC kaps_jr1_status jer_pp_kaps_search(void *xpt, 
                                     uint8_t *key, 
                                     ...
                                     struct kaps_jr1_search_result *kaps_result)
{
    ...
    rv = jer_pp_kaps_command_word_write(unit, blk_id, KAPS_JR1_CMD_READ, func, 0);
    if (rv != SOC_SAND_OK) {
        return KAPS_JR1_INTERNAL_ERROR;
    }
...
kaps_jr1_status jer_pp_kaps_write_command_arm(void *xpt,
                                            uint8 blk_id,
                                            uint32 cmd,
                                            uint32 func,
                                            uint32 offset,
                                            ...
                                            uint8 *bytes)
{
    ...
    if (nbytes == 60) {
        ...
        return KAPS_JR1_FATAL_TRANSPORT_ERROR;
    }
...
kaps_jr1_status jer_pp_kaps_write_command(void *xpt,  
                                            uint8 blk_id, 
                                            uint32 cmd,
                                            uint32 func,
                                            uint32 offset,
...
kaps_jr1_status jer_pp_kaps_read_command(void *xpt,  
                                           uint32 blk_id, 
                                           uint32 cmd,
                                           uint32 func,
                                           uint32 offset,
...
STATIC kaps_jr1_status jer_pp_kaps_command(void *xpt,
...
kaps_jr1_status kaps_register_read(void *xpt, uint32_t offset, uint32_t nbytes, uint8_t *bytes)
{
    int rv, unit;
    uint32 i, val;
    uint8 array_index;
...
kaps_jr1_status kaps_register_write(void *xpt, uint32_t offset, uint32_t nbytes, uint8_t *bytes)
{
    int rv, unit;
...
uint32 jer_pp_xpt_init(int unit, void **xpt)
{
    ...
    *xpt = soc_sand_os_malloc(sizeof(JER_KAPS_XPT), "kaps_xpt");
    ...
    sal_memset(xpt_p, 0 , sizeof(JER_KAPS_XPT));

    xpt_p->jer_kaps_xpt.device_type = KAPS_JR1_DEVICE;

    if (SOC_IS_JERICHO_PLUS(unit)) {
        xpt_p->jer_kaps_xpt.kaps_jr1_search = qax_pp_kaps_search;
        xpt_p->jer_kaps_xpt.kaps_jr1_register_read = qax_kaps_register_read;
        xpt_p->jer_kaps_xpt.kaps_jr1_command = qax_pp_kaps_command;
        xpt_p->jer_kaps_xpt.kaps_jr1_register_write = qax_kaps_register_write;

        xpt_p->jer_kaps_xpt.kaps_jr1_hb_read = jerp_kaps_xpt_hb_read;
        xpt_p->jer_kaps_xpt.kaps_jr1_hb_write = jerp_kaps_xpt_hb_write;

        
        if (!SOC_IS_QAX(unit) || SOC_IS_QUX(unit)) {
            xpt_p->jer_kaps_xpt.kaps_jr1_hb_dump = jerp_kaps_xpt_hb_dump;
            xpt_p->jer_kaps_xpt.kaps_jr1_hb_copy = jerp_kaps_xpt_hb_copy;
        } else {
            xpt_p->jer_kaps_xpt.kaps_jr1_hb_dump = NULL;
            xpt_p->jer_kaps_xpt.kaps_jr1_hb_copy = NULL;
        }
    } else if (SOC_IS_JERICHO(unit)) {
        xpt_p->jer_kaps_xpt.kaps_jr1_search = jer_pp_kaps_search;
        xpt_p->jer_kaps_xpt.kaps_jr1_register_read = kaps_register_read;
        xpt_p->jer_kaps_xpt.kaps_jr1_command = jer_pp_kaps_command;
        xpt_p->jer_kaps_xpt.kaps_jr1_register_write = kaps_register_write;
    }
