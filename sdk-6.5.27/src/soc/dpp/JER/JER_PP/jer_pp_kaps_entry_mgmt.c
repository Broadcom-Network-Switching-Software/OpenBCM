    ...
    )
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    kaps_jr1_db_get_prefix_handle(db_p, data, prefix_len, &db_entry);
    if (db_entry != NULL) {
        is_update = 1;
    }

    if (!is_update) {
        res = kaps_jr1_db_add_prefix(
                db_p, 
                data, 
                prefix_len,
                &db_entry);
        ...
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            kaps_jr1_db_delete_entry(db_p, db_entry); 
            *success = SOC_SAND_FAILURE_INTERNAL_ERR;
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("internal error occured")));
        }

        res = kaps_jr1_entry_add_ad(
                db_p,
                db_entry,
                ad_entry
              );
        ...
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            kaps_jr1_db_delete_entry(db_p, db_entry); 
            kaps_jr1_ad_db_delete_entry(ad_db_p, ad_entry); 
            *success = SOC_SAND_FAILURE_INTERNAL_ERR;
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("internal error occured")));
        }

        if (SOC_IS_JERICHO_PLUS(unit) && (hitbit_enabled != 0)) {
            res = kaps_jr1_hb_db_add_entry(hb_db_p, &hb_entry);
            if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
                kaps_jr1_db_delete_entry(db_p, db_entry); 
                kaps_jr1_ad_db_delete_entry(ad_db_p, ad_entry); 
                *success = SOC_SAND_FAILURE_INTERNAL_ERR;
            ...
            if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
                kaps_jr1_db_delete_entry(db_p, db_entry); 
                kaps_jr1_ad_db_delete_entry(ad_db_p, ad_entry); 
                kaps_jr1_hb_db_delete_entry(hb_db_p, hb_entry); 
                *success = SOC_SAND_FAILURE_INTERNAL_ERR;
                SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("internal error occured")));
            }
        }
        ...
        res = kaps_jr1_entry_get_ad(db_p, db_entry, &ad_entry);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            *success = SOC_SAND_FAILURE_INTERNAL_ERR;
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("internal error occured")));
        }

        res = kaps_jr1_ad_db_update_entry(ad_db_p, ad_entry, assoData);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            *success = SOC_SAND_FAILURE_INTERNAL_ERR;
            SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("internal error occured")));
        }
    }
    {
        ...
        if(res == KAPS_JR1_OUT_OF_DBA || res == KAPS_JR1_OUT_OF_UIT || res == KAPS_JR1_OUT_OF_UDA || res == KAPS_JR1_OUT_OF_AD){
    ...
    )
{
    ...
    res = kaps_jr1_db_get_prefix_handle(db_p, data, prefix_len, &db_entry);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
        if(!db_entry) {
    ...
    if (SOC_IS_JERICHO_PLUS(unit) && (hitbit_enabled != 0)) {
        ...
        res = kaps_jr1_entry_get_hb(db_p, db_entry, &hb_entry);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
    ...
    res = kaps_jr1_ad_db_delete_entry(
            ad_db_p, 
            ad_entry
          );
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
       LOG_ERROR(BSL_LS_SOC_TCAM,
                 (BSL_META_U(unit,
                             "Error in %s(): kaps_jr1_ad_db_delete_entry failed with error: %s!\n"), 
                             FUNCTION_NAME(),
                  kaps_jr1_get_status_string(res)));
    ...
    if (SOC_IS_JERICHO_PLUS(unit) && (hitbit_enabled != 0)) {
        ...
        res = kaps_jr1_hb_db_delete_entry(
                hb_db_p, 
                hb_entry
              );
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
      ...
      )
{
    ...
    res = kaps_jr1_hb_entry_get_bit_value(hb_db_p, hb_entry, &bit_value, clear);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
    ...
    )
{
    ...
    *found = FALSE;
    ...
    res = kaps_jr1_ad_db_get(ad_db_p, ad_entry, assoData);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
    ...
    )
{
    ...
    do {
        ++entry_index;
        res = kaps_jr1_db_entry_iter_next(tbl_p, iter, &kaps_e);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
    ...
    do {
        ...
        res = kaps_jr1_entry_get_info(tbl_p, kaps_e, &kaps_e_info);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 30, exit);
        }
        ...
        {
            res = kaps_jr1_ad_db_get(ad_db_p, kaps_e_info.ad_handle, ad_bytes_array);
            if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
...
exit:
    ...
    res = kaps_jr1_db_entry_iter_destroy(tbl_p, iter);
    ...
    )
{
    ...
    res = kaps_jr1_db_entry_iter_init(db_p, &iter_p);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 80, exit);
    }
    SOC_SAND_CHECK_NULL_PTR(iter_p, 90, exit);

    res = kaps_jr1_db_entry_iter_next(db_p, iter_p, &db_entry);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 80, exit);
    }

    while (db_entry != NULL) {
        res = kaps_jr1_entry_get_info(db_p, db_entry, &kaps_e_info);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 90, exit);
        }
        ...
        if (kaps_e_info.data[0] >> (SOC_SAND_NOF_BITS_IN_BYTE - prefix_len) == prefix) {
            ...
            res = kaps_jr1_db_entry_iter_destroy(db_p, iter_p);
            if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
                SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 80, exit);
            }
    ...
    )
{
    ...
    for (offset = 0; offset < JER_KAPS_DMA_DB_NOF_PMF_ENTRIES; offset++) {
        ...
        res = kaps_jr1_dma_db_add_entry(db_dma_p,
                                   entry_data,
                                   offset);
    ...
    )
{
    ...
    res = kaps_jr1_db_delete_all_entries(db_p);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 80, exit);
    }

    res = kaps_jr1_ad_db_delete_unused_entries(ad_db_p);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 90, exit);
    }
                   ...
                   SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE   *success)
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    res = kaps_jr1_dma_db_read_entry(db_p, 
                               offset, 
                               entry_data);
    ...
    res = kaps_jr1_dma_db_add_entry(db_p, 
                               entry_data, 
                               offset);
                   ...
                   SOC_SAND_OUT SOC_SAND_SUCCESS_FAILURE    *success)
{
    ...
    *success = SOC_SAND_SUCCESS;
    ...
    res = kaps_jr1_dma_db_delete_entry(db_p,  
                               offset);
