                            ...
                            JER_KAPS_SEARCH_RESULT *return_cmp_rslt)
{
    ...
    for(i = 0; i<search_cfg.valid_tables_num; i++) {
        ...
        if ((key_offset != i) && (return_cmp_rslt != NULL)) {
            sal_memcpy(return_cmp_rslt->assoc_data[i], cmp_rslt_p->assoc_data[key_offset], sizeof(uint8) * KAPS_JR1_INSTRUCTION_MAX_AD_BYTES);
...
int jer_kaps_sw_search_test(int unit, uint32 tbl_id, uint8 *key, uint8 *payload, int32 *prefix_len, int32 *is_matched)
{
    ...
    if (entry_p) {
        *is_matched = TRUE;
        res = kaps_jr1_entry_get_ad(table_cfg_p.tbl_p, entry_p, &ad_entry_p);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            LOG_ERROR(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
                        "Error in %s(): kaps_entry_get_ad failed: %s\n"),
                        FUNCTION_NAME(), kaps_jr1_get_status_string(res)));
            SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 20, exit);
        }
        res = kaps_jr1_ad_db_get(ad_db_p, ad_entry_p, payload);
        if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
            LOG_ERROR(BSL_LS_SOC_TCAM,
                (BSL_META_U(unit,
...
int jer_kaps_hw_search_test(int unit, uint32 search_id, uint8 *master_key, JER_KAPS_SEARCH_RESULT *cmp_rslt)
{
    ...
    jer_kaps_search_config_get(unit, search_id, &search_cfg);
    res = kaps_jr1_instruction_search(search_cfg.inst_p, master_key, 0, cmp_rslt);
    if(JER_KAPS_TO_SOC_RESULT(res) != SOC_SAND_OK) {
       LOG_ERROR(BSL_LS_SOC_TCAM,
                 (BSL_META_U(unit,
...
int jer_kaps_show_table(int unit, uint32 dbal_table_id, uint32 print_entries) {
    ...
    for (j=0; j<2; j++) {
        ...
        if ((table_cfg_p.clone_of_tbl_id == JER_KAPS_IP_NOF_TABLES)
            ...
            KAPS_JR1_TRY(kaps_jr1_db_entry_iter_init(tbl_p, &iter));
            num_of_entries = 0;
            
            do {
                KAPS_JR1_TRY(kaps_jr1_db_entry_iter_next(tbl_p, iter, &kaps_e));

                if (kaps_e == NULL)
                    break;

                KAPS_JR1_TRY(kaps_jr1_entry_get_info(tbl_p, kaps_e, &kaps_e_info));
            ...
            KAPS_JR1_TRY(kaps_jr1_db_entry_iter_destroy(tbl_p, iter));
            ...
            if (print_entries) {
                ...
                do {
                    ...
                    if ((ad_db_p != NULL) && (kaps_e_info.ad_handle != NULL)) {
                            KAPS_JR1_TRY(kaps_jr1_ad_db_get(ad_db_p, kaps_e_info.ad_handle, ad_8));
                    } else {
...
int jer_kaps_show_db_stats( int unit, uint32 dbal_table_id ) {
    ...
    for ( j = 0; j < 2 ; j++ ) {
        ...
        if ( ( tbl_p != NULL ) && ( dbal_table.is_table_initiated ) ) {
            ...
            res = kaps_jr1_db_stats( tbl_p, &stats );
            if ( JER_KAPS_TO_SOC_RESULT( res ) != SOC_SAND_OK ) {
...
int jer_pp_kaps_tcam_bist(int unit)
{
    ...
    if ((jer_kaps_app_data_get(unit) == NULL) && !SOC_IS_JERICHO_PLUS(unit)) {
        res = jer_pp_xpt_extended_command(&xpt_p, 0, KAPS_JR1_CMD_EXTENDED, KAPS_JR1_FUNC15, 0);
        ...
        res = jer_pp_xpt_extended_command(&xpt_p, 0, KAPS_JR1_CMD_EXTENDED, KAPS_JR1_FUNC14, 0);
...
int jer_pp_kbp_sdk_ver_test(int unit)
{
    ...
    int nof_supported_versions = 2;
    const char* kbp_sdk_ver_arr[1][2] = {
        { "KBP SDK 1.5.18_EA1", "KBP SDK 1.5.18" }
    };
    ...
    if (ver_str != NULL)
    {
        ...
        int ver_str_start_offset = sal_strnlen("KBP SDK ", 128);
        int ver_str_len = sal_strnlen(ver_str, 128);
        ...
        for (ver_iter = 0; ver_iter < nof_supported_versions; ver_iter++)
        {
            ...
            int longest_length =
                sal_strnlen(ver_str, version_end) > sal_strnlen(kbp_sdk_ver_arr[array_index][ver_iter], ver_str_len) ?
                sal_strnlen(ver_str, version_end) : sal_strnlen(kbp_sdk_ver_arr[array_index][ver_iter], ver_str_len);
            if (sal_strncmp(ver_str, kbp_sdk_ver_arr[array_index][ver_iter], longest_length) == 0)
            {
        ...
        {
            char ver_str_all[128] = { 0 };
            for (ver_iter = 0; ver_iter < nof_supported_versions; ver_iter++)
            {
                sal_strncat_s(ver_str_all, " \"", 3);
                sal_strncat_s(ver_str_all, kbp_sdk_ver_arr[array_index][ver_iter], sal_strnlen(kbp_sdk_ver_arr[array_index][ver_iter], 128));
    ...
    {
        LOG_ERROR(BSL_LS_SOC_TCAM, (BSL_META_U(unit, "Error getting KBPSDK version\n")));
        SOC_SAND_SET_ERROR_CODE(SOC_SAND_GEN_ERR, 20, exit);
    }
