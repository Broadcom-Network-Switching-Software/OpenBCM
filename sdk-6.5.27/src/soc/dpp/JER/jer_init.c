    ...
    if (!SOC_WARM_BOOT(unit)) {
       ...
       LOG_INFO(BSL_LS_SOC_INIT, (BSL_META_U(unit, "\t+ %d: Configure tables defaults\n"),unit));
        ...
        if (SOC_IS_QAX(unit)) {
            ...
            if (SOC_DPP_CONFIG(unit)->qax->link_bonding_enable) {
                SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_enhancement_support_get,(unit, &is_lb_enhance_support)));
                if (SOC_DPP_CONFIG(unit)->qax->lbi_en && (is_lb_enhance_support == 0)) {
                    SOCDNX_EXIT_WITH_ERR(SOC_E_CONFIG, (_BSL_SOCDNX_MSG("lbi_en SOC property must be set to 0 on this device")));
                }
