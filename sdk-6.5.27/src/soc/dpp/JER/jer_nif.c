...
extern int _default_port_enable;
...
soc_jer_portmod_pmh_init(int unit)
{
    ...
    for(pm=0 ; pm<nof_pm4x25 ; pm++) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_physim_check_sim(unit, phymodDispatchTypeTscf, &(pm_info.pm_specific_info.pm4x25.access), ident_bits, 1, &is_sim));
...
soc_jer_portmod_pml_init(int unit)
{
    ...
    for(pm=0 ; pm<nof_pm4x10 ; pm++) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_physim_check_sim(unit, phymodDispatchTypeTsce, &(pm_info.pm_specific_info.pm4x10.access), ident_bits, 1, &is_sim));
        if(is_sim) {
...
soc_jer_portmod_pmx_init(int unit)
{
    ...
    for(pm=0 ; pm<nof_pm4x2p5 ; pm++) {
        ...
        SOCDNX_IF_ERR_EXIT(soc_physim_check_sim(unit, phymodDispatchTypeViper, &(pm_info.pm_specific_info.pm4x2p5.access), ident_bits, 1, &is_sim));
...
_soc_jer_portmod_config_info_init(int unit, soc_port_t port, portmod_port_add_info_t *add_info, int is_init_sequence)
{
    ...
    preemphasis = soc_property_port_get(unit, port,
                                        spn_SERDES_PREEMPHASIS, 0xffffffff);
    if (preemphasis != 0xffffffff)
    {
        SOCDNX_IF_ERR_EXIT(soc_port_sw_db_num_lanes_get(unit, port, &num_lanes));
        for (i = 0; i < num_lanes; i++) {
            add_info->init_config.tx_params_user_flag[i] |=
                PORTMOD_USER_SET_TX_PREEMPHASIS_BY_CONFIG;
            add_info->init_config.tx_params[i].pre  = preemphasis & SOC_DCMN_PORT_SERDES_PRE_CURRENT_MASK;
            add_info->init_config.tx_params[i].main = (preemphasis & SOC_DCMN_PORT_SERDES_MAIN_CURRENT_MASK) >> SOC_DCMN_PORT_SERDES_MAIN_CURRENT_FIRST;
            add_info->init_config.tx_params[i].post = (preemphasis & SOC_DCMN_PORT_SERDES_POST_CURRENT_MASK) >> SOC_DCMN_PORT_SERDES_POST_CURRENT_FIRST;
        }
    }
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        SOC_PBMP_ITER(pbmp, port) {
            ...
            if ((add_info.interface_config.interface != SOC_PORT_IF_ILKN) &&
                ...
                if (-1 == init_speed ||_default_port_enable == 0) {
...
int
soc_jer_nif_sif_set(int unit, int core, uint32 first_phy, int enable)
{
    ...
    if (first_phy == JER_NIF_PHY_SIF_PORT_0_NBIH_PM5 || first_phy == JER_NIF_PHY_SIF_PORT_1_NBIH_PM5 || first_phy == JER_NIF_PHY_SIF_PORT_1_NBIH_PM4)
    {
        ...
        if (enable && (core != port_num)) {
            
            LOG_WARN(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
                                "u=%d soc_jer_nif_sif_set: core ID (%d) does not match sif port (%d). core ID ignored.\n"),unit, core, port_num));
        }

        SOCDNX_IF_ERR_EXIT(READ_NBIH_SIF_CFGr(unit, port_num, &reg_val));
        soc_reg_field_set(unit, NBIH_SIF_CFGr, &reg_val, SIF_PORT_N_ENf, enable);
        if(SOC_IS_JERICHO_PLUS_ONLY(unit))
        {
        ...
        soc_reg_field_set(unit, NBIH_ADDITIONAL_RESETSr, &reg_val, SIF_RSTNf, enable);
    ...
    else
    {
        ...
        soc_reg_field_set(unit, NBIL_SIF_CFGr, &reg_val, SIF_PORT_N_ENf, enable);
        ...
        soc_reg_field_set(unit, NBIL_ADDITIONAL_RESETSr, &reg_val, SIF_RSTNf, enable);
...
soc_jer_port_open_path(int unit, soc_port_t port) {
    ...
    if (SOC_PORT_IF_ILKN == interface_type) {
...
#endif
        {
            ...
            if (SOC_PORT_IS_STAT_INTERFACE(flags)) {
                SOCDNX_IF_ERR_EXIT(MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_nif_sif_set, (unit, core, first_phy, 1)));
            }
...
soc_jer_port_close_path(int unit, soc_port_t port) {
...
#endif
    {
        ...
        if (SOC_PORT_IS_STAT_INTERFACE(flags)) {
            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_core_get(unit, port, &core));
            SOCDNX_IF_ERR_EXIT(MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_nif_sif_set, (unit, core, first_phy, 0)));
        }
...
soc_jer_phy_nif_measure(int unit, soc_port_t port, uint32 *type_of_bit_clk, int *one_clk_time_measured_int, int *one_clk_time_measured_remainder, int *serdes_freq_int, int *serdes_freq_remainder, uint32 *lane)
{
    ...
    uint64 reg_val_64, default_configuration = 0;
    int synce_read = 0, pmh_synce_read = 0, gtime_read = 0;
