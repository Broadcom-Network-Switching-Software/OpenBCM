...
#define  SOC_JER_NOF_LB_CRTL_IF_CHANNELS_MAX 16
   ...
   )
{
    ...
    uint32 is_skip_nif_for_lb = 0, is_skip_egq_for_lb = 0, is_lb_ctrl_port = 0, lb_ctrl_channel = 0;
    ...
    if(interface != SOC_PORT_IF_ERP) {
        ...
        if ((is_skip_egq_for_lb != 1)) {
...
#ifdef BCM_LB_SUPPORT
            if (SOC_IS_QAX(unit) && SOC_DPP_CONFIG(unit)->qax->lbi_en && (SOC_QAX_EGR_IF_LBG_RESERVE == egr_if)) {
                ifc_val = 0;
            }
#endif
            ...
            for (q_pair_num = 0; q_pair_num < nof_q_pairs; q_pair_num++)
            {
                ...
                else if (is_lb_ctrl_port) {
                    lb_ctrl_channel = offset%SOC_JER_NOF_LB_CRTL_IF_CHANNELS_MAX;
                    soc_mem_field_set(unit, EGQ_PCTm, data, PORT_CH_NUMf, &lb_ctrl_channel);
                } else {
   ...
   )
{
    ...
    soc_pbmp_t pbmp;
    soc_port_t port_i;
    uint32 is_skip_nif_for_lb = 0, is_skip_egq_intf_for_lb = 0, is_skip_egq_ps_for_lb = 0;
    uint32 flags_i = 0;
#ifdef BCM_LB_SUPPORT
    ...
    if(interface != SOC_PORT_IF_ERP) {
        ...
        if (SOC_IS_QAX(unit) && SOC_DPP_CONFIG(unit)->qax->link_bonding_enable) 
        {
            if (SOC_DPP_CONFIG(unit)->qax->lbi_en && (SOC_QAX_EGR_IF_LBG_RESERVE == egr_if)) {
                ...
                if (lb_port_num != 0) {
                    is_skip_egq_intf_for_lb = 1;
                }
            }

            if (SOC_PORT_IS_LB_MODEM(flags)) {
                is_skip_egq_intf_for_lb = 1;
                is_skip_egq_ps_for_lb = 1;
            }
        }
#endif
        ...
        if (is_last_port && (is_skip_egq_intf_for_lb != 1)) {
            SOCDNX_IF_ERR_EXIT(READ_EGQ_PER_IFC_CFGm(unit, EGQ_BLOCK(unit, core), egr_if, data));
            soc_mem_field_set(unit, EGQ_PER_IFC_CFGm, data, FQ_RDY_THf, &fq_rdy_sel_val);
            soc_mem_field_set(unit, EGQ_PER_IFC_CFGm, data, NIF_CANCEL_EN_1f, &cancel_en1);
            soc_mem_field_set(unit, EGQ_PER_IFC_CFGm, data, NIF_CANCEL_EN_2f, &cancel_en2);
        ...
        if(egr_if < SOC_DPP_IMP_DEFS_GET(unit, nof_channelized_interfaces)) {
            if (is_skip_egq_ps_for_lb != 1) {
                
                ps = ARAD_BASE_PORT_TC2PS(base_q_pair);
#ifdef BCM_LB_SUPPORT
                if (SOC_IS_QAX(unit) && SOC_DPP_CONFIG(unit)->qax->lbi_en && (egr_if == SOC_QAX_EGR_IF_LBG_RESERVE)) {
                    
                    SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_lb_ports_on_reserve_intf_get, (unit, &pbmp, &nof_lb_ctrl_port)));
                }
                else
#endif
                {
  ...
  )
{
    ...
    int quad_port = 0;
    uint32 quad_port_pp = 0;
    int quad_port_core = 0;
    ARAD_PORT_HEADER_TYPE quad_port_header_type_in;
    ARAD_PORT_HEADER_TYPE quad_port_header_type_out;
    uint8 tdm_port_exists_on_quad = FALSE;
            ...
            if (interface_type == SOC_PORT_IF_ILKN) {
                ...
                if (!soc_feature(unit, soc_feature_no_tdm)) {
                    if ((direction_ndx == SOC_TMC_PORT_DIRECTION_INCOMING) || (direction_ndx == SOC_TMC_PORT_DIRECTION_BOTH)) {
                        ...
                        SOCDNX_IF_ERR_EXIT(soc_jer_port_ports_to_same_quad_get(unit, logical_port, &quad_ports));
                        SOC_PBMP_ITER(quad_ports, quad_port) {
                            
                            SOCDNX_IF_ERR_EXIT(soc_port_sw_db_local_to_pp_port_get(unit, quad_port, &quad_port_pp, &quad_port_core));
                            SOCDNX_IF_ERR_EXIT(handle_sand_result(arad_port_header_type_get(unit, quad_port_core, quad_port_pp, &quad_port_header_type_in, &quad_port_header_type_out)));
                            if (quad_port_header_type_in == ARAD_PORT_HEADER_TYPE_TDM) {
                                tdm_port_exists_on_quad = TRUE;
                                break;
                            }
                        }
                        ...
                        if (core_id) {
                            ...
                            if ((header_type == SOC_TMC_PORT_HEADER_TYPE_TDM) || (tdm_port_exists_on_quad == TRUE)) {
    ...
    SOC_SAND_IN  int        enable)
{
    ...
    SOC_REG_ABOVE_64_CLEAR(data_above_64);
    SOCDNX_IF_ERR_EXIT(READ_EPNI_CFG_LATENCY_PER_PORT_ENABLEr(unit, data_above_64));

    
    if (enable) {
        SHR_BITSET(data_above_64, port); 
    } else {
        SHR_BITCLR(data_above_64, port);
    }

    SOCDNX_IF_ERR_EXIT(WRITE_EPNI_CFG_LATENCY_PER_PORT_ENABLEr(unit, data_above_64));
    ...
    SOC_SAND_OUT int        *enable)
{
    ...
    *enable = (SHR_BITGET(data_above_64, port));
