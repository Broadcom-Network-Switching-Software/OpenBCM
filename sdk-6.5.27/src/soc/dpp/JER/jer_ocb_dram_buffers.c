...
int soc_qax_dram_buffer_conf_set(int unit) 
{
    ...
    uint32 max_bdbs = (SOC_DPP_DEFS_GET(unit, nof_dram_bds) / 8) / 1024;
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_field32_modify(unit, DQM_BDBLL_SIZEr, REG_PORT_ANY, BDBLL_SIZEf, (max_bdbs / max_nof_dram_interfaces) * nof_drams));
...
int soc_jer_ocb_dram_buffer_conf_set(int unit) 
{
    ...
    switch(arad_init->dram.fmc_dbuff_mode)
    {
    ...
    SOCDNX_IF_ERR_EXIT(soc_reg_above_64_field32_modify(unit, IQM_IQM_ENABLERSr, REG_PORT_ANY, 0, FLMC_4K_REP_ENf, field_val));
