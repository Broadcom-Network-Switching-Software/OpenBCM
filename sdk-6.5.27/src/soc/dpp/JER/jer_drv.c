...
soc_jer_ports_config(int unit) 
{
    ...
    SOC_DPP_CONFIG(unit)->pp.pon_port_ptc_size = soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "pon_port_ptc_size", 4);
    if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "gpon_application_enable", 0) && (SOC_DPP_CONFIG(unit)->pp.pon_port_ptc_size > 6)) {
        SOCDNX_EXIT_WITH_ERR(SOC_E_FAIL, (_BSL_SOCDNX_MSG("pon_port_ptc_size must not larger than 6.")));
    }
...
#ifdef BCM_LB_SUPPORT
            if (SOC_IS_QAX(unit) &&
                SOC_DPP_CONFIG(unit)->qax->link_bonding_enable &&
                SOC_DPP_CONFIG(unit)->qax->lbi_en)
            {
                SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_lb_is_control_port_get, (unit, port_i, &is_lb_ctrl_port, 1)));
            }
...
int soc_jer_init_dma_init(int unit)
{
    ...
    pktdma_init = !(SOC_WARM_BOOT(unit) &&
                    soc_property_get(unit, spn_WARMBOOT_KNET_SHUTDOWN_MODE, 0));
#endif 

    
    SOCDNX_IF_ERR_EXIT(soc_dma_attach(unit, pktdma_init));
    ...
    int reset_action)
{
    ...
    SOCDNX_IF_ERR_EXIT(soc_dcmn_iproc_config_paxb(unit , 1));
    ...
    SOCDNX_IF_ERR_EXIT(soc_dcmn_iproc_config_paxb(unit, 0));
