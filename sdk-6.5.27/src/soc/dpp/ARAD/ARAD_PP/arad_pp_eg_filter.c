  ...
  )
{
  ...
  if (src_sys_port_ndx->sys_port_type == SOC_SAND_PP_SYS_PORT_TYPE_LAG)
  {
    ARAD_ALLOC(in_info, SOC_PPC_LAG_INFO, 1, "arad_pp_eg_filter_pvlan_port_type_get_unsafe.in_info");
    ARAD_ALLOC(out_info, SOC_PPC_LAG_INFO, 1, "arad_pp_eg_filter_pvlan_port_type_get_unsafe.out_info");
    res = arad_ports_lag_get_unsafe(
        unit,
        src_sys_port_ndx->sys_id,
        in_info, 
