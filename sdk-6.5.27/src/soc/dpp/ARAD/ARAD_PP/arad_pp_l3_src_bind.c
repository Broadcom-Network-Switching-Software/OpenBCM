...
STATIC uint32
    arad_pp_src_bind_lem_payload_parse(
      SOC_SAND_IN  int                                     unit,
      SOC_SAND_OUT  SOC_PPC_FRWRD_DECISION_INFO            *frwrd_decision,
...
exit:
    SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_src_bind_lem_payload_parse()", 0, 0);
}
    ...
    )
{
    ...
    {
        SOC_PPC_FRWRD_DECISION_INFO_clear(&frwrd_info);
        arad_pp_src_bind_lem_payload_parse(unit, &frwrd_info, &payload);
        if (*found && (ipv4_info->lif_ndx != frwrd_info.dest_id))
        {
            *found = 0;
        }
    }
    ...
    )
{
    ...
    if (SOC_DPP_L3_SRC_BIND_IPV4_SUBNET_OR_ARP_ENABLE(unit))
    {
        if (*found && (src_bind_info->lif_ndx != JER_PP_KAPS_FEC_DECODE(payload)))
        {
            *found = 0;
        }
    }
    ...
    if (src_bind_info->smac_valid == 0)
    {
        ...
        else
        {
            if (src_bind_info->is_network)
            {
                ...
                if (SOC_DPP_L3_SRC_BIND_IPV4_SUBNET_OR_ARP_ENABLE(unit))
                {
                    if (*found && (src_bind_info->lif_ndx != fec_id))
                    {
                        *found = 0;
                    }
                }
    ...
    )
{
    ...
    qual_vals[1].is_valid.arr[0]= 0x3ffff;
    res = arad_pp_dbal_entry_add(unit, SOC_DPP_DBAL_SW_TABLE_ID_PPPOE_SESSION_ID_ANTI_SPOOFING,
                                 qual_vals, 0, &payload, success);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    )
{
    ...
    qual_vals[1].val.arr[0]= pppoe_entry->lif_ndx;
    qual_vals[1].is_valid.arr[0]= 0x3ffff;
    res = arad_pp_dbal_entry_delete(unit, SOC_DPP_DBAL_SW_TABLE_ID_PPPOE_SESSION_ID_ANTI_SPOOFING, qual_vals, success);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    )
{
    ...
    qual_vals[1].val.arr[0]= pppoe_entry->lif_ndx;
    qual_vals[1].is_valid.arr[0]= 0x3ffff;
    res = arad_pp_dbal_entry_get(unit, SOC_DPP_DBAL_SW_TABLE_ID_PPPOE_SESSION_ID_ANTI_SPOOFING,
