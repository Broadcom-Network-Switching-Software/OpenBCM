  ...
  )
{
  ...
  if ((classifier_mep_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) && update != 1) {
      ...
      if (SOC_SAND_CHECK_MAC_IS_UNICAST(dst_mac_address_ptr)) {
           res = _bcm_dpp_gport_to_phy_port(unit, classifier_mep_entry->gport, _BCM_DPP_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info);
           SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

           BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
                res = MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_local_to_pp_port_get, (unit, port_i, &pp_port, &core));
                SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

                res = arad_pp_oam_my_cfm_mac_set_unsafe(unit,core, dst_mac_address_ptr, pp_port);
                SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
           }
      }
  }
  ...
  )
{
  ...
  if ((mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) ||
      ...
      if ((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "maid48_enabled", 0)) && (mep_db_entry->is_maid_48 || mep_db_entry->is_maid_20)) {
    ...
    if(SOC_IS_QAX(unit)) {
      if (mep_db_entry->is_maid_48 || mep_db_entry->is_maid_20) {
          if(is_new_ep) {
          
          soc_maid_48_qax_mep_pe_profile(unit, mep_db_entry->is_upmep,mep_db_entry->mep_type,mep_db_entry->out_lif, mep_db_entry->is_maid_48, &reg_val);
          soc_mem_field_set(unit, mem, entry_above_64, MEP_PE_PROFILEf, &reg_val);
  ...
  )
{
 ...
 LOG_DEBUG(BSL_LS_SOC_OAM,
  ...
  if ((mep_db_entry->mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) ||
      ...
      if (!_ARAD_PP_OAM_IS_SHORT_FORMAT(mep_db_entry->icc_ndx) && (mep_db_entry->is_maid_48 == 0 && mep_db_entry->is_maid_20 == 0)) {
          if (allocate_icc_ndx) {
  ...
  )
{
    ...
    uint8 is_1dm = 0;
    uint32 len_of_data_in_last_event = 0;
    uint32 len_of_data_in_this_event = 0;
    ...
    LOG_DEBUG(BSL_LS_SOC_OAM,
              (BSL_META_U(unit,
                          "Interrupt message: last_interrupt_message_num(%d) additional_words_to_read(%d) last_rmeb_db_ndx(%d) last_report_mode(%d) last_event_type(%d)\n" \
        ...
        interrupt_data->additional_words_to_read,
        interrupt_data->last_rmeb_db_ndx,
        interrupt_data->last_report_mode,
        interrupt_data->last_event_type,
        interrupt_data->interrupt_message[19],interrupt_data->interrupt_message[18],interrupt_data->interrupt_message[17],interrupt_data->interrupt_message[16],
        interrupt_data->interrupt_message[15],interrupt_data->interrupt_message[14],interrupt_data->interrupt_message[13],interrupt_data->interrupt_message[12],
        interrupt_data->interrupt_message[11],interrupt_data->interrupt_message[10],interrupt_data->interrupt_message[9],interrupt_data->interrupt_message[8],
        interrupt_data->interrupt_message[7],interrupt_data->interrupt_message[6],interrupt_data->interrupt_message[5],interrupt_data->interrupt_message[4],
    ...
    if (interrupt_data->interrupt_message[interrupt_data->last_interrupt_message_num] == empty_fifo_value ||
        ...
        *valid = 0;
        ...
        if (interrupt_data->additional_words_to_read > 0) {
            LOG_WARN(BSL_LS_SOC_OAM,
                     (BSL_META_U(unit,
                                 "Warning: Part of last event lost: last_rmeb_db_ndx(%d) last_report_mode(%d) last_event_type(%d) \n"),
                                  interrupt_data->last_rmeb_db_ndx, interrupt_data->last_report_mode, interrupt_data->last_event_type));
        }
        interrupt_data->additional_words_to_read = 0;  
    } else {
        *valid = 1;
        *rmeb_db_ndx_lcl = 0;
        ...
        *rmeb_db_ndx = *rmeb_db_ndx_lcl;
        ...
        if (interrupt_data->additional_words_to_read > 0) {
            ...
            *rmeb_db_ndx = interrupt_data->last_rmeb_db_ndx;
            report_mode = interrupt_data->last_report_mode;
        }
        ...
        if (!report_mode) {
            ...
            if (report_mode == 2) {
                ...
                if (interrupt_data->additional_words_to_read > 0) {   
                    rx_report_data.event_type = interrupt_data->last_event_type;
                    len_of_data_in_last_event = interrupt_data->last_event_size - interrupt_data->additional_words_to_read - 1;
                    len_of_data_in_this_event = interrupt_data->additional_words_to_read;
                    SHR_BITCOPY_RANGE(event_buffer, 0, interrupt_data->last_event_data, 0, 32*len_of_data_in_last_event);
                    SHR_BITCOPY_RANGE(event_buffer, len_of_data_in_last_event*32, interrupt_data->interrupt_message, 0, 32*len_of_data_in_this_event);

                    interrupt_data->last_interrupt_message_num += interrupt_data->additional_words_to_read;
                    interrupt_data->additional_words_to_read = 0;
                } else {
                    field_32 = 0;
                    ...
                    interrupt_data->additional_words_to_read = rx_report_data.event_size;

                    field_32 = 0;
                    SHR_BITCOPY_RANGE(&field_32, 0, interrupt_data->interrupt_message, start_of_message+20, 1);
                    rx_report_data.event_type = (uint8)field_32;

                    if (interrupt_data->last_interrupt_message_num + rx_report_data.event_size > _ARAD_PP_OAM_NOF_INTERRUPT_MESSAGES_IN_FIFO) {
                        
                        interrupt_data->additional_words_to_read = interrupt_data->last_interrupt_message_num + rx_report_data.event_size - _ARAD_PP_OAM_NOF_INTERRUPT_MESSAGES_IN_FIFO;
                        len_of_data_in_this_event = rx_report_data.event_size - 1 - interrupt_data->additional_words_to_read;

                        SHR_BITCOPY_RANGE(interrupt_data->last_event_data, 0, interrupt_data->interrupt_message, start_of_message + 32, 32 * len_of_data_in_this_event);
                        interrupt_data->last_interrupt_message_num += (len_of_data_in_this_event + 1);
                        interrupt_data->last_event_size = rx_report_data.event_size;
                        interrupt_data->last_event_type = rx_report_data.event_type;
                        interrupt_data->last_report_mode = report_mode;
                        interrupt_data->last_rmeb_db_ndx = *rmeb_db_ndx;

                        SOC_SAND_EXIT_NO_ERROR;
                    } else {
                        interrupt_data->additional_words_to_read = 0;
                        len_of_data_in_this_event = rx_report_data.event_size - 1;

                        
                        SHR_BITCOPY_RANGE(event_buffer, 0, interrupt_data->interrupt_message, start_of_message + 32, 32 * len_of_data_in_this_event);
                ...
                if (rx_report_data.event_type == 1 ) {
                    if (SOC_IS_QAX(unit)) {
                        ...
                        dm_info.entry_id = (*rmeb_db_ndx & ((1<<14)-1)); 
        ...
        if ((report_mode != 3) && (report_mode != 2)) {
            interrupt_data->last_interrupt_message_num++;
        }
        LOG_DEBUG(BSL_LS_SOC_OAM,
                  (BSL_META_U(unit,
