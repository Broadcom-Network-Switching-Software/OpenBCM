...
uint32
arad_pp_flp_learn_key_const_ipv4compmc_with_mim_learning(
     int unit,
     int prog_id
   )
{
  uint32 res, fid_ce_inst = 0;
  ...
  flp_key_construction_tbl.instruction_0_16b        = fid_ce_inst;
  flp_key_construction_tbl.instruction_1_16b        = ARAD_PP_CE_SA_HDR2_16_MSB;
  flp_key_construction_tbl.instruction_3_32b        = ARAD_PP_CE_SA_HDR2_32_LSB;
  flp_key_construction_tbl.key_c_inst_0_to_5_valid  = 0xb;    
  ...
  flp_lookups_tbl.learn_key_select = 2;  

  res = arad_pp_ihb_flp_lookups_tbl_set_unsafe(unit, prog_id, &flp_lookups_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);

exit:
 SOC_SAND_EXIT_AND_SEND_ERROR("error in arad_pp_flp_learn_key_const_ipv4compmc_with_mim_learning", 0, 0);
}
...
{
  uint32
    res, pp_port_ce_inst = 0, vrf_ce_inst = 0, rif_ce_inst = 0, fid_ce_inst = 0;
  ARAD_PP_IHB_FLP_KEY_CONSTRUCTION_TBL_DATA
    flp_key_construction_tbl;
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  ...
  if (SOC_IS_JERICHO(unit)) {
     ...
     flp_key_construction_tbl.instruction_1_16b        = ARAD_PP_CE_SA_HDR2_16_MSB;
     flp_key_construction_tbl.instruction_3_32b        = ARAD_PP_CE_SA_HDR2_32_LSB;
     flp_key_construction_tbl.key_d_inst_0_to_7_valid = 0xb;    
  }
  res = arad_pp_ihb_flp_key_construction_tbl_set_unsafe(unit, prog_id+ARAD_PP_FLP_INSTRUCTIONS_NOF, &flp_key_construction_tbl);
  SOC_SAND_CHECK_FUNC_RESULT(res, 43, exit);
exit:
   ...
   )
{
  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 42, exit);
  {
      ...
      flp_lookups_tbl.learn_key_select      = SOC_IS_JERICHO(unit)? 3 : ARAD_PP_FLP_LKP_KEY_SELECT_FID_FWD_MAC_KEY_HW_VAL;
      ...
      if (SOC_DPP_CONFIG(unit)->l3.ipmc_vpn_lookup_enable && soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "mim_spb_enable", 0)){
          ...
          if(JER_KAPS_ENABLE(unit)
            ...
            || (ARAD_KBP_ENABLE_IPV4_MC && SOC_IS_JERICHO(unit))
              ...
              res = arad_pp_flp_learn_key_const_ipv4compmc_with_mim_learning(unit, prog_id);
              SOC_SAND_CHECK_FUNC_RESULT(res, 489, exit);
