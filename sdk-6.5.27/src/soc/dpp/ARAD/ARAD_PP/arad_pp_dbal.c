    ...
    *found = FALSE;
    ...
    for (i=0; i<2; i++) {
        ...
        if ((kaps_iterator != NULL) && (*found == FALSE)) {
            ...
            while (TRUE) { 
                KAPS_JR1_TRY(kaps_jr1_db_entry_iter_next(table_p, kaps_iterator, &kaps_e));

                if (kaps_e == NULL) {
                    KAPS_JR1_TRY(kaps_jr1_db_entry_iter_destroy(table_p, kaps_iterator));
                    if (i==0) {
                    ...
                    KAPS_JR1_TRY(kaps_jr1_entry_get_info(table_p, kaps_e, &kaps_e_info));
                    ...
                    if ((ad_db_p != NULL) && (kaps_e_info.ad_handle != NULL)) {
                            KAPS_JR1_TRY(kaps_jr1_ad_db_get(ad_db_p, kaps_e_info.ad_handle, ad_bytes));
                            SOCDNX_SAND_IF_ERR_EXIT(jer_pp_kaps_route_to_kaps_payload_buffer_decode(unit, ad_bytes, payload));
    ...
    arad_pp_dbal_iterator_init_kaps(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ...
    if (JER_KAPS_ENABLE_PUBLIC_DB(unit)) {
        KAPS_JR1_TRY(kaps_jr1_db_entry_iter_init(kaps_table_iterator->public_table_p, &kaps_table_iterator->public_table_iterator));
    }

    if (JER_KAPS_ENABLE_PRIVATE_DB(unit)) {
        KAPS_JR1_TRY(kaps_jr1_db_entry_iter_init(kaps_table_iterator->private_table_p, &kaps_table_iterator->private_table_iterator));
    }
    ...
    arad_pp_dbal_iterator_deinit_kaps(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ...
    if (kaps_table_iterator != NULL) {
        if (kaps_table_iterator->public_table_iterator != NULL) {
            KAPS_JR1_TRY(kaps_jr1_db_entry_iter_destroy(kaps_table_iterator->public_table_p, kaps_table_iterator->public_table_iterator));
            kaps_table_iterator->public_table_iterator = NULL;
        }

        if (kaps_table_iterator->private_table_iterator != NULL) {
            KAPS_JR1_TRY(kaps_jr1_db_entry_iter_destroy(kaps_table_iterator->private_table_p, kaps_table_iterator->private_table_iterator));
            kaps_table_iterator->private_table_iterator = NULL;
        }
    }
