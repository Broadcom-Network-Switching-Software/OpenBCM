...
dpp_snoop_mirror_stamping_config_get(
   ...
   SOC_SAND_OUT  SOC_TMC_ACTION_CMD_SNOOP_MIRROR_INFO  *info
    ...
    if (SOC_IS_JERICHO(unit) && !SOC_IS_QAX(unit)) {
        res = READ_IPT_ARAD_SNP_MIR_STAMP_ENm(unit, MEM_BLOCK_ANY, entry_offset, data);
        SOCDNX_IF_ERR_EXIT(res);
        soc_mem_field_get(unit, IPT_ARAD_SNP_MIR_STAMP_ENm, data, FTMH_LB_EXT_ENf, field);
        ...
        if (field[0] == 0xFF) {
            ...
            res = READ_IPT_ARAD_SNP_MIR_STAMP_VALUESm(unit, MEM_BLOCK_ANY, entry_offset, data);
            SOCDNX_IF_ERR_EXIT(res);
            soc_mem_field_get(unit, IPT_ARAD_SNP_MIR_STAMP_VALUESm, data, FTMH_LB_EXTf, field);
            info->stamping.lb_key = field[0];
        }
    }
...
dpp_snoop_mirror_stamping_config_set(
   ...
   SOC_SAND_IN  SOC_TMC_ACTION_CMD_SNOOP_MIRROR_INFO  *info
    ...
    if (SOC_IS_JERICHO(unit) && !SOC_IS_QAX(unit)) {
        ...
        soc_mem_field32_set(unit, IPT_ARAD_SNP_MIR_STAMP_ENm, data, FTMH_LB_EXT_ENf, val[0]);
    }
    ...
    if (SOC_IS_JERICHO(unit) && !SOC_IS_QAX(unit)) {
        ...
        if (info->stamping.lb_key_ext_valid == TRUE) {
            val[0] = info->stamping.lb_key;
        } else {
            val[0] = 0x0;
        }
        soc_mem_field32_set(unit, IPT_ARAD_SNP_MIR_STAMP_VALUESm, data, FTMH_LB_EXTf, val[0]);
    }
