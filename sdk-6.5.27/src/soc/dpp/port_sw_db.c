...
#ifdef BCM_LB_SUPPORT
    SOCDNX_IF_ERR_EXIT(soc_port_sw_db_flags_get(unit, port, &port_flags));

    if (SOC_IS_QAX(unit) &&
        SOC_DPP_CONFIG(unit)->qax->link_bonding_enable &&
        SOC_DPP_CONFIG(unit)->qax->lbi_en)
    {
        SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_lb_is_control_port_get, (unit, port, &is_lb_ctrl_port, 1)));
    }
#endif
...
#ifdef BCM_LB_SUPPORT
    else if ((is_lb_ctrl_port || (SOC_PORT_IF_LBG == interface_type)) && (!SOC_PORT_IS_LB_MODEM(port_flags))) {
        SOCDNX_IF_ERR_EXIT(MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_port_speed_get,(unit, port, &speed)));
        *rate = speed;
    }
#endif
    else {
