    ...
    dnxc_data_value_set_f set_func)
{
    ...
    if (_SHR_IS_FLAG_SET(*data_flags, DNXC_DATA_F_ALREADY_VERIFIED)
        && !_SHR_IS_FLAG_SET(*data_flags, DNXC_DATA_F_INIT_ONLY))
    {
        SHR_EXIT();
    }
    ...
    else
    {
        ...
        *data_flags |= DNXC_DATA_F_ALREADY_VERIFIED;
    }
...
int
dnxc_data_mgmt_image_verify(
    int unit,
    char *image_name)
{
    char *soc_prop_image_name = dnx_drv_soc_property_get_str(unit, spn_PROGRAMMABILITY_IMAGE_NAME);
    char *soc_prop_default_image_name =
        dnx_drv_soc_property_get_str(unit, "custom_feature_programmability_standard_image_name");

    if (soc_prop_image_name == NULL)
    {
#ifdef BCM_DNX_SUPPORT
        if (SOC_IS_DNX2(unit))
        {
            return !sal_strncmp(image_name, "standard_1", DNXC_DATA_UTILS_STRING_MAX);
        }
#endif
    }

    if (soc_prop_default_image_name == NULL)
    {
#ifdef BCM_DNX_SUPPORT
        if (SOC_IS_DNX2(unit))
        {
            ...
            if (soc_prop_image_name != NULL)
            {
                return sal_strncmp("standard_1", soc_prop_image_name, DNXC_DATA_UTILS_STRING_MAX);
            }
    ...
    if ((soc_prop_image_name == NULL) || (soc_prop_default_image_name == NULL))
    {
        return 0;
    }
    else
    {
        ...
        return (!sal_strncmp(image_name, soc_prop_image_name, DNXC_DATA_UTILS_STRING_MAX)
                || sal_strncmp(soc_prop_default_image_name, soc_prop_image_name, DNXC_DATA_UTILS_STRING_MAX));
    }
}
