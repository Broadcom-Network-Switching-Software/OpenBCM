...
const uint32 unsupported_element = 0;
char unsupported_element_char = '0';
    ...
    char *data_name)
{
    ...
    for (feature_index = 0; feature_index < nof_features; feature_index++)
    {
        ...
        if (features[feature_index].name == NULL)
        {
            continue;
        }
    ...
    char *data_name)
{
    ...
    for (define_index = 0; define_index < nof_defines; define_index++)
    {
        ...
        if (defines[define_index].name == NULL)
        {
            continue;
        }
        if (!sal_strncasecmp(defines[define_index].name, data_name, DNXC_DATA_UTILS_STRING_MAX))
        {
            ...
            data_flags = IGNORE_INIT_ONLY(defines[define_index].flags);
            rv = dnxc_data_mgmt_access_verify(unit, _dnxc_data[unit].state, &(data_flags), defines[define_index].set);
            if (rv != _SHR_E_NONE)
            {
    ...
    int key2)
{
    ...
    for (table_index = 0; table_index < nof_tables; table_index++)
    {
        ...
        if (tables[table_index].name == NULL)
        {
            continue;
        }
        if (!sal_strncasecmp(tables[table_index].name, data, DNXC_DATA_UTILS_STRING_MAX))
        {
            ...
            table_data = dnxc_data_mgmt_shell_table_data_get(unit, &tables[table_index], key1, key2);
            if (table_data == NULL)
            {
    ...
    int buffer_size)
{
    ...
    for (feature_index = 0; feature_index < nof_features; feature_index++)
    {
        ...
        if (!sal_strncasecmp(features[feature_index].name, data_name, DNXC_DATA_UTILS_STRING_MAX))
        {
            ...
            sal_snprintf(buffer, (buffer_size - 1) * sizeof(char), "%d", val);
            return buffer;
        ...
        if (!sal_strncasecmp(defines[define_index].name, data_name, DNXC_DATA_UTILS_STRING_MAX))
        {
            ...
            if (rv != _SHR_E_NONE)
            {
                ...
                LOG_ERROR(BSL_LOG_MODULE,
                          (BSL_META_U(unit, "DNX DATA - define %s access denied for device %s\n"),
                           defines[define_index].name, _dnxc_data[unit].name));
                sal_snprintf(buffer, (buffer_size - 1) * sizeof(char), "%s", "null");
            }

            sal_snprintf(buffer, (buffer_size - 1) * sizeof(char), "%d", defines[define_index].data);
            return buffer;
        }
    }
    ...
    int buffer_size)
{
    ...
    for (submodule_index = 0; submodule_index < module->nof_submodules; submodule_index++)
    {
        ...
        if (!sal_strncasecmp(submodules[submodule_index].name, submodule, DNXC_DATA_UTILS_STRING_MAX))
        {
            ...
            res =
                dnxc_data_utils_generic_feature_data_str_get(unit, module_index, submodule_index, data, buffer,
                                                             buffer_size);
            if (res != NULL)
            {
                return res;
            ...
            res =
                dnxc_data_utils_generic_define_data_str_get(unit, module_index, submodule_index, data, buffer,
                                                            buffer_size);
            if (res != NULL)
            {
                return res;
            ...
            return &unsupported_element_char;
        }
    }
    ...
    char *feature_name)
{
    ...
    for (feature_index = 0; feature_index < nof_features; feature_index++)
    {
        ...
        if (!sal_strncasecmp(features[feature_index].name, feature_name, DNXC_DATA_UTILS_STRING_MAX))
        {
            ...
            data_flags = IGNORE_INIT_ONLY(features[feature_index].flags);
            rv = dnxc_data_mgmt_access_verify(unit, _dnxc_data[unit].state, &(data_flags), features[feature_index].set);
            if (rv != _SHR_E_NONE)
            {
    ...
    char *table_name)
{
    ...
    for (table_index = 0; table_index < nof_tables; table_index++)
    {
        ...
        if (!sal_strncasecmp(tables[table_index].name, table_name, DNXC_DATA_UTILS_STRING_MAX))
        {
            ...
            data_flags = IGNORE_INIT_ONLY(tables[table_index].flags);
            rv = dnxc_data_mgmt_access_verify(unit, _dnxc_data[unit].state, &(data_flags), tables[table_index].set);
            if (rv != _SHR_E_NONE)
            {
    ...
    int key2)
{
    ...
    table_data = dnxc_data_mgmt_shell_table_data_get(unit, table_info, key1, key2);
    if (table_data == NULL)
    {
