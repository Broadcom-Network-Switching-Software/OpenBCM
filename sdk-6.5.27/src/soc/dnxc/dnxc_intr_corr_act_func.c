    ...
    char *special_msg)
{
    ...
    switch (corr_act)
    {
        ...
        case DNXC_INT_CORR_ACT_RETRAIN_LINK:
            ...
            break;
        case DNXC_INT_CORR_ACT_TCAM_SCAN_ECC_1B:
            sal_snprintf(cur_corr_act_msg, DNXC_INTERRUPT_COR_ACT_MSG_SIZE, "TCAM_SCAN_ECC_1B");
            break;
        case DNXC_INT_CORR_ACT_TCAM_SCAN_ECC_2B:
            sal_snprintf(cur_corr_act_msg, DNXC_INTERRUPT_COR_ACT_MSG_SIZE, "TCAM_SCAN_ECC_2B");
            break;
        default:
    ...
    soc_port_t * p_link)
{
    ...
    if (SOC_IS_DNXF(unit))
    {
        SHR_IF_ERR_EXIT(soc_dnxf_drv_block_to_link_mapping
                        (unit, fmac_block_instance, bit_in_field, SOC_DNXF_DRV_BLOCK_TYPE_FMAC, p_link));
    ...
    soc_port_t * p_link)
{
    ...
    {
        SHR_IF_ERR_EXIT(soc_dnxf_drv_block_to_link_mapping
                        (unit, fsrd_block_instance, bit_in_field, SOC_DNXF_DRV_BLOCK_TYPE_FSRD, p_link));
    }
#endif
...
int
dnxc_interrupt_handles_corrective_action_reset_fabric_link(
    int unit,
    int block_instance,
    uint32 interrupt_id,
    char *msg)
    ...
    void *param2)
{
    ...
    switch (corr_act)
    {
        ...
        case DNXC_INT_CORR_ACT_ILKN_RX_LANE_LOCK_STATUS_CHANGE:
            rc = dnx_port_ilkn_rx_lane_lock_status_change_interrupt_handler(unit, block_instance, (uint8 *) param1);
            SHR_IF_ERR_EXIT(rc);
            break;
        ...
        case DNXC_INT_CORR_ACT_NANOSYNC_TAS_TOD_UPDATE:
            rc = dnx_tsn_tas_tod_update_interrupt_handler(unit);
            SHR_IF_ERR_EXIT(rc);
            break;
        case DNXC_INT_CORR_ACT_W40_FLR:
            rc = dnx_port_w40_flr_interrupt_handler(unit, block_instance, (int *) param1);
            SHR_IF_ERR_EXIT(rc);
            break;
#endif
        case DNXC_INT_CORR_ACT_TCAM_QUERY_FAILURE_VALID:
        ...
        case DNXC_INT_CORR_ACT_TCAM_SCAN_ECC_1B:
            ...
            if (access_new_interrupt_available(unit))
            {
                rc = dnxc_access_handle_action_tcam_scan_ecc1b(unit, block_instance, interrupt_id,
                                                               (dnxc_interrupt_mem_err_info *) param1, msg);
            }
#endif
            SHR_IF_ERR_EXIT(rc);
        ...
        case DNXC_INT_CORR_ACT_TCAM_SCAN_ECC_2B:
            ...
            if (access_new_interrupt_available(unit))
            {
                rc = dnxc_access_handle_action_tcam_scan_ecc2b(unit, block_instance, interrupt_id,
                                                               (dnxc_interrupt_mem_err_info *) param1, msg);
            }
#endif
            SHR_IF_ERR_EXIT(rc);
            break;
        case DNXC_INT_CORR_ACT_RESET_FABRIC_LINK:
            rc = dnxc_interrupt_handles_corrective_action_reset_fabric_link(unit, block_instance, interrupt_id, msg);
            SHR_IF_ERR_EXIT(rc);
            break;
        default:
