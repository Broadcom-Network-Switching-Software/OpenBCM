...
do{                                                                                                                                         \
    ...
    {                                                                                                                                       \
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Error in swstate END_SANTINEL validation");                                                        \
    }                                                                                                                                       \
} while(0)
...
STATIC int dnxc_sw_state_alloc_plain_internal(int unit, uint8 **ptr, uint32 size, char *dbg_string)
{
    ...
    SHR_ALLOC_SET_ZERO_IGNORE_COUNTERS_ERR_EXIT(*ptr, size, dbg_string, "sw state ERROR: failed to allocate data\n%s%s%s", EMPTY, EMPTY, EMPTY);
...
STATIC int dnxc_sw_state_free_sw_state_pointer(int unit, uint32 node_id, uint8 *ptr)
{
    uint32 size = 0;
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_alloc_size_plain(unit, node_id, ptr, &nof_elements, &element_size));
    ...
    uint32 flags)
{
...
#endif
    {
        ...
        sw_state_layout_array[unit] = sal_alloc2(sizeof(dnxc_sw_state_layout_t*) * NOF_MODULE_ID, SAL_ALLOC_F_ZERO, "alloc sw state layout array (plain implementation)");
    }
    ...
    uint32 flags)
{
    ...
    while(entry != NULL) {
        ...
        {
            SHR_IF_ERR_EXIT(dnxc_sw_state_free_sw_state_pointer(unit, node_id, entry->ptr));
    ...
    if(dnxc_sw_state_is_warmboot_supported_get(unit) == FALSE) {
        ...
        SHR_FREE(sw_state_layout_array[unit]);
    }
#else
        SHR_FREE(sw_state_roots_array[unit]);
        SHR_FREE(sw_state_layout_array[unit]);
...
int dnxc_sw_state_module_init_plain(int unit, uint32 node_id, uint32 size, uint32 flags, char *dbg_string)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;

    SHR_IF_ERR_EXIT(dnxc_sw_state_alloc_plain(unit, node_id, (uint8**)&sw_state_roots_array[unit][DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)], 1, size, flags, dbg_string));

    DNXC_SW_STATE_FUNC_RETURN;
}


int dnxc_sw_state_module_deinit_plain(int unit, uint32 node_id, uint32 flags, char *dbg_string)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;

    SHR_IF_ERR_EXIT(dnxc_sw_state_free_plain(unit, node_id, (uint8**)&sw_state_roots_array[unit][DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)], flags, dbg_string));

    DNXC_SW_STATE_FUNC_RETURN;
}


int dnxc_sw_state_alloc_plain(int unit, uint32 node_id, uint8 **ptr_location, uint32 nof_elements, uint32 element_size, uint32 flags, char* dbg_string)
{
    ...
    DNXC_SW_STATE_IF_ERR_EXIT(dnxc_sw_state_alloc_verify_common(unit, node_id, ptr_location, _nof_elements, element_size, flags, dbg_string));
    ...
    *ptr_location += sizeof(dnxc_sw_state_allocation_data_prefix_t);
    ...
    if (DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id) != MULTITHREAD_ANALYZER_MODULE_ID) {
    ...
    if (dnx_sw_state_journal_is_done_init(unit) && !sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnx_sw_state_journal_log_alloc(unit, node_id, ptr_location));
    }
#endif 
...
int dnxc_sw_state_alloc_size_plain(int unit, uint32 node_id, uint8 *location, uint32 *nof_elements, uint32 *element_size)
{
    uint32 size = 0;
    dnxc_sw_state_allocation_data_prefix_t *ptr_prefix = NULL;
    ...
    DNXC_SW_STATE_IF_ERR_EXIT(dnxc_sw_state_alloc_size_verify_common(unit, node_id, location, nof_elements, element_size));
...
STATIC int dnxc_sw_state_free_plain_internal_verify(int unit, uint32 node_id, uint8 **ptr_location, uint32 flags, char *dbg_string, dnxc_sw_state_allocation_dll_entry_t *dll_entry)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;
...
int dnxc_sw_state_free_plain(int unit, uint32 node_id, uint8 **ptr_location, uint32 flags, char *dbg_string)
{
    ...
    DNXC_SW_STATE_IF_ERR_EXIT(dnxc_sw_state_free_verify_common(unit, node_id, ptr_location, flags, dbg_string));

    if (NULL == (*ptr_location)) {
        return SOC_E_NONE;
    }

    dnxc_sw_state_alloc_size_plain(unit, node_id, *ptr_location, &nof_elements, &element_size);

#ifdef BCM_DNX_SUPPORT
    if (dnx_sw_state_journal_is_done_init(unit) && !sw_state_is_flag_on(flags, DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        result = dnx_sw_state_journal_log_free(unit, node_id, nof_elements, element_size, ptr_location);
    }
#endif 
    ...
    if (DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id) != MULTITHREAD_ANALYZER_MODULE_ID) {
        ...
        DNXC_VERIFY_INVOKE(SHR_IF_ERR_EXIT(dnxc_sw_state_free_plain_internal_verify(unit, node_id, ptr_location, flags, dbg_string, dll_entry)));
    ...
    SHR_IF_ERR_EXIT(dnxc_sw_state_free_sw_state_pointer(unit, node_id, *ptr_location));
    *ptr_location = NULL;
...
void dnxc_sw_state_plain_module_size_update(int unit, uint32 node_id, uint32 size, uint32 flags)
{
...
uint32 dnxc_sw_state_plain_module_size_get(int unit, uint32 node_id)
{
...
int dnxc_sw_state_layout_init_plain(int unit, uint32 node_id, uint32 size, uint32 nof_params,  uint32 flags, char *dbg_string)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;

    SHR_IF_ERR_EXIT(dnxc_sw_state_alloc_plain(unit, node_id, (uint8**)&sw_state_layout_array[unit][DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)], DNXC_SW_STATE_LAYOUT_GET_PARAM_IDX(nof_params), size, 0, dbg_string));

    DNXC_SW_STATE_FUNC_RETURN;
}


int dnxc_sw_state_layout_deinit_plain(int unit, uint32 node_id, uint32 flags, char *dbg_string)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;

    SHR_IF_ERR_EXIT(dnxc_sw_state_free_plain(unit, node_id, (uint8**)&sw_state_layout_array[unit][DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)], 0, dbg_string));
...
#ifdef BCM_DNX_SUPPORT
    if(dnx_sw_state_journal_is_done_init(unit) && !sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnx_sw_state_journal_log_memcpy(unit, node_id, size, dest));
    }
#endif 
...
#ifdef BCM_DNX_SUPPORT
            if(dnx_sw_state_journal_is_done_init(unit) && !sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
                SHR_IF_ERR_EXIT(dnx_sw_state_journal_log_memcpy(unit, node_id, 1, ptr_location));
            }
#endif 
...
#ifdef BCM_DNX_SUPPORT
            if(dnx_sw_state_journal_is_done_init(unit) && !sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
                SHR_IF_ERR_EXIT(dnx_sw_state_journal_log_memcpy(unit, node_id, 4, ptr_location));
            }
#endif 
