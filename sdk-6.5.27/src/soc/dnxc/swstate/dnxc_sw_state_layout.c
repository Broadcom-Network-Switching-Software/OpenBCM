    ...
    uint32 nof_params)
{
    ...
    for (idx = 0; idx < DNXC_SW_STATE_LAYOUT_GET_PARAM_IDX(nof_params); idx++)
    {
        ...
        if (layout_address[idx].default_value != NULL){
            layout_address[idx].default_value = NULL;
            DNX_SW_STATE_ALLOC_BASIC(unit, node_id, (uint8 **)(&layout_address[idx].default_value), 1, layout_address[idx].size_of, DNXC_SW_STATE_NONE, "dnxc_sw_state_layout_alloc_default_value");
            sal_memcpy(layout_address[idx].default_value, layout_struct[idx].default_value, layout_address[idx].size_of);
        } 
    ...
    uint32 param_idx)
{
    ...
    for (dimension = 0; dimension < DNXC_SWSTATE_LAYOUT_MAX_ARRAY_DIMENSIONS; dimension++)
    {
        if (is_dynamic_array_dimension(unit, layout_address, param_idx, dimension))
        {
            is_dyn = TRUE;
            break;
        }
...
uint8 is_dynamic_array_dimension(
    ...
    uint32 dimension)
{
    if (layout_address[param_idx].array_indexes[dimension].index_type == DNXC_SWSTATE_ARRAY_INDEX_DYNAMIC
        || layout_address[param_idx].array_indexes[dimension].index_type ==
...
uint32 sw_state_layout_parameter_address_offset_get(
    ...
    uint32 node_id)
{
    uint8 is_dynamic = FALSE;
    uint32 param_iteration = DNXC_SW_STATE_LAYOUT_INVALID;
    uint32 param_idx = DNXC_SW_STATE_LAYOUT_INVALID;
    uint32 module_id = DNXC_SW_STATE_LAYOUT_INVALID;
    uint32 parent_first_child = DNXC_SW_STATE_LAYOUT_INVALID;
    uint32 offset = 0, array_dim_calc = 1;
    int dimension = 0;
    dnxc_sw_state_layout_t * layout_address = NULL;
    ...
    module_id = DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id);
    layout_address = sw_state_layout_array[unit][module_id];
    parent_first_child = layout_address[layout_address[param_idx].parent].first_child_index;


    for(param_iteration = parent_first_child; param_iteration < param_idx; param_iteration++)
    {
        for(dimension = 0; dimension < DNXC_SWSTATE_LAYOUT_MAX_ARRAY_DIMENSIONS; dimension++)
        {
            
            if(layout_address[param_iteration].array_indexes[dimension].num_elements == 0
                && layout_address[param_iteration].array_indexes[dimension].index_type == DNXC_SWSTATE_ARRAY_INDEX_INVALID)
            {
                break;
            }
            
            else if(is_dynamic_array_dimension(unit, layout_address, param_iteration, dimension))
            {
                is_dynamic = TRUE;
                break;
            }
            else if(layout_address[param_iteration].array_indexes[dimension].index_type == DNXC_SWSTATE_ARRAY_INDEX_BITMAP)
            {
                array_dim_calc *= layout_address[param_iteration].array_indexes[dimension].num_elements / 32;
                array_dim_calc += layout_address[param_iteration].array_indexes[dimension].num_elements % 32 != 0 ? layout_address[param_iteration].array_indexes[dimension].num_elements % 32 : 0;
                break;
            }
            
            else
            {
                array_dim_calc *= layout_address[param_iteration].array_indexes[dimension].num_elements;
            }
        }
        ...
        if(is_dynamic)
        {
            ...
            offset += offset % 8 != 0 ? offset % 8 : 0;
            offset += array_dim_calc * sizeof(void*);
        }
        else
        {
            offset += array_dim_calc * layout_address[param_iteration].size_of;
        }
        array_dim_calc = 1;
        is_dynamic = FALSE;
    }
    ...
    if (has_dynamic_dim(unit, layout_address, param_idx))
    {
        offset += offset % 8;
    }
...
void* sw_state_module_root_struct_address_get(
    ...
    uint32 module_id)
{
    return sw_state_roots_array[unit][module_id];
}
