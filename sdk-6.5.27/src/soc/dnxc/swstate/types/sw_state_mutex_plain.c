...
int dnxc_sw_state_mutex_create(int unit, uint32 node_id, sw_state_mutex_t *ptr_mtx, char *desc, uint32 flags)
{
    ...
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnx_sw_state_journal_mutex_create(unit, node_id, ptr_mtx));
    }
#endif 
...
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnx_sw_state_journal_mutex_destroy(unit, node_id, ptr_mtx));
    }
#endif 
...
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnx_sw_state_journal_mutex_take(unit, node_id, ptr_mtx, usec));
    }
#endif 
...
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnx_sw_state_journal_mutex_give(unit, node_id, ptr_mtx));
    }
#endif 
