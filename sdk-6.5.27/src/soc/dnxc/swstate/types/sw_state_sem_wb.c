...
int dnxc_sw_state_sem_create_wb(int unit, uint32 node_id, sw_state_sem_t *ptr_sem, int is_binary, int initial_count, char *desc, uint32 flags) {
    ...
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnx_sw_state_journal_sem_create(unit, node_id, ptr_sem));
    }
#endif 
...
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnx_sw_state_journal_sem_destroy(unit, node_id, ptr_sem, is_binary, initial_count));
    }
#endif 
    DNXC_SW_STATE_FUNC_RETURN;
}
...
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnx_sw_state_journal_sem_take(unit, node_id, ptr_sem, usec));
    }
#endif 
...
#ifdef BCM_DNX_SUPPORT
    if(!sw_state_is_flag_on(flags,  DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
        SHR_IF_ERR_EXIT(dnx_sw_state_journal_sem_give(unit, node_id, ptr_sem));
    }
#endif 
