    ...
    uint32 alloc_flags)
{
    ...
    DNX_SW_STATE_ALLOC(unit, node_id, *defragmented_chunk, **defragmented_chunk,  1,
                       DNXC_SW_STATE_NO_FLAGS, "sw_state defragmented_chunk");

    
    DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, &((*defragmented_chunk)->init_info), init_info,
                              sizeof(sw_state_defragmented_chunk_init_info_t), 0,
                              "sw_state defragmented_chunk init_info");

    
    DNX_SW_STATE_ALLOC(unit, node_id, (*defragmented_chunk)->slots_array, uint8,
            _chunk_size, alloc_flags, "defragmented_chunk init_info");

    DNX_SW_STATE_RANGE_FILL(unit, node_id, (*defragmented_chunk)->slots_array, uint8,
                            0,
                            _chunk_size,
                            tmp_exponent,
                            0,
    ...
    DNX_SW_STATE_SET(unit, node_id, (*defragmented_chunk)->free_slots_available, _chunk_size, uint32,
                     0,
    ...
    sw_state_defragmented_chunk_t * defragmented_chunk)
{
    ...
    DNX_SW_STATE_FREE(unit, node_id, (*defragmented_chunk)->slots_array, "sw state defragmented_chunk slots_array");

    
    DNX_SW_STATE_FREE(unit, node_id, (*defragmented_chunk), "sw state defragmented_chunk");
    ...
    uint32 *piece_offset)
{
    ...
    SHR_IF_ERR_EXIT(sw_state_defragmented_chunk_emptier_piece_search
                    (unit, node_id, defragmented_chunk, wanted_size, piece_offset));
    ...
    do
    {
        ...
        for (offset = *piece_offset; offset < (*piece_offset + wanted_size); offset += iterated_size)
        {
            ...
            rv = sw_state_defragmented_chunk_empty_piece_search(unit, node_id, defragmented_chunk, iterated_size,
                                                                &new_location_offset);
            if (rv == _SHR_E_NOT_FOUND)
            {
                
                SHR_IF_ERR_EXIT(sw_state_defragmented_chunk_defrag_internal
                                (unit, node_id, defragmented_chunk, iterated_size, move_cb_additional_info,
                                 &new_location_offset));
            }
            else
            {
    ...
    uint32 *piece_offset)
{
    ...
    SHR_IF_ERR_EXIT(sw_state_defragmented_chunk_defrag_internal(unit, node_id, defragmented_chunk, wanted_size, move_cb_additional_info, piece_offset));
    ...
    uint32 *piece_offset)
{
    ...
    rv = sw_state_defragmented_chunk_empty_piece_search(unit, node_id, defragmented_chunk, slots_in_piece,
                                                        piece_offset);
    if (rv == _SHR_E_NOT_FOUND)
    {
        
        SHR_IF_ERR_EXIT(sw_state_defragmented_chunk_defrag_internal
                        (unit, node_id, defragmented_chunk, slots_in_piece, move_cb_additional_info, piece_offset));
    }
    else
    {
        SHR_IF_ERR_EXIT(rv);
    ...
    uint32 piece_offset)
{
    ...
    DNX_SW_STATE_MEMCPY_BASIC(unit, node_id, &defragmented_chunk->free_slots_available, &nof_free_slots, sizeof(nof_free_slots), DNXC_SW_STATE_NO_FLAGS, "defragmented_chunk->free_slots_available");
    ...
    uint32 *output)
{
    ...
    *output = defragmented_chunk->free_slots_available;
