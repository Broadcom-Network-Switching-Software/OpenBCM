...
int
dnxc_sw_state_auto_sync(int unit, uint8* current_pointer_in_sw_state, uint32 size) 
{
    soc_scache_handle_t scache_handle = 0;

    DNXC_SW_STATE_INIT_FUNC_DEFS;

    DNXC_SW_STATE_SCACHE_HANDLE_SET(scache_handle, unit, DNXC_SW_STATE_SCACHE_HANDLE_DATA_BLOCK);

    SHR_IF_ERR_EXIT(soc_scache_partial_commit(unit, scache_handle, ((uint8*) current_pointer_in_sw_state) - ((uint8*) dnxc_sw_state_data_block_header[unit]), (int)size)); 
    ...
    uint32 sw_state_max_size)
{
    ...
    sw_state_layout_array[unit] = dnxc_sw_state_data_block_header[unit]->layout_array;
...
int dnxc_sw_state_restore_pointers(int unit, dnxc_sw_state_data_block_header_t *data_block_header){
    ...
    data_block_header->layout_array = (dnxc_sw_state_layout_t**)((uint8*)(data_block_header->roots_array) + sizeof(void*) * NOF_MODULE_ID);
    ...
    while (p <= data_block_header->ptr_offsets_stack_base) {
        ...
        switch (p->type) {
            ...
            case SW_STATE_ALLOC_ELEMENT_SEM:
                ...
                break;
            case SW_STATE_ALLOC_EXTERNAL:
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "dnxc_sw_state_restore_pointers: Invalid pointer type.");
        }
        p++;
    }
...
int dnxc_sw_state_alloc_wb(
    ...
    if (dnxc_sw_state_data_block_is_in_range(unit, (uint8 *)ptr_location) == FALSE && !sw_state_is_flag_on(flags, DNXC_SW_STATE_EXTERNAL_ALLOC_LOCATION)) {
    ...
    *ptr_location = ((uint8*)p) + DNXC_SW_STATE_ALIGN_SIZE(sizeof(dnxc_sw_state_allocation_data_prefix_wb_t));
    ...
    dnxc_sw_state_data_block_pointers_stack_push(unit, ptr_location, sw_state_is_flag_on(flags, DNXC_SW_STATE_EXTERNAL_ALLOC_LOCATION) ? SW_STATE_ALLOC_EXTERNAL : SW_STATE_ALLOC_ELEMENT_POINTER);
...
#ifdef BCM_DNX_SUPPORT
                if(dnx_sw_state_journal_is_done_init(unit) && !sw_state_is_flag_on(flags, DNXC_SW_STATE_JOURNAL_ROLLING_BACK)) {
                    SHR_IF_ERR_EXIT(dnx_sw_state_journal_log_free(unit, node_id, nof_elements, element_size, ptr_location));
                }
#endif 
...
int dnxc_sw_state_memcpy_wb(
    ...
    {
        SHR_IF_ERR_EXIT(dnxc_sw_state_auto_sync(unit, dest, size));
    }
...
int dnxc_sw_state_counter_inc_wb(
    ...
    switch(type_size) {
        case 1: 
            ...
            if (dnxc_sw_state_is_auto_sync_supported_get(unit) && sw_state_is_done_init(unit))
            {
                SHR_IF_ERR_EXIT(dnxc_sw_state_auto_sync(unit, ptr_location, sizeof(uint8)));
            }
            break;
        case 4: 
#ifdef BCM_DNX_SUPPORT
            ...
            if (dnxc_sw_state_is_auto_sync_supported_get(unit) && sw_state_is_done_init(unit))
            {
                SHR_IF_ERR_EXIT(dnxc_sw_state_auto_sync(unit, ptr_location, sizeof(uint32)));
            }
            break;
        default:
