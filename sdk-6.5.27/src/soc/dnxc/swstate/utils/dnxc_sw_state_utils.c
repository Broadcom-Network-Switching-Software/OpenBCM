...
sw_state_is_syncing(
    int unit)
{
    return sw_state_utils_db[unit].is_syncing;
}
...
uint8
dnxc_sw_state_alloc_during_sync_set(
    int unit,
    uint8 flag)
{
    sw_state_utils_db[unit].is_syncing = flag;
    return 0;
}
...
uint8 dnxc_sw_state_is_auto_sync_supported_get(int unit) {
    return (sw_state_utils_db[unit].is_warmboot_supported == 2) ? TRUE : FALSE;
}
