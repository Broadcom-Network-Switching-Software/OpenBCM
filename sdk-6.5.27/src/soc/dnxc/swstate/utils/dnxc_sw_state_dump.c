    ...
    void *ptr)
{
    ...
    result = dnxc_sw_state_dispatcher[unit][DNXC_SW_STATE_LAYOUT_GET_MODULE_ID(node_id)].alloc_size(unit, node_id, (uint8 *)(ptr), &swstate_nof_elements, &swstate_element_size);
    ...
    uint32 n)
{
    ...
    sal_snprintf(str_to_print, sizeof(str_to_print), "%s", "0x");
    if (dnx_sw_state_is_little_endian())
    {
        for (i = 0; i < n; i++)
        {
            sal_snprintf(str_tmp, sizeof(str_tmp), "%02x", p[n - i - 1]);
    ...
    else
    {
        for (i = 0; i < n; i++)
        {
            sal_snprintf(str_tmp, sizeof(str_tmp), "%02x", p[i]);
            sal_strncat_s(str_to_print, str_tmp, sizeof(str_to_print));
        }
    }

    sal_strncat_s(str_to_print, "\n", sizeof(str_to_print));
    ...
    int unit, uint8 is_broken)
{
    ...
    sal_snprintf(str_tmp, DNX_SW_STATE_STRIDE_DUMP_MAX_STRING_LEN, "STRIDE detected. arithmetic progression in the next %llu cells (cells values will not display)\n",
            (LLU) (dnx_sw_state_stride_dump[unit].same_diff_cnt[0]) - DNX_SW_STATE_DUMP_STRIDE_THRESH);
    if (is_broken) sal_strncat_s(str_tmp, dnx_sw_state_stride_dump_last_string[unit], sizeof(str_tmp));
    ...
    char *first_string_to_print)
{
    ...
    if (sal_strncmp(dnx_sw_state_stride_dump_directory_name[unit], "", sizeof(dnx_sw_state_stride_dump_directory_name[unit])) == 0 && sal_strncmp(first_string_to_print, "", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
        return;
    sal_memset(dnx_sw_state_stride_dump_file_name[unit], 0,
               sizeof(dnx_sw_state_stride_dump_file_name[unit]) / sizeof(dnx_sw_state_stride_dump_file_name[unit][0]));
    if (sal_strncmp(dnx_sw_state_stride_dump_directory_name[unit], "", sizeof(dnx_sw_state_stride_dump_directory_name[unit])) != 0) {
        ...
        sal_strncat_s(dnx_sw_state_stride_dump_file_name[unit], "/", sizeof(dnx_sw_state_stride_dump_file_name[unit]));
    }
    sal_strncat_s(dnx_sw_state_stride_dump_file_name[unit], filename, sizeof(dnx_sw_state_stride_dump_file_name[unit]));

    sal_strncat_s(shell_cmd, "mkdir -p", sizeof(shell_cmd));
    sal_strncat_s(shell_cmd, dnx_sw_state_stride_dump_file_name[unit], sizeof(shell_cmd));
    sal_strncat_s(shell_cmd, " >& /dev/null; rm -rf ", sizeof(shell_cmd));
    sal_strncat_s(shell_cmd, dnx_sw_state_stride_dump_file_name[unit], sizeof(shell_cmd));
    system(shell_cmd);
    ...
    if (sal_strncmp(first_string_to_print, "", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0) {
...
dnx_sw_state_dump_detach_file(
    int unit)
{
    if (dnx_sw_state_stride_dump_output_file[unit] != NULL && sal_strncmp(dnx_sw_state_stride_dump_directory_name[unit], "", sizeof(dnx_sw_state_stride_dump_directory_name[unit])) != 0)
    {
        sal_fclose(dnx_sw_state_stride_dump_output_file[unit]);
        dnx_sw_state_stride_dump_output_file[unit] = NULL;
    }
    ...
    char *str)
{
    ...
    if (sal_strncmp(str, " ", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0) {
        return;
    }
