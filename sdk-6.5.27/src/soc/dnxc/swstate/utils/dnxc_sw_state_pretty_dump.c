    ...
    void* data)
{
    ...
    switch (print_cb_data->type)
    {
        ...
        case SW_STATE_PRETTY_PRINT_TYPE_MAC:
        {
            ...
            if (print_cb_data->format_string != NULL)
            {
                sal_snprintf(string_to_print, SW_STATE_PRETTY_PRINT_MAX_PRINT_STRING_SIZE, print_cb_data->format_string, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
                DNX_SW_STATE_PRINT(unit, " %s |", string_to_print);
            }
            else
            {
                DNX_SW_STATE_PRINT(unit, " %02X:%02X:%02X:%02X:%02X:%02X |", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
            }
            break;
        }
