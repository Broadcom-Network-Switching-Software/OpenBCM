...
#define DNXC_CORE_GROUP_ONE_OFFSET (0x10000000)
#define DNXC_ADAPTER_CORE_GROUP_ONE_OFFSET (2058)
...
static soc_error_t
adapter_reg_access_core_group_id_set(
    int unit,
    int sub_unit_id)
{
#ifdef BCM_ACCESS_SUPPORT
    ...
    int unit)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        ...
        for (sub_unit_id = 0; sub_unit_id < adapter_nof_sub_units_get(unit); sub_unit_id++)
        {
            ...
            SHR_IF_ERR_EXIT(adapter_reg_access_core_group_id_set(unit, sub_unit_id));
        }
    }
    ...
    uint8 *buf)
{
    ...
    if (dnx_data_device.general.nof_core_groups_get(unit) > 1)
    {
        if (ms_id == 99999)
        {
            ms_id = dnx_data_adapter.general.oamp_ms_id_get(unit);
            base_sub_unit_id = 1;
        }
    }
...
/* *INDENT-OFF* */
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        adapter_send_buffer_add_signal(dnx_data_adapter.general.tx_packet_tag_swap_res_signal_id_get(unit),
                                       dnx_data_adapter.general.tx_packet_tag_swap_res_signal_width_get(unit),
...
static soc_error_t
dnxc_adapter_nbu_instance_special_swap(
    int unit,
    int core_idx_swap,
    int in_inst,
    int *instance_core_zero_index,
    int *instance_core_swap_index)
{
    int core_index;
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        if (in_inst < (dnx_data_nif.eth.nif_cores_ethus_get(unit, core_index)->first_ethu +
                       dnx_data_nif.eth.nif_cores_ethus_get(unit, core_index)->nof_ethus))
        {
            *instance_core_zero_index = (in_inst - dnx_data_nif.eth.nif_cores_ethus_get(unit, core_index)->first_ethu);
            break;
        }
    }
    
    if ((*instance_core_zero_index) >= dnx_data_nif.eth.nif_cores_ethus_get(unit, core_idx_swap)->nof_ethus)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "can't swap NBU block instance. in_inst=%d, core_idx_swap=%d \n",
                     in_inst, core_idx_swap);
    }
    *instance_core_swap_index = dnx_data_nif.eth.nif_cores_ethus_get(unit, core_idx_swap)->first_ethu +
        *instance_core_zero_index;
    ...
    uint32 *entry_data)
{
    ...
    int special_inst_swap_allowed = FALSE;
    int special_list_index, instance_core_swap_index, instance_core_zero_index = 0;
    ...
    *out_mem_index = in_mem_index;
    instance_core_swap_index = core_idx_swap;
        ...
        (unit, block->block_type)->indication == BLOCK_TYPE_SWAP_IND_SPECIAL_REGMEM)
    {
        ...
        for (special_list_index = 0; special_list_index < table_info->key_size[0]; special_list_index++)
        {
            ...
            if (special_swap_info->block_type == block->block_type)
            {
                ...
                if ((special_local_regmem == regmem) || (special_swap_info->regmem_id == 0))
                {
                    switch (special_swap_info->type)
                    {
                        ...
                        case SPECIAL_SWAP_TYPE_ENTRY_DATA:
                            ...
                            SHR_EXIT();
                            break;
                        case SPECIAL_SWAP_TYPE_NBU_BLK_INSTANCE:
                            SHR_IF_ERR_EXIT(dnxc_adapter_nbu_instance_special_swap
                                            (unit, core_idx_swap, in_inst, &instance_core_zero_index,
                                             &instance_core_swap_index));
                            special_inst_swap_allowed = TRUE;
                            break;
                        default:
    ...
    if (num_of_instances != dnx_data_device.general.nof_cores_get(unit) && special_inst_swap_allowed == FALSE)
    {
    ...
    else
    {
        ...
        if (in_inst == instance_core_zero_index)
        {
            *out_inst = instance_core_swap_index;
        }
        
        if (in_inst == instance_core_swap_index)
        {
            *out_inst = instance_core_zero_index;
        }
    }
...
static shr_error_e
dnxc_adapter_regmem_is_clear_on_read(
    int unit,
...
{
#ifdef BCM_DNX_SUPPORT
    int clear_on_read_idx;
    access_local_regmem_id_t clear_on_read_local_regmem;
    const dnxc_data_table_info_t *table_info;
#endif
    SHR_FUNC_INIT_VARS(unit);
    *is_clear_on_read = FALSE;
#ifdef BCM_DNX_SUPPORT
    if ((is_read == TRUE) && (SOC_IS_DNX(unit)))
    {
            
        table_info = dnx_data_adapter.reg_mem_access.clear_on_read_regmems_info_get(unit);
        for (clear_on_read_idx = 0; clear_on_read_idx < table_info->key_size[0]; clear_on_read_idx++)
        {
            SHR_IF_ERR_EXIT(access_regmem_global2local
                            (runtime_info,
                             dnx_data_adapter.reg_mem_access.clear_on_read_regmems_get(unit, clear_on_read_idx)->regmem,
                             &clear_on_read_local_regmem));
            if (clear_on_read_local_regmem == regmem)
            {
                break;
            }
        }
        if (clear_on_read_idx < table_info->key_size[0])
        {
            *is_clear_on_read = TRUE;
        }
    }
    ...
    uint32 *entry_data)
{
    ...
    SHR_IF_ERR_EXIT(dnxc_adapter_regmem_is_clear_on_read(unit, runtime_info, regmem, is_read, &is_clear_on_read));
...
#ifdef BCM_DNXF_SUPPORT
            sbus_access_info.address = rm_info->base_address;
            sbus_access_info.block_ids[0] = 1;
            ACCESS_SBUS_BLOCK_SET_SUBDEV(sbus_access_info.block_ids[0], swap_inst);
            sbus_access_info.nof_operations = 1;
#endif
        }
    ...
    for (int operation_idx = 0; operation_idx < sbus_access_info.nof_operations; operation_idx++)
    {
        SHR_IF_ERR_EXIT(adapter_memreg_access(unit,
                                              ...
                                              is_clear_on_read, (uint32 *) entry_data));
