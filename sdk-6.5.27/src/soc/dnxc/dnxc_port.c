...
#define DNXC_PORT_SIGNAL_QUALITY_MANTISSA_BITS_NOF 16
#define DNXC_PORT_SIGNAL_QUALITY_MANTISSA_OFFSET 16
#define DNXC_PORT_SIGNAL_QUALITY_EXPONENT_BITS_NOF 16
#define DNXC_PORT_SIGNAL_QUALITY_EXPONENT_OFFSET 0

shr_error_e
soc_dnxc_port_nof_fmacs_get(
    int unit,
    int *nof_fmacs)
{
    ...
    *nof_fmacs = 0;
    ...
    {
#ifdef BCM_DNX_SUPPORT
        *nof_fmacs = dnx_data_fabric.blocks.nof_instances_fmac_get(unit);
#endif
    }
    else
    {
#ifdef BCM_DNXF_SUPPORT
        *nof_fmacs = dnxf_data_device.blocks.nof_instances_fmac_get(unit);
#endif
    }
    ...
    uint32_t * val)
{
    ...
    rv = soc_dnxc_cmic_miim_operation(cl45_user_data->unit, 0, 45, core_addr, reg_addr, &val16);
    (*val) = val16;
    ...
    uint32_t val)
{
    ...
    val16 = (uint16) val;
    return soc_dnxc_cmic_miim_operation(cl45_user_data->unit, 1, 45, core_addr, reg_addr, &val16);
...
static phymod_bus_t dnxc_sim_bus = {
    "dnxc_sim",
    dnxc_sim_bus_read,
    dnxc_sim_bus_write,
    NULL,
...
static shr_error_e
soc_dnxc_phy_lpi_access_supported_get(
    int unit,
    int *lpi_access_supported)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    int *is_sim)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxc_phy_lpi_access_supported_get(unit, &lpi_access_supported));
    ...
    if (*is_sim)
    {
        ...
        PHYMOD_ACC_BUS(access) = &dnxc_sim_bus;
        SHR_IF_ERR_EXIT(soc_physim_check_sim(unit, phymod_type, access, 0, 0, is_sim));
    ...
    for (i = 0; i < phymod_firmware_load_info.ucode_len; i += UC_TABLE_ENTRY_SIZE)
    {
        ...
        if (ACCESS_IS_INITIALIZED(unit))
        {
            SHR_IF_ERR_EXIT(access_regmem
                            (unit, FLAG_ACCESS_IS_WRITE, mFSRD_FSRD_PROM_MEM, ACCESS_ALL_BLOCK_INSTANCES, 0, 0,
                             wr_data));
        }
        else
#endif
...
soc_dnxc_fabric_firmware_loader_callback(
    ...
    const uint8_t * data)
{
    dnxc_port_user_access_t *user_data = core->access.user_acc;
#ifdef BCM_ACCESS_SUPPORT
    uint32 wr_data[SBUS_MAX_NOF_DATA_UINT32S] = { 0 };
    int lpi_access_supported = 0;
#else
    soc_reg_above_64_val_t wr_data;
#endif
    int i = 0;
    int word_index = 0;
    uint32 mem_width_in_words;

    SHR_FUNC_INIT_VARS(user_data->unit);

#ifdef BCM_ACCESS_SUPPORT
    SHR_IF_ERR_EXIT(soc_dnxc_phy_lpi_access_supported_get(user_data->unit, &lpi_access_supported));
    if (ACCESS_IS_INITIALIZED(user_data->unit))
    {
        access_regmem_width_in_uint32(user_data->unit, mFSRD_FSRD_PROM_MEM, &mem_width_in_words);
    }
    else
#endif
    {
        mem_width_in_words = dnx_drv_soc_mem_words(user_data->unit, FSRD_FSRD_PROM_MEMm);
    }

    for (i = 0; i < length; i += UC_TABLE_ENTRY_SIZE)
    {
        SHR_IF_ERR_EXIT(utilex_bitstream_clear(wr_data, mem_width_in_words));
...
#ifdef BCM_ACCESS_SUPPORT
        if (ACCESS_IS_INITIALIZED(user_data->unit) && lpi_access_supported)
        {
            SHR_IF_ERR_EXIT(access_regmem
                            (user_data->unit, FLAG_ACCESS_IS_WRITE | FLAG_ACCESS_INST_TYPE_SBUS_ID,
                             mFSRD_FSRD_PROM_MEM, user_data->fsrd_blk_id, 0, 0, wr_data));
        }
        else
#endif
        {
            SHR_IF_ERR_EXIT(WRITE_FSRD_FSRD_PROM_MEMm(user_data->unit, user_data->fsrd_blk_id, 0, wr_data));
        }
...
{
#ifdef BCM_ACCESS_SUPPORT
    uint32 data[SBUS_MAX_NOF_DATA_UINT32S] = { 0 };
#else
    ...
    *single_pll_restriction_disable =
    ...
    if (!*single_pll_restriction_disable)
    {
        ...
        if (ACCESS_IS_INITIALIZED(unit))
        {
            SHR_IF_ERR_EXIT(access_regmem(unit, 0, rECI_RESERVED_51, 0, 0, 0, data));
        }
        else
#endif
        {
...
{
    int is_phy_rx_taps_supported = 1;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        is_phy_rx_taps_supported = dnx_data_port.general.pm_info_get(unit, pm_type)->is_phy_rx_taps_supported;
#endif
    }
    else
    {
    ...
    int *to_disable)
{
    ...
    *to_disable = 0;
    ...
    if (SOC_IS_DNX(unit))
    {
        *to_disable = dnx_data_port.general.phy_controls_features_get(unit, type)->port_disable_required;
    }
#endif
#ifdef BCM_DNXF_SUPPORT
    if (SOC_IS_DNXF(unit))
    {
        *to_disable = dnxf_data_port.general.phy_controls_features_get(unit, type)->port_disable_required;
    }
#endif
...
static shr_error_e
soc_dnxc_port_phy_scrambling_disable_supported(
    int unit,
    bcm_port_t gport,
    int *is_supported)
{
    ...
    *is_supported = 0;
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        *is_supported = dnx_data_port.general.pm_info_get(unit, pm_type)->is_scrambling_disable_supported;
    }
#endif
#ifdef BCM_DNXF_SUPPORT
    if (SOC_IS_DNXF(unit))
    {
        *is_supported =
            dnxf_data_port.general.feature_get(unit, dnxf_data_port_general_is_scrambling_disable_supported);
    }
#endif
    ...
    const bcm_port_resource_t * resource)
{
    ...
    int lane_rate = 0;
    int is_scrambling_disable_supported = 1;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(soc_dnxc_port_phy_scrambling_disable_supported
                    (unit, resource->port, &is_scrambling_disable_supported));
    if (is_scrambling_disable_supported == 0
        && PORTMOD_PORT_PHY_LANE_CONFIG_SCRAMBLING_DISABLE_GET(resource->phy_lane_config) == 1)
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG,
                     "port %d: scrambling disable configuration - bit 8 of phy_lane_config is not supported.\n",
                     resource->port);
    }
...
#ifdef BCM_DNX_SUPPORT
        uint32 flags = 0;
        SHR_IF_ERR_EXIT(dnx_algo_port_speed_get(unit, port, flags, speed));
#endif
    }
    else
    {
#ifdef BCM_DNXF_SUPPORT
        SHR_IF_ERR_EXIT(dnxf_state.port.speed.get(unit, port, speed));
#endif
    }
    ...
    bcm_port_phy_fec_t * fec_type)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(fec_type, _SHR_E_PARAM, "fec_type");
...
#ifdef BCM_DNXF_SUPPORT
        int fec_type_int;
        SHR_IF_ERR_EXIT(dnxf_state.port.fec_type.get(unit, port, &fec_type_int));
...
static void
soc_dnxc_port_signal_quality_float_to_man_and_exp_get(
    float val,
    uint16 *man,
    int16 *exp)
{
    ...
    *exp = 0;
    ...
    while (val >= (1 << DNXC_PORT_SIGNAL_QUALITY_MANTISSA_BITS_NOF))
    {
        val /= 10;
        *exp += 1;
    }
...
shr_error_e
soc_dnxc_port_signal_quality_float_to_encoded_val_get(
    int unit,
    float val,
    uint32 *encoded_val)
{
    ...
    soc_dnxc_port_signal_quality_float_to_man_and_exp_get(val, &man, &exp);

    SHR_IF_ERR_EXIT(utilex_bitstream_set_field
                    (encoded_val, DNXC_PORT_SIGNAL_QUALITY_EXPONENT_OFFSET,
                     DNXC_PORT_SIGNAL_QUALITY_EXPONENT_BITS_NOF, exp));
    SHR_IF_ERR_EXIT(utilex_bitstream_set_field
                    (encoded_val, DNXC_PORT_SIGNAL_QUALITY_MANTISSA_OFFSET,
                     DNXC_PORT_SIGNAL_QUALITY_MANTISSA_BITS_NOF, man));
...
shr_error_e
soc_dnxc_port_signal_quality_fabric_hw_thr_to_encoded_thr_get(
    int unit,
    bcm_port_t port,
    int interval,
    uint32 hw_val,
    uint32 *encoded_val)
{
    ...
    hw_val = utilex_power_of_2(hw_val);

    SHR_IF_ERR_EXIT(soc_dnxc_port_lane_rate_get(unit, port, &lane_rate));

    thr = hw_val / (float) (lane_rate * interval);

    SHR_IF_ERR_EXIT(soc_dnxc_port_signal_quality_float_to_encoded_val_get(unit, thr, encoded_val));
    ...
    uint32 *phy_data)
{
    ...
    int max_core_access_per_port = 1;
    portmod_access_get_params_t params;
    ...
    if (SOC_IS_DNX(unit))
    {
        max_core_access_per_port = dnx_data_port.general.max_core_access_per_port_get(unit);
    }
#endif
    ...
    {
        rv = portmod_port_phy_lane_access_get(unit, port, &params, max_core_access_per_port, phy_access,
    ...
    else
    {
        if (flags & SOC_PHY_NOMAP)
        {
            ...
            rv = soc_dnxc_cmic_miim_operation(unit, 0, 45, phy_id, phy_reg, &phy_rd_data);
            SHR_IF_ERR_EXIT(rv);
    ...
    uint32 phy_data)
{
    ...
    else
    {
        if (flags & BCM_PORT_PHY_NOMAP)
        {
            ...
            rv = soc_dnxc_cmic_miim_operation(unit, 1, 45, phy_id, phy_reg, &phy_wr_data);
            SHR_IF_ERR_EXIT(rv);
        }
        else
        {
    ...
    else
    {
        ...
        if (link != -1)
        {
            ...
            SHR_IF_ERR_EXIT(soc_dnxf_drv_link_to_block_mapping
                            (unit, link, pm_start, &pm_inner_link, SOC_DNXF_DRV_BLOCK_TYPE_FSRD));
            *nof_pms = 1;
        }
        else
        {
...
soc_dnxc_port_cable_swap_set_verify(
    ...
    int arg)
{
    int enable, pm_index, is_enable, pm_id = 0, nof_pms = 0;
    int link_iter;
    ...
    else
    {
        ...
        if (link != -1)
        {
            ...
            SHR_IF_ERR_EXIT(soc_dnxf_link_device_mapping_bmp_get(unit, pm_id, SOC_DNXF_DRV_BLOCK_TYPE_FSRD, &links));
        }
        else
        {
...
shr_error_e
soc_dnxc_port_fabric_fmacs_enabled_get(
    int unit,
    soc_pbmp_t * fmacs_enabled_bmp)
{
    int fmac_id, nof_fmacs = 0, enable;

    SHR_FUNC_INIT_VARS(unit);

    SOC_PBMP_CLEAR(*fmacs_enabled_bmp);

    SHR_IF_ERR_EXIT(soc_dnxc_port_nof_fmacs_get(unit, &nof_fmacs));

    for (fmac_id = 0; fmac_id < nof_fmacs; fmac_id++)
    {
        SHR_IF_ERR_EXIT(soc_dnxc_fabric_fmac_block_valid_get(unit, fmac_id, &enable));
        if (enable)
        {
            BCM_PBMP_PORT_ADD(*fmacs_enabled_bmp, fmac_id);
        }
    }
...
{
    soc_pbmp_t fmacs_enabled_bmp = { {0} };
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(soc_dnxc_port_fabric_fmacs_enabled_get(unit, &fmacs_enabled_bmp));
...
{
    soc_pbmp_t fmacs_enabled_bmp = { {0} };
    soc_pbmp_t fmacs_that_should_remain_enabled_bmp = { {0} };
    SHR_FUNC_INIT_VARS(unit);
    ...
    int *fmac_inner_link)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        *fmac_inner_link = link % dnx_data_fabric.blocks.nof_links_in_fmac_get(unit);
#endif
    }
    else if (SOC_IS_DNXF(unit))
    {
#ifdef BCM_DNXF_SUPPORT
        SHR_IF_ERR_EXIT(soc_dnxf_drv_link_to_block_mapping
                        (unit, link, fmac_index, fmac_inner_link, SOC_DNXF_DRV_BLOCK_TYPE_FMAC));
    ...
    int *actual_errcnt)
{
    ...
    sal_memset(&ber_proj_params, 0, sizeof(soc_port_phy_ber_proj_params_t));
