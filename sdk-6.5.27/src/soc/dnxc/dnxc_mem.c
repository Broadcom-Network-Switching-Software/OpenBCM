        ...
        for (array_index = array_index_min; array_index <= array_index_max; array_index++)
        {
            if (is_seq_array)
            {
                ...
                for (index = index_min; index <= index_max; index++)
                {
                    ...
                    {
                        *field_val = index + ((index_max + 1) * array_index);
                        soc_mem_field_width_fit_set(unit, mem, entry, field, field_val);
                    }
                }
            }
...
#ifdef BCM_DNX_SUPPORT
static int
dnx_egr_queuing_nif_calendar_ser_switch(
    int unit,
    soc_mem_t mem,
    int core_id,
    uint32 *switched)
{
    uint32 current_cal, mem_cal = 0xFF;

    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(switched, _SHR_E_PARAM, "switched");

    SHR_IF_ERR_EXIT(dnx_egr_queuing_nif_current_cal_get(unit, core_id, &current_cal));
    switch (mem)
    {
        case EPS_EPS_NIF_PORT_MUX_0m:
        case FQP_FQP_NIF_PORT_MUX_0m:
        {
            mem_cal = 0;
            break;
        }
        case EPS_EPS_NIF_PORT_MUX_1m:
        case FQP_FQP_NIF_PORT_MUX_1m:
        {
            mem_cal = 1;
            break;
        }
    ...
    if (mem_cal == current_cal)
    {
        SHR_IF_ERR_EXIT(dnx_egr_queuing_nif_calendar_activate(unit, core_id));
        *switched = 1;
    }
    else
    {
...
static int
dnx_esb_nif_calendar_ser_switch(
    int unit,
    soc_mem_t mem,
    int core_id,
    uint32 *switched)
{
    uint32 current_mux_idx, mem_mux_idx = 0xFF;

    SHR_FUNC_INIT_VARS(unit);
    SHR_NULL_CHECK(switched, _SHR_E_PARAM, "switched");

    SHR_IF_ERR_EXIT(dnx_esb_nif_current_calendar_get(unit, core_id, &current_mux_idx));
    switch (mem)
    {
    ...
    if (current_mux_idx == mem_mux_idx)
    {
        SHR_IF_ERR_EXIT(dnx_esb_nif_calendar_set(unit, core_id));
        *switched = 1;
    }
    else
    {
...
int
dnx_egr_esb_nif_calendar_mux_switch_start(
    int unit,
    soc_mem_t mem,
    int blk,
    uint32 *switched)
{
    ...
    switch (mem)
    {
        ...
        case FQP_FQP_NIF_PORT_MUX_1m:
        {
            ...
            SHR_IF_ERR_EXIT(dnx_egr_queuing_nif_calendar_ser_switch(unit, mem, reg_port, switched));
            break;
        }
        case ESB_ESB_QUEUE_MUX_0m:
        case ESB_ESB_QUEUE_MUX_1m:
        {
            reg_port = blk - SOC_MEM_BLOCK_MIN(unit, mem);
            SHR_IF_ERR_EXIT(dnx_esb_nif_calendar_ser_switch(unit, mem, reg_port, switched));
            break;
        }
        default:
        {
...
int
dnx_egr_esb_nif_calendar_mux_switch_end(
    int unit,
    soc_mem_t mem,
    int blk,
    uint32 switched)
{
    ...
    switch (mem)
    {
        ...
        {
            reg_port = blk - SOC_MEM_BLOCK_MIN(unit, mem);
            SHR_IF_ERR_EXIT(dnx_egr_queuing_nif_calendar_activate(unit, reg_port));
            break;
        }
        case ESB_ESB_QUEUE_MUX_0m:
        case ESB_ESB_QUEUE_MUX_1m:
        {
            reg_port = blk - SOC_MEM_BLOCK_MIN(unit, mem);
            SHR_IF_ERR_EXIT(dnx_esb_nif_calendar_set(unit, reg_port));
            break;
        }
        default:
        {
    ...
    soc_reg_above_64_val_t * eps_shaper_set)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        switch (mem)
        {
            ...
            {
                reg_port = blk - SOC_MEM_BLOCK_MIN(unit, mem);
                SHR_IF_ERR_EXIT(dnx_egr_queuing_nif_calendar_ser_switch(unit, mem, reg_port, eps_protect_flag));
                break;
            }
            case ESB_ESB_QUEUE_MUX_0m:
            case ESB_ESB_QUEUE_MUX_1m:
            {
                reg_port = blk - SOC_MEM_BLOCK_MIN(unit, mem);
                SHR_IF_ERR_EXIT(dnx_esb_nif_calendar_ser_switch(unit, mem, reg_port, eps_protect_flag));
                break;
            }
            case INVALIDm:
            {
