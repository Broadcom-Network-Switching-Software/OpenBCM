...
/* Sense Resistors Configuration Table for Wolfhound3 53642 */
static i2c_ltc_t sense_resistor_config_53642[WH3_MAX_VS_CONFIG]={
    { 0, "Core",     I2C_LTC7132_40, I2C_LTC7132_CH0,     1,  0 },
};
#endif
...
ltc388x_board_resistor_match(int unit, int in_offset)
{
    ...
    if (SOC_CONTROL(unit)->board_type == 0xD6) {
        return SOC_E_NONE;
    }
#endif
...
ltc388x_setmin_max(int unit, int devno, int len)
{
    ...
    if (SOC_CONTROL(unit)->board_type == 0xD6) {
        max_vs_config = sizeof(sense_resistor_config_53642)/sizeof(i2c_ltc_t);
    }
                                                        ...
                                                        unsigned short voltage)
{
    ...
    /* LTM4676A/LTC7132 does not support command POWER_GOOD_ON/POWER_GOOD_OFF */
    if ((soc_i2c_devtype(unit, devno) != LTC3884_DEVICE_TYPE) &&
        (soc_i2c_devtype(unit, devno) != LTM4676A_DEVICE_TYPE) &&
        (soc_i2c_devtype(unit, devno) != LTC7132_DEVICE_TYPE)) {
...
ltc388x_dac_set_ch_voltage(int unit, int devno, int ch, unsigned short voltage)
{
    ...
    if ((SOC_CONTROL(unit)->board_type == 0x94) ||
        ...
        (SOC_CONTROL(unit)->board_type == 0xCE) ||
        (SOC_CONTROL(unit)->board_type == 0xD6)) {
...
ltc388x_ioctl(int unit, int devno, int opcode, void* data, int len)
{
    ...
    case I2C_LTC_IOC_SET_CONFIG:
        ...
        if (SOC_CONTROL(unit)->board_type == 0xD6) {
            sense_resistor_config = sense_resistor_config_53642;
            rv = ltc388x_setmin_max(unit, devno, len);
            break;
        }
#endif
   ...
   case I2C_LTC_IOC_NOMINAL:
            ...
            for (iter = 0; iter < 100; iter++) {
                if (soc_i2c_addr(unit, devno) == I2C_LTC3884_SADDR42) {
                    ...
                    fval += fval1;
                        ...
                        (soc_i2c_addr(unit, devno) == I2C_LTC7132_SADDR40)) {
                    if ((SOC_CONTROL(unit)->board_type == 0xCD) ||
                        (SOC_CONTROL(unit)->board_type == 0xCE) ||
                        (SOC_CONTROL(unit)->board_type == 0xD6)) {
                        /* For Trident3x1/Wolfhound3 phase support */
                        no_ch = I2C_BOTH_CH;
    ...
    case I2C_LTC_IOC_SET_VOUT:
        ...
        if (DEV_DAC_PARAMS(dev)) {
             ...
             *
             * For Wolfhound3 2 Phase support
             * LTC7132 at 0x40: Channel 0,1 = VDD_Core
             */
            if ((SOC_CONTROL(unit)->board_type == 0x94) ||
                (SOC_CONTROL(unit)->board_type == 0x96) ||
                (SOC_CONTROL(unit)->board_type == 0xB0) ||
...
ltc388x_init(int unit, int devno, void* data, int len)
{
    ...
    if (devtype == LTC3880_DEVICE_TYPE) {
        ...
        soc_i2c_devdesc_set(unit, devno, "Linear Tech LTM4676A Step_Down DC/DC "
                "Controller");
    } else if (devtype == LTC7132_DEVICE_TYPE) {
        soc_i2c_devdesc_set(unit, devno, "Linear Tech LTC7132 Step_Down DC/DC "
                "Controller");
    } else {
...
STATIC int
ltc7132_init(int unit, int devno, void* data, int len)
{
    SOC_IF_ERROR_RETURN(
            ltc388x_check_device_id(unit, devno, LTC_MFR_ID, LTC_7132_MFR_MODEL));
    return ltc388x_init(unit, devno, data, len);
}
...
/* ltc 7132 voltage control Chip Driver callout */
i2c_driver_t _soc_i2c_ltc7132_driver = {
    0x0, 0x0, /* System assigned bytes */
    LTC7132_DEVICE_TYPE,
    ltc388x_read,
    ltc388x_write,
