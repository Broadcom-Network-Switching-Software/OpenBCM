...
int portmod_port_external_los_enable_set(int unit, int port, uint32 enable)
{
    ...
    if(NULL != __portmod__dispatch__[__type__]->f_portmod_port_external_los_enable_set) {
        pm_info_t pm_info;
        _SOC_IF_ERR_EXIT(portmod_pm_info_get(unit, port, &pm_info));
        __rv__ = __portmod__dispatch__[__type__]->f_portmod_port_external_los_enable_set(unit, port, pm_info, enable);
        _SOC_IF_ERR_EXIT(__rv__);
    } else { 
        _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL, ("portmod_port_external_los_enable_set isn't implemented for driver type"));
    }
...
int portmod_port_external_los_enable_get(int unit, int port, uint32* enable)
{
    ...
    if(NULL != __portmod__dispatch__[__type__]->f_portmod_port_external_los_enable_get) {
        pm_info_t pm_info;
        _SOC_IF_ERR_EXIT(portmod_pm_info_get(unit, port, &pm_info));
        __rv__ = __portmod__dispatch__[__type__]->f_portmod_port_external_los_enable_get(unit, port, pm_info, enable);
        _SOC_IF_ERR_EXIT(__rv__);
    } else { 
        _SOC_EXIT_WITH_ERR(SOC_E_UNAVAIL, ("portmod_port_external_los_enable_get isn't implemented for driver type"));
    }
