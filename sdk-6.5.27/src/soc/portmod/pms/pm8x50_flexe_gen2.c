...
STATIC portmod_ucode_buf_t pm8x50_flexe_gen2_flr_ucode_buf[SOC_MAX_NUM_DEVICES] = {{NULL, 0}};
STATIC portmod_ucode_buf_t pm8x50_flexe_gen2_flr_ucode_buf_2nd[SOC_MAX_NUM_DEVICES] = {{NULL, 0}};
    ...
    switch (an_state) {
       ...
       case PM8X50_FLEXE_GEN2_PORT_AN_CHECK_PCS_LINKUP:
       {
            ...
            if (link && (!remote_fault)) {
                ...
                if (wait_link_count > TIMEOUT_COUNT_400G_AN) {
                    ...
                    an_control.num_lane_adv = 8;
                    _SOC_IF_ERR_EXIT(phymod_phy_autoneg_set(&phy_access, &an_control));
                    an_state = PM8X50_FLEXE_GEN2_PORT_AN_DISABLE;
                }
            }
       ...
       case PM8X50_FLEXE_GEN2_PORT_AN_RESTART:
       {
            ...
            if (link && (!remote_fault)) {
                ...
                _SOC_IF_ERR_EXIT(_pm8x50_flexe_gen2_pm_port_mode_update(unit, port, pm_info, 0, 4));
...
/**
 * this function will download ucode through UCMEM
 * One entry used only of UCMEM each time, so no size limitation.
 */
int
_pm8x50_flexe_gen2_firmware_set(int unit,
                                int blk_id,
                                const uint8 *array,
                                uint32 datalen,
                                ...
                                int index_offset)
{
    ...
    _SOC_IF_ERR_EXIT
        (soc_mem_array_write_range_multi_cmc(unit, flags, ucmem_data, 0, blk_id, index_offset,
                             buf->ucode_alloc_size / entry_bytes - 1 + index_offset,
                             buf->ucode_dma_buf, -1));
#else
    flags = SOC_MEM_WRITE_INCR_NOADD;
    /* coverity[stack_use_overflow] */
    _SOC_IF_ERR_EXIT
        (soc_mem_array_write_range(unit, flags, ucmem_data, 0, blk_id, index_offset,
                             buf->ucode_alloc_size / entry_bytes - 1 + index_offset,
                             buf->ucode_dma_buf));
#endif
...
pm8x50_flexe_gen2_default_fw_loader(const phymod_core_access_t* core, uint32_t length, const uint8_t* data)
{
    ...
    int flr_ucode = FALSE, index_offset = 0;
    SOC_INIT_FUNC_DEFS;
    ...
    if ((core->access.lane_mask & 0x1) == 0x1) {
        index_offset = 0x0000;
    } else if ((core->access.lane_mask & 0x4) == 0x4) {
        index_offset = 0x0800;
    } else if ((core->access.lane_mask & 0x10) == 0x10) {
        index_offset = 0x1000;
    } else if ((core->access.lane_mask & 0x40) == 0x40) {
        index_offset = 0x1800;
    }
    rv = _pm8x50_flexe_gen2_firmware_set(unit,
                              user_data->blk_id,
                              data,
                              length,
                              load_order,
                              bcast_en,
                              flr_ucode ? &(pm8x50_flexe_gen2_flr_ucode_buf[unit]) : &(pm8x50_flexe_gen2_ucode_buf[unit]),
                              flr_ucode ? &(pm8x50_flexe_gen2_flr_ucode_buf_2nd[unit]) : &(pm8x50_flexe_gen2_ucode_buf_2nd[unit]),
                           ...
                           pm_info_t pm_info)
{
    ...
    /* Share one copy of pmd_info_ptr for 4 PMD instances */
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        if (is_core_initialized) {
            _SOC_IF_ERR_EXIT(portmod_common_serdes_probe(pm8x50_flexe_gen2_serdes_list, &pm8x50_flexe_gen2_data->int_core_access, &probe));
            if (!probe) {
                LOG_ERROR(BSL_LS_SOC_PORT,
                      (BSL_META_U(unit, "ERROR: serdes probe failed type=%d\n"), pm8x50_flexe_gen2_data->int_core_access.type));
            }
        }
    } else {
                             ...
                             int enable)
{
    ...
    if(enable && is_reset) {
        ...
        if (!mac_only) {
            ...
            sal_memset(PM_8x50_FLEXE_GEN2_INFO(pm_info)->int_core_access.access.pmd_info_ptr, 0, PMD_INFO_DATA_STRUCTURE_SIZE);
        }
    }
    ...
    switch (vco) {
        ...
        case portmodVCO25P781G:
            if (ref_clock == phymodRefClk156Mhz) {
                *pll = phymod_TSCBH_PLL_DIV165;
                ...
                if (is_otn_mode) {
                    *pll = phymod_TSCBH_PLL_DIV82P50128477;
                } else {
        ...
        case portmodVCO26P773G:
            if (ref_clock == phymodRefClk156Mhz) {
                ...
                *pll = phymod_TSCBH_PLL_DIV85P67380253;
            } else {
                rv = SOC_E_PARAM;
            }
            break;
        case portmodVCO27P622G:
            if (ref_clock == phymodRefClk156Mhz) {
                ...
                *pll = phymod_TSCBH_PLL_DIV88P39285714;
            } else {
                rv = SOC_E_PARAM;
            }
            break;
        case portmodVCO27P739G:
            if (ref_clock == phymodRefClk156Mhz) {
                ...
                *pll = phymod_TSCBH_PLL_DIV88P76582278;
            } else {
                rv = SOC_E_PARAM;
            }
            break;
        case portmodVCO27P952G:
            if (ref_clock == phymodRefClk156Mhz) {
                *pll = phymod_TSCBH_PLL_DIV178P8955584;
            } else if (ref_clock == phymodRefClk312Mhz) {
                *pll = phymod_TSCBH_PLL_DIV89P44777920;
            } else {
                rv = SOC_E_PARAM;
            }
            break;
        case portmodVCO28P076G:
            if (ref_clock == phymodRefClk156Mhz) {
                ...
                *pll = phymod_TSCBH_PLL_DIV89P84376637;
            } else {
                rv = SOC_E_PARAM;
            }
            break;
...
int _pm8x50_flexe_gen2_pm_serdes_core_init(int unit, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    phymod_core_access_t* core_access = &PM_8x50_FLEXE_GEN2_INFO(pm_info)->int_core_access;
    ...
    if (core_conf.interface.ref_clock == phymodRefClk156Mhz) {
        core_conf.pll0_div_init_value = phymod_TSCBH_PLL_DIV170;
        core_conf.pll1_div_init_value = phymod_TSCBH_PLL_DIV165;
    } else if (core_conf.interface.ref_clock == phymodRefClk312Mhz) {
        core_conf.pll0_div_init_value = phymod_TSCBH_PLL_DIV85;
        core_conf.pll1_div_init_value = phymod_TSCBH_PLL_DIV82P5;
    }
...
int pm8x50_flexe_gen2_port_speed_config_set(int unit, int port, pm_info_t pm_info, const portmod_speed_config_t* speed_config)
{
    ...
    /* Check if transparent AM configuration */
     if (PORTMOD_SPEED_CONFIG_F_TRANSPARENT_AM_GET(speed_config)) {
         PHYMOD_SPEED_CONFIG_TRANSPARENT_AM_SET(&phy_speed_config);
     } else {
         PHYMOD_SPEED_CONFIG_TRANSPARENT_AM_CLR(&phy_speed_config);
     }
...
int pm8x50_flexe_gen2_port_speed_config_get(int unit, int port, pm_info_t pm_info, portmod_speed_config_t* speed_config)
{
    ...
    /* Check if transparent AM configuration */
     if (PHYMOD_SPEED_CONFIG_TRANSPARENT_AM_GET(&phy_speed_config)) {
         PORTMOD_SPEED_CONFIG_F_TRANSPARENT_AM_SET(speed_config);
     }
                                    ...
                                    const phymod_autoneg_control_t* an)
{
    ...
    if (an_control.enable) {
        ...
        if (is_400g_an) {
            ...
            _SOC_IF_ERR_EXIT(_pm8x50_flexe_gen2_pm_port_mode_update(unit, port, pm_info, port_index, 4));
        } else {
...
/* 1. Adjust timesync settings for 1588. */
int _pm8x50_flexe_gen2_port_phy_timesync_link_up_handler (int unit, int port, pm_info_t pm_info)
{
    portmod_access_get_params_t params;
    phymod_phy_access_t phy_access;
    ...
    _SOC_IF_ERR_EXIT
        (pm8x50_flexe_gen2_port_phy_lane_access_get(unit, port, pm_info,
                                &params, 1, &phy_access, &nof_phys, NULL));
    _SOC_IF_ERR_EXIT
        (_pm8x50_flexe_gen2_port_index_get(unit, port, pm_info, &port_index, &bitmap));
    ...
    if (PORTMOD_PORT_TIMESYNC_CONFIG_ENABLE_GET(timesync_config)) {
        /*
         * Disable and enable the ts_update_enable
         * on link up.
         */
        flags = 0;
...
int pm8x50_flexe_gen2_port_phy_flr_link_up_event(int unit, int port, pm_info_t pm_info)
{
    ...
    _SOC_IF_ERR_EXIT
        (_pm8x50_flexe_gen2_port_phy_timesync_link_up_handler(unit, port, pm_info));
...
STATIC
int _pm8x50_flexe_gen2_flr_timeout_portmod_to_phymod(portmod_flr_timeout_t timeout, phymod_phy_flr_timeout_t* phymod_timeout)
{
    int rv = SOC_E_NONE;
...
STATIC
int _pm8x50_flexe_gen2_flr_timeout_phymod_to_portmod(phymod_phy_flr_timeout_t phymod_timeout, portmod_flr_timeout_t* timeout)
{
    int rv = SOC_E_NONE;
...
int pm8x50_flexe_gen2_port_flr_config_set(int unit, int port, pm_info_t pm_info, int enable, portmod_flr_timeout_t timeout_time)
{
    ...
    if (!SAL_BOOT_SIMULATION) {
        _SOC_IF_ERR_EXIT
            (_pm8x50_flexe_gen2_flr_timeout_portmod_to_phymod(timeout_time, &phymod_timeout));
        _SOC_IF_ERR_EXIT
...
int pm8x50_flexe_gen2_port_flr_config_get(int unit, int port, pm_info_t pm_info, int *enable, portmod_flr_timeout_t *timeout_time)
{
    ...
    *enable = flr_enable ? 1 : 0;
    _SOC_IF_ERR_EXIT
        (_pm8x50_flexe_gen2_flr_timeout_phymod_to_portmod(phymod_timeout, timeout_time));

exit:
    SOC_FUNC_RETURN;
}

int pm8x50_flexe_gen2_port_external_los_enable_set(int unit, int port, pm_info_t pm_info,
                                                   uint32 enable)
{
    int nof_phys;
    portmod_access_get_params_t params;
    ...
    _SOC_IF_ERR_EXIT
        (phymod_phy_external_los_enable_set(&phy_access, enable));

exit:
    SOC_FUNC_RETURN;
}

int pm8x50_flexe_gen2_port_external_los_enable_get(int unit, int port, pm_info_t pm_info,
                                                   uint32* enable)
{
    int nof_phys;
    portmod_access_get_params_t params;
    ...
    _SOC_IF_ERR_EXIT
        (phymod_phy_external_los_enable_get(&phy_access, enable));
