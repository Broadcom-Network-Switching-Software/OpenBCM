...
int xlmac_faults_status_get(int unit, int port,
                            portmod_port_fault_status_t *faults)
{
    SOC_INIT_FUNC_DEFS;

    /* Get local fault status. */
    _SOC_IF_ERR_EXIT(xlmac_local_fault_status_get(unit, port, &faults->local));

    /* Get remote fault status. */
    _SOC_IF_ERR_EXIT(xlmac_remote_fault_status_get(unit, port,
                                                   &faults->remote));
