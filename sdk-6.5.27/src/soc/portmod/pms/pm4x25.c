...
#include <soc/portmod/pm4x25d_int.h>
...
#define PM4x25_WB_BUFFER_VERSION        (17)
...
#define PM4X25_REG_READ(unit, reg_name, phy_acc, reg_val) \
...
#define PM4X25_REG_WRITE(unit, reg_name, phy_acc, reg_val) \
    ((phy_acc & SOC_REG_ADDR_PHY_ACC_MASK) ?\
...
{
    uint32 reg_val, pm_type, field_val;
    int phy_port;
     ...
     */
    if (UA_REG_IS_VALID(unit, CLPORT_PM_VERSION_ID) &&
        ...
        _SOC_IF_ERR_EXIT(UA_REG_READ_PHY_PORT(unit, CLPORT_PM_VERSION_ID, phy_port, 0, &reg_val));
        UA_REG_FIELD_GET(unit, CLPORT_PM_VERSION_ID, &reg_val, PM_TYPE, &field_val);
        ...
        UA_REG_FIELD_GET(unit, CLPORT_PM_VERSION_ID, &reg_val, TECH_PROCESS, &field_val);
        version->tech_process    = field_val + 1;
        UA_REG_FIELD_GET(unit, CLPORT_PM_VERSION_ID, &reg_val, REV_NUMBER, &field_val);
        version->revision_number = field_val + 1;
        UA_REG_FIELD_GET(unit, CLPORT_PM_VERSION_ID, &reg_val, REV_LETTER, &field_val);
        version->revision_letter = field_val + 1;
    } else {
        version->type = PM_TYPE_PM4X25;
        version->gen  = PM_GEN1;
    }
...
int portmod_pm4x25_wb_upgrade_func(int unit, void *arg, int recovered_version, int new_version)
{
    ...
    if ((recovered_version <= 16) &&
        (new_version >= 17)) {
        timesync_config = 0;
        for (i = 0; i < MAX_PORTS_PER_PM4X25; i++) {
            _SOC_IF_ERR_EXIT(
                PM4x25_TIMESYNC_CONFIG_SET(unit, pm_info, timesync_config, i));
        }
    }
...
int pm4x25_pm_wb_debug_log (int unit, pm_info_t pm_info)
{
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "S$BEGIN:portmod_pm4x25:{port,timesync_config}\n")));
    for (i = 0 ; i < MAX_PORTS_PER_PM4X25; i++) {
        rv = PM4x25_TIMESYNC_CONFIG_GET(unit, pm_info, &timesync_config, i);
        if (SOC_E_NONE == rv) {
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "{%d,%d}\n"), i, timesync_config));
        }
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "S$END:portmod_pm4x25:{port,timesync_config}\n")));
...
int pm4x25_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
   ...
   _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    ...
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "timesync_enable",
                           wb_buffer_index, sizeof(uint32), NULL, MAX_PORTS_PER_PM4X25, VERSION(17));
    _SOC_IF_ERR_EXIT(rv);
    ...
    if(!SOC_WARM_BOOT(unit)){
       ...
       rv = SOC_WB_ENGINE_MEMSET_ARR(unit, SOC_WB_ENGINE_PORTMOD,
                              pm_info->wb_vars_ids[timesyncEnable], 0);
       _SOC_IF_ERR_EXIT(rv);
    }
...
pm4x25_default_fw_loader(const phymod_core_access_t* core, uint32_t length, const uint8_t* data)
{
    ...
    rv = portmod_firmware_set_access(unit,
                          ...
                          flr_ucode ? &(pm4x25_flr_ucode_buf[unit]) : &(pm4x25_ucode_buf[unit]),
                          flr_ucode ? &(pm4x25_flr_ucode_buf_2nd[unit]) : &(pm4x25_ucode_buf_2nd[unit]),
                          mCLPORT_WC_UCMEM_DATA,
                          rCLPORT_WC_UCMEM_CTRL_fACCESS_MODE);
...
#else
    return portmod_common_phy_sbus_reg_write_access(mCLPORT_WC_UCMEM_DATA, user_acc, core_addr, reg_addr, val);
#endif /* __UA_HAS_OLD_ACCESS */
...
#else
    return portmod_common_phy_sbus_reg_read_access(mCLPORT_WC_UCMEM_DATA, user_acc, core_addr, reg_addr, val);
#endif /* __UA_HAS_OLD_ACCESS */
...
int _pm4x25_port_soft_reset_set(int unit, int port, pm_info_t pm_info, int idx, int val)
{
    ...
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_SOFT_RESET, phy_acc, &reg_val));

    field_val = (uint32)val;
    UA_REG_FIELD_SET(unit, CLPORT_SOFT_RESET, &reg_val, PORT0, &field_val);
    UA_REG_FIELD_SET(unit, CLPORT_SOFT_RESET, &reg_val, PORT1, &field_val);
    UA_REG_FIELD_SET(unit, CLPORT_SOFT_RESET, &reg_val, PORT2, &field_val);
    UA_REG_FIELD_SET(unit, CLPORT_SOFT_RESET, &reg_val, PORT3, &field_val);

    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_SOFT_RESET, phy_acc, &reg_val));
...
int pm4x25_port_soft_reset_toggle(int unit, int port, pm_info_t pm_info, int idx)
{
    ...
    fld_val = 1;
    UA_REG_FIELD_SET(unit, CLPORT_SOFT_RESET, &reg_val, PORT0, &fld_val);
    UA_REG_FIELD_SET(unit, CLPORT_SOFT_RESET, &reg_val, PORT1, &fld_val);
    UA_REG_FIELD_SET(unit, CLPORT_SOFT_RESET, &reg_val, PORT2, &fld_val);
    UA_REG_FIELD_SET(unit, CLPORT_SOFT_RESET, &reg_val, PORT3, &fld_val);

    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_SOFT_RESET, phy_acc, &reg_val));
    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_SOFT_RESET, phy_acc, &old_val));
...
int pm4x25_port_tsc_reset_set(int unit, int phy_acc,
                              pm_info_t pm_info, int in_reset)
{
    uint32 reg_val;
    SOC_INIT_FUNC_DEFS;

    /* If PM4X25D disaptch the call the PM4x25D driver. */
#ifdef PORTMOD_PM4X25D_SUPPORT
    if (pm_info->type == portmodDispatchTypePm4x25d) {
        _SOC_IF_ERR_EXIT(pm4x25d_port_tsc_reset_set(unit, phy_acc,
                                                    pm_info, in_reset));
    } else
#endif /* PORTMOD_PM4X25D_SUPPORT */
    {
        ...
        soc_reg_field_set(unit, CLPORT_XGXS0_CTRL_REGr, &reg_val,
                          PWRDWNf, in_reset ? 1 : 0);
        soc_reg_field_set(unit, CLPORT_XGXS0_CTRL_REGr, &reg_val,
                          IDDQf, in_reset ? 1 : 0);
        _SOC_IF_ERR_EXIT(WRITE_CLPORT_XGXS0_CTRL_REGr(unit, phy_acc, reg_val));

        /* Based on the feedback from SJ pmd support team, ~10-15usecs would
         * be sufficient for the PLL/AFE to settle down out of IDDQ reset.
         */
        sal_usleep(10);
    }
...
int pm4x25_pm_tsc_refclock_set(int unit, int pm_id, pm_info_t pm_info, int ref_in, int ref_out)
{
    ...
    if (ref_out == 1)
    {
        _SOC_IF_ERR_EXIT(pm4x25_port_tsc_reset_set(unit, phy_acc, pm_info, 1));
        _SOC_IF_ERR_EXIT(pm4x25_port_tsc_reset_set(unit, phy_acc, pm_info, 0));
    }
...
{
    uint32 reg_val, field_value;

    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_MODE_REG, phy_acc, &reg_val));
    UA_REG_FIELD_GET(unit, CLPORT_MODE_REG, &reg_val, XPORT0_CORE_PORT_MODE, &field_value);

    *core_mode = field_value;
...
int pm4x25_port_cntmaxsize_get(int unit, int port, pm_info_t pm_info, int *val)
{
    ...
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_CNTMAXSIZE, phy_acc, val));
...
{
    uint32 reg_val, field_value = (uint32)val;
    int phy_acc;
    SOC_INIT_FUNC_DEFS;

    PM4x25_PHY_ACCESS_GET(unit, port, pm_info, phy_acc);

    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_CNTMAXSIZE, phy_acc, &reg_val));
    UA_REG_FIELD_SET(unit, CLPORT_CNTMAXSIZE, &reg_val, CNTMAXSIZE, &field_value);

    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_CNTMAXSIZE, phy_acc, &reg_val));
...
int pm4x25_port_higig2_mode_set (int unit, int port, pm_info_t pm_info, int mode)
{
    ...
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_CONFIG, phy_acc, &rval));
    fld_val = (uint32) mode;
    UA_REG_FIELD_SET(unit, CLPORT_CONFIG, &rval, HIGIG2_MODE, &fld_val);
    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_CONFIG, phy_acc, &rval));
...
{
    int phy_acc;
    uint32_t rval, field_value;
    SOC_INIT_FUNC_DEFS;

    PM4x25_PHY_ACCESS_GET(unit, port, pm_info, phy_acc);

    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_CONFIG, phy_acc, &rval));
    UA_REG_FIELD_GET(unit, CLPORT_CONFIG, &rval, HIGIG2_MODE, &field_value);
    *(mode) = field_value;
...
int pm4x25_port_higig_mode_set (int unit, int port, pm_info_t pm_info, int mode)
{
    ...
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_CONFIG, phy_acc, &rval));
    fld_val = (uint32)mode;
    UA_REG_FIELD_SET(unit, CLPORT_CONFIG, &rval, HIGIG_MODE, &fld_val);
    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_CONFIG, phy_acc, &rval));
...
int pm4x25_port_higig_mode_get (int unit, int port, pm_info_t pm_info, int *mode)
{
    ...
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_CONFIG, phy_acc, &rval));
    UA_REG_FIELD_GET(unit, CLPORT_CONFIG, &rval, HIGIG_MODE, &field_value);
    *(mode) = field_value;
...
int pm4x25_port_led_chain_config (int unit, int port, pm_info_t pm, int value)
{
    ...
    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_LED_CHAIN_CONFIG, phy_acc, &value));
...
{
    uint32 reg_val, field_val, port_mode;
    uint32 caui_mode, three_ports_mode;
    int  rv, phy_acc;
    int pm_ver_is_d0 = 0;
    ...
    rv = PM4X25_REG_READ(unit, CLPORT_GENERAL_SPARE1_REG, port, &reg_val);
    if(rv == SOC_E_NONE) {
        pm_ver_is_d0 = 1;
...
#endif /* PORTMOD_PM4X25TD_SUPPORT */
    {
        is_master_pll = pm4x25_port_tsc_is_master_pll(unit, port, pm_info);

        _SOC_IF_ERR_EXIT(_pm4x25_pm_version_get(unit, pm_info, &pm_version));

        if(enable) {
            /* Power Save */
            _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_POWER_SAVE, phy_acc, &reg_val));
            field_val = 0;
            UA_REG_FIELD_SET(unit, CLPORT_POWER_SAVE, &reg_val, XPORT_CORE0, &field_val);
            _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_POWER_SAVE, phy_acc, &reg_val));
            ...
            } else if (UA_REG_FIELD_IS_VALID(unit, CLPORT_MODE_REG, SINGLE_PORT_MODE_SPEED_100G)) {
                UA_REG_FIELD_SET(unit, CLPORT_MODE_REG, &reg_val, SINGLE_PORT_MODE_SPEED_100G, &caui_mode);
            }
            UA_REG_FIELD_SET(unit, CLPORT_MODE_REG, &reg_val, XPORT0_CORE_PORT_MODE, &port_mode);
            UA_REG_FIELD_SET(unit, CLPORT_MODE_REG, &reg_val, XPORT0_PHY_PORT_MODE,  &port_mode);
            field_val = add_info->flags & PORTMOD_PORT_ADD_F_EGR_1588_TIMESTAMP_MODE_48BIT ? 1 : 0;
            UA_REG_FIELD_SET(unit, CLPORT_MODE_REG, &reg_val, EGR_1588_TIMESTAMPING_MODE, &field_val);
            _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_MODE_REG, phy_acc, &reg_val));
            ...
            if (PM_4x25_INFO(pm_info)->nof_phys[0] > 0) { 
                ...
                if(is_master_pll != TRUE) {  /* This PM is not master PM, reset it. */
                    _SOC_IF_ERR_EXIT(pm4x25_port_tsc_reset_set(unit, phy_acc,
                                                               pm_info, 1));
                    _SOC_IF_ERR_EXIT(pm4x25_port_tsc_reset_set(unit, phy_acc,
                                                               pm_info, 0));
                } else {
                    ...
                    /*Hard reset tsc core*/
                    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_XGXS0_CTRL_REG, phy_acc, &port_ctrl_reg_val));
                    field_val = 0;
                    UA_REG_FIELD_SET(unit, CLPORT_XGXS0_CTRL_REG, &port_ctrl_reg_val, RSTB_HW, &field_val);
                    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_XGXS0_CTRL_REG, phy_acc, &port_ctrl_reg_val));
            ...
            /* Bring MAC OOR */
            _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_MAC_CONTROL, phy_acc, &reg_val));
            field_val = 0;
            UA_REG_FIELD_SET(unit, CLPORT_MAC_CONTROL, &reg_val, XMAC0_RESET, &field_val);
            if (PORTMOD_PORT_ADD_F_SYS_16B_INTF_MODE_GET(add_info)) {
                if (UA_REG_FIELD_IS_VALID(unit, CLPORT_MAC_CONTROL,SYS_16B_INTF_MODE)) {
                    field_val = 1;
                    UA_REG_FIELD_SET(unit, CLPORT_MAC_CONTROL, &reg_val, SYS_16B_INTF_MODE, &field_val);
                }
                _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_MAC_CONTROL, phy_acc, &reg_val));
            }
            _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_MAC_CONTROL, phy_acc, &reg_val));
        } else { /* disable */
            /* put MAC in reset */
...
{
    int rv, phy_acc;
    uint32 reg_val, field_val, flags;
...
#endif /* PORTMOD_PM4X25TD_SUPPORT */
    {
        ...
        if(enable) {
            ...
            rsv_mask = 0;
            rsv_pfc_mask = 0;
            _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_MAC_RSV_MASK, phy_acc, &rsv_pfc_mask));
            ...
            _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_MAC_RSV_MASK, phy_acc, &rsv_mask));

            switch (internal_port) {
                case 3:
                    /* Soft reset */
                    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_SOFT_RESET, phy_acc, &reg_val));
                    field_val = 1;
                    ...
                    /* Port enable */
                    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_ENABLE_REG, phy_acc, &reg_val));
                    field_val = 1;
                    UA_REG_FIELD_SET(unit, CLPORT_ENABLE_REG, &reg_val, PORT3, &field_val);
                    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_ENABLE_REG, phy_acc, &reg_val));
                    break;
                case 2:
                    /* Soft reset */
                    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_SOFT_RESET, phy_acc, &reg_val));
                    ...
                    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_ENABLE_REG, phy_acc, &reg_val));
                    field_val = 1;
                    UA_REG_FIELD_SET(unit, CLPORT_ENABLE_REG, &reg_val, PORT2, &field_val);
                    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_ENABLE_REG, phy_acc, &reg_val));
                    break;
                case 1:
                    /* Soft reset */
                    ...
                    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_ENABLE_REG, phy_acc, &reg_val));
                    field_val = 1;
                    UA_REG_FIELD_SET(unit, CLPORT_ENABLE_REG, &reg_val, PORT1, &field_val);
                    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_ENABLE_REG, phy_acc, &reg_val));
                    break;
                case 0:
                default:
                    ...
                    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_ENABLE_REG, phy_acc, &reg_val));
                    field_val = 1;
                    UA_REG_FIELD_SET(unit, CLPORT_ENABLE_REG, &reg_val, PORT0, &field_val);
                    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_ENABLE_REG, phy_acc, &reg_val));
                    break;
            }
            ...
            /* LSS */
            _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_FAULT_LINK_STATUS, phy_acc, &reg_val));
            field_val = 1;
            UA_REG_FIELD_SET(unit, CLPORT_FAULT_LINK_STATUS, &reg_val, REMOTE_FAULT, &field_val);
            field_val = 1;
            UA_REG_FIELD_SET(unit, CLPORT_FAULT_LINK_STATUS, &reg_val, LOCAL_FAULT, &field_val);
            _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_FAULT_LINK_STATUS, phy_acc, &reg_val)); 

            /* Counter MAX size */ 
            _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_CNTMAXSIZE, phy_acc, &reg_val));
            field_val = 1518;
            UA_REG_FIELD_SET(unit, CLPORT_CNTMAXSIZE, &reg_val, CNTMAXSIZE, &field_val);
            _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_CNTMAXSIZE, phy_acc, &reg_val));
            
            /* Reset MIB counters */ 
            _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_MIB_RESET, phy_acc, &reg_val));
            /* coverity[ptr_arith:FALSE] */
            SHR_BITSET(&reg_val, internal_port);
            _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_MIB_RESET, phy_acc, &reg_val));
...
int pm4x25_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    /* Update WB DB */
    rv = PM4x25_TIMESYNC_CONFIG_SET(unit, pm_info, timesync_config_default, port_index);
    _SOC_IF_ERR_EXIT(rv);
                                     ...
                                     int phy_init_flags)
{
    ...
    if (UA_REG_FIELD_IS_VALID(unit, CLPORT_MAC_CONTROL, CLMAC_TS_DISABLE)) {
        _SOC_IF_ERR_EXIT
...
{
    int first_index, rv;
    uint32 reg32_val, field_val, bitmap;
    SOC_INIT_FUNC_DEFS;
    ...
    *link = 0;
    ...
    rv = PM4X25_REG_READ(unit, CLPORT_LINKSTATUS_DOWN, port, &reg32_val);
    _SOC_IF_ERR_EXIT(rv);

    switch(first_index) {
        case 3:
            UA_REG_FIELD_GET(unit, CLPORT_LINKSTATUS_DOWN, &reg32_val, PORT3_LINKSTATUS, &field_val);
            break;
        case 2:
            UA_REG_FIELD_GET(unit, CLPORT_LINKSTATUS_DOWN, &reg32_val, PORT2_LINKSTATUS, &field_val);
            break;
        case 1:
            UA_REG_FIELD_GET(unit, CLPORT_LINKSTATUS_DOWN, &reg32_val, PORT1_LINKSTATUS, &field_val);
            break;
        case 0:
            UA_REG_FIELD_GET(unit, CLPORT_LINKSTATUS_DOWN, &reg32_val, PORT0_LINKSTATUS, &field_val);
            break;
        default:
            _SOC_EXIT_WITH_ERR(SOC_E_INTERNAL,
    ...
    if (PORTMOD_PORT_LINK_LATCH_DOWN_F_CLEAR & flags) {
        switch (first_index) {
            case 3:
                rv = PM4X25_REG_READ(unit, CLPORT_LINKSTATUS_DOWN_CLEAR, port, &reg32_val);
                _SOC_IF_ERR_EXIT(rv);

                field_val = 1;
                UA_REG_FIELD_SET(unit, CLPORT_LINKSTATUS_DOWN_CLEAR, &reg32_val, PORT3_LINKDOWN_CLEAR, &field_val);

                rv = PM4X25_REG_WRITE(unit, CLPORT_LINKSTATUS_DOWN_CLEAR, port, &reg32_val);
                _SOC_IF_ERR_EXIT(rv);
            ...
            case 2:
                ...
                field_val = 1;
                UA_REG_FIELD_SET(unit, CLPORT_LINKSTATUS_DOWN_CLEAR, &reg32_val, PORT2_LINKDOWN_CLEAR, &field_val);
            ...
            case 1:
                ...
                field_val = 1;
                UA_REG_FIELD_SET(unit, CLPORT_LINKSTATUS_DOWN_CLEAR, &reg32_val, PORT1_LINKDOWN_CLEAR, &field_val);
            ...
            default:
                ...
                field_val = 1;
                UA_REG_FIELD_SET(unit, CLPORT_LINKSTATUS_DOWN_CLEAR, &reg32_val, PORT0_LINKDOWN_CLEAR, &field_val);
...
/* 1. timesync adjustment for 1588. */
int _pm4x25_port_phy_timesync_link_up_handler (int unit,
                                               int port,
                                               pm_info_t pm_info)
{
    portmod_access_get_params_t params;
    ...
    uint32 timesync_config, bitmap = 0;
    ...
    _SOC_IF_ERR_EXIT
        (pm4x25_port_phy_lane_access_get(unit, port, pm_info,
                                &params, 1, &phy_access, &nof_phys, NULL));
    _SOC_IF_ERR_EXIT
        (_pm4x25_port_index_get(unit, port, pm_info, &port_index, &bitmap));


    /* Adjust timesync settings for 1588 port. */
    _SOC_IF_ERR_EXIT
        (PM4x25_TIMESYNC_CONFIG_GET(unit, pm_info, &timesync_config, port_index));
    ...
    if (PORTMOD_PORT_TIMESYNC_CONFIG_ENABLE_GET(timesync_config)) {
        ...
        if (PORTMOD_PORT_TIMESYNC_CONFIG_COMP_MODE_ON_GET(timesync_config)) {
            ...
            timesync_adjust_config.am_norm_mode = phymodTimesyncCompensationModeNone;
        }
...
int pm4x25_port_phy_flr_link_up_event(int unit, int port, pm_info_t pm_info)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (_pm4x25_port_phy_timesync_link_up_handler(unit, port, pm_info));
...
{
    uint32 reg_val, field_val, bitmap;
    ...
    switch(port_index) {
        case 0:
            _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_XGXS0_LN0_STATUS0_REG, phy_acc, &reg_val));
            UA_REG_FIELD_GET(unit, CLPORT_XGXS0_LN0_STATUS0_REG, &reg_val, LINK_STATUS, &field_val);
            (*link) = field_val;
            break;

        case 1:
            _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_XGXS0_LN1_STATUS0_REG, phy_acc, &reg_val));
            UA_REG_FIELD_GET(unit, CLPORT_XGXS0_LN1_STATUS0_REG, &reg_val, LINK_STATUS, &field_val);
            (*link) = field_val;
            break;

        case 2:
            _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_XGXS0_LN2_STATUS0_REG, phy_acc, &reg_val));
            UA_REG_FIELD_GET(unit, CLPORT_XGXS0_LN2_STATUS0_REG, &reg_val, LINK_STATUS, &field_val);
            (*link) = field_val;
            break;

        case 3:
            _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_XGXS0_LN3_STATUS0_REG, phy_acc, &reg_val));
            UA_REG_FIELD_GET(unit, CLPORT_XGXS0_LN3_STATUS0_REG, &reg_val, LINK_STATUS, &field_val);
            (*link) = field_val;
            break;
...
int _clport_pfc_config_set(int unit, int port, uint32 value)
{
    ...
    if (UA_REG_IS_VALID(unit, CLPORT_MAC_RSV_MASK)) {
        rv = PM4X25_REG_READ(unit, CLPORT_MAC_RSV_MASK, port, &rval32);
        if (SOC_SUCCESS(rv)) {
            ...
            rv = PM4X25_REG_WRITE(unit, CLPORT_MAC_RSV_MASK, port, &rval32);
        }
    }
                                ...
                                const portmod_rx_control_t* rx_ctrl)
{
    ...
    PM4x25_PHY_ACCESS_GET(unit, port, pm_info, phy_acc);
    SOC_IF_ERROR_RETURN(PM4X25_REG_READ(unit, CLPORT_MAC_RSV_MASK, phy_acc, &reg_val));
    ...
    SOC_IF_ERROR_RETURN(PM4X25_REG_WRITE(unit, CLPORT_MAC_RSV_MASK, phy_acc, &reg_val));
...
int pm4x25_port_lag_failover_status_toggle (int unit, soc_port_t port, pm_info_t pm_info)
{
    ...
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_LAG_FAILOVER_CONFIG, phy_acc, &rval));
    field_val = 1;
    UA_REG_FIELD_SET(unit, CLPORT_LAG_FAILOVER_CONFIG, &rval, LINK_STATUS_UP, &field_val);
    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_LAG_FAILOVER_CONFIG, phy_acc, &rval));
    field_val = 0;
    UA_REG_FIELD_SET(unit, CLPORT_LAG_FAILOVER_CONFIG, &rval, LINK_STATUS_UP, &field_val);
    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_LAG_FAILOVER_CONFIG, phy_acc, &rval));
...
static int _clport_mode_set(int unit, int phy_acc, portmod_core_port_mode_t cur_mode)
{
    ...
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_MODE_REG, phy_acc, &rval));

    UA_REG_FIELD_SET(unit, CLPORT_MODE_REG, &rval, XPORT0_CORE_PORT_MODE, &mode);
    UA_REG_FIELD_SET(unit, CLPORT_MODE_REG, &rval, XPORT0_PHY_PORT_MODE, &mode);

    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_MODE_REG, phy_acc, &rval));
    ...
    if(in_out == 0) {
	    ...
	    /* Callback soft reset function */
        	...
        	rv = PM_4x25_INFO(pm_info)->portmod_mac_soft_reset(unit, port, portmodCallBackActionTypeDuring);
                ...
                UA_REG_FIELD_SET(unit, CLMAC_CTRL, &mac_ctrl, SOFT_RESET, &field_val); /*make sure restored data wont put mac back in reset*/
    	    }
       ...
       case 2:
        ...
        default:
            LOG_ERROR(BSL_LS_SOC_PORT,
              (BSL_META_U(unit, "error port idx %d port %d\n"), idx, port));
            return SOC_E_PARAM;
    }
                              ...
                              const portmod_eee_clock_t* eee_clk)
{
    ...
    fld_val = eee_clk->clock_gate;
    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_EEE_CLOCK_GATE, phy_acc, &fld_val));
    fld_val = eee_clk->clock_count;
    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_EEE_CORE0_CLOCK_GATE_COUNTER, phy_acc, &fld_val));
    fld_val = eee_clk->timer_pulse;
    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_EEE_DURATION_TIMER_PULSE, phy_acc, &fld_val));
                               ...
                               portmod_eee_clock_t* eee_clk)
{
    ...
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_EEE_CLOCK_GATE, phy_acc, &eee_clk->clock_gate));
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_EEE_CORE0_CLOCK_GATE_COUNTER, phy_acc, &eee_clk->clock_count));
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_EEE_DURATION_TIMER_PULSE, phy_acc, &eee_clk->timer_pulse));
                                  ...
                                  int merge_mode_en, int parallel_fc_en)
{
    ...
    rv = PM4X25_REG_READ(unit, CLPORT_FLOW_CONTROL_CONFIG, phy_acc, &rval);
    if (SOC_FAILURE(rv)) return (rv);

    fld_val = (uint32)merge_mode_en;
    UA_REG_FIELD_SET(unit, CLPORT_FLOW_CONTROL_CONFIG, &rval, MERGE_MODE_EN, &fld_val);
    fld_val = (uint32)parallel_fc_en;
    UA_REG_FIELD_SET(unit, CLPORT_FLOW_CONTROL_CONFIG, &rval, PARALLEL_FC_EN, &fld_val);

    return(PM4X25_REG_WRITE(unit, CLPORT_FLOW_CONTROL_CONFIG, phy_acc, &rval));
}
...
int pm4x25_port_mac_rsv_mask_set(int unit, int port, pm_info_t pm_info, uint32 rsv_mask)
{
    ...
    return(PM4X25_REG_WRITE(unit, CLPORT_MAC_RSV_MASK, port, &rsv_mask));
}
...
int pm4x25_port_mac_rsv_mask_get(int unit, int port, pm_info_t pm_info, uint32* rsv_mask)
{
    ...
    int i, rv;
    rv = PM4X25_REG_READ(unit, CLPORT_MAC_RSV_MASK, port, &clmac_rsv_mask);
...
int pm4x25_port_mib_reset_toggle(int unit, int port, pm_info_t pm_info, int port_index)
{
    ...
    rv = PM4X25_REG_READ(unit, CLPORT_MIB_RESET, phy_acc, reg_val);
    if (SOC_FAILURE(rv)) return (rv);
    SHR_BITSET(reg_val, port_index);

    rv = PM4X25_REG_WRITE(unit, CLPORT_MIB_RESET, phy_acc, reg_val);
    if (SOC_FAILURE(rv)) return (rv);
    SHR_BITCLR(reg_val, port_index);

    return(PM4X25_REG_WRITE(unit, CLPORT_MIB_RESET, phy_acc, reg_val));
}
...
{
    phymod_phy_access_t phy_access[1+MAX_PHYN];
    int nof_phys, enable = 0, port_index =0;
    ...
    _SOC_IF_ERR_EXIT(PM4x25_TIMESYNC_CONFIG_SET(unit, pm_info, timesync_config, port_index));
...
int pm4x25_port_control_phy_timesync_set(int unit, int port, pm_info_t pm_info, const portmod_port_control_phy_timesync_t config, uint64 value)
{
    ...
    _SOC_IF_ERR_EXIT(portmod_port_chain_phy_access_get(unit, port, pm_info,
                                                       ...
                                                       &nof_phys));
    _SOC_IF_ERR_EXIT(_pm4x25_port_index_get(unit, port, pm_info, &port_index, &bitmap));
    _SOC_IF_ERR_EXIT(PM4x25_TIMESYNC_CONFIG_GET(unit, pm_info, &timesync_config, port_index));
    ...
    if (config == SOC_PORT_CONTROL_PHY_TIMESYNC_ONE_STEP_ENABLE) {
        if (val_lo) {
            PORTMOD_PORT_TIMESYNC_CONFIG_ENABLE_SET(timesync_config);
        } else {
            PORTMOD_PORT_TIMESYNC_CONFIG_ENABLE_CLR(timesync_config);
        }
    } else if  (config == SOC_PORT_CONTROL_PHY_TIMESYNC_TIMESTAMP_ADJUST) {
        if (val_lo == SOC_PORT_PHY_TIMESYNC_COMPENSATION_MODE_LATEST_LANE) {
        ...
        } else if (val_lo == SOC_PORT_PHY_TIMESYNC_COMPENSATION_MODE_EARLIEST_LANE) {
            PORTMOD_PORT_TIMESYNC_CONFIG_COMP_MODE_ON_SET(timesync_config);
                                        ...
                                        uint32 frag, uint32 value)
{
    ...
    _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_MIB_MIN_FRAG_SIZE, phy_acc, &rval));

    if (frag == PORTMOD_PREEMPTION_FINAL_FRAG) {
        UA_REG_FIELD_SET(unit, CLPORT_MIB_MIN_FRAG_SIZE, &rval, MIN_FINAL_FRAG_SIZE, &value);
    } else {
        UA_REG_FIELD_SET(unit, CLPORT_MIB_MIN_FRAG_SIZE, &rval, MIN_NON_FINAL_FRAG_SIZE, &value);
    }

    _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_MIB_MIN_FRAG_SIZE, phy_acc, &rval));
                                        ...
                                        uint32 frag, uint32 *value)
{
    ...
    if (frag == PORTMOD_PREEMPTION_FINAL_FRAG) {
        UA_REG_FIELD_GET(unit, CLPORT_MIB_MIN_FRAG_SIZE, &rval, MIN_FINAL_FRAG_SIZE, value);
    } else {
        UA_REG_FIELD_GET(unit, CLPORT_MIB_MIN_FRAG_SIZE, &rval, MIN_NON_FINAL_FRAG_SIZE, value);
    }
...
int pm4x25_port_timesync_tx_info_get(int unit, int port, pm_info_t pm_info, portmod_fifo_status_t* tx_info)
{
    ...
    if (UA_REG_FIELD_IS_VALID(unit, CLPORT_MAC_CONTROL, CLMAC_TS_DISABLE)) {
        _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_MAC_CONTROL, phy_acc, &reg_val));
        UA_REG_FIELD_GET(unit, CLPORT_MAC_CONTROL, &reg_val, CLMAC_TS_DISABLE, &mac_ts_disable);
    }
...
_pm4x25_sbus_control_set(int unit, const phymod_core_access_t* core_acc, int bcast_id, int chain_last)
{
    ...
    SOC_IF_ERROR_RETURN(PM4X25_REG_READ(unit, CLPORT_SBUS_CONTROL, reg_acc_idx, &reg_val));
    field_val = (uint32)bcast_id;
    UA_REG_FIELD_SET(unit, CLPORT_SBUS_CONTROL, &reg_val, SBUS_BCAST_BLOCK_ID, &field_val);
    field_val = (uint32)chain_last;
    UA_REG_FIELD_SET(unit, CLPORT_SBUS_CONTROL, &reg_val, SBUS_CHAIN_LAST, &field_val);
    SOC_IF_ERROR_RETURN(PM4X25_REG_WRITE(unit, CLPORT_SBUS_CONTROL, reg_acc_idx, &reg_val));
...
_pm4x25_tsc_acc_mode_set(int unit, const phymod_core_access_t* core_acc, int acc_mem)
{
    ...
    SOC_IF_ERROR_RETURN(PM4X25_REG_READ(unit, CLPORT_WC_UCMEM_CTRL, reg_acc_idx, &reg_val));
    field_val = (uint32)acc_mem;
    UA_REG_FIELD_SET(unit, CLPORT_WC_UCMEM_CTRL, &reg_val, ACCESS_MODE, &field_val);
    SOC_IF_ERROR_RETURN(PM4X25_REG_WRITE(unit, CLPORT_WC_UCMEM_CTRL, reg_acc_idx, &reg_val));
...
int pm4x25_port_mac_timestamp_enable_set(int unit, int port, pm_info_t pm_info, int enable)
{
    ...
    PM4x25_PHY_ACCESS_GET(unit, port, pm_info, phy_acc);
    if (UA_REG_FIELD_IS_VALID(unit, CLPORT_MAC_CONTROL, CLMAC_TS_DISABLE)) {
        SOC_IF_ERROR_RETURN(PM4X25_REG_READ(unit, CLPORT_MAC_CONTROL, phy_acc, &reg_val));
        field_val = enable ? 0 : 1;
        UA_REG_FIELD_SET(unit, CLPORT_MAC_CONTROL, &reg_val, CLMAC_TS_DISABLE, &field_val);
        SOC_IF_ERROR_RETURN(PM4X25_REG_WRITE(unit, CLPORT_MAC_CONTROL, phy_acc, &reg_val));
    } else {
        return SOC_E_UNAVAIL;
    }
...
int pm4x25_port_mac_timestamp_enable_get(int unit, int port, pm_info_t pm_info, int* enable)
{
    ...
    if (UA_REG_FIELD_IS_VALID(unit, CLPORT_MAC_CONTROL, CLMAC_TS_DISABLE)) {
        SOC_IF_ERROR_RETURN(PM4X25_REG_READ(unit, CLPORT_MAC_CONTROL, phy_acc, &reg_val));
        UA_REG_FIELD_GET(unit, CLPORT_MAC_CONTROL, &reg_val, CLMAC_TS_DISABLE, &field_val);
    } else {
        return SOC_E_UNAVAIL;
    }
...
{
    uint32 reg_val, field_val, timestamp_mode;
    int phy_acc;
#ifdef PORTMOD_PM4X25TD_SUPPORT
    int rv;
#endif /* PORTMOD_PM4X25TD_SUPPORT */
...
#endif /* PORTMOD_PM4X25TD_SUPPORT */
    {
        ...
        field_val = (uint32)timestamp_mode;
        UA_REG_FIELD_SET(unit, CLPORT_MODE_REG, &reg_val, EGR_1588_TIMESTAMPING_MODE, &field_val);
        field_val = timestamp_config.cmic_48_overr_en ? 1 : 0;
        UA_REG_FIELD_SET(unit, CLPORT_MODE_REG, &reg_val, EGR_1588_TIMESTAMPING_CMIC_48_EN, &field_val);
        _SOC_IF_ERR_EXIT(PM4X25_REG_WRITE(unit, CLPORT_MODE_REG, phy_acc, &reg_val));
    }
exit:
    SOC_FUNC_RETURN;
                         ...
                         portmod_egr_1588_timestamp_config_t* timestamp_config)
{
...
#endif /* PORTMOD_PM4X25TD_SUPPORT */
    {
        ...
        _SOC_IF_ERR_EXIT(PM4X25_REG_READ(unit, CLPORT_MODE_REG, phy_acc, &reg_val));
        UA_REG_FIELD_GET(unit, CLPORT_MODE_REG, &reg_val, EGR_1588_TIMESTAMPING_MODE, &timestamp_mode);
        ...
        UA_REG_FIELD_GET(unit, CLPORT_MODE_REG, &reg_val, EGR_1588_TIMESTAMPING_CMIC_48_EN, &field_val);
        timestamp_config->cmic_48_overr_en = field_val;
    }
exit:
    SOC_FUNC_RETURN;
}
