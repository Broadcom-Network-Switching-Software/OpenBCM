...
int pm4x10_pm_wb_debug_log (int unit, pm_info_t pm_info)
{
    ...
    if (SOC_E_NONE == rv) {
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "{%d}\n"), pm_is_initialized));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "S$END:portmod_pm4x10:{is_initialized}\n")));
    }
    ...
    for (lane_index = 0, user_cfg_index = start_cfg_index, phy_param_index = 0; lane_index < PHYMOD_MAX_LANES_PER_PORT; lane_index++) {
        if (lane_mask & (0x1<<lane_index)) {
            ...
            if (config_flag[user_cfg_index] & PORTMOD_USER_SET_TX_PREEMPHASIS_BY_CONFIG) {
                ...
                phy_tx_params[phy_param_index].post2 = user_tx_params[user_cfg_index].post2;
                phy_tx_params[phy_param_index].post3 = user_tx_params[user_cfg_index].post3;
        }
                        ...
                        phymod_tx_t ext_phy_tx_params[])
{
    ...
    {   /* external phy. */
        ...
        if (tx_params_update) {
            ...
            _SOC_IF_ERR_EXIT(PM4x10_PORT_DYNAMIC_STATE_SET(unit, pm_info,
                                                           port_dynamic_state,
                                                           port_index));
        }
    }
                                 ...
                                 portmod_port_ability_t* ability)
{
    ...
    int     nof_phys = 0, port_index, flags = 0;
    uint32  bitmap;
    int     rv;
...
int pm4x10_port_lane_map_get(int unit, int port, pm_info_t pm_info,
                             uint32 flags, phymod_lane_map_t* lane_map)
{
     ...
     */
    rv = phymod_core_lane_map_get(&PM_4x10_INFO(pm_info)->int_core_access,
                                  lane_map);
...
int pm4x10_port_faults_status_get(int unit, int port, pm_info_t pm_info,
                                  portmod_port_fault_status_t *faults)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT(xlmac_faults_status_get(unit, port, faults));
exit:
    SOC_FUNC_RETURN;
}
...
int pm4x10_port_pmd_rx_lock_status_get(int unit, int port, pm_info_t pm_info, uint32* pmd_lock, uint32* pmd_lock_change)
{
    ...
    _SOC_IF_ERR_EXIT
        (phymod_phy_rx_pmd_lock_status_get(&phy_access[0], &rx_pmd_locked, &rx_pmd_lock_change));
