...
#define PM8x50_WB_BUFFER_VERSION        (10)
...
#define PM8x50_TIMESYNC_CONFIG_SET(unit, pm_info, timesync_config, port_index) \
    SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD,                       \
                          pm_info->wb_vars_ids[timesyncEnable], &timesync_config, port_index)
#define PM8x50_TIMESYNC_CONFIG_GET(unit, pm_info, timesync_config, port_index) \
    SOC_WB_ENGINE_GET_ARR(unit, SOC_WB_ENGINE_PORTMOD,                       \
                          pm_info->wb_vars_ids[timesyncEnable], timesync_config, port_index)
...
#define PM8x50_IS_100G_2LANE_SW_WAR_MPP0_DONE_SET(unit, pm_info, is_100g_2lane_sw_war_mpp0_done) \
    SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD,                     \
                          pm_info->wb_vars_ids[is100g2LaneWarMPP0Done],         \
                          &is_100g_2lane_sw_war_mpp0_done)
#define PM8x50_IS_100G_2LANE_SW_WAR_MPP0_DONE_GET(unit, pm_info, is_100g_2lane_sw_war_mpp0_done) \
    SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD,                     \
                          pm_info->wb_vars_ids[is100g2LaneWarMPP0Done],         \
                          is_100g_2lane_sw_war_mpp0_done)

#define PM8x50_IS_100G_2LANE_SW_WAR_MPP1_DONE_SET(unit, pm_info, is_100g_2lane_sw_war_mpp1_done) \
    SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD,                     \
                          pm_info->wb_vars_ids[is100g2LaneWarMPP1Done],         \
                          &is_100g_2lane_sw_war_mpp1_done)
#define PM8x50_IS_100G_2LANE_SW_WAR_MPP1_DONE_GET(unit, pm_info, is_100g_2lane_sw_war_mpp1_done) \
    SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD,                     \
                          pm_info->wb_vars_ids[is100g2LaneWarMPP1Done],         \
                          is_100g_2lane_sw_war_mpp1_done)
#define PM8x50_AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4_SET(unit, pm_info, link_fail_wait_time) \
    SOC_WB_ENGINE_SET_VAR(unit, SOC_WB_ENGINE_PORTMOD,                     \
                          pm_info->wb_vars_ids[anLinkfailInhibitTimerLtPam4],         \
                          &link_fail_wait_time)
#define PM8x50_AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4_GET(unit, pm_info, link_fail_wait_time) \
    SOC_WB_ENGINE_GET_VAR(unit, SOC_WB_ENGINE_PORTMOD,                     \
                          pm_info->wb_vars_ids[anLinkfailInhibitTimerLtPam4],         \
                          link_fail_wait_time)

#define PM8x50_PORT_LINK_RECOVERY_ENABLE_SET(unit, pm_info, link_recovery_enable, port_index) \
    SOC_WB_ENGINE_SET_ARR(unit, SOC_WB_ENGINE_PORTMOD,                       \
                          pm_info->wb_vars_ids[linkRecoveryEnable], &link_recovery_enable, port_index)
#define PM8x50_PORT_LINK_RECOVERY_ENABLE_GET(unit, pm_info, link_recovery_enable, port_index) \
    SOC_WB_ENGINE_GET_ARR(unit, SOC_WB_ENGINE_PORTMOD,                       \
                          pm_info->wb_vars_ids[linkRecoveryEnable], link_recovery_enable, port_index)
...
typedef enum pm8x50_wb_vars{
    ...
    is100g2LaneWarMPP0Done,  /*100g 2 lane war on MPP0*/
    is100g2LaneWarMPP1Done,  /*100g 2 lane war on MPP1*/
    anLinkfailInhibitTimerLtPam4, /*an link fail inhibit timer for PMA4 with link training*/
    linkRecoveryEnable /* link recovery war */
}pm8x50_wb_vars_t;
...
/*! Register address for pmd_x4_status. */
#define PMD_X4_STATUSr_REG 0xc012
/*! MASK for signal_detect_sts. */
#define PMD_X4_STATUSr_SIGNAL_DETECT_STS_MASK 0x1
/*! Offset for signal_detect_sts. */
#define PMD_X4_STATUSr_SIGNAL_DETECT_STS_OFFSET 0x1
...
STATIC portmod_ucode_buf_t pm8x50_flr_ucode_buf[SOC_MAX_NUM_DEVICES] = {{NULL, 0}};
STATIC portmod_ucode_buf_t pm8x50_flr_ucode_buf_2nd[SOC_MAX_NUM_DEVICES] = {{NULL, 0}};
    ...
    *is_done = 0;
    ...
    switch (an_state) {
       ...
       case PM8X50_PORT_AN_DISABLE:
       {
            ...
            /* before we disable AN, check if PCS link is up */
            /* first check link status */
            _SOC_IF_ERR_EXIT(phymod_phy_link_status_get(&phy_access, (uint32_t *)&link));
            _SOC_IF_ERR_EXIT(phymod_phy_link_status_get(&phy_access, (uint32_t *)&link));
       ...
       case PM8X50_PORT_AN_RESTART:
       {
            ...
            if (link && (!remote_fault)) {
                ...
                _SOC_IF_ERR_EXIT(_pm8x50_pm_port_mode_update(unit, port, pm_info, 0, 4));
        ...
        case PM8X50_PORT_AN_COMPLETE:
        {
            ...
            /* need to check if no link or remote fault */
            _SOC_IF_ERR_EXIT(phymod_phy_link_status_get(&phy_access, (uint32_t *)&link));
            _SOC_IF_ERR_EXIT(phymod_phy_link_status_get(&phy_access, (uint32_t *)&link));
...
pm8x50_default_fw_loader(const phymod_core_access_t* core, uint32_t length, const uint8_t* data)
{
    ...
    flr_ucode = PORTMOD_PORT_ADD_F_FLR_SUPPORT_GET(user_data) ? TRUE : FALSE;
    ...
    rv = portmod_firmware_set(unit,
                              ...
                              load_order,
                              bcast_en,
                              flr_ucode ? &(pm8x50_flr_ucode_buf[unit]) : &(pm8x50_ucode_buf[unit]),
                              flr_ucode ? &(pm8x50_flr_ucode_buf_2nd[unit]) : &(pm8x50_ucode_buf_2nd[unit]),
                                   ...
                                   int new_version)
{
    ...
    uint32 fec_null, timesync_config;
    ...
    int is_100g_2ln_sw_war_mpp0_done;
    int is_100g_2ln_sw_war_mpp1_done;
    int link_fail_wait_time;
    int link_recovery_enable;
    ...
    if ((recovered_version <= 1) &&
        ...
        for (i = 0; i < MAX_PORTS_PER_PM8X50; i++) {
            ...
            _SOC_IF_ERR_EXIT(
                PM8x50_TIMESYNC_CONFIG_SET(unit, pm_info, timesync_config, i));
        }
    }
   if ((recovered_version <= 7) &&
        ...
        for (i = 0; i < MAX_PORTS_PER_PM8X50; i++) {
            _SOC_IF_ERR_EXIT(
                PM8x50_TIMESYNC_CONFIG_GET(unit, pm_info, &timesync_config, i));
            if (timesync_config) {
    ...
    if ((recovered_version <= 6) &&
        (new_version >= 7)) {
        is_100g_2ln_sw_war_mpp0_done = 0;
        is_100g_2ln_sw_war_mpp1_done = 0;
        _SOC_IF_ERR_EXIT(
            PM8x50_IS_100G_2LANE_SW_WAR_MPP0_DONE_SET(unit, pm_info, is_100g_2ln_sw_war_mpp0_done));
        _SOC_IF_ERR_EXIT(
            PM8x50_IS_100G_2LANE_SW_WAR_MPP1_DONE_SET(unit, pm_info, is_100g_2ln_sw_war_mpp1_done));
    }
    ...
    if ((recovered_version <= 8) &&
        ...
        _SOC_IF_ERR_EXIT(
            PM8x50_AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4_SET(unit, pm_info, link_fail_wait_time));
    }

    if ((recovered_version <= 9) &&
        (new_version >= 10)) {
        link_recovery_enable = 0;
        for (i = 0; i < MAX_PORTS_PER_PM8X50; i++) {
            _SOC_IF_ERR_EXIT(
                PM8x50_PORT_LINK_RECOVERY_ENABLE_SET(unit, pm_info, link_recovery_enable, i));
        }
    }
...
int pm8x50_pm_wb_debug_log (int unit, pm_info_t pm_info)
{
    ...
    int is_100g_2ln_sw_war_mpp0_done = 0;
    int is_100g_2ln_sw_war_mpp1_done = 0;
    int link_fail_wait_time;
    int link_recovery_enable;
    ...
    if (SOC_E_NONE == rv) {
        ...
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "{%d}\n"), an_wait_link_count));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "S$END:portmod_pm8x50:{an_wait_link_count}\n")));
    }
    ...
    for (i = 0 ; i < MAX_PORTS_PER_PM8X50; i++) {
        rv = PM8x50_TIMESYNC_CONFIG_GET(unit, pm_info, &timesync_enable, i);
        if (SOC_E_NONE == rv) {
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "{%d,%d}\n"), i, timesync_enable));
        }
    ...
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "S$END:portmod_pm8x50:{port,port_flexe_mode}\n")));

    rv = PM8x50_IS_100G_2LANE_SW_WAR_MPP0_DONE_GET(unit, pm_info, &is_100g_2ln_sw_war_mpp0_done);
    if (SOC_E_NONE == rv) {
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "S$BEGIN:portmod_pm8x50:{is_100g_2ln_sw_war_mpp0_done}\n")));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "{%d}\n"), is_100g_2ln_sw_war_mpp0_done));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "S$END:portmod_pm8x50:{is_100g_2ln_sw_war_mpp0_done}\n")));
    }

    rv = PM8x50_IS_100G_2LANE_SW_WAR_MPP1_DONE_GET(unit, pm_info, &is_100g_2ln_sw_war_mpp1_done);
    if (SOC_E_NONE == rv) {
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "S$BEGIN:portmod_pm8x50:{is_100g_2ln_sw_war_mpp1_done}\n")));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "{%d}\n"), is_100g_2ln_sw_war_mpp1_done));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "S$END:portmod_pm8x50:{is_100g_2ln_sw_war_mpp1_done}\n")));
    }

    rv = PM8x50_AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4_GET(unit, pm_info, &link_fail_wait_time);
    if (SOC_E_NONE == rv) {
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "S$BEGIN:portmod_pm8x50:{an link fail inhibit timer for pam4 with lt}\n")));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "{%d}\n"), link_fail_wait_time));
        LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                        "S$END:portmod_pm8x50:{an link fail inhibit timer for pam4 with lt}\n")));
    }

    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "S$BEGIN:portmod_pm8x50:{port,link_recovery_enable}\n")));
    for (i = 0 ; i < MAX_PORTS_PER_PM8X50; i++) {
        rv = PM8x50_PORT_LINK_RECOVERY_ENABLE_GET(unit, pm_info, &link_recovery_enable, i);
        if (SOC_E_NONE == rv) {
            LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                            "{%d,%d}\n"), i, link_recovery_enable));
        }
    }
    LOG_VERBOSE(BSL_LS_SHARED_SCACHE, (BSL_META_U(unit,
                    "S$END:portmod_pm8x50:{port,link_recovery_enable}\n")));
...
int pm8x50_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id,
                         "is100g2LaneWarMPP0Done", wb_buffer_index, sizeof(int),
                          NULL, MAX_PORTS_PER_PM8X50, VERSION(7));
    _SOC_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[is100g2LaneWarMPP0Done] = wb_var_id;

    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id,
                          "is100g2LaneWarMPP1Done", wb_buffer_index, sizeof(int),
                          NULL, MAX_PORTS_PER_PM8X50, VERSION(7));
    _SOC_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[is100g2LaneWarMPP1Done] = wb_var_id;

    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id,
                          "anLinkfailInhibitTimerLtPam4", wb_buffer_index, sizeof(int),
                          NULL, MAX_PORTS_PER_PM8X50, VERSION(9));
    _SOC_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[anLinkfailInhibitTimerLtPam4] = wb_var_id;

    _SOC_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "link_recovery_enable",
                          wb_buffer_index, sizeof(int), NULL, MAX_PORTS_PER_PM8X50, VERSION(10));
    _SOC_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[linkRecoveryEnable] = wb_var_id;
...
STATIC
int _pm8x50_port_link_recover(int unit, int port, pm_info_t pm_info, int* is_completed)
{
    portmod_access_get_params_t params;
    phymod_phy_access_t phy_access, phy_copy;
    int nof_phys, port_index = 0, num_lane = 0, i;
    pm8x50_port_3_cw_bad_state_t war_state;
    uint32_t tick_cnt = 0, fec_bypass_indication = 0, bitmap = 0;
    uint32_t signal_detect = 1, three_bad, pmd_x4_status, am_loss_align = 0;
    ...
    *is_completed = 0;
    ...
    PORTMOD_BIT_COUNT(bitmap, num_lane);
    ...
    switch (war_state) {
        case PM8X50_PORT_3_CW_BAD_CHECK:
        {
            ...
            /* read signal_detect status from PCS */
            for (i = 0; i < num_lane; i++) {
                phy_copy.access.lane_mask = 1 << (port_index + i);
                _SOC_IF_ERR_EXIT
                    (phymod_phy_reg_read(&phy_copy, PMD_X4_STATUSr_REG, &pmd_x4_status));
                    signal_detect &=
                            ((pmd_x4_status >> PMD_X4_STATUSr_SIGNAL_DETECT_STS_OFFSET)
                            & PMD_X4_STATUSr_SIGNAL_DETECT_STS_MASK);
            }
            _SOC_IF_ERR_EXIT
                (phymod_phy_fec_three_cw_bad_state_get(&phy_access, &three_bad));
            _SOC_IF_ERR_EXIT
                (phymod_phy_am_loss_align_multi_state_get(&phy_access, &am_loss_align));
            if (!link && signal_detect && (am_loss_align || three_bad)) {
                if (am_loss_align) {
                    state_string = "3_CW_BAD_CHECK: LOSS_OF_ALIGN CHECK\n";
                }
                if (three_bad) {
                    state_string = "3_CW_BAD_CHECK: THREE CW BAD CHECK\n";
                }
...
{
    pm_info_t pm_info;
    int port_index, rlm_enabled, rlm_state, is_400g_an, enable;
    uint32 bitmap;
    int rlm_done = 1, an_done = 1, ber_war_off = 1;
    int is_ber_war_enable = 0, valid = 0;
    int is_3_cw_bad_enable =0, is_link_recovery_completed = 1, link_recovery_enable = 0;
    portmod_speed_config_t speed_config;
   ...
   if (rlm_enabled == 1) {
    ...
    _SOC_IF_ERR_EXIT(PM8x50_PORT_LINK_RECOVERY_ENABLE_GET(unit, pm_info, &link_recovery_enable, port_index));
    _SOC_IF_ERR_EXIT(PM8x50_3_CW_BAD_ENABLE_GET(unit, pm_info, &is_3_cw_bad_enable, port_index));
    _SOC_IF_ERR_EXIT(pm8x50_port_enable_get(unit, port, pm_info, 0, &enable));
    if (is_3_cw_bad_enable && enable && link_recovery_enable) {
        _SOC_IF_ERR_EXIT(_pm8x50_port_link_recover(unit, port, pm_info, &is_link_recovery_completed));
    }

     /* if both signal unregister - go for it */
    if (rlm_done && ber_war_off && is_link_recovery_completed && !is_400g_an) {
                   ...
                   pm_info_t pm_info)
{
    ...
    portmod_vco_type_t vco;
    int phy;
    int is_3_cw_bad_enable =0, three_cw_bad_state = 0, three_cw_bad_tick_cnt = 0, link_recovery_enable = 0;
    ...
    if (SOC_WARM_BOOT(unit)) {
        ...
        is_100g_2ln_sw_war_mpp0_done = 0;
        rv = PM8x50_IS_100G_2LANE_SW_WAR_MPP0_DONE_SET(unit, pm_info, is_100g_2ln_sw_war_mpp0_done);
        _SOC_IF_ERR_EXIT(rv);

        is_100g_2ln_sw_war_mpp1_done = 0;
        rv = PM8x50_IS_100G_2LANE_SW_WAR_MPP1_DONE_SET(unit, pm_info, is_100g_2ln_sw_war_mpp1_done);
        _SOC_IF_ERR_EXIT(rv);

        link_fail_wait_time = AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4_DEFAULT;
        rv = PM8x50_AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4_SET(unit, pm_info, link_fail_wait_time);
        _SOC_IF_ERR_EXIT(rv);

        link_recovery_enable = 0;
        for (i = 0; i < MAX_PORTS_PER_PM8X50; i++) {
            rv = PM8x50_PORT_LINK_RECOVERY_ENABLE_SET(unit, pm_info, link_recovery_enable, i);
            _SOC_IF_ERR_EXIT(rv);
        }
    }
                     ...
                     int enable)
{
    ...
    if(enable && is_reset) {
        ...
        if (!mac_only) {
            ...
            sal_memset(PM_8x50_INFO(pm_info)->int_core_access.access.pmd_info_ptr, 0, PMD_INFO_DATA_STRUCTURE_SIZE);
        }
    }
...
int _pm8x50_pm_serdes_core_init(int unit, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    pm_version_t version;
    uint32_t pm8x50_gen2_flag = 0;
    phymod_core_access_t* core_access = &PM_8x50_INFO(pm_info)->int_core_access;
    portmod_default_user_access_t* user_data = (portmod_default_user_access_t*)core_access->access.user_acc;
    ...
    /*TSCBH_GEN2 needs the default TVCO for 100G lane*/
    _SOC_IF_ERR_EXIT(_pm8x50_pm_version_get(unit, pm_info, &version));
    if (version.tech_process != PM_TECH_16NM) {
        pm8x50_gen2_flag = 1;
    }
    if (pm8x50_gen2_flag) {
        if (tvco == portmodVCOInvalid) {
            tvco = portmodVCO26P562G;
        }
    }
    ...
    if (PORTMOD_PORT_ADD_F_FLR_SUPPORT_GET(add_info)) {
        PHYMOD_CORE_INIT_F_FLR_SUPPORT_SET(&core_conf);
        PORTMOD_PORT_ADD_F_FLR_SUPPORT_SET(user_data);
    } else {
        PHYMOD_CORE_INIT_F_FLR_SUPPORT_CLR(&core_conf);
        PORTMOD_PORT_ADD_F_FLR_SUPPORT_CLR(user_data);
    }
                                 ...
                                 uint64 *saved_mac_ctrl)
{
    ...
    if (PM_8x50_INFO(pm_info)->portmod_mac_soft_reset) {
        rv = PM_8x50_INFO(pm_info)->portmod_mac_soft_reset(unit, port, portmodCallBackActionTypeLock);
        _SOC_IF_ERR_EXIT(rv);
    }
...
exit:
    ...
    if (PM_8x50_INFO(pm_info)->portmod_mac_soft_reset) {
        ...
        local_rv = PM_8x50_INFO(pm_info)->portmod_mac_soft_reset(unit, port, portmodCallBackActionTypeUnlock);
                       ...
                       const portmod_port_add_info_t* add_info)
{
    ...
    _SOC_IF_ERR_EXIT(_pm8x50_pm_version_get(unit, pm_info, &version));
    if ((version.tech_process != PM_TECH_16NM) && (num_lanes != 8)) {
        if (!SAL_BOOT_SIMULATION) {
            _SOC_IF_ERR_EXIT(
                PM8x50_IS_100G_2LANE_SW_WAR_MPP0_DONE_GET(unit, pm_info, &is_100g_2ln_sw_war_mpp0_done));
            _SOC_IF_ERR_EXIT(
                PM8x50_IS_100G_2LANE_SW_WAR_MPP1_DONE_GET(unit, pm_info, &is_100g_2ln_sw_war_mpp1_done));
            if (phy_access.access.lane_mask & 0xf) {
                if (!is_100g_2ln_sw_war_mpp0_done) {
                    PHYMOD_PHY_INIT_F_SW_WAR_100G_2LANE_MPP0_IS_DONE_CLR(&init_config);
                    is_100g_2ln_sw_war_mpp0_done = 1;
                    _SOC_IF_ERR_EXIT(
                        PM8x50_IS_100G_2LANE_SW_WAR_MPP0_DONE_SET(unit, pm_info, is_100g_2ln_sw_war_mpp0_done));
                } else {
                    PHYMOD_PHY_INIT_F_SW_WAR_100G_2LANE_MPP0_IS_DONE_SET(&init_config);
                }
            }

            if (phy_access.access.lane_mask & 0xf0) {
                if (!is_100g_2ln_sw_war_mpp1_done) {
                    PHYMOD_PHY_INIT_F_SW_WAR_100G_2LANE_MPP1_IS_DONE_CLR(&init_config);
                    is_100g_2ln_sw_war_mpp1_done = 1;
                    _SOC_IF_ERR_EXIT(
                        PM8x50_IS_100G_2LANE_SW_WAR_MPP1_DONE_SET(unit, pm_info, is_100g_2ln_sw_war_mpp1_done));
                } else {
                    PHYMOD_PHY_INIT_F_SW_WAR_100G_2LANE_MPP1_IS_DONE_SET(&init_config);
                }
            }
        } else {
            PHYMOD_PHY_INIT_F_SW_WAR_100G_2LANE_MPP0_IS_DONE_SET(&init_config);
    ...
    rv = PM8x50_TIMESYNC_CONFIG_SET(unit, pm_info, timesync_config_default, port_index);
    _SOC_IF_ERR_EXIT(rv);
                           ...
                           int flags, int enable)
{
    ...
    int nof_phys = 0;
    pm_version_t version;
    uint32_t phymod_port_en_support = 0;
    int rx_enable;
    uint64 mac_ctrl;
    int soft_reset_enable;
    ...
    if(enable){
        if((PORTMOD_PORT_ENABLE_MAC_GET(actual_flags)) && (!is_bypassed)) {
            ...
            else
            {
                ...
                _SOC_IF_ERR_EXIT(pm8x50_port_soft_reset(unit, port, pm_info, portmodMacSoftResetModeOut,
                                                        &rx_enable, &mac_ctrl));
            }
        }

        if(PORTMOD_PORT_ENABLE_PHY_GET(actual_flags)) {
            if (phymod_port_en_support) {
                _SOC_IF_ERR_EXIT
            ...
            else
            {
               ...
               _SOC_IF_ERR_EXIT(cdmac_soft_reset_get(unit, port, &soft_reset_enable));
                if (!soft_reset_enable)
                {
                 ...
                 _SOC_IF_ERR_EXIT(pm8x50_port_soft_reset(unit, port, pm_info, portmodMacSoftResetModeIn,
                                                         &rx_enable, &mac_ctrl));
                }
            }
        }
        /* disable phy port */
        if (PORTMOD_PORT_ENABLE_PHY_GET(actual_flags) && phymod_port_en_support) {
                           ...
                           int flags, int* enable)
{
    ...
    if (PORTMOD_PORT_ENABLE_PHY_GET(actual_flags)) {
        if (phymod_port_en_support) {
            _SOC_IF_ERR_EXIT
                (phymod_port_enable_get(&phy_access, (uint32_t*)&phy_enable));
        } else {
                           ...
                           const portmod_vco_type_t* vco)
{
    ...
    if (cur_tvco != vco[0]) {
        ...
        if (pll_div != phymod_TSCBH_PLL_DIVNONE) {
            ...
            if (PM_8x50_INFO(pm_info)->clock_buffer_disable_required) {
                PHYMOD_IF_ERR_RETURN
                    (phymod_phy_pll_clock_buffer_pwrdn(&phy_access, tvco_pll_index, 0, 0));
            }
            _SOC_IF_ERR_EXIT(
    ...
    if (cur_ovco != vco[1]) {
        ...
        if (pll_div !=phymod_TSCBH_PLL_DIVNONE) {
            ...
            if (PM_8x50_INFO(pm_info)->clock_buffer_disable_required) {
                PHYMOD_IF_ERR_RETURN
                    (phymod_phy_pll_clock_buffer_pwrdn(&phy_access, ovco_pll_index, 0, 0));
            }
            _SOC_IF_ERR_EXIT(
                                           ...
                                           const portmod_port_speed_ability_t* abilities)
{
    ...
    for (i = 0; i < num_abilities; i++) {
        ...
        /* Check parameters */
        if (abilities[i].fec_type >= PORTMOD_PORT_PHY_FEC_COUNT) {
            _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                               (_SOC_MSG("port %d: Invalid FEC type %d."), port, abilities[i].fec_type));
        } else if (abilities[i].pause >= PORTMOD_PORT_PHY_PAUSE_COUNT) {
            _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                               (_SOC_MSG("port %d: Invalid pause %d."), port, abilities[i].pause));
        } else if (abilities[i].medium >= PORTMOD_PORT_PHY_MEDIUM_COUNT) {
            _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                               (_SOC_MSG("port %d: Invalid medium %d."), port, abilities[i].medium));
        } else if (abilities[i].channel >= PORTMOD_PORT_PHY_CHANNEL_COUNT) {
            _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                               (_SOC_MSG("port %d: Invalid channel %d."), port, abilities[i].channel));
        } else if (abilities[i].an_mode >= PORTMOD_PORT_PHY_CONTROL_AUTONEG_MODE_COUNT) {
            _SOC_EXIT_WITH_ERR(SOC_E_PARAM,
                               (_SOC_MSG("port %d: Invalid AN mode %d."), port, abilities[i].an_mode));
        }
                            ...
                            const phymod_autoneg_control_t* an)
{
    ...
    portmod_fec_t fec;
    pm_version_t version;
    int portmon_disable, phy_acc;
    uint32_t link_fail_wait_time, reg_val = 0;
    ...
    if (an_control.enable) {
        ...
        if (is_400g_an) {
            /* call port level port mode update */
            _SOC_IF_ERR_EXIT(_pm8x50_pm_port_mode_update(unit, port, pm_info, port_index, 4));
        } else {
            /* clear SINGLE_PORT_MODE_SPEED_400G field for non-400g mode */
            _SOC_IF_ERR_EXIT(_pm8x50_phy_access_get(unit, port, pm_info, &phy_acc));
            _SOC_IF_ERR_EXIT(READ_CDPORT_MODE_REGr(unit, phy_acc, &reg_val));
            soc_reg_field_set(unit, CDPORT_MODE_REGr, &reg_val, SINGLE_PORT_MODE_SPEED_400Gf, 0);
            _SOC_IF_ERR_EXIT(WRITE_CDPORT_MODE_REGr(unit, phy_acc, reg_val));
     ...
     */
    if ((version.tech_process == PM_TECH_16NM) && (version.type == PM_TYPE_PM8X50)) {
        _SOC_IF_ERR_EXIT(PM8x50_AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4_GET(unit, pm_info, &link_fail_wait_time));
        if (!an_control.enable &&
            (link_fail_wait_time != AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4_DEFAULT) &&
            (port_num_lanes == 8)) {
                         ...
                         int flags, int* link)
{
    ...
    uint32_t signal_detect = 1, three_bad, pmd_x4_status = 0, am_loss_align = 0;
    uint32_t rlm_state, an_done;
    int rlm_enabled;
    phymod_autoneg_control_t an;
    int phy_acc, num_lane, i;
    int portmon_disable = 0;
    int is_done = 0, is_link_recovery_enable = 0, register_callback = 0;
    ...
    if (SAL_BOOT_SIMULATION){
        ...
        _SOC_IF_ERR_EXIT
            (portmod_portmon_disable_get(unit, &portmon_disable));
        _SOC_IF_ERR_EXIT
            (PM8x50_PORT_LINK_RECOVERY_ENABLE_GET(unit, pm_info, &is_link_recovery_enable, port_index));
        if (!link_status) {
            if (!is_flexe_rev1 && is_link_recovery_enable) {
                _SOC_IF_ERR_EXIT
                    (PM8x50_3_CW_BAD_ENABLE_GET(unit, pm_info, &is_3_cw_bad_enable, port_index));
                if (!is_3_cw_bad_enable) {
                    /* Check loss of align multiple states. */
                    _SOC_IF_ERR_EXIT
                        (phymod_phy_am_loss_align_multi_state_get(&phy_access, &am_loss_align));
                    /* read signal_detect status from PCS */
                    for (i = 0; i < num_lane; i++) {
                    ...
                    _SOC_IF_ERR_EXIT
                        (phymod_phy_fec_three_cw_bad_state_get(&phy_access, &three_bad));
                    if (signal_detect && am_loss_align) {
                        register_callback = 1;
                     } else if (signal_detect && three_bad) {
                        _SOC_IF_ERR_EXIT
                            (PM8x50_RLM_ENABLE_GET(unit, pm_info, &rlm_enabled, port_index));
                        _SOC_IF_ERR_EXIT
                            (PM8x50_RLM_STATE_GET(unit, pm_info, &rlm_state, port_index));
        ...
        _SOC_IF_ERR_EXIT
            (PM8x50_3_CW_BAD_ENABLE_GET(unit, pm_info, &is_3_cw_bad_enable, port_index));
        if (is_3_cw_bad_enable && !is_flexe_rev1 && portmon_disable && is_link_recovery_enable) {
            _SOC_IF_ERR_EXIT
                (_pm8x50_port_link_recover(unit, port, pm_info, &is_done));
            if (!is_done) {
                *link = 0;
            }
        }
...
int _pm8x50_port_phy_timesync_link_up_handler (int unit,
                                               int port,
                                               pm_info_t pm_info)
{
    portmod_access_get_params_t params;
    ...
    _SOC_IF_ERR_EXIT
        (PM8x50_TIMESYNC_CONFIG_GET(unit, pm_info, &timesync_config, port_index));

    if (PORTMOD_PORT_TIMESYNC_CONFIG_ENABLE_GET(timesync_config)) {
        /*
         * tscbh gen2 or flexe, disable and enable the ts_update_enable
         * on link up.
         */
        _SOC_IF_ERR_EXIT
            (_pm8x50_pm_version_get(unit, pm_info, &version));
        ...
        if (PORTMOD_PORT_TIMESYNC_CONFIG_SFD_GET(timesync_config)){
            ...
            PHYMOD_TIMESYNC_F_MAC_DA_CLR(timesync_adj_flags);
        } else {
            if (PORTMOD_PORT_TIMESYNC_CONFIG_MAC_DA_GET(timesync_config)) {
                PHYMOD_TIMESYNC_F_SOP_CLR(timesync_adj_flags);
                PHYMOD_TIMESYNC_F_MAC_DA_SET(timesync_adj_flags);
            } else {
        ...
        timesync_adjust_config.tx_lkup_1588_mem_400g = &PM_8x50_INFO(pm_info)->pm_1588_lookup_shadow_memory.tx_lkup_1588_mem_400g[0][0];
        timesync_adjust_config.tx_lkup_1588_mem_mpp0 = &PM_8x50_INFO(pm_info)->pm_1588_lookup_shadow_memory.tx_lkup_1588_mem_mpp0[0][0];
        timesync_adjust_config.tx_lkup_1588_mem_mpp1 = &PM_8x50_INFO(pm_info)->pm_1588_lookup_shadow_memory.tx_lkup_1588_mem_mpp1[0][0];
        timesync_adjust_config.rx_lkup_1588_mem_400g = &PM_8x50_INFO(pm_info)->pm_1588_lookup_shadow_memory.rx_lkup_1588_mem_400g[0][0];
        timesync_adjust_config.rx_lkup_1588_mem_mpp0 = &PM_8x50_INFO(pm_info)->pm_1588_lookup_shadow_memory.rx_lkup_1588_mem_mpp0[0][0];
        timesync_adjust_config.rx_lkup_1588_mem_mpp1 = &PM_8x50_INFO(pm_info)->pm_1588_lookup_shadow_memory.rx_lkup_1588_mem_mpp1[0][0];
...
/* flr link up flr event */
/*
 *
 * 2. 1588 ports: timesync adjust.
 *
 */
int pm8x50_port_phy_flr_link_up_event(int unit, int port, pm_info_t pm_info)
{
    SOC_INIT_FUNC_DEFS;

    _SOC_IF_ERR_EXIT
        (_pm8x50_port_phy_timesync_link_up_handler(unit, port, pm_info));
...
int pm8x50_port_phy_link_down_event(int unit, int port, pm_info_t pm_info)
{
    ...
    _SOC_IF_ERR_EXIT(_pm8x50_port_index_get(unit, port, pm_info, &port_index, &bitmap));
    _SOC_IF_ERR_EXIT(PM8x50_TIMESYNC_CONFIG_GET(unit, pm_info, &timesync_config, port_index));
...
int pm8x50_port_tx_threshold_set(int unit, int port,
                                   pm_info_t pm_info,
                                   int value)
{
    SOC_INIT_FUNC_DEFS;
...
int pm8x50_port_tx_threshold_get(int unit, int port, pm_info_t pm_info,
                                   int* value)
{
    SOC_INIT_FUNC_DEFS;
...
int pm8x50_port_local_fault_force_set(int unit, int port, pm_info_t pm_info, int enable)
{
    SOC_INIT_FUNC_DEFS;
    SOC_NULL_CHECK(pm_info);
...
int pm8x50_port_local_fault_force_get(int unit, int port, pm_info_t pm_info, int * enable)
{
    SOC_INIT_FUNC_DEFS;
    SOC_NULL_CHECK(pm_info);
...
int pm8x50_port_remote_fault_force_set(int unit, int port, pm_info_t pm_info, int enable)
{
    SOC_INIT_FUNC_DEFS;
    SOC_NULL_CHECK(pm_info);
...
int pm8x50_port_remote_fault_force_get(int unit, int port, pm_info_t pm_info, int * enable)
{
    SOC_INIT_FUNC_DEFS;
    SOC_NULL_CHECK(pm_info);
                                ...
                                int* nof_cores, int* is_most_ext)
{
    ...
    /*
     * PM8x50 does not have integrated external phy support. There is only
     * interal phy exist. PHYN 0 (internal) and -1 (outer most PHY) will
     * have valid core information.  Any querry of external PHYN will not
     * have any valid phy information and nof_core will be 0.
     */

    /*
     * There is only internal phys(1) on PM8x50, request of outer
     * most phy information will return with internal phy information.
     */
    if(phyn < 0)
    {
        phyn = 0;
                             ...
                             int flags, int reg_addr, uint32* value)
{
    ...
    _SOC_IF_ERR_EXIT(_pm8x50_port_index_get(unit, port, pm_info, &start_lane, &lane_mask));
                              ...
                              const portmod_phy_timesync_config_t* config)
{
    ...
    if (!enable) {
         ...
         */
        _SOC_IF_ERR_EXIT
            (pm8x50_port_autoneg_get(unit, port, pm_info, phy_flags, &an_control));
    ...
    _SOC_IF_ERR_EXIT(PM8x50_TIMESYNC_CONFIG_SET(unit, pm_info, timesync_config, port_index));
                                     ...
                                     uint32_t port_dynamic_state)
{
    ...
    if (PORTMOD_PORT_IS_AN_LINK_FAIL_INHIBIT_TIMER_UPDATED(port_dynamic_state)) {
        ...
        _SOC_IF_ERR_EXIT(PM8x50_AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4_SET(unit, pm_info, link_fail_wait_time));
    }
...
int _pm8x50_fix_2bit_ecc_memory ( portmod_default_user_access_t *user_data,
                                  pm8x50_1588_lookup_memory_t *shadow_data,
                                  phymod_interrupt_type_t intr_type,
                                  int mem_index)
{
    int rv = SOC_E_NONE;
    pm_ts_table_entry *mem_data;
    soc_mem_t tsc_mem_data;
    int skip = 0;
    ...
    switch (intr_type) {
        case phymodIntrEccTx1588400g:
            mem_data        = (pm_ts_table_entry *)shadow_data->tx_lkup_1588_mem_400g;
            tsc_mem_data    = TX_LKUP_1588_MEM_400Gm; 
            break;
        case phymodIntrEccTx1588Mpp0:
            mem_data        = (pm_ts_table_entry *)shadow_data->tx_lkup_1588_mem_mpp0;
            tsc_mem_data    = TX_LKUP_1588_MEM_MPP0m;
            break;
        ...
        case phymodIntrEccRx1588400g:
            mem_data        = (pm_ts_table_entry *)shadow_data->rx_lkup_1588_mem_400g;
            tsc_mem_data    = RX_LKUP_1588_MEM_400Gm;
            break;
        case phymodIntrEccRx1588Mpp0:
            mem_data        = (pm_ts_table_entry *)shadow_data->rx_lkup_1588_mem_mpp0;
            tsc_mem_data    = RX_LKUP_1588_MEM_MPP0m;
            break;
    ...
    if (!skip) {
        rv = soc_mem_write(user_data->unit, tsc_mem_data, user_data->blk_id, mem_index, (void *)&mem_data[mem_index][0]);
    }
    return rv;
}
        ...
        if (first_lport == prev_lport) {
            ...
            while (intr_type != portmodIntrTypeCount) {
                ...
                LOG_CLI(("BLOCK_CLEAR_REG\n"));
                    ...
                    /* clear tsc interrupt */
                    phymod_intr_info_t_init(&mem_info);
                    _SOC_IF_ERR_EXIT
                        (phymod_intr_handler(&phy_access,
                                             tsc_intr_info.phymod_intr_map,
                                             &is_handled,
                                             &mem_info));
                    if (!is_handled) {
                        ...
                        _SOC_IF_ERR_EXIT
                            (_pm8x50_fix_2bit_ecc_memory((portmod_default_user_access_t*) PHYMOD_ACC_USER_ACC(&phy_access.access),
                                                          &PM_8x50_INFO(pm_info)->pm_1588_lookup_shadow_memory,
                                                          tsc_intr_info.phymod_intr_map,
                                                          mem_info.mem_addr));
                    } else {
                          ...
                          portmod_egr_1588_timestamp_config_t timestamp_config)
{
    ...
    LOG_ERROR(BSL_LS_SOC_COMMON,
              (BSL_META_U(unit, "%s() : PM8x50 only support 48bit mode\n"), FUNCTION_NAME()));
    return SOC_E_UNAVAIL;
}
...
int pm8x50_port_faults_status_get(int unit, int port, pm_info_t pm_info,
...
STATIC
int _pm8x50_flr_timeout_portmod_to_phymod(portmod_flr_timeout_t timeout, phymod_phy_flr_timeout_t* phymod_timeout)
{
    int rv = SOC_E_NONE;

    switch (timeout) {
        case portmodFlrTimeoutInvaild:
            *phymod_timeout = phymodFlrTimeoutInvaild;
            break;
        case portmodFlrTimeout50ms:
            *phymod_timeout = phymodFlrTimeout50ms;
            break;
        case portmodFlrTimeout75ms:
            *phymod_timeout = phymodFlrTimeout75ms;
            break;
        case portmodFlrTimeout100ms:
            *phymod_timeout = phymodFlrTimeout100ms;
            break;
        case portmodFlrTimeout150ms:
            *phymod_timeout = phymodFlrTimeout150ms;
            break;
        case portmodFlrTimeout250ms:
            *phymod_timeout = phymodFlrTimeout250ms;
            break;
        case portmodFlrTimeout500ms:
            *phymod_timeout = phymodFlrTimeout500ms;
            break;
        default:
            rv = SOC_E_PARAM;
            break;
...
STATIC
int _pm8x50_flr_timeout_phymod_to_portmod(phymod_phy_flr_timeout_t phymod_timeout, portmod_flr_timeout_t* timeout)
{
    int rv = SOC_E_NONE;

    switch (phymod_timeout) {
        case phymodFlrTimeoutInvaild:
            *timeout = portmodFlrTimeoutInvaild;
            break;
        case phymodFlrTimeout50ms:
            *timeout = portmodFlrTimeout50ms;
            break;
        case phymodFlrTimeout75ms:
            *timeout = portmodFlrTimeout75ms;
            break;
        case phymodFlrTimeout100ms:
            *timeout = portmodFlrTimeout100ms;
            break;
        case phymodFlrTimeout150ms:
            *timeout = portmodFlrTimeout150ms;
            break;
        case phymodFlrTimeout250ms:
            *timeout = portmodFlrTimeout250ms;
            break;
        case phymodFlrTimeout500ms:
            *timeout = portmodFlrTimeout500ms;
            break;
        default:
            rv = SOC_E_PARAM;
            break;
...
int pm8x50_port_flr_config_set(int unit, int port, pm_info_t pm_info, int enable, portmod_flr_timeout_t timeout_time)
{
    ...
    portmod_access_get_params_t params;
    phymod_phy_access_t phy_access;
    phymod_phy_flr_timeout_t phymod_timeout;
    SOC_INIT_FUNC_DEFS;
    ...
    if (!SAL_BOOT_SIMULATION) {
        _SOC_IF_ERR_EXIT
            (_pm8x50_flr_timeout_portmod_to_phymod(timeout_time, &phymod_timeout));
        _SOC_IF_ERR_EXIT
            (phymod_phy_flr_config_set(&phy_access, enable, phymod_timeout));
        if (enable) {
            _SOC_IF_ERR_EXIT(cdmac_link_down_sequence_enable_set(unit, port, 0));
            _SOC_IF_ERR_EXIT(cdmac_drop_tx_data_on_ordered_set_enable(unit, port, 1));
        } else {
            /*set to the default of HW when disable*/
            _SOC_IF_ERR_EXIT(cdmac_link_down_sequence_enable_set(unit, port, 1));
            _SOC_IF_ERR_EXIT(cdmac_drop_tx_data_on_ordered_set_enable(unit, port, 0));
        }
    }
...
int pm8x50_port_flr_config_get(int unit, int port, pm_info_t pm_info, int *enable, portmod_flr_timeout_t *timeout_time)
{
    ...
    _SOC_IF_ERR_EXIT
        (phymod_phy_flr_config_get(&phy_access, &flr_enable, &phymod_timeout));
    *enable = flr_enable ? 1 : 0;
    _SOC_IF_ERR_EXIT
        (_pm8x50_flr_timeout_phymod_to_portmod(phymod_timeout, timeout_time));
...
int pm8x50_port_signal_detect_status_get(int unit, int port, pm_info_t pm_info, portmod_signal_detect_status_t* sd_status)
{
    portmod_access_get_params_t params;
    phymod_phy_access_t phy_access;
    int nof_phys;
    phymod_signal_detect_status_t sd_state;
    SOC_INIT_FUNC_DEFS;

    sal_memset(&sd_state, 0, sizeof(phymod_signal_detect_status_t));
    ...
    _SOC_IF_ERR_EXIT
        (phymod_phy_signal_detect_status_get(&phy_access, &sd_state));

    sd_status->signal_detect_bmp = sd_state.signal_detect_bmp;
    sd_status->signal_detect_change_bmp = sd_state.signal_detect_change_bmp;
...
int pm8x50_port_link_recovery_enable_set(int unit, int port, pm_info_t pm_info, int enable)
{
   ...
   _SOC_IF_ERR_EXIT
    ...
    _SOC_IF_ERR_EXIT(
        PM8x50_PORT_LINK_RECOVERY_ENABLE_SET(unit, pm_info, enable, port_index));

exit:
    SOC_FUNC_RETURN;
}

/* Get link_recovery_enable status. */
int pm8x50_port_link_recovery_enable_get(int unit, int port, pm_info_t pm_info, int* enable)
{
   ...
   _SOC_IF_ERR_EXIT
    ...
    _SOC_IF_ERR_EXIT(
        PM8x50_PORT_LINK_RECOVERY_ENABLE_GET(unit, pm_info, enable, port_index));
