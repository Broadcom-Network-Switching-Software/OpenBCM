...
portmod_pm_phy_ber_proj(phymod_phy_access_t *phy_access, soc_port_phy_ber_proj_params_t *args)
{
    ...
    options.without_print = args->without_print;
    ...
    switch (args->ber_proj_fec_type) {
        ...
        case PORTMOD_PORT_PHY_FEC_RS_FEC:
            options.ber_proj_fec_size = PORTMOD_PORT_FEC_FRAME_SIZE_RS_528;
            break;
        default:
    ...
    if (args->ber_proj_phase != SOC_PORT_PHY_BER_PROJ_P_ALL) {
        ...
        _SOC_IF_ERR_EXIT(phymod_core_access_t_init(&core));
        sal_memcpy(&core, phy_access, sizeof(core));
        _SOC_IF_ERR_EXIT(phymod_core_info_get(&core, &core_info));
        if ((core_info.core_version == phymodCoreVersionBlackhawk16) ||
            ...
            sample_time = args->ber_proj_timeout_s;
        }
