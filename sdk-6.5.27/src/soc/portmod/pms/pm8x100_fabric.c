...
{
#ifdef PHYMOD_PEREGRINE5_PC_SUPPORT
    phymodDispatchTypePeregrine5_pc,
#endif
    ...
    uint32_t val)
{
     ...
     */
    return portmod_common_phy_sbus_reg_write(mFSRD_FSRD_PMI_LP, user_acc, 0x0, reg_addr, val);
}
    ...
    uint32_t * val)
{
     ...
     * will force the core_addr to zero since it's NOT being used 
     */
    rv = portmod_common_phy_sbus_reg_read(mFSRD_FSRD_PMI_LP, user_acc, 0x0, reg_addr, val);
    /*
...
int pm8x100_fabric_pm_init(int unit, const portmod_pm_create_info_internal_t* pm_add_info, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    if (PHYMOD_ACC_BUS(&fabric_data->access) == NULL)
    {
        ...
        /*
         * On adapter the phy simulator needs to know the core address in order to 
         * allocate sim instace for every serdes/phy.
         * When not on adapter, addres or also known as core_address will be always set to 0.
...
int pm8x100_fabric_pm_fmac_is_quad_swap_get(int unit, int fmac_blk_id, int fmac_inner_link, int *is_quad_swap)
{
    ...
    _SOC_IF_ERR_EXIT(access_regmem(unit, FLAG_ACCESS_INST_TYPE_SBUS_ID, rx_lane_swap_regs[fmac_inner_link], fmac_blk_id, 0, 0, reg_val));
...
int pm8x100_fabric_pm_vco_reconfig(int unit, int pm_id, pm_info_t pm_info, const portmod_vco_type_t* vco)
{
     ...
     * Peregrine doesn't have such hardcoded pll value and if we don't execute this step for Adapter in Easy Reload test will fail.
