...
STATIC int soc_ramon_cell_snake_test_reg_reset(
    int unit);
STATIC int soc_ramon_cell_snake_test_sw_configuration_set(
    int unit);
STATIC int soc_ramon_cell_snake_test_interrupts_clear(
    int unit);
STATIC int soc_ramon_cell_snake_test_mac_loopback_set(
    int unit,
    int port);
STATIC int soc_ramon_cell_snake_test_disabled_fsrd_blocks_power_set(
    int unit,
    int enable);

STATIC int
soc_ramon_cell_snake_test_occg_override_nof_cells(
    int unit)
{
    uint64 reg_val64;
    uint32 reg_val;
    ...
    soc_reg64_field_set(unit, OCCG_REG_0130r, &reg_val64, FIELD_1_20f,
                        SOC_RAMON_SKU_CELL_SNAKE_NOF_DATA_CELLS_OVERRIDE);
    soc_reg64_field_set(unit, OCCG_REG_0130r, &reg_val64, FIELD_22_41f,
                        SOC_RAMON_SKU_CELL_SNAKE_NOF_DATA_CELLS_OVERRIDE);
    soc_reg64_field_set(unit, OCCG_REG_0130r, &reg_val64, FIELD_43_62f,
                        SOC_RAMON_SKU_CELL_SNAKE_NOF_DATA_CELLS_OVERRIDE);
    ...
    soc_reg64_field_set(unit, OCCG_REG_0132r, &reg_val64, FIELD_1_20f,
                        SOC_RAMON_SKU_CELL_SNAKE_NOF_CREDIT_CELLS_OVERRIDE);
...
STATIC int
soc_ramon_cell_snake_test_reconfigure_rtp(
    int unit)
{
...
STATIC int
soc_ramon_cell_snake_test_reconfigure_ocg_filters(
    int unit)
{
    soc_reg_above_64_val_t reg_val_above_64;
    uint32 filter_core_2[SOC_RAMON_SKU_CELL_SNAKE_NOF_FILTER_ELEMENTS] =
        { 0xffffffff, 0xffffffff, 0x0004ffff, 0x0aa80000, 0xffd0000 };
    uint32 filter_core_4[SOC_RAMON_SKU_CELL_SNAKE_NOF_FILTER_ELEMENTS] =
        { 0xffffffff, 0xffffffff, 0x0004ffff, 0x04440000, 0xffd0000 };
    int i = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(WRITE_DCH_AUTO_DOC_NAME_48r
                    (unit, SOC_RAMON_SKU_CELL_SNAKE_FIRST_ENABLED_CORE,
                     SOC_RAMON_SKU_CELL_SNAKE_NOF_CELLS_TO_BE_FILTERED));
    ...
    soc_dnxf_fabric_cell_snake_test_results_t * results)
{
    ...
    soc_reg_above_64_val_t reg_val_above_64, block_bitmap_default;
    bcm_port_t port;
    int rv;
    int nof_links;
    ...
    for (phy_core_idx = 0; phy_core_idx < nof_phy_cores; ++phy_core_idx)
    {
        ...
        if (nof_active_links_in_phy_core != nof_links_in_phy_core && nof_active_links_in_phy_core != 0)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG,
                         "the test is supported only if ALL ports in a phy core are active (probed) or ALL ports in the core are deactivated (detached).\n"
    ...
    if (dnxf_data_fabric.hw_snake.feature_get(unit, dnxf_data_fabric_hw_snake_is_sw_config_required)
        && !(flags & SOC_DNXF_ENABLE_MAC_LOOPBACK))
    {
        SHR_IF_ERR_EXIT(soc_ramon_cell_snake_test_sw_configuration_set(unit));
    }

    SHR_IF_ERR_EXIT(soc_ramon_stat_fmac_init(unit));
    ...
    soc_dnxf_fabric_cell_snake_test_results_t * results)
{
    ...
    while (1)
    {
        SOC_REG_ABOVE_64_CLEAR(reg_val_above_64);
        SHR_IF_ERR_EXIT(READ_OCCG_TEST_MODE_CMD_RESULT_REGr(unit, reg_val_above_64));
        soc_reg_above_64_field_get(unit, OCCG_TEST_MODE_CMD_RESULT_REGr, reg_val_above_64, TEST_MODE_TRGf,
                                   field_val_above_64);
    ...
    results->test_failed =
        soc_reg_above_64_field32_get(unit, OCCG_TEST_MODE_CMD_RESULT_REGr, reg_val_above_64, TEST_MODE_FAILf);

    if (results->test_failed)
    {
        extern const int soc_ramon_diag_flag_str_interrupts_names_size;

        soc_reg_above_64_field_get(unit, OCCG_TEST_MODE_CMD_RESULT_REGr, reg_val_above_64, TEST_MODE_INT_STATUSf,
                                   field_val_above_64);

        SHR_BITCLR_RANGE(results->interrupts_status, 0, 32 * SOC_DNXF_SNAKE_TEST_INTERRUPT_RESULT_SIZE_IN_WORDS);

        SHR_BITCOPY_RANGE(results->interrupts_status, 0, field_val_above_64, 0,
                          soc_ramon_diag_flag_str_interrupts_names_size);
    }

    SOC_REG_ABOVE_64_CLEAR(field_val_above_64);
    soc_reg_above_64_field_get(unit, OCCG_TEST_MODE_CMD_RESULT_REGr, reg_val_above_64, TEST_MODE_STAGE_STATUSf,
                               field_val_above_64);
...
#define _SOC_RAMON_SNAKE_FABRIC_CELL_TEST_FAILURE_STAGE_REGISTER_DATA_CELL_FILTER_WRITE_COMMAND_POS 4
    if (field_val &
        (0x1 << _SOC_RAMON_SNAKE_FABRIC_CELL_TEST_FAILURE_STAGE_REGISTER_DATA_CELL_FILTER_WRITE_COMMAND_POS))
    {
...
#define _SOC_RAMON_FABRIC_CELL_SNAKE_TEST_FAILURE_STAGE_REGISTER_CONTROL_CELL_FILTER_WRITE_COMMAND_POS 5
    if (field_val &
        (0x1 << _SOC_RAMON_FABRIC_CELL_SNAKE_TEST_FAILURE_STAGE_REGISTER_CONTROL_CELL_FILTER_WRITE_COMMAND_POS))
    {
...
#define _SOC_RAMON_FABRIC_CELL_SNAKE_TEST_FAILURE_STAGE_REGISTER_CONTROL_CELL_FILTER_READ_COMMAND_POS 7
    if (field_val &
        (0x1 << _SOC_RAMON_FABRIC_CELL_SNAKE_TEST_FAILURE_STAGE_REGISTER_CONTROL_CELL_FILTER_READ_COMMAND_POS))
    {
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(soc_reg_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, REG_PORT_ANY, 0, &reg_val64));
    soc_reg64_field_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, &reg_val64, LNK_LVL_FC_TX_ENf, 0);
    soc_reg64_field_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, &reg_val64, LNK_LVL_FC_RX_ENf, 0);
    SHR_IF_ERR_EXIT(soc_reg_set(unit, BRDC_FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, REG_PORT_ANY, 0, reg_val64));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(soc_reg_above_64_set
                    (unit, RTP_REACHABILITY_MESSAGE_GENERATOR_CONFIGURATIONr, REG_PORT_ANY, 0, reg_above64_val));
    ...
    SHR_IF_ERR_EXIT(soc_reg_above_64_set
                    (unit, BRDC_DCML_ECC_ERR_1B_MONITOR_MEM_MASKr, REG_PORT_ANY, 0, reg_above64_val));
    SHR_IF_ERR_EXIT(soc_reg_above_64_set
                    (unit, BRDC_DCML_ECC_ERR_2B_MONITOR_MEM_MASKr, REG_PORT_ANY, 0, reg_above64_val));
    ...
    for (index = 0; index < dnxf_data_device.blocks.nof_links_in_fmac_get(unit); index++)
    {
        ...
        SHR_IF_ERR_EXIT(soc_reg_set
                        (unit, BRDC_FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr, REG_PORT_ANY, index, 0x80000E09));
    ...
    SHR_IF_ERR_EXIT(soc_reg_set(unit, BRDC_FMAC_GENERAL_CONFIGURATION_REGISTERr, REG_PORT_ANY, 0, reg64_val));
...
STATIC int
soc_ramon_cell_snake_test_mac_loopback_set(
    int unit,
    int port)
{
    uint32 reg_val;
    ...
    SHR_IF_ERR_EXIT(soc_ramon_drv_link_to_block_mapping
                    (unit, port, &fmac_block, &fmac_inner_link, SOC_DNXF_DRV_BLOCK_TYPE_FMAC));
...
STATIC int
soc_ramon_cell_snake_test_disabled_fsrd_blocks_power_set(
    int unit,
    int power_up)
{
    ...
    for (fsrd_blcok_idx = 0; fsrd_blcok_idx < dnxf_data_device.blocks.nof_instances_fsrd_get(unit); fsrd_blcok_idx++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(soc_ramon_port_fsrd_block_power_set
                            (unit, fsrd_blcok_idx * dnxf_data_device.blocks.nof_links_in_fsrd_get(unit), power_up));
        }
    }
