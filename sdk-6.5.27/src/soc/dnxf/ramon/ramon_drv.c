    ...
    int unit)
{
    ...
    BCM_PBMP_ITER(dnxf_data_port.general.supported_lanes_get(unit)->pbmp, link)
    {
    ...
    int unit)
{
    ...
    if (SOC_DNXF_IS_FE13(unit))
    {
        PBMP_SFI_ITER(unit, link)
        {
            SHR_IF_ERR_EXIT(soc_dnxf_fabric_link_device_stage_get(unit, link, 0, &link_mode));
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                            (unit, mbcm_dnxf_drv_link_to_block_mapping,
                             (unit, link, &blk, &inner_link, SOC_DNXF_DRV_BLOCK_TYPE_FMAC)));
    ...
    int unit)
{
    ...
    rc = soc_ramon_set_mesh_topology_config(unit);
    SHR_IF_ERR_EXIT(rc);
    ...
    int *nof_relevant_dch_registers)
{
    ...
    for (reg = 0; reg < NUM_SOC_REG; reg++)
    {
        ...
        if (reginfo->numels > SOC_RAMON_DRV_MAX_NOF_REG_NUMELS)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Ramon SW reset ERROR - nof relevant DCH blocks numels %d bigger than maximum expected",
                         reginfo->numels);
        }
    ...
    *nof_relevant_dch_registers = dch_register_idx;
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Ramon SW reset ERROR - nof relevant DCH registers %d bigger than maximum expected",
    ...
    soc_pbmp_t * supported_lanes)
{
    ...
    SOC_PBMP_ASSIGN(*supported_lanes, dnxf_data_port.general.supported_lanes_get(unit)->pbmp);
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxc_endian_config(unit));
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        ...
        SHR_IF_ERR_EXIT(soc_ramon_reset_device(unit));
    }
