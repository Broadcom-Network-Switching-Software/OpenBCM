    ...
    soc_dnxc_isolation_status_t val)
{
    ...
    if (soc_dnxc_isolation_status_isolated == val)
    {
        ...
        gpd_rmgr_global_time =
            ((gpd_rmgr_global_at_core_clock_steps * 1000) / (dnxf_data_device.general.core_clock_khz_get(unit))) * 1000;
    ...
    soc_port_t * links_array)
{
    ...
    BCM_PBMP_COUNT(dnxf_data_port.general.supported_lanes_get(unit)->pbmp, nof_links);
    ...
    SHR_IF_ERR_EXIT(dnxf_state.lb.group_to_first_link.set(unit, 0, destination_local_id, links_array[0]));
    ...
    soc_port_t * links_array)
{
    ...
    SHR_IF_ERR_EXIT(dnxf_state.lb.group_to_first_link.get(unit, 0, destination_local_id, &link));
