    ...
    int is_first_link)
{
    ...
    if (is_first_link)
    {
        for (blk = fmac_first_block; blk < fmac_first_block + dnxf_data_device.blocks.nof_fmacs_in_fsrd_get(unit);
             blk++)
        {
            SHR_IF_ERR_EXIT(soc_reg32_get(unit, FMAC_LEAKY_BUCKET_CONTROL_REGISTERr, blk, 0, &reg32_val));
            ...
            SHR_IF_ERR_EXIT(soc_reg32_set(unit, FMAC_LEAKY_BUCKET_CONTROL_REGISTERr, blk, 0, reg32_val));
        }
    }
    ...
    int is_init_sequence)
{
    ...
    SOC_PBMP_ITER(pbmp, port)
    {
        ...
        SHR_IF_ERR_EXIT(soc_dnxf_port_pbmp_update(unit, port, 1));
    }
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &dcml_block, &inner_link, SOC_DNXF_DRV_BLOCK_TYPE_DCML)));
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &blk, &inner_link, SOC_DNXF_DRV_BLOCK_TYPE_FMAC)));
