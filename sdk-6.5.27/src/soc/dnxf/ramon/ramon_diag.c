...
const int soc_ramon_diag_flag_str_interrupts_names_size =
    sizeof(soc_ramon_diag_flag_str_interrupts_names) / sizeof(soc_dnxf_diag_flag_str_t);
...
shr_error_e
soc_ramon_temperature_monitors_get(
    int unit,
    ...
    int *temperature_monitors_count)
{
    ...
    soc_field_t curr_temp_fields[] = {
        THERMAL_DATA_Af,
        THERMAL_DATA_Bf,
        THERMAL_DATA_Cf,
        THERMAL_DATA_Df
    };
    soc_field_t peak_temp_fields[] = {
    ...
    if (temperature_monitors_max < dnxf_data_device.general.nof_pvt_monitors_get(unit))
    {
    ...
    for (i = 0; i < dnxf_data_device.general.nof_pvt_monitors_get(unit); i++)
    {
        ...
        while (1)
        {
            SHR_IF_ERR_EXIT(soc_reg32_get(unit, temp_regs[i], REG_PORT_ANY, 0, &reg32_val));
            curr = soc_reg_field_get(unit, temp_regs[i], reg32_val, curr_temp_fields[i]);
            peak = soc_reg_field_get(unit, temp_regs[i], reg32_val, peak_temp_fields[i]);

            curr_min = curr_max = curr;
            for (iter = 0; iter < 4; iter++)
            {
                SHR_IF_ERR_EXIT(soc_reg32_get(unit, temp_regs[i], REG_PORT_ANY, 0, &reg32_val));
                curr_tmp = soc_reg_field_get(unit, temp_regs[i], reg32_val, curr_temp_fields[i]);
                if (curr_tmp < curr_min)
                {
                    curr_min = curr_tmp;
                }
                if (curr_tmp > curr_max)
                {
                    curr_max = curr_tmp;
                }
            }

            if ((curr_max - curr_min) * dnxf_data_device.general.pvt_factor_get(unit) < 50 * 10000)
            {
    ...
    *temperature_monitors_count = dnxf_data_device.general.nof_pvt_monitors_get(unit);
