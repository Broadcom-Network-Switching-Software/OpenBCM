    ...
    int unit)
{
    ...
    for (dch_block_idx = 0; dch_block_idx < dnxf_data_device.blocks.nof_instances_dch_get(unit); dch_block_idx++)
    {
        ...
        SHR_IF_ERR_EXIT(soc_reg32_get(unit, DCH_DCH_ENABLERS_REGISTER_1r, dch_block_idx, 0, &reg_val32));
        soc_reg_field_set(unit, DCH_DCH_ENABLERS_REGISTER_1r, &reg_val32, AUTO_DOC_NAME_5f, 1);
        SHR_IF_ERR_EXIT(soc_reg32_set(unit, DCH_DCH_ENABLERS_REGISTER_1r, dch_block_idx, 0, reg_val32));
    }
...
{
    int i;
    uint32 table_size, start_bit, bit_length, mask, reg_val32;
    soc_reg_above_64_val_t filter_reg, mask_reg;
    soc_dnxf_cell_filter_mode_t filter_mode = soc_dnxf_cell_filter_copy_mode;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(soc_dnxf_cell_filter_mode_get(unit, flags, 0, (int *) &filter_mode));

    for (i = 0; i < dnxf_data_device.blocks.nof_instances_dch_get(unit); i++)
    {
        SHR_IF_ERR_EXIT(soc_reg32_get(unit, DCH_DCH_ENABLERS_REGISTER_1r, i, 0, &reg_val32));
        soc_reg_field_set(unit, DCH_DCH_ENABLERS_REGISTER_1r, &reg_val32, DCH_MTCH_ACTf, filter_mode);
        SHR_IF_ERR_EXIT(soc_reg32_set(unit, DCH_DCH_ENABLERS_REGISTER_1r, i, 0, reg_val32));
    }
    ...
    const dnxc_sr_cell_t * cell)
{
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping,
                               (unit, first_link, &blk_id, &inner_link, SOC_DNXF_DRV_BLOCK_TYPE_DCML));
    SHR_IF_ERR_EXIT(rc);
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping,
                               (unit, first_link, &blk_id, &inner_link, SOC_DNXF_DRV_BLOCK_TYPE_LCM));
    SHR_IF_ERR_EXIT(rc);
    ...
    uint32 *filter_type_val)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_cell_filter_mode_get(unit, flags, 1, (int *) &filter_mode));
