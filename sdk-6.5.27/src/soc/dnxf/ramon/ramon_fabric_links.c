    ...
    int enable)
{
    ...
    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping,
                               (unit, port, &fmac_instance, &fmac_inner_link, SOC_DNXF_DRV_BLOCK_TYPE_FMAC));
    SHR_IF_ERR_EXIT(rv);

    SHR_IF_ERR_EXIT(soc_reg32_get(unit, DCH_LINK_CELL_PACKING_ENr, dch_instance, 0, reg32_val));
    if (enable)
    {
        SHR_BITSET(reg32_val, dch_inner_link);
    }
    else
    {
        SHR_BITCLR(reg32_val, dch_inner_link);
    }
    SHR_IF_ERR_EXIT(soc_reg32_set(unit, DCH_LINK_CELL_PACKING_ENr, dch_instance, 0, reg32_val[0]));
    ...
    int *enable)
{
    ...
    SHR_IF_ERR_EXIT(soc_reg32_get(unit, DCH_LINK_CELL_PACKING_ENr, dch_instance, 0, reg32_val));
    *enable = SHR_IS_BITSET(reg32_val, dch_inner_link);
    ...
    soc_dnxf_fabric_link_remote_pipe_mapping_t pipe_map)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &instance_array[0], &inner_link_array[0], SOC_DNXF_DRV_BLOCK_TYPE_DCH)));
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &instance_array[1], &inner_link_array[1], SOC_DNXF_DRV_BLOCK_TYPE_DCML)));
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &instance_array[2], &inner_link_array[2], SOC_DNXF_DRV_BLOCK_TYPE_LCM)));
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_link_to_block_mapping,
                     (unit, port, &fmac_instance, &fmac_inner_link, SOC_DNXF_DRV_BLOCK_TYPE_FMAC)));
...
{
    uint32 reg32_val[] = { 0 };
    uint32 reg32_pipes[] = { 0 };
    soc_reg_above_64_val_t reg_above_64_val;
    int dch_instance, dch_inner_link;
    int rv;
    SHR_FUNC_INIT_VARS(unit);

    sal_memset(pipe_map, 0, sizeof(soc_dnxf_fabric_link_remote_pipe_mapping_t));
    ...
    rv = soc_reg32_get(unit, DCH_TWO_PIPES_BMPr, dch_instance, 0, &(reg32_pipes[0]));
    SHR_IF_ERR_EXIT(rv);
    if (SHR_IS_BITSET(reg32_pipes, dch_inner_link))
    {
    ...
    if (pipe_map->num_of_remote_pipes >= 1)
    {
        rv = soc_reg_above_64_get(unit, DCH_REMOTE_PRI_PIPE_IDXr, dch_instance, 0, reg_above_64_val);
        SHR_IF_ERR_EXIT(rv);
        SHR_BITCOPY_RANGE(&(pipe_map->remote_pipe_mapping[0]), 0, reg_above_64_val,
                          dch_inner_link * _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE, _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE);
    }

    if (pipe_map->num_of_remote_pipes >= 2)
    {
        rv = soc_reg_above_64_get(unit, DCH_REMOTE_SEC_PIPE_IDXr, dch_instance, 0, reg_above_64_val);
        SHR_IF_ERR_EXIT(rv);
        SHR_BITCOPY_RANGE(&(pipe_map->remote_pipe_mapping[1]), 0, reg_above_64_val,
                          dch_inner_link * _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE, _SOC_RAMON_REMOTE_PIPE_INDEX_SIZE);
    }
    ...
    soc_port_t port)
{
    ...
    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_block_pbmp_get,
                               (unit, SOC_DNXF_DRV_BLOCK_TYPE_DCH, dch_instance, &pbmp));
    SHR_IF_ERR_EXIT(rv);
