    ...
    uint32 *end_index)
{
    ...
    switch (index_type)
    {
        ...
        case _shr_dnxf_cgm_index_type_cast_prio:
            ...
            break;
        case _shr_dnxf_cgm_index_type_global:

            max_nof_indexes_dim[0] = 1;
            max_nof_indexes_dim[1] = 1;
        ...
        for (dim0 = start_index[0]; dim0 <= end_index[0]; dim0++)
        {
                 ...
                 dim1++)
            {
                ...
                if (set_or_get == _SHR_FABRIC_FLAG_SET)
                {
                    ...
                    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cgm_register_field_max_value_get,
                                                          (unit, reg_name, field_name, &max_th_value)));
                    ...
                    {
                        SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cgm_register_set,
                                                              (unit, reg_name, field_name, j, profile_id,
                                                               threshold_value_tmp)));
                    }
                }
                else if (set_or_get == _SHR_FABRIC_FLAG_GET)
                {
                    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cgm_register_get,
                                                          (unit, reg_name, field_name, first_block, profile_id,
                                                           threshold_value)));
    ...
    for (dim0 = start_index[0]; dim0 <= end_index[0]; dim0++)
    {
        for (dim1 = start_index[1]; dim1 <= end_index[1]; dim1++)
        {
            ...
            if (set_or_get == _SHR_FABRIC_FLAG_SET)
            {
                ...
                {
                    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cgm_register_set,
                                                          (unit, reg_name, field_name, j, dim1, *threshold_value)));
                }
            }
            else if (set_or_get == _SHR_FABRIC_FLAG_GET)
            {
                SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cgm_register_get,
                                                      (unit, reg_name, field_name, first_block, dim1,
                                                       threshold_value)));
    ...
    int *threshold_value)
{
    ...
    for (dim1 = start_index[1]; dim1 <= end_index[1]; dim1++)
    {
        ...
        if (set_or_get == _SHR_FABRIC_FLAG_SET)
        {
            ...
            {
                SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cgm_register_set,
                                                      (unit, reg_name, field_name, j, 0, *threshold_value)));
            }
        }
        else if (set_or_get == _SHR_FABRIC_FLAG_GET)
        {
            SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cgm_register_get,
                                                  (unit, reg_name, field_name, first_block, 0, threshold_value)));
    ...
    int *threshold_value)
{
    ...
    for (dim0 = start_index[0]; dim0 <= end_index[0]; dim0++)
    {
        ...
        SHR_IF_ERR_EXIT(soc_dnxf_drv_link_to_block_mapping
                        (unit, dim0, &fmac_index, &fmac_inner_link, SOC_DNXF_DRV_BLOCK_TYPE_FMAC));
        ...
        for (dim1 = start_index[1]; dim1 <= end_index[1]; dim1++)
        {
            ...
            if (set_or_get == _SHR_FABRIC_FLAG_SET)
            {
                ...
                SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cgm_register_set,
                                                      (unit, reg_name, field_name, fmac_index, fmac_inner_link,
                                                       *threshold_value)));
            }
            else if (set_or_get == _SHR_FABRIC_FLAG_GET)
            {
    ...
    int threshold_value)
{
    ...
    block_id =
        dnxf_data_fabric.congestion.thresholds_info_get(unit,
                                                        threshold_type)->is_tx ? SOC_DNXF_DRV_BLOCK_TYPE_DCML :
