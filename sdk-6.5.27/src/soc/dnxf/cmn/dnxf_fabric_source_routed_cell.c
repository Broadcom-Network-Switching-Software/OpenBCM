    ...
    uint32 *data_in)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_route2sr_link_list(unit, route, &sr_link_list));

    if (dnxf_data_fabric.rxtx.feature_get(unit, dnxf_data_fabric_rxtx_pkt_dma_supported))
    {

        SHR_IF_ERR_EXIT(bcm_pkt_alloc(unit, WORDS2BYTES(data_in_size), 0, &tx_pkt));
        sal_memcpy(tx_pkt->pkt_data->data, data_in, tx_pkt->pkt_data->len);

        SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_tx_prepare, (unit, tx_pkt, &sr_link_list)));

        SHR_IF_ERR_EXIT(dnxf_pkt_dma_tx(unit, tx_pkt, NULL));

    }
    else
    {
        SHR_IF_ERR_EXIT(soc_dnxf_cpu2cpu_write(unit, &sr_link_list, data_in_size, data_in));
    }

exit:
    if (dnxf_data_fabric.rxtx.feature_get(unit, dnxf_data_fabric_rxtx_pkt_dma_supported))
    {
        SHR_IF_ERR_EXIT(bcm_pkt_free(unit, tx_pkt));
    }
    ...
    uint32 *data_out)
{
    ...
    sal_memset(&cell, 0x00, sizeof(dnxc_sr_cell_t));
    DNXF_UNIT_LOCK_TAKE(unit);
