    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_soft_init, (unit, SOC_DNXC_RESET_ACTION_IN_RESET)));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_soft_init, (unit, SOC_DNXC_RESET_ACTION_OUT_RESET)));
...
int
soc_dnxf_drv_fmac_lane_real_get(
    int unit,
    int aligned_lane_id,
    int *real_lane_id)
{
    int nof_links_in_fmac, nof_links_in_fsrd, pm_idx, inner_lane;
    ...
    *real_lane_id = aligned_lane_id;
        ...
        dnxf_data_device.general.feature_get(unit, dnxf_data_device_general_is_not_all_cores_active))
    {
        ...
        pm_idx = (aligned_lane_id / nof_links_in_fmac) * nof_links_in_fsrd;
        inner_lane = aligned_lane_id % nof_links_in_fmac;

        if (pm_idx <
            (dnxf_data_device.blocks.nof_instances_fsrd_get(unit) / dnxf_data_device.general.nof_cores_get(unit)))
        {
            if (!is_core_zero_active)
            {
                inner_lane += nof_links_in_fmac;
            }
        ...
        *real_lane_id = pm_idx + inner_lane;
    }
    ...
    int unit)
{
    ...
    {
        for (phy_port = 0; phy_port < 512; phy_port++)
            si->port_type[phy_port] = 1;
    }
#endif
exit:
    SHR_FUNC_EXIT;
    ...
    int unit)
{
    ...
    if (soc_feature(unit, soc_feature_sbusdma))
    {
                                 ...
                                 SOC_MEM_BLOCK_ANY(unit, RTP_MULTI_CAST_TABLE_UPDATEm))))
        {
            ...
            SHR_IF_ERR_EXIT(soc_sbusdma_deinit(unit));
    ...
    int unit)
{
    ...
    if (soc_feature(unit, soc_feature_sbusdma))
    {
                                 ...
                                 SOC_MEM_BLOCK_ANY(unit, RTP_MULTI_CAST_TABLE_UPDATEm))))
        {
            ...
            SHR_IF_ERR_EXIT(soc_sbusdma_init
                            (unit, SOC_CONTROL(unit)->sbusDmaTimeout, SOC_CONTROL(unit)->sbusDmaIntrEnb));
    ...
    int *nof_block_instances)
{
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_nof_block_instance, (unit, block_type, nof_block_instances));
    SHR_IF_ERR_EXIT(rc);
    ...
    int *is_sku)
{
    ...
    *is_sku = 0;
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_drv_supported_lanes_get(unit, &supported_lanes));
    SOC_PBMP_COUNT(supported_lanes, supported_lanes_nof);

    if (supported_lanes_nof !=
        (dnxf_data_device.blocks.nof_instances_fmac_get(unit) * dnxf_data_device.blocks.nof_links_in_fmac_get(unit)))
    {
        *is_sku = 1;
    }
    ...
    soc_pbmp_t * block_bmp)
{
    ...
    if (block_stage == soc_dnxf_fabric_device_stage_fe1)
    {
        block_stage =
            block_type ==
            SOC_DNXF_DRV_BLOCK_TYPE_DCH ? soc_dnxf_fabric_device_stage_fe1 : soc_dnxf_fabric_device_stage_fe3;
    }
    else if (block_stage == soc_dnxf_fabric_device_stage_fe3)
    {
        block_stage =
            block_type ==
            SOC_DNXF_DRV_BLOCK_TYPE_DCH ? soc_dnxf_fabric_device_stage_fe3 : soc_dnxf_fabric_device_stage_fe1;
    }
    else
    {
    ...
    soc_dnxf_fabric_device_stage_t * link_device_stage)
{
    ...
    SHR_IF_ERR_EXIT(soc_dnxf_drv_link_to_block_mapping
                    (unit, port, &dch_instance, &dch_inner_link, SOC_DNXF_DRV_BLOCK_TYPE_DCH));
...
int
soc_dnxf_drv_access_block_convert(
    int unit,
    ...
    int *soc_block_type)
{
    ...
    *soc_block_type = access_block_type;
#ifdef BCM_ACCESS_SUPPORT
    switch (access_block_type)
    {
        case BLOCK_FMAC:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_FMAC;
            break;
        case BLOCK_FSRD:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_FSRD;
            break;
        case BLOCK_DCH:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_DCH;
            break;
        case BLOCK_DFL:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_DFL;
            break;
        case BLOCK_DTL:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_DTL;
            break;
        case BLOCK_DTM:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_DTM;
            break;
        case BLOCK_LCM:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_LCM;
            break;
        case BLOCK_QRH:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_QRH;
            break;
        case BLOCK_CCH:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_CCH;
            break;
        case BLOCK_CCML:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_CCML;
            break;
        case BLOCK_DFL_FBC:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_DFL_FBC;
            break;
        case BLOCK_RTP:
            *soc_block_type = SOC_DNXF_DRV_BLOCK_TYPE_RTP;
            break;
        default:
    ...
    int block_type)
{
    ...
    switch (block_type)
    {
        case SOC_DNXF_DRV_BLOCK_TYPE_RTP:
            *block_id = rtp_link / nof_links_per_core;
            *inner_link = rtp_link % nof_links_per_core;
            break;
        case SOC_DNXF_DRV_BLOCK_TYPE_FMAC:
            *block_id = fmac_lane / nof_links_in_fmac;
            *inner_link = fmac_lane % nof_links_in_fmac;
            break;
        case SOC_DNXF_DRV_BLOCK_TYPE_DCML:
        case SOC_DNXF_DRV_BLOCK_TYPE_DTM:
        case SOC_DNXF_DRV_BLOCK_TYPE_DTL:
        case SOC_DNXF_DRV_BLOCK_TYPE_LCM:
        case SOC_DNXF_DRV_BLOCK_TYPE_DFL:
        case SOC_DNXF_DRV_BLOCK_TYPE_DCH:
        case SOC_DNXF_DRV_BLOCK_TYPE_CCH:
        case SOC_DNXF_DRV_BLOCK_TYPE_CCML:
        case SOC_DNXF_DRV_BLOCK_TYPE_QRH:
            *block_id = rtp_link / nof_links_dch;
            *inner_link = rtp_link % nof_links_dch;
            break;
        case SOC_DNXF_DRV_BLOCK_TYPE_FSRD:
            *block_id = fmac_lane / nof_links_in_fsrd;
            ...
            break;
        case SOC_DNXF_DRV_BLOCK_TYPE_DFL_FBC:
    ...
    int return_only_full)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL
                    (unit, mbcm_dnxf_drv_fmac_bmp_to_internal_block_bmp_convert,
                     (unit, fmac_bmp, internal_block_bmp, block_type, return_only_full)));
...
shr_error_e
soc_dnxf_drv_virtual_fmac_lane_get(
    int unit,
    int fmac_lane,
    int *virtual_fmac_lane)
{
    ...
    if (dnxf_data_device.general.feature_get(unit, dnxf_data_device_general_is_not_all_cores_active)
        && !is_core_zero_active)
    {
        if (fmac_lane < dnxf_data_device.blocks.nof_links_in_rtp_get(unit))
        {
            *virtual_fmac_lane = fmac_lane - dnxf_data_device.blocks.nof_links_in_fmac_get(unit);
        }
        else
        {
            *virtual_fmac_lane = fmac_lane + dnxf_data_device.blocks.nof_links_in_fmac_get(unit);
...
shr_error_e
soc_dnxf_drv_virtual_block_get(
    int unit,
    int block_type,
    int block_idx,
    int inner_link,
...
{
    int fmac_lane, virtual_fmac_lane;
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(soc_dnxf_drv_block_to_fmac_lane_mapping(unit, block_idx, inner_link, block_type, &fmac_lane));

    SHR_IF_ERR_EXIT(soc_dnxf_drv_virtual_fmac_lane_get(unit, fmac_lane, &virtual_fmac_lane));

    SHR_IF_ERR_EXIT(soc_dnxf_drv_fmac_lane_to_block_mapping
                    (unit, virtual_fmac_lane, virtual_block_idx, virtual_inner_link, block_type));
