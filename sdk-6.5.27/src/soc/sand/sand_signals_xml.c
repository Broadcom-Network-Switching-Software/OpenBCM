    ...
    RHDATA_ITERATOR(cur, curSubTop, "structure")
    {
        ...
        if(!ISEMPTY(name))
        {
            ...
            else  if (SOC_IS_DNX(unit))
            {
                ...
                else if(!sal_strncasecmp(name, "trap_code", sizeof(name)))
                {
                    sigstruct->expansion_cb = trap_code_decode;
                }
            }
#endif 
            ...
            if((pp_stage != DNX_PP_STAGE_INVALID) && (dnx_data_pp.stages.params_get(unit, pp_stage)->is_pem))
            {
                ...
                if(sdk_block_id == pemla_block)
                {
                    ...
                    for(vw_mappping_idx = 0; vw_mappping_idx < virtual_wires->vw_info_arr[vw_idx].nof_mappings; vw_mappping_idx++)
                    {
                        ...
                        if(sand_signal_handle_get(unit, core, SIGNALS_MATCH_PEM, dnx_pp_block_name(unit, sdk_block_id), "PEM", dnx_pp_stage_name(unit, pp_stage),
                    ...
                    for(vw_mappping_idx = 0; vw_mappping_idx < virtual_wires->vw_info_arr[vw_idx].nof_mappings; vw_mappping_idx++)
                    {
                        ...
                        int core = 0; 
                        int msb_offset = virtual_wires->vw_info_arr[vw_idx].vw_mappings_arr[vw_mappping_idx].virtual_wire_msb - virtual_wires->vw_info_arr[vw_idx].vw_mappings_arr[vw_mappping_idx].virtual_wire_lsb;
                        if(sand_signal_handle_get(unit, core, SIGNALS_MATCH_PEM, dnx_pp_block_name(unit, sdk_block_id), dnx_pp_stage_name(unit, pp_stage), "PEM",
                                  virtual_wires->vw_info_arr[vw_idx].vw_mappings_arr[vw_mappping_idx].physical_wire_name, &signal) != _SHR_E_NONE)
                        {
                            break;
                        }
    ...
    device_t * device)
{
    ...
    RHDATA_ITERATOR(curBlock, curSubTop, "block")
    {
        ...
        SHR_IF_ERR_EXIT(sand_signal_get_stages(unit, cur_pp_block, curBlock));
        {
            ...
            if (SOC_IS_DNX(unit))
            {
                core_limit = dnx_data_device.general.nof_cores_get(unit);
            }
#endif
          for(i_core = 0; i_core < core_limit; i_core++)
            {
                for (i_stage = 0; i_stage < cur_pp_block->stage_num; i_stage++)
                {
                    if (cur_pp_block->stages[i_stage].stage_debug_signals[i_core] == NULL)
                    {
                        if((cur_pp_block->stages[i_stage].stage_debug_signals[i_core] =
                    ...
                    sal_memset(cur_pp_block->stages[i_stage].stage_debug_signals[i_core], 0, sizeof(debug_signal_t) * DNX_MAX_NOF_DEBUG_SIGNALS);
                    cur_pp_block->stages[i_stage].stage_signal_num[i_core] = 0;
                }
            }
...
#endif
                if(sand_signal_handle_get(unit, core, 0,cur_pp_block->name,
...
sand_signal_deinit(
    ...
    device_t * device)
{
    int i_ind, i_st, i_core= 0, core_limit = 1;
        ...
        if(device->pp_blocks != NULL)
        {
            for(i_core = 0; i_core < core_limit; i_core++)
            {
                for(i_st = 0; i_st < device->pp_blocks[i_ind].stage_num; i_st++)
                {
                    ...
                    {
                        utilex_free(device->pp_blocks[i_ind].stages[i_st].stage_debug_signals[i_core]);
                    }
                }
            }
