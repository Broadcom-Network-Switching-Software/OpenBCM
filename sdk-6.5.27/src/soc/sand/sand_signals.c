...
trap_code_decode(
    ...
    char to_n[RHNAME_MAX_SIZE])
{
	signal_output_t *trap_code_parse_sig = NULL;
    char trap_code_dbal_field[DSIG_MAX_SIZE_STR] = "INGRESS_TRAP_ID";
    char trap_code_parse_string[DSIG_MAX_SIZE_STR] = "";
    char trap_code_value_string[DSIG_MAX_SIZE_STR] = "";
    uint8 trap_name[BCM_RX_TRAP_MAX_NAME_LEN];
    rhhandle_t rhhandle;
    signal_output_t *new_sig;
    ...
    if (data_size_bits != TRAP_CODE_SIG_SIZE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Need %db for Context signal parsing. %db given.",
                     TRAP_CODE_SIG_SIZE, data_size_bits);
    }

    
    SHR_IF_ERR_EXIT(dbal_decode(unit, SOC_CORE_DEFAULT, trap_code_dbal_field, data, TRAP_CODE_SIG_SIZE, parsed_info, NULL, NULL));

    if (NULL == *parsed_info)
    {
        
        SHR_ERR_EXIT(_SHR_E_FAIL, "Could not find decoding for dbal field: %s and data: %d\n", trap_code_dbal_field, *data);
    }

    
    trap_code_parse_sig =  utilex_rhlist_entry_get_first(*parsed_info);
    sal_strncpy_s(trap_code_parse_string, trap_code_parse_sig->print_value, DSIG_MAX_SIZE_STR-1);
    sal_strncpy_s(trap_code_parse_sig->print_value, trap_code_dbal_field, DSIG_MAX_SIZE_STR-1);
    sal_strncat_s(trap_code_parse_sig->print_value, "_", sizeof(trap_code_parse_sig->print_value));
    sal_strncat_s(trap_code_parse_sig->print_value, trap_code_parse_string, sizeof(trap_code_parse_sig->print_value));
    sal_strncat_s(trap_code_parse_sig->print_value, "(0x", sizeof(trap_code_parse_sig->print_value));
    sal_itoa(trap_code_value_string, data[0], 16, 0, 1);
    sal_strncat_s(trap_code_parse_sig->print_value, trap_code_value_string, sizeof(trap_code_parse_sig->print_value));
    sal_strncat_s(trap_code_parse_sig->print_value, ")", sizeof(trap_code_parse_sig->print_value));

    SHR_IF_ERR_EXIT(dnx_rx_trap_ingress_ud_name_get(unit, *data, trap_name));

    if (trap_name[0] != '\0')
    {
        
        SHR_IF_ERR_EXIT(utilex_rhlist_entry_add_tail(*parsed_info, "TRAP_NAME", RHID_TO_BE_GENERATED, &rhhandle));
        new_sig = rhhandle;
        new_sig->size = TRAP_CODE_SIG_SIZE;
        sal_strncpy_s(new_sig->print_value, (char*) trap_name, DSIG_MAX_SIZE_STR);
        new_sig->value[0] = *data;
    }
    ...
    char *value_str)
{
    ...
    if(sand_signal_handle_get(unit, core, 0, block_n, from_n, NULL, tokens[0], &debug_signal) != _SHR_E_NONE)
    {
...
#ifdef ADAPTER_SERVER_MODE
    if (!ISEMPTY(match_p->to) && (sal_strncasecmp(match_p->to, debug_signal->from, RHSTRING_MAX_SIZE) == 0))
    {
    ...
    match_t * match_p)
{
    ...
    if(debug_signal == NULL)
    {
        ...
        if ( dbal_physical_table_string_to_id_no_error(unit, match_p->from, &phy_table_id) == _SHR_E_NONE  ||
             ...
             sand_signal_is_parsed_as_debug_signal(unit, match_p))
        {
            return _SHR_E_NONE;
        }
    }
    ...
    else
    {
        ...
        if (!(match_p->flags & SIGNALS_MATCH_ASIC) && !(match_p->flags & SIGNALS_MATCH_PEM))
        {
            if (!ISEMPTY(match_p->to)   && !ISEMPTY(to_n)   && (sal_strcasestr(to_n, match_p->to)     == NULL)
                    && (sal_strcasestr(from_n, match_p->to) == NULL) )
                return _SHR_E_NOT_FOUND;
        }
        else
#endif
    ...
    match_t * match_p)
{
    ...
    if(!ISEMPTY(match_p->to) && !ISEMPTY(debug_signal->to) && (sal_strcasestr(debug_signal->to, match_p->to)  == NULL))
    {
        return _SHR_E_NOT_FOUND;
    }
    else if(!ISEMPTY(match_p->from) && !ISEMPTY(debug_signal->from)   && (sal_strcasestr(debug_signal->from, match_p->from)  == NULL))
    {
        return _SHR_E_NOT_FOUND;
    }
    ...
    for (i_bl = 0; i_bl < device->block_num; i_bl++)
    {
        ...
        if(!(match_p->flags & SIGNALS_MATCH_ASIC) && !(match_p->flags & SIGNALS_MATCH_PEM))
        {
            ...
            for (i_st = 0; i_st < cur_pp_block->stage_num; i_st++)
            {
                ...
                if(sand_signal_filter_stage(unit, NULL, cur_pp_stage, cur_pp_stage->stage_to, match_p) != _SHR_E_NONE)
                    continue;

                
                if(!(match_p->flags & SIGNALS_MATCH_NOVALUE) && (cur_pp_stage->stage_signal_num[core] == 0))
                {
                    int rv;
                ...
                for (i_sig = 0; i_sig < cur_pp_stage->stage_signal_num[core]; i_sig++)
                {
                    debug_signal_t *debug_signal = &cur_pp_stage->stage_debug_signals[core][i_sig];
                    if (sand_signal_filter_mdb_signal(unit, debug_signal, match_p) != _SHR_E_NONE)
                    {
                        continue;
                    }
...
#endif
        {
            for (i_sig = 0; i_sig < cur_pp_block->block_signal_num; i_sig++)
            {
                ...
                if(sand_signal_filter_stage(unit, debug_signal, debug_signal->stage_from, debug_signal->stage_to, match_p)
                                                                                                        != _SHR_E_NONE)
                    continue;
