    ...
    parsing_db_t * pdb)
{
    ...
    next_headers_t * hdr_obj = NULL;

    RHITERATOR(hdr_obj, pdb->next_headers_lst)
    {
        utilex_rhlist_free_all(hdr_obj->option_lst);
    }
    ...
    pparse_condition_t * cond)
{
...
#ifdef BCM_DNX_SUPPORT
    if (!sal_strcasecmp(cb_name, "packet_eth"))
    {
        cond->content.plugin.callback = packet_eth;
    }
    ...
    if (!sal_strcasecmp(cb_name, "packet_rch_srv6_usp_psp") &&
         dnx_data_port.general.feature_get(unit, dnx_data_port_general_is_header_type_rch_srv6_usp_psp_supported))
    {
    ...
    if (!sal_strcasecmp(cb_name, "packet_ftmh_base"))
    {
        cond->content.plugin.callback = packet_ftmh_base;
    }
    ...
    if (!sal_strcasecmp(cb_name, "packet_post_mpls"))
    {
        cond->content.plugin.callback = packet_post_mpls;
    }
#endif
...
static shr_error_e
pparse_pdb_dynamic_next_headers_add(
    int unit,
    parsing_db_t * pdb)
{
    uint32 enum_val;
    ...
    char obj_name[RHNAME_MAX_SIZE], protocol_name[RHNAME_MAX_SIZE];
    ...
    
    {
        ...
        SHR_IF_ERR_EXIT(utilex_rhlist_entry_add_tail(pdb->next_headers_lst, "RCH_Next_Protocol", RHID_TO_BE_GENERATED, &rhhandle));
        ...
        SHR_IF_ERR_EXIT(dbal_fields_enum_next_enum_value_get(unit, DBAL_FIELD_CURRENT_PROTOCOL_TYPE, INVALID_ENUM, &enum_val));
        while (enum_val != INVALID_ENUM)
        {
             sal_strncpy(obj_name, dbal_enum_id_to_string(unit, DBAL_FIELD_CURRENT_PROTOCOL_TYPE, enum_val) , RHNAME_MAX_SIZE - 1);
             SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get(unit, DBAL_FIELD_CURRENT_PROTOCOL_TYPE, enum_val, &hw_value));

             if(sal_strncmp(obj_name, "ETHERNET", sal_strlen("ETHERNET")) == 0)
             {
                 sal_strncpy(protocol_name, "ETHERNET", RHNAME_MAX_SIZE - 1);
             }
             else if(sal_strncmp(obj_name, "IPV4", sal_strlen("IPV4")) == 0)
             {
                 
                 sal_strncpy(protocol_name, "IPV4", RHNAME_MAX_SIZE - 1);
             }
             else if(sal_strncmp(obj_name, "IPV6", sal_strlen("IPV6")) == 0)
             {
                 
                 sal_strncpy(protocol_name, "IPV6", RHNAME_MAX_SIZE - 1);
             }
             else if(sal_strncmp(obj_name, "MPLS", sal_strlen("MPLS")) == 0)
             {
                 sal_strncpy(protocol_name, "MPLS", RHNAME_MAX_SIZE - 1);
             }
             else
             {
                 SHR_IF_ERR_EXIT(dbal_fields_enum_next_enum_value_get(unit, DBAL_FIELD_CURRENT_PROTOCOL_TYPE, enum_val, &enum_val));
                 continue;
             }

             
             SHR_IF_ERR_EXIT(utilex_rhlist_entry_add_tail
                             (nxt_headers->option_lst, protocol_name, RHID_TO_BE_GENERATED, &rhhandle));
             option_obj = rhhandle;
             option_obj->field_value = hw_value;
...
#ifdef BCM_DNX_SUPPORT
        SHR_IF_ERR_EXIT(pparse_pdb_dynamic_next_headers_add(unit, pdb));
#endif
    }
    ...
    rhlist_t * pit)
{
    ...
    signal_output_t *recycle_cmd_prt_profile_sig = NULL;
    ...
    while (NULL != (next = pparse_list_get_next(unit, parse_list)))
    {
        ...
        if (next->pdb_entry->size > 0)
        {
            ...
            if (port_skip_support && met_real_header == FALSE)
            {
                ...
                rv = sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP",
                              is_header_vis_global ? "NIF" : dnx_data_pp.debug.from_signal_str_get(unit)->ptc,
                ...
                if ((rv == _SHR_E_NONE) && sal_strcasecmp(ptc_sig->print_value, "N/A"))
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_algo_port_db.in_tm.ref_count.get(unit, core, ptc, &valid));
                    if (valid)
                    {

                        
                        SHR_IF_ERR_EXIT(dnx_algo_port_in_tm_to_logical_get(unit, core, ptc, &logical_port));
                            ...
                            if (port_first_header_size_data.first_header_size_to_skip > 0)
                            {
                                ...
                                if ((port_first_header_size_data.is_before) && (port_first_header_size_data.is_after))
                                {
                                    SHR_ERR_EXIT(_SHR_E_NONE,
                                                  ...
                                                  "Stopping.\r\n", ptc, port_first_header_size_data.first_header_size_to_skip);
                ...
                rv = sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", is_header_vis_global ? "NIF" : NULL,
                                             is_header_vis_global ? "PRT" : NULL , "recycle_cmd", &recycle_cmd_prt_profile_sig);

                
                if ((rv == _SHR_E_NONE) && sal_strcasecmp(recycle_cmd_prt_profile_sig->print_value, "N/A"))
                {
                    if (recycle_cmd_prt_profile_sig->value[0] == DBAL_ENUM_FVAL_RECYCLE_CMD_PRT_PROFILE_ETH_APPEND64)
                    {
                        SHR_IF_ERR_EXIT(dnx_port_ingress_prt_info_layer_offset_get(unit, DBAL_ENUM_FVAL_RECYCLE_CMD_PRT_PROFILE_ETH_APPEND64,
                        DBAL_ENUM_FVAL_PORT_TERMINATION_PTC_PROFILE_ETHERNET, &ptch_skip_before_size_bytes));
                    }
                }
            ...
            if (rv != _SHR_E_NONE)
            {
                if(sal_strcmp("NWK_AFTER_PPH", RHNAME(next)) == 0)
                {
                    SHR_EXIT();
                }
            ...
            if (!sal_strcasecmp(next->rhentry.name, "RCH") &&
                dnx_data_port.general.feature_get(unit, dnx_data_port_general_is_rch_header_offset_supported))
            {
...
exit:
    ...
    sand_signal_output_free(recycle_cmd_prt_profile_sig);
    SHR_FUNC_EXIT;
}
...
#define PPARSE_MPLS_UNTERM_LAYER_INDEX  2
    ...
    bcm_port_t *src_port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_ptc_to_port_get(unit, ptc, core, &logical_port));
    if (logical_port == -1)
    {
        *src_port = -1;
        SHR_EXIT();
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_in_lag(unit, logical_port, &in_lag));
    if (in_lag)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_lag_id_get(unit, logical_port, &lag_id));
...
static shr_error_e
packet_src_port_header_type_get(
    int unit,
    int core,
    int *header_type)
{
    ...
    SHR_NULL_CHECK(header_type, _SHR_E_PARAM, "header_type");

    SHR_IF_ERR_EXIT(packet_src_port_get(unit, core, &gport));
    if (gport == -1)
    {
        *header_type = -1;
        SHR_EXIT();
    ...
    SHR_IF_ERR_EXIT(dnx_switch_header_type_in_get(unit, gport, FALSE, header_type));
    ...
    uint32 *enabled)
{
    ...
    *enabled = 0;
    ...
    SHR_IF_ERR_EXIT(packet_src_port_header_type_get(unit, core, &switch_header_type));
    ...
    uint32 *enabled)
{
    ...
    *enabled = 0;
    ...
    if (switch_header_type == BCM_SWITCH_PORT_HEADER_TYPE_STACKING)
    {
        *enabled = 1;
    }
    ...
    uint32 *enabled)
{
    ...
    *enabled = 0;
    ...
    rv = sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", "VTT1", "VTT2" , "Layer_Protocols", &layer_protocol_0_sig);
    if (rv == _SHR_E_NOT_FOUND)
    {
        SHR_EXIT();
    }
    ...
    uint32 *enabled)
{
    ...
    *enabled = 0;
    ...
    SHR_IF_ERR_EXIT(sand_signal_output_find(unit, core, 0, "ERPP", "ERparser", NULL, "is_TDM", &is_tdm_sig));
...
exit:
    sand_signal_output_free(is_tdm_sig);
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
...
packet_post_mpls(
    ...
    uint32 *enabled)
{
    uint32 global_in_lif = 0;
    uint32 fwd_layer_index, fwd_layer_index_vt4, mpls_unterm_layer_index;
    uint32 layer_protocol_value, layer_protocol_offset;
    bcm_gport_t gport_id;
    uint32 lif_flags;
    uint32 entry_handle_id;
    uint32 mpls_term_profile;
    uint32 layer_protocol_after_mpls;
    uint32 layer_protocol_bit_size;
    int ref_count, rv;
    dbal_core_mode_e core_mode;
    dnx_algo_gpm_gport_hw_resources_t gport_hw_resources;
    dnx_mpls_termination_profile_t mpls_term_profile_data;
    dbal_table_field_info_t field_info;

    signal_output_t *sig_glif_p = NULL;
    signal_output_t *sig_proto_p = NULL;
    signal_output_t *sig_fwd_index_p = NULL;
    signal_output_t *sig_fwd_index_vt4_p = NULL;
    signal_output_t *sig_layer_protocol_p = NULL;
    packet_plugin_arg_t *arg_entry = NULL;
    char * last_vtt_stage = "VTT5";
    char * penult_vtt_stage = "VTT4";
    ...
    *enabled = FALSE;
    ...
    DNX_PP_BLOCK_IS_READY_EXIT(core, DNX_PP_BLOCK_IRPP, FALSE);
    mpls_unterm_layer_index = PPARSE_MPLS_UNTERM_LAYER_INDEX;

    
    rv = sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", "VTT1", "VTT2", "Layer_Protocols", &sig_layer_protocol_p);
    if (rv != _SHR_E_NONE)
    {
        SHR_EXIT();
    }
    layer_protocol_bit_size = utilex_log2_round_up(dnx_data_device.general.max_nof_layer_protocols_get(unit));
    for (layer_protocol_offset = 0; layer_protocol_offset < sig_layer_protocol_p->size;)
    {
        SHR_IF_ERR_EXIT(utilex_bitstream_get_field
                        (sig_layer_protocol_p->value, layer_protocol_offset, layer_protocol_bit_size, &layer_protocol_value));
        SHR_IF_ERR_EXIT(dbal_fields_enum_value_get(unit, DBAL_FIELD_LAYER_TYPES, layer_protocol_value, &layer_protocol_value));
        if (layer_protocol_offset == 0 &&
            (layer_protocol_value == DBAL_ENUM_FVAL_LAYER_TYPES_ITMH ||
             layer_protocol_value == DBAL_ENUM_FVAL_LAYER_TYPES_ITMH_J1_VAL ||
             layer_protocol_value == DBAL_ENUM_FVAL_LAYER_TYPES_INGRESS_SCTP_EGRESS_FTMH))
        {
            
            SHR_EXIT();
        }
        if (layer_protocol_value == DBAL_ENUM_FVAL_LAYER_TYPES_MPLS_UNTERM)
        {
            mpls_unterm_layer_index = layer_protocol_offset / layer_protocol_bit_size;
            break;
        }
        else
        {
            layer_protocol_offset += layer_protocol_bit_size;
        }
    }

    
    rv = sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP",
            dnx_data_pp.debug.from_signal_str_get(unit)->ingress_layers, "FWD1", "Fwd_Layer_Index", &sig_fwd_index_p);
    if (rv != _SHR_E_NONE)
    {
        
        SHR_EXIT();
    }
    fwd_layer_index = sig_fwd_index_p->value[0];

    if (fwd_layer_index <= mpls_unterm_layer_index)
    {
    ...
    SHR_IF_ERR_EXIT_NO_MSG(sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", penult_vtt_stage, last_vtt_stage, "Fwd_Layer_Index", &sig_fwd_index_vt4_p));
    fwd_layer_index_vt4 = sig_fwd_index_vt4_p->value[0];

    
    if (fwd_layer_index_vt4 <= mpls_unterm_layer_index)
    {
        SHR_IF_ERR_EXIT_NO_MSG(sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", last_vtt_stage, "FWD1", "In_LIFs.0", &sig_glif_p));
    }
    else
    {
        SHR_IF_ERR_EXIT_NO_MSG(sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP", penult_vtt_stage, last_vtt_stage, "In_LIFs.0", &sig_glif_p));
    }
    global_in_lif = sig_glif_p->value[0];
    ...
    
    {
        ...
        rv = dnx_algo_lif_mapping_global_to_local_get(unit, DNX_ALGO_LIF_INGRESS, global_in_lif, &local_lif_payload);
        if (rv != _SHR_E_NONE)
        {
            SHR_EXIT();
        }
    ...
    arg_entry = utilex_rhlist_entry_get_by_name(args, "proto");
    SHR_NULL_CHECK(arg_entry, _SHR_E_NOT_FOUND, "arg_entry");

    rv = dnx_algo_gpm_gport_from_lif(unit, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_INGRESS,
                                     _SHR_CORE_ALL, global_in_lif, &gport_id);
    if (rv != _SHR_E_NONE)
    {
        SHR_EXIT();
    }
    ...
    rv = dnx_algo_gpm_gport_to_hw_resources(unit, gport_id, lif_flags, &gport_hw_resources);
    if (rv != _SHR_E_NONE)
    {
        SHR_EXIT();
    }

    rv = dbal_tables_field_info_get_no_err(unit, gport_hw_resources.inlif_dbal_table_id, DBAL_FIELD_TERMINATION_TYPE,
                                           FALSE, gport_hw_resources.inlif_dbal_result_type, INST_SINGLE, &field_info);
    if (rv != _SHR_E_NONE || field_info.field_id != DBAL_FIELD_TERMINATION_TYPE)
    {
        SHR_EXIT();
    }
    ...
    dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_TERMINATION_TYPE, INST_SINGLE, &mpls_term_profile);
    ...
    rv = algo_mpls_db.termination_profile.profile_data_get
                    (unit, mpls_term_profile, &ref_count, &mpls_term_profile_data);
    if (rv != _SHR_E_NONE)
    {
        SHR_EXIT();
    }
    if (mpls_term_profile_data.has_cw)
    {
        *enabled = !sal_strcasecmp(arg_entry->value, "Control_Word");
    }
    else
    {
        SHR_IF_ERR_EXIT_NO_MSG(sand_signal_output_find(unit, core, SIGNALS_MATCH_NO_FETCH_ALL, "IRPP",
               dnx_data_pp.debug.from_signal_str_get(unit)->ingress_layers, "FWD1", "Layer_Protocols.3", &sig_proto_p));
        SHR_IF_ERR_EXIT(dbal_fields_enum_value_get(unit, DBAL_FIELD_LAYER_TYPE, sig_proto_p->value[0], &layer_protocol_after_mpls));
        if (layer_protocol_after_mpls == DBAL_ENUM_FVAL_LAYER_TYPES_IPV4)
        {
            *enabled = !sal_strcasecmp(arg_entry->value, "IPv4");
        }
        else if (layer_protocol_after_mpls == DBAL_ENUM_FVAL_LAYER_TYPES_IPV6)
        {
            *enabled = !sal_strcasecmp(arg_entry->value, "IPv6");
        }
        else if (layer_protocol_after_mpls == DBAL_ENUM_FVAL_LAYER_TYPES_ETHERNET)
        {
            *enabled = !sal_strcasecmp(arg_entry->value, "ETH");
        }
        else if ((layer_protocol_after_mpls == DBAL_ENUM_FVAL_LAYER_TYPES_BIER_NON_MPLS) |
                 (layer_protocol_after_mpls == DBAL_ENUM_FVAL_LAYER_TYPES_IN_BIER_BASE_EG_BIER_MPLS))
        {
            *enabled = !sal_strcasecmp(arg_entry->value, "BIER");
        }
    }

exit:
    sand_signal_output_free(sig_glif_p);
    sand_signal_output_free(sig_proto_p);
    sand_signal_output_free(sig_fwd_index_p);
    sand_signal_output_free(sig_fwd_index_vt4_p);
    sand_signal_output_free(sig_layer_protocol_p);
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
#endif
    ...
    uint32 * parsed_size)
{
    ...
    {
        LOG_VERBOSE(BSL_LOG_MODULE, ("Parsing did not add a header to the parsed info tree.\n"));
...
static const pparse_nwk_sig_option_t nwk_sig_options[NOF_NWK_PROTOCOLS] = {
        ...
        [NWK_PROTO_IPV4_12B] = {160,   "IPV4"},
        [NWK_PROTO_IPV6_8B] = {320,   "IPV6"},
        [NWK_PROTO_PPP] = {16,   "PPP_NEXT_PROTOCOL"},
        [NWK_PROTO_ITMH] = {40,   "ITMH_Base"},
        [NWK_PROTO_FTMH] = {80,   "FTMH_Base"},
        [NWK_PROTO_RCH] = {240,   "RCH"},
};
    ...
    uint32 * parsed_size)
{
    ...
    
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_FAIL, "Expected MPLS header before speculative parser, but(%s) no such header found.", RHNAME(mpls_bos_sig));
        }
...
#define PPARSE_SRV6_EXTENSION_LENGTH_SIG  "Header_Length"
#define PPARSE_SRV6_TLV_MAX_BUFFER_UINT32_SIZE    80
...
#ifdef BCM_DNX_SUPPORT
    {
        uint32 buffer[PPARSE_SRV6_TLV_MAX_BUFFER_UINT32_SIZE];
        int tlv_size = (extension_length * 8) - (nof_segments * PPARSE_SRV6_SEGMENT_SIZE);
        int tlv_offset = bits_left - tlv_size;
        if( (tlv_size <= 0) || (tlv_offset < 0) || (tlv_size > WORDS2BITS(PPARSE_SRV6_TLV_MAX_BUFFER_UINT32_SIZE)) )
        {
            
            SHR_EXIT();
        }

        SHR_BITCOPY_RANGE(buffer, 0, data, tlv_offset, tlv_size);
        SHR_IF_ERR_EXIT(pparse_add_non_parsed_entry(unit, tlv_size,
                                                    tlv_size,
                                                    buffer, srv6_sig->field_list,
                                                    "SRH_TLV", "Skipped"));
    }
#endif 
exit:
    ...
    uint32 * parsed_size)
{
    ...
    signal_output_t * extension_header_sig = NULL;
    int data_size_bits = pdb_entry->size - (*parsed_size);
    ...
    if ((extension_header_sig = utilex_rhlist_entry_get_by_name(srv6_sig->field_list, PPARSE_SRV6_EXTENSION_LENGTH_SIG)) == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_FAIL, "Couldn't find '%s' field in %s parsed header.", PPARSE_SRV6_EXTENSION_LENGTH_SIG,
                     PPARSE_SRV6_BASE_SIG);
    }
    ...
    
    {
        ...
        int extension_length = extension_header_sig->value[0] * 8;
#else
        int extension_length = 0;
        int nof_segments = 0;
#endif
        int header_size = data_size_bits - PPARSE_SRV6_BASE_SIZE;
        SHR_IF_ERR_EXIT(pparse_srv6_segments_parse(unit, nof_segments, extension_length, data, header_size, srv6_sig));
    }
