    ...
    while ( data_index > 0)
    {
        ...
        else if (mdb_type != EEDB_MDB_TYPE)
        {
            ...
            SHR_IF_ERR_EXIT(sand_signals_adapter_get_interface_index(unit, stage_and_intrf/1000, stage_and_intrf%1000, memory_id,
                                         mdb_type, db_id, db_name, cur_pp_stage->pp_id, cur_pp_stage->name, interface_index));
        }
        ...
        for ( i = 0; i < nof_keys; i++ )
        {
            ...
            if ( skip_signal_flag == 0 )
            {
                if(mdb_type == DIRECT_MDB_TYPE)
                {
                    uint32 key_value = 0;
                    SHR_IF_ERR_EXIT(utilex_bitstream_get_field(adapter_value, data_index, key_sizes[i], &key_value));
                    key_value = bcm_ntohl(key_value);
                    SHR_IF_ERR_EXIT(add_stage_signal(unit, &key_value, 0, key_sizes[i], key_sizes[i], core,
    ...
    match_t * match_p)
{
    ...
    if(ISEMPTY(match_p->from) && ISEMPTY(match_p->to))
    {
        return FALSE;
    }

    for (i = 0; i < dnx_data_adapter.mdb.debug_signals_dest_mapping_info_get(unit)->key_size[0]; i++)
    {
        if((!ISEMPTY(match_p->to)   && sal_strcasestr(dnx_data_adapter.mdb.debug_signals_dest_mapping_get(unit, i)->to,   match_p->to) != NULL) ||
           (!ISEMPTY(match_p->from) && sal_strcasestr(dnx_data_adapter.mdb.debug_signals_dest_mapping_get(unit, i)->from, match_p->from) != NULL))
        {
            return TRUE;
        }
    }
...
static shr_error_e
sand_adapter_debug_signal_get_signal_attributes(
       int unit,
       char *signal_name,
       char *stage,
       char *signal_attribute,
       char *from,
       char *to)
{
    int i;
    ...
    for (i = 0; i < dnx_data_adapter.mdb.debug_signals_dest_mapping_info_get(unit)->key_size[0]; i++)
    {
        if(sal_strncmp(dnx_data_adapter.mdb.debug_signals_dest_mapping_get(unit, i)->key_signal_name, signal_name, sal_strnlen(signal_name, RHSTRING_MAX_SIZE)) == 0)
        {
            sal_snprintf(signal_attribute, RHSTRING_MAX_SIZE - 1, "%s", dnx_data_adapter.mdb.debug_signals_dest_mapping_get(unit, i)->signal_name);
            if(sal_strncmp(dnx_data_adapter.mdb.debug_signals_dest_mapping_get(unit, i)->from, "NotFound", RHSTRING_MAX_SIZE) != 0)
            {
                sal_snprintf(to, RHSTRING_MAX_SIZE - 1, "%s", stage);
                sal_snprintf(from, RHSTRING_MAX_SIZE - 1, "%s", dnx_data_adapter.mdb.debug_signals_dest_mapping_get(unit, i)->from);
            }
            else
            {
                sal_snprintf(from, RHSTRING_MAX_SIZE - 1, "%s", stage);
                sal_snprintf(to, RHSTRING_MAX_SIZE - 1, "%s", dnx_data_adapter.mdb.debug_signals_dest_mapping_get(unit, i)->to);
            }
            SHR_EXIT();
        }
    }
    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Signal name not found. signal_name: %s\n", signal_name);
...
static void
sand_signals_adapter_prepare_and_add_data(
    char *data_buffer,
    uint32 signal_data_len,
    uint32 *adapter_value)
{
    uint32 signal_data_len_in_bytes = signal_data_len/8;

    if(signal_data_len == 0)
    {
        data_buffer[0] = '0';
        data_buffer[1] = 'X';
        data_buffer[signal_data_len_in_bytes] = '\0';
    }
    else
    {
        data_buffer[signal_data_len_in_bytes] = 'X';
        data_buffer[signal_data_len_in_bytes + 1] = '0';
        data_buffer[signal_data_len_in_bytes + 2] = '\0';
        string_reverse(data_buffer, signal_data_len_in_bytes + 2);
    }
    parse_long_integer(adapter_value, DSIG_MDB_MAX_SIZE_UINT32, data_buffer);
}

static shr_error_e
sand_adapter_parse_debug_signal(
    int unit,
    int signal_size,
    int core,
    uint32 * adapter_value,
    pp_stage_t *cur_pp_stage)
{
    debug_signal_t *debug_signal;
    uint32 data_index = signal_size;
    uint32 signal_name_len = 0, signal_data_len = 0, signal_real_data_len_in_bits = 0;
    char signal_name[RHNAME_MAX_SIZE] = {0}, data_buffer[DSIG_MDB_MAX_SIZE_STR] = {0};
    SHR_FUNC_INIT_VARS(unit);
    ...
    while ( data_index > 0)
    {
        ...
        data_index-=16;
        SHR_IF_ERR_EXIT(utilex_bitstream_get_field(adapter_value, data_index, 16, &signal_name_len));
        signal_name_len = bcm_ntohs(signal_name_len);

        data_index-=signal_name_len;
        SHR_IF_ERR_EXIT(utilex_bitstream_get_any_field(
                adapter_value, data_index, signal_name_len, (uint32*)signal_name));
        signal_name[signal_name_len/8] = '\0';
        string_reverse(signal_name, signal_name_len/8);

        data_index-=16;
        SHR_IF_ERR_EXIT(utilex_bitstream_get_field(adapter_value, data_index, 16, &signal_data_len));
        signal_data_len = bcm_ntohs(signal_data_len);

        data_index-=signal_data_len;
        SHR_IF_ERR_EXIT(utilex_bitstream_get_any_field(
                adapter_value, data_index, signal_data_len, (uint32*)data_buffer));
        sand_signals_adapter_prepare_and_add_data(data_buffer, signal_data_len, debug_signal->adapter_value);

        
        data_index-=16;
        SHR_IF_ERR_EXIT(utilex_bitstream_get_field(adapter_value, data_index, 16, &signal_real_data_len_in_bits));
        signal_real_data_len_in_bits = bcm_ntohs(signal_real_data_len_in_bits);
        debug_signal->size = (signal_real_data_len_in_bits > 0) ? signal_real_data_len_in_bits : signal_data_len/2;

        SHR_IF_ERR_EXIT(sand_adapter_debug_signal_get_signal_attributes(unit,
                     signal_name, cur_pp_stage->name, debug_signal->attribute, debug_signal->from, debug_signal->to));
    }

    if (data_index != 0)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Parsing error, data_index expected to be 0. "
                "data_index: %d \n", data_index);
    }
        ...
        int flags)
{
    ...
    for(i_sig = 0; i_sig < adapter_info.nof; i_sig++)
    {
        ...
        else if(sal_strncmp(signal_name, "General_debug_signal", RHNAME_MAX_SIZE) == 0)
        {
            uint32 debug_signal_value[DSIG_MDB_MAX_SIZE_UINT32];
            char debug_signal_str[DSIG_MDB_MAX_SIZE_STR + 1];
            ...
            sal_memset(debug_signal_value, 0, sizeof(debug_signal_value));
            sal_memset(debug_signal_str, 0, DSIG_MDB_MAX_SIZE_STR + 1);
            ADAPTER_INFO_MDB_GET_MEM(debug_signal_str, (signal_size + 3) / 4 + 2, &adapter_info);
            parse_long_integer(debug_signal_value, DSIG_MDB_MAX_SIZE_UINT32, debug_signal_str);
            SHR_IF_ERR_EXIT(sand_adapter_parse_debug_signal(unit, signal_size, core, debug_signal_value, cur_pp_stage));
            continue;
        }
