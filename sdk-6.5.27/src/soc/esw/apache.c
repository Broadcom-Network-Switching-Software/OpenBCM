...
soc_apache_mem_config(int unit)
{
    ...
    if (alpm_enable) {
        ...
        if (shared_l2_banks + shared_l3_banks > 0) {
            ...
            index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_RAWm, index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV4m, index_max);
            index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV4m, index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV4_1m, index_max);
            index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV4_1m, index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_64m, index_max);
            index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_64m, index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_64_1m, index_max);
            index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_64_1m, index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_128m, index_max);
            index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_128m, index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_HIT_ONLYm, index_max);
            index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_HIT_ONLYm, index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_ECCm, index_max);
            index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_ECCm, index_max);
        }
    } else {
    ...
    if (SOC_SWITCH_BYPASS_MODE(unit) != SOC_SWITCH_BYPASS_MODE_NONE) {
        if (SOC_SWITCH_BYPASS_MODE(unit) == SOC_SWITCH_BYPASS_MODE_L3_AND_FP) {
            ...
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_GLOBAL_MASK_TCAMm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_GM_FIELDSm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_TCAMm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_SLICE_MAPm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_POLICY_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_METER_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_COUNTER_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_STORM_CONTROL_METERSm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_SLICE_KEY_CONTROLm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_PORT_METER_MAPm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_I2E_CLASSID_SELECTm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_HG_CLASSID_SELECTm, -1);

            SOP_MEM_STATE_MAX_INDEX_SET(unit, SVM_MACROFLOW_INDEX_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, SVM_METER_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, SVM_OFFSET_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, SVM_POLICY_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, SVM_DBG_01m, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, SVM_DBG_02m, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, SVM_DBG_03m, -1);

            /* EFP */
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EFP_TCAMm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EFP_POLICY_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EFP_METER_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_PW_INIT_COUNTERSm, -1);

            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_TRILL_PARSE_CONTROL_2m, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_1588_SAm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_FC_HEADER_TYPEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_PORT_1m, -1);
        }

        /* IVXLT */
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_PROTOCOLm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_PROTOCOL_DATAm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_SUBNETm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_SUBNET_ONLYm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_SUBNET_DATA_ONLYm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_MACm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATEm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VFP_TCAMm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VFP_POLICY_TABLEm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_VLAN_TAG_ACTION_PROFILEm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MPLS_ENTRYm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, UDF_CONDITIONAL_CHECK_TABLE_CAMm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, UDF_CONDITIONAL_CHECK_TABLE_RAMm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_ETAG_PCP_MAPPINGm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_ECCm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MPLS_ENTROPY_LABEL_DATAm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_LPm, -1);
...
_soc_apache_clear_all_port_data (int unit)
{
    ...
    /* SER_FIXME : Do these magic numbers hold good for apache as well? */
