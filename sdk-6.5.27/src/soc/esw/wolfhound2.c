...
soc_wolfhound2_port_config_init(int unit, uint16 dev_id)
{
    ...
    /* Increase core clk for certain SKUs when expected value is specified. */
    if (si->frequency == 62) {
        freq = soc_property_get(unit, spn_CORE_CLOCK_FREQUENCY, 62);
        if (freq == 69) {
            si->frequency = 69;
        }
    }
...
soc_wolfhound2_mem_config(int unit, int dev_id)
{
    ...
    switch (dev_id) {
        ...
        default:
            ...
            SOP_MEM_STATE_MAX_INDEX_SET(unit, MODPORT_MAPm, 383);
            break;
    }
...
soc_wolfhound2_chip_reset(int unit)
{
    ...
    si = &SOC_INFO(unit);
    /* set core clock to 69.4 MHz*/
    if (si->frequency == 69) {
        rval = 36;
    /* set core clock to 62.5MHz */
    } else if (si->frequency == 62) {
        rval = 0x28;
