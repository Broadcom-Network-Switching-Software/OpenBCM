...
/*
 * Function to convert to core clock factor encodings to
 * real core clock to pm clock factors.
 */
STATIC int
_soc_hurricane4_bond_option_to_core_clk_factor(int unit, int val)
{
    int clk_fact;
...
soc_hurricane4_bond_info_init(int unit)
{
    ...
    /* CLK to CLK Next factor */
    SOC_BOND_OPTIONS_COPY(val, uint32, bond_info_regval, 42, 45);
    bond_info->clk_to_pm_clk_factor = _soc_hurricane4_bond_option_to_core_clk_factor(unit, val);
