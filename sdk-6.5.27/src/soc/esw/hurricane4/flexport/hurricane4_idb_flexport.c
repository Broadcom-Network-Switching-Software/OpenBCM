 ...
 *
 *  File:        hurricane4_idb_flexport.c
 *  Purpose:
 *  Requires:
 */
...
/* Bubble MOP */
typedef struct soc_hurricane4_bmop_s {
    uint8 line_rate[SOC_SWITCH_BYPASS_MODE_LOW + 1];   /* Line-rate profile ::
                                                        * 0->FULL, 1->L3, 2->L2 */
    uint8 oversub[SOC_SWITCH_BYPASS_MODE_LOW + 1];     /* Oversub profile ::
                                                        * 0->FULL, 1->L3, 2->L2 */
} soc_hurricane4_bmop_t;

static const struct soc_hurricane4_bmop_cfg_s {
    int speed;
    soc_hurricane4_bmop_t bmop_enable;                  /* Bubble MOP enable */
} soc_hurricane4_bmop_cfg_tbl[] = {
    {    -1, {{0, 0, 0}, {0, 0, 0}}},       /* SAF     */
...
/*static const struct soc_hurricane4_obm_setting_s {
    int discard_limit;
...
} soc_hurricane4_obm_settings[4][5] = {
    {
        {
            0,               
            0,               
...
/*static const struct soc_hurricane4_fsaf_dur_timer {
    int frequency;
     ForceSAF detection duration timer. It is 1.5 * core clk in MHz 
    int duration_timer;
} soc_hurricane4_fsaf_dur_timer_tbl[] = {
    { 1525,  2288 },
    { 1425,  2138 },
    { 1325,  1988 },
    { 1012,  2550 },
    {  850,  1275 },      hurricane4_FORCE_SAF_TIMER_ENTRY_CNT=5
    {   -1,  12000}
};*/


static const soc_reg_t soc_hurricane4_ca_ctrl_regs[HURRICANE4_PIPES_PER_DEV][
    HURRICANE4_PBLKS_PER_PIPE] = {
...
static const soc_reg_t idb_ca_ctrl_regs[HURRICANE4_PIPES_PER_DEV][
    HURRICANE4_PBLKS_PER_PIPE] = {
        {IDB_CA_PM0_Q_CA_CONTROL_1r,  IDB_CA_PM1_Q_CA_CONTROL_1r,
         IDB_CA_PM2_Q_CA_CONTROL_1r,  IDB_CA_PM3_Q_CA_CONTROL_1r,
         IDB_CA_PM4_Q_CA_CONTROL_1r,  IDB_CA_PM5_Q_CA_CONTROL_1r,
         IDB_CA_PM6_Q_CA_CONTROL_1r,  IDB_CA_PM7_Q_CA_CONTROL_1r,
         IDB_CA_PM0_48_CA_CONTROL_1r, IDB_CA_PM1_48_CA_CONTROL_1r,
         IDB_CA_PM2_48_CA_CONTROL_1r
        }

    };

/*! @fn int soc_hurricane4_get_pipe_from_phy_pnum(int pnum)
 *  @param pnum Physical por number.
 *  @brief Helper function to calculate pipe number from physical port number.
 *  @returns pipe_number
 */
int
soc_hurricane4_get_pipe_from_phy_pnum(int pnum)
{
    return(0);
}

/*! @fn int soc_hurricane4_get_q_reg(int pnum)
 *  @param pnum Physical por number.
 *  @brief Helper function to calculate reg version for the OBM Q regs.
 *  @returns reg_ver
 */
int
soc_hurricane4_get_q_reg(int pnum)
{
    int reg_ver;
    if(soc_hurricane4_flex_is_pmq_port(pnum))
    {
        reg_ver = ( (pnum - HR4_PMQ_START)%HR4_NUM_PMQ_PORTS ) ;
        reg_ver = reg_ver/HR4_PORT_PER_PM ;
        return(reg_ver);
    } else if (soc_hurricane4_flex_is_qgphy_port(pnum)) {
        reg_ver = ( (pnum - HR4_GPHY_START)%HR4_NUM_QGPHY_PORTS );
        reg_ver = reg_ver/HR4_PORT_PER_PM ;
        return(reg_ver);
    } else
        return(0);
...
/*! @fn int soc_hurricane4_get_pm_from_phy_pnum(int pnum)
 *  @param pnum Physical por number.
 *  @brief Helper function to calculate PM number from physical port number.
 *  @returns PM number
 */
int
soc_hurricane4_get_pm_from_phy_pnum(int pnum)
{
    /* Return PM number for GPHY,PMQ,XL/CL Ports */
    /*pnum = pnum -1 ;*/
    if (soc_hurricane4_flex_is_pmq_port(pnum)) {  /* PMQ  */
        return(((pnum - HR4_PMQ_START)/HR4_NUM_PMQ_PORTS) + 2);
    } else if (soc_hurricane4_flex_is_qgphy_port(pnum)) {  /* QGPHY */
        return(((pnum - HR4_GPHY_START)/HR4_NUM_QGPHY_PORTS));
    } else if (pnum <= HR4_TOTAL_PHY_PORTS) { /* Falcon/Eagle */
        return(((pnum - HR4_CLP_START)/HR4_PORT_PER_PM) + 4);
    } else if (pnum <= HR4_TOTAL_PHY_PORTS + HR4_NUM_MACSEC_PORTS) {
        return(((pnum - HR4_MACSEC_START)/HR4_PORT_PER_PM) + 6);
    }
    return(HR4_NUM_EXT_PORTS);
}

/*! @fn int soc_hurricane4_get_pm_from_phy_pnum_idb(int pnum)
 *  @param pnum Physical por number.
 *  @brief Helper function to calculate PM number from physical port number.
 *  @returns PM number
 */
int
soc_hurricane4_get_pm_from_phy_pnum_idb(int pnum)
{
    /* Return PM number for GPHY,PMQ,XL/CL Ports */
    /*pnum = pnum -1 ;*/
    if (soc_hurricane4_flex_is_pmq_port(pnum)) {  /* PMQ  */
        return(((pnum - HR4_PMQ_START)/HR4_NUM_PMQ_PORTS) + 6);
    } else if (soc_hurricane4_flex_is_qgphy_port(pnum)) {  /* QGPHY */
        return(((pnum - HR4_GPHY_START)/HR4_PORT_PER_PM));
    } else if (pnum <= HR4_TOTAL_PHY_PORTS) { /* Falcon/Eagle */
        return(((pnum - HR4_CLP_START)/HR4_PORT_PER_PM) + 8);
    } else if (pnum <= HR4_TOTAL_PHY_PORTS + HR4_NUM_MACSEC_PORTS) {
        return(((pnum - HR4_MACSEC_START)/HR4_PORT_PER_PM) + 10);
    }
    return(HR4_NUM_EXT_PORTS);
}

/*! @fn int soc_hurricane4_get_subp_from_phy_pnum(int pnum)
 *  @param pnum Physical por number.
 *  @brief Helper function to calculate subport number from physical port number.
 *  @returns subport number
 */
int
soc_hurricane4_get_subp_from_phy_pnum(int pnum)
{
    return ((pnum - 1) & 0x03 );
}

/*! @fn int soc_hurricane4_idb_clear_stats_new_ports(int unit,
 ...
 */
/*int
  soc_hurricane4_idb_clear_stats_new_ports(
  ...
  const soc_reg_t obm_usage_regs[HURRICANE4_PIPES_PER_DEV][
  HURRICANE4_PBLKS_PER_PIPE] = {
  ...
  pnum = port_schedule_state->out_port_map.port_l2p_mapping[
  port_schedule_state
  ->resource[
  prt].logical_port];
  pipe_num = soc_hurricane4_get_pipe_from_phy_pnum(pnum);
  pm_num = soc_hurricane4_get_pm_from_phy_pnum(pnum);
  subp = soc_hurricane4_get_subp_from_phy_pnum(pnum);
...
/*! @fn int soc_hurricane4_idb_reset_buffer(int unit, int pipe_num, int pm_num,
 ...
 */
/*int
  soc_hurricane4_idb_obm_reset_buffer(int unit, int pipe_num, int pm_num, int subp,int pnum,
  int reset_buffer)
  {
  soc_reg_t reg;
  uint32 rval;
  ...
  offset = soc_hurricane4_get_q_reg(pnum);
...
/*! @fn int soc_hurricane4_idb_ca_reset_buffer(int unit, int pipe_num, int pm_num,
 *              int subp, int reset_buffer)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param pm_num PM number within the pipe: Ranges from 0 to 15.
 ...
 */
int
soc_hurricane4_idb_ca_reset_buffer(int unit, int pipe_num, int pm_num, int subp,
        int reset_buffer)
{
    soc_reg_t reg;
    uint32 rval;

    reg = soc_hurricane4_ca_ctrl_regs[pipe_num][pm_num];
...
/*! @fn int soc_hurricane4_idb_lpbk_ca_reset_buffer(int unit, int pipe_num,
  int reset_buffer)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param reset_buffer IDB Cell Assembly reset buffer: 1 - Apply reset.
 ...
 */
int
soc_hurricane4_idb_lpbk_ca_reset_buffer(int unit, int pipe_num, int reset_buffer)
{
    soc_reg_t reg;
    uint32 rval;
    const soc_reg_t idb_ca_lpbk_ctrl[HURRICANE4_PIPES_PER_DEV]= {
...
/*! @fn int soc_hurricane4_idb_cpu_ca_reset_buffer(int unit, int pipe_num,
  int reset_buffer)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param reset_buffer IDB Cell Assembly reset buffer: 1 - Apply reset.
 ...
 */
int
soc_hurricane4_idb_cpu_ca_reset_buffer(int unit, int pipe_num, int reset_buffer)
{
    soc_reg_t reg;
    uint32 rval;
    const soc_reg_t idb_ca_lpbk_ctrl[HURRICANE4_PIPES_PER_DEV]= {
...
/*! @fn int soc_hurricane4_idb_lpbk_ca_in_reset(int unit, int pipe_num)
 *  @param unit Chip unit number.
 *  @param pipe_num hurricane4 pipe number.
 *  @brief Helper function to check if the IDB loopback CA is in reset.
 *  @returns Reset status.
 */
int
soc_hurricane4_idb_lpbk_ca_in_reset(int unit, int pipe_num)
{
    soc_reg_t reg;
    uint32 rval;
    int buffer_in_reset;
...
/*! @fn int soc_hurricane4_idb_cpu_ca_in_reset(int unit, int pipe_num)
 *  @param unit Chip unit number.
 *  @param pipe_num hurricane4 pipe number.
 ...
 */
int
soc_hurricane4_idb_cpu_ca_in_reset(int unit, int pipe_num)
{
    soc_reg_t reg;
    uint32 rval;
    int buffer_in_reset;
    static const soc_reg_t idb_ca_cpu_ctrl[HURRICANE4_PIPES_PER_DEV]= {
        IDB_CA_CPU_CONTROL_PIPE0r
    };
...
/*! @fn int soc_hurricane4_idb_port_mode_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_hurricane4_idb_port_mode_set(int                        unit,
        soc_port_schedule_state_t *port_schedule_state)
{
    soc_reg_t reg;
    int i;
    ...
    int inst;
    uint64 rval64;
    static const int soc_hurricane4_hw_mode_values[SOC_HR4_PORT_RATIO_COUNT] = {
        0, 1, 1, 1, 3, 5, 4, 6, 2
    };


    /*
 *     SDK-203599: The issue was the usage of API soc_hurricane4_get_pm_from_phy_pnum_idb for getting PHY_BASE_PORT.
 *     The Fix is to use API soc_hurricane4_get_pm_from_phy_pnum to get correct pm num to get phy base port number.
 *     The original API call pm num is getting used to index static register array which has been retain in this fix.
 *   */
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port != -1) {
            ...
            idb_pm_num = soc_hurricane4_get_pm_from_phy_pnum_idb(pnum);
            pm_num = soc_hurricane4_get_pm_from_phy_pnum(pnum);
            ...
            if(soc_hurricane4_flex_is_pmq_port(pnum)) {
                inst = (((pnum - HR4_PMQ_START) / HR4_NUM_PMQ_PORTS) |
            ...
            LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "soc_hurricane4_idb_port_mode_set: mode: %1d, qmode: %1d,"
                            "pnum: %1d, pm_num: %1d idb_pm_num: %1d\n"),
                        mode, qmode, pnum, pm_num, idb_pm_num));

            reg = soc_hurricane4_ca_ctrl_regs[pipe_num][idb_pm_num];
            SOC_IF_ERROR_RETURN(soc_reg32_rawport_get(unit, reg, REG_PORT_ANY,
                        0, &rval));
            if(soc_hurricane4_flex_is_pmq_port(pnum) && (qmode)) {
                soc_reg_field_set(unit, reg, &rval, PORT_MODEf, 8);
            } else if (soc_hurricane4_flex_is_qgphy_port(pnum)) {
                soc_reg_field_set(unit, reg, &rval, PORT_MODEf, 2);
            } else {
                soc_reg_field_set(unit, reg, &rval, PORT_MODEf,
                                  soc_hurricane4_hw_mode_values[mode]);
            }
...
/*! @fn int soc_hurricane4_idb_wr_obm_shared_config(int unit, int pipe_num,
 ...
 */
/*int
  soc_hurricane4_idb_wr_obm_shared_config(int unit, int pipe_num, int pm_num,
  int subp,
  ...
  const soc_reg_t obm_shared_cfg[HURRICANE4_PIPES_PER_DEV][
  HURRICANE4_PBLKS_PER_PIPE] = {
  ...
  if(pm_num < 7){
  sop_discard_mode =
  soc_hurricane4_obm_settings[hurricane4_OBM_PARAMS_EG_LOSSLESS_INDEX]
  [num_lanes].sop_discard_mode;
  }
  else{
  sop_discard_mode =
  soc_hurricane4_obm_settings[hurricane4_OBM_PARAMS_FL_LOSSLESS_INDEX]
  [num_lanes].sop_discard_mode;
  }
  } else {
  if(pm_num < 7){
  sop_discard_mode =
  soc_hurricane4_obm_settings[hurricane4_OBM_PARAMS_EG_LOSSY_INDEX]
  [num_lanes].sop_discard_mode;
  }
  else{
  sop_discard_mode =
  soc_hurricane4_obm_settings[hurricane4_OBM_PARAMS_FL_LOSSY_INDEX]
  [num_lanes].sop_discard_mode;
  }
  }
...
/*! @fn int soc_hurricane4_idb_wr_obm_flow_ctrl_cfg(int unit, int pipe_num,
 *              int pm_num, int subp, int num_lanes)
 *  @param unit Chip unit number.
 *  @param pipe_num Pipe number.
 *  @param pm_num Port Macro number.
 ...
 */
/*int
  soc_hurricane4_idb_wr_obm_flow_ctrl_cfg(int unit, int pipe_num, int pm_num,
  int subp, int lossless)
  {
  soc_reg_t reg;
  uint64 rval64;

  const soc_reg_t obm_flow_ctrl_cfg[HURRICANE4_PIPES_PER_DEV][
  HURRICANE4_PBLKS_PER_PIPE] = {
...
/*! @fn int soc_hurricane4_idb_wr_obm_ovs_en(int unit, int pipe_num, int pm_num,
 *              int subp)
 *  @param unit Chip unit number.
 *  @param pipe_num Pipe number.
 *  @param pm_num Port Macro number.
 ...
 */
/*int
  soc_hurricane4_idb_wr_obm_ovs_en(int unit, int pipe_num, int pm_num, int subp , int pnum)
  {
  soc_reg_t reg;
  uint32 rval;
  uint32 offset;
...
/*! @fn int soc_hurricane4_idb_wr_obm_buffer_limit(int unit, int pipe_num, int pm_num,
 ...
 */
/*int
  soc_hurricane4_idb_wr_obm_buffer_limit(int unit, int pipe_num, int pm_num, int subp,
  int num_lanes)
  {
  soc_reg_t reg;
  uint64 rval64;
  ...
  const soc_reg_t obm_buffer_config[HURRICANE4_PIPES_PER_DEV][
  HURRICANE4_PBLKS_PER_PIPE] = {
  ...
  if(pm_num < 7)
  buffer_start 	= subp * hurricane4_OBM_BUFFER_PER_LANE_EG;
  else
  buffer_start 	= subp * hurricane4_OBM_BUFFER_PER_LANE_FL;


  if(num_lanes == 0) 	buffer_end = buffer_start;
  else{
  if(pm_num < 7){
  buffer_end = buffer_start + (num_lanes * hurricane4_OBM_BUFFER_PER_LANE_EG) - 1;}
  else{
  buffer_end = buffer_start + (num_lanes * hurricane4_OBM_BUFFER_PER_LANE_FL) - 1;}
  }
  LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
  "soc_hurricane4_idb_wr_obm_buffer_limit: pipe: %1d, pm_num: %1d,"
...
/*! @fn int soc_hurricane4_idb_wr_obm_thresh(int unit, int pipe_num, int pm_num,
 *              int subp)
 *  @param unit Chip unit number.
 *  @param pipe_num Pipe number.
 *  @param pm_num Port Macro number.
 ...
 */
/*int
  soc_hurricane4_idb_wr_obm_thresh(int unit, int pipe_num, int pm_num, int subp,
  int num_lanes, int lossless)
  {
  soc_reg_t reg;
  int i;
  ...
  uint64 rval64;
  uint32 HURRICANE4_OBM_PARAMS_LOSSLESS;
  uint32 HURRICANE4_OBM_PARAMS_LOSSY;

  const soc_reg_t obm_thresh_pp[HURRICANE4_PIPES_PER_DEV][
  HURRICANE4_PBLKS_PER_PIPE][2] = {
  ...
  if(pm_num < 7)
  {
  hurricane4_OBM_PARAMS_LOSSLESS = hurricane4_OBM_PARAMS_EG_LOSSLESS_INDEX;
  hurricane4_OBM_PARAMS_LOSSY    = hurricane4_OBM_PARAMS_EG_LOSSY_INDEX;
  }
  else
  {
  hurricane4_OBM_PARAMS_LOSSLESS =  hurricane4_OBM_PARAMS_FL_LOSSLESS_INDEX;
  hurricane4_OBM_PARAMS_LOSSY    =  hurricane4_OBM_PARAMS_FL_LOSSY_INDEX;
  }


  if (lossless) {
  discard_limit =
  soc_hurricane4_obm_settings[hurricane4_OBM_PARAMS_LOSSLESS]
  ...
  discard_limit =
  soc_hurricane4_obm_settings[hurricane4_OBM_PARAMS_LOSSY]
  [num_lanes].discard_limit;
  lossless1_discard =
  soc_hurricane4_obm_settings[hurricane4_OBM_PARAMS_LOSSY]
  [num_lanes].lossless_discard;
...
/*! @fn int soc_hurricane4_idb_wr_obm_fc_threshold(int unit, int pipe_num, int pm_num, *              int subp, int num_lanes)
 *  @param unit Chip unit number.
 *  @param pipe_num Pipe number.
 *  @param pm_num Port Macro number.
 *  @param subp Sub-port number.
 ...
 */
/*int
  soc_hurricane4_idb_wr_obm_fc_threshold(int unit, int pipe_num, int pm_num,
  int subp,
  int num_lanes, int lossless)
  {
  soc_reg_t reg;
  ...
  uint32 hurricane4_OBM_PARAMS_LOSSLESS;
  uint32 hurricane4_OBM_PARAMS_LOSSY;
  const soc_reg_t obm_fc_thresh_pp[HURRICANE4_PIPES_PER_DEV][
  HURRICANE4_PBLKS_PER_PIPE][2] = {
...
/*! @fn int soc_hurricane4_idb_obm_poll_buffer_empty(int unit, int pipe_num,
 *              int pm_num, int subp)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param pm_num PM number within the pipe: Ranges from 0 to 15.
 ...
 */
/*int
  soc_hurricane4_idb_obm_poll_buffer_empty(int unit, int pipe_num, int pm_num,
  int subp)
  {
  soc_reg_t reg;
  uint64 rval64;
...
/*! @fn int soc_hurricane4_idb_ca_poll_buffer_empty(int unit, int pipe_num,
 *              int pm_num, int subp)
 *  @param unit Chip unit number.
 *  @param pipe_num Trident3 pipe number.
 *  @param pm_num PM number within the pipe: Ranges from 0 to 15.
 ...
 */
int
soc_hurricane4_idb_ca_poll_buffer_empty(int unit, int pipe_num, int pm_num,
        int subp,int pnum)
{
    soc_reg_t reg;
    uint64 rval64;
    ...
    static const soc_reg_t ca_status_regs[HURRICANE4_PIPES_PER_DEV][
        HURRICANE4_PBLKS_PER_PIPE] = {
            {IDB_CA_PM0_Q_CA_HW_STATUS0r,   IDB_CA_PM1_Q_CA_HW_STATUS0r,
             IDB_CA_PM2_Q_CA_HW_STATUS0r,   IDB_CA_PM3_Q_CA_HW_STATUS0r,
             IDB_CA_PM4_Q_CA_HW_STATUS0r,   IDB_CA_PM5_Q_CA_HW_STATUS0r,
             IDB_CA_PM6_Q_CA_HW_STATUS0r,   IDB_CA_PM7_Q_CA_HW_STATUS0r,
             IDB_CA_PM0_48_CA_HW_STATUS_0r, IDB_CA_PM1_48_CA_HW_STATUS_0r,
             IDB_CA_PM2_48_CA_HW_STATUS_0r
            }
    ...
    do {
        ...
        if(cnt >= 20000) {
            LOG_CLI((BSL_META_U(unit, "CA is not going to empty even after"
                            "20000 attempts PHYPort %d PM_num %d SubPort %d\n"),
                             pnum, pm_num, subp));
            return SOC_E_FAIL;
        }
...
/*! @fn int soc_hurricane4_idb_ca_cpu_poll_buffer_empty(int unit, int pipe_num)
 *  @param unit Chip unit number.
 *  @param pipe_num hurricane4 pipe number.
 ...
 */
int
soc_hurricane4_idb_ca_cpu_poll_buffer_empty(int unit, int pipe_num)
{
    soc_reg_t reg;
    uint32 rval32;
    uint32 ca_fifo_empty;
    const soc_reg_t idb_ca_cpu_hw_status[HURRICANE4_PIPES_PER_DEV]= {
...
/*! @fn int soc_hurricane4_idb_ca_lpbk_poll_buffer_empty(int unit, int pipe_num)
 *  @param unit Chip unit number.
 *  @param pipe_num hurricane4 pipe number.
 ...
 */
int
soc_hurricane4_idb_ca_lpbk_poll_buffer_empty(int unit, int pipe_num)
{
    soc_reg_t reg;
    uint32 rval32;
    uint32 ca_fifo_empty;
    const soc_reg_t idb_ca_lpbk_hw_status[HURRICANE4_PIPES_PER_DEV]= {
...
/*! @fn int soc_hurricane4_speed_to_bmop_class_map(int speed)
 ...
 */
int
soc_hurricane4_speed_to_bmop_class_map(int speed)
{
    int bmop_class;
    int i;
    ...
    bmop_class=0;
    for (i = 0; i <= 12; i++) {
        if (soc_hurricane4_bmop_cfg_tbl[i].speed == speed) {
            bmop_class = i;
            break;
        }
    }
...
/*! @fn int soc_hurricane4_idb_bubble_mop_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 ...
 */
int
soc_hurricane4_idb_bubble_mop_set(
        int unit, soc_port_schedule_state_t *port_schedule_state)
{
    soc_reg_t reg;
    uint32 rval32;
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port != -1) {
            ...
            pipe_num = soc_hurricane4_get_pipe_from_phy_pnum(phy_port);
            pm_num = soc_hurricane4_get_pm_from_phy_pnum_idb(phy_port);
            ...
            speed_encoding =
                (port_schedule_state->cutthru_prop.asf_modes[port] ==
                 _SOC_ASF_MODE_SAF) ? 0 :
                soc_hurricane4_speed_to_bmop_class_map(
                        port_schedule_state->resource[i].speed);
            bubble_mop_disable =
                oversub ? !(soc_hurricane4_bmop_cfg_tbl[speed_encoding].
                        bmop_enable.
                        oversub[latency]) :
                !(soc_hurricane4_bmop_cfg_tbl[speed_encoding].
                        bmop_enable.line_rate[latency]);
            reg = idb_ca_ctrl_regs[pipe_num][pm_num];
            SOC_IF_ERROR_RETURN(soc_reg32_rawport_get(unit, reg, REG_PORT_ANY,
                        0, &rval32));
            if (subp == 0) {
...
/*! @fn int soc_hurricane4_idb_obm_port_config_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 ...
 */
/*int
  soc_hurricane4_idb_obm_port_config_set(
  int unit, soc_port_schedule_state_t *port_schedule_state)
  {
  soc_reg_t reg;
  uint32 rval32;
  ...
  int port;
  int i;
  static const soc_reg_t obm_port_config_regs[HURRICANE4_PIPES_PER_DEV][
  HURRICANE4_PBLKS_PER_PIPE] = {
...
/*! @fn int soc_hurricane4_idb_obm_dscp_map_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
/*int
  soc_hurricane4_idb_obm_dscp_map_set(
  int unit, soc_port_schedule_state_t *port_schedule_state)
  {
  soc_mem_t mem;
  ...
  int lossless;
  uint32 priority;
  static const soc_mem_t obm_pri_map_mem[][HURRICANE4_PIPES_PER_DEV][
  hurricane4_PORTS_PER_PBLK] = {
  ...
  priority = lossless ? hurricane4_OBM_PRIORITY_LOSSLESS0 :
  hurricane4_OBM_PRIORITY_LOSSY_LO;
...
/*! @fn int soc_hurricane4_idb_obm_force_saf_set(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
/* int
 *soc_hurricane4_idb_obm_force_saf_set(
 *    int unit, soc_port_schedule_state_t *port_schedule_state)
 *{
 *    soc_reg_t reg;
 *    uint32 rval32;
 ...
 *    int duration_timer;
 *    uint32 ovs_prt_up;
 *  static const soc_reg_t obm_force_saf_config_regs[HURRICANE4_PIPES_PER_DEV][
 *      HURRICANE4_PBLKS_PER_PIPE] = {
 ...
 *       }
 *  };
 *   static const soc_reg_t idb_force_saf_config_regs[HURRICANE4_PIPES_PER_DEV] =
 *        {IDB_DBG_B_PIPE0r};
 *
 *    ovs_prt_up=0;
 *    for (i = 0; i < port_schedule_state->nport; i++) {
 ...
 *    }
 *    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
 *              "soc_hurricane4_idb_obm_force_saf_set ovs_prt_up:: %x \n"),
 *              ovs_prt_up));
 *    duration_timer=
 *        soc_hurricane4_fsaf_dur_timer_tbl[hurricane4_FORCE_SAF_TIMER_ENTRY_CNT].
 *        duration_timer;
 *    for (i=0; i<hurricane4_FORCE_SAF_TIMER_ENTRY_CNT; i++) {
 *        if (port_schedule_state->frequency ==
*            soc_hurricane4_fsaf_dur_timer_tbl[i].frequency) {
    *            duration_timer=soc_hurricane4_fsaf_dur_timer_tbl[i].duration_timer;
    *        }
    *    }
    *    for (i = 0; i < HURRICANE4_PIPES_PER_DEV; i++) {
        *        pipe_num=i;
        *        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                        *                  "soc_hurricane4_idb_obm_force_saf_set ovs_prt_up:: %x "
                        *                  "pipe_num %d\n"), ovs_prt_up,pipe_num));
        *     if (ovs_prt_up>>pipe_num) {
            ...
            *     rval32=0;
            *     LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            *     "soc_hurricane4_idb_obm_force_saf_set Write to "
        ...
        */
        ...
        /*! @fn int soc_hurricane4_flex_dis_forwarding_traffic(int unit,
         *              soc_port_schedule_state_t *port_schedule_state)
         *  @param unit Chip unit number.
         *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
         ...
         */
int
soc_hurricane4_flex_dis_forwarding_traffic(
        int unit, soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    uint32 entry[SOC_MAX_MEM_WORDS];
     ...
     * "HR4_uA_flexport.pdf", Section 6.6.1 
     */
...
/*! @fn int soc_hurricane4_flex_en_forwarding_traffic(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 *  @brief Helper function to Write EPC_LINK_BMAP table to enable forwarding traffic
 *         to port. Follows the sequence specified in document "HR4_uA_flexport.pdf",
 *         Section 6.6.2, bullet number 5.
 *  @returns SOC_E_NONE
 */
int
soc_hurricane4_flex_en_forwarding_traffic(
        int unit, soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    uint32 entry[SOC_MAX_MEM_WORDS];
...
/*! @fn int soc_hurricane4_idb_port_down(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct
 *         variable
 *  @brief Helper function to follow IDB port down sequence for flexport operation.
 *         Follows the sequence specified in document "HR4_uA_flexport.pdf", Section
 *         6.6.1.1.
 *         Two main parts:
 ...
 */
int
soc_hurricane4_flex_idb_port_down(int                        unit,
        soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    int pipe_num;
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port == -1) {
            ...
            pm_num = soc_hurricane4_get_pm_from_phy_pnum_idb(phy_port);
            subp = soc_hurricane4_get_subp_from_phy_pnum(phy_port);
            if(phy_port == HURRICANE4_PHY_PORT_CPU) {
                if (!soc_hurricane4_idb_cpu_ca_in_reset(unit,pipe_num)) {
                    SOC_IF_ERROR_RETURN(soc_hurricane4_idb_ca_cpu_poll_buffer_empty(
                                unit, pipe_num));
                }
            } else if(phy_port == HURRICANE4_PHY_PORT_LPBK0) {
                if (!soc_hurricane4_idb_lpbk_ca_in_reset(unit,pipe_num)) {
                    SOC_IF_ERROR_RETURN(soc_hurricane4_idb_ca_lpbk_poll_buffer_empty(
                                unit, pipe_num));
                }
            } else {

                SOC_IF_ERROR_RETURN(soc_hurricane4_idb_ca_poll_buffer_empty(unit,
                            pipe_num,
                            pm_num,
                            subp,
                            phy_port));
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port == -1) {
            ...
            subp = soc_hurricane4_get_subp_from_phy_pnum(phy_port);
            reset_buffer = 1;
            /*SOC_IF_ERROR_RETURN(soc_hurricane4_idb_obm_reset_buffer(unit,
              pipe_num,
              pm_num, subp,
              phy_port,
              reset_buffer)); */
            if(soc_hurricane4_flex_is_pmq_port(phy_port))
                subp = (phy_port - HR4_PMQ_START)%HR4_NUM_PMQ_PORTS;
            if(soc_hurricane4_flex_is_qgphy_port(phy_port))
                subp = (phy_port - HR4_GPHY_START)%HR4_PORT_PER_PM;

            SOC_IF_ERROR_RETURN(soc_hurricane4_idb_ca_reset_buffer(unit,
                        pipe_num,
                        pm_num, subp,
                        reset_buffer));
        }
...
/*! @fn int soc_hurricane4_flex_idb_reconfigure(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
        ...
        int unit, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    if (1 == port_schedule_state->is_flexport) {
        ...
        SOC_IF_ERROR_RETURN(soc_hurricane4_tdm_flexport_idb(unit,
                    port_schedule_state));
        SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_idb_hsp(unit,
                    port_schedule_state));
        SOC_IF_ERROR_RETURN(_soc_hr4_tdm_set_idb_calendar(unit,
                    port_schedule_state));
    }
    ...
    /* Update IDB_TO_DEVICE_PORT_MAPPING TABLE */
        for(i = 0; i < port_schedule_state->nport; i++) {
            ...
            memfld = (physical_port == -1) ? 0x7f : logical_port;
            ...
            sal_memset(&entry, 0, sizeof(entry));
            soc_mem_field_set(unit, mem, entry, IDB_PORTf, &idb_port);
            soc_mem_field_set(unit, mem, entry, VALIDf, &memfld);
            SOC_IF_ERROR_RETURN(soc_mem_write(unit, mem, MEM_BLOCK_ALL,
                        (physical_port - 1), &entry));

        }
    SOC_IF_ERROR_RETURN(soc_hurricane4_idb_bubble_mop_set(unit,
                        port_schedule_state));

    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                    "Call soc_hurricane4_idb_port_mode_set \n")));
    SOC_IF_ERROR_RETURN(soc_hurricane4_idb_port_mode_set(unit,
                port_schedule_state));

    return SOC_E_NONE;
}

/*! @fn int soc_hurricane4_flex_idb_port_up(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_hurricane4_flex_idb_port_up(int                        unit,
        soc_port_schedule_state_t *port_schedule_state)
{
    int i;
    int pipe_num;
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port != -1) {
            ...
            subp = soc_hurricane4_get_subp_from_phy_pnum(phy_port);
            reset_buffer = 1;
            if (HURRICANE4_PHY_IS_FRONT_PANEL_PORT(phy_port)) {
                /*SOC_IF_ERROR_RETURN(soc_hurricane4_idb_obm_reset_buffer(unit,
                  pipe_num,
                  pm_num, subp,
                  phy_port,
            ...
            if (HURRICANE4_PHY_IS_FRONT_PANEL_PORT(phy_port) ||
                soc_hurricane4_flex_is_macsec_port(phy_port)) {
                if(soc_hurricane4_flex_is_pmq_port(phy_port)) {
                    subp = (phy_port - HR4_PMQ_START)%HR4_NUM_PMQ_PORTS;
                }
                if(soc_hurricane4_flex_is_qgphy_port(phy_port)) {
                ...
                SOC_IF_ERROR_RETURN(soc_hurricane4_idb_lpbk_ca_reset_buffer(unit,
                            pipe_num,
                            reset_buffer));
            } else if(phy_port == HURRICANE4_PHY_PORT_CPU) {
                SOC_IF_ERROR_RETURN(soc_hurricane4_idb_cpu_ca_reset_buffer(unit,
                            pipe_num,
                            reset_buffer));
            }
        }
...
int
soc_hurricane4_idb_flexport(int                        unit,
        soc_port_schedule_state_t *port_schedule_state)
{
    ...
    /* Reconfigure IDB. */
    SOC_IF_ERROR_RETURN(soc_hurricane4_flex_idb_reconfigure(unit,
                port_schedule_state));
    ...
    SOC_IF_ERROR_RETURN(soc_hurricane4_flex_end(unit, port_schedule_state));
