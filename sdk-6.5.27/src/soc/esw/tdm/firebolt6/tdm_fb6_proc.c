...
tdm_fb6_proc_pre_cal_lr( tdm_mod_t *_tdm, int *old_speed, int *pseudo_lr_num )
{
   ...
   if(_tdm->_chip_data.soc_pkg.clk_freq == FB6_CLK_4700_1250){
      _tdm->_chip_data.soc_pkg.speed[(lr_pm_num*4)+1] = SPEED_40G;
...
tdm_fb6_proc_post_cal_lr( tdm_mod_t *_tdm, int *old_speeds, int *pseudo_lr_num)
{
   ...
   int result = PASS;
    int cnt = 0;
    int pseudo_lr_val[24] = {0};
    int num_lanes_cnt[4];
   int single_slot_cnt = 0;
...
tdm_fb6_proc_cal(tdm_mod_t *_tdm)
{
   ...
   int mgmt_set = 0;
    ...
    for (port_phy = 1; port_phy <= 68; port_phy++) {
      if(_tdm->_chip_data.soc_pkg.soc_vars.fb6.mgmt_mode[port_phy] == 1) {
       TDM_PRINT2("\nTDM: Management Mode port_phy %0d mgmt_mode %0d\n",port_phy,  _tdm->_chip_data.soc_pkg.soc_vars.fb6.mgmt_mode[port_phy]);
       mgmt_set = 1;
      }
    }

    TDM_PRINT1("TDM_FB6_PROC::LR_SET %d \n",  lr_set);
   /*if( (_tdm->_chip_data.soc_pkg.clk_freq == FB6_CLK_4731_1012) ||
       (_tdm->_chip_data.soc_pkg.clk_freq == FB6_CLK_4732_1012)) {*/
   if((lr_set == 1) && (mgmt_set == 0)) {
