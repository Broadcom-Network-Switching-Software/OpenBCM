        ...
        if ( param_states[port_phy-1] == PORT_STATE__LINERATE   ||
            ...
            if (idx1<TDM_AUX_SIZE){
               if((_tdm->_chip_data.soc_pkg.clk_freq== FB6_CLK_4731_1012) ||
                  ...
                  (_tdm->_chip_data.soc_pkg.clk_freq== FB6_CLK_4732_1016) ||
                  (_tdm->_chip_data.soc_pkg.clk_freq== FB6_CLK_4700_1250)) {
                   pm_no = (port_phy-1)/4;
...
/*    _tdm->_chip_data.soc_pkg.soc_vars.fp_port_lo = 5; */
    ...
    for (index=0; index<FB6_NUM_PHY_PORTS_PER_PIPE+4; index++) {
            if (_tdm->_chip_data.soc_pkg.state[index] == PORT_STATE__LINERATE ||
                _tdm->_chip_data.soc_pkg.state[index-1] == PORT_STATE__LINERATE_HG) {
              lr_set = 1;
            }
    }
    for(index=0; index<FB6_NUM_EXT_PORTS; index++) {
      if(_tdm->_chip_data.soc_pkg.soc_vars.fb6.mgmt_mode[index] == 1) {
       TDM_PRINT2("\nTDM: Management Mode index %0d mgmt_mode %0d\n",index,  _tdm->_chip_data.soc_pkg.soc_vars.fb6.mgmt_mode[index]);
       mgmt_set = 1;
      }

    }
    switch(_tdm->_chip_data.soc_pkg.clk_freq) {
    case FB6_CLK_4740_1012:  _tdm->_chip_data.soc_pkg.tvec_size              = FB6_NUM_ANCL_4740_1012;
                             break;
    ...
    case FB6_CLK_4731_1012:  _tdm->_chip_data.soc_pkg.tvec_size              = FB6_NUM_ANCL_4731_1012;
                             break;
    case FB6_CLK_4732_1016:
    case FB6_CLK_4732_1012:  if((lr_set == 1) && (mgmt_set == 0)) {
                                _tdm->_chip_data.soc_pkg.tvec_size              = FB6_NUM_ANCL_4732_1012;
                             }
                             else if(mgmt_set == 1) {
                                _tdm->_chip_data.soc_pkg.tvec_size              = FB6_NUM_ANCL_4732_1012+24;
                             }
                             else {
                                _tdm->_chip_data.soc_pkg.tvec_size              = FB6_NUM_ANCL_4732_1012;
                             }
    ...
    case FB6_CLK_4721_1250:  _tdm->_chip_data.soc_pkg.tvec_size              = FB6_NUM_ANCL_4721_1250;
                             break;
    case FB6_CLK_4700_1250:  if((lr_set == 1) && (mgmt_set == 0)) {
                                _tdm->_chip_data.soc_pkg.tvec_size              = FB6_NUM_ANCL_4700_1250-20;
                             }
                             else if(mgmt_set == 1) {
                                _tdm->_chip_data.soc_pkg.tvec_size              = FB6_NUM_ANCL_4700_1250;
                             }
                             else {
                                _tdm->_chip_data.soc_pkg.tvec_size              = FB6_NUM_ANCL_4700_1250-20;
                             }
...
tdm_fb6_main_chk( tdm_mod_t *_tdm )
{
    ...
    */
    ...
    for (i = 1; i <= 68; i++) {
      if(_tdm->_chip_data.soc_pkg.soc_vars.fb6.mgmt_mode[i] == 1) {
       TDM_PRINT2("\nTDM: Management Mode port_phy %0d mgmt_mode %0d\n",i,  _tdm->_chip_data.soc_pkg.soc_vars.fb6.mgmt_mode[i]);
       mgmt_set = 1;
      }
    }
    ...
    switch(param_clk_freq){
        ...
        case     FB6_CLK_4704_1087: if((lr_set == 1) && (mgmt_set == 0)) {
                                       result = FAIL;
                                      TDM_ERROR2("TDM: Invalid Pseudo LR Configuration ... TDM supports Management only Mgmt %d LR %d\n", mgmt_set, lr_set);
                                      break;
                                    }
                                    else {
                                      break;
        ...
        case     FB6_CLK_4732_1016: if(mgmt_set == 1) {
                                       result = FAIL;
                                      TDM_ERROR2("TDM: Invalid Mgmt Configuration ... TDM supports Pseudo LR only LR %d Mgmt %d\n", lr_set, mgmt_set);
                                      break;
                                    }
                                    else {
                                      break;
