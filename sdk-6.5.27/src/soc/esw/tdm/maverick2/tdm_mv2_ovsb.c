...
tdm_mv2_ovsb_pkt_shaper_cpy_prev_pm_indx(tdm_mod_t *_tdm)
{
    ...
    for (hpipe = 0; hpipe < 2; hpipe ++) {
        ...
        /* Get the right pkt scheduler calendar*/
        if (param_cal_id == 0) {
            cal = _tdm->_chip_data.cal_0.cal_grp[grp_idx];
            cal_prev = _tdm->_prev_chip_data.cal_0.cal_grp[grp_idx];
        } else if (param_cal_id == 1) {
            cal = _tdm->_chip_data.cal_1.cal_grp[grp_idx];
            cal_prev = _tdm->_prev_chip_data.cal_1.cal_grp[grp_idx];
        } else if (param_cal_id == 2) {
            cal = _tdm->_chip_data.cal_2.cal_grp[grp_idx];
            cal_prev = _tdm->_prev_chip_data.cal_2.cal_grp[grp_idx];
        } else if (param_cal_id == 3) {
            cal = _tdm->_chip_data.cal_3.cal_grp[grp_idx];
            cal_prev = _tdm->_prev_chip_data.cal_3.cal_grp[grp_idx];
        } else {
