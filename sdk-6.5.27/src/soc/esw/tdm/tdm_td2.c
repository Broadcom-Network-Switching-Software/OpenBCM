...
int fill_ovs(int *z, enum tdm_td2_port_speed speed[TDM_TD2_NUM_EXT_PORTS], int tsc[TDM_TD2_NUM_WC][4], int ovs_buf[64], int bucket1[16], int *z11, int bucket2[16], int *z22, int bucket3[16], int *z33, int bucket4[16], int *z44)
{
	...
	int m, port_exist, port_i, port_s, port_l;
	...
	if (tsc_id <TDM_TD2_NUM_WC ){
		for (i=0; i<4; i++) {
            port_i = tsc[tsc_id][i];
            port_s = tsc_stack[0];
            if (port_i < TDM_TD2_NUM_EXT_PORTS && port_i >= 0 &&
                port_s < TDM_TD2_NUM_EXT_PORTS && port_s >= 0) {
                if (port_i != port_s && speed[port_i] == speed[port_s]) {
                    for (j=1; j<=*z; j++) {
			...
			if (((check_type_2(ovs_buf[l],tsc))+tsc_fetch_cnt)<4) {
				...
				if (tsc_id < TDM_TD2_NUM_WC) {
					for (i=0; i<4; i++) {
                        ...
                        port_l = ovs_buf[l];
                        if (port_i < TDM_TD2_NUM_EXT_PORTS && port_i >= 0 &&
                            port_l < TDM_TD2_NUM_EXT_PORTS && port_l >= 0) {
                            if (speed[port_i] == speed[port_l]) {
                                for (j=1; j<=*z; j++) {
                                    if (ovs_buf[j]==tsc[tsc_id][i]) {
...
static void
td2_tdm_print_mmu_cal(int cal_len, int mmu_cal[256])
{
    int j;
    for (j = 0; j < cal_len && j < 256; j++) {
        if (j % 16 == 0) {
            LOG_VERBOSE(BSL_LS_SOC_TDM,
                        (BSL_META("\n[%3d,%3d]\t"), j, j+16-1));
        }
        LOG_VERBOSE(BSL_LS_SOC_TDM, (BSL_META("%3d "), mmu_cal[j]));
    }
    LOG_VERBOSE(BSL_LS_SOC_TDM,(BSL_META("\n\n")));
}

static void
td2_tdm_special_10g_20g_40g(int lr_idx_limit, int prt_cnt_10g, int prt_cnt_20g,
                            int prt_cnt_40g, int prt_10g[64], int prt_20g[32],
                            int prt_40g[16], int mmu[256])
{
    int i, j, pm, subp, prt, prt_cnt = 0, pm_cnt = 0;
    int step_wid, step_offset, slot_cnt = 0, slot_idx, slot_idx_base;
    int pm_prt_40g[TDM_TD2_NUM_WC][4] = {{0}};
    int pm_prt_20g[TDM_TD2_NUM_WC][4] = {{0}};
    int pm_prt_10g[TDM_TD2_NUM_WC][4] = {{0}};
    int pm_en_40g[TDM_TD2_NUM_WC] = {0};
    int pm_en_20g[TDM_TD2_NUM_WC] = {0};
    int pm_en_10g[TDM_TD2_NUM_WC] = {0};

    LOG_VERBOSE(BSL_LS_SOC_TDM,
                (BSL_META("TDM: [special] core_bw=720G, prt_cnt_10g=%0d, "
                          "prt_cnt_20g=%0d, prt_cnt_40g=%0d,"
                          "lr_idx_limit=%0d\n"),
                          prt_cnt_10g, prt_cnt_20g, prt_cnt_40g,
                          lr_idx_limit));

    /* reset mmu subset calendar */
    for (j = 0; j < lr_idx_limit; j++) {
        mmu[j] = TDM_TD2_NUM_EXT_PORTS;
    }

    /* slot step width */
    step_wid = lr_idx_limit / 4;

    /* 40G */
    for (i = 1; i <= prt_cnt_40g; i++) {
        prt = prt_40g[i];
        pm = (prt - 1) / 4;
        subp = (prt - 1) % 4;
        pm_prt_40g[pm][subp] = prt;
        pm_en_40g[pm] = 1;

        LOG_VERBOSE(BSL_LS_SOC_TDM,
                    (BSL_META("TDM: [special] [40G] prt_cnt=%0d, "
                              "pm=%0d, subp=%0d, prt=%0d\n"),
                              i, pm, subp, prt));
    }
    /* 20G */
    for (i = 1; i <= prt_cnt_20g; i++) {
        prt = prt_20g[i];
        pm = (prt - 1) / 4;
        subp = (prt - 1) % 4;
        pm_prt_20g[pm][subp] = prt;
        pm_en_20g[pm]++;

        LOG_VERBOSE(BSL_LS_SOC_TDM,
                    (BSL_META("TDM: [special] [20G] prt_cnt=%0d, "
                              "pm=%0d, subp=%0d, prt=%0d\n"),
                              i, pm, subp, prt));
    }
    /* 10G */
    for (i = 1; i <= prt_cnt_10g; i++) {
        prt = prt_10g[i];
        pm = (prt - 1) / 4;
        subp = (prt - 1) % 4;
        pm_prt_10g[pm][subp] = prt;
        pm_en_10g[pm]++;

        LOG_VERBOSE(BSL_LS_SOC_TDM,
                    (BSL_META("TDM: [special] [10G] prt_cnt=%0d, "
                              "pm=%0d, subp=%0d, prt=%0d\n"),
                              i, pm, subp, prt));
    }

    /* 40G: single */
    for (pm = 0; pm < TDM_TD2_NUM_WC; pm++) {
        /* 4 slots per PM */
        if (pm_en_40g[pm] == 1) {
            subp = 0;
            prt = pm_prt_40g[pm][subp];
            slot_idx_base = pm_cnt;
            for (slot_cnt = 0; slot_cnt < 4; slot_cnt++) {
                slot_idx = slot_idx_base + step_wid * slot_cnt;
                mmu[slot_idx] = prt;
                LOG_VERBOSE(BSL_LS_SOC_TDM,
                            (BSL_META("TDM: [special] [40G] pm_cnt=%0d, "
                                      "pm=%0d, subp=%0d, prt=%0d, slot=%0d\n"),
                                      (pm_cnt + 1), pm, subp, prt, slot_idx));
            }
            pm_cnt++;
        }
    }
    ...
    for (pm = 0; pm < TDM_TD2_NUM_WC; pm++) {
        ...
        if (pm_en_20g[pm] == 2) {
            ...
            for (slot_cnt = 0; slot_cnt < 4; slot_cnt++) {
                slot_idx = slot_idx_base + step_wid * slot_cnt;
                subp = ((slot_cnt % 2) == 0) ? 0 : 2;
                prt = pm_prt_20g[pm][subp];
                mmu[slot_idx] = prt;
                LOG_VERBOSE(BSL_LS_SOC_TDM,
                            (BSL_META("TDM: [special] [20G:dual] pm_cnt=%0d, "
                                      "pm=%0d, subp=%0d, prt=%0d, slot=%0d\n"),
                                      (pm_cnt + 1), pm, subp, prt, slot_idx));
            }
            pm_cnt++;
    ...
    for (pm = 0; pm < TDM_TD2_NUM_WC; pm++) {
        ...
        if (pm_en_10g[pm] == 4) {
            slot_idx_base = pm_cnt;
            for (subp = 0; subp < 4; subp++) {
                step_offset = (subp == 0) ? 0 :
                              (subp == 1) ? 2 :
                              (subp == 2) ? 1 :
                                            3;
                slot_idx = slot_idx_base + step_wid * step_offset;
                prt = pm_prt_10g[pm][subp];
                mmu[slot_idx] = prt;
                LOG_VERBOSE(BSL_LS_SOC_TDM,
                            (BSL_META("TDM: [special] [10G:quad] pm_cnt=%0d, "
                                      "pm=%0d, subp=%0d, prt=%0d, slot=%0d\n"),
                                      (pm_cnt + 1), pm, subp, prt, slot_idx));
            }
            pm_cnt++;
    ...
    for (pm = 0; pm < TDM_TD2_NUM_WC; pm++) {
        ...
        if (pm_en_20g[pm] == 1) {
            for (subp = 0; subp < 4; subp++) {
                ...
                slot_idx_base = pm_cnt;
                for (slot_cnt = 0; slot_cnt < 2; slot_cnt++) {
                    step_offset = (prt_cnt % 2 == 0) ? 0 : 1;
                    slot_idx = slot_idx_base + (step_wid * 2) * slot_cnt +
                               step_wid * step_offset;
                    mmu[slot_idx] = prt;
                    LOG_VERBOSE(BSL_LS_SOC_TDM,
                                (BSL_META("TDM: [special] [20G:single] "
                                          "pm_cnt=%0d, pm=%0d, subp=%0d, "
                                          "prt=%0d, prt_cnt=%0d, slot=%0d\n"),
                                          (pm_cnt + 1), pm, subp, prt,
                                          prt_cnt, slot_idx));
                }
                prt_cnt++;
                if (prt_cnt % 2 == 1) pm_cnt++;
    ...
    prt_cnt = 0;
    for (pm = 0; pm < TDM_TD2_NUM_WC; pm++) {
        /* less than 4 slots per PM */
        if (pm_en_10g[pm] > 0 && pm_en_10g[pm] < 4) {
            for (subp = 0; subp < 4; subp++) {
                prt = pm_prt_10g[pm][subp];
                if (prt == 0)  continue;

                slot_idx_base = pm_cnt;
                slot_idx = slot_idx_base + step_wid * (prt_cnt % 4);
                mmu[slot_idx] = prt;
                LOG_VERBOSE(BSL_LS_SOC_TDM,
                            (BSL_META("TDM: [special] [10G:non-quad] "
                                      "pm_cnt=%0d, pm=%0d, subp=%0d, "
                                      "prt=%0d, offset=%0d, slot=%0d\n"),
                                      (pm_cnt + 1), pm, subp, prt,
                                      (prt_cnt % 4), slot_idx));
                prt_cnt++;
                if (prt_cnt % 4 == 3) pm_cnt++;
            }
        }
    }

    td2_tdm_print_mmu_cal(lr_idx_limit, mmu);
}
...
write_mmu_tdm_tbl(int pgw0[32], int pgw1[32], int ovs0[32], int ovs1[32], int mmu[256], int bucket1[16], int bucket2[16], int bucket3[16], int bucket4[16], int x_pipe, int port_state_map[128], int tsc[TDM_TD2_NUM_WC][4], int bw, enum tdm_td2_port_speed speed[TDM_TD2_NUM_EXT_PORTS], int (*op_flags)[1])
{
    ...
    int special_10g_20g_40g_en = 0, io_bw = 0;
    ...
    /* special process for configs containing only 10G/20G/40G */
    if ((bw == 720) &&
        (y1 > 0 || y2 > 0 || y3 > 0) &&
        (y0 == 0 && y4 == 0 && y5 == 0)) {
        special_10g_20g_40g_en = 1;

        td2_tdm_special_10g_20g_40g(lr_idx_limit, y1, y2, y3,
                                    pgw10, pgw20, pgw40, mmu);

        io_bw = y1 + 2 * y2 + 4 * y3;
        if (io_bw == 36) { /* 360G */
            LOG_VERBOSE(BSL_LS_SOC_TDM,
                        (BSL_META("TDM: [special] reduce lr_idx_limit "
                                  "from %0d to %0d \n"),
                                  lr_idx_limit, (lr_idx_limit - 1)));
            lr_idx_limit -= 1;
        }

        /* skip logic for 10G/20G/40G ports */
        y1 = 0;
        y2 = 0;
        y3 = 0;
    }
