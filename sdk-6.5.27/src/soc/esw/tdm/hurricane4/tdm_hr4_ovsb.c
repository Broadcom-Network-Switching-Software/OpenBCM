...
/**
@name: tdm_hr4_ovsb_apply_constraints
@param:

Partition OVSB PMs into 2 halfpipes
**/
int
tdm_hr4_ovsb_apply_constraints(tdm_mod_t *_tdm)
{
    int pms_per_pipe, pipe_id;
    int pm_num, ln_num, i;

    int speed_en_mtx[HR4_NUM_PHY_PM][6];
    ...
    pms_per_pipe = _tdm->_chip_data.soc_pkg.pm_num_phy_modules/HR4_NUM_QUAD;
    ...
    for (pm_num=pipe_id*pms_per_pipe; pm_num<(pipe_id+1)*pms_per_pipe; pm_num++) {
        _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num] = -1;
        phy_base_port = HR4_NUM_PM_LNS*pm_num+1;
            ...
            ((_tdm->_chip_data.soc_pkg.state[phy_port-1] == PORT_STATE__OVERSUB) ||
             (_tdm->_chip_data.soc_pkg.state[phy_port-1] == PORT_STATE__OVERSUB_HG) ) ) {
                /* Coding for pm_ovs_halfpipe:
                    ...
                    1 : Half Pipe 1 
                */
                _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num] = 2; /* OVSB but not assigned */
                switch(_tdm->_chip_data.soc_pkg.speed[phy_port]) {
                    case SPEED_10G : 
                        speed_en_mtx[pm_num][HR4_SPEED_IDX_10G] = 1; break;
                    case SPEED_20G : 
                        speed_en_mtx[pm_num][HR4_SPEED_IDX_20G] = 1; break;
                    case SPEED_25G : 
                        speed_en_mtx[pm_num][HR4_SPEED_IDX_25G] = 1; break;
                    case SPEED_40G : 
                        speed_en_mtx[pm_num][HR4_SPEED_IDX_40G] = 1; break;
                    case SPEED_50G : 
                        speed_en_mtx[pm_num][HR4_SPEED_IDX_50G] = 1; break;
                    case SPEED_100G : 
                        speed_en_mtx[pm_num][HR4_SPEED_IDX_100G]= 1; break;
                    default : break;
                }
            }
        }
    ...
    /* Restriction 13:No port configurations with more than 4 port speed classes are supported. */
        TDM_ERROR2("tdm_hr4_ovsb_apply_constraints() PIPE %d No OVSB port configurations with more than 4 port speed classes are supported; no_of_speeds_in_pipe=%d\n", pipe_id, no_of_speeds_in_pipe);
    ...
    /*Restriction 14: The only supported port configurations with 4 port speed classes are:
        ...
        10G/25G/50G/100G
    */
        if ( (speed_en_mtx_reduced[HR4_SPEED_IDX_20G] || speed_en_mtx_reduced[HR4_SPEED_IDX_40G]) && (speed_en_mtx_reduced[HR4_SPEED_IDX_25G] || speed_en_mtx_reduced[HR4_SPEED_IDX_50G]) ) {
        /* Group PMs with 25G/50G ports in HP0 and PMs with 20G/40G in HP1*/
            TDM_PRINT1("tdm_hr4_ovsb_apply_constraints() PIPE %d applying Restriction 14 \n",pipe_id);
            for (pm_num=pipe_id*pms_per_pipe; pm_num<(pipe_id+1)*pms_per_pipe; pm_num++) {
                if (speed_en_mtx[pm_num][HR4_SPEED_IDX_25G] || speed_en_mtx[pm_num][HR4_SPEED_IDX_50G]) {
                    _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num] = 0;
                } else if (speed_en_mtx[pm_num][HR4_SPEED_IDX_20G] || speed_en_mtx[pm_num][HR4_SPEED_IDX_40G]) {
                    _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num] = 1;
                }
            }
        }
    ...
    /*Restriction 15: All port configurations with 1-3 port speed classes are supported, except
        ...
        if ( speed_en_mtx_reduced[HR4_SPEED_IDX_20G] && speed_en_mtx_reduced[HR4_SPEED_IDX_25G]) {
        /* Group PMs with 25G/50G ports in HP0 and PMs with 20G/40G in HP1*/
            TDM_PRINT1("tdm_hr4_ovsb_apply_constraints() PIPE %d applying Restriction 15 \n",pipe_id);
            for (pm_num=pipe_id*pms_per_pipe; pm_num<(pipe_id+1)*pms_per_pipe; pm_num++) {
                if (speed_en_mtx[pm_num][HR4_SPEED_IDX_25G]) { /*if (speed_en_mtx[pm_num][HR4_SPEED_IDX_25G] || speed_en_mtx[pm_num][HR4_SPEED_IDX_50G]) {*/
                    _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num] = 0;
                } else if (speed_en_mtx[pm_num][HR4_SPEED_IDX_20G]) { /*} else if (speed_en_mtx[pm_num][HR4_SPEED_IDX_20G] || speed_en_mtx[pm_num][HR4_SPEED_IDX_40G]) {*/
                    _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num] = 1;
                }
            }
        }
...
/**
@name: tdm_hr4_ovsb_part_halfpipe
@param:
 ...
 */
int
tdm_hr4_ovsb_part_halfpipe(tdm_mod_t *_tdm)
{
    int tmp;
    int pm_speed[HR4_NUM_PHY_PM];
    int pm_num_subports[HR4_NUM_PHY_PM];
    int pm_num_sort[HR4_NUM_PHY_PM];
    ...
    pm_indx=0;
    for (pm_num= 0 ; pm_num < HR4_NUM_PHY_PM ; pm_num++) { /* considering only OS pm's */
        phy_base_port = HR4_NUM_PM_LNS*pm_num+1;
        pm_speed[pm_indx] = 0;
        pm_num_subports[pm_indx] = 0;
        for (ln_num=0; ln_num<_tdm->_chip_data.soc_pkg.pmap_num_lanes; ln_num++) {
            phy_port = phy_base_port+ln_num;
          /*    TDM_PRINT5("tdm_hr4_part_halfpipe() pm_num=%d ln_num=%d port=%d pm_speed=%d pm_indx=%d \n",pm_num, ln_num, phy_port, _tdm->_chip_data.soc_pkg.speed[phy_port]/1000, pm_indx); */ 
            if((_tdm->_chip_data.soc_pkg.speed[phy_port] != SPEED_0) &&
            ((_tdm->_chip_data.soc_pkg.state[phy_port-1] == PORT_STATE__OVERSUB) ||
             (_tdm->_chip_data.soc_pkg.state[phy_port-1] == PORT_STATE__OVERSUB_HG) ) ) {

	     /*if ( phy_port <= 24) {*/
         TDM_PRINT4("tdm_hr4_part_halfpipe1() pm_num=%d ln_num=%d port=%d pm_speed=%d\n",pm_num, ln_num, phy_port, _tdm->_chip_data.soc_pkg.speed[phy_port]/1000);
	    ...
	    else if (( phy_port <=60 ) && (phy_port >= 57 )) {  to include first half pipe first falcon 
                   TDM_PRINT4("tdm_hr4_part_halfpipe1() pm_num=%d ln_num=%d port=%d pm_speed=%d\n",pm_num, ln_num, phy_port, _tdm->_chip_data.soc_pkg.speed[phy_port]/1000);
                   pm_speed[pm_indx] = pm_speed[pm_indx] + _tdm->_chip_data.soc_pkg.speed[phy_port]/1000;
                   pm_num_subports[pm_indx]++;
                          } 
	    else if ( phy_port <=HR4_NUM_PHY_PORTS) {
                    TDM_PRINT4("tdm_hr4_part_halfpipe2() pm_num=%d ln_num=%d port=%d pm_speed=%d\n",pm_num, ln_num, phy_port, _tdm->_chip_data.soc_pkg.speed[phy_port]/1000);
                    pm_speed[pm_indx] = pm_speed[pm_indx] + _tdm->_chip_data.soc_pkg.speed[phy_port]/1000;
                    pm_num_subports[pm_indx]++;
                    }*/
            }
    ...
    for (i=0; i<HR4_NUM_PHY_PM; i++) {
        for (j=(HR4_NUM_PHY_PM-1); j>i; j--) {
    ...
    for (i=0; i<HR4_NUM_PHY_PM; i++) {
        ...
        if (_tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num] == 0) {
            hp0_speed += pm_speed[i];
            no_pms_hp0++;
          /*  TDM_PRINT3(" num of half_pipe0 ports =%d  half_pipe1 ports =%d ,i=%d \n", no_pms_hp0,no_pms_hp1,i ); */
        }
        if (_tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num] == 1) {
...
/**
@name: tdm_hr4_ovsb_fill_group
@param:

Partition OVSB PMs into 2 halfpipes
**/
int
tdm_hr4_ovsb_fill_group(tdm_mod_t *_tdm)
{
    int tmp;
    int pm_num_subports[HR4_NUM_PHY_PM];
    int pm_num_sort[HR4_NUM_PHY_PM];
    ...
    int start_group;
    int grp_index[HR4_OS_VBS_GRP_NUM];
    int phy_base_port;
    int phy_port;
    
    int t_ovs_grp_num;
    
    half_pipe_num = _tdm->_chip_data.soc_pkg.soc_vars.hr4.half_pipe_num;
    grp_speed     = _tdm->_chip_data.soc_pkg.soc_vars.hr4.grp_speed;
  ...
  /*  pms_per_pipe  = _tdm->_chip_data.soc_pkg.pm_num_phy_modules/HR4_NUM_QUAD; */
    ...
    for (pm_num=0 ; pm_num<HR4_NUM_PHY_PM ; pm_num++) {
        phy_base_port = HR4_NUM_PM_LNS*(pm_num)+1;
        pm_num_subports[pm_indx] = 0;
        if (_tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num] == half_pipe_num){
            for (ln_num=0; ln_num<_tdm->_chip_data.soc_pkg.pmap_num_lanes; ln_num++) {
                phy_port = phy_base_port+ln_num;
    ...
    switch (grp_speed) {
        ...
        default:													
            TDM_PRINT1("tdm_hr4_ovsb_fill_group3() Invalid group speed %0d\n",grp_speed);		
            return FAIL;							
    }
    /* ceil [(speed_max_num_ports_per_pm*pms_with_grp_speed) / HR4_OS_VBS_GRP_LEN ]*/
    min_num_ovs_groups = ((speed_max_num_ports_per_pm*pms_with_grp_speed) + HR4_OS_VBS_GRP_LEN-1) / HR4_OS_VBS_GRP_LEN;
    ...
    if (min_num_ovs_groups > 0) {
        ...
        switch (_tdm->_core_data.vars_pkg.cal_id) {
            ...
            default:
                TDM_PRINT1("tdm_hr4_ovsb_fill_group()Invalid calendar ID - %0d\n",_tdm->_core_data.vars_pkg.cal_id);
                return FAIL;
        }
        start_ovs_group = half_pipe_num *(HR4_OS_VBS_GRP_NUM/2);
        start_group = start_ovs_group;
        for (i=start_group; i< (start_group+(HR4_OS_VBS_GRP_NUM/2)); i++) {
            if (cal->cal_grp[i][0] != HR4_NUM_EXT_PORTS) {
                start_ovs_group++;
            } else {
                break;
            }
        ...
        for (i=0; i< HR4_OS_VBS_GRP_NUM; i++) { grp_index[i] = 0;}
...
/**
@name: tdm_hr4_ovsb_pkt_shaper
@param:

Populates Pkt shaper calendar
**/
int
tdm_hr4_ovsb_pkt_shaper(tdm_mod_t *_tdm)
{
    int pms_per_pipe, pipe_id;
    int pm_num, ln_num;
    int phy_base_port, phy_port;
        ...
        for (ln_num=0; ln_num<_tdm->_chip_data.soc_pkg.pmap_num_lanes; ln_num++) {
            ...
            if (phy_port<=_tdm->_chip_data.soc_pkg.soc_vars.fp_port_hi) {
                if((_tdm->_chip_data.soc_pkg.speed[phy_port] != SPEED_0) &&
                   ((_tdm->_chip_data.soc_pkg.state[phy_port-1] == PORT_STATE__OVERSUB) ||
                    (_tdm->_chip_data.soc_pkg.state[phy_port-1] == PORT_STATE__OVERSUB_HG) ) ) {
                    TDM_PRINT4("tdm_hr4_ovsb_pkt_shaper2() pm_num=%d ln_num=%d port=%d pm_speed=%d\n",
                               pm_num, ln_num, phy_port, _tdm->_chip_data.soc_pkg.speed[phy_port]);
                    _tdm->_core_data.vars_pkg.port = phy_port;
                    tdm_hr4_ovsb_pkt_shaper_per_port(_tdm);
                }
            }
        }
    }
...
/**
@name: tdm_hr4_ovsb_pkt_shaper_per_port
@param:

Populates Pkt shaper calendar - per port
**/
int
tdm_hr4_ovsb_pkt_shaper_per_port(tdm_mod_t *_tdm)
{
    int pms_per_pipe;
    int pm_num, i;
    int phy_port;
    ...
    int total_num_slots;
    int port_slot_tbl[HR4_SHAPING_GRP_LEN/8];
    int port_slot_tbl_shift[HR4_SHAPING_GRP_LEN/8];
    int slot_tbl_shift;
    int subport_no;
    int is_20G;
    int *pkt_shed_cal;
    ...
    pm_num = (phy_port-1)/HR4_NUM_PM_LNS;
    half_pipe_num = _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num];

    pms_per_pipe  = _tdm->_chip_data.soc_pkg.pm_num_phy_modules/HR4_NUM_QUAD;
    max_pms_per_halfpipe = pms_per_pipe/2;

    pkt_sched_repetitions = HR4_SHAPING_GRP_LEN/(pms_per_pipe/2)/HR4_NUM_PM_LNS; /* TH2: 5 = 160/8/4 */

    /*TDM_PRINT2("tdm_hr4_ovsb_pkt_shaper() pipe_id=%d pms_per_pipe=%d \n",pipe_id, pms_per_pipe);*/

    /* Determine the pm_indx in pkt shceduler calendar - 0 to 7*/
    pkt_shpr_pm_indx = tdm_hr4_ovsb_pkt_shaper_find_pm_indx(_tdm);
    ...
    switch (_tdm->_chip_data.soc_pkg.speed[phy_port]) {
        ...
        case SPEED_40G:	
            if ((_tdm->_chip_data.soc_pkg.speed[HR4_NUM_PM_LNS*pm_num+1+1] == SPEED_0) &&
                (_tdm->_chip_data.soc_pkg.speed[HR4_NUM_PM_LNS*pm_num+1+2] == SPEED_0) &&
                (_tdm->_chip_data.soc_pkg.speed[HR4_NUM_PM_LNS*pm_num+1+3] == SPEED_0) ) { /* SINGLE */
                no_of_lanes=4;
            } else { /* DUAL */
                no_of_lanes=2;
            }
    ...
    for (i=0; i<HR4_SHAPING_GRP_LEN/max_pms_per_halfpipe; i++) {
        port_slot_tbl[i] = HR4_NUM_EXT_PORTS;
        port_slot_tbl_shift[i]= HR4_NUM_EXT_PORTS;
    }
    ...
    switch (_tdm->_core_data.vars_pkg.cal_id) {
        case 0:	pkt_shed_cal = _tdm->_chip_data.cal_0.cal_grp[HR4_SHAPING_GRP_IDX_0+half_pipe_num]; break;
        case 1:	pkt_shed_cal = _tdm->_chip_data.cal_1.cal_grp[HR4_SHAPING_GRP_IDX_0+half_pipe_num]; break;
        case 2:	pkt_shed_cal = _tdm->_chip_data.cal_2.cal_grp[HR4_SHAPING_GRP_IDX_0+half_pipe_num]; break;
        case 3:	pkt_shed_cal = _tdm->_chip_data.cal_3.cal_grp[HR4_SHAPING_GRP_IDX_0+half_pipe_num]; break;
        default:
            TDM_PRINT1("tdm_hr4_ovsb_pkt_shaper_per_port() Invalid calendar ID - %0d\n",_tdm->_core_data.vars_pkg.cal_id);
            return FAIL;
    }

    /*  In order to spread tokens as uniformly as possible shift slot_tbl based on subport number */	
    subport_no = (phy_port-1)%HR4_NUM_PM_LNS;
    slot_tbl_shift = 0;
    if (no_of_lanes==1) {
        switch (subport_no) {
    ...
    for (i=0; i<total_num_slots; i++) {
        ...
        if (no_of_lanes==4) { /* SINGLE port mode */
            ...
            switch (lane) {
                ...
                default: 
                    TDM_PRINT1("tdm_hr4_ovsb_pkt_shaper_per_port() phy_port lane for phy_port=%d\n",phy_port);
                    return FAIL;			
            }
        }
...
/**
@name: tdm_hr4_ovsb_pkt_shaper_per_pm
@param:

Populates Pkt shaper calendar - per PM
**/
int
tdm_hr4_ovsb_pkt_shaper_per_pm(tdm_mod_t *_tdm, int shift_step)
{
    int pms_per_pipe;
    int pm_num, i;
    int port_phy;
    ...
    pm_num = (port_phy-1)/HR4_NUM_PM_LNS;
    half_pipe_num = _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[pm_num];
    ...
    for (i=0; i<HR4_SHAPING_GRP_LEN/8; i++) {
        port_slot_tbl[i] = HR4_NUM_EXT_PORTS;
    }
    ...
    pm_port_lo = HR4_NUM_PM_LNS * pm_num + 1;
    switch (param_speeds[port_phy]) {
        case SPEED_10G:
            if ((param_speeds[pm_port_lo + 1] == SPEED_0) &&
                ...
                /* SINGLE */
                base_offset = 0;
                port_slot_tbl[(base_offset + 0) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 8) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
            } else {
                /* QUAD */
                ...
                port_slot_tbl[(base_offset + 0 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 8 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 10) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 1;
                port_slot_tbl[(base_offset + 18) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 1;
                port_slot_tbl[(base_offset + 5 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(base_offset + 13) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(base_offset + 3 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 3;
                port_slot_tbl[(base_offset + 15) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 3;
            }
            break;
        case SPEED_20G:
            if ((param_speeds[pm_port_lo + 1] == SPEED_0) &&
                ...
                base_offset = 0;
                port_slot_tbl[(base_offset + 0 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 4 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 10) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 14) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
            } else {
                /* DUAL */
                port_slot_tbl[(0 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(4 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(10) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(14) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(1 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(5 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(11) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(15) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
            }
            break;
        case SPEED_40G:	
            if ((param_speeds[pm_port_lo + 1] == SPEED_0) &&
                ...
                port_slot_tbl[(base_offset + 8 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 10) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 18) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 5 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 13) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 3 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 15) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
            } else {
                /* DUAL */
                base_offset = 0;
                port_slot_tbl[(base_offset + 0 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 2 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 4) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 6) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 10) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 12) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 14) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 16) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(base_offset + 1 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(base_offset + 3 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(base_offset + 5 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(base_offset + 7 ) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(base_offset + 11) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(base_offset + 13) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(base_offset + 15) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(base_offset + 17) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
            }
            break;
        case SPEED_25G:
            /* QUAD */
            for (i = 0; i < pkt_sched_repetitions; i++) {
                port_slot_tbl[(i * 4 + 0) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                port_slot_tbl[(i * 4 + 2) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 1;
                port_slot_tbl[(i * 4 + 1) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                port_slot_tbl[(i * 4 + 3) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 3;
            }
            break;
        case SPEED_50G:
            if ((param_speeds[pm_port_lo + 1] == SPEED_0) &&
                ...
                base_offset = 0;
                for (i = 0; i < 10; i++) {
                    port_slot_tbl[(base_offset + i*2) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                }
            } else {
                /* DUAL */
                for (i = 0; i < 10; i++) {
                    port_slot_tbl[(i * 2)     % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo;
                    port_slot_tbl[(i * 2 + 1) % (HR4_SHAPING_GRP_LEN/8)] = pm_port_lo + 2;
                }
            }
            break;
        case SPEED_100G:
    ...
    for (i=0; i<HR4_SHAPING_GRP_LEN/8; i++) {
        if (port_slot_tbl[i] != HR4_NUM_EXT_PORTS) {
            lane = i % HR4_NUM_PM_LNS;
            switch (lane) {
            ...
            block_num = (i / HR4_NUM_PM_LNS) % pkt_sched_repetitions;
            block_offset = block_num * 32;
            cal_pos = (block_offset + lane_offset + pm_offset) % HR4_SHAPING_GRP_LEN;
...
/**
@name: tdm_hr4_ovsb_chk_port_is_os
@param:
...
**/
int
tdm_hr4_ovsb_chk_port_is_os(tdm_mod_t *_tdm, int port)
{
...
/**
@name: tdm_hr4_ovsb_chk_pm_is_4x10g
@param:
...
**/
int
tdm_hr4_ovsb_chk_pm_is_4x10g(tdm_mod_t *_tdm, int pm_num)
{
...
/**
@name: tdm_hr4_ovsb_hpipe_has_100g
@param:
...
**/
int
tdm_hr4_ovsb_hpipe_has_100g(tdm_mod_t *_tdm, int pipe_id, int hp_num)
{
    int result = BOOL_FALSE;
    ...
    pms_per_pipe = param_pm_num / HR4_NUM_QUAD;
...
/**
@name: tdm_hr4_ovsb_pkt_shaper_new
@param:

Populates Pkt shaper calendar
**/
int
tdm_hr4_ovsb_pkt_shaper_new(tdm_mod_t *_tdm)
{
    int pms_per_pipe, pipe_id;
    int pm_num, ln_num;
    int pm_s, pm_e;
    ...
    param_pm_lanes = _tdm->_chip_data.soc_pkg.pmap_num_lanes;
    param_pm_hp_id = _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe;
    ...
    for (pm_num=pm_s; pm_num<pm_e; pm_num++) {
        ...
        if (tdm_hr4_ovsb_chk_pm_is_4x10g(_tdm, pm_num) == BOOL_TRUE) {
            hp_num = param_pm_hp_id[pm_num];
            if (tdm_hr4_ovsb_hpipe_has_100g(_tdm, pipe_id, hp_num)
                                           == BOOL_TRUE) {
        ...
        for (ln_num=0; ln_num<param_pm_lanes; ln_num++) {
            ...
            if (tdm_hr4_ovsb_chk_port_is_os(_tdm, port_phy) == BOOL_TRUE) {
                _tdm->_core_data.vars_pkg.port = port_phy;
                tdm_hr4_ovsb_pkt_shaper_per_pm(_tdm, shift_step);
                break;
            }
        }
    }
...
/**
@name: tdm_hr4_ovsb_pkt_shaper_find_pm_indx
@param:

Find an existing or available pm_indx in pkt_shaper
**/
int
tdm_hr4_ovsb_pkt_shaper_find_pm_indx(tdm_mod_t *_tdm)
{
    ...
    if (pkt_shpr_pm_indx == -1) {
        for (i=0; i<(pms_per_pipe/2); i++){
            pm_indx_avail[i]=1;
            for (j=0; j<(HR4_SHAPING_GRP_LEN/(pms_per_pipe/2)); j++){
        ...
        if (pm_indx_avail[pkt_shpr_pm_indx] == 0) {
            pkt_shpr_pm_indx = -1;
            TDM_ERROR3("tdm_hr4_ovsb_pkt_shaper_find_pm_indx() PIPE %d pm_num=%d phy_port=%d Unable to find an available PM \n",pipe_id, pm_num, phy_port);
        }
    }
...
/**
@name: tdm_hr4_ovsb_map_pm_num_to_pblk
@param:

Maps PM num (block_id) to OVSB pblk id
**/
int
tdm_hr4_ovsb_map_pm_num_to_pblk(tdm_mod_t *_tdm)
{
int pm; 
    int *pm2pblk_map = NULL;

    pm2pblk_map  = _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_num_to_pblk;

    for (pm = 0 ; pm < HR4_NUM_PM_MOD; pm++) {
        pm2pblk_map[pm] = -1;
        if(pm < 4)
	pm2pblk_map[pm] = 0x2; 
	else if (pm < 8)
	...
	pm2pblk_map[pm] = 0x0; 
	else pm2pblk_map[pm] = 0x1; /* if (pm == 16)
	pm2pblk_map[pm] = 0x0; 
	else if (pm == 17)
	pm2pblk_map[pm] = 0x1; 
	else if (pm == 18)
...
**/
int
tdm_hr4_ovsb_flexmapping(tdm_mod_t *_tdm)
{	
    int i, j;
    tdm_calendar_t *cal;
    ...
    if(tdm_hr4_ovsb_apply_constraints(_tdm))
      TDM_PRINT1("OVSB Apply constraints PIPE %d DONE\n", _tdm->_core_data.vars_pkg.cal_id);
    /* Partition  */
    if(tdm_hr4_ovsb_part_halfpipe(_tdm))
      TDM_PRINT1("OVSB PM Half Pipe pre-partition for PIPE %d DONE\n", _tdm->_core_data.vars_pkg.cal_id);

    _tdm->_chip_data.soc_pkg.soc_vars.hr4.grp_speed = SPEED_10G;
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.half_pipe_num = 0;
    if(tdm_hr4_ovsb_fill_group(_tdm)) {
      TDM_PRINT2("10G OVSB partition for PIPE %d HALF_PIPE=%d DONE\n", _tdm->_core_data.vars_pkg.cal_id,_tdm->_chip_data.soc_pkg.soc_vars.hr4.half_pipe_num); 
    }
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.half_pipe_num = 1;
    if(tdm_hr4_ovsb_fill_group(_tdm)) {
      TDM_PRINT2("10G OVSB partition for PIPE %d HALF_PIPE=%d DONE\n", _tdm->_core_data.vars_pkg.cal_id,_tdm->_chip_data.soc_pkg.soc_vars.hr4.half_pipe_num);
    }

    _tdm->_chip_data.soc_pkg.soc_vars.hr4.grp_speed = SPEED_20G;
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.half_pipe_num = 0;
    if(tdm_hr4_ovsb_fill_group(_tdm)) {
    ...
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.grp_speed = SPEED_25G;
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.half_pipe_num = 0;
    if(tdm_hr4_ovsb_fill_group(_tdm)) {
    ...
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.grp_speed = SPEED_40G;
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.half_pipe_num = 0;
    if(tdm_hr4_ovsb_fill_group(_tdm)) {
    ...
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.grp_speed = SPEED_50G;
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.half_pipe_num = 0;
    if(tdm_hr4_ovsb_fill_group(_tdm)) {
    ...
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.grp_speed = SPEED_100G;
    _tdm->_chip_data.soc_pkg.soc_vars.hr4.half_pipe_num = 0;
    if(tdm_hr4_ovsb_fill_group(_tdm)) {
    ...
    if(tdm_hr4_ovsb_pkt_shaper(_tdm)) {
      TDM_PRINT1("OVSB partition for PIPE %d PKT_SHAPER CALENDAR DONE\n", _tdm->_core_data.vars_pkg.cal_id);
    }

    if(tdm_hr4_ovsb_map_pm_num_to_pblk(_tdm)) {
...
**/
int
tdm_hr4_ovsb_fixmapping(tdm_mod_t *_tdm)
{	
    int i, j;
    tdm_calendar_t *cal;
    ...
    /* Apply Half-Pipe constraints */
    {
       ...
       /* pm_num_lo     = param_pipe_id*pms_per_pipe;
        ...
        for (i=0; i < (param_pm_num ); i++) {
            _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[i] = -1;
            /*for (j=0; j<param_pm_lanes; j++) {
                ...
                if (port_phy<= 24) {
                                _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[i] = 0;
                            } 
			    else if ((port_phy<=64 ) && (port_phy >= 57 )) {  to include first halfpipe first falcon 
                                _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[i] = 0;
                            } 
			    else if ((port_phy<=60 ) && (port_phy >= 57 )) {  to include first halfpipe first falcon
                                _tdm->_chip_data.soc_pkg.soc_vars.hr4.pm_ovs_halfpipe[i] = 0;
                            }
			    else if (port_phy<=HR4_NUM_PHY_PORTS) {
                            } else {
    ...
    if (tdm_hr4_ovsb_pkt_shaper_new(_tdm)){
      TDM_PRINT1("OVSB partition for PIPE %d PKT_SHAPER CALENDAR DONE\n", _tdm->_core_data.vars_pkg.cal_id);
    }
...
int
tdm_hr4_vbs_scheduler_ovs(tdm_mod_t *_tdm)
{	
    /* return (tdm_hr4_ovsb_flexmapping(_tdm)); */
    
    return (tdm_hr4_ovsb_fixmapping(_tdm));
}
