   ...
   for (port_phy = 1;
        ...
        for (port_phy = param_phy_lo;
          ...
          for(j=port_phy ; j < (HR4_NUM_PMQ_SUBPORTS+port_phy)  ; j++){
              if (param_1g_en_gphy[port_phy - 1] >= HR4_1G_PORT_STATE_16x1G)
...
/**
  @name: tdm_hr4_main_adjust_HG_20G
  @param:
  -- _tdm: ptr to TDM global struct.

  If 20G speed is configured as HiGig, convert speed to 21G.

 **/
static void
tdm_hr4_main_adjust_HG_20G(tdm_mod_t *_tdm)
{
    int port_phy;
    int param_phy_lo, param_phy_hi;
    enum port_speed_e *param_speeds;
    ...
    for (port_phy = param_phy_lo;
        ...
        if (param_states[port_phy - 1] == PORT_STATE__LINERATE_HG) {
            switch(param_speeds[port_phy]){
                case SPEED_20G:
                    param_speeds[port_phy] = SPEED_21G;
                    TDM_PRINT1("[SPECIAL 20G HIGIG] physical port %0d, covert TDM speed from 20G to 21G\n", port_phy);
                    break;
                default:
                    break;
            }
...
tdm_hr4_main_transcription( tdm_mod_t *_tdm )
{
    ...
    /* change hg port speed from 20G->21G for x1 sku only */
    if (soc_feature(_tdm->_chip_data.soc_pkg.unit,
                          soc_feature_td3x1_enhanced_tdm)) {
        tdm_hr4_main_adjust_HG_20G(_tdm);
    }
...
tdm_hr4_main_init( tdm_mod_t *_tdm )
{
    ...
    /* set slot mode to mix only for x1 sku only */
    if (soc_feature(_tdm->_chip_data.soc_pkg.unit,
                                         soc_feature_td3x1_enhanced_tdm)) {
        _tdm->_core_data.vars_pkg.pipe_info.slot_mode = TDM_SLOT_MODE_MIX;
    }
