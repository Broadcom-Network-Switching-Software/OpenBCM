...
{
    int num_tcams, index_max;

    num_tcams = SOC_UTT_AGENT_NUM_TCAMS(unit, SOC_UTT_AGENT_IFP);

    index_max = num_tcams * SOC_UTT_TCAM_DEPTH(unit) - 1;
    SOP_MEM_STATE_MAX_INDEX_SET(unit, IFP_TCAM_WIDEm, index_max);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, IFP_TCAM_WIDE_PIPE0m, index_max);

    index_max = num_tcams * 2 * SOC_UTT_TCAM_DEPTH(unit) - 1;
    SOP_MEM_STATE_MAX_INDEX_SET(unit, IFP_TCAMm, index_max);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, IFP_TCAM_PIPE0m, index_max);

    if (soc_feature(unit, soc_feature_utt_ver1_5)) {
        index_max = num_tcams * 2 * SOC_UTT_TCAM_DEPTH(unit) - 1;
        SOP_MEM_STATE_MAX_INDEX_SET(unit, IFP_POLICY_TABLEm, index_max);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, IFP_POLICY_TABLE_PIPE0m, index_max);

        index_max = num_tcams * SOC_UTT_TCAM_DEPTH(unit) - 1;
        SOP_MEM_STATE_MAX_INDEX_SET(unit, IFP_POLICY_TABLE_WIDEm, index_max);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, IFP_POLICY_TABLE_WIDE_PIPE0m, index_max);
    }
    return SOC_E_NONE;
}
...
_soc_utt_ifp_feature_reset(int unit, int lookups)
{
    ...
    case 4:
        if (soc_feature(unit, soc_feature_td3x1_reduced_utt)) {
            SOC_FEATURE_SET(unit, soc_feature_field_slices4);
        } else {
            return SOC_E_PARAM;
        }
        break;
    case 5:
        if (soc_feature(unit, soc_feature_td3x1_reduced_utt)) {
            SOC_FEATURE_SET(unit, soc_feature_field_slices5);
        } else {
            return SOC_E_PARAM;
        }
        break;
    case 6:
        if (soc_feature(unit, soc_feature_td3x1_reduced_utt)) {
            SOC_FEATURE_SET(unit, soc_feature_field_slices6);
        } else {
            return SOC_E_PARAM;
        }
        break;
    case 7:
        if (soc_feature(unit, soc_feature_td3x1_reduced_utt)) {
            SOC_FEATURE_SET(unit, soc_feature_field_slices7);
        } else {
            return SOC_E_PARAM;
        }
        break;
...
_soc_utt_lpm_mem_config(int unit)
{
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_PAIR_128m, num_v6_128b - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_PAIR_128_ONLYm, num_v6_128b - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_PAIR_128_DATA_ONLYm, num_v6_128b - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_PAIR_128_HIT_ONLYm, num_v6_128b - 1);
...
_soc_utt_mem_process_config_var(int unit)
{
    ...
    /* Max tcams allowed */
    ifp_max_tcams = SOC_UTT_AGENT_MAX_TCAMS(unit, SOC_UTT_AGENT_IFP);
    if (soc_feature(unit, soc_feature_td3x1_reduced_utt)) {
        lpm_max_tcams = SOC_UTT_AGENT_MAX_TCAMS(unit, SOC_UTT_AGENT_LPM)/2;
    } else {
        lpm_max_tcams = SOC_UTT_AGENT_MAX_TCAMS(unit, SOC_UTT_AGENT_LPM);
    }
    ...
    if (ifp_mem_size == 0 && lpm_mem_size == 0) {
        ...
        ifp_mem_size = ((SOC_UTT_MAX_TCAMS(unit) * SOC_UTT_TCAM_DEPTH(unit))
                        - lpm_mem_size);
        if (ifp_mem_size >
           (ifp_max_tcams * SOC_UTT_TCAM_DEPTH(unit))) {
            ifp_mem_size = ifp_max_tcams * SOC_UTT_TCAM_DEPTH(unit);
        }
    } else if (lpm_mem_size == 0) {
        lpm_mem_size = ((SOC_UTT_MAX_TCAMS(unit) * SOC_UTT_TCAM_DEPTH(unit))
                        - ifp_mem_size);
        if (lpm_mem_size >
           (lpm_max_tcams * SOC_UTT_TCAM_DEPTH(unit))) {
            lpm_mem_size = lpm_max_tcams * SOC_UTT_TCAM_DEPTH(unit);
        }
        lpm_tcams = lpm_mem_size / SOC_UTT_TCAM_DEPTH(unit);
        if (soc_feature(unit, soc_feature_utt_ver1_5)) {
            lpm_tcams &= (~0x3);
