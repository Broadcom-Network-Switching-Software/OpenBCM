...
STATIC
int _soc_esw_portctrl_serdes_tx_taps_get(int unit, soc_port_t port,
                                         int* tx_params_user_flag, phymod_tx_t* tx_params)
{
    char *config_str, *sub_str, *sub_str_end;
    char str_buf[8];
    int16_t *field = NULL;
    ...
    int NRZ_MAX_SPEED_PER_LANE = 28125;

    sal_memset(tx_params, 0, sizeof(phymod_tx_t));

    config_str = soc_property_port_get_str(unit, port, spn_SERDES_TX_TAPS);

    /* Check if this config is specified */
    if (NULL == config_str) {
        return rv;
    }

    /* User defined serdes_tx_taps */
    *tx_params_user_flag |= PORTMOD_USER_SET_TX_PREEMPHASIS_BY_CONFIG;
    ...
    if (!(SOC_CONTROL(unit)->soc_flags & SOC_F_ALL_MODULES_INITED)) {
        speed_per_lane = si->port_speed_max[port] /
                            ...
                            phymod_operation_mode_t *phy_op_mode)
{
    ...
    sal_memset(&tx_taps, 0, sizeof(phymod_tx_t));
   ...
   /* Get the timestamp adjust */
    ...
    if (!SOC_IS_TOMAHAWK3(unit)) {
        SOC_IF_ERROR_RETURN(_soc_esw_portctrl_serdes_tx_taps_get(unit, port, &tx_taps_flag, &tx_taps));

        /* if "serdes_tx_taps" is used, copy the tx params to each lane */
        if (tx_taps_flag & PORTMOD_USER_SET_TX_PREEMPHASIS_BY_CONFIG) {
            for (i = 0; i < port_num_lanes; i++) {
                 init_config->tx_params_user_flag[i] |= PORTMOD_USER_SET_TX_PREEMPHASIS_BY_CONFIG;
                 init_config->tx_params[i].sig_method = tx_taps.sig_method;
                 init_config->tx_params[i].tap_mode = tx_taps.tap_mode;
                 init_config->tx_params[i].pre = tx_taps.pre;
                 init_config->tx_params[i].main = tx_taps.main;
                 init_config->tx_params[i].post = tx_taps.post;
                 init_config->tx_params[i].pre2 = tx_taps.pre2;
                 init_config->tx_params[i].post2 = tx_taps.post2;
                 init_config->tx_params[i].post3 = tx_taps.post3;
            }
        }
    }
...
soc_portctrl_xport_type_verify(int unit)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        ...
        if ((IS_XE_PORT(unit, port) || IS_CE_PORT(unit, port))
           ...
           if ((mode == SOC_ENCAP_IEEE) && !soc_feature(unit, soc_feature_hgoe_line_card_mode)){
...
#if defined(PORTMOD_SUPPORT) && defined(PORTMOD_PM8X50_SUPPORT)
STATIC int
soc_esw_portctrl_flr_config_get(int unit, soc_port_t port, int *flr_support, int *timeout)
{

    char *config_str, *sub_str, *sub_str_end;

    config_str = soc_property_port_get_str(unit, port, spn_PHY_INIT_FLR_ENABLE);
    if (config_str == NULL) {
        *flr_support = 0;
        *timeout = 0;
        return SOC_E_NONE;
    ...
    *flr_support = sal_ctoi(sub_str, &sub_str_end);
    if (sub_str == sub_str_end) {
        LOG_CLI((BSL_META_U(unit,
                            "Port %d: Missing flr support information \"%s\"\n"),
                 port, config_str));
        return SOC_E_FAIL;
    }
    if (*flr_support !=0 && *flr_support !=1) {
        LOG_CLI((BSL_META_U(unit,
                            "Port %d: Invalid flr support information\n"),
                 port));
        return SOC_E_FAIL;
    }
    ...
    if (*sub_str != '\0') {
        ...
        LOG_CLI((BSL_META_U(unit,
                            "Port %d: missing flr timeout information\n"),
                 port));
        return SOC_E_FAIL;
    }

    *timeout = sal_ctoi(sub_str, &sub_str_end);

    if (*timeout < 0 || *timeout > 6) {
        LOG_CLI((BSL_META_U(unit,
                            "Port %d: Invalid flr timeout time \n"),
                 port));
        return SOC_E_FAIL;
    }
 ...
 *      d1/pbmp       - Logical Port BitMap
 *      d2
 *      d3
 *      d4
 */
STATIC void
soc_esw_process_flr_link_up_intr(void *unit_vp, void *pbmp_vp,
                                      void *d2, void *d3, void *d4)
{
    ...
    SOC_PBMP_ITER(*flr_linkup_pbmp, lport) {
#ifdef PORTMOD_SUPPORT
...
STATIC void
soc_esw_portctrl_flr_link_up_cb (int unit, soc_pbmp_t flr_linkup_pbmp)
{
    ...
    rv = sal_dpc( soc_esw_process_flr_link_up_intr,
                  INT_TO_PTR(unit),
                  linkup_pbmp,
                  0,0,0);
    if (rv) {
...
/*
 * Function:
 *      soc_esw_portctrl_flr_wb_restore
 * Purpose:
 *      Restore WB
 * Parameters:
 *      unit      - (IN) Unit number.
 ...
 */
int
soc_esw_portctrl_flr_wb_restore (int unit)
{
    int rv = SOC_E_NONE;
    int flr_port_count = 0;


    SOC_PBMP_COUNT(SOC_PERSIST(unit)->lc_pbm_flr_enable, flr_port_count);

    if (flr_port_count) {
        rv = soc_linkscan_flr_linkup_callback_set(unit,
                                              soc_esw_portctrl_flr_link_up_cb);
    }
    return rv;
}
...
/*
 * Function:
 *      soc_esw_portctrl_flr_config_set
 * Purpose:
 *      Add given logical port to PM (Port Macro).
 * Parameters:
 *      unit      - (IN) Unit number.
 *      port      - (IN) Port number.
 *      enable    - (IN) FLR enable
 *      timeout   - (IN) FLR timeout
 * Returns:
 *      SOC_E_XXX
 * Notes:
 */
int
soc_esw_portctrl_flr_config_set (int unit,
                                 soc_port_t port,
                                 int enable,
                                 soc_linkscan_flr_timeout_t timeout)
{
    ...
    rv = soc_linkscan_flr_enable_set(unit, port, enable, timeout);
    if (rv != SOC_E_NONE) {
        return rv;
    }
    ...
    if (flr_port_count) {
        ...
        /* unregister */
        rv = soc_linkscan_flr_linkup_callback_set(unit, NULL);
    }
    return rv;
}
...
soc_esw_portctrl_add(int unit, soc_port_t port, int init_flag, void *port_add_info)
{
    ...
    uint32 mtu_size;
#ifdef BCM_TOMAHAWK3_SUPPORT
    soc_port_phy_timesync_config_t timesync_config;
    int timesync_enable;
    int one_step_enable;
#endif
#ifdef PORTMOD_PM8X50_SUPPORT
    ...
    if (pm_type == portmodDispatchTypePm8x50) {
...
#ifdef PORTMOD_PM8X50_SUPPORT
        /*Download the flr ucode if any port in the pm with flr configuration*/
        rv = portmod_pm_phys_from_pm_id_get(unit, portmod_pm_id, &phys);
        if (rv != SOC_E_UNAVAIL && rv < 0) {
            if (local_add_info != NULL) {
                sal_free(local_add_info);
            }
        ...
        PORTMOD_PBMP_ITER(phys, phy) {
            lport = SOC_INFO(unit).port_p2l_mapping[phy];
            if (lport == -1) {
                continue;
            }
            rv = soc_esw_portctrl_flr_config_get(unit, lport, &flr_support, &timeout);
            if (rv != SOC_E_NONE) {
                if (local_add_info != NULL) {
                    sal_free(local_add_info);
                }
...
#ifdef BCM_HURRICANE4_SUPPORT
        /* Set threshold on ports based on PM clock frequency.
         * If PM_CLK_FREQ >= 495Mhz TX_THRESHOLD = 1 else TX_THRESHOLD = 2 */
        if (SOC_IS_HURRICANE4(unit)) {
            soc_info_t *si = &SOC_INFO(unit);
            uint64 reg_val;
            uint8 tx_thresh;
            ...
            pm_clk_fre = si->frequency * si->clk_to_pm_clk_factor;

            /* Set threshold based on pm clock frequency */
            if (pm_clk_fre >= 495) {
                tx_thresh = 1;
            } else {
                tx_thresh = 2;
            ...
            if (IS_XL_PORT(unit, port)) {
                ...
                soc_reg64_field32_set(unit, XLMAC_TX_CTRLr,
                        &reg_val, TX_THRESHOLDf, tx_thresh);
            ...
            if (IS_CL_PORT(unit, port)) {
                ...
                soc_reg64_field32_set(unit, CLMAC_TX_CTRLr,
                        &reg_val, TX_THRESHOLDf, tx_thresh);
                rv = WRITE_CLMAC_TX_CTRLr(unit, port, reg_val);
                if (SOC_FAILURE(rv)) {
                    if (local_add_info != NULL) {
    ...
    if (SOC_IS_FIRELIGHT(unit)) {
...
#ifdef PORTMOD_PM8X50_SUPPORT
        if (pm_type == portmodDispatchTypePm8x50) {
            rv = soc_esw_portctrl_flr_config_get(unit, port, &flr_support, &timeout);
            if (rv != SOC_E_NONE) {
                if (local_add_info != NULL) {
                    sal_free(local_add_info);
                }
            ...
            if(timeout != 0) {
                rv = portmod_port_flr_config_set(unit, pport, 1, timeout);
                if (PORTMOD_FAILURE(rv)) {
                    if (local_add_info != NULL) {
                        sal_free(local_add_info);
                    }
                ...
                rv = soc_esw_portctrl_flr_config_set(unit, port, 1, timeout);
                if (rv != SOC_E_NONE) {
                    if (local_add_info != NULL) {
                        sal_free(local_add_info);
                    }
        ...
        if ( SOC_IS_TOMAHAWK3(unit) &&
             ...
             !SOC_WARM_BOOT(unit) ) {
            timesync_enable = soc_property_port_get(unit, port, spn_SERDES_TIMESYNC_ENABLE, 0);
            one_step_enable = soc_property_port_get(unit, port, spn_SERDES_TIMESYNC_ONE_STEP_ENABLE, 0);

            sal_memset(&timesync_config, 0, sizeof(soc_port_phy_timesync_config_t));
            /*
             * TOMAHAWK3 only support SOP, Comp Mode EARLIEST.
             */
            if (timesync_enable) {
                timesync_config.flags |= SOC_PORT_PHY_TIMESYNC_ENABLE;
            }
            if (one_step_enable) {
                timesync_config.flags |= SOC_PORT_PHY_TIMESYNC_ONE_STEP_ENABLE;
            }
            rv = soc_portctrl_phy_timesync_config_set(unit, port, &timesync_config);
            if (rv != SOC_E_NONE) {
                return rv;
            }
        }
...
soc_esw_portctrl_duplex_set(int unit, soc_port_t port, int duplex)
{
    ...
    if (is_legacy_phy_present && is_macsec_enabled) {
        /*
         * Specail case for macsec phy that doesn't need to invoke
         * the portmod duplex set function.
         */
    ...
    */
    ...
    switch(phy_ctrl) {
        ...
        case SOC_PHY_CONTROL_AN_LINK_FAIL_INHIBIT_TIMER_LT_PAM4:
            PORTMOD_PORT_AN_LINK_FAIL_INHIBIT_TIMER_UPDATED_SET(port_dynamic_state);
            portmod_port_update_dynamic_state(unit, port, port_dynamic_state);
            break;
        default:
            break;
    ...
    */
    ...
    switch (phy_ctrl) {
        ...
        case SOC_PHY_CONTROL_FEC_UNCORRECTED_CODEWORD_COUNT:
            per_lane_ctrl = 0;
            break;
        case SOC_PHY_CONTROL_RS_FEC_BIT_ERROR_COUNT:
            per_lane_ctrl = 0;
            break;
        default:
            break;
                           ...
                           int sys_side, soc_port_phy_tx_t *tx)
{
    ...
    switch(tx->tx_tap_mode) {
        case _SHR_PORT_PHY_TX_TAP_MODE_3_TAP:
            phy_tx.tap_mode = phymodTxTapMode3Tap;
            break;
        case _SHR_PORT_PHY_TX_TAP_MODE_6_TAP:
    ...
    switch(tx->signalling_mode) {
        case _SHR_PORT_PHY_SIGNALLING_MODE_NRZ:
            phy_tx.sig_method = phymodSignallingMethodNRZ;
            break;
        case _SHR_PORT_PHY_SIGNALLING_MODE_PAM4:
    ...
    if (SOC_SUCCESS(rv)) {
        ...
        switch(phy_tx.tap_mode) {
            case phymodTxTapMode3Tap:
                tx->tx_tap_mode = _SHR_PORT_PHY_TX_TAP_MODE_3_TAP;
                break;
            case phymodTxTapMode6Tap:
                tx->tx_tap_mode = _SHR_PORT_PHY_TX_TAP_MODE_6_TAP;
                break;
            default:
                return SOC_E_INTERNAL;
        }
        switch(phy_tx.sig_method) {
            case phymodSignallingMethodNRZ:
                tx->signalling_mode = _SHR_PORT_PHY_SIGNALLING_MODE_NRZ;
                break;
            case phymodSignallingMethodPAM4:
                tx->signalling_mode = _SHR_PORT_PHY_SIGNALLING_MODE_PAM4;
                break;
            default:
                return SOC_E_INTERNAL;
        }
