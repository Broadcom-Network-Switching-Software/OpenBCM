...
#define MIN_MACSEC_PORT            0
#define MAX_MACSEC_PORT           47
#define IDLE_MACSEC_PORT          49
#define MACSEC_TDM_CALENDAR_COLS   6
...
_soc_fl_tdm_set_macsec_calendar(int unit, soc_port_schedule_state_t *sch_info)
{
    ...
    /*
     * Post process to add dummy port in MACSEC TDM Calendar for
     * single MACSEC port or 3x100G MACSEC ports.
     */
    {
        uint8 match_found = FALSE, col_1_occupy = FALSE;
        int dummy_port = IDLE_MACSEC_PORT;
        int port, col_1_port;
        int rows, i;
        for (port = MAX_MACSEC_PORT ; port >= MIN_MACSEC_PORT ; port = port-4) {
            for (slot = 0; slot < length; slot ++) {
                macsec_port = sch_info->tdm_ingress_schedule_pipe[pipe]. \
                                  tdm_schedule_slice[1].linerate_schedule[slot];
                if (macsec_port == port) {
                    match_found = TRUE;
                    break;
                }
        ...
        rows = length / MACSEC_TDM_CALENDAR_COLS;

        for (i = 0; i < rows; i++) {
            col_1_port = sch_info->tdm_ingress_schedule_pipe[pipe]. \
                          tdm_schedule_slice[1].linerate_schedule[1 + i * \
                          MACSEC_TDM_CALENDAR_COLS];

            if (col_1_port != IDLE_MACSEC_PORT) {
                col_1_occupy = TRUE;
                break;
            }
        }
        ...
        for (i = 0; i < rows; i++) {
            ...
            if ((col_1_occupy == FALSE) && (col_1_port == IDLE_MACSEC_PORT)) {
                sch_info->tdm_ingress_schedule_pipe[pipe]. \
                          tdm_schedule_slice[1].linerate_schedule[1 + i * \
                          MACSEC_TDM_CALENDAR_COLS] = dummy_port;
            }
        }
    }
