...
soc_th3_portctrl_pm_portmod_deinit(int unit)
{
    ...
    /* Free mutex */
#ifdef PORTMOD_PM4X10_SUPPORT
    for (instances = 0; instances < SOC_TH3_PM4X10_COUNT; instances++) {
        if (pm4x10_th3_user_acc[unit][instances].mutex) {
            sal_mutex_destroy(pm4x10_th3_user_acc[unit][instances].mutex);
            pm4x10_th3_user_acc[unit][instances].mutex = NULL;
        }
    }
#endif /* PORTMOD_PM4X10_SUPPORT  */
#ifdef PORTMOD_PM8X50_SUPPORT
    for (instances = 0; instances < SOC_TH3_PM8X50_COUNT; instances++) {
        if (pm8x50_th3_user_acc[unit][instances].mutex) {
            sal_mutex_destroy(pm8x50_th3_user_acc[unit][instances].mutex);
            pm8x50_th3_user_acc[unit][instances].mutex = NULL;
        }
    }
#endif /* PORTMOD_PM8X50_SUPPORT  */
