...
/* Per-port setting for pending learn feature */
STATIC soc_th3_port_pending_lrn_cfg_t pending_learn_port_cfg[SOC_MAX_NUM_DEVICES];

#define SOC_PORT_ARG_CHECK(unit, port)                                  \
    if (!SOC_PORT_VALID(unit, port)) {                                  \
        return SOC_E_PORT;                                              \
    }
#define PORT_PENDING_LEARN_LOCK_CHECK(_unit_)                           \
    if (pending_learn_port_cfg[_unit_].pending_lrn_cfg_mutex == NULL) {    \
        return SOC_E_RESOURCE; \
    }
#define PORT_PENDING_LEARN_LOCK(_unit_)                           \
    sal_mutex_take(pending_learn_port_cfg[_unit_].pending_lrn_cfg_mutex, \
                   sal_mutex_FOREVER)
#define PORT_PENDING_LEARN_UNLOCK(_unit_)                         \
    sal_mutex_give(pending_learn_port_cfg[_unit_].pending_lrn_cfg_mutex)

#define PORT_PENDING_LEARN_BITMAP(unit) pending_learn_port_cfg[unit].pending_lrn_port_bmap

STATIC int pending_learn_en[SOC_MAX_NUM_DEVICES];
...
int
soc_th3_port_pending_learn_cfg_memsize_get(int unit, int *size)
{
    int pbmp_port_max;
    int pbmp_size_bytes;
    ...
    *size = pbmp_size_bytes;

    return SOC_E_NONE;
}

int
soc_th3_port_pending_learn_cfg_wb_sync(int unit, uint8 *scache_ptr)
{
    uint8 *sc_ptr = scache_ptr;

    if (!scache_ptr) {
        return SOC_E_PARAM;
    }

    PORT_PENDING_LEARN_LOCK_CHECK(unit);

    PORT_PENDING_LEARN_LOCK(unit);
    sal_memcpy(sc_ptr, &pending_learn_port_cfg[unit].pending_lrn_port_bmap,
               sizeof(pending_learn_port_cfg[unit].pending_lrn_port_bmap));
    PORT_PENDING_LEARN_UNLOCK(unit);

    return SOC_E_NONE;
}

int
soc_th3_port_pending_learn_cfg_wb_recover(int unit, uint8 *scache_ptr)
{
    uint8 *sc_ptr = scache_ptr;
    int pbmp_port_max;
    int pbmp_size_bytes;
    ...
    sal_memcpy(&(pending_learn_port_cfg[unit].pending_lrn_port_bmap),
               sc_ptr,
               SOC_SCACHE_SIZE_MIN(pbmp_size_bytes, sizeof(soc_pbmp_t)));
...
_soc_th3_flexport_scache_recovery(int unit, uint8 *flexport_scache_ptr)
{
    ...
    SOC_IF_ERROR_RETURN(_scache_th3_port_print_add(unit, -1, SPT_DISABLED_BITMAP, sizeof(pbmp_t),
            &flexport_scache_ptr[scache_offset - pbmp_scache_sz]));
...
int soc_th3_port_pending_learn_cfg_init(int unit)
{

    pending_learn_en[unit] = soc_property_get(unit, spn_L2_SW_PENDING_LEARN, 0);

    if (!pending_learn_en[unit]) {
        return SOC_E_UNAVAIL;
    }

    if (pending_learn_port_cfg[unit].pending_lrn_cfg_mutex != NULL) {
        PORT_PENDING_LEARN_LOCK(unit);
        SOC_PBMP_CLEAR(pending_learn_port_cfg[unit].pending_lrn_port_bmap);
        PORT_PENDING_LEARN_UNLOCK(unit);

        sal_mutex_destroy(pending_learn_port_cfg[unit].pending_lrn_cfg_mutex);
        pending_learn_port_cfg[unit].pending_lrn_cfg_mutex = NULL;
    }

    /* Create mutex */
    if (pending_learn_port_cfg[unit].pending_lrn_cfg_mutex == NULL) {
        pending_learn_port_cfg[unit].pending_lrn_cfg_mutex =
            sal_mutex_create("learn_pending_mutex");
...
int
soc_th3_port_pending_learn_cfg_cleanup(int unit)
{
    if (!pending_learn_en[unit]) {
        return SOC_E_UNAVAIL;
    }
...
int soc_th3_port_pending_learn_port_add(int unit, soc_port_t port)
{
    if (!pending_learn_en[unit]) {
        return SOC_E_UNAVAIL;
    }

    SOC_PORT_ARG_CHECK(unit, port);
    PORT_PENDING_LEARN_LOCK_CHECK(unit);

    PORT_PENDING_LEARN_LOCK(unit);
    SOC_PBMP_PORT_ADD(PORT_PENDING_LEARN_BITMAP(unit), port);
    PORT_PENDING_LEARN_UNLOCK(unit);
    return SOC_E_NONE;
}

int soc_th3_port_pending_learn_port_remove(int unit, soc_port_t port)
{
    if (!pending_learn_en[unit]) {
        return SOC_E_UNAVAIL;
    }
    ...
    PORT_PENDING_LEARN_LOCK(unit);
    SOC_PBMP_PORT_REMOVE(PORT_PENDING_LEARN_BITMAP(unit), port);
    PORT_PENDING_LEARN_UNLOCK(unit);

    return SOC_E_NONE;
}

int soc_th3_port_pending_learn_port_set(int unit, soc_pbmp_t *pbmp)
{
    if (!pending_learn_en[unit]) {
        return SOC_E_UNAVAIL;
    }
    ...
    PORT_PENDING_LEARN_LOCK(unit);
    SOC_PBMP_ASSIGN(PORT_PENDING_LEARN_BITMAP(unit), *pbmp);
    PORT_PENDING_LEARN_UNLOCK(unit);

    return SOC_E_NONE;
}

int soc_th3_port_pending_learn_state_get(int unit, soc_port_t port, int *state)
{
    ...
    PORT_PENDING_LEARN_LOCK(unit);
    *state = SOC_PBMP_MEMBER(PORT_PENDING_LEARN_BITMAP(unit), port) ? TRUE : FALSE;
    PORT_PENDING_LEARN_UNLOCK(unit);

    return SOC_E_NONE;
}

int soc_th3_port_pending_learn_bitmap_get(int unit, soc_pbmp_t *pbmp)
{
    ...
    PORT_PENDING_LEARN_LOCK(unit);
    SOC_PBMP_ASSIGN(*pbmp, PORT_PENDING_LEARN_BITMAP(unit));
    PORT_PENDING_LEARN_UNLOCK(unit);
