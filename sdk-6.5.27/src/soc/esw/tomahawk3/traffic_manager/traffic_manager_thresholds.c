            ...
            for (pool = 0; pool < _TH3_MMU_NUM_POOL; pool++) {
                SOC_IF_ERROR_RETURN(
                    soc_tomahawk3_itm_reg32_get(unit,
                        MMU_THDI_BUFFER_CELL_LIMIT_SPr, itm, itm, pool, &rval));
                limit = soc_reg_field_get(unit, MMU_THDI_BUFFER_CELL_LIMIT_SPr,
                                                     rval, LIMITf);
                thd_shared[itm] += limit;
            }
        }
        ...
        _soc_mmu_device_info_t *devcfg,  int lossless)
{
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        /* port_guarantee_enable = 0 implies use PGMin */
        /* port_guarantee_enable = 1 implies use PortSPMin */
        for (idx = 0; idx < _TH3_MMU_NUM_PG; idx++) {
            ...
            buf_prigroup->port_guarantee_enable = 0; /* Use PGMIN by default */
        ...
        int lossless)
{
    ...
    /* Qgroup memory programming based on port*/
        ...
        for (idx = 0; idx < _SOC_MMU_CFG_DEV_PG_NUM(devcfg); idx++) {
            ...
            for (itm = 0; itm < SOC_MMU_CFG_ITM_MAX; itm ++) {
                ...
                if (itm_map & (1 << pipe)) {
                    ...
                    if (buf_prigroup->port_guarantee_enable == 0) {
                        pg_min_itm[itm][buf_prigroup->pool_idx] +=
                                                     buf_prigroup->guarantee;
                    } else {
                        use_port_min = 1;
                    }
...
static int
_soc_th3_calc_sum_q_min_from_hw(int unit, int itm, _soc_mmu_cfg_buf_t *buf,
                                int *sum_q_min) {
    _soc_mmu_cfg_buf_pool_t *buf_pool;
    _soc_mmu_cfg_buf_queue_t *buf_queue;
    int idx, mem_idx, num_queues, start_q;
    int lport;
    soc_info_t *si = &SOC_INFO(unit);
    int hw_q_min, sw_q_min;
    soc_mem_t mem;
    int q_min_total = 0;
    uint32 pentry[SOC_MAX_MEM_WORDS];

    buf_pool = &buf->pools_itm[itm][0];
    /* Check for reservation based on initial config*/
    q_min_total += buf_pool->queue_guarantee;
    /* Calculate any differences compared to initial based on HW read */
    mem = MMU_THDO_QUEUE_CONFIGm;
    PBMP_ALL_ITER(unit, lport) {
        if (IS_CPU_PORT(unit, lport)) {
            num_queues = SOC_TH3_NUM_CPU_QUEUES;
            start_q = si->port_cosq_base[lport];
        } else {
            num_queues = SOC_TH3_NUM_GP_QUEUES;
            start_q = si->port_uc_cosq_base[lport];
        }
        for (idx = 0; idx < num_queues; idx++) {
            buf_queue = &buf->ports[lport].queues[idx];
            sw_q_min = buf_queue->guarantee;
            mem_idx = start_q + idx;
            sal_memset(pentry, 0, sizeof(pentry));
            SOC_IF_ERROR_RETURN
                (soc_mem_read(unit, mem, MEM_BLOCK_ANY, mem_idx, pentry));
            hw_q_min = soc_mem_field32_get(unit, mem, pentry, MIN_LIMITf);
            if (hw_q_min > sw_q_min) {
                q_min_total += (hw_q_min - sw_q_min);
            } else {
                q_min_total -= (sw_q_min - hw_q_min);
            }
        }
    }
    *sum_q_min = q_min_total;
    return SOC_E_NONE;
}

static int
_soc_th3_calc_sum_qgrp_min_from_hw(int unit, int itm, _soc_mmu_cfg_buf_t *buf,
                                   int *sum_qgrp_min) {
    _soc_mmu_cfg_buf_pool_t *buf_pool;
    _soc_mmu_cfg_buf_qgroup_t *queue_grp;
    int mem_idx;
    int lport;
    int hw_qgrp_min, sw_qgrp_min;
    soc_mem_t mem;
    int qgrp_min_total = 0;
    uint32 pentry[SOC_MAX_MEM_WORDS];

    buf_pool = &buf->pools_itm[itm][0];
    /* UC qgroup min */
    /* Check for reservation based on initial config*/
    qgrp_min_total += buf_pool->queue_group_guarantee;
    /* Calculate any differences compared to initial based on HW read */
    mem = MMU_THDO_CONFIG_UC_QGROUP0m;
    PBMP_ALL_ITER(unit, lport) {
        queue_grp = &buf->qgroups[lport];
        sw_qgrp_min = queue_grp->guarantee;
        mem_idx = SOC_TH3_MMU_CHIP_PORT(unit, lport);
        sal_memset(pentry, 0, sizeof(pentry));
        SOC_IF_ERROR_RETURN
            (soc_mem_read(unit, mem, MEM_BLOCK_ANY, mem_idx, pentry));
        hw_qgrp_min = soc_mem_field32_get(unit, mem, pentry, MIN_LIMITf);
        if (hw_qgrp_min > sw_qgrp_min) {
            qgrp_min_total += (hw_qgrp_min - sw_qgrp_min);
        } else {
            qgrp_min_total -= (sw_qgrp_min - hw_qgrp_min);
        }
    }
    /* MC qgroup min */
    /* Check for reservation based on initial config*/
    qgrp_min_total += buf_pool->mcq_entry_reserved;
    /* Calculate any differences compared to initial based on HW read */
    mem = MMU_THDO_CONFIG_MC_QGROUPm;
    PBMP_ALL_ITER(unit, lport) {
        queue_grp = &buf->qgroups[lport];
        sw_qgrp_min = queue_grp->guarantee_mc;
        mem_idx = SOC_TH3_MMU_CHIP_PORT(unit, lport);
        sal_memset(pentry, 0, sizeof(pentry));
        SOC_IF_ERROR_RETURN
            (soc_mem_read(unit, mem, MEM_BLOCK_ANY, mem_idx, pentry));
    ...
    *sum_qgrp_min = qgrp_min_total;
    return SOC_E_NONE;
}

static int
_soc_th3_calc_sum_pg_min_from_hw(int unit, int itm, _soc_mmu_cfg_buf_t *buf,
                                 int *sum_pg_min) {
    _soc_mmu_cfg_buf_pool_t *buf_pool;
    _soc_mmu_cfg_buf_prigroup_t *buf_prigroup;
    _soc_mmu_cfg_buf_port_t *buf_port;
    ...
    uint32 pentry[SOC_MAX_MEM_WORDS];
    soc_info_t *si = &SOC_INFO(unit);
    int pg, hw_pg_min, sw_pg_min;
    soc_mem_t mem;
    soc_field_t pgmin_fid[SOC_MMU_CFG_PRI_GROUP_MAX] = {
            PG0_MIN_LIMITf, PG1_MIN_LIMITf, PG2_MIN_LIMITf,
            PG3_MIN_LIMITf, PG4_MIN_LIMITf, PG5_MIN_LIMITf,
            PG6_MIN_LIMITf, PG7_MIN_LIMITf};
    int pg_min_total = 0;
    ...
    buf_pool = &buf->pools_itm[itm][0];
    /* Check for reservation based on initial config*/
    pg_min_total += buf_pool->prigroup_guarantee;
    /* Check for pg min differences in the ports belonging to this ITM */
    pipe_map = si->itm_ipipe_map[itm];
    /* Calculate any differences compared to initial based on HW read */
    PBMP_ALL_ITER(unit, lport) {
        pipe = si->port_pipe[lport];
        if (!(pipe_map & (1 << pipe))) {
            continue;
        }
        mport = si->port_l2i_mapping[lport];
        mem = SOC_MEM_UNIQUE_ACC(unit, MMU_THDI_PORT_PG_MIN_CONFIGm)[pipe];
        sal_memset(pentry, 0, sizeof(pentry));
        SOC_IF_ERROR_RETURN
            (soc_mem_read(unit, mem, MEM_BLOCK_ANY, mport, pentry));
        buf_port = &buf->ports[lport];
        for (pg = 0; pg < _TH3_MMU_NUM_PG; pg++) {
            buf_prigroup = &buf_port->prigroups[pg];
            sw_pg_min = buf_prigroup->guarantee;
            hw_pg_min = soc_mem_field32_get(unit, mem, pentry, pgmin_fid[pg]);
            if (hw_pg_min > sw_pg_min) {
                pg_min_total += (hw_pg_min - sw_pg_min);
            } else {
                pg_min_total -= (sw_pg_min - hw_pg_min);
            }
        }
    }
    *sum_pg_min = pg_min_total;
    return SOC_E_NONE;
}

int
soc_th3_mmu_config_reinit_thd_from_hw(int unit, int *shared_limit)
{
    int lossless = 1;
    _soc_mmu_cfg_buf_t *buf;
    _soc_mmu_device_info_t devcfg;
    int rsvd_buffer[_TH3_ITMS_PER_DEV] = {0};
    int itm, pool;
    uint32 itm_map;
    soc_info_t *si = &SOC_INFO(unit);
    soc_reg_t reg;
    soc_field_t field;
    uint32 rval;
    int sum_pg_min, sum_q_min, sum_qgrp_min;
    int rv = SOC_E_NONE;

    if (shared_limit == NULL) {
        return SOC_E_PARAM;
    }
    buf = soc_mmu_cfg_alloc(unit);
    if (!buf) {
    ...
    rv = _soc_th3_mmu_cfg_buf_calculate(unit, buf, &devcfg);
    if (rv < 0) {
        goto exit;
    }
    
    /* Read sum of hdrm pool and service pool from hardware */
    itm_map = si->itm_map;
    for (itm = 0; itm < _TH3_ITMS_PER_DEV; itm++) {
        if (itm_map & (1 << itm)) {
            ...
            for (pool = 0; pool < _TH3_MMU_NUM_POOL; pool++) {
                 rval = 0;
                 rv = soc_tomahawk3_itm_reg32_get(unit, reg, itm, itm, pool, &rval);
                if (rv < 0) {
                    goto exit;
                }
                 rsvd_buffer[itm] += soc_reg_field_get(unit, reg, rval, field);
            }
            /* Ingress portsp min changed throughg API doesnt result in delta
             * adjustment SDK-253511 */
            /* Retrieve from hardware any changes to PG min, UC/MC queue min and
             * queue group min during previous cold boot */
            rv = _soc_th3_calc_sum_pg_min_from_hw(unit, itm, buf, &sum_pg_min);
            if (rv < 0) {
                goto exit;
            }
            rv = _soc_th3_calc_sum_q_min_from_hw(unit, itm, buf, &sum_q_min);
            if (rv < 0) {
                goto exit;
            }
            rv = _soc_th3_calc_sum_qgrp_min_from_hw(unit, itm, buf, &sum_qgrp_min);
            if (rv < 0) {
                goto exit;
            }
            rsvd_buffer[itm] += sum_pg_min + sum_q_min + sum_qgrp_min;
            shared_limit[itm] = _SOC_MMU_CFG_MMU_TOTAL_CELLS(&devcfg) -
                                 rsvd_buffer[itm];
            LOG_VERBOSE(BSL_LS_SOC_MMU,
                        (BSL_META_U(unit, "threshold retrieve from hardware,"
                                      " itm : %d, sum_pg_min : %d, sum_q_min: %d,"
                                      " sum_qgrp_min: %d, rsvd_buffer[itm]: %d,"
                                      " shared_limit: %d \n"),
                                      itm, sum_pg_min, sum_q_min, sum_qgrp_min,
                                      rsvd_buffer[itm], shared_limit[itm]));
            LOG_CLI((BSL_META_U(unit, "Shared space retrieved from HW for itm:%d"
                                      " %d \n"), itm, shared_limit[itm]));
        }
    }
    LOG_VERBOSE(BSL_LS_SOC_MMU,
                (BSL_META_U(unit,
                            "MMU Threshold reinit from hardware done\n")));
...
/*
 * Check limit counter to confirm new count less than limit
 *
 * Parameter:
 *   reg/mem    - reg or mem
 *   field      - threshold field
 *   index      - reg/mem index
 *   inst       - reg/mem instance
 *   val        - threshold value
 *  Return error if value of counter > threshold val after time check.
 */
int
_soc_th3_mmu_config_check_limit_counter(int unit, soc_reg_t reg, soc_mem_t mem,
                                        soc_field_t field, int index, int inst,
                                        uint32 val)
{
    ...
    while (1) {
        if (reg != INVALIDr) {
            ...
            count_val = soc_reg_field_get(unit, reg, rval, field);
        } else if (mem != INVALIDm) {
            sal_memset(entry, 0, sizeof(entry));
            SOC_IF_ERROR_RETURN
                (soc_mem_read(unit, mem, inst, index, entry));

            count_val = soc_mem_field32_get(unit, mem, entry, field);
        }
        if (count_val <= val)  {
            /* Counter value is below current threshold */
            break;
        }
        sal_usleep(1 + (SAL_BOOT_QUICKTURN ? 1 : 0) * EMULATION_FACTOR);
        to_count++;
        if (to_count > 50000) {
            LOG_ERROR(BSL_LS_SOC_MMU, (BSL_META_U(unit,
                      "Time out waiting for count to fall below limit \n")));
        ...
        int itm, uint32 val, int decrease, int chk_zero)
{
    ...
    if (reg != INVALIDr) {
        rval = 0;
        SOC_IF_ERROR_RETURN
            (soc_tomahawk3_itm_reg32_get(unit, reg, itm, itm, index, &rval));

        cur_val = soc_reg_field_get(unit, reg, rval, field);
        if (((!chk_zero) || (chk_zero && (val != 0))) &&
            TH3_MMU_SHARED_LIMIT_CHK(cur_val, val, decrease)) {
            soc_reg_field_set(unit, reg, &rval, field, val);
            SOC_IF_ERROR_RETURN
                (soc_tomahawk3_itm_reg32_set(unit, reg, itm, itm, index, rval));
        }
    } else if (mem != INVALIDm) {
        sal_memset(entry, 0, sizeof(entry));
        SOC_IF_ERROR_RETURN
            (soc_tomahawk3_itm_mem_read(unit, mem, itm, itm,
        ...
        if (((!chk_zero) || (chk_zero && (cur_val != 0))) &&
            ...
            SOC_IF_ERROR_RETURN
                (soc_tomahawk3_itm_mem_write(unit, mem, itm, itm,
                    MEM_BLOCK_ALL, index, entry));
        }
    } else {
        return SOC_E_INTERNAL;
                                         ...
                                         int pool_update)
{
    ...
    soc_reg_t reg;
    soc_field_t field;
    int ing_limit[_TH3_ITMS_PER_DEV] = {0};
    int egr_limit[_TH3_ITMS_PER_DEV] = {0};
    int new_limit, itm;
    int resume_limit, pool_resume;
    uint32 rval;
    int ing_cur_val[_TH3_ITMS_PER_DEV] = {0};
    int egr_cur_val[_TH3_ITMS_PER_DEV] = {0};
    int rv = SOC_E_NONE;
    int decrease = 0;
    ...
    /* Read and check THDI: Per pool shared settings */
    for (itm = 0; itm < _TH3_ITMS_PER_DEV; itm ++) {
        if (si->itm_map & (1 << itm)) {
            ...
            rval = 0;
            SOC_IF_ERROR_RETURN
                (soc_tomahawk3_itm_reg32_get(unit, reg, itm, itm, pool, &rval));

            ing_cur_val[itm] = soc_reg_field_get(unit, reg, rval, field);
            ing_limit[itm] = ing_cur_val[itm] - delta[itm];
            if (ing_limit[itm] < 0) {
                return SOC_E_RESOURCE;
            }
        }
    ...
    /* Read and check THDO - Shared settings per Pool */
    for (itm = 0; itm < _TH3_ITMS_PER_DEV; itm ++) {
        if (si->itm_map & (1 << itm)) {
            ...
            SOC_IF_ERROR_RETURN
                (soc_tomahawk3_itm_reg32_get(unit, reg, itm, itm, pool, &rval));
            egr_cur_val[itm] = soc_reg_field_get(unit, reg, rval, field);
            egr_limit[itm] = egr_cur_val[itm] - delta[itm];
            if (egr_limit[itm] < 0) {
                return SOC_E_RESOURCE;
            }
        }
    ...
    for (itm = 0; itm < _TH3_ITMS_PER_DEV; itm ++) {
        if (si->itm_map & (1 << itm)) {
             /* Ing and Egr pool limits are maintained equal for any given ITM. */
            new_limit = MIN(ing_limit[itm], _TH3_MMU_TOTAL_CELLS_PER_ITM);
            if (new_limit < ing_cur_val[itm]) {
                decrease = 1;
            } else {
                decrease = 0;
            }
            ...
            if (decrease == 1) {
                /* Reduce resume limit only when shared limit is reduced */
                reg = MMU_THDO_SHARED_DB_POOL_RESUME_LIMITr;
                field = RESUME_LIMITf;
                SOC_IF_ERROR_RETURN
                    (_soc_th3_mmu_config_shared_limit_chk_set
                        (unit, reg, INVALIDm, field, pool, itm, resume_limit/8,
                         decrease, !pool_update));
            }

            if (decrease == 1 && pool_update == 0) {
                /* Check for use count only if shared limit needs decrease */
                /* Skip check in case of direct updates to pool*/
                rv = _soc_th3_mmu_config_check_limit_counter(unit,
                                                             MMU_THDI_POOL_SHARED_COUNT_SPr,
                                                             INVALIDm,
                                                             TOTAL_BUFFER_COUNTf,
                                                             pool, itm, new_limit);
                if (rv != 0) {
...
ing_restore:
    for (itm = 0; itm < NUM_ITM(unit); itm++) {
        ...
        soc_reg_field_set(unit, MMU_THDO_SHARED_DB_POOL_SHARED_LIMITr, &rval,
                          SHARED_LIMITf, egr_cur_val[itm]);
        SOC_IF_ERROR_RETURN
            (soc_tomahawk3_itm_reg32_set(unit,
                                         MMU_THDO_SHARED_DB_POOL_SHARED_LIMITr,
                                         itm, -1, pool, rval));
    }
    for (itm = 0; itm < NUM_ITM(unit); itm++) {
        /* Restore previous value */
