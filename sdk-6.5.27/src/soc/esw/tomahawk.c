...
{
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VFIm, 4095);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VFI_1m, 4095);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VFIm, 4095);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2MCm, 4095);
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_L3_NEXT_HOPm, 4095);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_L3_NEXT_HOPm, 4095);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, INITIAL_ING_L3_NEXT_HOPm, 4095);
...
#define _SOC_TH_FIXED_TBL_SZ      (8 * 1024)
    ...
    int defip_config = 0, mem_index_max = 0;
    ...
    switch(latency) {
        ...
        case SOC_SWITCH_BYPASS_MODE_LOW:
            ...
            SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_MPLSm, -1);
        ...
        case SOC_SWITCH_BYPASS_MODE_BALANCED:
            ...
            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_ECMPm, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ECMPm, mem_index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_ECMP_COUNTm, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ECMP_COUNTm, mem_index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, INITIAL_L3_ECMP_COUNTm, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, INITIAL_L3_ECMP_COUNTm, mem_index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, INITIAL_L3_ECMPm, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, INITIAL_L3_ECMPm, mem_index_max);
            break;
    ...
    if (SAL_BOOT_QUICKTURN) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MY_STATION_TCAMm, 7);
    }

    /* Adjust L2 table size */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2Xm, l2_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2_ENTRY_TILEm, l2_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2_ENTRY_ONLY_TILEm, l2_entries - 1);
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2_ENTRY_ISS_LPm,
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_ISS_LPm,
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_2_PIPE2m, fpem_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_2_PIPE3m, fpem_entries - 1);
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_2_ENTRY_ONLY_PIPE2m, fpem_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_2_ENTRY_ONLY_PIPE3m, fpem_entries - 1);
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_4_PIPE2m, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_4_PIPE3m, fpem_entries / 2 - 1);
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_4_ENTRY_ONLY_PIPE2m, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_4_ENTRY_ONLY_PIPE3m, fpem_entries / 2 - 1);

    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_HIT_ONLYm, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, FPEM_LPm, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, FPEM_ECCm, fpem_entries * 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, FPEM_ECC_PIPE0m, fpem_entries * 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, FPEM_ECC_PIPE1m, fpem_entries * 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, FPEM_ECC_PIPE2m, fpem_entries * 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, FPEM_ECC_PIPE3m, fpem_entries * 2 - 1);
    ...
    if (SOC_SWITCH_BYPASS_MODE_LOW == latency) {
        ...
        if (soc_property_get(unit, "l3_defip_sizing", TRUE)) {
            if (!(soc_feature(unit, soc_feature_alpm) &&
                ...
                mem_index_max = (SOC_CONTROL(unit)->l3_defip_max_tcams *
                                 SOC_CONTROL(unit)->l3_defip_tcam_size) -
                                 (num_ipv6_128b_entries * 2) - 1;
                SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIPm, mem_index_max);

                if (soc_feature(unit, soc_feature_l3_1k_defip_table)) {
                    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIPm, 1023);
                    mem_index_max = 1023;
                }
                SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ONLYm, mem_index_max);
                SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_DATA_ONLYm, mem_index_max);
                SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_HIT_ONLYm, mem_index_max);
    ...
    if ((SOC_SWITCH_BYPASS_MODE_NONE == latency) &&
        ...
        if ((shared_l2_banks + shared_l3_banks + shared_fpem_banks > 0)
            ...
            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_RAWm, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_RAWm, mem_index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV4m, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV4m, mem_index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV4_1m, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV4_1m, mem_index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_64m, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_64m, mem_index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_64_1m, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_64_1m, mem_index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_128m, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_128m, mem_index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_HIT_ONLYm, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_HIT_ONLYm, mem_index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_ECCm, mem_index_max);
            mem_index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_ECCm, mem_index_max);
        }
    } else {
    ...
    int cnt, uint32 seed)
{
    int i,j;
    int t = 256;
    soc_th_rtag7_flow_hash_entry_t tmp;
    if (0 == seed) {
        seed = sal_time();
    }
    sal_srand(seed);
    while (t--) {
        i = sal_rand() % cnt;
        j = sal_rand() % cnt;
        sal_memcpy(&tmp, &entry[i], sizeof(soc_th_rtag7_flow_hash_entry_t));
        sal_memcpy(&entry[i], &entry[j], sizeof(soc_th_rtag7_flow_hash_entry_t));
        sal_memcpy(&entry[j], &tmp, sizeof(soc_th_rtag7_flow_hash_entry_t));
    }
    return;
}
    ...
    int *entry_cur_cnt)
{
    ...
    uint8 inserted = 0;
    uint8 skipped = 0;
    int size = sizeof(soc_th_rtag7_flow_hash_entry_t);

    if (*entry_cur_cnt == 0) {
        sal_memcpy(&entry[0], entry_insert, size);
        (*entry_cur_cnt)++;
        return;
    }
    for (i = 0; i < *entry_cur_cnt; i++) {
        if (!sal_memcmp(&entry[i], entry_insert, size)) {
            skipped = 1;
            break;
        }
        if (entry_insert->sub_sel < entry[i].sub_sel) {
            sal_memmove(entry + i + 1, entry + i, (*entry_cur_cnt - i) * size);
            sal_memcpy(entry + i, entry_insert, size);
            (*entry_cur_cnt)++;
            inserted = 1;
            break;
        } else if (entry_insert->sub_sel == entry[i].sub_sel) {
            if (entry_insert->offset < entry[i].offset) {
                if (((*entry_cur_cnt) - i) > 0) {
                   sal_memmove(entry + i + 1, entry + i, (*entry_cur_cnt - i) * size);
                }
                sal_memcpy(entry + i, entry_insert, size);
                (*entry_cur_cnt)++;
                inserted = 1;
                break;
            } else if (entry_insert->offset == entry[i].offset) {
                skipped = 1;
                break;
            } else {
                if (entry[i + 1].sub_sel != entry_insert->sub_sel) {
                    if (((*entry_cur_cnt) - i - 1) > 0) {
                        sal_memmove(entry + i + 2, entry + i + 1, (*entry_cur_cnt - i - 1) * size);
                    }
                    sal_memcpy(entry + i + 1, entry_insert, size);
                    (*entry_cur_cnt)++;
                    inserted = 1;
                    break;
                } else {
    ...
    if (!inserted && !skipped) {
        sal_memcpy(entry + i, entry_insert, sizeof(soc_th_rtag7_flow_hash_entry_t));
        (*entry_cur_cnt)++;
    }
    return;
}
...
#pragma GCC diagnostic pop
    ...
    if (!SOC_WARM_BOOT(unit) && soc_mem_is_valid(unit, RTAG7_FLOW_BASED_HASHm)) {
...
soc_tomahawk_voltage_monitor_get(int unit, int* avg_voltage)
{
    ...
    int index = 0, avg = 0;
#ifdef BCM_TOMAHAWK2_SUPPORT
    uint32 old_rval = 0, old_rval_mask = 0;
#endif /* BCM_TOMAHAWK2_SUPPORT */
    soc_reg_t reg;
    soc_reg_t pvtmon_reg[] = {TOP_PVTMON_RESULT_5r,
...
#ifdef BCM_TOMAHAWK2_SUPPORT
    if (SOC_IS_TOMAHAWK2(unit)) {
        /* Setting TOP_PVTMON_CTRL_1.PVTMON0-7_HIGHTEMP_CTRL_ EN to 0 */
        if (soc_reg_field_get(unit, TOP_PVTMON_CTRL_1r, rval,
                              PVTMON_HIGHTEMP_STAT_CLEARf)) {
            LOG_CLI((BSL_META_U(unit,
                "HW PVTMON High Temperature Protection triggered!\n")));
        }
        old_rval = rval;
        rval &= 0xfffffe00;
        SOC_IF_ERROR_RETURN(WRITE_TOP_PVTMON_CTRL_1r(unit, rval));
        ...
        /* Clear PVTMON intr mask */
        SOC_IF_ERROR_RETURN(READ_TOP_PVTMON_INTR_MASKr(unit, &old_rval_mask));
        SOC_IF_ERROR_RETURN(WRITE_TOP_PVTMON_INTR_MASKr(unit, 0));
    }
#endif /* BCM_TOMAHAWK2_SUPPORT */
    ...
    /* Read Voltages. */
    for (index = 0; index < COUNTOF(pvtmon_reg); index++) {
        reg = pvtmon_reg[index];
        SOC_IF_ERROR_RETURN(soc_reg32_get(unit, reg, REG_PORT_ANY, 0, &rval));
        fval = soc_reg_field_get(unit, reg, rval, PVT_DATAf);
#ifdef BCM_TOMAHAWK2_SUPPORT
        if (SOC_IS_TOMAHAWK2(unit)) {
...
#ifdef BCM_TOMAHAWK2_SUPPORT
    if (SOC_IS_TOMAHAWK2(unit)) {
        /* Setting TOP_PVTMON_CTRL_1.PVTMON0-7_HIGHTEMP_CTRL_ EN back */
        SOC_IF_ERROR_RETURN(READ_TOP_PVTMON_CTRL_1r(unit, &rval));
        if (soc_reg_field_get(unit, TOP_PVTMON_CTRL_1r, rval,
                              PVTMON_HIGHTEMP_STAT_CLEARf)) {
            LOG_CLI((BSL_META_U(unit,
        ...
        /* Setting PVTMON intr mask back */
        SOC_IF_ERROR_RETURN(WRITE_TOP_PVTMON_INTR_MASKr(unit, old_rval_mask));
        sal_sleep(1);
    }
#endif /* BCM_TOMAHAWK2_SUPPORT */
