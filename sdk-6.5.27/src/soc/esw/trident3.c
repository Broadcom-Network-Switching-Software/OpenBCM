...
static ing_dnat_address_type_hash_control_entry_t  ing_dnat_hash_control;
...
static subport_id_to_sgpp_map_hash_control_entry_t subport_hash_control;
...
static ip_uat_16k_shared_banks_control_entry_t     ip_uat_16k_shared_banks_control;
static ip_uat_8k_shared_banks_control_entry_t      ip_uat_8k_shared_banks_control;
...
/*
 * Function:
 *      soc_trident3_voltage_monitor_get
 * Purpose:
 *      Readout trident3 core voltage from silicon ADC.
 */
int
soc_trident3_voltage_monitor_get(int unit, int* avg_voltage)
{
    uint32 rval, fval;
    int index = 0, avg = 0;
    soc_reg_t reg;
    ...
    if (SOC_IS_TRIDENT3(unit)) {
        /* Clear max and min history data */
        SOC_IF_ERROR_RETURN(READ_TOP_SOFT_RESET_REG_2r(unit, &rval));
        soc_reg_field_set(unit, TOP_SOFT_RESET_REG_2r, &rval, TOP_PVT_MON_MAX_RST_Lf, 0);
        soc_reg_field_set(unit, TOP_SOFT_RESET_REG_2r, &rval, TOP_PVT_MON_MIN_RST_Lf, 0);
        SOC_IF_ERROR_RETURN(WRITE_TOP_SOFT_RESET_REG_2r(unit, rval));

        /* Select to read out core voltage for sensor 0 - 10 */
        SOC_IF_ERROR_RETURN(READ_TOP_PVTMON_CTRL_2r(unit, &rval));
        ...
        soc_reg_field_set(unit, TOP_PVTMON_CTRL_2r, &rval, PVTMON_SELECT6f, 1);
        soc_reg_field_set(unit, TOP_PVTMON_CTRL_2r, &rval, PVTMON_SELECT7f, 1);
        soc_reg_field_set(unit, TOP_PVTMON_CTRL_2r, &rval, PVTMON_SELECT8f, 1);
        soc_reg_field_set(unit, TOP_PVTMON_CTRL_2r, &rval, PVTMON_SELECT9f, 1);
        soc_reg_field_set(unit, TOP_PVTMON_CTRL_2r, &rval, PVTMON_SELECT10f, 1);
        ...
        /* Enable max and min history readout */
        SOC_IF_ERROR_RETURN(READ_TOP_SOFT_RESET_REG_2r(unit, &rval));
        soc_reg_field_set(unit, TOP_SOFT_RESET_REG_2r, &rval, TOP_PVT_MON_MAX_RST_Lf, 1);
        soc_reg_field_set(unit, TOP_SOFT_RESET_REG_2r, &rval, TOP_PVT_MON_MIN_RST_Lf, 1);
        SOC_IF_ERROR_RETURN(WRITE_TOP_SOFT_RESET_REG_2r(unit, rval));
    }

    sal_usleep(1000);

    /* Read Voltages from sensor 0 - 10, skip last one per spec. */
    for (index = 0; index < (COUNTOF(pvtmon_result_reg) - 1); index++) {
        ...
        /* Conversion equation: V = (ADC[9:0]/1024)* 0.95 */
        fval = (fval * 950) / 1024;
    ...
    *avg_voltage = avg;
    ...
    /* Setting back the VTMON to default temp mode */
    if (SOC_IS_TRIDENT3(unit)) {
        /* Clear max and min history data */
        SOC_IF_ERROR_RETURN(READ_TOP_SOFT_RESET_REG_2r(unit, &rval));
        soc_reg_field_set(unit, TOP_SOFT_RESET_REG_2r, &rval, TOP_PVT_MON_MAX_RST_Lf, 0);
        ...
        /* Set back to temperature readout mode */
        SOC_IF_ERROR_RETURN(READ_TOP_PVTMON_CTRL_2r(unit, &rval));
        ...
        soc_reg_field_set(unit, TOP_PVTMON_CTRL_2r, &rval, PVTMON_SELECT8f, 0);
        soc_reg_field_set(unit, TOP_PVTMON_CTRL_2r, &rval, PVTMON_SELECT9f, 0);
        soc_reg_field_set(unit, TOP_PVTMON_CTRL_2r, &rval, PVTMON_SELECT10f, 0);
        SOC_IF_ERROR_RETURN(WRITE_TOP_PVTMON_CTRL_2r(unit, rval));
...
#define _SOC_TD3_DEFIP_TCAM_DEPTH   2048
    ...
    index_max = (2 * 4 * 1024) + (shared_l2_banks * 14 * 1024) - 1;
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2_ENTRY_LPm, index_max);
    ...
    index_max = (7 * 1024) + (shared_l3_banks * 14 * 1024) - 1;
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_LPm, index_max);

    /* Adjust FP exact match table size */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_2m, fpem_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_2_PIPE0m, fpem_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_2_PIPE1m, fpem_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_2_ENTRY_ONLYm, fpem_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_2_ENTRY_ONLY_PIPE0m, fpem_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_2_ENTRY_ONLY_PIPE1m, fpem_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_4m, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_4_PIPE0m, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_4_PIPE1m, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_4_ENTRY_ONLYm, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_4_ENTRY_ONLY_PIPE0m, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_4_ENTRY_ONLY_PIPE1m, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_HIT_ONLYm, fpem_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_ECCm, fpem_entries * 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_ECC_PIPE0m, fpem_entries * 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_ECC_PIPE1m, fpem_entries * 2 - 1);
    /* 14k per shared FPEM bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_LPm, shared_fpem_banks * 14 * 1024 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_LP_PIPE0m, shared_fpem_banks * 14 * 1024 -1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_LP_PIPE1m, shared_fpem_banks * 14 * 1024 -1);
    ...
    index_max = (mpls_8k_banks * 3 * 1024 + mpls_16k_banks * 2 * 3 * 1024) - 1;
    SOP_MEM_STATE_MAX_INDEX_SET(unit, MPLS_ENTRY_LPm, index_max);
    ...
    /* 3k per 8k bank and 6k per 8k bank */
    index_max = ((vlan_xlate_1_16k_banks * 2 * 3 * 1024)
                  + (vlan_xlate_1_8k_banks * 3 * 1024)) - 1;
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_1_LPm, index_max);
    ...
    /* 3k per 8k bank and 6k per 8k bank */
    index_max = (vlan_xlate_2_16k_banks * 2 * 3 * 1024)
                 + (vlan_xlate_2_8k_banks * 3 * 1024) - 1;
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_2_LPm, index_max);
    ...
    /* 3k per bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VLAN_XLATE_1_LPm, egr_vlan_xlate_1_banks * 3 * 1024 - 1);
    ...
    /* 3k per bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VLAN_XLATE_2_LPm, egr_vlan_xlate_2_banks * 3 * 1024 - 1);
    ...
    if (alpm_enable) {
        ...
        if (shared_l2_banks + shared_l3_banks + shared_fpem_banks > 0) {
            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_RAWm, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_RAWm, index_max / 2);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV4m, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV4m, index_max / 2);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV4_1m, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV4_1m, index_max / 2);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_64m, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_64m, index_max / 2);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_64_1m, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_64_1m, index_max / 2);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_128m, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_128m, index_max / 2);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_HIT_ONLYm, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_HIT_ONLYm, index_max / 2);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_ECCm, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_ECCm, index_max / 2);
        }
    } else {
    ...
    */
    ...
    switch (num_lag_rh_flowset_entries) {
        case 0:
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_LAG_FLOWSETm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_LAG_FLOWSET_PIPE0m, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_LAG_FLOWSET_PIPE1m, -1);
            break;
        case 65536:
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_HGT_FLOWSETm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_HGT_FLOWSET_PIPE0m, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_HGT_FLOWSET_PIPE1m, -1);
            break;
        case 32768:
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_HGT_FLOWSETm, 32767);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_HGT_FLOWSET_PIPE0m, 32767);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_HGT_FLOWSET_PIPE1m, 32767);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_LAG_FLOWSETm, 32767);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_LAG_FLOWSET_PIPE0m, 32767);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_LAG_FLOWSET_PIPE1m, 32767);
            break;
        default:
            return SOC_E_CONFIG;
    }
...
soc_td3_set_idb_dpp_ctrl(int unit)
{
    ...
    do {
        ...
        if (in_progress && soc_timeout_check(&to)) {
...
int
soc_trident3_mmu_pfc_tx_config_set(int unit, soc_port_t port, int pfc_enable)
{
    ...
    /* IDB OBM Admission Thresholds controls are not applicable
...
soc_trident3_uft_uat_config(int unit) {
    ...
    for (index = dedicated_banks; index < num_l2_banks; index++) {
        ...
        soc_mem_field32_set(unit, UFT_SHARED_BANKS_CONTROLm,
                    shared_hash_control_entry,
                    shared_fields[index + shared_bank_offset - (2 * dedicated_banks)],
                    shared_hash_offset);
    }
...
int
soc_trident3_egr_port_min_pkt_size_set(int unit, int port, uint8 min)
{
    uint32 rval;
    ...
    SOC_IF_ERROR_RETURN(READ_EGR_PORT_MIN_PKT_SIZEr(unit, port, &rval));
    soc_reg_field_set(unit, EGR_PORT_MIN_PKT_SIZEr, &rval, MIN_PKT_SIZEf, min);
    SOC_IF_ERROR_RETURN(WRITE_EGR_PORT_MIN_PKT_SIZEr(unit, port, rval));

    return SOC_E_NONE;
}

int
soc_trident3_egr_ports_min_pkt_size_set(int unit)
{
    int port;

    PBMP_ALL_ITER(unit, port) {
        /* EGR_PORT_MIN_PKT_SIZE needs to be 76 for hg, cpu, and lb ports */
        if (IS_HG_PORT(unit, port) || IS_CPU_PORT(unit, port)
            || IS_LB_PORT(unit, port)) {
            SOC_IF_ERROR_RETURN(
                    soc_trident3_egr_port_min_pkt_size_set(unit, port, 76));
        }
    }
...
soc_trident3_mdio_rate_divisor_set(int unit)
{
    ...
    ext_divisor = soc_property_get(unit, spn_RATE_EXT_MDIO_DIVISOR,
                                   RATE_EXT_MDIO_DIVISOR_DEF(unit));
...
#pragma GCC diagnostic pop
    ...
    if(soc_feature(unit, soc_feature_higig3)) {
        /* In legacy devices, the MGID field was introduced to optimize the
         * usage of L2MC and IPMC tables. With HIGIG3, TD3 is not supporting
         * interop with any legacy devices. Hence set bcast, mcast and ipmc size
         * as 0 for HIGIG3.
         */
        soc_hbx_higig2_mcast_sizes_set(unit, 0, 0, 0);
    } else {
    SOC_IF_ERROR_RETURN
                              ...
                              void *d3, void *d4)
{
    ...
    for (i = 0; i < num_entries_out; i++) {
        if (i & soc_td3_temp_mon_mask[unit]) {
            ...
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit,
                                  "HighTemp (status: 0x%x) Sensor %d: %d deg C.\n"), rval, i, trval));
...
void soc_td3_hg3_feature_update(int unit) {

    if (!soc_feature(unit, soc_feature_higig3)) {
        return;
    }

    /* if HG3 is enabled, disable the following features */
    SOC_FEATURE_CLEAR(unit, soc_feature_niv);
    SOC_FEATURE_CLEAR(unit, soc_feature_l2gre);
    SOC_FEATURE_CLEAR(unit, soc_feature_fcoe);
    SOC_FEATURE_CLEAR(unit, soc_feature_snap);
    SOC_FEATURE_CLEAR(unit, soc_feature_qcn);
    SOC_FEATURE_CLEAR(unit, soc_feature_mim);
    SOC_FEATURE_CLEAR(unit, soc_feature_CnTag);
    SOC_FEATURE_CLEAR(unit, soc_feature_port_extension);
    SOC_FEATURE_CLEAR(unit, soc_feature_port_extension_kt2_key_type);
    SOC_FEATURE_CLEAR(unit, soc_feature_port_extender_vp_sharing);
...
#endif
    /* Using wide key(single-wide) in Pair-TCAM mode will cause improper behavior
     * when uRPF is turned on and pair/unpair boundary (128b_entries) is not in
     * the middle since key selection on TCAM block will be like:
     *     TCAM0/1: DST-128
     *     TCAM2/3: DST-64
     *     TCAM4/5: SRC-128
     *     TCAM6/7: SRC-64
     * and some slots in TCAM2/3(DST-64) may be used for installing DST-128 entries.
     */
    if (soc_property_get(unit, spn_DEFIP_WIDE_KEY_FOR_IPMC_DISABLE, 0)) {
        if (soc_feature(unit, soc_feature_separate_key_for_ipmc_route)) {
            soc_reg_field_set(unit, ING_FLEX_CONFIGr, &rval,
                              USE_DEFIP_FOR_IPMCf, 0);
            SOC_FEATURE_CLEAR(unit, soc_feature_separate_key_for_ipmc_route);
            SOC_FEATURE_SET(unit, soc_feature_td3_lpm_ipmc_war);
        }
    }
