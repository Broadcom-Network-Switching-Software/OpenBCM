...
soc_firebolt6_bond_info_init(int unit)
{
    ...
    /* EFP_SLICE  depth
     * FB6 EFP has 4 slice of 3k each, total TCAM size 12K
     */
    SOC_BOND_OPTIONS_COPY(val, uint32, bond_info_regval, 425, 425);
    if (val == 0) {
        SOC_BOND_INFO_FEATURE_SET(unit, socBondInfoFeatureEfp3kHalfSlice);
    }
