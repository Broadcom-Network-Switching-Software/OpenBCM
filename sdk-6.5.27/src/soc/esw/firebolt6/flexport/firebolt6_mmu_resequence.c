 ...
 *
 *  File:        firebolt6_mmu_port_resequence.c
 *  Purpose:
 *  Requires:
 */
...
/*! @fn int soc_firebolt6_mmu_set_mmu_to_phy_port_mapping(int unit,
        ...
        int unit,soc_port_resource_t *port_resource_t, soc_port_schedule_state_t *port_schedule_state)
{
    ...
    int mmu_port;
    uint32 phy_port_new, logical_port_new;
    soc_reg_t reg;
    uint64 rval;
    int inst;
    ...
    if (port_resource_t->physical_port == -1) {
        phy_port_new = FB6_INVALID_PHY_PORT_NUMBER; /* All 1's */
        logical_port_new = FB6_INVALID_PHY_PORT_NUMBER;
    }
    else {
        phy_port_new= port_resource_t->physical_port;
        logical_port_new = port_resource_t->logical_port;
    }

            LOG_DEBUG(BSL_LS_SOC_PORT,
                      (BSL_META_U(unit,
                             "MMU to Phy Port Mapping phy_port_new=%1d  mmu_port=%1d\n"),
                      phy_port_new, mmu_port));
    ...
    COMPILER_64_ZERO(rval);
    soc_reg64_field32_set(unit, reg, &rval, DEVICE_PORTf, logical_port_new);
    /* BT:IPORT, AT: SINGLE, Block ID: MMU_GLOBAL */
    SOC_IF_ERROR_RETURN(soc_reg_rawport_set(unit, reg, inst, 0, rval));
...
/*! @fn int soc_firebolt6_mmu_vbs_port_flush(int unit,
 *              soc_port_schedule_state_t *port_schedule_state_t, int pipe, int up_dwn)
 *  @param unit Device number
 ...
 */
int
soc_firebolt6_mmu_vbs_port_flush(int unit,soc_port_schedule_state_t *port_schedule_state_t,
...
/*      if (port_schedule_state_t->resource[port].physical_port == -1) { */
	    if (port_schedule_state_t->resource[port].pipe == pipe) {
	      ...
	      lcl_mmu_port = mmu_port % FB6_MMU_PORT_PIPE_OFFSET;
	        COMPILER_64_ADD_32(temp64, FB6_MMU_FLUSH_ON);
...
/*! @fn int soc_firebolt6_mmu_rqe_port_flush(int unit,
 ...
 */
int
soc_firebolt6_mmu_rqe_port_flush(int unit,int pipe,
        uint64 set_val)
{
    ...
    while (1) {
        ...
        if (count > 30000) {
            LOG_ERROR(BSL_LS_SOC_COMMON,
                    (BSL_META_U(unit,
                                "Initiate isn't reset even after 30000 "
                                "iterations\n")));
            return SOC_E_FAIL;
        }
    }
...
/*! @fn int soc_firebolt6_mmu_mtro_port_flush(int unit,
 *              soc_port_resource_t *port_resource_t, int set_val)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 *  @param set_val Value to be set to the snapshot register
 ...
 */
int
soc_firebolt6_mmu_mtro_port_flush(int unit,soc_port_resource_t *port_resource_t,
        uint64 set_val)
{
...
/*Not suppported in FB6*/
/*! @fn int soc_firebolt6_mmu_disable_ct(int unit,
 *              soc_port_resource_t *port_resource_t)
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 ...
 */
/*int
  soc_firebolt6_mmu_disable_ct(int                  unit,
  soc_port_resource_t  *port_resource_t)
  {
