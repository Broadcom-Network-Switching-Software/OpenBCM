        ...
        int unit, soc_port_schedule_state_t *port_schedule_state_t)
{
    ...
    /* Derived from maximum value of SCH_PORT_L0_MAPPING_CONFIG_RAM.NUM_LOS
     * and multiplied by 8, as in soc_firebolt6_mmu_wred_clr for Per-Q loop,
     * then add FB6_MMU_NUM_SPS for the Per-SP loop*/
    int queues_sps_per_port =  (FB6_SUBPORT_COE_PORT_PER_CASPORT_MAX*8 +
                                                FB6_MMU_NUM_SPS);
    ...
    for (port=0; port<port_schedule_state_t->nport; port++) {
        if (port_schedule_state_t->resource[port].physical_port != -1) {
            ...
            if (port_schedule_state_t->resource[port].op !=
                ...
                port_schedule_state_t->resource[port].op !=
                    (SOC_PORT_RESOURCE_OP_SPEED |
