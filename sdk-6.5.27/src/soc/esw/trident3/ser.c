...
static int _soc_td3_ser_single_bit_report_for_bus_buff_skip(int uint, char *bb_name);
    ...
    for (pcount = 0;;pcount++) { /* walk thro each entry of block_info */
        ...
        case _SOC_SER_TYPE_MEM:
            if (mem_info[pcount].mem == INVALIDm) {
                ...
                if (!soc_td3_ser_single_bit_sw_correction_skip(unit, mem_info[pcount].mem)) {
...
_soc_utt_reverse_xlate(int unit, soc_mem_t mem, soc_field_t field, int *index)
{
    ...
    for (i = 0; i <= soc_mem_index_max(unit, mem); i++) {
        (void)soc_mem_read(unit, mem, MEM_BLOCK_ANY, i, entry);
...
int soc_td3_ser_single_bit_report_for_bus_buf_set(int unit, int enable)
{
    ...
    for (k = 0; k < BUS_BUF_SER_INFO_COUNT; k++) {
        ...
        for (i = 0; bus_ser_info[i].en_reg != INVALIDr; i++) {
            if (_soc_td3_ser_single_bit_report_for_bus_buff_skip(unit,
                                                                 bus_ser_info[i].bus_name)){
                continue;
            }
...
const soc_mem_t td3_sw_correction_skip_mem[] = {
    INVALIDm
};

int
soc_td3_ser_single_bit_sw_correction_skip(int unit, soc_mem_t mem)
{
    int i = 0;

    for (i = 0; i < COUNTOF(td3_sw_correction_skip_mem); i++) {
        if (mem == td3_sw_correction_skip_mem[i]) {
            return TRUE;
        }
    }
...
#define TD3_BB_SKIP_SIZE 2
char *td3_bus_buf_skip[TD3_BB_SKIP_SIZE] = {
    "IS_TDM_CALENDAR0",
    "IS_TDM_CALENDAR1"
};

static int
_soc_td3_ser_single_bit_report_for_bus_buff_skip(int uint, char *bb_name)
{
    int i = 0;

    for (i = 0; i < TD3_BB_SKIP_SIZE; i++) {
        if (sal_strcmp(bb_name, td3_bus_buf_skip[i]) == 0) {
            return TRUE;
        }
    }
