...
**/
STATIC int
soc_trident3_set_tdm_soc_pkg(int unit, soc_port_schedule_state_t
			*port_schedule_state, tdm_soc_t *soc_pkg, int new_or_prev)
{
    int port, phy_port, lane, index, pm_num, subp_phy_port;
    int mgmt_pm_hg = 0, mgmt_port_cnt = 0, mgmt_mode = 0;
    uint32 speed[TRIDENT3_TDM_PHY_PORTS_PER_DEV];
    uint32 port_state[TRIDENT3_TDM_PHY_PORTS_PER_DEV]; /* enum port_state_e */
    ...
    soc_pkg->unit = unit;
    soc_pkg->num_ext_ports = TD3_NUM_EXT_PORTS;
    soc_pkg->clk_freq = port_schedule_state->frequency;
    soc_pkg->pmap_num_modules       = 0;
    soc_pkg->pmap_num_lanes         = 0;
    soc_pkg->pm_num_phy_modules     = 0;
    soc_pkg->lr_idx_limit           = 0;
    soc_pkg->tvec_size              = 0;
    ...
    soc_pkg->soc_vars.td3.mgmt_pm_hg = mgmt_pm_hg;
	soc_pkg->soc_vars.td3.mgmt_mode = (mgmt_port_cnt > 1) ? (mgmt_mode + 0)
													: (mgmt_mode + 1);

    /* Set speed and state arrays */
    sal_memset(speed,      0, TRIDENT3_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));
    sal_memset(port_state, 0, TRIDENT3_TDM_PHY_PORTS_PER_DEV*sizeof(uint32));

    for (port = 0; port < TRIDENT3_DEV_PORTS_PER_DEV; port++) {
        if (port_map->log_port_speed[port] > 0 ) {
            ...
            if (phy_port < TRIDENT3_TDM_PHY_PORTS_PER_DEV) {
                ...
                soc_td3_tdm_get_log_port_speed(unit, port_schedule_state, port,
                    prev_or_new, &l_speed, &l_num_lanes);
                for (lane = 1; lane < l_num_lanes; lane++) {
                    subp_phy_port = phy_port + lane;
                    if (subp_phy_port < TRIDENT3_TDM_PHY_PORTS_PER_DEV) {
    ...
    if (1 == port_schedule_state->is_flexport) {
        ...
        for (pm_num = 0; pm_num < TD3_NUM_PHY_PM; pm_num++) { /* TRIDENT3_PBLKS_PER_PIPE * TRIDENT3_PIPES_PER_DEV */
            soc_pkg->soc_vars.td3.pm_num_to_pblk[pm_num] =
                  port_schedule_state->in_port_map.port_p2PBLK_inst_mapping[
									(pm_num * TRIDENT3_PORTS_PER_PBLK) + 1];
        }
    }
    ...
    for (index = 0; index < TD3_NUM_EXT_PORTS; index++) {
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
...
/**
@name: soc_trident3_tdm_copy_prev_tables
@param: int unit, soc_port_schedule_state_t *port_schedule_state
        tdm_soc_t *soc_pkg, int new_or_prev
 * Description:
 *      Helper function to allocate & populate state and speed in a tdm_soc_t structure
...
**/
STATIC int
soc_trident3_tdm_copy_prev_tables(int unit,
			soc_port_schedule_state_t *port_schedule_state, tdm_mod_t *_tdm_pkg)
{
    ...
    sal_memcpy(_tdm_pkg->_prev_chip_data.cal_0.cal_main,
               port_schedule_state->tdm_ingress_schedule_pipe[
				...
				sizeof(int)*TRIDENT3_TDM_LENGTH);
    ...
    /* OVS tables */
    for (grp_num = 0; grp_num < TRIDENT3_OVS_GROUP_COUNT_PER_PIPE; grp_num ++) {
        hpipe = grp_num / TRIDENT3_OVS_GROUP_COUNT_PER_HPIPE;
        hpipe_grp = grp_num % TRIDENT3_OVS_GROUP_COUNT_PER_HPIPE;
        sal_memcpy(_tdm_pkg->_prev_chip_data.cal_0.cal_grp[grp_num],
                   port_schedule_state->tdm_ingress_schedule_pipe[
					0].tdm_schedule_slice[hpipe].oversub_schedule[hpipe_grp],
                   sizeof(int) * TRIDENT3_OVS_GROUP_TDM_LENGTH);
    ...
    for (hpipe = 0; hpipe < 2; hpipe ++) {
        sal_memcpy(_tdm_pkg->_prev_chip_data.cal_0.cal_grp[
							TD3_SHAPING_GRP_IDX_0 + hpipe],
                   port_schedule_state->tdm_ingress_schedule_pipe[
					0].tdm_schedule_slice[hpipe].pkt_sch_or_ovs_space[0],
                   sizeof(int) * TD3_SHAPING_GRP_LEN);
...
/**
@name: soc_trident3_tdm_set_out_map
@param: int unit, soc_port_schedule_state_t *port_schedule_state
...
**/
STATIC int
soc_trident3_tdm_set_out_map(int unit,
				soc_port_schedule_state_t *port_schedule_state)
{
    ...
    sal_memcpy(port_schedule_state->out_port_map.port_l2p_mapping,
               ...
               sizeof(int)*TRIDENT3_DEV_PORTS_PER_DEV);
    ...
    sal_memcpy(port_schedule_state->out_port_map.port_m2p_mapping,
               ...
               sizeof(int)*TRIDENT3_MMU_PORTS_PER_DEV);
    ...
    if (1 == port_schedule_state->is_flexport) {
        ...
        for (i = 0; i < port_schedule_state->nport; i++) {
            ...
            if (-1 != physical_port) { /* that is, port up */
                ...
                if (TRIDENT3_PHY_PORT_MNG0 == physical_port) {
    ...
    soc_trident3_port_schedule_speed_remap(unit, port_schedule_state);
...
/**
@name: soc_trident3_tdm_flexport_ovs_add_rem_port
...
**/
int
soc_trident3_tdm_flexport_ovs_add_rem_port(int unit,
	soc_port_schedule_state_t *port_schedule_state,
    int logical_port,
    int rem_add_cmd,
    int set_idb,
    int set_mmu )
{
    ...
    static const soc_reg_t idb_grp_cfg_regs[TRIDENT3_PIPES_PER_DEV][2] = {
    ...
    static const soc_reg_t idb_grp_ovs_regs[TRIDENT3_PIPES_PER_DEV][2][6] = {
        {
    ...
    static const soc_reg_t idb_pblk_cal_regs[TRIDENT3_PIPES_PER_DEV][2][8] = {
        {
    ...
    static const soc_mem_t idb_pkt_shaper_mems[TRIDENT3_PIPES_PER_DEV][2] = {
        {PKT_SCH_CALENDAR0_PIPE0m, PKT_SCH_CALENDAR1_PIPE0m},
        {PKT_SCH_CALENDAR0_PIPE1m, PKT_SCH_CALENDAR1_PIPE1m},
    };

    cookie = port_schedule_state->cookie;
    pipe = logical_port / TRIDENT3_DEV_PORTS_PER_PIPE;
    ...
    switch(rem_add_cmd) {
        ...
        case 0: port_map = &port_schedule_state->in_port_map;
                ...
                idb_port = TRIDENT3_TDM_IDB_UNUSED_TOKEN & 0x3f;
                mmu_port = TRIDENT3_TDM_MMU_UNUSED_TOKEN & 0x3f;
                id = 0x7;
        ...
        case 1: port_map = &port_schedule_state->out_port_map;
                ...
                prev_or_new = 0;
                soc_td3_tdm_get_log_port_speed(unit, port_schedule_state,
                        logical_port, prev_or_new, &l_speed, &l_num_lanes);

                pblk_spacing = _soc_td3_tdm_get_same_port_spacing_e(unit,
                                                                    l_speed);
    ...
    for (hpipe_i = 0; hpipe_i < TRIDENT3_OVS_HPIPE_COUNT_PER_PIPE; hpipe_i++) {
        for (group_i = 0; group_i < TRIDENT3_OVS_GROUP_COUNT_PER_HPIPE; group_i++) {
            for (slot_i = 0; slot_i < TRIDENT3_OVS_GROUP_TDM_LENGTH; slot_i++) {
                if (phy_port ==
    ...
    LOG_DEBUG(BSL_LS_SOC_TDM, (BSL_META_U(unit,
              " found %d hpipe=%1d log_port=%3d phy_port=%3d group=%1d slot=%1d \n"),
                found, hpipe, logical_port, phy_port, group, slot));

    /* If logical port to be removed is not found in OVS table
     * no further action is taken.
     */
    if ((rem_add_cmd == 0) && (found == 0)) {
        LOG_INFO(BSL_LS_SOC_TDM, (BSL_META_U(unit,
            "Abort removal of entry not found for logical_port in OVS tables %d"), logical_port));
        return SOC_E_NONE;
    }
    ...
    switch(rem_add_cmd) {
        ...
        case 2:
                ...
                curr_tdm_schedule_pipe->tdm_schedule_slice[
					hpipe].oversub_schedule[group][slot] = TD3_NUM_EXT_PORTS;
                /* If group gets empty due to remove then configure ovs_class to 0; otherwise skip*/
                cfg_en_OVR_SUB_GRP_CFG = 1;
                for (i = 0; i < TRIDENT3_OVS_GROUP_TDM_LENGTH; i++) {
                    if (TD3_NUM_EXT_PORTS >
						curr_tdm_schedule_pipe->tdm_schedule_slice[
        ...
        case 3:
                ...
                if (cfg_en_OVR_SUB_GRP_CFG == 1)  {
                    ...
                    soc_td3_tdm_get_log_port_speed(unit, port_schedule_state,
                            logical_port, prev_or_new, &l_speed, &l_num_lanes);
                    ovs_class = _soc_td3_tdm_get_speed_ovs_class(unit,
                                                                 l_speed);
                }
                /* Add to temp table */
                curr_tdm_schedule_pipe->tdm_schedule_slice[
    ...
    if (cfg_en_OVR_SUB_GRP_CFG == 1) {
        ...
        soc_td3_tdm_get_log_port_speed(unit, port_schedule_state,
                logical_port, prev_or_new, &l_speed, &l_num_lanes);
        ovs_same_spacing = _soc_td3_tdm_get_same_port_spacing_e(unit, l_speed);
    ...
    if (cfg_en_PBLK_CALENDAR == 1) {
        pm_num  = (phy_port - 1) / TRIDENT3_PORTS_PER_PBLK;
        subport = (phy_port - 1) % TRIDENT3_PORTS_PER_PBLK;
        pblk_cal_idx = port_map->port_p2PBLK_inst_mapping[phy_port];
        soc_td3_tdm_get_port_ratio(unit, port_schedule_state, pm_num,
...
/**
@name: soc_trident3_tdm_flexport_remove_ports_sel
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
...
**/
int
soc_trident3_tdm_flexport_remove_ports_sel(int unit,
	soc_port_schedule_state_t *port_schedule_state,
    int set_idb,
    int set_mmu )
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        ...
        if ((-1 == physical_port) && (1 == is_port_oversub)) { /* that is, OVS port going DOWN */
            soc_trident3_tdm_flexport_ovs_add_rem_port(unit,
                                                        port_schedule_state,
                                                        logical_port,
                                                        rem_add_cmd,
                                                        set_idb,
...
/**
@name: soc_trident3_tdm_flexport_add_ports_sel
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
...
**/
int
soc_trident3_tdm_flexport_add_ports_sel(int unit,
	soc_port_schedule_state_t *port_schedule_state,
    int set_idb,
    int set_mmu )
{
...
/**
@name: soc_trident3_tdm_flexport_ovs_consolidate_per_speed
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
...
**/
int
soc_trident3_tdm_flexport_ovs_consolidate_per_speed(int unit,
	soc_port_schedule_state_t *port_schedule_state,
    int pipe,
    int hpipe,
    int ovs_class,
    ...
    int set_mmu )
{
    ...
    int ovs_class_i;
    int grp_bmp[TRIDENT3_OVS_GROUP_COUNT_PER_HPIPE];
    int num_of_ovs_grps;
    ...
    for (group = 0; group < TRIDENT3_OVS_GROUP_COUNT_PER_HPIPE; group++) {
        for (slot = 0; slot < TRIDENT3_OVS_GROUP_TDM_LENGTH; slot++) {
            physical_port =
               curr_tdm_schedule_hpipe->oversub_schedule[group][slot];
            if (physical_port < TD3_NUM_EXT_PORTS) {
                logical_port =
				port_schedule_state->out_port_map.port_p2l_mapping[physical_port];
                ...
                ovs_class_i = _soc_td3_tdm_get_speed_ovs_class(unit, l_speed);
    ...
    LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
			"START soc_trident3_tdm_flexport_ovs_consolidate_per_speed"
            ...
            for (slot = 0; slot < TRIDENT3_OVS_GROUP_TDM_LENGTH; slot++) {
                ...
                if (physical_port < TD3_NUM_EXT_PORTS) {
                    ...
                    if (physical_port ==
                        ...
                        found = 0;
                        for (i = 0; i < TRIDENT3_OVS_GROUP_COUNT_PER_HPIPE; i++) {
                            for (j = 0; j < TRIDENT3_OVS_GROUP_TDM_LENGTH; j++) {
                                if (physical_port ==
				...
				"___ERROR could not find group and slot in new OVS for phy_port=%d"),
                        ...
                        if (curr_tdm_schedule_hpipe->oversub_schedule[
								dst_group][dst_slot] < TD3_NUM_EXT_PORTS) {
                            continue;
                        } else {
                            logical_port =
								port_schedule_state->out_port_map.port_p2l_mapping[
...
/**
@name: soc_trident3_tdm_flexport_ovs_consolidate_sel
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
...
**/
int
soc_trident3_tdm_flexport_ovs_consolidate_sel(int unit,
	soc_port_schedule_state_t *port_schedule_state,
    int set_idb,
    int set_mmu )
{
     ...
     * 5 : 100G
     */
    int prev_num_grp[TRIDENT3_PIPES_PER_DEV][TRIDENT3_OVS_HPIPE_COUNT_PER_PIPE][7];
    int  new_num_grp[TRIDENT3_PIPES_PER_DEV][TRIDENT3_OVS_HPIPE_COUNT_PER_PIPE][7];
            ...
            for (group = 0; group < TRIDENT3_OVS_GROUP_COUNT_PER_HPIPE; group++) {
                ...
                for (slot = 0; slot < TRIDENT3_OVS_GROUP_TDM_LENGTH; slot++) {
                    ...
                    if (physical_port < TD3_NUM_EXT_PORTS) {
                        ...
                        prev_or_new = 1;
                        soc_td3_tdm_get_log_port_speed(unit,
        ...
        for (hpipe = 0; hpipe < TRIDENT3_OVS_HPIPE_COUNT_PER_PIPE; hpipe++) {
            ...
            for (ovs_class = 1; ovs_class < 7; ovs_class++) {
                if (new_num_grp[pipe][hpipe][ovs_class] <= prev_num_grp[
												pipe][hpipe][ovs_class]) {
                     soc_trident3_tdm_flexport_ovs_consolidate_per_speed(unit,
								port_schedule_state,
                                pipe,
                                hpipe,
                                ovs_class,
...
/*! @fn int soc_trident3_tdm_calculate_ovs_per_pipe(int unit,
 *              soc_port_schedule_state_t *port_schedule_state,
 ...
 */
int
soc_trident3_tdm_calculate_ovs_per_pipe(
    ...
    int pipe, int prev_or_new)
{
    ...
    int pipe_ovs_slots;
    int hpipe_ovs_slots[TRIDENT3_OVS_HPIPE_COUNT_PER_PIPE];
    soc_port_map_type_t *port_map;
    int pipe_ovs_ratio; /* per PIPE ovs_ratio */
    int hpipe_ovs_ratio[TRIDENT3_OVS_HPIPE_COUNT_PER_PIPE]; /* per Half PIPE ovs_ratio */
	...
	num_ancl_ports = (mgmt_port_cnt > 1) ? (TD3_NUM_ANCL) : (TD3_NUM_ANCL - 4);
...
/*! @fn int soc_trident3_tdm_calculate_ovs(int unit,
 *              soc_port_schedule_state_t *port_schedule_state,
 *              int pipe, int prev_or_new)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_trident3_tdm_calculate_ovs(int                        unit,
                                soc_port_schedule_state_t *port_schedule_state,
                                int                        prev_or_new)
{
    int pipe;
    for (pipe = 0; pipe < TRIDENT3_PIPES_PER_DEV; pipe++) {
        soc_trident3_tdm_calculate_ovs_per_pipe(unit, port_schedule_state,
                                                 pipe,
                                                 prev_or_new);
    }
    return SOC_E_NONE;
...
/**
@name: soc_trident3_tdm_calculation_flexport
@param: int unit, soc_port_schedule_state_t *port_schedule_state
 ...
 *      SOC_E_*
**/
int soc_trident3_tdm_calculation_flexport(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    int port, pm_num;
    int pipe, hpipe;
    ...
    tdm_soc_t soc_pkg;
    tdm_mod_t *_tdm_pkg;
    tdm_mod_t *_tdm_pkg_rv;

 	LOG_DEBUG(BSL_LS_SOC_TDM, (BSL_META_U(unit,
                      "Flexport TDM calc started...")));

    /* If init then just copy in_port_map to out_port_map*/
    soc_trident3_tdm_set_out_map(unit, port_schedule_state);

  	/* Compute OVS ratios before FlexPort*/
    soc_trident3_tdm_calculate_ovs(unit, port_schedule_state, 1);
    ...
    soc_trident3_set_tdm_soc_pkg(unit, port_schedule_state, &soc_pkg, 1);
    ...
    if (1 == port_schedule_state->is_flexport) {
        soc_trident3_set_tdm_soc_pkg(unit, port_schedule_state,
					&_tdm_pkg->_prev_chip_data.soc_pkg, 0);
        /* Allocate memeory for previous TDM tables in _tdm_pkg */
        tdm_td3_alloc_prev_chip_data(_tdm_pkg);
        soc_trident3_tdm_copy_prev_tables(unit, port_schedule_state, _tdm_pkg);
    }

    /* Generate the new config */
    _tdm_pkg_rv = _soc_set_tdm_tbl(_tdm_pkg);

    /* Free the memory allocated in soc_trident3_set_tdm_soc_pkg calls */
    sal_free(soc_pkg.state);
    sal_free(soc_pkg.speed);
    if (1 == port_schedule_state->is_flexport) {
        sal_free(_tdm_pkg->_prev_chip_data.soc_pkg.state);
        sal_free(_tdm_pkg->_prev_chip_data.soc_pkg.speed);
        tdm_td3_free_prev_chip_data(_tdm_pkg);
    }
    if (!_tdm_pkg_rv) {
        LOG_CLI((BSL_META_U(unit,
            "TDM algorithm failed to generate TDM calendars\n")));
        return SOC_E_FAIL;
    }

    /* Copy from _tdm_pkg back to port_schedule_state */
    /* IDB calendar (0-1), MMU calendar (4-5) */
    for (cal_id = 0; cal_id < TRIDENT3_PIPES_PER_DEV; cal_id++) {
        switch (cal_id) {
            case 0:
        ...
        sal_memcpy(port_schedule_state->tdm_ingress_schedule_pipe[
                   ...
                   tdm_idb_pipe_main, sizeof(int)*TRIDENT3_TDM_LENGTH);

        /* MMU TDM main calendar */
        sal_memcpy(port_schedule_state->tdm_egress_schedule_pipe[
					cal_id].tdm_schedule_slice[0].linerate_schedule,
                   tdm_mmu_pipe_main, sizeof(int)*TRIDENT3_TDM_LENGTH);

        /* IDB and MMU OverSub Groups */
        for (index = 0; index < TRIDENT3_OVS_GROUP_COUNT_PER_PIPE; index++) {
            /* Group 0:5 -> hpipe 0, Group 6:11 -> hpipe1 */
            hpipe = index / TRIDENT3_OVS_GROUP_COUNT_PER_HPIPE;
            cal_grp = index % TRIDENT3_OVS_GROUP_COUNT_PER_HPIPE;
            sal_memcpy(port_schedule_state->tdm_egress_schedule_pipe[
        ...
        for (hpipe = 0; hpipe < 2; hpipe ++) {
            sal_memcpy(port_schedule_state->tdm_ingress_schedule_pipe[
                    ...
                    tdm_pipe_group[hpipe+TD3_SHAPING_GRP_IDX_0], sizeof(int)
					* TD3_SHAPING_GRP_LEN);
        }
    }
    /*
    ...
    for (pm_num = 0; pm_num < TD3_NUM_PHY_PM; pm_num++) { /* TRIDENT3_PBLKS_PER_PIPE * TRIDENT3_PIPES_PER_DEV */
        for (port = 0; port < TRIDENT3_PORTS_PER_PBLK; port++) {
            port_schedule_state->out_port_map.port_p2PBLK_inst_mapping[
				(pm_num * TRIDENT3_PORTS_PER_PBLK) + 1 + port] =
    ...
    soc_trident3_tdm_calculate_ovs(unit, port_schedule_state, 0);


    for (pipe = 0; pipe < TRIDENT3_PIPES_PER_DEV; pipe++) {
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "Pipe %d idb_tdm:"), pipe));
        for (index = 0; index < TRIDENT3_TDM_LENGTH; index++) {
            if (index % 16 == 0) {
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                                    "\n    ")));
            }
        ...
        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "\n")));
        for (hpipe = 0; hpipe < TRIDENT3_OVS_HPIPE_COUNT_PER_PIPE; hpipe++) {
            for (group = 0; group < TRIDENT3_OVS_GROUP_COUNT_PER_HPIPE;
                    group++) {
                LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                            "Pipe %d hpipe %d group %d ovs_tdm"),
                            pipe, hpipe, group));
                for (index = 0; index < TRIDENT3_OVS_GROUP_TDM_LENGTH;
                        index++) {
                    if (index == 0) {
                        LOG_DEBUG(BSL_LS_SOC_PORT, (BSL_META_U(unit,
                                            "\n    ")));
	...
	_soc_td3_tdm_print_schedule_state(unit, port_schedule_state);
...
/**
@name: soc_trident3_tdm_flexport_remove_ports
@param: int unit, soc_port_schedule_state_t *port_schedule_state, int set_idb, int set_mmu
...
**/
int
soc_trident3_tdm_flexport_remove_ports(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    return (soc_trident3_tdm_flexport_remove_ports_sel(
...
/**
@name: soc_trident3_tdm_flexport_ovs_consolidate
@param: int unit, soc_port_schedule_state_t *port_schedule_state
...
**/
int
soc_trident3_tdm_flexport_ovs_consolidate(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    return (soc_trident3_tdm_flexport_ovs_consolidate_sel(
				unit, port_schedule_state, 1, 1));
}


/**
@name: soc_trident3_tdm_flexport_add_ports
@param: int unit, soc_port_schedule_state_t *port_schedule_state
...
**/
int
soc_trident3_tdm_flexport_add_ports(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    return (soc_trident3_tdm_flexport_add_ports_sel(unit,
				port_schedule_state, 1, 1));
}

/*! @fn int soc_trident3_tdm_flexport_idb(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_trident3_tdm_flexport_idb(int unit,
	soc_port_schedule_state_t *port_schedule_state)
{
    SOC_IF_ERROR_RETURN(soc_trident3_tdm_flexport_remove_ports_sel(unit,
                                                   port_schedule_state, 1, 0));
    SOC_IF_ERROR_RETURN(soc_trident3_tdm_flexport_ovs_consolidate_sel(unit,
                                                   port_schedule_state, 1, 0));
    SOC_IF_ERROR_RETURN(soc_trident3_tdm_flexport_add_ports_sel(unit,
                                                   port_schedule_state, 1, 0));
    return SOC_E_NONE;
}

/*! @fn int soc_trident3_tdm_flexport_mmu(int unit,
 *              soc_port_schedule_state_t *port_schedule_state)
 *  @param unit Chip unit number.
 *  @param port_schedule_state Pointer to a soc_port_schedule_state_t struct variable
 ...
 */
int
soc_trident3_tdm_flexport_mmu(int                        unit,
                               soc_port_schedule_state_t *port_schedule_state)
{
    SOC_IF_ERROR_RETURN(soc_trident3_tdm_flexport_remove_ports_sel(unit,
...
/**
@name: tdm_td3_alloc_prev_chip_data
@param:
...
tdm_td3_alloc_prev_chip_data( tdm_mod_t *_tdm_pkg )
{
    ...
    _tdm_pkg->_prev_chip_data.cal_0.cal_main=(int *) TDM_ALLOC(
				(TD3_LR_VBS_LEN)*sizeof(int), "OLD TDM inst 0 main calendar");
    _tdm_pkg->_prev_chip_data.cal_0.cal_grp=(int **) TDM_ALLOC(
				(TD3_OS_VBS_GRP_NUM+2)*sizeof(int *), "OLD TDM inst 0 groups");
    for (index=0; index<(TD3_OS_VBS_GRP_NUM); index++) {
        _tdm_pkg->_prev_chip_data.cal_0.cal_grp[index]=
			(int *) TDM_ALLOC((TD3_OS_VBS_GRP_LEN)*sizeof(int),
				"OLD TDM inst 0 group calendars");
    }
    _tdm_pkg->_prev_chip_data.cal_0.cal_grp[TD3_SHAPING_GRP_IDX_0]=
			(int *) TDM_ALLOC((TD3_SHAPING_GRP_LEN)*sizeof(int),
			"OLD TDM inst 0 shaping calendars");
    _tdm_pkg->_prev_chip_data.cal_0.cal_grp[TD3_SHAPING_GRP_IDX_1]=
			(int *) TDM_ALLOC((TD3_SHAPING_GRP_LEN)*sizeof(int),
			"OLD TDM inst 0 shaping calendars");
    /* Pipe 1 calendar group */
    _tdm_pkg->_prev_chip_data.cal_1.cal_main=
			(int *) TDM_ALLOC((TD3_LR_VBS_LEN)*sizeof(int),
			"OLD TDM inst 1 main calendar");
    _tdm_pkg->_prev_chip_data.cal_1.cal_grp=
			(int **) TDM_ALLOC((TD3_OS_VBS_GRP_NUM+2)*sizeof(int *),
    ...
    _tdm_pkg->_prev_chip_data.cal_1.cal_grp[TD3_SHAPING_GRP_IDX_0]=
			(int *) TDM_ALLOC((TD3_SHAPING_GRP_LEN)*sizeof(int),
			"OLD TDM inst 1 shaping calendars");
    _tdm_pkg->_prev_chip_data.cal_1.cal_grp[TD3_SHAPING_GRP_IDX_1]=(
			int *) TDM_ALLOC((TD3_SHAPING_GRP_LEN)*sizeof(int),
			"OLD TDM inst 1 shaping calendars");
...
/**
@name: tdm_td3_free_prev_chip_data
@param:

 * Description:
 *      Helper function to free previous TDM tables in
 *      in _tdm_pkg->_prev_chip_data at FlexPort.
**/
int
tdm_td3_free_prev_chip_data( tdm_mod_t *_tdm_pkg )
{
    ...
    TDM_FREE(_tdm_pkg->_prev_chip_data.cal_0.cal_grp[TD3_SHAPING_GRP_IDX_0]);
    TDM_FREE(_tdm_pkg->_prev_chip_data.cal_0.cal_grp[TD3_SHAPING_GRP_IDX_1]);
    ...
    TDM_FREE(_tdm_pkg->_prev_chip_data.cal_1.cal_grp[TD3_SHAPING_GRP_IDX_0]);
    TDM_FREE(_tdm_pkg->_prev_chip_data.cal_1.cal_grp[TD3_SHAPING_GRP_IDX_1]);
