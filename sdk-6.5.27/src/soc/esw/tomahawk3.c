...
#define _SOC_TH3_FIXED_L3_TBL_SZ    (16 * 1024)
    ...
    int def_ipv6_128b_entries = 0, index_max;
    ...
    if (soc_property_get(unit, "l3_defip_sizing", TRUE)) {
        if (!(soc_feature(unit, soc_feature_alpm) && alpm_enable)) {
            ...
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_PAIR_LEVEL1m, num_ipv6_128b_entries - 1);
            index_max = (SOC_CONTROL(unit)->l3_defip_max_tcams *
                         SOC_CONTROL(unit)->l3_defip_tcam_size) -
                         (num_ipv6_128b_entries * 2) - 1;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_LEVEL1m, index_max);
            ...
            if (soc_property_get(unit, spn_IPV6_LPM_128B_ENABLE, 1)) {
                ...
                SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_PAIR_LEVEL1m, -1);
            }
        }
    ...
    */
    ...
    if (!SAL_BOOT_SIMULATION || SAL_BOOT_QUICKTURN)
    {
        ...
        if (dev_id == BCM56983_DEVICE_ID) {
            /* In TH3 56983 SKU, EGR_PORT and POR_TAB has non-zero values after
               chip power-up for memory entries of invalid pipes
               (index 40 - 119). However, using EGR_HW_RESET_CONTROL_1r is not
               able to reset those memory entries on 56983, so we have to
               manually clear them here. Failed to do so can cause incorrect
               usage of flex counter resource after WB.
            */
            SOC_IF_ERROR_RETURN(soc_mem_clear(unit, EGR_PORTm, COPYNO_ALL, 1));
            SOC_IF_ERROR_RETURN(soc_mem_clear(unit, PORT_TABm, COPYNO_ALL, 1));
        }
    }
...
_soc_tomahawk3_ipep_default_init (int unit)
{
    ...
    soc_th_rtag7_flow_hash_entry_t *rtag7_flow_entry = NULL;
    int rv;
    ...
    /* Populate and enable RTAG7 Macro flow offset table in cold boot */
    if (!SOC_WARM_BOOT(unit) &&
        soc_mem_is_valid(unit, RTAG7_FLOW_BASED_HASHm)) {
        count = soc_mem_index_max(unit, RTAG7_FLOW_BASED_HASHm) + 1;
        if (rtag7_flow_entry == NULL) {
            rtag7_flow_entry = sal_alloc(count * sizeof(soc_th_rtag7_flow_hash_entry_t),
                                         "Tomahawk3 Rtag flow entries");
            if (rtag7_flow_entry == NULL) {
                return SOC_E_MEMORY;
            }
        }
        ...
        for (index = 0; index < count;) {
            for (sub_sel = 0; sub_sel < 8 && index < count; sub_sel++) {
                for (offset = 0;
                     ...
                     offset++) {
                    rtag7_flow_entry[index].concat = 0;
                    rtag7_flow_entry[index].sub_sel = (uint8)sub_sel;
                    rtag7_flow_entry[index].offset = (uint8)offset;
                    index++;
                }
            }
        }
        soc_th_rtag7_flow_hash_entry_shuffle(
           rtag7_flow_entry,
           count,
           soc_property_get(unit, spn_MACRO_FLOW_HASH_SHUFFLE_RANDOM_SEED, 0));
        sal_memset(entry, 0, sizeof(rtag7_flow_based_hash_entry_t));
        for (index = 0; index < count; ) {
            if (ecmp_mode == 1) {
                soc_mem_field32_set(unit, RTAG7_FLOW_BASED_HASHm, &entry,
                                    SUB_SEL_ECMP_LEVEL2f, rtag7_flow_entry[index].sub_sel);
                soc_mem_field32_set(unit, RTAG7_FLOW_BASED_HASHm, &entry,
                                    OFFSET_ECMP_LEVEL2f, rtag7_flow_entry[index].offset);
            }
            soc_mem_field32_set(unit, RTAG7_FLOW_BASED_HASHm, &entry,
                                SUB_SEL_ECMPf, rtag7_flow_entry[index].sub_sel);
            soc_mem_field32_set(unit, RTAG7_FLOW_BASED_HASHm, &entry,
                                OFFSET_ECMPf, rtag7_flow_entry[index].offset);
            rv = soc_mem_write(unit, RTAG7_FLOW_BASED_HASHm,
                               MEM_BLOCK_ANY, index, &entry);
            if (SOC_FAILURE(rv)) {
                sal_free(rtag7_flow_entry);
                return rv;
            }
            index++;
        }
...
soc_tomahawk3_misc_init_fast_reboot(int unit)
{
    ...
    */
    ...
    int phy_port, pm_num, pipe_num, pm;
        ...
        */
        ...
        PBMP_PORT_ITER(unit, port) {
            ...
            if (used_pm_map[pm_num] == 0) {
                ...
                /* Only do this once per pm */
                used_pm_map[pm_num] = 0;
            }
...
int
soc_tomahawk3_voltage_monitor_get(int unit, int* avg_voltage)
{
    uint32 rval, fval;
   int index = 0;
    uint32 val;
    uint32 maskval = 0;
    uint32 pvtmon_reset_old = 0;
    uint32 pvtmon_select_old = 0;
    uint32 pvtmon_13_hi_temp_en_old = 0;
    uint32 pvtmon_14_hi_temp_en_old = 0;
    soc_reg_t reg;
    int count = 10;

    SOC_IF_ERROR_RETURN(READ_TOP_PVTMON_13_17_CTRL_1r(unit, &rval));

    pvtmon_reset_old = soc_reg_field_get(unit, TOP_PVTMON_13_17_CTRL_1r, rval, PVTMON_RESET_Nf);
    pvtmon_select_old = soc_reg_field_get(unit, TOP_PVTMON_13_17_CTRL_1r, rval, PVTMON_SELECTf);
    pvtmon_13_hi_temp_en_old = soc_reg_field_get(unit, TOP_PVTMON_13_17_CTRL_1r, rval,
                                                 PVTMON13_PVTMON_HIGHTEMP_CTRL_ENf);
    pvtmon_14_hi_temp_en_old = soc_reg_field_get(unit, TOP_PVTMON_13_17_CTRL_1r, rval,
                                                 PVTMON14_PVTMON_HIGHTEMP_CTRL_ENf);

    SOC_IF_ERROR_RETURN(READ_TOP_PVTMON_INTR_MASK_0r(unit, &val));
    maskval = (uint32) ~(PVT_INTR_MON_13_MAX | PVT_INTR_MON_13_MIN |
                         PVT_INTR_MON_14_MAX | PVT_INTR_MON_14_MIN);

    /* Setting the VTMON in voltage mode */
    SOC_IF_ERROR_RETURN(WRITE_TOP_PVTMON_INTR_MASK_0r(unit, val & maskval));
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON13_PVTMON_HIGHTEMP_CTRL_ENf, 0);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON14_PVTMON_HIGHTEMP_CTRL_ENf, 0);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON_RESET_Nf, 0);
    SOC_IF_ERROR_RETURN(WRITE_TOP_PVTMON_13_17_CTRL_1r(unit, rval));
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON_RESET_Nf, 1);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON_SELECTf, 4);
    SOC_IF_ERROR_RETURN(WRITE_TOP_PVTMON_13_17_CTRL_1r(unit, rval));
    sal_usleep(1000);

    /* Read Voltages. */
    reg = TOP_PVTMON_RESULT_13r;
    fval = 0;
    ...
    *avg_voltage = fval;
    ...
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON_RESET_Nf, 0);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON13_TOP_PVT_MON_MAX_RST_Lf, 0);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON13_TOP_PVT_MON_MIN_RST_Lf, 0);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON14_TOP_PVT_MON_MAX_RST_Lf, 0);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON14_TOP_PVT_MON_MIN_RST_Lf, 0);
    SOC_IF_ERROR_RETURN(WRITE_TOP_PVTMON_13_17_CTRL_1r(unit, rval));
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON_RESET_Nf, pvtmon_reset_old);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON_SELECTf, pvtmon_select_old);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON13_TOP_PVT_MON_MAX_RST_Lf, 1);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON13_TOP_PVT_MON_MIN_RST_Lf, 1);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON14_TOP_PVT_MON_MAX_RST_Lf, 1);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON14_TOP_PVT_MON_MIN_RST_Lf, 1);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON13_PVTMON_HIGHTEMP_CTRL_ENf,
                      pvtmon_13_hi_temp_en_old);
    soc_reg_field_set(unit, TOP_PVTMON_13_17_CTRL_1r, &rval, PVTMON14_PVTMON_HIGHTEMP_CTRL_ENf,
                      pvtmon_14_hi_temp_en_old);
    SOC_IF_ERROR_RETURN(WRITE_TOP_PVTMON_INTR_MASK_0r(unit, val));
    sal_usleep(1000);
    ...
    for (i = 0; i < _TH3_PIPES_PER_DEV; i++) {
        for (j = 0; j < _TH3_PBLKS_PER_PIPE; j++) {
            ...
            if (mem_it == mem || mem_it_unique == mem) {
                ...
                if (mem_it_unique == mem) {
                    /* check if any ports are mapped to the PM */
                    for (phy_port_iter = (pm_num * _TH3_PORTS_PER_PBLK) + 1;
                            phy_port_iter < ((pm_num + 1) * _TH3_PORTS_PER_PBLK) + 1;
                            phy_port_iter += 1) {
                        /* if we found a mapped port, the PM is active */
                        if (si->port_p2l_mapping[phy_port_iter] != -1) {
                            break;
                        }
                    }

                    if (phy_port_iter ==
                        ((pm_num + 1) * _TH3_PORTS_PER_PBLK) + 1) {
                        /* reached here with the PM having power but no mapped
                         * ports. Skip memory as it will not be accessible */
                        *skip = 1;
                        return SOC_E_NONE;
                    }
                }
