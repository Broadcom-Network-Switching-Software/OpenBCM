 ...
 *  @param unit Device number
 *  @param *port_resource_t Port Resource Struct
 *  @param start : 1 to start by setting cap_average to 1, 0 to finish, restoring
 *         the value of cap_average
 *  @param saved: Pointer to array of cap_average (fill in if start == 1, use if 0)
 *  @param index: Pointer to maintain index into saved across function calls.
 *                Increment after each read or write
 *  @brief API to set cap_average bit in the WRED block for a given destination
 *         port. This will clear the average queue size memory after 140 us.
 *
 */
int
soc_tomahawk2_mmu_wred_clr(int unit, soc_port_resource_t *port_resource_t,
                            int start, uint32 *saved, int *index)
{
    /*Variables*/
        ...
        if (epipes[pipe] == 1) {
            ...
            for(q = 0; q < TH2_MMU_NUM_UCQ_PER_HSP_PORT; q++) {
                ...
                SOC_IF_ERROR_RETURN(soc_mem_read(unit, mmu_wred_config,
                                    ...
                                    wred_config_data));
                /* If start, save value to restore later, else, restoring
                 * previous value */
                if (start) {
                    saved[*index] = soc_mem_field32_get(unit, mmu_wred_config,
                                        wred_config_data, CAP_AVERAGEf);
                    cap_avg = 1;
                } else {
                    cap_avg = saved[*index];
                }
