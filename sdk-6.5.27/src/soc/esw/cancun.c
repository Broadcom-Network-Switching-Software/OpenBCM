...
void soc_cancun_deinit(uint32 unit) {
    ...
    if(cc != NULL) {
        ...
        if(cc->flow_db != NULL) {
            if(cc->flow_db->str_tbl != NULL) {
                sal_free(cc->flow_db->str_tbl);
            }
...
int soc_cancun_chip_rev_validate(const uint32 chip_rev_id,
    ...
    if ((cancun_dev_id == BCM56275_DEVICE_ID) &&
        ...
        if (((dev_id == BCM56273_DEVICE_ID) && (rev_id == BCM56273_A0_REV_ID)) ||
            ...
            ((dev_id == BCM53642_DEVICE_ID) && (rev_id == BCM53642_A0_REV_ID))) {
            return TRUE;
        }
    }
                                      ...
                                      uint32 *field_id)
{
    ...
    offset  = 4;
    /* coverity complains the overrun of the last element entry of
     * the structure soc_cancun_hash_table_t. The element entry
     * is simply used as a starting point to a dynamic sized array.
     * So it is expected to write to beyond this element.
     */
    ...
    for (i = 0; i < cch_map->dest_mem_num; i++) {
        ...
        for (j = 0; j < d_ind_num; j++) {
            ...
            if (d_mem & SOC_CANCUN_FLAG_REG_ENUM) {
                ...
                if (cch_map->format == 2 || cch_map->format == 3) {
 ...
 */
int _soc_cancun_bind_soc_features_int(int unit, uint32 running_cancun_ver,
                                 soc_cancun_file_branch_id_e running_branch_id,
                                 soc_feature_cancun_version_map_t *map,
                                 int map_count) {
    int entry = 0;
    ...
    for (entry = 0; entry < map_count; entry++) {
        ...
        if (running_branch_id == branch_id) {
            ...
            if (map_entry.feature_enable == CANCUN_SOC_FEATURE_ENABLE) {
                SOC_FEATURE_SET(unit, map_entry.soc_feature);
            } else if (map_entry.feature_enable == CANCUN_SOC_FEATURE_DISABLE) {
...
int _soc_cancun_bind_soc_features(int unit, uint32 running_cancun_ver,
    ...
    return _soc_cancun_bind_soc_features_int(unit,
            running_cancun_ver, running_branch_id,
            soc_feature_cancun_version_map,
            COUNTOF(soc_feature_cancun_version_map));
}

int _soc_cancun_branch_soc_features_exception(int unit, uint32 running_cancun_ver,
        soc_cancun_file_branch_id_e running_branch_id) {

    return _soc_cancun_bind_soc_features_int(unit,
            running_cancun_ver, running_branch_id,
            soc_feature_cancun_branch_exception_map,
            COUNTOF(soc_feature_cancun_branch_exception_map));
}
...
int soc_cancun_version_soc_feature_bind(int unit) {
    ...
    SOC_IF_ERROR_RETURN(_soc_cancun_branch_soc_features_exception
                        (unit, running_cancun_ver, running_branch_id));
