...
/* Reduced UTT info */
#define HR4_REDUCED_UTT_MAX_TCAMS              (12)
#define HR4_REDUCED_UTT_IFP_MIN_TCAMS          (4)
#define HR4_REDUCED_UTT_LPM_MIN_TCAMS          (4)
#define HR4_REDUCED_UTT_IFP_MAX_TCAMS          (8)
/*
#define HR4_REDUCED_UTT_LPM_MAX_TCAMS          (8)
*/

/*! Reduced UTT info for WH3 */
#define WH3_REDUCED_UTT_IFP_MIN_TCAMS          (8)
...
soc_hr4_utt_init(int u)
{
    ...
    /* Update UTT params for reduced feature in TD3X1 */
    if (soc_feature(u, soc_feature_td3x1_reduced_utt)) {
        SOC_UTT_MAX_TCAMS(u)  = HR4_REDUCED_UTT_MAX_TCAMS;
        /* IFP */
        aid = SOC_UTT_AGENT_IFP;
        SOC_UTT_AGENT_MIN_TCAMS(u, aid)        = HR4_REDUCED_UTT_IFP_MIN_TCAMS;
        SOC_UTT_AGENT_MAX_TCAMS(u, aid)        = HR4_REDUCED_UTT_IFP_MAX_TCAMS;
        SOC_UTT_AGENT_DEF_NUM_LOOKUPS(u, aid)  = 8;
        SOC_UTT_AGENT_DEF_TABLE_SIZE(u, aid)   = 8 * HR4_UTT_TCAM_DEPTH;
        /* LPM
         * MAX_TCAMS is actually 8 in x1 but utt algo expects this to be 16
         * to derive total lpm look ups in SKU which remains same as HR4. */
        aid = SOC_UTT_AGENT_LPM;
        SOC_UTT_AGENT_MIN_TCAMS(u, aid)        = HR4_REDUCED_UTT_LPM_MIN_TCAMS;
        SOC_UTT_AGENT_MAX_TCAMS(u, aid)        = HR4_UTT_LPM_MAX_TCAMS;
        SOC_UTT_AGENT_DEF_NUM_LOOKUPS(u, aid)  = 4;
        SOC_UTT_AGENT_DEF_TABLE_SIZE(u, aid)   = 4 * HR4_UTT_TCAM_DEPTH;
    }

    /* Update UTT params for fixed ifp heavy utt mode in wh3 */
    if (soc_feature(u, soc_feature_wh3_reduced_utt)) {
        aid = SOC_UTT_AGENT_IFP;
        SOC_UTT_AGENT_MIN_TCAMS(u, aid)        = WH3_REDUCED_UTT_IFP_MIN_TCAMS;
    }
...
soc_hurricane4_mem_config(int unit)
{
    ...
    int num_l2_dedicated_banks = 2, num_l3_dedicated_banks = 2;
    int iuat_max_mpls_entries = 2048;
    int iuat_max_vlan_xlate_entries = 1024;
    int iuat_max_banks = HR4_SOC_IUAT_BANKS;
    int index_max;

    /* For TD3X1 dedicated bank is reduced to 1 */
    if (soc_feature(unit, soc_feature_td3x1_reduced_uft)) {
        num_l2_dedicated_banks = num_l3_dedicated_banks = 1;
    }
    /* No UFT */
    /* 64k dedicated L2 entries by default, (32k * 2 banks) */
    fixed_l2_entries = num_l2_dedicated_banks * 32 * 1024;
    l2_entries = fixed_l2_entries;

    /* 32k dedicated L3 entries by default, (16k * 2 banks) */
    fixed_l3_entries = num_l3_dedicated_banks * 16 * 1024;
     ...
     * HR4 - Banks 8, Total Entries - 4K
     *          Default MPLS 2K, VLAN_XLATE_1 1K, VLAN_XLATE_2 1K
     * TD3X1 - Banks 8, Total Entries - 4K
     *          Default MPLS 2K, VLAN_XLATE_1 1K, VLAN_XLATE_2 1K
     * WH3 - Banks 4, Total Entries - 2K
     *          Default MPLS 1K, VLAN_XLATE_1 512, VLAN_XLATE_2 512
     */
    if (soc_feature(unit, soc_feature_wh3_reduced_iuat)) {
        iuat_max_mpls_entries /= 2;
        iuat_max_vlan_xlate_entries /= 2;
        iuat_max_banks /= 2;
    }

    mpls_entries = soc_property_get(unit, spn_MPLS_MEM_ENTRIES,
            iuat_max_mpls_entries);
     ...
     * HR4/TD3X1/WH3 - Banks 4, Total Entries - 2K
     *                  EGR_VLAN_XLATE_1 1K, EGR_VLAN_XLATE_2 1K
     */
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2_ENTRY_SINGLEm, l2_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2_ENTRY_ONLY_SINGLEm, l2_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2_ENTRY_ECCm, l2_entries - 1);
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2_ENTRY_LPm, (num_l2_dedicated_banks * 8192) - 1);

    /* Adjust L3 table size */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_SINGLEm, l3_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_ONLY_SINGLEm, l3_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_ECCm, l3_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_DOUBLEm, l3_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_ONLY_DOUBLEm, l3_entries / 2 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_QUADm, l3_entries / 4 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_ONLY_QUADm, l3_entries / 4 - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_HIT_ONLYm, l3_entries / 4 - 1);
    /* 7k per dedicated L3 bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_LPm, (num_l3_dedicated_banks * 7168) - 1);


    /* Adjust MPLS table size */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, MPLS_ENTRY_SINGLEm, mpls_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, MPLS_ENTRYm, mpls_entries / 2 - 1);
    /* 192 per bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, MPLS_ENTRY_LPm, (mpls_banks * 192) - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, MPLS_ENTRY_ECCm, mpls_entries - 1);

    /* Adjust VLAN_XLATE_1 table size */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_1_SINGLEm, vlan_xlate_1_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_1_DOUBLEm, vlan_xlate_1_entries / 2 - 1);
    /* 192 per bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_1_LPm, (vlan_xlate_1_banks * 192) - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_1_ECCm, vlan_xlate_1_entries - 1);

    /* Adjust VLAN_XLATE_2 table size */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_2_SINGLEm, vlan_xlate_2_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_2_DOUBLEm, vlan_xlate_2_entries / 2 - 1);
    /* 192 per bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_2_LPm, (vlan_xlate_2_banks * 192) - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_2_ECCm, vlan_xlate_2_entries - 1);

    /* Adjust EGR_VLAN_XLATE_1 table size */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VLAN_XLATE_1_SINGLEm, egr_vlan_xlate_1_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VLAN_XLATE_1_DOUBLEm, egr_vlan_xlate_1_entries / 2 - 1);
    /* 192 per bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VLAN_XLATE_1_LPm, (egr_vlan_xlate_1_banks * 192) - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VLAN_XLATE_1_ECCm, egr_vlan_xlate_1_entries - 1);

    /* Adjust EGR_VLAN_XLATE_2 table size */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VLAN_XLATE_2_SINGLEm, egr_vlan_xlate_2_entries - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VLAN_XLATE_2_DOUBLEm, egr_vlan_xlate_2_entries / 2 - 1);
    /* 192 per bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VLAN_XLATE_2_LPm, (egr_vlan_xlate_2_banks * 192) - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VLAN_XLATE_2_ECCm, egr_vlan_xlate_2_entries - 1);
    ...
    if (soc_property_get(unit, "l3_defip_sizing", TRUE)) {
        if (!soc_feature(unit, soc_feature_utt)) {
            ...
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIPm, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ONLYm, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_DATA_ONLYm, index_max);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_HIT_ONLYm, index_max);
    ...
    if (SAL_BOOT_QUICKTURN) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MY_STATION_TCAMm, 31);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MY_STATION_TCAM_2m, 31);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MY_STATION_TCAM_2_DATA_ONLYm, 31);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MY_STATION_TCAM_2_ENTRY_ONLYm, 31);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MY_STATION_TCAM_DATA_ONLYm, 31);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MY_STATION_TCAM_ENTRY_ONLYm, 31);
    }
    ...
    if (soc_feature(unit, soc_feature_flowtracker_3banks_ft_table)) {
        /* 3 banks x 1k */
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_SINGLEm, 3071);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_DOUBLEm, 1535);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_DOUBLE_PIPE0m, 1535);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_ECCm, 3071);
    } else if (soc_feature(unit, soc_feature_flowtracker_5banks_ft_table)) {
        /* 5 banks x 1k */
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_SINGLEm, 5119);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_DOUBLEm, 2559);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_DOUBLE_PIPE0m, 2559);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_ECCm, 3071);
    }
    ...
    if (soc_feature(unit, soc_feature_flowtracker_half_entries_ft_group)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, BSC_KG_GROUP_TABLEm, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, BSC_DG_GROUP_TABLEm, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, BSC_KG_FLOW_ACTIVE_COUNTERm, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, BSC_KG_FLOW_LEARNED_COUNTERm, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, BSC_KG_FLOW_AGE_OUT_COUNTERm, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, BSC_KG_FLOW_EXCEED_COUNTERm, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, BSC_KG_FLOW_MISSED_COUNTERm, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, BSC_DG_FLOW_OPAQUE_COUNTER_0m, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, BSC_DG_FLOW_OPAQUE_COUNTER_1m, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, BSC_DG_FLOW_METER_EXCEED_COUNTERm, 127);
    }

    /* Adjust IPMC replication head and list memories */
    if (soc_feature(unit, soc_feature_td3x1_reduced_ipmc)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MMU_REPL_GROUP_INFO_TBL_PIPE0m, 2047);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MMU_REPL_HEAD_TBL_SPLIT0_PIPE0m, 8191);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MMU_REPL_HEAD_TBL_SPLIT1_PIPE0m, 8191);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MMU_REPL_LIST_TBL_PIPE0m, 8191);
    } else if (soc_feature(unit, soc_feature_wh3_reduced_ipmc)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MMU_REPL_GROUP_INFO_TBL_PIPE0m, 1023);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MMU_REPL_HEAD_TBL_SPLIT0_PIPE0m, 4095);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MMU_REPL_HEAD_TBL_SPLIT1_PIPE0m, 4095);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, MMU_REPL_LIST_TBL_PIPE0m, 4095);
    }

    /* Adjust ECMP Group and Member Memories */
    if (soc_feature(unit, soc_feature_td3x1_reduced_ecmp)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ECMP_COUNTm, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, INITIAL_L3_ECMP_GROUPm, 127);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, INITIAL_L3_ECMPm, 511);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ECMPm, 511);
    }

    /* No OTP bit for NHop in HR4 hence reduced NHop by soc feature */
    if (soc_feature(unit, soc_feature_td3x1_reduced_nhop)) {
    ...
    /* Adjust Ingress and Egress L3 Interface */
    if (soc_feature(unit, soc_feature_td3x1_reduced_l3intf)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_IIFm, 1023);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_IIF_ATTRS_1m, 1023);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_IIF_ATTRS_1m, 1023);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_L3_INTFm, 1023);
    }

    /* Adjust VRF */
    if (soc_feature(unit, soc_feature_td3x1_novrf)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VRF_ATTRS_2m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VRF_PROFILEm, -1);
    } else if (soc_feature(unit, soc_feature_wh3_reduced_vrf)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VRF_ATTRS_2m, 15);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VRF_PROFILEm, 15);
    }

    /* Adjust VFI */
    if (soc_feature(unit, soc_feature_td3x1_reduced_vfi)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VFIm, 511);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, VFI_ATTRS_1m, 511);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VFIm, 511);
    }

    /* Adjust Flex Counters */
    if (soc_feature(unit, soc_feature_td3x1_reduced_flex_counter)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_10m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_11m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_12m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_13m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_14m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_15m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_16m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_17m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_18m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_19m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_10_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_11_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_12_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_13_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_14_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_15_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_16_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_17_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_18_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_COUNTER_TABLE_19_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_OFFSET_TABLE_10m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_OFFSET_TABLE_11m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_OFFSET_TABLE_12m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_OFFSET_TABLE_13m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_OFFSET_TABLE_14m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_OFFSET_TABLE_15m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_OFFSET_TABLE_16m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_OFFSET_TABLE_17m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_OFFSET_TABLE_18m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FLEX_CTR_OFFSET_TABLE_19m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_FLEX_CTR_COUNTER_TABLE_2m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_FLEX_CTR_COUNTER_TABLE_3m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_FLEX_CTR_COUNTER_TABLE_2_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_FLEX_CTR_COUNTER_TABLE_3_PIPE0m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_FLEX_CTR_OFFSET_TABLE_2m, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_FLEX_CTR_OFFSET_TABLE_3m, -1);
    }
        ...
        *frequency = soc_property_get(unit, spn_CORE_CLOCK_FREQUENCY, *frequency);
        if (*frequency > (SOC_BOND_INFO(unit)->max_core_freq + 1)) {
            LOG_CLI((BSL_META_U(unit,
                    "Core clock frequency set %d exceeds max %d allowed.\n"),
                        *frequency, (SOC_BOND_INFO(unit)->max_core_freq)));
            return SOC_E_PARAM;
        }
    } else {
        switch (dev_id) {
...
_soc_hr4_port_flex_init(int unit, int is_any_cap)
{
    ...
    if (SOC_PBMP_NOT_NULL(SOC_PORT_DISABLED_BITMAP(unit, all)) || is_any_cap) {
        ...
        for (phy_port = 1; phy_port <= HURRICANE4_GPHY_PORTS_PER_PIPE; phy_port++) {
            pm = HR4_PHY_PORT_SERDERS(phy_port);
            phy_base = SOC_HR4_PORT_BLOCK_BASE_PHY_PORT(phy_port);
            ...
            if (flex_en == -1) {
                for (i = 0; i < SOC_DRIVER(unit)->port_num_blktype; i++) {
                    blk_idx = SOC_PORT_IDX_BLOCK(unit, phy_port, i);
                    if (blk_idx != -1) {
                        SHR_BITCLR(si->flexible_pm_bitmap, blk_idx);
                    }
                }
            }
        }
...
_soc_hr4_port_lanes_set(int unit, int phy_port, int *lanes, int port_bandwidth)
{
    if (HR4_PORT_IS_FALCON(unit, phy_port)) {
        *lanes = (port_bandwidth > 27000) ? -1
        ...
        if (soc_feature(unit, soc_feature_td3x1_macsec_port_speed)) {
            *lanes = (port_bandwidth == 20000 ||
                      port_bandwidth == 25000 ||
                      port_bandwidth == 12500 ||
...
soc_hurricane4_port_config_init(int unit, uint16 dev_id)
{
    ...
    static const char str_12p5[] = "12.5";
    char str_buf[8];
    int rv;
    ...
    SOC_IF_ERROR_RETURN
        (soc_hurricane4_max_frequency_get(unit, dev_id,
                                          -1, -1, &si->frequency));
    ...
    for (port = 1; port < num_port; port++) {
        ...
        /* Parse bandwidth */
        for (index = 0; index < sizeof(str_12p5) - 1; index++) {
            if ((sub_str[index] == '\0') ||
                (sub_str[index] == ':')) {
                break;
            }
        ...
        } else if((!sal_strcmp(str_buf, str_12p5)) &&
                  (soc_feature(unit, soc_feature_td3x1_macsec_port_speed))) {
            port_bandwidth = 12500;
            sub_str_end = &sub_str[sizeof(str_12p5) - 1];
        } else {
...
int
soc_hurricane4_port_blk_reset(int unit, int blk)
{
    int port, blktype;
    uint32 rval;
    uint64 rval64;
    ...
    if (blktype == SOC_BLK_CLPORT) {
        ...
        if (!IS_QSGMII_PORT(unit, port)) {
            ...
            if (((si->port_l2p_mapping[port] >= 25) &&
                ...
                /* External Clock, provides clock to other port macro */
                soc_reg64_field32_set(unit, XLPORT_XGXS0_CTRL_REGr, &rval64,
                        REFSELf, 3);
                soc_reg64_field32_set(unit, XLPORT_XGXS0_CTRL_REGr, &rval64,
                        PWRDWN_CML_LCf, 0);
            } else {
                /* Internal Clock, does not provide clock to other port macro */
                soc_reg64_field32_set(unit, XLPORT_XGXS0_CTRL_REGr, &rval64,
                        REFSELf, 5);
                soc_reg64_field32_set(unit, XLPORT_XGXS0_CTRL_REGr, &rval64,
                        PWRDWN_CML_LCf, 1);
    ...
    } else if (blktype == SOC_BLK_PMQPORT) {
        ...
        if (qtc_mode == Q_MODE) {
            ...
            if (index == 0) {
                /* External Clock, provides clock to other port macro */
                soc_reg64_field32_set(unit, PMQ_XGXS0_CTRL_REGr, &rval64,
                        REFSELf, 3);
                soc_reg64_field32_set(unit, PMQ_XGXS0_CTRL_REGr, &rval64,
                        PWRDWN_CML_LCf, 0);
            } else {
...
soc_hurricane4_port_reset(int unit)
{
    ...
    SOC_BLOCK_ITER(unit, blk, SOC_BLK_CLPORT) {
        SOC_IF_ERROR_RETURN(soc_hurricane4_port_blk_reset(unit, blk));
    }
...
_soc_hurricane4_idb_init(int unit)
{
    ...
    rval0 = 0;
    soc_cmicx_verify_before_idb_reset(unit);
...
/*
 * Function - soc_hurricane4_gphy_tsc_disable
 * Purpose  - Except gphy other port blocks are Port Macros and flex enabled.
 *            Hence can be added in run time, do not disable those blocks
 *            even if not present in port config loaded. Disable only when
 *            they are inactive through OTP
 */
STATIC int
soc_hurricane4_gphy_tsc_disable(int unit)
{
    uint32 rval = 0;
    int phy_port = 0, port = 0, blk = 0;
    /* GPHY_0_ENABLEf                 - phy port 1-4
     * GPHY_1_ENABLEf                 - phy_port 5-8
     * GPHY_2_ENABLEf                 - phy_port 9-12
     * GPHY_3_ENABLEf                 - phy_port 14-16
     * GPHY_4_ENABLEf                 - phy_port 17-20
     * GPHY_5_ENABLEf                 - phy_port 21-24
     * TSC_0_ENABLEf(PM4x10Q0)        - phy_port 25-40
     * TSC_1_ENABLEf(PM4x10Q1)        - phy_port 41-56
     * TSC_2_ENABLEf(PM4x25)(Falcon)  - phy_port 57-60
     * TSC_3_ENABLEf(PM4x10HG)(Eagle) - phy_port 61-64
     */
    soc_field_t reg_field[10] = {
        GPHY_0_ENABLEf, GPHY_1_ENABLEf,
        GPHY_2_ENABLEf, GPHY_3_ENABLEf,
        GPHY_4_ENABLEf, GPHY_5_ENABLEf,
        TSC_0_ENABLEf, TSC_1_ENABLEf,
        TSC_2_ENABLEf, TSC_3_ENABLEf
    };
    uint8 pblk[10] = {0};
    soc_info_t *si = &SOC_INFO(unit);
    ...
    for (phy_port = 1;
         phy_port <= _HR4_TDM_GDEV_PORTS_PER_PIPE; phy_port++) {
        port = si->port_p2l_mapping[phy_port];
        if (port != -1) {
            if ((phy_port >= 1) && (phy_port <= 4)) {
                pblk[0] = 1;
            } else if ((phy_port >= 5) && (phy_port <= 8)) {
                pblk[1] = 1;
            } else if ((phy_port >= 9) && (phy_port <= 12)) {
                pblk[2] = 1;
            } else if ((phy_port >= 13) && (phy_port <= 16)) {
                pblk[3] = 1;
            } else if ((phy_port >= 17) && (phy_port <= 20)) {
                pblk[4] = 1;
            } else if ((phy_port >= 21) && (phy_port <= 24)) {
                pblk[5] = 1;
            } else if ((phy_port >= 25) && (phy_port <= 40)) {
                pblk[6] = 1;
            } else if ((phy_port >= 41) && (phy_port <= 56)) {
                pblk[7] = 1;
    ...
    for (blk = 0; blk < 6; blk++) {
        if (pblk[blk] == 0) {
            /* gphy block is not enabled, disable it */
            soc_reg_field_set(unit, TOP_TSC_ENABLEr, &rval, reg_field[blk], 0);
        } else {
            soc_reg_field_set(unit, TOP_TSC_ENABLEr, &rval, reg_field[blk], 1);
        }
    }

    /* PM4x10Q0, PM4x10Q1 ports */
    for (blk = 6; blk < 8; blk++) {
        if (pblk[blk] == 0) {
            /* Not present in port config but disable only if OTP allows it */
            if (soc_feature(unit, soc_feature_untethered_otp)) {
                if (soc_feature(unit, soc_feature_PM4x10QInst0Disable) &&
                                                                (blk == 6)) {
                    soc_reg_field_set(unit, TOP_TSC_ENABLEr, &rval,
                                                            reg_field[blk], 0);
                }
                if (soc_feature(unit, soc_feature_PM4x10QInst0Disable) &&
                                                                (blk == 7)) {
    ...
    /* PM4x25 Falcon and PM4x10HG Eagle does not have OTP control
     * to disable and hence remains untouched */
...
#define _SOC_HR4_DEF_TEMP_MAX   110
    ...
    rval = 0;
    clk_nx_factor = SOC_BOND_INFO(unit)->clk_to_pm_clk_factor;
    clk_nx_factor = soc_property_get(unit,
                            spn_CORE_CLOCK_TO_PM_CLOCK_FACTOR, clk_nx_factor);
    if ((clk_nx_factor > 0) && (clk_nx_factor < 3) &&
        ...
        si->clk_to_pm_clk_factor = clk_nx_factor;
    } else {
        si->clk_to_pm_clk_factor = 1;
    }
    ...
    if (soc_property_get(unit, spn_IPEP_CLOCK_GATING_DISABLE, 0)) {
        /*Need to explicitly reset these registers since we bypass
         * DMU_CRU_RESET programming during chip reset*/
        SOC_IF_ERROR_RETURN(WRITE_TOP_CLOCKING_ENFORCE_PSGr(unit, 0x3fffff));
        SOC_IF_ERROR_RETURN(WRITE_TOP_CLOCKING_ENFORCE_PCGr(unit, 0xfffff));
    } else {
    ...
    if (soc_feature(unit, soc_feature_xflow_macsec)) {
        /* bring MACSEC block out of reset only when feature enabled */
    ...
    if (soc_property_get(unit, "disable_unused_port_blocks", TRUE)) {
        SOC_IF_ERROR_RETURN(soc_hurricane4_gphy_tsc_disable(unit));
    }
...
/*
 * Function:
 *     _soc_hr4_gphy_war_enable
 * Purpose:
 *     For HR4 GPHY WAR, a GPIO interrupt is enabled to notify M0.0 reset all GPHY.
 *       - "hr4_gphy_reset_gpio" vary from 0 ~ 31 to specify which GPIO is used for WAR
 *         according to the customer board design.
 * Notes:
 *      Called from misc init
 */
STATIC int
_soc_hr4_gphy_war_enable(int unit)
{

    uint32 rval;
    uint8 gpio;
    CMICX_LED_WAR_DATAr_t cmicx_led_war_data;

    SOC_IF_ERROR_RETURN
        (READ_CMICX_LED_WAR_DATAr(SOC_CONTROL(unit)->led_shmem,
                                  &cmicx_led_war_data));

    gpio = soc_property_get(unit, "hr4_gphy_reset_gpio", -1);
    if (gpio < 32) {
        /* Config GPIO into input mode. */
        SOC_IF_ERROR_RETURN(READ_IPROCPERIPH_GPIO_GP_OUT_ENr(unit, &rval));
        rval &= ~(1 << gpio);
        SOC_IF_ERROR_RETURN(WRITE_IPROCPERIPH_GPIO_GP_OUT_ENr(unit, rval));

        /* Config GPIO into dual edge trigger. */
        SOC_IF_ERROR_RETURN(READ_IPROCPERIPH_GPIO_GP_INT_TYPEr(unit, &rval));
        rval &= ~(1 << gpio);
        SOC_IF_ERROR_RETURN(WRITE_IPROCPERIPH_GPIO_GP_INT_TYPEr(unit, rval));
        SOC_IF_ERROR_RETURN(READ_IPROCPERIPH_GPIO_GP_INT_DEr(unit, &rval));
        rval |= (1 << gpio);
        SOC_IF_ERROR_RETURN(WRITE_IPROCPERIPH_GPIO_GP_INT_DEr(unit, rval));

        /* Clear interrupt status. */
        rval = (1 << gpio);
        SOC_IF_ERROR_RETURN(WRITE_IPROCPERIPH_GPIO_GP_INT_CLRr(unit, rval));

        /* Enable interrupt mask. */
        SOC_IF_ERROR_RETURN(READ_IPROCPERIPH_GPIO_GP_INT_MSKr(unit, &rval));
        rval |= (1 << gpio);
        SOC_IF_ERROR_RETURN(WRITE_IPROCPERIPH_GPIO_GP_INT_MSKr(unit, rval));

        CMICX_LED_WAR_DATAr_BYTE_0f_SET(cmicx_led_war_data, (0x80 | gpio));
    } else {
        CMICX_LED_WAR_DATAr_BYTE_0f_SET(cmicx_led_war_data, 0);
    }

    SOC_IF_ERROR_RETURN
        (WRITE_CMICX_LED_WAR_DATAr(SOC_CONTROL(unit)->led_shmem,
                                   cmicx_led_war_data));
...
_soc_hr4_ledup_init(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(_soc_hr4_gphy_war_enable(unit));
...
_soc_hurricane4_misc_init(int unit)
{
    ...
    SOC_IF_ERROR_RETURN(soc_trident3_egr_ports_min_pkt_size_set(unit));
                             ...
                             int lossless)
{
    ...
    if (soc_feature(unit, soc_feature_td3x1_reduced_mmu_internal_buffer)) {
        devcfg->mmu_total_cell = _TD3X1_MMU_TOTAL_CELLS_PER_XPE;
    } else if (soc_feature(unit, soc_feature_wh3_reduced_mmu_internal_buffer)) {
        devcfg->mmu_total_cell = _WH3_MMU_TOTAL_CELLS_PER_XPE;
    } else {
...
#ifdef INCLUDE_XFLOW_MACSEC
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if ((port == 67) || (port == 68)) {
                        soc_mem_field32_set(unit, mem0, entry0, Q_LIMIT_ENABLEf, 0);
                        soc_mem_field32_set(unit, mem0, entry0, Q_SPIDf, 0x3);
                    }
                }
#endif
            ...
            soc_mem_field32_set(unit, mem2, entry0, Q_SPIDf, 0);
                ...
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if ((port == 67) || (port == 68)) {
                        ...
                        soc_mem_field32_set(unit, mem2, entry0, Q_SPIDf, 0x3);
                    }
                }
#endif
            SOC_IF_ERROR_RETURN
...
#ifdef INCLUDE_XFLOW_MACSEC
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((port == 67) || (port == 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, entry0, SHARED_LIMIT_ENABLEf, 0);
                    }
                }
#endif
        ...
        for (idx = 0; idx < numq; idx++) {
            ...
            if (buf_queue->pool_scale != -1) {
                ...
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((port != 67) && (port != 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, entry0, Q_SHARED_ALPHA_CELLf,
                                (buf_queue->pool_scale - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE));
                    }
                }
#endif
            } else {
                ...
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((port != 67) && (port != 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, entry0, Q_SHARED_LIMIT_CELLf,
                                (buf_queue->pool_limit - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE));
                    }
                }
#endif
            }
            ...
            if ((buf_queue->yellow_limit & _MMU_CFG_BUF_DYNAMIC_FLAG) ||
                ...
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((port != 67) && (port != 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, entry0, LIMIT_YELLOW_CELLf,
                                (buf_queue->yellow_limit - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE) & ~_MMU_CFG_BUF_DYNAMIC_FLAG);
                        soc_mem_field32_set(unit, mem0, entry0, LIMIT_RED_CELLf,
                                (buf_queue->red_limit - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE) & ~_MMU_CFG_BUF_DYNAMIC_FLAG);
                    }
                }
#endif
            } else {
                ...
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((port != 67) && (port != 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, entry0, LIMIT_YELLOW_CELLf,
                                sal_ceil_func((buf_queue->yellow_limit - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE), 8));
                        soc_mem_field32_set(unit, mem0, entry0, LIMIT_RED_CELLf,
                                sal_ceil_func((buf_queue->red_limit - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE), 8));
                    }
                }
#endif
            }
                               ...
                               _soc_hr4_mmu_sw_config_info_t *swcfg)
{
    ...
    if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war) &&
            (idx == 0) && (lossless)) {
        limit -= _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE;
    }
#endif
...
#ifdef INCLUDE_XFLOW_MACSEC
        if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war) &&
                ((idx != 0x3 ) && (buf_pool->size & ~_MMU_CFG_BUF_PERCENT_FLAG) == 0)) {
            continue;
        } else
#endif
        if ((buf_pool->size & ~_MMU_CFG_BUF_PERCENT_FLAG) == 0) {
        ...
        for (idx = 0; idx < SOC_HR4_MMU_CFG_QGROUP_MAX; idx++) {
            ...
            if (queue_grp->pool_scale != -1) {
...
#ifdef INCLUDE_XFLOW_MACSEC
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((idx != 67) && (idx != 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, &cfg_qgrp,
                                Q_SHARED_ALPHA_CELLf,
                                (queue_grp->pool_scale - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE));
                    }
                }
#endif
            } else {
                ...
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((idx != 67) && (idx != 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, &cfg_qgrp,
                                Q_SHARED_LIMIT_CELLf,
                                (queue_grp->pool_limit - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE));
                    }
                }
#endif
            }
            ...
            if (queue_grp->red_limit & _MMU_CFG_BUF_DYNAMIC_FLAG) {
                ...
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((idx != 67) && (idx != 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, &cfg_qgrp,
                                LIMIT_RED_CELLf, sal_ceil_func((pval - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE), 8));
                    }
                }
#endif
            } else {
                ...
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((idx != 67) && (idx != 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, &cfg_qgrp,
                                LIMIT_RED_CELLf,
                                sal_ceil_func((queue_grp->red_limit - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE), 8));
                    }
                }
#endif
            }
            ...
            if (queue_grp->yellow_limit & _MMU_CFG_BUF_DYNAMIC_FLAG) {
                ...
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((idx != 67) && (idx != 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, &cfg_qgrp,
                                LIMIT_YELLOW_CELLf, sal_ceil_func((pval - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE), 8));
                    }
                }
#endif
                ...
                if (soc_feature(unit, soc_feature_xflow_macsec_sa_table_corruption_war)) {
                    if (((idx != 67) && (idx != 68)) && lossless) {
                        soc_mem_field32_set(unit, mem0, &cfg_qgrp,
                                LIMIT_YELLOW_CELLf,
                                sal_ceil_func((queue_grp->yellow_limit - _HR4_MMU_ISEC_CELLS_RSVD_PER_XPE), 8));
                    }
                }
#endif
            }
...
int soc_hr4_cosq_threshold_dynamic_change_check(
    int unit,
    int new_threshold,
    soc_reg_t ctr_reg,
    ...
    uint8 *allowed)
{
    ...
    uint64 val64s[_HR4_XPES_PER_DEV];
    ...
    *allowed = FALSE;
    soc_timeout_init(&timeout, SOC_MMU_THRESHOLD_DYNAMIC_CHECK_TIMEOUT, 1);
     do {
        ...
        for (index = 0; index < _HR4_XPES_PER_DEV; index++) {
    ...
    soc_switch_temperature_monitor_t *temperature_array, int *temperature_count)
{
    ...
    int index;
    uint32 rval;
    uint8 skip_avs = 1; /* TOP_PVTMON_RESULT_6r is used for AVS,skip the same */
    int fval, cur, peak;
    int num_entries_out;
    int iter_count = 2;
    ...
    while (iter_count--) {
        ...
        for (index = 0; index < num_entries_out; index++) {
            ...
            if (skip_avs && (reg == TOP_PVTMON_RESULT_6r)) {
                continue;
            }
...
soc_hr4_support_speeds(int unit, int port, uint32 *speed_mask)
{
    ...
    case 1:
        ...
        if (HR4_PORT_IS_MACSEC(unit, phy_port)) {
            if (soc_feature(unit, soc_feature_td3x1_macsec_port_speed)) {
                speed_valid = SOC_PA_SPEED_10GB | SOC_PA_SPEED_12P5GB |
                              SOC_PA_SPEED_20GB | SOC_PA_SPEED_25GB;
            } else {
                speed_valid = SOC_PA_SPEED_10GB |
                              SOC_PA_SPEED_20GB | SOC_PA_SPEED_25GB;
            }
        } else {
...
int _soc_hr4_reg_skip(int unit, soc_reg_t reg)
{
    ...
    /* Should not access this register when clock gating is enabled.
     * Skip the register.
     */
    if (!(soc_property_get(unit, spn_IPEP_CLOCK_GATING_DISABLE, 0))) {
        if (reg == CLK2X_PHASE_INVERSION_DEBUG_CONTROLr ||
            reg == CLK2X_PHASE_INVERSION_DEBUG_CONTROL_PIPE0r) {
            return TRUE;
        }
    }
    if (!soc_feature(unit, soc_feature_xflow_macsec)) {
...
/*
 * Function:
 *      soc_hr4_xflow_macse_svtag_dot1p_map_add
 * Purpose:
 *      Map the egress packet's internal priority and color
 *      to SVTAG's 'Priority' and color.
 * Parameters:
 *      unit      - (IN) Device Number
 *      map       - (IN) qos map
 * Returns:
...
int soc_hr4_xflow_macse_svtag_dot1p_map_add(int unit,
                                            bcm_qos_map_t *map)
{
    soc_mem_t mem;
    int index;
    ...
    /* EGR_ZONE_3_DOT1P_MAPPING_TABLE_1m table is indexed with
     * internal priority - 4 bits [5:2] and CNG - 2 bits[1:0]
     */
    index = ((map->int_pri << 2) | map->color) & soc_mem_index_max(unit, mem);

    sal_memset(tab_ent, 0, SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32));
    soc_mem_field32_set(unit, mem, tab_ent, PRIf, map->etag_pcp);
    soc_mem_field32_set(unit, mem, tab_ent, CFIf, map->etag_de);
...
/*
 * Function:
 *      soc_hr4_xflow_macse_svtag_dot1p_map_delete
 * Purpose:
 *      Delete the map the egress packet's
 *      internal priority and color to SVTAG's Priority and color.
 * Parameters:
 *      unit      - (IN) Device Number
 *      map       - (IN) qos map
 * Returns:
 ...
 */
int
soc_hr4_xflow_macse_svtag_dot1p_map_delete(int unit,
                                           bcm_qos_map_t *map)
{
    soc_mem_t mem;
    int index;
    ...
    sal_memset(tab_ent, 0, SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32));
    soc_mem_field32_set(unit, mem, tab_ent, PRIf, 0);
    soc_mem_field32_set(unit, mem, tab_ent, CFIf, 0);
    SOC_IF_ERROR_RETURN(soc_mem_write(unit, mem, MEM_BLOCK_ALL, index,
                       tab_ent));
...
static void
soc_hr4_sec_sa_table_mem_write_special_cb(int unit, int status, sbusdma_desc_handle_t handle, void *data)
{
    struct cb_vals_s {
        int         done;
        int         rv;
...
int
soc_hr4_sec_sa_table_mem_write_special(int unit, uint32 flags, soc_mem_t mem_orig,
        unsigned array_index, int copyno, int index, void *entry_data)
{
    const int               num_ops = 10;
    static int              pp = 0;

    volatile struct cb_vals_s {
        int         done;
        int         rv;
    }                       cb_vals;
    ...
    soc_reg_t               reg;
    uint8                   at;
    int                     p0_mmu_req_idx, p1_mmu_req_idx;
    int                     p0_port_req_idx, p1_port_req_idx;
    int                     p0_macsec_idx = 0, p1_macsec_idx = 0;
    int                     p0_init_cnt, p1_init_cnt;
    int                     p0_post_cnt, p1_post_cnt;
    int                     rv;
    soc_timeout_t           timeout;
    uint32                  ssram_data_addr = 0;
    uint32                  temp_addr = 0;
    int                     i = 0;
    int                     mmu_port[2] = {0};
    int                     macsec_port[2] = {0};
    ...
    cb_vals.done = 0;
    cb_vals.rv = SOC_E_NONE;

    cfg = sal_alloc(num_ops * sizeof(soc_sbusdma_desc_cfg_t), "hr4_war_sbusdma_cfg");
    if (!cfg) {
        return SOC_E_MEMORY;
    }
    sal_memset(cfg, 0, num_ops * sizeof(soc_sbusdma_desc_cfg_t));

    data_buf = sal_alloc(1024, "hr4_war_sbusdma_data");
    if (!data_buf) {
        sal_free(cfg);
        return SOC_E_MEMORY;
    }
    sal_memset(data_buf, 0, 1024);

    /* Using Shared SRAM for storing config and data descriptors.*/
    ssram_data_addr = soc_iproc_m0ssq_sram_alloc(unit, 1024, "SocHr4MacsecWarData");
    if (!ssram_data_addr) {
        sal_free(data_buf);
        sal_free(cfg);
        return SOC_E_MEMORY;
    ...
    if (mem_orig == ESEC_SA_TABLEm) {
        mmu_port[0] = 65;
        mmu_port[1] = 66;
        macsec_port[0] = 65;
        macsec_port[1] = 67;
        stall_ports = 2;
    } else if (mem_orig == ISEC_SA_TABLEm) {
        mmu_port[0] = 67 + pp;
        macsec_port[0] = 66 + (2 * pp);
        stall_ports = 1;
        pp ^= 1; /* ping pong */
    } else {
        sal_free(data_buf);
    ...
    /* Schan reg read from MMU_THDM_DB_PORT_TOTAL_COUNT for both ports */
    reg = MMU_THDM_DB_PORT_TOTAL_COUNTr;
    cfg[ci].flags = SOC_SBUSDMA_CFG_PREFETCH_ENB;
    cfg[ci].addr = soc_reg_addr_get(unit, reg, mmu_port[0], 0, SOC_REG_ADDR_OPTION_NONE, (int *) &cfg[ci].blk, &at);
    cfg[ci].acc_type = at;
    cfg[ci].width = SOC_REG_IS_64(unit, reg) ? 2 : 1;
    cfg[ci].count = 2;
    p0_init_cnt = di;
    p1_init_cnt = di + cfg[ci].width;
    di += cfg[ci].width * cfg[ci].count;
    ci++;

    /* Timing critical SBUSDMA descriptors start here */
    /* Schan mem write to MMU_MTRO_EGRMETERINGCONFIG_MEM_PIPE0 to stall both ports */
    mem = MMU_MTRO_EGRMETERINGCONFIG_MEM_PIPE0m;
    cfg[ci].flags = SOC_SBUSDMA_WRITE_CMD_MSG | SOC_SBUSDMA_MEMORY_CMD_MSG | SOC_SBUSDMA_CFG_PREFETCH_ENB;
    cfg[ci].blk = SOC_MEM_BLOCK_ANY(unit, mem);
    cfg[ci].addr = soc_mem_addr_get(unit, mem, 0, cfg[ci].blk, mmu_port[0], &at);
    cfg[ci].acc_type = at;
    cfg[ci].blk = SOC_BLOCK2SCH(unit, cfg[ci].blk);
    cfg[ci].width = soc_mem_entry_words(unit, mem);
    cfg[ci].count = stall_ports;
    soc_mem_field32_set(unit, mem, &(data_buf[di]), THD_SELf, 1);  /* mmu_port[0] */
    di += cfg[ci].width;
    if (stall_ports > 1) {
        soc_mem_field32_set(unit, mem, &(data_buf[di]), THD_SELf, 1);  /* mmu_port[1] */
        di += cfg[ci].width;
    }
    ci++;

    /* Read bogus entries before the real read just to give additional time to flush */
    mem = EGR_MMU_REQUESTSm;
    cfg[ci].flags = SOC_SBUSDMA_MEMORY_CMD_MSG | SOC_SBUSDMA_CFG_PREFETCH_ENB;
    cfg[ci].blk = SOC_MEM_BLOCK_ANY(unit, mem);
    cfg[ci].addr = soc_mem_addr_get(unit, mem, 0, cfg[ci].blk, 0, &at);
    cfg[ci].acc_type = at;
    cfg[ci].blk = SOC_BLOCK2SCH(unit, cfg[ci].blk);
    cfg[ci].width = soc_mem_entry_words(unit, mem);
    cfg[ci].count = soc_property_get(unit, "hr4_macsec_war_delay", 7);
    if (cfg[ci].count < 1) {
        cfg[ci].count = 1;
    } else if (cfg[ci].count > 71) {
        cfg[ci].count = 71;
    }
    di += cfg[ci].width * cfg[ci].count;
    ci++;

    /* Schan mem read from EGR_MMU_REQUESTS for both ports to see if credits have been returned to MMU */
    cfg[ci].flags = SOC_SBUSDMA_MEMORY_CMD_MSG | SOC_SBUSDMA_CFG_PREFETCH_ENB;
    cfg[ci].blk = SOC_MEM_BLOCK_ANY(unit, mem);
    cfg[ci].addr = soc_mem_addr_get(unit, mem, 0, cfg[ci].blk, mmu_port[0], &at);
    cfg[ci].acc_type = at;
    ...
    cfg[ci].count = 2;
    p0_mmu_req_idx = di;
    p1_mmu_req_idx = di + cfg[ci].width;
    di += cfg[ci].width * cfg[ci].count;
    ci++;

    /* Schan mem read from EGR_PORT_REQUESTS for both ports to see if credits have been returned to Edatabuf */
    mem = EGR_PORT_REQUESTSm;
    cfg[ci].flags = SOC_SBUSDMA_MEMORY_CMD_MSG | SOC_SBUSDMA_CFG_PREFETCH_ENB;
    cfg[ci].addr = soc_mem_addr_get(unit, mem, 0, cfg[ci].blk, mmu_port[0], &at);
    cfg[ci].blk = SOC_MEM_BLOCK_ANY(unit, mem);
    cfg[ci].acc_type = at;
    ...
    cfg[ci].count = 2;
    p0_port_req_idx = di;
    p1_port_req_idx = di + cfg[ci].width;
    di += cfg[ci].width * cfg[ci].count;
    ci++;

    if (mem_orig == ESEC_SA_TABLEm) {
        /* Schan reg read from ESEC_PKTBUF_STS to see if MACSEC encrypt engines are empty */
        reg = ESEC_EPBUF_STSr;
    } else {
        /* Schan reg read from ISEC_PKTBUF_STS to see if MACSEC decrypt engine is empty */
        reg = ISEC_PKTBUF_STSr;
    }
    cfg[ci].flags = SOC_SBUSDMA_CFG_PREFETCH_ENB;
    cfg[ci].addr = soc_reg_addr_get(unit, reg, macsec_port[0], 0, SOC_REG_ADDR_OPTION_NONE, (int *) &cfg[ci].blk, &at);
    cfg[ci].acc_type = at;
    cfg[ci].width = SOC_REG_IS_64(unit, reg) ? 2 : 1;
    cfg[ci].count = 1;
    p0_macsec_idx = di;
    ...
    if (mem_orig == ESEC_SA_TABLEm) {
        cfg[ci].flags = SOC_SBUSDMA_CFG_PREFETCH_ENB;
        cfg[ci].addr = soc_reg_addr_get(unit, reg, macsec_port[1], 0, SOC_REG_ADDR_OPTION_NONE, (int *) &cfg[ci].blk, &at);
        cfg[ci].acc_type = at;
        cfg[ci].width = SOC_REG_IS_64(unit, reg) ? 2 : 1;
        cfg[ci].count = 1;
        p1_macsec_idx = di;
    ...
    /* Schan mem write to "mem_orig" with desired data */
    mem = mem_orig;
    cfg[ci].flags = SOC_SBUSDMA_WRITE_CMD_MSG | SOC_SBUSDMA_MEMORY_CMD_MSG | SOC_SBUSDMA_CFG_PREFETCH_ENB;
    cfg[ci].blk = SOC_MEM_BLOCK_ANY(unit, mem);
    cfg[ci].addr = soc_mem_addr_get(unit, mem, 0, cfg[ci].blk, index, &at);
    cfg[ci].acc_type = at;
    cfg[ci].blk = SOC_BLOCK2SCH(unit, cfg[ci].blk);
    cfg[ci].width = soc_mem_entry_words(unit, mem);
    cfg[ci].count = 1;
    sal_memcpy(&(data_buf[di]), entry_data, cfg[ci].width * 4);
    di += cfg[ci].width;
    ci++;

    /* Schan mem write to MMU_MTRO_EGRMETERINGCONFIG_MEM_PIPE0 to un-stall both ports */
    mem = MMU_MTRO_EGRMETERINGCONFIG_MEM_PIPE0m;
    cfg[ci].flags = SOC_SBUSDMA_WRITE_CMD_MSG | SOC_SBUSDMA_MEMORY_CMD_MSG | SOC_SBUSDMA_CFG_PREFETCH_ENB;
    cfg[ci].blk = SOC_MEM_BLOCK_ANY(unit, mem);
    cfg[ci].addr = soc_mem_addr_get(unit, mem, 0, cfg[ci].blk, mmu_port[0], &at);
    ...
    cfg[ci].width = soc_mem_entry_words(unit, mem);
    cfg[ci].count = 2;
    soc_mem_field32_set(unit, mem, &(data_buf[di]), THD_SELf, 0);  /* mmu_port[0] */
    di += cfg[ci].width;
    soc_mem_field32_set(unit, mem, &(data_buf[di]), THD_SELf, 0);  /* mmu_port[1] */
    di += cfg[ci].width;
    ci++;
    /* Timing critical SBUSDMA descriptors end here */
    ...
    cfg[ci].count = 2;
    p0_post_cnt = di;
    p1_post_cnt = di + cfg[ci].width;
    di += cfg[ci].width * cfg[ci].count;
    ci++;

    /* Place the data descriptor in Shared SRAM */
    temp_addr = ssram_data_addr;
    for (i = 0 ; i < 256 ; i++) {
        soc_cm_iproc_write (unit, temp_addr, data_buf[i]);
        temp_addr += 4;
    }

    /* Configure the main SBUS DMA control structure */
    ctrl.flags = SOC_SBUSDMA_CFG_USE_FLAGS | SOC_SBUSDMA_CFG_PREFETCH_ENB;
    ctrl.cfg_count = ci;
    ctrl.flags |= SOC_SBUSDMA_CFG_SSRAM_MEMORY;
    ctrl.buff = INT_TO_PTR(ssram_data_addr);
    ctrl.cb = soc_hr4_sec_sa_table_mem_write_special_cb;
    ctrl.data = (void *) &cb_vals;

    /* Create the descriptor chain */
    rv = soc_sbusdma_desc_create(unit, &ctrl, cfg, TRUE, &desc_handle);
    if (SOC_FAILURE(rv)) {
        sal_free(cfg);
        sal_free(data_buf);
        return rv;
    ...
    /* Let it rip */
    soc_timeout_init(&timeout, 100000, 0);
    do {
        rv = soc_sbusdma_desc_start(unit, desc_handle);
        if ((rv == SOC_E_BUSY) || (rv == SOC_E_INIT)) {
            if (rv == SOC_E_INIT) {
                break;
            }
            if (soc_timeout_check(&timeout)) {
                LOG_WARN(BSL_LS_APPL_TESTS,
                        (BSL_META_U(unit,
                                    "HR4 Shared SRAM SBUS DMA write workaround timed out\n")));
                break;
            }
            sal_usleep(10);
        }
    ...
    /* Read the data from Shared SRAM */
    temp_addr = ssram_data_addr;
    sal_memset(data_buf, 0, 1024);
    for (i = 0 ; i < 256 ; i++) {
        data_buf[i] = soc_cm_iproc_read (unit, temp_addr);
        temp_addr += 4;
    }

    /* Check that the all read values are ok */
    mem = EGR_MMU_REQUESTSm;
    if (soc_mem_field32_get(unit, mem, &(data_buf[p0_mmu_req_idx]),
                OUTSTANDING_MMU_REQUESTSf) != 0x20) {
        LOG_WARN(BSL_LS_SOC_DMA,
                (BSL_META_U(unit, "MMU credit check failed for first port during MACSEC write workaround\n")));
    }
    if (stall_ports > 1 && soc_mem_field32_get(unit, mem, &(data_buf[p1_mmu_req_idx]),
                OUTSTANDING_MMU_REQUESTSf) != 0x20) {
        LOG_WARN(BSL_LS_SOC_DMA,
                (BSL_META_U(unit, "MMU credit check failed for second port during MACSEC write workaround\n")));
    }

    mem = EGR_PORT_REQUESTSm;
    if (soc_mem_field32_get(unit, mem, &(data_buf[p0_port_req_idx]),
                OUTSTANDING_PORT_REQUESTSf) != 0x10) {
        LOG_WARN(BSL_LS_SOC_DMA,
                (BSL_META_U(unit, "Port credit check failed for first port during MACSEC write workaround\n")));
    }
    if (stall_ports > 1 && soc_mem_field32_get(unit, mem, &(data_buf[p1_port_req_idx]),
                OUTSTANDING_PORT_REQUESTSf) != 0x10) {
        LOG_WARN(BSL_LS_SOC_DMA,
                (BSL_META_U(unit, "Port credit check failed for second port during MACSEC write workaround\n")));
    }

    if (mem_orig == ESEC_SA_TABLEm) {
        reg = ESEC_EPBUF_STSr;
        if (!soc_reg_field_get(unit, reg, data_buf[p0_macsec_idx], EPBUF_EMPTYf)) {
            LOG_WARN(BSL_LS_SOC_DMA,
                    (BSL_META_U(unit, "Encrypt pipeline empty check failed for first port during MACSEC write workaround\n")));
        }
        if (!soc_reg_field_get(unit, reg, data_buf[p1_macsec_idx], EPBUF_EMPTYf)) {
            LOG_WARN(BSL_LS_SOC_DMA,
                    (BSL_META_U(unit, "Encrypt pipeline empty check failed for second port during MACSEC write workaround\n")));
        }
    } else {
        reg = ISEC_PKTBUF_STSr;
        if (!soc_reg_field_get(unit, reg, data_buf[p0_macsec_idx], EP_EMPTYf)) {
            LOG_WARN(BSL_LS_SOC_DMA,
                    (BSL_META_U(unit, "Decrypt pipeline empty check failed for first port during MACSEC write workaround\n")));
        }
    }

    LOG_VERBOSE(BSL_LS_SOC_OTHER,
            (BSL_META_U(unit, "MACSEC write workaround Init/Post counts: Port0: %d/%d, Port1: %d/%d\n"),
             data_buf[p0_init_cnt], data_buf[p0_post_cnt],
             data_buf[p1_init_cnt], data_buf[p1_post_cnt]));

    sal_free(cfg);
    sal_free(data_buf);
    soc_iproc_m0ssq_sram_free(unit, ssram_data_addr);
    SOC_IF_ERROR_RETURN(soc_sbusdma_desc_delete(unit, desc_handle));
