...
typedef struct tsc_dpll_refclk_connect
{
    ...
    int pll1_lc_refclk_select;   /* 0: select pll0_lc_refclk; 1: select pll1_lc_refclk.
                                       These bit definiton are for SW. HW bit definitions are different. */
...
_soc_monterey_idb_port_init(int unit, int reset, int lossless, soc_port_t port)
{
    ...
    static const soc_reg_t obm_express_fc_config_regs[] = {
        IDB_OBM0_EXPRESS_FLOW_CONTROL_CONFIGr, IDB_OBM1_EXPRESS_FLOW_CONTROL_CONFIGr,
        IDB_OBM2_EXPRESS_FLOW_CONTROL_CONFIGr, IDB_OBM3_EXPRESS_FLOW_CONTROL_CONFIGr,
        IDB_OBM4_EXPRESS_FLOW_CONTROL_CONFIGr, IDB_OBM5_EXPRESS_FLOW_CONTROL_CONFIGr,
        IDB_OBM6_EXPRESS_FLOW_CONTROL_CONFIGr, IDB_OBM7_EXPRESS_FLOW_CONTROL_CONFIGr
    };
    ...
    */
    ...
    if (si->port_speed_max[port] <= 11000) {
            ...
            if (lossless &&  SOC_PBMP_MEMBER(si->pbmp_mmu_lossless, port)) {
               lossy_low_pri =  100;
               lossy_discard =  196;
               express_discard = 0;
               port_xon = 238;
    ...
    if (lossless && (si->lossless_headroom_reserve != 0)) {
        if (SOC_PBMP_MEMBER(si->pbmp_mmu_lossless, port)) {
            SOC_IF_ERROR_RETURN(soc_reg_get(unit, reg, pgw_inst, subport, &rval64));
            soc_reg64_field32_set(unit, reg, &rval64, XOFF_REFRESH_TIMERf, 256);
            soc_reg64_field32_set(unit, reg, &rval64, PORT_FC_ENf, 1 );
            soc_reg64_field32_set(unit, reg, &rval64, FC_TYPEf, 1 );
    ...
    COMPILER_64_ZERO(rval64);
    if (lossless && (si->lossless_headroom_reserve !=0) &&
                   SOC_PBMP_MEMBER(si->pbmp_mmu_lossless, port)) {
        reg = obm_express_fc_config_regs[obm];
        SOC_IF_ERROR_RETURN(soc_reg_get(unit, reg, pgw_inst, subport, &rval64));
        soc_reg64_field32_set(unit, reg, &rval64, EXPRESS_FC_ENf,
             1);
        SOC_IF_ERROR_RETURN(soc_reg_set(unit, reg, pgw_inst, subport, rval64));
    }
    COMPILER_64_ZERO(rval64);
            ...
            for (index = 0; index < COUNTOF(offset_ob_prio); index++) {
                 if (index == 1 || index == 2) {
                     soc_mem_field32_set(unit, mem, &entry,
                                offset_ob_prio[index], 2);
                 } else if ( index == 3 ) {
                     soc_mem_field32_set(unit, mem, &entry,
                                offset_ob_prio[index], 1);
                 } else {
                 soc_mem_field32_set(unit, mem, &entry,
                                offset_ob_prio[index], 0);
                 }
            }
        } else {
           for (index = 0; index < COUNTOF(offset_ob_prio); index++) {
                                 ...
                                 int mem_index[][_SOC_SER_MAX_ENTRIES_PER_BKT])
{
    ...
    for (i = 0; i < COUNTOF(mem_list); i++) {
        ...
        SOC_MEM_STATE_MEMBER_GET(unit, mem_list[i].mem, cache[copyno], cache);
        SOC_MEM_STATE_MEMBER_GET(unit, mem_list[i].mem, vmap[copyno], vmap);
                                 ...
                                 uint32 stage, uint32 index)
{
    ...
    /* SER_FIXME
     *   What is stage 22 or 21?? Was 21 for TD2P. For us
     * it was 22 as found during debugging.
     */
...
_soc_monterey_process_ser_fifo (int unit, soc_block_t blk, char *prefix_str)
{
    ...
    case SOC_BLK_IPIPE:
        ...
        ser_reg_info_table = _soc_bcm56670_a0_ip_reg_ser_info;
        sal_sprintf(blk_str, "IPIPE");
        break;
    case SOC_BLK_EPIPE:
        mem = EGR_SER_FIFOm;
        ser_info_table = _soc_bcm56670_a0_ep_mem_ser_info;
        ser_reg_info_table = _soc_bcm56670_a0_ep_reg_ser_info;
...
soc_monterey_mem_config(int unit)
{
    ...
    switch (num_ecmp_rh_flowset_entries) {
        case 0:
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_ECMP_FLOWSETm, -1);
            break;
        case 512:
            SOP_MEM_STATE_MAX_INDEX_GET(unit, RH_LAG_FLOWSETm, index_max);
            index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_LAG_FLOWSETm, index_max);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, RH_ECMP_FLOWSETm, index_max);
            index_max /= 2;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_ECMP_FLOWSETm, index_max);
            break;
        case 1024:
    ...
    if (!soc_feature(unit, soc_feature_lpm_tcam)) {
        ...
        if (!soc_property_get(unit, spn_L3_ALPM_ENABLE, 0)) {
            ...
            LOG_ERROR(BSL_LS_SOC_COMMON,
                      (BSL_META_U(unit, "'%s' config is not supported on this device.\n"), spn_L3_ALPM_ENABLE));
            return SOC_E_CONFIG;
        }
...
_soc_monterey_tsc_refclk_set(int unit)
{
    ...
    for(pm = 0; pm < PM_NUM; pm++) {
        ...
        /*
         * Program the same setting to the CPRI Framer.
         */
        if ((SOC_PORT_BLOCK_TYPE(unit, phy_port) == SOC_BLK_CPRI) &&
            (monterey_tsc_is_master_pll(pm))){
            soc_cm_get_id(unit, &dev_id, &rev_id);
            /*
             * Skip 56671  SOC_MN_56671_PM4X25_PORT_5 core as it is PM4x25.
             */
            if (!((dev_id == BCM56671_DEVICE_ID) && (phy_port == 53))) {
                SOC_IF_ERROR_RETURN(soc_reg32_rawport_get(unit, TOP_CTRL_CONFIGr, phy_port, 0, &rval));

                soc_reg_field_set(unit, TOP_CTRL_CONFIGr, &rval, PLL0_REFIN_ENf, refclk_connect->pll0_refclk_select);
                soc_reg_field_set(unit, TOP_CTRL_CONFIGr, &rval, PLL1_REFIN_ENf, refclk_connect->pll1_refclk_select);

                soc_reg_field_set(unit, TOP_CTRL_CONFIGr, &rval, PLL0_REFOUT_ENf, refclk_connect->pll0_master_enable);
                soc_reg_field_set(unit, TOP_CTRL_CONFIGr, &rval, PLL1_REFOUT_ENf, refclk_connect->pll1_master_enable);

                SOC_IF_ERROR_RETURN(soc_reg32_rawport_set(unit, TOP_CTRL_CONFIGr, phy_port, 0, rval));

                SOC_IF_ERROR_RETURN(soc_reg32_rawport_get(unit, CPMPORT_PLL0_CTRL_CONFIGr, phy_port, 0, &rval));
                soc_reg_field_set(unit, CPMPORT_PLL0_CTRL_CONFIGr,
                                  &rval, LCREF_SELf,
                                  refclk_connect->pll0_lc_refclk_select);
                SOC_IF_ERROR_RETURN(soc_reg32_rawport_set(unit, CPMPORT_PLL0_CTRL_CONFIGr, phy_port, 0, rval));

                SOC_IF_ERROR_RETURN(soc_reg32_rawport_get(unit, CPMPORT_PLL1_CTRL_CONFIGr, phy_port, 0, &rval));
                soc_reg_field_set(unit, CPMPORT_PLL1_CTRL_CONFIGr,
                                  &rval, LCREF_SELf,
                                  !refclk_connect->pll1_lc_refclk_select);
                SOC_IF_ERROR_RETURN(soc_reg32_rawport_set(unit, CPMPORT_PLL1_CTRL_CONFIGr, phy_port, 0, rval));
            }
        }
...
soc_monterey_tsc_reset(int unit)
{
    ...
    PBMP_PORT_ITER(unit, port) {
        if(IS_ROE_PORT(unit, port)) {
            ...
            /*
             * Match the same PLL0 setting to the CPRI Framer.
             */
            SOC_IF_ERROR_RETURN
                (READ_CPMPORT_PLL0_CTRL_CONFIGr(unit, port, &pll0_ctrl_cfg));
            soc_reg_field_set(unit, CPMPORT_PLL0_CTRL_CONFIGr, &pll0_ctrl_cfg, LCREF_SELf, 0);
            SOC_IF_ERROR_RETURN
                (WRITE_CPMPORT_PLL0_CTRL_CONFIGr(unit, port, pll0_ctrl_cfg));
...
_soc_monterey_clear_all_port_data (int unit)
{
    ...
    /* SER_FIXME : Do these magic numbers hold good for monterey as well? */
...
_soc_monterey_default_lossless_pg_headroom(int unit, soc_port_t port)
{
    soc_info_t *si;
    si = &SOC_INFO(unit);
    if (IS_ROE_ENABLED(unit )  && (si->lossless_headroom_reserve == 0)) {
        return 0;
    }
                                ...
                                _soc_mmu_device_info_t *devcfg, int lossless)
{
    ...
    int pool0_share = 0 ;
    int pool1_share = 0;
    int headroom_reserve = 0, total_headroom =0;
    ...
    headroom_reserve = si->lossless_headroom_reserve;
    ...
    if ((lossless) &&  (headroom_reserve)) {
         per_q_guarentee = default_mtu_cells;
    }
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        if (lossless) {
            if (SOC_PBMP_MEMBER(si->pbmp_mmu_lossless, port)) {
                 q_reserved += (2) * per_q_guarentee;
            } else {
                 q_reserved += (4) * per_q_guarentee;
            }
        } else {
 ...
 */
    if (IS_ROE_ENABLED(unit)) {
            if (lossless && headroom_reserve != 0) {
                mn_mmu_express_buffer_total =  (total_pool_size - 50 - headroom_reserve)/2;
            } else {
    ...
    if (IS_ROE_ENABLED(unit)) {
        if (headroom_reserve !=0) {
            pool0_share = total_pool_size - mn_mmu_express_buffer_total - q_reserved;
        } else {
    ...
    for (idx = 0; idx < _MONTEREY_MMU_NUM_POOL; idx++) {
        ...
        if (idx == 0) {
            ...
            buf_pool->total_mcq_entry = (((double)pool0_share)/total_pool_size) *
                                              _MONTEREY_MMU_TOTAL_MCQ_ENTRY(unit);
            ...
            buf_pool->total_mcq_entry = (((double)pool1_share)/total_pool_size) *
                                              _MONTEREY_MMU_TOTAL_MCQ_ENTRY(unit);
            ...
            if (IS_ROE_ENABLED(unit)) {
                ...
                switch (idx) {
                     case 1:
                           if (lossless &&  (headroom_reserve != 0) &&
                               (SOC_PBMP_MEMBER(si->pbmp_mmu_lossless, port))) {
                               buf_port->pri_to_prigroup[idx] = 6;
                           }
                     break;
                     case 2:
                           if (lossless &&  (headroom_reserve != 0) &&
                               (SOC_PBMP_MEMBER(si->pbmp_mmu_lossless, port))) {
                               buf_port->pri_to_prigroup[idx] = 2;
                           }
                     break;
                     case 3:
                 ...
                 if (SOC_PBMP_MEMBER(si->pbmp_mmu_lossless, port)) {
                     if (idx == 2 || idx == 6)  {
                        ...
                        _soc_monterey_default_lossless_pg_headroom(unit, port);
                         buf_prigroup->guarantee = 16;
                        buf_prigroup->pool_scale = 4;
                        buf_prigroup->flow_control_enable = 1;
                        total_headroom += buf_prigroup->headroom;
                        if (total_headroom > headroom_reserve) {
                             LOG_WARN(BSL_LS_SOC_MMU,
                              (BSL_META_U(unit,
                             "pg headroom allocation exceed reserved %d\n"),
                             total_headroom));
                        }
                    }
               } else {
        ...
        for (idx = 0;
            ...
            if (IS_CPRI_PORT(unit,port) && (idx == 0 || idx ==6 || idx ==7 ||
                ...
                buf_port->queues[idx].pool_idx = 1;
                      ...
                      IS_ROE_ENABLED(unit) && ((idx == 15 || idx == 17 || idx == 5 ||
              idx == 7)  && !IS_CPU_PORT(unit, port) &&  !IS_LB_PORT(unit,port) && !IS_MACSEC_PORT(unit,port))) {
               buf_port->queues[idx].pool_idx = 1;
            } else  {
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        for (idx = 0; idx < _MONTEREY_MMU_NUM_PG; idx++) {
             buf_prigroup = &buf_port->prigroups[idx];
             if (!lossless || ( (lossless) && (headroom_reserve != 0))) {
                 if (IS_CPRI_PORT(unit,port) && (idx == 0 || idx == 6 || idx ==7 )) {
        ...
        for (idx = 0; idx < _MONTEREY_MMU_NUM_POOL; idx++) {
            ...
            if (lossless && si->lossless_headroom_reserve ==0 ) {
                ...
                if(idx == 0) {
                 ...
                 if (lossless && si->lossless_headroom_reserve !=0 ) {
                     limit = devcfg->mmu_total_cell - ((si->lossless_portmax * 2 * 16) +
                        (buf->headroom/2) +
                        buf_pool->queue_guarantee + 8675  + si->lossless_headroom_reserve );
                  } else {
    ...
    PBMP_ALL_ITER(unit, port) {
        ...
        for (idx = 0; idx < _MONTEREY_MMU_NUM_POOL; idx++) {
            ...
            soc_mem_field32_set(unit, mem2, entry0,
                 SHARED_LIMIT_ENABLEf,!lossless || (si->lossless_headroom_reserve!=0));
            ...
            soc_mem_field32_set(unit, mem2, entry0, SHARED_RESUME_LIMITf,
                    ((rlimit/8) - 1));
            soc_mem_field32_set(unit, mem2, entry0, SHARED_LIMIT_ENABLEf,
                           !lossless || (si->lossless_headroom_reserve!=0));
            soc_mem_field32_set(unit, mem2, entry0, RED_SHARED_LIMITf, (limit + 7)/8);
                ...
                if (IS_CPRI_PORT(unit,port) && ( idx == 0 || idx == 6 || idx ==7 )) {
                    ...
                    if (lossless && (si->lossless_headroom_reserve !=0) &&
                        ...
                        if (idx < 4 || (idx > 7)) {
                            ...
                            soc_mem_field32_set(unit, mem2, entry0, QGROUP_VALIDf,  0);
                            soc_mem_field32_set(unit, mem2, entry0, USE_QGROUP_MINf,0);
                        } else {
                            soc_mem_field32_set(unit, mem2, entry0, Q_LIMIT_ENABLEf, 0 ) ;
                            soc_mem_field32_set(unit, mem2, entry0, QGROUP_VALIDf,  0);
                    ...
                    soc_mem_field32_set(unit, mem2, entry0, QGROUP_VALIDf,
                                          (lossless && si->lossless_headroom_reserve == 0) ? 0 : 1);
                    soc_mem_field32_set(unit, mem2, entry0, Q_LIMIT_ENABLEf,
                                          (lossless && si->lossless_headroom_reserve == 0)  ? 0 : 1);
                    soc_mem_field32_set(unit, mem2, entry0, USE_QGROUP_MINf,
                                          (lossless && si->lossless_headroom_reserve == 0) ? 0 :1);
                }
...
soc_monterey_ser_test_skip_check (int unit, soc_mem_t mem)
{
    ...
    /* SER_FIXME
     * Learnings from past devices : Skip known trouble makers
     */
    /* Skip MPLS memories */
                             ...
                             void *d2, void *d3, void *d4)
{
    ...
    for (rbi = 0; ; rbi++) {
        ...
        if (port != -1) {
            /*
             * Do not process during warmboot process.
             */
            if (!SOC_WARM_BOOT(unit)) {
                /* Process intr if portmod is created. */
                if (portmod_was_created(unit)){
                    /* get portmod  pm_id from the logical port. */
                    rv = portmod_port_pm_id_get(unit, port, &portmod_pm_id);
                    if (rv == SOC_E_NONE) {
                ...
                LOG_VERBOSE(BSL_LS_SOC_COMMON, (BSL_META_U(unit,
                            "Skipping Intr Processing During Warmboot\n")));
                sal_msleep(500);
            }
        } /* (port != -1) */
    } /* For loop */
