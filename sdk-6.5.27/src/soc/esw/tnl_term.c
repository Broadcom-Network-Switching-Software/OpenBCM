                                ...
                                soc_tnl_term_hash_key_t *entry_key)
{
    ...
    uint8 dip_mask[_SHR_L3_IP6_ADDRLEN];     /* Destination ip subent mask.   */
    ...
    if (entry_type == ipv6_key_type) { /* IPV6 Type */
        if (soc_feature(unit, soc_feature_tunnel_term_hash_table)) {
            ...
            /* DIP MASK [0-63] */
            soc_mem_ip6_addr_get(unit, mem,
                                 (uint32 *)&entry->entry_arr[2], ipv4_maskf,
                                 dip_mask, SOC_MEM_IP6_LOWER_ONLY);
            /* DIP MASK [64-127] */
            soc_mem_ip6_addr_get(unit, mem,
                                 (uint32 *)&entry->entry_arr[3], ipv4_maskf,
                                 dip_mask, SOC_MEM_IP6_UPPER_ONLY);
            entry_key->ip_hash_key.sip_plen =  _shr_ip6_mask_length(sip_mask);
            entry_key->ip_hash_key.dip_plen =  _shr_ip6_mask_length(dip_mask);
        }
        ...
        if (SOC_MEM_FIELD_VALID(unit, mem, DIP_MASKf)) {
            soc_mem_field_get(unit, mem, entry_ptr, DIP_MASKf,
                              (uint32 *)dip_mask);
            entry_key->ip_hash_key.dip_plen =
                            _shr_ip_mask_length((*(uint32 *)dip_mask));
        } else {
            entry_key->ip_hash_key.dip_plen =
                            (*(uint32 *)entry_key->ip_hash_key.dip) ? 32 : 0;
        }
...
/*
 * Function:
 *   _soc_tunnel_term_dip_validate
 * Purpose:
 *	 Validate tunnel terminator entry DIP.
 * Parameters:
 *      unit      - (IN)  BCM device number.
 *      key       - (IN)  Tunnel terminator lookup key.
 * Returns:
 ...
 */
STATIC int
_soc_tunnel_term_dip_validate(int unit, soc_tunnel_term_t *key)
{
    int         entry_type; /* Entry type               */
    int         rv;         /* Operation return status. */
    int idx, idx_min, idx_max;
    soc_tunnel_term_t tmp_entry;
    int type;
    int entry_size;
    soc_tnl_term_hash_key_t hash_key;
    soc_tnl_term_hash_key_t tmp_key;
    uint32 ip4_mask;
    uint8 ip6_mask[_SHR_L3_IP6_ADDRLEN];
    uint8 key_ip6[_SHR_L3_IP6_ADDRLEN];
    uint8 tmp_ip6[_SHR_L3_IP6_ADDRLEN];
    int i;

    if (!soc_feature(unit, soc_feature_tunnel_term_hash_table)) {
        /* Get entry type. */
        SOC_IF_ERROR_RETURN
            (_soc_tunnel_term_type_get(unit, key, &type, &entry_type));

        if (SOC_TNL_TERM_STATE_VENT(unit, type) == 0) {
            return SOC_E_NONE;
        }

        _soc_tunnel_term_hash_entry_get(unit, key, entry_type, &hash_key);

        entry_size = (entry_type == 0x1) ? SOC_TNL_TERM_IPV6_ENTRY_WIDTH :
                                           SOC_TNL_TERM_IPV4_ENTRY_WIDTH;

        idx_min = SOC_TNL_TERM_STATE_START(unit, type);
        idx_max = SOC_TNL_TERM_STATE_END(unit, type);

        for (idx = idx_min; idx <= idx_max; idx += entry_size) {
            rv = _soc_tunnel_term_entry_read(unit, idx, &tmp_entry,
                                             &entry_type);
            if (SOC_FAILURE(rv)) {
                return (rv);
            }

            _soc_tunnel_term_hash_entry_get(unit, &tmp_entry, entry_type,
                                            &tmp_key);
            if ((hash_key.ip_hash_key.l4_src_port ==
                    tmp_key.ip_hash_key.l4_src_port) &&
                (hash_key.ip_hash_key.l4_dst_port ==
                    tmp_key.ip_hash_key.l4_dst_port) &&
                (hash_key.ip_hash_key.ip_protocol ==
                    tmp_key.ip_hash_key.ip_protocol)) {
                if (hash_key.ip_hash_key.dip_plen ==
                    tmp_key.ip_hash_key.dip_plen) {
                    continue;
                }
                ...
                if (entry_type == 1) {
                    /* IPv6 */
                    SOC_IF_ERROR_RETURN
                        (_shr_ip6_mask_create(ip6_mask,
                             hash_key.ip_hash_key.sip_plen));
                    for (i = 0; i < _SHR_L3_IP6_ADDRLEN; i++) {
                        key_ip6[i] = hash_key.ip_hash_key.sip[i] & ip6_mask[i];
                        tmp_ip6[i] = tmp_key.ip_hash_key.sip[i] & ip6_mask[i];
                        if (key_ip6[i] != tmp_ip6[i]) {
                            break;
                        }
                    }
                    if (i != _SHR_L3_IP6_ADDRLEN) {
                        /* SIP is not identical. */
                        continue;
                    }
                    if (hash_key.ip_hash_key.dip_plen >=
                            tmp_key.ip_hash_key.dip_plen) {
                        ...
                        SOC_IF_ERROR_RETURN
                            (_shr_ip6_mask_create(ip6_mask,
                                 hash_key.ip_hash_key.dip_plen));
                    }
                    for (i = 0; i < _SHR_L3_IP6_ADDRLEN; i++) {
                        key_ip6[i] = hash_key.ip_hash_key.dip[i] & ip6_mask[i];
                        tmp_ip6[i] = tmp_key.ip_hash_key.dip[i] & ip6_mask[i];
                        if (key_ip6[i] != tmp_ip6[i]) {
                            break;
                        }
                    }
                    ...
                    /* IPv4 */
                    ip4_mask =
                        _shr_ip_mask_create(tmp_key.ip_hash_key.sip_plen);
                    if ((*(uint32*)hash_key.ip_hash_key.sip & ip4_mask) !=
                        (*(uint32*)tmp_key.ip_hash_key.sip & ip4_mask)) {
                        /* SIP is not identical. */
                        continue;
                    }
                    ...
                    if (hash_key.ip_hash_key.dip_plen >=
                            tmp_key.ip_hash_key.dip_plen) {
                        ip4_mask =
                            _shr_ip_mask_create(tmp_key.ip_hash_key.dip_plen);
                    } else {
                        ip4_mask =
                            _shr_ip_mask_create(hash_key.ip_hash_key.dip_plen);
                    }
                    if ((*(uint32*)hash_key.ip_hash_key.dip & ip4_mask) ==
                        (*(uint32*)tmp_key.ip_hash_key.dip & ip4_mask)) {
                        /* DIP range conflict. */
                        return SOC_E_PARAM;
                    }
                }
...
soc_tunnel_term_insert(int unit, soc_tunnel_term_t *entry, uint32 *index)
{
    ...
    rv = _soc_tunnel_term_dip_validate(unit, entry);
    if (SOC_FAILURE(rv)) {
        SOC_TNL_TERM_UNLOCK(unit);
        return(rv);
    }
...
soc_tunnel_term_used_get(int unit)
{
    ...
    /*
     * Needs to iterate every type to collect vent information.
     *
     * _type_ of SOC_TNL_TERM_STATE_VENT(_u_, _type_) is indeed
     * tunnel header size - it is the sum of predefined header
     * size and key hash size (added if tunnel type matches during
     * allocation time). This will create a different tunnel header
     * size from predefined.
     *
     * Since this API doesn't have the key hash information, we have
     * no intelligence to calculate the tunnel header size and iterate
     * every index is the best way to derive total used_count.
     */
    for (ix = 0; ix < MAX_TNL_TERM_TYPES; ix++) {
        used_count += SOC_TNL_TERM_STATE_VENT(unit, ix);
    }
