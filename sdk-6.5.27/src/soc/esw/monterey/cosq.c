...
_soc_monterey_dump_hsp_sched_at(int unit, int port, int level, int offset, int index)
{
    ...
    uint32     ucmap =0;
    uint32     sched_mode =0,sched_mode1 = 1;
    int        uc_cosq, mc_cosq;
    ...
    for (mem_idx = 0; mem_idx < COUNTOF(shadow_mems); mem_idx++) {
        ...
        *load_addr += tbl_size;
        ...
        SOC_MEM_BLOCK_ITER(unit, mem, blk) {
            if (SOC_MEM_STATE(unit, mem) == NULL ||
                SOC_MEM_STATE(unit, mem)->cache[blk] == NULL) {
                continue;
            }
            SOC_MEM_STATE_MEMBER_GET(unit, mem, cache[blk], cache);
            SOC_MEM_STATE_MEMBER_GET(unit, mem, vmap[blk], vmap);
            shadow = soc_monterey_shadow[unit].shadow_array[mem_idx].mem_shadow;
