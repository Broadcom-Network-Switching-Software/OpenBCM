...
#define SOC_FLEXPORT_WB_VERSION_1_4                SOC_SCACHE_VERSION(1,4)
#define SOC_FLEXPORT_WB_DEFAULT_VERSION            SOC_FLEXPORT_WB_VERSION_1_4
...
soc_mn_port_resource_validate(int unit, int nport, soc_port_resource_t *resource)
{
    ...
    soc_info_t *si;
    int lossless_port_count = 0, new_lossless_count = 0;
    si = &SOC_INFO(unit);
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if (!(pr->flags & SOC_PORT_RESOURCE_SPEED)) {
            ...
            if (pr->mmu_lossless) {
               if ((si->lossless_headroom_reserve == 0) || (si->mmu_lossless == 0)) {
                   return SOC_E_PARAM;
               }
               new_lossless_count++;
            }
    ...
    if (new_lossless_count) {
        SOC_PBMP_COUNT(si->pbmp_mmu_lossless,lossless_port_count);
        if ((lossless_port_count + new_lossless_count) > si->lossless_portmax) {
                     return SOC_E_FULL;
        }
    }
                             ...
                             soc_port_resource_t *post_resource)
{
    ...
    for (i = 0, pr = &post_resource[0]; i < post_count; i++, pr++) {
        ...
        if (pr->mmu_lossless) {
            SOC_PBMP_PORT_ADD(si->pbmp_mmu_lossless, logic_port);
        }
        if (pr->oversub) {
                                ...
                                soc_port_resource_t *pre_resource)
{
    ...
    for (i = 0, pr = &pre_resource[0]; i < pre_count; i++, pr++) {
        ...
        SOC_PBMP_PORT_REMOVE(si->pbmp_mmu_lossless, logic_port);
        SOC_PBMP_PORT_REMOVE(si->oversub_pbm, logic_port);
    }
...
soc_monterey_flexport_scache_allocate(int unit)
{
    ...
    alloc_size =   (sizeof(int) * MN_FLEX_MAX_NUM_PORTS) + /* phy to logical*/
                   ...
                   (sizeof(soc_ptype_t))                 + /* rsvd4 ports */
                   (sizeof(pbmp_t))                      +/* pbmp_mmu_lossless */
                   (sizeof(int))                          +/* lossless_portmax */
                   (sizeof(int));                         /* max headroom reserve */
...
soc_monterey_flexport_scache_sync(int unit)
{
    ...
    SOC_SCACHE_HANDLE_SET(scache_handle, unit, SOC_SCACHE_FLEXIO_HANDLE, 0);
    rv = soc_versioned_scache_ptr_get(unit, scache_handle, TRUE, &alloc_get, &flexport_scache_ptr,
    ...
    /* mmu lossless */
    sal_memcpy(&flexport_scache_ptr[scache_offset],
               &si->pbmp_mmu_lossless, sizeof(pbmp_t));
    scache_offset += sizeof(pbmp_t);

    /* mmu lossless port max*/
    sal_memcpy(&flexport_scache_ptr[scache_offset],
               &si->lossless_portmax, sizeof(int));
    scache_offset += sizeof(int);

    /* mmu lossless  headroom reserve */
    sal_memcpy(&flexport_scache_ptr[scache_offset],
               &si->lossless_headroom_reserve, sizeof(int));
    scache_offset += sizeof(int);
...
soc_monterey_flexport_scache_recovery(int unit)
{
    ...
    int max_num_ports, max_num_pp_ports;
    int scache_max_num_ports, scache_max_num_pp_ports;
    soc_ptype_t *ptype;
    ...
    max_num_pp_ports = SOC_MAX_NUM_PP_PORTS;
    scache_max_num_ports = soc_scache_dictionary_entry_get(unit,
                                                    ssden_SOC_MAX_NUM_PORTS,
                                                    SOC_MAX_NUM_PORTS);
    scache_max_num_pp_ports = soc_scache_dictionary_entry_get(unit,
                                                    ssden_SOC_MAX_NUM_PP_PORTS,
                                                    SOC_MAX_NUM_PP_PORTS);

    /* coverity[Logically dead code] */
    if (max_num_pp_ports > max_num_ports) {
        max_num_ports = max_num_pp_ports;
    }
    if (scache_max_num_pp_ports > scache_max_num_ports) {
        scache_max_num_ports = scache_max_num_pp_ports;
    }
    ...
    if (recovered_ver >= SOC_FLEXPORT_WB_VERSION_1_2) {
        ...
        /* cpri port */
        sal_memcpy(&ptype->port,
                   &flexport_scache_ptr[scache_offset],
                   SOC_SCACHE_SIZE_MIN(sizeof(int) * scache_max_num_ports,
                                       sizeof(int) * max_num_ports));
        scache_offset += sizeof(int) * scache_max_num_ports;

        /* cpri num */
        sal_memcpy(&ptype->num,
                   &flexport_scache_ptr[scache_offset],
                   sizeof(int));
        scache_offset += sizeof(int);

        /* cpri min */
        sal_memcpy(&ptype->min,
                   &flexport_scache_ptr[scache_offset],
                   sizeof(int));
        scache_offset += sizeof(int);

        /* cpri max */
        sal_memcpy(&ptype->max,
                   &flexport_scache_ptr[scache_offset],
                   sizeof(int));
        scache_offset += sizeof(int);

        /* cpri bitmap */
        sal_memcpy(&ptype->bitmap,
                   &flexport_scache_ptr[scache_offset],
                   SOC_SCACHE_SIZE_MIN(pbmp_scache_sz, sizeof(pbmp_t)));
        scache_offset += pbmp_scache_sz;

        /* cpri disabled_bitmap */
        sal_memcpy(&ptype->disabled_bitmap,
                   &flexport_scache_ptr[scache_offset],
                   SOC_SCACHE_SIZE_MIN(pbmp_scache_sz, sizeof(pbmp_t)));
        scache_offset += pbmp_scache_sz;
    ...
    if (recovered_ver >= SOC_FLEXPORT_WB_VERSION_1_4) {
        /* Monterey lossless/lossy mixed mode support */
        sal_memcpy(&si->pbmp_mmu_lossless,
                   &flexport_scache_ptr[scache_offset],
                   SOC_SCACHE_SIZE_MIN(pbmp_scache_sz, sizeof(pbmp_t)));
        scache_offset += pbmp_scache_sz;
        sal_memcpy(&si->lossless_portmax,
                   &flexport_scache_ptr[scache_offset],
                    sizeof(int));
        scache_offset += hole_size;
        sal_memcpy(&si->lossless_headroom_reserve,
                   &flexport_scache_ptr[scache_offset],
                   sizeof(int));
        scache_offset += hole_size;
    } else {
