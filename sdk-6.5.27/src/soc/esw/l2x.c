               ...
               int *index_ptr)
{
    ...
    if (opcode != READ_MEMORY_ACK_MSG) {
        ...
        soc_schan_dump(unit, &schan_msg, 3, BSL_ERROR);
...
#ifdef BCM_TRIDENT3_SUPPORT
    /* If L2 mode is FIFO, the bulk op records to FIFO and the property
     * l2x_bulk_op_learn_pause is enabled, pause L2 learning
     */
    if ((SOC_IS_TRIDENT3(unit)) && (mode == L2MODE_FIFO) &&
...
_soc_l2x_frozen_cml_save(int unit)
{
    ...
    if (SOC_MEM_IS_VALID(unit, PORT_TABm)) {
        ...
        soc_mem_unlock(unit, ING_DEVICE_PORTm);    /* PORT_UNLOCK */
    }
...
/*
 * Function:
 *  soc_l2x_vp_frozen_cml_set
 * Purpose:
 *  Update the saved Cpu Managed Learning mode for a VP
 *  if the device is frozen.
 * Parameters:
 *  unit            device number
 *  index           SVP table index
 *  cml             Cpu-Managed-Learning mode to update
 *  cml_move        Station move Cpu-Managed-Learning mode to update
 *  repl_cml        CML mode to use in SVP table update
 *  repl_cml_move   Station move CML mode to use in SVP table update
 * Returns:
 *  SOC_E_NONE  frozen cml is updated
 *  SOC_E_FAIL  unit is not frozen
 *
 */
int
soc_l2x_vp_frozen_cml_set(int unit, int index, int cml, int cml_move, int *repl_cml, int *repl_cml_move)
{
    cml_freeze_t    *f_cml = &cml_freeze_state[unit];
    int  index_min, buf_index;
    ...
    if (f_cml->frozen) {
        if (SOC_MEM_IS_VALID(unit, SOURCE_VPm) && NULL != f_cml->vp_bitmap) {
            if (!SHR_BITGET(f_cml->vp_bitmap, index)) {
                /* VP not in use */
                return SOC_E_FAIL;
            }
            ...
            f_cml->save_vp_cml[buf_index] = cml;
            f_cml->save_vp_cml_move[buf_index] = cml_move;
            if (repl_cml != NULL) {
    #if defined(BCM_TRIUMPH3_SUPPORT) || defined(BCM_TRX_SUPPORT)
                if (SOC_IS_TRIUMPH3(unit) || SOC_IS_TRX(unit)) {
                    /* Since L2 is frozen, bits 4, 3 should be clear, they would
