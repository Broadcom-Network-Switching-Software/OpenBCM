...
soc_helix5_hash_bank_count_get(int unit, soc_mem_t mem, int *num_banks)
{
    ...
    case L2Xm:
        /*
         * HX5 - 8k single wide entries in each of 2 dedicated L2 bank(B0,B1)
         *     - 16k single wide entries in each shared bank(B2-B9)(6 shared banks max)
         * HR4 - 32k single wide entries in each of 2 dedicated L2 bank(B0, B1)
         *     - No shared bank
         * FB6 - 16k single wide entries in each of 2 dedicated L2 bank(B0, B1)
         *     - 32k single wide entries in each shared bank(B2-B9)(8 shared banks max)
         * FB6_AX(56474) - 16k single wide entries in each of 2 dedicated L2 bank(B0, B1)
         *     - 16k single wide entries in each shared bank(B2-B9)(8 shared banks max)
         */
        ...
        *num_banks = 2;
        ...
        if (ip_uft_bank_mul) {
            if (soc_feature(unit, soc_feature_td3x4_ax_uft_shared_8_banks_16k)) {
                /* Total size of dedicated banks (2*16k) and
                 * size of single shared bank (16k) is different
                 */
                int dedicated_bank_size = 2 * 16 * 1024;
                *num_banks += ((count - dedicated_bank_size) /
    ...
    case L3_ENTRY_QUADm:
        /*
         * HX5 - 8k single wide entries in each of 2 dedicated L3 bank(B10,B11)
         *     - 16k single wide entries in each shared bank(B2-B9)(6 shared banks max)
         * HR4 - 32k single wide entries in each of 2 dedicated L3 bank(B0, B1)
         *     - No shared bank
         * FB6 - 16k single wide entries in each of 2 dedicated L3 bank(B10, B11)
         *     - 32k single wide entries in each shared bank(B2-B9)(8 shared banks max)
         * FB6_AX(56474) - 16k single wide entries in each of 2 dedicated L3 bank(B10, B11)
         *     - 16k single wide entries in each shared bank(B2-B9)(8 shared banks max)
         */
                            ...
                            int *phy_bank_id)
{
    ...
    case L2Xm:
        /* Dedicated Banks */
        if (log_bank < num_l2_dedicated_banks) {
...
#ifdef BCM_HURRICANE4_SUPPORT
        if (SOC_IS_HURRICANE4(unit)) {
            if (log_bank < num_l3_dedicated_banks) {
                *phy_bank_id = log_bank;
                done = 1;
            } else { /* Shared Banks */
                return SOC_E_INTERNAL;
                            ...
                            int *log_bank)
{
    ...
    case L3_ENTRY_QUADm:
    case L3_ENTRY_ONLY_QUADm:
        bank_min = num_l3_dedicated_banks;
        /* shared banks ID + dedicated banks which happens to be 2. */
        bank_max = max_uft_shared_banks_id + num_l3_dedicated_banks;
#ifdef BCM_HURRICANE4_SUPPORT
        if (SOC_IS_HURRICANE4(unit)) {
            if (phy_bank < num_l3_dedicated_banks) {
                *log_bank = phy_bank;
                done = 1;
            } else { /* Shared Banks */
                return SOC_E_INTERNAL;
...
soc_hx5_hash_bank_info_get(int unit, soc_mem_t mem, int bank,
                           ...
                           int *bucket_offset)
{
    int bank_size = 0, row_size, bucket_size, base, offset;
    int seq_num = 0;
    int l3_bank_size;
    ...
    case L2Xm:
        ...
        if (bank < 0 || bank > max_uft_shared_banks_id) {
            return SOC_E_INTERNAL;
        } else if (bank < num_l2_dedicated_banks) { /* Dedicated bank */
#ifdef BCM_HURRICANE4_SUPPORT
            if (SOC_IS_HURRICANE4(unit)) {
            ...
            /* Shared banks. */
            if (soc_feature(unit, soc_feature_td3x4_ax_uft_shared_8_banks_16k)) {
                int dedicated_banks_size = 2 * 16384;
                int shared_bank_size = 16384;
                /* Dedicated banks + (shared_logical_bank * shared bank size)*/
                base = dedicated_banks_size + (bank - 2) * shared_bank_size;
            } else {
    ...
    case L3_ENTRY_QUADm:
        ...
        if (bank < 0 || bank > max_uft_shared_banks_id) {
            return SOC_E_INTERNAL;
        } else if (bank < num_l3_dedicated_banks) { /* Dedicated bank */
...
#endif
            if (soc_feature(unit, soc_feature_td3x4_ax_uft_shared_8_banks_16k)) {
                /* Each shared bank is of 16K size */
                bank_ent_mul_factor = 1;
            }
            ...
            if (soc_feature(unit, soc_feature_td3x4_ax_uft_shared_8_banks_16k)) {
                /* Two dedicated banks of 16k each hence base should start from 2 * 16k */
                base = 2 * base;
            }
                        ...
                        int *use_lsb)
{
    ...
    case L2Xm:
        ...
        is_shared = bank > (num_l2_dedicated_banks - 1);
...
#ifdef BCM_HURRICANE4_SUPPORT
        if (SOC_IS_HURRICANE4(unit)) {
            if (bank >= num_l3_dedicated_banks) {
                /* No Shared Banks */
                return SOC_E_INTERNAL;
            }
            is_shared = 0;
                        ...
                        int use_lsb)
{
    ...
    case L3_ENTRY_ONLY_SINGLEm:
        ...
        {
           /* L3: 2 dedicated banks. so max is ded_start+1 */
            if (bank < num_l3_dedicated_banks || bank > (l3_phy_ded_start_id+1)) {
                return SOC_E_PARAM;
            }
            /* Dedicated banks */
            if ((bank >= l3_phy_ded_start_id) && (bank <= l3_phy_ded_start_id+1)) {
