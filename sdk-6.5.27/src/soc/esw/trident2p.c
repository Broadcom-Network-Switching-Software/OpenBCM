...
soc_td2p_mem_config(int unit, uint16 dev_id, uint8 rev_id)
{
    ...
    for (ipmc_tindex = 0; ipmc_tindex < sizeof(ipmc_tbl)/sizeof(ipmc_tbl[0]); 
                          ipmc_tindex++ ) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, ipmc_tbl[ipmc_tindex], index_max);
    }
    ...
    index_max--;
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_IIFm, index_max);

    /* Disable CoE related memories for appr. SKUs */
    if ((dev_id != BCM56866_DEVICE_ID) && (dev_id != BCM56867_DEVICE_ID)) {
        SOP_MEM_STATE_MAX_INDEX_SET(unit, SUBPORT_TAG_SGPP_MAPm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, SUBPORT_TAG_SGPP_MAP_ONLYm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, SUBPORT_TAG_SGPP_MAP_DATA_ONLYm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_LPORT_PROFILEm, -1);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_SUBPORT_TAG_DOT1P_MAPm, -1);
    }
    ...
    switch (dev_id) {
       case BCM56868_DEVICE_ID:
            ...
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ECMP_COUNTm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_L3_NEXT_HOPm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, INITIAL_L3_ECMPm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, INITIAL_L3_ECMP_GROUPm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, INITIAL_ING_L3_NEXT_HOPm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_AUX_HITBIT_UPDATEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_AUX_SCRATCHm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_AUX_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_TUNNELm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_TUNNEL_DATA_ONLYm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_TUNNEL_ONLYm, -1);

            /* MPLS related memories */
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_MPLS_EXP_MAPPINGm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_MPLS_EXP_PRI_MAPPINGm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_MPLS_VC_AND_SWAP_LABEL_TABLEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_MPLS_EXP_PRI_MAPPINGm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_IP_TUNNEL_MPLSm, -1);
        ...
        case BCM56867_DEVICE_ID:
            /* FCoE */
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_FC_HEADER_TYPEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_VFT_PRI_MAPm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_VSANm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FC_MAP_PROFILEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, FCOE_HOP_COUNT_FNm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VSAN_INTPRI_MAPm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_FCOE_CONTROL_1m, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VFT_FIELDS_PROFILEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_VFT_PRI_MAPm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_FC_HEADER_TYPEm, -1);

            /* NAT */
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_SNATm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_SNAT_DATA_ONLYm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_SNAT_HIT_ONLYm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_SNAT_HIT_ONLY_Xm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_SNAT_HIT_ONLY_Ym, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_SNAT_ONLYm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_DNAT_ADDRESS_TYPEm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_NAT_PACKET_EDIT_INFOm, -1);

            /* TRILL */
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_TRILL_PARSE_CONTROLm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, ING_TRILL_PAYLOAD_PARSE_CONTROLm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_TRILL_PARSE_CONTROLm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_TRILL_PARSE_CONTROL_2m, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_TRILL_RBRIDGE_NICKNAMESm, -1);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, EGR_TRILL_TREE_PROFILEm, -1);
            break;
        case BCM56860_DEVICE_ID:
            if (SOC_SWITCH_BYPASS_MODE(unit) != SOC_SWITCH_BYPASS_MODE_NONE) {
                if (SOC_SWITCH_BYPASS_MODE(unit) ==
                    ...
                    SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_COUNTER_TABLE_Xm, -1);
                    SOP_MEM_STATE_MAX_INDEX_SET(unit, FP_COUNTER_TABLE_Ym, -1);
