...
soc_mmu_pool_map_cfg_read(int unit, int *pool_map)
{
    ...
    for (egr_idx = 0; egr_idx < SOC_MMU_CFG_SERVICE_POOL_MAX; egr_idx ++) {
        for (ing_idx = 0; ing_idx < SOC_MMU_CFG_SERVICE_POOL_MAX; ing_idx ++) {
            pool_map_arr[ing_idx] = SOC_MMU_CFG_SERVICE_POOL_MAX + 1;
        }
        ...
        for (ing_idx = 0; ing_idx < SOC_MMU_CFG_SERVICE_POOL_MAX; ing_idx ++) {
            if ((((pool_map_arr[ing_idx] < 0) ||
                  (pool_map_arr[ing_idx] >= SOC_MMU_CFG_SERVICE_POOL_MAX))
                 && (ing_idx < count))) {
               LOG_ERROR(BSL_LS_SOC_MMU,
                        (BSL_META_U(unit,
                                     "Invalid pool_map for egress pool:%d\n"),
                          egr_idx));
                return SOC_E_PARAM;
            }
            if (pool_map_arr[ing_idx] != SOC_MMU_CFG_SERVICE_POOL_MAX + 1) {
