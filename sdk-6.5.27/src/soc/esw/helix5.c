...
#define _SOC_HX5_DEFIP_TCAM_DEPTH   1024 /* HX5 has 8 tcams of 1K deep depth  */
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L2_ENTRY_LPm,
        ((2 * 2 * 1024) + (shared_l2_banks * 7 * 1024) - 1));
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_ENTRY_LPm,
        ((7 * 1024) + (shared_l3_banks * 7 * 1024) - 1));
    ...
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_LPm, (shared_fpem_banks * 7 * 1024) - 1);
    SOP_MEM_STATE_MAX_INDEX_SET(unit, EXACT_MATCH_LP_PIPE0m, (shared_fpem_banks * 7 * 1024) - 1);
    ...
    /* 1.5k per IUAT bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, MPLS_ENTRY_LPm, (mpls_4k_banks * 1536) - 1);
    ...
    /* 1.5k per IUAT bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_1_LPm, (vlan_xlate_1_4k_banks * 1536) - 1);
    ...
    /* 1.5k per IUAT bank */
    SOP_MEM_STATE_MAX_INDEX_SET(unit, VLAN_XLATE_2_LPm, (vlan_xlate_2_4k_banks * 1536) - 1);
    ...
    if (soc_property_get(unit, "l3_defip_sizing", TRUE)) {
        if (!alpm_enable) {
            ...
            index_max_mem = (SOC_CONTROL(unit)->l3_defip_max_tcams *
                 SOC_CONTROL(unit)->l3_defip_tcam_size) -
                 (num_ipv6_128b_entries * 2) - 1;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIPm, index_max_mem);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ONLYm, index_max_mem);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_DATA_ONLYm, index_max_mem);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_HIT_ONLYm, index_max_mem);
    ...
    if (alpm_enable) {
        ...
        if (shared_l2_banks + shared_l3_banks + shared_fpem_banks > 0) {
            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_RAWm, index_max_mem);
            index_max_mem = (index_max_mem * 4) / 6;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_RAWm, index_max_mem);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV4m, index_max_mem);
            index_max_mem = (index_max_mem * 4) / 6;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV4m, index_max_mem);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV4_1m, index_max_mem);
            index_max_mem = (index_max_mem * 4) / 6;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV4_1m, index_max_mem);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_64m, index_max_mem);
            index_max_mem = (index_max_mem * 4) / 6;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_64m, index_max_mem);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_64_1m, index_max_mem);
            index_max_mem = (index_max_mem * 4) / 6;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_64_1m, index_max_mem);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_IPV6_128m, index_max_mem);
            index_max_mem = (index_max_mem * 4) / 6;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_IPV6_128m, index_max_mem);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_HIT_ONLYm, index_max_mem);
            index_max_mem = (index_max_mem * 4) / 6;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_HIT_ONLYm, index_max_mem);

            SOP_MEM_STATE_MAX_INDEX_GET(unit, L3_DEFIP_ALPM_ECCm, index_max_mem);
            index_max_mem = (index_max_mem * 4) / 6;
            SOP_MEM_STATE_MAX_INDEX_SET(unit, L3_DEFIP_ALPM_ECCm, index_max_mem);
        }
    } else {
    ...
    switch (num_lag_rh_flowset_entries) {
        ...
        case 32768:
            ...
            break;
        case 16384:
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_HGT_FLOWSETm, 16383);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_HGT_FLOWSET_PIPE0m, 16383);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_LAG_FLOWSETm, 16383);
            SOP_MEM_STATE_MAX_INDEX_SET(unit, RH_LAG_FLOWSET_PIPE0m, 16383);
            break;
        default:
            return SOC_E_CONFIG;
    }
    ...
    if (soc_feature(unit, soc_feature_flowtracker_3banks_ft_table)) {
        /* 3 banks x 4k */
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_SINGLEm, 12287);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_DOUBLEm, 6143);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_DOUBLE_PIPE0m, 6143);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_ECCm, 12287);
    } else if (soc_feature(unit, soc_feature_flowtracker_5banks_ft_table)) {
        /* 5 banks x 4k */
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_SINGLEm, 20479);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_DOUBLEm, 10239);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_DOUBLE_PIPE0m, 10239);
        SOP_MEM_STATE_MAX_INDEX_SET(unit, FT_KEY_ECCm, 20479);
    }
...
int soc_hx5_cosq_threshold_dynamic_change_check(
    int unit,
    int new_threshold,
    soc_reg_t ctr_reg,
    ...
    uint8 *allowed)
{
    ...
    uint64 val64s[_HX5_XPES_PER_DEV];
    ...
    *allowed = FALSE;
    ...
    do {
        ...
        for (index = 0; index < _HX5_XPES_PER_DEV; index++) {
