...
shr_error_e
dnx_data_property_device_general_nof_faps_read(
    int unit,
    uint32 *nof_faps)
{
    int fabric_connect_mode = dnx_data_fabric.general.connect_mode_get(unit);
    SHR_FUNC_INIT_VARS(unit);

    if (fabric_connect_mode == DNX_FABRIC_CONNECT_MODE_MESH
        || fabric_connect_mode == DNX_FABRIC_CONNECT_MODE_SINGLE_FAP)
    {
        *nof_faps = 16;
    }
    else
    {
        *nof_faps = dnx_data_device.general.max_nof_faps_get(unit);
    }
    ...
    dnx_data_port_static_add_ucode_port_t * ucode_port)
{
    ...
    if (propval)
    {
        ...
        while (str_pointer && (*str_pointer == ':'))
        {
            ...
            if (!sal_strncasecmp(str_pointer, prefix, prefix_len))
            {
                ucode_port->is_framer_mgmt = 1;
                str_pointer += prefix_len;
                
                if (*str_pointer == ':')
                {
                    ++str_pointer;
                    ucode_port->framer_mgmt_channel = sal_ctoi(str_pointer, &str_pointer_end);
                    if (str_pointer == str_pointer_end)
                    {
                        SHR_ERR_EXIT(_SHR_E_FAIL, "Framer channel is not specified (\"%s\") for %s", propval,
                                     spn_UCODE_PORT);
                    }
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_FAIL, "Plesae specify the Framer mgmt channel (\"%s\") for %s", propval,
                                 spn_UCODE_PORT);
                }
                continue;
            }
