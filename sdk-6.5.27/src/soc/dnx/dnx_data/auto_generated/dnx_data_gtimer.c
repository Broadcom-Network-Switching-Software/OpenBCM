...
static shr_error_e
dnx_data_gtimer_ipc_init(
    int unit,
    dnxc_data_submodule_t *submodule_data)
{
    SHR_FUNC_INIT_VARS(unit);

    submodule_data->name = "ipc";
    submodule_data->doc = "";
    
    submodule_data->nof_features = _dnx_data_gtimer_ipc_feature_nof;
    DNXC_DATA_ALLOC(submodule_data->features, dnxc_data_feature_t,  submodule_data->nof_features, "_dnxc_data gtimer ipc features");

    submodule_data->features[dnx_data_gtimer_ipc_is_supported].name = "is_supported";
    submodule_data->features[dnx_data_gtimer_ipc_is_supported].doc = "";
    submodule_data->features[dnx_data_gtimer_ipc_is_supported].flags |= DNXC_DATA_F_FEATURE;

    
    submodule_data->nof_defines = _dnx_data_gtimer_ipc_define_nof;
    DNXC_DATA_ALLOC(submodule_data->defines, dnxc_data_define_t, submodule_data->nof_defines, "_dnxc_data gtimer ipc defines");

    submodule_data->defines[dnx_data_gtimer_ipc_define_block_index_size].name = "block_index_size";
    submodule_data->defines[dnx_data_gtimer_ipc_define_block_index_size].doc = "";
    
    submodule_data->defines[dnx_data_gtimer_ipc_define_block_index_size].flags |= DNXC_DATA_F_DEFINE;

    submodule_data->defines[dnx_data_gtimer_ipc_define_block_index_max_value].name = "block_index_max_value";
    submodule_data->defines[dnx_data_gtimer_ipc_define_block_index_max_value].doc = "";
    
    submodule_data->defines[dnx_data_gtimer_ipc_define_block_index_max_value].flags |= DNXC_DATA_F_DEFINE;

    
    submodule_data->nof_tables = _dnx_data_gtimer_ipc_table_nof;
    DNXC_DATA_ALLOC(submodule_data->tables, dnxc_data_table_t, submodule_data->nof_tables, "_dnxc_data gtimer ipc tables");
...
int
dnx_data_gtimer_ipc_feature_get(
    int unit,
    dnx_data_gtimer_ipc_feature_e feature)
{
    return dnxc_data_mgmt_feature_data_get(unit, dnx_data_module_gtimer, dnx_data_gtimer_submodule_ipc, feature);
}


uint32
dnx_data_gtimer_ipc_block_index_size_get(
    int unit)
{
    return dnxc_data_mgmt_define_data_get(unit, dnx_data_module_gtimer, dnx_data_gtimer_submodule_ipc, dnx_data_gtimer_ipc_define_block_index_size);
}

uint32
dnx_data_gtimer_ipc_block_index_max_value_get(
    int unit)
{
    return dnxc_data_mgmt_define_data_get(unit, dnx_data_module_gtimer, dnx_data_gtimer_submodule_ipc, dnx_data_gtimer_ipc_define_block_index_max_value);
}
    ...
    dnxc_data_module_t *module_data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_data_gtimer_ipc_init(unit, &module_data->submodules[dnx_data_gtimer_submodule_ipc]));
