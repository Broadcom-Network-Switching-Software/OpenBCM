    ...
    dnxc_data_submodule_t *submodule_data)
{
    ...
    submodule_data->features[dnx_data_dbal_db_init_tcam_cs_as_hl_tcam].name = "tcam_cs_as_hl_tcam";
    submodule_data->features[dnx_data_dbal_db_init_tcam_cs_as_hl_tcam].doc = "";
    submodule_data->features[dnx_data_dbal_db_init_tcam_cs_as_hl_tcam].flags |= DNXC_DATA_F_FEATURE;
    ...
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].name = "default_std";
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].doc = "";
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].flags |= DNXC_DATA_F_TABLE;
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].size_of_values = sizeof(dnx_data_dbal_db_init_default_std_t);
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].entry_get = dnx_data_dbal_db_init_default_std_entry_str_get;

    
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].nof_keys = 0;

    
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].nof_values = 1;
    DNXC_DATA_ALLOC(submodule_data->tables[dnx_data_dbal_db_init_table_default_std].values, dnxc_data_value_t, submodule_data->tables[dnx_data_dbal_db_init_table_default_std].nof_values, "_dnx_data_dbal_db_init_table_default_std table values");
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].values[0].name = "image_name";
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].values[0].type = "char *";
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].values[0].doc = "";
    submodule_data->tables[dnx_data_dbal_db_init_table_default_std].values[0].offset = UTILEX_OFFSETOF(dnx_data_dbal_db_init_default_std_t, image_name);
...
const dnx_data_dbal_db_init_default_std_t *
dnx_data_dbal_db_init_default_std_get(
    int unit)
{
    char *data;
    dnxc_data_table_t *table;
    ...
    table = dnxc_data_mgmt_table_get(unit, dnx_data_module_dbal, dnx_data_dbal_submodule_db_init, dnx_data_dbal_db_init_table_default_std);
    
    data = dnxc_data_mgmt_table_data_get(unit, table, 0, 0);
    return (const dnx_data_dbal_db_init_default_std_t *) data;

}


shr_error_e
dnx_data_dbal_db_init_default_std_entry_str_get(
    int unit,
    char *buffer,
    int key0,
    int key1,
...
{
    dnxc_data_table_t *table;
    const dnx_data_dbal_db_init_default_std_t *data;
    SHR_FUNC_INIT_VARS(unit);

    
    table = dnxc_data_mgmt_table_get(unit, dnx_data_module_dbal, dnx_data_dbal_submodule_db_init, dnx_data_dbal_db_init_table_default_std);
    data = (const dnx_data_dbal_db_init_default_std_t *) dnxc_data_mgmt_table_data_diag_get(unit, table, 0, 0);
    switch (value_index)
    {
        case 0:
            sal_snprintf(buffer, DNXC_DATA_MGMT_MAX_TABLE_VALUE_LENGTH, "%s", data->image_name == NULL ? "" : data->image_name);
            break;
    }
...
const dnxc_data_table_info_t *
dnx_data_dbal_db_init_default_std_info_get(
    int unit)
{

    
    return dnxc_data_mgmt_table_info_get(unit, dnx_data_module_dbal, dnx_data_dbal_submodule_db_init, dnx_data_dbal_db_init_table_default_std);
