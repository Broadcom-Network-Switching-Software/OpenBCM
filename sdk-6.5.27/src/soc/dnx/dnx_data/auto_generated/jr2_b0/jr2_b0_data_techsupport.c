...
static shr_error_e
jr2_b0_dnx_data_techsupport_tm_num_of_tables_per_block_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_techsupport;
    int submodule_index = dnx_data_techsupport_submodule_tm;
    int define_index = dnx_data_techsupport_tm_define_num_of_tables_per_block;
...
static shr_error_e
jr2_b0_dnx_data_techsupport_tm_num_of_tm_command_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_techsupport;
    int submodule_index = dnx_data_techsupport_submodule_tm;
    int define_index = dnx_data_techsupport_tm_define_num_of_tm_command;
    SHR_FUNC_INIT_VARS(unit);
...
static shr_error_e
jr2_b0_dnx_data_techsupport_tm_tm_block_set(
    int unit)
{
    int block_index;
    dnx_data_techsupport_tm_tm_block_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_techsupport;
    int submodule_index = dnx_data_techsupport_submodule_tm;
    int table_index = dnx_data_techsupport_tm_table_tm_block;
    SHR_FUNC_INIT_VARS(unit);
    ...
    DNXC_DATA_ALLOC(table->data, dnx_data_techsupport_tm_tm_block_t, (1 * (table->keys[0].size) + 1  ), "data of dnx_data_techsupport_tm_table_tm_block");

    
    default_data = (dnx_data_techsupport_tm_tm_block_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->block_name = NULL;
    default_data->is_mask_ecc = 0;
    default_data->ecc_mask_words = 0;
    
    for (block_index = 0; block_index < table->keys[0].size; block_index++)
    {
        data = (dnx_data_techsupport_tm_tm_block_t *) dnxc_data_mgmt_table_data_get(unit, table, block_index, 0);
        sal_memcpy(data, default_data, table->size_of_values);
    }
    
    if (dnx_techsupport_cgm < table->keys[0].size)
    {
        data = (dnx_data_techsupport_tm_tm_block_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_cgm, 0);
        data->block_name = "CGM";
        data->is_mask_ecc = 1;
        data->ecc_mask_words = 3;
    }
    if (dnx_techsupport_ecgm < table->keys[0].size)
    {
        data = (dnx_data_techsupport_tm_tm_block_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ecgm, 0);
        data->block_name = "ECGM";
        data->is_mask_ecc = 1;
        data->ecc_mask_words = 1;
    }
...
static shr_error_e
jr2_b0_dnx_data_techsupport_tm_tm_command_set(
    int unit)
{
    int index_index;
    dnx_data_techsupport_tm_tm_command_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_techsupport;
    int submodule_index = dnx_data_techsupport_submodule_tm;
    int table_index = dnx_data_techsupport_tm_table_tm_command;
    SHR_FUNC_INIT_VARS(unit);
    ...
    table->keys[0].size = dnx_data_techsupport.tm.num_of_tm_command_get(unit);
    table->info_get.key_size[0] = dnx_data_techsupport.tm.num_of_tm_command_get(unit);
    ...
    DNXC_DATA_ALLOC(table->data, dnx_data_techsupport_tm_tm_command_t, (1 * (table->keys[0].size) + 1  ), "data of dnx_data_techsupport_tm_table_tm_command");

    
    default_data = (dnx_data_techsupport_tm_tm_command_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->command = NULL;
    
    for (index_index = 0; index_index < table->keys[0].size; index_index++)
    {
        data = (dnx_data_techsupport_tm_tm_command_t *) dnxc_data_mgmt_table_data_get(unit, table, index_index, 0);
        sal_memcpy(data, default_data, table->size_of_values);
    }
    
    if (0 < table->keys[0].size)
    {
        data = (dnx_data_techsupport_tm_tm_command_t *) dnxc_data_mgmt_table_data_get(unit, table, 0, 0);
        data->command = "tm ingress queue non_empty";
    }
    if (1 < table->keys[0].size)
    {
        data = (dnx_data_techsupport_tm_tm_command_t *) dnxc_data_mgmt_table_data_get(unit, table, 1, 0);
        data->command = "tm ingress congestion";
    }
    if (2 < table->keys[0].size)
    {
        data = (dnx_data_techsupport_tm_tm_command_t *) dnxc_data_mgmt_table_data_get(unit, table, 2, 0);
        data->command = "tm egress congestion";
    }
    if (3 < table->keys[0].size)
    {
        data = (dnx_data_techsupport_tm_tm_command_t *) dnxc_data_mgmt_table_data_get(unit, table, 3, 0);
        data->command = "fabric connectivity";
    }
    if (4 < table->keys[0].size)
    {
        data = (dnx_data_techsupport_tm_tm_command_t *) dnxc_data_mgmt_table_data_get(unit, table, 4, 0);
        data->command = "fabric mesh_topology";
    }
    if (5 < table->keys[0].size)
    {
        data = (dnx_data_techsupport_tm_tm_command_t *) dnxc_data_mgmt_table_data_get(unit, table, 5, 0);
        data->command = "tm scheduler connection ingress";
    }
    if (6 < table->keys[0].size)
    {
        data = (dnx_data_techsupport_tm_tm_command_t *) dnxc_data_mgmt_table_data_get(unit, table, 6, 0);
        data->command = "tm scheduler connection egress";
    }
    if (7 < table->keys[0].size)
    {
        data = (dnx_data_techsupport_tm_tm_command_t *) dnxc_data_mgmt_table_data_get(unit, table, 7, 0);
        data->command = "tm egress shaper";
    }
...
static shr_error_e
jr2_b0_dnx_data_techsupport_tm_tm_mem_set(
    int unit)
{
    int block_index;
    int index_index;
    ...
    if (dnx_techsupport_cgm < table->keys[0].size && 16 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_cgm, 16);
        data->name = "CGM_VOQ_GRNTD_RJCT_MASK";
    }
    if (dnx_techsupport_cgm < table->keys[0].size && 17 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_cgm, 17);
        data->name = "CGM_PP_RJCT_MASK";
    }
    if (dnx_techsupport_cgm < table->keys[0].size && 18 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_cgm, 18);
        data->name = "CGM_VSQ_GRNTD_RJCT_MASK";
    }
    ...
    if (dnx_techsupport_spb < table->keys[0].size && 3 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_spb, 3);
    ...
    if (dnx_techsupport_ecgm < table->keys[0].size && 11 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ecgm, 11);
        data->name = "ECGM_QDCM";
    }
    if (dnx_techsupport_ecgm < table->keys[0].size && 12 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ecgm, 12);
        data->name = "ECGM_QDCMAX";
    }
    if (dnx_techsupport_ecgm < table->keys[0].size && 13 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ecgm, 13);
        data->name = "ECGM_PQSM";
    }
    if (dnx_techsupport_ecgm < table->keys[0].size && 14 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ecgm, 14);
        data->name = "ECGM_PQSMAX";
    }
    if (dnx_techsupport_ecgm < table->keys[0].size && 15 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ecgm, 15);
        data->name = "ECGM_PDCM";
    }
    if (dnx_techsupport_ecgm < table->keys[0].size && 16 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ecgm, 16);
    ...
    if (dnx_techsupport_rqp < table->keys[0].size && 1 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_rqp, 1);
        data->name = "RQP_CMS";
    }
    if (dnx_techsupport_rqp < table->keys[0].size && 2 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_rqp, 2);
        data->name = "RQP_RSM";
    }
    if (dnx_techsupport_rqp < table->keys[0].size && 3 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_rqp, 3);
        data->name = "RQP_RPDM";
    }
    if (dnx_techsupport_rqp < table->keys[0].size && 4 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_rqp, 4);
    ...
    if (dnx_techsupport_fqp < table->keys[0].size && 2 < table->keys[1].size)
    {
        ...
        data->name = "FQP_PHANTOM_QS_CFG";
    }
    if (dnx_techsupport_fqp < table->keys[0].size && 3 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_fqp, 3);
        data->name = "FQP_OTM_METER_NIF_DC";
    }
    if (dnx_techsupport_fqp < table->keys[0].size && 4 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_fqp, 4);
        data->name = "FQP_OTM_METER_TABLE";
    }
    if (dnx_techsupport_fqp < table->keys[0].size && 5 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_fqp, 5);
        data->name = "FQP_OTM_METER_CRDT_BALANCE";
    }
    ...
    if (dnx_techsupport_sch < table->keys[0].size && 5 < table->keys[1].size)
    {
        ...
        data->name = "SCH_PORT_SCHEDULER_MAP_PSM";
    }
    if (dnx_techsupport_sch < table->keys[0].size && 6 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_sch, 6);
        data->name = "SCH_PORT_SCHEDULER_WEIGHTS_PSW";
    }
    if (dnx_techsupport_sch < table->keys[0].size && 7 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_sch, 7);
    ...
    if (dnx_techsupport_ips < table->keys[0].size && 0 < table->keys[1].size)
    {
        ...
        data->name = "IPS_SQM_DEQ_CMD_PRMS";
    }
    ...
    if (dnx_techsupport_ips < table->keys[0].size && 7 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ips, 7);
        data->name = "IPS_QTYPE";
    }
    ...
    if (dnx_techsupport_ipt < table->keys[0].size && 2 < table->keys[1].size)
    {
        ...
        data->name = "IPT_PCP_CFG";
    }
    if (dnx_techsupport_ipt < table->keys[0].size && 3 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ipt, 3);
        data->name = "IPT_PER_DTQ_GIPT_CFG";
    }
    if (dnx_techsupport_ipt < table->keys[0].size && 4 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ipt, 4);
        data->name = "IPT_PER_GIPT_SHAPER_CFG";
    }
    if (dnx_techsupport_ipt < table->keys[0].size && 5 < table->keys[1].size)
    {
        data = (dnx_data_techsupport_tm_tm_mem_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_techsupport_ipt, 5);
...
shr_error_e
jr2_b0_data_techsupport_attach(
    int unit)
{
    ...
    data_index = dnx_data_techsupport_tm_define_num_of_tables_per_block;
    define = &submodule->defines[data_index];
    define->set = jr2_b0_dnx_data_techsupport_tm_num_of_tables_per_block_set;
    data_index = dnx_data_techsupport_tm_define_num_of_tm_command;
    define = &submodule->defines[data_index];
    define->set = jr2_b0_dnx_data_techsupport_tm_num_of_tm_command_set;

    

    
    data_index = dnx_data_techsupport_tm_table_tm_block;
    table = &submodule->tables[data_index];
    table->set = jr2_b0_dnx_data_techsupport_tm_tm_block_set;
    data_index = dnx_data_techsupport_tm_table_tm_command;
    table = &submodule->tables[data_index];
    table->set = jr2_b0_dnx_data_techsupport_tm_tm_command_set;
    data_index = dnx_data_techsupport_tm_table_tm_mem;
    table = &submodule->tables[data_index];
    table->set = jr2_b0_dnx_data_techsupport_tm_tm_mem_set;
