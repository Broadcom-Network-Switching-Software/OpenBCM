    ...
    int unit)
{
    ...
    if (DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_PKT_SIZE < table->keys[0].size)
    {
        data = (dnx_data_ingr_congestion_dbal_admission_bits_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_PKT_SIZE, 0);
        data->index = 38;
    }
    if (DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_GATE_STATE < table->keys[0].size)
    {
        data = (dnx_data_ingr_congestion_dbal_admission_bits_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_INGRESS_CONGESTION_REJECT_BIT_TAF_GATE_STATE, 0);
        data->index = 39;
    }
