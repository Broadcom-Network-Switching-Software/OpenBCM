...
static shr_error_e
jr2_a0_dnx_data_multicast_params_olp_ing_multicast_id_set(
    int unit)
{
    int cores_group_index;
    dnx_data_multicast_params_olp_ing_multicast_id_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_multicast;
    int submodule_index = dnx_data_multicast_submodule_params;
    int table_index = dnx_data_multicast_params_table_olp_ing_multicast_id;
    SHR_FUNC_INIT_VARS(unit);
    ...
    table->keys[0].size = dnx_data_device.general.nof_core_groups_get(unit);
    table->info_get.key_size[0] = dnx_data_device.general.nof_core_groups_get(unit);
    ...
    DNXC_DATA_ALLOC(table->data, dnx_data_multicast_params_olp_ing_multicast_id_t, (1 * (table->keys[0].size) + 1  ), "data of dnx_data_multicast_params_table_olp_ing_multicast_id");

    
    default_data = (dnx_data_multicast_params_olp_ing_multicast_id_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->multicast_id = 0;
    
    for (cores_group_index = 0; cores_group_index < table->keys[0].size; cores_group_index++)
    {
        data = (dnx_data_multicast_params_olp_ing_multicast_id_t *) dnxc_data_mgmt_table_data_get(unit, table, cores_group_index, 0);
        sal_memcpy(data, default_data, table->size_of_values);
    }
    
    if (0 < table->keys[0].size)
    {
        data = (dnx_data_multicast_params_olp_ing_multicast_id_t *) dnxc_data_mgmt_table_data_get(unit, table, 0, 0);
        data->multicast_id = dnx_data_multicast.params.max_ing_mc_groups_get(unit)-1;
    }
    ...
    int unit)
{
    ...
    data_index = dnx_data_multicast_params_table_olp_ing_multicast_id;
    table = &submodule->tables[data_index];
    table->set = jr2_a0_dnx_data_multicast_params_olp_ing_multicast_id_set;
