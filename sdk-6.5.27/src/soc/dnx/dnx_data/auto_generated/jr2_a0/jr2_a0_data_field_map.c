    ...
    int unit)
{
    ...
    table->keys[0].size = DNX_FIELD_STAGE_NOF;
    table->info_get.key_size[0] = DNX_FIELD_STAGE_NOF;
    table->keys[1].size = DNX_FIELD_CS_QUAL_NOF;
    table->info_get.key_size[1] = DNX_FIELD_CS_QUAL_NOF;
    ...
    if (DNX_FIELD_STAGE_IPMF1 < table->keys[0].size && DNX_FIELD_CS_QUAL_PEM_SCRATCH_PAD < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF1, DNX_FIELD_CS_QUAL_PEM_SCRATCH_PAD);
        data->dbal_field = DBAL_FIELD_PEM_SCRATCH_PAD;
    }
    ...
    if (DNX_FIELD_STAGE_IPMF3 < table->keys[0].size && DNX_FIELD_CS_QUAL_PEM_SCRATCH_PAD < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF3, DNX_FIELD_CS_QUAL_PEM_SCRATCH_PAD);
        data->dbal_field = DBAL_FIELD_PEM_SCRATCH_PAD;
    }
...
static shr_error_e
jr2_a0_dnx_data_field_map_qual_cs_special_general_data_qual_set(
    int unit)
{
    int stage_index;
    int qual_index;
    dnx_data_field_map_qual_cs_special_general_data_qual_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_field_map;
    int submodule_index = dnx_data_field_map_submodule_qual;
    int table_index = dnx_data_field_map_qual_table_cs_special_general_data_qual;
    SHR_FUNC_INIT_VARS(unit);
    ...
    DNXC_DATA_ALLOC(table->data, dnx_data_field_map_qual_cs_special_general_data_qual_t, (1 * (table->keys[0].size) * (table->keys[1].size) + 1  ), "data of dnx_data_field_map_qual_table_cs_special_general_data_qual");

    
    default_data = (dnx_data_field_map_qual_cs_special_general_data_qual_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->offset = -1;
    default_data->size = 0;
    default_data->vw_name = NULL;
    ...
    for (stage_index = 0; stage_index < table->keys[0].size; stage_index++)
    {
        for (qual_index = 0; qual_index < table->keys[1].size; qual_index++)
        {
            data = (dnx_data_field_map_qual_cs_special_general_data_qual_t *) dnxc_data_mgmt_table_data_get(unit, table, stage_index, qual_index);
            sal_memcpy(data, default_data, table->size_of_values);
        }
    }
    ...
    {
        data = (dnx_data_field_map_qual_cs_special_general_data_qual_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF1, DNX_FIELD_CS_QUAL_AC_IN_LIF_WIDE_DATA);
        data->offset = 16;
        data->size = 8;
    }
...
static shr_error_e
jr2_a0_dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_set(
    int unit)
{
    int stage_index;
    int container_index;
    dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_field_map;
    int submodule_index = dnx_data_field_map_submodule_qual;
    int table_index = dnx_data_field_map_qual_table_cs_pem_scratch_pad_mapping;
    SHR_FUNC_INIT_VARS(unit);
    ...
    table->keys[0].size = DNX_FIELD_STAGE_NOF;
    table->info_get.key_size[0] = DNX_FIELD_STAGE_NOF;
    table->keys[1].size = dnx_data_field.virtual_wire.general_data_nof_containers_get(unit);
    table->info_get.key_size[1] = dnx_data_field.virtual_wire.general_data_nof_containers_get(unit);
    ...
    DNXC_DATA_ALLOC(table->data, dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t, (1 * (table->keys[0].size) * (table->keys[1].size) + 1  ), "data of dnx_data_field_map_qual_table_cs_pem_scratch_pad_mapping");

    
    default_data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->nof_bits = 0;
    default_data->lsb_on_general_data = -1;
    default_data->lsb_on_pem_scratch_pad = -1;
    
    for (stage_index = 0; stage_index < table->keys[0].size; stage_index++)
    {
        for (container_index = 0; container_index < table->keys[1].size; container_index++)
        {
            data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, stage_index, container_index);
            sal_memcpy(data, default_data, table->size_of_values);
        }
    }
    ...
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF1, 0);
        data->nof_bits = 1;
        data->lsb_on_general_data = (0 + 159);
        data->lsb_on_pem_scratch_pad = (28+(dnx_data_field.base_ipmf1.cs_container_5_selected_bits_size_get(unit)));
    }
    if (DNX_FIELD_STAGE_IPMF1 < table->keys[0].size && 1 < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF1, 1);
        data->nof_bits = 1;
        data->lsb_on_general_data = (160 + 95);
        data->lsb_on_pem_scratch_pad = (27+(dnx_data_field.base_ipmf1.cs_container_5_selected_bits_size_get(unit)));
    }
    if (DNX_FIELD_STAGE_IPMF1 < table->keys[0].size && 2 < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF1, 2);
        data->nof_bits = 1;
        data->lsb_on_general_data = (160 + 96 + 39);
        data->lsb_on_pem_scratch_pad = (25+(dnx_data_field.base_ipmf1.cs_container_5_selected_bits_size_get(unit)));
    }
    if (DNX_FIELD_STAGE_IPMF1 < table->keys[0].size && 3 < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF1, 3);
        data->nof_bits = 1;
        data->lsb_on_general_data = (160 + 96 + 40 + 39);
        data->lsb_on_pem_scratch_pad = (26+(dnx_data_field.base_ipmf1.cs_container_5_selected_bits_size_get(unit)));
    }
    if (DNX_FIELD_STAGE_IPMF1 < table->keys[0].size && 4 < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF1, 4);
        data->nof_bits = 24;
        data->lsb_on_general_data = (160 + 96 + 40 + 40);
        data->lsb_on_pem_scratch_pad = 0;
    }
    if (DNX_FIELD_STAGE_IPMF1 < table->keys[0].size && 5 < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF1, 5);
        data->nof_bits = 1 + (dnx_data_field.base_ipmf1.cs_container_5_selected_bits_size_get(unit));
        data->lsb_on_general_data = (160 + 96 + 40 + 40 + 24 + 23 - (dnx_data_field.base_ipmf1.cs_container_5_selected_bits_size_get(unit)));
        data->lsb_on_pem_scratch_pad = 24;
    }
    if (DNX_FIELD_STAGE_IPMF3 < table->keys[0].size && 0 < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF3, 0);
        data->nof_bits = 1;
        data->lsb_on_general_data = (0 + 159);
        data->lsb_on_pem_scratch_pad = 32;
    }
    if (DNX_FIELD_STAGE_IPMF3 < table->keys[0].size && 1 < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF3, 1);
        data->nof_bits = 1;
        data->lsb_on_general_data = (160 + 95);
        data->lsb_on_pem_scratch_pad = 31;
    }
    if (DNX_FIELD_STAGE_IPMF3 < table->keys[0].size && 2 < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF3, 2);
        data->nof_bits = 1;
        data->lsb_on_general_data = (160 + 96 + 39);
        data->lsb_on_pem_scratch_pad = 29;
    }
    if (DNX_FIELD_STAGE_IPMF3 < table->keys[0].size && 3 < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF3, 3);
        data->nof_bits = 1;
        data->lsb_on_general_data = (160 + 96 + 40 + 39);
        data->lsb_on_pem_scratch_pad = 30;
    }
    if (DNX_FIELD_STAGE_IPMF3 < table->keys[0].size && 4 < table->keys[1].size)
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF3, 4);
        data->nof_bits = 24;
        data->lsb_on_general_data = (160 + 96 + 40 + 40);
        data->lsb_on_pem_scratch_pad = 0;
    }
    ...
    {
        data = (dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_STAGE_IPMF3, 5);
        data->nof_bits = 5;
        data->lsb_on_general_data = (160 + 96 + 40 + 40 + 24 + 19);
        data->lsb_on_pem_scratch_pad = 24;
    }
...
static shr_error_e
jr2_a0_dnx_data_field_map_qual_header_set(
    int unit)
{
    int qual_index;
    dnx_data_field_map_qual_header_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_field_map;
    int submodule_index = dnx_data_field_map_submodule_qual;
    int table_index = dnx_data_field_map_qual_table_header;
    SHR_FUNC_INIT_VARS(unit);
    ...
    table->keys[0].size = DNX_FIELD_QUAL_NOF;
    table->info_get.key_size[0] = DNX_FIELD_QUAL_NOF;
    ...
    DNXC_DATA_ALLOC(table->data, dnx_data_field_map_qual_header_t, (1 * (table->keys[0].size) + 1  ), "data of dnx_data_field_map_qual_table_header");

    
    default_data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->dbal_field = DBAL_FIELD_EMPTY;
    default_data->offset = -1;
    default_data->size = -1;
    
    for (qual_index = 0; qual_index < table->keys[0].size; qual_index++)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, qual_index, 0);
        sal_memcpy(data, default_data, table->size_of_values);
    }
    
    if (DNX_FIELD_HEADER_QUAL_MAC_DST < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_MAC_DST, 0);
        data->dbal_field = DBAL_FIELD_MAC_DST;
        data->offset = 0;
        data->size = 48;
    }
    if (DNX_FIELD_HEADER_QUAL_MAC_SRC < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_MAC_SRC, 0);
        data->dbal_field = DBAL_FIELD_MAC_SRC;
        data->offset = 48;
        data->size = 48;
    }
    if (DNX_FIELD_HEADER_QUAL_ETHERTYPE < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_ETHERTYPE, 0);
        data->dbal_field = DBAL_FIELD_ETHERTYPE;
        data->offset = 96;
        data->size = 16;
    }
    if (DNX_FIELD_HEADER_QUAL_VLAN_TPID < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_VLAN_TPID, 0);
        data->dbal_field = DBAL_FIELD_VLAN_TPID;
        data->offset = 0;
        data->size = 16;
    }
    if (DNX_FIELD_HEADER_QUAL_VLAN_ID < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_VLAN_ID, 0);
        data->dbal_field = DBAL_FIELD_VLAN_ID;
        data->offset = 20;
        data->size = 12;
    }
    if (DNX_FIELD_HEADER_QUAL_VLAN_PRI < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_VLAN_PRI, 0);
        data->dbal_field = DBAL_FIELD_VLAN_PRI;
        data->offset = 16;
        data->size = 3;
    }
    if (DNX_FIELD_HEADER_QUAL_VLAN_CFI < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_VLAN_CFI, 0);
        data->dbal_field = DBAL_FIELD_VLAN_CFI;
        data->offset = 19;
        data->size = 1;
    }
    if (DNX_FIELD_HEADER_QUAL_VLAN_TCI < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_VLAN_TCI, 0);
        data->dbal_field = DBAL_FIELD_VLAN_TCI;
        data->offset = 16;
        data->size = 16;
    }
    if (DNX_FIELD_HEADER_QUAL_VLAN_PRI_CFI < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_VLAN_PRI_CFI, 0);
        data->dbal_field = DBAL_FIELD_VLAN_PRI_CFI;
        data->offset = 16;
        data->size = 4;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV4_TOS < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV4_TOS, 0);
        data->dbal_field = DBAL_FIELD_IPV4_TOS;
        data->offset = 8;
        data->size = 8;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV4_DSCP < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV4_DSCP, 0);
        data->dbal_field = DBAL_FIELD_IPV4_DSCP;
        data->offset = 8;
        data->size = 6;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV4_ECN < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV4_ECN, 0);
        data->dbal_field = DBAL_FIELD_IPV4_ECN;
        data->offset = 14;
        data->size = 2;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV4_FLAGS < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV4_FLAGS, 0);
        data->dbal_field = DBAL_FIELD_IPV4_FLAGS;
        data->offset = 48;
        data->size = 3;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV4_TTL < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV4_TTL, 0);
        data->dbal_field = DBAL_FIELD_IPV4_TTL;
        data->offset = 64;
        data->size = 8;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV4_PROTOCOL < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV4_PROTOCOL, 0);
        data->dbal_field = DBAL_FIELD_IPV4_PROTOCOL;
        data->offset = 72;
        data->size = 8;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV4_SRC < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV4_SRC, 0);
        data->dbal_field = DBAL_FIELD_IPV4_SIP;
        data->offset = 96;
        data->size = 32;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV4_DST < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV4_DST, 0);
        data->dbal_field = DBAL_FIELD_IPV4_DIP;
        data->offset = 128;
        data->size = 32;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_SIP < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_SIP, 0);
        data->dbal_field = DBAL_FIELD_IPV6_SIP;
        data->offset = 64;
        data->size = 128;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_DIP < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_DIP, 0);
        data->dbal_field = DBAL_FIELD_IPV6_DIP;
        data->offset = 192;
        data->size = 128;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_SIP_LOW < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_SIP_LOW, 0);
        data->dbal_field = DBAL_FIELD_IPV6_SIP_LOW;
        data->offset = 128;
        data->size = 64;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_SIP_HIGH < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_SIP_HIGH, 0);
        data->dbal_field = DBAL_FIELD_IPV6_SIP_HIGH;
        data->offset = 64;
        data->size = 64;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_DIP_LOW < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_DIP_LOW, 0);
        data->dbal_field = DBAL_FIELD_IPV6_DIP_LOW;
        data->offset = 256;
        data->size = 64;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_DIP_HIGH < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_DIP_HIGH, 0);
        data->dbal_field = DBAL_FIELD_IPV6_DIP_HIGH;
        data->offset = 192;
        data->size = 64;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_TC < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_TC, 0);
        data->dbal_field = DBAL_FIELD_IPV6_TC;
        data->offset = 4;
        data->size = 8;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_FLOW_LABEL < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_FLOW_LABEL, 0);
        data->dbal_field = DBAL_FIELD_IPV6_FLOW_LABEL;
        data->offset = 12;
        data->size = 20;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_NEXT_HEADER < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_NEXT_HEADER, 0);
        data->dbal_field = DBAL_FIELD_IPV6_NEXT_HEADER;
        data->offset = 48;
        data->size = 8;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_HOP_LIMIT < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_HOP_LIMIT, 0);
        data->dbal_field = DBAL_FIELD_IPV6_HOP_LIMIT;
        data->offset = 56;
        data->size = 8;
    }
    if (DNX_FIELD_HEADER_QUAL_IPV6_1ST_EXTENSION_NEXT_HEADER < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_IPV6_1ST_EXTENSION_NEXT_HEADER, 0);
        data->dbal_field = DBAL_FIELD_IPV6_1ST_EXTENSION_NEXT_HEADER;
        data->offset = 320;
        data->size = 8;
    }
    if (DNX_FIELD_HEADER_QUAL_L4_SRC_PORT < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_L4_SRC_PORT, 0);
        data->dbal_field = DBAL_FIELD_L4_SRC_PORT;
        data->offset = 0;
        data->size = 16;
    }
    if (DNX_FIELD_HEADER_QUAL_L4_DST_PORT < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_L4_DST_PORT, 0);
        data->dbal_field = DBAL_FIELD_L4_DST_PORT;
        data->offset = 16;
        data->size = 16;
    }
    if (DNX_FIELD_HEADER_QUAL_MPLS_LABEL < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_MPLS_LABEL, 0);
        data->dbal_field = DBAL_FIELD_MPLS_HEADER;
        data->offset = 0;
        data->size = 32;
    }
    if (DNX_FIELD_HEADER_QUAL_MPLS_LABEL_ID < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_MPLS_LABEL_ID, 0);
        data->dbal_field = DBAL_FIELD_MPLS_LABEL;
        data->offset = 0;
        data->size = 20;
    }
    if (DNX_FIELD_HEADER_QUAL_MPLS_TC < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_MPLS_TC, 0);
        data->dbal_field = DBAL_FIELD_MPLS_TC;
        data->offset = 20;
        data->size = 3;
    }
    if (DNX_FIELD_HEADER_QUAL_MPLS_BOS < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_MPLS_BOS, 0);
        data->dbal_field = DBAL_FIELD_MPLS_BOS;
        data->offset = 23;
        data->size = 1;
    }
    if (DNX_FIELD_HEADER_QUAL_MPLS_TTL < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_MPLS_TTL, 0);
        data->dbal_field = DBAL_FIELD_MPLS_TTL;
        data->offset = 24;
        data->size = 8;
    }
    if (DNX_FIELD_HEADER_QUAL_ARP_OPCODE < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_ARP_OPCODE, 0);
        data->dbal_field = DBAL_FIELD_ARP_OPCODE;
        data->offset = 48;
        data->size = 16;
    }
    if (DNX_FIELD_HEADER_QUAL_ARP_SENDER_IP < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_ARP_SENDER_IP, 0);
        data->dbal_field = DBAL_FIELD_ARP_SENDER_IP;
        data->offset = 112;
        data->size = 32;
    }
    if (DNX_FIELD_HEADER_QUAL_ARP_TARGET_IP < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_ARP_TARGET_IP, 0);
        data->dbal_field = DBAL_FIELD_ARP_TARGET_IP;
        data->offset = 192;
        data->size = 32;
    }
    if (DNX_FIELD_HEADER_QUAL_TCP_CTL < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_TCP_CTL, 0);
        data->dbal_field = DBAL_FIELD_TCP_CTL;
        data->offset = 106;
        data->size = 6;
    }
    if (DNX_FIELD_HEADER_QUAL_RCH_TYPE < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_RCH_TYPE, 0);
        data->dbal_field = DBAL_FIELD_RCH_TYPE;
        data->offset = 0;
        data->size = 4;
    }
    if (DNX_FIELD_HEADER_QUAL_RCH_VISIBILITY < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_RCH_VISIBILITY, 0);
        data->dbal_field = DBAL_FIELD_RCH_VISIBILITY;
        data->offset = 32;
        data->size = 1;
    }
    if (DNX_FIELD_HEADER_QUAL_RCH_FORWARDING_ADDITIONAL_INFO < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_RCH_FORWARDING_ADDITIONAL_INFO, 0);
        data->dbal_field = DBAL_FIELD_RCH_FORWARDING_ADDITIONAL_INFO;
        data->offset = 132;
        data->size = 4;
    }
    if (DNX_FIELD_HEADER_QUAL_RCH_TIME_STAMP < table->keys[0].size)
    {
        data = (dnx_data_field_map_qual_header_t *) dnxc_data_mgmt_table_data_get(unit, table, DNX_FIELD_HEADER_QUAL_RCH_TIME_STAMP, 0);
        data->dbal_field = DBAL_FIELD_RCH_TIME_STAMP;
        data->offset = 136;
        data->size = 32;
    }
    ...
    int unit)
{
    ...
    if (bcmFieldAppDbMplsIpVXParse < table->keys[0].size)
    {
        data = (dnx_data_field_map_tcam_tcam_pp_app_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmFieldAppDbMplsIpVXParse, 0);
        data->dbal_table = DBAL_TABLE_IPVX_PARSING_TCAM_DB;
    }
...
static shr_error_e
jr2_a0_dnx_data_field_map_port_profile_port_class_profile_set(
    int unit)
{
    int port_class_index;
    dnx_data_field_map_port_profile_port_class_profile_t *data, *default_data;
    dnxc_data_table_t *table;
    int module_index = dnx_data_module_field_map;
    int submodule_index = dnx_data_field_map_submodule_port_profile;
    int table_index = dnx_data_field_map_port_profile_table_port_class_profile;
    SHR_FUNC_INIT_VARS(unit);
    ...
    table->keys[0].size = bcmPortClassCount;
    table->info_get.key_size[0] = bcmPortClassCount;

    
    table->values[0].default_val = "DNX_FIELD_PORT_PROFILE_TYPE_INVALID";
    ...
    DNXC_DATA_ALLOC(table->data, dnx_data_field_map_port_profile_port_class_profile_t, (1 * (table->keys[0].size) + 1  ), "data of dnx_data_field_map_port_profile_table_port_class_profile");

    
    default_data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, -1, -1);
    default_data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_INVALID;
    default_data->dbal_table = DBAL_TABLE_EMPTY;
    default_data->dbal_key = DBAL_FIELD_EMPTY;
    default_data->dbal_result = DBAL_FIELD_EMPTY;
    
    for (port_class_index = 0; port_class_index < table->keys[0].size; port_class_index++)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, port_class_index, 0);
        sal_memcpy(data, default_data, table->size_of_values);
    }
    
    if (bcmPortClassFieldIngressPMF1PacketProcessingPort < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF1PacketProcessingPort, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PP_PORT_INFO_IPMF1;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_IN_PORT_PMF_DATA;
    }
    if (bcmPortClassFieldIngressPMF3PacketProcessingPort < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF3PacketProcessingPort, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PP_PORT_INFO_IPMF3;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_IN_PORT_PMF_DATA;
    }
    if (bcmPortClassFieldEgressPacketProcessingPort < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldEgressPacketProcessingPort, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_EGRESS_PP_PORT;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_OUT_PP_PORT_PMF_DATA;
    }
    if (bcmPortClassFieldIngressPMF1TrafficManagementPort < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF1TrafficManagementPort, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PTC_INFO_IPMF1;
        data->dbal_key = DBAL_FIELD_PTC;
        data->dbal_result = DBAL_FIELD_PTC_PMF_DATA;
    }
    if (bcmPortClassFieldIngressPMF3TrafficManagementPort < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF3TrafficManagementPort, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PTC_INFO_IPMF3;
        data->dbal_key = DBAL_FIELD_PTC;
        data->dbal_result = DBAL_FIELD_PTC_PMF_DATA;
    }
    if (bcmPortClassFieldEgressTrafficManagementPort < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldEgressTrafficManagementPort, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_OUT_TM_PORT_INFO_EPMF;
        data->dbal_key = DBAL_FIELD_OUT_TM_PORT;
        data->dbal_result = DBAL_FIELD_OUT_TM_PORT_PMF_DATA;
    }
    if (bcmPortClassFieldIngressPMF1PacketProcessingPortCs < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF1PacketProcessingPortCs, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PP_PORT_INFO_IPMF1;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_PP_PORT_CS_PROFILE;
    }
    if (bcmPortClassFieldIngressPMF3PacketProcessingPortCs < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF3PacketProcessingPortCs, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PP_PORT_INFO_IPMF3;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_PP_PORT_CS_PROFILE;
    }
    if (bcmPortClassFieldEgressPacketProcessingPortCs < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldEgressPacketProcessingPortCs, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_EGRESS_PP_PORT;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_PP_PORT_CS_PROFILE;
    }
    if (bcmPortClassFieldExternalPacketProcessingPortCs < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldExternalPacketProcessingPortCs, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PP_PORT_INFO_IFWD;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_PP_PORT_CS_PROFILE;
    }
    if (bcmPortClassFieldIngressPMF1TrafficManagementPortCs < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF1TrafficManagementPortCs, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PTC_INFO_IPMF1;
        data->dbal_key = DBAL_FIELD_PTC;
        data->dbal_result = DBAL_FIELD_PTC_CS_PROFILE;
    }
    if (bcmPortClassFieldIngressPMF3TrafficManagementPortCs < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF3TrafficManagementPortCs, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PTC_INFO_IPMF3;
        data->dbal_key = DBAL_FIELD_PTC;
        data->dbal_result = DBAL_FIELD_PTC_CS_PROFILE;
    }
    if (bcmPortClassFieldExternalTrafficManagementPortCs < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldExternalTrafficManagementPortCs, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PTC_INFO_IFWD;
        data->dbal_key = DBAL_FIELD_PTC;
        data->dbal_result = DBAL_FIELD_PTC_CS_PROFILE;
    }
    if (bcmPortClassFieldIngressPMF1PacketProcessingPortGeneralData < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF1PacketProcessingPortGeneralData, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT_GENERAL_DATA;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PP_PORT_INFO_IPMF1;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_IN_PORT_PMF_DATA;
    }
    if (bcmPortClassFieldIngressPMF1PacketProcessingPortGeneralDataHigh < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF1PacketProcessingPortGeneralDataHigh, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT_GENERAL_DATA_HIGH;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PP_PORT_INFO_IPMF1;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_IN_PORT_PMF_DATA;
    }
    if (bcmPortClassFieldIngressPMF3PacketProcessingPortGeneralData < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF3PacketProcessingPortGeneralData, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT_GENERAL_DATA;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PP_PORT_INFO_IPMF3;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_IN_PORT_PMF_DATA;
    }
    if (bcmPortClassFieldIngressPMF3PacketProcessingPortGeneralDataHigh < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressPMF3PacketProcessingPortGeneralDataHigh, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_PORT_GENERAL_DATA_HIGH;
        data->dbal_table = DBAL_TABLE_FIELD_PMF_PP_PORT_INFO_IPMF3;
        data->dbal_key = DBAL_FIELD_PP_PORT;
        data->dbal_result = DBAL_FIELD_IN_PORT_PMF_DATA;
    }
    if (bcmPortClassFieldIngressVport < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldIngressVport, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_IN_LIF_INGRESS;
    }
    if (bcmPortClassFieldEgressVport < table->keys[0].size)
    {
        data = (dnx_data_field_map_port_profile_port_class_profile_t *) dnxc_data_mgmt_table_data_get(unit, table, bcmPortClassFieldEgressVport, 0);
        data->profile_type = DNX_FIELD_PORT_PROFILE_TYPE_OUT_ETH_RIF_EGRESS;
    }
    ...
    int unit)
{
    ...
    data_index = dnx_data_field_map_qual_table_cs_special_general_data_qual;
    table = &submodule->tables[data_index];
    table->set = jr2_a0_dnx_data_field_map_qual_cs_special_general_data_qual_set;
    data_index = dnx_data_field_map_qual_table_cs_pem_scratch_pad_mapping;
    table = &submodule->tables[data_index];
    table->set = jr2_a0_dnx_data_field_map_qual_cs_pem_scratch_pad_mapping_set;
    ...
    data_index = dnx_data_field_map_qual_table_header;
    table = &submodule->tables[data_index];
    table->set = jr2_a0_dnx_data_field_map_qual_header_set;
    ...
    submodule_index = dnx_data_field_map_submodule_port_profile;
    submodule = &module->submodules[submodule_index];
    ...
    data_index = dnx_data_field_map_port_profile_table_port_class_profile;
    table = &submodule->tables[data_index];
    table->set = jr2_a0_dnx_data_field_map_port_profile_port_class_profile_set;
