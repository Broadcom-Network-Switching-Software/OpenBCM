...
mdb_em_db_dump(int  unit,  dnx_sw_state_dump_filters_t  filters)
{
    ...
    SHR_IF_ERR_EXIT(mdb_em_db_shadow_em_db_dump(unit, -1, -1, filters));
...
int
mdb_em_db_shadow_em_db_dump(int  unit,  int  shadow_em_db_idx_0,  int  shadow_em_db_idx_1,  dnx_sw_state_dump_filters_t  filters)
{
    int i0 = shadow_em_db_idx_0, I0 = shadow_em_db_idx_0 + 1;
    int i1 = shadow_em_db_idx_1, I1 = shadow_em_db_idx_1 + 1, org_i1 = shadow_em_db_idx_1;
    char *s0 = "", *s1 = "";
    DNXC_SW_STATE_INIT_FUNC_DEFS;
        ...
        for(; i0 < I0; i0++) {
            ...
            if (i1 == -1) {
                ...
                i1 = dnx_sw_state_dump_check_all_the_same(unit,
                    ((mdb_em_sw_state_t*)sw_state_roots_array[unit][MDB_EM_MODULE_ID])->shadow_em_db[i0]
                    , sizeof(*((mdb_em_sw_state_t*)sw_state_roots_array[unit][MDB_EM_MODULE_ID])->shadow_em_db[i0]), I1, &s1) ? I1 - 1 : 0;
            }

            if(i1 >= dnx_sw_state_get_nof_elements(unit, DNX_SW_STATE_MDB_EM_DB__SHADOW_EM_DB, ((mdb_em_sw_state_t*)sw_state_roots_array[unit][MDB_EM_MODULE_ID])->shadow_em_db[i0]))
            {
                LOG_CLI((BSL_META("mdb_em_db[]->((mdb_em_sw_state_t*)sw_state_roots_array[][MDB_EM_MODULE_ID])->shadow_em_db[][]: ")));
                LOG_CLI((BSL_META("Invalid index: %d \n"),i1));
                SHR_EXIT();
            ...
            if(dnx_sw_state_get_nof_elements(unit, DNX_SW_STATE_MDB_EM_DB__SHADOW_EM_DB, ((mdb_em_sw_state_t*)sw_state_roots_array[unit][MDB_EM_MODULE_ID])->shadow_em_db[i0]) == 0)
            {
                SHR_EXIT();
            }
            ...
            for(; i1 < I1; i1++) {
                ...
                DNX_SW_STATE_PRINT_MONITOR(
                    unit,
                    "mdb_em_db[%d]->","((mdb_em_sw_state_t*)sw_state_roots_array[%d][MDB_EM_MODULE_ID])->","shadow_em_db[%s%d][%s%d]: ", s0, i0, s1, i1);
                ...
                SW_STATE_HTB_DEFAULT_PRINT(
                    unit,
                    MDB_EM_MODULE_ID,
                    &((mdb_em_sw_state_t*)sw_state_roots_array[unit][MDB_EM_MODULE_ID])->shadow_em_db[i0][i1]);
