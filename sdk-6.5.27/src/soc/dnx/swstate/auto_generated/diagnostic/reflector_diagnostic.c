...
int
reflector_sw_db_info_sbfd_reflector_discriminator_dump(int  unit,  dnx_sw_state_dump_filters_t  filters)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;
    ...
    if (dnx_sw_state_compare(filters.namefilter, "reflector sbfd_reflector_discriminator") != TRUE)
    {
         SHR_EXIT();
    }
    ...
    if (filters.nocontent)
    {
         DNX_SW_STATE_PRINT(unit, "swstate reflector sbfd_reflector_discriminator\n");
    }
    else
    {
        dnx_sw_state_dump_attach_file(
            unit,
            "reflector_sw_db_info/sbfd_reflector_discriminator.txt",
            "reflector_sw_db_info[%d]->","((reflector_sw_db_info_t*)sw_state_roots_array[%d][REFLECTOR_MODULE_ID])->","sbfd_reflector_discriminator: ");
...
const char* reflector_sw_db_info_layout_str[REFLECTOR_SW_DB_INFO_INFO_NOF_ENTRIES] = {
    ...
    "REFLECTOR_SW_DB_INFO~SBFD_REFLECTOR_DISCRIMINATOR~",
};
#endif 
