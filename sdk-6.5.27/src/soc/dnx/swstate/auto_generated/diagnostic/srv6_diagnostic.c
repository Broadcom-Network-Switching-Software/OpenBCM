...
int
srv6_modes_lpm_destination_for_usp_trap_dump(int  unit,  dnx_sw_state_dump_filters_t  filters)
{
    DNXC_SW_STATE_INIT_FUNC_DEFS;
    ...
    if (dnx_sw_state_compare(filters.namefilter, "srv6 lpm_destination_for_usp_trap") != TRUE)
    {
         SHR_EXIT();
    }
    ...
    if (filters.nocontent)
    {
         DNX_SW_STATE_PRINT(unit, "swstate srv6 lpm_destination_for_usp_trap\n");
    }
    else
    {
        dnx_sw_state_dump_attach_file(
            unit,
            "srv6_modes/lpm_destination_for_usp_trap.txt",
            "srv6_modes[%d]->","((srv6_modes_struct*)sw_state_roots_array[%d][SRV6_MODULE_ID])->","lpm_destination_for_usp_trap: ");
...
const char* srv6_modes_layout_str[SRV6_MODES_INFO_NOF_ENTRIES] = {
    ...
    "SRV6_MODES~LPM_DESTINATION_FOR_USP_TRAP~",
};
#endif 
