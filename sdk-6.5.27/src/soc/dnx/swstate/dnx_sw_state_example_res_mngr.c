    ...
    int unit)
{
    ...
    create_data.advanced_algorithm = dnx_data_infra_tests.res_mngr_example.advanced_algorithm_get(unit);

    if (dnx_data_infra_tests.res_mngr_example.extra_arg_get(unit))
    {
        sal_memset(&extra_create_info, 0, sizeof(resource_tag_bitmap_utils_extra_arg_create_info_t));
        ...
        extra_create_info.data_per_entry_size = dnx_data_infra_tests.res_mngr_example.data_per_entry_size_get(unit);
