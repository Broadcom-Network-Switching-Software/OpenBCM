    ...
    DNX_PLL4_TYPE pll4_type)
{
    ...
    if (dnx_data_pll.pll4.feature_get(unit, dnx_data_pll_pll4_extra_post_div_dis))
    {
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EXTRA_POST_DIV_DIS, INST_SINGLE, TRUE);
    }
    ...
    int enable)
{
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_TS_PLL_DITHER_CONFIG, &entry_handle_id));

    if (enable == TRUE)
    {

        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_IN_RESET, INST_SINGLE, 0);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

        sal_usleep(1);

        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_ENABLE, INST_SINGLE, 1);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    else
    {
        ...
        dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_IN_RESET, INST_SINGLE, 1);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    if (soc_sand_is_emulation_system(unit) == 0)
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_PLL4_STATUS, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PLL4_ID,
                                   dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_TS)->pll4_id);
        SHR_IF_ERR_EXIT(dnxcmn_polling
                        (unit, DNXCMN_TIMEOUT, DNXCMN_MIN_POLLS, entry_handle_id, DBAL_FIELD_LOCK_STATUS, 1));
    }
#endif
...
static shr_error_e
soc_dnx_pll_bs_holdover_enable(
    int unit,
    int enable)
{
    uint32 entry_handle_id;
    ...
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_BS_PLL_HOLDOVER_CONFIG, &entry_handle_id));

    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_BYPASS, INST_SINGLE, ((enable == TRUE) ? 0 : 1));
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));

    if (enable == FALSE)
    {

        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_NDIV_INT_VALUE, INST_SINGLE,
                                     dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_BS)->ndiv_int);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }

    dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_WRITE_EN, INST_SINGLE, 1);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    ...
    if (soc_sand_is_emulation_system(unit) == 0)
    {
        ...
        SHR_IF_ERR_EXIT(dnxcmn_polling
                        (unit, DNXCMN_TIMEOUT, DNXCMN_MIN_POLLS, entry_handle_id, DBAL_FIELD_WRITE_EN, 0));

        SHR_IF_ERR_EXIT(DBAL_HANDLE_CLEAR(unit, DBAL_TABLE_PLL4_STATUS, entry_handle_id));
        dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_PLL4_ID,
                                   dnx_data_pll.pll4.config_get(unit, DNX_PLL4_TYPE_BS)->pll4_id);
        SHR_IF_ERR_EXIT(dnxcmn_polling
                        (unit, DNXCMN_TIMEOUT, DNXCMN_MIN_POLLS, entry_handle_id, DBAL_FIELD_LOCK_STATUS, 1));
    }
#endif
...
soc_dnx_pll_clk_div_get(
    int unit,
    DNXC_SERDES_REF_CLOCK_TYPE ref_clock_in,
    ...
    uint32 is_bypass)
{
    SHR_FUNC_INIT_VARS(unit);
    {
        if (ref_clock_in == DNXC_SERDES_REF_CLOCK_125)
        {
            *pdiv = 1;
        ...
        else if (ref_clock_in == DNXC_SERDES_REF_CLOCK_156_25)
        {
            *pdiv = 1;
        ...
        else if (ref_clock_in == DNXC_SERDES_REF_CLOCK_312_5)
        {
            *pdiv = 2;
    ...
    DNXC_SERDES_REF_CLOCK_TYPE ref_clock_out)
{
    ...
    is_bypass = (ref_clock_out == DNXC_SERDES_REF_CLOCK_BYPASS) ? 1 : 0;
    ...
    int unit)
{
    ...
    if (dnx_data_pll.pll3.nof_pll3_get(unit) != 0)
    {
        for (pll3_type = DNX_PLL3_TYPE_FIRST; pll3_type < DNX_PLL3_TYPE_COUNT; pll3_type++)
        {
            ...
            if ((in_freq != DNXC_SERDES_REF_CLOCK_DISABLE) && (out_freq != DNXC_SERDES_REF_CLOCK_DISABLE))
            {
    ...
    int unit)
{
    ...
    if ((dnx_data_pll.general.feature_get(unit, dnx_data_pll_general_ts_freq_lock) == TRUE)
        && dnx_data_pll.pll4.feature_get(unit, dnx_data_pll_pll4_ts_pll_dither_support))
    {

        SHR_IF_ERR_EXIT(soc_dnx_pll_ts_dither_enable
                        (unit, dnx_data_pll.pll4.feature_get(unit, dnx_data_pll_pll4_ts_pll_dither_enable)));
    }

    if (dnx_data_pll.pll4.feature_get(unit, dnx_data_pll_pll4_bs_pll_holdover_support))
    {

        SHR_IF_ERR_EXIT(soc_dnx_pll_bs_holdover_enable
                        (unit, dnx_data_pll.pll4.feature_get(unit, dnx_data_pll_pll4_bs_pll_holdover_enable)));
    }
...
int
soc_dnx_pre_reset_pll_init(
    int unit)
{
    return soc_dnx_pll_init(unit, 1);
}
int
soc_dnx_post_reset_pll_init(
    int unit)
{
    return soc_dnx_pll_init(unit, 0);
}
