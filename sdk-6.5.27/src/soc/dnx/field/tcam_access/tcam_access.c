    ...
    uint32 *position)
{
    ...
    SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(dnx_field_tcam_access_sw.entry_location_hash.find(unit, core, &access_id, position),
                                    _SHR_E_NOT_FOUND);
    ...
    uint8 *valid_bits)
{
    ...
    *valid_bits = key_valid_bits;
    ...
    sal_memcpy(key, data_key, BITS2BYTES(SINGLE_KEY_BIT_SIZE));
    sal_memcpy(key_mask, data_mask, BITS2BYTES(SINGLE_KEY_BIT_SIZE));
...
static shr_error_e
dnx_field_tcam_access_entry_key_hw_write(
    int unit,
    int core,
    uint32 bank_id,
    uint32 bank_offset,
    ...
    if (ACCESS_HW)
    {
        if (dnx_data_field.tcam.feature_get(unit, dnx_data_field_tcam_mixed_ratio_supported))
        {
            SHR_IF_ERR_EXIT(dnx_tcam_access_hw_v1_entry_key_write
                            (unit, core, bank_id, bank_offset, key_mode, part, valid, key, key_mask));
        }
        else
        {
        }
    ...
    uint8 valid)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_key_hw_write
                    (unit, core, bank_id, bank_offset, entry_key, entry_mask, key_mode, part, valid));
    ...
    uint32 *payload)
{
    ...
    else if (ACCESS_HW)
    {
        if (dnx_data_field.tcam.feature_get(unit, dnx_data_field_tcam_mixed_ratio_supported))
        {
            SHR_IF_ERR_EXIT(dnx_tcam_access_hw_v1_payload_read(unit, core, action_bank_id, bank_offset, payload));
        }
        else
        {
        }
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "both TCAM_SHADOW_READ and ACCESS_HW internal flags are turned off\n");
    }
...
static shr_error_e
dnx_field_tcam_access_tcam_hit_sw_state_get(
    int unit,
    int core_id_specific,
    uint32 hw_hitibt_unit,
    uint32 line_chunk_id,
    uint32 hitbit_indication_bmp[1])
{
    uint32 bitmap_size = dnx_data_field.tcam.tcam_entries_per_hit_indication_entry_get(unit);
    SHR_BITDCL hw_bitmap_get[1] = { 0 };
    SHR_FUNC_INIT_VARS(unit);

    SHR_BITCLR_RANGE(hw_bitmap_get, 0, bitmap_size);
    SHR_IF_ERR_EXIT(dnx_field_tcam_hit_indication.tcam_action_hit_indication.bmp.bit_range_read(unit,
                                                                                                core_id_specific,
                                                                                                hw_hitibt_unit,
                                                                                                (bitmap_size *
                                                                                                 line_chunk_id), 0,
                                                                                                bitmap_size,
                                                                                                hw_bitmap_get));
    SHR_BITCOPY_RANGE(hitbit_indication_bmp, 0, hw_bitmap_get, 0, bitmap_size);
...
static shr_error_e
dnx_field_tcam_access_tcam_hit_sw_state_set(
    int unit,
    int core_id_specific,
    uint32 hw_hitibt_unit,
    uint32 line_chunk_id,
    uint32 hitbit_indication_bmp)
{
    uint32 bitmap_size = dnx_data_field.tcam.tcam_entries_per_hit_indication_entry_get(unit);
    SHR_BITDCL hw_bitmap_set[1] = { 0 };
    SHR_FUNC_INIT_VARS(unit);

    SHR_BITCOPY_RANGE(hw_bitmap_set, 0, &hitbit_indication_bmp, 0, bitmap_size);
    SHR_IF_ERR_EXIT(dnx_field_tcam_hit_indication.tcam_action_hit_indication.bmp.bit_range_write(unit,
                                                                                                 core_id_specific,
                                                                                                 hw_hitibt_unit,
                                                                                                 (bitmap_size *
                                                                                                  line_chunk_id), 0,
...
static void
dnx_field_tcam_access_hit_bit_position_to_ids(
    int unit,
    uint32 position,
    uint32 *hw_bank_id,
    uint32 *line_chunk_id,
    uint32 *entry_id_in_chunk)
{
    uint32 max_nof_entries_in_chunk;
    uint32 bank_id;
    ...
    if (dnx_data_field.tcam.feature_get(unit, dnx_data_field_tcam_hitbit_splits_banks_like_sram))
    {
        ...
        *hw_bank_id = bank_sram_id;
        *entry_id_in_chunk =
            (((position % max_nof_entries_in_chunk) - (bank_sram_id % 2)) / max_nof_srams_per_tcam_bank);
    }
    else
    {
        max_nof_entries_in_chunk = (dnx_data_field.tcam.tcam_entries_per_hit_indication_entry_get(unit));
        *hw_bank_id = bank_id;
        *entry_id_in_chunk = position % max_nof_entries_in_chunk;
    }

    *line_chunk_id = ((DNX_FIELD_TCAM_LOCATION_ABSOLUTE_TO_BANK_OFFSET(position)) / max_nof_entries_in_chunk);
}
    ...
    uint8 *entry_hit_p)
{
    ...
    *entry_hit_p = 0;
    ...
    dnx_field_tcam_access_hit_bit_position_to_ids(unit, position, &hw_bank_id, &line_chunk_id, &entry_id_in_chunk);

    if (dnx_data_field.tcam.feature_get(unit, dnx_data_field_tcam_hitbit_splits_banks_like_sram))
    {
        SHR_IF_ERR_EXIT(dnx_tcam_access_hw_v1_tcam_hit_per_core_get
                        (unit, core_id_specific, position, hw_bank_id, line_chunk_id, mem_hit_field_value));
    }
    else
    {
    }
    ...
    {
        uint32 hit_bit_sw_state_value_uint32[1];
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_tcam_hit_sw_state_get
                        (unit, core_id_specific, hw_bank_id, line_chunk_id, hit_bit_sw_state_value_uint32));
        SHR_IF_ERR_EXIT(utilex_bitstream_or(hit_bit_sw_state_value_uint32, mem_hit_field_value, 1));
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_tcam_hit_sw_state_set
                        (unit, core_id_specific, hw_bank_id, line_chunk_id, hit_bit_sw_state_value_uint32[0]));
        hit_indication_value = utilex_bitstream_test_bit(hit_bit_sw_state_value_uint32, entry_id_in_chunk);
    }
    else
    {
        hit_indication_value = utilex_bitstream_test_bit(mem_hit_field_value, entry_id_in_chunk);
    }
    ...
    uint16 *entry_hit_p)
{
    ...
    *entry_hit_p = 0;
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_iter)
    {
        ...
        rv = dnx_field_tcam_access_index_get(unit, core_iter, entry_handle, &position);
        ...
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_tcam_hit_per_core_get(unit, core_iter, position, &entry_hit));
    ...
    uint32 position)
{
    ...
    tcam_indirect_mask = 0x1 << entry_id_in_chunk;

    if (dnx_data_field.tcam.feature_get(unit, dnx_data_field_tcam_hitbit_splits_banks_like_sram))
    {
        SHR_IF_ERR_EXIT(dnx_tcam_access_hw_v1_tcam_hit_per_core_flush
                        (unit, core_id_specific, position, hw_bank_id, line_chunk_id, tcam_indirect_mask));
    }
    else
    {
    }
    ...
    if (dnx_data_field.features.hitbit_volatile_get(unit))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_tcam_hit_sw_state_get
                        (unit, core_id_specific, hw_bank_id, line_chunk_id, hit_bit_sw_state_value_uint32));
        hit_bit_sw_state_value_uint32[0] = hit_bit_sw_state_value_uint32[0] & ~tcam_indirect_mask;
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_tcam_hit_sw_state_set
                        (unit, core_id_specific, hw_bank_id, line_chunk_id, hit_bit_sw_state_value_uint32[0]));
    }
    ...
    uint32 entry_handle)
{
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_iter)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_tcam_hit_per_core_flush(unit, core_iter, position));
    }
...
shr_error_e
dnx_field_tcam_access_tcam_hit_indication_reset_all(
    int unit)
{
    bcm_core_t core_iter;
    int hitbit_unit_index;
    int hitbit_unit_per_bank_index;
    uint32 bank_id;
    uint32 nof_banks = dnx_data_field.tcam.nof_banks_get(unit);
    int chunk_index;
    int nof_chunks;
    int nof_big_bank_lines_per_hitbit_unit = dnx_data_field.tcam.nof_big_bank_lines_per_hitbit_unit_get(unit);
    int nof_small_bank_lines_per_hitbit_unit = dnx_data_field.tcam.nof_small_bank_lines_per_hitbit_unit_get(unit);
    uint32 mem_entry_data;
    soc_mem_t mem = INVALIDm;
    uint32 nof_hitbit_units_per_bank;
#ifndef ADAPTER_SERVER_MODE
    uint32 mem_addr;
    uint8 at;
    uint32 reg_val_address;
    uint32 reg_val_command;
    soc_reg_above_64_val_t reg_val_zero;
    uint32 mask = SAL_UPTO_BIT(dnx_data_field.tcam.tcam_entries_per_hit_indication_entry_get(unit));
    int tcam_indirect_command_version = dnx_data_field.tcam.tcam_indirect_command_version_get(unit);
    int is_volatile = dnx_data_field.features.hitbit_volatile_get(unit);
#endif
    ...
    if (dnx_data_field.tcam.feature_get(unit, dnx_data_field_tcam_hitbit_splits_banks_like_sram))
    {
        nof_hitbit_units_per_bank = 2;
    }
    else
    {
        nof_hitbit_units_per_bank = 1;
    }
    ...
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core_iter)
    {
        ...
        for (bank_id = 0; bank_id < nof_banks; bank_id++)
        {
            if (FIELD_TCAM_BANK_IS_BIG_BANK(bank_id))
            {
                ...
                nof_chunks = nof_big_bank_lines_per_hitbit_unit;
            }
            else
            {
                mem = TCAM_TCAM_ACTION_HIT_INDICATION_SMALLm;
                nof_chunks = nof_small_bank_lines_per_hitbit_unit;
            }
            for (hitbit_unit_per_bank_index = 0; hitbit_unit_per_bank_index < nof_hitbit_units_per_bank;
                 hitbit_unit_per_bank_index++)
            {
                for (chunk_index = 0; chunk_index < nof_chunks; chunk_index++)
                {
                    ...
                    if (ACCESS_IS_INITIALIZED(unit))
                    {
                        SHR_IF_ERR_EXIT(access_regmem(unit, FLAG_ACCESS_IS_WRITE,
                                                      mem >=
                                                      ACCESS_NOF_MCM_MAPPED_MEMS ? ACCESS_INVALID_REGMEM_ID :
                                                      access_mcm2access_mem_ids[(unsigned) mem],
                                                      core_iter, hitbit_unit_index, chunk_index, &mem_entry_data));
                    }
                    else
#endif
                    {
                        SHR_IF_ERR_EXIT(soc_mem_array_write
                                        (unit, mem, hitbit_unit_index, TCAM_BLOCK(unit, core_iter), chunk_index,
                                         &mem_entry_data));
                    }
#else
                    if (is_volatile)
                    ...
                    {
                        mem_addr =
                            soc_mem_addr_get(unit, mem, hitbit_unit_index, TCAM_BLOCK(unit, core_iter), chunk_index,
                                             &at);

                        reg_val_address = 0;
                        soc_reg_field_set(unit, TCAM_INDIRECT_COMMAND_ADDRESSr, &reg_val_address,
                                          INDIRECT_COMMAND_ADDRf, mem_addr);
                        soc_reg_field_set(unit, TCAM_INDIRECT_COMMAND_ADDRESSr, &reg_val_address,
                                          INDIRECT_COMMAND_TYPEf, 0x0);
                        SHR_IF_ERR_EXIT(WRITE_TCAM_INDIRECT_COMMAND_ADDRESSr(unit, _SHR_CORE_ALL, reg_val_address));
                        SHR_IF_ERR_EXIT(WRITE_TCAM_INDIRECT_WR_MASKr(unit, _SHR_CORE_ALL, mask));
                        SHR_IF_ERR_EXIT(WRITE_TCAM_INDIRECT_COMMAND_WR_DATAr(unit, _SHR_CORE_ALL, reg_val_zero));
                        SHR_IF_ERR_EXIT(WRITE_TCAM_INDIRECT_COMMANDr(unit, _SHR_CORE_ALL, reg_val_command));
                    }
#endif
                }
                hitbit_unit_index++;
    ...
    uint32 position)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_hash_key_insert_at_index
                    (unit, core, bank_id, key, key_mask, key_size, bank_offset));
    ...
    uint32 line_to)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_single_entry_payload_write
                    (unit, core, DNX_FIELD_TCAM_LOCATION_ABSOLUTE_TO_BANK_ID(line_to),
                     DNX_FIELD_TCAM_LOCATION_ABSOLUTE_TO_BANK_OFFSET(line_to), shadow_payload,
                     DNX_FIELD_TCAM_ACCESS_KEY_MODE_HALF, DNX_FIELD_TCAM_ACCESS_PART_ALL, FALSE));

    SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_key_hw_write
                    (unit, core, DNX_FIELD_TCAM_LOCATION_ABSOLUTE_TO_BANK_ID(line_to),
                     DNX_FIELD_TCAM_LOCATION_ABSOLUTE_TO_BANK_OFFSET(line_to), shadow_key, shadow_mask, key_mode,
                     DNX_FIELD_TCAM_ACCESS_PART_ALL, shadow_key_valid_bits));
    ...
    uint32 flags)
{
    ...
    uint8 id_allocated = FALSE;
    uint8 location_alloc = FALSE;
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_handler_entry_valid_set(unit, core, entry->entry_hw_id, !entry->is_invalid));
...
exit:
    if (id_allocated && !location_alloc)
    {
    ...
    uint32 *access_id)
{
    ...
    SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(dnx_field_tcam_access_sw.
                                    entry_location_hash.get_by_index(unit, core, absolute_location, access_id),
                                    _SHR_E_NOT_FOUND);
    ...
    uint8 remove_old_from_hw)
{
    ...
    if (remove_old_from_hw)
    {
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_delete_aux(unit, core, app_id, position_from, TRUE));
    }
    ...
    uint32 line_to)
{
    ...
    if (entry_in_use)
    {
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_move(unit, core, app_id, line_from, line_to, FALSE));
    }

    SHR_IF_ERR_EXIT(dnx_field_tcam_access_sw.entry_in_use_bmp.v_bit.bit_get(unit, core, line_from + 1, &entry_in_use));
    if (entry_in_use)
    {
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_move(unit, core, app_id, line_from + 1, line_to + 1, FALSE));
    }
    ...
    uint32 bank_offset)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_key_read_shadow_with_key_mode
                    (unit, core, bank_id, bank_offset, shadow_key, &shadow_key_valid, &key_mode));
    ...
    SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_key_hw_write
                    (unit, core, bank_id, bank_offset, shadow_key, shadow_mask, key_mode,
                     DNX_FIELD_TCAM_ACCESS_PART_ALL, shadow_key_valid));
