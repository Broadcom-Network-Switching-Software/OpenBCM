...
dnx_field_tcam_access_sw_state_init(
    int unit)
{
    sw_state_htb_create_info_t hash_init;
    int core;
    ...
    hash_init.max_nof_elements = dnx_data_field.tcam.tcam_banks_size_get(unit);
    hash_init.hash_func = SWSTATE_HTB_LARGE_HASH;
    DNXCMN_CORES_ITER(unit, BCM_CORE_ALL, core)
    {
    ...
    int unit)
{
    ...
    int nof_srams_per_bank = dnx_data_field.tcam.action_width_selector_size_get(unit);
    bcm_core_t core;
    ...
    {
        int bank_id;
        int hitbit_unit_index = 0;
        for (bank_id = 0; bank_id < nof_banks; bank_id++)
        {
            int nof_bits_to_alloc;
            int bank_size;
            int hitibt_units_to_alloc;
            int hitibt_unit_in_bank;
            if (FIELD_TCAM_BANK_IS_BIG_BANK(bank_id))
            {
                bank_size = dnx_data_field.tcam.nof_big_bank_lines_get(unit);
            }
            else
            {
                bank_size = dnx_data_field.tcam.nof_small_bank_lines_get(unit);
            }
            if (dnx_data_field.tcam.feature_get(unit, dnx_data_field_tcam_hitbit_splits_banks_like_sram))
            {
                nof_bits_to_alloc = bank_size / nof_srams_per_bank;
                hitibt_units_to_alloc = nof_srams_per_bank;
                if (bank_size % nof_srams_per_bank != 0)
                {
                    SHR_ERR_EXIT(_SHR_E_CONFIG, "Bank %d has size %d, not divisible by %d.\r\n",
                                 bank_id, bank_size, nof_srams_per_bank);
                }
            }
            else
            {
                nof_bits_to_alloc = bank_size;
                hitibt_units_to_alloc = 1;
            }

            for (hitibt_unit_in_bank = 0; hitibt_unit_in_bank < hitibt_units_to_alloc; hitibt_unit_in_bank++)
            {
                SHR_IF_ERR_EXIT(dnx_field_tcam_hit_indication.tcam_action_hit_indication.
                                bmp.alloc_bitmap(unit, core, hitbit_unit_index, nof_bits_to_alloc));
                hitbit_unit_index++;
            }
        }
    }
