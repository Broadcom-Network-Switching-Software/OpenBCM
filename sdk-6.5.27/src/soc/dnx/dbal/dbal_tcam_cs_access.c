    ...
    int check_validity_only)
{
    ...
    if ((entry_handle->table->core_mode != DBAL_CORE_MODE_SBC) && (entry_handle->phy_entry.core_id != DBAL_CORE_ALL))
    {

        SHR_IF_ERR_EXIT(dbal_hl_access_block_calc
                        (unit, entry_handle, entry_handle->phy_entry.core_id, stage_info->cmd_memory, &block_index_info,
                         NULL, 1, &first_block, &num_of_blocks));
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (entry_handle->table->core_mode == DBAL_CORE_MODE_DPC)
    {
        ...
        entry_handle->phy_entry.core_id = DBAL_CORE_ALL;
    }
    ...
    while ((!entry_found) && (!iterator_info->is_end))
    {
        if (iterator_info->used_first_key)
        {
            if (entry_handle->table->core_mode == DBAL_CORE_MODE_DPC)
            {
                ...
                entry_handle->phy_entry.core_id = (entry_handle->phy_entry.core_id + 1) % DBAL_MAX_NUM_OF_CORES;
            }

            if ((entry_handle->table->core_mode == DBAL_CORE_MODE_SBC) || (entry_handle->phy_entry.core_id == 0))
            {
