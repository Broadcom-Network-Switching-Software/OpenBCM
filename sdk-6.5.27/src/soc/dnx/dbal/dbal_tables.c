    ...
    int unit)
{
    ...
    SHR_ALLOC_ERR_EXIT(logical_tables_info[unit], sizeof(dbal_logical_tables_info_t), "dbal_logical_tables_info_t",
                       "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    ...
    int unit)
{
    ...
    DNX_INIT_TIME_ANALYZER_DBAL_INIT_START(unit, DNX_INIT_TIME_ANALYSER_DBAL_L_POST_PROCESS);
    SHR_IF_ERR_EXIT(dbal_db_init_tables_post_process(unit, logical_tables_info[unit]->logical_tables));
    DNX_INIT_TIME_ANALYZER_DBAL_INIT_STOP(unit, DNX_INIT_TIME_ANALYSER_DBAL_L_POST_PROCESS);

    if (DBAL_DYNAMIC_XML_TABLES_ENABLED)
    {
        char file_path[RHFILE_MAX_SIZE] = { 0 };

        SHR_IF_ERR_EXIT(dbx_file_get_db_directory(RHFILE_MAX_SIZE, file_path));
    ...
    dbal_logical_table_t * table)
{
    ...
    {
        sal_memset(table->kbp_handles, 0, sizeof(kbp_db_handles_t) * DNX_KBP_NOF_DBS_PER_CORE);
    }
#endif
    ...
    int table_id)
{
    ...
    if (table_entry->multi_res_info)
    {
        ...
        if (SHR_IS_BITSET(table_entry->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE)
            && (table_id < DBAL_NOF_STATIC_TABLES)
            &&
            (sal_strncasecmp
             (table_entry->table_name, "ETM_PP_DESCRIPTOR_EXPANSION", sizeof(table_entry->table_name)) != 0))
        {
    ...
    int unit)
{
    ...
    for (ii = 0; ii < DBAL_NOF_TABLES; ii++)
    {
        ...
        dbal_table_deinit(unit, table_entry, ii);
    }
    ...
    void *access_info)
{
    ...
    switch (access_method)
    {
        ...
        case DBAL_ACCESS_METHOD_KBP:
            ...
            if (table->kbp_handles)
            {
                int core_id = ((kbp_db_handles_t *) (access_info))->core_id;
                if (core_id < DNX_KBP_NOF_DBS_PER_CORE)
                {
                    sal_memcpy(&((kbp_db_handles_t *) (table->kbp_handles))[core_id],
                               ((kbp_db_handles_t *) (access_info)), sizeof(kbp_db_handles_t));
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Invalid core_id (%d) provided for table %s\n",
                                 core_id, table->table_name);
                }
    ...
    switch (access_method)
    {
        ...
        case DBAL_ACCESS_METHOD_KBP:
            ...
            sal_memcpy(((kbp_db_handles_t *) (access_info)), ((kbp_db_handles_t *) (table->kbp_handles)),
                       sizeof(kbp_db_handles_t) * DNX_KBP_NOF_DBS_PER_CORE);
            break;
#endif
        case DBAL_ACCESS_METHOD_PEMLA:
        case DBAL_ACCESS_METHOD_TCAM_CS:
    ...
    dbal_tables_e table_id)
{
    ...
    for (char_index = 0; char_index < sal_strnlen(table->table_name, sizeof(table->table_name)); char_index++)
    {
        SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.
...
{
    int iter;
    int nof_range_key_fields = 0;
    int max_ranges_per_entry = 0;
    int valid_range_key_field_min_width = 0;
    int valid_range_key_field_max_width = 0;
    uint8 is_range_key_field_valid_for_table = FALSE;
    char invalid_range_size_error_message[35] = { 0 };
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        is_range_key_field_valid_for_table = TRUE;
        max_ranges_per_entry = DNX_KBP_MAX_NOF_RANGES;
        valid_range_key_field_min_width = DNX_KBP_MAX_RANGE_SIZE_IN_BITS;
        valid_range_key_field_max_width = DNX_KBP_MAX_RANGE_SIZE_IN_BITS;
        sal_snprintf(invalid_range_size_error_message, 35, "can only be %d bits in size",
    ...
    for (iter = 0; iter < nof_key_fields; iter++)
    {
        ...
        {
            if (is_range_key_field_valid_for_table == FALSE)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "Range key fields are not supported for tables of type %s with %s access method\n",
                             dbal_table_type_to_string(unit, table->table_type),
                             dbal_access_method_to_string(unit, table->access_method));
            }
            nof_range_key_fields++;
            if (nof_range_key_fields > max_ranges_per_entry)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Number of range key fields allowed (%d) is exceeded\n",
                             max_ranges_per_entry);
            }
            if ((keys_info[iter].field_nof_bits != valid_range_key_field_min_width) &&
                (keys_info[iter].field_nof_bits != valid_range_key_field_max_width))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Range fields for %s tables %s\n",
                             dbal_access_method_to_string(unit, table->access_method),
        ...
        SHR_IF_ERR_EXIT(dbal_fields_field_type_max_value_get(unit, keys_info[iter].field_id, &field_type_max_value));
        table->keys_info[iter].max_value =
            UTILEX_MIN(field_type_max_value, utilex_power_of_2(table->keys_info[iter].field_nof_bits) - 1);
        if (keys_info[iter].max_value != 0)
        {
            table->keys_info[iter].max_value = UTILEX_MIN(table->keys_info[iter].max_value, keys_info[iter].max_value);
        }
    ...
    dbal_logical_table_t * table)
{
    ...
    for (iter = 0; iter < nof_result_fields; iter++)
    {
        uint32 field_type_max_value;
        dbal_field_types_basic_info_t *field_type_info;
        dbal_table_field_info_t *table_field = &table->multi_res_info[result_type_index].results_info[iter];
        ...
        table_field->field_id = results_info[iter].field_id;

        if (results_info[iter].field_nof_bits != DBAL_USE_DEFAULT_SIZE)
        {
            table_field->field_nof_bits = results_info[iter].field_nof_bits;
        }
        else
        {
            table_field->field_nof_bits = field_type_info->max_size;
        }

        SHR_IF_ERR_EXIT(dbal_fields_field_type_max_value_get(unit, results_info[iter].field_id, &field_type_max_value));
        table_field->max_value = UTILEX_MIN(field_type_max_value, utilex_power_of_2(table_field->field_nof_bits) - 1);
        if (results_info[iter].max_value != 0)
        {
            table_field->max_value = UTILEX_MIN(table_field->max_value, results_info[iter].max_value);
        }

        if (results_info[iter].is_valid_indication_needed)
        {
            if (table_field->field_nof_bits > 32)
            {
            ...
            else
            {
                ...
                table_field->field_nof_bits += 1;
                SHR_BITSET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_VALID_INDICATION);
            }
        }

        table_field->nof_instances = 1;
        table->multi_res_info[result_type_index].entry_payload_size += table_field->field_nof_bits;
        table_field->arr_prefix = results_info[iter].arr_prefix;
        table_field->arr_prefix_size = results_info[iter].arr_prefix_size;
    }
    table->multi_res_info[result_type_index].entry_payload_size += result_lsb_padding;
    ...
    dbal_logical_table_t * table)
{
    ...
    for (result_type_iter = 0; result_type_iter < table->nof_result_types; result_type_iter++)
    {
        ...
        if (sal_strncmp(result_types_names[result_type_iter], EMPTY, DBAL_MAX_STRING_LENGTH))
        {
        ...
        SHR_IF_ERR_EXIT(dbal_tables_table_create_result_type_set_internal
                        (unit, nof_result_fields[result_type_iter], result_type_iter,
                         &results_info[result_type_iter * DBAL_TABLE_MAX_NUM_OF_RESULT_FIELDS_PER_RES_TYPE],
                         result_lsb_padding, table));
    }
    ...
    dbal_tables_e * table_id)
{
    ...
    if (DBAL_TABLE_IS_TCAM(table) && (access_method == DBAL_ACCESS_METHOD_MDB))
    {
        ...
        table->physical_db_id[1] = DBAL_PHYSICAL_TABLE_TCAM;
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_create_results_set
                    (unit, nof_result_fields, result_types_names, results_info, result_lsb_padding, table));

    SHR_IF_ERR_EXIT(dbal_db_init_table_interface_post_process(unit, *table_id, result_lsb_padding, table));

    if (access_method == DBAL_ACCESS_METHOD_MDB)
    {
        SHR_IF_ERR_EXIT(dbal_db_init_table_mdb_post_process(unit, *table_id, result_lsb_padding, table));
    }
    else if (access_method == DBAL_ACCESS_METHOD_KBP)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_db_init_table_kbp_post_process(unit, *table_id, result_lsb_padding, table));
    }
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_SW_STATE_TBL_PROP.nof_entries.set(unit, *table_id, 0));
    }
    ...
    xml_node top_node)
{
    ...
    char str[DBAL_MAX_EXTRA_LONG_STRING_LENGTH] = { '\0' };
    char valid_result_type_str[DBAL_MAX_EXTRA_LONG_STRING_LENGTH] = { '\0' };
    ...
    dbal_db_int_or_dnx_data_info_struct_t dnx_data_size, dnx_arr_prefix, dnx_arr_width;
    dbal_field_types_defs_e reference_field_type_id;
    ...
    RHDATA_ITERATOR(appdb_node, top_node, "AppDB")
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_PARAM, "Dynamic tables loading is not supported for %s,"
                         " supports only MDB, KBP and PEMLA access methods", str);
        }
        ...
        RHDATA_ITERATOR(cur_node, keys_node, "Field")
        {
            ...
            SHR_IF_ERR_EXIT_EXCEPT_IF(dbx_xml_property_get_str(cur_node, "Type", field_type_name, RHNAME_MAX_SIZE),
                                      _SHR_E_NOT_FOUND);
            if (!sal_strncmp(field_type_name, EMPTY, sizeof(field_type_name)))
            {

                sal_strncpy(field_type_name, str, RHNAME_MAX_SIZE);
            }
            field_type = DBAL_FIELD_TYPE_DEF_EMPTY;
            SHR_IF_ERR_EXIT_EXCEPT_IF(dbal_field_types_def_string_to_id_no_error(unit, field_type_name, &field_type),
                                      _SHR_E_NOT_FOUND);
            ...
            if (keys_info[nof_keys].field_id == DBAL_FIELD_EMPTY)
            {
                ...
                RHDATA_GET_STR_STOP(cur_node, "Size", str);
            }
            else
            {

                sal_strncpy_s(str, EMPTY, 1);
                SHR_IF_ERR_EXIT_EXCEPT_IF(dbx_xml_property_get_str(cur_node, "Size", str, RHSTRING_MAX_SIZE),
                                          _SHR_E_NOT_FOUND);
            }

            if (sal_strncmp(str, EMPTY, sizeof(str)))
            {
                SHR_IF_ERR_EXIT(dbal_db_parse_dnx_data_indication(unit, &dnx_data_size, str, FALSE, 0, "", ""));
                nof_bits = dnx_data_size.int_val;
            }
        ...
        RHDATA_ITERATOR(results_node, multiple_results_node, "Result")
        {
            ...
            SHR_IF_ERR_EXIT_EXCEPT_IF(dbx_xml_property_get_str
                                      (results_node, "Valid", valid_result_type_str, RHSTRING_MAX_SIZE),
                                      _SHR_E_NOT_FOUND);
            if (sal_strncmp(valid_result_type_str, EMPTY, sizeof(valid_result_type_str)))
            {
                dbal_db_int_or_dnx_data_info_struct_t dnx_data_valid;

                SHR_IF_ERR_EXIT(dbal_db_parse_dnx_data_indication
                                (unit, &dnx_data_valid, valid_result_type_str, FALSE, 0, "", ""));
                if (!dnx_data_valid.int_val)
                {
                    continue;
                }
            }
            ...
            RHDATA_ITERATOR(cur_node, results_node, "Field")
            {
                ...
                field_type = DBAL_FIELD_TYPE_DEF_EMPTY;
                SHR_IF_ERR_EXIT_EXCEPT_IF(dbal_field_types_def_string_to_id_no_error
                                          (unit, field_type_name, &field_type), _SHR_E_NOT_FOUND);
                if (field_type == DBAL_FIELD_TYPE_DEF_EMPTY)
                {
                    field_type = DBAL_FIELD_TYPE_DEF_UINT;
                }
        ...
        SHR_IF_ERR_EXIT(dbal_tables_table_create_internal
                        (unit, access_method, table_type, core_mode, nof_keys, keys_info, nof_fields, fields_info,
                         nof_result_types, result_type_names, table_name, TRUE, 0, &table_id));
    ...
    dbal_tables_e * table_id)
{
    ...
    char result_type_names[1][DBAL_MAX_STRING_LENGTH] = { {0} };
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_create_internal
                    (unit, access_method, table_type, core_mode, nof_key_fields, keys_info, &nof_result_fields,
                     ...
                     result_type_names, table_name, FALSE, result_lsb_padding, table_id));

    SHR_IF_ERR_EXIT(dnx_dbal_journal_log_dbal_table_create(unit, *table_id));
    ...
    int *result_type_index)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_create_result_type_set_internal
                    (unit, nof_result_fields, new_res_type_idx, results_info, 0, table));
    ...
    dbal_tables_e table_id)
{
    ...
    int nof_lables = dnx_data_dbal.table.nof_dynamic_tables_labels_get(unit);
    int label_index;
    int multi_res_index;
    int res_fields_index;
    int key_index;
    multi_res_info_t multi_res_info;
    ...
    SHR_IF_ERR_EXIT(dnx_dbal_journal_log_dbal_table_destroy(unit, table_id));

    sw_state_table_index = table_id - DBAL_NOF_TABLES;
    SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.table_id.set(unit, sw_state_table_index, DBAL_TABLE_EMPTY));
    for (char_index = 0; char_index < sizeof(table->table_name); char_index++)
    {
        SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.
                        table_name.set(unit, sw_state_table_index, char_index, &null_char));
    }
    sal_memset(&data, 0x0, sizeof(data));
    SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.data.set(unit, sw_state_table_index, &data));
    for (label_index = 0; label_index < nof_lables; label_index++)
    {
        SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.
                        labels.set(unit, sw_state_table_index, label_index, DBAL_LABEL_NONE));
    }
    sal_memset(&multi_res_info, 0x0, sizeof(multi_res_info));
    sal_memset(&res_info_data, 0x0, sizeof(res_info_data));
    for (multi_res_index = 0; multi_res_index < table->nof_result_types; multi_res_index++)
    {
        SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.
                        multi_res_info_data.set(unit, sw_state_table_index, multi_res_index, &multi_res_info));
        for (res_fields_index = 0; res_fields_index < table->multi_res_info[multi_res_index].nof_result_fields;
             res_fields_index++)
        {
            SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.
                            res_info_data.set(unit, sw_state_table_index, multi_res_index, res_fields_index,
                                              &res_info_data));
        }
    }
    sal_memset(&key_info_data, 0x0, sizeof(key_info_data));
    for (key_index = 0; key_index < table->nof_key_fields; key_index++)
    {
        SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.
                        key_info_data.set(unit, sw_state_table_index, key_index, &key_info_data));
    }
    SHR_IF_ERR_EXIT(DBAL_DYNAMIC_TBL_IN_SW_STATE.
                    key_info_data.set(unit, sw_state_table_index, key_index, &key_info_data));
    ...
    SHR_IF_ERR_EXIT(DBAL_SW_STATE_TBL_PROP.indications_bm.set(unit, table_id, 0));
    SHR_IF_ERR_EXIT(DBAL_SW_STATE_TBL_PROP.tcam_handler_id.set(unit, table_id, 0));
    ...
    int result_type_index)
{
    ...
    if (sal_strnstr
        (table->multi_res_info[result_type_index].result_type_name, "DYN_RES_",
         sizeof(table->multi_res_info[result_type_index].result_type_name)) == NULL)
    {
    ...
    dbal_tables_e table_id)
{
    ...
    {
        if (sal_strnstr
            (table->multi_res_info[iter].result_type_name, "DYN_RES_",
             sizeof(table->multi_res_info[iter].result_type_name)) != NULL)
        {
            break;
        }
    }
    ...
    dbal_table_field_info_t * field_info)
{
    ...
    else
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "field_info_p is NULL\n");
        }
    ...
    int *field_size)
{
    ...
    else
    {
        if (field_info_p == NULL)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "field_info_p is NULL. Illegal.\n");
        }
...
shr_error_e
dbal_tables_hl_access_field_cbs_set(
    int unit,
    dbal_tables_e table_id,
    dbal_fields_e field_id,
    int result_type_idx,
    int inst_idx,
    dbal_hl_access_cbs_t user_cbs)
{
    dbal_logical_table_t *table = NULL;
    dbal_hard_logic_access_types_e access_type;
    int cb_was_set = FALSE, field_idx, is_active;

    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dbal_tables_is_table_active(unit, table_id, &is_active));

    table = &logical_tables_info[unit]->logical_tables[table_id];

    if (result_type_idx >= table->nof_result_types)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "result type out of bounds for table\n");
    }

    for (access_type = DBAL_HL_ACCESS_MEMORY; access_type < DBAL_HL_ACCESS_SW; access_type++)
    {
        for (field_idx = 0;
             field_idx < table->hl_mapping_multi_res[result_type_idx].l2p_hl_info[access_type].num_of_access_fields;
             field_idx++)
        {
            dbal_hl_l2p_field_info_t *curr_field =
                &table->hl_mapping_multi_res[result_type_idx].l2p_hl_info[access_type].l2p_fields_info[field_idx];
            if ((curr_field->field_id == field_id) && (curr_field->inst_idx == inst_idx))
            {
                if (user_cbs.array_offset_info_cb)
                {
                    curr_field->array_offset_info.formula->cb = user_cbs.array_offset_info_cb;
                    cb_was_set = TRUE;
                }

                if (user_cbs.entry_offset_info_cb)
                {
                    curr_field->entry_offset_info.formula->cb = user_cbs.entry_offset_info_cb;
                    cb_was_set = TRUE;
                }

                if (user_cbs.data_offset_info_cb)
                {
                    curr_field->data_offset_info.formula->cb = user_cbs.data_offset_info_cb;
                    cb_was_set = TRUE;
                }

                if (user_cbs.block_index_info_cb)
                {
                    curr_field->block_index_info.formula->cb = user_cbs.block_index_info_cb;
                    cb_was_set = TRUE;
                }

                if (user_cbs.group_offset_info_cb)
                {
                    curr_field->group_offset_info.formula->cb = user_cbs.group_offset_info_cb;
                    cb_was_set = TRUE;
                }
            }
        }
    ...
    if (!cb_was_set)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "field %snot found in mapping\n", dbal_field_to_string(unit, field_id));
    }
    ...
    int field_size)
{
    ...
    if (!is_active)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "table is not active\n");
    }
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "API is supported only for HL tables\n");
    }

    SHR_IF_ERR_EXIT(dbal_tables_field_info_get_internal(unit, table_id, field_id, is_key, result_type_idx, inst_idx,
                                                        &field_info_p, &field_index_in_table, &parent_field_id));
    {
        int field_size_to_add = field_size - field_info_p->field_nof_bits;
        uint32 max_value = ((uint32) 1 << (field_size)) - 1;

        field_info_p->max_value = max_value;
        field_info_p->field_nof_bits = field_size;

        if (is_key)
        {
            table->key_size += field_size_to_add;
            SHR_IF_ERR_EXIT(dbal_db_init_table_interface_key_post_process(unit, table));
        }
        else
        {
            table->multi_res_info[result_type_idx].entry_payload_size += field_size_to_add;
            SHR_IF_ERR_EXIT(dbal_db_init_table_interface_result_post_process(unit, table_id, 0, table));
            SHR_IF_ERR_EXIT(dbal_db_init_table_hl_direct_post_process(unit, table));
        }
    }
    ...
    int *max_value)
{
    ...
    if (rv)
    {
        if (rv != _SHR_E_NOT_FOUND)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Error in table %s field %s res type %d, not part of the %s fields\n",
                         dbal_logical_table_to_string(unit, table_id), dbal_field_to_string(unit, field_id),
                         result_type_idx, is_key ? "key" : "result");
        }
    }
    else
    {
        ...
        (*max_value) = field_info_p->max_value;
    }
    ...
    dbal_tables_e * next_table_id)
{
    ...
    for (table_counter = table_id + 1; table_counter < nof_tables; table_counter++)
    {
        ...
        if (mdb_physical_db != DBAL_PHYSICAL_TABLE_NONE)
        {

            int continue_flag = TRUE;
            int table_iter;

            for (table_iter = 0; table_iter < table->nof_physical_tables; table_iter++)
            {
                if (table->physical_db_id[table_iter] == mdb_physical_db)
                {
                    continue_flag = FALSE;
                    break;
                }
...
shr_error_e
dbal_tables_is_result_type_valid(
    int unit,
    dbal_tables_e table_id,
    int result_type,
    int *is_valid)
{
    ...
    if (table->multi_res_info[result_type].is_disabled)
    {
        (*is_valid) = 0;
    }
    ...
    dbal_fields_e * next_field_id)
{
    ...
    else
    {
        ...
        if (table->multi_res_info[result_type].is_disabled)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "result type %d disabled, cannot perform the operation \n", result_type);
        }
...
dbal_tables_app_db_id_get(
    ...
    int *app_db_size)
{
    uint8 access_method_is_applicable = FALSE;
    uint8 validate_app_id_not_found = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if (logical_tables_info[unit]->logical_tables[table_id].access_method == DBAL_ACCESS_METHOD_KBP)
    {
        access_method_is_applicable = TRUE;
        validate_app_id_not_found = TRUE;
    }
#endif

    if ((logical_tables_info[unit]->logical_tables[table_id].access_method != DBAL_ACCESS_METHOD_MDB) &&
        !access_method_is_applicable)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "app DB get is not applicable for table %s\n",
                     logical_tables_info[unit]->logical_tables[table_id].table_name);
    }

    if (validate_app_id_not_found && (logical_tables_info[unit]->logical_tables[table_id].app_id == 0))
    {
    ...
    *app_db_id = logical_tables_info[unit]->logical_tables[table_id].app_id;
    ...
    if (!access_method_is_applicable)
    {
    ...
    uint32 *ind_val)
{
    ...
    switch (ind_type)
    {
        ...
        case DBAL_TABLE_IND_IS_NONE_DIRECT_FORCE_OPTIMIZED:
        ...
        case DBAL_TABLE_IND_IS_NEW_ACCESS:
        case DBAL_TABLE_IND_LART_SUPPORT:
        case DBAL_TABLE_IND_SW_RESULT_TYPE_NO_ACCESS:
    ...
    uint32 ind_val)
{
    ...
    switch (ind_type)
    {
        ...
        case DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED:
        case DBAL_TABLE_IND_IS_DEFAULT_NON_STANDARD:
        case DBAL_TABLE_IND_IS_ASYMMETRIC:
        case DBAL_TABLE_IND_ZERO_SIZE_PAYLOAD_SUPPORT:
        case DBAL_TABLE_IND_IS_NEW_ACCESS:
        case DBAL_TABLE_IND_LART_SUPPORT:
        case DBAL_TABLE_IND_SW_RESULT_TYPE_NO_ACCESS:
    ...
    uint32 mode)
{
    ...
    if (!sw_state_is_warm_boot(unit))
    {
        if (mode)
        {
            SHR_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_NONE_DIRECT_FORCE_OPTIMIZED);
        }
        else
        {
            SHR_BITCLR(table->indications_bm, DBAL_TABLE_IND_IS_NONE_DIRECT_FORCE_OPTIMIZED);
        }
        SHR_IF_ERR_EXIT(DBAL_SW_STATE_TBL_PROP.indications_bm.set(unit, table_id, table->indications_bm[0]));
    }
    ...
    uint32 predef_value)
{
    ...
    switch (value_type)
    {
        ...
        {
            uint32 max_value = ((uint32) 1 << (fields_info[iter].field_nof_bits)) - 1;
            ...
            if (predef_value > max_value)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "New max value %d is bigger than nof bits of the field %d\n", predef_value,
                             fields_info[iter].field_nof_bits);
            }
    ...
    uint8 is_add_operation)
{
    ...
    if ((dbal_tables_is_non_direct(unit, entry_handle->table_id) == FALSE) &&
        ...
        (!dbal_table_is_in_lif(unit, table) && !dbal_table_is_out_lif(table)))
    {
        SHR_EXIT();
    }
    ...
    if (table->access_method == DBAL_ACCESS_METHOD_MDB)
    {
        ...
        {
            if ((entry_handle->phy_entry.core_id != DBAL_CORE_ALL) && (jj != entry_handle->phy_entry.core_id))
            {
                continue;
            }
            SHR_IF_ERR_EXIT(dbal_mdb_phy_table_is_valid(unit, table->physical_db_id[jj]));
    ...
    if ((table->core_mode == DBAL_CORE_MODE_DPC) && ((entry_handle->phy_entry.core_id == DBAL_CORE_ALL)))
    {
        int core_index = 0, nof_cores = 0;
        DNXCMN_CORES_ITER(unit, DBAL_CORE_ALL, core_index)
        {
            ++nof_cores;
        }
        num_of_entries_to_update *= nof_cores;
    }
    ...
    char buffer_to_print[DBAL_MAX_PRINTABLE_BUFFER_SIZE])
{
    ...
    SHR_IF_ERR_EXIT(dbal_field_in_table_printable_string_get
                    (unit, field_id, inst_idx, table_id, buffer_val, buffer_mask, result_type_idx, is_key, 0,
                     buffer_to_print));
...
shr_error_e
dbal_table_field32_printable_string_get(
    int unit,
    dbal_tables_e table_id,
    dbal_fields_e field_id,
    uint8 inst_idx,
    ...
    dbal_printable_entry_t * entry_print_info)
{
    ...
    if (table_id == DBAL_TABLE_EMPTY)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Empty table was passed\n");
    }
    ...
    rv = dbal_entry_handle_key_payload_update(unit, entry_handle_id, result_type_idx, key_buffer, NULL, core_id,
                                              payload_size, payload_buffer);
        ...
        if (key_buffer)
        {
            ...
            for (ii = 0; ii < table->nof_key_fields; ii++)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get
                                (unit, table_id, cur_field_id, 0, field_value, NULL, 0, TRUE, field_value_to_print));
        ...
        if (payload_buffer)
        {
            if (entry_handle->cur_res_type != DBAL_RESULT_TYPE_NOT_INITIALIZED)
            {
                ...
                for (ii = 0; ii < DBAL_RES_INFO.nof_result_fields; ii++)
                {
                    ...
                    uint8 inst_idx = DBAL_RES_INFO.results_info[ii].inst_idx;
                    ...
                    SHR_IF_ERR_EXIT(dbal_table_field_printable_string_get
                                    (unit, table_id, cur_field_id, inst_idx, field_value, NULL,
                                     entry_handle->cur_res_type, FALSE, field_value_to_print));
