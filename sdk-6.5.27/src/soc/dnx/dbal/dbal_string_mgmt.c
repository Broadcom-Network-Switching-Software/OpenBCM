...
char *field_instance_prefix = "#";

static char *unknown_string = "unknown_id";
char dbal_tcam_hitbit_strings[DBAL_NOF_TCAM_HITBIT_COMBINATIONS][DBAL_MAX_STRING_LENGTH] = { {0} };
...
char *dbal_hl_access_type_strings[DBAL_NOF_HL_ACCESS_TYPES] = {
    "Memory",
    "Register",
    "SW"
};
...
static char *dbal_logger_print_types_strings[DBAL_LOGGER_NOF_PRINT_TYPES] = {
    "Default",
    "Numerics as Hex"
};
...
void
dbal_global_tcam_hitbit_string_init(
    void)
{
    uint32 combination_idx, bit_idx;
    char buffer[8] = "";

    sal_strncpy(dbal_tcam_hitbit_strings[0], "NONE", DBAL_MAX_STRING_LENGTH);

    for (combination_idx = 1; combination_idx < DBAL_NOF_TCAM_HITBIT_COMBINATIONS; ++combination_idx)
    {
        sal_strncpy(dbal_tcam_hitbit_strings[combination_idx], "CORE", DBAL_MAX_STRING_LENGTH);
        for (bit_idx = 0; bit_idx < sizeof(uint8); ++bit_idx)
        {
            if (utilex_bitstream_test_bit(&combination_idx, bit_idx))
            {
                sal_snprintf(buffer, 8, "_%d", combination_idx);
                sal_strncat_s(dbal_tcam_hitbit_strings[combination_idx], buffer, DBAL_MAX_STRING_LENGTH);
            }
        }
    }
}
...
char *
dbal_field_with_inst_to_string(
    int unit,
    dbal_fields_e field_id,
    uint8 inst_idx,
    char inst_field_name_str[DBAL_MAX_STRING_LENGTH])
{
    sal_snprintf(inst_field_name_str, DBAL_MAX_STRING_LENGTH, "%s", dbal_field_to_string(unit, field_id));
    if (inst_idx > 0)
    {
        sal_snprintf(inst_field_name_str, DBAL_MAX_STRING_LENGTH, "%s_%s%d", inst_field_name_str, field_instance_prefix,
                     inst_idx);
    }
    return inst_field_name_str;
}

char *
dbal_field_encoded_with_inst_to_string(
    int unit,
    dbal_fields_e field_id,
    char inst_field_name_str[DBAL_MAX_STRING_LENGTH])
{
    ...
    return dbal_field_with_inst_to_string(unit, field_id, inst_idx, inst_field_name_str);
}

shr_error_e
dbal_field_string_to_inst_idx(
    int unit,
    char *str,
    dbal_fields_e * field_id,
    uint8 *inst_idx)
{
    ...
    while (str[ii] != 0)
    {
        if (str[ii] == field_instance_prefix[0])
        {

            *inst_idx = sal_atoi(&str[++ii]);
    ...
    dbal_entry_action_flags_e * action_flag)
{
    ...
    if (sal_strncasecmp(flag_name, "DBAL_COMMIT", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        *action_flag = DBAL_COMMIT;
    }
    else if (sal_strncasecmp(flag_name, "DBAL_COMMIT_UPDATE", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        *action_flag = DBAL_COMMIT_UPDATE;
    }
    else if (sal_strncasecmp(flag_name, "DBAL_COMMIT_FORCE", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        *action_flag = DBAL_COMMIT_FORCE;
    }
    else if (sal_strncasecmp(flag_name, "DBAL_GET_ALL_FIELDS", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        *action_flag = DBAL_GET_ALL_FIELDS;
    }
    else if (sal_strncasecmp(flag_name, "DBAL_COMMIT_OVERRUN", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        *action_flag = DBAL_COMMIT_OVERRUN;
    }
    else if (sal_strncasecmp(flag_name, "DBAL_COMMIT_OVERRIDE_DEFAULT", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ==
             0)
    {
        *action_flag = DBAL_COMMIT_OVERRIDE_DEFAULT;
    }
    else if (sal_strncasecmp
             (flag_name, "DBAL_COMMIT_DISABLE_ACTION_PRINTS", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        *action_flag = DBAL_COMMIT_DISABLE_ACTION_PRINTS;
    }
    else if (sal_strncasecmp(flag_name, "DBAL_COMMIT_VALIDATE_OTHER_CORE", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
             == 0)
    {
        *action_flag = DBAL_COMMIT_VALIDATE_OTHER_CORE;
    }
    else if (sal_strncasecmp(flag_name, "DBAL_COMMIT_IGNORE_ALLOC_ERROR", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
             == 0)
    {
        *action_flag = DBAL_COMMIT;
    ...
    dbal_hl_tcam_access_type_e * access_type)
{
    ...
    {
        if (sal_strncasecmp(dbal_hl_tcam_access_type_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ==
            0)
        {
            *access_type = ii;
            SHR_EXIT();
    ...
    dbal_fields_e * field_id)
{
    ...
    if (sal_strncasecmp(dbal_field_strings[DBAL_FIELD_EMPTY], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
    ...
    {
        if (sal_strncasecmp(dbal_field_strings[field_index], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    for (field_index = 0; field_index < DBAL_NOF_DYNAMIC_FIELDS; field_index++)
    {
        ...
        if (field_type != DBAL_FIELD_TYPE_DEF_EMPTY)
        {
            ...
            if (sal_strncasecmp(field_name, str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
            {
...
shr_error_e
dbal_field_types_def_string_to_id_no_error(
    int unit,
    char *str,
    dbal_field_types_defs_e * field_types_def_id)
{
    ...
    {
        if (sal_strncasecmp(dbal_field_types_defs_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    uint32 *res_type)
{
    ...
    {
        if (sal_strncasecmp(table->multi_res_info[res_type_index].result_type_name, res_type_str,
                            sizeof(table->multi_res_info[res_type_index].result_type_name)) == 0)
        {
    ...
    uint32 *res_type)
{
    ...
    *res_type = DBAL_RESULT_TYPE_NOT_INITIALIZED;
    ...
    uint32 *enum_value)
{
    ...
    {
        if (!sal_strncmp
            (enum_value_string, field_type_info->enum_val_info[enum_val_index].name,
    ...
    dbal_tables_e * log_table_id)
{
    ...
    {
        if ((dbal_table_strings[table_index])
            && (sal_strncasecmp(dbal_table_strings[table_index], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ==
                0))
        {
    ...
    if (DBAL_DYNAMIC_XML_TABLES_ENABLED)
    {
        ...
        for (table_index = 0; table_index < nof_xml_dynamic_tables; table_index++)
        {
            ...
            if (table_status != DBAL_TABLE_NOT_INITIALIZED)
            {
                ...
                if (sal_strncasecmp(table_name, str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
                {
    ...
    int *hw_entity_id)
{
    ...
    if (entity_type == DBAL_HL_ENTITY_REGISTER)
    {
        ...
        str_to_check[sal_strnlen(str_to_check, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) - 1] = 0;
        for (ii = 0; ii < NUM_SOC_REG; ii++)
        {
            if (SOC_REG_IS_VALID(unit, ii))
            {
                if (sal_strncasecmp(SOC_REG_NAME(unit, ii), str_to_check, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
                    == 0)
                {
                    *hw_entity_id = ii;
                    SHR_EXIT();
    ...
    if (entity_type == DBAL_HL_ENTITY_MEMORY)
    {
        ...
        for (ii = 0; ii < NUM_SOC_MEM; ii++)
        {
            ...
            {
                if (sal_strncasecmp(SOC_MEM_NAME(unit, ii), str_to_check, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
                    == 0)
                {
                    *hw_entity_id = ii;
                    SHR_EXIT();
    ...
    int *hw_entity_id)
{
    ...
    for (ii = 0; ii < SOC_REG_INFO(unit, reg).nFields; ii++)
    {
        ...
        if (sal_strncasecmp
            (SOC_FIELD_NAME(unit, hw_field->field), str_to_check, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    int *hw_entity_id)
{
    ...
    for (ii = 0; ii < SOC_MEM_INFO(unit, mem).nFields; ii++)
    {
        ...
        if (sal_strncasecmp(SOC_FIELD_NAME(unit, hw_field->field), str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ==
            0)
        {
            *hw_entity_id = hw_field->field;
            SHR_EXIT();
...
shr_error_e
dbal_physical_table_string_to_id_no_error(
    int unit,
    char *str,
    dbal_physical_tables_e * phy_table_id)
{
    ...
    {
        if (sal_strncasecmp
            (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, ii)->dbal_str, str,
    ...
    dbal_labels_e * label_id)
{
    ...
    {
        if (sal_strncasecmp(dbal_label_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    dbal_core_mode_e * core_mode_id)
{
    ...
    {
        if (sal_strncasecmp(dbal_core_mode_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    dbal_condition_types_e * condition_id)
{
    ...
    {
        if (sal_strncasecmp(dbal_condition_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    dbal_field_print_type_e * field_print_type_id)
{
    ...
    {
        if (sal_strncasecmp(dbal_field_print_type_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    dbal_value_field_encode_types_e * encode_type_id)
{
    ...
    {
        if (sal_strncasecmp(dbal_field_encode_type_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    dbal_table_type_e * table_type_id)
{
    ...
    {
        if (sal_strncasecmp(dbal_logical_table_type_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ==
            0)
        {
    ...
    dbal_access_method_e * access_method_id)
{
    ...
    {
        if (sal_strncasecmp(dbal_access_method_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    dbal_maturity_level_e * maturity_level_id)
{
    ...
    {
        if (sal_strncasecmp(dbal_maturity_level_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    dbal_stage_e * dbal_stage)
{
    ...
    {
        if (sal_strncasecmp(dbal_stage_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
    ...
    dbal_table_status_e * table_status_id)
{
    ...
    {
        if (sal_strncasecmp(dbal_logical_table_status_strings[ii], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) ==
            0)
        {
    ...
    dbal_fields_e field_id)
{
    ...
    if (field_id < DBAL_NOF_FIELDS)
    {
        ...
        rv = dbal_db.field_names_override_locations.find(unit, &field_id, &index, &found);
        if ((rv == _SHR_E_NONE) && found)
        {
            CONST encap_string_type_t *name;
            rv = dbal_db.field_names_override_strings.get(unit, index, &name);
            if (rv == _SHR_E_NONE)
            {
                return name->string;
            }
...
char *
dbal_hl_access_type_to_string(
    int unit,
    dbal_hard_logic_access_types_e hl_access_type)
{
    if (hl_access_type < DBAL_NOF_HL_ACCESS_TYPES)
    {
        return dbal_hl_access_type_strings[hl_access_type];
    }
    return unknown_string;
}
...
char *
dbal_logger_print_type_to_string(
    int unit,
    dbal_logger_print_type_e dbal_logger_print_type)
{
    if (dbal_logger_print_type < DBAL_LOGGER_NOF_PRINT_TYPES)
    {
        return dbal_logger_print_types_strings[dbal_logger_print_type];
    }
    return unknown_string;
}

char *
dbal_physical_table_name_no_index(
    int unit,
    char *phys_table_name)
{
    while (*phys_table_name != '\0')
    {
        ++phys_table_name;
    }
    --phys_table_name;
    ...
    dbal_physical_tables_e phy_table_id)
{
    ...
    {
        return dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, phy_table_id)->dbal_str;
    }
    return unknown_string;
}

char *
dbal_physical_table_to_string_2(
    int unit,
    dbal_physical_tables_e phy_table_id,
    int remove_index)
{
    static char phys_table_string[32];

    sal_strlcpy(phys_table_string, dbal_physical_table_to_string(unit, phy_table_id), sizeof(phys_table_string));
    if (phy_table_id < DBAL_NOF_PHYSICAL_TABLES && remove_index)
    {
        dbal_physical_table_name_no_index(unit, phys_table_string);
    }
    return phys_table_string;
}
    ...
    dbal_table_string_t * table_string)
{
    ...
    sal_strlcpy(table_string->table_name, table->table_name, sizeof(table_string->table_name));

    str_to_print = dbal_table_status_to_string(unit, table_status);
    if (sal_strnlen(str_to_print, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) > DBAL_MAX_SHORT_STRING_LENGTH)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "\n string too big table status ");
    }
    sal_strlcpy(table_string->table_status, str_to_print, sizeof(table_string->table_status));
    ...
    sal_strlcpy(table_string->maturity_level, str_to_print, sizeof(table_string->maturity_level));
    ...
    sal_strlcpy(table_string->table_type, str_to_print, sizeof(table_string->table_type));
    ...
    sal_strlcpy(table_string->access_method, str_to_print, sizeof(table_string->access_method));

    if (table->maturity_level == DBAL_MATURITY_LOW)
    {

        sal_strlcpy(table_string->core_mode, EMPTY, sizeof(table_string->core_mode));
        sal_strlcpy(table_string->range_set_supported, EMPTY, sizeof(table_string->range_set_supported));
        SHR_EXIT();
    }
    ...
    sal_strlcpy(table_string->core_mode, str_to_print, sizeof(table_string->core_mode));

    if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_RANGE_SET_SUPPORTED))
    {
        sal_strlcpy(table_string->range_set_supported, "Bulk mode range supported",
                    sizeof(table_string->range_set_supported));
    }
    else
    {
        sal_strlcpy(table_string->range_set_supported, "Bulk mode range NOT supported",
                    sizeof(table_string->range_set_supported));
    }
    ...
    dbal_hw_entity_group_e * group_id)
{
    ...
    {
        if (sal_strncasecmp
            (dbal_hw_entity_group_strings[group_index], str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
        {
