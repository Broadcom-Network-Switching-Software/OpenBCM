...
CONST dbal_action_info_t dbal_actions_info[DBAL_NOF_ACTIONS] =
{
    ...
    {"GET",            POST_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | RESULT_FIELD_MUST_BE_SET | VALIDATE_ALLOCATOR_FIELD | CORE_ID_MUST_BE_SET_TO_SPECIFIC_CORE | CORE_GROUP_MUST_BE_SET_TO_SPECIFIC_GROUP, COMMON_LEGAL_FLAGS | DBAL_COMMIT_VALIDATE_OTHER_CORE | DBAL_COMMIT_HW_ONLY},

    
    {"GET ALL FIELDS", POST_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST| VALIDATE_ALLOCATOR_FIELD | CORE_ID_MUST_BE_SET_TO_SPECIFIC_CORE | CORE_GROUP_MUST_BE_SET_TO_SPECIFIC_GROUP, COMMON_LEGAL_FLAGS | DBAL_GET_ALL_FIELDS | DBAL_COMMIT_HW_ONLY},

    
    {"GET ACCESS ID BY KEY", POST_ACCESS_PRINTS_ENABLED, ALL_KEY_FIELDS_MUST_EXIST | VALIDATE_ALLOCATOR_FIELD | CORE_ID_MUST_BE_SET_TO_SPECIFIC_CORE | CORE_GROUP_MUST_BE_SET_TO_SPECIFIC_GROUP, COMMON_LEGAL_FLAGS},
    ...
    dbal_entry_handle_t * secondary_entry)
{
    ...
    {
        if (secondary_entry && (prim_entry->cur_res_type != secondary_entry->cur_res_type))
        {
        ...
        {
            sal_memcpy(converted_payload, zero_buffer_to_compare, WORDS2BYTES(payload_num_of_uint32));
        }
...
shr_error_e
dbal_entry_direct_is_default_internal(
    int unit,
    dbal_entry_handle_t * entry_handle,
    int *is_default)
{
...
static uint8
dbal_actions_is_merge_entries_required(
    int unit,
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    int is_merge_entries_required = (DBAL_HANDLE_IS_MERGE_NEEDED(entry_handle) &&
                                     dbal_actions_is_merge_entries_required(unit, entry_handle->table));

    SHR_FUNC_INIT_VARS(unit);

    if (is_merge_entries_required || entry_handle->er_flags)
    {
        ...
        {
            int is_exist = FALSE;
            SHR_IF_ERR_EXIT(dbal_actions_optimized_is_exists(unit, entry_handle, &is_exist));
            if (!is_exist)
            {
                rv = _SHR_E_NOT_FOUND;
            }
        ...
        if (rv == _SHR_E_NONE)
        {
            uint8 disable_get_all_fields = FALSE;
            if (!is_merge_entries_required && !(entry_handle->er_flags & DNX_DBAL_JOURNAL_COMPARE_FLAG)
                && (table->table_type != DBAL_TABLE_TYPE_TCAM_DIRECT)
                && (entry_handle->table_id != DBAL_TABLE_EGRESS_PP_PORT)
                && (entry_handle->table_id != DBAL_TABLE_INGRESS_PP_PORT)
                && (table->access_method != DBAL_ACCESS_METHOD_MDB) && (entry_handle->table_id != DBAL_TABLE_MCDB))
            {
                
                disable_get_all_fields = TRUE;
            }

            SHR_IF_ERR_EXIT(dbal_internal_entry_handle_alloc(unit, entry_handle->handle_id, &get_entry_handle));

            rv = dbal_actions_access_entry_get(unit, entry_handle, get_entry_handle, disable_get_all_fields);
        }
        ...
        if ((rv != _SHR_E_NOT_FOUND) && entry_handle->er_flags)
        {
            ...
            {
                int is_default = 0;
                SHR_IF_ERR_EXIT(dbal_entry_direct_is_default_internal(unit, get_entry_handle, &is_default));
                if (!is_default)
                {
...
exit:
    SHR_ERR_HANDLE_AFTER_EXIT(dbal_internal_entry_handle_free(unit, &get_entry_handle));
    SHR_FUNC_EXIT;
}
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (DBAL_TABLE_IS_NONE_DIRECT(entry_handle->table))
    {
        ...
        if (entry_handle->entry_merged)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_merge_entries(unit, entry_handle, NULL));
        }
        SHR_IF_ERR_EXIT(dbal_actions_access_entry_commit(unit, entry_handle, NULL));
    }
    else
    {
        entry_handle->phy_entry.indirect_commit_mode = DBAL_NOF_INDIRECT_COMMIT_MODES;
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (DBAL_HANDLE_IS_MERGE_NEEDED(entry_handle) || entry_handle->er_flags ||
        (entry_handle->phy_entry.mdb_action_apply == DBAL_MDB_ACTION_APPLY_HW_ONLY))
    {
        SHR_IF_ERR_EXIT(dbal_internal_entry_handle_alloc(unit, entry_handle->handle_id, &get_entry_handle));
        ...
        if (entry_handle->phy_entry.mdb_action_apply != DBAL_MDB_ACTION_APPLY_HW_ONLY)
        {
            ...
            rv = dbal_actions_access_entry_get(unit, entry_handle, get_entry_handle, FALSE);
        }
        else    
        {
            ...
            if (rv == _SHR_E_NONE)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_actions_access_entry_clear(unit, get_entry_handle, get_entry_handle));
    ...
    SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dbal_actions_access_entry_commit(unit, entry_handle, get_entry_handle),
                                       _SHR_E_NOT_FOUND);
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_IS_NONE_DIRECT_FORCE_OPTIMIZED) &&
    ...
    else
    {
        ...
        else if (rv == _SHR_E_NOT_FOUND)
        {
            ...
            entry_handle->phy_entry.indirect_commit_mode = DBAL_INDIRECT_COMMIT_MODE_NORMAL;
            SHR_IF_ERR_EXIT(dbal_internal_entry_handle_free(unit, &get_entry_handle));
        }
        else
        {
            SHR_IF_ERR_EXIT(rv);
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    SHR_IF_ERR_EXIT(dbal_actions_access_entry_clear(unit, entry_handle, entry_handle_for_er));
    ...
    dbal_actions_e action)
{
        ...
        && !entry_handle->er_flags)
    {
        ...
        for (ii = 0; ii < entry_handle->nof_key_fields; ii++)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_fields_has_illegal_values
                            (unit, entry_handle->key_field_ids[ii], &curr_field_illegal_value));
            is_illegal_value_exists |= curr_field_illegal_value;
            if (is_illegal_value_exists)
            {
                break;
            }
            ...
            if (entry_handle->key_field_ranges[ii] > 0)
            {
                ...
                uint32 max_key_field_value = (key_field_min_values[ii] - 1) + entry_handle->key_field_ranges[ii];
                LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(unit, "\nSetting max value %d for field %s \n"),
                                             key_field_min_values[ii] + entry_handle->key_field_ranges[ii],
                                             dbal_field_to_string(unit, entry_handle->key_field_ids[ii])));
                dbal_entry_key_field32_set(unit, max_key_handle_id, entry_handle->key_field_ids[ii],
                                           max_key_field_value);
                if (max_key_handle->error_info.error_exists)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, " field %s out of range min val = %d range = %d \n",
                                 dbal_field_to_string(unit, max_key_handle->error_info.field_and_instance),
                                 key_field_min_values[ii], entry_handle->key_field_ranges[ii]);
                }
            }
        }
        ...
        if (((entry_handle->phy_entry.key[0] + num_of_entries - 1) == max_key_handle->phy_entry.key[0])
    ...
    dbal_entry_action_flags_e flags)
{
    ...
    if (entry_handle->error_info.error_exists)
    {
        ...
        if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
        {
            SHR_ERR_EXIT(entry_handle->error_info.error_exists,
                         ...
                         dbal_field_encoded_with_inst_to_string(unit, entry_handle->error_info.field_and_instance,
                                                                field_string), table->table_name,
    ...
    {
        if ((table->core_mode == DBAL_CORE_MODE_SBC) && (entry_handle->phy_entry.core_id != DBAL_CORE_ALL))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "table %s of type SBC, but core in handle is not CORE_ALL. core=%d\n",
                         table->table_name, entry_handle->phy_entry.core_id);
        }

        if ((table->core_mode == DBAL_CORE_MODE_DPC) && (entry_handle->phy_entry.core_id == DBAL_CORE_NOT_INITIALIZED))
        {
    ...
    if ((table->access_method != DBAL_ACCESS_METHOD_HARD_LOGIC)
        && (actions_info->action_validation_flags & CORE_ID_MUST_BE_SET_TO_SPECIFIC_CORE)
        && (table->core_mode == DBAL_CORE_MODE_DPC))
    {
        if ((entry_handle->phy_entry.core_id < 0) || (entry_handle->phy_entry.core_id > DBAL_MAX_NUM_OF_CORES - 1))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "table %s is %s, but core_id is not initialized (core_id = %d)\n",
                         table->table_name, dbal_core_mode_to_string(unit, table->core_mode),
                         entry_handle->phy_entry.core_id);
        }
    }

    if ((actions_info->action_validation_flags & CORE_GROUP_MUST_BE_SET_TO_SPECIFIC_GROUP)
        && (table->core_mode == DBAL_CORE_MODE_DPCG))
    {
        if ((entry_handle->phy_entry.core_id < 0) || (entry_handle->phy_entry.core_id > DBAL_NOF_CORE_GROUPS - 1))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "table %s is %s, but core group id is not initialized (core_id = %d)\n",
                         table->table_name, dbal_core_mode_to_string(unit, table->core_mode),
                         entry_handle->phy_entry.core_id);
        }
    }
    ...
    {
        if ((entry_handle->nof_result_fields == 0) && !(entry_handle->attr_flags & DBAL_ENTRY_ATTR_FLAG_NO_PAYLOAD))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
        ...
        if (status == DBAL_STATUS_DEVICE_INIT_DONE)
        {
            LOG_ERROR(BSL_LOG_MODULE,
                      (BSL_META_U(unit, "Operation skipped key field %s for table %s is not mapped for image\n\n"),
                       dbal_field_to_string(unit, entry_handle->error_info.field_and_instance), table->table_name));
        }
        (*action) = DBAL_ACTION_SKIP;
        SHR_EXIT();
    }
    ...
    *entry_found = FALSE;
    ...
    while (!*entry_found && !iterator_info->is_end)
    {
        if (!iterator_info->used_first_key)
        {
            ...
            else if (entry_handle->table->core_mode == DBAL_CORE_MODE_DPCG)
            {
        ...
        optimized_iterator_bitmap_val = entry_handle->table->iterator_optimized[core_id][word_offset];
    ...
    if (*entry_found)
    {
        ...
        {
            dbal_tables_e dbal_table_id = entry_handle->table_id;
            rv = dnx_lif_mngr_outlif_sw_info_get
                (unit, *entry_handle->phy_entry.key, &dbal_table_id, (uint32 *) &entry_handle->cur_res_type,
                 NULL, NULL, NULL, NULL);
            if (rv != _SHR_E_NONE)
            {
        ...
        if (dbal_table_is_in_lif(unit, entry_handle->table))
        {
            int core_id = entry_handle->phy_entry.core_id;
    ...
    for (entry_count = 0; entry_count < num_of_entries; entry_count++)
    {
        ...
        if (entry_handle->table->core_mode == DBAL_CORE_MODE_DPC)
        {
            ...
            else
            {
                if (bit_set)
                {
                    SHR_BITSET(&entry_handle->table->iterator_optimized[entry_handle->phy_entry.core_id][word_offset],
                               bit_offset);
                ...
                {
                    SHR_BITCLR(&entry_handle->table->iterator_optimized[entry_handle->phy_entry.core_id][word_offset],
                               bit_offset);
                    DBAL_SW_STATE_TBL_PROP.iterator_optimized.bitmap.bit_clear(unit, entry_handle->table_id,
                                                                               entry_handle->phy_entry.core_id,
                                                                               entry_key);
    ...
    int *is_entry_exists)
{
    ...
    if (entry_handle->table->core_mode == DBAL_CORE_MODE_DPC)
    {
        ...
        {
            bitmap_val = entry_handle->table->iterator_optimized[entry_handle->phy_entry.core_id][word_offset];
        }
    }
    else
    {
    ...
    dbal_entry_handle_t * get_handle)
{
    ...
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED))
    {
        if ((dbal_table_is_in_lif(unit, entry_handle->table) || dbal_table_is_out_lif(entry_handle->table)))
        {
            int is_entry_exist;
            SHR_IF_ERR_EXIT(dbal_actions_optimized_is_exists(unit, entry_handle, &is_entry_exist));
            if (!is_entry_exist)
            {
    ...
    uint8 disable_get_all_fields)
{
    ...
    if (get_entry_handle != NULL)
    {
        ...
        DBAL_ENTRY_HANDLE_KEY_FIELD_RANGES_COPY(*entry_handle, *get_entry_handle);
        ...
        if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
        {
            ...
            if (table->access_method != DBAL_ACCESS_METHOD_MDB
                || (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_RESULT_TYPE_MAPPED_TO_SW)
                    && !(dbal_table_is_out_lif(table)) && !(dbal_table_is_in_lif(unit, table))))
            {
                get_entry_handle->cur_res_type = DBAL_RESULT_TYPE_NOT_INITIALIZED;
            }
        }
        if (!disable_get_all_fields)
        {
    ...
    if (!SHR_FUNC_ERR())
    {
        ...
        if (!SHR_FUNC_ERR())
        {
            ...
            if (is_get_on_new_entry && (!disable_get_all_fields))
            {
    ...
    dbal_iterator_info_t * iterator_info)
{
    ...
    if (iterator_attrib_info->hit_bit_rule_valid)
    {
            ...
            || iterator_info->mdb_iterator.start_non_flush_iteration)
        {
            SHR_IF_ERR_EXIT(dbal_entry_attribute_request_internal
                            (unit, entry_handle, DBAL_ENTRY_ATTR_HITBIT, iterator_attrib_info->hit_bit_rule_type));
        }
    }
    ...
    if (iterator_attrib_info->age_rule_valid)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_entry_attribute_request_internal(unit, entry_handle, DBAL_ENTRY_ATTR_AGE, 0));
    }
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_request_internal
                        (unit, entry_handle, DBAL_ENTRY_ATTR_HITBIT, iterator_attrib_info->hit_bit_action_get_type));
    }
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_attribute_request_internal
                        (unit, entry_handle, DBAL_ENTRY_ATTR_HITBIT, iterator_attrib_info->hit_bit_action_clear_type));
    }
    ...
    uint8 *continue_iterating)
{
    ...
    if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_ITERATOR_OPTIMIZED))
    {
        ...
        if (entry_found)
        {
            SHR_IF_ERR_EXIT(dbal_actions_access_entry_get(unit, entry_handle, NULL, FALSE));
        }
    }
    else
    {
    ...
    if (!iterator_info->is_end)
    {
        ...
        if (is_value_rules_pass && iterator_attrib_info->age_rule_valid)
        {
            SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle->handle_id,
                                                     DBAL_ENTRY_ATTR_AGE, &iterator_attrib_info->age_rule_entry_value));
        ...
        if (is_value_rules_pass && iterator_attrib_info->hit_bit_rule_valid)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_entry_attribute_get(unit, entry_handle->handle_id, DBAL_ENTRY_ATTR_HITBIT, &attr_val));

            
            if ((iterator_attrib_info->hit_bit_rule_is_hit == 0)
                &&
                (((iterator_attrib_info->hit_bit_rule_type & DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY) &&
                  (((attr_val & DBAL_PHYSICAL_KEY_PRIMARY_HITBIT_ACCESSED_A) != 0) ||
                   ((attr_val & DBAL_PHYSICAL_KEY_PRIMARY_HITBIT_ACCESSED_B) != 0)))
                 ||
                 ((iterator_attrib_info->hit_bit_rule_type & DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY) &&
