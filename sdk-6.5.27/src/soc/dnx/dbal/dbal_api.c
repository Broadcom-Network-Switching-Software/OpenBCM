    ...
    int unit)
{
    ...
    if (!dbal_is_initiated(unit))
    {
        ...
        if (!sw_state_is_warm_boot(unit))
        {
            ...
            init_info.max_nof_elements = DBAL_NOF_FIELD_NAME_OVERRIDE_INSTANCES;
            init_info.expected_nof_elements = DBAL_NOF_FIELD_NAME_OVERRIDE_INSTANCES;

            SHR_IF_ERR_EXIT(dbal_db.field_names_override_locations.create(unit, &init_info));
        }
    }
    else
    {
...
shr_error_e
dbal_logger_print_type_set(
    int unit,
    dbal_logger_print_type_e print_type)
{
    SHR_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(dbal_logger_print_type_set_internal(unit, print_type));
    ...
    dbal_field_predefine_value_type_e value_type)
{
    ...
    DBAL_FIELD_ERR_HANDLE((rc), "handle not taken", 0);
    rc = dbal_fields_predefine_value_get(unit, entry_handle_id, field_id, 1, value_type, field_val_u32);
    DBAL_FIELD_ERR_HANDLE((rc), "predefine value get failed", 0);
    ...
    dbal_field_predefine_value_type_e value_type)
{
    ...
    uint32 *field_mask_u32 = G_dbal_field_full_mask;
    int rc;
    int inst_id = inst_id_in;
    dbal_entry_handle_t *entry_handle;

    if (inst_id_in == INST_ALL)
    {

        inst_id = 0;
    }
    if (inst_id_in == INST_SINGLE)
    {
        inst_id = 0;
    }

    rc = dbal_entry_handle_get_internal(unit, entry_handle_id, &entry_handle);
    DBAL_FIELD_ERR_HANDLE((rc), "handle not taken", inst_id);

    rc = dbal_fields_predefine_value_get(unit, entry_handle_id, field_id + inst_id, 0, value_type, field_val_u32);
    DBAL_FIELD_ERR_HANDLE((rc), "predefine value get failed", inst_id);

    DBAL_ENTRY_VALUE_FIELD_SET_INST(unit, entry_handle_id, field_id, inst_id_in, DBAL_POINTER_TYPE_ARR_UINT32);
}

shr_error_e
dbal_entry_handle_payload_clear(
    int unit,
    uint32 entry_handle_id)
{
    dbal_entry_handle_t *entry_handle;
    ...
    sal_memset(&entry_handle->phy_entry.payload, 0, DBAL_TABLE_BUFFER_IN_BYTES(entry_handle->table));
    sal_memset(&entry_handle->phy_entry.p_mask, 0, DBAL_TABLE_BUFFER_IN_BYTES(entry_handle->table));
    ...
    uint32 *max_val)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Range key fields are not supported for tables with %s access method\n",
                     dbal_access_method_to_string(unit, entry_handle->table->access_method));
    }

    if (entry_handle->handle_status != DBAL_HANDLE_STATUS_ACTION_PERFORMED)
    {
    ...
    uint8 *field_val)
{
    ...
    rv = dbal_entry_handle_get_internal(unit, entry_handle_id, &entry_handle);
    DBAL_FIELD_ERR_HANDLE((rv), "handle not taken", inst_id);
    ...
    DBAL_FIELD_ERR_HANDLE((rv), "Error transform arr8 to arr32", inst_id);
...
dbal_entry_attribute_set(
    int unit,
    uint32 entry_handle_id,
    dbal_entry_attribute_t attr_type,
    uint32 attr_val)
{
    dbal_entry_handle_t *entry_handle;
    ...
    if (attr_type == DBAL_ENTRY_ATTR_PRIORITY)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "priority set not supported for this table %s\n",
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_NO_PAYLOAD)
    {
        if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_ZERO_SIZE_PAYLOAD_SUPPORT))
        {
            if (attr_val)
            {
                entry_handle->attr_flags |= DBAL_ENTRY_ATTR_FLAG_NO_PAYLOAD;
            }
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "no_payload attribute is not supported for table %s\n",
                         entry_handle->table->table_name);
        }
    }
    else if (attr_type == DBAL_ENTRY_ATTR_INVALID)
    {
        if (DBAL_TABLE_IS_TCAM(entry_handle->table))
        {
            if (attr_val)
            {
                entry_handle->phy_entry.is_invalid = 1;
            }
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "attribute invalid is supported for TCAM tables only\n");
        }
    }
    else if (attr_type == DBAL_ENTRY_ATTR_CORE_GROUP)
    {
        if ((entry_handle->table->core_mode == DBAL_CORE_MODE_DPCG)
            && ((attr_val < DBAL_NOF_CORE_GROUPS) || (attr_val == DBAL_CORE_GROUP_ALL)))
        {
            entry_handle->phy_entry.core_id = attr_val;
        }
        else
        {
            if (entry_handle->table->core_mode != DBAL_CORE_MODE_DPCG)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "core group attribute is supported on DPCG tables only\n");
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "core group id %d is invalid: should be DBAL_CORE_GROUP_ALL"
                             " or less than number of core groups\n", attr_val);
            }
        }
    }
    else
    ...
    uint32 *attr_val)
{
    ...
    if (attr_type == DBAL_ENTRY_ATTR_PRIORITY)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "priority get not supported for this table %s\n",
                         entry_handle->table->table_name);
        }
    }
    else if (attr_type == DBAL_ENTRY_ATTR_AGE)
    {
        if (!(entry_handle->attr_flags & DBAL_ENTRY_ATTR_FLAG_AGE))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "dbal_entry_attribute_request needs to be called before dbal_entry_commit "
                         "to get AGE attribute");
        }
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_HITBIT)
    {
        if (!(entry_handle->attr_flags & DBAL_ENTRY_ATTR_FLAG_HIT_GET))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "dbal_entry_attribute_request needs to be called before dbal_entry_commit "
                         "to get HITBIT attribute");
        }
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_INVALID)
    {
        if (DBAL_TABLE_IS_TCAM(entry_handle->table))
        {
            (*attr_val) = entry_handle->phy_entry.is_invalid ? TRUE : FALSE;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Attribute Invalid is supported for TCAM tables only\n");
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_NO_PAYLOAD)
    {
        ...
        if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_ZERO_SIZE_PAYLOAD_SUPPORT))
        {
            if (entry_handle->attr_flags & DBAL_ENTRY_ATTR_FLAG_NO_PAYLOAD)
            {
                (*attr_val) = TRUE;
            }
        }
    ...
    {
        (*attr_val) = FALSE;
        if (entry_handle->attr_flags & DBAL_ENTRY_ATTR_CACHED)
        {
            (*attr_val) = TRUE;
        }
    }
    else if (attr_type == DBAL_ENTRY_ATTR_CORE_GROUP)
    {
        if (entry_handle->table->core_mode == DBAL_CORE_MODE_DPCG)
        {
            (*attr_val) = entry_handle->phy_entry.core_id;
        }
        else
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "core group attribute is supported on DPCG tables only\n");
    ...
    uint32 flags)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_attribute_request_internal(unit, entry_handle, attr_type, flags));
...
shr_error_e
dbal_entry_handle_is_field_on_handle(
    int unit,
    uint32 entry_handle_id,
    dbal_fields_e field_id,
    int inst_id,
    uint8 *is_field_on_handle)
{
    ...
    rv = dbal_tables_field_info_get_internal
        (unit, entry_handle->table_id, field_id, FALSE, entry_handle->cur_res_type, inst_id, &table_field_info,
...
shr_error_e
dbal_entry_handle_nof_value_fields_on_handle_get(
    int unit,
    uint32 entry_handle_id,
    int *nof_fields)
{
    ...
    *nof_fields = entry_handle->nof_result_fields;
    ...
    uint32 *payload_buffer)
{
    ...
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE)
        && (result_type_idx == DBAL_RESULT_TYPE_NOT_INITIALIZED))
    {
    ...
    else
    {
        ...
        if (result_type_idx != DBAL_RESULT_TYPE_NOT_INITIALIZED)
        {
            entry_handle->cur_res_type = result_type_idx;
        }
        copy_length =
            UTILEX_MIN(payload_size,
                       entry_handle->table->multi_res_info[entry_handle->cur_res_type].entry_payload_size);
        write_start_bit =
            entry_handle->table->multi_res_info[entry_handle->cur_res_type].entry_payload_size - copy_length;
        read_start_bit = payload_size - copy_length;
    }
    for (ii = 0; ii < entry_handle->table->nof_key_fields; ii++)
    {
        ...
        {
            entry_handle->phy_entry.core_id = core_id;
        }
        else
        {
    ...
    if (res_type_resolution_needed)
    {
        ...
        else if (dbal_table_is_in_lif(unit, entry_handle->table))
        {
            rv = dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                (unit, *entry_handle->phy_entry.key, entry_handle->phy_entry.core_id,
...
shr_error_e
dbal_entry_current_result_type_get(
    int unit,
    uint32 entry_handle_id,
    uint32 *result_type)
{
    ...
    int *is_default)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_get_internal(unit, entry_handle_id, &entry_handle));
    SHR_IF_ERR_EXIT(dbal_entry_direct_is_default_internal(unit, entry_handle, is_default));
    ...
    int result_type)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Illegal result type %d. max values if %d", result_type,
    ...
    dbal_entry_action_flags_e flags)
{
        ...
        !SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_DEFAULT_NON_STANDARD))
    {
        ...
        {
            rv = dbal_actions_access_entry_get(unit, entry_handle, NULL, FALSE);
        }
    }
    else
    {
    ...
    else
    {
        switch (action)
        {
            ...
            case DBAL_ACTION_ENTRY_COMMIT:
                SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dbal_entry_commit_normal(unit, entry_handle), _SHR_E_ACCESS);
                break;
            case DBAL_ACTION_ENTRY_COMMIT_UPDATE:
                SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(dbal_entry_commit_update(unit, entry_handle), _SHR_E_NOT_FOUND);
                break;
    ...
    uint32 entry_access_id)
{
    dbal_entry_handle_t *entry_handle;
    dbal_logical_table_t *table;
    uint8 validate_entry_access_id = TRUE;
    ...
    if (DBAL_TABLE_IS_TCAM(table))
    {
        ...
        if (table->access_method == DBAL_ACCESS_METHOD_KBP)
        {
            ...
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "access ID set not supported for KBP TCAM by KEY tables - %s \n",
                             table->table_name);
            }
            validate_entry_access_id = FALSE;
        }
#endif

        if (validate_entry_access_id && (entry_access_id >= table->max_capacity) &&
    ...
    dbal_entry_action_flags_e flags)
{
    ...
    {
        SHR_IF_ERR_EXIT_NO_MSG(dbal_entry_clear_normal(unit, entry_handle));
    }
...
exit:
    SHR_ERR_HANDLE_AFTER_EXIT(dbal_action_finish(unit, entry_handle, flags, action));
    ...
    dbal_result_type_update_mode_e mode)
{
    ...
    for (field_index = 0; field_index < table->multi_res_info[new_result_type].nof_result_fields; field_index++)
    {
        ...
        uint32 value_to_use[DBAL_FIELD_ARRAY_MAX_SIZE_IN_WORDS] = { 0 };
        ...
        if (field_info.field_nof_bits)
        {
            ...
            sal_memcpy(value_to_use, (sub_field_id == DBAL_FIELD_EMPTY) ? val : sub_field_val,
    ...
    int collect_mode)
{
    ...
    switch (access_method)
    {
        ...
        case DBAL_ACCESS_METHOD_KBP:
        {
            ...
            {
                SHR_IF_ERR_EXIT(dbal_kbp_fwd_caching_enabled_set(unit, collect_mode));
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "bulk_module not supported %d \n", bulk_module);
    ...
    int *collect_mode)
{
    ...
    switch (access_method)
    {
        ...
        case DBAL_ACCESS_METHOD_KBP:
        {
            ...
            if (bulk_module == DBAL_BULK_MODULE_KBP_FWD)
            {
                SHR_IF_ERR_EXIT(dbal_kbp_fwd_caching_enabled_get(unit, collect_mode));
            }
            else
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "bulk_module not supported %d \n", bulk_module);
    ...
    dbal_bulk_module_e bulk_module)
{
    ...
    switch (access_method)
    {
        ...
        {
            if ((bulk_module == DBAL_BULK_MODULE_MDB_LEM_FLUSH)
                || (bulk_module == DBAL_BULK_MODULE_MDB_LEM_NON_BLOCKING_FLUSH))
            {
                int lem_id;
                MDB_LEM_ITER(unit, lem_id)
                {
                    SHR_IF_ERR_EXIT(dbal_flush_start
                                    (unit, TRUE, TRUE, (bulk_module == DBAL_BULK_MODULE_MDB_LEM_NON_BLOCKING_FLUSH),
                                     lem_id));
                }
            }
            else
...
#if defined(INCLUDE_KBP)
            SHR_IF_ERR_EXIT(dbal_kbp_fwd_cache_commit(unit));
#endif
            break;
        }
        default:
    ...
    dbal_bulk_module_e bulk_module)
{
    ...
    switch (access_method)
    {
        case DBAL_ACCESS_METHOD_MDB:
            ...
            MDB_LEM_ITER(unit, lem_id)
            {
                SHR_IF_ERR_EXIT(dbal_flush_db_clear(unit, lem_id));
            }
            break;
