...
typedef enum
{
    ...
    CORE_ID_MUST_BE_SET_TO_SPECIFIC_CORE = SAL_BIT(6),

    CORE_GROUP_MUST_BE_SET_TO_SPECIFIC_GROUP = SAL_BIT(7)
...
do                                                                    \
{                                                                     \
    output = field_id | ((uint32)inst_idx << 24);                     \
} while(0);
...
#define DBAL_ITERATE_OVER_CORES(entry_handle, core_id)                              \
    ...
    {                                                                               \
        ...
        else if ((entry_handle->phy_entry.core_id != DBAL_CORE_ALL) &&                        \
                 (entry_handle->phy_entry.core_id != DBAL_CORE_NOT_INITIALIZED))              \
        {                                                                           \
            first_core = entry_handle->phy_entry.core_id;                                     \
            nof_cores = 1;                                                          \
        }                                                                           \
    }                                                                               \
...
shr_error_e dbal_internal_entry_handle_alloc(
    int unit,
    uint32 entry_handle_id,
    dbal_entry_handle_t ** handle);

shr_error_e dbal_internal_entry_handle_free(
    int unit,
...
shr_error_e dbal_sbc_group_id_set(
    int unit,
    uint8 cg_id);

shr_error_e dbal_sbc_group_id_get(
    int unit,
    uint8 *cg_id);
...
shr_error_e dbal_fields_field_type_max_value_get(
    int unit,
    dbal_fields_e field_id,
    uint32 *max_value);

shr_error_e dbal_fields_field_type_bit_size_get(
    int unit,
    dbal_fields_e field_id,
    uint32 *field_size);
...
shr_error_e dbal_entry_clear_normal(
    int unit,
    dbal_entry_handle_t * entry_handle);
