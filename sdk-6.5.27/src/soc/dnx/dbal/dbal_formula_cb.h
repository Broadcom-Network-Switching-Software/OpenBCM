...
#define DBAL_FORMULA_CB_CONCAT_FIELD(unit, entry_handle, field_id, key_value, buffer, offset) \
        do                                                                                    \
        {                                                                                     \
            dbal_table_field_info_t *field_info;                                              \
            int field_index_in_table;                                                         \
            dbal_fields_e parent_field_id;                                                    \
            SHR_IF_ERR_EXIT(dbal_tables_field_info_get_internal(unit, entry_handle->table_id, \
                                                       field_id, 1, entry_handle->cur_res_type, INST_SINGLE,\
                                                        &field_info, &field_index_in_table, &parent_field_id));\
            SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field(&key_value, offset, field_info->field_nof_bits, &buffer));\
            offset += field_info->field_nof_bits;\
        }while(0);
