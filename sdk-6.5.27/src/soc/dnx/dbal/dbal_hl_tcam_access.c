    ...
    int *num_of_blocks)
{
    ...
    SHR_IF_ERR_EXIT(dbal_hl_access_block_calc
                    (unit, entry_handle, entry_handle->phy_entry.core_id, curr_l2p_info->memory[0],
                     &(curr_l2p_info->block_index_info), curr_l2p_info, 1, block, num_of_blocks));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (int ii = 0; ii < access_info->num_of_access_fields; ++ii)
    {
...
{
    dbal_hl_l2p_info_t *access_info;
    dbal_hl_l2p_field_info_t *access_field_info;
    ...
    *hw_cmd_valid_field = DBAL_HL_ACCESS_HW_FIELD_INVALID;
    ...
    if (access_info->num_of_access_fields > DBAL_HL_TCAM_ACCESS_TYPE_VALID_BIT)
    {
        access_field_info = &access_info->l2p_fields_info[DBAL_HL_TCAM_ACCESS_TYPE_VALID_BIT];
        if (access_field_info->hw_field != DBAL_HL_ACCESS_HW_FIELD_INVALID)
        {
            *memory = access_field_info->memory[0];
            *hw_cmd_valid_field = access_field_info->hw_field;
#if defined(BCM_ACCESS_SUPPORT)
            if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_IS_NEW_ACCESS))
            {

                *hw_cmd_valid_field =
                    DBAL_HL_NEW_ACCESS_DEVICE_TYPE_INFO(unit)->fields_global2local_map[access_field_info->hw_field];
            }
#endif
        }
    }
    else
    {
        ...
        access_field_info = &access_info->l2p_fields_info[DBAL_HL_TCAM_ACCESS_TYPE_KEY_MASK];
        *memory = access_field_info->memory[0];
    ...
    uint8 *is_valid)
{
    ...
    *is_valid = 1;
    ...
    SHR_IF_ERR_EXIT(dbal_hl_tcam_valid_bits_get(unit, entry_handle, data, &valid_bits));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (iter = 0; iter < DBAL_NOF_HL_TCAM_ACCESS_TYPES - 1; ++iter)
    {
        uint32 offset = 0;
    ...
    int *is_default)
{
    ...
    for (ii = 0; ii < access_info->num_of_access_fields; ++ii)
    {
        l2p_mask_info = &access_info->l2p_fields_info[ii];
    ...
    while ((!entry_found) && (!iterator_info->is_end))
    {
        ...
        if (is_valid_entry == DBAL_KEY_IS_VALID)
        {
            ...
            if (rv == _SHR_E_NONE)
            {
                if (is_valid_bits_supported)
                {
                    if (!entry_handle->phy_entry.is_invalid)
                    {
                        entry_found = TRUE;
                    }
                    else
                    {
                        ...
                        entry_handle->phy_entry.is_invalid = 0;
                    }
                }
                else
                {
