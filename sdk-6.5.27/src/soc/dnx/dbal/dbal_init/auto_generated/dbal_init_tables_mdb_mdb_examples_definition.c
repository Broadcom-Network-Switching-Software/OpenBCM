...
dbal_logical_table_t * table_info)
{
    ...
    SHR_IF_ERR_EXIT(dbal_db_init_table_general_info_and_image_handle(unit, table_entry, DBAL_TABLE_EXAMPLE_MDB_WITH_RESULT_TYPE_IN_SW_DIRECT, is_valid, maturity_level, "EXAMPLE_MDB_WITH_RESULT_TYPE_IN_SW_DIRECT", DBAL_ACCESS_METHOD_MDB, current_image, &is_skip_table_init));
...
dbal_logical_table_t * table_info)
{
    ...
    dbal_init_table_db_struct_clear(cur_table_param);
    maturity_level = DBAL_MATURITY_HIGH;
    SHR_IF_ERR_EXIT(dbal_db_init_table_general_info_and_image_handle(unit, table_entry, DBAL_TABLE_EXAMPLE_IN_LIF_FORMAT_DIFFERENT_SIZES, is_valid, maturity_level, "EXAMPLE_IN_LIF_FORMAT_DIFFERENT_SIZES", DBAL_ACCESS_METHOD_MDB, current_image, &is_skip_table_init));
    if (is_skip_table_init)
    {
        SHR_EXIT();
    }
...
shr_error_e
_dbal_init_tables_mdb_mdb_examples_definition_multiple_core_groups_example_init(
int unit,
table_db_struct_t * cur_table_param,
dbal_logical_table_t * table_info)
{
    dbal_logical_table_t * table_entry = table_info + DBAL_TABLE_MULTIPLE_CORE_GROUPS_EXAMPLE;
    int is_valid = TRUE;
    uint8 is_skip_table_init = FALSE;
    char * current_image = "standard_1";
    dbal_maturity_level_e maturity_level;
    ...
    dbal_init_table_db_struct_clear(cur_table_param);
    maturity_level = DBAL_MATURITY_HIGH;
    SHR_IF_ERR_EXIT(dbal_db_init_table_general_info_and_image_handle(unit, table_entry, DBAL_TABLE_MULTIPLE_CORE_GROUPS_EXAMPLE, is_valid, maturity_level, "MULTIPLE_CORE_GROUPS_EXAMPLE", DBAL_ACCESS_METHOD_MDB, current_image, &is_skip_table_init));
    if (is_skip_table_init)
    {
        SHR_EXIT();
    }
    ...
    SHR_IF_ERR_EXIT(dbal_db_init_table_set_mdb_info(unit, cur_table_param, "LEM", 6, 7, DBAL_STAGE_NONE));
    SHR_IF_ERR_EXIT(dbal_db_init_table_add(unit, cur_table_param, DBAL_TABLE_MULTIPLE_CORE_GROUPS_EXAMPLE, table_info));
exit:
    SHR_FUNC_EXIT;
}
...
dbal_logical_table_t * table_info)
{
    ...
    SHR_IF_ERR_EXIT(_dbal_init_tables_mdb_mdb_examples_definition_multiple_core_groups_example_init(unit, cur_table_param, table_info));
exit:
    SHR_FUNC_EXIT;
}
