    ...
    for (ii = 0; ii < iterator_info->nof_actions; ii++)
    {
        switch (iterator_info->actions_info[ii].action_type)
        {
            ...
            case DBAL_ITER_ACTION_DELETE:
                if ((entry_handle->table->physical_db_id[0] == DBAL_PHYSICAL_TABLE_FLP_LARGE_TCAM) ||
                    (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, entry_handle->table->physical_db_id[0])->db_type ==
            ...
            case DBAL_ITER_ACTION_READ_TO_DMA:
                ...
                break;
            case DBAL_ITER_ACTION_CORE_GROUP:
                break;
            default:
    ...
    uint32 iterated_entry_handle_id)
{
    ...
    DBAL_ENTRY_HANDLE_KEY_FIELD_RANGES_COPY(*updated_entry_handle, *iter_entry_handle);
    ...
    uint32 entry_handle_id)
{
    ...
    {
        SHR_IF_ERR_EXIT(dbal_entry_clear_normal(unit, entry_handle));
    }
    ...
    dbal_iterator_mode_e mode)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_is_table_active(unit, entry_handle->table_id, &is_table_active));
    if (!is_table_active)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Table %s is not active in this device",
    ...
    uint32 attr_val)
{
    ...
    switch (attr_type)
    {
        ...
        case DBAL_ENTRY_ATTR_AGE:
            if (!dbal_logical_table_is_mact(unit, entry_handle->table_id))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "DBAL_ENTRY_ATTR_AGE rule is valid for FWD_MACT and FWD_MACT_IVL only");
            }
            iterator_attrib_info->age_rule_valid = TRUE;
            iterator_attrib_info->age_rule_compare_value = attr_val;
            break;
        case (DBAL_ENTRY_ATTR_FLAG_HIT_GET | DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY):
        case (DBAL_ENTRY_ATTR_FLAG_HIT_GET | DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY):
        case (DBAL_ENTRY_ATTR_FLAG_HIT_GET | DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY | DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY):
            ...
            entry_handle->attr_flags |= DBAL_ENTRY_ATTR_FLAG_HIT_GET;
            ...
            break;
        case (DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR | DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY):
        case (DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR | DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY):
        case (DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR | DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY | DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY):
            SHR_ERR_EXIT(_SHR_E_PARAM, "DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR is not supported");
            break;
        case DBAL_ENTRY_ATTR_CORE_GROUP:
            if (entry_handle->table->core_mode != DBAL_CORE_MODE_DPCG)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "DBAL_ENTRY_ATTR_CORE_GROUP is supported in DPCG table only");
            }
            if (attr_val > DBAL_NOF_CORE_GROUPS)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Valid index of core group should be specified, from 0 to %d",
                             DBAL_NOF_CORE_GROUPS - 1);
            }
            iterator_attrib_info->core_group_rule_valid = TRUE;
            if (entry_handle->table->access_method == DBAL_ACCESS_METHOD_MDB)
            {
                iterator_info->mdb_iterator.core_id = attr_val;
            }
            else
            {
    ...
    uint32 attr_type)
{
    ...
    switch (attr_type)
    {
        ...
        case (DBAL_ENTRY_ATTR_FLAG_HIT_GET):
        case (DBAL_ENTRY_ATTR_FLAG_HIT_GET | DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY):
        case (DBAL_ENTRY_ATTR_FLAG_HIT_GET | DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY):
        case (DBAL_ENTRY_ATTR_FLAG_HIT_GET | DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY | DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY):
            ...
            SHR_IF_ERR_EXIT(dbal_iterator_action_add(unit, entry_handle, DBAL_FIELD_EMPTY, DBAL_ITER_ACTION_HIT_GET,
                                                     NULL, NULL));
            break;
        case (DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR):
        case (DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR | DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY):
        case (DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR | DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY):
        case (DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR | DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY | DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY):
    ...
    dbal_iterator_action_types_e action_type)
{
    ...
    if (action_type == DBAL_ITER_ACTION_DELETE)
    {

        entry_handle->er_flags |= DNX_DBAL_JOURNAL_IGNORE_CORE_ID_VALIDATION;
    }
