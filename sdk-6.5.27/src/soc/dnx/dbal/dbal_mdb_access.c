    ...
    uint8 *is_eedb_ll)
{
    ...
    {
        *is_eedb_ll = FALSE;
        *rt_has_link_list = FALSE;
        physical_phase =
            (dnx_data_lif.out_lif.
             logical_to_physical_phase_map_get(unit, DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_1 + 1))->physical_phase;
        *physical_table = physical_phase + DBAL_PHYSICAL_TABLE_EEDB_1;
    }
    else
    {
        dbal_tables_e dbal_table_id = table_id;

        SHR_IF_ERR_EXIT(dnx_lif_mngr_outlif_sw_info_get
                        (unit, phy_entry->key[0], &dbal_table_id, NULL, &logical_phase, NULL, is_eedb_ll, NULL));
        physical_phase =
    ...
    int unit)
{
    ...
    for (table_idx = 0; table_idx < DBAL_NOF_PHYSICAL_TABLES; table_idx++)
    {
        ...
        if ((table_idx == DBAL_PHYSICAL_TABLE_TCAM)
            || (table_idx == DBAL_PHYSICAL_TABLE_FLP_LARGE_TCAM)
...
dbal_table_is_in_lif(
    ...
    CONST dbal_logical_table_t * table)
{
    if ((dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, table->physical_db_id[0])->db_type ==
         DBAL_ENUM_FVAL_MDB_DB_TYPE_DIRECT)
        && (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, table->physical_db_id[0])->db_subtype == MDB_DIRECT_TYPE_INLIF))
    {
        return TRUE;
    }
    else
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    else
    {
        ...
        if (table->core_mode == DBAL_CORE_MODE_SBC)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_sbc_group_id_get(unit, &core_group_id));
            iterator_info->mdb_iterator.core_id = core_group_id;
        }
    ...
    uint32 entry_flags)
{
    ...
    if (dnx_data_lif.global_lif.glem_rif_optimization_enabled_get(unit))
    {
        ...
        else
        {
            ...
            SHR_IF_ERR_EXIT(physical_table_actions->entry_get
                            (unit, physical_db_id, app_id, &tmp_entry_handle->phy_entry, 0));
    ...
    SHR_IF_ERR_EXIT(physical_table_actions->entry_add
                    (unit, physical_db_id, app_id, &tmp_entry_handle->phy_entry, entry_flags));
    ...
    uint32 entry_flags)
{
    ...
    if (dnx_data_lif.global_lif.glem_rif_optimization_enabled_get(unit))
    {
        ...
        if (num_entries == 1)
        {
            SHR_IF_ERR_EXIT(physical_table_actions->entry_delete
                            (unit, physical_db_id, app_id, &entry_handle->phy_entry, entry_flags));
        }
...
static shr_error_e
dbal_mdb_access_mapped_to_sw_get_next(
    int unit,
    dbal_entry_handle_t * entry_handle,
    dbal_iterator_info_t * iterator_info)
{
    ...
    while (!entry_found && !iterator_info->is_end)
    {
        ...
        if (!iterator_info->is_end)
        {
            ...
            {
                iterator_info->physical_db_index = entry_handle->phy_entry.core_id;
            }
        }
    }
...
static shr_error_e
dbal_mdb_access_allocator_get_next(
    int unit,
    dbal_entry_handle_t * entry_handle,
    dbal_iterator_info_t * iterator_info)
{
    ...
    if (table->core_mode == DBAL_CORE_MODE_DPC)
    {
        ...
        {
            if (entry_handle->phy_entry.core_id != iterator_info->physical_db_index)
            {

                entry_handle->phy_entry.key[0] = 0;
            }
            entry_handle->phy_entry.core_id = iterator_info->physical_db_index;
        }
        else
        {
            if (entry_handle->phy_entry.core_id != iterator_info->mdb_iterator.core_id)
            {

                entry_handle->phy_entry.key[0] = 0;
            }
            entry_handle->phy_entry.core_id = iterator_info->mdb_iterator.core_id;
        }
    }
    ...
    dbal_iterator_info_t * iterator_info)
{
    ...
    if (SHR_IS_BITSET(table->indications_bm, DBAL_TABLE_IND_RESULT_TYPE_MAPPED_TO_SW))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_mdb_access_mapped_to_sw_get_next(unit, entry_handle, iterator_info));
    }
    else if (table->allocator_field_id != DBAL_FIELD_EMPTY)
    {

        SHR_IF_ERR_EXIT(dbal_mdb_access_allocator_get_next(unit, entry_handle, iterator_info));
    }
    else
    {
        while (!entry_found && !iterator_info->is_end)
    ...
    *continue_calling_mdb_get_next = FALSE;
         ...
         && (((!dnx_data_dbal.table.feature_get(unit, dnx_data_dbal_table_support_mdb_general_phys_db)
             ...
             ((dnx_data_dbal.table.feature_get(unit, dnx_data_dbal_table_support_mdb_general_phys_db)
               && (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit,
                                                       table->physical_db_id[iterator_info->
                                                                             physical_db_index])->core_mode ==
                   DBAL_CORE_MODE_DPC)) && (table->core_mode == DBAL_CORE_MODE_DPC))) && (DBAL_MAX_NUM_OF_CORES > 1)))
    {
            ...
            if (!dnx_data_dbal.table.feature_get(unit, dnx_data_dbal_table_support_mdb_general_phys_db))
            {
                ...
                else
                {
                    break;
                     ...
                     && (iterator_info->mdb_iterator.core_id < (DBAL_MAX_NUM_OF_CORES - 1)))
            {
                ++iterator_info->mdb_iterator.core_id;
            }
            else
            {
                break;
    ...
    else if (table->core_mode == DBAL_CORE_MODE_DPCG)
    {
        ...
        while (iterator_info->is_end)
        {
            ...
            if ((iterator_info->mdb_iterator.core_id < (DBAL_NOF_CORE_GROUPS - 1))
                && !iterator_info->attrib_info.core_group_rule_valid)
            {
                ++iterator_info->mdb_iterator.core_id;
            }
            else
    ...
    if (!iterator_info->is_end)
    {
        ...
        if ((((table->core_mode == DBAL_CORE_MODE_DPC) && (DBAL_MAX_NUM_OF_CORES > 1))
             ||
             ((table->core_mode == DBAL_CORE_MODE_DPCG) && (DBAL_NOF_CORE_GROUPS > 1)))
            &&
    ...
    uint8 is_eedb_ll)
{
    ...
    int orig_core_id = phy_entry->core_id;
    ...
    {
        if ((DBAL_NOF_CORE_GROUPS > 1) && (entry_handle->table->core_mode == DBAL_CORE_MODE_SBC))
        {
            uint8 core_group_id;

            SHR_IF_ERR_EXIT(dbal_sbc_group_id_get(unit, &core_group_id));
            phy_entry->core_id = core_group_id;
        }
    ...
    if (!(DBAL_TABLE_IS_TCAM(entry_handle->table) && entry_handle->table->core_mode == DBAL_CORE_MODE_DPC))
    {
        ...
        phy_entry->core_id = orig_core_id;
    }
    ...
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_HAS_RESULT_TYPE))
    {
        ...
        {
            if (entry_handle->table_id != DBAL_TABLE_EEDB_RIF_BASIC)
            {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (dbal_table_is_out_lif(table))
    {
        ...
        physical_table = eedb_physical_table;
             ...
             && (entry_handle->table->core_mode == DBAL_CORE_MODE_DPC)
             && (entry_handle->phy_entry.core_id != DBAL_CORE_ALL) && (DBAL_MAX_NUM_OF_CORES > 1))
    {
    ...
    uint8 is_eedb_ll)
{
    ...
    DBAL_MDB_ACCESS_ACTION_VALIDATE(physical_table_actions->entry_delete, physical_db_id);
    ...
    if (entry_handle->table_id == DBAL_TABLE_GLOBAL_RIF_EM)
    {
        ...
        SHR_IF_ERR_EXIT(dbal_mdb_global_rif_delete
                        (unit, entry_handle, physical_table_actions, app_id, physical_db_id, entry_flags[0]));
    }
    else
    {
        SHR_IF_ERR_EXIT(physical_table_actions->entry_delete(unit, physical_db_id, app_id, phy_entry, entry_flags[0]));
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_RESULT_TYPE_MAPPED_TO_SW))
    {
        ...
        {
            if (!SHR_IS_BITSET(entry_handle->table->indications_bm, DBAL_TABLE_IND_SW_RESULT_TYPE_NO_ACCESS))
            {
    ...
    SHR_IF_ERR_EXIT(dbal_mdb_access_delete_one_table(unit, app_id, eedb_physical_table, entry_handle, is_eedb_ll));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (!support_mdb_general_phys_db)
    {
        ...
        if ((entry_handle->phy_entry.core_id == DBAL_CORE_ALL) || (DBAL_MAX_NUM_OF_CORES == 1))
        {
            int at_least_one_found = FALSE;
            shr_error_e rv = _SHR_E_NONE;
            for (ii = 0; ii < entry_handle->table->nof_physical_tables; ++ii)
            {

                rv = dbal_mdb_access_delete_one_table
                    (unit, app_id, entry_handle->table->physical_db_id[ii], entry_handle, 0);
                SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(rv, _SHR_E_NOT_FOUND);
                if (!SHR_FAILURE(rv))
                {
        ...
        else
        {
            SHR_IF_ERR_EXIT(dbal_mdb_access_delete_one_table
                            (unit, app_id, entry_handle->table->physical_db_id[entry_handle->phy_entry.core_id],
                             entry_handle, 0));
        }
    }
    else
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    {
        phy_entry->core_id = DBAL_CORE_ALL;
    }
    else
    {
        phy_entry->core_id = entry_handle->phy_entry.core_id;
    }
    ...
    else if (dbal_table_is_in_lif(unit, table))
    {
    ...
    if (!support_mdb_general_phys_db
        && (entry_handle->table->core_mode == DBAL_CORE_MODE_DPC) && (entry_handle->phy_entry.core_id != DBAL_CORE_ALL))
    {
        first_phy_table = table->physical_db_id[entry_handle->phy_entry.core_id];
    }
    ...
    uint8 is_eedb_ll)
{
    ...
    DBAL_MDB_ACCESS_ACTION_VALIDATE(physical_table_actions->entry_add, physical_db_id);
    ...
    {
        SHR_IF_ERR_EXIT(physical_table_actions->entry_add(unit, physical_db_id, app_id, phy_entry, entry_flags[0]));
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    SHR_IF_ERR_EXIT(dbal_mdb_access_write_one_table(unit, app_id, eedb_physical_table, entry_handle, is_eedb_ll));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (!support_mdb_general_phys_db)
    {
        ...
        if ((entry_handle->phy_entry.core_id == DBAL_CORE_ALL) || (DBAL_MAX_NUM_OF_CORES == 1))
        {
            ...
            for (ii = 0; ii < entry_handle->table->nof_physical_tables; ++ii)
            {
                ...
                if (entry_handle->phy_entry.indirect_commit_mode == DBAL_INDIRECT_COMMIT_MODE_UPDATE)
                {
                    ...
                    if (ii == (entry_handle->table->nof_physical_tables - 1))
                    {
                ...
                else
                {
                    ...
                    if (rv != _SHR_E_NONE)
                    {
                        ...
                        {
                            SHR_IF_ERR_EXIT(dbal_mdb_access_delete_one_table
                                            (unit, app_id, entry_handle->table->physical_db_id[jj], entry_handle, 0));
                        }
                        SHR_IF_ERR_EXIT_WITH_MSG_EXCEPT_IF(rv, _SHR_E_EXISTS);
                    }
                }
            }
        }
        ...
        {
            SHR_IF_ERR_EXIT(dbal_mdb_access_write_one_table
                            (unit, app_id, entry_handle->table->physical_db_id[entry_handle->phy_entry.core_id],
                             entry_handle, 0));
        }
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    SHR_IF_ERR_EXIT(physical_table_actions->table_clear(unit, physical_db_id, app_id));
