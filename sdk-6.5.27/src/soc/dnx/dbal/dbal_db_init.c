...
#define DBAL_STANDARD_IMAGE_NAME "S121"
...
#define DBAL_DB_INIT_TABLE_IMAGE_SPECIFIC_TABLE_INDICATION_SET(_table_entry, _current_image)  \
    ...
    for (ii = 0; ii < formula->nof_vars; ii++)
    {
        if (formula->nof_vars == 1)
        {
            ...
            else if (formula->field_id != DBAL_FIELD_EMPTY)
            {
                ...
                {
                    vars[ii] = entry_handle->phy_entry.core_id;
                    if ((entry_handle->phy_entry.core_id == DBAL_CORE_ALL)
                        || (entry_handle->phy_entry.core_id == DBAL_CORE_NOT_INITIALIZED))
                    {
    ...
    int size)
{
    ...
    {
        uint32 field_type_size = 0;
        SHR_IF_ERR_EXIT(dbal_fields_field_type_bit_size_get(unit, field_info->field_id, &field_type_size));
        field_info->field_nof_bits = field_type_size;
    }
    ...
    dbal_table_field_info_t * field_info)
{
    ...
    if (field_effective_size > field_type_size)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
    ...
    uint8 has_arr_prefix_width)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_bit_size_get(unit, field_info->field_id, &field_type_size));
    SHR_IF_ERR_EXIT(dbal_fields_field_type_max_value_get(unit, field_info->field_id, &field_type_max_value));
    ...
    if (field_type_info->encode_info.encode_mode != DBAL_VALUE_FIELD_ENCODE_ENUM)
    {
        ...
        if (field_type_size > 32)
        {
            field_info->max_value = max_value_effective_size;
        }
        else
        {
            field_info->max_value = UTILEX_MIN(field_type_max_value, max_value_effective_size);
        }
    }
    else
    {

        field_info->max_value = field_type_max_value;

        if (!has_arr_prefix_width)
        {
            if ((field_type_size > field_info->field_nof_bits))
            {
                uint32 field_max_value = utilex_max_value_by_size(field_info->field_nof_bits);
                field_info->max_value = UTILEX_MIN(field_type_max_value, field_max_value);
            }
        }
    }
...
dbal_db_init_table_incompatible_image_indication_set(
    ...
    char *current_image)
{
    uint8 is_compatible_with_all_images = (current_image == NULL);
    uint8 is_dbal_table_standard = FALSE;

    SHR_FUNC_INIT_VARS(unit);

    if (!is_compatible_with_all_images)
    {
        SHR_IF_ERR_EXIT(dnx_pp_prgm_is_standard_image(unit, current_image, &is_dbal_table_standard));
    }

    if (!is_dbal_db_init_status_incompatible_image(unit, is_dbal_table_standard, is_compatible_with_all_images))
    {
        SHR_EXIT();
    }
...
static shr_error_e
dbal_db_init_dbal_image_is_standard(
    int unit,
    char *dbal_current_image,
    uint8 *dbal_image_is_standard)
{
    uint8 is_compatible_with_all_images = (dbal_current_image == NULL);
    SHR_FUNC_INIT_VARS(unit);

    if (is_compatible_with_all_images)
    {
        *dbal_image_is_standard = FALSE;
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_pp_prgm_is_standard_image(unit, dbal_current_image, dbal_image_is_standard));
    }
exit:
    SHR_FUNC_EXIT;
}
...
static shr_error_e
dbal_db_init_invalid_dbal_image_skip(
    int unit,
    char *dbal_current_image,
    uint8 *is_skip_table_init)
{
    uint8 is_compatible_with_all_images = (dbal_current_image == NULL);
    uint8 is_dbal_image_standard = FALSE;
    SHR_FUNC_INIT_VARS(unit);

    if (is_compatible_with_all_images)
    {
        *is_skip_table_init = FALSE;
        SHR_EXIT();
    }

    SHR_IF_ERR_EXIT(dnx_pp_prgm_is_standard_image(unit, dbal_current_image, &is_dbal_image_standard));
    *is_skip_table_init = (!is_dbal_image_standard);
    ...
    uint8 *is_skip_field_type_init)
{
    ...
    *is_skip_field_type_init = FALSE;
    SHR_IF_ERR_EXIT(dbal_db_init_invalid_dbal_image_skip(unit, field_type_current_image, is_skip_field_type_init));
    ...
    SHR_IF_ERR_EXIT(dbal_db_init_dbal_image_is_standard(unit, field_type_current_image, &is_dbal_image_standard));
    if (is_dbal_image_standard)
    {
...
static void
dbal_db_init_table_not_valid_or_not_mature_duplicated_table_skip(
    dbal_logical_table_t * table_entry,
    uint8 new_table_is_valid_and_mature,
    uint8 *is_skip_table_init)
{
...
dbal_db_init_table_init_skip(
    int unit,
    dbal_logical_table_t * table_entry,
    uint8 table_is_valid_and_mature,
    char *table_current_image,
    uint8 *is_skip_table_init)
{
    uint8 not_valid_or_not_mature_duplicated_table_skip = FALSE;
    uint8 invalid_dbal_image_skip = FALSE;

    SHR_FUNC_INIT_VARS(unit);

    dbal_db_init_table_not_valid_or_not_mature_duplicated_table_skip(table_entry, table_is_valid_and_mature,
                                                                     &not_valid_or_not_mature_duplicated_table_skip);
    if (not_valid_or_not_mature_duplicated_table_skip)
    {
        *is_skip_table_init = TRUE;
        SHR_EXIT();
    }

    SHR_IF_ERR_EXIT(dbal_db_init_invalid_dbal_image_skip(unit, table_current_image, &invalid_dbal_image_skip));
    if (invalid_dbal_image_skip)
    {
        *is_skip_table_init = TRUE;
        SHR_EXIT();
    }
    ...
    uint8 *is_skip_table_init)
{
    ...
    dbal_db_init_table_init_skip(unit, table_entry, is_valid_and_mature, current_image, is_skip_table_init);
    ...
    DBAL_DB_INIT_TABLE_IMAGE_SPECIFIC_TABLE_INDICATION_SET(table_entry, current_image);
    SHR_IF_ERR_EXIT(dbal_db_init_table_incompatible_image_indication_set(unit, table_id, current_image));
...
dbal_superset_result_type_field_add(
    ...
    dbal_table_field_info_t * field_info)
{
    uint32 max_unprefixed_value = 0;
    SHR_FUNC_INIT_VARS(unit);

    if ((field_info->arr_prefix_size) && (!(SHR_BITGET(field_info->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM))))
    {
        max_unprefixed_value = ((1 << field_info->field_nof_bits) - 1);
        SHR_IF_ERR_EXIT(dbal_fields_encode_prefix(unit, &max_unprefixed_value, field_info->arr_prefix,
                                                  field_info->field_nof_bits,
                                                  field_info->field_nof_bits + field_info->arr_prefix_size,
                                                  &(superset_res_type->results_info[field_idx].max_value)));
        superset_res_type->results_info[field_idx].min_value =
            superset_res_type->results_info[field_idx].max_value - max_unprefixed_value;
    }
    else
    {
...
dbal_superset_result_type_field_modify(
    ...
    dbal_table_field_info_t * field_info)
{
    uint32 max_unprefixed_value = 0, field_max_value = 0, field_min_value = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    if ((field_info->arr_prefix_size) && (!(SHR_BITGET(field_info->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM))))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_fields_encode_prefix(unit, &max_unprefixed_value, field_info->arr_prefix,
                                                  ...
                                                  field_info->field_nof_bits + field_info->arr_prefix_size,
                                                  &field_max_value));
        field_min_value = field_max_value - max_unprefixed_value;
    }
    else
    {
        field_max_value = field_info->max_value;
        field_min_value = field_info->min_value;
    }
    ...
    if (superset_res_type->results_info[field_idx].min_value > field_min_value)
    {
        superset_res_type->results_info[field_idx].min_value = field_min_value;
    }
    ...
    dbal_table_field_info_t * field_info)
{
    ...
    for (field_idx = 0; field_idx < (superset_res_type->nof_result_fields); field_idx++)
    {
        ...
        rv = dbal_fields_sub_field_info_get(unit, field_info->field_id, exising_field_id, &sub_field_info);
        if ((SHR_SUCCESS(rv) && (sub_field_info != NULL)) || (exising_field_id == field_info->field_id))
        {
            break;
        }
        ...
        {
            rv = dbal_fields_sub_field_info_get(unit, exising_field_id, field_info->field_id, &sub_field_info);
            if (SHR_SUCCESS(rv) && (sub_field_info != NULL))
            {
    ...
    if (field_idx == (superset_res_type->nof_result_fields))
    {
        ...
        SHR_IF_ERR_EXIT(dbal_superset_result_type_field_add(unit, superset_res_type, field_idx, field_info));
    ...
    dbal_logical_table_t * table)
{
    ...
    {
        if (table_id == DBAL_TABLE_EEDB_RIF_BASIC)
        {
            return FALSE;
        }
        return TRUE;
    ...
    dbal_logical_table_t * table)
{
    ...
    if (alloc_bit_num > 0)
    {
        ...
        {
            nof_optimized_bitmaps = DBAL_MAX_NUM_OF_CORES;
        }
        else if (table->core_mode == DBAL_CORE_MODE_DPCG)
        {
            nof_optimized_bitmaps = DBAL_NOF_CORE_GROUPS;
        }

        for (core_index = 0; core_index < nof_optimized_bitmaps; core_index++)
        {
    ...
    dbal_logical_table_t * table)
{
    ...
    if ((table->core_mode <= DBAL_CORE_NONE) || (table->core_mode > (DBAL_NOF_CORE_MODE_TYPES - 1)))
    {
