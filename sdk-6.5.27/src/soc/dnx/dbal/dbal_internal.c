    ...
    int unit)
{
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(dbal_mngr[unit]->internal_entry_handles_pool, handles_size,
                                "internal handles pool", "%d%s%s\r\n", handles_size, EMPTY, EMPTY);
    ...
    int unit)
{
    ...
    if (dbal_mngr[unit] != NULL)
    {
        ...
        SHR_FREE(dbal_mngr[unit]->internal_entry_handles_pool);
    ...
    int unit)
{
    ...
    dbal_mngr[unit]->sbc_table_core_group_id = 0;
    ...
    int unit)
{
    ...
    for (handle_index = 0; handle_index < DBAL_SW_NOF_ENTRY_HANDLES; handle_index++)
    {
        dbal_mngr[unit]->internal_entry_handles_pool[handle_index].handle_status = DBAL_HANDLE_STATUS_AVAILABLE;
    }
...
shr_error_e
dbal_handles_range_ptr_free_all(
    int unit)
{
    SHR_FUNC_INIT_VARS(unit);
...
shr_error_e
dbal_sbc_group_id_set(
    int unit,
    uint8 cg_id)
{
    SHR_FUNC_INIT_VARS(unit);

    if (cg_id > (DBAL_NOF_CORE_GROUPS - 1))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Core Group Id should not be bigger than %d\n", cg_id);
    }
    dbal_mngr[unit]->sbc_table_core_group_id = cg_id;
...
shr_error_e
dbal_sbc_group_id_get(
    int unit,
    uint8 *cg_id)
{
    SHR_FUNC_INIT_VARS(unit);

    *cg_id = dbal_mngr[unit]->sbc_table_core_group_id;
...
dbal_entry_handle_status_performed_set(
    dbal_entry_handle_t * entry_handle)
{
    entry_handle->handle_status = DBAL_HANDLE_STATUS_ACTION_PERFORMED;
}
shr_error_e
dbal_internal_entry_handle_alloc(
    int unit,
    ...
    dbal_entry_handle_t ** handle)
{
    ...
    else if (dbal_mngr[unit]->internal_entry_handles_pool[entry_handle_id].handle_status !=
             DBAL_HANDLE_STATUS_AVAILABLE)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "handle is not available %d, used in table %s\n", entry_handle_id,
                     dbal_mngr[unit]->internal_entry_handles_pool[entry_handle_id].table->table_name);
    }

    sal_memset(&(dbal_mngr[unit]->internal_entry_handles_pool[entry_handle_id]), 0x0, sizeof(dbal_entry_handle_t));

    (*handle) = &(dbal_mngr[unit]->internal_entry_handles_pool[entry_handle_id]);
...
shr_error_e
dbal_internal_entry_handle_free(
    int unit,
    dbal_entry_handle_t ** user_entry_handle)
{
    dbal_entry_handle_t *entry_handle = (*user_entry_handle);

    SHR_FUNC_INIT_VARS(unit);

    if (entry_handle)
    {
        if (entry_handle->handle_id >= DBAL_SW_NOF_ENTRY_HANDLES)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Entry handle out of bound = %d \n", entry_handle->handle_id);
        }
        else if (dbal_mngr[unit]->internal_entry_handles_pool[entry_handle->handle_id].handle_status ==
                 DBAL_HANDLE_STATUS_AVAILABLE)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "handle is already taken %d \n", entry_handle->handle_id);
        }

        dbal_mngr[unit]->internal_entry_handles_pool[entry_handle->handle_id].handle_status =
            DBAL_HANDLE_STATUS_AVAILABLE;

        SHR_FREE(entry_handle->key_field_ranges);

        (*user_entry_handle) = NULL;
    }
    ...
    uint8 operation)
{
    ...
    else
    {
        ...
        SHR_FREE(dbal_mngr[unit]->entry_handles_pool[*entry_handle_id].user_output_info);
        SHR_FREE(dbal_mngr[unit]->entry_handles_pool[*entry_handle_id].key_field_ranges);
    ...
    entry_handles_pool_ptr->nof_ranged_fields = 0;
    ...
    {
        entry_handles_pool_ptr->phy_entry.core_id = DBAL_CORE_ALL;;
    }
    else if (table->core_mode == DBAL_CORE_MODE_DPCG)
    {

        entry_handles_pool_ptr->phy_entry.core_id = DBAL_CORE_GROUP_ALL;
    }
    else
    {
        entry_handles_pool_ptr->phy_entry.core_id = DBAL_CORE_NOT_INITIALIZED;
    }
    ...
    uint32 *dst_handle_id)
{
    ...
    *dst_handle_id = handle_index;
    ...
    DBAL_ENTRY_HANDLE_KEY_FIELD_RANGES_COPY(*src_handles_pool_ptr, *dst_handles_pool_ptr);
...
void
dbal_internal_key_field_ranges_alloc(
    int unit,
    dbal_entry_handle_t * entry_handle)
{
    SHR_FUNC_INIT_VARS(unit);

    if (entry_handle->key_field_ranges == NULL)
    {
        SHR_ALLOC_SET_ZERO_ERR_EXIT(entry_handle->key_field_ranges,
                                    DBAL_TABLE_MAX_NUM_OF_KEY_FIELDS * sizeof(uint32),
                                    entry_handle->table->table_name, "%s%s%s", EMPTY, EMPTY, EMPTY);
    }
...
static void
dbal_internal_user_output_info_alloc(
    int unit,
    dbal_entry_handle_t * entry_handle)
{
    SHR_FUNC_INIT_VARS(unit);

    if (entry_handle->user_output_info == NULL)
    {
        SHR_ALLOC_SET_ZERO_ERR_EXIT(entry_handle->user_output_info,
    ...
    uint32 *field_mask)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_bit_size_get(unit, field_id, &field_type_size));
    ...
    if (encode_type == DBAL_VALUE_FIELD_ENCODE_PREFIX)
    {
        ...
        uint32 mask_value;
        SHR_IF_ERR_EXIT(dbal_fields_field_type_bit_size_get(unit, field_id, &prefix_length));
    ...
    {
        uint32 mask_value = (1 << (nof_bits - field_type_size)) - 1;

        SHR_IF_ERR_EXIT(dbal_fields_encode_suffix
                        (unit, field_mask, mask_value, field_type_size, nof_bits, encoded_field_mask));
    ...
    dbal_pointer_type_e pointer_type)
{
    ...
    if (field_id == DBAL_FIELD_CORE_ID)
    {
        ...
        {
            entry_handle->phy_entry.core_id = field_val[0];
        ...
        && !SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ADDRESS))
    {
        ...
        if (is_sub_field)
        {
            DBAL_FIELD_ERR_HANDLE(dbal_fields_field_type_max_value_get(unit, field_id, &max_value),
    ...
    if ((entry_handle->key_field_ranges != NULL) && (entry_handle->key_field_ranges[curr_field_index] != 0))
    {
...
static shr_error_e
dbal_internal_is_enum_valid(
    int unit,
    dbal_table_field_info_t * table_field_info,
    uint32 *field_val,
    int *is_invalid)
{
    uint32 field_hw_value, field_max_hw_value;
    ...
    *is_invalid = 0;
    ...
    SHR_IF_ERR_EXIT(dbal_field_types_info_get(unit, table_field_info->field_type, &field_type_info));
    SHR_IF_ERR_EXIT(dbal_fields_enum_hw_value_get(unit, table_field_info->field_id, field_val[0], &field_hw_value));

    field_max_hw_value = (uint32) (((uint64) 1 << table_field_info->field_nof_bits) - 1);

    if (table_field_info->arr_prefix)
    {
        int rv = 0;
        rv = dbal_field_arr_prefix_decode(unit, table_field_info, field_hw_value, &field_hw_value, TRUE);
        if (SHR_FAILURE(rv))
        {
    ...
    {
        *is_invalid = (field_val[0] > table_field_info->max_value) || (field_val[0] < table_field_info->min_value);
    }

    *is_invalid = (*is_invalid) || (field_val[0] > field_type_info->max_value)
        || (field_val[0] < field_type_info->min_value) || (field_type_info->enum_val_info[field_val[0]].is_invalid)
        || (field_hw_value > field_max_hw_value);
    ...
    uint32 *field_mask)
{
        ...
        && !SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ADDRESS))
    {
        ...
        if (!is_sub_field && (table_field->arr_prefix_size != 0)
        ...
        else if (SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM))
        {
            int is_invalid = 0;
            ret_val = dbal_internal_is_enum_valid(unit, table_field, field_val, &is_invalid);
            DBAL_FIELD_ERR_HANDLE_NO_RETURN(ret_val, "dbal_internal_is_enum_valid");
            if (is_invalid)
            {

                if (dbal_image_name_is_std_1(unit))
                {
                    DBAL_FIELD_ERR_HANDLE_NO_RETURN(_SHR_E_INTERNAL, "Illegal value for field");
                    LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "enum value = %d is invalid\n"), field_val[0]));
                }
                return;
            }
        }
        else
        {
            ...
            else if (field_val[0] < table_field->min_value)
            {
                DBAL_FIELD_ERR_HANDLE_NO_RETURN(_SHR_E_INTERNAL, "Illegal value for field, too low");
                LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "field value = %d min value %d\n"), field_val[0],
                                           table_field->min_value));
    ...
    if ((SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENCODED)) || is_sub_field)
    {
        int total_field_size = table_field->field_nof_bits;
        ...
        if (is_sub_field && (table_field->arr_prefix_size != 0))
        {
            ...
            dbal_field_types_basic_info_t *field_type_info;
            ret_val = dbal_fields_field_types_info_get(unit, parent_field_id, &field_type_info);
            total_field_size = field_type_info->max_size;
        }

        ret_val = dbal_fields_field_encode(unit, field_id, parent_field_id, total_field_size,
                                           curr_field_value_ptr, encoded_field_value);
        ...
        if (is_sub_field && (table_field->arr_prefix_size != 0))
        {
            ...
            ret_val =
                dbal_field_arr_prefix_decode(unit, table_field, encoded_field_value[0], encoded_field_value, TRUE);
            DBAL_FIELD_ERR_HANDLE(ret_val, "dbal_field_arr_prefix_decode", inst_id);
        }
        ...
        if (SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM))
        {
            uint32 enum_max_value;
            if (table_field->field_nof_bits == 32)
            {
                enum_max_value = 0xffffffff;
            }
            else
            {
                enum_max_value = (1U << table_field->field_nof_bits) - 1;
            }
    ...
    void *field_mask)
{
    ...
    DBAL_FIELD_ERR_HANDLE(((entry_handle->table->nof_result_types > 1)
                           && (entry_handle->cur_res_type == DBAL_RESULT_TYPE_NOT_INITIALIZED)),
                          "Request API cannot be used for table with multiple result types. Please use GET_ALL_FIELDS\n",
                          inst_id);

    DBAL_FIELD_ERR_HANDLE((entry_handle->handle_status == DBAL_HANDLE_STATUS_ACTION_PERFORMED),
    ...
    dbal_internal_user_output_info_alloc(unit, entry_handle);
    ...
    uint32 *field_val)
{
    ...
    if (entry_handle->phy_entry.core_id != DBAL_CORE_NOT_INITIALIZED)
    {
        (*field_val) = entry_handle->phy_entry.core_id;
    }
    else
    {
    ...
    if ((log_enabled) || (dbal_mngr[unit]->logger_info.dbal_file != NULL))
    {
        ...
        for (iter = 0; iter < table->nof_key_fields; iter++)
        {
            ...
            if (key_field == DBAL_FIELD_CORE_ID)
            {
                ...
                {
                    sal_snprintf(buffer, DBAL_MAX_STRING_LENGTH, " %d ", entry_handle->phy_entry.core_id);
                }
            }
            else
            {
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    for (iter = 0; iter < table->multi_res_info[entry_handle->cur_res_type].nof_result_fields; iter++)
    {
        ...
        switch (entry_handle->user_output_info[iter].pointer_type)
        {
            ...
            default:
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "Illegal pointer type %d\n",
    ...
    uint32 flags)
{
    ...
    if (attr_type == DBAL_ENTRY_ATTR_AGE)
    {
        ...
        entry_handle->attr_flags |= DBAL_ENTRY_ATTR_FLAG_AGE;
    }
    else if (attr_type == DBAL_ENTRY_ATTR_HITBIT)
    {
        ...
        if (hitbit_en)
        {
            ...
            if (flags & DBAL_ENTRY_ATTR_FLAG_HIT_GET)
            {
            ...
            if (flags & DBAL_ENTRY_ATTR_FLAG_HIT_CLEAR)
            {

                entry_handle->phy_entry.hitbit |= DBAL_PHYSICAL_KEY_HITBIT_CLEAR;
            }
            if (flags & DBAL_ENTRY_ATTR_FLAG_HIT_PRIMARY)
            {
                entry_handle->phy_entry.hitbit |= DBAL_PHYSICAL_KEY_HITBIT_PRIMARY;
            }
            if (flags & DBAL_ENTRY_ATTR_FLAG_HIT_SECONDARY)
            {
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Request called for an unsupported attribute. Only hitbit and age are expected");
    }
    ...
    uint32 *returned_field_value)
{
    ...
    else
    {
        ...
        {
            int total_field_size = table_field_info->field_nof_bits;

            if (is_sub_field && (table_field_info->arr_prefix_size != 0))
            {

                int total_size = table_field_info->arr_prefix_size + table_field_info->field_nof_bits;
                dbal_field_types_basic_info_t *parent_field_type_info;
                SHR_IF_ERR_EXIT(dbal_fields_field_types_info_get(unit, parent_field_id, &parent_field_type_info));
                total_field_size = parent_field_type_info->max_size;
                SHR_IF_ERR_EXIT(dbal_fields_encode_prefix(unit, curr_field_value_ptr, table_field_info->arr_prefix,
                                                          table_field_info->field_nof_bits, total_size,
                                                          curr_field_value_ptr));
            }

            SHR_IF_ERR_EXIT(dbal_fields_field_decode
                            (unit, field_id, parent_field_id, total_field_size, curr_field_value_ptr,
                             field_val_after_decoding));
    ...
    dbal_key_value_validity_e * is_valid)
{
    ...
    *is_valid = DBAL_KEY_IS_VALID;
    ...
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
        ...
        {
            if ((entry_handle->phy_entry.core_id == DBAL_CORE_ALL) ||
                ((entry_handle->phy_entry.core_id < DBAL_MAX_NUM_OF_CORES)
                 && (entry_handle->phy_entry.core_id >= DBAL_CORE_DEFAULT)))
            {
                key_info.field_val[0] = entry_handle->phy_entry.core_id;
            }
            else
            {
    ...
    if (is_allocator)
    {
        ...
        else
        {
            SHR_IF_ERR_EXIT(dnx_algo_res_dbal_get_next
                            (unit, entry_handle->phy_entry.core_id, 0, entry_handle->table_id,
                             table->allocator_field_id, &entry_handle->cur_res_type, &index));
        }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    core_bmp[0] = dnx_data_device.general.valid_cores_bitmap_get(unit);
    table = entry_handle->table;

    if (table->core_mode == DBAL_CORE_MODE_DPCG)
    {

        entry_handle->phy_entry.core_id = DBAL_NOF_CORE_GROUPS - 1;
    }
    ...
    for (ii = 0; ii < table->nof_key_fields; ii++)
    {
        ...
        if (table->keys_info[ii].field_id == DBAL_FIELD_CORE_ID)
        {
            if (table->core_mode == DBAL_CORE_MODE_DPC)
            {
                ...
                while (entry_handle->phy_entry.core_id >= 0)
                {
                    if (SHR_IS_BITSET(core_bmp, entry_handle->phy_entry.core_id))
                    {
                        break;
                    }
                    else
    ...
    uint32 *field_mask)
{
    ...
    {
        SHR_IF_ERR_EXIT(dbal_fields_field_type_bit_size_get(unit, field_id, &field_max_size));
        SHR_IF_ERR_EXIT(dbal_fields_print_type_get(unit, field_id, &field_print_type));
        if (field_print_type == DBAL_FIELD_PRINT_TYPE_STR)
        {
            SHR_ERR_EXIT(_SHR_E_EMPTY, "Rule not supported for field print type string.\n");
    ...
    uint32 *field_mask)
{
    ...
    if (action_type == DBAL_ITER_ACTION_UPDATE)
    {
        ...
        {
            uint32 field_type_size;

            SHR_IF_ERR_EXIT(dbal_fields_field_type_bit_size_get(unit, field_id, &field_type_size));

            SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field(field_value, 0, field_type_size,
                                                           iterator_info->actions_info[iterator_info->
                                                                                       nof_actions].value));
            if (field_mask != NULL)
            {
                SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field(field_mask, 0, field_type_size,
                                                               iterator_info->actions_info[iterator_info->
                                                                                           nof_actions].mask));
            }
            else
            {
                SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field(G_dbal_field_full_mask, 0, field_type_size,
                                                               iterator_info->actions_info[iterator_info->
                                                                                           nof_actions].mask));
            }
        }
    ...
    char *top_node_name)
{
    ...
    while ((dirp = sal_readdir(dir)) != NULL)
    {
        ...
        SHR_FREE(path);
        path_length = sal_strnlen(parent_dir, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) +
            sal_strnlen("/", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) +
            sal_strnlen(dirp->d_name, sizeof(dirp->d_name)) + 100;
        if (path_length > SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)
        {
            SHR_ERR_EXIT(_SHR_E_RESOURCE, "Path length (%d) exceeds the max string size (%d)\n", path_length,
                         SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
        }
        SHR_ALLOC_ERR_EXIT(path, path_length, "dir path", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

        sal_snprintf(path, path_length, "%s/%s", parent_dir, dirp->d_name);
...
dbal_internal_lpt_encode_word(
    ...
    uint32 *lpt_y)
{
    uint32 lpt_x0, lpt_y0, lpt_x1, lpt_y1;
    uint32 KEY[2], MASK[2];
    SHR_FUNC_INIT_VARS(unit);

    KEY[0] = key & 0x55555555;
    KEY[1] = (key & 0xaaaaaaaa) >> 1;

    MASK[0] = mask & 0x55555555;
    MASK[1] = (mask & 0xaaaaaaaa) >> 1;

    lpt_y1 = ((~KEY[1]) & MASK[1]) | (KEY[0] & MASK[0]);
    lpt_x1 = (KEY[1] & MASK[1]) | ((~KEY[0]) & MASK[0]);
    lpt_y0 = ((~KEY[1]) & MASK[1]) | ((~KEY[0]) & MASK[0]);
    lpt_x0 = (KEY[1] & MASK[1]) | (KEY[0] & MASK[0]);

    *lpt_x = (lpt_x1 << 1) | lpt_x0;
    *lpt_y = (lpt_y1 << 1) | lpt_y0;
...
dbal_internal_lpt_encode_byte(
    ...
    uint8 *lpt_y)
{
    uint8 lpt_x0, lpt_y0, lpt_x1, lpt_y1;
    uint8 KEY[2], MASK[2];
    SHR_FUNC_INIT_VARS(unit);
...
dbal_internal_lpt_encode(
    ...
    uint8 *out_y,
    uint32 length,
    uint32 flag_use_one_as_dont_care)
{
    uint32 i;
    uint32 num_words;
    uint32 *in_word_x;
    ...
    if (length & 3)
    {
        for (i = 0; i < length; i++)
        {
            SHR_IF_ERR_EXIT(dbal_internal_lpt_encode_byte(unit, in_x[i], in_y[i], &(out_x[i]), &(out_y[i])));
        }
    }
    else
    {
        in_word_x = (uint32 *) in_x;
        in_word_y = (uint32 *) in_y;
        out_word_x = (uint32 *) out_x;
        out_word_y = (uint32 *) out_y;
        num_words = length >> 2;

        for (i = 0; i < num_words; i++)
        {
            if (flag_use_one_as_dont_care)
            {
                SHR_IF_ERR_EXIT(dbal_internal_lpt_encode_word
                                (unit, in_word_x[i], ~in_word_y[i], &(out_word_x[i]), &(out_word_y[i])));
            }
            else
            {
                SHR_IF_ERR_EXIT(dbal_internal_lpt_encode_word
...
shr_error_e
dbal_internal_xy_encode(
    int unit,
    uint32 *in_data,
    uint32 *in_mask,
    uint32 *out_data,
    uint32 *out_mask,
    uint32 length_in_bits)
{
    uint32 out_data_not[DBAL_HL_MAX_MEMORY_LINE_SIZE_IN_WORDS] = { 0 };

    SHR_FUNC_INIT_VARS(unit);

    SHR_BITAND_RANGE(in_data, in_mask, 0, length_in_bits, out_data);
    SHR_BITNEGATE_RANGE(out_data, 0, length_in_bits, out_data_not);
    SHR_BITAND_RANGE(out_data_not, in_mask, 0, length_in_bits, out_mask);
    ...
    dbal_encoding_types_t type)
{
    ...
    switch (type)
    {
        case DBAL_ENCODING_TYPE_XY:
            SHR_IF_ERR_EXIT(dbal_internal_xy_encode(unit, in_data, in_mask, out_data, out_mask, length_in_bits));
            break;
        case DBAL_ENCODING_TYPE_LPT:
            SHR_IF_ERR_EXIT(dbal_internal_lpt_encode(unit, (uint8 *) in_data, (uint8 *) in_mask, (uint8 *) out_data,
                                                     (uint8 *) out_mask, BITS2BYTES(length_in_bits), 0));
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "Non supported encoding type");
    }
...
dbal_internal_lpt_decode_byte(
    ...
    uint8 *mask)
{
    uint8 data0, mask0, data1, mask1;
    uint8 LPT_X[2], LPT_Y[2];
    SHR_FUNC_INIT_VARS(unit);

    LPT_X[0] = lpt_x & 0x55;
    LPT_X[1] = (lpt_x & 0xaa) >> 1;

    LPT_Y[0] = lpt_y & 0x55;
    LPT_Y[1] = (lpt_y & 0xaa) >> 1;

    data1 = ((~LPT_Y[1]) | (~LPT_Y[0])) & LPT_X[0] & LPT_X[1];

    mask1 =
        ((~LPT_Y[1]) & LPT_X[1] & LPT_X[0]) | (LPT_X[1] & (~LPT_Y[0]) & LPT_X[0]) | (LPT_Y[1] & LPT_Y[0] & (~LPT_X[0]))
        | (LPT_Y[1] & (~LPT_X[1]) & LPT_Y[0]);

    data0 = LPT_Y[1] & ((~LPT_X[1]) | (~LPT_Y[0])) & LPT_X[0];
    mask0 =
        ((~LPT_Y[1]) & LPT_X[1] & LPT_Y[0]) | (LPT_X[1] & LPT_Y[0] & (~LPT_X[0])) | (LPT_Y[1] & (~LPT_Y[0]) & LPT_X[0])
...
dbal_internal_lpt_decode_word(
    ...
    uint32 *mask)
{
    uint32 data0, mask0, data1, mask1;
    uint32 LPT_X[2], LPT_Y[2];

    SHR_FUNC_INIT_VARS(unit);

    LPT_X[0] = lpt_x & 0x55555555;
    LPT_X[1] = (lpt_x & 0xaaaaaaaa) >> 1;

    LPT_Y[0] = lpt_y & 0x55555555;
    LPT_Y[1] = (lpt_y & 0xaaaaaaaa) >> 1;
    ...
    uint32 flag_use_one_as_dont_care)
{
    ...
    if (length & 3)
    {
        for (i = 0; i < length; i++)
        {
            SHR_IF_ERR_EXIT(dbal_internal_lpt_decode_byte(unit, in_x[i], in_y[i], &(out_x[i]), &(out_y[i])));
            if (flag_use_one_as_dont_care)
            {
                out_y[i] = ~out_y[i];
            }
    ...
    else
    {
        ...
        for (i = 0; i < num_words; i++)
        {
            SHR_IF_ERR_EXIT(dbal_internal_lpt_decode_word
                            (unit, in_word_x[i], in_word_y[i], &(out_word_x[i]), &(out_word_y[i])));
            if (flag_use_one_as_dont_care)
            {
                out_word_y[i] = ~out_word_y[i];
            }
        }
    }
    ...
    uint32 length_in_bits)
{
    ...
    if (in_data != out_data)
    {
        sal_memcpy(out_data, in_data, BITS2BYTES(length_in_bits));
    }

    SHR_BITOR_RANGE(in_data, in_mask, 0, length_in_bits, out_mask);
    ...
    dbal_encoding_types_t type)
{
    ...
    {
        case DBAL_ENCODING_TYPE_XY:
            SHR_IF_ERR_EXIT(dbal_internal_xy_decode(unit, in_data, in_mask, out_data, out_mask, length_in_bits));
            break;
        case DBAL_ENCODING_TYPE_LPT:
            SHR_IF_ERR_EXIT(dbal_internal_lpt_decode(unit, (uint8 *) in_data, (uint8 *) in_mask, (uint8 *) out_data,
                                                     (uint8 *) out_mask, BITS2BYTES(length_in_bits), 0));
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_PARAM, "Non supported decoding type");
    }
    ...
    uint8 *is_valid_value)
{
    ...
    *is_valid_value = TRUE;
    ...
    if (SHR_BITGET(table_field_info->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM)
        || (table_field_info->arr_prefix_size == 0))
    {
        int is_invalid = 0;
        if (SHR_BITGET(table_field_info->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM))
        {
            SHR_IF_ERR_EXIT(dbal_internal_is_enum_valid(unit, table_field_info, field_value, &is_invalid));
        }
        else if (table_field_info->arr_prefix_size == 0)
        {
