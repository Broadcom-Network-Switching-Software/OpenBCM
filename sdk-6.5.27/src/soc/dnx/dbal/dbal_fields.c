...
#include <src/soc/dnx/dbal/dbal_init/auto_generated/dbal_init_shared_field_types.h>
#endif
...
#define DBAL_FIELD_TYPE_VALIDATE(field_type)                                                                   \
        DBAL_FIELD_TYPE_ID_VALIDATE(field_type);                                                               \
        if (!SHR_BITGET(FIELD_TYPE_INFO(field_type)->field_indication_bm, DBAL_FIELD_IND_IS_VALID))            \
        {                                                                                                      \
            SHR_IF_ERR_EXIT_NO_MSG(_SHR_E_UNAVAIL); \
        }

#define FIELD_TYPE(_unit, _field_id)  ((_field_id < DBAL_NOF_FIELDS)? dbal_field_id_to_field_type(_unit, _field_id) : dbal_dynamic_field_id_to_type[_field_id - DBAL_NOF_FIELDS])

#define FIELD_TYPE_INFO(_field_type) field_types_basic_info[unit][(_field_type)]

#define DBAL_DYNAMIC_FIELD_TYPE_DIR "/dbal_dynamic/field_types"
    ...
    dbal_field_types_defs_e field_type)
{
    ...
    switch (field_types_info->print_type)
    {
        ...
        case DBAL_FIELD_PRINT_TYPE_SYSTEM_CORE:
            max_field_size = DBAL_CORE_SIZE_IN_BITS;
            break;
        ...
        case DBAL_FIELD_PRINT_TYPE_IPV6:
            ...
            break;
        default:
            max_field_size = DBAL_FIELD_ARRAY_MAX_SIZE_IN_BITS;
    }
    ...
    int unit)
{
    ...
    for (field_type = DBAL_FIELD_TYPE_DEF_EMPTY + 1; field_type < DBAL_NOF_FIELD_TYPES_DEF; field_type++)
    {
        ...
        if (field_types_basic_info[unit][field_type]->nof_struct_fields > 0)
        {
    ...
    int unit)
{
    ...
    SHR_ALLOC_ERR_EXIT(cur_field_type_param, sizeof(field_type_db_struct_t),
                       "field_types struct", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    cur_field_type_param->childs = NULL;
    SHR_ALLOC_ERR_EXIT(cur_field_type_param->childs,
                       DBAL_FIELD_MAX_NUM_OF_CHILD_FIELDS * sizeof(dbal_db_child_field_info_struct_t),
                       "field_types,childs", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    cur_field_type_param->enums = NULL;
    SHR_ALLOC_ERR_EXIT(cur_field_type_param->enums,
                       DBAL_FIELD_MAX_NUM_OF_ENUM_VALUES * sizeof(dbal_db_enum_info_struct_t), "field_types,enums",
                       "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    cur_field_type_param->define = NULL;
    SHR_ALLOC_ERR_EXIT(cur_field_type_param->define,
                       DBAL_FIELD_MAX_NUM_OF_DEFINE_VALUES * sizeof(dbal_db_defines_info_struct_t),
                       "field_types,define", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    cur_field_type_param->illegal_values = NULL;
    SHR_ALLOC_ERR_EXIT(cur_field_type_param->illegal_values,
                       DBAL_DB_MAX_NUM_OF_ILLEGAL_VALUES * sizeof(int),
                       "field_types,illegal_values", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

    if (DBAL_IS_DNX2(unit))
    {
        SHR_IF_ERR_EXIT(dbal_init_shared_field_types_init(unit, cur_field_type_param, field_types_basic_info[unit]));
    ...
    xml_node top_node)
{
    ...
    RHDATA_ITERATOR(field_type_node, top_node, "FieldType")
    {
        ...
        if (!sal_strncmp(str, "EMUM", sizeof(str)))
        {
        ...
        RHDATA_ITERATOR(enum_values_mapping_node, cur_node, "EnumVal")
        {
            ...
            if (sal_strncmp(name_str, str, sizeof(name_str)))
            {
            ...
            if (!sal_strncmp(str, "INVALID", sizeof(str)))
            {
                field_type_info->enum_val_info[enum_idx].is_invalid = TRUE;
    ...
    CONST dbal_field_types_basic_info_t ** field_type_info)
{
    ...
    DBAL_FIELD_TYPE_VALIDATE(field_type);
    ...
    uint32 *hw_value)
{
    ...
    for (ii = 0; ii < nof_enum_vals; ii++)
    {
        ...
        if (!sal_strncmp
            (enum_value_name, field_types_basic_info[unit][field_type]->enum_val_info[ii].name, DBAL_MAX_STRING_LENGTH))
        {
...
dbal_fields_field_create(
    int unit,
    dbal_field_types_defs_e field_type,
    const char field_name[DBAL_MAX_STRING_LENGTH],
    dbal_fields_e * field_id)
{
    int char_index;
    char eos = '\0';
    ...
    dbal_fields_e field_id)
{
    ...
    for (char_index = 0; char_index < DBAL_MAX_STRING_LENGTH; char_index++)
    {
        SHR_IF_ERR_EXIT(DBAL_SW_STATE_FIELD_PROP.field_name.set(unit, sw_state_field_index, char_index, &eos));
    }
    ...
    dbal_fields_e field_id)
{
    ...
    return (field_types_basic_info[unit][field_type]->encode_info.encode_mode != DBAL_VALUE_FIELD_ENCODE_NONE);
}
...
shr_error_e
dbal_fields_field_type_struct_field_encode(
    int unit,
    dbal_field_types_defs_e field_type,
    ...
    uint32 *full_field_val)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode_masked
                    (unit, field_type, field_in_struct_id, field_in_struct_val, NULL, full_field_val, NULL));
...
shr_error_e
dbal_fields_field_type_struct_field_encode_masked(
    int unit,
    dbal_field_types_defs_e field_type,
    dbal_fields_e field_in_struct_id,
    uint32 *field_in_struct_val,
    ...
    if (is_sub_field_found || dbal_fields_is_field_encoded(unit, field_in_struct_id))
    {
        ...
        LOG_VERBOSE(BSL_LOG_MODULE,
                    (BSL_META_U
                     ...
                     dbal_field_types_def_to_string(unit, field_type), field_in_struct_val[0], encoded_field_value[0],
    ...
    uint32 *full_field_mask)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_encode_masked
                    (unit, field_type, field_in_struct_id, field_in_struct_val, field_in_struct_mask, full_field_val,
                     full_field_mask));
...
shr_error_e
dbal_fields_field_type_struct_field_decode(
    int unit,
    dbal_field_types_defs_e field_type,
    dbal_fields_e field_in_struct_id,
    uint32 *field_in_struct_val,
    uint32 *full_field_val)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_struct_field_decode_masked
                    (unit, field_type, field_in_struct_id, internal_field_in_struct_val, NULL, full_field_val, NULL,
                     1));
...
shr_error_e
dbal_fields_field_type_struct_field_decode_masked(
    int unit,
    dbal_field_types_defs_e field_type,
    dbal_fields_e field_in_struct_id,
    uint32 *field_in_struct_val,
    ...
    int is_uint32)
{
    ...
    if ((field_type < DBAL_NOF_FIELD_TYPES_DEF) && (field_in_struct_id < DBAL_NOF_FIELDS))
    {
        ...
        if (ii == nof_struct_fields)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "field_type %d is not a struct field of field_id %d\n", field_type,
                         field_in_struct_id);
        }
        offset = field_types_basic_info[unit][field_type]->struct_field_info[ii].offset;
        length = field_types_basic_info[unit][field_type]->struct_field_info[ii].length;
        ...
        if (is_sub_field_found || dbal_fields_is_field_encoded(unit, field_in_struct_id))
        {
            ...
            LOG_VERBOSE(BSL_LOG_MODULE,
                        (BSL_META_U
                         ...
                         dbal_field_types_def_to_string(unit, field_type), field_in_struct_val[0],
                         decoded_field_value[0], dbal_field_to_string(unit, field_in_struct_id)));
            field_in_struct_val[0] = 0;
            SHR_IF_ERR_EXIT(utilex_bitstream_set_any_field
    ...
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Illegal field_type %d or %d, DBAL_NOF_FIELD_TYPES_DEF=%d\n", field_type,
...
shr_error_e
dbal_fields_field_type_subfield_info_get(
    int unit,
    dbal_fields_e field_id,
...
shr_error_e
dbal_fields_field_type_field_encode(
    int unit,
    dbal_fields_e field_id,
    ...
    uint32 *field_value_out)
{
    ...
    if (parent_field_type != DBAL_FIELD_TYPE_DEF_EMPTY)
    {
        dbal_sub_field_info_t *sub_field_info;
        int rv;

        rv = dbal_fields_field_type_subfield_info_get(unit, field_id, parent_field_type, &sub_field_info);
        if (rv || (sub_field_info == NULL))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Cannot find child field. child %s, parent %s\n",
                         dbal_field_to_string(unit, field_id), dbal_field_types_def_to_string(unit, parent_field_type));
        }
    ...
    uint32 *field_value_out)
{
    ...
    SHR_IF_ERR_EXIT_NO_MSG(dbal_fields_field_type_field_encode
                           (unit, field_id, parent_field_type, field_size, field_val, field_value_out));
    ...
    uint32 *parent_field_full_mask)
{
    ...
    if (parent_field_id != DBAL_FIELD_EMPTY)
    {
        ...
        rv = dbal_fields_sub_field_info_get(unit, field_id, parent_field_id, &sub_field_info);
        if (rv || (sub_field_info == NULL))
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Cannot find child field. child %s, parent %s\n",
...
shr_error_e
dbal_fields_field_type_field_decode(
    int unit,
    dbal_fields_e field_id,
    dbal_field_types_defs_e parent_field_type,
    int field_size,
    ...
    uint32 *field_value_out)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_field_decode
                    (unit, field_id, parent_field_type, field_size, field_val, field_value_out));
    ...
    uint32 *predef_value)
{
    ...
    switch (value_type)
    {
        ...
        case DBAL_PREDEF_VAL_DEFAULT_VALUE:
            if (!is_key && (entry_handle->table->access_method == DBAL_ACCESS_METHOD_HARD_LOGIC)
    ...
    uint8 *is_found)
{
    ...
    rv = dbal_fields_sub_field_info_get(unit, sub_field_id, parent_field_id, &sub_field_info);
    if (!rv && (sub_field_info != NULL))
    {
        (*is_found) = 1;
    }
    else if (sub_field_info != NULL)
    {
        SHR_SET_CURRENT_ERR(rv);
    }
...
shr_error_e
dbal_fields_field_type_bit_size_get(
    int unit,
    dbal_fields_e field_id,
    uint32 *field_size)
{
    ...
    uint32 *field_size)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_bit_size_get(unit, field_id, field_size));
...
shr_error_e
dbal_fields_field_type_max_value_get(
    int unit,
    dbal_fields_e field_id,
    uint32 *max_value)
{
    ...
    uint8 *has_illegals)
{
    ...
    if (FIELD_TYPE_INFO(field_type)->encode_info.encode_mode == DBAL_VALUE_FIELD_ENCODE_ENUM)
    {
...
shr_error_e
dbal_fields_field_type_uint32_sub_field_info_get(
    int unit,
    dbal_field_types_defs_e parent_field_type,
    uint32 orig_val,
    dbal_fields_e * sub_field_id,
    uint32 *sub_field_val)
{
    ...
    DBAL_FIELD_TYPE_VALIDATE(parent_field_type);

    if (field_types_basic_info[unit][parent_field_type]->max_size > 32)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error, this API support only 32 bit parent fields, field %s is %d bits \n",
                     dbal_field_types_def_to_string(unit, parent_field_type),
                     field_types_basic_info[unit][parent_field_type]->max_size);
    }

    orig_value_arr[0] = orig_val;

    SHR_IF_ERR_EXIT(dbal_fields_field_type_parent_field_decode
                    (unit, parent_field_type, orig_value_arr, sub_field_id, sub_field_val_arr));
    ...
    uint32 *sub_field_val)
{
    ...
    DBAL_FIELD_ID_VALIDATE(parent_field_id);

    GET_FIELD_TYPE(unit, parent_field_id, &parent_field_type);

    SHR_IF_ERR_EXIT(dbal_fields_field_type_uint32_sub_field_info_get
                    (unit, parent_field_type, orig_val, sub_field_id, sub_field_val));
    ...
    dbal_fields_e field_id)
{
    ...
    if ((field_id) < DBAL_NOF_FIELDS)
    {
        field_type = dbal_field_id_to_field_type(unit, field_id);
    }
    else
    {
    ...
    dbal_sub_field_info_t ** sub_field_info)
{
    ...
    GET_FIELD_TYPE(unit, parent_field_id, &parent_type);
    SHR_IF_ERR_EXIT(dbal_fields_field_type_subfield_info_get(unit, field_id, parent_type, sub_field_info));
...
shr_error_e
dbal_fields_field_type_parent_field_decode(
    int unit,
    dbal_field_types_defs_e parent_field_type,
    uint32 *orig_val,
    dbal_fields_e * sub_field_id,
    uint32 *sub_field_val)
{
    ...
    dbal_field_types_defs_e sub_field_type;
    ...
    if (field_types_basic_info[unit][parent_field_type]->nof_child_fields == 0)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "field %s has no child fields\n",
                     dbal_field_types_def_to_string(unit, parent_field_type));
    }

    for (iter = 0; iter < field_types_basic_info[unit][parent_field_type]->nof_child_fields; iter++)
    {
        dbal_fields_e curr_sub_field_id =
            field_types_basic_info[unit][parent_field_type]->sub_field_info[iter].sub_field_id;
        if (curr_sub_field_id != DBAL_FIELD_EMPTY)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_fields_field_type_field_decode(unit, curr_sub_field_id, parent_field_type,
                                                                field_types_basic_info[unit]
                                                                [parent_field_type]->max_size, orig_val,
                                                                decoded_value));
            SHR_IF_ERR_EXIT(dbal_fields_field_type_field_encode
                            (unit, curr_sub_field_id, parent_field_type,
                             field_types_basic_info[unit][parent_field_type]->max_size, decoded_value,
                             re_encoded_value));
            if (sal_memcmp
                (re_encoded_value, orig_val,
                 WORDS2BYTES(BITS2WORDS(field_types_basic_info[unit][parent_field_type]->max_size))) == 0)
            {
                int size_to_copy;
                GET_FIELD_TYPE(unit, curr_sub_field_id, &sub_field_type);
...
shr_error_e
dbal_fields_field_type_parent_field32_value_set(
    int unit,
    dbal_field_types_defs_e parent_field_type,
    dbal_fields_e sub_field_id,
    uint32 *sub_field_val,
    uint32 *parent_field_val)
{
    ...
    if (FIELD_TYPE_INFO(parent_field_type)->max_size > 32)
    {
        LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "API supported for fields lower than 32bit\n")));
    }
    sub_field_val_arr[0] = (*sub_field_val);
    SHR_IF_ERR_EXIT(dbal_fields_field_type_field_encode(unit, sub_field_id, parent_field_type,
                                                        FIELD_TYPE_INFO(parent_field_type)->max_size, sub_field_val_arr,
    ...
    uint32 *parent_field_val)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_parent_field32_value_set
                    (unit, parent_field_type, sub_field_id, sub_field_val, parent_field_val));
exit:
    SHR_FUNC_EXIT;
}

shr_error_e
dbal_fields_field_type_32_hw_value_get(
    int unit,
    dbal_field_types_defs_e field_type,
    uint32 logical_value,
    uint32 *hw_value)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    {
        case DBAL_VALUE_FIELD_ENCODE_ENUM:
            SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_hw_value_get(unit, field_type, logical_value, hw_value));
            break;
        ...
        case DBAL_VALUE_FIELD_ENCODE_NONE:
            ...
            *hw_value = logical_value;
            break;
        case DBAL_VALUE_FIELD_ENCODE_MULTIPLY:
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Encode type %s is not supported \n",
        ...
        {
            uint32 hw_val_arr[2];
            uint32 logical_value_arr[2] = { 0 };
            logical_value_arr[0] = logical_value;
            SHR_IF_ERR_EXIT(dbal_fields_encode_decode_bitwise_not(unit, logical_value_arr,
                                                                  field_types_basic_info[unit][field_type]->max_size,
                                                                  hw_val_arr));
            *hw_value = hw_val_arr[0];
        }
            break;
        case DBAL_VALUE_FIELD_ENCODE_PREFIX:
        case DBAL_VALUE_FIELD_ENCODE_SUFFIX:
        case DBAL_VALUE_FIELD_ENCODE_VALID_IND:
            ...
            break;
        default:
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Ilegal Encode type %d \n",
                         field_types_basic_info[unit][field_type]->encode_info.encode_mode);
            break;
    ...
    uint32 *hw_value)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Error, this API support only 32 bit fields, field %s is %d bits \n",
                     dbal_field_to_string(unit, (field_id)), field_types_basic_info[unit][field_type]->max_size);
    }

    SHR_IF_ERR_EXIT(dbal_fields_field_type_32_hw_value_get(unit, field_type, logical_value, hw_value));
...
shr_error_e
dbal_fields_field_type_enum_hw_value_get(
    int unit,
    dbal_field_types_defs_e field_type,
    uint32 enum_value,
    uint32 *hw_value)
{
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Illegal input value for enum field %s (%d). Input value: %d, max value: %d\n",
                     FIELD_TYPE_INFO(field_type)->name, field_type, enum_value,
                     FIELD_TYPE_INFO(field_type)->nof_enum_values);
    }
    ...
    uint32 *hw_value)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_hw_value_get(unit, field_type, enum_value, hw_value));
...
shr_error_e
dbal_fields_field_type_enum_value_get(
    int unit,
    dbal_field_types_defs_e field_type,
    uint32 hw_value,
    uint32 *enum_value)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_IF_ERR_EXIT_NO_MSG(dbal_field_types_decode_enum(unit, field_type, hw_value, enum_value));
    ...
    uint32 *enum_value)
{
    ...
    GET_FIELD_TYPE(unit, field_id, &field_type);
    SHR_IF_ERR_EXIT_NO_MSG(dbal_fields_field_type_enum_value_get(unit, field_type, hw_value, enum_value));
...
shr_error_e
dbal_fields_field_type_enum_next_enum_value_get(
    int unit,
    dbal_field_types_defs_e field_type,
    uint32 enum_value,
    uint32 *next_valid_enum_value)
{
    int iter;
    int start_index;
    ...
    (*next_valid_enum_value) = INVALID_ENUM;

    if ((enum_value != INVALID_ENUM) && (enum_value >= FIELD_TYPE_INFO(field_type)->nof_enum_values))
    {
    ...
    if (enum_value == INVALID_ENUM)
    {
        start_index = 0;
    }
    else
    {
        start_index = enum_value + 1;
    }

    for (iter = start_index; iter < FIELD_TYPE_INFO(field_type)->nof_enum_values; iter++)
    {
        if (FIELD_TYPE_INFO(field_type)->enum_val_info[iter].is_invalid)
        {
            continue;
        }
        else
        {
            (*next_valid_enum_value) = iter;
            break;
        }
    }

    if (iter == FIELD_TYPE_INFO(field_type)->nof_enum_values)
    {
        (*next_valid_enum_value) = INVALID_ENUM;
    }
...
shr_error_e
dbal_fields_enum_next_enum_value_get(
    int unit,
    dbal_fields_e field_id,
    uint32 enum_value,
    uint32 *next_valid_enum_value)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_field_type_enum_next_enum_value_get
                    (unit, field_type, enum_value, next_valid_enum_value));
...
shr_error_e
dbal_fields_name_update(
    int unit,
    dbal_fields_e field_id,
    char new_field_name[DBAL_MAX_STRING_LENGTH])
{
    uint8 success = FALSE;
    encap_string_type_t name;
    uint32 index = 0;
    ...
    sal_strncpy(name.string, new_field_name, DBAL_MAX_STRING_LENGTH - 2);

    SHR_IF_ERR_EXIT(dbal_db.field_names_override_locations.insert(unit, &field_id, &index, &success));
    if (!success)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "failed to update field name field %s\n", dbal_field_to_string(unit, field_id));
    }
    SHR_IF_ERR_EXIT(dbal_db.field_names_override_strings.set(unit, index, &name));
    ...
    dbal_fields_e field_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_db.field_names_override_locations.find(unit, &field_id, &index, &found));
    if (!found)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "failed to find field %s\n", dbal_field_to_string(unit, field_id));
    }

    SHR_IF_ERR_EXIT(dbal_db.field_names_override_locations.delete(unit, &field_id));

    sal_strncpy(name.string, "\0", DBAL_MAX_STRING_LENGTH);
    SHR_IF_ERR_EXIT(dbal_db.field_names_override_strings.set(unit, index, &name));
