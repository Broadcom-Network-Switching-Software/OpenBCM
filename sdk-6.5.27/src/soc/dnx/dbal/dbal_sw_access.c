...
{
    sw_state_htb_create_info_t hash_tbl_init_info;
    ...
    switch (table->table_type)
    {
        ...
        case DBAL_TABLE_TYPE_TCAM_DIRECT:
            ...
            hash_tbl_init_info.key_size = BITS2BYTES(table->key_size);
            hash_tbl_init_info.hash_func = SWSTATE_HTB_LARGE_HASH;
    ...
    int *resolved_res_type)
{
    ...
    if (entry_handle->table->core_mode == DBAL_CORE_MODE_DPC)
    {
        ...
        {
            entry_index |= (entry_handle->phy_entry.core_id << key_length);
        }
    }
    ...
    dbal_entry_handle_t * entry_handle)
{
                                                   ...
                                                   key_val));
    {
        DBAL_ITERATE_OVER_CORES(entry_handle, core_id)
        {
            ...
            {
                uint32 data_indx = 0;
                SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(DBAL_SW_STATE_TABLES.
                                                hash_table.find(unit, entry_handle->table_id, &key, &data_indx),
                                                _SHR_E_NOT_FOUND);
            ...
            SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(DBAL_SW_STATE_TABLES.
                                            hash_table.insert(unit, entry_handle->table_id, &key, &data_indx),
                                            _SHR_E_FULL);
    ...
    int *resolved_res_type)
{
    ...
    if (entry_handle->table->core_mode == DBAL_CORE_MODE_DPC)
    {
        ...
        if (entry_handle->phy_entry.core_id >= 0)
        {
            SHR_IF_ERR_EXIT(utilex_bitstream_set_field
                            (key_val, key_length, DBAL_CORE_SIZE_IN_BITS, entry_handle->phy_entry.core_id));
        }
    ...
    SHR_IF_ERR_EXIT_ELSE_SET_EXPECT(DBAL_SW_STATE_TABLES.
                                    hash_table.find(unit, entry_handle->table_id, &key, &data_indx), _SHR_E_NOT_FOUND);
    if (SHR_GET_CURRENT_ERR() == _SHR_E_NOT_FOUND)
    {
        SHR_EXIT();
    }
    ...
    dbal_sw_table_iterator_t * sw_iterator)
{
    ...
    SW_STATE_HTB_RH_ITER_SET_BEGIN(&sw_iterator->hash_entry_index);
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    do
    {
        ...
        if (entry_handle->table->core_mode == DBAL_CORE_MODE_DPC)
        {
            ...
            entry_handle->phy_entry.core_id = core_id_u32;
        }
    ...
    while (!SW_STATE_HTB_RH_ITER_IS_END(&iterator_info->sw_iterator.hash_entry_index));
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    switch (table_type)
    {
        ...
        case DBAL_SW_TABLE_HASH:
            SHR_IF_ERR_EXIT(DBAL_SW_STATE_TABLES.hash_table.delete_all(unit, entry_handle->table_id));
            break;
