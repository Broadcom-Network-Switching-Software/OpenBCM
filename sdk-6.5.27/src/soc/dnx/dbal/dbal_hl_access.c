    ...
    access_local_regmem_id_t * local_regmem_id)
{
    ...
    rv = access_regmem_global2local(ACCESS_RUNTIME_INFO(unit), global_regmem, local_regmem_id);
    if (rv != 0)
    {
    ...
    uint32 *regmem_size_bits)
{
    ...
    if (!is_new_access)
    {
        ...
        else
        {
            ...
            nof_fields = SOC_REG_INFO(unit, global_regmem).nFields;
            fields_array = SOC_REG_INFO(unit, global_regmem).fields;

            for (jj = 0; jj < nof_fields; jj++)
            {
                regmem_size_bits += fields_array[jj].len;
            }
        }
    }
#if defined(BCM_ACCESS_SUPPORT)
    ...
    int *num_of_blocks)
{
    ...
    else if ((entry_handle->table->core_mode != DBAL_CORE_MODE_SBC) & (core_id != DBAL_CORE_ALL))
    {
        uint8 nof_hw_entities = DBAL_MAX_NUM_OF_CORES;
        if (entry_handle->table->core_mode == DBAL_CORE_MODE_DPCG)
        {
            nof_hw_entities = DBAL_NOF_CORE_GROUPS;
        }

        (*num_of_blocks) = total_nof_blocks;
        if (nof_hw_entities > 1)
        {
            (*num_of_blocks) = (total_nof_blocks + 1) / nof_hw_entities;
        }
    ...
    uint32 *entry)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dbal_hl_access_memreg_size_get_per_access(unit, reg, TRUE, FALSE, &entry_size_in_bits));
    ...
    for (field_pos = 0; field_pos < DBAL_HL_MEM_MUL_RES_INFO.num_of_access_fields; field_pos++)
    {
        ...
        if (is_condition_pass)
        {
            ...
            if (is_non_standard)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_hl_access_block_calc(unit, entry_handle, entry_handle->phy_entry.core_id, memory,
                                                          &(curr_l2p_info->block_index_info), curr_l2p_info,
                                                          1, &block, &num_of_blocks));
#ifdef BCM_ACCESS_SUPPORT
                if (ACCESS_IS_INITIALIZED(unit))
                {
                    SHR_IF_ERR_EXIT(dnx_mem_sw_init_value_get(unit, memory, array_offset,
        ...
        if (is_condition_pass)
        {
            ...
            if (is_non_standard)
            {
                ...
                SHR_IF_ERR_EXIT(dbal_hl_access_block_calc
                                (unit, entry_handle, entry_handle->phy_entry.core_id, reg,
    ...
    int num_of_entries)
{
    ...
    for (core_iteration = start_iteration; core_iteration < end_iteration; core_iteration++)
    {
        ...
        if (SHR_IS_BITSET(core_bmp, core_iteration) == FALSE)
        {
            continue;
        }
    ...
    int result_type_get)
{
    ...
    for (iter = 0; iter < l2p_hl_info->num_of_access_fields; iter++)
    {
        ...
        if (is_condition_pass)
        {
            ...
            SHR_IF_ERR_EXIT(dbal_hl_access_block_calc
                            (unit, entry_handle, entry_handle->phy_entry.core_id, curr_reg,
                             &curr_l2p_info->block_index_info, curr_l2p_info, 0, &block, &nof_blocks));
    ...
    else if (table->table_type == DBAL_TABLE_TYPE_DIRECT)
    {
        while ((!entry_found) && (!iterator_info->is_end))
        {
            ...
            if (iterator_info->used_first_key)
            {
                ...
                {
                    if (iterator_info->attrib_info.core_group_rule_valid)
                    {
                        break;
                    }
    ...
    int *is_non_standard)
{
    ...
    SHR_IF_ERR_EXIT(dbal_hl_access_memreg_size_get_per_access(unit, reg, TRUE, FALSE, &mem_entry_size_bits));
    SHR_IF_ERR_EXIT(dbal_hl_new_access_reg_default_get(unit, reg, &default_value_pointer));
