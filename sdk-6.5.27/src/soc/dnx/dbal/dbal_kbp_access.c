...
#define DNX_KBP_HANDLES_GET(_table_, _kbp_handles_, _core_)                                                         \
    do                                                                                                              \
    {                                                                                                               \
        int _core = (_table_->core_mode == DBAL_CORE_MODE_SBC) ? 0 : _core_;                                        \
        if (_table_->kbp_handles)                                                                                   \
        {                                                                                                           \
            if (kbp_mngr_use_different_dbs_per_core(unit, _table_->core_mode) &&                                    \
                ((_core < 0) || (_core >= DNX_KBP_NOF_DBS_PER_CORE)))                                               \
            {                                                                                                       \
                SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid core ID %d provided for DPC table\n", _core);                   \
            }                                                                                                       \
            if (((kbp_db_handles_t *) _table_->kbp_handles)[_core].db_p == NULL)                                    \
            {                                                                                                       \
                SHR_ERR_EXIT(_SHR_E_CONFIG, "KBP Device is not Synced or the table is not in use (core %d)\n", _core);               \
            }                                                                                                       \
            _kbp_handles_ = &((kbp_db_handles_t *)_table_->kbp_handles)[_core];                                     \
        }                                                                                                           \
        else                                                                                                        \
        {                                                                                                           \
    ...
    char *action)
{
    ...
    int logger_action = 0;
    int core_id = (entry_handle->table->core_mode == DBAL_CORE_MODE_SBC) ? 0 : entry_handle->phy_entry.core_id;
    kbp_db_handles_t *db_handle = &((kbp_db_handles_t *) entry_handle->table->kbp_handles)[core_id];
    ...
    if (bsl_severity >= bslSeverityInfo)
    {
        if (dbal_logger_is_enable(unit, entry_handle->table_id))
        {
            ...
            LOG_CLI((BSL_META("Entry %s db_p %p ad_db_p (regular) %p ad_db_p (optimized) %p\n"),
                     action,
                     ((void *) db_handle->db_p),
                     ((void *) db_handle->ad_db_p[DNX_KBP_AD_DB_INDEX_REGULAR]),
                     ((void *) db_handle->ad_db_p[DNX_KBP_AD_DB_INDEX_OPTIMIZED])));
    ...
    uint8 payload_size)
{
    ...
    if ((payload_size == ((kbp_db_handles_t *) entry_handle->table->kbp_handles)[core_id].opt_result_size)
        || (payload_size == ((kbp_db_handles_t *) entry_handle->table->kbp_handles)[core_id].large_opt_result_size))
    {
        is_payload_opt = TRUE;
    }
    ...
    struct kbp_entry_info *entry_info)
{
    ...
    for (key_index = 0, range_id = 0; key_index < entry_handle->table->nof_key_fields; key_index++)
    {
        if (SHR_BITGET(entry_handle->table->keys_info[key_index].field_indication_bm, DBAL_FIELD_IND_IS_RANGED))
        {
            ...
            dbal_internal_key_field_ranges_alloc(unit, entry_handle);
    ...
    uint32 *payload_size)
{
    ...
    DNX_KBP_HANDLES_GET(entry_handle->table, kbp_handles, entry_handle->phy_entry.core_id);
    ...
    uint32 *payload_size)
{
    ...
    entry_handle->attr_flags |= (entry_info.active == 0) ? DBAL_ENTRY_ATTR_FLAG_CACHED : 0;

    if ((entry_info.active == 0) && DBAL_TABLE_IS_TCAM_BY_KEY(entry_handle->table))
    {
    ...
    if (entry_found)
    {
        ...
        else
        {
...

            {
                ...
                DBAL_ENTRY_HANDLE_KEY_FIELD_RANGES_COPY(*entry_handle, entry_handle_get);
                ...
                DBAL_ENTRY_HANDLE_KEY_FIELD_RANGES_COPY(*entry_handle, entry_handle_range_filled);
                for (key_index = 0; key_index < entry_handle->table->nof_key_fields; key_index++)
                {
            ...
            SHR_FREE(entry_handle_range_filled.key_field_ranges);
            SHR_FREE(entry_handle_get.key_field_ranges);
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (add_entry)
    {
        ...
        DNX_KBP_TRY(kbp_db_get_property(db_p, KBP_PROP_MAP_DB_TO_LPM, &is_acl_as_lpm));
        if (is_acl_as_lpm)
        {

            if (entry_handle->phy_entry.prefix_length != 0)
            {
                SHR_ERR_EXIT(_SHR_E_PARAM,
                             "KBP ACL databases configured as LPM only support entries with priority = 0; (priority = %d)\n",
                             entry_handle->phy_entry.prefix_length);
            }
        }
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    iter = (kbp_mngr_use_different_dbs_per_core(unit, entry_handle->table->core_mode)) ? DNX_KBP_NOF_DBS_PER_CORE : 1;
    while (iter--)
    {

        DNX_KBP_HANDLES_GET(entry_handle->table, kbp_handles, iter);
        db_p = kbp_handles->db_p;

        {
            ...
            if (DNX_KBP_USE_OPTIMIZED_RESULT)
            {
                ...
                {
                    DNX_KBP_TRY(kbp_ad_db_delete_all_entries
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    entry_handle->phy_entry.core_id =
        (kbp_mngr_use_different_dbs_per_core(unit, entry_handle->table->core_mode)) ? entry_handle->
        phy_entry.core_id : 0;
    ...
    dbal_entry_handle_t * entry_handle)
{
    ...
    if (kpb_entry == NULL)
    {
        ...
        if (kbp_mngr_use_different_dbs_per_core(unit, entry_handle->table->core_mode))
        {
    ...
    else
    {
        ...
        if (kpb_entry != NULL)
        {
            ...
            {
                int core_id_orig = entry_handle->phy_entry.core_id;
                kbp_db_handles_t *db_handles = &((kbp_db_handles_t *) entry_handle->table->kbp_handles)[core_id_orig];
                uint8 asso_data[DBAL_PHYSICAL_RES_SIZE_IN_BYTES] = { 0 };

                DNX_KBP_TRY(kbp_ad_db_get(entry_info.ad_db, entry_info.ad_handle, asso_data));

                sal_memset(phy_entry, 0x0, sizeof(*phy_entry));
                entry_handle->phy_entry.core_id = core_id_orig;
                ...
                if (entry_info.ad_db == db_handles->ad_db_p[DNX_KBP_AD_DB_INDEX_OPTIMIZED])
                {
                    payload_size = db_handles->opt_result_size;
                }

                else if (entry_info.ad_db == db_handles->ad_db_p[DNX_KBP_AD_DB_INDEX_LARGE_OPTIMIZED])
                {
                    payload_size = db_handles->large_opt_result_size;
                }

                else if (entry_info.ad_db == db_handles->ad_db_zero_size_p)
                {
                    payload_size = 0;
                    entry_handle->access_id_set = TRUE;
                }

                else if (entry_info.ad_db == db_handles->ad_db_p[DNX_KBP_AD_DB_INDEX_REGULAR])
                {
                    payload_size = db_handles->regular_result_size;
                }
