    ...
    uint32 *max_value)
{
    ...
    {
        if (i > max_value_internal)
        {
            max_value_internal = i;
        }
    ...
    int unit)
{
    ...
    if ((dnx_visibility_block_map[unit] =
         utilex_alloc(sizeof(dnx_visibility_block_map_t) * (block_max_index + 1))) == NULL)
    {
    ...
    while (i_bl != INVALID_ENUM)
    {
        ...
        sal_strncpy(dnx_visibility_block_map[unit][i_bl].name,
                    dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_PP_ASIC_BLOCK, i_bl), RHNAME_MAX_SIZE);
    ...
    if ((mem_name_per_block_array[unit] = utilex_alloc(sizeof(int *) * (block_max_index + 1))) == NULL)
    {
    ...
    if ((dnx_visibility_block_info[unit] =
         utilex_alloc(sizeof(dnx_visibility_block_info_t *) * (block_max_index + 1))) == NULL)
    {
    ...
    int line_index)
{
    ...
    {
        int block;
        int num_of_blocks, num_of_blocks_per_core;
        ...
        {
            num_of_blocks_per_core = (num_of_blocks + 1) / DBAL_MAX_NUM_OF_CORES;
        }
        else
        {
            num_of_blocks_per_core = num_of_blocks;
        }
        block = (core * num_of_blocks_per_core);
        block += SOC_MEM_BLOCK_MIN(unit, mem_name_per_block_array[unit][asic_block][mem_array_index]);
    ...
    int *enable_p)
{
    ...
    if (dnx_data_pp.stages.is_erpp_supported_get(unit) && dnx_data_debug.general.has_etpp_visibility_enabler_get(unit))
    {
