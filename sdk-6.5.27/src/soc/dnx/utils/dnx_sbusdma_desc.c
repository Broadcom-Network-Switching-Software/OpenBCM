...
STATIC uint32 dnx_sbusdma_desc_time_out_max[SOC_MAX_NUM_DEVICES] = { 0 };

STATIC sbusdma_desc_handle_t dnx_sbusdma_desc_handle[SOC_MAX_NUM_DEVICES];
STATIC sal_mutex_t dnx_sbusdma_desc_timeout_mutex[SOC_MAX_NUM_DEVICES];

STATIC sal_usecs_t dnx_sbusdma_desc_timeout_add_time[SOC_MAX_NUM_DEVICES];
STATIC shr_thread_manager_handler_t dnx_sbusdma_desc_timeout_tid[SOC_MAX_NUM_DEVICES];

STATIC uint32 dnx_sbusdma_desc_timeout_enabled[SOC_MAX_NUM_DEVICES];
    ...
    void *data)
{
...
#endif
    {
        ...
        SHR_ALLOC_ERR_EXIT(cfg,
                           sizeof(soc_sbusdma_desc_cfg_t) * dnx_sbusdma_desc_cfg_max[unit],
                           "soc_sbusdma_desc_cfg_t",
                           "%s%s%s", "Error: Fail to allocate memory for SBUSDMA desc_cfg for failure log print.\n",
                           EMPTY, EMPTY);
        {
    ...
    uint8 safe)
{
    ...
    if ((dnx_sbusdma_desc_timeout_enabled[unit] == TRUE) && (dnx_sbusdma_desc_timeout_mutex[unit] != NULL) && !safe)
    {
    ...
    if (dnx_sbusdma_desc_counter[unit] != 0)
    {
        ...
        rv = soc_sbusdma_desc_run(unit, (sbusdma_desc_handle_t) dnx_sbusdma_desc_handle[unit]);
        if (rv != SOC_E_NONE)
        {
            if (rv == SOC_E_BUSY)
            {
                ...
                while ((rv == SOC_E_BUSY) && (soc_timeout_check(&to) == 0));

                if (rv == SOC_E_BUSY)
                {
                    SHR_ERR_EXIT(SOC_E_TIMEOUT, "Timeout waiting for descriptor DMA to be available.");
                }
            }
            else
            {
                SHR_ERR_EXIT(rv, "Unexpected error attempting to run descriptor chain.");
            }
        }
    }
    ...
    void *entry_data)
{
    ...
    entries_to_shift = is_clear ? 1 : count;
         ...
         (SOC_REG_ABOVE_64_MAX_SIZE_U32 + 2) >= dnx_sbusdma_desc_mem_max[unit]))
    {
        ...
        SHR_IF_ERR_EXIT(shr_thread_manager_is_active_get(dnx_sbusdma_desc_timeout_tid[unit], &is_active));

        if ((dnx_sbusdma_desc_timeout_enabled[unit] == TRUE) && is_active && give_sem)
        {
            SHR_IF_ERR_EXIT(shr_thread_manager_trigger(dnx_sbusdma_desc_timeout_tid[unit]));
        }
    }
    ...
    if (is_mem)
    {
        cfg.flags |= SOC_SBUSDMA_MEMORY_CMD_MSG;
    }

    cfg.acc_type = acc_type;
    cfg.addr = addr;
    cfg.addr_shift = 0;
    cfg.blk = blk;
    ...
    void *entry_data)
{
    ...
    if ((dnx_sbusdma_desc_timeout_enabled[unit] == TRUE) && (dnx_sbusdma_desc_timeout_mutex[unit] != NULL))
    {
        sal_mutex_take(dnx_sbusdma_desc_timeout_mutex[unit], sal_mutex_FOREVER);
    }
    ...
    for (array_id = array_index_min; array_id < array_index_max; array_id++)
    {
        if (blk == MEM_BLOCK_ALL)
        {
            SOC_MEM_BLOCK_ITER(unit, mem, blk_i)
            {
                SHR_IF_ERR_EXIT(dnx_sbusdma_desc_add_mem_reg(unit, mem, INVALIDr, array_id, blk_i, offset, 1,
                                                             FALSE, entry_data));
                if (SOC_MEM_STATE(unit, mem) && SOC_MEM_STATE(unit, mem)->cache[blk_i])
                {
        ...
        else
        {
            SHR_IF_ERR_EXIT(dnx_sbusdma_desc_add_mem_reg(unit, mem, INVALIDr, array_id, blk, offset, 1,
                                                         FALSE, entry_data));
            if (SOC_MEM_STATE(unit, mem) && SOC_MEM_STATE(unit, mem)->cache[blk])
            {
    ...
    int unit)
{
    ...
    if (is_active)
    {
        SHR_IF_ERR_CONT(shr_thread_manager_destroy(&dnx_sbusdma_desc_timeout_tid[unit]));
    }
    ...
    dnx_sbusdma_desc_timeout_enabled[unit] = FALSE;
    ...
    uint32 timeout_usec)
{
    ...
    if (timeout_usec != 0)
    {
        ...
        if (!is_active)
        {
            ...
            thread_config_info.name = "Desc DMA Timeout";
            ...
            thread_config_info.callback = dnx_sbusdma_desc_init_timeout_thread;
            thread_config_info.user_data = NULL;
            thread_config_info.stop_timeout_usec = (dnx_sbusdma_desc_time_out_max[unit] * 100);
            rv = shr_thread_manager_create(unit, &thread_config_info, &(dnx_sbusdma_desc_timeout_tid[unit]));
        ...
        dnx_sbusdma_desc_timeout_enabled[unit] = TRUE;
    }
...

    {
        ...
        soc_sbusdma_desc_cfg_t *desc_cfg_arr_p = NULL;

        SHR_ALLOC(desc_cfg_arr_p, sizeof(soc_sbusdma_desc_cfg_t) * chain_length_max,
                  "soc_sbusdma_desc_cfg_t", "%s%s%s", EMPTY, EMPTY, EMPTY);
...
shr_error_e
dnx_sbusdma_desc_timeout_enable(
    int unit,
    uint32 enable)
{
    SHR_FUNC_INIT_VARS(unit);
    ...
    dnx_sbusdma_desc_timeout_enabled[unit] = (enable == TRUE) ? TRUE : FALSE;
    ...
    dnx_sbusdma_desc_stats_t * desc_stats)
{
    ...
    if (dnx_sbusdma_desc_enabled[unit])
    {
        ...
        desc_stats->timeout_enabled = dnx_sbusdma_desc_timeout_enabled[unit];
