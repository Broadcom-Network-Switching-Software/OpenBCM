    ...
    const char *function_name)
{
    ...
    if (dnx_err_recovery_is_top_level_trans_in_top_level_api(unit, dnx_er_db))
    {
        ...
        if (dnx_er_regress_testing_conditions_are_met(unit, dnx_er_db))
        {
            if (dnx_er_db->dnx_err_recovery_manager->trans_info.top_level_trans_in_api_count > 1)
            {
            ...
            SHR_IF_ERR_EXIT(dnx_er_regress_affirm_er_macro_usage_legality(unit, dnx_er_db, function_name, FALSE));
        }
    }
...
shr_error_e
dnx_er_regress_affirm_er_macro_usage_legality(
    int unit,
    dnx_er_db_t * dnx_er_db,
    const char *function_name,
    uint8 check_er_regress_conds)
{
    ...
    if (check_er_regress_conds && !dnx_er_regress_testing_conditions_are_met(unit, dnx_er_db))
    {
        SHR_EXIT();
    }

    if ((function_name != NULL)
        && (sal_strncmp(function_name, "bcm_", sal_strnlen("bcm_", DNX_ROLLBACK_JOURNAL_API_NAME_MAX_LENGTH - 1)) != 0))
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "ERROR: Bad usage of DNX_ERR_RECOVERY macros - usage only allowed in bcm_ and bcm_dnx_ APIs but was used in %s.\n",
                     function_name);
    }
