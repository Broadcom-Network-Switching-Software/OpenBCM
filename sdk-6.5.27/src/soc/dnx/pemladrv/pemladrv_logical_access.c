    ...
    PhysicalWriteInfo ** phy_wire)
{
    ...
    while (target_physical_memory_entry_data_list_curr_element && (chunk_phy_mem_row_index != 4294967295))
    {
    ...
    PhysicalWriteInfo ** phy_wire)
{
    ...
    for (chunk_index = 0; chunk_index < nof_chunk_matrix_cols; ++chunk_index)
    {
        ...
        for (implamentation_index = 0; implamentation_index < nof_implamentations; ++implamentation_index)
        {
            ...
            if (db_chunk_mapper_matrix[implamentation_index].db_chunk_mapper[chunk_matrix_row][chunk_index] == NULL)
            {
                continue;
            }
