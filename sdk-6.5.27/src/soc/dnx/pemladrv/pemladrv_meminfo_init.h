...
#define PEMLA_UCODE_LINE_MAX_WIDTH                             512
...
#define   PEM_NOF_DB_DIRECT_INFO_TOKEN  26
...
#define   PEM_NOF_REGISTER_INFO_TOKEN  19

#define   VIRTUAL_REGISTER_NOF_FIELDS_TOKEN   5

#define   VIRTUAL_REGISTER_FIELD_NOF_MAPPINGS_TOKEN  7

#define   PEM_NOF_DB_CAM_KEY_MAPPING_INFO_TOKEN  30

#define   PEM_NOF_DB_CAM_RESULT_MAPPING_INFO_TOKEN  24

#define   DB_SINGLE_CHUNCK_INFO_DIRECT_TOKEN 6

#define   DB_SINGLE_CHUNCK_INFO_MAP_TOKEN 8
...
void free_vw_container(
    const int unit);

void free_reg_container(
    const int unit);

void free_version_info(
    const int unit);

void free_logical_direct_info(
    int unit,
    LogicalDirectInfo * logical_direct_info);

void free_logical_tcam_info(
    int unit,
    LogicalTcamInfo * logical_tcam_info);

void free_logical_lpm_info(
    int unit,
    LogicalLpmInfo * logical_lpm_info);

void free_logical_em_info(
    int unit,
    LogicalEmInfo * logical_em_info);

void free_vw_mapping_info(
    VirtualWireInfo * vw_info_arr);

void free_logical_reg_info(
    int unit,
    LogicalRegInfo * logical_reg_info);

void free_reg_field_info(
    int unit,
    LogicalRegFieldInfo * reg_field_info);

void free_chunk_mapper_matrix(
...
shr_error_e dnx_pemladrv_mem_line_write(
    int unit,
    const char *line,
    int use_dma);

shr_error_e dnx_pemladrv_reg_line_write(
    int unit,
    const char *line,
    int use_dma);

shr_error_e dnx_pemladrv_db_info_insert(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_db_field_info_insert(
    const int unit,
    const char *line);

void db_key_field_info_insert(
    const int unit,
    const char *db_type,
    const int db_id,
    const int field_id,
...
void db_result_field_info_insert(
    const int unit,
    const char *db_type,
    const int db_id,
    const int field_id,
...
void is_field_mapped_update(
    const int num_of_fields,
    FieldBitRange * field_bit_range_arr,
    DbChunkMapper * const chunk_info_ptr);

shr_error_e dnx_pemladrv_direct_result_chunk_insert(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_tcam_key_chunk_insert(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_tcam_result_chunk_insert(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_em_key_chunk_insert(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_em_result_chunk_insert(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_lpm_key_chunk_insert(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_lpm_result_chunk_insert(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_register_insert(
    const int unit,
    const char *line);

shr_error_e build_direct_chunk_from_ucode(
    const int unit,
    int *core_id,
    const char *line,
...
shr_error_e build_cam_key_chunk_from_ucode(
    const int unit,
    const char *line,
    int *core_id,
    DbChunkMapper * chunk_info_ptr,
...
shr_error_e build_cam_result_chunk_from_ucode(
    const int unit,
    const char *line,
    int *core_id,
    DbChunkMapper * chunk_info_ptr,
...
void db_chunk_insert(
    LogicalDbChunkMapperMatrix * logical_db_mapper,
    DbChunkMapper * const chunk_info_ptr);

shr_error_e dnx_pemladrv_dnx_init_all_db_arr_by_size(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_init_logical_db_chunk_mapper(
    const int unit,
    const char *line);

shr_error_e init_logical_db_mapper_matrix(
    int unit,
...
shr_error_e realloc_logical_db_mapper_matrix(
    int unit,
    LogicalDbChunkMapperMatrix ** logical_db_mapper_matrix_ptr,
    const int nof_chunk_cols_to_add,
    const int nof_implementations);

shr_error_e init_logical_fields_location(
    int unit,
    FieldBitRange ** field_bit_range_arr,
    const int nof_fields);

shr_error_e dnx_pemladrv_init_reg_field_info(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_init_reg_field_mapper(
    const int unit,
    const char *line);

shr_error_e init_pem_version_control(
    const int unit,
    const char *line,
    int use_dma);

shr_error_e init_pem_applet_reg(
    const int unit,
    const char *line);
shr_error_e init_pem_applet_mem(
    const int unit,
    const char *line);
shr_error_e init_meminfo_array_for_applets(
    const int unit,
    const char *line);
shr_error_e insert_meminfo_to_array_for_applets(
    const int unit,
    const char *line);

shr_error_e init_em_cache(
    const int unit,
    EmDbCache * em_cache_info);

shr_error_e init_lpm_cache(
    const int unit,
    LpmDbCache * lpm_cache_info);

shr_error_e dnx_pemladrv_vw_mapping_insert(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_init_vw_info(
    const int unit,
    const char *line);

shr_error_e dnx_pemladrv_init_vw_arr_by_size(
    const int unit,
    const char *line);

shr_error_e init_dbs_content_and_program_selection_tcams_from_write_commands_file(
    const int unit,
    const int core_id,
    const char *write_commands_file_name);

shr_error_e hexstr2addr(
    int unit,
    char *str,
    unsigned int *block_id,
    unsigned int *addr);

shr_error_e parse_meminfo_definition_file(
    int unit,
