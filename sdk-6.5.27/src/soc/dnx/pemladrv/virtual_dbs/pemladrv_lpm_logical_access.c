    ...
    const int unit)
{
    ...
    for (int core_id = 0; core_id < PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_id)
    {
        ...
        {
            if (NULL ==
                api_info[unit].db_lpm_container.db_lpm_info_arr[index].logical_lpm_info.
            ...
            for (entry = 0; entry < PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES; ++entry)
            {
                ...
                SHR_IF_ERR_EXIT(dnx_build_physical_db_key_data_array(unit, sub_unit_idx,
                                                                     api_info[unit].
                                                                     db_lpm_container.db_lpm_info_arr[index].
                                                                     logical_lpm_info.db_tcam_per_core_arr
                ...
                else
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_get_prefix_length_from_physical_data
                                    (unit, total_key_width, tcam_data, &prefix_length));
...
shr_error_e
dnx_pemladrv_lpm_info_allocate(
    const int unit,
    ...
    int *size)
{
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(*arr, api_info[unit].db_lpm_container.nof_lpm_dbs * sizeof(pemladrv_db_info), "arr",
                                "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    ...
    for (lpm_index = 0; lpm_index < api_info[unit].db_lpm_container.nof_lpm_dbs; ++lpm_index)
    {
        ...
        if (api_info[unit].db_lpm_container.db_lpm_info_arr == NULL
            || api_info[unit].db_lpm_container.db_lpm_info_arr[lpm_index].
            logical_lpm_info.db_tcam_per_core_arr[core_id].result_chunk_mapper_matrix_arr == NULL)
        {
            continue;
        }
        ...
        name_len =
            sal_strnlen(api_info[unit].db_lpm_container.db_lpm_info_arr[lpm_index].
                        logical_lpm_info.db_tcam_per_core_arr[core_id].name, MAX_NAME_LENGTH);
        SHR_ALLOC_SET_ZERO_ERR_EXIT((*arr)[mapped_lpm_size].db_name, name_len + 1, "(*arr)[].db_name", "%s%s%s\r\n",
                                    EMPTY, EMPTY, EMPTY);
        sal_strncpy((*arr)[mapped_lpm_size].db_name,
                    api_info[unit].db_lpm_container.db_lpm_info_arr[lpm_index].
                    logical_lpm_info.db_tcam_per_core_arr[core_id].name, name_len);
        (*arr)[mapped_lpm_size].db_name[name_len] = '\0';
        nof_key_fields =
            api_info[unit].db_lpm_container.db_lpm_info_arr[lpm_index].logical_lpm_info.
        ...
        SHR_ALLOC_SET_ZERO_ERR_EXIT((*arr)[mapped_lpm_size].ptr,
                                    (*arr)[mapped_lpm_size].nof_fields * sizeof(pemladrv_db_field_info), "(*arr)[].ptr",
                                    "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

        for (field_index = 0; field_index < nof_fields; ++field_index)
        {
            lpm_field_key_info_arr =
                &lpm_info_arr->logical_lpm_info.db_tcam_per_core_arr[core_id].key_field_bit_range_arr[field_index];
            ...
            if (lpm_field_key_info_arr->is_field_mapped)
            {
                ...
                (*arr)[mapped_lpm_size].total_nof_entries =
                    api_info[unit].db_lpm_container.db_lpm_info_arr[lpm_index].logical_lpm_info.
                    db_tcam_per_core_arr->total_nof_entries;
                (*arr)[mapped_lpm_size].total_key_width =
                    api_info[unit].db_lpm_container.db_lpm_info_arr[lpm_index].logical_lpm_info.
                    db_tcam_per_core_arr->total_key_width;
            }

            lpm_field_result_info_arr =
                &lpm_info_arr->logical_lpm_info.db_tcam_per_core_arr[core_id].result_field_bit_range_arr[field_index];
            ...
            if (lpm_field_result_info_arr->is_field_mapped)
            {
                ...
                (*arr)[mapped_lpm_size].ptr[field_index].field_size_in_bits +=
                    lpm_field_result_info_arr->msb - lpm_field_result_info_arr->lsb + 1;
                (*arr)[mapped_lpm_size].total_result_width =
                    api_info[unit].db_lpm_container.db_lpm_info_arr[lpm_index].logical_lpm_info.
...
void
dnx_pemladrv_lpm_info_de_allocate(
    const int unit,
    ...
    int *index)
{
    ...
    if (key->nof_fields != nof_fields_in_key)
    {
        SHR_ERR_EXIT(_SHR_E_FULL,
                     "ERROR. One or More fields are missing while trying to write key. Exact-Match - #%d id", table_id);
    }
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_does_key_exists_in_physical_lpm_db(unit,
                                                                    &api_info[unit].
                                                                    db_lpm_container.db_lpm_info_arr[table_id].
                                                                    logical_lpm_info, uint_len_in_bits,
                                                                    &api_info[unit].
                                                                    db_lpm_container.db_lpm_info_arr[table_id].
                                                                    lpm_cache[core_id], virtual_key_data, index));
    if (*index != -1)
    {
    ...
    if (dnx_pemladrv_is_lpm_full(unit,
                                 api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].
                                 logical_lpm_info.db_tcam_per_core_arr[core_id].total_nof_entries,
                                 api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].
                                 lpm_cache[core_id].physical_entry_occupation))
    {
    ...
    *index = dnx_pemladrv_find_lpm_index_to_insert(unit,
                                                   ...
                                                   db_lpm_info_arr[table_id].lpm_cache[core_id]);

    if (api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].
        lpm_cache[core_id].physical_entry_occupation[*index] == 0)
    {
        SHR_IF_ERR_EXIT(dnx_pem_lpm_set_by_id(unit, core_id, table_id, *index, key, &length_in_bits, result));
    }
    else
    ...
    int *index)
{
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_find_physical_entry_index_by_key
                    (unit, core_id, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info,
                     api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].
                     lpm_cache[core_id].physical_entry_occupation,
                     api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache[core_id].lpm_key_entry_arr,
                     key, index));
    if (*index == -1)
    {
    ...
    int *index)
{
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_find_physical_lpm_entry_index_by_key
                    (unit, core_id, &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info,
                     &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache[core_id], key, index));
    if (*index == -1)
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Key doesn't exist in DB");
    }
    ...
    pemladrv_mem_t * result)
{
    ...
    *length_in_bits =
        api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].
        lpm_cache[core_id].lpm_key_entry_prefix_length_arr[index];
    dnx_pemladrv_get_lpm_data_by_entry(unit, core_id, table_id, index, key, result);
    ...
    pemladrv_mem_t * result)
{
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_build_lpm_mask
                    (unit, key_fields_arr, *length_in_bits, total_key_width_in_bits, key, &mask));
    ...
    SHR_IF_ERR_EXIT(dnx_logical_cam_based_db_write
                    (&api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].logical_lpm_info, unit, core_id, index,
                     key, mask, valid, result));
    dnx_pemladrv_update_lpm_cache(core_id, index, *length_in_bits, virtual_key_data, virtual_result_data,
                                  &api_info[unit].db_lpm_container.db_lpm_info_arr[table_id]);
    ...
    table_id_t table_id)
{
    ...
    {
        if (api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].
            lpm_cache[core_id].physical_entry_occupation[index] == 1)
        {
            SHR_IF_ERR_EXIT(pemladrv_lpm_remove_by_index(unit, core_id, table_id, index));
        }
    }
    ...
    int *index)
{
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(temp_key_data, (size_t) pemladrv_ceil(key_width, UINT_WIDTH_IN_BITS) * sizeof(uint32),
                                "temp_key_data", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    ...
    pemladrv_mem_t ** mask)
{
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(virtual_mask, num_of_uint_chunks_in_mask * sizeof(unsigned int), "virtual_mask",
                                "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
...
{
    unsigned int *key_data = lpm_cache->lpm_cache[core_id].lpm_key_entry_arr[row_index];
    unsigned int *result_data = lpm_cache->lpm_cache[core_id].lpm_result_entry_arr[row_index];

    int nof_key_parts =
        (size_t) pemladrv_ceil(lpm_cache->logical_lpm_info.db_tcam_per_core_arr[core_id].total_key_width,
                               UINT_WIDTH_IN_BITS);
    int nof_result_parts =
        (size_t) pemladrv_ceil(lpm_cache->logical_lpm_info.db_tcam_per_core_arr[core_id].total_result_width,
                               UINT_WIDTH_IN_BITS);

    for (int key_part = 0; key_part < nof_key_parts; ++key_part)
    {
        key_data[key_part] = virtual_key_data[key_part];
    }

    for (int result_part = 0; result_part < nof_result_parts; ++result_part)
    {
        result_data[result_part] = virtual_result_data[result_part];
    }
...
dnx_pemladrv_get_lpm_data_by_entry(
    ...
    pemladrv_mem_t * result)
{
    dnx_set_pem_mem_accesss_fldbuf_from_physical_sram_data_array(api_info[unit].db_lpm_container.
                                                                 db_lpm_info_arr[db_id].lpm_cache[core_id].
    ...
    LpmDbCache * lpm_cache)
{
    ...
    last_index_of_same_prefix_length =
        dnx_pemladrv_find_last_index_with_same_prefix(unit, index, lpm_cache->lpm_key_entry_prefix_length_arr);
    ...
    const int table_id)
{
    ...
    int distance_to_free_apace_above = 0;
    int distance_to_free_apace_below = 0;
    int last_index_of_same_prefix_length = dnx_pemladrv_find_last_index_with_same_prefix(unit,
                                                                                         index,
                                                                                         api_info
                                                                                         [unit].
                                                                                         db_lpm_container.db_lpm_info_arr
                                                                                         [table_id].lpm_cache
                                                                                         [core_id].lpm_key_entry_prefix_length_arr);

    for (i = index - 1; i >= 0; --i)
    {
        ++distance_to_free_apace_above;
        if (api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].lpm_cache[core_id].physical_entry_occupation[i] ==
            0)
            break;
    }
    ...
    const int table_id)
{
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(key->fields[0]->fldbuf, 2 * sizeof(unsigned int), "key->fields[0]->fldbuf",
                                "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    result->nof_fields = 1;
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(result->fields[0]->fldbuf, 2 * sizeof(unsigned int), "result->fields[0]->fldbuf",
                                "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

    length_in_bits =
        api_info[unit].db_lpm_container.db_lpm_info_arr[table_id].
        lpm_cache[core_id].lpm_key_entry_prefix_length_arr[old_index];

    dnx_set_pem_mem_accesss_fldbuf_from_physical_sram_data_array(api_info[unit].
                                                                 db_lpm_container.db_lpm_info_arr[table_id].
                                                                 lpm_cache[core_id].lpm_key_entry_arr[old_index],
                                                                 api_info[unit].
                                                                 db_lpm_container.db_lpm_info_arr[table_id].
                                                                 logical_lpm_info.db_tcam_per_core_arr[core_id].
                                                                 key_field_bit_range_arr, key);

    dnx_set_pem_mem_accesss_fldbuf_from_physical_sram_data_array(api_info[unit].
                                                                 db_lpm_container.db_lpm_info_arr[table_id].
                                                                 lpm_cache[core_id].lpm_result_entry_arr[old_index],
                                                                 api_info[unit].
                                                                 db_lpm_container.db_lpm_info_arr[table_id].
                                                                 logical_lpm_info.db_tcam_per_core_arr
                                                                 [core_id].result_field_bit_range_arr, result);
    ...
    const int table_id)
{
    ...
    if (shift_direction == UP)
    {
        *index = *index - 1;
        ...
        for (int core_index = 0; core_index < PEM_CFG_API_NOF_CORES_DYNAMIC; ++core_index)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_pemladrv_move_physical_lpm_entry
                                (unit, core_index, index_to_write_to + 1, index_to_write_to, table_id));
            }
        }
    }
    else if (shift_direction == DOWN)
    {
        ...
        next_free_entry = dnx_pemladrv_find_next_lpm_free_entry(unit,
                                                                core_id,
                                                                *index,
                                                                DOWN,
                                                                api_info[unit].
    ...
    pemladrv_mem_t ** result)
{
    ...
    if (api_info[unit].db_lpm_container.db_lpm_info_arr[db_id].logical_lpm_info.
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_pem_allocate_fldbuf_by_bit_range_array(unit, nof_fields_in_key,
                                                                        api_info[unit].
                                                                        db_lpm_container.db_lpm_info_arr[db_id].
                                                                        logical_lpm_info.db_tcam_per_core_arr
        ...
        for (i = 0; i < nof_dbs; ++i)
        {
            ...
            for (entry_index = 0; entry_index < PEM_CFG_API_CAM_TCAM_CHUNK_N_ENTRIES; ++entry_index)
                SHR_IF_ERR_EXIT(dnx_pemladrv_set_valid_bit_to_zero(unit, core_id, entry_index,
                                                                   &api_info[unit].db_lpm_container.
                                                                   db_lpm_info_arr[i].logical_lpm_info));
        }
    }
