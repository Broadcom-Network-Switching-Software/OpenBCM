...
{
    if ((virtual_key_data != NULL) && (virtual_result_data != NULL))
    {
        unsigned int *key_data = em_cache->em_cache[core_id].em_key_entry_arr[row_index];
        unsigned int *result_data = em_cache->em_cache[core_id].em_result_entry_arr[row_index];

        int nof_key_parts =
            (size_t) pemladrv_ceil(em_cache->logical_em_info.db_tcam_per_core_arr[core_id].total_key_width,
                                   UINT_WIDTH_IN_BITS);
        int nof_result_parts =
            (size_t) pemladrv_ceil(em_cache->logical_em_info.db_tcam_per_core_arr[core_id].total_result_width,
                                   UINT_WIDTH_IN_BITS);
