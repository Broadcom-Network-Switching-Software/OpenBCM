    ...
    char **target_name)
{
    ...
    {
        if (api_info[unit].reg_container.reg_info_arr[reg_id].
            reg_field_info_arr[i].reg_field_mapping_per_core_arr[core_id].nof_mappings > 0)
        {
            break;
        }
    }
    SHR_IF_ERR_EXIT(dnx_pemladrv_get_reg_or_reg_field_name
                    (unit,
                     api_info[unit].reg_container.reg_info_arr[reg_id].
                     reg_field_info_arr[i].reg_field_mapping_per_core_arr[core_id].reg_field_mapping_arr[0].
                     register_name, 1, target_name));
    ...
    char **target_name)
{
    ...
    for (i = 0; i < nof_fields; ++i)
    {
        if ((api_info[unit].reg_container.reg_info_arr[reg_id].
             reg_field_info_arr[i].reg_field_mapping_per_core_arr[core_id].nof_mappings > 0)
            && (api_info[unit].reg_container.reg_info_arr[reg_id].
                reg_field_info_arr[i].reg_field_mapping_per_core_arr[core_id].reg_field_mapping_arr[0].field_id ==
                field_id))
        {
            break;
        }
    ...
    const unsigned int *data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_pem_allocate_fldbuf_by_bit_range_array
                    (unit, nof_fields, api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr,
                     reg_p));
    dnx_set_pem_mem_accesss_fldbuf(0, data, api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr,
                                   reg_p);
    ...
    int *size)
{
    ...
    if (reg_id > (int) api_info[unit].reg_container.nof_registers
        || !api_info[unit].reg_container.reg_info_arr[reg_id].is_mapped)
        SHR_ERR_EXIT(_SHR_E_PARAM, "Invalid reg_id %d", reg_id);

    *size =
        api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr[field_id].msb -
        api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr[field_id].lsb + 1;
...
unsigned int
calculate_mapped_register_size(
    unsigned int unit,
    ...
    unsigned int field_index)
{
    ...
    nof_chunks =
        dnx_pemladrv_get_nof_unsigned_int_chunks(api_info[unit].reg_container.
                                                 reg_info_arr[reg_index].register_total_width);

    nof_mappings =
        api_info[unit].reg_container.reg_info_arr[reg_index].
        reg_field_info_arr[field_index].reg_field_mapping_per_core_arr[core_id].nof_mappings;
    for (mapping_index = 0; mapping_index < nof_mappings; mapping_index++)
    {
        lsb_bit =
            api_info[unit].reg_container.reg_info_arr[reg_index].
            reg_field_info_arr[field_index].reg_field_mapping_per_core_arr[core_id].
            reg_field_mapping_arr[mapping_index].virtual_field_lsb;
        msb_bit =
            api_info[unit].reg_container.reg_info_arr[reg_index].
            reg_field_info_arr[field_index].reg_field_mapping_per_core_arr[core_id].
            reg_field_mapping_arr[mapping_index].virtual_field_msb;
    ...
    int *size)
{
    ...
    SHR_ALLOC_SET_ZERO_ERR_EXIT(*arr, api_info[unit].reg_container.nof_registers * sizeof(pemladrv_reg_info), "arr",
                                "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);
    ...
    for (reg_index = 0; reg_index < api_info[unit].reg_container.nof_registers; ++reg_index)
    {
        ...
        SHR_ALLOC_SET_ZERO_ERR_EXIT((*arr)[mapped_regs_size].ptr,
                                    (*arr)[mapped_regs_size].nof_fields * sizeof(pemladrv_reg_field_info),
                                    "(*arr)[].ptr", "%s%s%s\r\n", EMPTY, EMPTY, EMPTY);

        mapped_fields_index = 0;
        for (field_index = 0; field_index < api_info[unit].reg_container.reg_info_arr[reg_index].nof_fields;
             ++field_index)
        {
            ...
            if (reg_field_info_arr->reg_field_mapping_per_core_arr[core_id].nof_mappings > 0)
            {
                ...
                {
                    SHR_IF_ERR_EXIT(dnx_pemladrv_get_reg_field_name
                                    (unit, reg_index, field_index,
                                     &(*arr)[mapped_regs_size].ptr[mapped_fields_index].field_name));
                }
                else
                {
                ...
                (*arr)[mapped_regs_size].ptr[mapped_fields_index].field_id =
                    reg_field_info_arr->reg_field_mapping_per_core_arr[core_id].reg_field_mapping_arr->field_id;
                (*arr)[mapped_regs_size].ptr[mapped_fields_index].field_size_in_bits =
                    calculate_mapped_register_size(unit, reg_index, field_index);
                (*arr)[mapped_regs_size].ptr[mapped_fields_index].nof_mappings =
                    reg_field_info_arr->reg_field_mapping_per_core_arr[core_id].nof_mappings;
                SHR_ALLOC_SET_ZERO_ERR_EXIT((*arr)[mapped_regs_size].ptr[mapped_fields_index].addresses,
                                            reg_field_info_arr->reg_field_mapping_per_core_arr[core_id].nof_mappings *
                                            sizeof(pemladrv_reg_mapping_info), "(*arr)[].ptr[].addresses", "%s%s%s\r\n",
                                            EMPTY, EMPTY, EMPTY);

                for (int mapping_index = 0;
                     mapping_index < reg_field_info_arr->reg_field_mapping_per_core_arr[core_id].nof_mappings;
                     ++mapping_index)
                {
                    (*arr)[mapped_regs_size].ptr[mapped_fields_index].addresses[mapping_index].core_address =
                        reg_field_info_arr->reg_field_mapping_per_core_arr[core_id].
                        reg_field_mapping_arr->pem_mem_address;
                    (*arr)[mapped_regs_size].ptr[mapped_fields_index].addresses[mapping_index].block_id =
                        reg_field_info_arr->reg_field_mapping_per_core_arr[core_id].
                        reg_field_mapping_arr->pem_mem_block_id;
                }
                mapped_fields_index++;
            }
        }
    ...
    PhysicalWriteInfo ** pw_info)
{
    ...
    for (mapping_ndx = 0; mapping_ndx < reg_field_info_pnt->reg_field_mapping_per_core_arr[core_id].nof_mappings;
         ++mapping_ndx)
    {
        RegFieldMapper *reg_field_mapper =
            &reg_field_info_pnt->reg_field_mapping_per_core_arr[core_id].reg_field_mapping_arr[mapping_ndx];
        ...
        SHR_IF_ERR_EXIT(dnx_modify_entry_data(unit, 0, 0,
                                              ...
                                              reg_field_mapper->pem_mem_offset, 0,
                                              virtual_mask,
                                              virtual_data,
    ...
    pemladrv_mem_t * result_read_bits_mask)
{
    ...
    SHR_IF_ERR_EXIT(dnx_build_virtual_db_mask_and_data_from_result_fields
                    (unit, register_total_width,
                     api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr, result, 0,
                     &virtual_reg_mask, &virtual_reg_data));

    for (field_ndx = 0; field_ndx < nof_fields; ++field_ndx)
    {
        LogicalRegFieldInfo *reg_field_info_ptr =
            &(api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_info_arr[field_ndx]);
        FieldBitRange *reg_field_bit_range =
            &(api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr[field_ndx]);

        if (api_info[unit].reg_container.reg_info_arr[reg_id].
            reg_field_info_arr[field_ndx].reg_field_mapping_per_core_arr[core_id].nof_mappings == 0)
        {
            continue;
        }
    ...
    pemladrv_mem_t * data)
{
    ...
    SHR_IF_ERR_EXIT(dnx_build_virtual_db_mask_and_data_from_result_fields
                    (unit, register_total_width,
                     api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr, data, 1,
                     &virtual_reg_mask, &virtual_reg_data));
    ...
    for (field_ndx = 0; field_ndx < nof_fields; ++field_ndx)
    {
        ...
        SHR_IF_ERR_EXIT
            (dnx_pemladrv_reg_field_run_over_all_chunks_read_physical_data_and_update_physical_memory_entry_data_list
             (unit, core_id, reg_field_info_ptr, reg_field_bit_range, virtual_reg_mask, virtual_reg_data,
              target_reg_physical_memory_entry_data_list, &target_reg_physical_memory_entry_data_list));
    }

    SHR_IF_ERR_EXIT(dnx_write_all_physical_data_from_list
                    (unit, sub_unit_idx, target_reg_physical_memory_entry_data_list));
    ...
    pemladrv_mem_t * result)
{
    ...
    SHR_IF_ERR_EXIT(dnx_pemladrv_allocate_pemladrv_mem_struct
                    (unit, &result_read_bits_mask, nof_fields,
                     api_info[unit].reg_container.reg_info_arr[reg_id].reg_field_bit_range_arr));

    SHR_IF_ERR_EXIT(dnx_pemladrv_reg_read_and_retreive_read_bits_mask
                    (unit, core_id, reg_id, result, result_read_bits_mask));

    SHR_IF_ERR_EXIT(dnx_pemladrv_allocate_pemladrv_mem_struct
                    (unit, &expected_result_read_bits_mask_when_all_mapped, nof_fields, reg_field_bit_range_arr));
