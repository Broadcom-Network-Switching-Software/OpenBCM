...
{
    int core_group_idx;
    dbal_core_mode_e table_core_mode = dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id)->core_mode;

    SHR_FUNC_INIT_VARS(unit);

    if ((core_id == BCM_CORE_ALL) || (table_core_mode != DBAL_CORE_MODE_DPC))
    {
        *is_valid = TRUE;
        SHR_EXIT();
    }
    ...
    {
        if (core_id == (dnx_data_adapter.reg_mem_access.swap_core_index_zero_with_core_index_get(unit) +
    ...
    dbal_physical_entry_t * entry)
{
    ...
    adapter_context_db.params.adapter_mem_reg_mutex.take(unit, sub_unit_id, sal_mutex_FOREVER);
    adapter_context_db.params.adapter_mem_reg_fd.get(unit, sub_unit_id, &adapter_mem_reg_fd);
    SHR_IF_ERR_EXIT(adapter_mdb_verify_valid_core(unit, dbal_physical_table_id, entry->core_id, &valid_core));
    if (valid_core == FALSE)
    {
        SHR_EXIT();
    }
    ...
    dbal_physical_entry_t * entry)
{
    ...
    if (dnx_data_mdb.pdbs.dbal_pdb_info_get(unit, dbal_physical_table_id)->db_subtype == MDB_EM_TYPE_GLEM)
    {
        ...
        if ((!dnx_data_lif.global_lif.three_intf_glem_get(unit)) &&
...
{
    int sub_unit_id = 0;
    int core_group_iter = 0;
    SHR_FUNC_INIT_VARS(unit);
    sub_unit_id = dnx_data_device.general.nof_core_groups_get(unit);

    for (core_group_iter = 0; core_group_iter < sub_unit_id; core_group_iter++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(adapter_mdb_outlif_profile_width_init(unit, core_group_iter));
        }
    }
