...
dcmn_get_ser_entry_from_cache(int unit,  soc_mem_t mem, int copyno, int array_index, int index, uint32 *data_entry)
{
    ...
    switch (mem) {
        ...
        default:
            ...
            rc = soc_mem_array_read_extended(unit, SOC_MEM_IGNORE_FORCE_READ_THROUGH, mem, array_index, copyno, index, data_entry);
    }
...
int dcmn_interrupt_handle_EDB_EsemErrorTableCoherency_Int(int unit, int block_instance, uint32 en_interrupt, char *msg)
{
    int rc;
    char special_msg[DCMN_INTERRUPT_SPECIAL_MSG_SIZE];
    uint32 regVal;
    ...
    rc = soc_reg32_get(unit, EDB_ESEM_ERROR_TABLE_COHERENCY_COUNTERr, block_instance, 0, &regVal);

    SOCDNX_IF_ERR_EXIT(rc);
    counter = soc_reg_field_get(unit, EDB_ESEM_ERROR_TABLE_COHERENCY_COUNTERr, regVal, ESEM_ERROR_TABLE_COHERENCY_COUNTERf);
    overflow = soc_reg_field_get(unit, EDB_ESEM_ERROR_TABLE_COHERENCY_COUNTERr, regVal, ESEM_ERROR_TABLE_COHERENCY_COUNTER_OVERFLOWf);

    sal_sprintf(special_msg, "ESEM_ERROR_TABLE_COHERENCY_COUNTER is %d, ESEM_ERROR_TABLE_COHERENCY_COUNTER_OVERFLOW is %d.", counter, overflow);
...
int dcmn_interrupt_handle_EDB_GlemErrorTableCoherency_Int(int unit, int block_instance, uint32 en_interrupt, char *msg)
{
    int rc;
    char special_msg[DCMN_INTERRUPT_SPECIAL_MSG_SIZE];
    uint32 regVal;
    ...
    rc = soc_reg32_get(unit, EDB_GLEM_ERROR_TABLE_COHERENCY_COUNTERr, block_instance, 0, &regVal);

    SOCDNX_IF_ERR_EXIT(rc);
    counter = soc_reg_field_get(unit, EDB_GLEM_ERROR_TABLE_COHERENCY_COUNTERr, regVal, GLEM_ERROR_TABLE_COHERENCY_COUNTERf);
    overflow = soc_reg_field_get(unit, EDB_GLEM_ERROR_TABLE_COHERENCY_COUNTERr, regVal, GLEM_ERROR_TABLE_COHERENCY_COUNTER_OVERFLOWf);

    sal_sprintf(special_msg, "GLEM_ERROR_TABLE_COHERENCY_COUNTER is %d, GLEM_ERROR_TABLE_COHERENCY_COUNTER_OVERFLOW is %d.", counter, overflow);
