...
STATIC
int dcmn_sim_bus_read(void* user_acc, uint32_t core_addr, uint32_t reg_addr, uint32_t* val){
...
STATIC
int dcmn_sim_bus_write(void* user_acc, uint32_t core_addr, uint32_t reg_addr, uint32_t val){
    dcmn_port_user_access_t *user_data;
    uint16_t tmp = 0;
    uint16_t msk = 0;
...
soc_dcmn_mdio_phy_access_get(int unit, dcmn_port_user_access_t *acc_data, int is_clause45, uint16 addr, phymod_access_t *access, int *is_sim)
{
    ...
    if(*is_sim) {
        PHYMOD_ACC_BUS(access) = &dcmn_sim_bus;
    } else if(!is_clause45){
    ...
    SOCDNX_IF_ERR_EXIT(soc_physim_check_sim(unit, phymodDispatchTypeFalcon, access, 0, 0, is_sim));
