 ...
 *  54140/94    4     4 Copper / Fiber        SGMII/QSGMII(54186)
...
#define PHY_IS_BCM54285(_pc) (PHY_MODEL_CHECK((_pc), \
                                  ...
                                  PHY_BCM54285_MODEL) \
                                  && (!((_pc)->phy_rev & 0x8)))
#define PHY_IS_BCM5428XX(_pc) (PHY_MODEL_CHECK((_pc), \
                                  PHY_BCM5428X_OUI,  \
                                  PHY_BCM5428X_MODEL))
...
#define PHY_IS_INTERFACE_QSGMII(pc)  \
                    ( PHY_IS_BCM54182(pc) || PHY_IS_BCM54192(pc) || \
                      PHY_IS_BCM54185(pc) || PHY_IS_BCM54195(pc) || \
                      PHY_IS_BCM54282(pc) || PHY_IS_BCM54292(pc) || \
                      PHY_IS_BCM54295(pc) || PHY_IS_BCM54296(pc) || \
                                             PHY_IS_BCM54285(pc)  )
...
#define PHY_BCM542XX_QSGMII_DEV_ADDR(_pc, addr) \
                (PHY_BCM542XX_FLAGS((_pc)) & PHY_BCM542XX_PHYADDR_REVERSE) \
                ? (PHY_BCM542XX_DEV_PHY_ID_BASE(_pc) + 1) \
                : (PHY_BCM542XX_DEV_PHY_ID_BASE(_pc) + addr)
...
#define SOC_IF_ERROR_SHUT(_rv, op)        if ( ((_rv) = (op)) < 0 ) {  goto SHUT; }
...
/* QSGMII TX FIR control setting defines */
#define PHY_BCM542XX_QSGMII_PRE_TAP_GET(v) ((v) & PHY_BCM542XX_QSGMII0_TX_CONTOL1_PRE_TAP_BIT_MASK)
#define PHY_BCM542XX_QSGMII_MAIN_TAP_GET(v) (((v) >> 5) & PHY_BCM542XX_QSGMII0_TX_CONTOL2_MAIN_TAP_BIT_MASK)
#define PHY_BCM542XX_QSGMII_POST_TAP_GET(v) (((v) >> 12) & PHY_BCM542XX_QSGMII1_TX_CONTOL0_MAIN_TAP_BIT_MASK)
#define PHY_BCM542XX_QSGMII_TX_AMP_GET(v) (((v) >> 18) & PHY_BCM542XX_QSGMII0_TX_AMP_CTRL_BIT_MASK)
#define PHY_BCM542XX_QSGMII_QUARTER_UNIT_GET(v) (((v) >> 22) & 0x1)

#define PHY_BCM542XX_QSGMII_PRE_TAP_SET(s, v) ( s |= v )
#define PHY_BCM542XX_QSGMII_MAIN_TAP_SET(s, v) (s |= (v << 5))
#define PHY_BCM542XX_QSGMII_POST_TAP_SET(s, v) (s |= (v << 12))
#define PHY_BCM542XX_QSGMII_TX_AMP_SET(s, v) (s |= (v << 18))
#define PHY_BCM542XX_QSGMII_QUARTER_UNIT_SET(s, v) (s |= (v << 22))


int phy_bcm542xx_reset(int unit, soc_port_t port, void *user_arg);
STATIC int phy_bcm542xx_link_up(int unit, soc_port_t port);
...
extern int phy_ecd_cable_diag_init_40nm_541xx(int unit, soc_port_t port);
...
STATIC int phy_bcm542xx_ability_remote_get(int unit, soc_port_t port, soc_port_ability_t *ability);
...
/* select 1000Base-X mapping for IEEE registers 0x00 to 0x0F *\
\*        and keep the original value of RDB Reg.0x21        */
#define PHY_IEEE_REG_1000X_SELECT(_u, _p, _r, _v)   do {                        \
        SOC_IF_ERROR_RETURN( PHY_BCM542XX_WRITE_PHY_REG(_u, _p,                 \
                                    PHY_BCM542XX_RDB_ADDR_REG_OFFSET,           \
                                    PHY_BCM542XX_MODE_CONTROL_REG_OFFSET   ) ); \
        SOC_IF_ERROR_RETURN( PHY_BCM542XX_READ_PHY_REG(_u, _p,                  \
                                    PHY_BCM542XX_RDB_DATA_REG_OFFSET, &(_v)) ); \
        if ( ! (_v & PHY_BCM542XX_MODE_CONTROL_REG_1000X_EN) ) {     _r = TRUE; \
            SOC_IF_ERROR_RETURN( PHY_BCM542XX_WRITE_PHY_REG(_u, _p,             \
                                    PHY_BCM542XX_RDB_DATA_REG_OFFSET, _v |      \
                      ...
                      uint8 reg_addr, uint16 *data)
{
    ...
    if ( flags & PHY_BCM542XX_REG_QSGMII ) {
        ...
        int  resume_ieee_reg_bank = FALSE;
        if ( reg_addr <= 0x0f ) {
            if ( PHY_IS_BCM54210_220(pc) && (flags & PHY_BCM542XX_REG_1000X) ) {
                ...
                SOC_IF_ERROR_RETURN
                    (PHY_BCM542XX_WRITE_PHY_REG(unit, pc, 0x1c, val));
#else
                PHY_IEEE_REG_1000X_SELECT(unit, pc, resume_ieee_reg_bank, val);
#endif
                /* Read 1000X IEEE register */
                SOC_IF_ERROR_RETURN
                    (PHY_BCM542XX_READ_PHY_REG(unit, pc, reg_addr, data));

                /* Restore IEEE Registers mapping (copper or 1000Base-X) */
#ifdef PHY_BCM542XX_DEFAULT_TO_LEGACY_ADDRESSING
                val = (blk_sel & 0xfffe) | 0x8000;
                SOC_IF_ERROR_RETURN
                    (PHY_BCM542XX_WRITE_PHY_REG(unit, pc, 0x1c, val));
#else
                if ( resume_ieee_reg_bank ) {    /* RDB Reg.0x21[0] been changed */
                    SOC_IF_ERROR_RETURN( PHY_BCM542XX_WRITE_PHY_REG(unit, pc,
                                            PHY_BCM542XX_RDB_DATA_REG_OFFSET, val) );
                }
#endif
            }
        } else {  /*  (reg_addr > 0xf)  */
...
int
phy_bcm542xx_mmd_reg_write(int unit, phy_ctrl_t *pc, int aer, uint16 regaddr, uint16 data)

{
    SOC_IF_ERROR_RETURN( PHY_BCM542XX_WRITE_PHY_REG(unit, pc, 0x1F, 0xFFD0 ) );
    SOC_IF_ERROR_RETURN( PHY_BCM542XX_WRITE_PHY_REG(unit, pc, 0x1E, aer    ) );
    SOC_IF_ERROR_RETURN( PHY_BCM542XX_WRITE_PHY_REG(unit, pc, 0x1F, regaddr) );
    SOC_IF_ERROR_RETURN( PHY_BCM542XX_WRITE_PHY_REG(unit, pc, 0x1E, data   ) );
    ...
    *medium = SOC_PORT_MEDIUM_COPPER;
    ...
    LOG_VERBOSE(BSL_LS_SOC_PHY,
                (BSL_META_U(unit, "_phy_bcm542xx_medium_check: "
                 ...
                 (*medium == SOC_PORT_MEDIUM_FIBER) ? TRUE : FALSE));
    return SOC_E_NONE;
}
...
#endif  /* INCLUDE_PLP_IMACSEC  */
    if(PHY_IS_BCM54180(pc)) {
        /* Enable SGMII auto-negotiation */
        SOC_IF_ERROR_RETURN(PHY_MODIFY_BCM542XX_MODE_CTRLr(unit, pc,
            PHY_MODE_CTRL_REG_1000X_EN, PHY_MODE_CTRL_REG_1000X_EN) );
        SOC_IF_ERROR_RETURN(PHY_MODIFY_BCM542XX_AN_MODE_CTRL_INFOr(
            unit, pc, 0, PHY_BCM542XX_XGMII_AN_MOD_CTRL_INFO));
        SOC_IF_ERROR_RETURN(PHY_MODIFY_BCM542XX_MODE_CTRLr(unit, pc,
            0, PHY_MODE_CTRL_REG_1000X_EN) );
    }
                   ...
                   int ext_phy_autodetect_en, int ext_phy_fiber_iface)
{
    ...
    if(PHY_IS_BCM54140(pc) || PHY_IS_BCM54194(pc) ||
        ...
        if(PHY_IS_BCM54140(pc) || PHY_IS_BCM54194(pc)) {
            /* Restore Copper/Fiber Register Space */
            data = 0x5040;
            SOC_IF_ERROR_RETURN
                (PHY_WRITE_BCM542XX_EXT_SERDES_CTRLr(unit, pc, data));
            data = 0x5000;
                         ...
                         int ext_phy_fiber_iface)
{
    ...
    if ( PHY_BCM54240_28X_REV_C1(model, rev) ) {
        ...
        phy_bcm542xx_direct_reg_write(unit, pc,
                                      PHY_BCM542XX_AUX_CTRL_REG_OFFSET,
                                      0x0400);
    } else if (PHY_IS_BCM5418x_19x(pc)) {
...
phy_bcm542xx_init(int unit, soc_port_t port)
{
    ...
    if ( ! (SOC_WARM_BOOT(unit) || SOC_IS_RELOADING(unit)) ) {
        ...
        if(PHY_IS_BCM5429X(pc) || PHY_IS_BCM5428x(pc)) {
...
phy_bcm542xx_duplex_get(int unit, soc_port_t port, int *duplex)
{
    ...
    if ( PHY_COPPER_MODE(unit, port) ) {
        ...
        } else if (pc->ext_phy_fiber_iface == EXT_PHY_FIBER_IFACE_SGMII) {
            SOC_IF_ERROR_RETURN(
                PHY_READ_BCM542XX_SGMII_SLAVEr(unit, pc, &sgmii_slave));
            if (sgmii_slave & PHY_BCM542XX_SGMII_DUPLEX) {
                *duplex = TRUE;
            } else {
                *duplex = FALSE;
            }
...
phy_bcm542xx_autoneg_set(int unit, soc_port_t port, int autoneg)
{
    ...
    if ( PHY_COPPER_MODE(unit, port) ) {
        ...
        if ( ! PHY_IS_BCM54210_220(pc) && ! PHY_IS_BCM5418x_19x(pc) ) {
...
phy_bcm542xx_timesync_enhanced_capture_get(int unit, soc_port_t port,
                              soc_port_phy_timesync_enhanced_capture_t *fifo) {
    ...
    /* select which Port to read FIFO register from  (RDB.0xA8E) */
    SOC_IF_ERROR_SHUT( rv,
            phy_bcm542xx_direct_reg_modify(unit, pc,
                                PHY_BCM542XX_TOP_1588_CNTR_DBG_REG_OFFSET,
                                (dev_port << CNTR_DBG_TS_SLICE_SEL_SHIFT),
                                PHY_BCM542XX_TOP_1588_CNTR_DBG_TS_SLICE_SEL) );

    /* start FIFO read to retrieve the next entry  (RDB.0xA85) ****************/
    SOC_IF_ERROR_SHUT( rv,
            phy_bcm542xx_direct_reg_write(unit, pc,
                                PHY_BCM542XX_TOP_1588_TS_RD_START_END_REG_OFFSET,
                                PHY_BCM542XX_TOP_1588_TS_RD_START(dev_port)) );

    /* DOMAIN_NUM, MSG_TYPE, DIRECTION  (RDB.0xA8C) */
    SOC_IF_ERROR_SHUT( rv,
            phy_bcm542xx_direct_reg_read(unit, pc,
                                PHY_BCM542XX_TOP_1588_TS_INFO_1_REG_OFFSET, &value0) );
    fifo->domain    = (value0 & PHY_BCM542XX_TOP_1588_TS_INFO_DOMAIN_NUM)
                             >> PHY_BCM542XX_TOP_1588_TS_INFO_DOMAIN_NUM_SHIFT;
    fifo->protocol  = (value0 & PHY_BCM542XX_TOP_1588_TS_INFO_PROTOCOL)
                             >> PHY_BCM542XX_TOP_1588_TS_INFO_PROTOCOL_SHIFT;
    fifo->msg_type  = (value0 & PHY_BCM542XX_TOP_1588_TS_INFO_MSG_TYPE)
                             >> PHY_BCM542XX_TOP_1588_TS_INFO_MSG_TYPE_SHIFT;
    fifo->direction = (value0 & PHY_BCM542XX_TOP_1588_TS_INFO_DIR)
    ...
    /* TIMESTAMP  (RDB.0x0AC4/A8B/A8A/A89) */
    SOC_IF_ERROR_SHUT( rv,
            phy_bcm542xx_direct_reg_read(unit, pc,
                    PHY_BCM542XX_TOP_1588_TIMESTAMP_0_REG_OFFSET, &value0) );
    SOC_IF_ERROR_SHUT( rv,
            phy_bcm542xx_direct_reg_read(unit, pc,
                    PHY_BCM542XX_TOP_1588_TIMESTAMP_1_REG_OFFSET, &value1) );
    SOC_IF_ERROR_SHUT( rv,
            phy_bcm542xx_direct_reg_read(unit, pc,
                    PHY_BCM542XX_TOP_1588_TIMESTAMP_2_REG_OFFSET, &value2) );
    SOC_IF_ERROR_SHUT( rv,
            phy_bcm542xx_direct_reg_read(unit, pc,
                    PHY_BCM542XX_TOP_1588_TIMESTAMP_3_REG_OFFSET, &value3) );
    COMPILER_64_SET( (fifo->timestamp),
                     ((value3 << 16) | value2),  ((value1 << 16) | value0) );

    /* SRC_PORT_CLOCK_ID[63:0] / MAC_DA[47:0]  (RDB.0x0AFC/AFB/AFA/AF9)*/
    SOC_IF_ERROR_SHUT( rv,
            phy_bcm542xx_direct_reg_read(unit, pc,
    ...
    COMPILER_64_SET( (fifo->source_port_identity),
                     ((value3 << 16) | value2),  ((value1 << 16) | value0) );
    ...
    fifo->source_port = value0;

    /* VLAN ID  (RDB.0xAFE) */
    SOC_IF_ERROR_SHUT( rv,
        phy_bcm542xx_direct_reg_read(unit, pc,
                    PHY_BCM542XX_TOP_1588_TS_INFO_8_REG_OFFSET  , &value0) );
    fifo->vlan_id  = value0 & PHY_BCM542XX_TOP_1588_TS_INFO_VLAN_ID;

 SHUT:
    /* end FIFO read (RDB.0xA85) to stop reading this FIFO entry **************/
    rv  =   phy_bcm542xx_direct_reg_write(unit, pc,
                                PHY_BCM542XX_TOP_1588_TS_RD_START_END_REG_OFFSET,
                                PHY_BCM542XX_TOP_1588_TS_RD_END(dev_port)  );
                              ...
                              soc_port_cable_diag_t *status)
{
    ...
    if (PHY_IS_BCM5418x_19x(pc)) {
        SOC_IF_ERROR_RETURN(
            phy_ecd_cable_diag_init_40nm_541xx(unit, port));
    } else {
        SOC_IF_ERROR_RETURN(
...
phy_bcm542xx_power_mode_set(int unit, phy_ctrl_t *pc, int mode)
{
    ...
    if (mode == SOC_PHY_CONTROL_POWER_LOW) {
        ...
        /* Select mode: SGMII->Copper MODE_CONTROL[0x21] MODE_SEL BIT[2:1] set to 0x00 to set copper mode */
        phy_bcm542xx_direct_reg_modify(unit, pc,
                           PHY_BCM542XX_MODE_CONTROL_REG_OFFSET,
                           0x00,
                           PHY_BCM542XX_MODE_CONTROL_REG_MODE_SEL);

        /* Set standby Power down mode */
        SOC_IF_ERROR_RETURN(
        PHY_MODIFY_BCM542XX_MII_CTRLr(unit,pc,
                                      PHY_BCM542XX_MII_CTRL_PWR_DOWN,
                                      PHY_BCM542XX_MII_CTRL_PWR_DOWN));
        ...
        /* Normal operation mode - disable standby power down */
        SOC_IF_ERROR_RETURN(
        PHY_MODIFY_BCM542XX_MII_CTRLr(unit,pc,
                                      0x00,
                                      PHY_BCM542XX_MII_CTRL_PWR_DOWN));
...
/*
 * Function:
 *    phy_bcm542xx_preemphasis_set
 * Purpose:
 *    Implements preemphasis 'setting' controls for chip 5418X (QSGMII interface) family chips
 *
 * Parameters:
 *    port     - PHY's port number or device address
 *    value    - Value to be set for preemphasis in following value format
 *
 *   The value format:
 * bit 22   :  1  Quarter unit enable (Finer preemphasis steps in constant
 *             impedance mode), 0 Quarter unit disable
 * bit 21:18:  TX_AMP_CTRL (AMS TX Amplitude control)
 * bit 17:12:  post_tap value
 * bit 11:05:  main tap value (FIR main tap max code 112)
 * bit 04:00:  pre_tap value
 *
 * Returns:
 */
int
phy_bcm542xx_preemphasis_set(int unit, soc_port_t port, uint32 value)
{
    int          rv = SOC_E_NONE;
    uint16       temp16 = 0;
    int          dev_port = 0;
    phy_ctrl_t  *pc = NULL;
    uint16 tx_pre = 0, tx_main = 0, tx_post = 0, tx_amp = 0;
    uint8 quarter_unit_en=0;
    pc = EXT_PHY_SW_STATE(unit, port);
    dev_port = PHY_BCM542XX_DEV_PHY_SLICE(pc);  /* Dev port numbers from 0-7 */
    if(!PHY_IS_INTERFACE_QSGMII(pc)) {
        return SOC_E_UNAVAIL;
    }
    if (PHY_IS_BCM54185(pc) && (0x8 & _phy_bcm542xx_hidden_rev_num_get(unit, pc))) {
       pc->phy_id = PHY_BCM542XX_QSGMII_DEV_ADDR(pc, 4);    /* 54186(54140 QSGMII) has 4 ports */
    } else {
       pc->phy_id = PHY_BCM542XX_QSGMII_DEV_ADDR(pc, 8);    /* QSGMII PHY addr for 8 ports*/
    }
    if(dev_port <=3) { /* QSGMII0 TX control register */
        /* Set AER to 0 */
        SOC_IF_ERROR_RETURN(phy_bcm542xx_mmd_reg_write(pc->unit, pc, 0, PHY_BCM542XX_QSGMII_AER_BAR, 0x0));
        /* Write cl45 registers */
        tx_pre = PHY_BCM542XX_QSGMII_PRE_TAP_GET(value);
        tx_main = PHY_BCM542XX_QSGMII_MAIN_TAP_GET(value);
        tx_post = PHY_BCM542XX_QSGMII_POST_TAP_GET(value);
        tx_amp = PHY_BCM542XX_QSGMII_TX_AMP_GET(value);
        SOC_IF_ERROR_RETURN(phy_bcm542xx_cl45_reg_read(unit, pc, 0, PHY_BCM542XX_QSGMII0_AMS_TX_CONTROL_REG_ADDR, &temp16));
        temp16 = tx_amp ? (tx_amp & PHY_BCM542XX_QSGMII0_TX_AMP_CTRL_BIT_MASK) : temp16;
        SOC_IF_ERROR_RETURN(phy_bcm542xx_cl45_reg_write(unit, pc, 0, PHY_BCM542XX_QSGMII0_AMS_TX_CONTROL_REG_ADDR, temp16));
        SOC_IF_ERROR_RETURN(phy_bcm542xx_cl45_reg_read(unit, pc, 0, PHY_BCM542XX_QSGMII0_TX_FIR_CONTROL1_REG_ADDR, &temp16));
        temp16 &= ~(PHY_BCM542XX_QSGMII0_TX_CONTOL1_PRE_TAP_BIT_MASK | (PHY_BCM542XX_QSGMII0_TX_CONTOL1_POST_TAP_BIT_MASK<<PHY_BCM542XX_QSGMII0_TX_CONTOL1_POST_TAP_BIT_POS));
        temp16 |= ((tx_pre & PHY_BCM542XX_QSGMII0_TX_CONTOL1_PRE_TAP_BIT_MASK) | ((tx_post  & PHY_BCM542XX_QSGMII0_TX_CONTOL1_POST_TAP_BIT_MASK)<<PHY_BCM542XX_QSGMII0_TX_CONTOL1_POST_TAP_BIT_POS));
        SOC_IF_ERROR_RETURN(phy_bcm542xx_cl45_reg_write(unit, pc, 0, PHY_BCM542XX_QSGMII0_TX_FIR_CONTROL1_REG_ADDR, temp16));
        SOC_IF_ERROR_RETURN(phy_bcm542xx_cl45_reg_read(unit, pc, 0, PHY_BCM542XX_QSGMII0_TX_FIR_CONTROL2_REG_ADDR, &temp16));
        temp16 &= ~(PHY_BCM542XX_QSGMII0_TX_CONTOL2_MAIN_TAP_BIT_MASK);
        temp16 |= (tx_main & PHY_BCM542XX_QSGMII0_TX_CONTOL2_MAIN_TAP_BIT_MASK);
        SOC_IF_ERROR_RETURN(phy_bcm542xx_cl45_reg_write(unit, pc, 0, PHY_BCM542XX_QSGMII0_TX_FIR_CONTROL2_REG_ADDR, temp16));
    } else {  /* QSGMII1 TX control settings */
        tx_main = PHY_BCM542XX_QSGMII_MAIN_TAP_GET(value);
        tx_post = PHY_BCM542XX_QSGMII_POST_TAP_GET(value);
        quarter_unit_en = PHY_BCM542XX_QSGMII_QUARTER_UNIT_GET(value);
        /* Set QSGMII1 TX control register for  */
        dev_port = 4;
        SOC_IF_ERROR_RETURN(phy_bcm542xx_qsgmii_reg_read(unit, pc, dev_port, PHY_BCM542XX_QSGMII1_TX_CONTROL_BAR,
                                                         PHY_BCM542XX_QSGMII1_TX_CONTROL0_REG_ADDR, &temp16));
        temp16 &= ~((PHY_BCM542XX_QSGMII1_TX_CONTOL0_MAIN_TAP_BIT_MASK<<PHY_BCM542XX_QSGMII1_TX_CONTROL0_MAIN_CONTROL_BIT_POS) | (1<<PHY_BCM542XX_QSGMII1_TX_CONTROL0_QUARTER_EN_BIT_POS));
        temp16 |= (((tx_main & PHY_BCM542XX_QSGMII1_TX_CONTOL0_MAIN_TAP_BIT_MASK) << PHY_BCM542XX_QSGMII1_TX_CONTROL0_MAIN_CONTROL_BIT_POS) | (quarter_unit_en << PHY_BCM542XX_QSGMII1_TX_CONTROL0_QUARTER_EN_BIT_POS));
        SOC_IF_ERROR_RETURN( phy_bcm542xx_qsgmii_reg_write(unit, pc,
                                        dev_port,
                                        PHY_BCM542XX_QSGMII1_TX_CONTROL_BAR,
                                        PHY_BCM542XX_QSGMII1_TX_CONTROL0_REG_ADDR,
                                        temp16));
        SOC_IF_ERROR_RETURN(phy_bcm542xx_qsgmii_reg_read(unit, pc, dev_port, PHY_BCM542XX_QSGMII1_TX_CONTROL_BAR,
                                                         PHY_BCM542XX_QSGMII1_TX_CONTROL1_REG_ADDR, &temp16));
        temp16 &= ~((PHY_BCM542XX_QSGMII1_TX_CONTOL1_POST_TAP_BIT_MASK <<PHY_BCM542XX_QSGMII1_TX_CONTROL1_POST_CONTROL_BIT_POS) | (1<<PHY_BCM542XX_QSGMII1_TX_CONTROL1_AMP_MODE_BIT_POS));
        temp16 |= (((tx_post & PHY_BCM542XX_QSGMII1_TX_CONTOL1_POST_TAP_BIT_MASK)<<PHY_BCM542XX_QSGMII1_TX_CONTROL1_POST_CONTROL_BIT_POS) | (1<<PHY_BCM542XX_QSGMII1_TX_CONTROL1_POST_ENABLE_BIT_POS) | (1<<PHY_BCM542XX_QSGMII1_TX_CONTROL1_CONST_IMPEDANCE_BIT_POS));
        SOC_IF_ERROR_RETURN( phy_bcm542xx_qsgmii_reg_write(unit, pc,
                                        dev_port,
                                        PHY_BCM542XX_QSGMII1_TX_CONTROL_BAR,
                                        PHY_BCM542XX_QSGMII1_TX_CONTROL1_REG_ADDR,
                                        temp16));
...
/*
 * Function:
 *    phy_bcm542xx_preemphasis_get
 * Purpose:
 *    Implements preemphasis controls get parameter for 5418X (QSGMII interface) family chip
 *
 * Parameters:
 *    port     - PHY's port number or device address
 *    value     - Get the value for preemphasis in following value format
 *   The value format:
 * bit 22   :  1  Quarter unit enable, 0 Quarter unit disable
 * bit 21:18:  TX_AMP_CTRL (AMS TX Amplitude control)
 * bit 17:12:  post_tap value
 * bit 11:05:  main tap value (FIR main tap max code 112)
 * bit 04:00:  pre_tap value
 ...
 */
int
phy_bcm542xx_preemphasis_get(int unit, soc_port_t port, uint32 *value)
{
    int          rv = SOC_E_NONE;
    uint16       temp16 = 0;
    int          dev_port;
    ...
    *value = 0;
    ...
    if(dev_port <=3) { /* QSGMII0 TX control register */
        ...
        tx_amp = (temp16 & PHY_BCM542XX_QSGMII0_TX_AMP_CTRL_BIT_MASK);
        PHY_BCM542XX_QSGMII_TX_AMP_SET(*value, tx_amp);
        SOC_IF_ERROR_RETURN(phy_bcm542xx_cl45_reg_read(unit, pc, 0, PHY_BCM542XX_QSGMII0_TX_FIR_CONTROL1_REG_ADDR, &temp16));
        tx_pre = (temp16 & PHY_BCM542XX_QSGMII0_TX_CONTOL1_PRE_TAP_BIT_MASK);
        tx_post = ((temp16 >> PHY_BCM542XX_QSGMII0_TX_CONTOL1_POST_TAP_BIT_POS) & PHY_BCM542XX_QSGMII0_TX_CONTOL1_POST_TAP_BIT_MASK);
        PHY_BCM542XX_QSGMII_PRE_TAP_SET(*value,tx_pre);
        PHY_BCM542XX_QSGMII_POST_TAP_SET(*value,tx_post);
        SOC_IF_ERROR_RETURN(phy_bcm542xx_cl45_reg_read(unit, pc, 0, PHY_BCM542XX_QSGMII0_TX_FIR_CONTROL2_REG_ADDR, &temp16));
        tx_main = (temp16 & PHY_BCM542XX_QSGMII0_TX_CONTOL2_MAIN_TAP_BIT_MASK);
        PHY_BCM542XX_QSGMII_MAIN_TAP_SET(*value,tx_main);
    } else {  /* QSGMII1 TX control settings */
        /* Set QSGMII1 TX control register for  */
        dev_port = 4;
        SOC_IF_ERROR_RETURN(phy_bcm542xx_qsgmii_reg_read(unit, pc, dev_port, PHY_BCM542XX_QSGMII1_TX_CONTROL_BAR,
                                                         PHY_BCM542XX_QSGMII1_TX_CONTROL0_REG_ADDR, &temp16));
        tx_main = ((temp16 >> PHY_BCM542XX_QSGMII1_TX_CONTROL0_MAIN_CONTROL_BIT_POS) & PHY_BCM542XX_QSGMII1_TX_CONTOL0_MAIN_TAP_BIT_MASK);

        quarter_unit_en = ((temp16 >> PHY_BCM542XX_QSGMII1_TX_CONTROL0_QUARTER_EN_BIT_POS) & 0x1);
        PHY_BCM542XX_QSGMII_MAIN_TAP_SET(*value,tx_main);
        PHY_BCM542XX_QSGMII_QUARTER_UNIT_SET(*value,quarter_unit_en);
        SOC_IF_ERROR_RETURN(phy_bcm542xx_qsgmii_reg_read(unit, pc, dev_port, PHY_BCM542XX_QSGMII1_TX_CONTROL_BAR,
                                                         PHY_BCM542XX_QSGMII1_TX_CONTROL1_REG_ADDR, &temp16));
        tx_post = ((temp16 >> PHY_BCM542XX_QSGMII1_TX_CONTROL1_POST_CONTROL_BIT_POS) & PHY_BCM542XX_QSGMII1_TX_CONTOL1_POST_TAP_BIT_MASK);
        PHY_BCM542XX_QSGMII_POST_TAP_SET(*value,tx_post);
    }
    pc->phy_id = PHY_BCM542XX_DEV_PHY_ID_ORIG(pc);     /* resume PHY address */
    return rv;
                         ...
                         soc_phy_control_t type, uint32 value)
{
    ...
    case SOC_PHY_CONTROL_PREEMPHASIS:
        SOC_IF_ERROR_RETURN
            (phy_bcm542xx_preemphasis_set(unit, port,value));
         break;
                         ...
                         soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_PREEMPHASIS:
        SOC_IF_ERROR_RETURN
            (phy_bcm542xx_preemphasis_get(unit, port,value));
         break;
...
phy_bcm542xx_link_up(int unit, soc_port_t port)
{
    ...
    */
    ...
    if  (pc->macsec_enable) {
        ...
        SOC_IF_ERROR_RETURN(phy_bcm542xx_ability_remote_get(unit, port,
                                              &ability));
        ...
        pc->macsec_switch_fixed_pause = ability.pause;
