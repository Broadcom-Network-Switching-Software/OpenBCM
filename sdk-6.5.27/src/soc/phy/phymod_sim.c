...
STATIC int
sim_bus_mutex_take(void* user_acc)
{
    portmod_default_user_access_t* user_data;
...
STATIC int
sim_bus_mutex_give(void* user_acc)
{
    portmod_default_user_access_t* user_data;
...
int sim_bus_driver_init(int unit)
{
    ...
    if (unit < SOC_MAX_NUM_DEVICES)
    {
        if (sim_bus_per_unit[unit].read == NULL && sim_bus_per_unit[unit].write == NULL)
        {
            ...
            sim_bus_per_unit[unit].mutex_take = sim_bus_mutex_take;
            sim_bus_per_unit[unit].mutex_give = sim_bus_mutex_give;
...
In this case addr_ident is ORed with the phy_id to distinguish between the cores                                   .
                                                                                                                   .
use_default_bus - use default bus provided in this file.                                                                                                                 .
Note: the default bus assumes the user_access is from type portmod_default_user_access_t                                                                                                                                                                         .
*/
int
soc_physim_check_sim(int unit, phymod_dispatch_type_t type, phymod_access_t* access, uint32 addr_ident, int use_default_bus, int* is_sim)
{
    int use_sim;
