...
typedef struct {
    ...
    int mac_loopback_en;           /*mac loopback enable*/
...
phy_qtce_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    for (idx = 0; idx < pmc->num_phys; idx++) {
        ...
        if ((SOC_IS_GREYHOUND2(unit)) && !enable && pCfg->mac_loopback_en && !PHYMOD_ACC_F_QMODE_GET(&pm_phy->access)) {
            /*
             * Port layer disables the PHY loopback after MAC loopback enabling.
             * Override the SD and rx_lock for MAC loopback WO lane swap in case
             * they are disabled in phy loopback disable.
             */
            SOC_IF_ERROR_RETURN
                (phymod_phy_pmd_override_enable_set(pm_phy, phymodPMDRxLock, 1, 1));
            SOC_IF_ERROR_RETURN
                (phymod_phy_pmd_override_enable_set(pm_phy, phymodPMDSignalDetect, 1, 1));
        }
    }
...
_qtce_notify_mac_loopback(int unit, soc_port_t port, uint32 enable)
{
    ...
    if (SOC_IS_GREYHOUND2(unit)) {
        SOC_IF_ERROR_RETURN
            (phymod_core_lane_map_get(pm_core, &lane_map));
        for ( lane = 0 ; lane < NUM_LANES ; lane++) {
            if(lane_map.lane_map_rx[lane] >= NUM_LANES){
                return PHYMOD_E_FAIL;
            }
            lane_swap += lane_map.lane_map_rx[lane]<<(lane*4);
        }
        /*
         * SD and RX_LOCK have been overriden in init if there is lane swap.
         * If there is no lane swap, MAC loopback needs to override SD and RX_LOCK.
         */
        if (lane_swap == 0x3210) {
            if (enable) {
                SOC_IF_ERROR_RETURN
                ...
                SOC_IF_ERROR_RETURN
                    (phymod_phy_pmd_override_enable_set(pm_phy, phymodPMDRxLock, 0, 0));
                SOC_IF_ERROR_RETURN
                    (phymod_phy_pmd_override_enable_set(pm_phy, phymodPMDSignalDetect, 0, 0));
            }
            if (enable) {
                pCfg->mac_loopback_en = TRUE;
            } else {
                pCfg->mac_loopback_en = FALSE;
            }
        }
