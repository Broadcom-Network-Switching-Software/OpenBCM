...
typedef struct {
    ...
    int serdes_1000x_at_12500_vco;  /* run 1G at 12.5G VCO */
...
tsce_show_serdes_info(phy_ctrl_t *pc, tsce_dev_info_t *pInfo, phymod_core_info_t *core_info)
{
    ...
    } else if (core_info->core_version == phymodCoreVersionTsce16) {
        sal_strcpy(pInfo->name,"TSCE16-");
    } else {
                 ...
                 phymod_phy_init_config_t  *pm_phy_init_config)
{
    ...
    */
    ...
    pCfg->serdes_1000x_at_12500_vco = 0;
    pCfg->serdes_1000x_at_12500_vco = soc_property_port_get(unit, port,
                                        spn_SERDES_1000X_AT_12500_VCO,
                                        pCfg->serdes_1000x_at_12500_vco);
...
phy_tsce_init(int unit, soc_port_t port)
{
    ...
    } else if (pCfg->serdes_1000x_at_12500_vco) {
        speed_config->pll_divider_req = 80;
    }
...
STATIC int
tsce_fec_correctable_counter_get(soc_phymod_ctrl_t *pmc, uint32 *value)
{
   phymod_phy_access_t  *pm_phy;
...
STATIC int
tsce_fec_uncorrectable_counter_get(soc_phymod_ctrl_t *pmc, uint32 *value)
{
   phymod_phy_access_t  *pm_phy;
...
STATIC int
tsce_power_get(soc_phymod_ctrl_t *pmc, uint32 *value)
{
    phymod_phy_access_t  *pm_phy;
...
STATIC int
tsce_per_lane_rx_ppm_get(soc_phymod_ctrl_t *pmc, int lane, uint32 *value)
{
    soc_phymod_phy_t    *p_phy;
    uint32              lane_map;
    int16               rx_ppm = 0;
...
/*
 * tsce_eye_marign_est_get
 *
 */
STATIC int
tsce_eye_margin_est_get(soc_phymod_ctrl_t *pmc,
                        ...
                        uint32 *value)
{
    ...
    SOC_IF_ERROR_RETURN
        (phymod_phy_eye_margin_est_get(pm_phy, eye_margin_mode, value));

    return(SOC_E_NONE);
}

/*
 * tsce_per_lane_eye_marign_est_get
 *
 */
STATIC int
tsce_per_lane_eye_margin_est_get(soc_phymod_ctrl_t *pmc,
                                 ...
                                 uint32 *value)
{
    ...
    /* locate the desired phy and lane */
    SOC_IF_ERROR_RETURN
        (_tsce_find_soc_phy_lane(pmc, lane, &p_phy, &lane_map));
    ...
    SOC_IF_ERROR_RETURN
        (phymod_phy_eye_margin_est_get(&pm_phy_copy, eye_margin_mode, value));
...
phy_tsce_control_get(int unit, soc_port_t port, soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_FEC_CORRECTED_BLOCK_COUNT:
        rv = tsce_fec_correctable_counter_get(pmc, value);
        break;
    case SOC_PHY_CONTROL_FEC_UNCORRECTED_BLOCK_COUNT:
        rv = tsce_fec_uncorrectable_counter_get(pmc, value);
        break;
    case SOC_PHY_CONTROL_POWER:
        /* power types like _SHR_PORT_PHY_CONTROL_POWER_LOW doesn't apply to SerDes, limit this for UM usage only */
        if (SOC_IS_FIRELIGHT(unit)) {
            rv = tsce_power_get(pmc, value);
        } else {
            rv = SOC_E_UNAVAIL;
        }
        break;
    case SOC_PHY_CONTROL_EYE_VAL_HZ_L:
        rv = tsce_eye_margin_est_get(pmc, phymod_eye_marign_HZ_L, value);
        break;
    case SOC_PHY_CONTROL_EYE_VAL_HZ_R:
        rv = tsce_eye_margin_est_get(pmc, phymod_eye_marign_HZ_R, value);
        break;
    case SOC_PHY_CONTROL_EYE_VAL_VT_D:
        rv = tsce_eye_margin_est_get(pmc, phymod_eye_marign_VT_D, value);
        break;
    case SOC_PHY_CONTROL_EYE_VAL_VT_U:
        rv = tsce_eye_margin_est_get(pmc, phymod_eye_marign_VT_U, value);
        break;
    default:
        rv = SOC_E_UNAVAIL;
        break; 
                     ...
                     soc_phy_control_t type, uint32 *value)
{
    ...
    case SOC_PHY_CONTROL_RX_SEQ_DONE:
        ...
        break;
    case SOC_PHY_CONTROL_RX_PPM:
        rv = tsce_per_lane_rx_ppm_get(pmc, lane, value);
        break;
    case SOC_PHY_CONTROL_EYE_VAL_HZ_L:
        rv = tsce_per_lane_eye_margin_est_get(pmc, lane, phymod_eye_marign_HZ_L, value);
        break;
    case SOC_PHY_CONTROL_EYE_VAL_HZ_R:
        rv = tsce_per_lane_eye_margin_est_get(pmc, lane, phymod_eye_marign_HZ_R, value);
        break;
    case SOC_PHY_CONTROL_EYE_VAL_VT_D:
        rv = tsce_per_lane_eye_margin_est_get(pmc, lane, phymod_eye_marign_VT_D, value);
        break;
    case SOC_PHY_CONTROL_EYE_VAL_VT_U:
        rv = tsce_per_lane_eye_margin_est_get(pmc, lane, phymod_eye_marign_VT_U, value);
        break;
    default:
        rv = SOC_E_UNAVAIL;
        break; 
...
phy_tsce_probe(int unit, phy_ctrl_t *pc)
{
    ...
    SOC_IF_ERROR_RETURN(phymod_bus_t_init(&core_bus));
   {
    ...
    if (SOC_IS_FIRELIGHT(unit)) {
        ...
        pmc->symbols = &bcmi_tsce16_xgxs_symbols;
#else
    ...
    if (SOC_IS_FIRELIGHT(unit)) {
#ifdef PHYMOD_TSCE16_SUPPORT
        phy_type = phymodDispatchTypeTsce16;
#else
...
STATIC int
phy_tsce_timesync_tx_info_get(int unit, soc_port_t port, soc_port_timesync_tx_info_t *info)
{
    phy_ctrl_t* pc;
    soc_phymod_ctrl_t *pmc;
    phymod_phy_access_t *pm_phy;
    ...
    memset(&tx_info, 0, sizeof(phymod_ts_fifo_status_t));
...
phy_tsce_fec_error_inject_set(int unit, soc_port_t port, uint16 error_control_map, soc_phy_control_fec_error_mask_t bit_error_mask)
{
    ...
    sal_memset(&error_mask, 0, sizeof(error_mask));
...
STATIC int
phy_tsce_diag_stats_get(int unit, soc_port_t port, soc_phy_diag_stats_t *err_stat)
{
    phy_ctrl_t* pc;
    soc_phymod_ctrl_t *pmc;
    phymod_phy_access_t *pm_phy;
...
phy_driver_t phy_tsce_drv = {
    ...
    /* .pd_timesync_tx_info_get            = */ phy_tsce_timesync_tx_info_get,
    /* .pd_diag_stats_get                  = */ phy_tsce_diag_stats_get,
    /* .pd_pause_addr_set                  = */ NULL,
    /* .pd_pause_addr_get                  = */ NULL,
    /* .pd_phy_signal_detect_status_get    = */ NULL,
};
