    ...
    for (blk = 0; SOC_BLOCK_INFO(unit, blk).type >= 0; blk++) {
        ...
        switch(blktype)
        {
            ...
            case SOC_BLK_FSRD:
                ...
                break;
            case SOC_BLK_RTP:
                if(SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_RTP_BLKS) {
                    si->rtp_blocks[SOC_BLOCK_INFO(unit, blk).number] = blk;
                    si->block_port[blk] = SOC_BLOCK_INFO(unit, blk).number | SOC_REG_ADDR_INSTANCE_MASK;
                }
                else {
                    SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("soc_dfe_info_config: too much RTP blocks")));
                }
                break;
            case SOC_BLK_OCCG:
                si->occg_block = blk;
            ...
            case SOC_BLK_BRDC_FSRD:
                if(SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_BRDC_FSRD_BLKS) {
                    ...
                    SOCDNX_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_SOCDNX_MSG("soc_dfe_info_config: too much BRDC FSRD blocks")));
                }
                break;
            case SOC_BLK_DCM:
...
soc_dfe_detach(int unit)
{
    ...
    SOCDNX_IF_ERR_CONT(soc_sand_indirect_mem_access_deinit(unit));
...
soc_dfe_attach(int unit)
{
    ...
    for (mem = 0; mem < NUM_SOC_MEM; mem++) {
        if (SOC_MEM_IS_VALID(unit, mem)) {
            ...
            sal_memset(SOC_MEM_STATE(unit, mem)->cache,
                       0,
                       sizeof(SOC_MEM_STATE(unit, mem)->cache));
        }
    }
    ...
    rc = soc_sand_indirect_mem_access_init(unit);
    if (rc != SOC_E_NONE) {
       SOCDNX_EXIT_WITH_ERR(rc, (_BSL_SOCDNX_MSG("failed to Initialize indirect memory access")));
    }
