...
int cint_fap_l2_addr_add_mc(int fap_unit, bcm_mac_t mac, bcm_vlan_t vlan, bcm_multicast_t mc_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(fap_unit, &l2_addr), "");
...
int cint_fe_fabric_multicast_set(int fe_unit, bcm_multicast_t mc_id, int nof_modids, bcm_module_t * modids_array)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_create(fe_unit, BCM_MULTICAST_WITH_ID, &mc_id), error_msg);

    BCM_IF_ERROR_RETURN_MSG(bcm_fabric_multicast_set(fe_unit, mc_id, 0, nof_modids, modids_array), error_msg);
...
int cint_fap_egress_multicast_is_multicast_v2_supported(int fap_unit, int *egress_multicast_v2_supported)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "unit %d.", fap_unit);
    BCM_IF_ERROR_RETURN_MSG(bcm_device_member_get(fap_unit, 0, bcmDeviceMemberDNX, &is_dnx), error_msg);
    if(is_dnx)
    {
...
int cint_fap_egress_multicast_set(int fap_unit, bcm_multicast_t mc_id, int nof_ports, bcm_port_t * ports_array)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_fap_egress_multicast_is_multicast_v2_supported(fap_unit, &egress_multicast_v2_supported), "");
    ...
    snprintf(error_msg, sizeof(error_msg), "mc_id=%d", mc_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_create(fap_unit, (multicast_create_flags | BCM_MULTICAST_WITH_ID), &mc_id), error_msg);

    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_set(fap_unit, mc_id, multicast_create_flags, nof_ports, rep), error_msg);
...
int cint_fap_egress_multicast_delete(int fap_unit, bcm_multicast_t mc_id, int nof_ports, bcm_port_t * ports_array)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "(%d)", fap_unit);
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_delete(fap_unit, mc_id, multicast_delete_flags, nof_ports, rep), error_msg);
