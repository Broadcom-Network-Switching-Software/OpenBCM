 ...
 *
 * Test 3
 *     SAT generates UDP packets and stamps Sequence_Number to the packet.
 *     PMF group is configured to qualify those packets, add an OAM action.
 *     ETPP stamps the OAM timestamp to the packet and update UDP.checksum.
 *
 * 1. Run:
 *     cd ../../../../src/examples/
 *     cint sat/dnx/cint_dnx_sat.c
 *     l2 add mac=00:00:00:00:00:02 vlan=1 pbm=<outP>
 *     cint
 *     print sat_udp_checksum_setup(unit, inP, outP);
 *     print bcm_sat_gtf_packet_start(unit, sat_info.gtf_id,bcmSatGtfPriCir);
 *
 *     Expect that the packets received at outP has timestamp/sequence_number
 *     stamped, and the UDP checksum is correct.
 */
...
int sat_header_build(int unit, int inP, int outP, int *vlan_pri, uint8* pkt_data, int *seq_num_offset) {
    ...
    offset = build_ase_oam(pkt_data,
                           ...
                           0,
                           /* PTCH+ITMH+ASE+PPH+UDH */
                           2+5+6+12+1+sat_info.timestamp_offset);

    pkt_data[offset+11] = 2; /* Parsing_start_type ETH */
...
/**
  * Create a PMF group and install an entry to set OAM action
  * for SAT generated packets.
  */
int sat_udp_checksum_pmf_config(
    int unit
)
{
    bcm_field_presel_entry_data_t presel_data;
...
/**
  * Clean up procedure for sat_udp_checksum_pmf_config
  */
int sat_udp_checksum_pmf_destroy(int unit)
{
    return sat_pmf_destroy(unit);
}
...
/**
  ...
  *
  * outP - the port the SAT packets will be forwarded to.
  *       This isthe PP forwarding destination port of SAT packets
  */
int sat_udp_checksum_gtf_setup(int unit, int inP, int outP) {
    bcm_port_interface_info_t interface_info;
    bcm_port_mapping_info_t mapping_info;
    bcm_sat_gtf_packet_config_t packet_config;
    bcm_sat_gtf_bandwidth_t bw;
    ...
    uint8 udp_packet[] = {
        0x00, 0x00, /* place holder for PTCH-2 */
        0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x01, 0x81, 0x00, 0x00, 0x0a,
        0x08, 0x00, 0x45, 0x00, 0x00, 0x2e, 0x00, 0x00,
        0x00, 0x00, 0x40, 0x11, 0x74, 0xba, 0x02, 0x02,
        0x02, 0x02, 0x01, 0x01, 0x01, 0x01, 0x00, 0x3f,
        0x00, 0x3f, 0x00, 0x1a, 0xf9, 0x36, 0x00, 0x00,
    ...
    if (!BCM_GPORT_IS_TRUNK(inP) && !BCM_GPORT_IS_TRUNK(outP)) {
        BCM_PBMP_PORT_SET(p, inP);
        BCM_PBMP_PORT_ADD(p, outP);
        BCM_PBMP_CLEAR(u);
        ...
        rv = bcm_vlan_gport_add(unit, vlan, outP, 0);
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_vlan_gport_add(vlan=%d)\n", vlan);
            return rv;
        }
    ...
    sal_memcpy(&l2addr.mac, &udp_packet[2], sizeof(l2addr.mac));
    l2addr.vid = vlan;
    l2addr.flags = BCM_L2_STATIC;
    l2addr.port = outP;
    rv = bcm_l2_addr_add(unit, l2addr);

    sat_info.in_port = inP;
    sat_info.out_port = inP;
    sat_info.vlan[0] = vlan;
    sat_info.da = l2addr.mac;
    ...
    build_ptch2(udp_packet, 0, 0, 0, mapping_info.pp_port);
    sal_memcpy(sat_header->pkt_data->data, udp_packet, sizeof(udp_packet));
    sat_header->pkt_data->len = sizeof(udp_packet);

    packet_config.header_info.pkt_data = sat_header->pkt_data;
    packet_config.header_info.blk_count = 1;

    packet_config.payload.payload_type = bcmSatPayloadConstant8Bytes;
    sal_memset(packet_config.payload.payload_pattern, 0, BCM_SAT_PAYLOAD_MAX_PATTERN_SIZE);
    packet_config.packet_edit[0].packet_length[0] = 66;
    packet_config.packet_edit[0].pattern_length = 1;
    packet_config.packet_edit[0].packet_length_pattern[0] = 0;
    packet_config.packet_edit[0].number_of_ctfs = 1;
     ...
     * seq_number_offset is counted from the start of PTCH-2.
     */
    packet_config.offsets.seq_number_offset = 48; /* 2+12+4+2+20+8 */
    ...
    /* bcm_sat_gtf_packet_start(unit, gtf_id, bcmSatGtfPriCir); */
    sat_info.gtf_id = gtf_id;
...
int sat_udp_checksum_gtf_cleanup(int unit, bcm_sat_gtf_t gtf_id) {
    bcm_pbmp_t p;
    bcm_vlan_t vlan;
    int rv;

    vlan = sat_info.vlan[0];

    if (rv=bcm_l2_addr_delete(unit, sat_info.da, vlan)) {
        printf("Error: bcm_l2_addr_delete %d\n", rv);
    }

    if (!BCM_GPORT_IS_TRUNK(sat_info.in_port) && !BCM_GPORT_IS_TRUNK(sat_info.out_port)) {
        BCM_PBMP_PORT_SET(p, sat_info.in_port);
        BCM_PBMP_PORT_ADD(p, sat_info.out_port);

        if (rv=bcm_vlan_port_remove(unit, vlan, p)) {
            printf("Error: bcm_vlan_port_remove %d\n", rv);
        }
    } else {
        rv = bcm_vlan_gport_delete(unit, vlan, sat_info.in_port);
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_vlan_gport_delete(vlan=%d)\n", vlan);
        }

        rv = bcm_vlan_gport_delete(unit, vlan, sat_info.out_port);
        if (rv != BCM_E_NONE) {
            printf("Error, in bcm_vlan_gport_delete(vlan=%d)\n", vlan);
        }
    }

    if (rv=bcm_vlan_destroy(unit, vlan)) {
        printf("Error: bcm_vlan_destroy %d\n", rv);
    }

    return sat_gtf_cleanup(unit, gtf_id);
}
...
/**
  ...
  *     IF InPort == inP THEN
  *     ACTION OAM = {
  *                      OAM-Stamp-Offset: point to the first byte right after UDP header;
  *                      OAM-Sub-Type: TWAMP_TX;
  *                      OAM-Up-Mep: 1;
  *                  }
  *
  * Create GTF to generate packets. The packets have the following format:
  *     PAYLOADoTIMESTAMPoSEQ-NUMoUDPoIPv4oETH1oPTCH-2
  * The PAYLOAD should be repeatable constant bytes.
  * UDP.checksum should be calculated as if TIMESTAMP == 0 and SEQ-NUM == 0;
  * When SAT generates the packets, it stamps 4 bytes SEQ-NUM into the packet right after UDP header;
  * Ingress PMF will qualify this packet, and set the OAM action;
  * Once ETPP receives those packets with OAM-Sub-Type == TWAMP_TX, it stamps 8 bytes timestamp to the
  * packet at the position right after SEQ-NUM;
  */
int sat_udp_checksum_setup(int unit, int inP, int outP, int vlan)
{
    bcm_gport_t gport;
    int rv = BCM_E_NONE;

    /**
     * Qualify inP and set OAM action
     */
    sat_field_info.nof_quals = 1;
    sat_field_info.qual_type[0] = bcmFieldQualifyInPort;
    sat_field_info.qual_info[0].input_type = bcmFieldInputTypeMetaData;
    sat_field_info.qual_info[0].input_arg = 1;
    sat_field_info.qual_info[0].offset = 0;
    BCM_GPORT_LOCAL_SET(gport, inP);
    sat_field_info.qual_val[0] = gport;
    sat_field_info.qual_mask[0] = 0xffffffff;

    sat_field_info.nof_actions = 1;
    /* value[0]: OAM-Stamp-Offset. value[1]: OAM-offset. value[2]: OAM-Sub-Type. value[3]: OAM-Up-Mep. */
    sat_field_info.action[0].type = bcmFieldActionOam;
    sat_field_info.action[0].value[0] = 52; /* 2+12+4+2+20+8+4 */
    sat_field_info.action[0].value[2] = 15; /* TWAMP_TX */
    sat_field_info.action[0].value[3] = 0; /* Down-Mep */

    rv = sat_udp_checksum_pmf_config(unit);
    if (BCM_E_NONE != rv) {
        printf("Error in sat_udp_checksum_pmf_config: %d\n", rv);
        return rv;
    }

    rv = sat_udp_checksum_gtf_setup(unit, inP, outP);
    if (BCM_E_NONE != rv) {
        printf("Error in sat_udp_checksum_gtf_setup: %d\n", rv);
        return rv;
    }
...
/**
  * Clean up procedure for sat_udp_checksum_setup
  */
int sat_udp_checksum_cleanup(int unit) {
    int rv = BCM_E_NONE;

    rv = sat_udp_checksum_pmf_destroy(unit);
    if (BCM_E_NONE != rv) {
        printf("Error in sat_udp_checksum_pmf_destroy: %d\n", rv);
        return rv;
    }

    rv = sat_udp_checksum_gtf_cleanup(unit, sat_info.gtf_id);
    if (BCM_E_NONE != rv) {
        printf("Error in sat_udp_checksum_gtf_cleanup: %d\n", rv);
        return rv;
    }
