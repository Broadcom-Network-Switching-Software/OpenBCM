...
traffic_example(int unit, int in_port, int out_port, int is_arad, int mode, int color_blind) {
    ...
    if (!is_arad) {
        ...
        BCM_IF_ERROR_RETURN_MSG(get_core_from_port(unit, out_port, &core_id), "");
        BCM_IF_ERROR_RETURN_MSG(get_tm_port_from_port(unit, out_port, &DstPortValue), "");
        ...
        rv = bcm_field_entry_stat_attach(unit, ent, counter_pointer_id);
        if (rv != BCM_E_NONE) {
...
int get_counter_value(int unit, int in_port, int counter_ptr_id)
{
    ...
    for (proc_id=0; proc_id<nof_engines; ++proc_id) {
        engine.engine_id = proc_id;
        ...
        if ((config.source.core_id == mapping_info.core)
            && (config.source.engine_source == bcmStatCounterSourceIngressField)
            && (config.source.command_id == 0)) { /* 0 for bcmFieldActionStat0 */
            if ((counter_ptr_id >= config.source.pointer_range.start) && (counter_ptr_id <= config.source.pointer_range.end)) {
                int stat_id;

                BCM_FIELD_STAT_ID_SET(stat_id, proc_id, counter_ptr_id);
                BCM_IF_ERROR_RETURN_MSG(bcm_field_stat_get(unit, stat_id, bcmFieldStatPackets, &stat_value), "");

                return COMPILER_64_LO(stat_value);
            }
        }
    }
...
/*
 * Get pp_port from input local port. On fail, return '-1'
 */
int get_tm_port_from_port(int unit, int port, int * pp_port)
{
    bcm_error_t rc ;
    uint32 dummy_flags ;
    bcm_port_interface_info_t interface_info;
