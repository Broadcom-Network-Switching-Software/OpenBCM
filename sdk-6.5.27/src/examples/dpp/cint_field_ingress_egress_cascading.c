...
/* ***************************************************************** */
/*
 * It indicates to check on egress PMF both bcmFieldQualifyDstClassField and bcmFieldQualifySrcClassField
 * Valid only for User-Header example(method == 1)
 */
int check_src_dst_class_on_egress = 0;
   ...
   */
    ...
    */
    ...
    if (method == 1) {
        ...
        if (check_src_dst_class_on_egress) {
    ...
    if (method == 1) {
        ...
        if (check_src_dst_class_on_egress) {
            rv = bcm_field_action_add(unit, ent, bcmFieldActionClassSourceSet, 6, 0xff);
            if (rv != BCM_E_NONE) {
                printf("Error in bcm_field_action_add User-Header\n");
                return rv;
            }
            rv = bcm_field_action_add(unit, ent1, bcmFieldActionClassSourceSet, 67, 0xff);
            if (rv != BCM_E_NONE) {
                printf("Error in bcm_field_action_add User-Header\n");
                return rv;
            }
   ...
   */
    ...
    */
    ...
    if (method == 1) {
        ...
        if (check_src_dst_class_on_egress) {
            BCM_FIELD_QSET_ADD(egr_group.qset, bcmFieldQualifySrcClassField);
        }
    }else if (method == 2) {
    ...
    if (method == 1) {
        ...
        if (check_src_dst_class_on_egress) {
            rv = bcm_field_qualify_SrcClassField(unit, ent, 6, 0xff);
            if (rv != BCM_E_NONE) {
                printf("Error in bcm_field_qualify_InPort returned %d\n", rv);
                return rv;
            }
