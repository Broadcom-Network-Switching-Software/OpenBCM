/*  Feature  : Aggregate Monitor for Trunk/ECMP
 *
 *  Usage    : BCM.0> cint cint_agm.c YES 0/1
 *              E.g. cint cint_agm.c YES 0      <<<<< TRUNK AGM
 *              E.g. cint cint_agm.c YES 1      <<<<< ECMP AGM
 *
 *  config   : trunk_config.bcm
 *
 *  Log file : trunk_agm_log.txt
 ...
 *   1. Create a trunk/ecmp group with 3 port/interface memebers
 ...
 *     
 *     CASE2: ECMP (Done in example_run_ecmp() )
 *       a) Create VLAN(100) and add ing_portX.
...
int trunk_create(int unit, int *port, int nport, int psc, bcm_trunk_t *tid)
{
    ...
    int member_max = 64;
    bcm_trunk_member_t  trunk_member[member_max];
    
    if( nport > member_max ) {
        printf("Only allowed nports is less than %d. Specified: %d\n",
                member_max, nport);
        return -1;
    }
    ...
    for(i=0; i<nport; i++) {
        ...
        BCM_GPORT_MODPORT_SET(trunk_member[i].gport, 0, port[i]);
    }
...
bcm_error_t verify()
{
    ...
    if( opt_TestTypeEcmp )
    {
        bcm_mac_t router_mac_out = {0x00, 0x00, 0x00, 0x00, 0x11, 0x23};
        bcm_mac_t dst_mac_nhop = {0x00, 0x00, 0x00, 0x00, 0x22, 0x23};
        bcm_vlan_t vlan_out = test_vid;
        int vrf = 0;
        ...
        // This will be smac for egress pkt
