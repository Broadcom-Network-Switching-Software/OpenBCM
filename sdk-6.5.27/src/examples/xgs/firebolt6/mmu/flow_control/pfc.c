 ...
 *  
 * DMAC=0x1            +------------+         DMAC=0x1
 * SMAC=0x2   +-------->   56470    +-------> SMAC=0x2
 * VLAN=2,Pri=2  xe0(1)|            |xe1(2)   VLAN=2,Pri=2
 *                     | l2:mac=0x1 |
 *                     |    vlan=2  |
 *      <-------+      |    port=2  |     <-------+
 ...
 *    ==========================================
 *      a) send at linerate to xe0 L2 learnt packets (vlan=1 prio=2 dmac=0x1 smac=0x2)
 *      b) Along with a), send at linerate to xe1 PFC frames with pause-control class 2 enabled
 *      c) Expected Result:
 *         ================
 *         For test case mentioned in a)
 *         On xe0 (Port-1): verify that PFC frames should be transmitted at regular intervals as
 *         egress port is congested.
 *         In 'show c' output, PFC frame Tx should be seen on xe0 (port-1)
 *
 *         On xe1 (Port-2): packets should egress at configured rate
 *
 *         For test case mentioned in b)
 *         Port xe1 should not egress any packets as pfc pasue frames are being
 *         received. In 'show c' output, there should not be any Tx on on xe1 (port-2)
 */
