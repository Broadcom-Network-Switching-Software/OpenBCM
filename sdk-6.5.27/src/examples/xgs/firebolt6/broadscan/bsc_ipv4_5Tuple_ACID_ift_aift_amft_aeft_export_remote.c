/*  Feature  : Broadscan
 *
 *  Usage    : BCM.0> bsc_ipv4_5Tuple_ACID_ift_aift_amft_aeft_export_remote.c 
 *
 *  config   : broadscan_config.bcm
 *
 *  Log file : bsc_ipv4_5Tuple_ACID_ift_aift_amft_aeft_export_remote_log.txt
 *
 *  Test Topology :
 *
 *                   +------------------------------+
 ...
 *  Summary:
 *  ========
 *  This CINT demonstrates how to configure Broadscan Engine to learn micro flow in IFT and assign aggregate class ID (ACID).
 *  Macro/Aggregate flow groups will learn flow based on ACID.
 *  Script also shows how to attach remote collectors to micro flow group and aggregate flow ggroups.
 *  Also programs thresholds to trigger export of flow records to remote collectors.
 *  In this test we send a flow from ingress port and configure broadscan engine to export the record
 *  to remote collector when flow check thresholds are crossed. We use CPU port as remote collector in this case.
 *  The micro flow group record will contain data from IFT, MFT and EFT tables and sent to micro group collector.
 *  The macro/aggregate flow group record will contain data from AIFT, AMFT and AEFT tables and sent to 
 *  corresponding aggregate group collector. 
 *
 ...
 *    =============================================
 *      a) Select ingress port, egress port and configure in LB mode.
 *      b) Create vlan 10 and add ingress, egress port.
 *      c) Start "ftrmon" utility to decode recieved IPFX packet which has export record
 *
 *    2) Step2 - Configuration (Done in broadscan_setup()):
 *    =====================================================
 *      a) Micro Flow Group setup
 *         a.1) Create Collector for FT Group
 *         a.2) Create FT Group 
 *         a.3) Set Ft Group Flow Limit to 10 
 *         a.4) Create Flow Check
 *         a.5) Add Check to FT Group
 *         a.6) Set Tracking param for FT Group
 *         a.7) Validate and create template for FT Group
 *         a.8) Attach Collector/Template to FT Group
 *      b) Aggregte Ingress Flow Group setup
 *         b.1) Create Collector for Aggregate Ingress FT Group
 *         b.2) Create Aggregate Ingress FT Group
 *         b.3) Create Flow Check
 *         b.4) Add Check to AIFT Group
 *         b.5) Set Tracking param for AIFT Group
 *         b.6) Validate and create template for AIFT Group
 *         b.7) Attach Collector/Template to AIFT Group
 *         b.8) Create User Entry in AIFT Group
 *      c) Aggregte MMU Flow Group setup
 *         c.1) Create Collector for Aggregate MMU FT Group
 *         c.2) Create Aggregate MMU FT Group
 *         c.3) Create Flow Check
 *         c.4) Add Check to AMFT Group
 *         c.5) Set Tracking param for AMFT Group
 *         c.6) Validate and create template for AMFT Group
 *         c.7) Attach Collector/Template to AMFT Group
 *         c.8) Create User Entry in AMFT Group
 *      d) Aggregte Egress Flow Group setup
 *         d.1) Create Collector for Aggregate Egress FT Group
 *         d.2) Create Aggregate Egress FT Group
 *         d.3) Create Flow Check
 *         d.4) Add Check to AEFT Group
 *         d.5) Set Tracking param for AEFT Group
 *         d.6) Validate and create template for AEFT Group
 *         d.7) Attach Collector/Template to AEFT Group
 *         d.8) Create User Entry in AEFT Group
 *      e) Flow Selection Steps for IFT, AIFT
 *         e.1) Create Aggreagte Class (generate ACID) 
 *         e.2) Create FTFP group with presel (IPv4 packets) 
 *         e.3) Qualify on IP protocol=TCP and IngressPort
 *         e.4) Add action to assign micro flow group ID and ACID
 *         e.5) Add action to assign AIFT group ID and User Entry Index
 *         e.6) Install FTFP entry
 *      f) Flow Selection Steps for AMFT, AEFT
 *         f.1) Create AIFTFP, AEFTFP group
 *         f.2) Qualify on ACID
 *         f.3) Add action to assign AIFT/AEFT group ID and User Entry Index 
 *         f.4) Install AIFTFP/AEFTFP entry
 *    3) Step3 - Verification (Done in verify()):
 *    ===========================================
 *      a) Transmit the below IPv4 TCP Syn packet two times on ingress_port.
 *        Packet:
 *        ======
 ...
 *      b) Expected Result:
 *      ===================
 *      The first packet will trigger flow learning across IFT, AIFT, AMFT and AEFT tables.
 *      The second packet will trigger the export of the flow records from IFT, AIFT, AMFT and AEFT tables.
 *      CPU receives 4 export records and ftrmon dumps on the console as below
 *      
 *      AEFT export record
 *        [bcmFtRMon.0]*************************************************                                                                       
 ...
 *        [bcmFtRMon.0]IPFIX sequence number = 04 (0x04)                                                                                       
 *        [bcmFtRMon.0]IPFIX observation domain id = 01 (0x01)                                                                                 
 *        [bcmFtRMon.0]*************************************************                                                                       
 *        [bcmFtRMon.0]*************************************************                                                                       
 *        [bcmFtRMon.0]IPFIX data set # 01, set_id = 7924 (0x1ef4) record length = 128                                                         
 *        [bcmFtRMon.0]Info Element:: 15 (            Reserved) | Size: 02 | Value: (????)                                                     
 *        [bcmFtRMon.0]Info Element:: 14 (    FlowtrackerGroup) | Size: 02 | Value: 0x0000(0000)                                               
 *        [bcmFtRMon.0]Info Element:: 33 (       ExportReasons) | Size: 02 | Value: 0x0020(0032)                                               
 *        [bcmFtRMon.0]Info Element:: 34 (         ExportFlags) | Size: 01 | Value: 0x54(84)                                                   
 *        [bcmFtRMon.0]Info Element:: 15 (            Reserved) | Size: 57 | Value: (????)                                                     
 *        [bcmFtRMon.0]Info Element:: 08 (           ByteCount) | Size: 04 | Value: 0x000000d8(00000216)                                       
 *        [bcmFtRMon.0]Info Element:: 07 (            PktCount) | Size: 04 | Value: 0x00000002(00000002)                                       
 *        [bcmFtRMon.0]Info Element:: 47 (    FlowtrackerCheck) | Size: 04 | Value: 0x00000002(00000002)                                       
 *        [bcmFtRMon.0]Info Element:: 15 (            Reserved) | Size: 48 | Value: (????)                                                     
 *        [bcmFtRMon.0]*************************************************                      
 *        
 *      AMFT export record
 *        [bcmFtRMon.0]*************************************************                                                                       
 ...
 *        [bcmFtRMon.0]IPFIX message length = 144 (0x90)                                                                                       
 *        [bcmFtRMon.0]IPFIX export timestamp = 00 (0x00)                                                                                      
 *        [bcmFtRMon.0]IPFIX sequence number = 03 (0x03)                                                                                       
 *        [bcmFtRMon.0]IPFIX observation domain id = 01 (0x01)                                                                                 
 *        [bcmFtRMon.0]*************************************************                                                                       
 *        [bcmFtRMon.0]*************************************************                                                                       
 *        [bcmFtRMon.0]IPFIX data set # 01, set_id = 7923 (0x1ef3) record length = 128                                                         
 *        [bcmFtRMon.0]Info Element:: 15 (            Reserved) | Size: 02 | Value: (????)                                                     
 *        [bcmFtRMon.0]Info Element:: 14 (    FlowtrackerGroup) | Size: 02 | Value: 0x0000(0000)                                               
 *        [bcmFtRMon.0]Info Element:: 33 (       ExportReasons) | Size: 02 | Value: 0x0020(0032)                                               
 *        [bcmFtRMon.0]Info Element:: 34 (         ExportFlags) | Size: 01 | Value: 0x2c(44)                                                   
 *        [bcmFtRMon.0]Info Element:: 15 (            Reserved) | Size: 73 | Value: (????)                                                     
 *        [bcmFtRMon.0]Info Element:: 08 (           ByteCount) | Size: 04 | Value: 0x000000d8(00000216)                                       
 *        [bcmFtRMon.0]Info Element:: 07 (            PktCount) | Size: 04 | Value: 0x00000002(00000002)                                       
 *        [bcmFtRMon.0]Info Element:: 47 (    FlowtrackerCheck) | Size: 04 | Value: 0x00000002(00000002)                                       
 *        [bcmFtRMon.0]Info Element:: 15 (            Reserved) | Size: 32 | Value: (????)                                                     
 *        [bcmFtRMon.0]*************************************************         
 *        
 *      AIFT export record
 *        [bcmFtRMon.0]*************************************************                                                                       
 ...
 *        [bcmFtRMon.0]IPFIX message length = 144 (0x90)                                                                                       
 *        [bcmFtRMon.0]IPFIX export timestamp = 00 (0x00)                                                                                      
 *        [bcmFtRMon.0]IPFIX sequence number = 02 (0x02)                                                                                       
 *        [bcmFtRMon.0]IPFIX observation domain id = 01 (0x01)                                                                                 
 *        [bcmFtRMon.0]*************************************************                                                                       
 *        [bcmFtRMon.0]*************************************************                                                                       
 *        [bcmFtRMon.0]IPFIX data set # 01, set_id = 7922 (0x1ef2) record length = 128                                                         
 *        [bcmFtRMon.0]Info Element:: 15 (            Reserved) | Size: 02 | Value: (????)                                                     
 *        [bcmFtRMon.0]Info Element:: 14 (    FlowtrackerGroup) | Size: 02 | Value: 0x0001(0001)                                               
 *        [bcmFtRMon.0]Info Element:: 33 (       ExportReasons) | Size: 02 | Value: 0x0020(0032)                                               
 *        [bcmFtRMon.0]Info Element:: 34 (         ExportFlags) | Size: 01 | Value: 0x84(132)                                                  
 *        [bcmFtRMon.0]Info Element:: 15 (            Reserved) | Size: 105 | Value: (????)                                                    
 *        [bcmFtRMon.0]Info Element:: 08 (           ByteCount) | Size: 04 | Value: 0x000000d8(00000216)                                       
 *        [bcmFtRMon.0]Info Element:: 07 (            PktCount) | Size: 04 | Value: 0x00000002(00000002)                                       
 *        [bcmFtRMon.0]Info Element:: 47 (    FlowtrackerCheck) | Size: 04 | Value: 0x00000002(00000002)                                       
 *        [bcmFtRMon.0]*************************************************     
 ...
 *        [bcmFtRMon.0]*************************************************                                                                       
 *        [bcmFtRMon.0]*************************************************                                                                       
 *        [bcmFtRMon.0]IPFIX data set # 01, set_id = 7921 (0x1ef1) record length = 128                                                         
 *        [bcmFtRMon.0]Info Element:: 15 (            Reserved) | Size: 02 | Value: (????)                                                     
 *        [bcmFtRMon.0]Info Element:: 14 (    FlowtrackerGroup) | Size: 02 | Value: 0x0000(0000)                                               
 *        [bcmFtRMon.0]Info Element:: 33 (       ExportReasons) | Size: 02 | Value: 0x0020(0032)                                               
 *        [bcmFtRMon.0]Info Element:: 34 (         ExportFlags) | Size: 01 | Value: 0xe4(228)                                                  
 *        [bcmFtRMon.0]Info Element:: 15 (            Reserved) | Size: 39 | Value: (????)                                                     
