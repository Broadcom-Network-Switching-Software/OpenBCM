/*  Feature  : Broadscan
 *
 *  Usage    : BCM.0> cint bsc_ipv4_5Tuple_ageOut_export_remote.c
 *
 *  config   : broadscan_config.bcm
 *
 *  Log file : bsc_ipv4_5Tuple_ageOut_export_remote_log.txt
 *  
 *  Test Topology :
 *
 *                   +------------------------------+ 
 ...
 *  Summary:
 *  ========
 *  This CINT configures Broadscan Engine to export a record on a age out event event to remote collector.
 *  In this test we send a flow from ingress port and configure broadscan engine to export the record
 *  to remote collector when there is a flow age out. We use CPU port as remote collector in this case.
 *
 ...
 *      d) Set Age timer to 10000 milli seconds to trigger a flow record export.
 ...
 *
 * 	+++++++++++++++++++++++++++++++++++++++++++++++++++++
 *  	Printing Group stats for Group Id : = 10000
 ...
 *  	+++++++++++++++++++++++++++++++++++++++++++++++++++++
 *
 *      Up on age timer, entry gets aged out and a flow record was sent to remote collector which is CPU. 
 *      'ftrmon' utility decode the recieved IPFIX packet and display the below information.
 *
 * waiting 10000 millisec for packet to learn and age out
 * Sleeping for 10 seconds
 * [bcmFtRMon.0]
 * [bcmFtRMon.0]Packet[2]: Total 2
 * [bcmFtRMon.0]*************************************************
 ...
 * [bcmFtRMon.0]0005: 08 07 00 98 f7 8a 00 0a
 * [bcmFtRMon.0]0006: 00 90 00 00 00 00 00 00
 * [bcmFtRMon.0]0007: 00 00 00 00 00 01 1e fe
 * [bcmFtRMon.0]0008: 00 80 0e e8 03 e8 00 04
 * [bcmFtRMon.0]0009: 00 00 00 00 00 00 00 00
 * [bcmFtRMon.0]0010: 00 00 00 00 00 00 00 00
 * [bcmFtRMon.0]0011: 00 77 88 55 66 00 00 00
 * [bcmFtRMon.0]0012: 02 00 03 00 04 00 05 00
 * [bcmFtRMon.0]0013: 06 00 07 00 02 00 00 00
 * [bcmFtRMon.0]0014: 01 00 02 00 03 00 04 00
 * [bcmFtRMon.0]0015: 00 00 00 00 02 01 00 00
 ...
 * [bcmFtRMon.0]*************************************************
 * [bcmFtRMon.0]*************************************************
 * [bcmFtRMon.0]Packet[2]:
 * Packet[2]:   Ethernet: dst<00:00:00:00:00:c0> src<00:00:00:00:00:f0> Tagged Packet ProtID<0x8100> Ctrl<0x0600> Internet Protocol (IP)
 * Packet[2]:   IP: V(4) src<10.130.86.42> dst<10.130.176.10> hl<5> service-type<132> tl<172> id<0> frg-off<0> ttl<9> > chk-sum<0x6979>
 * [bcmFtRMon.0]*************************************************
 ...
 * [bcmFtRMon.0]*************************************************
 * Printing stats after aging flow entry '
 * ////////////////////////////////////////////////////////////
 * Printing Group stats for Group Id : = 1000
 * bcm_flowtracker_group_stat_t group_stats = {
 *   uint64 flow_exceeded_count = {0x00000000 0x00000000}
 *   uint64 flow_missed_count = {0x00000000 0x00000000}
 *   uint64 flow_aged_out_count = {0x00000000 0x00000001}
 *   uint64 flow_learnt_count = {0x00000000 0x00000001}
 *   uint64 flow_meter_exceeded_count = {0x00000000 0x00000000}
 *   uint64 flow_start_count = {0x00000000 0x00000000}
 *   uint64 flow_end_count = {0x00000000 0x00000000}
 * }
 * ////////////////////////////////////////////////////////////
 */
...
verify(int unit, bcm_flowtracker_group_t flow_group_id)
{
	...
	snprintf(str, 512, "tx 1 pbm=%d data=0x2233445566771020304050608100001E8100002E08004508003800004000400649980A0A02020A0A02033344556600A1A2A300B1B2B3500211F5F8C30000F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5;
                        sleep 1", ingress_port); 
	bshell(unit, str);
	printf("Executing 'show c'\n");
	bshell(unit, "show c");
        /* Broadscan stats for learning */
	printf("Printing stats after learning flow entry '\n");
	rv = broadscan_stat_check(unit, flow_group_id);
	if (BCM_FAILURE(rv)) {
		printf("Failed to fetch Group stats for group: %d\n", flow_group_id);
		return rv;
	}
	printf("waiting 10000 millisec for packet to learn and age out \n");
	bshell(unit, "sleep 10");
	printf("Printing stats after aging flow entry '\n");
	rv = broadscan_stat_check(unit, flow_group_id);
	if (BCM_FAILURE(rv)) {
		printf("Failed to fetch Group stats for group: %d\n", flow_group_id);
		return rv;
 ...
 *	Sets Age time to 10000 milli seconds
 */	
int
		...
		int num_tracking_list, int flow_limit)
{
	...
	/* Setting Flow grouop Age time to iage at 10000 milli secs */
	rv = bcm_flowtracker_group_age_timer_set(unit, flow_group_id, 10000);
	if (BCM_FAILURE(rv)) {
...
int execute()
{
	...
	rv = broadscan_set_trigger(unit, flow_group_id, bcmFlowtrackerExportTriggerAgeOut);
	if (BCM_FAILURE(rv)) {
