 ...
 *  Summary:
 *  ========
 *  This cint example demonstrates pause feature on HR4
 ...
 *         at linerate to ge1 (Port-2)
 *         (Pause frame: dmac=0x0180C2000001, smac=0x1, type=0x8808, opcode=0x0001)
 *      c) Expected Result:
 *         ================
 *         For test case mentioned in a)
 *         On ge0 (Port-1): verify that Pause frames should be transmitted at regular intervals as
 *         egress port is congested.
 *         In 'show c' output Pause frame Tx should be seen on ge0 (port-1)
 *
 *         On ge1 (Port-2): verify that packets egress at configured rate
 *
 *         For test case mentioned in b)
 *         On ge1(Port-2): verify that Port does not egress any packets as pause frames are being
 *         received. In 'show c' output, there should not be any Tx on on ge1 (port-2)
 */
