...
int dlb_trunk_create(int unit, int *port, int nport, bcm_trunk_t *tid)
{
    ...
    *tid = trunk_chip_info.trunk_fabric_id_min;
    ...
    for (i = 0; i < nport; i++) {
        bcm_trunk_member_t_init(&trunk_member[i]);
        //BCM_GPORT_MODPORT_SET(trunk_member[i].gport, my_modid, port[i]);
        BCM_GPORT_LOCAL_SET(trunk_member[i].gport, port[i]);
        trunk_member[i].dynamic_load_weight = 100;
        trunk_member[i].dynamic_scaling_factor = 10;  // {10, 25, 40, 50, 75, 100} indicating 10G
    }
