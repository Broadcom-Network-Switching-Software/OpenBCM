 ...
 *             PayloadWiping: (Not supported on Hurricane4)
 *                  0: No payload wiping
...
/* Payload wiping is not supported for Hurricane4 */
...
if( ARGC >= 5 ) {
    if( *ARGV[4] == '1' ) {
        opt_DoPayloadWiping = 1;    // Set it to 0 as Hurricane4 does not support Payload Wiping.
    } else {
        opt_DoPayloadWiping = 0;
    }
}

// Settings for Hurricane4
opt_DoPayloadWiping = 0;
...
bcm_error_t test_setup(int unit)
{
    ...
    int loopBackType = BCM_PORT_LOOPBACK_MAC;
...
bcm_error_t verify()
{
    ...
    if( opt_DoPayloadWiping )   // Not supported on Hurricane4
    {
        //bshell(unit, "sleep 1");
