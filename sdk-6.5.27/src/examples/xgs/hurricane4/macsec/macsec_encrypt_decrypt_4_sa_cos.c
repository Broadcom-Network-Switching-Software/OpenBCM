 ...
 *
 *  Usage    : BCM.0> cint macsec_encrypt_decrypt_4_sa_cos.c
 *
 *  config   : hr4_sdk6_macsec_config.bcm
 *
 *  Log file : macsec_encrypt_decrypt_4_sa_cos_log.txt
 *
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *  Summary:
 *  ========
 *    This CINT configures macsec encryption/decryption with 4 SA's for a single port.
 *
 ...
 *      3.4) Test manual switching to a new egress SA and repeat 3.1)
 *           Expected result:
 *           The packet is encrypted with a new AN and forwarded to egress_port1
 *      3.5) Test 3.2) using packet encrypted with the next ingress SA
 *           Expected result:
 *           The packet is decrypted and forwarded to ingress_port
 */
...
macsec_port_create(int unit, bcm_port_t port, macsec_port_s *macsec_port)
{
    ...
    int i;
    for (i=0; i < 4; i++) {
        BCM_IF_ERROR_RETURN(macsec_secure_assoc_create(unit, decrypt_flags, macsec_port->decrypt_sc_id, &macsec_port->decrypt_sa_id[i], i));
    }
    ...
    for (i=0; i < 4; i++){
        BCM_IF_ERROR_RETURN(macsec_secure_assoc_create(unit, encrypt_flags, macsec_port->encrypt_sc_id, &macsec_port->encrypt_sa_id[i], i));
    }
...
/*    bshell(unit, "m mib_mode mib_encor=0");
 */
    ...
    BCM_IF_ERROR_RETURN(bcm_xflow_macsec_secure_chan_get(unit, macsec_port1.encrypt_sc_id, &chan_info, &priority));
    chan_info.active_an++;
    BCM_IF_ERROR_RETURN(bcm_xflow_macsec_secure_chan_set(unit, BCM_XFLOW_MACSEC_ENCRYPT, macsec_port1.encrypt_sc_id, &chan_info, priority));
    ...
    BCM_IF_ERROR_RETURN(bcm_xflow_macsec_secure_assoc_destroy(unit, macsec_port1.encrypt_sa_id[0]));
    ...
    /* Manually delete the old SA */
    BCM_IF_ERROR_RETURN(bcm_xflow_macsec_secure_assoc_destroy(unit, macsec_port1.decrypt_sa_id[0]));
