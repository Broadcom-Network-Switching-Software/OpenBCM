 ...
 *
 *  Usage    : BCM.0> cint macsec_encrypt_decrypt_4_sa_mtu_cos.c
 *
 *  config   : hr4_sdk6_macsec_config.bcm
 *
 *  Log file : macsec_encrypt_decrypt_4_sa_mtu_cos_log.txt
 *
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *           1.1.5) Configure mtu's for encryption/decryption mtu check in the MACsec engine
 *
 ...
 *           Expected result:
 *           The packet is decrypted and forwarded to ingress_port
 *      3.6) Test mtu check on egress SC
 *           Expected result:
 *           When egress mtu is set to a small number, encrypted packet will not be sent out.
 *           When egress mtu is set to a big number, encrypted packet will be sent out.
 *      3.7) Test mtu check on ingress policy
 *           Expected result:
 *           When ingress mtu is set to a small number, decrypted packet will not be sent out.
 *           When ingress mtu is set to a big number, decrypted packet will be sent out.
 */
...
bcm_error_t
macsec_secure_chan_mtu_set(int unit, int flags, bcm_xflow_macsec_secure_chan_id_t sc_id, int mtu_sel)
{
    int priority;
    bcm_xflow_macsec_secure_chan_info_t sc_info;
    bcm_xflow_macsec_secure_chan_info_t_init(&sc_info);

    BCM_IF_ERROR_RETURN(bcm_xflow_macsec_secure_chan_get(unit, sc_id, sc_info, &priority));

    if (flags & BCM_XFLOW_MACSEC_ENCRYPT) {
        sc_info.mtu_sel     = mtu_sel;
        BCM_IF_ERROR_RETURN(bcm_xflow_macsec_secure_chan_set(unit, flags, sc_id, &sc_info, priority));
    }
    return BCM_E_NONE;
}
...
bcm_error_t
macsec_decrypt_policy_mtu_set(int unit, bcm_xflow_macsec_policy_id_t pol_id, int mtu_sel)
{
    bcm_xflow_macsec_decrypt_policy_info_t pol_info;

    BCM_IF_ERROR_RETURN(bcm_xflow_macsec_decrypt_policy_get(unit, pol_id, &pol_info));
    pol_info.mtu_sel = mtu_sel;
    BCM_IF_ERROR_RETURN(bcm_xflow_macsec_decrypt_policy_set(unit, pol_id, &pol_info));
...
config_macsec(int unit)
{
    ...
    /* set up a small mtu for testing purpose */
    mtu_sel = bcmXflowMacsecMtu1;
    BCM_IF_ERROR_RETURN(bcm_xflow_macsec_mtu_set(
                            unit, BCM_XFLOW_MACSEC_ENCRYPT | BCM_XFLOW_MACSEC_MTU_WITH_ID,
                            0, 64, &mtu_sel));
    mtu_sel = bcmXflowMacsecMtu1;
...
unsigned char expected_encrypted_packet3[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00,
    0x00, 0x0a, 0x88, 0xe5, 0x2d, 0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x08, 0x47, 0x8b, 0x7c, 0x0d, 0x24, 0xd2, 0xe5, 0x32, 0x42, 0x43,
    0x0a, 0x79, 0xad, 0x15, 0xcb, 0xc4, 0x36, 0xed, 0xe5, 0x33, 0x14, 0xbd, 0x0e, 0xcb,
    0xf2, 0x7a, 0x87, 0xef, 0xf9, 0x10, 0x13, 0x0c, 0x13, 0xfe, 0x69, 0xb8, 0x01, 0xee,
    0xad, 0x84, 0xb4, 0xb2, 0x0c, 0x81, 0x10, 0xff, 0xd7, 0xe6, 0xe0, 0xbc, 0x7b, 0x80,
    0xee, 0x60, 0xb3, 0xca, 0x62, 0x83, 0x04, 0xd3, 0x21, 0x83, 0x51, 0x81, 0x1d, 0xdc,
    0x33, 0xb3, 0xef, 0x0d, 0x92, 0x3c};

unsigned char expected_encrypted_packet4[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00,
    0x00, 0x0a, 0x88, 0xe5, 0x2d, 0x00, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x08, 0x9b, 0x10, 0x5c, 0xde, 0xee, 0x63, 0x24, 0x22, 0xff, 0x18,
    0x99, 0x6c, 0x66, 0x33, 0xa3, 0x91, 0x66, 0xa4, 0x66, 0x8a, 0xf7, 0x15, 0x97, 0x47,
    0x60, 0x32, 0x92, 0x99, 0x92, 0xbf, 0x58, 0x98, 0xbd, 0xaf, 0x02, 0xb1, 0x5a, 0x90,
    0xfd, 0x0d, 0xa9, 0xf4, 0x4f, 0x7a, 0x48, 0x7b, 0xc4, 0xc9, 0x54, 0xa2, 0xfa, 0x41,
    0x2f, 0x39, 0xc4, 0xac, 0x1d, 0x99, 0xe3, 0x08, 0xa8, 0x37, 0xfc, 0x55, 0x37, 0xc3,
    0xf5, 0xd4, 0xfa, 0x9f, 0x2c, 0xff};
 ...
 */
    ...
    print "==========  Test6: MACsec Encryption - Unicast (post encryption MTU test) ==================";
    int pass6_1, pass6_2;
    port_discard(unit, ingress_port, 0);
    port_discard(unit, egress_port1, 1);
    snprintf(str, 512, "l2 add module=%d port=%d mac=00:00:00:00:00:22 vlan=10 ReplacePriority=f static=t", mod_id, egress_port1);
    ...
    /* Set the egress SC to have a 64 bytes mtu */
    BCM_IF_ERROR_RETURN(macsec_secure_chan_mtu_set(unit, BCM_XFLOW_MACSEC_ENCRYPT, macsec_port1.encrypt_sc_id, bcmXflowMacsecMtu1));
    ...
    destroy_ifp_to_cpu_rule (unit, eid1);
    bshell(unit, "sleep quiet 1");
    pass6_1 = (test_failed == 0) && (rx_count == expected_rx_count);
    printf("\n----------------------------------------------------------- \n");
    printf("Test6_1 = [%s]", pass6_1? "PASS":"FAIL");
    printf("\n----------------------------------------------------------- \n");
    ...
    print macsec_get_stats(unit, BCM_XFLOW_MACSEC_ENCRYPT, macsec_port1.encrypt_sc_id,
                           bcmXflowMacsecSecyStatsTxTooLongPkts, &val);
    printf("Egress TooLong pkt count:\n");
    print val;
    printf("\n");

    /* Set the egress SC to have a 1514 bytes mtu */
    BCM_IF_ERROR_RETURN(macsec_secure_chan_mtu_set(unit, BCM_XFLOW_MACSEC_ENCRYPT, macsec_port1.encrypt_sc_id, bcmXflowMacsecMtu0));
    ...
    destroy_ifp_to_cpu_rule (unit, eid1);
    bshell(unit, "sleep quiet 1");
    pass6_2 = (test_failed == 0) && (rx_count == expected_rx_count);
    printf("\n----------------------------------------------------------- \n");
    printf("Test6_2 = [%s]", pass6_2? "PASS":"FAIL");
    printf("\n----------------------------------------------------------- \n");
    ...
    pass6 = pass6_1 && pass6_2;

    print "==========  Test7: MACsec decryption - Unicast (post decryption MTU test) ==================";
    int pass7_1, pass7_2;
    port_discard(unit, ingress_port, 1);
    port_discard(unit, egress_port1, 0);
    snprintf(str, 512, "l2 add module=%d port=%d mac=00:00:00:00:00:22 vlan=10 ReplacePriority=f static=t", mod_id, ingress_port);
    ...
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    BCM_PBMP_PORT_ADD(expected_pbmp[0] , ingress_port);
    /* Set the ingress subport policy to have a 64 bytes mtu */
    BCM_IF_ERROR_RETURN(macsec_decrypt_policy_mtu_set(unit,
                                                      macsec_port1.decrypt_flow.policy_id,
                                                      bcmXflowMacsecMtu1));
    bshell(unit, str);
    destroy_ifp_to_cpu_rule (unit, eid1);
    bshell(unit, "sleep quiet 1");
    pass7_1 = (test_failed == 0) && (rx_count == expected_rx_count);
    printf("\n----------------------------------------------------------- \n");
    printf("Test7_1 = [%s]", pass7_1? "PASS":"FAIL");
    printf("\n----------------------------------------------------------- \n");
    ...
    /* There is no MTU failure counter */
    ...
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    BCM_PBMP_PORT_ADD(expected_pbmp[0] , ingress_port);
    /* Set the ingress subport policy to have a 1514 bytes mtu */
    BCM_IF_ERROR_RETURN(macsec_decrypt_policy_mtu_set(unit,
                                                      macsec_port1.decrypt_flow.policy_id,
                                                      bcmXflowMacsecMtu0));
    create_ifp_to_cpu_rule(unit, _group_config.group, &eid1, ingress_port);
    ...
    destroy_ifp_to_cpu_rule (unit, eid1);
    bshell(unit, "sleep quiet 1");
    pass7_2 = (test_failed == 0) && (rx_count == expected_rx_count);
    printf("\n----------------------------------------------------------- \n");
    printf("Test7_2 = [%s]", pass7_2? "PASS":"FAIL");
    printf("\n----------------------------------------------------------- \n");
