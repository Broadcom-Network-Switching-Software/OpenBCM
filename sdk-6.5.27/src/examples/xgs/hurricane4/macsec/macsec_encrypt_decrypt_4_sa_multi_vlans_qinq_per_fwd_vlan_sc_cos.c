 ...
 *
 *  Usage    : BCM.0> cint macsec_encrypt_decrypt_4_sa_multi_vlans_qinq_per_fwd_vlan_sc.c
 *
 *  config   : hr4_sdk6_macsec_config.bcm
 *
 *  Log file : macsec_encrypt_decrypt_4_sa_multi_vlans_qinq_per_fwd_vlan_sc_log.txt
 *
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *    QinQ is configured on egress_port1.
 *
 *    Detailed steps done in the CINT script:
 *    =======================================
 *    1) Step1 - Test Setup (Done in test_setup())
 *    ============================================
 *      1.1) Configure QinQ (access port: a_port1, network port: p_port)
 *           c_vlan = 10, p_vlan = 100, intenal fwd_vlan = 1000
 *           1.1.1) Assign port class x for egress vlan xlate to a_port1
 *           1.1.2) Assign port class y for egress vlan xlate to p_port
 *           1.1.3) Map ingress packet from a_port1 with c_vlan to fwd_vlan
 *           1.1.4) Map ingress packet from p_port with <p_vlan, c_vlan> to fwd_vlan
 *           1.1.5) Delete otag for double-tagged packet forwarded to a_port1 (port class x)
 *           1.1.6) Add otag with p_vlan for packet in fwd_vlan egressing to p_port (port class y)
 *
 *      1.2) Configure VFP rules, IFP rules and EFP rules for MACsec
 *           1.2.1) Install VFP rules to avoid vlan checking for 1st pass MACsec decryption packets
 *           1.2.2) Install an IFP rule to redirect MACsec packets received from
 *                  MACsec port to one of MACsec decrypt engine ports (decrypt-1st pass)
 *           1.2.3) Install an EFP rule to modify SVTAG in the packet (decrypt-1st pass)
 *           1.2.4) Install VFP/IFP rules to map int_pri/cng in SVTAG to encrypt-2nd pass internal priority/cng.
 *           1.2.5) Install an IFP rule to assign Modidbase for the encrypted
 *                  packet from MACsec engine (encrypt-2nd pass).
 *
 *      1.3) Configure MACsec on macsec-enabled port (egress_port1)
 *           1.3.1) Enable MACsec on the port
 *           1.3.2) Add a flex flow-based L2 entry to map SVTAG.DEST_PORT to DGPP (encrypt-2nd pass IPIPE).
 *           1.3.3) Set the port class to 1 (to be used as a qualifier for the VFP rule in 1.2.1)
 *           1.3.4) Set the port class to 1 (to be used as a qualifier for the IFP rule in 1.2.2)
 *           1.3.5) Configure a L3_IPMC egress object with L3_FLAGS3_L2_TAG_ONLY
 *           1.3.6) Creat a per-port egress SC with encryption disabled
 *
 *      1.4) Configure vlan 1000 (ingress_port and egress_port1)
 *           1.4.1) Create an ipmc remap group and map the original ipmc group of the vlan to the corresponding remap group.
 ...
 *                  (b) L3_IPMC egress object created in 1.3.4 for port x will be chained under port 65/66.
 *
 *      1.5) For MACsec-enabled port (egress_port1) and fwd_vlan 1000
 *           1.5.1) enable and configure TPID0 and TPID1
 *           1.5.2) For decryption direction
 *                  1.5.2.1) Configure a decryption SC and 4 SA's
 *                  1.5.2.2) Configure a decryption policy
 *                  1.5.2.3) Configure a decryption flow (port, outer and inner vlan)
 *           1.5.3) For encryption direction:
 *                  1.5.3.1) Configure an encryption SC and 4 SA's (secTag offset = 20)
 ...
 *           3.1.1) Test unicast MACsec encryption
 *                  3.1.1.1) Add L2 entries for unicast L2 switching <fwd_vlan, dmac> to egress_port1
 *                  3.1.1.2) Unicast packet is received from ingress port
 *                           Expected result:
 *                           The packet is doubled-tagged, encrypted with the egress SC for <fwd_vlan, egress_port1> and forwarded to egress_port1.
 *                           SecTag is added after otag/itag.
 *           3.1.2) Test unicast MACsec decryption
 *                  3.1.2.1) Add L2 entries for unicast L2 switching to ingress_port
 *                  3.1.2.2) Unicast packet encrypted for ingress SC for <ovid, ivid, egress_port1> is received from egress_port1
 *                           Expected result:
 *                           The packet is decrypted, outer-tagged removed, and forwarded to ingress_port
 */
...
/**** global variables ****/
int vfp_decrypt_stat_id[3];
int ifp_encrypt_stat_id;
int ifp_decrypt_stat_id[3];
...
struct macsec_decrypt_flow_s {
    ...
    bcm_vlan_t  ovlan;
    bcm_vlan_t  ovlan_mask;
    bcm_vlan_t  ivlan;
    bcm_vlan_t  ivlan_mask;
                          ...
                          bcm_xflow_macsec_secure_chan_id_t *sc_id)
{
    ...
    int sectag_off  = 20; /* secTag inserted after otag and itag */
...
macsec_decrypt_flow_create(int unit, macsec_decrypt_flow_s *decrypt_flow, bcm_xflow_macsec_policy_id_t policy_id)
{
    ...
    uint32 tpid = BCM_XFLOW_MACSEC_FLOW_TPID_SEL_0 | BCM_XFLOW_MACSEC_FLOW_TPID_SEL_1;
    uint32 tpid_mask = 0x0F;
    /* xflow_macsec_vlan_mpls_tag_status_t vlan_mpls_tag_status = bcmXflowMacsecTagAny; */
    xflow_macsec_vlan_mpls_tag_status_t vlan_mpls_tag_status = bcmXflowMacsecTagDoubleVlan;
    ...
    dec_flow.udf.first_vlan = decrypt_flow->ovlan;
    dec_flow.udf_mask.first_vlan = decrypt_flow->ovlan_mask;
    dec_flow.udf.second_vlan = decrypt_flow->ivlan;
    dec_flow.udf_mask.second_vlan = decrypt_flow->ivlan_mask;
...
bcm_error_t
assign_egress_sc_by_efp(int unit, bcm_port_t port, bcm_vlan_t ovid, bcm_vlan_t ivid, bcm_xflow_macsec_secure_chan_id_t sc_id,
...
macsec_port_config(int unit, macsec_port_s *macsec_port)
{
    ...
    bshell(unit, "s isec_tpid0 val=0x8100 EN=1");
    bshell(unit, "s isec_tpid1 val=0x8100 EN=1");

    /* macsec port: class 1 or 2 */
    BCM_IF_ERROR_RETURN(bcm_port_class_set(unit, port, bcmPortClassFieldLookup, macsec_port->macsec_port_class));
...
macsec_vlan_port_config_sc_sa_setup(int unit, macsec_vlan_port_s *macsec_vlan_port)
{
    ...
    bcm_vlan_t p_vlan = macsec_vlan_port->p_vlan;
    bcm_vlan_t c_vlan = macsec_vlan_port->c_vlan;
    int  pri = isec_sc_tcam_index++;
    int  i;

    printf("\n-------------------------------------------------------------\n");
    printf("Start macsec_vlan_port_config() for vlan=%d, port=%d, p_vlan=%d\n", vlan, port, p_vlan);
    printf("---------------------------------------------------------------\n");
    ...
    BCM_IF_ERROR_RETURN(assign_egress_sc_by_efp(unit, port, p_vlan, c_vlan, macsec_vlan_port->encrypt_sc_id, bcmColorGreen, macsec_efp_group, &macsec_vlan_port->efp_encrypt_stat_id[0]));
    BCM_IF_ERROR_RETURN(assign_egress_sc_by_efp(unit, port, p_vlan, c_vlan, macsec_vlan_port->encrypt_sc_id, bcmColorYellow, macsec_efp_group, &macsec_vlan_port->efp_encrypt_stat_id[1]));
    BCM_IF_ERROR_RETURN(assign_egress_sc_by_efp(unit, port, p_vlan, c_vlan, macsec_vlan_port->encrypt_sc_id, bcmColorRed, macsec_efp_group, &macsec_vlan_port->efp_encrypt_stat_id[2]));
...
macsec_vlan_port_config_flow_setup(int unit, macsec_vlan_port_s *macsec_vlan_port, bcm_xflow_macsec_policy_id_t policy_id)
{
    ...
    flow->ovlan = p_vlan;
    flow->ovlan_mask = 0xfff;
    flow->ivlan = c_vlan;
    flow->ivlan_mask = 0xfff;
    flow->pri = pri;
    if (macsec_vlan_port->flags & VLAN_PORT_MACSEC_DISABLE_FLAG)
...
bcm_error_t
vfp_setup(int unit, int *stat_id) {
    bcm_field_group_config_t group_cfg;
    bcm_field_entry_t entry;
    bcm_field_stat_t stat_type[]  = { bcmFieldStatBytes, bcmFieldStatPackets };
    char str[512];
    ...
    BCM_FIELD_ASET_ADD(group_cfg.aset, bcmFieldActionDoNotCheckVlan);
    snprintf(str, 512,"%s","> VFP rules installed (decrypt).");

    BCM_IF_ERROR_RETURN( bcm_field_group_config_create(unit, &group_cfg));

    /* create VFP entry to avoid DoNotCheckVlan for 2nd pass decryption */
    BCM_IF_ERROR_RETURN( bcm_field_entry_create(unit, group_cfg.group, &entry));
    ...
    /* create VFP entry to DoNotCheckVlan for 1st pass decryption */
    BCM_IF_ERROR_RETURN( bcm_field_entry_create(unit, group_cfg.group, &entry));
    BCM_IF_ERROR_RETURN( bcm_field_qualify_InterfaceClassPort(unit, entry, macsec_port_class1, BCM_FIELD_EXACT_MATCH_MASK));
    BCM_IF_ERROR_RETURN( bcm_field_action_add(unit, entry, bcmFieldActionDoNotCheckVlan, 0, 0));
    ...
    BCM_IF_ERROR_RETURN( bcm_field_entry_prio_set(unit, entry, 1)); /* lower priority */
    BCM_IF_ERROR_RETURN( bcm_field_stat_create(unit, group_cfg.group,
                                               sizeof(stat_type)/sizeof(stat_type[0]), stat_type, stat_id+2));
    BCM_IF_ERROR_RETURN( bcm_field_entry_stat_attach(unit, entry, *(stat_id+2)));
    BCM_IF_ERROR_RETURN( bcm_field_entry_install(unit, entry));
...
/*********************** QinQ switching configuration **************************/
bcm_port_t a_port1;
bcm_port_t p_port;
uint32 port_group_c;
uint32 port_group_p;
bcm_vlan_t c_vlan = 10;
bcm_vlan_t p_vlan = 100;
bcm_vlan_t fwd_vlan = 1000;

/* Configure vlan xlate keys to derive the forwarding VFI */
bcm_error_t
vlan_xlate_network_setup(int unit, bcm_port_t port, bcm_vlan_t p_vlan,
                         bcm_vlan_t c_vlan, bcm_vlan_t forwarding_vlan)
{
    bcm_error_t rv = BCM_E_NONE;
    bcm_gport_t gport;
    ...
    /* Use <port, ovid, ivid> as key in ingress adaptation lookup */
    rv = bcm_vlan_control_port_set(unit, port, bcmVlanPortTranslateKeyFirst,
    ...
    action.new_outer_vlan = forwarding_vlan;
    rv = bcm_vlan_translate_action_add(unit, gport,
                                       bcmVlanTranslateKeyPortDouble, p_vlan, c_vlan, &action);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_translate_action_add: %s\n", bcm_errmsg(rv));
        return rv;
    }

    printf("vlan_xlate_network_setup()\n");
    return BCM_E_NONE;
}

bcm_error_t
egress_vlan_xlate_network_setup(int unit, bcm_port_t port, bcm_vlan_t forwarding_vlan,
                                bcm_vlan_t p_vlan)
{
    bcm_error_t rv = BCM_E_NONE;
    bcm_gport_t gport;
    ...
    action.dt_outer = bcmVlanActionReplace;
    action.dt_inner = bcmVlanActionNone;
    action.new_outer_vlan = p_vlan;
    rv = bcm_vlan_translate_egress_action_add(unit, port_group_p, forwarding_vlan, BCM_VLAN_NONE, &action);
    ...
    printf("egress_vlan_xlate_network_setup()\n");
    return BCM_E_NONE;
}
...
bcm_error_t
vlan_xlate_access_setup(int unit, bcm_port_t port, bcm_vlan_t c_vlan, bcm_vlan_t forwarding_vlan)
{
    bcm_error_t rv = BCM_E_NONE;
    bcm_gport_t gport;
    ...
    action.new_outer_vlan = forwarding_vlan;
    rv = bcm_vlan_translate_action_add(unit, gport,
                                       bcmVlanTranslateKeyPortOuter, c_vlan, BCM_VLAN_NONE, &action);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_translate_action_add: %s\n", bcm_errmsg(rv));
        return rv;
    }

    printf("vlan_xlate_access_setup()\n");
    return BCM_E_NONE;
}

bcm_error_t
egress_vlan_xlate_access_setup(int unit, bcm_port_t port, bcm_vlan_t forwarding_vlan)
{
    bcm_error_t rv = BCM_E_NONE;
    bcm_gport_t gport;
    ...
    rv = bcm_vlan_translate_egress_action_add(unit, port_group_c, forwarding_vlan, BCM_VLAN_NONE, &action);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_vlan_translate_egress_action_add: %s\n", bcm_errmsg(rv));
        return rv;
    }

    printf("egress_vlan_xlate_access_setup()\n");
    return BCM_E_NONE;
}
...
/* QinQ switch configuration function */
bcm_error_t
config_qinq(int unit)
{
    bcm_error_t rv = BCM_E_NONE;

    a_port1 = ingress_port;
    p_port = egress_port1;
    port_group_c = 3;
    port_group_p = 4;

    /* Assign per port portGroup */
    rv = bcm_port_class_set(unit, a_port1, bcmPortClassVlanTranslateEgress, port_group_c);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_port_class_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
    rv = bcm_port_class_set(unit, p_port, bcmPortClassVlanTranslateEgress, port_group_p);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcm_port_class_set: %s\n", bcm_errmsg(rv));
        return rv;
    }

    /* Map ingress packet from a_port1 with c_vlan to fwd_vlan */
    rv = vlan_xlate_access_setup(unit, a_port1, c_vlan, fwd_vlan);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vlan_xlate_access_setup(): %s\n", bcm_errmsg(rv));
        return rv;
    }

    /* Map ingress packet from p_port with <p_vlan, c_vlan> to fwd_vlan */
    rv = vlan_xlate_network_setup(unit, p_port, p_vlan, c_vlan, fwd_vlan);
    if (BCM_FAILURE(rv)) {
        printf("\nError in vlan_xlate_network_setup(): %s\n", bcm_errmsg(rv));
        return rv;
    }

    /* Delete otag for double-tagged packet forwarded to a_port1 */
    rv = egress_vlan_xlate_access_setup(unit, a_port1, fwd_vlan);
    if (BCM_FAILURE(rv)) {
        printf("\nError in egress_vlan_xlate_access_setup(): %s\n", bcm_errmsg(rv));
        return rv;
    }

    /* Add otag with p_vlan for packet in fwd_vlan egressing to p_port */
    rv = egress_vlan_xlate_network_setup(unit, p_port, fwd_vlan, p_vlan);
    if (BCM_FAILURE(rv)) {
        printf("\nError in egress_vlan_xlate_network_setup(): %s\n", bcm_errmsg(rv));
        return rv;
    }

    /* Learn mode port config */
    rv = port_learn_mode_set(unit, a_port1, BCM_PORT_LEARN_ARL | BCM_PORT_LEARN_FWD);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_learn_mode_set: %s\n", bcm_errmsg(rv));
        return rv;
    }

    rv = port_learn_mode_set(unit, p_port, BCM_PORT_LEARN_ARL | BCM_PORT_LEARN_FWD);
    if (BCM_FAILURE(rv)) {
        printf("\nError in port_learn_mode_set: %s\n", bcm_errmsg(rv));
        return rv;
    }
...
config_macsec(int unit)
{
    ...
    /* Disable vlan check for 1st pass decryption */
    BCM_IF_ERROR_RETURN(vfp_setup(unit, &vfp_decrypt_stat_id[0]));
    ...
    macsec_vlan[0].vid = fwd_vlan;   /* the internal forwarding vlan */
    ...
    */
    ...
    macsec_vlan_port[0].p_vlan = p_vlan;          /* outer-vlan     */
    macsec_vlan_port[0].c_vlan = c_vlan;          /* inner-vlan     */
...
*/
    ...
    */
    ...
    if (BCM_FAILURE((rv = config_qinq(unit)))) {
        printf("MACsec Setup Failed %s.\n", bcm_errmsg(rv));
        return -1;
    }
    ...
    /* SOURCE_TRUNK_MAP.disable_vlan_checks will cause issue in BUM replication */
/*
    rv = bcm_port_control_set(unit, egress_port1, bcmPortControlDoNotCheckVlan, 1);
    if (BCM_FAILURE(rv)) {
        printf("\nError in bcmPortControlDoNotCheckVlan: %s\n", bcm_errmsg(rv));
        return rv;
    }
*/
...
unsigned char expected_encrypted_packet1[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00, 0x00, 0x64,
    0x81, 0x00, 0x00, 0x0a, 0x88, 0xe5, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x08, 0x8b, 0x94, 0x0d, 0x0a, 0x42, 0x3e, 0xf6, 0x7e, 0x03, 0x92, 0x30, 0xce,
    0x0b, 0xbf, 0x27, 0xaa, 0x54, 0x42, 0xde, 0x93, 0x9c, 0x35, 0xad, 0x51, 0x0d, 0x5c, 0x0b, 0x0d,
    0xfb, 0x18, 0x97, 0x11, 0x7e, 0x6d, 0x6f, 0xe5, 0x75, 0xe1, 0x21, 0x2b, 0x0a, 0x46, 0x74, 0x54,
    0x4e, 0xb0, 0xec, 0xab, 0x8d, 0x54, 0xd2, 0xc0, 0xe7, 0xeb, 0x3b, 0x18, 0x30, 0x92, 0x86, 0xb4,
    0x58, 0xd1, 0x2b, 0x88, 0xdc, 0x3b, 0x8a, 0x39, 0xbd, 0xd2, 0x42, 0x77};

unsigned char expected_encrypted_packet2[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00, 0x00, 0x64,
    0x81, 0x00, 0x00, 0x0a, 0x88, 0xe5, 0x2d, 0x00, 0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x08, 0x8b, 0x6e, 0x39, 0x5f, 0xd9, 0x2c, 0x63, 0x68, 0x35, 0xb7, 0x98, 0x9a,
    0x1e, 0x2b, 0x36, 0x57, 0x66, 0x0a, 0x3a, 0x0b, 0x33, 0x84, 0xad, 0x0a, 0x39, 0x2f, 0xf9, 0x97,
    0x4c, 0xb9, 0xd1, 0xdd, 0x89, 0x13, 0x30, 0xc6, 0xe7, 0x38, 0x8d, 0xda, 0x8b, 0x87, 0x95, 0xde,
    0x3b, 0x21, 0xd2, 0xc6, 0xfe, 0x30, 0x2c, 0x8e, 0x41, 0xb0, 0x8b, 0x73, 0xb0, 0xa2, 0x57, 0x74,
    0x88, 0x20, 0x08, 0x5c, 0xa1, 0xd5, 0x10, 0x50, 0x1a, 0x96, 0x12, 0x25};

unsigned char expected_encrypted_mc_packet1[512] = {
    0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00, 0x00, 0x64,
    0x81, 0x00, 0x00, 0x0a, 0x88, 0xe5, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x08, 0x21, 0xb3, 0xe4, 0x22, 0x8c, 0x2e, 0xb5, 0xd4, 0x18, 0x56, 0xb8, 0xc1,
    0x54, 0x27, 0xa3, 0x33, 0x2e, 0xda, 0x04, 0x44, 0xe2, 0xc9, 0x1b, 0x1f, 0xb6, 0xcb, 0x25, 0x71,
    0x23, 0x31, 0x00, 0xa1, 0x8f, 0x9f, 0x36, 0x96, 0x2c, 0xdb, 0xa6, 0xb1, 0x4a, 0x9a, 0x13, 0x4c,
    0xd3, 0xb0, 0xab, 0x37, 0x38, 0x70, 0x9f, 0x0c, 0x2e, 0xb4, 0xb3, 0x20, 0xd3, 0xa1, 0x38, 0xc0,
    0x2f, 0x9e, 0x67, 0xc8, 0x75, 0xb8, 0x6e, 0x02, 0xc6, 0x8c, 0x73, 0x69};
...
test_verify(int unit)
{
    ...
    snprintf(uc_enc_pkt_1, 512,"%s","000000000022000000000001810000648100000a88e52c000000000500000000000000088b940d0a423ef67e039230ce0bbf27aa5442de939c35ad510d5c0b0dfb1897117e6d6fe575e1212b0a4674544eb0ecab8d54d2c0e7eb3b18309286b458d12b88dc3b8a39");
    snprintf(uc_enc_pkt_2, 512,"%s","000000000022000000000001810000648100000a88e52d000000000500000000000000088b6e395fd92c636835b7989a1e2b3657660a3a0b3384ad0a392ff9974cb9d1dd891330c6e7388dda8b8795de3b21d2c6fe302c8e41b08b73b0a257748820085ca1d51050");
    ...
    snprintf(str, 512, "l2 add module=%d port=%d mac=00:00:00:00:00:22 vlan=1000 ReplacePriority=f static=t", mod_id, egress_port1);
    bshell(unit, str);
    ...
    snprintf(str, 512, "l2 add module=%d port=%d mac=00:00:00:00:00:22 vlan=1000 ReplacePriority=f static=t", mod_id, ingress_port);
    bshell(unit, str);
