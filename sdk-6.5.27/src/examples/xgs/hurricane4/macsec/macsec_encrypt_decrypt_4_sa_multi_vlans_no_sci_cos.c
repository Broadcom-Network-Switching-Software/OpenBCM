 ...
 *
 *  Usage    : BCM.0> cint macsec_encrypt_decrypt_4_sa_multi_vlans_no_sci_cos.c
 *
 *  config   : hr4_sdk6_macsec_config.bcm
 *
 *  Log file : macsec_encrypt_decrypt_4_sa_multi_vlans_no_sci_cos_log.txt
 *
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *    Both P2P MACsec CA (Connectivity Association) and EndStation MACsec are tested.
 *
 ...
 *
 *      1.3) Configure vlan 10, vlan 11, vlan 12, vlan 13, and vlan 14 with 2 ports (ingress_port and egress_port1)
 ...
 *
 *      1.6) For vlan 13 (P2P CA)
 *           1.6.1) For decryption direction
 *                  1.6.1.1) Configure a decryption SC and 4 SA's
 *                  1.6.1.2) Configure a decryption policy with P2P flag and SCI
 *                  1.6.1.3) Configure a decryption flow
 *           1.6.2) For encryption direction:
 *                  1.6.2.1) Configure an encryption SC and 4 SA's.
 *                           SC is marked not to include SCI in SecTag
 *                  1.6.2.2) Install EFP rules to modify SVTAG of the packet (encrypt-1st pass).
 *                           The subport id is passed in to MACsec engine through SVTAG.
 *                           Packet Cng (2 bits) are passed in SVTAG also.
 *
 *      1.7) For vlan 14 (where EndStation's reside)
 *           1.7.1) For decryption direction
 *                  1.7.1.1) Configure a decryption SC and 4 SA's
 *                  1.7.1.2) Configure a decryption policy
 *                  1.7.1.3) Configure a decryption flow
 *           1.7.2) For encryption direction:
 *                  1.7.2.1) Configure an encryption SC and 4 SA's.
 *                           SC is marked not to include SCI and ES = 1 in SecTag
 *                  1.7.2.2) Install EFP rules to modify SVTAG of the packet (encrypt-1st pass).
 *                           The subport id is passed in to MACsec engine through SVTAG.
 *                           Packet Cng (2 bits) are passed in SVTAG also.
 *           Note thet an ES may be in any of the above CA's which may be P2P or not P2P.
 *           For simplicity, this is not tested.
 *
 *      1.8) Add mapping from internal priority (4 bits) to SVTAG.PRI (encrypt-1st pass EPIPE)
 *
 ...
 *      3.3) Test vlan 12 traffics (bypass)
 *           Expected result:
 *           3.3.1) Encrypt direction:
 *                  Vlan 12 egress packets forwarded to egress_port1 will not be encrypted
 *           3.3.2) Decryption direction:
 *                  Vlan 12 ingress packets received from egress_port1 will be treated as management packet and bypass MACsec.
 *      3.4) Test vlan 13 traffics (P2P CA)
 *           Expected result:
 *           3.4.1) Encrypt direction:
 *                  Vlan 13 egress packets forwarded to egress_port1 will be encrypted without SCI (SC=0) in SecTag
 *           3.4.1) Decrypt direction:
 *                  Vlan 13 ingress encrypted packets received from egress_port1 without SCI (SC=0) in SecTag will be decrypted and forwarded to ingress_port
 *      3.5) Test vlan 14 traffics (EndStation)
 *           Expected result:
 *           3.5.1) Encrypt direction:
 *                  Vlan 14 egress packets forwarded to egress_port1 will be encrypted without SCI (SC=0) and ES=1 in SecTag
 *           3.5.1) Decrypt direction:
 *                  Vlan 14 ingress encrypted packets received from egress_port1 without SCI (SC=0) and ES=1 in SecTag will be decrypted and
 *                  forwarded to ingress_port
 */
                             ...
                             bcm_xflow_macsec_policy_id_t *pol_id)
{
    ...
    /* SCI for point-to-polint MACsec CA */
...
{
bcm_error_t rv = BCM_E_NONE;
bcm_vlan_t  vid1 = 10, vid2 = 11, vid3 = 12, vid4 = 13, vid5 = 14;
bcm_multicast_t vlan_ipmc_group;
    ...
    printf("Configure macsec_vlan[3]\n");
    macsec_vlan_s_init(&macsec_vlan[3]);
    macsec_vlan[3].vid = vid4;
    macsec_vlan[3].port[0] = ingress_port;
    macsec_vlan[3].port[1] = egress_port1;
    macsec_vlan[3].num_of_ports = 2;
    BCM_IF_ERROR_RETURN(macsec_vlan_config(unit, &macsec_vlan[3]));

    printf("Configure macsec_vlan[4]\n");
    macsec_vlan_s_init(&macsec_vlan[4]);
    macsec_vlan[4].vid = vid5;
    macsec_vlan[4].port[0] = ingress_port;
    macsec_vlan[4].port[1] = egress_port1;
    macsec_vlan[4].num_of_ports = 2;
    BCM_IF_ERROR_RETURN(macsec_vlan_config(unit, &macsec_vlan[4]));
    ...
    uint64 p2p_sci;
    COMPILER_64_SET(p2p_sci, 0x00, 0x0a);
    BCM_IF_ERROR_RETURN(macsec_decrypt_policy_create(unit, p2p_sci,
                                                     BCM_XFLOW_MACSEC_DECRYPT_POLICY_TAGGED_CONTROL_PORT_ENABLE |
                                                     BCM_XFLOW_MACSEC_DECRYPT_POLICY_UNTAGGED_CONTROL_PORT_ENABLE,
    ...
    */
    ...
    printf("Configure SC/SA for macsec_vlan_port[3]\n");
    macsec_vlan_port_s_init(&macsec_vlan_port[3]);
    macsec_vlan_port[3].macsec_port_index = 0;     /* macsec_port[0] */
    macsec_vlan_port[3].macsec_vlan_index = 3;     /* macsec_vlan[3] */
    macsec_vlan_port[3].encrypt_sci = p2p_sci;
    macsec_vlan_port[3].decrypt_sci = p2p_sci;
    macsec_vlan_port[3].flags = VLAN_PORT_MACSEC_NO_SCI_P2P_FLAG;
    BCM_IF_ERROR_RETURN(macsec_vlan_port_config_sc_sa_setup(unit, &macsec_vlan_port[3]));

    /*  Bind MACsec <policy, sci> to SC */
    sc_id = macsec_vlan_port[3].decrypt_sc_id;
    BCM_IF_ERROR_RETURN(macsec_secure_chan_decrypt_policy_bind(unit, sc_id, policy_id[1]));

    printf("Configure decrypt flow for macsec_vlan_port[3]\n");
    BCM_IF_ERROR_RETURN(macsec_vlan_port_config_flow_setup(unit, &macsec_vlan_port[3], policy_id[1]));

    printf("Configure SC/SA for macsec_vlan_port[4]\n");
    uint64 es_sci;
    COMPILER_64_SET(es_sci, 0x00000000, 0x00010001); /* ES (SMAC=0x01) ==> SCI=0x0000000000010001 based on 802.1AE standard) */

    macsec_vlan_port_s_init(&macsec_vlan_port[4]);
    macsec_vlan_port[4].macsec_port_index = 0;     /* macsec_port[0] */
    macsec_vlan_port[4].macsec_vlan_index = 4;     /* macsec_vlan[4] */
    macsec_vlan_port[4].encrypt_sci = es_sci;
    macsec_vlan_port[4].decrypt_sci = es_sci;
    macsec_vlan_port[4].flags = VLAN_PORT_MACSEC_NO_SCI_ES_FLAG;
    BCM_IF_ERROR_RETURN(macsec_vlan_port_config_sc_sa_setup(unit, &macsec_vlan_port[4]));

    /*  Bind MACsec <policy, sci> to SC */
    sc_id = macsec_vlan_port[4].decrypt_sc_id;
    BCM_IF_ERROR_RETURN(macsec_secure_chan_decrypt_policy_bind(unit, sc_id, policy_id[0]));

    printf("Configure decrypt flow for macsec_vlan_port[4]\n");
    BCM_IF_ERROR_RETURN(macsec_vlan_port_config_flow_setup(unit, &macsec_vlan_port[4], policy_id[0]));
...
unsigned char expected_encrypted_packet4[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00,
    0x00, 0x0d, 0x88, 0xe5, 0x0c, 0x00, 0x00, 0x00, 0x00, 0x05, 0xf7, 0x5b, 0xda, 0x7f,
    0x76, 0x37, 0xaa, 0xeb, 0x37, 0x78, 0xb6, 0x35, 0x0c, 0xac, 0x99, 0x8f, 0x7a, 0x10,
    0xf5, 0xc0, 0x80, 0x26, 0x94, 0x95, 0x4d, 0xe4, 0xc6, 0x07, 0xf6, 0xdd, 0xf8, 0x3e,
    0x23, 0x7b, 0x80, 0xeb, 0xe7, 0xad, 0x34, 0xaa, 0xca, 0x71, 0x4c, 0xca, 0xd1, 0xbd,
    0xa1, 0x13, 0x80, 0xb6, 0xa2, 0x6e, 0xb6, 0x59, 0x62, 0xdb, 0x70, 0x7f, 0xa2, 0xd0,
    0xbc, 0xdc, 0x22, 0x74, 0xe5, 0x5d, 0x48, 0x91, 0x1d, 0x98, 0x5f, 0x30};

unsigned char expected_decrypted_packet3[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00,
    0x00, 0x0d, 0xff, 0xff, 0x45, 0x78, 0x00, 0x2e, 0x00, 0x00, 0x00, 0x00, 0x40, 0xff,
    0x84, 0x57, 0x14, 0x00, 0x00, 0x01, 0xe1, 0x00, 0x00, 0x01, 0x14, 0x15, 0x16, 0x17,
    0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25,
    0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2d, 0x0a, 0x03, 0xab, 0x8f, 0x06,
    0x2c, 0x02};

unsigned char expected_encrypted_packet5[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00,
    0x00, 0x0e, 0x88, 0xe5, 0x4c, 0x00, 0x00, 0x00, 0x00, 0x05, 0x85, 0x03, 0xb0, 0xf3,
    0xda, 0x95, 0x28, 0x45, 0xb5, 0xf9, 0x65, 0x4d, 0x8f, 0xd9, 0xba, 0x34, 0x02, 0x75,
    0x8f, 0x08, 0xce, 0x74, 0xbe, 0x94, 0xc8, 0xfb, 0x98, 0x7a, 0xcd, 0xbc, 0xeb, 0xdc,
    0xfe, 0x4c, 0xd0, 0x23, 0x00, 0x0d, 0x25, 0x0f, 0xda, 0xce, 0x54, 0xc9, 0xf3, 0xa5,
    0x70, 0x98, 0x4f, 0x0e, 0xd8, 0x28, 0xe1, 0xb4, 0xe5, 0x5e, 0x42, 0x46, 0x9f, 0x80,
    0xbb, 0x00, 0xc1, 0x97, 0x2b, 0x5c, 0xda, 0xff, 0x4e, 0x6a, 0xc7, 0x5d};

unsigned char expected_decrypted_packet4[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00,
    0x00, 0x0e, 0xff, 0xff, 0x45, 0x78, 0x00, 0x2e, 0x00, 0x00, 0x00, 0x00, 0x40, 0xff,
    0x84, 0x57, 0x14, 0x00, 0x00, 0x01, 0xe1, 0x00, 0x00, 0x01, 0x14, 0x15, 0x16, 0x17,
    0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25,
    0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2d, 0x0a, 0x03, 0xab, 0x4c, 0xd6,
    0x45, 0x81};
...
test_verify(int unit)
{
    ...
    char mc_pkt[512];
    char uc_pkt_for_p2p_macsec[512];
    char uc_pkt_from_es_macsec[512];
    char uc_enc_pkt_for_p2p_macsec[512];
    char uc_enc_pkt_from_es_macsec[512];
    ...
    snprintf(uc_pkt_for_p2p_macsec, 512,"%s","0000000000220000000000018100000dffff4578002E0000000040FF845714000001E10000011415161718191A1B1C1D1E1F202122232425262728292A2B2C2D2D0A03AB");

    snprintf(uc_enc_pkt_for_p2p_macsec, 512,"%s","0000000000220000000000018100000d88e50c0000000005f75bda7f7637aaeb3778b6350cac998f7a10f5c0802694954de4c607f6ddf83e237b80ebe7ad34aaca714ccad1bda11380b6a26eb65962db707fa2d0bcdc2274e55d4891");

    snprintf(uc_pkt_from_es_macsec, 512,"%s","0000000000220000000000018100000effff4578002E0000000040FF845714000001E10000011415161718191A1B1C1D1E1F202122232425262728292A2B2C2D2D0A03AB");

    snprintf(uc_enc_pkt_from_es_macsec, 512,"%s","0000000000220000000000018100000e88e54c00000000058503b0f3da952845b5f9654d8fd9ba3402758f08ce74be94c8fb987acdbcebdcfe4cd023000d250fdace54c9f3a570984f0ed828e1b4e55e42469f80bb00c1972b5cdaff");
    ...
    print "==========  Test10: MACsec p2p Encryption - Unicast ==================";
    port_discard(unit, ingress_port, 0);
    port_discard(unit, egress_port1, 1);
    snprintf(str, 512, "l2 add module=%d port=%d mac=00:00:00:00:00:22 vlan=13 ReplacePriority=f static=t", mod_id, egress_port1);
    bshell(unit, str);
    printf(">>> Input Unicast packet @ port-%d:\n", ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x%s; sleep quiet 1", ingress_port, uc_pkt_for_p2p_macsec);
    ...
    expected_rx_count = 1;
    packet_patterns = 1;
    expected_packets[0]  = expected_encrypted_packet4;
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    BCM_PBMP_PORT_ADD(expected_pbmp[0] , egress_port1);
    bshell(unit, str);
    bshell(0,"dmirror 2 mode=off");
    ...
    macsec_get_stats(unit, BCM_XFLOW_MACSEC_ENCRYPT, macsec_vlan_port[3].encrypt_sc_id,
                     bcmXflowMacsecSecyTxSCStatsEncryptedPkts, &val);
    printf("Encrypted pkt count:\n");
    print val;
    printf("\n");
    macsec_get_stats(unit, BCM_XFLOW_MACSEC_DECRYPT, macsec_vlan_port[3].decrypt_sc_id,
                     bcmXflowMacsecSecyRxSCStatsOKPkts, &val);
    printf("Decrypted pkt count:\n");
    print val;
    printf("\n");
    ...
    print "==========  Test11: MACsec p2p decryption - Unicast ==================";
    port_discard(unit, ingress_port, 1);
    port_discard(unit, egress_port1, 0);
    snprintf(str, 512, "l2 add module=%d port=%d mac=00:00:00:00:00:22 vlan=13 ReplacePriority=f static=t", mod_id, ingress_port);
    bshell(unit, str);
    printf(">>> Input Unicast packet @ port-%d:\n", egress_port1);
    snprintf(str, 512, "tx 1 pbm=%d data=0x%s; sleep quiet 1", egress_port1, uc_enc_pkt_for_p2p_macsec);
    ...
    expected_rx_count = 1;
    packet_patterns = 1;
    expected_packets[0]  = expected_decrypted_packet3;
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    BCM_PBMP_PORT_ADD(expected_pbmp[0] , ingress_port);
    bshell(unit, str);
    bshell(0,"dmirror 1 mode=off");
    ...
    print "==========  Test12: MACsec ES Encryption - Unicast ==================";
    port_discard(unit, ingress_port, 0);
    port_discard(unit, egress_port1, 1);
    snprintf(str, 512, "l2 add module=%d port=%d mac=00:00:00:00:00:22 vlan=14 ReplacePriority=f static=t", mod_id, egress_port1);
    bshell(unit, str);
    printf(">>> Input Unicast packet @ port-%d:\n", ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x%s; sleep quiet 1", ingress_port, uc_pkt_from_es_macsec);
    ...
    expected_rx_count = 1;
    packet_patterns = 1;
    expected_packets[0]  = expected_encrypted_packet5;
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    BCM_PBMP_PORT_ADD(expected_pbmp[0] , egress_port1);
    bshell(unit, str);
    bshell(0,"dmirror 2 mode=off");
    ...
    macsec_get_stats(unit, BCM_XFLOW_MACSEC_ENCRYPT, macsec_vlan_port[4].encrypt_sc_id,
                     bcmXflowMacsecSecyTxSCStatsEncryptedPkts, &val);
    printf("Encrypted pkt count:\n");
    print val;
    printf("\n");
    macsec_get_stats(unit, BCM_XFLOW_MACSEC_DECRYPT, macsec_vlan_port[4].decrypt_sc_id,
                     bcmXflowMacsecSecyRxSCStatsOKPkts, &val);
    printf("Decrypted pkt count:\n");
    print val;
    printf("\n");

    print "==========  Test13: MACsec ES decryption - Unicast ==================";
    port_discard(unit, ingress_port, 1);
    port_discard(unit, egress_port1, 0);
    snprintf(str, 512, "l2 add module=%d port=%d mac=00:00:00:00:00:22 vlan=14 ReplacePriority=f static=t", mod_id, ingress_port);
    bshell(unit, str);
    printf(">>> Input Unicast packet @ port-%d:\n", egress_port1);
    snprintf(str, 512, "tx 1 pbm=%d data=0x%s; sleep quiet 1", egress_port1, uc_enc_pkt_from_es_macsec);
    ...
    expected_rx_count = 1;
    packet_patterns = 1;
    expected_packets[0]  = expected_decrypted_packet4;
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    BCM_PBMP_PORT_ADD(expected_pbmp[0] , ingress_port);
    bshell(unit, str);
    bshell(0,"dmirror 1 mode=off");
