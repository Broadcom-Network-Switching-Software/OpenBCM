 ...
 *
 *  Usage    : BCM.0> cint macsec_encrypt_decrypt_replay_prot_cos.c
 *
 *  config   : hr4_sdk6_macsec_config.bcm
 *
 *  Log file : macsec_encrypt_decrypt_replay_proto_cos_log.txt
 *
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *                  1.3.2.1) Configure a decryption SC/SA with replay_window=0
 ...
 *     3.4) Test encrypted packet replay
 *           3.2.2) Same encrypted unicast packet in 3.2) is received from egress_port1
 *                  Expected result:
 *                  The packet is dropped by replay protection
 *
*/
...
void
macsec_ifp_stat_print(int unit, int flags)
{
    uint64 val64;

    if (flags == BCM_XFLOW_MACSEC_ENCRYPT) {
        bcm_field_stat_get(unit, ifp_encrypt_stat_id, bcmFieldStatPackets, &val64);
        printf("IFP(Encryption) stat: $val64\n");
    } else if (flags == BCM_XFLOW_MACSEC_DECRYPT) {
        bcm_field_stat_get(unit, ifp_decrypt_stat_id[0], bcmFieldStatPackets, &val64);
        printf("IFP(Decryption) stat[0]: $val64\n");
        bcm_field_stat_get(unit, ifp_decrypt_stat_id[1], bcmFieldStatPackets, &val64);
        printf("IFP(Decryption) stat[1]: $val64\n");
    }
}


void
macsec_efp_stat_print(int unit, int flags)
{
    uint64 val64;

    if (flags == BCM_XFLOW_MACSEC_ENCRYPT) {
        bcm_field_stat_get(unit, efp_encrypt_stat_id, bcmFieldStatPackets, &val64);
        printf("EFP(Encryption) stat: $val64\n");
    } else if (flags == BCM_XFLOW_MACSEC_DECRYPT) {
        bcm_field_stat_get(unit, efp_decrypt_stat_id, bcmFieldStatPackets, &val64);
        printf("EFP(Decryption) stat: $val64\n");
    }
}
...
/* Create a UDF to match on one 2-bytes chunk */
...
unsigned char expected_decrypted_mc_packet1[512] = {
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00,
    0x00, 0x0a, 0xff, 0xff, 0x45, 0x78, 0x00, 0x2e, 0x00, 0x00, 0x00, 0x00, 0x40, 0xff,
    0x84, 0x57, 0x14, 0x00, 0x00, 0x01, 0xe1, 0x00, 0x00, 0x01, 0x14, 0x15, 0x16, 0x17,
    0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25,
    0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2d, 0x0a, 0x03, 0xab, 0x4e, 0x6d,
    0xc1, 0xee};
...
test_verify(int unit)
{
    ...
    snprintf(mc_enc_pkt_1, 512,"%s","ffffffffffff0000000000018100000A88e52C0000000006000000000000000821b3e4228c2eb5d41856b8c15427a3332eda0444e2c91b1fb6cb2571233100a18f9f36962cdba6b14a9a134cd3b0ab3738709f0c5c48a682ad64b5a212cbe59fc41ac561");
    ...
    print "==========  Test3: MACsec decryption - multicast ==================";
    port_discard(unit, ingress_port, 1);
    port_discard(unit, egress_port1, 0);

    printf(">>> Input Multicast packet @ port-%d:\n", egress_port1);
    snprintf(str, 512, "tx 1 pbm=%d data=0x%s; sleep quiet 1", egress_port1, mc_enc_pkt_1);
    ...
    expected_rx_count = 1;
    packet_patterns = 1;
    expected_packets[0]  = expected_decrypted_mc_packet1;
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    ...
    print "==========  Test4: MACsec decryption - Out of Window Unicast ==================";
    port_discard(unit, ingress_port, 1);
    port_discard(unit, egress_port1, 0);
    ...
    print macsec_get_stats(unit, BCM_XFLOW_MACSEC_DECRYPT, macsec_port1.decrypt_sc_id,
                           bcmXflowMacsecSecyRxSCStatsLatePkts, &val);
    printf("Late pkt count:\n");
    print val;
    printf("\n");
