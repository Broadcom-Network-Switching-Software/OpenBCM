 ...
 *
 *  Usage    : BCM.0> cint macsec_encrypt_decrypt_drop_pass_untagged_cos.c
 *
 *  config   : hr4_sdk6_macsec_config.bcm
 *
 *  Log file : macsec_encrypt_decrypt_drop_pass_untagged_cos_log.txt
 *
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *                  1.3.2.2) Configure a policy with control port enabled and configure mapping from <ms_subport, SCI> to SC
 *                  1.3.2.3) Configure a decryption flow for SecTag packets and bind to the decryption policy in 1.3.2.2)
 *                  1.3.2.4) Configure a policy with uncontrolled port enabled
 *                  1.3.2.5) Configure a flow for clear text packets and bind to the policy in 1.3.2.4)
 *                  1.3.2.6) Configure a policy with uncontrolled port disabled
 ...
 *    ======================================================
 *      3.1) Test clear text packets received from egress_port1
 *           Expected result:
 *           The clear text packet is forwarded to ingress_port
 *      3.2) Bind clear text flow to policy with uncontrolled port disabled 1.4.2.6) and repeat 3.1)
 *           Expected result:
 *           The clear text packet is dropped
*/
...
/*
    ...
    BCM_IF_ERROR_RETURN(multicast_port_add(unit, ipmc_group, ingress_port, 0));
    BCM_IF_ERROR_RETURN(multicast_port_add(unit, ipmc_group, macsec_port1.port, egr_obj_id1));
*/
