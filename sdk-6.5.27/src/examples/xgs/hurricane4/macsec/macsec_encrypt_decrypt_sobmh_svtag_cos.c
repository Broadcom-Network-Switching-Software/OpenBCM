/*  Feature  : Hurricane4 MACsec Encryp<tion and decryption
 *
 *  Usage    : BCM.0> cint macsec_encrypt_decrypt_sobmh_svtag_cos.c
 *
 *  config   : hr4_sdk6_macsec_config.bcm
 *
 *  Log file : macsec_encrypt_decrypt_cos_sobmh_svtag_log.txt
 *
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *  Summary:
 *  ========
 *    This CINT configures macsec encryption/decryption for a single port.
 *
 ...
 *
 *      1.2) Configure vlan 10 with 2 ports
 *      1.3) Configure MACsec on egress port 1
 *           1.3.1) Enable MACsec on the egress port
 *           1.3.2) For decryption direction
 *                  1.3.2.1) Configure a decryption SC/SA
 *                  1.3.2.2) Configure a decryption policy
 *                  1.3.2.3) Configure a decryption flow
 *           1.3.3) For encryption direction:
 *                  1.3.3.1) Configure an encryption SC/SA
 *                  1.3.3.2) Install EFP rules to modify SVTAG of the packet (encrypt-1st pass).
 *                           The subport id is passed in to MACsec engine through SVTAG.
 *                           Packet Cng (2 bits) are passed in SVTAG also.
 *                  1.3.3.3) Add a flex flow-based L2 entry to map
 *                           SVTAG.DEST_PORT to DGPP (encrypt-2nd pass IPIPE).
 *           1.3.4) Set the port class to 1 (to be used as a qualifier for the
 *                  IFP rule in 1.1.1)
 *      1.4) Add mapping from internal priority (4 bits) to SVTAG.PRI (encrypt-1st pass EPIPE)
 *      1.5) Set up L3 IPMC L2 forwarding for BUM traffics
 *           1.5.1) Configure a dummy egress l3_intf
 *           1.5.2) For each MACsec-enabled port,
 *                  1.5.2.1) Configure a L3_IPMC egress object with L3_FLAGS3_L2_TAG_ONLY
 *           1.5.3) For each vlan with MACsec-enabled port (egress_port1),
 *                  1.5.3.1) remove egress_port1 from L2 bitmap of the vlan's ipmc group
 *                  1.5.3.2) add MACsec engine port into the L3 bitmap of the vlan's ipmc group
 *                           using the egress object created in 1.5.2.1)
 *
 ...
 *    ======================================================
 *      3.1) Test unicast forwarding to a MACsec-enabled port
 *           3.1.1) Add L2 entries for unicast L2 switching to egress_port1
 *           3.1.2) Unicast packet is received from ingress port
 *                  Expected result:
 *                  The packet is encrypted and forwarded to egress_port1
 *      3.2) Test unicast MACsec decryption
 *           3.2.1) Add L2 entries for unicast L2 switching to ingress_port
 *           3.2.2) Encrypted unicast packet is received from egress_port1
 *                  Expected result:
 *                  The packet is decrypted and forwarded to ingress_port
 *     3.3) Remove L2 entry added in 3.1)/3.2) and repeat for multicast packets for encryption and decryption
 *     3.4) Send packet from CPU to MACsec engine with SVTAG (control=1, SC) using SOBMH
 *          Expected result:
 *          The packet is not encrypted and forwarded to egress_port1
 *     3.5) Send packet from CPU to MACsec engine with SVTAG (control=0, SC) using SOBMH
 *          Expected result:
 *          The packet is encrypted and forwarded to egress_port1
*/
...
bcm_error_t
macsec_decrypt_flow_create(int unit, int pri, macsec_decrypt_flow_s *decrypt_flow)
{
    /* Decrypt flow vars */
    ...
    dec_flow.policy_id  = decrypt_flow->policy_id;
    ...
    print bcm_xflow_macsec_decrypt_flow_create(unit, flow_flags, 0, &dec_flow, pri, &flow_id);
    printf("\nCreated decrypt flow with id:0x%x\n", flow_id);
    print bcm_xflow_macsec_decrypt_flow_enable_set(unit, flow_id, flow_enable);
    ...
    int pri_fget = 0;
    print bcm_xflow_macsec_decrypt_flow_get(unit, flow_id,  &dec_flow_get, &pri_fget);
...
bcm_error_t
assign_egress_sc_by_efp(int unit, bcm_port_t port, bcm_xflow_macsec_secure_chan_id_t sc_id, bcm_color_t pkt_color, bcm_field_group_t group, int *stat_id)
{
 ...
 */
    ...
    BCM_IF_ERROR_RETURN(assign_egress_sc_by_efp(unit, port, macsec_port->encrypt_sc_id, bcmColorYellow, macsec_efp_group, &macsec_port->efp_encrypt_stat_id[1]));
    BCM_IF_ERROR_RETURN(assign_egress_sc_by_efp(unit, port, macsec_port->encrypt_sc_id, bcmColorRed, macsec_efp_group, &macsec_port->efp_encrypt_stat_id[2]));
...
bcm_error_t test_setup(int unit)
{
    ...
    /* Create FP group */
    BCM_IF_ERROR_RETURN(create_ifp_group(unit, &_group_config));
...
/*
  Transmit packets from CPU to a front panel port using HiGig TX
 */
bcm_error_t
tx_hgpkt_from_cpu(uint16 dstmod, bcm_port_t dstport, bcm_vlan_t vlan_id,
                  uint8 internal_pri, bcm_color_t color, uint8 *payload, int payload_len, int withTag)
{
    int rv;
    int unit = 0;
    uint32 flags;
    uint16 smodid;
    uint8 sportid = 0;
    uint16 dmodid = dstmod;
    uint8 dportid = dstport;
    int my_modid;

    print "tx_hgpkt_from_cpu: entry";
    BCM_IF_ERROR_RETURN(bcm_stk_modid_get(unit, &my_modid));
    smodid = my_modid;

    if (withTag)
        flags = BCM_TX_CRC_APPEND | BCM_TX_ETHER | BCM_TX_PKT_PROP_ANY | BCM_TX_PRIO_INT |
                BCM_TX_SRC_MOD | BCM_TX_SRC_PORT;
    else
        flags = BCM_TX_CRC_APPEND | BCM_TX_ETHER | BCM_TX_PKT_PROP_ANY | BCM_TX_PRIO_INT |
                BCM_TX_SRC_MOD | BCM_TX_SRC_PORT | BCM_PKT_F_NO_VTAG;

    bcm_pkt_t *ppd_pkt;

    /* BCM_TX_PKT_PROP_ANY in the flag makes bcm_tx send this packet through
       HiGig ingress */
    rv = bcm_pkt_alloc(unit, payload_len, flags, &ppd_pkt);
    if (BCM_FAILURE(rv)) {
        printf("bcm_pkt_alloc(): %s\n", bcm_errmsg(rv));
        return rv;
    }
    if (!withTag) {
        ppd_pkt->_vtag[0] = 0x81;
        ppd_pkt->_vtag[1] = 0x00;
        ppd_pkt->_vtag[2] = vlan_id >> 8;
        ppd_pkt->_vtag[3] = vlan_id & 0xFF;
        ppd_pkt->rx_untagged = BCM_PKT_OUTER_UNTAGGED;
    }

    bcm_port_t hgport;
    BCM_IF_ERROR_RETURN(bcm_stk_modport_get(unit, dstmod, &hgport));

    BCM_PKT_PORT_SET(&ppd_pkt, hgport, ~withTag, 0);

    ppd_pkt->prio_int = internal_pri;
    ppd_pkt->color = color;

    bcm_pkt_memcpy(ppd_pkt, 0, payload, payload_len);

    ppd_pkt->opcode = BCM_HG_OPCODE_UC;

    ppd_pkt->src_mod = smodid;
    ppd_pkt->src_port = sportid;
    ppd_pkt->dest_mod = dmodid;
    ppd_pkt->dest_port = dportid;
    ppd_pkt->vlan = vlan_id;

    ppd_pkt->stk_forward = BCM_PKT_STK_FORWARD_CPU;

    rv = bcm_tx(unit, ppd_pkt, NULL);
    if (BCM_FAILURE(rv)) {
        printf("bcm_tx(): %s\n", bcm_errmsg(rv));
        bcm_pkt_free(unit, ppd_pkt);
        return rv;
    }

    bshell(unit, "sleep quiet 0 100000");

    bcm_pkt_free(unit, ppd_pkt);

    print "tx_hgpkt_from_cpu: exit";
...
bcm_error_t
macsec_cpu_sobmh_svtag_tx(int unit, uint8 *buf, int leng,
                          bcm_xflow_macsec_secure_chan_id_t sc_id, uint16 dst_mod_id, bcm_port_t dst_port,
                          uint8 ctrl_pkt, uint8 int_pri, bcm_color_t color)
{
    bcm_pkt_t  *txPkt;
    uint8 svtag[4];
    bcm_pkt_blk_t blk[3];
    ...
    if (dst_mod_id != my_modid) {
        BCM_IF_ERROR_RETURN(tx_hgpkt_from_cpu(dst_mod_id, dst_port, 0,
                                              int_pri, color, buf, leng, 1));
        return BCM_E_NONE;
    }

    bcm_xflow_macsec_port_info_t port_info;
    BCM_IF_ERROR_RETURN(bcm_xflow_macsec_port_info_get(unit, dst_port, &port_info));
    if (!port_info.enable)
        return BCM_E_UNAVAIL;
    dst_port = 65 + port_info.encrypt_port_offset;

    BCM_IF_ERROR_RETURN(bcm_xflow_macsec_subport_id_get(unit, sc_id, &subport_id));
    subport_num = BCM_XFLOW_MACSEC_SUBPORT_NUM_GET(subport_id);

    /* set up svtag */
    uint8 color_code = (color & 0XFF);
    svtag[0] = (ctrl_pkt << 14 | (leng + 4)) >> 8; /* SVTAG.lengh includes CRC, not includes SVTAG  */
    svtag[1] = (ctrl_pkt << 14 | (leng + 4)) & 0xFF;

    svtag[2] = (int_pri << 12 | color_code << 10 | subport_num) >> 8;
    svtag[3] = (int_pri << 12 | color_code << 10 | subport_num) & 0xFF;

    printf("svtag[0] = 0x%x, svtag[1] = 0x%x, svtag[2] = 0x%x, svtag[3] = 0x%x \n",
           svtag[0], svtag[1], svtag[2], svtag[3]);

    BCM_IF_ERROR_RETURN(bcm_pkt_alloc(unit, leng+4,
                                      BCM_TX_CRC_APPEND | BCM_TX_SRC_MOD | BCM_TX_SRC_PORT |
                                      BCM_TX_PRIO_INT,
                                      &txPkt));
    sal_memcpy(pktData, buf, 12);
    sal_memcpy(pktData+12, svtag, 4);
    sal_memcpy(pktData+16, buf+12, leng-12);

    bcm_pkt_memcpy(txPkt, 0, pktData, leng+4);

    txPkt->src_mod = my_modid;
    txPkt->src_port = 0;
    txPkt->dest_mod = my_modid;
    txPkt->dest_port = dst_port;

    /* port 65/66 */
    BCM_PBMP_PORT_SET(txPkt->tx_pbmp, dst_port);

    txPkt->prio_int = int_pri;
    txPkt->color = color;;
    txPkt->cos = newUcTcToQMap[int_pri];
...
unsigned char expected_sobmh_packet[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00,
    0x00, 0x0a, 0xff, 0xff, 0x45, 0x78, 0x00, 0x2e, 0x00, 0x00, 0x00, 0x00, 0x40, 0xff,
    0x84, 0x57, 0x14, 0x00, 0x00, 0x01, 0xe1, 0x00, 0x00, 0x01, 0x14, 0x15, 0x16, 0x17,
    0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25,
...
unsigned char expected_encrypted_sobmh_packet[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00,
    0x00, 0x0a, 0x88, 0xe5, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x08, 0x2d, 0xd1, 0x3e, 0x82, 0xd7, 0xcd, 0x56, 0x16, 0x52, 0x21,
    0xaf, 0x94, 0x27, 0x51, 0x0d, 0x2b, 0x29, 0xa8, 0x5d, 0x8e, 0x52, 0x5c, 0x2a, 0x5a,
    0x2a, 0x4c, 0x77, 0xeb, 0x3b, 0x24, 0xc0, 0x92, 0x75, 0xaf, 0x72, 0x72, 0xc6, 0xfc,
    0xc3, 0xee, 0xbf, 0xbc, 0x28, 0x88, 0x2d, 0x49, 0x6c, 0xde, 0xf1, 0x8d, 0x3f, 0x1d,
    0x3a, 0x3d, 0x4f, 0x55, 0x40, 0x46, 0x99, 0xbc, 0xd0, 0xa3, 0x6d, 0x9d, 0xa1, 0xe2,
    0x80, 0xe9, 0x83, 0x69, 0x35, 0xaf};
/*
 *
...
test_verify(int unit)
{
    ...
    snprintf(mc_pkt, 512,"%s","FFFFFFFFFFFF0000000000018100000Affff4578002E0000000040FF845714000001E10000011415161718191A1B1C1D1E1F202122232425262728292A2B2C2D2D0A03AB");
     print "==========  Test: MACsec Encryption - BC/DLF ==================";
    port_discard(unit, ingress_port, 0);
    port_discard(unit, egress_port1, 1);
    ...
    print "==========  Test4: MACsec encryption - SOBMH+SVTAG (control pkt) ==================";
    uint8 sobmh_packet[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x22, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x81, 0x00,
    0x00, 0x0a, 0xff, 0xff, 0x45, 0x78, 0x00, 0x2e, 0x00, 0x00, 0x00, 0x00, 0x40, 0xff,
    0x84, 0x57, 0x14, 0x00, 0x00, 0x01, 0xe1, 0x00, 0x00, 0x01, 0x14, 0x15, 0x16, 0x17,
    0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25,
    0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d, 0x2d, 0x0a, 0x03, 0xab};
    ...
    expected_rx_count = 1;
    packet_patterns = 1;
    expected_packets[0]  = expected_sobmh_packet;
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    BCM_PBMP_PORT_ADD(expected_pbmp[0] , egress_port1);

    BCM_IF_ERROR_RETURN(macsec_cpu_sobmh_svtag_tx(unit, sobmh_packet, 68,
                                                  macsec_port1.encrypt_sc_id, mod_id,
                                                  egress_port1, 1, int_pri, bcmColorGreen));
    ...
    print "==========  Test5: MACsec encryption - SOBMH+SVTAG (data pkt) ==================";
    port_discard(unit, egress_port1, 1);
    ...
    expected_rx_count = 1;
    packet_patterns = 1;
    expected_packets[0]  = expected_encrypted_sobmh_packet;
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    BCM_PBMP_PORT_ADD(expected_pbmp[0] , egress_port1);

    BCM_IF_ERROR_RETURN(macsec_cpu_sobmh_svtag_tx(unit, sobmh_packet, 68,
                                                  macsec_port1.encrypt_sc_id, mod_id,
                                                  egress_port1, 0, int_pri, bcmColorGreen));
    destroy_ifp_to_cpu_rule (unit, eid1);
    bshell(unit, "sleep quiet 1");
