 ...
 *
 *  Usage    : BCM.0> cint macsec_encrypt_decrypt_bypass_ifp_cos.c
 *
 *  config   : hr4_sdk6_macsec_config.bcm
 *
 *  Log file : macsec_encrypt_decrypt_bypass_ifp_cos_log.txt
 *
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *
 *      1.2) Configure IFP rules to match various control protocols in etherII, LLC, and SNAP format,
 *           and set EditCtrlId=16
 ...
 *
 *      1.3) Configure vlan 10 with 2 ports
 *
 *      1.4) Configure MACsec on egress port 1
 *           1.4.1) Enable MACsec on the egress port
 *           1.4.2) For decryption direction
 *                  1.4.2.1) Configure a decryption SC/SA
 *                  1.4.2.2) Configure a decryption policy with control port enabled and configure mapping from <ms_subport, SCI> to SC
 *                  1.4.2.3) Configure a flow for SecTag packets and bind to the decryption policy in 1.4.2.2)
 *                  1.4.2.4) Configure a policy with untag control port enabled
 *                  1.4.2.5) Configure various control flows for clear text control packets and bind to the policy in 1.4.2.4)
 *           1.4.3) For encryption direction:
 *                  1.4.3.1) Configure an encryption SC/SA
 *                  1.4.3.2) Install EFP rules to modify SVTAG of the packet (encrypt-1st pass).
 *                           The subport id is passed in to MACsec engine through SVTAG.
 *                           Packet Cng (2 bits) are passed in SVTAG also.
 *                  1.4.3.3) For control packets (EditCtrlID=16), set SVTAG.CtrlBit = 1
 *                           Otherwise, set SVTAG.CtrlBit = 0
 *                  1.4.3.4) Add a flex flow-based L2 entry to map
 *                           SVTAG.DEST_PORT to DGPP (encrypt-2nd pass IPIPE).
 *           1.4.4) Set the port class to 1 (to be used as a qualifier for the
 *                  IFP rule in 1.1.1)
 *
 *      1.5) Add mapping from internal priority (4 bits) to SVTAG.PRI (encrypt-1st pass EPIPE)
 *
 *      1.6) Set up L3 IPMC L2 forwarding for BUM traffics
 *           1.6.1) Configure a dummy egress l3_intf
 *           1.6.2) For each MACsec-enabled port,
 *                  1.6.2.1) Configure a L3_IPMC egress object with L3_FLAGS3_L2_TAG_ONLY
 *           1.6.3) For each vlan with MACsec-enabled port (egress_port1),
 *                  1.6.3.1) remove egress_port1 from L2 bitmap of the vlan's ipmc group
 *                  1.6.3.2) add MACsec engine port into the L3 bitmap of the vlan's ipmc group
 *                           using the egress object created in 1.6.2.1)
 *
...
/**** global variables ****/
int ifp_stat_id_eapol;
int ifp_stat_id_bpdu;
int ifp_stat_id_cdp;
int ifp_stat_id_lacp;
int ifp_encrypt_stat_id;
int ifp_decrypt_stat_id[2];
int ifp_decrypt_default_stat_id;
...
/* Per port class
   Used in IFP control flow matching
   Ports with the same port class have the same matching rules for control packets
*/
int port_class[MAX_PORTS];
...
bcm_error_t
ifp_control_flow_add_ethii(int unit, uint8 *dstmac, uint8 *dstmac_mask, bcm_vlan_t ovid, bcm_vlan_t ivid, uint8 vformat_data, uint8 vformat_mask,
                           int port_class, int macsec_port, ctrl_flow_format_s *ctrl_flow_fmt, int *stat_id)
{
    bcm_field_entry_t entry;
    bcm_field_stat_t stat_type[]  = { bcmFieldStatBytes, bcmFieldStatPackets };
    bcm_mac_t mac;
    ...
    BCM_IF_ERROR_RETURN( bcm_field_stat_create(unit, macsec_encrypt_ifp_group,
                                               sizeof(stat_type)/sizeof(stat_type[0]), stat_type, stat_id));
    rv = bcm_field_entry_create(unit, macsec_encrypt_ifp_group, &entry);
    if (rv != BCM_E_NONE)
        goto error1;

    rv = bcm_field_qualify_InterfaceClassPort(unit, entry, port_class, BCM_FIELD_EXACT_MATCH_MASK);
    if (rv != BCM_E_NONE)
        goto error;
`   rv = bcm_field_qualify_DstMac(unit, entry, mac, mac_mask);
    ...
    if (macsec_port) {
        ...
        BCM_IF_ERROR_RETURN( bcm_field_entry_prio_set(unit, entry, 3)); /* highest priority compared to entries created in ifp_decryption_default_setup()
                                                                           and ifp_decryption_redirect_setup() */
    }
    else
    ...
    rv = bcm_field_action_add(unit, entry, bcmFieldActionAssignEditCtrlId, 16, 0xFF);
    if (rv != BCM_E_NONE)
        goto error;
    ...
    printf("IFP ethii rule to assign EditCtrlId=16 added \n");
    return BCM_E_NONE;
...
bcm_error_t
ifp_control_flow_add_llc(int unit, uint8 *dstmac, uint8 *dstmac_mask, bcm_vlan_t ovid, bcm_vlan_t ivid, uint8 vformat_data, uint8 vformat_mask,
                         int port_class, int macsec_port, ctrl_flow_format_s *ctrl_flow_fmt, int *stat_id)
{
    bcm_field_entry_t entry;
    bcm_field_stat_t stat_type[]  = { bcmFieldStatBytes, bcmFieldStatPackets };
...
;
    ...
    printf("IFP llc rule to assign EditCtrlId=16 added \n");
    return BCM_E_NONE;
...
bcm_error_t
ifp_control_flow_add_snap(int unit, uint8 *dstmac, uint8 *dstmac_mask, bcm_vlan_t ovid, bcm_vlan_t ivid, uint8 vformat_data, uint8 vformat_mask,
                          int port_class, int macsec_port, ctrl_flow_format_s *ctrl_flow_fmt, int *stat_id)
{
    bcm_field_entry_t entry;
    bcm_field_stat_t stat_type[]  = { bcmFieldStatBytes, bcmFieldStatPackets };
...
`   rv = bcm_field_qualify_DstMac(unit, entry, mac, mac_mask);
    ...
    printf("IFP snap rule to assign EditCtrlId=16 added \n");
    return BCM_E_NONE;
...
bcm_error_t
ifp_control_flow_add(int unit, uint8 *dstmac, uint8 *dstmac_mask, bcm_vlan_t ovid, bcm_vlan_t ivid,
                     uint8 vformat_data, uint8 vformat_mask, int port_class, int macsec_port,
                     ctrl_flow_format_s *ctrl_flow_fmt, int *stat_id)
{

    switch (ctrl_flow_fmt->format) {
    case bcmFieldL2FormatEthII:
        BCM_IF_ERROR_RETURN(ifp_control_flow_add_ethii(unit, dstmac, dstmac_mask, ovid, ivid, vformat_data, vformat_mask,
                                                       port_class, macsec_port, ctrl_flow_fmt, stat_id));
        break;

    case bcmFieldL2FormatLlc:
        BCM_IF_ERROR_RETURN(ifp_control_flow_add_llc(unit, dstmac, dstmac_mask, ovid, ivid, vformat_data, vformat_mask,
                                                     port_class, macsec_port, ctrl_flow_fmt, stat_id));
        break;

    case bcmFieldL2FormatSnap:
        BCM_IF_ERROR_RETURN(ifp_control_flow_add_snap(unit, dstmac, dstmac_mask, ovid, ivid, vformat_data, vformat_mask,
                                                      port_class, macsec_port, ctrl_flow_fmt, stat_id));
        break;
...
bcm_error_t
create_decrypt_ifp_group(int unit, bcm_field_group_t *group)
{
    bcm_field_group_config_t group_cfg;
    char str[512];
    ...
    BCM_FIELD_QSET_ADD(group_cfg.qset, bcmFieldQualifyStageIngress);
    BCM_FIELD_ASET_ADD(group_cfg.aset, bcmFieldActionStatGroup);
    BCM_FIELD_QSET_ADD(group_cfg.qset, bcmFieldQualifySrcPort);
    ...
    snprintf(str, 512,"%s","> Decrypt IFP group installed (decrypt-1st pass).");
...
/* Use hint to reduce the length of the IFP control flow rules (dstIp6) */
/* Hints are available in IFP for Hurricane4 */
bcm_field_hintid_t hint_id;

bcm_error_t
create_ifp_dstIp6_hint(int unit, bcm_field_hint_type_t hint_type, int start_bit, int end_bit)
{
    ...
    hint.start_bit = start_bit;
    hint.end_bit = end_bit;
...
bcm_error_t
create_encrypt_ifp_group(int unit, bcm_field_group_t *group)
{
    bcm_field_group_config_t group_cfg;
    char str[512];
    ...
    snprintf(str, 512,"%s","> Encrypt IFP group (encrypt-1st/2nd pass) installed.");
    ...
    BCM_FIELD_QSET_ADD(group_cfg.qset, bcmFieldQualifyVlanFormat);
    BCM_FIELD_QSET_ADD(group_cfg.qset, bcmFieldQualifyOuterVlanId);
    BCM_FIELD_QSET_ADD(group_cfg.qset, bcmFieldQualifyInnerVlanId);
    BCM_FIELD_QSET_ADD(group_cfg.qset, bcmFieldQualifyL2Format);
    BCM_FIELD_QSET_ADD(group_cfg.qset, bcmFieldQualifyEtherType);
    BCM_FIELD_QSET_ADD(group_cfg.qset, bcmFieldQualifyDstIp6);
    BCM_FIELD_ASET_ADD(group_cfg.aset, bcmFieldActionAssignEditCtrlId);

    /* Associating hints with IFP Group */
    group_cfg.hintid = hint_id;
...
bcm_error_t
ifp_decryption_redirect_setup(int unit, bcm_field_group_t group, int ing_modid, bcm_port_t ing_port,
                              int macsec_engine_port, int *stat_id)
{
    bcm_field_entry_t entry;
    bcm_field_stat_t stat_type[]  = { bcmFieldStatBytes, bcmFieldStatPackets };
    int my_modid;
    bcm_gport_t macsec_gport;

    /* Do not match on 0x88e5 to allow for clear text packet pass-through */
    /* Create decryption IFP entry to redirect to macsec_engine_port
       Packet from <ing_mod, ing_port>
       action: UnmodifiedPacketRedirectPort to macsec decryption engine port (67/68)
    */
    BCM_IF_ERROR_RETURN(bcm_stk_modid_get(unit, &my_modid));
    BCM_GPORT_MODPORT_SET(macsec_gport, my_modid, macsec_engine_port);
    BCM_IF_ERROR_RETURN( bcm_field_stat_create(unit, group,
                                               sizeof(stat_type)/sizeof(stat_type[0]), stat_type, stat_id));
    BCM_IF_ERROR_RETURN( bcm_field_entry_create(unit, group, &entry));

    BCM_IF_ERROR_RETURN( bcm_field_qualify_SrcPort(unit, entry, ing_modid, BCM_FIELD_EXACT_MATCH_MASK, ing_port, BCM_FIELD_EXACT_MATCH_MASK));
    /* not dropped */
    BCM_IF_ERROR_RETURN( bcm_field_qualify_Drop(unit, entry, 0, 1));

    BCM_IF_ERROR_RETURN( bcm_field_action_add(unit, entry, bcmFieldActionUnmodifiedPacketRedirectPort, macsec_gport, macsec_engine_port));
    BCM_IF_ERROR_RETURN( bcm_field_entry_stat_attach(unit, entry, *stat_id));
    BCM_IF_ERROR_RETURN( bcm_field_entry_prio_set(unit, entry, 1)); /* lower priority */
    BCM_IF_ERROR_RETURN( bcm_field_entry_install(unit, entry));
    printf("> IFP rule redirecting MACsec packets to MACsec engine port %d for ingress port <%d, %d> installed (decrypt-1st pass).\n",
           macsec_engine_port, ing_modid, ing_port);
    return BCM_E_NONE;
}
...
ifp_encryption_default_setup(int unit, bcm_field_group_t group, int *stat_id)
{
    ...
    BCM_IF_ERROR_RETURN( bcm_field_entry_prio_set(unit, entry, 2));  /* higher priority */
    BCM_IF_ERROR_RETURN( bcm_field_entry_install(unit, entry));
    printf("%s","> IFP rule to use IFP modidBase added (encrypt-2nd pass).\n");
    return BCM_E_NONE;
}
...
config_macsec(int unit)
{
    ...
    BCM_IF_ERROR_RETURN(create_decrypt_ifp_group(unit, &macsec_decrypt_ifp_group));

    /* Create dstIpv6 hint for macsec_encrypt_ifp_group */
    /* 64 is the longest MSB dstIpv6 bits used in ifp_control_flow_add_XXX() */
    /* Bit position 127 is the most significant bit */
    BCM_IF_ERROR_RETURN(create_ifp_dstIp6_hint(unit, bcmFieldHintTypeExtraction, 64, 127));

    BCM_IF_ERROR_RETURN(create_encrypt_ifp_group(unit, &macsec_encrypt_ifp_group));

    /* FP rules for Decryption */
    BCM_IF_ERROR_RETURN(ifp_decryption_default_setup(unit, macsec_decrypt_ifp_group, &(ifp_decrypt_default_stat_id)));

    BCM_IF_ERROR_RETURN(ifp_decryption_redirect_setup(unit, macsec_decrypt_ifp_group, my_modid, egress_port1,
                                                      67, &(ifp_decrypt_stat_id[0])));
    BCM_IF_ERROR_RETURN(ifp_decryption_redirect_setup(unit, macsec_decrypt_ifp_group, my_modid, egress_port1,
                                                      68, &(ifp_decrypt_stat_id[1])));
    ...
    /* IFP rule to match EAPoL control packet format from ingress_port */
    ...
    BCM_IF_ERROR_RETURN(ifp_control_flow_add(unit, eapol_dst, eapol_dst_mask, vlan_id, 0, BCM_FIELD_VLAN_FORMAT_OUTER_TAGGED,
                                             BCM_FIELD_VLAN_FORMAT_OUTER_TAGGED | BCM_FIELD_VLAN_FORMAT_INNER_TAGGED,
                                             port_class[ingress_port], 0, &ctrl_flow_fmt, &ifp_stat_id_eapol));

    /* IFP rule to match BPDU control packet format from ingress_port */
    ctrl_flow_format_s_init(&ctrl_flow_fmt);
    ...
    BCM_IF_ERROR_RETURN(ifp_control_flow_add(unit, bpdu_dst, bpdu_dst_mask, vlan_id, 0, BCM_FIELD_VLAN_FORMAT_OUTER_TAGGED,
                                             BCM_FIELD_VLAN_FORMAT_OUTER_TAGGED | BCM_FIELD_VLAN_FORMAT_INNER_TAGGED,
                                             port_class[ingress_port], 0, &ctrl_flow_fmt, &ifp_stat_id_bpdu));

    /* IFP rule to match CDP control packet format from ingress_port */
    ctrl_flow_format_s_init(&ctrl_flow_fmt);
    ...
    BCM_IF_ERROR_RETURN(ifp_control_flow_add(unit, cdp_dst, cdp_dst_mask, vlan_id, 0, BCM_FIELD_VLAN_FORMAT_OUTER_TAGGED,
                                             BCM_FIELD_VLAN_FORMAT_OUTER_TAGGED | BCM_FIELD_VLAN_FORMAT_INNER_TAGGED,
                                             port_class[ingress_port], 0, &ctrl_flow_fmt, &ifp_stat_id_cdp));

    /* IFP rule to match LACP control packet format from ingress_port */
    ctrl_flow_format_s_init(&ctrl_flow_fmt);
    ctrl_flow_fmt.format = bcmFieldL2FormatEthII;
    ...
    BCM_IF_ERROR_RETURN(ifp_control_flow_add(unit, slow_protocol_dst, slow_protocol_dst_mask, vlan_id, 0, BCM_FIELD_VLAN_FORMAT_OUTER_TAGGED,
                                             BCM_FIELD_VLAN_FORMAT_OUTER_TAGGED | BCM_FIELD_VLAN_FORMAT_INNER_TAGGED,
                                             port_class[ingress_port], 0, &ctrl_flow_fmt, &ifp_stat_id_lacp));
    uint64 zero_sci;
...
bcm_error_t test_setup(int unit)
{
    ...
    port_class[ingress_port] = 1;
    port_class[egress_port1] = 2;

    /* 9-bit interface class id will mark ingress traffic eligible for the IFP rule */
    for (i = 0; i < port_num; i++) {
        int index = port[i];
        int class = port_class[index];
...
dump_fp_stats(int unit)
{
    ...
    bcm_field_stat_get(unit, ifp_stat_id_eapol, bcmFieldStatPackets, &val);
    printf("ifp_stat_id_eapol:\n");
    print val;

    bcm_field_stat_get(unit, ifp_stat_id_bpdu, bcmFieldStatPackets, &val);
    printf("ifp_stat_id_bpdu:\n");
    print val;

    bcm_field_stat_get(unit, ifp_stat_id_cdp, bcmFieldStatPackets, &val);
    printf("ifp_stat_id_cdp:\n");
    print val;

    bcm_field_stat_get(unit, ifp_stat_id_lacp, bcmFieldStatPackets, &val);
    printf("ifp_stat_id_lacp:\n");
    print val;
    ...
    bcm_field_stat_get(unit, ifp_decrypt_default_stat_id, bcmFieldStatPackets, &val);
    printf("ifp_decrypt_default_stat_id:\n");
    print val;
...
test_verify(int unit)
{
    ...
    /* Note that packets looped back from ingress_port will also hit control packet IFP filter */
    uint8 eapol_dst[6] = {0x01,0x80,0xc2,0x00,0x00,0x03};
    uint8 eapol_dst_mask[6] = {0xff,0xff,0xff,0xff,0xff,0xff};
    BCM_PBMP_CLEAR(pbmp);
