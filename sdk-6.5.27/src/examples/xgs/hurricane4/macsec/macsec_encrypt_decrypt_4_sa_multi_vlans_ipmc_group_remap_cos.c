 ...
 *
 *  Usage    : BCM.0> cint macsec_encrypt_decrypt_4_sa_multi_vlans_ipmc_group_remap_cos.c
 *
 *  config   : hr4_sdk6_macsec_config.bcm
 *
 *  Log file : macsec_encrypt_decrypt_4_sa_multi_vlans_ipmc_group_remap_cos_log.txt
 *
 *  Test Topology :
 *
 *                   +----------------------------+
 ...
 *  Summary:
 *  ========
 *    This CINT configures macsec encryption/decryption for vlan and L3 IPMC multicast traffics for a single port with multiple vlans
 *
 ...
 *
 *      1.4) Configure first ip multicast routing
 *
 *      1.5) For each vlan with MACsec-enabled port (egress_port1) and MACsec processing enabled on <vlan, port> (vlan 10, vlan 11)
 *           1.5.1) For decryption direction
 *                  1.5.1.1) Configure a decryption SC and 4 SA's
 *                  1.5.1.2) Configure a decryption policy
 *                  1.5.1.3) Configure a decryption flow
 *           1.5.2) For encryption direction:
 *                  1.5.2.1) Configure an encryption SC and 4 SA's
 *                  1.5.2.2) Install EFP rules to modify SVTAG of the packet (encrypt-1st pass).
 *                           The subport id is passed in to MACsec engine through SVTAG.
 *                           Packet Cng (2 bits) are passed in SVTAG also.
 *      1.6) For each vlan with MACsec-enabled port (egress_port1) and MACsec processing disabled on <vlan, port> (vlan 12)
 *           1.6.1) For decryption direction
 *                  1.6.1.1) Configure a flow for <vlan, port> and mark it as management flow
 *           1.6.2) For encryption direction,
 *                  1.6.2.1) Install EFP rules to modify SVTAG of the packet (encrypt-1st pass).
 *                           The subport id is passed in to MACsec engine through SVTAG.
 *                           Packet Cng (2 bits) are passed in SVTAG also.
 *                           The egress SC created in 1.2.5 should be used
 *
 *      1.7) Add mapping from internal priority (4 bits) to SVTAG.PRI (encrypt-1st pass EPIPE)
 *
 *      1.8) Create the second ip muticast routing
 *
 *      1.9) ipmc group remap:
 *           1.9.1) For each vlan with MACsec-enabled port, call macsec_ipmc_group_remap().
 *                  This function will create a new vlan l3_ipmc group for the vlan.
 *                  1.9.1.1) For MACsec-enabled ports, add the per-macsec-enabled-port egress-object to the new group
 *                  1.9.1.2) For non-MACsec-enabled ports, just add the original egress object to the group
 *
 *           1.9.2) For L3 IPMC group created before egress port is MACsec-enabled (i.e. group for the first ip multicast routing),
 *                  call macsec_ipmc_group_remap().
 *                  This function will create an new IPMC group,
 *                  1.9.2.1) For MACsec-enabled ports, creat a new egr_object (same as the original object) and add it to the new group
 *                  1.9.2.2) For non-MACsec-enabled ports, just add the original egress object to the group
 *
 *           1.9.3) In both cases, set up l3_ipmc group remap from the original group to the new group
 *
 *       This ipmc remap design is based on the following two SDK behaviors:
 *       (a) When an egress obj is created on a MACsec-enabled port,
 *           NH.HG_CHANGE_DESTINATION=1, and HG_MODID/HG_PORTID will be set to the egress port.
 *       (b) When an egress obj created on a MACsec-enabled port is added to an ipmc group, it will be chained under port 65/66 and 
 *           port 65/66 will be set in the L3_BITMAP of the group
 *
 ...
 *      3.4) Test traffics for the 1st L3_IPMC traffic
 *           Expected result:
 *           3.4.1) IPMC traffics are correctly routed and encrypted
 *      3.5) Test traffics for the 2nd L3_IPMC traffic
 *           Expected result:
 *           3.5.1) IPMC traffics are correctly routed and encrypted
 */
...
/* Create L3 egress interface with qos */
bcm_error_t
l3_intf_create_qos(int unit, bcm_vlan_t vlan, bcm_mac_t mac_addr, uint32 flags, bcm_l3_intf_qos_t *intf_qos, bcm_if_t *intf_id)
{
    bcm_l3_intf_t l3_interface;
    ...
    l3_interface.l3a_vid = vlan;
    l3_interface.l3a_flags = flags;
    l3_interface.l3a_intf_id = *intf_id;

    l3_interface.vlan_qos = *intf_qos;
...
bcm_error_t
l3_egress_create_new(int unit, uint32 flags, bcm_port_t port,
                     bcm_if_t intf_id,
                     bcm_if_t *egr_obj_id)
{
    bcm_l3_egress_t l3_egress;
...
macsec_port_config(int unit, macsec_port_s *macsec_port)
{
    ...
    /* Create a NH object egressing through the port, one per macsec-enabled port */
    /* This should be used for L2 forwarding only (no packet header change) */
...
/*
    print "port_config";
    bshell(unit, "dump chg ing_l3_next_hop 0 10");
    bshell(unit, "dump chg egr_l3_next_hop 0 10");
*/
    uint64 zero_sci;
...
*/
    ...
    BCM_IF_ERROR_RETURN(bcm_multicast_create(unit, BCM_MULTICAST_TYPE_L3, remapped_ipmc_group));
    bcm_multicast_t remapped_group = *remapped_ipmc_group;

    BCM_IF_ERROR_RETURN(bcm_multicast_egress_get(unit, ipmc_group, MAX_PORTS, gport_array, encap_id_array, &port_cnt));
        ...
        if (port != 65 && port !=66) {
            ...
            if (port_info.enable) {
                ...
                if (encap_id_array[i] == -1) {
                    /* This is only L2 forwarding */
                    bcm_if_t    encap_id = -1;
                    if (macsec_port[j].egr_obj_id) {
                        BCM_IF_ERROR_RETURN(bcm_multicast_egress_object_encap_get(unit, ipmc_group, macsec_port[j].egr_obj_id, &encap_id));
                    }
                    else
                        return BCM_E_NOT_FOUND;

                    printf("remap: port = %d, egr_obj_id=0x%x, encap=0x%x\n", port, macsec_port[j].egr_obj_id, encap_id);

                    /* Per macsec port egr_obj is created with L3MC:HG_MC_DST_PORT_NUM, L3MC:HG_MC_DST_MODID, L3MC:HG_CHANGE_DESTINATION=1 */
                    ...
                    /* This is for L3 egress object added to L3MC group before MACsec is enabled */
                    bcm_if_t egr_obj_id, new_l3_intf;
                    int flags3 = 0;
                    bcm_if_t encap_id = -1, org_encap_id;

                    org_encap_id = encap_id_array[i];
                    printf("org_encap_id = 0x%x\n", org_encap_id);

                    bcm_l3_egress_t egr;
                    BCM_IF_ERROR_RETURN(bcm_l3_egress_get(unit, org_encap_id, &egr));

                    bcm_l3_intf_t l3_intf;
                    l3_intf.l3a_intf_id = egr.intf;
                    BCM_IF_ERROR_RETURN(bcm_l3_intf_get(unit, &l3_intf));

                    /* Have to create a new l3_intf.
                       Otherwise, bcm_l3_egress_create() will fail in _bcm_tr3_l3_ipmc_nh_add()
                    */
                    BCM_IF_ERROR_RETURN(l3_intf_create_qos(unit, l3_intf.l3a_vid, l3_intf.l3a_mac_addr, 0, &(l3_intf.vlan_qos), &new_l3_intf));

                    printf("org_encap_id=0x%x, new_l3_intf=0x%x\n", org_encap_id, new_l3_intf);

                    BCM_GPORT_MODPORT_SET(gport, my_modid, port);
                    /*  This egress object will be created with L3MC:HG_MC_DST_PORT_NUM, L3MC:HG_MC_DST_MODID, L3MC:HG_CHANGE_DESTINATION=1 */
                    BCM_IF_ERROR_RETURN(l3_egress_create_new(unit, BCM_L3_IPMC|BCM_L3_KEEP_DSTMAC, port, new_l3_intf, &egr_obj_id));
/*
                    print "remap after l3_egress_create_new():";
                    bshell(unit, "dump chg ing_l3_next_hop 0 10");
                    bshell(unit, "dump chg egr_l3_next_hop 0 10");
*/
                    BCM_IF_ERROR_RETURN(bcm_multicast_egress_object_encap_get(unit, ipmc_group, egr_obj_id, &encap_id));
                    printf("remap: egr_obj_id=0x%x, encap_id=0x%x\n", egr_obj_id, encap_id);
                    /* SDK will convert the port to the corresponding MACsec engine port to add to L3_bitmap */
                    BCM_IF_ERROR_RETURN(bcm_multicast_egress_add(unit, remapped_group, gport, encap_id));
                }
            }
            else {
...
/*
    ...
    bshell(unit, "dump chg egr_l3_next_hop 0 10");
*/
    printf("Call bcm_multicast_control_set() to remap group 0x%x to 0x%x\n", ipmc_group, remapped_group);
    /* enable remap */
    BCM_IF_ERROR_RETURN(bcm_multicast_control_set(unit, ipmc_group, bcmMulticastRemapXflowMacsec, 1));
    BCM_IF_ERROR_RETURN(bcm_multicast_control_set(unit, ipmc_group, bcmMulticastRemapGroup, remapped_group));
...
int
config_ipmc(int unit, bcm_vlan_t start_vlan, bcm_multicast_t *mcast_grp)
{
    bcm_multicast_t multicast_group;
    bcm_error_t rv;
    int i;
    int num_ports = 2;
    bcm_if_t egr_l3_if[2];
    bcm_mac_t src_mac = {0x00, 0x00, 0x00, 0x00, 0x01, 0x00};
    bcm_port_t egr_port[2] = {1,2};
...
/*
        rv = vlan_create_add_port(unit, start_vlan+i, egr_port[i], 0);
        if (BCM_FAILURE(rv)) {
            printf("Error in configuring vlan : %s.\n", bcm_errmsg(rv));
            return rv;
        }
*/
        intf_qos.flags = BCM_L3_INTF_QOS_OUTER_VLAN_PRI_SET;
        intf_qos.pri = ovlan_pri;

        src_mac[5] = i+1;
        rv = l3_intf_create_qos(unit, start_vlan+i, src_mac, 0, &intf_qos, &egr_l3_if[i]);
        if (BCM_FAILURE(rv)) {
            printf("Error in configuring l3 interface : %s.\n", bcm_errmsg(rv));
            return rv;
        }

        BCM_GPORT_MODPORT_SET(egr_gport, my_modid, egr_port[i]);

/*
        BCM_IF_ERROR_RETURN(bcm_multicast_l3_encap_get(unit, *mcast_grp, egr_gport, egr_l3_if[i], &encap));
        printf("config_ipmc(): l3_intf=0x%x, encap=0x%x\n", egr_l3_if[i], encap);
*/
        /*  This egress object will be created with L3MC:HG_MC_DST_PORT_NUM, L3MC:HG_MC_DST_MODID, L3MC:HG_CHANGE_DESTINATION=1 */
        BCM_IF_ERROR_RETURN(l3_egress_create_new(unit, BCM_L3_IPMC|BCM_L3_KEEP_DSTMAC, egr_port[i], egr_l3_if[i], &egr_obj_id));
        BCM_IF_ERROR_RETURN(bcm_multicast_egress_object_encap_get(unit, *mcast_grp, egr_obj_id, &encap));
        BCM_IF_ERROR_RETURN(bcm_multicast_egress_add(unit, *mcast_grp, egr_gport, encap));
    }
...
int
add_ipmc(int unit, bcm_vlan_t vlan, bcm_port_t port, int vrf, bcm_ip_t sip, bcm_ip_t mc_ip,
         bcm_if_t ingress_if, bcm_multicast_t ipmcast_group)
{
    bcm_error_t rv;
    ...
    ipmc_addr.vid = vlan;
    ipmc_addr.ing_intf = ingress_if; /* ingresss l3_iif */
                  ...
                  bcm_port_t ing_port)
{
    ...
    rv = vlan_create_add_port(unit, ing_vlan, ing_port, 0);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring vlan : %s.\n", bcm_errmsg(rv));
        return rv;
    }
...
int ipmc(int unit, int vrf, bcm_ip_t sip_addr, bcm_ip_t mc_ip_addr, bcm_if_t ingress_if,
         bcm_multicast_t *multicast_group)
{
    bcm_error_t rv;
    ;
    /* bcm_multicast_t remapped_multicast_group; */

    bcm_mac_t mac = {0x01, 0x00, 0x5E, 0x01, 0x01, 0x01};
    bcm_vlan_t ing_vlan = 10;
    bcm_port_t ing_port = 1;

    rv = config_ipmc(unit, ing_vlan, multicast_group);
    if(BCM_FAILURE(rv)) {
        printf("\nError in configuring ipmc: %s.\n",bcm_errmsg(rv));
        return rv;
    }
    printf("ipmc multicast_group = 0x%x\n", *multicast_group);

    /*
      Don't need to remap because the group is created after MACsec port is enabled
      BCM_IF_ERROR_RETURN(macsec_ipmc_group_remap(unit, multicast_group, &remapped_multicast_group));
    */
    rv = add_ipmc(unit, ing_vlan, ing_port, vrf, sip_addr, mc_ip_addr, ingress_if, *multicast_group);
    if(BCM_FAILURE(rv)) {
        printf("\nError in adding ipmc: %s.\n",bcm_errmsg(rv));
        return rv;
    }

    rv = add_multicast_mac(unit, mac, ing_vlan, ing_port);
    if(BCM_FAILURE(rv)) {
...
bcm_error_t test_setup(int unit)
{
    ...
    bcm_ip_t sip_addr1 = 0x02010101;
    bcm_ip_t mc_ip_addr1 = 0xe4010101;
    bcm_ip_t sip_addr2 = 0x02010102;
    bcm_ip_t mc_ip_addr2 = 0xe4010102;
    bcm_multicast_t l3mc_group1;
    bcm_multicast_t l3mc_group2;
    bcm_multicast_t l3mc_remap_group1;
    bcm_mac_t egr_mac = {0x00, 0x00, 0x06, 0x10, 0x03, 0x05};
    int vrf = 1;

    BCM_IF_ERROR_RETURN(test_init(unit));
...
*/
    ...
    */
    ...
    bcm_if_t l3_intf = 0;
    rv = l3_intf_create(unit, vid1, egr_mac, BCM_L3_ADD_TO_ARL, &l3_intf);
    if (BCM_FAILURE(rv)) {
    ...
    l3_ingress.flags = BCM_L3_INGRESS_WITH_ID | BCM_L3_INGRESS_REPLACE;
    l3_ingress.vrf = vrf;
    ...
    print bcm_vlan_control_vlan_get(unit, vid1, &vlan_control);
    vlan_control.ingress_if = ingress_if;
    vlan_control.flags |= BCM_VLAN_CONTROL_VLAN_INGRESS_IF_MASK;
    rv = bcm_vlan_control_vlan_set(unit, vid1, vlan_control);
    if (BCM_FAILURE(rv)) {
        printf("Error in configuring vlan to l3_iif mapping : %s.\n", bcm_errmsg(rv));
        return rv;
    }

    if (BCM_FAILURE((rv = ipmc(unit, vrf, sip_addr1, mc_ip_addr1, ingress_if, &l3mc_group1)))) {
        printf("IPMC Setup Failed %s.\n", bcm_errmsg(rv));
        return -1;
    }
    ...
    if (BCM_FAILURE((rv = ipmc(unit, vrf, sip_addr2, mc_ip_addr2, ingress_if, &l3mc_group2)))) {
        printf("IPMC Setup Failed %s.\n", bcm_errmsg(rv));
        return -1;
    }

    BCM_IF_ERROR_RETURN(macsec_ipmc_group_remap(unit, macsec_vlan[0].vlan_ipmc_group, &macsec_vlan[0].remapped_ipmc_group));
    BCM_IF_ERROR_RETURN(macsec_ipmc_group_remap(unit, macsec_vlan[1].vlan_ipmc_group, &macsec_vlan[1].remapped_ipmc_group));
    BCM_IF_ERROR_RETURN(macsec_ipmc_group_remap(unit, macsec_vlan[2].vlan_ipmc_group, &macsec_vlan[2].remapped_ipmc_group));

    /* Need to remap l3mc group created before MACsec port is enabled */
    BCM_IF_ERROR_RETURN(macsec_ipmc_group_remap(unit, l3mc_group1, &l3mc_remap_group1));
...
unsigned char expected_encrypted_ipmc_packet1[512] = {
    0x01, 0x00, 0x5e, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x81, 0x00,
    0x20, 0x0b, 0x88, 0xe5, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x09, 0x5f, 0x68, 0x3e, 0x13, 0xb2, 0xa0, 0x52, 0xb7, 0x17, 0xd3,
    0x7c, 0xd2, 0x67, 0xd4, 0x55, 0x78, 0xc6, 0xe8, 0x71, 0xc4, 0x3c, 0xaf, 0xd2, 0xca,
    0xe3, 0xf8, 0x94, 0xda, 0x30, 0x68, 0x68, 0xbf, 0x47, 0xdd, 0x10, 0xb0, 0x7e, 0xd1,
    0x65, 0xc7, 0xf9, 0x93, 0xe4, 0x70, 0x84, 0xbf, 0xa4, 0x64, 0x1b, 0x1d, 0x61, 0x49,
    0xf4, 0x07, 0xa2, 0x52, 0x6a, 0xc5, 0x55, 0x12, 0x14, 0xf4, 0xd7, 0x59, 0x17, 0x41,
    0x8c, 0x7f, 0x2b, 0x30, 0x96, 0x2f};

unsigned char expected_encrypted_ipmc_packet2[512] = {
    0x01, 0x00, 0x5e, 0x01, 0x01, 0x02, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x81, 0x00, 0x20, 0x0b,
    0x88, 0xe5, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x09,
    0xcd, 0xfe, 0x2a, 0x2c, 0x41, 0xc4, 0xa8, 0xd8, 0x42, 0xf6, 0x37, 0xfe, 0xfa, 0x26, 0xe4, 0x11,
    0xd6, 0x97, 0xd0, 0xf5, 0x67, 0x4e, 0x82, 0x3b, 0xc2, 0x30, 0xcb, 0x36, 0x72, 0x42, 0xe4, 0xd0,
    0x7c, 0xeb, 0x3b, 0x7a, 0x99, 0x92, 0x28, 0xee, 0x45, 0x0f, 0xac, 0x25, 0x67, 0x48, 0xb2, 0x61,
    0xd4, 0x04, 0x96, 0x12, 0x19, 0xfd, 0x0b, 0x93, 0xa3, 0xae, 0xcd, 0xf7, 0xac, 0x9a, 0x6e, 0x94,
    0xa6, 0x7a, 0xab, 0x3a, 0xcb, 0xa1, 0x13, 0x51};
...
test_verify(int unit)
{
    ...
    print "==========  Test10: MACsec encryption - L3 IPMC  (ipmc group created before MACsec port enabled) ==================";
    port_discard(unit, ingress_port, 0);
    port_discard(unit, egress_port1, 1);
    snprintf(ipmc_pkt1, 512,"%s", "01005E0101010000060003008100000a08004500002E0000000040FF91CD02010101E4010101000102030405060708090A0B0C0D0E0F10111213141516171819BF2E44C3");
    printf(">>> Input IPMC packet @ port-%d:\n", ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x%s; sleep quiet 1", ingress_port, ipmc_pkt1);
    ...
    expected_rx_count = 1;
    packet_patterns = 1;
    expected_packets[0]  = expected_encrypted_ipmc_packet1;
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    BCM_PBMP_PORT_ADD(expected_pbmp[0] , egress_port1);
    bshell(unit, str);
    bshell(0,"dmirror 2 mode=off");
    ...
    print "==========  Test11: MACsec encryption - L3 IPMC  (ipmc group created after MACsec port enabled) ==================";
    port_discard(unit, ingress_port, 0);
    port_discard(unit, egress_port1, 1);
    snprintf(ipmc_pkt2, 512,"%s", "01005E0101020000060003008100000a08004500002E0000000040FF91CB02010102E4010102000102030405060708090A0B0C0D0E0F10111213141516171819BF2E44C3");
    printf(">>> Input IPMC packet @ port-%d:\n", ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x%s; sleep quiet 1", ingress_port, ipmc_pkt2);
    ...
    expected_rx_count = 1;
    packet_patterns = 1;
    expected_packets[0]  = expected_encrypted_ipmc_packet2;
    BCM_PBMP_CLEAR(expected_pbmp[0]);
    BCM_PBMP_PORT_ADD(expected_pbmp[0] , egress_port1);
    bshell(unit, str);
    bshell(0,"dmirror 2 mode=off");
