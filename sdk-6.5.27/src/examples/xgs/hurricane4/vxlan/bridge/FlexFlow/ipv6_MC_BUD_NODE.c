...
/*
 * Feature  : TD3(56870) Vxlan Bridging ,  Network to network & Acess  known  multicast Flow
 *
 * Usage    : BCM.0> cint ipv6_MC_BUD_NODE.c
 *
 * config   : BCM56275_A1-BRIDGE-FLEXFLOW.bcm
 *
 * Log file : ipv6_MC_BUD_NODE_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
Ingress Hex Packet:
tx 1 pbm=1 data=
73747465737474657374746573747465737474657374
...
data[0000]: {333301010101} {000000003333} 8100 0017
data[0010]: 86dd 6000 0000 006e 113f 0202 0202 3600
data[0020]: 7000 0000 0102 0304 0506 ff1e 0101 0000
data[0030]: 0000 0000 0000 0101 0101 d903 2118 006e
data[0040]: b325 0800 0000 0123 4500 0100 5e01 0101
data[0050]: 0000 0000 cccc 0800 4500 0050 0001 0000
data[0060]: 4011 9496 0202 0202 e101 0101 0035 0035
data[0070]: 003c 52f5 7465 7374 7465 7374 7465 7374
data[0080]: 7465 7374 7465 7374 7465 7374 7465 7374
data[0090]: 7465 7374 7465 7374 7465 7374 7465 7374
data[00a0]: 7465 7374 7465 7374 ccf4 bada
...

{
	...
	return rv;
		...
		bcm_error_t
	config_vxlan_net_mc(int unit, vxlan_cfg_t *vxlan_cfg, vxlan_mcnet_cfg_t *vxlan_mcnet)

	{
		...
		rv = bcm_port_local_get(unit, vxlan_mcnet->phy_Gport, &(phy_port));
		if (BCM_FAILURE(rv)) {
			printf("\nError in getting LocalPort: %s.\n", bcm_errmsg(rv));
			return rv;
		}
		...
		rv = vlan_create_add_port(unit, vxlan_mcnet->net_vlan, phy_port);
		if (BCM_FAILURE(rv)) {
			printf("\nError in configuring vlan: %s.\n", bcm_errmsg(rv));
			return rv;
		}

		rv = vxlan_network_port_config(unit, vxlan_mcnet->phy_Gport);
		if (BCM_FAILURE(rv)) {
			printf("\nError in network config: %s.\n", bcm_errmsg(rv));
			return rv;
		}
		...
		printf("Create network L3 interface\n");
		rv = create_l3_interface(unit, BCM_L3_ADD_TO_ARL, vxlan_mcnet->net_local_mac, vxlan_mcnet->net_vlan,
			0,0,&net_intf_id);
		if (BCM_FAILURE(rv)) {
			printf("\nError in configuring l3 interface: %s.\n", bcm_errmsg(rv));
			return rv;
		}
		vxlan_mcnet->intf_id=net_intf_id;
		...
		bcm_error_t
	config_vxlan_transit_mcast(int unit,bcm_multicast_t *mcast_group,int flags ,bcm_if_t IntId,bcm_gport_t gport,int GroupCreate)
	{
    	bcm_error_t rv = BCM_E_NONE;
	    bcm_if_t encap_id;
	    ...
	    rv = bcm_multicast_create(unit, flags, mcast_group);
			if (BCM_FAILURE(rv)) {
		  	printf("\nError in configuring Transit Mcast Group: %s.\n", bcm_errmsg(rv));
		  	return rv;
			}
        }
	    rv= bcm_multicast_l3_encap_get(unit,*mcast_group,gport,IntId,&encap_id);
		if (BCM_FAILURE(rv)) {
		printf("\nError in Getting l3 encap: %s.\n", bcm_errmsg(rv));
		return rv;
		}

	    rv= bcm_multicast_egress_add(unit,*mcast_group,gport,encap_id);
		if (BCM_FAILURE(rv)) {
...
config_vxlan_ipv6_vtep(int unit)
{
	...
	rv = config_vxlan_net_mc(unit, &vxlan_cfg, &vxlan_mcnw);
	if (BCM_FAILURE(rv)) {
		printf("\nError in network MC config: %s.\n", bcm_errmsg(rv));
		return rv;
	}
    ...
    printf("\nMcast config:Access ");
	rv=config_vxlan_mcast(unit,&mcast_group,BCM_MULTICAST_TYPE_FLOW,vxlan_acc1.egr_id,vxlan_acc1.acc_gport,1);
	if (BCM_FAILURE(rv)) {
	...
	rv=config_vxlan_mcast(unit,&(vxlan_cfg.mc_group),BCM_MULTICAST_TYPE_FLOW,vxlan_net.egr_id,vxlan_net.nw_gport,0);
   	...
   	rv=config_vxlan_transit_mcast(unit,&mcast_group_transit,BCM_MULTICAST_TYPE_L3,vxlan_mcnw.intf_id,vxlan_mcnw.phy_Gport,1);
	if (BCM_FAILURE(rv)) {
		printf("\nError in Mcast config Network: %s.\n", bcm_errmsg(rv));
		return rv;
	}
   	...
   	ip_mcast.flags = BCM_IPMC_SOURCE_PORT_NOCHECK | BCM_IPMC_IP6;
   	ip_mcast.vid = vxlan_net.net_vlan;
   	ip_mcast.s_ip6_addr = vxlan_net.remote_ip6;
   	ip_mcast.mc_ip6_addr = vxlan_net.mcast_ip6;
