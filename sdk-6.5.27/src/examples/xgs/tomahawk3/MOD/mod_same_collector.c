...
int em_in_per_pipe_mode = 0; /* For TH3. */
uint32            NUM_PIPES  = 8;
...
uint8 DMA_CHANNEL  = 9;
//uint8 DMA_CHANNEL  = 13;
uint8 EM_MISS_COSQ = 47;
uint8 DROP_COSQ    = 47;

bcm_port_t cpu_port  = 0;
bcm_port_t coll_port = 31;
...
bcm_ip_t                    coll_dst_ip      = 0x0a82b00a;
...
int export_profile_id1 = 8;
int export_profile_id2 = 9;

bcm_flowtracker_export_template_t tmpl_id;
uint16                            set_id      = 257;

//uint32 group_flow_limit       = 57000;
uint32 group_flow_limit       = 10000;
uint32 aging_interval_msecs   = 24 * 1000;
...
bcm_vlan_t vid_network_ingress = 20;
...
bcm_port_t port_acc_ingress = 29;
bcm_port_t port_network_ingress = 30;
...
bcm_ip_t pkt_in_dst_ip = 0x0a0a0201;
...
int drop_reasons_set(int enable)
{
    ...
    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit,
                                               ...
                                               enable));
    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit,
                                               bcmSwitchMartianAddrToCpu,
                                               enable));
    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit,
                                               bcmSwitchL3MtuFailToCpu,
                                               enable));
    BCM_IF_ERROR_RETURN(bcm_switch_control_set(unit,
...
int setup()
{
    ...
    BCM_IF_ERROR_RETURN(bcm_vlan_create(unit, coll_vlan));
    BCM_PBMP_CLEAR(pbmp);
    BCM_PBMP_CLEAR(upbmp);
    BCM_PBMP_PORT_ADD(pbmp, cpu_port);
    BCM_PBMP_PORT_ADD(pbmp, coll_port);
    BCM_IF_ERROR_RETURN(bcm_vlan_port_add(unit, coll_vlan, pbmp, upbmp));

    /* Add static MAC address */
    bcm_l2_addr_t_init(&l2_addr, coll_dst_mac, coll_vlan);
    l2_addr.flags = BCM_L2_STATIC;
    l2_addr.port  = coll_port;
    ...
    BCM_IF_ERROR_RETURN(
            bcm_mirror_port_dest_add(unit, 1,
...
int vfp_src_class_set()
{
    ...
    bcm_field_qset_t  qset;
    bcm_field_entry_t entry;
    bcm_port_t        port = port_acc_ingress;
    int               class_id;
...
int presel_create()
{
    ...
    for (pipe = 0; pipe < /*NUM_PIPES*/ 1; pipe++) {
        BCM_IF_ERROR_RETURN(
        ...
        for (group = 1; group <= NUM_GROUPS; group++) {
            ...
            BCM_IF_ERROR_RETURN(
                        bcm_field_qualify_MixedSrcClassId(unit,
                                                          presel_id[pipe][group] | BCM_FIELD_QUALIFY_PRESEL,
                                                          /*pc.per_pipe[pipe]*/ pc.all, data, mask));
        }
    }
    return BCM_E_NONE;
}
...
int udf_create()
{
    ...
    BCM_IF_ERROR_RETURN(
              bcm_udf_pkt_format_create(unit,
    ...
    /* Inner Src Ip */
    bcm_udf_t_init(&udf);
    udf.layer = bcmUdfLayerL4OuterHeader;
    udf.start = 46 * 8;
    udf.width = 4 * 8;  /* Convert to bits */

    BCM_IF_ERROR_RETURN(bcm_udf_create(unit, &hints, &udf, &(vxlan_udf_id[0])));

    BCM_IF_ERROR_RETURN(bcm_udf_pkt_format_add(unit,
                                               vxlan_udf_id[0],
                                               pkt_fmt_id));
    ...
    BCM_IF_ERROR_RETURN(bcm_udf_create(unit, &hints, &udf, &(vxlan_udf_id[1])));
    ...
    udf.layer = bcmUdfLayerL4OuterHeader;
    udf.start = 54 * 8;
    udf.width = 2 * 8;  /* Convert to bits */

    BCM_IF_ERROR_RETURN(bcm_udf_create(unit, &hints, &udf, &(vxlan_udf_id[2])));
    ...
    BCM_IF_ERROR_RETURN(bcm_udf_create(unit, &hints, &udf, &(vxlan_udf_id[3])));
    ...
    BCM_IF_ERROR_RETURN(bcm_udf_create(unit, &hints, &udf, &(vxlan_udf_id[4])));
    ...
    BCM_IF_ERROR_RETURN(bcm_udf_create(unit, &hints, &udf, &prop_vxlan_udf_id));
...
int vxlan_em_group_create(bcm_flowtracker_group_t flow_group_id)
{
    ...
    for (pipe = 0; pipe < NUM_PIPES; pipe++) {
        ...
        em_groups[flow_group_id][pipe] = gcfg.group;
         /*
         * If em is not in per pipe mode, then break out of
         * the loop after first iteration.
         */
        if (!em_in_per_pipe_mode) {
            break;
        }
...
int ipv4_em_group_create(bcm_flowtracker_group_t flow_group_id)
{
    ...
    for (pipe = 0; pipe < NUM_PIPES; pipe++) {
        ...
        em_groups[flow_group_id][pipe] = gcfg.group;
        printf("Em groups FG %d pipe %d filled with %d\r\n", flow_group_id, pipe, gcfg.group);
        /*
         * If em is not in per pipe mode, then break out of
         * the loop after first iteration.
         */
...
int collector_create()
{
    ...
    collector_info.ipv4.dst_ip            = coll_dst_ip;
    ...
    collector_info.ipfix.observation_domain_id         = 0xef;
    ...
    coll_id = 4;
    rv = bcm_collector_create(unit, BCM_COLLECTOR_WITH_ID, &coll_id, &collector_info);
...
int export_profile_create(int id, bcm_collector_wire_format_t wire_format)
{
    ...
    profile_info.wire_format = wire_format;
    profile_info.export_interval   = 1000000;
    profile_info.max_packet_length = 1500;

    rv = bcm_collector_export_profile_create(unit, BCM_COLLECTOR_EXPORT_PROFILE_WITH_ID,
                                             &id, &profile_info);
    BCM_IF_ERROR_RETURN(rv);
 ...
 * Notes:
 */
int group_coll_attach(bcm_flowtracker_group_t flow_group_id)
{
    BCM_IF_ERROR_RETURN(
             bcm_flowtracker_group_collector_attach(unit, flow_group_id,
                                                    coll_id, export_profile_id1,
                                                    tmpl_id));
    BCM_IF_ERROR_RETURN(
             bcm_flowtracker_group_collector_attach(unit, flow_group_id,
                                                    coll_id, export_profile_id2,
                                                    tmpl_id));
 ...
 *      to call this API
 */
int group_coll_detach(bcm_flowtracker_group_t flow_group_id)
{
    ...
    if (default_group) {
     ...
     if (em_in_per_pipe_mode) {
        for (i = 0; i < NUM_PIPES; i++) {
            flow_group_info.field_group[i] = field_groups[i];
        }
    } else {
...
int ipv4_flow_data_get()
{
    ...
    BCM_IF_ERROR_RETURN(bcm_flowtracker_group_data_get(unit, 2,
                                                       &key, &data));
...
int fp_catch_all()
{
    ...
    bcm_field_group_t         group;
    bcm_field_entry_t         entry;
    bcm_field_entry_t         egr_policer_entry;

    /* Catch-all IFP rule to assign EH header */
    /*BCM_FIELD_QSET_INIT(qset);
    BCM_FIELD_QSET_ADD(qset, bcmFieldQualifyStageIngress);
    BCM_FIELD_QSET_ADD(qset, bcmFieldQualifyDrop);
    ...
    BCM_IF_ERROR_RETURN(bcm_field_entry_create(unit, group, &entry));
    BCM_IF_ERROR_RETURN(bcm_field_qualify_Drop(unit, entry, 1, 0xffffffff));
    BCM_IF_ERROR_RETURN(bcm_field_action_add(unit, entry,
                                             bcmFieldActionFabricQueue,
                                             BCM_FABRIC_QUEUE_DEST_OFFSET, 0));
    BCM_IF_ERROR_RETURN(bcm_field_entry_install(unit, entry));*/
...
void run(int run_setup)
{
    ...
    /*rv = prop_vxlan_em_group_create(1);
    if (rv != BCM_E_NONE) {
        printf("prop_vxlan_em_group_create() failed rv=%d \n", rv);
        return;
    }*/

    //rv = ipv4_em_group_create(2);
    ...
    rv = flow_group_create(NUM_GROUPS, em_groups[NUM_GROUPS], 0);
    if (rv != BCM_E_NONE) {
        printf("flow_group_create() failed i=%d rv=%d \n", i, rv);
        return;
    }
    ...
    /*rv = prop_vxlan_tracking_params_set(1);
    if (rv != BCM_E_NONE) {
    ...
    /*rv = vxlan_tracking_params_set(3);
    if (rv != BCM_E_NONE) {
    ...
    rv = collector_create();
    if (rv != BCM_E_NONE) {
        printf("collector_create() failed rv=%d \n", rv);
    }

    rv = export_profile_create(export_profile_id1, wire_format);
    if (rv != BCM_E_NONE) {
        printf("export_profile_create() failed rv=%d \n", rv);
    }

    wire_format = bcmCollectorWireFormatIpfix;
    rv = export_profile_create(export_profile_id2, wire_format);
    if (rv != BCM_E_NONE) {
        printf("export_profile_create() failed rv=%d for id %d\n", rv, export_profile_id2);
    }
    ...
    for (i = 1; i <= NUM_GROUPS; i++) {
        rv = group_coll_attach(i);
        if (rv != BCM_E_NONE) {
            printf("group_coll_attach() failed i=%d rv=%d \n", i, rv);
            return;
        }
    }
    ...
    rv = ifp_entry_collector_copy_to_cpu(ifp_group,coll_port) ;
    if (rv != BCM_E_NONE) {
