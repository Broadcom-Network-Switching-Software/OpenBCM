 /* Feature  : L2 QoS
 *
 *  Usage    : BCM.0> cint l2_qos_pkt_verify.c
 *
...
uint8 priority, internal_priority, CFI, new_priority;
 ...
 */
  ...
  */
  rv = qos_port_setup(unit, egress_port, BCM_QOS_MAP_EGRESS | BCM_QOS_MAP_L2, internal_priority, bcmColorYellow, new_priority, CFI);
  if (BCM_FAILURE(rv)) {
...
pkt_verify(int unit, bcm_pkt_t *pkt)
{
    ...
    if (ingress_port == pkt->rx_port) {
      if (pkt->prio_int != internal_priority || pkt->vlan_pri != priority || pkt->vlan_cfi != 1) {
          printf("Ingress: internal_priority=%3d priority=%d\n",internal_priority, priority);
          failed = 1;
      }
    }

    if (egress_port == pkt->rx_port) {
      if (pkt->prio_int != new_priority || pkt->vlan_pri != new_priority || pkt->vlan_cfi != 0) {
          printf("Egress: new_priority=%d\n",new_priority);
          failed = 1;
      }
    }

    printf("RX : len=%3d rxp=%3d srcp=%3d dstp=%3d rs=%3d cos=%d\n vlan_pri=%d vlan_cfi=%d prio_int=%d\n",
       pkt->tot_len, pkt->rx_port, pkt->src_port, pkt->dest_port,
       pkt->rx_reason, pkt->cos, pkt->vlan_pri, pkt->vlan_cfi, pkt->prio_int);
    ...
    printf("TH3 l2_qos_pkt_verify verification %s.\n", failed ? "failed" : "passed");
