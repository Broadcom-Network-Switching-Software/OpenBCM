/*
ABSTRACT:
  SRv6 Transit for L2-IPv6-SRH-IPv4 with U-SID.

        network port     ---- TD3 ---- access port

 IPv4 in IPv6 with SRH header in USID format    Transit via Host table
...
/* DIP6 in packet is 1234:5678:9abc:defa:aabb:ccdd:eeff:1122 */
bcm_ip6_t sid1 = {0x12,0x34,0x56,0x78,0xde,0xfa,0xaa,0xbb,0xcc,0xdd,0xee,0xff,0x11,0x22,0x00,0x00};
...
/* Set SRV6_CSID_MODE.MODE = 1 */
rv = bcm_switch_control_set(unit, bcmSwitchSrv6SidMode, BCM_SWITCH_SRV6_CSID_MODE_USID);
/* Set SRV6_USID_CONFIG.PREFIX_1_SIZE = 2 */
rv = bcm_switch_control_set(unit, bcmSwitchSrv6UsidPrefix1, BCM_SWITCH_SRV6_USID_PREFIX_32BIT);
printf("USID setting ...... %s\n", bcm_errmsg(rv));
...
/* Flow match setup */
rv = flow_match_dip6_intf_add(unit, "ADAPT_2_LOOKUP_DIP_ASSIGN_SVP_L3_IIF_AND_TUNNEL_TERM",
                              dip6, intf_id_network_1, bcmFlowTunnelSrv6ActionTransitUsid);
printf("Create flow match entry ...... %s\n", bcm_errmsg(rv));
...
char *pkt_data = "";
...
[bcmPW.0]Packet[1]: data[0020]: 0001 0001 0002 0003 0004 1234 5678 defa 
[bcmPW.0]Packet[1]: data[0030]: aabb ccdd eeff 1122 0000 0406 0402 0200 
...
[bcmPW.0]Packet[1]: data[0060]: 03f1 1234 5678 9abc defa aabb ccdd eeff 
[bcmPW.0]Packet[1]: data[0070]: 1122 4538 0084 0000 0000 4006 520c 101a 
...
[bcmPW.0]Packet[1]: data[00f0]: 5657 5859 0003 0c3c ca1a f8c2 393c 
