 ...
 *            payload dest-mac is added for arp termination and ingress payload
 *            is lifted to cpu.
 *
 * Usage    : BCM.0> cint ipv4_riot_NtoA_arp_protocol.c
 *
 * config   : config_td3_flex_riot.bcm
 *
 * Log file : ipv4_riot_NtoA_arp_protocol_log.txt
 *
 * Test Topology :
 *
 *                                  +----------------------+
...
Ingress Hex Packet:
tx port=1 data=0x00000000bbbb00000000aaaa8100000b080045000064000000003f113f6a141414010a0a0a01ffff2118003a000008000000012345000000000101010000000202028100000c08060001080006040002000000010101c0a81401000000020202c0a80a01000102030405060708090A0B0C0DB27ABAC7
...
Packet Dump - ingress packet is lifted to cpu
...
[bcmPW.0]Packet[1]: data[0040]: 0202 8100 000c 0806 0001 0800 0604 0002
[bcmPW.0]Packet[1]: data[0050]: 0000 0001 0101 c0a8 1401 0000 0002 0202
[bcmPW.0]Packet[1]: data[0060]: c0a8 0a01 0001 0203 0405 0607 0809 0a0b
[bcmPW.0]Packet[1]: data[0070]: 0c0d b27a bac7 0f64 346b
...
[bcmPW.0]Packet[1]: reasons: Protocol   >>> Arp packet lifted to cpu with reason
*
*/
...
verify(int unit)
{
    ...
    printf("\nSending known unicast packet to ingress network side:%d\n",
                                                                ingress_port);
    snprintf(str, 512, "tx 1 pbm=%d data=0x00000000bbbb00000000aaaa8100000b080045000064000000003f113f6a141414010a0a0a01ffff2118003a000008000000012345000000000101010000000202028100000c08060001080006040002000000010101c0a81401000000020202c0a80a01000102030405060708090A0B0C0DB27ABAC7; sleep quiet 1", ingress_port);
    bshell(unit, str);
    bshell(unit, "show c");
                     ...
                     bcm_flow_handle_t flow_handle)
{
    ...
    printf("Network: overlay l2 station arp entry for termination\n");
    rv = l2_station_add(unit,vxlan_net->net_ol_mac,vxlan_net->net_vpn,0,
                        BCM_L2_STATION_ARP_RARP | BCM_L2_STATION_IPV4,
                        &l2_station_net_ol);
    if (BCM_FAILURE(rv)) {
        printf("\nError in l2_station_add: %s\n", bcm_errmsg(rv));
        return rv;
