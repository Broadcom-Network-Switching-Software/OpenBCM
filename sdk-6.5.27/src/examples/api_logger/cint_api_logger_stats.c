 ...
 */
void
print_logger_stats_header (int top_border)
{
    if (top_border) {
...
show_api_logger_stats ()
{
    ...
    uint64 api_ovhdus, api_totalus, ovhdus, totalus, avg;
    int arg_ptrs_avg, preload_avg, call1_avg, dispatch_avg, call2_avg, total_avg;
    /*
     * buffers to hold accumulated Overhead and Total run-times as strings
     * showing hours, minutes, seconds, milliseconds and microseconds.
     */
    ...
    for (ii = 0; ii < cint_logger_cfg.next_free_ctxt; ii++) {
         ...
         */
        avg = sp->arg_ptrs.total;
        COMPILER_64_UDIV_32(&avg, sp->called_count);
        arg_ptrs_avg = COMPILER_64_LO(&avg);

        avg = sp->preload.total;
        COMPILER_64_UDIV_32(&avg, sp->called_count);
        preload_avg = COMPILER_64_LO(&avg);

        avg = sp->call1.total;
        COMPILER_64_UDIV_32(&avg, sp->called_count);
        call1_avg = COMPILER_64_LO(&avg);

        avg = sp->dispatch.total;
        COMPILER_64_UDIV_32(&avg, sp->called_count);
        dispatch_avg = COMPILER_64_LO(&avg);

        avg = sp->call2.total;
        COMPILER_64_UDIV_32(&avg, sp->called_count);
        call2_avg = COMPILER_64_LO(&avg);

        avg = sp->total.total;
        COMPILER_64_UDIV_32(&avg, sp->called_count);
        total_avg = COMPILER_64_LO(&avg);

        /* absolute value of overhead in last call and avg */
        ohdl = sp->total.last - sp->dispatch.last;
        ohda = total_avg - dispatch_avg;

        /* percent of overhead in last call and avg */
        ohdlp = (100.0 * ohdl) / sp->total.last;
        ohdap = (100.0 * ohda) / total_avg;

        /* overhead accumulated by this API = total.total - dispatch.total ... */
        api_ovhdus = sp->total.total;
        COMPILER_64_SUB_64(&api_ovhdus, sp->dispatch.total);
        /* ... to be printed as a string ... */
        ...
        printf("|%-40s|%-5d|%-7d|%7d / %-7d|%7d / %-7d|%7d / %-7d|%7d / %-7d|%7d / %-7d|%7d / %-7d|%7d / %-7d|\n",
               ...
               sp->arg_ptrs.last        , arg_ptrs_avg       ,
               sp->preload.last         , preload_avg        ,
               sp->call1.last           , call1_avg          ,
               sp->dispatch.last        , dispatch_avg       ,
               sp->call2.last           , call2_avg          ,
               sp->total.last           , total_avg          ,
               ohdl                     , ohda);
