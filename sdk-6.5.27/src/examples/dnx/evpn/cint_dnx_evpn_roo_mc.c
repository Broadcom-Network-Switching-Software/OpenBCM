    ...
    int core_port_2)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(evpn_roo_mc_iml_add_encapsulation(unit), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(evpn_roo_mc_iml_add_termination(unit), "");
    ...
    int unit)
{
    ...
    for (label_i = 0; label_i < NUM_OF_CORE_PORTS + 1; label_i++)
    {
        ...
        snprintf(error_msg, sizeof(error_msg), "iml %d", label_i);
        BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_initiator_create(unit, 0, 1, plabel), error_msg);
        ...
        snprintf(error_msg, sizeof(error_msg), "for dual homed peer id (%d) of lif (0x%08x)", IML_DUAL_HOME_PEER_PROFILE, lif);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, gport, bcmPortClassEgress, IML_DUAL_HOME_PEER_PROFILE), error_msg);
    }

    /*
     * Add ESI enapsulation (should be ignored)
     */
    BCM_IF_ERROR_RETURN_MSG(evpn_roo_mc_esi_encaps_add(unit), "");
    ...
    int unit)
{
    ...
    for (port_i = 0; port_i < NUM_OF_ACCESS_PORTS; port_i++)
    {
        ...
        snprintf(error_msg, sizeof(error_msg), "port=%d", esi_info.src_port);
        BCM_IF_ERROR_RETURN_MSG(bcm_mpls_esi_encap_add(unit, &esi_info), error_msg);
    }
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_range_action_add(unit,cint_vpls_roo_mc_info.core_overlay_pwe_label,
    ...
    pswitch->flags |= (feature_mpls_term_till_bos ? 0 : BCM_MPLS_SWITCH_EXPECT_BOS);
