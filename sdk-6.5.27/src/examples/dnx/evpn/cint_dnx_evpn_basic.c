 ...
 * Test Scenario - start
  ...
  cint ../../../../src/examples/dnx/utility/cint_sand_utils_l3.c
  cint ../../../../src/examples/dnx/utility/cint_sand_utils_vlan.c
  cint ../../../../src/examples/dnx/utility/cint_sand_utils_vlan_translate.c
...
int has_fl = 0;
int is_evpn_etree = 0;
int is_virtual_esi = 0;
int is_df_switch = 0;
int virtual_ce3_sys_port = 1000;
    ...
    int unit)
{
    ...
    mpls_util_entity[0].mpls_switch_tunnel[0].class_id = cint_evpn_info.vmpi;
    ...
    mpls_util_entity[1].mpls_switch_tunnel[0].class_id = cint_evpn_info.vmpi;;
    ...
    int unit)
{
     ...
     */
    evpn_global_info.dual_homed_peer_profile = *dnxc_data_get(unit, "l2", "vlan_domain", "nof_vlan_domains", NULL) - 20;
    ...
    /*
     * Setting MPLS termination till bos Label enabler.
     * It's enabled(supported) in some devices only.
     */
    evpn_global_info.mpls_term_till_bos_en = *dnxc_data_get(unit, "mpls", "general", "mpls_term_till_bos", NULL);
    ...
    int p2_port)
{
    ...
    if (!is_jr2_system_header_mode(unit)) {
        ...
        BCM_IF_ERROR_RETURN_MSG(field_jr2_comp_evpn_esi_db(unit, jr2_ing_presel_id-1/*presel_id*/), "");

        /* Configure EPMF to drop when dst_port matches learn_extension */
        BCM_IF_ERROR_RETURN_MSG(field_jr2_comp_evpn_egress_filter_db(unit, jr2_egr_presel_id), "");
    }
    ...
    int unit)
{
    ...
    printf("- Create VSI\n");
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_create_with_id(unit, cint_evpn_info.vsi), "");

    printf("- Create MC group\n");
    BCM_IF_ERROR_RETURN_MSG(multicast__open_mc_group(unit, &cint_evpn_info.vsi, 0), "");
    ...
    int *ports)
{
    ...
    while (nof_ports--)
    {
        snprintf(error_msg, sizeof(error_msg), "port=%d", ports[nof_ports]);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, cint_evpn_info.vsi, ports[nof_ports], 0), error_msg);

        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, cint_evpn_info.vsi + 1, ports[nof_ports], 0), error_msg);
    }
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_field_evpn_ingress(unit, evpn_global_info.in_lif_acl_profile_esi), "");
...
struct evpn_es_info_s
{
    ...
    int virtual_sys_port;           /* Used by MC traffic in switching Designated Forwarding PE. It's mapped to the same modport as port.*/
};
    ...
    evpn_es_info_s * esi)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(evpn_configure_port(unit, esi->port), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, esi->vsi, esi->port, 0), "");

    if (esi->vsi)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, esi->vsi, vport->vlan_port_id), "");
    }

    printf("Add ESI filter entry to FP ESI DB\n");
    BCM_IF_ERROR_RETURN_MSG(cint_field_evpn_egress_esi_port_entry_add(unit, dnx_evpn_field_info.egr_fg.context_id, dnx_evpn_field_info.egr_fg.fg_id,
                                                   &esi_port_handle, esi->label, esi->port), "");
    BCM_IF_ERROR_RETURN_MSG(cint_field_evpn_egress_esi_entry_add(unit, dnx_evpn_field_info.egr_fg.context_id, dnx_evpn_field_info.egr_fg.fg_id,
    ...
    {
        bcm_gport_t gport;
        /** In DF swicth tests, using the virtual gport for MC.*/
        if (esi->virtual_sys_port == BCM_GPORT_INVALID || !is_df_switch)
        {
            BCM_GPORT_MODPORT_SET(gport, unit, esi->port);
        }
        else
        {
            gport = esi->virtual_sys_port;
        }
        BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit, esi->vsi, &gport,
    ...
    if (is_evpn_etree && vport->group == 8)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_port_wide_data_set(unit, vport->vlan_port_id, BCM_PORT_WIDE_DATA_INGRESS, wide_data), "");

        bcm_port_class_t pclass = bcmPortClassFieldIngressVport;
        if (*dnxc_data_get(unit, "lif", "in_lif", "use_mapped_in_lif_profile_field_mapping_table", NULL))
        {
            pclass = bcmPortClassFieldIngressVportCs; 
        }

        printf("Set In-LIF profile to select E-Tree FP ingress application\n");
        snprintf(error_msg, sizeof(error_msg), "inAC 0x%08X", vport->vlan_port_id);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit,vport->vlan_port_id, pclass, evpn_global_info.in_lif_acl_profile_esi), error_msg);
    }
    ...
    int ce3_port)
{
    ...
    esi_200.virtual_sys_port = BCM_GPORT_INVALID;
    BCM_IF_ERROR_RETURN_MSG(evpn_es_add(unit, &esi_200), "");
    ...
    esi_300.virtual_sys_port = is_df_switch ? virtual_ce3_sys_port : BCM_GPORT_INVALID;
    BCM_IF_ERROR_RETURN_MSG(evpn_es_add(unit, &esi_300), "");
    ...
    int p2_port)
{
     ...
     * Set In-Port vmpi
     */
    int vmpi_support = *dnxc_data_get(unit, "mpls", "general", "per_port_ilm_lookup_supported", NULL);
    if (vmpi_support)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit,p1_port,bcmPortClassMplsForwardIngress,cint_evpn_info.vmpi), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit,p2_port,bcmPortClassMplsForwardIngress,cint_evpn_info.vmpi), "");
    }
    ...
    bcm_mpls_egress_label_t * plabel)
{
    ...
    printf("- Create egress IML label %d initiator\n", iml_info->iml_label);
    ...
    plabel->flags2 |= (is_virtual_esi ? BCM_MPLS_EGRESS_LABEL_FLAGS2_ESI_INTF_NAMESPACE : 0);
    ...
    if (iml_info->dual_homed_peer_profile)
    {
        ...
        snprintf(error_msg, sizeof(error_msg), "dual homed peer id (%d) of lif (0x%08x)", iml_info->dual_homed_peer_profile, gport);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, gport, bcmPortClassEgress, iml_info->dual_homed_peer_profile), error_msg);
    }
    ...
    if (iml_info->add_to_flooding)
    {
        if (iml_info->lsp_tunnel_id > 0)
        {
            ...
            snprintf(error_msg, sizeof(error_msg), "ID = 0x%08x", mc_ext_id);
            BCM_IF_ERROR_RETURN_MSG(bcm_multicast_encap_extension_create(unit, 0, &mc_ext_id, 2, cuds), error_msg);
            printf(" - Created PPMC entry with ID = 0x%08x\n", mc_ext_id);

            printf("- Add to PPMC pointer and port to MC group\n");
            BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit, cint_evpn_info.vsi,
    ...
    if (evpn_global_info.use_split_horizon)
    {
        ...
        if (nwk_grp_from_esem)
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_port_match_add(unit, evpn_global_info.default_native_ac.vlan_port_id, &match_info), "");
        }
    }
    ...
    int unit)
{
    ...
    if (evpn_global_info.use_split_horizon && nwk_grp_from_esem)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &evpn_global_info.default_native_ac), "");
    }

    /** JR2: Create fabric MC for each core in case of egress Multicast. */
    if (egress_mc && (!is_jr2_system_header_mode(unit)) && (*dnxc_data_get(unit, "fabric", "general", "blocks_exist", NULL)))
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_fabric_multicast_set(unit, cint_evpn_info.vsi, 0, nof_cores, mreps), "");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(iml_encap_add(unit, &pe1_pe2_iml_info, &evpn_global_info.pe1_pe2_iml_egress_label), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(iml_encap_add(unit, &pe1_pe3_iml_info, &evpn_global_info.pe1_pe3_iml_egress_label), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(iml_encap_add(unit, &pe1_pe4_iml_info, &evpn_global_info.pe1_pe4_iml_egress_label), "");
    ...
    if (is_virtual_esi)
    {
        pe1_pe2_esi_200_encap.flags = BCM_MPLS_ESI_INTF_NAMESPACE;
        BCM_L3_ITF_SET(pe1_pe2_esi_200_encap.src_encap_id, BCM_L3_ITF_TYPE_LIF,
                       BCM_GPORT_SUB_TYPE_LIF_VAL_GET(evpn_global_info.ce1_vlan_port.vlan_port_id));
    }   
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_esi_encap_add(unit, &pe1_pe2_esi_200_encap), "esi-200");
    ...
    if (!is_evpn_etree)
    {
        ...
        if (is_virtual_esi)
        {
            pe1_pe2_esi_200_encap.flags = BCM_MPLS_ESI_INTF_NAMESPACE;
            BCM_L3_ITF_SET(pe1_pe2_esi_200_encap.src_encap_id, BCM_L3_ITF_TYPE_LIF,
                           BCM_GPORT_SUB_TYPE_LIF_VAL_GET(evpn_global_info.ce3_vlan_port.vlan_port_id));
        }
        BCM_IF_ERROR_RETURN_MSG(bcm_mpls_esi_encap_add(unit, &pe1_pe2_esi_300_encap), "esi-300");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_esi_encap_add(unit, &pe1_pe4_esi_400_encap), "esi-400");
    ...
    int with_stat)
{
    ...
    pswitch->flags2 |= (with_stat ? BCM_MPLS_SWITCH2_STAT_ENABLE : 0);
    if ((!expect_esi) && (!evpn_global_info.mpls_term_till_bos_en))
    {
        pswitch->flags |= BCM_MPLS_SWITCH_EXPECT_BOS;
    }
    if (has_fl && !evpn_global_info.mpls_term_till_bos_en)
    {
        pswitch->flags |= BCM_MPLS_SWITCH_ENTROPY_ENABLE;
    }
    pswitch->label = iml_label;
    pswitch->vpn = vsi;
    pswitch->class_id = cint_evpn_info.vmpi;
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_switch_create(unit, pswitch), "");
    ...
    /*
     * evpn_global_info.mpls_term_till_bos_en == 0: If ESI is expected, Set In-LIF profile for iPMF selection of ESI filter application
     * evpn_global_info.mpls_term_till_bos_en == 1: If has_fl == 1, set the In-LIF profile for iPMF selection
     */
    if ((expect_esi && !evpn_global_info.mpls_term_till_bos_en) ||
        (has_fl && evpn_global_info.mpls_term_till_bos_en))
    {
    ...
    int vsi)
{
    ...
    tunnel->vpn = vsi;
    tunnel->class_id = cint_evpn_info.vmpi;
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_switch_create(unit, tunnel), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_range_action_add(unit,cint_evpn_info.iml_range_low_label,
    ...
    /** If mpls_term_till_bos is available, no need to add the 2nd IML term entry.*/
    if (!(evpn_global_info.mpls_term_till_bos_en || has_fl)) {
        printf("%s(): Add IML termination without ESI\n", proc_name);
        BCM_IF_ERROR_RETURN_MSG(iml_term_add(unit, vsi, cint_evpn_info.pe1_pe2_iml_label,
    ...
    int unit)
{
    ...
    l2_addr.learn_strength = learn_strength;
    ...
    int p2_port)
{
    ...
    printf("Create ETH segments\n");
    BCM_IF_ERROR_RETURN_MSG(evpn_p2p_eth_segments_create(unit, vswitch_ports[0], vswitch_ports[1]), "");

    printf("Setup MPLS L3 network\n");
    BCM_IF_ERROR_RETURN_MSG(evpn_mpls_core_setup(unit, vswitch_ports[2], vswitch_ports[3]), "");

    printf("Setup EVPN Services\n");
    BCM_IF_ERROR_RETURN_MSG(evpn_p2p_service_egress_config(unit), "");
    BCM_IF_ERROR_RETURN_MSG(evpn_service_ingress_config(unit, 0/** vsi */), "");

    printf("Cross connect CEs to UC EVPN services");
    BCM_IF_ERROR_RETURN_MSG(evpn_p2p_cross_connect_uc(unit), "");

    printf("Cross connect IML to CEs MC group\n");
    BCM_IF_ERROR_RETURN_MSG(evpn_p2p_cross_connect_mc(unit), "");
    ...
    int ce3_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(evpn_es_add(unit, &esi_200), "ce1_vlan");
    ...
    BCM_IF_ERROR_RETURN_MSG(evpn_es_add(unit, &esi_300), "ce1_vlan");
    ...
    int unit)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "AC(0x%08X) to EVPN(0x%08X)",
    ...
    snprintf(error_msg, sizeof(error_msg), "EVPN(0x%08X) to AC(0x%08X)",
    ...
    int unit)
{
    ...
    printf("- Create MC group\n");
    BCM_IF_ERROR_RETURN_MSG(multicast__open_mc_group(unit, &mc_id, egress_mc), "");
    ...
    if (egress_mc && (!is_jr2_system_header_mode(unit)) && (*dnxc_data_get(unit, "fabric", "general", "blocks_exist", NULL)))
    {
        ...
        mreps[0] = 0;
        mreps[1] = 1; /* Ignore it in single core devices*/
        BCM_IF_ERROR_RETURN_MSG(bcm_fabric_multicast_set(unit, mc_id, 0, nof_cores, mreps), "");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit, mc_id, ce_ports, ce_encaps, 2, egress_mc), "");
    ...
    if (!(evpn_global_info.mpls_term_till_bos_en || has_fl)) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_cross_connect_add(unit, &gports), "AC to EVPN");
    }
    ...
    int p2_port)
{
    ...
    printf("Setup EVPN Services\n");
    BCM_IF_ERROR_RETURN_MSG(evpn_php_service_egress_config(unit), "");
    BCM_IF_ERROR_RETURN_MSG(evpn_service_ingress_config(unit, cint_evpn_info.vsi), "");

    printf("Connect ES to egress in UC flow\n");
    BCM_IF_ERROR_RETURN_MSG(evpn_php_egress_uc_l2_address_add(unit), "");
    ...
    int unit)
{
    ...
    pe1_pe2_iml_info.dual_homed_peer_profile = evpn_global_info.dual_homed_peer_profile;
    pe1_pe2_iml_info.out_port = *(mpls_util_entity[1].fecs[0].port);
    BCM_IF_ERROR_RETURN_MSG(iml_encap_add(unit, &pe1_pe2_iml_info, &evpn_global_info.pe1_pe2_iml_egress_label), "pe1 to pe2");
    BCM_IF_ERROR_RETURN_MSG(evpn_php_egress_next_encap_add(unit, &evpn_global_info.pe1_pe2_iml_egress_label,
                                        mpls_util_entity[1].arps[0].arp), "pe1 to pe2");
    ...
    pe1_pe3_iml_info.dual_homed_peer_profile = 0; /* Peer is not dual homed to this PE */
    pe1_pe3_iml_info.out_port = *(mpls_util_entity[0].fecs[0].port);
    BCM_IF_ERROR_RETURN_MSG(iml_encap_add(unit, &pe1_pe3_iml_info, &evpn_global_info.pe1_pe3_iml_egress_label), "pe1 to pe3");
    BCM_IF_ERROR_RETURN_MSG(evpn_php_egress_next_encap_add(unit, &evpn_global_info.pe1_pe3_iml_egress_label,
                                        mpls_util_entity[0].mpls_encap_tunnel[0].tunnel_id), "pe1 to pe3");
    ...
    pe1_pe4_iml_info.dual_homed_peer_profile = evpn_global_info.dual_homed_peer_profile - 1; /* For testing, Peer is
                                                                                                dual homed to this PE */
    pe1_pe4_iml_info.out_port = *(mpls_util_entity[0].fecs[1].port);
    BCM_IF_ERROR_RETURN_MSG(iml_encap_add(unit, &pe1_pe4_iml_info, &evpn_global_info.pe1_pe4_iml_egress_label), "pe1 to pe4");
    BCM_IF_ERROR_RETURN_MSG(evpn_php_egress_next_encap_add(unit, &evpn_global_info.pe1_pe4_iml_egress_label,
                                        mpls_util_entity[0].mpls_encap_tunnel[1].tunnel_id), "pe1 to pe4");
    ...
    bcm_if_t egress_if)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_initiator_create(unit, 0, 1, eg_label), "");
    ...
    int p2_port)
{
    ...
    printf("Setup EVPN Services\n");
    BCM_IF_ERROR_RETURN_MSG(evpn_cw_service_egress_config(unit), "");
    BCM_IF_ERROR_RETURN_MSG(evpn_cw_service_ingress_config(unit, cint_evpn_info.vsi), "");
    ...
    int unit)
{
    ...
    pe1_pe2_iml_info.lsp_tunnel_id = *(mpls_util_entity[1].fecs[0].tunnel_gport);
    pe1_pe2_iml_info.out_port = *(mpls_util_entity[1].fecs[0].port);
    BCM_IF_ERROR_RETURN_MSG(iml_cw_encap_add(unit, &pe1_pe2_iml_info, &evpn_global_info.pe1_pe2_iml_egress_label), "pe1 to pe2");
    ...
    pe1_pe3_iml_info.lsp_tunnel_id = *(mpls_util_entity[0].fecs[0].tunnel_gport);
    pe1_pe3_iml_info.out_port = *(mpls_util_entity[0].fecs[0].port);
    BCM_IF_ERROR_RETURN_MSG(iml_cw_encap_add(unit, &pe1_pe3_iml_info, &evpn_global_info.pe1_pe3_iml_egress_label), "pe1 to pe3");
    ...
    bcm_mpls_egress_label_t * plabel)
{
    ...
    {
        printf("- Set dual homed peer profile on IML label lif\n");
        snprintf(error_msg, sizeof(error_msg), "for dual homed peer id (%d) of lif (0x%08x)", iml_info->dual_homed_peer_profile, gport);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, gport, bcmPortClassEgress, iml_info->dual_homed_peer_profile), error_msg);
    }
    ...
    int vsi)
{
    ...
    if (!(evpn_global_info.mpls_term_till_bos_en || has_fl)) {
        /**If the feature term_till_bos is available, no need to add the 2nd IML term entry.*/
        printf("%s(): Add IML termination without ESI\n", proc_name);
        BCM_IF_ERROR_RETURN_MSG(iml_cw_term_add(unit, vsi, cint_evpn_info.pe1_pe2_iml_label,
                            &evpn_global_info.pe1_pe2_iml_tunnel_switch_no_esi,
                            0 /** No ESI **/), "no ESI");
    }
    ...
    int expect_esi)
{
     ...
     * In the device with mpls_term_till_bos, the in-lif-profile indicates FL
     */
    if ((expect_esi && !evpn_global_info.mpls_term_till_bos_en) ||
        (has_fl && evpn_global_info.mpls_term_till_bos_en))
    {
    ...
    int p2_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(evpn_main(unit, ce1_port, ce3_port, p1_port, p2_port), "");

    printf("Setting flooding group profiles to EVPN/IML lifs\n");

    BCM_IF_ERROR_RETURN_MSG(evpn_flood_group_profile_set(unit, evpn_global_info.evpn_in_tunnel.tunnel_id, 1), "evpn");
    if (!evpn_global_info.mpls_term_till_bos_en) {
        /** If the device support to terminate till bos in one LIF, this tunnel-id (LIF) won't be created.*/
        BCM_IF_ERROR_RETURN_MSG(evpn_flood_group_profile_set(unit, evpn_global_info.pe1_pe2_iml_tunnel_switch_no_esi.tunnel_id, 2), "iml no esi");
    }

    BCM_IF_ERROR_RETURN_MSG(evpn_flood_group_profile_set(unit, evpn_global_info.pe1_pe2_iml_tunnel_switch_esi.tunnel_id, 3), "iml with esi");
    ...
    int flooding_group_profile)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_flood_group_set(unit, gport, 0, &flood_groups), "");
    ...
    int p2_port)
{
    ...
    evpn_global_info.recycle_entry_encap_id = recycle_entry.encap_id;

    if (!ibch1_supported)
    {
        /** configure port as recycle port, Part of context selection at 2nd pass VTT1.*/
        BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit, recyle_port, bcmPortControlRecycleApp, bcmPortControlRecycleAppExtendedEncapUncollapse), "");
    }
    /** Set port class in order for PMF to crop RCH ad IRPP */
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, recyle_port, bcmPortClassFieldIngressPMF3TrafficManagementPortCs, 4), "");
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        *(mpls_util_entity[i].mpls_encap_tunnel[0].l3_intf_id) = recycle_entry.encap_id;
        ...
        BCM_IF_ERROR_RETURN_MSG(mpls_create_initiator_tunnels(unit, mpls_util_entity[i].mpls_encap_tunnel, 2), "");
    }
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_create(unit, BCM_L3_INGRESS_ONLY, &l3_egr, &(dst_fec[i])), "");
    }
    ...
    /*
     * In case this, we expected the LSP label is swaped to lsp+1 at 2nd pass. 
     * So delete additional mpls termination label to avoid mpls termination at 2nd pass.
     */
    bcm_mpls_tunnel_switch_t entry;
    bcm_mpls_tunnel_switch_t_init(&entry);
    entry.label = cint_evpn_info.pe1_pe2_lsp_label;
    entry.action = BCM_MPLS_SWITCH_ACTION_POP;
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_switch_delete(unit, &entry), "pe1_pe2_lsp_label");
    bcm_mpls_tunnel_switch_t_init(&entry);
    entry.label = cint_evpn_info.pe1_pe3_lsp_label;
    entry.action = BCM_MPLS_SWITCH_ACTION_POP;
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_switch_delete(unit, &entry), "pe1_pe3_lsp_label");

    return BCM_E_NONE;
}

/*
 * Simple example to switching Designated Forwarding(DF) PE in EVPN. The CE may be multi-homed.
 * But only the DF PE is allowed to flood the BUM traffic to the multi-homed CE.
 * There are many solutions to switching DF PE from one to another, incluing
 * 1) removing the CE from the original MC group and adding it to the new PE MC;
 * 2) adding the CE to all related PE MC groups and adding filter (e.g. by epmf) in non-DF PE;
 * 3) creating separate outAC LIFs for the CE for BUM traffic in all related PEs, adding the CE to
 *    all the related PE MC groups with the AC LIFs. Then binding the AC-LIFs to a failover_id;
 * 4) define virtual-system-port for the CE. The virtual-system-port is mapped to the same modport
 *    as the orginal system-port. adding the virtual-system-port intead of the orignal system-port
 *    to the MC groups in all related PEs. Enable the virtual-system-port in DF PE only.
 *
 * Here the example is based on solution 4. The CE here is CE3, and related PEs are PE1 and PE2.
 *
 */
int
evpn_bum_df_switch_port_set(
    int unit,
    bcm_gport_t virtual_sys_port,
    bcm_port_t port,
    int enable)
{
    int rv;
    bcm_gport_t org_sys_port;
    bcm_gport_t voq_gport;
    bcm_port_mapping_info_t mapping_info;
    bcm_port_interface_info_t interface_info;
    int nof_mods = *dnxc_data_get(unit, "device", "general", "max_nof_fap_ids_per_device", NULL);
    uint32 flags;
    char error_msg[200]={0,};

    snprintf(error_msg, sizeof(error_msg), "port %d", port);
    BCM_IF_ERROR_RETURN_MSG(bcm_stk_gport_sysport_get(unit, port, &org_sys_port), error_msg);

    /** Check if the virtual_sys_port is mapped to a modport already, if not add the maps.*/
    int modport_gport = 0, org_modport_gport, modid;
    rv = bcm_stk_sysport_gport_get(unit, virtual_sys_port, &modport_gport);
    modid = BCM_GPORT_MODPORT_MODID_GET(modport_gport);
    if (rv == BCM_E_NOT_FOUND || modid > nof_mods)
    {
        snprintf(error_msg, sizeof(error_msg), "port %d", port);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_get(unit, port, &flags, &interface_info, &mapping_info), error_msg);

        snprintf(error_msg, sizeof(error_msg), "port %d", org_sys_port);
        BCM_IF_ERROR_RETURN_MSG(bcm_stk_sysport_gport_get(unit, org_sys_port, &org_modport_gport), error_msg);

        snprintf(error_msg, sizeof(error_msg), "with sysport %d, port %d", virtual_sys_port, org_modport_gport);
        BCM_IF_ERROR_RETURN_MSG(bcm_stk_sysport_gport_set(unit, virtual_sys_port, org_modport_gport), error_msg);
    }

    snprintf(error_msg, sizeof(error_msg), "with port port %d", org_sys_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_sysport_ingress_queue_map_get(unit, 0, org_sys_port, &voq_gport), error_msg);

    /** Map the virtual system port to the same VOQ as the UC port*/
    flags = enable ? 0 : BCM_COSQ_SYSPORT_INGRESS_QUEUE_UNMAP;
    snprintf(error_msg, sizeof(error_msg), "port 0x%x, voq 0x%x", virtual_sys_port, voq_gport);
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_sysport_ingress_queue_map_set(unit, flags, virtual_sys_port, voq_gport), error_msg);
    
    return BCM_E_NONE;
}

/*
 * The fucntion is based on evpn_main().
 */
int evpn_df_switch_main(
    int unit,
    int ce1_port,
    int ce3_port,
    int p1_port,
    int p2_port)
{
    ...
    is_df_switch = 1;
    BCM_GPORT_SYSTEM_PORT_ID_SET(virtual_ce3_sys_port, virtual_ce3_sys_port);

    snprintf(error_msg, sizeof(error_msg), "port %d", virtual_ce3_sys_port);
    BCM_IF_ERROR_RETURN_MSG(evpn_bum_df_switch_port_set(unit, virtual_ce3_sys_port, ce3_port, TRUE), error_msg);

    return evpn_main(unit, ce1_port, ce3_port, p1_port, p2_port);
}
