/*
 * File: cint_dnx_vpls_mp_basic.c Purpose: basic example for VPLS.
 */
 ...
 * Test Scenario - start
  ...
  cint
  vpls_mp_basic_main(0,200,201);
  exit;
 *
 * pwe -> ac
    tx 1 psrc=201 data=0x000c0002000100000000cd1d81000000884700d0504000d80140001100000112000c0002000091000005ffff000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20
 *
 * Received packets on unit 0 should be:
 *   Data: 0x001100000112000c0002000091000005ffff000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20
 *
 * ac -> pwe
    tx 1 psrc=200 data=0x000c0002000000010203040591000005ffff000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20
 *
 * Received packets on unit 0 should be:
 *   Data: 0x00000000cd1d000c000200018100001e884700d050fe00d801fe000c0002000000010203040591000005ffff000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f200000000000000000000000
 * Test Scenario - end
 *
 */
...
int is_connect_lsp_to_pwe_lif = 0;
    ...
    int pwe_port)
{
     ...
     */
    snprintf(error_msg, sizeof(error_msg), "port=%d", ac_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, ac_port, bcmPortClassId, ac_port), error_msg);

    snprintf(error_msg, sizeof(error_msg), "port=%d", pwe_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, pwe_port, bcmPortClassId, pwe_port), error_msg);
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(out_port_set(unit, cint_vpls_basic_info.pwe_port), "intf_out");

    if (!is_section_oam) {
        /*
         * Set In-Port to In ETh-RIF
         */
        BCM_IF_ERROR_RETURN_MSG(in_port_intf_set(unit, cint_vpls_basic_info.pwe_port, cint_vpls_basic_info.pwe_intf), "intf_in");
    } else {
        /*
         * Configuring and associating symmetric LIF with the ingress port in case of Section OAM
         */
        BCM_IF_ERROR_RETURN_MSG(in_port_section_oam_intf_set(unit, cint_vpls_basic_info.pwe_port), "");
    }
    /*
     * Set Out-Port default properties
     */
    BCM_IF_ERROR_RETURN_MSG(out_port_set(unit, cint_vpls_basic_info.ac_port), "intf_out");
    ...
    int unit)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(intf_eth_rif_create(unit, cint_vpls_basic_info.pwe_intf, cint_vpls_basic_info.pwe_intf_mac_address),
        "pwe_intf");

    if (pwe_protection_with_one_fec)
    {
        BCM_IF_ERROR_RETURN_MSG(intf_eth_rif_create(unit, cint_vpls_basic_info.pwe_intf_primary, cint_vpls_basic_info.pwe_intf_mac_address),
            "pwe_intf");
    }
    ...
    int unit)
{
     ...
     * Configure ARP entry
     */
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_encap__create(unit, 0, cint_vpls_basic_info.core_arp_id,
        cint_vpls_basic_info.pwe_arp_next_hop_mac, cint_vpls_basic_info.pwe_intf), "create AC egress object ARP only");

    if (pwe_protection_with_one_fec)
    {
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_encap__create(unit, 0, cint_vpls_basic_info.core_arp_id_primary,
            cint_vpls_basic_info.pwe_arp_next_hop_mac, cint_vpls_basic_info.pwe_intf_primary), "create AC egress object ARP only");
    }
    ...
    bcm_vpn_t vpn)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_FEC, &cint_vpls_basic_info.pwe_failover_id_fec),
        "for PWE FEC");
    ...
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_ENCAP, &cint_vpls_basic_info.pwe_failover_id_outlif),
            "for PWE Egress");
    ...
    int unit)
{
    ...
    /**
     * Under JR1 system headers mode, this case tests the FEC format of dest+EEI:
     */
    if (*(dnxc_data_get(unit, "headers", "system_headers", "system_headers_mode", NULL)) == 0)
    {
        flags |= BCM_L3_ENCAP_SPACE_OPTIMIZED;
    }
    if (*(dnxc_data_get(unit, "l3", "feature", "separate_fwd_rpf_dbs", NULL)) != 0)
    {
    ...
    {
        BCM_IF_ERROR_RETURN_MSG(get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &cint_vpls_basic_info.mpls_encap_fec_id), "");
    ...
    if (pwe_protection_with_one_fec)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, cint_vpls_basic_info.mpls_encap_fec_id,
            out_rif_or_tunnel, 0, cint_vpls_basic_info.pwe_port, flags, flags2, cint_vpls_basic_info.pwe_failover_id_fec, 0, NULL),
        ...
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, cint_vpls_basic_info.mpls_encap_fec_id_primary,
            out_rif_or_tunnel, 0, cint_vpls_basic_info.pwe_port, flags, flags2, cint_vpls_basic_info.pwe_failover_id_fec, 1, NULL),
            "create egress object FEC only");
    }
    else
    {
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create(unit, cint_vpls_basic_info.mpls_encap_fec_id,
            cint_vpls_basic_info.mpls_tunnel_id, 0, cint_vpls_basic_info.pwe_port, flags, flags2),
            "create egress object FEC only");
    }
    ...
    bcm_vpn_t vpn)
{
    ...
    uint32 l2_flags, learn_strength = 0;
    ...
    int unit)
{
    ...
    for (is_primary = 0; is_primary < nof_objects; is_primary++)
    {
        ...
        if (is_connect_lsp_to_pwe_lif) {
            mpls_port.egress_tunnel_if = cint_vpls_basic_info.mpls_tunnel_id;
        }
    ...
    bcm_gport_t port_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, cint_vpls_basic_info.vpn, vlan_port.vlan_port_id), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, cint_vpls_basic_info.vpn, port_id, 0), "");
...
vpls_mp_basic_ive_swap(int unit, int port)
{
    ...
    /* configure ive swap */
    BCM_IF_ERROR_RETURN_MSG(ive_eve_translation_set( unit,  cint_vpls_basic_info.vlan_port_id, 0x8100, 0, bcmVlanActionReplace, bcmVlanActionNone,  1111,  0, 5 /*edit_profile*/, 4 /*tag_format*/, 1), "");

    int next_layer_network_domain = vpls_mpls_next_layer_network_domain_value(unit, 3);
    snprintf(error_msg, sizeof(error_msg), "port=0x%x", cint_vpls_basic_info.mpls_port_id_ingress);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, cint_vpls_basic_info.mpls_port_id_ingress, bcmPortClassIngress, next_layer_network_domain), error_msg);
    ...
    /* configure ive swap */
    BCM_IF_ERROR_RETURN_MSG(ive_eve_translation_set( unit,  vlan_port.vlan_port_id, 0x8100, 0, bcmVlanActionReplace, bcmVlanActionNone,  1111,  0, 5 /*edit_profile*/, 8 /*tag_format*/, 1), "");
    ...
    int pwe_port)
{
    ...
    if (pwe_protection_with_one_fec)
    {
         ...
         */
        BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_create_failover(unit, cint_vpls_basic_info.vpn), "");
    }
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_configure_port_properties(unit, ac_port, pwe_port), "");

    /*
     * create vlan based on the vsi (vpn)
     */
    snprintf(error_msg, sizeof(error_msg), "Failed to create VLAN %d", cint_vpls_basic_info.vpn);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, cint_vpls_basic_info.vpn), error_msg);

    if(is_section_oam) {
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, cint_vpls_basic_info.pwe_intf), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, cint_vpls_basic_info.pwe_intf, section_oam_in_gport), "");
    }

    /*
     * Configure L3 interfaces
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_create_l3_interfaces(unit), "");

    /*
     * Configure an ARP entries
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_create_arp_entry(unit), "");

    /*
     * Configure a push entry.
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_create_mpls_tunnel(unit), "");

    /*
     * Configure fec entry
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_create_fec_entry(unit), "");

    /*
     * Configure a termination label for the ingress flow
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_create_termination_stack(unit), "");

    /*
     * configure PWE tunnel - egress flow
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_mpls_port_add_encapsulation(unit), "");

    /*
     * configure PWE tunnel - ingress flow
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_mpls_port_add_termination(unit, cint_vpls_basic_info.vpn), "");

    /** define ac and pwe ports */
    BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_vswitch_add_access_port(unit, ac_port), "");

    /*
     * add l2 addresses to be defined as static - no learning needed
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mp_basic_l2_addr_add(unit, cint_vpls_basic_info.vpn), "");
