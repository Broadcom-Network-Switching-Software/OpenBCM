...
vpls_hvpls_mpls_tunnels_create(int unit)
{
    ...
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_ENCAP, &vpls_hvpls_info.mpls_failover_id_outlif),
            "for MPLS Egress");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_create_initiator_tunnels(unit, mpls_encap_tunnel_2, 1),
        "for secondary MPLS tunnel");
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_create_initiator_tunnels(unit, mpls_encap_tunnel_1, 1), "for primary MPLS tunnel");
...
vpls_hvpls_mpls_fec_create(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, flags, &vpls_hvpls_info.mpls_failover_id_fec), "for MPLS FEC");
    ...
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, vpls_hvpls_info.mpls_fec, vpls_hvpls_info.mpls_tunnel_id_protected,
        0, gport, BCM_L3_2ND_HIERARCHY, fec_flags2, vpls_hvpls_info.mpls_failover_id_fec, 0, &encoded_fec), "for secondary FEC");


    /* Primary FEC is even */
    vpls_hvpls_info.mpls_fec = vpls_hvpls_info.mpls_fec - 1;
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, vpls_hvpls_info.mpls_fec, vpls_hvpls_info.mpls_tunnel_id_primary,
        0, gport, BCM_L3_2ND_HIERARCHY, fec_flags2, vpls_hvpls_info.mpls_failover_id_fec, 1, &encoded_fec), "for primary FEC");
...
vpls_hvpls_mpls_port_add_encapsulation(int unit)
{
    ...
    if (*dnxc_data_get(unit, "multicast", "params", "mc_pruning_actions_not_supported", NULL)) 
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_ENCAP, &vpls_hvpls_info.pwe_failover_id_outlif),
            "for PWE Egress");
...
vpls_hvpls_pwe_fec_create(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_FEC, &vpls_hvpls_info.pwe_failover_id_fec), "for PWE FEC");
    printf("PWE Failover id: 0x%x\n", vpls_hvpls_info.pwe_failover_id_fec);

    /* Secondary */
    if (mc_in_fec) {
        BCM_GPORT_MCAST_SET(gport, mc_group);
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, vpls_hvpls_info.pwe_fec, 0,
            0, gport, 0, fec_flags2, vpls_hvpls_info.pwe_failover_id_fec, 0, &encoded_fec), "for secondary FEC");
    } else {
        ...
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, vpls_hvpls_info.pwe_fec, lif_encode,
            0, gport, 0, fec_flags2, vpls_hvpls_info.pwe_failover_id_fec, 0, &encoded_fec), "for secondary FEC");
    }
    ...
    if (mc_in_fec) {
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, vpls_hvpls_info.pwe_fec, 0,
            0, gport, 0, fec_flags2, vpls_hvpls_info.pwe_failover_id_fec, 1, &encoded_fec), "for primary FEC");
    } else {
...
vpls_hvpls_mpls_port_add_termination(int unit)
{
    ...
    for (is_primary=0; is_primary <= 1; is_primary++) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_mpls_port_add(unit, vpn, &mpls_port_ingress), "");
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_INGRESS_MC, &vpls_hvpls_info.mpls_failover_id_outlif),
        "of LSP egress");

    /* create egress failover id for PWE */
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_INGRESS_MC, &vpls_hvpls_info.pwe_failover_id_outlif),
        "for PWE Egress");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_create(unit, flags, pri_pwe_prune_group_id), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_create(unit, flags, backup_pwe_prune_group_id), "");
    ...
    snprintf(error_msg, sizeof(error_msg), "cud0 = 0x%08X, cud1 = 0x%08X", cuds[0], cuds[1]);
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_encap_extension_create(unit, 0, &rep_index, 2, cuds), error_msg);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, mc_id, flags, 1, reps), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, mc_id, flags, 2, reps), "");
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_set(unit, mc_id, flags, 5, reps), "");
    ...
    int no_learning)
{
    ...
    /* Init parameters */
    BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_info_init(unit, port1, port2), "");

    /*
     * create VPN and multicast group
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_vswitch_create(unit, vpn), "");

    /*
     * Configure AC and PWE ports
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_add_access_port(unit, ac_port, vpn), "ac_port");

    BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_add_core_port(unit, pwe_port), "pwe_port");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_mpls_tunnels_create(unit), "");

    /*
     * Configure fec entry
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_mpls_fec_create(unit), "");

    /*
     * configure PWE tunnel - egress flow, set next pointer to MPLS tunnel
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_mpls_port_add_encapsulation(unit), "");

    /*
     * configure PWE FEC
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_pwe_fec_create(unit), "");

    /*
     * configure PWE tunnel - ingress flow
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_mpls_port_add_termination(unit), "");

    if (no_learning)
    {
        /* add l2 addresses to be defined as static - no learning needed */
        BCM_GPORT_FORWARD_PORT_SET(gport, vpls_hvpls_info.pwe_fec);
        BCM_IF_ERROR_RETURN_MSG(vpls_l2_addr_add(unit, vpn, pwe_mac, gport, ac_port.stat_id, ac_port.stat_pp_profile), "");

        BCM_IF_ERROR_RETURN_MSG(vpls_l2_addr_add(unit, vpn, ac_mac, ac_port.vlan_port_id, pwe_port.stat_id, pwe_port.stat_pp_profile), "");
    } else {

        if (!(*dnxc_data_get(unit, "multicast", "params", "mc_pruning_actions_not_supported", NULL)))
        {
            BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_ingress_prune_mc_set(unit), "");

        } 
        else
        {
            BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_legacy_mc_set(unit), "");
        }
   }
    ...
    int no_learning)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(vpls_hvpls_add_access_port(unit, ac_port_2, vpn), "ac_port");
...
int vpls_hvpls_failover_set(int unit, uint8 is_pwe, int enable)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_set(unit, failover_id, enable), error_msg);

    egress_enable = enable;
    if (egress_failover_id)
    {
        snprintf(error_msg, sizeof(error_msg), "failover_id:  0x%08x", egress_failover_id);
        BCM_IF_ERROR_RETURN_MSG(bcm_failover_set(unit, egress_failover_id, egress_enable), error_msg);
    }
