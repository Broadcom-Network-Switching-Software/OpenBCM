    ...
    int unit)
{
    ...
    */
    ...
    vlan_params[0].class_id = (*dnxc_data_get(unit, "port", "general", "ingress_vlan_domain_size", NULL) > 9) ? 2019 : 507;
    ...
    */
    ...
    vlan_params[1].class_id = (*dnxc_data_get(unit, "port", "general", "ingress_vlan_domain_size", NULL) > 9) ? 2020 : 508;
    ...
    pwe_tagged_mode_split_horizon_native_ac_s* native_ac)
{
    ...
    if (native_ac->is_indexed)
    {
        ...
        vlan_port.vlan_port_id = native_ac->ingress_vlan_port_id;
        if (vswitch_vpls_params.test_split_horizon)
        {
    ...
    pwe_tagged_mode_split_horizon_vlan_params_s* vlan_params)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "for 0x%x", vlan_params->outer_tpid);
    BCM_IF_ERROR_RETURN_MSG(tpid__tpid_add(unit, vlan_params->outer_tpid), error_msg);

    /* add TPID to global array */
    snprintf(error_msg, sizeof(error_msg), "for 0x%x", vlan_params->inner_tpid);
    BCM_IF_ERROR_RETURN_MSG(tpid__tpid_add(unit, vlan_params->inner_tpid), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "double_tagged_port=%d", *vlan_params->double_tagged_port);
    ...
    int nwk_group_pwe_2)
{
    ...
    for (src_idx = 0; src_idx < 4; src_idx++)
    {
        for (dst_idx = 0; dst_idx < 4; dst_idx++)
        {
            ...
            snprintf(error_msg, sizeof(error_msg), "src=%d -> dst=%d", src_idx, dst_idx);
            BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, src_idx, &config), error_msg);
        }
    }
    ...
    snprintf(error_msg, sizeof(error_msg), "src=%d -> dst=%d", nwk_group_ac_1, nwk_group_pwe_1);
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, nwk_group_ac_1, &config), error_msg);

    printf("    source_network_group_id(%d) -> dest_network_group_id(%d) PASS\n", nwk_group_ac_1, nwk_group_ac_2);
    config.config_flags = 0;
    config.dest_network_group_id = nwk_group_ac_2;
    snprintf(error_msg, sizeof(error_msg), "src=%d -> dst=%d", nwk_group_ac_1, nwk_group_ac_2);
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, nwk_group_ac_1, &config), error_msg);

    printf("    source_network_group_id(%d) -> dest_network_group_id(%d) PASS\n", nwk_group_ac_1, nwk_group_ac_1);
    config.config_flags = 0;
    config.dest_network_group_id = nwk_group_ac_1;
    snprintf(error_msg, sizeof(error_msg), "src=%d -> dst=%d", nwk_group_ac_1, nwk_group_ac_1);
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, nwk_group_ac_1, &config), error_msg);

    printf("    source_network_group_id(%d) -> dest_network_group_id(%d) PASS\n", nwk_group_ac_2, nwk_group_ac_2);
    config.config_flags = 0;
    config.dest_network_group_id = nwk_group_ac_2;
    snprintf(error_msg, sizeof(error_msg), "src=%d -> dst=%d", nwk_group_ac_2, nwk_group_ac_2);
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, nwk_group_ac_2, &config), error_msg);

    printf("    source_network_group_id(%d) -> dest_network_group_id(%d) PASS\n", nwk_group_ac_2, nwk_group_pwe_2);
    config.config_flags = 0;
    config.dest_network_group_id = nwk_group_pwe_2;
    snprintf(error_msg, sizeof(error_msg), "src=%d -> dst=%d", nwk_group_ac_2, nwk_group_pwe_2);
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, nwk_group_ac_2, &config), error_msg);

    printf("    source_network_group_id(%d) -> dest_network_group_id(%d) PASS\n", nwk_group_pwe_1, nwk_group_pwe_1);
    config.config_flags = 0;
    config.dest_network_group_id = nwk_group_pwe_1;
    snprintf(error_msg, sizeof(error_msg), "src=%d -> dst=%d", nwk_group_pwe_1, nwk_group_pwe_1);
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, nwk_group_pwe_1, &config), error_msg);

    printf("    source_network_group_id(%d) -> dest_network_group_id(%d) PASS\n", nwk_group_pwe_1, nwk_group_ac_1);
    config.config_flags = 0;
    config.dest_network_group_id = nwk_group_ac_1;
    snprintf(error_msg, sizeof(error_msg), "src=%d -> dst=%d", nwk_group_pwe_1, nwk_group_ac_1);
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, nwk_group_pwe_1, &config), error_msg);

    printf("    source_network_group_id(%d) -> dest_network_group_id(%d) PASS\n", nwk_group_pwe_2, nwk_group_pwe_2);
    config.config_flags = 0;
    config.dest_network_group_id = nwk_group_pwe_2;
    snprintf(error_msg, sizeof(error_msg), "src=%d -> dst=%d", nwk_group_pwe_2, nwk_group_pwe_2);
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, nwk_group_pwe_2, &config), error_msg);

    printf("    source_network_group_id(%d) -> dest_network_group_id(%d) PASS\n", nwk_group_pwe_2, nwk_group_ac_2);
    config.config_flags = 0;
    config.dest_network_group_id = nwk_group_ac_2;
    snprintf(error_msg, sizeof(error_msg), "src=%d -> dst=%d", nwk_group_pwe_2, nwk_group_ac_2);
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, nwk_group_pwe_2, &config), error_msg);
    ...
    pwe_tagged_mode_split_horizon_native_ac_s* native_ac)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "port=0x%x", *native_ac->ingress_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, *native_ac->ingress_port, bcmPortClassIngress, vlan_params->class_id), error_msg);

    /* Set tag formats for un tagged and double tagged packets */
    printf("Configure tag formats\n");
    BCM_IF_ERROR_RETURN_MSG(pwe_tm_sh_tag_formats_set(unit, vlan_params), "");

    /* Add AC for inner ETH EVE (adding service tags) - for Egress flow */
    BCM_IF_ERROR_RETURN_MSG(pwe_tm_sh_add_core_native_ac(unit, vsi, native_ac), "");

    if(native_ac->is_indexed)
    {
        native_vlan_gport = BCM_GPORT_SUB_TYPE_LIF_VAL_GET(native_ac->egress_vlan_port_id);
        BCM_GPORT_SUB_TYPE_LIF_SET(native_ac->ingress_vlan_port_id, BCM_GPORT_SUB_TYPE_LIF_EXC_INGRESS_ONLY, native_vlan_gport);
        BCM_GPORT_VLAN_PORT_ID_SET(native_ac->ingress_vlan_port_id, native_ac->ingress_vlan_port_id);
    }
    /* Add AC for inner ETH IVE (removing/replacing service tags) - for Ingress flow */
    printf("Inner AC configuration\n");
    BCM_IF_ERROR_RETURN_MSG(pwe_tm_sh_add_network_native_ac(unit, vsi, native_ac), "");

    /* Configure IVE for core to access traffic: delete two service tags */
    snprintf(error_msg, sizeof(error_msg), "port=0x%08x", native_ac->ingress_vlan_port_id);
    BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set(unit, native_ac->ingress_vlan_port_id,
    ...
    snprintf(error_msg, sizeof(error_msg), "port=0x%08x", native_ac->egress_vlan_port_id);
    BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set(unit, native_ac->egress_vlan_port_id,
    ...
    bcm_if_t ac_encap_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, mc_id, flags, 1, &rep), "");
    ...
    pwe_tagged_mode_split_horizon_add_mc_s add_mc_dev)
{
    ...
    /* Init AC port */
    BCM_IF_ERROR_RETURN_MSG(vswitch_vpls_ac_create(unit, ac_port, vsi), "");

    /* create PWE */
    BCM_IF_ERROR_RETURN_MSG(vswitch_vpls_pwe_create(unit, pwe_param), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(pwe_tm_sh_configure_eve_ive(unit, vsi, &vlan_params, &native_ac), "");

    /* Connect GPORT (VLAN-Port) to its QOS-profile */
    snprintf(error_msg, sizeof(error_msg), "vlan_port_ing = 0x%x, vlan_port_eg = 0x%x",
        native_ac.ingress_vlan_port_id, native_ac.egress_vlan_port_id);
    BCM_IF_ERROR_RETURN_MSG(pwe_tm_sh_qos_map_gport(unit, pwe_tm_sh_eth_ingress_qos_id, native_ac.ingress_vlan_port_id, pwe_tm_sh_eth_egress_qos_id, native_ac.egress_vlan_port_id), error_msg);

    snprintf(error_msg, sizeof(error_msg), "mpls_port_id_ing = 0x%x, mpls_port_id_eg = 0x%x",
        pwe_param->pwe_term.mpls_port_id, pwe_param->pwe_encap.mpls_port_id);
    BCM_IF_ERROR_RETURN_MSG(pwe_tm_sh_qos_map_gport(unit, pwe_tm_sh_eth_ingress_qos_id,/*pwe_param->pwe_term.mpls_port_id*/ac_port->vlan_port_id, pwe_tm_sh_eth_egress_qos_id, pwe_param->pwe_encap.mpls_port_id), error_msg);

    printf("Connect AC and PWE to multicast\n\n");
    /* Add AC to multicast VPN */
    if(add_mc_dev.add_ac)
    {
        BCM_IF_ERROR_RETURN_MSG(vswitch_vpls_vpn_vlan_port_add(unit, vsi, ac_port), "");
    }
    /* Add pwe to multicast VPN */
    if(add_mc_dev.add_pwe)
    {
        BCM_IF_ERROR_RETURN_MSG(pwe_tm_sh_vpn_pwe_add(unit, vsi, pwe_param, native_ac.is_indexed, native_ac.ac_encap_id), "");
    }
    ...
    bcm_gport_t mpls_port_id)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "setting up ingress gport(0x%08x) to map", vlan_port_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_port_map_set(unit, vlan_port_id, ingress_qos_map_id, -1), error_msg);

    /** set remark profile to pwe*/
    snprintf(error_msg, sizeof(error_msg), "setting up egress gport(0x%08x) to map", mpls_port_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_port_map_set(unit, mpls_port_id, -1, egress_qos_map_id), error_msg);
    ...
    int pwe_port_2)
{
    ...
    /* Eth QOS */
    BCM_IF_ERROR_RETURN_MSG(qos_map_l2_example(unit, pwe_tm_sh_eth_ingress_qos_id, pwe_tm_sh_eth_egress_qos_id, pwe_tm_sh_ing_opcode_id, pwe_tm_sh_egr_opcode_id), "");

    /* MPLS QOS */
    BCM_IF_ERROR_RETURN_MSG(dnx_qos_map_mpls(unit,1,1), "");

    /* create vlan based on the vsi (vpn) */
    snprintf(error_msg, sizeof(error_msg), "Failed to create VLAN %d", vswitch_vpls_params.vpn);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, vswitch_vpls_params.vpn), error_msg);
    ...
    if(vswitch_vpls_params.test_split_horizon)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(pwe_tm_sh_configure_nwk_groups(unit, vswitch_vpls_params.ac_ports[0].ac_nwk_group_id, vswitch_vpls_params.ac_ports[1].ac_nwk_group_id, 
    ...
    {
        snprintf(error_msg, sizeof(error_msg), "failed for idx = %d", idx);
        BCM_IF_ERROR_RETURN_MSG(pwe_tm_sh_create_vpls(unit, vswitch_vpls_params.vpn, &(vswitch_vpls_params.ac_ports[idx]), &(vswitch_vpls_params.pwe_params[idx]), 
                                    vlan_params[idx], native_ac[idx], add_mc_dev[idx]), error_msg);
    }
