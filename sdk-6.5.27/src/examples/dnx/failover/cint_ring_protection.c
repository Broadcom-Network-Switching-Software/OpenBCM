 ...
 * Test Scenario - start
  ...
  cint ../../../../src/examples/sand/cint_sand_l2_learning.c;
  cint ../../../../src/examples/sand/cint_sand_multi_device_utils.c
  cint ../../../../src/examples/sand/cint_advanced_vlan_translation_mode.c
  cint ../../../../src/examples/sand/cint_vswitch_metro_mp.c;
  cint ../../../../src/examples/dnx/failover/cint_ring_protection.c;
  cint;
  int rv;
 ...
 */
  ...
  bcm_failover_t rx_failovers[NOF_ERP_GROUPS] = {0, 0};
...
int ring_protection_vlan_port_create(int *units_ids, int nof_units, int out_sysport, int in_sysport)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(vswitch_metro_mp_multi_device_fixed_ports(units_ids, nof_units, 0xce, 3), "");
    ...
    for (gport_num = 0; gport_num < NOF_RING_VP_GPORTS; gport_num++) {
        ...
        snprintf(error_msg, sizeof(error_msg), "for #%ld", gport_num);
        BCM_IF_ERROR_RETURN_MSG(vswitch_metro_find_port(units_ids[0], gport_num, &outlifs[gport_num]), error_msg);
    ...
    vswitch_metro_mp_info.sysports[3] = in_sysport;
    vswitch_metro_mp_info.p_vlans[12] = 100;
    vswitch_metro_mp_info.p_vlans[13] = 150;
    vswitch_metro_mp_info.p_vlans[14] = 200;
    vswitch_metro_mp_info.p_vlans[15] = 250;
    vswitch_metro_mp_info.flush_group_id[3] = 0;
    vswitch_metro_mp_info.ingress_failover_id[3] = 0;
    vswitch_metro_mp_info.egress_failover_id[3] = 0;

    /* Init the new vlans */
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        for (j = 12 ; j < 16 ; j++) {
            BCM_IF_ERROR_RETURN_MSG(vlan__init_vlan(unit,vswitch_metro_mp_info.p_vlans[j]), "");
        }
    }
    ...
    flags = 0;
    outlifs[3] = 0;
    units_array_make_local_first(units_ids, nof_units, vswitch_metro_mp_info.sysports[3]);
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        int sysport_is_local;
        BCM_IF_ERROR_RETURN_MSG(is_system_port_is_local(unit, vswitch_metro_mp_info.sysports[3], &sysport_is_local), "");
        if (sysport_is_local) {
            BCM_IF_ERROR_RETURN_MSG(vswitch_metro_add_port(unit, 3, &outlifs[3], flags), "");
        }
        if(verbose){
    ...
    for (i = 0 ; i < nof_units ; i++){
        unit = units_ids[i];
        BCM_IF_ERROR_RETURN_MSG(vswitch_add_port(unit, vswitch_metro_mp_info.vsi, in_sysport, outlifs[3]), "");
    }
...
int ring_protection_fast_flush(int unit, int flush_group_id)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "with flush group (%ld)", l2_replace_addr.group);
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_replace(unit, l2_replace_flags, &l2_replace_addr, 0, 0, 0), error_msg);
...
int ring_protection_erp_failover_create(int unit, uint32 flags, int erp_idx)
{
    ...
    if (rx_failovers[erp_idx])
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, (flags | BCM_FAILOVER_INGRESS | BCM_FAILOVER_WITH_ID), &rx_failovers[erp_idx]), "");
    }
    else
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, (flags | BCM_FAILOVER_INGRESS), &rx_failovers[erp_idx]), "");
    }
    printf("rx ERP group created with failover_id 0x%x\n", rx_failovers[erp_idx]);

    /* Set the default state */
    snprintf(error_msg, sizeof(error_msg), "with erp_failover_id - 0x%x to init state - 0", rx_failovers[erp_idx]);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_set(unit, rx_failovers[erp_idx], 0), error_msg);
    ...
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, flags | BCM_FAILOVER_ENCAP, &tx_failovers[erp_idx]), "");
        printf("tx ERP group created with failover_id 0x%x\n", tx_failovers[erp_idx]);

        /* Set the default state */
        snprintf(error_msg, sizeof(error_msg), "with erp_failover_id - 0x%x to init state - 0", tx_failovers[erp_idx]);
        BCM_IF_ERROR_RETURN_MSG(bcm_failover_set(unit, tx_failovers[erp_idx], 0), error_msg);
...
int ring_protection_erp_failover_destroy(int unit, int erp_idx)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "with gport - 0x%x", rx_failovers[erp_idx]);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_destroy(unit, rx_failovers[erp_idx]), error_msg);

    if (tx_failovers[erp_idx])
    {
        snprintf(error_msg, sizeof(error_msg), "with gport - 0x%x", tx_failovers[erp_idx]);
        BCM_IF_ERROR_RETURN_MSG(bcm_failover_destroy(unit, tx_failovers[erp_idx]), error_msg);
    }
              ...
              int block_state)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "with failover_id - 0x%x to state - %ld", rx_failovers[failover_id_idx], block_state);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_set(unit, rx_failovers[failover_id_idx], block_state), error_msg);
    printf("ERP Ingress block state for failover_id 0x%x set to %ld\n", rx_failovers[failover_id_idx], block_state);

    /* Block/Unblock the Egress */
    if (tx_failovers[failover_id_idx])
    {
        snprintf(error_msg, sizeof(error_msg), "with erp_failover_id - 0x%x to state - %ld", tx_failovers[failover_id_idx], block_state);
        BCM_IF_ERROR_RETURN_MSG(bcm_failover_set(unit, tx_failovers[failover_id_idx], block_state), error_msg);
                                            ...
                                            int enable)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "with gport - 0x%x", gport);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_find(unit, &vp), error_msg);
    ...
    vswitch_metro_mp_ring_protection_info_init(outlif_idx, vp.group, vp.ingress_failover_id, vp.egress_failover_id);
...
int ring_protection_destroy(int *units_ids, int nof_units)
{
    ...
    for (erp_idx = 0; erp_idx < NOF_ERP_GROUPS; erp_idx++) {
        for (i = 0 ; i < nof_units ; i++) {
            ...
            snprintf(error_msg, sizeof(error_msg), "for erp_idx - %ld", erp_idx);
    ...
    for (gport_num = 0; gport_num < NOF_RING_VP_GPORTS; gport_num++) {
        for (i = 0 ; i < nof_units ; i++) {
            unit = units_ids[i];
            int sysport_is_local;
            BCM_IF_ERROR_RETURN_MSG(is_system_port_is_local(unit, vswitch_metro_mp_info.sysports[gport_num], &sysport_is_local), "");
            if (sysport_is_local) {
                snprintf(error_msg, sizeof(error_msg), "with gport (0x%x)", outlifs[gport_num]);
                BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, outlifs[gport_num]), error_msg);
            }
        }
    }
...
int ring_protection_create(int *units_ids, int nof_units, int outSysport, int inSysport)
{
    ...
    for (erp_idx = 0; erp_idx < NOF_ERP_GROUPS; erp_idx++)
    {
        ...
        for (i = 0 ; i < nof_units ; i++) {
            ...
            BCM_IF_ERROR_RETURN_MSG(ring_protection_erp_failover_create(unit, failover_flags, erp_idx), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(ring_protection_vlan_port_create(units_ids, nof_units, outSysport, inSysport), "");
...
int ring_protection_fast_flush_all(int out_unit, int in_unit, int erp_idx)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "for flush_group_id - %ld", erp_idx);
    BCM_IF_ERROR_RETURN_MSG(ring_protection_fast_flush(in_unit, flush_group_id), error_msg);

    BCM_IF_ERROR_RETURN_MSG(ring_protection_fast_flush(out_unit, flush_group_id), error_msg);
...
int ring_protect_fast_flush_by_dest_only(int unit, bcm_gport_t gport)
{
    ...
    /* Perform the entry deletion */
    snprintf(error_msg, sizeof(error_msg), "with gport(0x%x)", l2_replace_addr.port);
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_replace_match(unit, l2_replace_flags, &l2_replace_addr, &match_mask_addr, NULL, NULL), error_msg);
...
/*
 * Add a static l2 address with the ring attributes
 */
int ring_protect_add_l2_addr(int *units_ids, int nof_units, int gport_idx, bcm_mac_t mac_lsbs)
{
    int unit_idx;

    for (unit_idx = 0 ; unit_idx < nof_units ; unit_idx++) {
        int unit = units_ids[unit_idx];
        int sysport_is_local;
        BCM_IF_ERROR_RETURN_MSG(is_system_port_is_local(unit, vswitch_metro_mp_info.sysports[gport_idx], &sysport_is_local), "");
        if (sysport_is_local) {
            BCM_IF_ERROR_RETURN_MSG(vswitch_add_l2_addr_w_group_id(unit, outlifs[gport_idx], mac_lsbs, vswitch_metro_mp_info.flush_group_id[gport_idx], vswitch_metro_mp_info.vsi), "");
        }
    }
...
int ring_protect_with_jr1_mode_create(int unit, int inP, int outP)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_create(unit, &vsi_id), "");
    g_fast_flush_vsi = vsi_id;

    bcm_l3_egress_t_init(&l3_egr);
    l3_egr.flags = 0;
    l3_egr.port = inP;
    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_create(unit, BCM_L3_INGRESS_ONLY, &l3_egr, &fec_id), "1st FEC");
    ...
    snprintf(error_msg, sizeof(error_msg), "vid:%d", match_vid[vid_idx]);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, vsi_id, vlan_port.vlan_port_id), "vlan_port_1");
    ...
    snprintf(error_msg, sizeof(error_msg), "vid:%d", match_vid[vid_idx]);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, vlan_port_2), error_msg);
    g_fast_flush_vlan_port_id[vid_idx] = vlan_port_2.vlan_port_id;

    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, vsi_id, vlan_port_2.vlan_port_id), "vlan_port_2");
    ...
    l3_egr.flags = 0;
    l3_egr.port = inP;
    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_create(unit, BCM_L3_INGRESS_ONLY, &l3_egr, &fec_id_2), "2nd FEC");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, vlan_port_3), "vlan_port_3");
    g_fast_flush_vlan_port_id[vid_idx] = vlan_port_3.vlan_port_id;

    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, vsi_id, vlan_port_3.vlan_port_id),"vlan_port_3");
...
int ring_protect_with_jr1_mode_destroy(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_destroy(unit, g_fast_flush_fec_gport[0]), "1st FEC");
    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_destroy(unit, g_fast_flush_fec_gport[1]), "2nd FEC");

    for (idx = 0; idx < 3; idx++)
    {
        snprintf(error_msg, sizeof(error_msg), "vlan_port:%d", idx);
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_delete(unit, vsi_id, g_fast_flush_vlan_port_id[idx]), error_msg);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, g_fast_flush_vlan_port_id[idx]), error_msg);
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_destroy(unit, vsi_id), "");
