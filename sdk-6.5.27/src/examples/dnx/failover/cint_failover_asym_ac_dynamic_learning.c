...
int dnx_asym_ac_failover_create(int unit) {
    ...
    snprintf(error_msg, sizeof(error_msg), "for fec failover_id %d", g_dnx_asym_ac_failover.fec_failover_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_FEC | BCM_FAILOVER_WITH_ID, g_dnx_asym_ac_failover.fec_failover_id), error_msg);

    /* create the 1st egress failover id */
    snprintf(error_msg, sizeof(error_msg), "for fec failover_id %d", g_dnx_asym_ac_failover.eg_failover_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_ENCAP | BCM_FAILOVER_WITH_ID, g_dnx_asym_ac_failover.eg_failover_id), error_msg);

    /* create the 2nd egress failover id */
    snprintf(error_msg, sizeof(error_msg), "for fec failover_id %d", g_dnx_asym_ac_failover.eg_failover_id2);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_ENCAP | BCM_FAILOVER_WITH_ID, g_dnx_asym_ac_failover.eg_failover_id2), error_msg);
...
int dnx_asym_ac_failover_mc_service_setup(int unit) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        snprintf(error_msg, sizeof(error_msg), "for vlan %d", vlan);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, vlan), error_msg);

        for (j = 0; j < 3; j++) {
            snprintf(error_msg, sizeof(error_msg), "for vlan %d port %d", vlan, port_list[j]);
            BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, vlan, port_list[j], 0), error_msg);
        }
    }

    snprintf(error_msg, sizeof(error_msg), "for vsi %d", g_dnx_asym_ac_failover.vsi);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, g_dnx_asym_ac_failover.vsi), error_msg);

    for (j = 0; j < 3; j++) {
        snprintf(error_msg, sizeof(error_msg), "for vsi %d port %d", g_dnx_asym_ac_failover.vsi, port_list[j]);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, g_dnx_asym_ac_failover.vsi, port_list[j], 0), error_msg);
    }
    ...
    snprintf(error_msg, sizeof(error_msg), "for secondary ingress ac match_vlan %d port %d",
        g_dnx_asym_ac_failover.in_ac_secondary.vlan, g_dnx_asym_ac_failover.in_ac_secondary.port);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &port_vlan), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "for primary ingress ac match_vlan %d port %d",
        g_dnx_asym_ac_failover.in_ac_primary.vlan, g_dnx_asym_ac_failover.in_ac_primary.port);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &port_vlan), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "for secondary egress ac egress_failover_id %d",
        g_dnx_asym_ac_failover.eg_failover_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &port_vlan), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "for primary egress ac egress_failover_id %d egress_failover_port_id %d",
        g_dnx_asym_ac_failover.eg_failover_id, g_dnx_asym_ac_failover.vlan_port_egress_secondary_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &port_vlan), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "for access ac match_vlan %d port %d",
        g_dnx_asym_ac_failover.in_ac_access.vlan, g_dnx_asym_ac_failover.in_ac_access.port);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &port_vlan), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "for vlan gport %d added to vsi %d",
        g_dnx_asym_ac_failover.in_ac_access.gport, g_dnx_asym_ac_failover.vsi);
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, g_dnx_asym_ac_failover.vsi, g_dnx_asym_ac_failover.in_ac_access.gport), error_msg);

    /* Create multicast group and add ports to MC group */
    BCM_IF_ERROR_RETURN_MSG(multicast__open_egress_mc_group_with_gports(unit, g_dnx_asym_ac_failover.vsi, port_list, encap_id_list, 3, 0), "");

    BCM_IF_ERROR_RETURN_MSG(multicast__open_fabric_mc_or_ingress_mc_for_egress_mc(unit, g_dnx_asym_ac_failover.vsi, g_dnx_asym_ac_failover.vsi), "");
...
int dnx_asym_ac_fec_encap_failover_update(int unit) {
    ...
    snprintf(error_msg, sizeof(error_msg), "secondary fec creation for port %d encap_id %d failover_id %d",
        g_dnx_asym_ac_failover.in_ac_secondary.port, g_dnx_asym_ac_failover.vlan_port_egress_secondary_id, fec_failover_id);
    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_fec_create(unit, &fec_structure), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "during fec creation for port %d encap_id %d failover_id %d",
        g_dnx_asym_ac_failover.in_ac_primary.port, g_dnx_asym_ac_failover.vlan_port_egress_primary_id, fec_failover_id);
    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_fec_create(unit, &fec_structure), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "during find secondary ingress ac for vlan gport %d",
        g_dnx_asym_ac_failover.in_ac_secondary.gport);
    ...
    snprintf(error_msg, sizeof(error_msg), "during updating secondary ingress AC with FEC protection for vlan gport %d using failover_port_id %d",
        g_dnx_asym_ac_failover.in_ac_secondary.gport, fec_if_secondary);
    ...
    snprintf(error_msg, sizeof(error_msg), "during find primary ingress ac for vlan gport %d",
        g_dnx_asym_ac_failover.in_ac_primary.gport);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_find(unit, &vlan_port), error_msg);

    BCM_L3_ITF_FEC_TO_GPORT_FORWARD_GROUP(fec_gport, fec_if_primary);
    vlan_port.flags |= BCM_VLAN_PORT_WITH_ID | BCM_VLAN_PORT_REPLACE;
    vlan_port.failover_port_id = fec_gport;
    snprintf(error_msg, sizeof(error_msg), "during updating primary ingress AC with FEC protection for vlan gport %d using failover_port_id %d",
        g_dnx_asym_ac_failover.in_ac_primary.gport, fec_if_primary);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &vlan_port), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "during find secondary egress ac for vlan gport %d",
        g_dnx_asym_ac_failover.vlan_port_egress_secondary_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_find(unit, &vlan_port), error_msg);

    vlan_port.flags |= BCM_VLAN_PORT_WITH_ID | BCM_VLAN_PORT_REPLACE;
    vlan_port.egress_failover_id = g_dnx_asym_ac_failover.eg_failover_id2;
    snprintf(error_msg, sizeof(error_msg), "during updating secondary egress AC with egress encap protection for vlan gport %d using egress_failover_id %d",
        g_dnx_asym_ac_failover.vlan_port_egress_secondary_id, g_dnx_asym_ac_failover.eg_failover_id2);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &vlan_port), error_msg);

    /* Update primary egress AC with new failover id */
    bcm_vlan_port_t_init(&vlan_port);
    vlan_port.vlan_port_id = g_dnx_asym_ac_failover.vlan_port_egress_primary_id;
    snprintf(error_msg, sizeof(error_msg), "during find primary egress ac for vlan gport %d",
        g_dnx_asym_ac_failover.vlan_port_egress_primary_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_find(unit, &vlan_port), error_msg);

    vlan_port.flags |= BCM_VLAN_PORT_WITH_ID | BCM_VLAN_PORT_REPLACE;
    vlan_port.egress_failover_id = g_dnx_asym_ac_failover.eg_failover_id2;
    vlan_port.egress_failover_port_id = g_dnx_asym_ac_failover.vlan_port_egress_secondary_id;
    snprintf(error_msg, sizeof(error_msg), "during updating primary egress AC with egress encap protection for vlan gport %d using egress_failover_id %d egress_failover_port_id %d",
        g_dnx_asym_ac_failover.vlan_port_egress_primary_id, g_dnx_asym_ac_failover.eg_failover_id2, g_dnx_asym_ac_failover.vlan_port_egress_secondary_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &vlan_port), error_msg);
...
int dnx_asym_ac_fec_failover_cleanup(int unit) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &vlan_port), "during updating secondary ingress AC with none protection");
    ...
    vlan_port.flags |= BCM_VLAN_PORT_WITH_ID | BCM_VLAN_PORT_REPLACE;
    vlan_port.failover_port_id = 0;
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &vlan_port), "during updating primary ingress AC with none protection");
...
int dnx_asym_ac_failover_switch(int unit, int is_primary) {
    ...
    snprintf(error_msg, sizeof(error_msg), "for fec failover_id %d", g_dnx_asym_ac_failover.fec_failover_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_set(unit, g_dnx_asym_ac_failover.fec_failover_id, is_primary), error_msg);

    snprintf(error_msg, sizeof(error_msg), "for egress failover_id %d", g_dnx_asym_ac_failover.eg_failover_id2);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_set(unit, g_dnx_asym_ac_failover.eg_failover_id2, is_primary), error_msg);
...
int dnx_asym_ac_failover_destroy(int unit) {
    ...
    snprintf(error_msg, sizeof(error_msg), "for fec failover_id %d", g_dnx_asym_ac_failover.fec_failover_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_destroy(unit, g_dnx_asym_ac_failover.fec_failover_id), error_msg);

    snprintf(error_msg, sizeof(error_msg), "for egress failover_id %d", g_dnx_asym_ac_failover.eg_failover_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_destroy(unit, g_dnx_asym_ac_failover.eg_failover_id), error_msg);

    snprintf(error_msg, sizeof(error_msg), "for egress failover_id %d", g_dnx_asym_ac_failover.eg_failover_id2);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_destroy(unit, g_dnx_asym_ac_failover.eg_failover_id2), error_msg);
