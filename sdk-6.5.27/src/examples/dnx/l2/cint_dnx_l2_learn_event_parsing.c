...
/*
 * File: cint_dnx_l2_learn_event_parsing.c
 * Purpose: Verify DSP packets are processed correctly
 *
 * Notes:
 *   cint_dnx_l2_learn_event_parsing.c is built up on the top of cint_dnx_l2_learn_event_parsing.c
 *
 ...
 * Basic example:
 * Test Scenario - start
  cint ../../../../src/examples/dnx/utility/cint_dnxutils_global.c
  cint ../../../../src/examples/dnx/cint_dnx_l2_learn_event_parsing.c
  tx 1 PSRC=200 DATA=0x000000000011ab67ff093e2281000064000000000
  cint
  print init_learn_event_parsing(0);
 * Test Scenario - end
 *
...
int construct_buffer(int unit, int payload_type, int valid_outlif, int outlif, int valid_sid, int sid, int eei_cmd, int eei_data, int entry) {
    ...
    dnx_dbal_fields_enum_value_get(unit, "IRPP_FWD_STAGE_FORMATS", "FWD_MACT_RESULT_DEST__LIF__XL_FGID", &lpkgv_with_outlif);
    dnx_dbal_fields_enum_value_get(unit, "IRPP_FWD_STAGE_FORMATS", "FWD_MACT_RESULT_DEST__XL_FGID", &lpkgv_wo_outlif);
...
int get_and_add_address_manually(int unit){
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_get(unit, mac, fid, &addr), "");

    /* Clear LEM Table */
    BCM_IF_ERROR_RETURN_MSG(bshell(unit, "l2 clear all"), "");

    /* Add the entry that was read earlier */
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(unit, &addr), "");
...
int tx_packet(int unit, char *data) {
    ...
    BCM_IF_ERROR_RETURN_MSG(tx_packet_via_bcm(unit, data, &ptch, 2), "");
