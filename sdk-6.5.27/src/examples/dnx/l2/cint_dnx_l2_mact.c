...
int l2_mact_run(int unit) {
    ...
    /* set aging */
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_age_timer_set(unit,l2_mact_table_info1.aging_time), "");

    /* insert some entries */
    BCM_IF_ERROR_RETURN_MSG(l2_mac_table_init(unit), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(multicast__open_ingress_mc_group_with_local_ports(unit, l2_mact_table_info1.mc_id, gports, cuds, 2, 0), "");
...
int validate_mac_fid_limit_interrupt(int unit,int is_up,int is_sim) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_event_control_get(unit,BCM_SWITCH_EVENT_DEVICE_INTERRUPT,type,&interrupt_val), "");
...
int mac_fid_limit_interrupt_clear(int unit) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_event_control_set(unit,BCM_SWITCH_EVENT_DEVICE_INTERRUPT,type,0x1), "");
