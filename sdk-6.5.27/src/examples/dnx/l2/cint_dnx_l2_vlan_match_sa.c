...
int l2_vlan_match_sa_inlif_del(int unit, int idx)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "(IDX = %d)", idx);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, inlif[idx]), error_msg);
...
int l2_vlan_match_sa_isem_add(int unit, int idx)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "(IDX = %d)", idx);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_match_add(unit, inlif[idx], &match_info), error_msg);
...
int l2_vlan_match_sa_isem_del(int unit, int idx)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "(IDX = %d)", idx);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_match_delete(unit, inlif[idx], &match_info), error_msg);
    ...
    int oPort3)
{
    ...
    /* set vlan domain */
    snprintf(error_msg, sizeof(error_msg), "(port = %d)", l2_vlan_match_sa_info.in_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, l2_vlan_match_sa_info.in_port, bcmPortClassId, l2_vlan_match_sa_info.in_port), error_msg);

    for (i = 0; i < 3; i++) 
    {
        snprintf(error_msg, sizeof(error_msg), "(port = %d)", l2_vlan_match_sa_info.out_port[i]);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, l2_vlan_match_sa_info.out_port[i], bcmPortClassId, l2_vlan_match_sa_info.out_port[i]), error_msg);
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, l2_vlan_match_sa_info.tagged_outer_vid, pbmp, untag_pbmp), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, l2_vlan_match_sa_info.vlan_tagged_outer_vid, pbmp, untag_pbmp), "");
    
    /* Enable the source MAC lookup */
    snprintf(error_msg, sizeof(error_msg), "(port = %d)", l2_vlan_match_sa_info.in_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_control_port_set(unit, l2_vlan_match_sa_info.in_port, bcmVlanLookupMACEnable, 1), error_msg);

    /* Create VSI */
    for (i = 0; i < 4; i++) 
    {
        snprintf(error_msg, sizeof(error_msg), "(vsi=%d)", vsi[i]);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, vsi[i]), error_msg);
    }      
    ...
    { 
        ...
        snprintf(error_msg, sizeof(error_msg), "(VSI = %d)", vsi[1 + i]);
    ...
    switch (step)
    {
        case 2:
        {
            ...
            */
            ...
            {
                snprintf(error_msg, sizeof(error_msg), "(IDX = %d)", idx);
                BCM_IF_ERROR_RETURN_MSG(l2_vlan_match_sa_inlif_add(unit, idx), error_msg);
            } 

            /*
            * Create IVE (replace, add) command for vlan tagged-packet:
            */ 
            BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set(unit, l2_vlan_match_sa_info.vlan_tagged_inlif,           /* lif gport */
       ...
       case 3:
       {
          ...
          */
            ...
            {
                snprintf(error_msg, sizeof(error_msg), "(IDX = %d)", idx);
                BCM_IF_ERROR_RETURN_MSG(l2_vlan_match_sa_isem_add(unit, idx), error_msg);
            }                
            break;
       }
       case 4:
       {
          ...
          * 2. Destroy the In-LIF associated with VID(Outer_VLAN),MAC(SA),PP_Port vlan specific tagged packet  (created in step 2).
          */
          snprintf(error_msg, sizeof(error_msg), "(IDX = %d)", 2);
          BCM_IF_ERROR_RETURN_MSG(l2_vlan_match_sa_isem_del(unit, 2), error_msg);

          BCM_IF_ERROR_RETURN_MSG(l2_vlan_match_sa_inlif_del(unit, 2), error_msg);
       ...
       case 5:
       {
          ...
          * 2. Destroy the In-LIF associated with MAC(SA),PP_Port tagged packet  (created in step 2).
          */
          snprintf(error_msg, sizeof(error_msg), "(IDX = %d)", 1);
          BCM_IF_ERROR_RETURN_MSG(l2_vlan_match_sa_isem_del(unit, 1), error_msg);

          BCM_IF_ERROR_RETURN_MSG(l2_vlan_match_sa_inlif_del(unit, 1), error_msg);
    ...
    int unit)
{
     ...
     * IVE actions:
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_id_destroy_all(unit, BCM_VLAN_ACTION_SET_INGRESS), "(BCM_VLAN_ACTION_SET_INGRESS)");

    /*
     * ISEM lookup
     */
    BCM_IF_ERROR_RETURN_MSG(l2_vlan_match_sa_isem_del(unit, 0), "(IDX = 0)");

    /*
     * LIF
     */
    snprintf(error_msg, sizeof(error_msg), "(gport = 0x%08X)", l2_vlan_match_sa_info.untagged_inlif);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, l2_vlan_match_sa_info.untagged_inlif), error_msg);

    snprintf(error_msg, sizeof(error_msg), "(gport = 0x%08X)", l2_vlan_match_sa_info.default_inlif);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, l2_vlan_match_sa_info.default_inlif), error_msg);
    ...
    {
        snprintf(error_msg, sizeof(error_msg), "(vsi=%d)", vsi[i]);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_destroy(unit, vsi[i]), error_msg);
    }
     ...
     */
    snprintf(error_msg, sizeof(error_msg), "(port = %d)", l2_vlan_match_sa_info.in_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_control_port_set(unit, l2_vlan_match_sa_info.in_port, bcmVlanLookupMACEnable, 0), error_msg);
