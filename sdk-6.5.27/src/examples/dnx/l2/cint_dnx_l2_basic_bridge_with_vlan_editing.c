...
int l2_basic_bridge_run(int unit,  l2_basic_bridge_s *param,int step){
    ...
    BCM_IF_ERROR_RETURN_MSG(l2_basic_bridge_init(unit, param), "");
    ...
    /* Set vlan domain:*/
    BCM_IF_ERROR_RETURN_MSG(vlan_domain_create(unit), "");

    /* Set TPIDs*/
    BCM_IF_ERROR_RETURN_MSG(tpids_assign(unit), "");
    ...
    /* Create VSI:*/
    snprintf(error_msg, sizeof(error_msg), "(vsi=%d)", g_l2_basic_bridge.vsi);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, g_l2_basic_bridge.outer_vid, g_l2_basic_bridge.in_port, 0), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, g_l2_basic_bridge.outer_vid, g_l2_basic_bridge.out_port, 0), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_port_untagged_vlan_set(unit, g_l2_basic_bridge.in_port, g_l2_basic_bridge.outer_vid), "");

    
    /* Create Lif and IVE based on the incoming tag_type */
    BCM_IF_ERROR_RETURN_MSG(create_lif_and_ive(unit,step), "");
    ...
    if (step != 3) {
        snprintf(error_msg, sizeof(error_msg), "(port = %d)", g_l2_basic_bridge.outlif);
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, g_l2_basic_bridge.vsi,g_l2_basic_bridge.outlif), error_msg);
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_tpid_class_set(unit, &port_tpid_class), "( oPort )");

    /* Programming EVE based on the outlif*/
    BCM_IF_ERROR_RETURN_MSG(set_eve_based_on_incoming_tag(unit,step), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_control_vlan_set (unit, g_l2_basic_bridge.vsi, control), "");
    /*
     * Add l2 address - 
    ...
    if (step == 5) {
        l2_addr.modid = ivl_feature_support ? new_outer_vid : 0;;
    }
    ...
    if ((step == 12) || (step == 13)) {
        BCM_IF_ERROR_RETURN_MSG(l2_basic_bridge_learning(unit), "");
    }
    ...
    if (step == 8) {
        ...
        int opcode_egress = -1;
        BCM_IF_ERROR_RETURN_MSG(qos_map_l2(unit,&ingress_qos_id,&egress_qos_id,&opcode_ingress,&opcode_egress), "");
        egr_qos_map_id = egress_qos_id;

        BCM_IF_ERROR_RETURN_MSG(qos_map_gport(unit, g_l2_basic_bridge.inlif,ingress_qos_id,-1), "");

        BCM_IF_ERROR_RETURN_MSG(qos_map_gport(unit, g_l2_basic_bridge.outlif,-1,egress_qos_id), "");
    }
...
int vlan_domain_create(int unit) {
    ...
    /*Input Port */
    snprintf(error_msg, sizeof(error_msg), "(port = %d)", g_l2_basic_bridge.in_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, g_l2_basic_bridge.in_port, bcmPortClassId, g_l2_basic_bridge.in_port), error_msg);
    
    /*Output Port*/
    snprintf(error_msg, sizeof(error_msg), "(port = %d)", g_l2_basic_bridge.out_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, g_l2_basic_bridge.out_port, bcmPortClassId, g_l2_basic_bridge.out_port), error_msg);
...
int create_lif_and_ive(int unit,int step) {
    ...
    if (vlan_port.flags & BCM_VLAN_PORT_CREATE_INGRESS_ONLY) {
       ...
       {
           BCM_GPORT_SUB_TYPE_VIRTUAL_INGRESS_NON_NATIVE_SET(inlif0,global_lif_id);
       } else {
    ...
    if (step != 1 && step != 4 && step != 7 &&
        ...
        snprintf(error_msg, sizeof(error_msg), "(port = %d)", g_l2_basic_bridge.inlif);
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, g_l2_basic_bridge.vsi, g_l2_basic_bridge.inlif), error_msg);
    }
    ...
    snprintf(error_msg, sizeof(error_msg), "(step = %d, tag_format = %d, port = %d)", step, port_tpid_class.tag_format_class_id, port_tpid_class.port);
...
int set_eve_based_on_incoming_tag(int unit,int step){
    ...
    snprintf(error_msg, sizeof(error_msg), "(setting 0Port%d EVE action)", g_l2_basic_bridge.out_port);
    if (step != 8 ) {
        /* For DTAG pkt tag actions is replace along with tpid */
        BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set_with_tpid_action(
                                                        unit,
...
qos_map_l2(int unit, int* ingress_qos_id, int* egress_qos_id, int* ingress_opcode_id, int* egress_opcode_id)
{
    ...
    if (ingress_qos_id != illegal_qos_id)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_create(unit, flags, ingress_qos_id), "");
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_create(unit, flags, &opcode), "");

        BCM_IF_ERROR_RETURN_MSG(qos_map_ingress_profile_map(unit, *ingress_qos_id, opcode, BCM_QOS_MAP_PHB | BCM_QOS_MAP_L2_OUTER_TAG), "");
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_create(unit, flags, &opcode), "");
        BCM_IF_ERROR_RETURN_MSG(qos_map_ingress_profile_map(unit, *ingress_qos_id, opcode, BCM_QOS_MAP_REMARK | BCM_QOS_MAP_L2_OUTER_TAG), "");
    }
    ...
    if (*egress_qos_id != illegal_qos_id)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_create(unit, flags, egress_qos_id), "");
        ...
        BCM_IF_ERROR_RETURN_MSG(qos_map_egress_profile_map(unit, *egress_qos_id, opcode, BCM_QOS_MAP_REMARK | BCM_QOS_MAP_L2_OUTER_TAG), "");
    }
    return BCM_E_NONE;
}
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_learn_set(unit,g_l2_basic_bridge.in_port,BCM_PORT_LEARN_ARL), "");
