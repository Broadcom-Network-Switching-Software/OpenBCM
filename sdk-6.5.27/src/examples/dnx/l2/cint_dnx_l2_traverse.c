 ...
 * Run Script:
 * Test Scenario - start
       cint examples/dnx/l3/cint_dnx_ip_route_basic.c
       cint examples/dnx/utility/cint_dnx_utils_multicast.c
       cint examples/dnx/cint_dnx_l2_traverse.c
       cint
       print l2_mact_run(unit, port1, port2);
       print l2_traverse(unit, api_idx, flags, l2_addr, l2_addr_mask);
 * Test Scenario - end
 */
...
int l2_entry_add_internal(int unit, bcm_mac_t mac, bcm_vlan_t vlan, int dest_type, int dest_gport, int dest_mc_id, int modid, int group, int flags){
    ...
    if (separate_fwd_learn_mact)
    {
        if (l2_addr.flags & BCM_L2_STATIC)
        {
            l2_addr.age_profile = 0;
            if (l2_addr.flags & BCM_L2_MOVE_PORT)
            {
                l2_addr.learn_strength = 0;
                l2_addr.flags &= (~(BCM_L2_STATIC | BCM_L2_MOVE_PORT));
            }
            else
            {
                l2_addr.learn_strength = 1;
                l2_addr.flags &= (~BCM_L2_STATIC);
            }
        }
        else if (l2_addr.flags2 & BCM_L2_FLAGS2_NO_MOVE)
        {
            l2_addr.age_profile = 1;
            l2_addr.learn_strength = 1;
            l2_addr.flags2 &= (~BCM_L2_FLAGS2_NO_MOVE);
        }
        else
        {   /* Dynamic */
            l2_addr.age_profile = 1;
    ...
    snprintf(error_msg, sizeof(error_msg), "dest_type %d dest_id %d", dest_type, dest_gport);
...
l2_mac_table_init2(int unit){
    ...
    int vlan_port_id;
    uint32 separate_fwd_learn_mact = *dnxc_data_get(unit, "l2", "general", "separate_fwd_learn_mact", NULL);
    uint32 dynamic_age_profile = (separate_fwd_learn_mact ? 1 : 0);
    uint32 static_l2_flags = (separate_fwd_learn_mact ? 0 : BCM_L2_STATIC);
    uint32 static_learn_strength = (separate_fwd_learn_mact ? 1 : 0);
    ...
    l2_addr.age_profile = dynamic_age_profile;
    ...
    l2_addr.flags = static_l2_flags;
    l2_addr.learn_strength = static_learn_strength;
    l2_addr.port = l2_mact_table_info2.port1;
    l2_addr.group = l2_mact_table_info2.group1;
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(unit,&l2_addr), "");
    if(verbose){
    ...
    {
        BCM_IF_ERROR_RETURN_MSG(add_route_ipv4(unit, l3_host.l3a_ip_addr, 0,l3_host.l3a_vrf, l3_host.l3a_intf), "");
    }
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_create(unit, (flags | BCM_MULTICAST_WITH_ID), &l2_mact_table_info1.mc_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, l2_mact_table_info1.mc_id, flags, 2, replications), "");
...
int l2_mact_run(int unit, int port1, int port2){
    ...
    /* set aging */
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_age_timer_meta_cycle_set(unit, l2_mact_table_info1.aging_time), "l2_mact_table_info1");
    ...
    /* open mc */
    BCM_IF_ERROR_RETURN_MSG(create_multicast(unit), "");

    /* get next mac-address */
    generat_mac_address(mac);

    /* add MAC points to MC address */
    BCM_IF_ERROR_RETURN_MSG(l2_entry_add(unit, mac, l2_mact_table_info1.vid1, 1, 0, l2_mact_table_info1.mc_id), "");
    generat_mac_address(mac);
    BCM_IF_ERROR_RETURN_MSG(l2_entry_add(unit, mac, l2_mact_table_info1.vid2, 1, 0, l2_mact_table_info1.mc_id), "");
...
int l2_delete_by_port(int unit,int port ){
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_delete_by_port(unit, 0, port,flags), "");
    return BCM_E_NONE;
}
...
int l2_traverse(int unit, int api_idx, int flags, bcm_l2_addr_t *l2_addr, bcm_l2_addr_t *l2_addr_mask)
{
    ...
    switch(api_idx) {
        case l2_traverse:
            BCM_IF_ERROR_RETURN_MSG(bcm_l2_traverse(unit, call_back_get_block, &count), "");
            printf("count:%d\n", count);
            break;
        case l2_match_traverse:
            BCM_IF_ERROR_RETURN_MSG(bcm_l2_matched_traverse(unit, flags, l2_addr, call_back_get_block, &count), "");
            printf("count:%d\n", count);
            break;
        case l2_match_mask_traverse:
            BCM_IF_ERROR_RETURN_MSG(bcm_l2_match_masked_traverse(unit, flags, l2_addr, l2_addr_mask, call_back_get_block, &count), "");
            printf("count:%d\n", count);
            break;
        case l2_traverse_and_abort:
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_l2_traverse(unit, call_back_get_block_and_abort, &count), "");
            break;
        case l2_match_traverse_and_abort:
            BCM_IF_ERROR_RETURN_MSG(bcm_l2_matched_traverse(unit, flags, l2_addr, call_back_traverse_and_return_error, &count), "");
            break;
        default:
               ...
               bcm_l2_addr_t *l2_replace_addr, bcm_l2_addr_t *l2_replace_addr_mask)
{
    ...
    case l2_replace_mask:
            ...
            if (rv != BCM_E_FAIL)
            {
                printf("Error, bcm_l2_replace_match didn\'t fail. Returned %s\n", bcm_errmsg(rv));
                return rv;
            }
            break;
        default:
...
/*
 * traverse all entries, count and print each entry, return entry count
 */
...
/*
 * Get the MACT HW counter from the DBAL table
 */
int l2_entries_get_mact_hw_count(int unit)
{
    uint32 entry_handle_id;
    int inst_id = -1;
    uint32 mact_counter;

    dnx_dbal_entry_handle_take(unit, "MACT_MANAGEMENT_GENERAL_CONFIGURATION", &entry_handle_id);
    dnx_dbal_entry_get(unit, entry_handle_id);
    dnx_dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, "MACT_ENTRY_COUNTER", inst_id, &mact_counter);
