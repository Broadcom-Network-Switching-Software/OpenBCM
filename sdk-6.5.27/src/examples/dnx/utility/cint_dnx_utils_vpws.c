    ...
    int pwe_port)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(out_port_set(unit, cint_pwe_basic_info.pwe_port), "intf_out");

    /*
     * Set In-Port to In ETh-RIF
     */
    BCM_IF_ERROR_RETURN_MSG(in_port_intf_set(unit, cint_pwe_basic_info.pwe_port, cint_pwe_basic_info.pwe_intf), "intf_in");
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(intf_eth_rif_create(unit, cint_pwe_basic_info.pwe_intf, cint_pwe_basic_info.pwe_intf_mac_address), "pwe_intf");
    ...
    int unit)
{
     ...
     * Configure ARP entry
     */
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_encap__create(unit, 0, cint_pwe_basic_info.core_arp_id,
        cint_pwe_basic_info.pwe_arp_next_hop_mac, cint_pwe_basic_info.pwe_intf), "create AC egress object ARP only");
    ...
    int unit)
{
     ...
     * 4) Give the out port.
     */
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create(unit, cint_pwe_basic_info.mpls_encap_fec_id,
        cint_pwe_basic_info.mpls_tunnel_id, 0, cint_pwe_basic_info.pwe_port, flags, flags2), "create egress object FEC only");
    ...
    int pwe_port)
{
     ...
     * Configure PWE port
     */
    BCM_IF_ERROR_RETURN_MSG(pwe_basic_configure_port_properties(unit, pwe_port), "");


    /*
     * Configure L3 interfaces
     */
    BCM_IF_ERROR_RETURN_MSG(pwe_basic_create_l3_interfaces(unit), "");

    /*
     * Configure an ARP entries
     */
    BCM_IF_ERROR_RETURN_MSG(pwe_basic_create_arp_entry(unit), "");

    /*
     * Configure a push entry.
     */
    BCM_IF_ERROR_RETURN_MSG(pwe_basic_create_mpls_tunnel(unit), "");

    /*
     * Configure fec entry
     */
    BCM_IF_ERROR_RETURN_MSG(pwe_basic_create_fec_entry(unit), "");


    /*
     * Configure a termination label for the ingress flow
     */
    BCM_IF_ERROR_RETURN_MSG(pwe_basic_create_termination_stack(unit), "");

    /*
     * configure PWE tunnel - egress flow
     */
    BCM_IF_ERROR_RETURN_MSG(pwe_basic_mpls_port_add_encapsulation(unit), "");


    /*
     * configure PWE tunnel - ingress flow vpn is set to 0 - so P2P is created.
     */
    BCM_IF_ERROR_RETURN_MSG(pwe_basic_mpls_port_add_termination(unit, 0), "");
