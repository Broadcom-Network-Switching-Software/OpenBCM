    ...
    bcm_gport_t* vlan_port_id_2)
{
    ...
    *vlan_port_id_1 = vp1.vlan_port_id;
    ...
    vp2.port = port2;
    vp2.match_vlan = vid2;
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit,&vp2), "");
...
dnx_oam__switch_contol_indexed_port_set(int unit, bcm_port_t port, int port_header_type, int direction){
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_indexed_port_set(unit,port,port_key_info,port_value_info), "");
...
oam__vswitch_metro_add_port(int unit, int port_num, bcm_gport_t *port_id, int flags){
    ...
    *port_id = vp1.vlan_port_id;
    ...
    /* Add VLAN membership */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, vswitch_metro_mp_info.p_vlans[base + 0], vswitch_metro_mp_info.sysports[port_num], 0), "");
...
oam__vswitch_metro_run(int unit, int known_mac_lsb, int known_vlan)
{
    ...
    for (index = 0; index < 12; index++) {
        ...
        snprintf(error_msg, sizeof(error_msg), "vlan=%d", vswitch_metro_mp_info.p_vlans[index]);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, vswitch_metro_mp_info.p_vlans[index]), error_msg);
    }

    /* 2. Create vswitch + create Multicast for flooding
     * If vsi defined create with id*/
    BCM_IF_ERROR_RETURN_MSG(vswitch_create(unit, &vsi, 1), "");
    printf("Created vswitch vsi=0x%x\n", vsi);

    /* 3. create first vlan-port */
    gport1 = 0;
    BCM_IF_ERROR_RETURN_MSG(oam__vswitch_metro_add_port(unit, 0, &gport1, flags), "gport1");
    if(verbose){
        printf("created vlan-port   0x%08x in unit %d\n",gport1, unit);
    }
    dnx_oam_endpoint_info_glb.mc_gport_1 = gport1;

    /* 4. Add vlan-port to the vswitch and multicast */
    BCM_IF_ERROR_RETURN_MSG(vswitch_add_port(unit, vsi, vswitch_metro_mp_info.sysports[0], gport1), "");

    /* 5. Create second vlan-port */
    gport2 = 0;
    BCM_IF_ERROR_RETURN_MSG(oam__vswitch_metro_add_port(unit, 1, &gport2, flags), "gport2");
    if(verbose){
        printf("created vlan-port   0x%08x\n\r",gport2);
    }
    dnx_oam_endpoint_info_glb.mc_gport_2 = gport2;

    /* 6. Add vlan-port to the vswitch and multicast */
    BCM_IF_ERROR_RETURN_MSG(vswitch_add_port(unit, vsi,vswitch_metro_mp_info.sysports[1], gport2), "");

    /* 7. Create third vlan-port */
    gport3 = 0;
    BCM_IF_ERROR_RETURN_MSG(oam__vswitch_metro_add_port(unit, 2, &gport3, flags), "");
    if(verbose){
        printf("created vlan=port   0x%08x in unit %d\n",gport3, unit);
    }
    dnx_oam_endpoint_info_glb.mc_gport_3 = gport3;

    /* 8. add vlan-port to the vswitch and multicast */
    BCM_IF_ERROR_RETURN_MSG(vswitch_add_port(unit, vsi,vswitch_metro_mp_info.sysports[2], gport3), "");

    /* 9. Add MACT entry for known unicast */
    BCM_IF_ERROR_RETURN_MSG(vswitch_add_l2_addr_to_gport(unit, gport3, kmac, known_vlan), "");

    /* 10. Configure ingress/egress VLAN actions */
    BCM_IF_ERROR_RETURN_MSG(vlan_translation_default_mode_init(unit), "");

    BCM_IF_ERROR_RETURN_MSG(hairpin_filter_with_max_strength(unit), "");
...
int oam__vswitch_gport_add(int unit, bcm_port_t port, int vlan)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, dnx_oam_endpoint_info_glb.vsi, gport4), "");
    if(verbose)
    {
    ...
    BCM_IF_ERROR_RETURN_MSG(multicast__gport_encap_add(unit, dnx_oam_endpoint_info_glb.vsi, port, gport4, 0), "");
...
int create_vswitch_and_mac_entries(int unit) {
    ...
    BCM_IF_ERROR_RETURN_MSG(oam__vswitch_metro_run(unit, known_mac_lsb, known_vlan), "");

    printf("Adding mep MAC addresses to MAC table\n");
    BCM_IF_ERROR_RETURN_MSG(vswitch_add_l2_addr_to_gport(unit, gport1, dnx_oam_endpoint_info_glb.mac_mep_1, vsi), "");

    BCM_IF_ERROR_RETURN_MSG(vswitch_add_l2_addr_to_gport(unit, gport2, dnx_oam_endpoint_info_glb.mac_mep_2, vsi), "");
...
/**
 * Create Hairpin filter with max strength so that OAM 
 * classifier doesn't affect such packets. 
 * 
 * @param unit 
 * 
 * @return int 
 */
int hairpin_filter_with_max_strength(int unit) {
    bcm_gport_t hairpin_gport;
    int hairpin_trap_id;

    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_action_profile_get(unit, bcmRxTrapEgHairPinFilter, &hairpin_gport), "bcmRxTrapEgHairPinFilter");

    hairpin_trap_id = BCM_GPORT_TRAP_GET_ID(hairpin_gport);
    BCM_GPORT_TRAP_SET(hairpin_gport, hairpin_trap_id, 0xf, 0);
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_action_profile_set(unit, 0, bcmRxTrapEgHairPinFilter, hairpin_gport), "bcmRxTrapEgHairPinFilter");
