...
int create_user_defined_trap(int unit, int *trap_id)
{
    ...
    printf("Trap Create - bcmRxTrapUserDefine, ID=0x%x \n", *trap_id);

    /* Set the trap */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_set(unit, *trap_id, &trap_config), "");
    printf("Trap Set - ID=0x%x, Flags=0x%x, DestPort=0x%x \n", *trap_id, trap_config.flags, trap_config.dest_port);
...
int field_grp_create_trap_action (int unit, bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_attach_fg_id, context_id, &attach_info), "");
...
int cint_utils_flow_id_set(int unit, int in_port, int flow_id, bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(create_user_defined_trap(unit, &trap_id), "");
    BCM_GPORT_TRAP_SET(cpu_trap_gport, trap_id, 8, 0);

    cpu_voq_number = flow_id;
    field_grp_create_trap_action(unit, context_id);
    BCM_IF_ERROR_RETURN_MSG(set_ipmf1_trap_action(unit, cpu_trap_gport, cpu_voq_number, in_port), "");
...
int cint_utils_flow_id_clean(int unit, bcm_field_context_t context_id)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "(fg = %d, context_id = %d)", cint_attach_fg_id, context_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_detach(unit, cint_attach_fg_id, context_id), error_msg);
