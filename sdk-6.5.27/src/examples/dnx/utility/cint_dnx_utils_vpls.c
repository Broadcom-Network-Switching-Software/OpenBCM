    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &ac_port.encap_fec_id), "");
    ...
    uint8 create_rif)
{
     ...
     */
    snprintf(error_msg, sizeof(error_msg), "port=%d", l2_port->port);
    if(!BCM_GPORT_IS_TRUNK(l2_port->port)) {
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, l2_port->port, bcmPortClassId, l2_port->port), error_msg);
    } else {
        BCM_TRUNK_ID_GROUP_GET(group_id, BCM_GPORT_TRUNK_GET(l2_port->port));
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, l2_port->port, bcmPortClassId, group_id), error_msg);
    }

    /*
     * Set Out-Port default properties 
     */
    BCM_IF_ERROR_RETURN_MSG(out_port_set(unit, l2_port->port), error_msg);
    ...
    if (create_rif)
    {
         ...
         */
        BCM_IF_ERROR_RETURN_MSG(in_port_intf_set(unit, l2_port->port, l2_port->intf), error_msg);
    }

    /* port must be added to the vlan for membership check */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, l2_port->intf, l2_port->port, 0), "");
    ...
    l2_port_properties_s * port)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(intf_eth_rif_create(unit, port->intf, port->mac_addr), "pwe_intf");
    ...
    l2_port_properties_s * port)
{
     ...
     * Configure ARP entry 
     */
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_encap__create(unit, 0, &port->arp, port->next_hop, port->intf),
    ...
    int flag)
{
     ...
     * 4) Give the out port. 
     */
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create(unit, fec_id, mpls_tunnel_id, 0, port, flags, flags2),
    ...
    mpls_tunnel_initiator_create_s * mpls_tunnel)
{
    ...
    for (i = MAX_NOF_TUNNELS - 1; i >= 0; i--)
    {
        ...
        if ((mpls_tunnel[i].label[0] >= MIN_LABEL) && (mpls_tunnel[i].label[0] <= MAX_LABEL))
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_initiator_create(unit, 0, mpls_tunnel[i].num_labels, label_array), "");
    ...
    mpls_port_add_s * port_term)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_port_add(unit, port_term->forwarding_domain, mpls_port), "");
    ...
    int port2)
{
     ...
     * Configure fec entry 
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_create_fec_entry(unit, ac_port.encap_fec_id, mpls_encap_tunnel[0].tunnel_id, pwe_port.port,mpls_tunnel_in_eei?BCM_L3_ENCAP_SPACE_OPTIMIZED:0), "");
    ...
    int is_ive)
{
     ...
     * configure ive swap
     */
    BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set(unit, cint_vpls_basic_info.vlan_port_id, 0x8100, 0, bcmVlanActionReplace, bcmVlanActionNone, 1111, 0, 5        /* edit_profile 
                                  */ , 4 /* tag_format */ , is_ive), "");
    return BCM_E_NONE;
}
    ...
    int is_ive)
{
     ...
     * configure ive swap
     */
    BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set_with_pri_action(unit, cint_vpls_basic_info.vlan_port_id, 0x8100, 0x09100, bcmVlanTpidActionNone, bcmVlanTpidActionNone, bcmVlanActionReplace, bcmVlanActionNone, bcmVlanActionAdd, bcmVlanActionNone, 1111, 0, 5        /* edit_profile 
                                                                 */ , 4 /* tag_format */ , is_ive), "");
    return BCM_E_NONE;
}
...
vpls_mpls_port_delete_all(int unit, int vpn_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_port_traverse(unit, additional_info, NULL, NULL), "");
    return BCM_E_NONE;
}
...
vpls_mpls_next_layer_network_domain_value(int unit, int next_layer_network_domain)
    ...
    /** prefix NEXT_LAYER_NETWORK_DOMAIN_PWE_WO_SD_TAGS of format IRPP_IN_LIF_FORMATS */
