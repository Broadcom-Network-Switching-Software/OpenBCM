    ...
    bcm_mpls_port_t * mpls_port)
{
    ...
    if (is_egress)
    {
        ...
        else if (BCM_L3_ITF_TYPE_IS_FEC(mpls_port->egress_tunnel_if))
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_get(unit, mpls_port->egress_tunnel_if, &egr),
                "failed for getting tunnel encap id from FEC");
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_mpls_port_add(unit, 0, &mpls_port_egr), "failed for egress");
    ...
    if (is_forward_group || is_1to1_protect)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_create(unit, l3eg_flags, &l3eg, &if_fec_id), "create FEC failed");
    ...
    if (is_ingress)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_mpls_port_add(unit, vpn, &mpls_port_ing), "failed for ingress");
    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    for (label_index = num_labels - 1; label_index >= 0; label_index = label_index - 1 - skip_count)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_initiator_create(unit, intf, nof_labels, outer_label_array), "");
...
mpls__add_switch_entry(int unit, mpls__ingress_tunnel_utils_s *mpls_tunnel_properties)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "with lable %d", entry.label);
