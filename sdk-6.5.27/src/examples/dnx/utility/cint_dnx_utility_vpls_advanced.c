    ...
    int nof_tpids)
{
    ...
    for(tpid_idx = 0; tpid_idx < nof_tpids; tpid_idx++)
    {
        if(tpids[tpid_idx].valid)
        {
            ...
            snprintf(error_msg, sizeof(error_msg), "port=0x%x, class_id = %d", *tpids[tpid_idx].port, tpids[tpid_idx].class_id);
            BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, *tpids[tpid_idx].port, bcmPortClassIngress, tpids[tpid_idx].class_id), error_msg);

            /* add TPID to global array */
            if(tpids[tpid_idx].tpid1)
            {
                snprintf(error_msg, sizeof(error_msg), "for 0x%x", tpids[tpid_idx].tpid1);
                BCM_IF_ERROR_RETURN_MSG(tpid__tpid_add(unit, tpids[tpid_idx].tpid1), error_msg);
            }

            if(tpids[tpid_idx].tpid2)
            {
                /* add TPID to global array */
                snprintf(error_msg, sizeof(error_msg), "for 0x%x", tpids[tpid_idx].tpid2);
                BCM_IF_ERROR_RETURN_MSG(tpid__tpid_add(unit, tpids[tpid_idx].tpid2), error_msg);
            }
            ...
            snprintf(error_msg, sizeof(error_msg), "port = 0x%x", *tpids[tpid_idx].port);
    ...
    for(vlan_idx = 0; vlan_idx < nof_vlan; vlan_idx++)
    {
        if(vlan_edits[vlan_idx].valid)
        {
            ...
            if(vlan_edits[vlan_idx].port_match.match != BCM_PORT_MATCH_INVALID)
            {
                ...
                BCM_IF_ERROR_RETURN_MSG(bcm_port_match_add(unit, *vlan_edits[vlan_idx].vlan.vlan_port_id, &match_info), "");
            ...
            /* Configure IVE */
            snprintf(error_msg, sizeof(error_msg), "port=0x%08x", *vlan_edits[vlan_idx].vlan.vlan_port_id);
            BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set(unit, *vlan_edits[vlan_idx].vlan.vlan_port_id,
                                                   ...
                                                   vlan_edits[vlan_idx].ve_translate.vlan_edit_profile, vlan_edits[vlan_idx].ve_translate.tag_format,vlan_edits[vlan_idx].ve_translate.is_ive), error_msg);
    ...
    int nof_groups)
{
    ...
    for(group_idx = 0; group_idx < nof_groups; group_idx++)
    {
        if(groups[group_idx].valid)
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, groups[group_idx].src, &config), "");
    ...
    int nof_vpls_term)
{
    ...
    for(vpls_idx = 0; vpls_idx < nof_vpls_term; vpls_idx++)
    {
        if(mpls_is_valid_lable(port_term[vpls_idx].match_label))
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_mpls_port_add(unit, *port_term[vpls_idx].forwarding_domain, mpls_port), "");
    ...
    for(vlan_idx = 0; vlan_idx < nof_vlan; vlan_idx++)
    {
        if(vlans[vlan_idx].criteria != BCM_VLAN_PORT_MATCH_INVALID)
        {
            ...
            *vlans[vlan_idx].vlan_port_id = vlan_port.vlan_port_id;
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, *vlans[vlan_idx].vsi, vlan_port.vlan_port_id), "");
    ...
    int port2)
{
     ...
     */
    snprintf(error_msg, sizeof(error_msg), "Failed to create VLAN %d", vpls_util_advanced_vpn);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, vpls_util_advanced_vpn), error_msg);
    /* add vlan to Ac port */
    snprintf(error_msg, sizeof(error_msg), "(port = %d)", port1);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, vpls_util_advanced_vpn, port1, BCM_VLAN_GPORT_ADD_INGRESS_ONLY), error_msg);
    for (i = 0; i < VPLS_UTIL_ADVANCED_MAX_NOF_ENTITIES; i++)
    {
        ...
        for(vpls_idx = 0; vpls_idx < VPLS_UTIL_ADVANCED_MAX_NOF_VPLS; vpls_idx++)
        {
            if(mpls_is_valid_lable(vpls_util_entity[i].vpls_ingress[vpls_idx].match_label))
            {
                ...
                snprintf(error_msg, sizeof(error_msg), "Failed to create VLAN %d", *vpls_util_entity[i].vpls_ingress[vpls_idx].forwarding_domain);
                BCM_IF_ERROR_RETURN_MSG(rv, error_msg);
            }
        }
    }

    BCM_IF_ERROR_RETURN_MSG(mpls_util_main(unit, 0, 0), "");
    ...
    for (i = 0; i < VPLS_UTIL_ADVANCED_MAX_NOF_ENTITIES; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(vpls_util_advanced_vlan_port_config(unit, vpls_util_entity[i].vlan_ports, VPLS_UTIL_ADVANCED_MAX_NOF_VLAN_PORT), "");
    }
    ...
    for (i = 0; i < VPLS_UTIL_ADVANCED_MAX_NOF_ENTITIES; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(vpls_util_advanced_l2_addr_add(unit, vpls_util_entity[i].l2_macs, VPLS_UTIL_ADVANCED_MAX_NOF_L2_MAC_FWD), "");
    }
    ...
    for (i = 0; i < VPLS_UTIL_ADVANCED_MAX_NOF_ENTITIES; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(vpls_util_advanced_config_networt_group_id(unit, vpls_util_entity[i].nwk_groups, VPLS_UTIL_ADVANCED_MAX_NOF_NWK_GROUPS), "");
    }
