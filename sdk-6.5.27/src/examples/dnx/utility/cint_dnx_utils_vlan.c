                              ...
                              int *action_id)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "for action_id - %d", *action_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_id_set(unit, flags, *action_id, &action), error_msg);
                            ...
                            uint32 vlan_edit_profile)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "failed for gport_id - %d", lif_translation_info.gport);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_translation_set(unit, &lif_translation_info), error_msg);
                            ...
                            uint32 vlan_edit_profile)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(vlan__avt_vid_action__set(unit, is_ingress, outer_tpid, inner_tpid, outer_action, inner_action, &action_id), "");

    snprintf(error_msg, sizeof(error_msg), "failed for lif - %d", lif);
    BCM_IF_ERROR_RETURN_MSG(vlan__avt_lif_info__set(unit, is_ingress, lif, new_outer_vid, new_inner_vid, vlan_edit_profile), error_msg);

    snprintf(error_msg, sizeof(error_msg), "failed for action_id - %d", action_id);
    BCM_IF_ERROR_RETURN_MSG(vlan__translate_action_class__set(unit, action_id, vlan_edit_profile, g_eve_edit_utils.tag_format, is_ingress), error_msg);
...
int vlan__eve_default__set(int unit, bcm_gport_t  lif, int outer_vlan, int inner_vlan , bcm_vlan_action_t outer_action, bcm_vlan_action_t inner_action){
    ...
    BCM_IF_ERROR_RETURN_MSG(vlan_port_translation_set(unit, outer_vlan, inner_vlan, lif, g_eve_edit_utils.edit_profile, 0), "");
    ...
    */
    BCM_IF_ERROR_RETURN_MSG(vlan_translate_action_set(unit, 
    ...
    BCM_IF_ERROR_RETURN_MSG(vlan_default_translate_action_class_set(unit, g_eve_edit_utils.action_id), "");
...
/*  vlan_port.flags = BCM_VLAN_PORT_CREATE_EGRESS_ONLY; */
    ...
    /* set vlan edit profile for out AC.
       set outer vlan and inner for out AC.*/
    BCM_IF_ERROR_RETURN_MSG(vlan_port_translation_set(unit, 
    ...
    BCM_IF_ERROR_RETURN_MSG(vlan_translate_action_set(
       unit, 
       action_id_1, 
       action_id_2, 
       0, /* is_ingress */ 
...
int vlan__translate_action_with_id__set(int unit, int action_id,
    ...
    /* Create action ID*/
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_id_create( unit, flags | BCM_VLAN_ACTION_SET_WITH_ID, &action_id), "");
...
int vlan__init_vlan(int unit, int vlan) {
    ...
    bcm_port_config_t c;
    bcm_pbmp_t p,u;
    uint32 max_nof_cores = 0, is_mesh = 0, is_mesh_mc, is_tdm_supported, flags_get;

    BCM_IF_ERROR_RETURN_MSG(bcm_port_config_get(unit, &c), "");
    BCM_PBMP_ASSIGN(p, c.e);

    snprintf(error_msg, sizeof(error_msg), "vlan=%d", vlan);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, vlan), error_msg);
    
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, vlan, p, u), error_msg);
     ...
     */
    snprintf(error_msg, sizeof(error_msg), "mc_group_id %d", vlan);
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_group_get(unit, vlan, &flags_get), error_msg);
    ...
    if (flags_get)
    {
        snprintf(error_msg, sizeof(error_msg), "Multicast group exists, try to destroy. flags_get %d mc_group_id %d",
            flags_get, vlan);
        BCM_IF_ERROR_RETURN_MSG(bcm_multicast_destroy(unit, vlan), error_msg);
    }

    uint32 flags;
    if (*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL)) {
        flags = BCM_MULTICAST_WITH_ID | BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_ELEMENT_STATIC;
    } else {
        flags = BCM_MULTICAST_WITH_ID | BCM_MULTICAST_EGRESS_GROUP;
    }
    snprintf(error_msg, sizeof(error_msg), "flags %d mc_group_id %d", flags, vlan);
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_create(unit, flags, vlan), error_msg);

    BCM_PBMP_CLEAR(p);
    snprintf(error_msg, sizeof(error_msg), "vlan %d", vlan);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_get(unit, vlan, &p, &u), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "group %d", vlan);
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, vlan, flags, nof_rep, rep_array), error_msg);
    ...
    if (!(*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL))) {
        ...
        if (*dnxc_data_get(unit, "fabric", "general", "blocks_exist", NULL)) {
            ...
            if (is_mesh && (is_mesh_mc || is_tdm_supported)) {
                snprintf(error_msg, sizeof(error_msg), "group %d", vlan);
                int max_nof_cores = *dnxc_data_get(unit, "device", "general", "nof_cores", NULL);
                bcm_module_t mreps[max_nof_cores];
                uint32 valid_cores_bmp = *(dnxc_data_get(unit, "device", "general", "valid_cores_bitmap", NULL));
                int core_index, nof_copies;
                ...
                BCM_IF_ERROR_RETURN_MSG(bcm_fabric_multicast_set(unit, vlan, 0, nof_copies, mreps), error_msg);
            }
        }
    }
...
int vlan__vlan_port_vsi_create(int unit, int vsi, bcm_port_t port, bcm_vlan_t outer_vid, bcm_vlan_t inner_vid, uint32 flags, bcm_gport_t *gport){
    ...
    snprintf(error_msg, sizeof(error_msg), "port %d, vlan_domain %d", port, port);
...
vlan__open_vlan_per_ports(int unit, uint16 vid, bcm_gport_t *ports, uint8 *tagged, int nof_ports) {
    ...
    if (rv != BCM_E_NONE) {
        ...
        snprintf(error_msg, sizeof(error_msg), "vlan=%d", vid);
    ...
    for(port_id = 0; port_id < nof_ports; ++port_id) {
        ...
        snprintf(error_msg, sizeof(error_msg), "fail add port(0x%08x) to vlan(%d)", ports[port_id], vid);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, vid, ports[port_id], flags), error_msg);
...
vlan__open_vlan_per_mc(int unit, int vlan, bcm_multicast_t mc_group)
{
  ...
  if (vlan <= BCM_VLAN_MAX) {
      ...
      rv = bcm_vswitch_create_with_id(unit, vlan);
  }
  if (rv != BCM_E_NONE) {
  ...
  /* Set VLAN with MC */
  snprintf(error_msg, sizeof(error_msg), "fail get control vlan(%d)", vlan);
  BCM_IF_ERROR_RETURN_MSG(bcm_vlan_control_vlan_get(unit, vlan, &control_vlan), error_msg);
  ...
  snprintf(error_msg, sizeof(error_msg), "fail set control vlan(%d)", vlan);
  BCM_IF_ERROR_RETURN_MSG(bcm_vlan_control_vlan_set(unit, vlan, control_vlan), error_msg);
...
int vlan__not_vlan_member_trap_config(int unit, int strength, bcm_gport_t dst_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_create(unit,0,trap,&trap_id), "");
...
int vlan__ingress_membership_create(int unit, bcm_vlan_t vlan, bcm_gport_t port, bcm_gport_t trap_dst_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(vlan__not_vlan_member_trap_config(unit, trap_strength, trap_dst_port), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit,vlan, port, flags), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_port_vlan_member_set(unit,port,BCM_PORT_VLAN_MEMBER_INGRESS), "");
...
int vlan__egress_membership_create(int unit, bcm_vlan_t vsi, bcm_gport_t port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit,vsi, port,flags), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_port_vlan_member_set(unit,port,BCM_PORT_VLAN_MEMBER_EGRESS), "");
...
int vlan__default_ve_profile_set(int unit, int ve_profile) {
    ...
    BCM_IF_ERROR_RETURN_MSG(vlan__avt_vid_action__set(unit,
                                   0 /* is ingress */,
                                   0x8100 /* outer TPID */,
                                   0x8100 /* inner TPID */,
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit,  bcmSwitchDefaultEgressVlanEditClassId, ve_profile), "");
...
int vlan__default_out_ac_set(int unit, bcm_gport_t port, bcm_gport_t vlan_port) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_match_add(unit,vlan_port, &port_match_info), "");
...
int vlan__default_out_ac_allocate_and_set(int unit, int port) {
    ...
    BCM_IF_ERROR_RETURN_MSG(vlan__default_out_ac_set(unit,port,vlan_port.vlan_port_id), "");
...
int vlan__native_default_out_ac_set(int unit, bcm_gport_t l2lif_port, bcm_gport_t native_vlan_port) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_match_add(unit,native_vlan_port, &port_match_info), "");
...
int vlan__native_default_out_ac_allocate_and_set(int unit, 
    ...
    if (!is_device_or_above(unit, JERICHO2)) {
        BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchDefaultNativeOutVlanPort, vlan_port.vlan_port_id), "");
    } else if (l2lif_port != -1) {
        /* set port's default out ac */
        BCM_IF_ERROR_RETURN_MSG(vlan__native_default_out_ac_set(unit,l2lif_port,vlan_port.vlan_port_id), "");
    }
...
int vlan__native_default_out_ac_allocate_and_set_dflt_action(int unit, int outer_vlan, int inner_vlan, bcm_gport_t l2lif_port) {
    ...
    BCM_IF_ERROR_RETURN_MSG(vlan__native_default_out_ac_allocate_and_set(unit, outer_vlan, inner_vlan, &default_action, l2lif_port), "");
