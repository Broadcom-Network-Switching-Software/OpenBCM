 ...
 * 
 ...
 Copyright 2007-2022 Broadcom Inc. All rights reserved. File: cint_sand_utility_mpls.c Purpose: Various examples for MPLS. 
 */
...
struct mpls_util_fec_s
{
    ...
    uint32 flags;               /* Interface flags (BCM_L3_TGID, BCM_L3_L2TOCPU). */
    uint32 allocation_flags;    /* flags passed to bcm_l3_egress_create */
...
struct mpls_util_mpls_tunnel_initiator_create_s
{
    ...
    int ttl; /* ttl of label */
    int encap_size_expected; /** nof labels expected to be encapsulated which associated to that egress object */
...
int mpls_concatened_tunnels_scenario = 0;
    ...
    int port2)
{
    ...
    if (!mpls_params_set)
    {
        BCM_IF_ERROR_RETURN_MSG(init_default_mpls_params(unit, port1, port2), "");
    }
    ...
    for (i = 0; i < MPLS_UTIL_MAX_NOF_ENTITIES; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(mpls_configure_port_properties(unit, mpls_util_entity[i].ports, MPLS_UTIL_MAX_NOF_PORTS, vlan_default), "");
    }
    ...
    for (i = 0; i < MPLS_UTIL_MAX_NOF_ENTITIES; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(mpls_create_arp_entries(unit, mpls_util_entity[i].arps, MPLS_UTIL_MAX_NOF_ARPS), "");
    }
    ...
    for (i = 0; i < MPLS_UTIL_MAX_NOF_ENTITIES; i++)
    {
        ...
        {
            BCM_IF_ERROR_RETURN_MSG(mpls_create_fec_entries(unit, mpls_util_entity[i].fecs, MPLS_UTIL_MAX_NOF_FECS), "");
        }
    }
    ...
    for (i = 0; i < MPLS_UTIL_MAX_NOF_ENTITIES; i++)
    {
        ...
        {
            BCM_IF_ERROR_RETURN_MSG(mpls_create_l3_forwarding(unit, mpls_util_entity[i].hosts_ipvx, MPLS_UTIL_MAX_NOF_HOST_IPV4), "");
        }
    }
    ...
    bcm_gport_t vlan_default)
{
    ...
    for (port_idx = 0; port_idx < nof_ports; port_idx++)
    {
        ...
        {
            dnx_utils_l3_port_s_common_init(unit, 0, &l3_port, l2_port[port_idx].port, *l2_port[port_idx].eth_rif_id);
            l3_port.eg_vlan_port_id = vlan_default;
            snprintf(error_msg, sizeof(error_msg), "for index %d", port_idx);
            BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_port_set(unit, l3_port), error_msg);
            l2_port[port_idx].eg_vlan_port_id = l3_port.eg_vlan_port_id;
            l2_port[port_idx].ing_vlan_port_id = l3_port.ing_vlan_port_id;
        }
    }
    return BCM_E_NONE;
}
    ...
    int nof_rifs)
{
    ...
    for (rif_idx = 0; rif_idx < nof_rifs; rif_idx++)
    {
        ...
        {
            dnx_utils_l3_eth_rif_s_common_init(unit, 0, &l3_eth_rif, rifs[rif_idx].intf, 0, rifs[rif_idx].ingress_flags, rifs[rif_idx].mac_addr, rifs[rif_idx].vrf);
            snprintf(error_msg, sizeof(error_msg), "for index %d", rif_idx);
            BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_eth_rif_create(unit, l3_eth_rif), error_msg);
        }
    }
    return BCM_E_NONE;
}
    ...
    int nof_arps)
{
    ...
    for (arp_idx = 0; arp_idx < nof_arps; arp_idx++)
    {
        ...
        {
            dnx_utils_l3_arp_s_common_init(unit, arps[arp_idx].allocation_flags, &arp_entry, arps[arp_idx].arp, arps[arp_idx].flags, arps[arp_idx].flags2, arps[arp_idx].next_hop, *arps[arp_idx].intf);
            snprintf(error_msg, sizeof(error_msg), "for index %d", arp_idx);
    ...
    int nof_fecs)
{
    ...
    for (fec_idx = 0; fec_idx < nof_fecs; fec_idx++)
    {
        if (fecs[fec_idx].fec_id != MPLS_UTIL_INVALID_VALUE)
        {
            ...
            dnx_utils_l3_fec_s_common_init(unit, allocation_flags | fecs[fec_idx].allocation_flags, 0, &fec_entry, fecs[fec_idx].fec_id, fecs[fec_idx].flags, 0, destination, *fecs[fec_idx].tunnel_gport, *fecs[fec_idx].tunnel_gport2);
            ...
            {
                fec_entry.flags2 |= BCM_L3_FLAGS2_FWD_ONLY;
            }
            snprintf(error_msg, sizeof(error_msg), "for index %d", fec_idx);
            BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_fec_create(unit, fec_entry), error_msg);
    ...
    int nof_tunnels)
{
    ...
    for (tunnel_idx = nof_tunnels - 1; tunnel_idx >= 0; tunnel_idx--)
    {
        if (mpls_is_valid_lable(mpls_tunnel[tunnel_idx].label[0]) || (mpls_tunnel[tunnel_idx].action == BCM_MPLS_EGRESS_ACTION_PHP)
            || mpls_tunnel[tunnel_idx].label[0] == 0)
        {
            ...
            if (mpls_tunnel[tunnel_idx].flags & BCM_MPLS_EGRESS_LABEL_PROTECTION) {
                label_array[0].egress_failover_id = mpls_tunnel[tunnel_idx].egress_failover_id;
                label_array[0].egress_failover_if_id = mpls_tunnel[tunnel_idx].egress_failover_if_id;
            }
            if (mpls_tunnel[tunnel_idx].egress_qos_model.egress_ttl != bcmQosEgressModelUniform) {
                label_array[0].ttl = mpls_tunnel[tunnel_idx].ttl;
            }
            ...
            if (mpls_tunnel[tunnel_idx].flags & BCM_MPLS_EGRESS_LABEL_PROTECTION) {
                label_array[1].egress_failover_id = mpls_tunnel[tunnel_idx].egress_failover_id;
                label_array[1].egress_failover_if_id = mpls_tunnel[tunnel_idx].egress_failover_if_id;
            }
            if (mpls_tunnel[tunnel_idx].egress_qos_model.egress_ttl != bcmQosEgressModelUniform) {
                label_array[1].ttl = mpls_tunnel[tunnel_idx].ttl;
            }


            nof_actual_tunnels++;

            /** estimated_encap_size should describe total nof bytes encapsulate with this object */
            /** It will influence the global lif allocation of the tunnel */
            /** For concatenated tunnels, it is essential to calc it in the "HEAD" */
            /** Assuming the first tunnel is the "HEAD" */
            if ((tunnel_idx == 0) && (mpls_concatened_tunnels_scenario == 1) && (estimate_encap_size_required))
            {
                uint32 mpls_encap_size = *dnxc_data_get(unit, "lif", "out_lif", "global_mpls_outlif_bta_sop", NULL);
                label_array[0].estimated_encap_size = mpls_tunnel[tunnel_idx].encap_size_expected =
                        nof_actual_tunnels * mpls_tunnel[tunnel_idx].num_labels * mpls_encap_size;
            }
            snprintf(error_msg, sizeof(error_msg), "for index %d", tunnel_idx);
            BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_initiator_create(unit, 0, mpls_tunnel[tunnel_idx].num_labels, label_array), error_msg);

            mpls_tunnel[tunnel_idx].tunnel_id = label_array[0].tunnel_id;

            if(verbose >= 1) {
                printf("Created MPLS tunnel: id = 0x%08x with: label_1 = 0x%x, label_2 = 0x%x, flags = 0x%x", mpls_tunnel[tunnel_idx].tunnel_id, mpls_tunnel[tunnel_idx].label[0], mpls_tunnel[tunnel_idx].label[1], mpls_tunnel[tunnel_idx].flags);
                if(mpls_tunnel[tunnel_idx].l3_intf_id)
                {
    ...
    int action_pop)
{
    ...
    for (switch_idx = 0; switch_idx < nof_switches; switch_idx++)
    {
            ...
            (!action_pop & (mpls_switch[switch_idx].action != BCM_MPLS_SWITCH_ACTION_POP))))
        {
            ...
            entry.class_id = mpls_switch[switch_idx].class_id;
            ...
            snprintf(error_msg, sizeof(error_msg), "for index %d", switch_idx);
            BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_switch_create(unit, &entry), error_msg);
            ...
            if (mpls_switch[switch_idx].action == BCM_MPLS_SWITCH_ACTION_POP)
            {
                ...
                snprintf(error_msg, sizeof(error_msg), "for index %d", switch_idx);
                BCM_IF_ERROR_RETURN_MSG(bcm_l3_ingress_create(unit, &l3_ing_if, l3_intf_id), error_msg);
            }
        }
    }
    ...
    int nof_hosts)
{
    ...
    bcm_ip6_t ipv6_mask = {0};
    dnx_utils_l3_host_ipv4_s l3_host_ipv4;
    dnx_utils_l3_host_ipv6_s l3_host_ipv6;
    char error_msg[200]={0,};
    ...
    for (host_idx = 0; host_idx < nof_hosts; host_idx++)
    {
        ...
        {
            /* check if address is IPv4 multicast */
            if ((host[host_idx].ipv4 & 0xf0000000) != 0xe0000000)
            {
                if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
                {
                    dnx_utils_l3_host_ipv4_s_common_init(unit, 0, &l3_host_ipv4, host[host_idx].ipv4, host[host_idx].vrf, 0, 0, *host[host_idx].fec_id);
                    snprintf(error_msg, sizeof(error_msg), "for index %d", host_idx);
                    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_host_ipv4_add(unit, l3_host_ipv4), error_msg);
                }
                else
                {
                    bcm_l3_route_t l3rt;
                    bcm_l3_route_t_init(l3rt);
                    l3rt.l3a_subnet = host[host_idx].ipv4;
                    l3rt.l3a_ip_mask = 0xFFFFFFFF;
                    l3rt.l3a_vrf = host[host_idx].vrf;
                    l3rt.l3a_intf = *host[host_idx].fec_id;       /* FEC-ID */
            ...
            {
                /* IPv4 multicast address, full match */
                bcm_ipmc_addr_t_init(&ipmc_addr);
                ipmc_addr.mc_ip_addr = host[host_idx].ipv4;
                ipmc_addr.mc_ip_mask = 0xFFFFFFFF;
                ipmc_addr.vrf = host[host_idx].vrf;
                ipmc_addr.l3a_intf = *host[host_idx].fec_id;
                snprintf(error_msg, sizeof(error_msg), "for index %d", host_idx);
        ...
        {
            /* check if address is IPv6 multicast */
            if (host[host_idx].ipv6[0] != 0xff)           
            {                
                if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL))
                {
                    dnx_utils_l3_host_ipv6_s_common_init(unit, 0, &l3_host_ipv6, host[host_idx].ipv6, host[host_idx].vrf, 0, 0, *host[host_idx].fec_id);
                    snprintf(error_msg, sizeof(error_msg), "for index %d", host_idx);
                    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_host_ipv6_add(unit, l3_host_ipv6), error_msg);
                }
                else
                {
                    bcm_l3_route_t l3rt;
                    ...
                    l3rt.l3a_ip6_net = host[host_idx].ipv6;
            ...
            else
            {
                ...
                /* IPv6 multicast address, full match */
                bcm_ipmc_addr_t_init(&ipmc_addr);
                ipmc_addr.flags = BCM_IPMC_IP6;
                if (!*(dnxc_data_get(unit, "l3", "fwd", "ipmc_config_cmprs_vrf_group", NULL)))
                {
                    ipmc_addr.mc_ip6_addr = host[host_idx].ipv6;
                    ipmc_addr.mc_ip6_mask = full_mask;
                    ipmc_addr.vrf = host[host_idx].vrf;
                }
                ipmc_addr.l3a_intf = *host[host_idx].fec_id;
                snprintf(error_msg, sizeof(error_msg), "for index %d", host_idx);
