...
int l2__learning__config(int unit){
    ...
    BCM_IF_ERROR_RETURN_MSG(l2__olp_port__get(unit, &olp_port), "");
...
int l2__egress_create(int unit, l2__arp_entry_properties_s *arp_entry_properties){
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_egress_create(unit,&arp_entry), "");
...
int get_payload_from_fid_and_mac(int unit, int fid, bcm_mac_t mac, reg_val *res){
    ...
    BCM_IF_ERROR_RETURN_MSG(diag_reg_get(unit, cmd, temp_key), "running command PPDB_B_LARGE_EM_DIAGNOSTICS_KEY");

    /* Set KEY register */
    sprintf(cmd, "PPDB_B_LARGE_EM_DIAGNOSTICS_KEY");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_set(unit, cmd, key), "running command PPDB_B_LARGE_EM_DIAGNOSTICS_KEY");

    /* Save DIAGNOSTICS register */
    sprintf(cmd, "PPDB_B_LARGE_EM_DIAGNOSTICS");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_get(unit, cmd, temp_diag), "running command PPDB_B_LARGE_EM_DIAGNOSTICS");

    /* Set LOOKUP register */
    sprintf(cmd, "PPDB_B_LARGE_EM_DIAGNOSTICS");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_set(unit, cmd, lookup), "running command PPDB_B_LARGE_EM_DIAGNOSTICS");

    /* Read LOOKUP RESULT */
    sprintf(cmd, "PPDB_B_LARGE_EM_DIAGNOSTICS_LOOKUP_RESULT");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_get(unit, cmd, *res), "running command PPDB_B_LARGE_EM_DIAGNOSTICS_LOOKUP_RESULT");

    /* Revert LOOKUP register */
    sprintf(cmd, "PPDB_B_LARGE_EM_DIAGNOSTICS");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_set(unit, cmd, temp_diag), "running command PPDB_B_LARGE_EM_DIAGNOSTICS");

    /* Revert KEY register */
    sprintf(cmd, "PPDB_B_LARGE_EM_DIAGNOSTICS_KEY");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_set(unit, cmd, temp_key), "running command PPDB_B_LARGE_EM_DIAGNOSTICS_KEY");
...
* profile_idx - 
*******************************************************************************/
int l2__vpn_to_vsi_profile_set(int unit, uint32 vsi, uint32 profile_idx)
{
 
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_cache_vpn_to_profile_map_set (unit, vsi, profile_idx), "");
    return BCM_E_NONE;
}
