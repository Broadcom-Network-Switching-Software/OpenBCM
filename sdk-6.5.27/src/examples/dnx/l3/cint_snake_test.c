...
int cint_l3_snake(int unit, bcm_port_t *ports, int nof_ports) {
    ...
    for (i = 0; i < nof_ports; i++) { 
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add (unit, vid[i], pbmp, ubmp), "");

        bcm_l3_intf_t_init(&(l3if[i]));
        sal_memcpy(l3if[i].l3a_mac_addr, mac_l3_ingress, 6);
        l3if[i].l3a_vrf = vid[i];
        l3if[i].l3a_vid = vid[i];
        BCM_IF_ERROR_RETURN_MSG(bcm_l3_intf_create(unit, l3if[i]), ""); 
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_create(unit, BCM_L3_EGRESS_ONLY, &l3eg, &l3egid), error_msg);

        l3eg.port = egress_port;
        l3eg.intf = ingress_intf[i];
        BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_create(unit, BCM_L3_INGRESS_ONLY, &l3eg, &l3egid), error_msg);
