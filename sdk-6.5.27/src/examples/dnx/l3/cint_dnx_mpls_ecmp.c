...
int mpls_ecmp_tunnel_initiator_create(int unit, int level, int label, int out_interface, int* tunnel_id) {
    ...
    snprintf(error_msg, sizeof(error_msg), "Label = 0x%x", label_array[0].label);
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_tunnel_initiator_create(unit,0,1,label_array), error_msg);
...
int mpls_ecmp_cascaded_fec_create(int unit, int out_interface, int out_port, int out_vlan, bcm_mac_t next_hop_mac, bcm_if_t* fec_id) {
    ...
    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_arp_create(unit, &l3_arp), "");
...
int mpls_dual_ecmp_main(int unit, int in_port, int out_port, int slb_resolve) {
     ...
     */
    rv = in_port_intf_set(unit, in_port, mpls_ecmp_info_1.in_vlan);
    if (rv == BCM_E_EXISTS) {
        rv = BCM_E_NONE;
    }
    snprintf(error_msg, sizeof(error_msg), "in_port = %d, intf_in = %d", in_port, mpls_ecmp_info_1.in_vlan);
     ...
     */
    rv = intf_eth_rif_create(unit, mpls_ecmp_info_1.in_vlan, mpls_ecmp_info_1.my_mac1);
    if (rv == BCM_E_EXISTS) {
        rv = BCM_E_NONE;
    }
    snprintf(error_msg, sizeof(error_msg), "intf_in = %d", mpls_ecmp_info_1.in_vlan);
    BCM_IF_ERROR_RETURN_MSG(rv, error_msg);

    rv = intf_eth_rif_create(unit, mpls_ecmp_info_1.out_vlan, mpls_ecmp_info_1.my_mac2);
    if (rv == BCM_E_EXISTS) {
        rv = BCM_E_NONE;
    }
    snprintf(error_msg, sizeof(error_msg), "intf_in = %d", mpls_ecmp_info_1.out_vlan);
    BCM_IF_ERROR_RETURN_MSG(rv, error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "intf_in = %d, vrf = %d", mpls_ecmp_info_1.in_vlan, vrf);
    BCM_IF_ERROR_RETURN_MSG(rv, error_msg);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_ecmp_create(unit, &ecmp1_1, interface_count, interfaces1_1), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_ecmp_create(unit, &ecmp1_2, interface_count, interfaces1_2), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_ecmp_first_fec_create(unit, tunnel_id_1, 0, out_gport, &fec_id1), "");


    BCM_L3_ITF_FEC_TO_GPORT_FORWARD_GROUP(out_gport, ecmp1_2.ecmp_intf);
    BCM_IF_ERROR_RETURN_MSG(mpls_ecmp_first_fec_create(unit, tunnel_id_2, 0, out_gport, &fec_id2), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_ecmp_create(unit, &ecmp1, interface_count, interfaces1), "");
    ...
    {
        bcm_l3_route_t l3host_r;
        bcm_l3_route_t_init(l3host_r);
        l3host_r.l3a_subnet = mpls_ecmp_info_1.host;
        l3host_r.l3a_ip_mask = 0xFFFFFFFF;
        l3host_r.l3a_vrf = vrf;
        l3host_r.l3a_intf = ecmp1.ecmp_intf;
        l3host_r.l3a_flags2 = fec_flags2;
        BCM_IF_ERROR_RETURN_MSG(bcm_l3_route_add(unit, l3host_r), "");
    }
