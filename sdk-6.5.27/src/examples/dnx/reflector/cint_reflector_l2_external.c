 ...
 * Unicast sequence:
  "
 * Test Scenario - start
  cint ../../../../src/examples/dnx/reflector/cint_reflector_l2_external.c
  cint
  int unit = 0;
    ...
    bcm_field_group_info_t * fg_info)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, fg_info, pmf1_fg_id), "Map SSP to TRAP");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, *pmf1_fg_id, pmf1_context_id, &attach_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_create(unit, 0, &action_info, &action_ssp), "");
    ...
    qual_info.size = 1;
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &qual_info, &uc_reflector_valid_qual_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &pmf2_fg_info, &fg_id), "DEST<-SSP");
    ...
    for (qual_index = 0; qual_index < pmf2_fg_info.nof_quals; qual_index++)
    {
        ...
        if (pmf2_fg_info.qual_types[qual_index] == uc_reflector_valid_qual_id)
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_field_group_action_offset_get(unit, 0, *pmf1_fg_id, bcmFieldActionTrap, &trap_offset),
                "DEST<-SSP");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, fg_id, pmf1_context_id, &attach_info), "");
    ...
    bcm_field_context_t pmf1_context_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_reflector_l2_external_fp_create(unit, pmf1_context_id, 0, &fg_id, &fg_info), "");

        /** add entry to field processor program */
    BCM_GPORT_TRAP_SET(trap_gport, trap_id, 15, 0);
    BCM_IF_ERROR_RETURN_MSG(cint_reflector_l2_external_fp_entry_add(unit, fg_id, &fg_info, sysport_gport, trap_gport, -1), "");
    ...
    bcm_field_context_t pmf1_context_id)
{
    ...
    /** Create L2 unicast field processor program */
    BCM_IF_ERROR_RETURN_MSG(cint_reflector_l2_external_fp_create(unit, pmf1_context_id, 1, &fg_id, &fg_info), "");

    /** add entry to field processor program */
    /** create trap gport */
    BCM_GPORT_TRAP_SET(trap_gport, trap_id, 15, 0);
    BCM_IF_ERROR_RETURN_MSG(cint_reflector_l2_external_fp_entry_add(unit, fg_id, &fg_info, sysport_gport, trap_gport, encap_id), "");
    return BCM_E_NONE;
}
/*
 * }
