    ...
    bcm_vlan_port_t *vp)
{
    ...
    *gport = vp->vlan_port_id;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_match_add(unit, vp->vlan_port_id, &port_match_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set(unit, vp->vlan_port_id,           /* lif  */
                                                      0x8100,                     /* outer_tpid */
                                                      0x9100,                     /* inner_tpid */
    ...
    int port2)
{
    ...
    uint32 separate_fwd_learn_mact = *dnxc_data_get(unit1, "l2", "general", "separate_fwd_learn_mact", NULL);
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(system_vswitch_create(unit1, vpn), "");

    BCM_IF_ERROR_RETURN_MSG(system_vswitch_create(unit2, vpn), "");

    /*
     * Create ACs
     */
    /* create gport1 in unit1 attach to vsi */
    BCM_IF_ERROR_RETURN_MSG(system_vlan_port_create(unit1, port1, vlan1, vpn, &gport1, &vp1), "port 1");

    /* create second_gport1 in unit1 */
    BCM_IF_ERROR_RETURN_MSG(system_vlan_port_create(unit1, second_port1, vlan1, vpn, &second_gport1, &second_vp1),
        "second port 1");

    /* create gport2 (different gport) in unit2 */
    BCM_IF_ERROR_RETURN_MSG(system_vlan_port_create(unit2, port2, vlan2, vpn, &gport2, &vp2), "port 2");

    /* Add ACs to multicast group */
    /* unit1 */
    BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit1, vpn, &port1, &vp1.encap_id, 1, 0), "for port1");

    BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit1, vpn, &second_port1, &second_vp1.encap_id, 1, 0),
        "for second_port1");

    BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit1, vpn, &port2, &vp2.encap_id, 1, 0), "for port2");

    /* unit2 */
    BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit2, vpn, &port1, &vp1.encap_id, 1, 0), "for port1");

    BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit2, vpn, &second_port1, &second_vp1.encap_id, 1, 0),
        "for second_port1");

    BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit2, vpn, &port2, &vp2.encap_id, 1, 0), "for port2");
    ...
    int port2)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(system_vlan_port_create(unit1, port1, vlan1, 0, &gport1, &vp1), "port 1");

    /* create gport2 in unit2 */
    BCM_IF_ERROR_RETURN_MSG(system_vlan_port_create(unit2, port2, vlan2, 0, &gport2, &vp2), "port 2");
    ...
    int is_mp)
{
    ...
    int gport;
    int flags2 = 0;
    int l2_flags = 0, learn_strength = 0;
    ...
    BCM_IF_ERROR_RETURN_MSG(get_first_fec_in_range_which_not_in_ecmp_range(unit1, 0, 0, &primary_fec), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(system_vlan_port_create(unit1, port1, vlan1, vpn, &gport1_lif, &vp1), "port 1");
    ...
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit1, secondary_fec, gport, 0, port3, 0, flags2, failover_id, 0, NULL),
        "create egress object FEC only");

    /* primary path */
    BCM_L3_ITF_SET(gport,BCM_L3_ITF_TYPE_LIF, gport2_lif_a);
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit1, primary_fec, gport, 0, port2, 0, flags2, failover_id, 1, NULL),
        "create egress object FEC only");
    ...
    BCM_IF_ERROR_RETURN_MSG(system_vlan_port_create(unit2, port2, vlan2, vpn, &gport2_lif_a, &vp2_a), "port 2");

    /* create gport2.LIFb in unit2 */
    BCM_IF_ERROR_RETURN_MSG(system_vlan_port_create(unit2, port3, vlan3, vpn, &gport2_lif_b, &vp2_b), "port 3");
    ...
    if (is_mp) {
        ...
        BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit1, vpn, &port2, &vp2_a.encap_id, 1, 0), "for port2");

        BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit1, vpn, &port3, &vp2_b.encap_id, 1, 0), "for port3");

        /* unit2 */
        BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit2, vpn, &port1, &vp1.encap_id, 1, 0), "for port1");

        BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit2, vpn, &port2, &vp2_a.encap_id, 1, 0), "for port2");

        BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit2, vpn, &port3, &vp2_b.encap_id, 1, 0), "for port3");

        /*
         * Add l2 addresses to be defined as static
         */
        l2_flags = (*dnxc_data_get(unit1, "l2", "general", "separate_fwd_learn_mact", NULL)) ? 0 : BCM_L2_STATIC;
        learn_strength = (*dnxc_data_get(unit1, "l2", "general", "separate_fwd_learn_mact", NULL)) ? 1 : 0;
