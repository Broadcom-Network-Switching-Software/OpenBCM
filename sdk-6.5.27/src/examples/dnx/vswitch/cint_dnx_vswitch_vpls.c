 ...
 * Test Scenario - start
  ...
  cint ../../../../src/examples/dnx/vswitch/cint_dnx_vswitch_vpls.c
  cint
...
void
vswitch_vpls_tunnel_initiator_create_s_init(
...
void
vswitch_vpls_tunnel_switch_create_s_init(
    ...
    int pwe_port_2)
{
    ...
    pwe_param->pwe_port.arp = 9001;
        /** Tunnel initiator **/
    vswitch_vpls_tunnel_initiator_create_s_init(pwe_param->mpls_encap_tunnel);
    ...
    tunnel_init->tunnel_id = 0;
        /** Tunnel termination **/
    vswitch_vpls_tunnel_switch_create_s_init(pwe_param->mpls_term_tunnel);
    ...
    int pwe_port_2)
{
    ...
    if (!vswitch_vpls_params_initialized)
    {
         ...
         * Init parameters
         */
        BCM_IF_ERROR_RETURN_MSG(vswitch_vpls_params_init(unit, &vswitch_vpls_params, ac_port_1, ac_port_2, pwe_port_1, pwe_port_2), "");
    }
    ...
    if (vswitch_vpls_params.test_split_horizon)
    {
         ...
         */
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &vswitch_vpls_params.default_native_ac), "");
    }
    /*
     * Create ACs
     */
    for (ac_idx = 0; ac_idx < vswitch_vpls_params.ac_ports_nof; ++ac_idx)
    {
        ...
        snprintf(error_msg, sizeof(error_msg), "%d", ac_idx);
        BCM_IF_ERROR_RETURN_MSG(vswitch_vpls_ac_create(unit, ac_port, vswitch_vpls_params.vpn), error_msg);
        /*
         * Add AC to VPN
         */
        BCM_IF_ERROR_RETURN_MSG(vswitch_vpls_vpn_vlan_port_add(unit, vswitch_vpls_params.vpn, ac_port), error_msg);
    }
    /*
     * Create PWEs
     */
    for (pwe_idx = 0; pwe_idx < vswitch_vpls_params.pwe_ports_nof; ++pwe_idx)
    {
        ...
        snprintf(error_msg, sizeof(error_msg), "pwe_idx=%d", pwe_idx);
        BCM_IF_ERROR_RETURN_MSG(vswitch_vpls_pwe_create(unit, pwe_param), error_msg);
        if ((vswitch_vpls_params.test_split_horizon) &&
           !(*dnxc_data_get(unit, "esem", "access_cmd", "esem_cmd_predefine_allocations", NULL)))
        {
            /*
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_port_match_add(unit, vswitch_vpls_params.default_native_ac.vlan_port_id, &match_info), "");
        }
        /*
         * Add pwe to VPN
         */
        BCM_IF_ERROR_RETURN_MSG(vswitch_vpls_vpn_pwe_add(unit, vswitch_vpls_params.vpn, pwe_param), error_msg);
    }

    /*
     * JR2 IOP: use following to translate the 15bit global fec_id to 18bit local one.
     */
    int header_mode = *(dnxc_data_get(unit,  "headers", "system_headers", "system_headers_mode", NULL));
    if (header_mode == 0) {
        BCM_IF_ERROR_RETURN_MSG(fec_id_alignment_field_add(unit, ac_port_1, BCM_L3_ITF_VAL_GET(vswitch_vpls_params.pwe_params[0].pwe_term.egress_tunnel_if)), "");
    }
    ...
    int vid)
{
     ...
     * Init port
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_configure_port_properties(unit, ac_port, 0 /* create_rif */ ), "");
    /*
    ...
    vswitch_vpls_pwe_info_s * pwe_param)
{
     ...
     * Init port
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_configure_port_properties(unit, &pwe_param->pwe_port, 1 /* create_rif */ ), "pwe_port");

    /*
     * Configure L3 interfaces
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_create_l3_interfaces(unit, &pwe_param->pwe_port), "");
    /*
     * Configure an ARP entries
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_create_arp_entry(unit, &pwe_param->pwe_port), "");
    pwe_param->mpls_encap_tunnel[0].l3_intf_id = pwe_param->pwe_port.arp;
    /*
     * Configure a tunnel initiator.
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_create_mpls_tunnel(unit, pwe_param->mpls_encap_tunnel), "");
    ...
    if (pwe_param->pwe_term.egress_tunnel_if)
    {
        ...
        if (mpls_tunnel_in_eei || (system_headers_mode == 0))
        {
            fec_flags = BCM_L3_ENCAP_SPACE_OPTIMIZED;
        }

        BCM_IF_ERROR_RETURN_MSG(vpls_create_fec_entry(unit, pwe_param->pwe_term.egress_tunnel_if,
                                   outlif_in_fec, pwe_param->pwe_port.port, fec_flags), "");
    }
    /*
     * Configure a termination label for the ingress flow
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_create_termination_stack(unit, pwe_param->mpls_term_tunnel), "");
    /*
     * configure PWE tunnel - egress flow
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mpls_port_add_encapsulation(unit, &pwe_param->pwe_encap), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(vpls_mpls_port_add_termination(unit, &pwe_param->pwe_term), "");
    ...
    bcm_vpn_t vpn_id)
{
     ...
     */
    int mc_id = vpn_id;
    snprintf(error_msg, sizeof(error_msg), "mc_id=%d", mc_id);
    BCM_IF_ERROR_RETURN_MSG(multicast__open_mc_group(unit, &mc_id, 0), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "vpn_id=%d", vpn_id);
    ...
    if (rv == BCM_E_NONE)
    {
         ...
         */
        BCM_IF_ERROR_RETURN_MSG(bcm_mpls_vpn_id_destroy(unit, vpn_id), error_msg);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mpls_vpn_id_create(unit, &info), error_msg);
    ...
    l2_port_properties_s * ac_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, vpn_id, ac_port->vlan_port_id), "");

    BCM_GPORT_LOCAL_SET(gport, ac_port->port);
    BCM_IF_ERROR_RETURN_MSG(multicast__add_multicast_entry(unit, mc_id, &gport, &ac_port->encap_id, 1 /* nof_mc_entries */ , egress_mc), "");
    return BCM_E_NONE;
}
    ...
    vswitch_vpls_pwe_info_s * pwe_param)
{
    ...
    if ((egress_mc == FALSE) && (*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL))) {
    ...
    snprintf(error_msg, sizeof(error_msg), "port = %d, encap1 = 0x%08X, encap2 = 0x%08X", rep.port, rep.encap1, rep.encap2);
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, mc_id, flags, 1, &rep), error_msg);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_encap_extension_create(unit, BCM_MULTICAST_ENCAP_EXTENSION_WITH_ID, &(pwe_param->pwe_mc_rep_idx),
            2, cuds), "");
