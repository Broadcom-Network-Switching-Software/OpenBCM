 ...
 *  
 * dnx_vswitch_cross_connect_p2p_run(0, <port1>, <type1>, <with_protection_1>, <second_port1>, <port2>, <type2>, <with_protection_2>, <second_port2>, <facility_protection>); 
 ...
 * cint
 * dnx_vswitch_cross_connect_p2p_run(0,201,1,0,201,202,1,0,202,0);
 * exit;
 *
 * tx 1 psrc=201 data=0x000000000002000000000005910000640899c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5
 *
 * // Received packets on unit 0 should be: 
 * // Source port: 0, Destination port: 0 
 * // Data: 0x0000000000020000000000058100c3e90899c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5c5 
 *
 * Test Scenario - end
 */
...
int is_oam_primary_vlan = 0; /* global variable that determines whether enable primary VLAN mode */
...
vlan_port_create(
    ...
    if (vp.ingress_failover_id || vp.failover_mc_group) {
        if (protect_gport == 0) {
            ...
            BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create(unit, &l3_egress), "for FEC");
    ...
    *gport = vp.vlan_port_id;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, match_vlan, pbmp, ubmp), "");
...
int cross_connect(int unit, int p1, int *gport1, int p2, int *gport2, int vid)
{
    ...
    for (i = 0; i < sizeof(ports) / sizeof(ports[0]); i++) {
        ...
        vp.vsi = 0; /* will be populated when the gport is added to service, using vswitch_port_add */
        vp.flags = 0;
        if(is_oam_primary_vlan) {
            vp.flags2 |= BCM_VLAN_PORT_FLAGS2_OAM_PRIMARY_VLAN;
        }
        ...
        *(gport_out_arr[i]) = vp.vlan_port_id;
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, vid+i, ports[i], 0), "");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_cross_connect_add(unit, &xconn_info), "");
...
mpls_tunnels_config(
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, vlan, pbmp, pbmp), error_msg);
    ...
    intf.skip_mymac = 1;
    BCM_IF_ERROR_RETURN_MSG(l3__intf_rif__create(unit, &intf), "");
...
mpls_port_create(
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_tunnels_config(unit, port, &egress_intf, in_vc_label, eg_vc_label), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(sand_mpls_port_add(unit, 0, mpls_port), "");
...
mim_bvid_config(
    ...
    snprintf(error_msg, sizeof(error_msg), "mc_group %d", mc_group);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_EGRESS_GROUP | BCM_MULTICAST_WITH_ID | BCM_MULTICAST_TYPE_MIM, &mc_group), error_msg);
...
mim_bvid_create(
    ...
    BCM_IF_ERROR_RETURN_MSG(mim_bvid_config(unit, vpn_config, vpn_id), "");

    /* create the B-VID */
    BCM_IF_ERROR_RETURN_MSG(bcm_mim_vpn_create(unit, vpn_config), "");
  
    /* set the B-VID port membership */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, vpn_config->vpn, pbmp, ubmp), "");
...
mim_port_create(
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit, port, bcmPortControlMacInMac, 1), "");

    /* make the mim physical port recognize the B-tag ethertype */
    port_tpid_init(port, 1, 0);
    port_tpid_info1.outer_tpids[0] = 0x81a8;
    BCM_IF_ERROR_RETURN_MSG(port_tpid_set(unit), "with port_1");


    /* set MiM port ingress vlan-editing to remove B-tag if exists 
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_default_action_set(unit, port, &action), "with port_2");
    */
    ...
    /* create B-VID */
    BCM_IF_ERROR_RETURN_MSG(mim_bvid_create(unit, bvid, cross_connect_info.bvid_base++, pbmp, ubmp), "with bvid"); 
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mim_port_add(unit, mim_p2p_vsi, mim_port), ""); /* use invalid VPN (=0xffff/0x7fff) to create P2P port depending on device type*/
...
mim_init(int unit) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mim_init(unit), "");

    /* set the global and per port L2 station for the B-DA (MyMac LSB + MSB) */
    BCM_IF_ERROR_RETURN_MSG(mim_l2_station_add(unit, 1 /* global */, 0, cross_connect_info.dest_bmac), "(global)");
    BCM_IF_ERROR_RETURN_MSG(mim_l2_station_add_for_termination(unit, cross_connect_info.dest_bmac), "");
...
int vswitch_cross_connect_double_tag_port_configuration(int unit, int port1, int protection1, int second_port1, int port2, int protection2, int second_port2,int *dbl_tag){
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, port1, bcmPortClassId, 0), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, port2, bcmPortClassId, 0), "");

    if (protection1) {
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, second_port1, bcmPortClassId, 0), "");
    }
    if (protection2) {
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, second_port2, bcmPortClassId, 0), "");
    }
...
int
dnx_vswitch_cross_connect_p2p_run(int unit, int port1, int type1, int protection1, int second_port1 /* used if protection1 is set */,
    ...
    BCM_IF_ERROR_RETURN_MSG(vswitch_cross_connect_double_tag_port_configuration(unit, port1, protection1,second_port1,port2, protection2, second_port2, &dbl_tag), "");
    ...
    if ((type1 == 3) || (type1 == 4) || (type2 == 3) || (type2 == 4)) { /* mim */
        /* init mim on the device */
        BCM_IF_ERROR_RETURN_MSG(mim_init(unit), "");
    }
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, vlan, port1, 0), "");
    ...
    if (protection1 == 1 || protection1 == 2) {
        if (facility) {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, failover_flags, &cross_connect_info.failover_id_1), "(failover_id_1)");

            if (decoupled_mode && protection1 == 2) {
                /* create failover_id for second_port1 */
                BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, failover_flags, &cross_connect_info.failover_id_3), "(failover_id_3)");
            }
    ...
    if (protection1 == 2) {
        ...
        egress_mc = 0;
        BCM_IF_ERROR_RETURN_MSG(multicast__open_mc_group(unit, &cross_connect_info.protection_mc1, 0), "");
        if(verbose >= 1) {
    ...
    if (protection2) {
        if (facility) {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_failover_create(unit, BCM_FAILOVER_FEC, &cross_connect_info.failover_id_2), "(failover_id_2)");
            if(verbose >= 1) {
    ...
    /* 1+1 protection */
        ...
        /* create MC for protection */
        egress_mc = 0;
        BCM_IF_ERROR_RETURN_MSG(multicast__open_mc_group(unit, &cross_connect_info.protection_mc2, 0), "");
    ...
    case 1:
        ...
        if (protection1 == 1 || protection1== 2) {
            ...
            snprintf(error_msg, sizeof(error_msg), "with port %d", cross_connect_info.port_3);
            if (!decoupled_mode) {
                BCM_IF_ERROR_RETURN_MSG(vlan_port_create(unit, 
            ...
            if(protection1 == 2) {
                ...
                snprintf(error_msg, sizeof(error_msg), "with port %d", cross_connect_info.port_1);
                BCM_IF_ERROR_RETURN_MSG(multicast__vlan_port_add(unit,cross_connect_info.protection_mc1,cross_connect_info.port_1,cross_connect_info.vlan_port_1, 0), error_msg);
                snprintf(error_msg, sizeof(error_msg), "with port %d", cross_connect_info.port_3);
                BCM_IF_ERROR_RETURN_MSG(multicast__vlan_port_add(unit,cross_connect_info.protection_mc1,cross_connect_info.port_3,cross_connect_info.vlan_port_3, 0), error_msg);
            }
        }
        else {
            cross_connect_info.gports.port1 = cross_connect_info.vlan_port_1;
    ...
    case 2:
        /* create mpls port */
        snprintf(error_msg, sizeof(error_msg), "with port %d", cross_connect_info.port_1);
        BCM_IF_ERROR_RETURN_MSG(mpls_port_create(unit, 
                              cross_connect_info.port_1, 
                              &(cross_connect_info.mpls_port_1), 
                              cross_connect_info.failover_id_1, 
                              0,
                              cross_connect_info.protection_mc1), error_msg);
        ...
        if (protection1) {
           ...
           if (!decoupled_mode) {
            ...
            if(protection1 == 2) {
                ...
                /* add gport to protection MC */
                snprintf(error_msg, sizeof(error_msg), "with port %d", cross_connect_info.port_1);
                BCM_IF_ERROR_RETURN_MSG(multicast__mpls_port_add(unit,cross_connect_info.protection_mc1,cross_connect_info.port_1,egr_mpls_port_id_1, 0/*is_egress*/), error_msg);
                snprintf(error_msg, sizeof(error_msg), "with port %d", cross_connect_info.port_3);
                BCM_IF_ERROR_RETURN_MSG(multicast__mpls_port_add(unit,cross_connect_info.protection_mc1,cross_connect_info.port_3,egr_mpls_port_id_2, 0 /*is_egress*/), error_msg);
            }
        }
        else {
            cross_connect_info.gports.port1 = cross_connect_info.mpls_port_1.mpls_port_id;
    ...
    case 3:
        /* create mim port */
        snprintf(error_msg, sizeof(error_msg), "with port %d", cross_connect_info.port_1);
        BCM_IF_ERROR_RETURN_MSG(mim_port_create(unit, 
                             cross_connect_info.port_1, 
    ...
    case 1:
        /* create vlan port */
        snprintf(error_msg, sizeof(error_msg), "with port %d", cross_connect_info.port_2);
        BCM_IF_ERROR_RETURN_MSG(vlan_port_create(unit,
        ...
        if (protection2) {
            /* create another vlan port - use first port as protection */
            snprintf(error_msg, sizeof(error_msg), "with port %d", cross_connect_info.port_4);
            BCM_IF_ERROR_RETURN_MSG(vlan_port_create(unit, 
    ...
    case 2:
        ...
        BCM_IF_ERROR_RETURN_MSG(mpls_port_create(unit, 
                              ...
                              cross_connect_info.failover_id_2, 
                              0,
                              cross_connect_info.protection_mc2), error_msg);
    ...
    if (protection1 == 2) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_cross_connect_add(unit, &gports2), "");
    }
...
int failover1_set(int unit, int enable){
    ...
    snprintf(error_msg, sizeof(error_msg), "failover_id:  0x%08x", cross_connect_info.failover_id_1);
    BCM_IF_ERROR_RETURN_MSG(bcm_failover_set(unit, cross_connect_info.failover_id_1, enable), error_msg);
...
traverse_run(int unit) {
    ...
    snprintf(error_msg, sizeof(error_msg), "with port %d", cross_connect_info.port_5);
    BCM_IF_ERROR_RETURN_MSG(vlan_port_create(unit, 
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_cross_connect_traverse(unit, call_back, &traverse_no), "");
...
delete_all(int unit) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_cross_connect_delete_all(unit), "");
