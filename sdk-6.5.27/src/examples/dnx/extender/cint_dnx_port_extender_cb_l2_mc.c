 ...
 * Test Scenario - start
  ...
  cint extender/cint_dnx_port_extender_cb_l2_mc.c
  cint
 *
...
int port_extender_cb_l2_mc__start_run(int unit, port_extender_cb_l2_mc_s *param) {
    ...
    BCM_IF_ERROR_RETURN_MSG(port_extender_cb_l2_mc_init(unit, param), error_msg);

    /* Define a VSI for the switch */
    snprintf(error_msg, sizeof(error_msg), "failed for vsi - %d", g_port_extender_cb_l2_mc.vsi);
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_create_with_id(unit, g_port_extender_cb_l2_mc.vsi), error_msg);

    mc_group = g_port_extender_cb_l2_mc.vsi;
    /* MC Group */
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_create(unit, (BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_WITH_ID), &mc_group), "");

    for (port_id = 0; port_id < NOF_EXTENDER_PORTS; port_id++)
    {
        snprintf(error_msg, sizeof(error_msg), "for vlan %d", g_port_extender_cb_l2_mc.ports[port_id].vid);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, g_port_extender_cb_l2_mc.ports[port_id].vid, g_port_extender_cb_l2_mc.ports[port_id].port, 0), error_msg);

        if (g_port_extender_cb_l2_mc.ports[port_id].is_cascaded)
        {
            /* Configure cascaded ports */
            snprintf(error_msg, sizeof(error_msg), "for port - %d", g_port_extender_cb_l2_mc.ports[port_id].port);
            BCM_IF_ERROR_RETURN_MSG(extender__cascaded_port_configure(unit, g_port_extender_cb_l2_mc.ports[port_id].port), error_msg);
            ...
            snprintf(error_msg, sizeof(error_msg), "port - %d", g_port_extender_cb_l2_mc.ports[port_id].port);
            BCM_IF_ERROR_RETURN_MSG(extender__extender_port_mc_configure(unit, &extender_port_mc), error_msg);
            ...
            snprintf(error_msg, sizeof(error_msg), "port_id - %d", port_id);
            BCM_IF_ERROR_RETURN_MSG(port_extender_cb_l2_mc_eve_config(unit,
                                                port_id,
                                                g_port_extender_cb_l2_mc.ports[port_id].mc_gport,
                                                g_port_extender_cb_l2_mc.mc_ecid,
                                                0), error_msg); /* Not first call */
            ...
            snprintf(error_msg, sizeof(error_msg), "in Port= %d,encap = 0x%x", rep_array.port, rep_array.encap1);
            BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, mc_group, BCM_MULTICAST_INGRESS_GROUP, 1, &rep_array), error_msg);
        }
        ...
        snprintf(error_msg, sizeof(error_msg), "vsi - %d, extender_port_id - %d",
            g_port_extender_cb_l2_mc.vsi, extender_port.extender_gport_id);
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, g_port_extender_cb_l2_mc.vsi, extender_port.extender_gport_id), error_msg);
        ...
        snprintf(error_msg, sizeof(error_msg), "port_id - %d", port_id);
        port_extender_cb_l2_mc_eve_config(unit,
                                            port_id,
        ...
        bcm_l2_addr_t_init(&l2_address, g_port_extender_cb_l2_mc.ports[port_id].mac, g_port_extender_cb_l2_mc.vsi);
        l2_address.flags = BCM_L2_STATIC; /* static entry */
        l2_address.port = g_port_extender_cb_l2_mc.ports[port_id].gport;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(unit, mc_addr), "");
    return BCM_E_NONE;
}
...
int port_extender_cb_l2_mc_eve_config(int unit, int port_id, int lif, int ecid, uint8 is_first)
{
    ...
    if (is_first) {
        ...
        snprintf(error_msg, sizeof(error_msg), "edit profile %d", 10 + port_id);
