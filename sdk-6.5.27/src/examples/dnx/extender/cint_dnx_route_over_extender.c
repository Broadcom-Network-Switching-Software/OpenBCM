 ...
 * Test Scenario - start
  ...
  cint extender/cint_dnx_route_over_extender.c
  cint
...
int route_over_extender__start_run(int unit, route_over_extender_s *param) {
    ...
    BCM_IF_ERROR_RETURN_MSG(route_over_extender_init(unit, param), "");

    /* set VLAN domain to each port */
    for (i=0; i< ROUTE_OVER_EXTENDER_NUM_OF_RIFS; i++) {
        snprintf(error_msg, sizeof(error_msg), "for vsi - %d", g_route_over_extender.rif_def[i].vsi);
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_create_with_id(unit, g_route_over_extender.rif_def[i].vsi), error_msg);
        snprintf(error_msg, sizeof(error_msg), "for port %d", g_route_over_extender.rif_def[i].sys_port);
        if (g_route_over_extender.host_def[i].is_cascaded_port) {
            BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, g_route_over_extender.host_def[i].vlan_i, g_route_over_extender.rif_def[i].sys_port, 0), error_msg);
        } else {
            BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, g_route_over_extender.host_def[i].vlan_o, g_route_over_extender.rif_def[i].sys_port, 0), error_msg);
        }
    }
    ...
    for (i=0; i< ROUTE_OVER_EXTENDER_NUM_OF_RIFS; i++) {
        snprintf(error_msg, sizeof(error_msg), "for rif index %d", i);
        BCM_IF_ERROR_RETURN_MSG(route_over_extender_rif_mac_create(unit, i), error_msg);
    }

    /* ****************************************************************** */
    /*  Create l2 logical interface, egress object and add route          */
    /* ****************************************************************** */
    snprintf(error_msg, sizeof(error_msg), "for rif index %d, host index %d", RIF_1, HOST_1);
    BCM_IF_ERROR_RETURN_MSG(route_over_extender_l2_l3_create(unit,RIF_1,HOST_1), error_msg);

    snprintf(error_msg, sizeof(error_msg), "for rif index %d, host index %d", RIF_2, HOST_2);
    BCM_IF_ERROR_RETURN_MSG(route_over_extender_l2_l3_create(unit,RIF_2,HOST_2), error_msg);
    ...
    for (i=0; i< ROUTE_OVER_EXTENDER_NUM_OF_HOSTS; i++) {
        if (!g_route_over_extender.host_def[i].is_extender_port) {
            BCM_IF_ERROR_RETURN_MSG(vlan__default_out_ac_allocate_and_set(unit, g_route_over_extender.rif_def[i].sys_port), "");
        } else {
            BCM_IF_ERROR_RETURN_MSG(route_over_extender_vlan_edit_extender_ac_set(unit, g_route_over_extender.host_def[i].gport, 0), "");
        }
    }
    return BCM_E_NONE;
}
...
int route_over_extender_rif_mac_create(int unit, int rif_index) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l3_ingress_create(unit, l3_ing_if, g_route_over_extender.rif_def[rif_index].vsi), "");
...
int route_over_extender__fec_extender_port_create(int unit, int rif_index, int host_index) {
    ...
    if (g_route_over_extender.host_def[host_index].is_cascaded_port) {
        snprintf(error_msg, sizeof(error_msg), "for port - %d", g_route_over_extender.rif_def[rif_index].sys_port);
        BCM_IF_ERROR_RETURN_MSG(extender__cascaded_port_configure(unit, g_route_over_extender.rif_def[rif_index].sys_port), error_msg);
    }
    ...
    snprintf(error_msg, sizeof(error_msg), "vsi - %d, extender_port_id - %d",
        g_route_over_extender.rif_def[rif_index].vsi, extender_port.extender_gport_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, g_route_over_extender.rif_def[rif_index].vsi, extender_port.extender_gport_id), error_msg);
...
int route_over_extender_l2_l3_create(int unit, int rif_index, int host_index) {
    ...
    if (g_route_over_extender.host_def[host_index].is_extender_port)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(route_over_extender__fec_extender_port_create(unit, rif_index, host_index), "");
        ...
        BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_fec_create(unit, &l3_fec), "");
        ...
        snprintf(error_msg, sizeof(error_msg), "vsi - %d, vlan_port - #%d",
            g_route_over_extender.rif_def[rif_index].vsi, vp.vlan_port_id);
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, g_route_over_extender.rif_def[rif_index].vsi, vp.vlan_port_id), error_msg);
    }
    else
    {
        /* Create non protected l2 logical interface according <port,vlan>*/
        BCM_IF_ERROR_RETURN_MSG(l2__fec_forward_group_port_vlan__create(unit,  g_route_over_extender.rif_def[rif_index].sys_port,
                                                           ...
                                                           &outLif_dummy), "Fail  L2 and FEC entry create");
    ...
    ecmp.max_paths = 1;
    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_ecmp_create(unit, &ecmp,
                                   1, /* nof paths */
...
int route_over_extender_cleanup(int unit) {
    ...
    for (i = (ROUTE_OVER_EXTENDER_NUM_OF_HOSTS - 1); i  >= 0; i--) {
        ...
        if (g_route_over_extender.host_def[i].is_extender_port) {
            BCM_IF_ERROR_RETURN_MSG(extender__extender_port_cleanup(unit, g_route_over_extender.host_def[i].gport), "");
        }

        snprintf(error_msg, sizeof(error_msg), "(0x%x)", g_route_over_extender.host_def[i].encap_id);
        BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_destroy(unit, g_route_over_extender.host_def[i].encap_id), error_msg);

        bcm_l3_egress_ecmp_t_init(&ecmp);
        ecmp.max_paths = 1;
        ecmp.ecmp_intf = g_route_over_extender.host_def[i].ecmp_intf;
        snprintf(error_msg, sizeof(error_msg), "(0x%x)", g_route_over_extender.host_def[i].ecmp_intf);
        BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_ecmp_destroy(unit, &ecmp), error_msg);
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_l3_host_delete_all(unit, &host_info), "");
