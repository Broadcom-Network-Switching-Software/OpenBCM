    ...
    int pwe_mode/*1- pwe raw; 2- pwe unindexed tagged*/)
{
    ...
    {
        BCM_IF_ERROR_RETURN_MSG(vpls_pwe_tagged_mode_basic_main(unit, ac_port, pwe_port), "");
        ...
        printf("Error BCM_E_UNAVAIL, unsupported pwe mode(%d) in oam_run_over_inner_eth_with_pwe\n", pwe_mode);
        return BCM_E_UNAVAIL;
    }

    /* Get counter resource */
    BCM_IF_ERROR_RETURN_MSG(set_counter_resource(unit, pwe_port, counter_if, 1, &counter_base_id_pwe), "");
    BCM_IF_ERROR_RETURN_MSG(set_counter_resource(unit, ac_port, counter_if, 1, &counter_base_id_access), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(unit, oam_profile_pwe, bcmOAMProfileIngressLIF, &flags, &ing_profile_pwe_id), "(ingress)");
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(unit, oam_profile_pwe, bcmOAMProfileEgressLIF, &flags, &egr_profile_pwe_id), "(egress)");

    oam_profile_eth = oam_lif_profiles.oam_profile_eth_single_ended;
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(unit, oam_profile_eth, bcmOAMProfileIngressLIF, &flags, &ing_profile_eth_id), "(ingress)");
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(unit, oam_profile_eth, bcmOAMProfileEgressLIF, &flags, &egr_profile_eth_id), "(egress)");

    /* Bind lif with ingress and egress profiles */
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_lif_profile_set(unit, 0, in_pwe_lif, ing_profile_pwe_id, BCM_OAM_PROFILE_INVALID), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_oam_lif_profile_set(unit, 0, eg_access_ac, ing_profile_eth_id, egr_profile_eth_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(oam__my_cfm_mac_set(unit, ac_port, native_dmac), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_endpoint_create(unit, &dnmep_info), "");
    ...
    if (add_upmep_on_egress_ac) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_oam_endpoint_create(unit, &upmep_info), "");
    }
