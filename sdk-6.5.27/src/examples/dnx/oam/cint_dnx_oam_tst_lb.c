    ...
    bcm_gport_t sat_port, int lb_tst_flag, int mep_id, int ctf_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_endpoint_get(unit, mep_id, &endpoint_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_set(unit, is_upmep ? up_mep_ingress_trap_id : down_mep_sat_trap_id, trap_config), "");
    ...
    if(is_upmep)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_set(unit, up_mep_sat_trap_id, trap_config), "");
        ...
        BCM_IF_ERROR_RETURN_MSG(rv, "");
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, flags, bcmRxTrapEgTxOamUpMEPDest1, &up_mep_stamp_trap), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_get(unit, up_mep_stamp_trap, &trap_config), "");
        trap_config.stamped_trap_code =  up_mep_sat_trap_id & 0x1FF;
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_set(unit, up_mep_stamp_trap, trap_config), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(unit, ing_lif_profile_id, bcmOAMProfileIngressLIF, &flags, &enc_ing_lif_profile_id), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(unit, egr_lif_profile_id, bcmOAMProfileEgressLIF, &flags, &enc_egr_lif_profile_id), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_lif_profile_set(unit, 0, gport, enc_ing_lif_profile_id, (endpoint_info.type == bcmOAMEndpointTypeBHHPwe)? BCM_OAM_PROFILE_INVALID : enc_egr_lif_profile_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_action_set(unit, 0, acc_ing_action_profile_id, &action_key, &action_result), "");
...
int dnx_oam_sat_start(int unit, int gtf_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_sat_gtf_bandwidth_set(unit, gtf_id, 0, &bw1), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_sat_gtf_rate_pattern_set(unit,gtf_id, 0, &rate_pattern1), "");

    /* 3. Start transmitting */
    BCM_IF_ERROR_RETURN_MSG(bcm_sat_gtf_packet_start(unit, gtf_id, bcmSatGtfPriCir), "");
...
int dnx_oam_lb_tst_sat_run(int unit, int port1, int port2, int port3, int lb_tst_flag, int mep_is_up, int lb_pcp_dei_flag)
{
    ...
    if (1 == lb_tst_flag){ 
        BCM_IF_ERROR_RETURN_MSG(bcm_oam_loopback_add(unit, &lb), "");
        gtf_id = lb.gtf_id;
        ctf_id = lb.ctf_id;
        printf("lb gtf %d \n\n",lb.gtf_id);
    }
    else if (2 == lb_tst_flag){
        BCM_IF_ERROR_RETURN_MSG(bcm_oam_tst_rx_add(unit, &tst_rx_ptr), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_oam_tst_tx_add(unit, &tst_tx_ptr), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(dnx_oam_sat_start(unit, gtf_id), "");
...
int dnx_oam_run_lb_tst_mpls_pwe(int unit, int port1, int port2, int lb_tst_flag, int mpls_pwe_flag)
{
    ...
    if(mpls_pwe_flag == 1) { /*mpls*/
        BCM_IF_ERROR_RETURN_MSG(oam_run_with_defaults_mpls_tp(unit, port1, port2, 0), "");
    } else if(mpls_pwe_flag == 2) {/*pwe*/
        BCM_IF_ERROR_RETURN_MSG(oam_run_with_defaults_pwe(unit, port1, port2, 0), "");
    } else {/*MPLS-TP Section */
        BCM_IF_ERROR_RETURN_MSG(oam_run_with_defaults_section(unit, port1, port2, 0), "");
    }
