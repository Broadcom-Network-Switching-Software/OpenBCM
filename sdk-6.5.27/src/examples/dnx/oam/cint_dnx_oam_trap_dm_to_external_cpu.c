...
dnx_oam_trap_dm_to_rcpu_info_t trap_dm_to_rcpu_info = {
    ...
    /* rcpu_tpid | rcpu_vlan | rcpu_ethertype */
    0x8100,            100,            0x8999,
    /* rcpu_eth_out_rif | rcpu_l3a_intf_id */
    101,                0,
    /* is_upmep */
    0,
    ...
    int unit)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchFtmhEtherType, trap_dm_to_rcpu_info.rcpu_ethertype),
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, trap_dm_to_rcpu_info.rcpu_port, bcmPortClassFieldEgressPacketProcessingPortCs, trap_dm_to_rcpu_info.rcpu_port_profile),
    ...
    int unit)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(intf_eth_rif_create(unit, eth_out_rif, src_mac), "intf_out");

    /*
     * Create ARP+AC entry for outer Ethernet and set its properties
     * ARP+AC indication is flag BCM_L3_FLAGS2_VLAN_TRANSLATION
     */
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_encap__create_inner(unit, 0, &trap_dm_to_rcpu_info.rcpu_l3a_intf_id, trap_dm_to_rcpu_info.rcpu_dst_mac_trap, eth_out_rif, 0, BCM_L3_FLAGS2_VLAN_TRANSLATION),
     ...
     */
    arp_outlif = BCM_L3_ITF_VAL_GET(trap_dm_to_rcpu_info.rcpu_l3a_intf_id);
    ...
    port_trans.new_outer_vlan = trap_dm_to_rcpu_info.rcpu_vlan;
    ...
    action.outer_tpid = trap_dm_to_rcpu_info.rcpu_tpid;
     ...
     * Init remark profile
     */
    BCM_IF_ERROR_RETURN_MSG(rcpu_arp_plus_ac_qos(unit, rcpu_pcp, rcpu_dei, &rcpu_egr_qos_map), "");

    /*
     * Connect ARP VLAN translation to Remark profile which is used to decide the PCP value on VLAN tag
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_port_map_set(unit, arp_ac_gport, -1, rcpu_egr_qos_map), "");
    ...
    int update_rcpu_encap_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_get(unit, trap_dm_to_rcpu_info.rcpu_ingress_cpu_trap_id, &config), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_set(unit, trap_dm_to_rcpu_info.rcpu_ingress_cpu_trap_id, &config), "");
