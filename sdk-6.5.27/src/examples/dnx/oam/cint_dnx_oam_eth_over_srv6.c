    ...
    int nof_sids)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(srv6_ingress_tunnel_config(unit, port2, port2, port1, nof_sids, secondary_srh_flags, secondary_sids_flags, is_t_insert_encap, last_sid_is_dip), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_cross_connect_add(unit, &gports), "to connect inlif to Srv6 tunnel");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(dnx_oam_accelerated_endpoint_create(unit, oam_up_mep_id, gport2, group_info.id,
                                             counter_if_0, lm_counter_base_id_0, acc_egress_profile, 0, 1), "(Up MEP)");
    printf("finished create accelerated Up-MEP\n");

    /*
     * Add loss on Up-MEP
     */
    BCM_IF_ERROR_RETURN_MSG(dnx_oam_loss_add(unit, oam_up_mep_id, 65552, BCM_OAM_ENDPOINT_CCM_PERIOD_10MS, 0, 1), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(dnx_oam_delay_add(unit, oam_up_mep_id, 0, BCM_OAM_ENDPOINT_CCM_PERIOD_10MS), "");
