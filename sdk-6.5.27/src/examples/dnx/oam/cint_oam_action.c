    ...
    int is_dual_ended)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapBfdOamDownMEP, &trap_id), "");
    BCM_GPORT_TRAP_SET(cpu_dest_gport, trap_id, 7, 0);

    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamLevel, &trap_id_level), "");
    ...
    int is_dual_ended)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_create(unit, flags, profile_type, &ingress_profile_id), "");
    /*
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_create(unit, flags, profile_type, &egress_profile_id), "");

    /*
     * Set ingress profile action
     */
    BCM_IF_ERROR_RETURN_MSG(oam_lm_action(unit, is_ingress, ingress_profile_id, is_dual_ended), "");

    /*
     * Bind profiles to oam_lif
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_lif_profile_set(unit, 0, oam_lif, ingress_profile_id, egress_profile_id), "");
    return BCM_E_NONE;
}
    ...
    oam_destinations_s * oam_destinations)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapBfdOamDownMEP, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamLevel, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapEgTxOamLevel, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPassive, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapEgTxOamPassive, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamEthAccelerated, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPerformanceEthAccelerated, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamY1731Pwe, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPerformanceY1731Pwe, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamY1731MplsTp, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPerformanceY1731MplsTp, &trap_code), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapSnoopOamPacket, &trap_code), "");
     ...
     * Get the trap code dedicated for CPU trap. 
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit,0 , bcmRxTrapEgTxOamUpMEPDest1, &trap_code), "");
    BCM_GPORT_TRAP_SET(oam_destinations->active_up_dest_gport, trap_code, 10, 0);

    /*
     * Get the trap code dedicated for CPU Recycle. 
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit,0 , bcmRxTrapOamReflector, &trap_code), "");
    ...
    for (lif_type = MEP_TYPE_ETH; lif_type <= MEP_TYPE_PWE; lif_type++)
    {
        for (lm_type = LM_TYPE_SINGLE_ENDED; lm_type <= last_lm_type; lm_type++)
        {
            ...
            *(oam_lif_profiles_t[lif_profile_index]) = ingress_profile_id;
            ...
            if (lif_type == MEP_TYPE_ETH)
            {
                /* Use same egress profile id as ingress for test purpose */
                egress_profile_id = ingress_profile_id & 0xF;
                profile_type = bcmOAMProfileEgressLIF;
                BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_create(unit, flags|BCM_OAM_PROFILE_WITH_ID, profile_type, &egress_profile_id), "");
            }

            /*
             * Set ingress profile action
             */
            BCM_IF_ERROR_RETURN_MSG(oam_action_config(unit, is_ingress, is_acc, ingress_profile_id, oam_destinations, lif_type, lm_type), "");
             ...
             )
            {
                ...
                */
               BCM_IF_ERROR_RETURN_MSG(oam_action_config(unit, is_egress, is_acc, egress_profile_id, oam_destinations, lif_type, lm_type), "");
            }
            lif_profile_index++;
            /*
    ...
    int unit)
{
     ...
     * Get Destinations
     */
    BCM_IF_ERROR_RETURN_MSG(oam_get_destinations(unit, &oam_destinations), "");

    BCM_IF_ERROR_RETURN_MSG(oam_non_acc_set_of_action_profiles_create(unit, oam_destinations), "");

    BCM_IF_ERROR_RETURN_MSG(oam_acc_set_of_action_profiles_create(unit, is_ingress, oam_destinations), "");
    return BCM_E_NONE;
}
    ...
    bcm_oam_profile_t *acc_ingress_profile)
{
    ...
    is_acc = 1;
    profile_type = bcmOAMProfileIngressAcceleratedEndpoint;
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_create(unit, flags, profile_type, acc_ingress_profile), "");
    oam_section_acc_profile = *acc_ingress_profile;
    /*
     * Set ingress profile action
     */
    BCM_IF_ERROR_RETURN_MSG(oam_action_config(unit, is_ingress, is_acc, *acc_ingress_profile, oam_destinations, lif_type, lm_type), "");
 ...
 * The OAMP may reply to LMMs, DMMs, LBMs, SLMs with their 
 * respective reply packets. This is an example of how to set 
 * the reply mechanism. 
 *
 * OAMP can also handle DMRs, LMRs, SLRs  after loss/delay_add 
 * in oamp-v1 or endpoint_create() with the appropriate 
 * opcode_flag in oamp-V2 has been called. 
 *
...
int oam_set_opcode_destination_to_oamp(int unit,int is_ingress ,int acc_profile_id, int opcode, bcm_oam_endpoint_type_t type) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(unit, acc_profile_id,
    ...
    if (type == bcmOAMEndpointTypeBHHMPLS) {
        ...
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPerformanceY1731MplsTpAcceleratedResponse, &trap_code), "");
        }
        else if (device_type_v2 && (opcode == bcmOamOpcodeSLR))
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamSlrEthAccelerated, &trap_code), "");
        }
        else if (opcode == bcmOamOpcodeLMM || opcode == bcmOamOpcodeDMM ||
                (!device_type_v2 && (opcode == bcmOamOpcodeLMR || opcode == bcmOamOpcode1DM || opcode == bcmOamOpcodeDMR)))
        {
        ...
        if (device_type_v2 && (opcode == bcmOamOpcodeLMR || opcode == bcmOamOpcode1DM || opcode == bcmOamOpcodeDMR))
        {
            if (is_pcp_counting_enabled)
            {
                BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPerformanceEthAcceleratedResponsePriority, &trap_code), "");
            }
            else
            {
                BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPerformanceEthAcceleratedResponse, &trap_code), "");
            }
        }
        else if (device_type_v2 && (opcode == bcmOamOpcodeSLM))
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamSlmEthAccelerated, &trap_code), "");
        }
        else if (device_type_v2 && (opcode == bcmOamOpcodeSLR))
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamSlrEthAccelerated, &trap_code), "");
                ...
                (!device_type_v2 && (opcode == bcmOamOpcode1DM || opcode == bcmOamOpcodeLMR || opcode == bcmOamOpcodeDMR)))
        {
            /** Set trap for Performance PDUs */ 
            BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamPerformanceEthAccelerated, &trap_code), "");
        }
        ...
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapEgTxOamUpMEPDest2, &trap_code), "");
        }
        else
        {
            if(device_type_v2 && (opcode == bcmOamOpcodeLMR || opcode == bcmOamOpcode1DM || opcode == bcmOamOpcodeDMR || opcode == bcmOamOpcodeSLR))
            {
                if(is_pcp_counting_enabled)
                {
                    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapEgTxOamUpMEPOampPerformenceResponsePriority, &trap_code), "");
                }
                else
                {
                    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapEgTxOamUpMEPOampPerformenceResponse, &trap_code), "");
                }
            }
            else
            {
                BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapEgTxOamUpMEPOamp, &trap_code), "");
            }
        }
        ...
        if (opcode == bcmOamMplsLmDmOpcodeTypeLm)
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapOamBfdPwe, &trap_code), "");
        }
    ...
    if (type == bcmOAMEndpointTypeEthernet) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_action_set(unit, 0, enc_profile_id, &action_key, &action_result), "");
    }
    ...
    bcm_oam_profile_t profile_id)
{
    ...
    bcm_oam_action_result_t action_result;
    bcm_rx_trap_config_t trap_config;
    int ing_trap_code, egr_trap_code, trap_code;
    int trap_strength = 15;
    int snoop_strength = 0;
    int flags = 0;
    int is_oamp_v2 = *(dnxc_data_get(unit,  "oam", "oamp", "oamp_v2", NULL));
    bcm_oam_action_key_t_init(&action_key);
    bcm_oam_action_result_t_init(&action_result);
    bcm_rx_trap_config_t_init(&trap_config);

    /** 
     * For egress side trapping, in OAMPv2 the igress trap has to be configured with encap_id.
     * There are already configured encap ids for all trap codes in  the application step.
     * In order to get the existing encap id.
     *  Firstly access the configured ingress trap code for bcmRxTrapEgTxOamUpMEPDest1 trap type.
     *  Then extract the encap id from the ingress trap code configuration.
     */
    if (!is_ingress && is_oamp_v2)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, flags, bcmRxTrapEgTxOamUpMEPDest1, &egr_trap_code), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_get(unit, egr_trap_code, &trap_config), "");
        trap_code = BCM_GPORT_TRAP_GET_ID(trap_config.cpu_trap_gport);
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_get(unit, trap_code, &trap_config), "");
    }
    ...
    if (dest_port != BCM_GPORT_INVALID && !BCM_GPORT_IS_TRAP(dest_port))
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(dnx_oam__switch_contol_indexed_port_set(unit, dest_port, BCM_SWITCH_PORT_HEADER_TYPE_CPU, 2),
            "Setting port of type CPU failed");
    ...
    int unit)
{
    ...
    */
    BCM_IF_ERROR_RETURN_MSG(bcm_port_internal_get(unit, BCM_PORT_INTERNAL_OAMP, 2, oamp_gport, &count), "1");

    /** Define User defined traps(on ingress side) in order not to add system headers on 2'nd pass */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit,flags,bcmRxTrapOamPerformanceEthAccelerated,oamp_eth_performance_trap_id), "2");

    bcm_rx_trap_config_t_init(&trap_config);
    trap_config.dest_port = oamp_gport[0];
    trap_config.flags = BCM_RX_TRAP_UPDATE_DEST;
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_create(unit, flags, bcmRxTrapUserDefine, &up_mep_ingress_trap_id), "3");

    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_set(unit, up_mep_ingress_trap_id, trap_config), "4");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_create(unit, flags, bcmRxTrapEgTxOamUpMEPDest2, &up_mep_ingress_trap_id2), "5");

    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_set(unit, up_mep_ingress_trap_id2, trap_config), "6");

    bcm_field_context_info_t_init(&context_info);
    dest_char = &(context_info.name[0]);
    sal_strncpy_s(dest_char, "OAM_No_Header", sizeof(context_info.name));
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_create(unit, 0, bcmFieldStageIngressPMF1, &context_info, &context_id), "7");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_presel_set(unit, 0, &presel_entry_id, &presel_data), "6");
    printf("Done\n");
    return BCM_E_NONE;
}
