    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_param_set(unit, 0, bcmFieldStageIngressPMF2, context_id, &context_param), "oam_fg_id");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &oam_fg_id), "oam_fg_id");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, oam_fg_id, context_id, &attach_info), "ipmf1");
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_dnx_field_header_profile_update_entry_add(unit, oam_fg_id, &entry_handle), "");
    ...
    int out_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(port_to_system_port(tx_unit, in_port, &inSP), "");

    BCM_IF_ERROR_RETURN_MSG(port_to_system_port(rx_unit, out_port, &outSP), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(tx_unit, acOuterVlan_1, in_port, 0), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(tx_unit, acOuterVlan_2, out_port, 0), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(tx_unit, in_port, bcmPortClassId, in_port), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(tx_unit, out_port, bcmPortClassId, out_port), "");

    if (!is_device_or_above(tx_unit, JERICHO2)) {
        BCM_IF_ERROR_RETURN_MSG(bcm_port_tpid_delete_all(tx_unit, in_port), "");
    }

    if (!is_device_or_above(rx_unit, JERICHO2)) {
        BCM_IF_ERROR_RETURN_MSG(bcm_port_tpid_delete_all(rx_unit, out_port), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_port_tpid_set(rx_unit, in_port, 0x8100), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_port_tpid_set(rx_unit, out_port, 0x8100), "");
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(rx_unit, acOuterVlan_1, in_port, 0), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(rx_unit, acOuterVlan_2, out_port, 0), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(rx_unit, in_port, bcmPortClassId, in_port), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(rx_unit, out_port, bcmPortClassId, out_port), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_create_with_id(tx_unit, vsi_id), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_create_with_id(rx_unit, vsi_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(tx_unit, &vlan_port_on_tx), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(tx_unit, vsi_id, vlan_port_on_tx.vlan_port_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(rx_unit, &vlan_port_on_rx), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(rx_unit, vsi_id, vlan_port_on_rx.vlan_port_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(tx_unit, &l2addr), "");
    ...
    int out_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_dnx_oam_upmep_on_2nd_device_iop_init(tx_unit, rx_unit, in_port, out_port), "");

    if (is_device_or_above(rx_unit, JERICHO2)) {
        BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(rx_unit, ingress_profile, bcmOAMProfileIngressLIF, &flags2, &ingress_profile), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(rx_unit, egress_profile, bcmOAMProfileEgressLIF, &flags2, &egress_profile), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(rx_unit, acc_ingress_profile, bcmOAMProfileIngressAcceleratedEndpoint, &flags2, &acc_ingress_profile), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_id_get_by_type(rx_unit, acc_egress_profile, bcmOAMProfileEgressAcceleratedEndpoint, &flags2, &acc_egress_profile), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_oam_lif_profile_set(rx_unit, 0, outlif_on_rx, ingress_profile, egress_profile), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(rx_unit, 0, bcmRxTrapEgTxOamUpMEPOamp, &oamp_up_mep_trap_id), "");
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_action_get(rx_unit, 0, acc_egress_profile, &action_key, &action_result_get), "");

        action_result = action_result_get;
        action_result.destination = oamp_dest_gport;

        BCM_IF_ERROR_RETURN_MSG(bcm_oam_profile_action_set(rx_unit, 0, acc_egress_profile, &action_key, &action_result), "");

        BCM_IF_ERROR_RETURN_MSG(oam__my_cfm_mac_set(rx_unit, out_port, dmac), "");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_group_create(rx_unit, &group_info_short_ma), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_endpoint_create(rx_unit, &mep_acc_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_oam_endpoint_create(rx_unit, &rmep_info), "");
