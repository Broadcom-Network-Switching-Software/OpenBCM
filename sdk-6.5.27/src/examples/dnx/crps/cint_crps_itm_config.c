    ...
    int *engine_id_arr)
{
    ...
    int enable_flags = 0;
    bcm_stat_counter_interface_type_t interface_src = bcmStatCounterInterfaceIngressVoq;
    ...
    /** configure the structure for API bcm_stat_counter_database_eviction_set */
    ...
    /** step 3: configure the counter eviction per database */
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_database_eviction_set(unit, eviction_flags, &database, &eviction), "");
    /** configure interface and data mapping structure  */
    cnt_interface.source = interface_src;;
    ...
    {
        /* select expansion if nessesery*/
        if(*dnxc_data_1d_get(unit, "crps", "expansion", "source_expansion_info", "expansion_size", interface_src) >
                   *dnxc_data_get(unit, "crps", "expansion", "expansion_size", NULL))
        {
           interface_key.core_id = core;
           interface_key.command_id = cnt_interface.command_id;
           interface_key.interface_source = cnt_interface.source;

           expansion_select.nof_elements = 2;
           expansion_select.expansion_elements[0].type = bcmStatExpansionTypeDropPrecedenceMeterResolved;
           expansion_select.expansion_elements[1].type = bcmStatExpansionTypeDispositionIsDrop;
    ...
    int *engine_id_arr)
{
    ...
    /** get the eviction structure of the database */
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_database_eviction_get(unit, 0, &database, &eviction), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_interface_get(unit, 0, &database, &cnt_interface), "");
    ...
    int *engine_id_arr)
{
    ...
    for (i = 0; i < nof_engines; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_engine_detach(unit, 0, &database, engine_id_arr[i]), "");
    }
    ...
    crps_itm_config_database_t create_db)
{
    ...
    for(i = 0; i < nof_engines; i++)
    {
        engine_size = crps_engine_nof_counters_get(unit, engine_id[i], format_type);
        end_object_stat_id += engine_size;
    }
    /* just for the test, we want that end will be in the middle of the last engine */
    engine_size = crps_engine_nof_counters_get(unit, engine_id[i-1], format_type);
    end_object_stat_id = end_object_stat_id - (engine_size/2);
    /** using counter_set_size=4, need to divide the range in this number */
    end_object_stat_id = (end_object_stat_id - 1)/4;
    if (create_db == CRPS_ITM_NORMAL_DATABASE )
    {
        BCM_IF_ERROR_RETURN_MSG(crps_itm_voq_data_base_create(unit, core_id, database_id, format_type, start_object_stat_id, end_object_stat_id, nof_engines, engine_id), "");
    }
    else if (create_db == CRPS_ITM_ADD_NEW_RANGE)
    {
        /** add another range of 128 stat_ids, in new engine to the database */
        /** add engine id 21 */
        engine_size = crps_engine_nof_counters_get(unit, engine_id_add[0], format_type);
        BCM_IF_ERROR_RETURN_MSG(crps_itm_voq_data_base_range_add(unit, core_id, database_id, engine_id[nof_engines-1], (engine_size/4), 1, engine_id_add), "");
    }
    else /** create_db = CRPS_ITM_REMOVE_RANGE */
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(crps_itm_crps_data_base_range_remove(unit, core_id, database_id, 8*1024, 3, engine_id_remove), "");
    }
    ...
    int command_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_stat_info_get(unit, 0, &info_key, &config), "");
       
    if(config.enable_mapping == TRUE)
    {                   
        map_key.gport = gport;
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_stat_obj_map_get(unit, 0, &map_key, &object_stat_id), "");
    }        
    else
    {
