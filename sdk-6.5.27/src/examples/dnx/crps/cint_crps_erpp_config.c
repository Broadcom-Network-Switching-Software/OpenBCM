...
int ERPP_DATA_MAPPING_ARRAY_MAX = 16;        /* defines the max number of entries for data mapping table for all examples in
                                         * this file */
bcm_stat_counter_format_type_t COUNTER_FORMAT_TYPE = bcmStatCounterFormatDoublePackets;
    ...
    int expansion_type_flag)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(crps_erpp_expansion_builder(unit, expansion_type_flag, &expansion_select, &ctr_set_map, &counter_set_size), "");
   ...
   /** step 3: configure and create database - use user selected
    ...
    cnt_interface.format_type = COUNTER_FORMAT_TYPE;
    ...
    for (type_id = 0; type_id < nof_types; type_id++)
    {
        ...
        if (COUNTER_FORMAT_TYPE == bcmStatCounterFormatPackets)
        {
            /* Don't double the total number of entries */
            cnt_interface.type_config[type_id].end = end_object_stat_id[type_id] / 2;
        }
    }
    ...
    crps_erpp_config_database_t create_db)
{
    ...
    for (index = 0; index < 3; index++)
    {
        ...
        type_offset[0] = 0;
        BCM_IF_ERROR_RETURN_MSG(crps_erpp_data_base_create(unit, core_id, database_id, command_id, nof_types,
    ...
    int expansion_type_flag)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(crps_erpp_data_base_create(unit, core, database_id, command_id, nof_types, type_start_arr, type_end_arr, type_valid_arr, type_offset_arr, 1,   /* nof_engines 
                                                                                                                                                         */
