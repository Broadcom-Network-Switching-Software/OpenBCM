    ...
    int expansion_type_flag)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(crps_irpp_expansion_builder(unit, expansion_type_flag, &expansion_select, &ctr_set_map, &counter_set_size), "");
    ...
    crps_irpp_config_database_t create_db)
{
    ...
    for (index = 0; index < 4; index++)
    {
        ...
        /** configure type 0 */
        /** each type range holds 1 engine full entries, except type=2,3 which split one engine together */
        ...
        end_object_stat_id_arr[2] = (((engine_size) / counter_set_size[index]) / 2) - 1;
        type_offset[2] = (end_object_stat_id_arr[1] - start_object_stat_id_arr[1] + 1) + type_offset[1];
        /** configure type 3 */
        end_object_stat_id_arr[3] = (((engine_size) / counter_set_size[index]) / 2) - 1;
        type_offset[3] = (end_object_stat_id_arr[2] - start_object_stat_id_arr[2] + 1) + type_offset[2];

        BCM_IF_ERROR_RETURN_MSG(crps_irpp_data_base_create(unit, core_id, database_id, command_id, nof_types,
                                        start_object_stat_id_arr, end_object_stat_id_arr, type_valid, type_offset,
                                        nof_engines, engine_arr, format_type, expansion_flag[index]), "");
    ...
    int expansion_type_flag)
{
    ...
    bcm_stat_counter_format_type_t format_type = bcmStatCounterFormatPacketsAndBytes;
    ...
    BCM_IF_ERROR_RETURN_MSG(crps_irpp_data_base_create(unit, core, database_id, command_id, nof_types, type_start_arr, type_end_arr, type_valid_arr, type_offset_arr, 1,   /* nof_engines 
                                                                                                                                                         */
                                    &engine_id, format_type, expansion_type_flag), "");
