    ...
    int engine_id)
{
    ...
    uint32 engine_size = crps_engine_nof_counters_get(unit, engine_id, counter_format);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_engine_attach(unit, 0, &database, engine_id), "");
    ...
    if(*dnxc_data_1d_get(unit, "crps", "expansion", "source_expansion_info", "expansion_size", bcmStatCounterInterfaceEgressReceiveQueue) >
            *dnxc_data_get(unit, "crps", "expansion", "expansion_size", NULL))
    {
        ...
        expansion_select.expansion_elements[0].type = bcmStatExpansionTypeTrafficClass;
    ...
    int core_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(crps_etm_data_base_create(unit, core_id, database_id,
