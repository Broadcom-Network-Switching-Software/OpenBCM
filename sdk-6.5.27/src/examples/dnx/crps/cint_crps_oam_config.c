    ...
    uint32 *actual_nof_counters)
{
    ...
    * command id will be verified inside the API */
    ...
    for (i = 0; i < nof_engines; i++)
    {
        ...
        if ((*dnxc_data_1d_get(unit, "crps", "src_interface", "crps_eviction", "valid_for_eviction", source)))
        {
        ...
        engine_size = crps_engine_nof_counters_get(unit, engine_id_array[i], format_type);
    ...
    *actual_nof_counters = total_nof_counters;
    ...
    interface.format_type = format_type; /** double entry*/
    ...
    uint32 *actual_nof_counters)
{
    ...
    for (i = 0; i < nof_new_engines_to_add; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_engine_attach(unit, flags, database, new_engines_id_to_add_array[i]), "");
        /** add engine size to the total number of counters in the
         *  database to get the database size */
        engine_size = crps_engine_nof_counters_get(unit, new_engines_id_to_add_array[i], bcmStatCounterFormatDoublePackets);
        additonal_range_size = additonal_range_size + engine_size;
    }

    /** step 2: get the database configuration of interface */
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_interface_get(unit, flags, &database, &interface), "");

    /** hold the new end point of the data base range */
    *actual_nof_counters = (interface.type_config[type_id].end - interface.type_config[type_id].start + 1 + additonal_range_size);

    /** step 3: extend the range of the last engine of the current
     *  data base and connect it to new engine if needed */
    interface.type_config[0].end = *actual_nof_counters + interface.type_config[type_id].start - 1;
    ...
    uint32 *actual_nof_counters)
{
    ...
    for (i = 0; i < nof_new_engines_to_remove; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_engine_detach(unit, flags, &database, engines_id_to_remove_array[i]), "");
        /** add engine size to the total number of counters in the
         *  database to get the database size */
        engine_size = crps_engine_nof_counters_get(unit, engines_id_to_remove_array[i], bcmStatCounterFormatDoublePackets);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_interface_get(unit, flags, &database, &interface), "");
    /** hold the new end point of the data base range */
    *actual_nof_counters = (interface.type_config[type_id].end - interface.type_config[type_id].start + 1 - remove_range_size);

    /** reduce the range of the last engine of the
     *  current data base  */
    interface.type_config[type_id].end = *actual_nof_counters + interface.type_config[type_id].start - 1;
    /** this flag will allow the user to extend the range while the engine is enabled */
    flags = BCM_STAT_COUNTER_MODIFY_RANGE_ONLY;
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_interface_set(unit, flags, &database, &interface), "");
    ...
    int database_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_database_enable_set(unit, flags, &database, &disable_config), "");
    /** Destroy the database */
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_database_destroy(unit, flags, &database), "");
    ...
    int core_id)
{
    ...
    int ingress_oam_engines[3], ingress_oam_engines_add[2];
    int egress_oam_engines[2], egress_oam_engines_add[2];
    int oamp_engines[3] = {2,9,19}, oamp_engines_add[2] = {12,13};
    ...
    remove_engine[0] = ingress_oam_engines_add[1];
    ...
    remove_engine[1] = egress_oam_engines_add[1];
    ...
    remove_engine[2] = oamp_engines_add[1];
 
    for (i = 0; i < 3; i++)
    {
        /** Skip unsupported interface source */
        if((*dnxc_data_1d_get(unit, "crps", "src_interface", "command_id", "size", oam_source[i])) <= 0)
        {
            continue;
        }
        ...
        BCM_IF_ERROR_RETURN_MSG(crps_oam_database_set(unit, core_id, oam_source[i], command_id[i], nof_engines[i], &engine_array[i][0], counter_base[i], database_id[i], &total_nof_counters_get), "");
        /** calculate the size of the engines in double entry; done
         *  only for comparison */
        for (j = 0; j < nof_engines[i]; j++)
        {
             engine_size = crps_engine_nof_counters_get(unit, engine_array[i][j], bcmStatCounterFormatDoublePackets);
        ...
        BCM_IF_ERROR_RETURN_MSG(crps_oam_database_engines_add(unit, core_id, database_id[i], nof_new_engines, &new_engines[i][0], &total_nof_counters_get), "");
        /** calculate the size of the engines in double entry; done
         *  only for comparison */
        for (j = 0; j < nof_new_engines; j++)
        {
            engine_size = crps_engine_nof_counters_get(unit, new_engines[i][j], bcmStatCounterFormatDoublePackets);
             total_nof_counters_to_compare = total_nof_counters_to_compare + engine_size; /** since in double entry mode*/
        }
        if (total_nof_counters_get != total_nof_counters_to_compare)
        {
        ...
        BCM_IF_ERROR_RETURN_MSG(crps_oam_database_engines_remove(unit, core_id, database_id[i], nof_engines_to_remove, remove_engine[i], &total_nof_counters_get), "");
        engine_size = engine_size = crps_engine_nof_counters_get(unit, remove_engine[i], bcmStatCounterFormatDoublePackets);
        total_nof_counters_to_compare = total_nof_counters_to_compare - engine_size;
        if (total_nof_counters_get != total_nof_counters_to_compare)
        {
        ...
        BCM_IF_ERROR_RETURN_MSG(crps_oam_database_destroy(unit, core_id, database_id[i]), "");
    }
