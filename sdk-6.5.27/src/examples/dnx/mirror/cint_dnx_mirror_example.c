 ...
 * Test Scenario - start
  ...
  cint ../../../../src/examples/dnx/mirror/cint_dnx_mirror_example.c
...
struct erspan_tunnel_info_t
{
    ...
    bcm_field_array_n_position_t fem_position;
...
/** gre_with_sn counter_command_id   counter_engine_id database_id   fem_position */
    0,          0,                   0,                0,            {3, 4}
};
...
int mirror_span_example(int unit, int in_port, int out_port, int mirror_port) {
    ...
    snprintf(error_msg, sizeof(error_msg), "(vlan=%d)", mirror_info.vlan);
    if (!BCM_GPORT_IS_TRUNK(in_port) && !BCM_GPORT_IS_TRUNK(out_port)) {
        BCM_PBMP_PORT_SET(p, in_port);
        BCM_PBMP_PORT_ADD(p, out_port);
        BCM_PBMP_CLEAR(u);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, mirror_info.vlan, p, u), error_msg);
    } else {
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, mirror_info.vlan, in_port, 0), error_msg);

        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, mirror_info.vlan, out_port, 0), error_msg);
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_dest_add(unit, (G_mirror_type & BCM_MIRROR_PORT_INGRESS) ? in_port : out_port, G_mirror_type, dest.mirror_dest_id), "");
...
int mirror_span_example_clean_up(int unit) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_dest_delete(unit, 
            (G_mirror_type & BCM_MIRROR_PORT_INGRESS) ? mirror_info.in_port : mirror_info.out_port, 
            G_mirror_type, mirror_info.mirror_dest_id), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_destination_destroy(unit, mirror_info.mirror_dest_id), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_delete(unit, mirror_info.da, mirror_info.vlan), "");
    ...
    if (!BCM_GPORT_IS_TRUNK(mirror_info.in_port) && !BCM_GPORT_IS_TRUNK(mirror_info.out_port)) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_remove(unit, mirror_info.vlan, p), error_msg);
    } else {
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_delete(unit, mirror_info.vlan, mirror_info.in_port), error_msg);

        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_delete(unit, mirror_info.vlan, mirror_info.out_port), error_msg);
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_destroy(unit, mirror_info.vlan), "");
...
int mirror_rspan_example(int unit, int in_port, int in_vlan, int out_port, int mirror_port, int rspan_tag) {
    ...
    */
    ...
    if (*(dnxc_data_get(unit, "vlan", "llvp", "eve_llvp_from_fhei_ext", NULL)))
    {
        mirror_info.tag_format_class_id = 0;
    }
    else
    {
        /*
        ...
        */
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_port_tpid_class_get(unit, &tpid_class), "");
    ...
    */
    ...
    if (mirror_info.udh_enable)
    {
        for (ii=0; ii<(sizeof(mirror_info.udh_size)/sizeof(mirror_info.udh_size[0])); ++ii)
        {
            ...
            mirror_header_info.udh[ii].data_ext = mirror_info.udh_data_ext[ii];
        }
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_header_info_set(unit, 0, dest.mirror_dest_id, &mirror_header_info), "");

    /* set in-bound mirror on <in_port, in_vlan> to created mirror destination */
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_vlan_dest_add(unit, (G_mirror_type & BCM_MIRROR_PORT_INGRESS) ? in_port : out_port, in_vlan, G_mirror_type, dest.mirror_dest_id),
        "Failed to add inbound in_port+vlan1 to be mirrored to mirr_dest2");
...
int mirror_rspan_example_clean_up(int unit) {
    ...
    /* set in-bound mirror on <in_port, in_vlan> to created mirror destination */
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_vlan_dest_delete(unit, 
            (G_mirror_type & BCM_MIRROR_PORT_INGRESS) ? mirror_info.in_port : mirror_info.out_port, 
    ...
    */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_id_destroy(unit, BCM_VLAN_ACTION_SET_EGRESS, mirror_info.eve_action_id), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, mirror_info.vlan_port_id), "");
 ...
 */
int get_mirror_incoming_tag_structure(int unit, uint32 llvp_profile, bcm_port_tag_format_class_t *val){
    uint32 entry_handle_id;
    uint32 key_field;
    int inst_id = -1;

    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_handle_take(unit, "EGRESS_LLVP_CLASSIFICATION", &entry_handle_id), "");
    /*
    * For mirrored packets, IVE ethernet tag format is  0.
    */
    key_field = 0;
    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_key_field_arr32_set(unit, entry_handle_id, "INNER_TPID", &key_field), "");
    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_key_field_arr32_set(unit, entry_handle_id, "IS_OUTER_PRIO", &key_field), "");
    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_key_field_arr32_set(unit, entry_handle_id, "OUTER_TPID", &key_field), "");
    key_field = 0;
    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_key_field_arr32_set(unit, entry_handle_id, "EGRESS_LLVP_PROFILE", &key_field), "");
    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_get(unit, entry_handle_id), "");
    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, "INCOMING_TAG_STRUCTURE", inst_id, &get_val[0]), "");

    /* Get the SW-ID for this HW-ID */
    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_handle_take(unit, "TAG_STRUCT_INGRESS_HW_ID_TO_SW_ID", &entry_handle_id), "");
    key_field = get_val[0][0];
    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_key_field_arr32_set(unit, entry_handle_id, "INCOMING_TAG_STRUCTURE", &key_field), "");
    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_get(unit, entry_handle_id), "");
    BCM_IF_ERROR_RETURN_MSG(dnx_dbal_entry_handle_value_field_arr32_get(unit, entry_handle_id, "BCM_TAG_FORMAT_CLASS_ID", inst_id, &get_val[0]), "");
...
int mirror_rspan_advanced_example(int unit, int in_port, int in_vlan, int out_port, int mirror_port, int rspan_tag) {
    ...
    */
    ...
    else
    {
        ...
        */
        int llvp_profile = 0;
        BCM_IF_ERROR_RETURN_MSG(get_mirror_incoming_tag_structure(unit, llvp_profile, &(mirror_info.tag_format_class_id)), "");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_create(unit, &rspan_tunnel_intf, &rspan_tunnel), "");

    erspan_info.rspan_intf = rspan_tunnel_intf;    

    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_get(unit, &rspan_tunnel_intf, &rspan_tunnel_get),
        "get RSPAN Advanced tunnel initiator");
...
int mirror_rspan_advanced_example_clean_up(int unit) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_clear(unit, &erspan_info.rspan_intf), "");
    ...
    int database_id)
{
    ...
    int cores[max_nof_cores];
    int nof_cores = 0;
    int core_index;
    for(core_index = 0; core_index < max_nof_cores; core_index++) {
        if ( valid_cores_bmp & (1 << (core_index))){
            cores[nof_cores]= core_index;
            nof_cores ++;
        }
    }
    for (core_id=0; core_id<nof_cores; ++core_id)
    {
        engine_id = counter_engine_id + cores[core_id];
        snprintf(error_msg, sizeof(error_msg), "core_id %d", core_id);
        BCM_IF_ERROR_RETURN_MSG(crps_oam_database_set(unit, core_id, bcmStatCounterInterfaceEgressOam, counter_command_id, 1, &engine_id, 0, database_id+core_id, &actual_nof_counters), error_msg);
    }
...
int mirror_erspan_tunnel_create(int unit, bcm_if_t *encap_id) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_create(unit, &ip_tunnel_intf, &ip_tunnel), "");
    ...
    */
    ...
    if (erspan_info.gre_with_sn)
    {
        ...
        if (*(dnxc_data_get(unit, "vlan", "llvp", "eve_llvp_from_fhei_ext", NULL)))
        {
            ...
            erspan_tunnel.counter_command_id = 1;
        }
        else
        {
    ...
    if (erspan_info.is_erspan_v3) {
        ...
        /*
        * In JR2 mode, EEDB_ERSPAN.RAW_DATA contains ERSPANv2.index[19:17] bits.
        * In JR1 mode, EEDB_ERSPAN.RAW_DATA contains ERSPANv2.session_id bits.
        *
        * In JR2 mode, ERSPANv2.session_id is stamped through Sniff qualifier.
        */
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_create(unit, &erspan_tunnel_intf, &erspan_tunnel), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_get(unit, &erspan_tunnel_intf, &erspan_tunnel_get),
        "get ERSPAN tunnel initiator");
...
int mirror_erspan_destination_create(int unit, int in_port, bcm_vlan_t in_vlan, int out_port, int mirror_port, int is_mc, int mc_id, bcm_if_t encap_id) {
    ...
    if (is_mc == TRUE) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID | BCM_MULTICAST_EGRESS_GROUP, mc_id), "");
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, mc_id, BCM_MULTICAST_EGRESS_GROUP, 1, &replications), "");
        ...
        else
        {
            ...
            */
            BCM_IF_ERROR_RETURN_MSG(bcm_port_internal_get(unit, BCM_PORT_INTERNAL_EGRESS_REPLICATION, 1, &erp_port, &count_erp), "");

            BCM_IF_ERROR_RETURN_MSG(bcm_stk_gport_sysport_get(unit, erp_port, &dest.gport), "");
        }
    }
    else {
        dest.gport = mirror_port;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_header_info_set(unit, mirror_info.flags, dest.mirror_dest_id, &mirror_header_info), "");

    bcm_mirror_header_info_t_init(&mirror_header_info_get);
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_header_info_get(unit, dest.mirror_dest_id, &flags, &mirror_header_info_get), "");
    ...
    if (G_mirror_type == BCM_MIRROR_PORT_INGRESS) {
        BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_vlan_dest_add(unit, in_port, mirror_info.vlan, G_mirror_type, dest.mirror_dest_id),
            "Failed to add inbound in_port+vlan1 to be mirrored to mirr_dest2");
    }
    else {
        /* out_port + in_vlan, for bridged packets */
        BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_vlan_dest_add(unit, out_port, mirror_info.vlan, G_mirror_type, dest.mirror_dest_id),
            "Failed to add inbound in_port+vlan1 to be mirrored to mirr_dest2");

        /* out_port + out_vlan, for routed packets */
        BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_vlan_dest_add(unit, out_port, mirror_info.out_vlan, G_mirror_type, dest.mirror_dest_id),
            "Failed to add inbound in_port+vlan1 to be mirrored to mirr_dest2");
    }
...
int mirror_erspan_example(int unit, int in_port, bcm_vlan_t in_vlan, int out_port, int mirror_port){
    ...
    BCM_PBMP_PORT_ADD(p, out_port);
    BCM_PBMP_CLEAR(u);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, in_vlan, p, u), "");
    ...
    if (erspan_info.gre_with_sn)
    {
        BCM_IF_ERROR_RETURN_MSG(erspan_init_counter_engine(unit, 0, 0, 0), "");
    }

    /* build IP tunnels, and get back interfaces */
    BCM_IF_ERROR_RETURN_MSG(mirror_erspan_tunnel_create(unit, &encap_id), "");

    /* In JR2 mode, setup field group to update mirror qualifier */
    if (dnx_utils_is_jr2_system_header_mode(unit)) {
        BCM_IF_ERROR_RETURN_MSG(mirror_erspan_update_mirror_qualifier(unit, erspan_info.session_id), "");
    }

    /* add inbound mirror to send out of the create ip tunnel with span encapsulation */
    snprintf(error_msg, sizeof(error_msg), "in_port=%d", in_port);
    BCM_IF_ERROR_RETURN_MSG(mirror_erspan_destination_create(unit, in_port, in_vlan, out_port, mirror_port, 0, 0, encap_id), error_msg);
...
int mirror_erspan_example_v2(int unit, int in_port, bcm_vlan_t in_vlan, int out_port, int mirror_port){
    ...
    BCM_IF_ERROR_RETURN_MSG(mirror_erspan_example(unit, in_port, in_vlan, out_port, mirror_port), "");
...
int mirror_erspan_multicast_example(int unit, int in_port, int in_vlan, int out_port, int mirror_port, int mc_id){
    ...
    /* add inbound mirror to send out of the create ip tunnel with span encapsulation */
    snprintf(error_msg, sizeof(error_msg), "in_port=%d", in_port);
    BCM_IF_ERROR_RETURN_MSG(mirror_erspan_destination_create(unit, in_port, in_vlan, out_port, mirror_port, 1, mc_id, encap_id), error_msg);

    return BCM_E_NONE;
}

/*
 * Creates field group to update mirror_qualifier in the mirror action,
 * which is stamped to ERSPAN header as the session ID(8 bits). 
 * - mirror_qualifier : used to update the mirror qualifier
 */
int
mirror_erspan_update_mirror_qualifier(
    int unit,
    int mirror_qualifier)
{
    bcm_field_group_info_t fg_info;
    bcm_field_group_attach_info_t attach_info;
    void *dest_char;
    char error_msg[200]={0,};
    int conditions_index, mapping_bits_index;
    ...
    sal_strncpy(dest_char, "user_action_10", sizeof(action_info.name));
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_create(unit, 0, &action_info, &erspan_info.action_id), "");
    ...
    fg_info.nof_quals = 1;
    fg_info.qual_types[0] = bcmFieldQualifyMirrorCode;

    fg_info.nof_actions = 1;
    fg_info.action_types[0] = erspan_info.action_id;
    fg_info.action_with_valid_bit[0] = 0;

    dest_char = &(fg_info.name[0]);
    sal_strncpy_s(dest_char, "mirror_qualifier", sizeof(fg_info.name));

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &erspan_info.fg_id), "");

    bcm_field_fem_action_info_t_init(&fem_action_info);
    fem_action_info.fem_input.input_offset = 0;

    /**
     *  This selects mirror_code[3:0] as the condition bits.
     */
    fem_action_info.condition_msb = 3;

    /**
     *  The following code sets FEM action valid when mirror_code[3:0] is one of 1~15
     */
    fem_action_info.fem_condition[0].extraction_id = 0;
    fem_action_info.fem_condition[0].is_action_valid = 0;

    for (conditions_index = 1; conditions_index < 16; conditions_index++)
    {
        fem_action_info.fem_condition[conditions_index].extraction_id = 0;
        fem_action_info.fem_condition[conditions_index].is_action_valid = 1;
    }

    fem_action_info.fem_extraction[0].action_type = bcmFieldActionMirrorIngressRaw;

    /**
     *  If mirror_qualifier is 0 when comes to the PMF stage,
     *  we can also use increment here to set the mirror_qualifier.
     */
    fem_action_info.fem_extraction[0].increment = 0;

    /**
     *  MirrorIngressRaw = {mirror_qualifier[7:0], mirror_code[8:0]}
     */        
    for (mapping_bits_index = 0; mapping_bits_index < 9; mapping_bits_index++)
    {
        fem_action_info.fem_extraction[0].output_bit[mapping_bits_index].source_type = bcmFieldFemExtractionOutputSourceTypeKey;
        fem_action_info.fem_extraction[0].output_bit[mapping_bits_index].offset = mapping_bits_index;
    }

    for (mapping_bits_index = 9; mapping_bits_index < 17; mapping_bits_index++)
    {
        fem_action_info.fem_extraction[0].output_bit[mapping_bits_index].source_type = bcmFieldFemExtractionOutputSourceTypeForce;
        fem_action_info.fem_extraction[0].output_bit[mapping_bits_index].forced_value = (mirror_qualifier >> (mapping_bits_index-9)) & 0x1;
    }

    array_id = erspan_info.fem_position.array_id;
    fem_position_in_array = erspan_info.fem_position.fem_position_in_array;
    action_priority = BCM_FIELD_ACTION_POSITION(array_id,fem_position_in_array);

    snprintf(error_msg, sizeof(error_msg), "for fg_id = %d | array_id = %d | fem_position_in_array = %d",
        erspan_info.fg_id, array_id, fem_position_in_array);
    BCM_IF_ERROR_RETURN_MSG(bcm_field_fem_action_add(unit, 0, erspan_info.fg_id, action_priority, &fem_action_info), error_msg);

    erspan_info.action_priority = action_priority;
    ...
    if ((G_mirror_type == BCM_MIRROR_PORT_EGRESS) &&
        *(dnxc_data_get(unit, "port_pp", "general", "rcy_info_prt_qualifier_eth_over_append64_support", NULL))!= 0)
    {
        /* Outbound mirroring selects RCY_MIRROR_SYS_HDR_CHG context in the 2nd cycle */
        erspan_info.context_id = 1;
    }
    else
    {
        erspan_info.context_id = BCM_FIELD_CONTEXT_ID_DEFAULT;
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, erspan_info.fg_id, erspan_info.context_id, &attach_info), "");
...
int mirror_erspan_example_clean_up(int unit){
    ...
    /* In JR2 mode, clean up field group which updates mirror qualifier */
    if (dnx_utils_is_jr2_system_header_mode(unit)) {
        BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_detach(unit, erspan_info.fg_id, erspan_info.context_id), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_field_fem_action_delete(unit, erspan_info.fg_id, erspan_info.action_priority), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_field_group_delete(unit, erspan_info.fg_id), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_field_action_destroy(unit, erspan_info.action_id), "");
    }
    ...
    if (G_mirror_type & BCM_MIRROR_PORT_EGRESS) {
        /* out_port + out_vlan, for routed packets */
        BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_vlan_dest_delete(unit, mirror_info.out_port, mirror_info.out_vlan, G_mirror_type, mirror_info.mirror_dest_id), "");
    }

    bcm_mirror_header_info_t_init(&mirror_header_info);
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_header_info_set(unit, 0, mirror_info.mirror_dest_id, &mirror_header_info), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_destination_destroy(unit, mirror_info.mirror_dest_id), "");

    if (mirror_info.is_mc) {
        BCM_IF_ERROR_RETURN_MSG(bcm_multicast_destroy(unit, mirror_info.mc_id), "");
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_clear(unit, &erspan_info.erspan_intf), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_clear(unit, &erspan_info.ip_tunnel_intf), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_destroy(unit, erspan_info.next_hop_if), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_remove(unit, mirror_info.vlan, p), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_destroy(unit, mirror_info.vlan), "");
    return BCM_E_NONE;
}
...
int mirror_egress_probabiliy_sample_mirror_config(int unit, int in_port, int out_port, int mirror_port) {
    ...
    BCM_PBMP_PORT_ADD(p, out_port);
    BCM_PBMP_CLEAR(u);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, mirror_prob_sample_info.vlan, p, u), "");
    ...
    /** set in-bound mirror on <in_port, in_vlan> to created mirror destination */
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_dest_add(unit, out_port, BCM_MIRROR_PORT_EGRESS, dest.mirror_dest_id), "");
...
int mirror_egress_probabiliy_sample_compare_config(int unit, uint32 compare_bits)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &pp_port_key_gen_qual_info, &pp_port_key_gen_qual_id),
        "creating PP port key gen UDF");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &egress_tod_qual_info, &egress_tod_qual_id),
        "when creating egress TOD UDF");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_info_get(unit, bcmFieldQualifyPortClassPacketProcessingGeneralData, bcmFieldStageIngressPMF1, &pp_port_key_gen_info_get), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_compare_create(unit, 0, bcmFieldStageIngressPMF1, context_id, 0, &compare_info), "");
...
int mirror_egress_probabiliy_sample_presel_set(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_presel_set(unit, 0, &presel_ent, &presel_info), "");
...
int mirror_egress_probabiliy_sample_fg_config(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &mirror_code_qual_info, &mirror_code_qual_id),
        "when creating mirror code UDF");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_compare_operand_offset_get(unit, 1, bcmFieldCompareOperandFirstKeyNotBigger, &op_offset), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, fg_id, mirror_prob_sample_info.ipmf1_context_id, &attach_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, fg_id, &entry_info, &ent), "");
...
int mirror_egress_probabiliy_sample_example(int unit, int in_port, int out_port, int mirror_port, uint32 prob, uint32 compare_bits) {
    ...
    if (*(dnxc_data_get(unit, "snif", "egress", "probability_sample", NULL)))
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(mirror_span_example(unit, in_port, out_port, mirror_port), "");
        return BCM_E_NONE;
    }
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(mirror_egress_probabiliy_sample_mirror_config(unit, in_port, out_port, mirror_port), "");

    /**
     * Create mirror RCY context, compare egress TOD with PP port key GEN var
     */
    BCM_IF_ERROR_RETURN_MSG(mirror_egress_probabiliy_sample_compare_config(unit, compare_bits), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, rcy_mirror_gport,
                            bcmPortClassFieldIngressPMF1PacketProcessingPortCs,
                            mirror_prob_sample_info.rcy_cs_profile),
                            "in bcmPortClassFieldIngressPMF1PacketProcessingPortCs");

    /**
     * Set PP port key GEN var to prob for compare
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, rcy_mirror_gport, bcmPortClassFieldIngressPMF1PacketProcessingPortGeneralData, prob),
        "in bcmPortClassFieldIngressPMF1PacketProcessingPortGeneralData");

    /**
     * Set preselector for mirror RCY context
     */
    BCM_IF_ERROR_RETURN_MSG(mirror_egress_probabiliy_sample_presel_set(unit), "");

    /**
     * Create probability sampling field group
     */
    BCM_IF_ERROR_RETURN_MSG(mirror_egress_probabiliy_sample_fg_config(unit), "");

    /**
     * Create strip header field group
     */
    BCM_IF_ERROR_RETURN_MSG(mirror_egress_probabiliy_sample_strip_header_config(unit), "");
...
int mirror_egress_probabiliy_sample_clean_up(int unit)
{
    ...
    if (*(dnxc_data_get(unit, "snif", "egress", "probability_sample", NULL)))
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(mirror_span_example_clean_up(unit), "");
        set_outbound_mode(unit, 0);

        return BCM_E_NONE;
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_delete(unit, mirror_prob_sample_info.strip_header_fg_id, entry_qual_info, mirror_prob_sample_info.strip_header_ent), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_detach(unit, mirror_prob_sample_info.strip_header_fg_id, mirror_prob_sample_info.ipmf3_context_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_destroy(unit, bcmFieldStageIngressPMF3, mirror_prob_sample_info.ipmf3_context_id), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_delete(unit, mirror_prob_sample_info.strip_header_fg_id), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_delete(unit, mirror_prob_sample_info.prob_sample_fg_id, entry_qual_info, mirror_prob_sample_info.prob_sample_ent), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_detach(unit, mirror_prob_sample_info.prob_sample_fg_id, mirror_prob_sample_info.ipmf1_context_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_compare_destroy(unit, bcmFieldStageIngressPMF1, mirror_prob_sample_info.ipmf1_context_id, 0), "pair1");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_destroy(unit, bcmFieldStageIngressPMF1, mirror_prob_sample_info.ipmf1_context_id), "eth");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_delete(unit, mirror_prob_sample_info.prob_sample_fg_id), "strip_header");

    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_dest_delete(unit, mirror_prob_sample_info.out_port,
            BCM_MIRROR_PORT_EGRESS, mirror_prob_sample_info.mirror_dest_id), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_destination_destroy(unit, mirror_prob_sample_info.mirror_dest_id), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_delete(unit, mirror_prob_sample_info.da, mirror_prob_sample_info.vlan), "");

    BCM_PBMP_PORT_SET(p, mirror_prob_sample_info.in_port);
    BCM_PBMP_PORT_ADD(p, mirror_prob_sample_info.out_port);

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_remove(unit, mirror_prob_sample_info.vlan, p), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_destroy(unit, mirror_prob_sample_info.vlan), "");
...
int mirror_vxlan_example(int unit, int in_port, int out_port, int mirror_port, int access_port, int provider_port) {
    ...
    BCM_IF_ERROR_RETURN_MSG(vxlan_example(unit, 0, access_port, provider_port, 0), "");
    ...
    bcm_mirror_header_info_t_init(&mirror_header_info);
    mirror_header_info.tm.src_sysport = in_port;
    mirror_header_info.tm.out_vport = cint_vxlan_basic_info.encap_tunnel_id;
