    ...
    bcm_encap_access_t vpls_encap_access)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, 20, in_port, 0), "");
    ...
    snprintf(error_msg, sizeof(error_msg), "AC(0x%08X) to PWE(0x%08X)", vlan_port.vlan_port_id, mpls_util_entity[0].fecs[0].fec_id);
        ...
        int enc4_nof_tunnels)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_deep_stack_extended_entry_inner(unit,in_port,out_port, start_phase, nof_labels_per_stage, enc_labels_base_per_stage, 0), "");

    BCM_IF_ERROR_RETURN_MSG(mpls_deep_stack_vpls_config(unit, in_port, out_port, vpls_encap_access), "");
    ...
    int nof_tunnel)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_deep_stack_ipv6_example(unit,in_port,out_port,nof_tunnel,start_phase), "create_mpls_tunnel");
