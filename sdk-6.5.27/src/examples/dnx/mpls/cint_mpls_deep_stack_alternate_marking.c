 ...
 * In different chip, its value is different.
 */
int CINT_INSTRU_IPT_TAIL_EDIT_PROFILE;

/** egress oam counter is used for ingress node when L_bit_mapping is supported*/
int use_crps_egress_oam_counter = 0;

/*
 *egress counter base value
 */
uint32 cint_am_egress_counter_base = 100;
   ...
   int shim_type)
{
    ...
    if (!(*dnxc_data_get (unit,"instru","ipt","ipt_mapping_v2",NULL)))
    {
        ...
        /** get default */
        BCM_IF_ERROR_RETURN_MSG(bcm_instru_ipt_profile_get(unit, 0, ipt_profile, &default_tail_edit_config),
            "failed for Tail Edit");
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_instru_ipt_profile_set(unit, 0, ipt_profile, &config), "failed for Tail Edit");
    }
   ...
   int unit)
{
    ...
    /** set tail-edit profile */
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_instru_ipt_profile_set(unit, 0, ipt_profile, default_tail_edit_config),
            "failed for default Tail Edit");
    }

    /** set switch ID */
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_control_set(unit, 0, bcmInstruControlIptSwitchId, default_switch_id),
        "failed for setting default switch id");
        ...
        int unit)
{
   ...
   if(!L_bit_mapping)
   {
       ...
       printf("L bit 1: g_cint_field_am_ing_ent.out_lif_0_raw: %d", g_cint_field_am_ing_ent.out_lif_0_raw);

       BCM_IF_ERROR_RETURN_MSG(cint_field_alternate_marking_ingress_node_iPMF1(unit), "");

       BCM_IF_ERROR_RETURN_MSG(cint_field_alternate_marking_ingress_node_entry_add(unit,&entry_handle_L_bit_1), "");
       ...
       printf("L bit 0 : g_cint_field_am_ing_ent.out_lif_0_raw: %d", g_cint_field_am_ing_ent.out_lif_0_raw);

       BCM_IF_ERROR_RETURN_MSG(cint_field_alternate_marking_ingress_node_entry_add(unit,&entry_handle_L_bit_0), "");

       printf("Ingress node iPMF1 configuration Done! \n");
   }

   BCM_IF_ERROR_RETURN_MSG(cint_field_alternate_marking_ingress_node_ePMF(unit), "");
    ...
    uint32 *actual_nof_counters)
{
    ...
    * command id will be verified inside the API */
    if(!use_crps_egress_oam_counter)
    {
        /** configure expansion */
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_expansion_select_set(unit, flags, &interface_key, &expansion_select), "");
    }

    /** create database */
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_database_create(unit, BCM_STAT_DATABASE_CREATE_WITH_ID, core_id, database_id), "");
    ...
    for (i = 0; i < nof_engines; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_engine_attach(unit, flags, &database, engine_id_array[i]), "");
    ...
    *actual_nof_counters = total_nof_counters;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_interface_set(unit, flags, &database, &interface), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_set_mapping_set(unit, flags, &database, &ctr_set_map), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_database_enable_set(unit, flags, &database, &enable_config), "");

    return BCM_E_NONE;
}

/* Add CRPS configuration, is_egress parameter is valid only in case L bit mapping supported*/
int
mpls_deep_stack_alternate_marking_crps(int unit,  int port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(get_core_and_tm_port_from_port(unit, port, &core_id, &tm_port), "");
    cint_mpls_deep_stack_alternate_marking_crps_params.core = core_id;

    /*In case L bit profile mapping supported, the counting does not support both packets and bytes, but only one*/
    if (!L_bit_mapping || (stat_mode_pkt_and_byte == 0))
    {
        if(use_crps_egress_oam_counter)
        {
            intf_type =  bcmStatCounterInterfaceEgressOam;
            /**command must be 1*/
            g_cint_field_am_ing_ent.command_id = 1;
        }
        else
        {
            intf_type= bcmStatCounterInterfaceIngressOam;
        }
        BCM_IF_ERROR_RETURN_MSG(crps_oam_database_set(unit, core_id, intf_type, g_cint_field_am_ing_ent.command_id , 1,
    ...
    if (stat_mode_pkt_and_byte == 1) {
        ...
        {
            intf_type =  bcmStatCounterInterfaceEgressOam;
            g_cint_field_am_ing_ent.byte_command_id1 = 1;
        }
        else
        {
            intf_type = bcmStatCounterInterfaceIngressReceivePp;
        }
        /* configure counter database for byte counting */
        BCM_IF_ERROR_RETURN_MSG(mpls_deep_stack_alternate_marking_byte_counter_database_set(unit, core_id, intf_type, g_cint_field_am_ing_ent.byte_command_id1 , 1,
        ...
        if(!use_crps_egress_oam_counter)
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_stat_pkt_size_adjust_select_get(unit, 0, &key, bcmStatPktSizeSelectCounterIngressHeaderTruncate, &enable), "");
            enable = 0;
            BCM_IF_ERROR_RETURN_MSG(bcm_stat_pkt_size_adjust_select_set(unit, 0, &key, bcmStatPktSizeSelectCounterIngressHeaderTruncate, enable), "");
        }
    }
...
/*
 * Configuration for the iOAM header encapsualtion.
 * For the device without L_bit_mapping:
 * It set the FII label and header encapsulation entry.
 * For the device with L_bit_mapping:
 * It set the FII label only since the header info is set by iPMF in UDH.
 */
int
mpls_deep_stack_alternate_marking_enacp_ioam_config(
    int unit, 
    int in_port,
    int out_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchMplsAlternateMarkingSpecialLabel, speacial_label_value),
...
/*
 * Field and counter configuration for iFIT or iOAM for the ingress node.
 * For the device without L_bit_mapping:
 * iPMF with 5-tuple is used to set the iFIT/iOAM encapsulation pointer and the counter-id.
 * ePMF is used to update the ace context value for encap context selection, and also the snoop info.
 * For the device with L_bit_mapping:
 * iPMF is not used if all the header information can by set ePMF.
 * ePMF with 5-tupleis used to set the iFIT/iOAM header info and the counter, ace context value, snoop info in UDH.
 *    Since the UDH is resolved by ucode, the data structure should be:
 *    Control info in udh0ext:  rsvd(24 bits ) + is_ingress_node (1 bit) + L_bit_profile (3 bits) + snoop_id (4 bits)
 *    Header info:
 *       iOAM - UDH0: flow indication label, UDH1: flow-id label, UDH2-3 not used.
 *       iFIT - UDH0: flow-id label, UDH1-3 FIEH header per the header type and FIEH.length.(User should set it correctly)
 */
int
mpls_deep_stack_alternate_marking_enacp_instru_config(
    int unit, 
    int in_port,
    int out_port)
{
...
int
mpls_deep_stack_alternate_marking_enacp_configuration(
    int unit, 
    int in_port,
    int out_port)
{
...
/*
 * Configuration for MPLS tunnel encapsulation.
 * It's the general configuration for MPLS tunnel.
 *                                                                .
 * in_port - traffic incoming port.
 * out_port - traffic outgoing port.
 * nof_tunnel - number of mpls tunnels to encapsuate in the application.
 * encap_access_array - encap access array for each of the mpls tunnel.
 */
int
mpls_deep_stack_alternate_marking_encap_tunnel_config(
    int unit,
    int in_port,
    int out_port,
    int nof_tunnel,
    int *encap_access_array)
{

    int encap_access_idx = 0;
    int mpls_tunnel_id = 0, mpls_label_id = 0;
    ...
    else
    {
        ...
        int nof_labels_per_stage[3];
    
        CINT_INSTRU_IPT_TAIL_EDIT_PROFILE=2;
    
        /* encapsulation entries, not including alternate marking entries */
        nof_labels_per_stage[0] = nof_tunnel;
        nof_labels_per_stage[1] = nof_tunnel;
        nof_labels_per_stage[2] = 0;
    
        /* encapsulation entries - assume at least one tunnel is built in this scope */
        for (mpls_tunnel_id = 0; mpls_tunnel_id < 4; mpls_tunnel_id++, encap_access_idx++)
        {
            if (nof_labels_per_stage[mpls_tunnel_id] == 0)
            {
                break;
            }
    
            update_encap_access = TRUE;
            for (mpls_label_id = 0; mpls_label_id < nof_labels_per_stage[mpls_tunnel_id]; mpls_label_id++)
            {
                if (mpls_label_id == 0){
                    mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].label[0] = stage_enc_label_1++;
                    mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].flags |= BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT;
                    mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].l3_intf_id = &mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id+1].tunnel_id;
                    mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].num_labels = nof_labels_per_stage[mpls_tunnel_id];
                    mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].encap_access = encap_access_array[encap_access_idx];
                    mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].egress_qos_model.egress_qos = bcmQosEgressModelPipeMyNameSpace;
                    mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].exp0 = 1;
                    mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].exp1 = 1;
                }
                else if (mpls_label_id == 1){
                    mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].label[1] = stage_enc_label_2++;
                }
                else
                {
                    ...
                    if (mpls_label_id % 2 )
                    {
                        mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].data_label[mpls_label_id - 2] = stage_enc_label_2++;
                    }
                    else
                    {
                        mpls_util_entity[0].mpls_encap_tunnel[mpls_tunnel_id].data_label[mpls_label_id - 2] = stage_enc_label_1++;
                    }
                }
            }
        }
    ...
    int nof_tunnel)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_deep_stack_alternate_marking_enacp_ioam_config(unit, in_port, out_port), "");
...
int mpls_deep_stack_alternate_marking_get_counter(int unit, int l_bit_set, int * counter) {
    ...
    if (use_crps_egress_oam_counter)
    {
        /** cint_am_egress_counter_base, is set by ePMF in cint_field_alternate_marking.c*/
        source_id = cint_am_egress_counter_base + l_bit_set;
    }
    else
    {
...
int switch_counter_procedure_before_reading(int unit, int active_L_bit){
    ...
    /*in case egress oam counter is used, only process bytes count*/
    if(use_crps_egress_oam_counter && (stat_mode_pkt_and_byte == 1))
    {
        i= 1;
    }
    for (; i < nof_counter_type; i++) {
...
int read_counter_and_handle_eviction(int unit, int active_L_bit, int * counter) {
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_deep_stack_alternate_marking_get_counter(unit,!active_L_bit,counter), "");
...
int mpls_deep_stack_alternate_marking_get_counter2(int unit, int l_bit_set, int* pkt_counter, int* byte_counter) {
    ...
    for (; i < NOF_COUNTER_TYPE; i++) {
        ...
        if (use_crps_egress_oam_counter)
        {
            /** cint_field_am_egress_counter_base, is set by ePMF in cint_field_alternate_marking.c*/
            source_id = cint_am_egress_counter_base + l_bit_set ;
        }
        else
        {
...
int read_counter2_and_handle_eviction(int unit, int active_L_bit, int* pkt_counter, int* byte_counter) {
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_deep_stack_alternate_marking_get_counter2(unit,!active_L_bit,pkt_counter,byte_counter), "");
...
int mpls_alternate_marking_intermediate_field(int unit, int is_inter_pe) {
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_field_alternate_marking_inter_or_egress_node_iPMF1(unit, is_inter_pe), "");
    ...
    if (is_inter_pe)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(cint_field_alternate_marking_inter_pe_ipmf2_udh(unit), "");
        ...
        if (L_bit_mapping)
        {
            BCM_IF_ERROR_RETURN_MSG(cint_field_alternate_marking_inter_pe_epmf_ace(unit), "");
        }
        else
        {
            BCM_IF_ERROR_RETURN_MSG(cint_field_alternate_marking_inter_pe_epmf_udh(unit), "");
        }
...
int mpls_alternate_marking_termination(int unit, int in_port, int out_port, int flow_id, int is_inter_pe) {
    ...
    if (ifit_enable)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit,bcmSwitchMplsFlowInstructionIndicatorSpecialLabel, 12), "");
    }


    /* Set up snoop + snoop header*/
    BCM_IF_ERROR_RETURN_MSG(cint_field_ip_snoop_set(unit,cpu_port,1,0,0,snoop_trap_gport_id,&snoop_code), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_header_info_set(unit,BCM_MIRROR_DEST_IS_SNOOP ,snoop_code,&mirror_header_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_alternate_marking_intermediate_field(unit, is_inter_pe), "");
...
int mpls_am_inter_pe_route_into_tunnel (int unit, int in_port, int out_port)
{
    ...
    for (label_index = 0; label_index < 2; label_index++)
    {
        ...
        mpls_encap_tunnel[0].label[label_index] = common_mpls_tunnel_label[label_index];
    }
    mpls_encap_tunnel[0].num_labels = 2;
    mpls_encap_tunnel[0].flags = BCM_MPLS_EGRESS_LABEL_TTL_DECREMENT;
    ...
    mpls_encap_tunnel[0].action = 0;
    mpls_encap_tunnel[0].l3_intf_id = &cint_termination_mpls_basic_info.arp_id;
    ...
    common_mpls_tunnel_id = mpls_encap_tunnel[0].tunnel_id;
    ...
    /** Create ipv4 host.*/
    BCM_IF_ERROR_RETURN_MSG(add_host_ipv4(unit, dip_routing_into_tunnel, vrf, common_tunnel_fec_id, 0, 0), "");
