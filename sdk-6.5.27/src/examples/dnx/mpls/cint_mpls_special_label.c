 ...
 * 
 ...
 Copyright 2007-2022 Broadcom Inc. All rights reserved. File: cint_mpls_special_label.c Purpose: special label termination and encapsulation example. 
 */
    ...
    int update_hw)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(mpls_special_label_init_params(unit), "");

    BCM_IF_ERROR_RETURN_MSG(mpls_special_label_tunnel_property_set(unit,pwe_flag,mpls_tunnel_flag,mpls_tunnel_num), "");

    if (update_hw) {
        BCM_IF_ERROR_RETURN_MSG(vpls_main(unit,pwe_port.port,ac_port.port), "");
    }
