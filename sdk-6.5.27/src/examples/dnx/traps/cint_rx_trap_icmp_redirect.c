    ...
    int out_port)
{
    ...
    {
        BCM_IF_ERROR_RETURN_MSG(get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, &kaps_result), "");
    ...
    if (failover_id != 0)
    {
        snprintf(error_msg, sizeof(error_msg), "failover_out_port %d", failover_out_port);
        BCM_IF_ERROR_RETURN_MSG(out_port_set(unit, failover_out_port), error_msg);
    }
    ...
    if (host_format != 2)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, fec, ((host_format == 1) || (host_format == 3) ) ? 0 : intf_out, encap_id, gport, 0,
        ...
        if (failover_id != 0)
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, fec, (host_format == 1) ? 0 : intf_out, encap_id, gport, 0,
                                                   failover_id, failover_is_primary,&encoded_fec), "create egress object FEC only");
