/*
 * File: cint_rx_trap_etpp_mtu_trap_lif.c
 * Purpose: Configuration that creates an ETPP User defined trap.
 *          Creates an MTU violation configuration PER OUT-LIF and attaches it,
 *          to the created ETPP trap.
 *
 * cint ../../../../src/examples/dnx/traps/cint_rx_trap_etpp_mtu_trap_lif.c
 *
 * Main Function:
 *      cint_rx_trap_etpp_mtu_trap_lif_main(unit,out_lif,layer_type,compressed_layer_type,trap_id,fwd_strength,snp_strength,mtu_val);
 * Destroy Function:
 *      cint_rx_trap_etpp_mtu_trap_lif_destroy(unit,out_lif,layer_type,compressed_layer_type);
 *
 ...
 * Test Scenario - start
  ...
  cint ../../../../src/examples/dnx/traps/cint_rx_trap_etpp_mtu_trap_lif.c
  cint
  cint_rx_trap_etpp_mtu_trap_lif_main(0,out_lif,2,2,4,7,0,0x20);
 * Test Scenario - end
 *
 * Note: For some devices the layer_type to compressed_layer_type mapping is done on init and
 *       Only compressed_layer_type need to be supplied (layer_type must be 0)
 *       See trap etpp feature mtu_layer_to_compressed_layer_mapping_support
 */

/* The encoded User defined trap id */
int cint_etpp_mtu_trap_lif_trap_id;

/**
* \brief
* Creates an ETPP user defined trap, sets an action different than drop.
* Sets an MTU configuration and attaches it to the created trap, for LIF.
* \par DIRECT INPUT:
*   \param [in] unit         - Unit
*   \param [in] lif_gport    - OutLIF value.
*   \param [in] layer_type   - Layer Type
...
*   * None
*/
int cint_rx_trap_etpp_mtu_trap_lif_main(
    int unit,
    ...
    int mtu_profile)
{
    ...
    cint_etpp_mtu_trap_lif_trap_id = trap_id;
    BCM_GPORT_TRAP_SET(trap_gport, trap_id, fwd_strength, snp_strength);

    BCM_IF_ERROR_RETURN_MSG(cint_etpp_mtu_trap_mtu_lif_set(unit, lif_gport, trap_gport, mtu_val, layer_type, compressed_layer_type, mtu_profile), "");
...
*   * None
*/
int cint_etpp_mtu_trap_mtu_lif_set(
    int unit,
    bcm_gport_t lif_gport,
    bcm_gport_t trap_gport,
    int mtu_val,
    ...
    int mtu_profile)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_mtu_profile_set(unit, BCM_RX_MTU_LIF, &mtu_key, &mtu_value), "");

    mtu_config.flags = BCM_RX_MTU_LIF;
    mtu_config.gport = lif_gport;
...
/**
* \brief
* Destroys the created trap and MTU configuration for the supplied OUT-LIF.
* \par DIRECT INPUT:
*   \param [in] unit                        - Unit
*   \param [in] rif_intf                    - OUT-LIF value.
*   \param [in] layer_type                  - Layer type
...
*   * None
*/
int cint_rx_trap_etpp_mtu_trap_lif_destroy(
    int unit,
    bcm_gport_t lif_gport,
    int layer_type,
    int compressed_layer_type,
    int mtu_profile)
{
    ...
    if(cint_etpp_mtu_trap_lif_trap_id != BCM_RX_TRAP_EG_TX_TRAP_ID_DROP)
    {
        /* delete trap */
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_destroy(unit, cint_etpp_mtu_trap_lif_trap_id), "");
    }
