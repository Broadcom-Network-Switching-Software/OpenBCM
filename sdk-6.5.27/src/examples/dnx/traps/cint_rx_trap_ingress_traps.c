...
struct cint_rx_trap_ingress_traps_config_utils_s{
    ...
    /* BCM_RX_TRAP_FLAGS2_XXX FLAGS.
        ...
        * BCM_RX_TRAP_FLAGS2_UPDATE_ACL_PROFILE - Enables the trap to update the ACL profile.
        * BCM_RX_TRAP_FLAGS2_REPLICATION_FAILOVER_DISABLE - Disable replication failover.
        * BCM_RX_TRAP_FLAGS2_FALLBACK_TO_BRIDGE_DISABLE - Disable fallback to bridge.
        * BCM_RX_TRAP_FLAGS2_NETWORK_GROUP_DISABLE - Disable network group filter.
        * BCM_RX_TRAP_FLAGS2_SAME_INTERFACE_DISABLE - Disable same interface filter.
        * BCM_RX_TRAP_FLAGS2_UPDATE_TRUNK_MEMBERS_LOCAL_ONLY - Indicates that the LAG member is local.
        */
    uint32 flags2;
    ...
    /* Is EXEM learning.
        * Flag to be set for field use:
        * BCM_RX_TRAP_FLAGS2_UPDATE_EXEM_LEARN
        */
    uint32 is_exem_learn;

    /* Fabric header.
        * Flag to be set for field use:
        * BCM_RX_TRAP_FLAGS2_FABRIC_HEADER_SET
        */
    int fabric_header;
...
bcm_rx_trap_config_t* trap_config_p)
{
    ...
    trap_config_p->is_exem_learn = ingress_config_p->is_exem_learn;
    trap_config_p->fabric_header = ingress_config_p->fabric_header;
...
cint_rx_trap_ingress_traps_config_utils_s* ingress_config_p)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(trap_config_from_ingress_config_fill(&cint_rx_trap_ingress_traps_config, &trap_config), "");

    /* Set the trap */
    BCM_IF_ERROR_RETURN_MSG(cint_utils_rx_trap_create_and_set(unit, trap_create_flags, trap_type, trap_config, &cint_rx_trap_ingress_traps_trap_id), "");
...
int unit)
{
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_destroy(unit, cint_rx_trap_ingress_traps_trap_id), "");
