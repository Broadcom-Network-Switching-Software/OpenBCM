    ...
    int * etpp_trap_id)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_create(unit, 0, bcmRxTrapUserDefine, &ingress_trap_id), "");
    ...
    int etpp_trap_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_get(unit, etpp_trap_id, &trap_config), "");

    ingress_trap_id = BCM_GPORT_TRAP_GET_ID(trap_config.cpu_trap_gport);

    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_destroy(unit, ingress_trap_id), "");
