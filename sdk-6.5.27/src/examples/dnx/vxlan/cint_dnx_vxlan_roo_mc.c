    ...
    int unit)
{
    ...
    for (idx = 0; idx < 2; idx++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(vlan_port_add(unit, g_vxlan_roo_mc.access_port[idx], g_vxlan_roo_mc.access_eth_rif[idx], &g_vxlan_roo_mc.vlan_port_id[idx]), "");

        /* Create VSI */
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_create_with_id(unit, g_vxlan_roo_mc.access_eth_rif[idx]), "");

        /* Set LIF -> VSI(RIF) */
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, g_vxlan_roo_mc.access_eth_rif[idx], g_vxlan_roo_mc.vlan_port_id[idx]), "");

        /* VLAN membership */
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, g_vxlan_roo_mc.access_eth_rif[idx], g_vxlan_roo_mc.access_port[idx], 0), "");
    }

    /*
     * Provider port
     */
    BCM_IF_ERROR_RETURN_MSG(in_port_intf_set(unit, g_vxlan_roo_mc.provider_port[0], cint_vxlan_roo_basic_info.core_overlay_eth_rif), "provider_eth_rif");
    ...
    int unit)
{
    ...
    for (idx = 0; idx < 2; idx++)
    {
        ...
        if (*dnxc_data_get(unit, "l3", "feature", "public_routing_support", NULL))
        {
            ...
            dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, g_vxlan_roo_mc.access_eth_rif[idx], 0, 0, cint_vxlan_roo_basic_info.access_eth_rif_mac, cint_vxlan_roo_basic_info.access_eth_vrf);
        }
        ...
        snprintf(error_msg, sizeof(error_msg), "access_eth_rif[%d]", idx);
        BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure), error_msg);
    }
    ...
    int unit)
{
    ...
    eth_rif_structure.mtu_valid = g_vxlan_roo_mc.mtu_valid;
    eth_rif_structure.mtu = g_vxlan_roo_mc.mtu;
    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure), "provider_eth_rif");
...
int vxlan_roo_mc_add_native_mc(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(multicast__open_mc_group(unit, &g_vxlan_roo_mc.native_ipmc, 0), "");
    ...
    for (idx = 0; idx < 2; idx++)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_multicast_l3_encap_get(unit, -1 /* not used */, -1 /* not used */,
                                   g_vxlan_roo_mc.access_eth_rif[idx], &(encap_rif[idx])), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_multicast_vlan_encap_get(unit, -1 /* not used */, -1 /* not used */,
    ...
    for (idx = 0; idx < 4; idx++)
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_multicast_encap_extension_create(unit, BCM_MULTICAST_ENCAP_EXTENSION_WITH_ID, &(reps[idx].encap1), 2, ifs), "");
        printf("PPMC: CUD=0x%x  -->  ITF0=0x%x, ITF1=0x%x\n", reps[idx].encap1, ifs[0], ifs[1]);
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_set(unit, g_vxlan_roo_mc.native_ipmc, flags, 4, &reps[0]), "");
    ...
    if(egress_mc == 1)
    {
        BCM_IF_ERROR_RETURN_MSG(multicast__open_fabric_mc_or_ingress_mc_for_egress_mc(unit, g_vxlan_roo_mc.native_ipmc, g_vxlan_roo_mc.native_ipmc), "");
    }
    /* Add IPv4 IPMC */
    if (*(dnxc_data_get(unit, "l3", "fwd", "ipmc_config_cmprs_vrf_group", NULL)))
    {
        BCM_IF_ERROR_RETURN_MSG(add_ipv4_ipmc(unit, g_vxlan_roo_mc.native_mc_dip, g_vxlan_roo_mc.native_mc_dip_mask, 0, 0,
                       0, cint_vxlan_roo_basic_info.access_eth_vrf, g_vxlan_roo_mc.native_ipmc, 0,0), "");
        BCM_IF_ERROR_RETURN_MSG(add_ipv4_ipmc(unit, g_vxlan_roo_mc.native_mc_dip, g_vxlan_roo_mc.native_mc_dip_mask, 0, 0,
                       0, cint_vxlan_roo_basic_info.core_overlay_eth_vrf, g_vxlan_roo_mc.native_ipmc, 0,0), "");
        /* Add IPv6 IPMC */
        BCM_IF_ERROR_RETURN_MSG(add_ipv6_ipmc(unit, g_vxlan_roo_mc.native_mc_dip6, g_vxlan_roo_mc.native_mc_dip6_mask, sip, sip,
                       0, cint_vxlan_roo_basic_info.access_eth_vrf, 0x1, 0xffff, g_vxlan_roo_mc.native_ipmc, 0, 0, 0), "");

        /* IPv6 MC for 2nd pass (drop and continue) */
        BCM_IF_ERROR_RETURN_MSG(add_ipv6_ipmc(unit, g_vxlan_roo_mc.native_mc_dip6, g_vxlan_roo_mc.native_mc_dip6_mask, sip, sip,
                       0, cint_vxlan_roo_basic_info.core_overlay_eth_vrf, 2, 0xffff, g_vxlan_roo_mc.native_ipmc, 0, 0, 0), "");
    } else {
        BCM_IF_ERROR_RETURN_MSG(add_ipv4_ipmc(unit, g_vxlan_roo_mc.native_mc_dip, g_vxlan_roo_mc.native_mc_dip_mask, 0, 0,
...
int vxlan_roo_mc_add_overlay_mc(int unit, int is_leaf_node)
{
    ...
    /* Create multicast group */
    egress_mc = 0;
    BCM_IF_ERROR_RETURN_MSG(multicast__open_mc_group(unit, &g_vxlan_roo_mc.overlay_ipmc, 0), "");
    ...
    if (!is_leaf_node) {
        ...
        if (*dnxc_data_get(unit, "multicast", "params", "mcdb_formats_v2", NULL))
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, g_vxlan_roo_mc.overlay_ipmc,  BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_ELEMENT_STATIC, 2, replications), error_msg);
        } else {
            BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, g_vxlan_roo_mc.overlay_ipmc, BCM_MULTICAST_INGRESS_GROUP, 2, replications), error_msg);
        }
    }

    /* Add IPv4 IPMC */
    BCM_IF_ERROR_RETURN_MSG(add_ipv4_ipmc(unit, g_vxlan_roo_mc.overlay_mc_dip, g_vxlan_roo_mc.overlay_mc_dip_mask, 0, 0,
                       cint_vxlan_roo_basic_info.core_overlay_eth_rif, cint_vxlan_roo_basic_info.core_overlay_eth_vrf, g_vxlan_roo_mc.overlay_ipmc, 0,0), "IPv4");
    if (*(dnxc_data_get(unit, "l3", "fwd", "ipmc_config_cmprs_vrf_group", NULL)))
    {
        /* Add IPv6 IPMC */
        BCM_IF_ERROR_RETURN_MSG(add_ipv6_ipmc(unit, g_vxlan_roo_mc.overlay_mc_dip6, g_vxlan_roo_mc.overlay_mc_dip6_mask, sip, sip,
                           0, cint_vxlan_roo_basic_info.core_overlay_eth_vrf, 3, 0xffff, g_vxlan_roo_mc.overlay_ipmc, 0, 0, 0), "");
    } else {
        BCM_IF_ERROR_RETURN_MSG(add_ipv6_ipmc(unit, g_vxlan_roo_mc.overlay_mc_dip6, g_vxlan_roo_mc.overlay_mc_dip6_mask, sip, sip,
...
int vxlan_roo_mc_disable_split_horizon(int unit) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_network_group_config_set(unit, inlif_forwarding_group, &group_config), "");
    ...
    int provider_port2)
{
    ...
    /* Init parameters */
    vxlan_roo_mc_init(is_leaf_node, access_port1, access_port2, provider_port1, provider_port2);
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(vxlan_roo_mc_split_horizon_set(unit, cint_vxlan_roo_basic_info.vpn_id), "");

    /*
     * Configure port properties
     */
    BCM_IF_ERROR_RETURN_MSG(vxlan_roo_mc_configure_port_properties(unit), "");

    /*
     * Create recycle entry
     */
    BCM_IF_ERROR_RETURN_MSG(create_recycle_entry_with_defaults(unit, &g_vxlan_roo_mc.recycle_entry_encap_id), "");

    /*
     * Configure ingress interfaces
     */
    BCM_IF_ERROR_RETURN_MSG(vxlan_roo_mc_open_route_interfaces_access(unit), "");

    /*
     * Configure egress interfaces
     */
    BCM_IF_ERROR_RETURN_MSG(vxlan_roo_mc_open_route_interfaces_provider(unit), "");
    ...
    if (is_leaf_node) {
        BCM_IF_ERROR_RETURN_MSG(vxlan_port_add(unit, provider_port1, term_tunnel_id, &vxlan_port_id), "");
    } else {
        BCM_IF_ERROR_RETURN_MSG(vxlan_port_add(unit, g_vxlan_roo_mc.recycle_port, term_tunnel_id, &vxlan_port_id), "");
    }
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(vxlan_roo_mc_add_native_mc(unit), "");

    /* configure recycle port for 2nd pass*/
    if (!is_leaf_node && !ibch1_supported) {
        BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit,g_vxlan_roo_mc.recycle_port, bcmPortControlOverlayRecycle, 1), "");
    }

    /*
     * Configure overlay IPMC
     */
    BCM_IF_ERROR_RETURN_MSG(vxlan_roo_mc_add_overlay_mc(unit, is_leaf_node), "");
