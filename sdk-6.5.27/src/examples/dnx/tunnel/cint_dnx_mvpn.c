    ...
    int *tunnel_intf)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_encap__create(unit, 0, &tunnel_arp_id,
                                       tunnel_next_hop_mac,
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(in_port_intf_set(unit, cint_dnx_mvpn_info.provider_port, cint_dnx_mvpn_info.provider_vlan), "");
    ...
    if (*dnxc_data_get(unit, "l3", "feature", "public_routing_support", NULL))
    {
        ...
        dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, cint_dnx_mvpn_info.provider_vlan, 0, 0, cint_dnx_mvpn_info.my_mac, vrf);
    }
    /* 1. create RIF for IP routing for IP of the overlay : inRif allow the packet to be routed */
    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure), "provider_vlan");
    ...
    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure), "provider_vlan2");
    ...
    if (*dnxc_data_get(unit, "l3", "feature", "public_routing_support", NULL))
    {
        ...
        dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, cint_dnx_mvpn_info.internal_vlan, 0, 0, cint_dnx_mvpn_info.my_mac, vrf);
                
    }
    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure), "internal_vlan");
    printf("%s:\t rif configured for internal_vlan(%d)\n", proc_name, cint_dnx_mvpn_info.internal_vlan);

    /* add access port to access vlan. */
    BCM_IF_ERROR_RETURN_MSG(in_port_intf_set(unit, cint_dnx_mvpn_info.access_port, cint_dnx_mvpn_info.access_vlan), "");
    ...
    if (*dnxc_data_get(unit, "l3", "feature", "public_routing_support", NULL))
    {
        ...
        dnx_utils_l3_eth_rif_s_common_init(unit, 0, &eth_rif_structure, cint_dnx_mvpn_info.access_vlan, 0, 0, cint_dnx_mvpn_info.my_mac, vrf);
    }
    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3_eth_rif_create(unit, &eth_rif_structure), "access_vlan");
    ...
    int unit)
{
    ...
    if (!ibch1_supported)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit, cint_dnx_mvpn_info.recycle_port, bcmPortControlOverlayRecycle, 1), "");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(create_recycle_entry_with_defaults(unit, &cint_dnx_mvpn_info.recycle_entry_encap_id), "");
    ...
    int unit )
{
    ...
    BCM_IF_ERROR_RETURN_MSG(mvpn_ip_tunnel_terminator_create(unit, mc_ip, &term_tunnel_id, BCM_TUNNEL_TERM_BUD), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(multicast__open_mc_group(unit,&ipmc_index,BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_WITH_ID), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(mvpn_multicast_ingress_set(unit,ipmc_index, members_of_mc, nof_member_of_mc, encaps), "");
    printf("%s:\t %d members are added to MC\n", proc_name, nof_member_of_mc);

    /* add routing table entry, point to MC */

    BCM_IF_ERROR_RETURN_MSG(mvpn_ipmc_add(unit, mc_ip, 0/*src ip*/, ipmc_index, cint_dnx_mvpn_info.provider_vlan/* inRif */, 0/*flags*/), "");
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(mvpn_ipmc_add(unit, mc_ip, 0/*src ip*/, ipmc_index, cint_dnx_mvpn_info.internal_vlan/* inRif */, 0/*flags*/), "");
    printf("%s:\t mc group entry(0x%08X) added to routing table (0x%08X)\n", proc_name, ipmc_index, mc_ip);
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(mvpn_ip_tunnel_terminator_create(unit, mc_ip, &term_tunnel_id, 0), "");
    printf("%s:\t ip_tunnel_terminator_create term_tunnel_id(0x%08X)\n", proc_name, term_tunnel_id);
    ...
    int unit)
{
    ...
    int tunnel_encap_id;
    BCM_IF_ERROR_RETURN_MSG(mvpn_ip_tunnel_initiator_create(unit, &tunnel_encap_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(mvpn_ipmc_add(unit, mc_ip, 0/*src ip*/, ipmc_index, cint_dnx_mvpn_info.access_vlan/* inRif */, 0/*flags*/), "");
    printf("%s:\t mc group entry(0x%08X) added to routing table (0x%08X)\n", proc_name, ipmc_index, mc_ip);
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(mvpn_recycle_configuration(unit), "");


    /* create routing interface*/
    BCM_IF_ERROR_RETURN_MSG(mvpn_l3_intf_configuration(unit), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(mvpn_bud_provider_mc(unit), "");

    /* we configure only 1 MC group at access. (used for bud node and leaf node)*/
    BCM_IF_ERROR_RETURN_MSG(mvpn_bud_leaf_access_mc(unit), "");

    /* "continue" node MC configuration:
     * - configure MC and routing table entry at provider side */
    BCM_IF_ERROR_RETURN_MSG(mvpn_continue_mc(unit), "");

    /* leaf node configuration:
     * for leaf node, one-pass is enough.
     *   outer ip header is terminated in VT, and inner ip header is used as a key in FLP. .
     * configure MC at access side: see mvpn_bud_leaf_access_mc */
    BCM_IF_ERROR_RETURN_MSG(mvpn_leaf_configuration(unit), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(mvpn_access_mc(unit), "");
