 ...
 *                  - dnx_port_phy_loopback_set - Set PHY loopback for given port
 *                  - dnx_port_phy_loopback_clear - Clear PHY loopback for given port
 *
 ...
 *                 Please call dnx_port_phy_loopback_clear to clear the PHY loopback if a regular connection
 *                 with peer is needed.
 *
 ...
 * unit=0;
 * port=13;
 * print dnx_port_phy_loopback_set (unit, port);
 * tx 1 pbm=1 vl=100
 * bshell(unit,"port sts");
 * bshell(unit,"show co full");
 * print dnx_port_phy_loopback_clear (unit, port);
 *
 */
...
int
dnx_port_phy_loopback_port_lane_boundary_get(
    int unit,
    bcm_port_t port,
    int *lower_bound,
    int *upper_bound)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "for port %d - unit %d.", port, unit);
 ...
 */
int
dnx_port_phy_loopback_pm_ports_get(
    int unit,
    bcm_port_t port,
    ...
    int *port_list)
{
     ...
     */
    snprintf(error_msg, sizeof(error_msg), "failed for port %d - unit %d.", port, unit);
    BCM_IF_ERROR_RETURN_MSG(dnx_port_phy_loopback_port_lane_boundary_get(unit, port, &port_first_lane, &port_last_lane), error_msg);
    ...
    *nof_ports = 0;
    ...
    {
        snprintf(error_msg, sizeof(error_msg), "failed for port %d - unit %d.", nif_port, unit);
        BCM_IF_ERROR_RETURN_MSG(dnx_port_phy_loopback_port_lane_boundary_get(unit, nif_port, &port_first_lane, &port_last_lane), error_msg);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_get(unit, nif_port, &flags, &interface_info, &mapping_info), error_msg);
 ...
 */
int
dnx_port_phy_loopback_medium_type_and_dfe_mode_get(
    int unit,
    int phy_lane_config,
    medium_type_t *medium_type,
    dfe_mode_t *dfe_mode)
 ...
 */
int
dnx_port_phy_loopback_port_info_get(
    int unit,
    bcm_port_t port)
{
    char error_msg[200]={0,};
    ...
    snprintf(error_msg, sizeof(error_msg), "failed for port %d - unit %d.", port, unit);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_get(unit, port, &port_info_db[port].flags, &port_info_db[port].interface_info,
                      &port_info_db[port].mapping_info), error_msg);

    BCM_IF_ERROR_RETURN_MSG(bcm_port_resource_get(unit, port, &port_info_db[port].resource), error_msg);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_tpid_class_get(unit, &port_info_db[port].tpid_class), error_msg);

    /* Save port loopback info */
    BCM_IF_ERROR_RETURN_MSG(bcm_port_loopback_get(unit, port, &port_info_db[port].loopback), error_msg);

    BCM_IF_ERROR_RETURN_MSG(bcm_port_enable_get(unit, port, &port_info_db[port].enable), error_msg);
    BCM_IF_ERROR_RETURN_MSG(dnx_port_phy_loopback_medium_type_and_dfe_mode_get(unit, port_info_db[port].resource.phy_lane_config,
        &port_info_db[port].medium_type, &port_info_db[port].dfe_mode), error_msg);
    key.index = 2;      /* 1 is in, 2 is out */
    key.type = bcmSwitchPortHeaderType;
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_indexed_port_get(unit, port, key, &value_out), "out header type");

    key.index = 1;      /* 1 is in, 2 is out */
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_indexed_port_get(unit, port, key, &value_in), "in header type");
 ...
 */
int
dnx_port_phy_loopback_port_info_set(
    int unit,
    bcm_port_t port)
{
    char error_msg[200]={0,};
    ...
    snprintf(error_msg, sizeof(error_msg), "failed for port %d - unit %d.", port, unit);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_tpid_class_set(unit, &port_info_db[port].tpid_class), error_msg);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_enable_set(unit, port, port_info_db[port].enable), error_msg);
    /*
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_port_loopback_set(unit, port, port_info_db[port].loopback), error_msg);
    return BCM_E_NONE;
}
 ...
 */
int
dnx_dyn_port_properties_construct(
    int unit,
    int nof_ports,
    int *port_list,
    bcm_port_mapping_info_t *mapping_info,
...
)
{
   ...
   /*
...

    {
        ...
        tmp_port = port_list[port_index];
        snprintf(error_msg, sizeof(error_msg), "failed for port %d - unit %d.", tmp_port, unit);
        BCM_IF_ERROR_RETURN_MSG(dnx_port_phy_loopback_port_info_get(unit, tmp_port), error_msg);

        BCM_IF_ERROR_RETURN_MSG(cint_dyn_port_remove_port_full_example(unit, tmp_port, 0), error_msg);
    }
    /*
     * Configure Lane Map 
     */
    snprintf(error_msg, sizeof(error_msg), "failed - unit %d.", unit);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_lane_to_serdes_map_set(unit, 0, *(dnxc_data_get(unit, "nif", "phys", "nof_phys", NULL)), serdes_map), error_msg);
    BCM_IF_ERROR_RETURN_MSG(dnx_dyn_port_properties_construct(unit, nof_ports, port_list, mapping_info, interface_info, resource,
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_dyn_port_add_ports_full_example(unit, nof_ports, port_list, device_ids, mapping_info, interface_info, resource,
                                              dfe_mode, medium_type, header_type_in, header_type_out, flags), error_msg);
    for (port_index = 0; port_index < nof_ports; ++port_index)
    {
        /*
        ...
        tmp_port = port_list[port_index];
        snprintf(error_msg, sizeof(error_msg), "failed for port %d - unit %d.", tmp_port, unit);
        BCM_IF_ERROR_RETURN_MSG(dnx_port_phy_loopback_port_info_set(unit, tmp_port), error_msg);
        /*
 ...
 */
int
dnx_port_phy_loopback_lane_swap_align(
    int unit,
    bcm_port_t port)
{
    char error_msg[200]={0,};
     ...
     */
    snprintf(error_msg, sizeof(error_msg), "failed for port %d - unit %d.", port, unit);
    BCM_IF_ERROR_RETURN_MSG(dnx_port_phy_loopback_pm_ports_get(unit, port, &pm_first_lane, &pm_last_lane, &nof_ports, port_list), error_msg);
    snprintf(error_msg, sizeof(error_msg), "failed - unit %d.", unit);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_lane_to_serdes_map_get(unit, 0, *(dnxc_data_get(unit, "nif", "phys", "nof_phys", NULL)), serdes_map), error_msg);
    for (lane = pm_first_lane; lane <= pm_last_lane; ++lane)
    {
        /*
    ...
    snprintf(error_msg, sizeof(error_msg), "failed for port %d - unit %d.", port, unit);
    BCM_IF_ERROR_RETURN_MSG(dnx_port_pm_lane_swap_adjust(unit, nof_ports, port_list, *(dnxc_data_get(unit, "nif", "phys", "nof_phys", NULL)), serdes_map, 1), error_msg);
 ...
 * \param [in] unit - chip unit id
 * \param [in] port - logical port #
 * \param [out] need_to_align_swap - need_to_align_swap #
 *
 * \return
 *   int - see _SHR_E_*
 *
 ...
 */
int
dnx_port_phy_loopback_lane_swap_recover(
    int unit,
    int port)
{
    char error_msg[200]={0,};
    ...
    for (port_index = 0; port_index < nof_ports; ++port_index)
    {
        ...
        snprintf(error_msg, sizeof(error_msg), "failed for port %d- unit %d.", port_list[port_index], unit);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_loopback_get(unit, port_list[port_index], &loopback), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "failed for port %d- unit %d.", port, unit);
    BCM_IF_ERROR_RETURN_MSG(dnx_port_pm_lane_swap_adjust(unit, nof_ports, port_list, *(dnxc_data_get(unit, "nif", "phys", "nof_phys", NULL)), serdes_map, 0), error_msg);
    return BCM_E_NONE;
}
    ...
    bcm_port_t port)
{
    ...
    if (is_dnx)
    {
        BCM_IF_ERROR_RETURN_MSG(dnx_port_phy_has_lane_swap(unit, port, &need_to_align_swap), "");
        if (need_to_align_swap)
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_port_loopback_set(unit, port, BCM_PORT_LOOPBACK_NONE), error_msg);
            BCM_IF_ERROR_RETURN_MSG(dnx_port_phy_loopback_lane_swap_align(unit, port), error_msg);
        }
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_port_loopback_set(unit, port, BCM_PORT_LOOPBACK_PHY), error_msg);
    return BCM_E_NONE;
}
    ...
    bcm_port_t port)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(dnx_port_phy_loopback_lane_swap_recover(unit, port), error_msg);
    return BCM_E_NONE;
}
 ...
 */
int
dnx_port_phy_loopback_db_clear(
    int unit,
    bcm_port_t port)
{
    /*
 ...
 */
int
dnx_port_phy_loopback_enable_set(
    int unit,
    bcm_port_t port,
    int en)
{
    ...
    int unit)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "failed for unit %d.", unit);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_config_get(unit, &config), error_msg);
 ...
 */
int
dnx_port_phy_loopback_clear_all(
    int unit)
{
    char error_msg[200]={0,};
    bcm_port_t port;
