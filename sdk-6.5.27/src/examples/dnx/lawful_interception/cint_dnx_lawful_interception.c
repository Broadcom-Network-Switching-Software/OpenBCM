...
int is_snoop = 0;
uint8 is_jr1_system_header_mode=0;

/*
 * init for mirror information
 */
void lawful_interception_info_init(int pon_port, int nni_port, int rcy_port, int out_vlan, uint32 content_id, uint16 udp_dst_port, uint16 udp_src_port, uint32 dip, int strip_l2, int l3_enable) {
 ...
 *
 * Under JR1 system header mode, to remove ETH:
 *  1. create different mir_dest according to TAG_structure for removing ETH.
 *  2. should set bytes_to_remove explictly for each mir_dest.
 *  3. PMF add qulifier and action select mir_dest according to TAG_structure.
 */
int lawful_interception_mirror_dest_create(int unit)
{
    ...
    uint32 flags = (is_snoop) ? BCM_MIRROR_DEST_IS_SNOOP : 0;
    ...
    if (lawful_interception_info.strip_l2 || lawful_interception_info.l3_enable) {
        if (is_jr1_system_header_mode)
        {
            /**set single tag ETH length here for example*/
            mirror_header_info.pp.bytes_to_remove = 12 + 4 + 2;
        }
        else
        {
        ...
        mirror_header_info.pp.force_mirror = 1;
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_header_info_set(unit, flags, dest.mirror_dest_id, &mirror_header_info), "");
 ...
 */
int lawful_interception_filter_config(int unit, int in_port,bcm_field_context_t context_id, int is_remove_ll)
{
    ...
    fg_info.nof_quals = (is_jr1_system_header_mode && is_remove_ll) ? 2 : 1;
    fg_info.qual_types[0] = bcmFieldQualifyInPort;
    if (is_jr1_system_header_mode && is_remove_ll)
    {
        /**add this qulifier to select mir_dest_id*/
        fg_info.qual_types[1] = bcmFieldQualifyVlanFormat;    
    }
    fg_info.nof_actions = 1;
    fg_info.action_types[0] = is_snoop ? bcmFieldActionSnoop : bcmFieldActionMirrorIngress;

    snprintf(error_msg, sizeof(error_msg), "for %s sys_header mode", is_jr1_system_header_mode?"JR1":"JR2");
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &li_mirror_fg_id), error_msg);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, li_mirror_fg_id, context_id, &attach_info), "");
    ...
    if (is_jr1_system_header_mode && is_remove_ll)
    {
        ...
        ent_info.entry_qual[1].value[0] = bcmTagStructTypeCTag;
    ...
    if (is_snoop) {
        /* set snoop strength */
        ent_info.entry_action[0].value[2] = 7;
    }

    printf("Adding Entry\n");
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, li_mirror_fg_id, &ent_info, &li_mirror_ent_id), "");
 ...
 */
int lawful_interception_LI_tunnel_replace(int unit, uint32 content_id, uint32 content_id_hi)
{
    int encap_id;
    ...
    li_tunnel.flags = (content_id_hi == 0) ? 0 : BCM_TUNNEL_INIT_WIDE;
    ...
    li_tunnel.aux_data2 = content_id_hi;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_create(unit, &li_intf, &li_tunnel), "replace LI tunnel");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_get(unit, &li_intf, &li_tunnel_get), "LI tunnel");
...
int lawful_interception_tunnel_create(int unit, int is_ipv4_encap)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(dnx_utils_l3__intf_rif__create(unit, &intf), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_create(unit, flags, &l3eg, &l3egid), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_create(unit, &ip_intf, &tunnel), "IP tunnel");
    ...
    {
        /** since all out lifs are pointed from this tunnel tunnel, need to estimate the entire encap size */
        /** ARP = min 14, IP = 20 for Ipv4 or 40 for Ipv6 */
        li_tunnel.estimated_encap_size = 14 + (is_ipv4_encap ? 20 : 40);
    }
    bcm_l3_intf_t_init(&li_intf);
    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_create(unit, &li_intf, &li_tunnel), "LI tunnel");
...
int lawful_interception_main_config(int unit, int in_port, int mir_port, int is_remove_ll, int is_ipv4_encap)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(lawful_interception_tunnel_create(unit, is_ipv4_encap), "");
    printf(">>>lawful_interception_tunnel_create pass!\n");
    
    /*
     * mirror create
     */
    BCM_IF_ERROR_RETURN_MSG(lawful_interception_mirror_dest_create(unit), "");
    printf(">>>lawful_interception_mirror_dest_create pass!\n");

    /*
     * filter config
     */
    BCM_IF_ERROR_RETURN_MSG(lawful_interception_filter_config(unit, in_port, BCM_FIELD_CONTEXT_ID_DEFAULT, is_remove_ll), "");
...
int lawful_interception_main_config_clean(int unit)
{
    ...
    /**clear PMF*/
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_delete(unit, li_mirror_fg_id, NULL, li_mirror_ent_id), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_detach(unit, li_mirror_fg_id, BCM_FIELD_CONTEXT_ID_DEFAULT), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_delete(unit, li_mirror_fg_id), "");

    /**clear mirror dest*/
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_destination_destroy(unit, lawful_interception_info.mirror_dest_id), "");

    /**clear encap*/
    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_clear(unit,lawful_interception_info.lawful_interception_if), "for LI tunnel");
    lawful_interception_info.lawful_interception_if.l3a_intf_id = 0;

    BCM_IF_ERROR_RETURN_MSG(bcm_tunnel_initiator_clear(unit,lawful_interception_info.vxlan_if), "for IP tunnel");

    BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_destroy(unit, lawful_interception_info.next_hop_if), "");
