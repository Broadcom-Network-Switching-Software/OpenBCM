    ...
    int add)
{
    ...
    if (add){
        snprintf(error_msg, sizeof(error_msg), "Unit %d, match_port: %d, match_vid is: %d", unit, match_port, match_vlan);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_match_add(unit, gport, &matchInfo), error_msg);
        printf("Unit %d: Added port match <port, vid> = <%d,%d> to gport 0x%x\n", unit, match_port, match_vlan, gport);
    } else {
        BCM_IF_ERROR_RETURN_MSG(bcm_port_match_delete(unit, gport, &matchInfo), "");
    }
    ...
    int no_initial_match)
{
    ...
    */
    ...
    for (i = 0 ; i < 2 ; i++)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit1, port_match_info_1.in_vids[i]), "");

        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit1, port_match_info_1.in_vids[i], port_match_info_1.ports[0], 0), "port1");
    }
    ...
    */
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit1, &vlan_port), "");
    ...
    if (no_initial_match) {
        ...
        BCM_IF_ERROR_RETURN_MSG(port_match_add_remove(unit1,
    ...
    int port4)
{
    ...
    */
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit1, port_match_info_1.in_vids[2]), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit1, port_match_info_1.in_vids[2], port_match_info_1.ports[0], 0), "port1");
    ...
    */
    ...
    vlan_port.flags = flags;
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit2, &vlan_port), "");


    port_match_info_1.eg_gport_egress = vlan_port.vlan_port_id;

    /* Create EVE on egress AC */
    BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set(unit2, port_match_info_1.eg_gport_egress, /* lif  */
                                                      0x8100,                         /* outer_tpid */
                                                      0,                              /* inner_tpid */
    ...
    for (i = 0 ; i < nof_eg_ports ; i++) {
        ...
        if (i < (nof_eg_ports - nof_dropped_ports)) {
            BCM_IF_ERROR_RETURN_MSG(port_match_add_remove(unit2,
                                       BCM_PORT_MATCH_PORT,
    ...
    if (!*dnxc_data_get (unit, "dev_init", "general", "application_v2", NULL)) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_lif_set(unit2, 0, &lif_config), "in Default Egress");
    }
    ...
    int no_initial_match)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit1, port_match_info_1.in_gport), "");

    for (i = 0 ; i < nof_units ; i++) {
        unit = units_ids[i];
        BCM_IF_ERROR_RETURN_MSG(vlan_translation_default_mode_destroy(unit), "");
    }
