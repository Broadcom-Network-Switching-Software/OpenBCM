    ...
    int oPort)
{
    ...
    vlan_match_untagged_packet_struct_get(&param);
    ...
    vlan_match_untagged_packet_s * param)
{
    ...
    vlan_match_untagged_packet_init(param);
    ...
    {
        snprintf(error_msg, sizeof(error_msg), "(port = %d)", g_vlan_match_untagged_packet.in_ports[i]);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, g_vlan_match_untagged_packet.in_ports[i], bcmPortClassId,
                                g_vlan_match_untagged_packet.in_ports[i]), error_msg);
    }

    snprintf(error_msg, sizeof(error_msg), "(port = %d)", g_vlan_match_untagged_packet.out_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, g_vlan_match_untagged_packet.out_port, bcmPortClassId,
                            g_vlan_match_untagged_packet.out_port), error_msg);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, g_vlan_match_untagged_packet.unknown_outer_vid, pbmp, untag_pbmp), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(unit, &l2_addr), "(VSI = 1)");

    bcm_l2_addr_t_init(&l2_addr, g_vlan_match_untagged_packet.d_mac, g_vlan_match_untagged_packet.unknown_outer_vid);
    l2_addr.flags = BCM_L2_STATIC;
    l2_addr.port = g_vlan_match_untagged_packet.out_port;

    snprintf(error_msg, sizeof(error_msg), "(VSI = %d)", g_vlan_match_untagged_packet.unknown_outer_vid);
    ...
    int step)
{
    ...
    switch (step)
    {
        case 2:
        {
             ...
             * Create VSI:
             */
            snprintf(error_msg, sizeof(error_msg), "(vis=%d)", g_vlan_match_untagged_packet.vsi);
            BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, g_vlan_match_untagged_packet.vsi), error_msg);
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, g_vlan_match_untagged_packet.vsi, pbmp, untag_pbmp), "");
            ...
            BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set(unit, g_vlan_match_untagged_packet.inLif,         /* lif gport */
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(unit, &l2_addr), "(VSI = 0)");
        ...
        case 3:
        {
            ...
            snprintf(error_msg, sizeof(error_msg), "(port = 0x%08X)", g_vlan_match_untagged_packet.inLif);
            BCM_IF_ERROR_RETURN_MSG(bcm_port_match_add(unit, g_vlan_match_untagged_packet.inLif, &match_info), error_msg);
        ...
        case 4:
        {
            ...
            snprintf(error_msg, sizeof(error_msg), "(port = %d)", g_vlan_match_untagged_packet.inLif);
            BCM_IF_ERROR_RETURN_MSG(bcm_port_match_delete(unit, g_vlan_match_untagged_packet.inLif, &match_info), error_msg);

            BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, g_vlan_match_untagged_packet.inLif), error_msg);
            ...
            snprintf(error_msg, sizeof(error_msg), "(port = %d, match_ethertype = 0x%04X)",
    ...
    int unit)
{
     ...
     * LIF
     */
    snprintf(error_msg, sizeof(error_msg), "(gport = 0x%08X)", g_vlan_match_untagged_packet.inLif);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, g_vlan_match_untagged_packet.inLif), error_msg);
    
    /*
     * VSI
     */
    snprintf(error_msg, sizeof(error_msg), "(vsi=%d)", g_vlan_match_untagged_packet.vsi);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_destroy(unit, g_vlan_match_untagged_packet.vsi), error_msg);
