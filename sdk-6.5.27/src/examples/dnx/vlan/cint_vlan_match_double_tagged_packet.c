    ...
    int oPort3)
{
    ...
    vlan_match_double_tagged_packet_struct_get(&param);
    ...
    vlan_match_double_tagged_packet_s * param)
{
    ...
    vlan_match_double_tagged_packet_init(param);
   ...
   */
    ...
    {
        snprintf(error_msg, sizeof(error_msg), "(vis=%d)", g_vlan_match_double_tagged_packet.vsi[i]);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_create(unit, g_vlan_match_double_tagged_packet.vsi[i]), error_msg);
    }
    ...
    for (i = 0; i < NUM_OF_OUT_PORTS; i++)
    {
        snprintf(error_msg, sizeof(error_msg), "(port = %d)", g_vlan_match_double_tagged_packet.out_ports[i]);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, g_vlan_match_double_tagged_packet.out_ports[i], bcmPortClassId,
                                g_vlan_match_double_tagged_packet.out_ports[i]), error_msg);
    }

    snprintf(error_msg, sizeof(error_msg), "(port = %d)", g_vlan_match_double_tagged_packet.in_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, g_vlan_match_double_tagged_packet.in_port, bcmPortClassId,
                            g_vlan_match_double_tagged_packet.in_port), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "(vid=%d)", g_vlan_match_double_tagged_packet.outer_vid);
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_add(unit, g_vlan_match_double_tagged_packet.outer_vid, pbmp, untag_pbmp), error_msg);
    ...
    for (i = 0; i < NUM_OF_OUT_PORTS; i++)
    {
        ...
        snprintf(error_msg, sizeof(error_msg), "(VSI = %d)", g_vlan_match_double_tagged_packet.vsi[i]);
    ...
    switch (step)
    {
        case 1:
        {
            ...
            for (i = 0; i < NUM_OF_OUT_PORTS; i++)
            {
                ...
                snprintf(error_msg, sizeof(error_msg), "(vsi = %d)", vlan_port.vsi);
        ...
        case 2:
        {
            ...
            if (g_vlan_match_double_tagged_packet.inLifs[0])
            {
                for (i = 0; i < NUM_OF_OUT_PORTS; i++)
                {
                    snprintf(error_msg, sizeof(error_msg), "(gport = 0x%08X)", g_vlan_match_double_tagged_packet.inLifs[i]);
                    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, g_vlan_match_double_tagged_packet.inLifs[i]), error_msg);
                }
            ...
            for (i = 0; i < NUM_OF_OUT_PORTS; i++)
            {
                ...
                int criteria_int = criteria[i];

                snprintf(error_msg, sizeof(error_msg), "(port = 0x%08X, criteria = %d)",
                    g_vlan_match_double_tagged_packet.inLifs[i], criteria_int);
                BCM_IF_ERROR_RETURN_MSG(bcm_port_match_add(unit, g_vlan_match_double_tagged_packet.inLifs[i], &match_info), error_msg);
            }
    ...
    int unit)
{
    ...
    for (i = 0; i < NUM_OF_OUT_PORTS; i++)
    {
        ...
        snprintf(error_msg, sizeof(error_msg), "(port = 0x%08X, criteria = %d)",
            g_vlan_match_double_tagged_packet.inLifs[i], criteria_int);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_match_delete(unit, g_vlan_match_double_tagged_packet.inLifs[i], &match_info), error_msg);
    }
    ...
    {
        snprintf(error_msg, sizeof(error_msg), "(vsi=%d)", g_vlan_match_double_tagged_packet.vsi[i]);
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_destroy(unit, g_vlan_match_double_tagged_packet.vsi[i]), error_msg);
    }
