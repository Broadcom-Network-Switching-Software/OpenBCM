                                                ...
                                                bcm_field_entry_t *entry_handle)
{
    ...
    ent_info.nof_entry_quals = is_egress_sflow ? 5 : 4;
    ...
    ent_info.entry_qual[3].type = bcmFieldQualifyInPort;
    ent_info.entry_qual[3].value[0] = 0;
    ent_info.entry_qual[3].mask[0] = 0;

    if (is_egress_sflow) 
    {
        ent_info.entry_qual[4].type = bcmFieldQualifyMirrorCode;
        ent_info.entry_qual[4].value[0] = BCM_GPORT_MIRROR_GET(mirror_code);
        ent_info.entry_qual[4].mask[0] = 0x7;
    }
...
int cint_sflow_field_main(int unit, int is_egress_sflow, bcm_field_context_t context_id)
{
    ...
    * Set the range.
    */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_range_set(unit, 0, bcmFieldStageIngressPMF1, cint_sflow_field_range_id, &range_info), "");
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 5;
    dest_char = &(qual_info.name[0]);
    sal_strncpy_s(dest_char, "Fwd_plus_1_layer_t", sizeof(qual_info.name));
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit,0,qual_info, &cint_sflow_field_qual_id), "");
    ...
    /* Set quals */
    fg_info.nof_quals = is_egress_sflow ? 5 : 4;
    ...
    if (is_egress_sflow) {
        fg_info.qual_types[4] = bcmFieldQualifyMirrorCode;
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &cint_sflow_field_fg_id), "");
...
int cint_sflow_egress_field_main(int unit, bcm_field_context_t context_id)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "on field_group_id = %d", cint_sflow_egress_field_fg_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &cint_sflow_egress_field_fg_id), error_msg);
    ...
    snprintf(error_msg, sizeof(error_msg), "on contex_id = %d", context_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_sflow_egress_field_fg_id, context_id, &attach_info), error_msg);
    ...
    int mirror_profile)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, cint_sflow_egress_field_fg_id, &entry_info, &entry_id), "");
