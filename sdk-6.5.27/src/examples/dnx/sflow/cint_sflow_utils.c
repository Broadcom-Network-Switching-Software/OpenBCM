...
cint_sflow_utils_timestamp_update(int unit, uint32 time_msec)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "(type = bcmInstruControlSFlowUpTime, value = %d)", time_msec);
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_control_set(unit, 0, bcmInstruControlSFlowUpTime, time_msec), error_msg);
...
cint_sflow_utils_udp_tunnel_create(int unit, bcm_gport_t *udp_tunnel)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(intf_eth_rif_create(unit, cint_sflow_basic_info.out_rif,
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(create_arp_with_next_ac_type_vlan_translate_none(unit, cint_sflow_basic_info.tunnel_next_hop_mac, &arp_itf, cint_sflow_basic_info.collector_vlan), "");
					...
					int is_replace)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_header_info_set(unit, BCM_MIRROR_DEST_IS_STAT_SAMPLE | BCM_MIRROR_DEST_EGRESS_ADD_ORIG_SYSTEM_HEADER, mirror_dest.mirror_dest_id, &mirror_header_info), "");
...
cint_sflow_utils_egress_create(int unit, int in_port, int out_port, bcm_gport_t *sflow_destination, int eventor_id, int is_aggregated)
{
    ...
    if (!is_aggregated)
    {
       if (cint_sflow_basic_info.udp_tunnel_id == -1) {
            ...
            */
           BCM_IF_ERROR_RETURN_MSG(cint_sflow_utils_udp_tunnel_create(unit, &udp_tunnel_id), "");
    ...
    *sflow_destination = sflow_encap_info.sflow_encap_id;
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_sflow_field_egress_estimate_encap_size(unit), "");
    return BCM_E_NONE;
}
...
cint_sflow_utils_sflow_sn_config(int unit, int eventor_port, int sflow_out_port, int context_id)
{
   ...
   BCM_IF_ERROR_RETURN_MSG(cint_sflow_field_utils_sn_update_quals_set(unit, eventor_port, context_id), "");
   
   /** Add SFLOW SN entry */
   BCM_IF_ERROR_RETURN_MSG(cint_sflow_field_utils_sn_update_entry_add(unit, eventor_port), "");
   return BCM_E_NONE; 
}
...
cint_sflow_utils_sflow_sn_reset(int unit, int eventor_port)
{
    ...
    */
   BCM_IF_ERROR_RETURN_MSG(get_core_and_tm_port_from_port(unit, eventor_port, &core_id, &tm_port), "");
   ...
   */
   ...
   BCM_IF_ERROR_RETURN_MSG(bcm_stat_database_eviction_boundaries_set(unit, 0, &database, 0, &boundaries), "");
...
cint_sflow_utils_route_sflow_datagram(int unit, int in_port, int sflow_out_port, int kaps_result)
{
   ...
   */
   ...
   printf("1. %s: unit = %d, port = %d, vsi = %d, vlan_port_id = 0x%08X\n", proc_name, unit, vlan_port.port, vlan_port.vsi, vlan_port.vlan_port_id);
   ...
   */
   ...
   printf("2. %s: unit = %d, port = %d, vlan_port_id = 0x%08X\n", proc_name, unit, sflow_out_port, vlan_port.vlan_port_id);
   ...
   */
   ...
   printf("5. %s: unit = %d, arp_encap_id = 0x%08X\n", proc_name, unit, arp_encap_id);
   ...
   */
   ...
   BCM_IF_ERROR_RETURN_MSG(bcm_l3_egress_create(unit, BCM_L3_INGRESS_ONLY | BCM_L3_WITH_ID, &l3eg, &encoded_fec), "");
    ...
    printf("6: %s: unit = %d, fec = 0x%08X, encoded_fec = 0x%08X \n",proc_name, unit, fec, encoded_fec);
...
cint_sflow_utils_eventor_set(int unit, int eventor_id, int buffer_size, int nof_samples, int eventor_port, int sflow_out_port, int reconfigure)
{
   ...
   uint8 header_data_udp_tunnel[128] = {
    ...
    {
        printf("%s: Error (BCM_E_FAIL), only the following tunnel types are supported: bcmTunnelTypeUdp, bcmTunnelTypeUdp6 \n", proc_name);
        return BCM_E_FAIL;
    }
...
cint_sflow_utils_registers_init(int unit)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_control_set(unit, 0, bcmInstruControlSFlowSamplingRate, cint_sflow_basic_info.sample_rate_divisor), "");

    /*
     * Set the global sFlow agent IP address.
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_control_set(unit, 0, bcmInstruControlSFlowAgentIPAddress, cint_sflow_basic_info.tunnel_sip), "");

    /*
     * For the purpose of the example cint, we'll use a meaningless timestamp.
     */
    BCM_IF_ERROR_RETURN_MSG(cint_sflow_utils_timestamp_update(unit, cint_sflow_basic_info.uptime), "");
...
cint_sflow_utils_egress_interface_create(int unit, int port, int interface, int is_input)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_sflow_sample_interface_add(unit, &sample_interface_info), "");
...
cint_sflow_utils_egress_interface_remove(int unit, int port, int is_input)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_sflow_sample_interface_remove(unit, &sample_interface_info), "");
...
cint_sflow_utils_lag_create(int unit, int port1, int port2, int port3)
{
    ...
    /* Create trunk  */
    BCM_IF_ERROR_RETURN_MSG(cint_trunk_utils_create(unit, &trunk), "");
