 ...
 *
 * TWAMP LAG Reflector Scenario:
 * 1. TWAMP Reflector 1st pass:
 *     1.1 IPMF1 TCAM lookup covered - {SIP, DIP, UDP.SRC_PORT, UDP.DST_PORT, REF_MEM_LINK_ID}.
 *           Action:
 *               bcmFieldActionTrap -
 ...
 *                   OAM-Up-MEP = 0 (DOWN) 
 *
 * 2. TWAMP LAG Reflector 2nd pass:
 *    2.1 PMF lookup covered - {SIP, DIP, UDP.SRC_PORT, UDP.DST_PORT, REF_MEM_LINK_ID}.
 *         Action:
 ...
 *            bcmFieldActionForward - destination
...
bcm_field_qualify_t twamp_reflecor_pmf2_q1_qual_id;
bcm_field_qualify_t twamp_reflecor_pmf2_q2_qual_id;
bcm_field_qualify_t twamp_reflecor_member_link_id_qual_id;
bcm_field_action_t twamp_reflector_pass_act_id;
bcm_field_action_t twamp_reflector_dest_act_id;
 ...
 * \brief
 *
 *  This function creates the user qualifier
 *
 * \param [in] unit - The unit number.
 *
 * \return
 ...
 *
 */
int cint_field_twamp_reflector_user_qual_create(int unit) {
    void *dest_char;
    ...
    sal_strncpy_s(dest_char, "TWAMP_REF_PMF2_Q1", sizeof(qual_info.name));

    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &qual_info, &twamp_reflecor_pmf2_q1_qual_id), "");

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 1; 
    dest_char = &(qual_info.name[0]);
    sal_strncpy_s(dest_char, "TWAMP_REF_PMF2_Q2", sizeof(qual_info.name));

    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &qual_info, &twamp_reflecor_pmf2_q2_qual_id), "");

    /* Create an user-qualifier in ipmf1 that will parse the reflector member link id */
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 16;
    dest_char = &(qual_info.name[0]);
    sal_strncpy_s(dest_char, "REF_MEM_LINK_ID", sizeof(qual_info.name));

    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &qual_info, &twamp_reflecor_member_link_id_qual_id), "");
...
int cint_field_twamp_reflector_user_act_create(int unit) {
    ...
    */
    ...
    sal_strncpy_s(dest_char, "TWAMP_REF_PASS_ACT", sizeof(user_action_info.name));
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_create(unit, 0, &user_action_info, &twamp_reflector_pass_act_id), "");

    /** Create destination action to ignore destination qualifier which is 0*/
    bcm_field_action_info_t_init(&user_action_info);
    user_action_info.action_type = bcmFieldActionForward;
    user_action_info.prefix_size = 11;
    user_action_info.prefix_value = 0;
    user_action_info.size = 21;
    user_action_info.stage = bcmFieldStageIngressPMF1;
    sal_strncpy_s(dest_char, "TWAMP_REF_DEST_ACT", sizeof(user_action_info.name));
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_create(unit, 0, &user_action_info, &twamp_reflector_dest_act_id), "");
 ...
 * \param [in] unit - The unit number.
 * \param [in] is_ipv6 - The flag indicates it is the ipv6.
 * \param [in] is_lag - The flag indicates it is the LAG.
 ...
 * \param [in] ref_mem_link_id1 - The reflector member link id 1. 
 * \param [in] ref_mem_link_id2 - The reflector member link id 2. 
 * \param [in] gport_trap1 - The gport trap id 1. 
 * \param [in] gport_trap2 - The gport trap id 2.
 *
 * \return
 *  int - Error Type, in sense of bcm_error_t
 *
 */
int cint_field_twamp_reflector_1st_pass_set_ipmf1(int unit, int is_ipv6, int is_lag, 
    uint32 local_ip_addr, uint32 remote_ip_addr, 
    bcm_ip6_t *local_ip6_addr, bcm_ip6_t *remote_ip6_addr,
    int ref_mem_link_id1, int ref_mem_link_id2,
    bcm_gport_t gport_trap1,bcm_gport_t gport_trap2) {
    int i, ent_nums = 1;
    void *dest_char;
    ...
    if (is_lag) {
        ent_nums = 2;
        fg_info.nof_quals = 5;
        fg_info.qual_types[4] = twamp_reflecor_member_link_id_qual_id;
    }
    ...
    fg_info.action_types[3] = twamp_reflector_pass_act_id;  
    fg_info.action_with_valid_bit[3] = FALSE;
    dest_char = &(fg_info.name[0]);
    sal_strncpy_s(dest_char, "TWAMP_REF1_PMF1_FG", sizeof(fg_info.name));

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &twamp_reflector_1st_pass_ipmf1_fg_id), "");
    ...
    for (i = 0; i < fg_info.nof_actions; i++)
    {
        ...
        if (fg_info.action_types[i] == twamp_reflector_pass_act_id) {
    ...
    if (is_lag) {
        ...
        attach_info.key_info.qual_info[4].input_arg = 3;
        attach_info.key_info.qual_info[4].offset = 18*8; /* Sesssion-Sender: Reflector member link id */
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, twamp_reflector_1st_pass_ipmf1_fg_id, twamp_context_id, &attach_info), "");

    /* Add field entry */
    for (i = 0; i < ent_nums; i++) {
        ref_mem_link_id = (i == 0) ? ref_mem_link_id1 : ref_mem_link_id2;
        gport_trap = (i == 0) ? gport_trap1 : gport_trap2;
        ...
        if (is_lag) {
            ent_info.entry_qual[4].type = fg_info.qual_types[4];
            ent_info.entry_qual[4].value[0] = ref_mem_link_id;
            ent_info.entry_qual[4].mask[0] = 0xFFFF;
        }

        ent_info.nof_entry_actions = fg_info.nof_actions;
        ent_info.entry_action[0].type = fg_info.action_types[0];
        ent_info.entry_action[0].value[0] = gport_trap;
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, twamp_reflector_1st_pass_ipmf1_fg_id, &ent_info, &ent_id), "");
    }
...
{   int i;   
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_create(unit, 0, &user_action_info, &user_act_id), "");
     ...
     */
    fg_info.nof_quals = 3;    
    fg_info.qual_types[0] = twamp_reflecor_pmf2_q1_qual_id;
    fg_info.qual_types[1] = twamp_reflecor_pmf2_q2_qual_id;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &twamp_reflector_1st_pass_ipmf2_fg_id), "ipmf2_fg");
    ...
    * 13 MSB as force for oam_offset, oam_sub_type and oam_up_mep */
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_fem_action_add(unit,0 ,twamp_reflector_1st_pass_ipmf2_fg_id, BCM_FIELD_ACTION_POSITION(3, 2), &fem_action_info), "ipmf2");
    bcm_field_group_attach_info_t_init(&attach_info);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_action_offset_get(unit, 0, twamp_reflector_1st_pass_ipmf1_fg_id, fg_info.action_types[2], &action_offset), "");
    ...
    /* Get the offset for the action */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_action_offset_get(unit, 0, twamp_reflector_1st_pass_ipmf1_fg_id, fg_info.action_types[3], &action_offset), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, twamp_reflector_1st_pass_ipmf2_fg_id, twamp_context_id, &attach_info), "ipmf2");
 ...
 * \param [in] ref_mem_link_id1 - The reflector member link id 1. 
 * \param [in] ref_mem_link_id2 - The reflector member link id 2. 
 * \param [in] fec_id1 - The FEC ID 1
 * \param [in] fec_id2 - The FEC ID 2 *
 * \return
 *  int - Error Type, in sense of bcm_error_t
 *
 */
int cint_field_twamp_reflector_2nd_pass_set_ipmf1(int unit, int is_ipv6, int is_lag, int is_stateless, 
    uint32 local_ip_addr, uint32 remote_ip_addr, 
    bcm_ip6_t *local_ip6_addr, bcm_ip6_t *remote_ip6_addr,
    int ref_mem_link_id1, int ref_mem_link_id2,
    bcm_gport_t gport_fec1,bcm_gport_t gport_fec2) {
    int i, ent_nums = 1;
	void *dest_char;
    bcm_field_entry_t ent_id;
    bcm_field_group_info_t fg_info;
     ...
     */
    fg_info.nof_actions = is_stateless ? 3: 5;    
    fg_info.action_types[0] = bcmFieldActionForwardingAdditionalInfo;
    fg_info.action_types[1] = twamp_reflector_pass_act_id;
    fg_info.action_with_valid_bit[1] = FALSE;
    fg_info.action_types[2] = twamp_reflector_dest_act_id;
    if (!is_stateless) {
        fg_info.action_types[3] = bcmFieldActionStatId7;
        fg_info.action_types[4] = bcmFieldActionStatProfile7;
    }
    dest_char = &(fg_info.name[0]);
    sal_strncpy_s(dest_char, "TWAMP_REF2_PMF1_FG", sizeof(fg_info.name));

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &twamp_reflector_2nd_pass_ipmf1_fg_id), "");
    ...
    if (is_lag) {
        attach_info.key_info.qual_info[4].input_type = bcmFieldInputTypeLayerAbsolute;
        attach_info.key_info.qual_info[4].input_arg = 3;
        attach_info.key_info.qual_info[4].offset = 42*8; /* Session-Reflector: Reflector member link id */
    }

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, twamp_reflector_2nd_pass_ipmf1_fg_id, twamp_context_id, &attach_info), "");

    /* Add field entry */
    for (i = 0; i < ent_nums; i++) {
        ref_mem_link_id = (i == 0) ? ref_mem_link_id1 : ref_mem_link_id2;
        gport_fec = (i == 0) ? gport_fec1 : gport_fec2;
        ...
        ent_info.entry_action[1].type = fg_info.action_types[1];
        ent_info.entry_action[1].value[0] = 1; /* Present the 2nd pass */

        ent_info.entry_action[2].type = fg_info.action_types[2];
        ent_info.entry_action[2].value[0] = gport_fec & 0x3FFFF;

        if (!is_stateless) {
            ent_info.entry_action[3].type = fg_info.action_types[3];
            ent_info.entry_action[3].value[0] = 8; /* counter_id */
    
            ent_info.entry_action[4].type = fg_info.action_types[4];
            ent_info.entry_action[4].value[0] = 1; /* is_meter or lm_incr */
            ent_info.entry_action[4].value[1] = 1; /* is_lm */
            ent_info.entry_action[4].value[2] = 0; /* type */
            ent_info.entry_action[4].value[3] = 1; /* valid */
        }

        BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, twamp_reflector_2nd_pass_ipmf1_fg_id, &ent_info, &ent_id), "");
    }
...
{   int i;   
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &twamp_reflector_2nd_pass_ipmf2_fg_id), "ipmf2_fg");
    ...
    * 13 MSB as force for oam_offset, oam_sub_type and oam_up_mep */
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_fem_action_add(unit,0 ,twamp_reflector_2nd_pass_ipmf2_fg_id, BCM_FIELD_ACTION_POSITION(3, 3), &fem_action_info), "ipmf2");
    bcm_field_group_attach_info_t_init(&attach_info);
    ...
    /* Get the offset for the action */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_action_offset_get(unit, 0, twamp_reflector_2nd_pass_ipmf1_fg_id, fg_info.action_types[0], &action_offset), "");
    attach_info.key_info.qual_info[0].offset = action_offset - 1;   
    ...
    /* Get the offset for the action */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_action_offset_get(unit, 0, twamp_reflector_2nd_pass_ipmf1_fg_id, fg_info.action_types[1], &action_offset), "");
    attach_info.key_info.qual_info[1].offset = action_offset;   

    attach_info.key_info.qual_info[2].input_type = bcmFieldInputTypeLayerRecordsFwd;
    attach_info.key_info.qual_info[2].input_arg = 1;
    
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, twamp_reflector_2nd_pass_ipmf2_fg_id, twamp_context_id, &attach_info), "ipmf2");
...
int cint_field_twamp_reflector_lm_read_index_set_ipmf3(int unit) {
    int i;
    void *dest_char;
    bcm_field_context_t context_id = *(dnxc_data_get(unit, "field", "context", "default_context", NULL));;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, fg_id, &ent_info, &ent_id), "");
 ...
 * \param [in] gport_trap1 - The gport trap id 1. 
 * \param [in] gport_trap2 - The gport trap id 2. 
 * \param [in] gport_fec1 - The gport fec 1. 
 * \param [in] gport_fec2 - The gport fec 2.
 *
 * \return
 *  int - Error Type, in sense of bcm_error_t
 *
 */
int cint_field_twamp_reflector_rule(int unit, int is_ipv6, int is_lag, int is_stateless,
    uint32 local_ip_addr, uint32 remote_ip_addr,
    bcm_ip6_t *local_ip6_addr, bcm_ip6_t *remote_ip6_addr,
    int ref_mem_link_id1, int ref_mem_link_id2,
    bcm_gport_t gport_trap1, bcm_gport_t gport_trap2,
    ...
    snprintf(error_msg, sizeof(error_msg), "context %d", twamp_context_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_create(unit, 0, bcmFieldStageIngressPMF1, &context_info, &twamp_context_id), error_msg);
    printf("twamp_context_id:%d\n", twamp_context_id);

    /* Add flow_id in field */
    if (*dnxc_data_get(unit, "oam", "general", "oam_twamp_flow_id_ext_enable", NULL))
    {
        BCM_IF_ERROR_RETURN_MSG(appl_dnx_field_flow_id_init(unit,twamp_context_id), "");
    }

    /* Create presel */
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_presel_set(unit, is_ipv6, ingress_presel_id, twamp_context_id), "");

    /*** create user qualifier ***/
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_reflector_user_qual_create(unit), "");

    /*** create user action ***/
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_reflector_user_act_create(unit), "");
    
    /*** create IPMF1 rule for 1st pass ***/
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_reflector_1st_pass_set_ipmf1(unit, is_ipv6, is_lag, local_ip_addr, remote_ip_addr, 
        local_ip6_addr, remote_ip6_addr, 0, ref_mem_link_id2, gport_trap1, gport_trap2), "");

    /*** create IPMF2 rule for 1st pass ***/
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_reflector_1st_pass_set_ipmf2(unit), "");
	
    /*** create IPMF1 rule for 2nd pass ***/
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_reflector_2nd_pass_set_ipmf1(unit, is_ipv6, is_lag, is_stateless, local_ip_addr, remote_ip_addr, 
        local_ip6_addr, remote_ip6_addr, ref_mem_link_id1, ref_mem_link_id2, gport_fec1, gport_fec2), "");

    /*** create IPMF2 rule for 2nd pass ***/
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_reflector_2nd_pass_set_ipmf2(unit), "");

    /* Add lm_read_index for TWAMP */
    if ((is_stateless == 0) &&(*(dnxc_data_get(unit,  "field", "init", "oam_stat", NULL)) == 0)) {
        BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_reflector_lm_read_index_set_ipmf3(unit), "");
    }
 ...
 *
 */
int cint_field_twamp_rx_set_ipmf1(int unit, int is_ipv6, bcm_gport_t gport_trap1, int trap_code_qual, 
	uint32 local_ip_addr, uint32 remote_ip_addr,
	bcm_ip6_t *local_ip6_addr, bcm_ip6_t *remote_ip6_addr) {
    int i;
    void *dest_char;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &twamp_rx_ipmf1_fg_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, twamp_rx_ipmf1_fg_id, twamp_context_id, &attach_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, twamp_rx_ipmf1_fg_id, &ent_info, &ent_id), "");
...
{   int i;   
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &qual_info, &user_qual_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &twamp_rx_ipmf2_fg_id), "ipmf2_fg");
    ...
    * 13 MSB as force for oam_offset, oam_sub_type and oam_up_mep */
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_fem_action_add(unit,0 ,twamp_rx_ipmf2_fg_id, BCM_FIELD_ACTION_POSITION(3, 3), &fem_action_info), "ipmf2");
    bcm_field_group_attach_info_t_init(&attach_info);
    ...
    /* Get the offset for the action */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_action_offset_get(unit, 0, twamp_rx_ipmf1_fg_id, fg_info.action_types[0], &action_offset), "");
    attach_info.key_info.qual_info[0].offset = action_offset - 1;  

    attach_info.key_info.qual_info[1].input_type = bcmFieldInputTypeLayerRecordsFwd;
    attach_info.key_info.qual_info[1].input_arg = 1;
    
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, twamp_rx_ipmf2_fg_id, twamp_context_id, &attach_info), "ipmf2");
 ...
 *
 */
int cint_field_twamp_tx_set_ipmf1(int unit, int is_ipv6, uint32 local_ip_addr, uint32 remote_ip_addr, 
    bcm_ip6_t *local_ip6_addr, bcm_ip6_t *remote_ip6_addr) {
    int i;
    void *dest_char;
    bcm_field_entry_t ent_id;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &twamp_tx_ipmf1_fg_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, twamp_tx_ipmf1_fg_id, twamp_context_id, &attach_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, twamp_tx_ipmf1_fg_id, &ent_info, &ent_id), "");
...
{   int i;   
    ...
    bcm_field_group_attach_info_t attach_info;    
    bcm_field_fem_action_info_t  fem_action_info;
    int fem_action_oam_value = 0x0F0000; /* MEP-type: Down, MEP-sub-type: TWAMP-TX */
    int action_offset;
     ...
     *     oam_offset[8:15]: 0
     *     oam_sub_type[16:19]: 15(TWAMP_TX)
     *     oam_up_mep[20]: 1
     */
    fg_info.nof_actions = 1;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &twamp_tx_ipmf2_fg_id), "ipmf2_fg");
    ...
    * 13 MSB as force for oam_offset, oam_sub_type and oam_up_mep */
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_fem_action_add(unit,0 ,twamp_tx_ipmf2_fg_id, BCM_FIELD_ACTION_POSITION(3, 2), &fem_action_info), "ipmf2");
    bcm_field_group_attach_info_t_init(&attach_info);
    ...
    /* Get the offset for the action */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_action_offset_get(unit, 0, twamp_tx_ipmf1_fg_id, fg_info.action_types[0], &action_offset), "");
    attach_info.key_info.qual_info[0].offset = action_offset - 1;    

    attach_info.key_info.qual_info[1].input_type = bcmFieldInputTypeLayerRecordsFwd;
    attach_info.key_info.qual_info[1].input_arg = 1;
    
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, twamp_tx_ipmf2_fg_id, twamp_context_id, &attach_info), "ipmf2");
 ...
 *
 */
int cint_field_twamp_tx_rx_rule(int unit, int is_ipv6, bcm_gport_t gport_trap1, int trap_code_qual, 
    uint32 local_ip_addr, uint32 remote_ip_addr,
    bcm_ip6_t *local_ip6_addr, bcm_ip6_t *remote_ip6_addr)
{
    char error_msg[200]={0,};
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_rx_set_ipmf1(unit, is_ipv6, gport_trap1, trap_code_qual, 
        local_ip_addr, remote_ip_addr, local_ip6_addr, remote_ip6_addr), "");

    /*** create IPMF2 rule for RX ***/
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_rx_set_ipmf2(unit), "");
    
    /*** create IPMF1 rule for TX ***/
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_tx_set_ipmf1(unit, is_ipv6, local_ip_addr, remote_ip_addr, local_ip6_addr, remote_ip6_addr), "");

    /*** create IPMF2 rule for TX ***/
    BCM_IF_ERROR_RETURN_MSG(cint_field_twamp_tx_set_ipmf2(unit), "");
