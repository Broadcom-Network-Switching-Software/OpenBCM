    ...
    int *dp)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &in_vlan_port), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_create(unit, flags, &policer_generic_qos_map_id), "");

    flags = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_PHB | BCM_QOS_MAP_OPCODE ;
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_create(unit, flags, &opcode_ingress), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &l2_in_map, policer_generic_qos_map_id), "");
    bcm_qos_map_t_init(&l2_in_map);
    ...
    for(i = 0; i < nof_entries; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &l2_in_map, opcode_ingress), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_port_map_set(unit, in_vlan_port.vlan_port_id, policer_generic_qos_map_id, -1), "");
    ...
    int base_engine)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_database_create(unit, flags, database_handle, &database_config), "");
    ...
    if (*ingress_special_engine == base_engine || *egress_special_engine == base_engine)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_policer_engine_database_attach(unit, flags, &engine, &attach_config), "(special_engine)");
    }
    ...
    else
    {
        ...
        engine_size = (*dnxc_data_get(unit, "meter", "mem_mgmt", "nof_buckets_in_big_bank", NULL)) / (*dnxc_data_get(unit, "meter", "mem_mgmt", "nof_banks_in_big_engine", NULL));
        ...
        for (i = 0; i < nof_elements; i++)
        {
            ...
            if ((*dnxc_data_get(unit, "meter", "mem_mgmt", "nof_banks_in_big_engine", NULL)) == 1)
            {
            ...
            snprintf(error_msg, sizeof(error_msg), "(engine=%d)", engine.engine_id);
            BCM_IF_ERROR_RETURN_MSG(bcm_policer_engine_database_attach(unit, flags, &engine, &attach_config), error_msg);
        }
    }

    /** step 4: get back the configuration (from one core), just for validation - not mandatory  */
    snprintf(error_msg, sizeof(error_msg), "(engine=%d)", engine.engine_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_engine_database_get(unit, flags, &engine, &attach_config_get), error_msg);
    /** verification  */
    ...
    if (configure_meters == TRUE)
    {
        ...
        for (i = 0; i < total_nof_meters; i++)
        {
            ...
            snprintf(error_msg, sizeof(error_msg), "(meter_idx=%d)", meter_idx);
            BCM_IF_ERROR_RETURN_MSG(bcm_policer_create(unit, &pol_cfg, &policer_id), error_msg);

        }
    }
    /** step 5: enable the policer database*/
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_database_enable_set(unit, flags, database_handle, TRUE), "");
    ...
    int nof_engines)
{
    ...
    int database_handle, curr_engine, i;
    int flags = 0;
    bcm_policer_engine_t engine;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_database_enable_set(unit, flags, database_handle, FALSE), "");
    ...
    for(i = 0; i < nof_engines; i++)
    {
        ...
        if(single_bucket_mode && !special_engine && engine.section == bcmPolicerEngineSectionAll && (*dnxc_data_get(unit, "meter", "mem_mgmt", "nof_banks_in_big_engine", NULL)) != 1)
        {
            engine.section = bcmPolicerEngineSectionLow;

            BCM_IF_ERROR_RETURN_MSG(bcm_policer_engine_database_detach(unit, flags, &engine), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_database_destroy(unit, flags, database_handle), "");
    ...
    int *core)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_get(unit, gport, &dummy_flags, &interface_info, &mapping_info), "");
    ...
    uint32 ckbits_sec)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "with policer_id %d", policer_id);
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_create(unit, &pol_cfg, &policer_id), error_msg);
    ...
    uint32 qual_value)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        bcm_field_presel_entry_data_t presel_entry_data;
        bcm_field_presel_entry_id_t presel_entry_id;
        BCM_IF_ERROR_RETURN_MSG(bcm_field_context_create(unit, BCM_FIELD_FLAG_WITH_ID, stage, &context_info, &context_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, fg_id, &ent_info, &fe_id), "");
    for(idx=0; idx < nof_meter; idx++) {
    ...
    int unit)
{
     ...
     * Delete field entry
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_delete(unit, fg_id, NULL, fe_id), "");
    ...
    int profile)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, epmf_fg_id, context_id, &epmf_attach_info), "");
    ...
    int color_blind)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(policer_core_from_port_get(unit, inPortValue, &core_id), "");

    BCM_IF_ERROR_RETURN_MSG(policer_generic_meter_database_example(unit, core_id, TRUE, database_id, 0 /** base_pointer */ , FALSE    /** single_bucket_mode
                                                                                                                         */ ,
    ...
    BCM_IF_ERROR_RETURN_MSG(policer_generic_create_example(unit, core_id, database_handle, meter_idx, mode, color_blind, 30000), "");

    /** map packet to meter_id, according to in-port */
    BCM_IF_ERROR_RETURN_MSG(policer_ingress_meter_id_mapping_by_acl(unit, &database_id, &meter_idx, 1, bcmFieldQualifyInPort, in_port), "");
    /** send traffic with DA=1 and Vlan tag id 100 to out_port */
    bcm_l2_addr_t_init(&l2addr, mac, 100);
    l2addr.port = out_port;
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(unit, l2addr), "with vid 1");
     ...
     *
     */
    BCM_IF_ERROR_RETURN_MSG(policer_generic_qos_ingress_port_set(unit, in_port, 100, 2, pkt_pri /* pkt_pri */, pkt_cfi /* cfi */, tc /* tc */, dp /* dp */), "");

    /*
     * set discard DP to drop all packets with DP = 2 or 3
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_discard_set(unit, BCM_COSQ_DISCARD_ENABLE | BCM_COSQ_DISCARD_COLOR_BLACK | BCM_COSQ_DISCARD_COLOR_RED), "");

    /** in case the in_port=out_port, need to disable filtering */
    if(in_port == out_port)
    {
        BCM_IF_ERROR_RETURN_MSG(disable_same_if_filter_all(unit, out_port), "");
    }
    ...
    int is_ingress)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(policer_core_from_port_get(unit, out_port, &core_id), "");
    /** create ingress/egress policer database # 0 */
    BCM_IF_ERROR_RETURN_MSG(policer_generic_meter_database_example(unit, core_id, is_ingress, 0 /* database_id */ ,
    ...
    BCM_IF_ERROR_RETURN_MSG(policer_generic_meter_database_example(unit, core_id, is_ingress , 1 /* database_id */ ,
    ...
    snprintf(error_msg, sizeof(error_msg), "with policer_id %d", policer_id_share);
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_create(unit, &pol_cfg_share, &policer_id_share), error_msg);
    ...
    if(is_ingress)
    {
        ...
        database_id=1;
        BCM_IF_ERROR_RETURN_MSG(policer_ingress_meter_id_mapping_by_acl(unit, &database_id, &meter_idx_share, 1, bcmFieldQualifyInPort, in_port), "");

        /** map packet to meter_id, according to tc */
        database_id=0;
        BCM_IF_ERROR_RETURN_MSG(policer_ingress_meter_id_mapping_by_acl(unit, &database_id, &meter_idx,1, bcmFieldQualifyIntPriority, tc[0]), "");
    }
    else
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_stat_pp_profile_create(unit, 0, bcmStatCounterInterfaceEgressTransmitPp,
            &egress_pp_stat_profile, &egress_stat_pp_profile_info), "");
        BCM_IF_ERROR_RETURN_MSG(policer_etpp_ace_generate_stat_id(unit, out_port, meter_idx_share, egress_pp_stat_profile), "");

        /** map traffic with vlan=100 to meter database-0 in group=0 and to the a meter in group=1 */
        BCM_IF_ERROR_RETURN_MSG(policer_egress_pointer_generation(unit, core_id, meter_idx, 0, out_port, 100), "");
    }
    ...
    BCM_POLICER_ID_SET(policer_id_share, database_handle1, meter_idx_share);
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_create(unit, &pol_cfg_share, &policer_id_share), "");

    if(is_ingress)
    {
        /** map packet to meter_id, according to tc */
        database_id=0;
        BCM_IF_ERROR_RETURN_MSG(policer_ingress_meter_id_mapping_by_acl(unit, &database_id, &meter_idx, 1, bcmFieldQualifyIntPriority, tc[1]), "");
    }
    ...
    l2addr.port = out_port;
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(unit, l2addr), "with vid 100");

    if(is_ingress)
    {
        /** set discard DP to drop all packets with DP = 3 */
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_discard_set(unit,
    ...
    bcm_policer_t policer)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_get(unit, policer, &cfg), "for same_prf_1_policer_id");
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_action_profile_set(unit, 0, bcmRxTrapEgTxMetering, gport), "");
    ...
    int vlan)
{
    ...
    for(tc_index = 0; tc_index < 8; tc_index++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &map_entry, map_id), "");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &out_vlan_port), "");
    /** connect the gport to the stat-id */
    stat_info.flags = 0;
    stat_info.stat_id = base_meter_ptr;
    stat_info.stat_pp_profile = egress_pp_stat_profile;
    BCM_IF_ERROR_RETURN_MSG(bcm_gport_stat_set(unit, out_vlan_port.vlan_port_id, core_id, bcmStatCounterInterfaceEgressTransmitPp, stat_info), "");
    ...
    int vlan)
{
    ...
    /** 1. Destroy Qos meter */
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_destroy(unit, map_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_find(unit, &out_vlan_port), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, out_vlan_port.vlan_port_id), "");
    ...
    int out_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(policer_generic_meter_database_example(unit, core_id, TRUE /** is ingress */, 0 /* database_id */,
            0 /** base_pointer */, TRUE    /** single_bucket_mode*/ , FALSE /* expansion_enable */ , FALSE /* expand_per_tc */ ,
            1280 /* (port*fwd-type) total_nof_meters */ , TRUE   /* configure_meters */ , 0 /* base_engine */ ), "database0");

    /** second database */
    BCM_IF_ERROR_RETURN_MSG(policer_generic_meter_database_example(unit, core_id, TRUE /** is ingress */, 1 /* database_id */,
            0 /** base_pointer */, FALSE    /** single_bucket_mode*/ , FALSE /* expansion_enable */ , FALSE /* expand_per_tc */ ,
            1024 /* total_nof_meters */ , FALSE      /* configure_meters */ , first_big_engine_id /* base_engine */ ), "database1");
    ...
    BCM_IF_ERROR_RETURN_MSG(policer_generic_meter_database_example(unit, core_id, TRUE /** is ingress */, 2 /* database_id */,
            0 /** base_pointer */, FALSE    /** single_bucket_mode*/ , FALSE /* expansion_enable */ , FALSE /* expand_per_tc */ ,
            (engine_size*2)-1 /* total_nof_meters */ , FALSE      /* configure_meters */ , (first_big_engine_id + 2) /* base_engine */ ), "database2");
    ...
    meter_idx = 32;
    /* create a single meter */
    BCM_IF_ERROR_RETURN_MSG(policer_generic_create_example(unit, core_id, database_handle1, meter_idx, bcmPolicerModeCascade, 0, 50000), "");
    ...
    /* create a single meter */
    BCM_IF_ERROR_RETURN_MSG(policer_generic_create_example(unit, core_id, database_handle2, meter_idx, bcmPolicerModeTrTcmDs, 0, 60000), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_get(unit, gport, &dummy_flags, &interface_info, &mapping_info), "");
    database_id=2;
    BCM_IF_ERROR_RETURN_MSG(policer_ingress_meter_id_mapping_by_acl(unit, &database_id, &meter_idx, 1, bcmFieldQualifyDstPort, mapping_info.tm_port), "");
    ...
    int color_blind)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(policer_core_from_port_get(unit, in_port, &core_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_expansion_groups_set(unit, expansion_flags, core_id, &expansion_group), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(policer_generic_meter_database_example(unit, core_id, TRUE, database_id, 0 /** base_pointer */ ,
    ...
    for(meter_idx = base_meter_idx; meter_idx < (base_meter_idx + nof_groups); meter_idx++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(policer_generic_create_example(unit, core_id, database_handle, meter_idx, mode, color_blind, rate_kbits_sec), "");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(policer_ingress_meter_id_mapping_by_acl(unit, &database_id, &meter_ptr, 1, bcmFieldQualifyInPort, in_port), "");
    ...
    l2addr.port = out_port;
    l2addr.flags = BCM_L2_STATIC;
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(unit, l2addr), "with DA=00:00:00:00:00:01 and VLAN Tag ID=1");
    ...
    l2addr.port = out_port;
    l2addr.flags = BCM_L2_STATIC;
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(unit, l2addr), "with DA=01:00:5e:00:00:01 and VLAN Tag ID=1");
    ...
    l2addr.port = out_port;
    l2addr.flags = BCM_L2_STATIC;
    BCM_IF_ERROR_RETURN_MSG(bcm_l2_addr_add(unit, l2addr), "with DA=FF:FF:FF:FF:FF:FF and VLAN Tag ID=1");
    ...
    if (color_blind == FALSE)
    {
         ...
         */
        BCM_IF_ERROR_RETURN_MSG(policer_generic_qos_ingress_port_set(unit, in_port, 1 /* VLAN ID */, 8 /* nof entries in the QOS profile */, pkt_pri /* pkt_pri */, pkt_cfi /* cfi */, tc /* tc */, dp /* dp */), "");
    }
    ...
    int color_blind)
{
     ...
     * Create a meter
     */
    BCM_IF_ERROR_RETURN_MSG(policer_generic_create_example(unit, core_id, database_handle, meter_idx, mode, TRUE /* color_blind */, 100000 /** Rate 100 Mbit/s */), "");

    /**
     * Enable Traffic Class sensitive metering for the database
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_control_set(unit, 0, database_handle, bcmPolicerControlTypeTCSMEnable, TRUE), "");

    /**
     * Enable PP Traffic Class to be used for determining the threshold
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_policer_control_set(unit, 0, database_handle, bcmPolicerControlTypeTCSMUsePPTC, TRUE), "");
    ...
    for (tc_idx = 0; tc_idx < BCM_COS_COUNT; tc_idx ++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, gport, tc_idx, bcmCosqControlPolicerTCSMThresh, threshold), "");
    }
    ...
    int mode)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(policer_generic_meter_database_destroy(unit, core, TRUE, database_id, base_engine, bcmPolicerEngineSectionAll), "");
    ...
    for (tc = 0; tc < BCM_COS_COUNT; tc ++)
    {
         ...
         */
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, gport, tc, bcmCosqControlPolicerTCSMThresh, 0), "");
    }

    /**
     * Destroy the mapping packets to the base meter using Ingress PMF to create the Stat Object per qualifier InPort.
     */
    BCM_IF_ERROR_RETURN_MSG(policer_ingress_meter_id_mapping_by_acl_destroy(unit), "");
