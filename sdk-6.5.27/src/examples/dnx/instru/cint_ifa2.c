 ...
 *  IFA 2.0 is supported for Jericho2 device.
 *
 *                       IFA Zone Framework
 *
 *                             +----------+
...
int IPT_PROFILE_IFA2_MD_INSERTION = 6; /* Used for intermediate and termination with md nodes */
...
bcm_field_context_t cint_field_IFA_datapath_ipmf1_context_id_ipv4;
...
/* PMF hashing params */
bcm_field_context_hash_action_key_t cint_field_IFA_datapath_action_keys[] = {bcmFieldContextHashActionKeyEcmpLbKey0, bcmFieldContextHashActionKeyEcmpLbKey1, bcmFieldContextHashActionKeyEcmpLbKey2, bcmFieldContextHashActionKeyNetworkLbKey, bcmFieldContextHashActionKeyLagLbKey};
bcm_field_crc_select_t cint_field_IFA_datapath_crc_selects[] = {bcmFieldCrcSelectCrc16P0x10039, bcmFieldCrcSelectCrc16P0x100d7, bcmFieldCrcSelectCrc16P0x1015d, bcmFieldCrcSelectCrc16P0x10939, bcmFieldCrcSelectCrc16P0x109e7};
bcm_field_context_hash_function_t cint_field_IFA_datapath_hash_function = bcmFieldContextHashFunctionCrc16Bisync;
...
int cint_instru_ipt_init(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_ipt_profile_set(unit, 0, ipt_profile, &config), "Tail Edit");
...
int cint_ifa2_node_parameters_config(int unit)
{
    ...
    config_data.max_payload_length = DEFAULT_IFA2_MAX_SIZE;
    config_data.template_id = DEFAULT_IFA_TEMPLATE_ID;

    /** Set ifa node */
    BCM_IF_ERROR_RETURN_MSG(bcm_ifa_config_info_set(unit , 0, &config_data), "");

    /** Hashing of src-ip and dst-ip is done in parser */
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_get(unit, bcmSwitchHashIP4OuterField, &ipv4_hash_args), "");

    /** Hashing of next-protocol is not part of the parser
     *  Disable the parser from adding next-protocol to the LB-key */
    ipv4_hash_args = ipv4_hash_args & ~BCM_HASH_FIELD_PROTOCOL;
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchHashIP4OuterField, ipv4_hash_args), "");
    return BCM_E_NONE;
}

/* Example of configuring IFA 2.0 header fields */
int cint_set_IFA2_protocol(int unit)
{
    int rv = BCM_E_NONE;
...
int cint_config_IFA2_header(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_ifa_header_create(unit,0,0,&header), "");
    return BCM_E_NONE;
}
...
int cint_field_IFA2_datapath_initiator_main(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_create(unit,0,&action_info,&cint_field_IFA_datapath_ingress_act_container_1b), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &cint_field_IFA_datapath_ingress_fg_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_create(unit, 0, bcmFieldStageIngressPMF1, &context_info, &cint_field_IFA_datapath_ingress_context_id_ipv4), "");

    /* Add flow_id in field */
    BCM_IF_ERROR_RETURN_MSG(appl_dnx_field_flow_id_init(unit,cint_field_IFA_datapath_ingress_context_id_ipv4), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_field_IFA_datapath_ingress_fg_id, cint_field_IFA_datapath_ingress_context_id_ipv4, &attach_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit,0,&qual_info,&cint_field_IFA_datapath_ingress_qual_container_1b), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &cint_field_IFA_datapath_ingress_fg_id_ipmf3), "");
    printf("Created TCAM field group (%d) in iPMF1 \n", cint_field_IFA_datapath_ingress_fg_id);
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_create(unit, 0, bcmFieldStageIngressPMF3, &context_info, &cint_field_IFA_datapath_ingress_context_id_ipmf3), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_field_IFA_datapath_ingress_fg_id_ipmf3, cint_field_IFA_datapath_ingress_context_id_ipmf3, &attach_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, cint_field_IFA_datapath_ingress_fg_id, &ent_info, &cint_field_ip_snoop_ent_id), "");
...
int cint_instru_ifa2_initiator(int unit)
{
    ...
    if (*dnxc_data_get (unit,"instru","ifa","ifa2_is_supported",NULL)==0)
    {
      /* devices require encapsulation size estimation */
        printf("Error unavailable, IFA2 is not supported on device\n");
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_ifa2_node_parameters_config(unit), "in configuration IFA node parameters");

    /* Configure IFA 2.0 header Parameters */
    BCM_IF_ERROR_RETURN_MSG(cint_config_IFA2_header(unit), "in configuration IFA node parameters");

    /* Configure Field */
    BCM_IF_ERROR_RETURN_MSG(cint_field_IFA2_datapath_initiator_main(unit), "in adding iPMF1 rule");
...
int cint_field_IFA2_datapath_intermediate_main(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_ifa2_hash_configuration(unit, cint_field_IFA_datapath_ingress_context_id_ipv4), "");
    ...
    dest_char = &(action_info.name[0]);
    sal_strncpy_s(dest_char, "IFA_IPT_c_1b", sizeof(action_info.name));
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_create(unit,0,&action_info,&cint_field_IFA_datapath_ingress_act_IPTCommand_1b), "");
    ...
    ent_info.nof_entry_actions = 2;
    ent_info.entry_action[0].type = bcmFieldActionIPTProfile;
    ent_info.entry_action[0].value[0] = IPT_PROFILE_IFA2_MD_INSERTION;
    ent_info.entry_action[1].type = cint_field_IFA_datapath_ingress_act_container_1b;
    ent_info.entry_action[1].value[0] = 1;
...
int cint_instru_ifa2_intermediate(int unit)
{
    ...
    /* Configure Initiator Node Parameters */
    BCM_IF_ERROR_RETURN_MSG(cint_ifa2_node_parameters_config(unit), "");
    /* Configure IFA 2.0 header Parameters */
    BCM_IF_ERROR_RETURN_MSG(cint_config_IFA2_header(unit), "in configuration IFA node parameters");

    /* Configure Field */
    BCM_IF_ERROR_RETURN_MSG(cint_field_IFA2_datapath_intermediate_main(unit), "in adding iPMF1 rule");
...
*
*  Additional configuration for Estimated BTR:
*    Additional configuration is required to set the estimated_btr (bytes to remove) for some devices.
*    In the termination node we remove all IFA headers and the metadata stack. Since the
*    size of the metadata stack is not known, CurrentLength field of the metadata header
*    is read. The value is multiplied by 4 (CurrentLength indicates the length in 4 byte units)
*    and then written into estimated_btr signal.
*    This is done in ePMF.
*    - Additional entry with qualifier as Forward port is added to the main field group,
*      to trap the forwarded copy that is going to be stripped from the metadata stack.
*    - Additional Direct Extract field group is defined and attached to the ePMF context
*      with 2 qualifiers and 1 action.
*       - Qualifiers:
*           - Place holder of 2 bits, to multiply the read value by 4
*           - Extract CurrentLength field (size 8bits)
*       - Action:
*           - Set estimated_btr - bcmFieldActionEstimatedStartPacketStrip
*
*    Configuration is done by setting estimate_btr = 1,
*    The input parameter "out_port" is required for this configuration only
*/
int cint_field_IFA2_datapath_termination_main(int unit, int out_port)
{
	bcm_field_qualify_t cint_field_IFA_2_estimate_encap_size_multiply_4;
	bcm_field_qualify_t cint_field_IFA_2_estimate_encap_size_current_length;
	bcm_field_action_t cint_field_IFA_datapath_egress_estimate_encap_size;
	bcm_field_group_t cint_field_IFA_datapath_egress_estimate_encap_size_epmf=2;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_info_get(unit,action_type,stage,&action_info_get_size), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_field_ip_snoop_set(unit, cpu_port, 1, 0, 0, &snoop_trap_gport_id, &mirror_dest_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_field_IFA_datapath_ingress_fg_id, context_id, &attach_info), "");
    printf("FG (%d) Attached to context: %d \n",id,context_id);
    cint_field_IFA_datapath_ipmf1_context_id_ipv4 = context_id;
    ...
    /* *********************** */
      ...
      */
      ...
      BCM_IF_ERROR_RETURN_MSG(bcm_field_ace_entry_add(unit, 0, ace_format_id, &ace_entry_info, &cint_field_IFA_datapath_ingress_ace_entry_handle), "");
      ...
      {
          fg_info.nof_actions = 2;
          fg_info.action_types[1] = bcmFieldActionInvalidNext;
      }

      dest_char = &(fg_info.name[0]);
      sal_strncpy_s(dest_char, "IFA_datapath_e_init", sizeof(fg_info.name));
      BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &cint_field_IFA_datapath_ingress_node_egress_fg_id), "");
      ...
      BCM_IF_ERROR_RETURN_MSG(bcm_field_context_create(unit, 0, fg_info.stage, &context_info, &cint_field_IFA_datapath_ingress_node_egress_context_id_ipv4), "");
      ...
      BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_field_IFA_datapath_ingress_node_egress_fg_id, cint_field_IFA_datapath_ingress_node_egress_context_id_ipv4, &attach_info), "");
      ...
      {
          ent_info.nof_entry_actions = 2;
          ent_info.entry_action[1].type = bcmFieldActionInvalidNext;
          ent_info.entry_action[1].value[0] = 1;
      }
      BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, cint_field_IFA_datapath_ingress_node_egress_fg_id, &ent_info, &cint_field_IFA_datapath_ingress_node_egress_entry_handle), "");
      ...
      if(estimate_encap_size==1)
      {
        ...
        /**The value should be encoded as GPORT_SYTEM_PORT*/
        ent_info.entry_qual[0].value[0] = system_out_port;
        ent_info.entry_qual[0].mask[0] = -1;
        ...
        bcm_field_entry_info_t_init(&ent_info);
        ent_info.priority = priority_initiator+1;
        ...
        bcm_field_qualifier_info_create_t_init(&qual_info);
        qual_info.size=2; /* Shift CurrentLength value by 2 bits to multiply it by 4*/
        dest_char = &(qual_info.name[0]);
        sal_strncpy_s(dest_char, "IFA_2_estimate_encap_size_multiply_4", sizeof(qual_info.name));
        BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit,0,&qual_info,&cint_field_IFA_2_estimate_encap_size_multiply_4), "");

        bcm_field_qualifier_info_create_t_init(&qual_info);
        qual_info.size=8; /* CurrentLength field size */
        dest_char = &(qual_info.name[0]);
        sal_strncpy_s(dest_char, "IFA_2_estimate_encap_size_q_current_length", sizeof(qual_info.name));
        BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit,0,&qual_info,&cint_field_IFA_2_estimate_encap_size_current_length), "");

        /* Direct Extraction group for copying current length into estimated_btr */
        bcm_field_group_info_t_init(&fg_info);
        fg_info.fg_type = bcmFieldGroupTypeDirectExtraction;
        fg_info.stage = bcmFieldStageEgress;
        ...
        /* Set quals */
        fg_info.nof_quals = 2;
        fg_info.qual_types[0] = cint_field_IFA_2_estimate_encap_size_multiply_4;
        fg_info.qual_types[1] = cint_field_IFA_2_estimate_encap_size_current_length;
        ...
        dest_char = &(fg_info.name[0]);
        sal_strncpy_s(dest_char, "IFA_2_estimate_encap_size_fg", sizeof(fg_info.name));
        BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &cint_field_IFA_datapath_egress_estimate_encap_size_epmf), "");
        ...
        attach_info.key_info.qual_types[1] = fg_info.qual_types[1];
        attach_info.key_info.qual_info[1].input_type = bcmFieldInputTypeLayerAbsolute;
        /* IFA2 Metadata is layer 3, CurentLength is at offset 24bits */
        attach_info.key_info.qual_info[1].input_arg = 3;
        attach_info.key_info.qual_info[1].offset = 24;

        BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_field_IFA_datapath_egress_estimate_encap_size_epmf, cint_field_IFA_datapath_ingress_node_egress_context_id_ipv4, &attach_info), "");
      }
...
/* Example of deleting the field configuration for IFA 2.0 transit node.*/
int cint_field_destroy(int unit)
{
    bcm_field_presel_entry_data_t p_data;
    bcm_field_presel_entry_id_t p_id;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_detach(unit, cint_field_IFA_datapath_ingress_fg_id, cint_field_IFA_datapath_ipmf1_context_id_ipv4), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_delete(unit,cint_field_IFA_datapath_ingress_fg_id), "");
...
int cint_ifa2_termination_with_md_recycle_trap(int unit, int collector_port, int rcy_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_create(unit, flags, bcmRxTrapUserDefine, &ingress_trap_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_create(unit, trap_config.flags, bcmRxTrapEgTxIfaEgressMetadata, &trap_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_gport_control_set(unit, rcy_port,0,
 ...
 *    - For estimated_btr - ePMF will also set estimated bytes to remove
 */

int cint_instru_ifa2_termination(int unit, int out_port)
{
    ...
    if (*dnxc_data_get (unit,"instru","ifa","ifa2_is_supported",NULL)==0)
    {
      /* devices require encapsulation size estimation */
        rv = BCM_E_UNAVAIL;
        printf("Error (%d), IFA2 is not supported on device\n", rv);
        return rv;
    }

    if (*dnxc_data_get (unit,"instru","ipt","advanced_ipt",NULL))
    {
      /* require ipt configuration */
      BCM_IF_ERROR_RETURN_MSG(cint_instru_ipt_init(unit), "in ePMF rule creation");
    }
    ...
    /* Configure Field */
    BCM_IF_ERROR_RETURN_MSG(cint_field_IFA2_datapath_termination_main(unit,out_port), "in adding iPMF1 rule");
...
*             2- Set Tail Edit Profile = IPT_PROFILE_IFA2_MD_INSERTION
*
*/
int cint_field_IFA2_datapath_termination_with_md(int unit, int out_port)
{
    ...
    BCM_GPORT_SYSTEM_PORT_ID_SET(system_out_port, out_port);
    BCM_IF_ERROR_RETURN_MSG(cint_field_ip_snoop_set(unit, system_out_port, 1, 0, 0, &snoop_trap_gport_id, &mirror_dest_id), "");
    ...
    attach_info.key_info.qual_info[0].input_type = bcmFieldInputTypeMetaData;
    bcm_field_qualifier_info_get_t_init(& qual_info_get_offset);
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_info_get(unit,bcmFieldQualifyContainer,bcmFieldStageIngressPMF3,&qual_info_get_offset), "");
    attach_info.key_info.qual_info[0].offset = qual_info_get_offset.offset;
...
int cint_field_IFA2_egress_datapath_group(int unit,int IFA2_IP_protocol)
{
    ...
    */
    ...
    ace_entry_info.entry_action[1].value[0] = IPT_PROFILE_IFA2_MD_INSERTION;

    BCM_IF_ERROR_RETURN_MSG(bcm_field_ace_entry_add(unit, 0, ace_format_id, &ace_entry_info, &cint_field_IFA_datapath_egress_ace_entry_handle), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit,0,&qual_info,&cint_field_IFA_datapath_egress_snooped_qual), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &cint_field_IFA_datapath_egress_fg_id), "");
    ...
    dest_char = &(context_info.name[0]);
    sal_strncpy_s(dest_char, "IFA_IPv4", sizeof(context_info.name));
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_create(unit, 0, fg_info.stage, &context_info, &cint_field_IFA_datapath_egress_context_id_ipv4), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_field_IFA_datapath_egress_fg_id, cint_field_IFA_datapath_egress_context_id_ipv4, &attach_info), "");
 ...
 */
int cint_field_IFA2_datapath_egress_entry_add(int unit, bcm_field_entry_t *entry_handle, int out_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, cint_field_IFA_datapath_egress_fg_id, &ent_info, entry_handle), "");
    ...
    if(estimate_encap_size==1)
    {
      ...
      /**The value should be encoded as GPORT_SYTEM_PORT*/
       ...
       BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, cint_field_IFA_datapath_egress_fg_id, &ent_info, &cint_field_IFA_datapath_ingress_node_egress_entry_handle), "");
       ...
       BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_field_IFA_datapath_egress_estimate_encap_size_epmf, cint_field_IFA_datapath_egress_context_id_ipv4, &attach_info), "");
    }
 ...
 *             2- Set Tail Edit Profile = IPT_PROFILE_IFA2_MD_INSERTION
 *
 *  Additional configuration for estimated_btr:
 *    Additional configuration is required to set the estimated_btr (bytes to remove) for some devices.
 *    See cint_field_IFA2_datapath_termination_main documentation for details.
 *
        ...
        int collector_port)
{
    ...
    /* Configure field */
    BCM_IF_ERROR_RETURN_MSG(cint_field_IFA2_egress_datapath_group(unit,DEFAULT_IP_PROTOCOL), "");

    /* Set ePMF to trap IFA packet */
    BCM_IF_ERROR_RETURN_MSG(cint_field_IFA2_datapath_egress_entry_add(unit,&entry_handle, out_port), "");

    /* Ports configuration */
    BCM_IF_ERROR_RETURN_MSG(cint_ifa2_termination_with_md_recycle_trap(unit,collector_port,out_port), "");
    return BCM_E_NONE;
}
...
int cint_instru_ifa2_termination_with_md(int unit, int in_port, int recycle_port_0, int recycle_port_1, int out_port)
{
    ...
    if (*dnxc_data_get (unit,"instru","ipt","advanced_ipt",NULL))
    {
      /* advance IPT require additional configuration */
      BCM_IF_ERROR_RETURN_MSG(cint_instru_ipt_init(unit), "in ePMF rule creation");
    }
    ...
    /* Configure Initiator Node Parameters */
    BCM_IF_ERROR_RETURN_MSG(cint_set_IFA2_protocol(unit), "in configuration of IFA protocol.");

    /* Configure Field */
    BCM_IF_ERROR_RETURN_MSG(cint_field_IFA2_datapath_termination_with_md(unit,out_port), "in adding IFA2 termination rule");

    /* Configure recycling of ifa packet and forwarding to collector */
    BCM_IF_ERROR_RETURN_MSG(cint_instru_ifa2_egress_recycle(unit, out_port, collector_port), "in cint_instru_ifa2_egress_recycle");
    return BCM_E_NONE;
}

/*
 * When implementing IFA, the user required to add next-header in addition to Src-IP and Dst-IP for LB keys using PMF HASH mechanism for ALL FLOWS
 * Initiator node doesn't apply because packets come in to the initiator node without IFA 2 headers and the headers are only added in the egress pipeline.
 * */
int cint_ifa2_hash_configuration(int unit, bcm_field_context_t context_id)
{
    bcm_field_context_hash_info_t hash_info;
    bcm_field_qualifier_info_create_t qual_info;
    bcm_field_qualifier_info_get_t qual_info_get;
    bcm_field_qualify_t user_defined_qual;
    void *dest_char;

    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_info_get(unit, bcmFieldQualifyIp4Protocol, bcmFieldStageIngressPMF1, &qual_info_get), "");

    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = qual_info_get.size;
    dest_char = &(qual_info.name[0]);
    sal_strncpy_s(dest_char, "IFA_NEXT_HEADER", sizeof(qual_info.name));
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &qual_info, &user_defined_qual), "");

    bcm_field_context_hash_info_t_init(&hash_info);
    hash_info.hash_function = cint_field_IFA_datapath_hash_function;

    /*
     * All LB-keys should be updated by PMF hash, So all protocols could use this hashing
     */
    hash_info.hash_config.function_select = bcmFieldContextHashActionValueAugmentCrc;
    hash_info.hash_config.action_key = cint_field_IFA_datapath_action_keys[0];
    hash_info.hash_config.crc_select = cint_field_IFA_datapath_crc_selects[0];
    hash_info.nof_additional_hash_config = 4;
    hash_info.additional_hash_config[0].function_select = bcmFieldContextHashActionValueAugmentCrc;
    hash_info.additional_hash_config[0].crc_select = cint_field_IFA_datapath_crc_selects[1];
    hash_info.additional_hash_config[0].action_key = cint_field_IFA_datapath_action_keys[1];
    hash_info.additional_hash_config[1].function_select = bcmFieldContextHashActionValueAugmentCrc;
    hash_info.additional_hash_config[1].crc_select = cint_field_IFA_datapath_crc_selects[2];
    hash_info.additional_hash_config[1].action_key = cint_field_IFA_datapath_action_keys[2];
    hash_info.additional_hash_config[2].function_select = bcmFieldContextHashActionValueAugmentCrc;
    hash_info.additional_hash_config[2].crc_select = cint_field_IFA_datapath_crc_selects[3];
    hash_info.additional_hash_config[2].action_key = cint_field_IFA_datapath_action_keys[3];
    hash_info.additional_hash_config[3].function_select = bcmFieldContextHashActionValueAugmentCrc;
    hash_info.additional_hash_config[3].crc_select = cint_field_IFA_datapath_crc_selects[4];
    hash_info.additional_hash_config[3].action_key = cint_field_IFA_datapath_action_keys[4];

    hash_info.compression = TRUE;
    hash_info.key_info.nof_quals = 1;
    hash_info.key_info.qual_types[0] = user_defined_qual;
    /*
     * Since IPv4 header's size isn't fixed, we'll extract IFA next header by going back 3B from the beginning next layer
     */
    hash_info.key_info.qual_info[0].input_type = bcmFieldInputTypeLayerAbsolute;
    hash_info.key_info.qual_info[0].input_arg = 2;
    hash_info.key_info.qual_info[0].offset = -24;

    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_hash_create(unit, 0, bcmFieldStageIngressPMF1, context_id, &hash_info), "");
    return BCM_E_NONE;
}
