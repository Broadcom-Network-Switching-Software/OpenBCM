 ...
 *--------------
 * Test Scenario - start
  cint ../../../../src/examples/dnx/instru/cint_instru_eventor.c
  cint
   ...
   int header_length)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_context_set(unit, flags, context_id, bcmEventorContextTypeRx, &context_conf_set), "");
    
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_context_get(unit, flags, context_id, bcmEventorContextTypeRx, &context_conf_get), "");
    ...
    */
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_builder_set(unit, flags, builder_id, &builder_conf_set), "");

    uint8 header_data_get[128];
    builder_conf_get.header_data = header_data_get;
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_builder_get(unit, flags, builder_id, &builder_conf_get), "");
        ...
        int offset, int push, int nof_operations)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_memory_access_info(unit, 0, mem_id, block_instance_number, 0, 0, &operations[0].access_info), "");

    diag_mem_info_get(unit, "CGM_INSTRUMENTATION_MEMORY[0].CGM0", &mem_id, &block_instance_id, &block_instance_number, &array_index);
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_memory_access_info(unit, 0, mem_id, block_instance_number, 0, 1, &operations[1].access_info), "");
    diag_mem_info_get(unit, "CGM_INSTRUMENTATION_MEMORY[0].CGM0", &mem_id, &block_instance_id, &block_instance_number, &array_index);
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_memory_access_info(unit, 0, mem_id, block_instance_number, 0, 2, &operations[2].access_info), "");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_info_get(unit, "TDU_WRITE_REQUEST_PATH_COUNTERS.TDU0", &reg_id, &block_instance_id, &block_instance_number, &array_index), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_register_access_info(unit, 0, reg_id, 0, 0, &operations[3].access_info), "");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_info_get(unit, "TDU_READ_REQUEST_PATH_COUNTERS.TDU0", &reg_id, &block_instance_id, &block_instance_number, &array_index), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_register_access_info(unit, 0, reg_id , 0, 0, &operations[4].access_info), "");
    diag_mem_info_get(unit, "CGM_INSTRUMENTATION_MEMORY[0].CGM0", &mem_id, &block_instance_id, &block_instance_number, &array_index);
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_memory_access_info(unit, 0, mem_id, block_instance_number, 0, 0, &operations[5].access_info), "");
    diag_mem_info_get(unit, "CGM_INSTRUMENTATION_MEMORY[0].CGM0", &mem_id, &block_instance_id, &block_instance_number, &array_index);
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_memory_access_info(unit, 0, mem_id, block_instance_number, 0, 1, &operations[6].access_info), "");
    diag_mem_info_get(unit, "CGM_INSTRUMENTATION_MEMORY[0].CGM0", &mem_id, &block_instance_id, &block_instance_number, &array_index);
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_memory_access_info(unit, 0, mem_id, block_instance_number, 0, 2, &operations[7].access_info), "");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_info_get(unit, "TDU_WRITE_REQUEST_PATH_COUNTERS.TDU0", &reg_id, &block_instance_id, &block_instance_number, &array_index), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_register_access_info(unit, 0, reg_id, 1, 0, &operations[8].access_info), "");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_info_get(unit, "TDU_READ_REQUEST_PATH_COUNTERS.TDU0", &reg_id, &block_instance_id, &block_instance_number, &array_index), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_register_access_info(unit, 0, reg_id,  1, 0, &operations[9].access_info), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_context_set(unit, flags, context_id, bcmEventorContextTypeTx, &context_conf_set), "");
        ...
        int offset, int push, int nof_operations)
{
    ...
    for (operation_index=0;operation_index<nof_operations; operation_index++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_memory_access_info(unit, 0, mem_id, block_instance_number, 0, array_index, &operations[operation_index].access_info), "");
        ...
        int nof_events_to_aggregate, int offset, int nof_operations)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_tx_context_set(unit, context_id, bank1, bank2, buffer_size, buffer1_start, buffer2_start, program1_start, program2_start, builders, nof_events_to_aggregate, offset, nof_operations), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_context_get(unit, flags, context_id, bcmEventorContextTypeTx, &context_conf_get), "");
    /*
    * Compare context set and get fields:
    */
...
cint_instru_eventor_rx_eventor_set(int unit, int rx_context_id, int buffer_size, int nof_events_to_aggregate, int in_eventor_port)
{
    ...
    /* Get Eventor PP port */
    BCM_IF_ERROR_RETURN_MSG(bcm_port_get(unit, in_eventor_port, &flags, &int_inf, &mapping_info), "");
    /* in PTCH2, In_PP_Port is 10 bits [9:0]*/
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_context_set(unit, flags, rx_context_id, bcmEventorContextTypeRx, &context_conf), "");
...
cint_instru_eventor_rx_dma_mode_set(int unit, int rx_context_id, int buffer_size, int nof_events_to_aggregate, int in_eventor_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(diag_reg_info_get(unit, "ECI_VERSION_REGISTER", &reg_id, &block_instance_id, &block_instance_number, &array_index), "");
    //diag_reg_info_get(unit, "ECI_SCRATCH_PAD_0", &reg_id, &block_instance_id, &block_instance_number, &array_index);
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_register_access_info(unit, BCM_INSTRU_ACCESS_FLAG_IS_WRITE, reg_id, 0, 0, &operation[0].access_info), "");

    //diag_reg_info_get(unit, "ECI_SCRATCH_PAD_1", &reg_id, &block_instance_id, &block_instance_number, &array_index);
    BCM_IF_ERROR_RETURN_MSG(diag_reg_info_get(unit, "ECI_TEST_REGISTER", &reg_id, &block_instance_id, &block_instance_number, &array_index), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_register_access_info(unit, BCM_INSTRU_ACCESS_FLAG_IS_WRITE, reg_id, 0, 0, &operation[1].access_info), "");
    /* init eventor RX context */
    /*
     * 1. Can simply use different pair of banks per context (but since there are 8 banks, there can be only 4 context ...)
     * 2. Can use same pair of banks but with different offset (bank size is 13K bytes)
     ...
     * Set eventor Rx context (Rx buffers)
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_context_set(unit, flags, rx_context_id, bcmEventorContextTypeRxDma, &context_conf), "");
        ...
        int enable0)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_synced_triggers_enable_set(unit, flags, nof_sources, sources, enable), "");
...
/*
 *  set Eventor builder for tx eventor shell testing:
 *
 */
int
cint_instru_eventor_tx_builder_set_for_shell(
        int unit,
        int builder_id,
        int thresh_size,
        int thresh_time)
...
int
cint_instru_eventor_tx_context_set_for_shell(
        int unit, int context_id,
        int bank1, int bank2,
        int buffer_size, int buffer1_start, int buffer2_start,
        int program1_start, int program2_start, int builders,
        int generate_eventor_header, int push, int use_little_endian_packet_words, int nof_operations)
{
    uint32 flags = 0;
    bcm_instru_eventor_operation_t operations[30];
    int reg_id;
    ...
    int mem_id;
    int operation_index;
    int data_word_offset_in_buffer = 0;

    printf("cint_instru_eventor_tx_context_set_for_shell: context_id = %d, buffer_size = %d nof_operations = %d\n", context_id, buffer_size,nof_operations);

    /* get src register information for descriptor creation */
    BCM_IF_ERROR_RETURN_MSG(diag_reg_info_get(unit, "ECI_SCRATCH_PAD_0", &reg_id, &block_instance_id, &block_instance_number, &array_index), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_register_access_info(unit, 0, reg_id, 0, 0, &operations[0].access_info), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_register_access_info(unit, 0, reg_id, 0, 0, &operations[1].access_info), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_register_access_info(unit, 0, reg_id, 0, 0, &operations[2].access_info), "");
    BCM_IF_ERROR_RETURN_MSG(diag_reg_info_get(unit, "ECI_SCRATCH_PAD_1", &reg_id, &block_instance_id, &block_instance_number, &array_index), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_get_register_access_info(unit, 0, reg_id, 0, 0, &operations[3].access_info), "");

    context_conf_set.flags = push;
    if (use_little_endian_packet_words)
    {
        context_conf_set.flags |= BCM_INSTRU_EVENTOR_CONTEXT_CONF_WORD_LITTLE_ENDIAN;
    }
    if (generate_eventor_header)
    {
        data_word_offset_in_buffer = 1;
    }
    else
    {
        context_conf_set.flags |= BCM_INSTRU_EVENTOR_CONTEXT_CONF_DONT_GENERATE_EVENTOR_HEADER;
    }
    ...
    operations[0].flags = 0;
    operations[0].buf_offset = data_word_offset_in_buffer;
    for (operation_index=1;operation_index<nof_operations; operation_index++)
    {
        operations[operation_index].flags = 0;
        operations[operation_index].buf_offset = operations[operation_index-1].buf_offset + operations[operation_index-1].access_info.entry_size;
...
/*
 * This functions configures the eventor for shell application.
 */
int
cint_instru_eventor_rx_context_set_for_shell(
        int unit, int rx_context_id, int bank0, int bank1, int buffer_size,
        int buffer1_start, int buffer2_start, int program1_start, int program2_start, int use_little_endian_packet_words, int nof_operations)
{
    uint32 flags = 0;
    uint32 header_length;
    bcm_instru_eventor_context_conf_t context_conf = {0};
    ...
    printf("%s: rx_context_id = %d, buffer_size = %d,  \n", proc_name, rx_context_id, buffer_size);
    ...
    if (use_little_endian_packet_words)
    {
        context_conf.flags |= BCM_INSTRU_EVENTOR_CONTEXT_CONF_WORD_LITTLE_ENDIAN;
    }

    context_conf.bank1 = bank0;
    context_conf.bank2 = bank1;
    printf("%s: bank1 = %d, bank2 = %d\n", proc_name, context_conf.bank1, context_conf.bank2);

    context_conf.buffer_size = buffer_size;
    context_conf.buffer1_start = buffer1_start;
    context_conf.buffer2_start = buffer2_start;

    context_conf.program1_start = program1_start;
    context_conf.program2_start = program2_start;
    context_conf.nof_operations = 2;
    context_conf.operations = &operation;
    operation[0].buf_offset = 0;
    operation[1].buf_offset = 1;
