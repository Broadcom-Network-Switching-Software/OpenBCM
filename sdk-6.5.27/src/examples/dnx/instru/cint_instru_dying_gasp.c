        ...
        int thresh_time)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_get(unit, cpu_port, &flags, &int_inf, &mapping_info), "");
    flags = 0;
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_builder_set(unit, flags, builder_id, &builder_conf), "");

    BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_vlan_translation(unit, out_vlan_port_edit_prfoile, &vlan_port), "");
    BCM_IF_ERROR_RETURN_MSG(cint_multicast_to_all_ports_cfg(unit, ing_mc_id, vlan_port.encap_id), "");
    BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_packet_forward(unit, ing_mc_id), "");
...
int cint_multicast_to_all_ports_cfg(int unit, bcm_multicast_t mc_group_id, bcm_if_t encap)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_create(unit, BCM_MULTICAST_INGRESS_GROUP | BCM_MULTICAST_WITH_ID, &mc_group_id), "(ingress)");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_set(unit, mc_group_id, flags_mc, nof_ports_config, replications), "");
...
int cint_instru_eventor_event_cfg(int unit, bcm_eventor_context_id_t context_id, bcm_eventor_event_id_t event_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_event_id_config_set(unit, 0, event_id,  &event_config), "");
...
int cint_dying_gasp_pkt_gen_config_example(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_tx_init_sram_descriptor_data(unit,nof_operations), "");
    /*Configure eventor context id*/
    BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_tx_context_set(unit,tx_context_id,bank,bank+1,buffer_size,buffer_offset,buffer_offset,program_offset,program_offset,builders,offset,0,nof_operations), "");
    /*Configure eventor build id to generate dying gasp packet*/
    BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_tx_builder_dying_gasp_set(unit, builder_id, builder_thresh, -1), "");
    /*Configure eventor event*/
    BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_event_cfg(unit, tx_context_id, trigger_event_id), "");
    /*Activate eventor block*/
    BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_activate(unit), "");
    return BCM_E_NONE;
}
...
int cint_instru_eventor_tx_manual_dying_gasp_test(int unit, int tx_context_id,int event_id,int nof_events_to_send,int nof_events_in_packet,int nof_operations)
{
    ...
    for(i = 0; i < nof_events_to_send; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_event_generate(unit,0,event_id), "");
...
int cint_packet_manual_generate(int unit)
{
    ...
    int i=0;
    BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_tx_manual_dying_gasp_test(unit,tx_context_id,trigger_event_id,nof_events_to_send,nof_events_in_packet,nof_operations), "");
    for (i = 0; i < 5; i++)
    {
...
int cint_instru_eventor_tx_periodic_dying_gasp_events_test(int unit, int tx_context_id,int event_id,int trigger_source,int source_index,uint64 trigger_period,int nof_events_to_send)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_periodic_trigger_enable_set(unit,flags,nof_sources,trigger_source,enable), "");
    config.nof_triggers = nof_events_to_send;

    COMPILER_64_SET(config.start_time,0,0);
    config.trigger_period = trigger_period;
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_synced_trigger_config_set(unit,flags,trigger_source,&config), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_eventor_generate_periodic_set(unit,flags,&source,&eventor_config), "");
...
int cint_packet_period_generate(int unit)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(cint_instru_eventor_tx_periodic_dying_gasp_events_test(unit,tx_context_id,trigger_event_id,trigger_source,source_index,trigger_period,nof_events_to_send), "");
        sal_sleep(5);
    }
