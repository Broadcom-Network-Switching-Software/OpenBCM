 ...
 * Test Scenario - start
   cd ../../../src/examples/dnx
   cint cint_dnx_qos_l2_port.c
   cint
   dnx_qos_l2_service_init(unit,port_in,port_out);
   dnx_qos_l2_bridge_service(unit);
   dnx_qos_l2_dscp_map(unit, color, is_tagged);
   ...
   dnx_qos_l2_untag_map(unit, default_color, defaut_tc); 
         *  color_idx can be 0 or 1. 0 stands for green pakcet, 1 stands for yellow packet
         *  Send Traffic from port 15 and receive it on port 16:
         *  1. In port 15 tos 0   <-----CrossConnect----->  Out port 16 vid 200 prio 0 cfi $color tos 0
         *  2. In port 15 tos 64  <-----CrossConnect----->  Out port 16 vid 200 prio 2 cfi $color tos 64
         *  3. In port 15 tos 128 <-----CrossConnect----->  Out port 16 vid 200 prio 4 cfi $color tos 128
         *  4. In port 15 tos 192 <-----CrossConnect----->  Out port 16 vid 200 prio 6 cfi $color tos 192
   dnx_qos_l2_port_dft_map(unit,port,dft_color,dft_tc);
...
int dnx_qos_l2_dscp_map(int unit, int color, int is_tagged) {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_create(unit, flags, &qos_l3_ingress_id), "");

    /*set qos profile phb to be used for L2 bridge use L3 mapping*/
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_control_set(unit, qos_l3_ingress_id, BCM_QOS_MAP_PHB, bcmQosMapControlL3L2, 1), "");

    /** allocate opcode for phb*/
    flags = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_PHB | BCM_QOS_MAP_OPCODE;
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_create(unit, flags, &map_opcode), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &ingress_phb_map, qos_l3_ingress_id), "");
    ...
    for (tos = 0; tos < 256; tos++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &ingress_phb_map, map_opcode), "");
    }

    /*set qos profile qos to be used for L2 bridge use L3 mapping*/
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_control_set(unit, qos_l3_ingress_id, BCM_QOS_MAP_REMARK, bcmQosMapControlL3L2, 1), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &ingress_rm_map, qos_l3_ingress_id), "");
    /** remark mapping */
    ...
    for (tos = 0; tos < 256; tos++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &ingress_rm_map, map_opcode), "");
    }

    /*in_vlan_port1 is for untagged packet, in_vlan_port2 is for tagged */
    if (!is_tagged)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_port_map_set(unit, in_vlan_port1.vlan_port_id, qos_l3_ingress_id, -1), "");
    }
    else
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_port_map_set(unit, in_vlan_port2.vlan_port_id, qos_l3_ingress_id, -1), "");
    }
    return BCM_E_NONE;
}
...
int dnx_qos_l2_untag_map(int unit, int dft_color, int dft_tc) {
    ...
    /**  allocate profile */
    flags = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_PHB | BCM_QOS_MAP_REMARK;
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_create(unit, flags,&qos_l2_ingress_untag_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &ingress_phb_map, qos_l2_ingress_untag_id), "");

    /**  qos remark mapping */
    flags = BCM_QOS_MAP_L2 | BCM_QOS_MAP_L2_UNTAGGED | BCM_QOS_MAP_REMARK;
    bcm_qos_map_t_init(&ingress_rm_map);
    ingress_rm_map.remark_int_pri = dft_tc;
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &ingress_rm_map, qos_l2_ingress_untag_id), "");

    /*set profile to port*/
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_port_map_set(unit, in_vlan_port1.vlan_port_id, qos_l2_ingress_untag_id, -1), "");
...
int dnx_qos_l2_port_dft_map(int unit, int port, int dft_color, int dft_tc) {
    ...
    /**  allocate profile */
    flags = BCM_QOS_MAP_INGRESS | BCM_QOS_MAP_PHB | BCM_QOS_MAP_REMARK;
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_create(unit, flags,&qos_ingress_default_id), "");
    ...
    ingress_phb_map.int_pri = dft_tc;
    ingress_phb_map.color = dft_color;
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &ingress_phb_map, qos_ingress_default_id), "");

    /**  remark mapping */
    flags = BCM_QOS_MAP_PORT | BCM_QOS_MAP_REMARK;
    bcm_qos_map_t_init(&ingress_rm_map);
    ingress_rm_map.remark_int_pri = dft_tc;
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_add(unit, flags, &ingress_rm_map, qos_ingress_default_id), "");

    /*set profile to port*/
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_port_map_set(unit, port, qos_ingress_default_id, -1), "");

    /*set port qos model to select port phb*/
    BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit, port, bcmPortControlIngressQosModelPhb, bcmQosIngressModelStuck), "");

    /*set port qos model to select port qos*/
    BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit, port, bcmPortControlIngressQosModelRemark, bcmQosIngressModelStuck), "");
...
int dnx_qos_l2_bridge_service(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(port_tpid_class_add(unit, in_vlan_port2.port, s_tpid, BCM_PORT_TPID_CLASS_TPID_INVALID, 4), "");

    /* create inLIF for untagged service */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &in_vlan_port1), "lif_create failed!");
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, up_ovlan, in_vlan_port1.port, 0), "");
    in_gport1 = in_vlan_port1.vlan_port_id;
    printf("in_gport1 = 0x%08X %d\n", in_gport1, in_gport1);
    
    /* create inLIF for tagged service */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &in_vlan_port2), "lif_create failed!");
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, up_ovlan, in_vlan_port2.port, 0), "");
    in_gport2 = in_vlan_port2.vlan_port_id;
    printf("in_gport2 = 0x%08X %d\n", in_gport2, in_gport2);
    
    /* create outLIF for untagged service */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &out_vlan_port1), "lif_create failed!");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_create(unit, &out_vlan_port2), "lif_create failed!");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_cross_connect_add(unit, &gports_untag), "");
    ...
    /* cross connect two lifs for tagged service */
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_cross_connect_add(unit, &gports_tag), "");
...
int dnx_qos_l2_service_cleanup(int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_cross_connect_delete(unit, &gports_untag), "");
    
    /* Delete the cross connected LIFs for tagged service */
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_cross_connect_delete(unit, &gports_tag), "");

    /* Delete inLIF for untagged service */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, in_vlan_port1.vlan_port_id), "");
    
    /* Delete outLIF for untagged service */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, out_vlan_port1.vlan_port_id), "");
    
    /* Delete inLIF for tagged service */
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, in_vlan_port2.vlan_port_id), "");
    
    /* Delete outLIF for tagged service*/
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_destroy(unit, out_vlan_port2.vlan_port_id), "");
...
int dnx_qos_map_cleanup(int unit)
{
    ...
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_destroy(unit, qos_l2_map_phb_code), "");
        qos_l2_map_phb_code = -1;
    }
    if (qos_l2_map_remark_code != -1)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_destroy(unit, qos_l2_map_remark_code), "");
        qos_l2_map_remark_code = -1;
    }
    if (qos_l3_map_phb_code != -1)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_destroy(unit, qos_l3_map_phb_code), "");
        qos_l3_map_phb_code = -1;
    }
    if (qos_l3_map_remark_code != -1)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_destroy(unit, qos_l3_map_remark_code), "");
        qos_l3_map_remark_code = -1;
    }
    if(qos_l2_ingress_id != -1)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_destroy(unit, qos_l2_ingress_id), "");
        qos_l2_ingress_id = -1;
    }
    if(qos_l3_ingress_id != -1)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_destroy(unit, qos_l3_ingress_id), "");
        qos_l3_ingress_id = -1;
    }
    if (qos_ingress_default_id != -1)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_qos_map_destroy(unit, qos_ingress_default_id), "");
        qos_ingress_default_id = -1;
    }

    /*set port qos model to disable port phb*/
    BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit, in_vlan_port1.port, bcmPortControlIngressQosModelPhb, bcmQosIngressModelShortpipe), "");

    /*set port qos model to disable port qos*/
    BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit, in_vlan_port1.port, bcmPortControlIngressQosModelRemark, bcmQosIngressModelShortpipe), "");
...
dnx_qos_l2_vlan_pcp_dei_update(int unit, bcm_port_t out_port)
{
    ...
    /** Connect it to VPN 1 */
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, 1, vlan_port.vlan_port_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_qos_port_map_set(unit, vlan_port.vlan_port_id, -1, qos_egress), "in L2 egress");
