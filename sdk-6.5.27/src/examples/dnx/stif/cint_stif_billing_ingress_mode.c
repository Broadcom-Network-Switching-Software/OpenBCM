    ...
    int unit)
{
    ...
    if (nof_cores > 1)
    {
        ...
        source.src_type = bcmStatStifSourceIngressEnqueue;
        BCM_IF_ERROR_RETURN_MSG(bcm_stat_stif_source_mapping_set(unit, flags, source, stif_logical_port_3), "");
        /** step 1.2 connect IngressDequeue source */
        /** core 0 */
        source.core = 0;
        source.src_type = bcmStatStifSourceIngressDequeue;
        BCM_IF_ERROR_RETURN_MSG(bcm_stat_stif_source_mapping_set(unit, flags, source, stif_logical_port_2), "");
    }
    /** core 1 */
    source.core = nof_cores - 1;
    source.src_type = bcmStatStifSourceIngressDequeue;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_filter_group_set(unit, BCM_STAT_EXTERNAL, key, bcmStatCounterGroupFilterMiscDrop, 1), "");
