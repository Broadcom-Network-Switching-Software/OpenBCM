...
bcm_field_context_t cint_stif_context_id_ipmf1 = -1;

int in_port_core_0 = 0;
int in_port_core_1 = 0;
    ...
    int type_id)
{
    ...
    /*No need to create context if already created*/
    if(cint_stif_context_id_ipmf1 == -1)
    {
        BCM_IF_ERROR_RETURN_MSG(cint_field_always_hit_context_main(unit,cint_stif_presel_id,bcmFieldStageIngressPMF1,&cint_stif_context_id_ipmf1), "Ingress");
    }
    ...
    attach_info.payload_info.action_info[0].priority = BCM_FIELD_ACTION_PRIORITY(0,fg_id);

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, fg_id, cint_stif_context_id_ipmf1, &attach_info), "");
    ...
    int unit)
{
    ...
    int core_id = 0, type_id = 0, command_id[3]={0,1,2}, stat_id[3]={100, 65535, 2048};
    uint32 nof_cores;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_stif_source_mapping_set(unit, flags, source, stif_logical_port_0), "");
    ...
    if (nof_cores > 1)
    {
        ...
        source.src_type = bcmStatStifSourceIngressEnqueue;
        BCM_IF_ERROR_RETURN_MSG(bcm_stat_stif_source_mapping_set(unit, flags, source, stif_logical_port_1), "");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_control_set(unit, flags, bcmStatControlStifFcEnable, 1), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_pkt_size_adjust_select_set(unit, BCM_STAT_EXTERNAL, key, bcmStatPktSizeSelectCounterIngressHeaderTruncate, 1), "");
     ...
     *  reasons */
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_filter_group_set(unit, BCM_STAT_EXTERNAL, key, bcmStatCounterGroupFilterGlobalResourcesDrop, 1), "");
    /** step 4.2: activate filter group Wred drop reasons  */
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_filter_group_set(unit, BCM_STAT_EXTERNAL, key, bcmStatCounterGroupFilterWredDrop , 1), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(stif_billing_generate_pp_stat_object(unit, core_id, in_port_core_0, stat_id[0], command_id[0], type_id), "");
    BCM_IF_ERROR_RETURN_MSG(stif_billing_generate_pp_stat_object(unit, core_id, in_port_core_0, stat_id[1], command_id[1], type_id), "");
    BCM_IF_ERROR_RETURN_MSG(stif_billing_generate_pp_stat_object(unit, core_id, in_port_core_0, stat_id[2], command_id[2], type_id), "");
    if (nof_cores > 1)
    {
        core_id = 1;
        BCM_IF_ERROR_RETURN_MSG(stif_billing_generate_pp_stat_object(unit, core_id, in_port_core_1, stat_id[0], command_id[0], type_id), "");
    }
 ...
 *   error indication.
 * \remark
 *   stif_logical_port_0 and stif_logical_port_2 should be on core 0, and stif_logical_port_1 and stif_logical_port_3 on core 1
 * \see
 *   NONE
 */
int
    ...
    int unit)
{
    ...
    source.core = 0;
    source.src_type = bcmStatStifSourceIngressEnqueue;
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_stif_source_mapping_set(unit, BCM_STAT_SOURCE_MAPPING_FIRST_PORT, source, stif_logical_port_0), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_stif_source_mapping_set(unit, BCM_STAT_SOURCE_MAPPING_SECOND_PORT, source, stif_logical_port_2), "");
    /** core 1 */
    source.core = 1;
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_stif_source_mapping_set(unit, BCM_STAT_SOURCE_MAPPING_FIRST_PORT, source, stif_logical_port_1), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_stif_source_mapping_set(unit, BCM_STAT_SOURCE_MAPPING_SECOND_PORT, source, stif_logical_port_3), "");
