...
cint_elephant_flow_set_probability(int unit, int in_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_instru_gport_control_set(unit, in_port, 0, bcmInstruGportControlTraceProbability, trace_prob * 10), "");
...
cint_elephant_flow_main(int unit, int in_port, int route_out_port, int cpu_port)
{
    ...
    if (!elephant_flow_skip_l3_cfg)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(dnx_basic_example(unit, in_port, route_out_port, cint_elephant_flow_info.kaps_result), "");
    }

    /*
     * Create SNOOP to CPU
     */
    BCM_IF_ERROR_RETURN_MSG(cint_field_ele_flow_utils_mirror_create(unit, cpu_port, &snoop_cmd), "");

    /*
     * Create FG and add flush-machine entries (rule and action)
     */
    snoop_str = 7;
    BCM_IF_ERROR_RETURN_MSG(cint_field_ele_flow_main(unit, cint_elephant_flow_info.fwd_layer_type, snoop_cmd, snoop_str, cint_elephant_flow_info.flush_profile_id), "");
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(cint_field_ele_flow_get_flush_profile_offset( unit,  &age_action_offset), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_indexed_set(unit, control_key, control_info), "bcmSwitchExemFlushProfilePayloadOffset");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_flush_profile_create(unit, BCM_FIELD_FLAG_WITH_ID, &flush_profile_info, &cint_elephant_flow_info.flush_profile_id), "");
    printf("cint_elephant_flow_main: flush profile (%d) is created.\n", cint_elephant_flow_info.flush_profile_id);

    /*
     * Attach the FG to the created Flush profile
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_flush_profile_attach(unit, 0, cint_field_ele_flow_exem_fg_id, cint_elephant_flow_info.flush_profile_id), "");
...
cint_elephant_flow_set_age_period(int unit, int age_scan_period_usec)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_indexed_set(unit, control_key, control_info), "bcmSwitchExemScanPeriod");
...
cint_elephant_flow_delete(int unit, int in_port)
{
   ...
   */
    BCM_IF_ERROR_RETURN_MSG(cint_field_ele_flow_destroy(unit), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_flush_profile_destroy(unit, 0, cint_elephant_flow_info.flush_profile_id), "");
