    ...
    int out_port)
{
     ...
     *  not relevant */
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_gport_add(unit, port, 0, BCM_COSQ_GPORT_LATENCY_AQM_FLOW_ID, &aqm_flow_id_gport), "");
    /** map (egress port + tc) to aqm flow id */
    BCM_GPORT_LOCAL_SET(port,out_port);
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,port,tc,bcmCosqControlLatencyEgressAqmFlowId, BCM_GPORT_LATENCY_AQM_FLOW_ID_GET(aqm_flow_id_gport)), "");
    /** create a latency profile */
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_latency_profile_create(unit, BCM_COSQ_LATENCY_END_TO_END_AQM, &aqm_latency_gport), "");
    /** map L4Q flow id gport to latency profile */
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,aqm_flow_id_gport,-1,bcmCosqControlLatencyEgressAqmFlowIdToProfile, BCM_GPORT_LATENCY_END_TO_END_AQM_LATENCY_PROFILE_GET(aqm_latency_gport)), "");
    ...
    int drop_thresh)
{
    ...
    if(feature_coupled_mode_support)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,aqm_latency_gport,0,bcmCosqControlLatencyEcnEnable, ecn_en), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,aqm_latency_gport,0,bcmCosqControlLatencyEcnProbEnable, ecn_prob_en), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,aqm_latency_gport,0,bcmCosqControlLatencyDropEnable, drop_en), "");
    if (drop_en)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set
    ...
    )
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &lat_fg_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, lat_fg_id, context, &attach_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, lat_fg_id, &ent_info, &lat_ent_id), "");
