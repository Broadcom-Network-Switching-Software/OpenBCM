    ...
    bcm_field_context_t pmf_context)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_gport_get(unit, gport_q, &phy_port, &num_cos_levels, &flags), "");
    ...
    for(profile_idx = 0; profile_idx < quartet; profile_idx++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_latency_profile_create(unit, (BCM_COSQ_LATENCY_PROFILE_ID_EXTENSION | BCM_COSQ_LATENCY_PROFILE_WITH_ID), 
            &gport), "");
        /** set the properties of the profile. by default all actions related to latency are disabled. */
        /** Example activate some of them */
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, gport, 0, bcmCosqControlLatencyTrack, TRUE), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, gport, 0, bcmCosqControlLatencyCounterEnable, TRUE), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,gport,0,bcmCosqControlLatencyTrackDropPacket, TRUE), "");
         /** set bin threshold in jump of 1000 nano-second each, just for the example */
        for(bin_idx = 0; bin_idx < 7; bin_idx++)
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, gport, bin_idx, bcmCosqControlLatencyBinThreshold, (1000 + bin_idx*1000)), "");
        }
        
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,gport,0,bcmCosqControlLatencyDropEnable, TRUE), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,gport,0,bcmCosqControlLatencyEcnEnable, FALSE), "");
        /** set 10000 nano-second drop threshold */
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,gport, 0, bcmCosqControlLatencyDropThreshold, 100000), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_profile_mapping_set(unit, gport_q, 0, 0, &profile_mapping), "");
    /** if 8 queues bundle, map also the second quartet of the bundle */
    if(num_cos_levels == 8)
    {
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_profile_mapping_set(unit, gport_q, 4, 0, &profile_mapping), "");        
    }
    ...
    profile_idx = 0;
    BCM_IF_ERROR_RETURN_MSG(ingress_latency_flow_acl_rule_set(unit, pmf_context, &profile_idx, &profile_idx, bcmFieldQualifyInPort, 0x1FF, &gport_qual, nof_profiles), "");
    ...
    bcm_gport_t gport_q)
{
    ...
    for(profile_idx = 0; profile_idx < quartet; profile_idx++)
    {
         ...
         *  needed for mark cni */
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,gport,0,bcmCosqControlLatencyDropEnable, TRUE), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,gport,0,bcmCosqControlLatencyDropProbEnable, TRUE), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,gport, 0, bcmCosqControlLatencyDropProbBaseThreshold, 0), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,gport, 0, bcmCosqControlLatencyDropThreshold, 8000));

        if(*dnxc_data_get(unit, "latency", "features", "ingress_ecn_probabilistic_mechanism_support", NULL))
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,gport,0,bcmCosqControlLatencyEcnProbEnable, FALSE), "");
        }
    ...
    int nof_profiles)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &ing_lat_fg_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, ing_lat_fg_id, context, &attach_info), "");
    ...
    for (i = 0; i < nof_profiles; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, ing_lat_fg_id, &ent_info, &ing_lat_ent_id), "");
    }
    ...
    bcm_field_context_t pmf_context)
{
    ...
    for(i =  0; i < nof_profiles; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_latency_profile_create(unit, flags, &latency_gport), "");

        /** set the properties of the profile. by default all actions related to latency are disabled. */
        /** Example activate some of them */
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, latency_gport, 0, bcmCosqControlLatencyTrack, TRUE), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit , latency_gport, 0, bcmCosqControlLatencyCounterEnable, TRUE), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,latency_gport,0,bcmCosqControlLatencyDropEnable, latency_drop_enable), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,latency_gport,0,bcmCosqControlLatencyEcnEnable, FALSE));
        /** set 10000 nano-second drop threshold */
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,latency_gport, 0, bcmCosqControlLatencyDropThreshold, 20000), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,latency_gport,0,bcmCosqControlLatencyTrackDropPacket, TRUE), "");
        /** set bin threshold in jump of 1000 nano-second each, just for the example */
        for(bin_idx = 0; bin_idx < 7; bin_idx++)
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,latency_gport, bin_idx, bcmCosqControlLatencyBinThreshold, (1000 + bin_idx*1000)), "");
        }
    ...
    BCM_IF_ERROR_RETURN_MSG(ingress_latency_flow_acl_rule_set(unit, pmf_context, flow_ids, latency_profiles,
    ...
    int unit)
{
    ...
    dest_gport = 0;
    latency_flow = 0;
    BCM_IF_ERROR_RETURN_MSG(cint_get_nof_cores(unit, &nof_cores),"");

    for (core_idx = 0; core_idx < nof_cores; core_idx++)
    {
        BCM_COSQ_GPORT_CORE_SET(gport, core_idx);
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_max_latency_pkts_get(unit, gport, 0, 8, max_latency_pkts, &memory_actual_count), "");
    ...
    bcm_stat_expansion_types_t expansion_type)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_database_create(unit, BCM_STAT_DATABASE_CREATE_WITH_ID, BCM_CORE_ALL, &database.database_id), "");
    /** attach the engines to the database */
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_engine_attach(unit, 0, &database, engineId), "");
   /** configure the counter eviction per engine */
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_eviction_set(unit, 0, &engine, &eviction), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_interface_set(unit, 0, &database, &cnt_interface), "");
    /**  configure the counter set mapping per database */
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_set_mapping_set(unit, BCM_STAT_COUNTER_MAPPING_FULL_SPREAD, &database, &ctr_set_map), "");
    ...
    bcm_field_context_t pmf_context)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_profile_mapping_get(unit, base_voq_gport, 0, 0, &voq_profile_mapping), "");
    ...
    for (profile_id=0; profile_id < 8; profile_id++) {
        if (feature_voq_latency_based_admission_all_profiles_support)
        {
             ...
             *  16 and above */
            BCM_IF_ERROR_RETURN_MSG(bcm_cosq_latency_profile_create(unit, BCM_COSQ_LATENCY_PROFILE_ID_EXTENSION, &latency_profile_gport[profile_id]), "");
        ...
        else
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_cosq_latency_profile_create(unit, BCM_COSQ_LATENCY_PROFILE_WITH_ID, &latency_profile_gport[profile_id]), "");
        }
        /** enable latency measurements */
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, latency_profile_gport[profile_id], 0, bcmCosqControlLatencyCounterEnable, TRUE), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, latency_profile_gport[profile_id], 0, bcmCosqControlLatencyTrack, TRUE), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, latency_profile_gport[profile_id], 0, bcmCosqControlLatencyTrackDropPacket, TRUE), "");
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, latency_profile_gport[profile_id], 0, bcmCosqControlLatencyDropEnable, TRUE), "");

        for (bin_id=0; bin_id < 7; bin_id++) {
            BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, latency_profile_gport[profile_id], bin_id, bcmCosqControlLatencyVoqRejectBinThreshold, Latencies[bin_id]), "");
        }
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_profile_mapping_set(unit, base_voq_gport, 0, 0, &voq_profile_mapping), "");

    voq_profile_mapping.mapped_profile = latency_profile_gport[4];
    voq_profile_mapping.profile_type = bcmCosqIngressQueuesToLatencyProfile;
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_profile_mapping_set(unit, base_voq_gport, 4, 0, &voq_profile_mapping), "");


    for (dp_index=0; dp_index < 4; dp_index++) {
        key = BCM_COSQ_GENERIC_KEY_COLOR_SET(key, dp_index);
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_generic_control_set(unit, 0, base_voq_rate_class_gport, key, bcmCosqGenericControlLatencyVoqRejectEnable, TRUE), "");
    ...
    for (dp_index=0; dp_index < 4; dp_index++) {
        for (bin_id=0; bin_id < 8; bin_id++) {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_cosq_generic_control_set(unit, 0, base_voq_rate_class_gport, key, bcmCosqGenericControlLatencyVoqRejectProb, Probabilities[dp_index][bin_id]), "");
        }
    }

    for (dp_index=0; dp_index < 4; dp_index++) {
        key = BCM_COSQ_GENERIC_KEY_COLOR_SET(key, dp_index);
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_generic_control_set(unit, 0, base_voq_rate_class_gport, key, bcmCosqGenericControlLatencyVoqRejectMinThreshold, LatencyDisabledWhenVOQBelowThisInBytes), "");
    }
     ...
     *  as the valid bit will not be set */
    BCM_GPORT_LOCAL_SET(gport_qual, in_port);
    BCM_IF_ERROR_RETURN_MSG(ingress_latency_flow_acl_rule_set(unit, pmf_context, &profile_id, &profile_id, bcmFieldQualifyInPort, 0x1FF, &gport_qual, nof_profiles), "");
