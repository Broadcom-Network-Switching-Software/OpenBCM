    ...
    int qual_data)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &e2e_lat_fg_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, e2e_lat_fg_id, pmf_context, &attach_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, e2e_lat_fg_id, &ent_info, &e2e_lat_ent_id), "");
    ...
    bcm_field_context_t pmf_context)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchEndToEndLatencyPerDestinationPort, FALSE), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_pp_profile_create(unit, 0, bcmStatCounterInterfaceEgressTransmitPp, &stat_pp_profile, &stat_pp_profile_info), "");
    /** set the properties of the profile. by default all actions related to latency are disabled. */
    /** Example activate some of them */
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit, latency_gport, 0, bcmCosqControlLatencyEgressCounterCommand, stat_pp_profile), "");
    if (!feature_end_to_end_aqm_support)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,latency_gport,0,bcmCosqControlLatencyDropEnable, TRUE), "");
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_control_set(unit,latency_gport, 0, bcmCosqControlLatencyDropThreshold, 10000), "");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(end_to_end_latency_flow_acl_rule_set(unit, pmf_context, flow_id, BCM_GPORT_END_TO_END_LATENCY_PROFILE_GET(latency_gport),
    ...
    int out_port)
{
    ...
    int feature_egress_latency_track_support = *dnxc_data_get(unit, "latency", "features", "egress_latency_track_support", NULL);
    /** create a latency profile */
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_latency_profile_create(unit, BCM_COSQ_LATENCY_END_TO_END, &latency_gport), "");

    /** configure device to work in mode: end to end latency per port - Not mandatory, since this is the default */
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchEndToEndLatencyPerDestinationPort, TRUE), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_profile_mapping_set(unit, out_port, 0, 0, &profile_mapping), "");
    return BCM_E_NONE;
}
    ...
    int unit)
{
    ...
    for (core_idx; core_idx < nof_cores; core_idx++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_cosq_max_latency_pkts_get(unit, gport, BCM_COSQ_LATENCY_END_TO_END, 8, max_latency_pkts, &memory_actual_count), "");
    ...
    int is_per_flow)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_cosq_max_latency_pkts_get(unit, gport, BCM_COSQ_LATENCY_END_TO_END, 8, max_latency_pkts, &e2e_flow_latency_max_actual_count), "");
    ...
    int count_packets_per_bin)
{
   ...
   /** configure the counter eviction per engine */
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_interface_set(unit, flags, &database, &cnt_interface), "");
