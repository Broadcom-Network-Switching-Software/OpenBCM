 ...
 * Test Scenario
 * Test Scenario - start
  config add tm_port_header_type_in_0.0=ETH
  config add tm_port_header_type_out_0.0=ETH
  config add tm_port_header_type_in_202.0=ETH
  config add rcpu_rx_pbmp.0=0xf00000000000000000000000000000000000000000000000001
  0: tr 141 NoInit=1
  ...
  0: tr 141 NoDeinit=1
  cint ../../../../src/examples/dnx/rcpu/cint_dnx_cmic_rcpu.c
 ...
 *
  config delete tm_port_header_type_in_0.0
  config delete tm_port_header_type_out_0.0
  config delete tm_port_header_type_in_202.0
  config delete rcpu_rx_pbmp.0
  0: tr 141 NoInit=1
  config add custom_feature_init_verify=1
  config delete custom_feature_multithread_en*
  config delete custom_feature_kbp_multithread_en*
...
int cmic_rcpu_example(int unit, int cpu_rcpu_port, int out_port, bcm_mac_t lmac, bcm_mac_t rmac, uint32 sig, uint32 ethertype, uint32 vlan, uint32 tpid)
{
    ...
    /* Enabling CMIC. */
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuCmicEnable, 0x1),
        "bcmSwitchRemoteCpuCmicEnable");
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuMatchLocalMac, 0x1),
        "bcmSwitchRemoteCpuMatchLocalMac");
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuSchanEnable, 0x1),
        "bcmSwitchRemoteCpuSchanEnable");
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuMatchVlan, 0x1),
        "bcmSwitchRemoteCpuMatchVlan");

    /* Configuring LMAC and RMAC */
    lmac_lo = ((lmac[5]) | (lmac[4] << 8) | (lmac[3] << 16)) & 0xffffff;
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuLocalMacNonOui, lmac_lo),
        "bcmSwitchRemoteCpuLocalMacNonOui");
    lmac_hi = ((lmac[2]) | (lmac[1] << 8) | (lmac[0] << 16)) & 0xffffff;
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuLocalMacOui, lmac_hi),
        "bcmSwitchRemoteCpuLocalMacOui");

    rmac_lo = ((rmac[5]) | (rmac[4] << 8) | (rmac[3] << 16)) & 0xffffff;
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuDestMacNonOui, rmac_lo),
        "bcmSwitchRemoteCpuDestMacNonOui");
    rmac_hi = ((rmac[2]) | (rmac[1] << 8) | (rmac[0] << 16)) & 0xffffff;
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuDestMacOui, rmac_hi),
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuSignature, sig),
        "bcmSwitchRemoteCpuSignature");
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuEthertype, ethertype),
        "bcmSwitchRemoteCpuEthertype");
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuVlan, vlan),
        "bcmSwitchRemoteCpuVlan");
    BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchRemoteCpuTpid, tpid), "bcmSwitchRemoteCpuTpid");
...
{
    int core, port_i, rcpu_base_q_pair_min = 0, rcpu_base_q_pair_max = 96;
    ...
    BCM_PBMP_ITER(pbmp, port_i) {
        BCM_IF_ERROR_RETURN_MSG(bcm_port_get(unit, port_i, &flags, &interface_info, &mapping_info), "");
...
int cmic_rcpu_appl(int unit, int rcpu_port, int out_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cmic_rcpu_example(unit, rcpu_port, out_port, lmac, rmac, sig, ethertype, vlan, tpid), "");
