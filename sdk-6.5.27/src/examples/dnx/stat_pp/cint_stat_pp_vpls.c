 ...
 * 
 ...
 Copyright 2007-2022 Broadcom Inc. All rights reserved. File: cint_stat_pp_vpls.c Purpose: Statistic configuration for vpls (pwe in/out lif, mpls in/out lif).
 */
   ...
   int pwe_p)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_stat_pp_config_main(unit, vpls_pwe_inlif_statistics_properties, bcmStatCounterInterfaceIngressReceivePp, 0, BCM_CORE_ALL, 0),
        "failed for vpls_pwe_inlif_statistics_properties");

    /** mpls inlif statistics configuration */
    BCM_IF_ERROR_RETURN_MSG(cint_stat_pp_config_main(unit, vpls_mpls_inlif_statistics_properties, bcmStatCounterInterfaceIngressReceivePp, 0, BCM_CORE_ALL, 0),
        "failed for vpls_mpls_inlif_statistics_properties");

    /** Call vpls cint to create vpls, configure ports and lifs */
    BCM_IF_ERROR_RETURN_MSG(vpls_main(unit, ac_p, pwe_p), "");
    ...
    for(idx = 0; idx < 2; idx++) {
        ...
        print stat_info;
        BCM_IF_ERROR_RETURN_MSG(bcm_gport_stat_set(unit, ports[idx], BCM_CORE_ALL, bcmStatCounterInterfaceIngressReceivePp, stat_info), "in IRPP");
    }
   ...
   int pwe_p)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_stat_pp_config_main(unit, vpls_pwe_outlif_statistics_properties, bcmStatCounterInterfaceEgressTransmitPp, 0, BCM_CORE_ALL, 0),
        "failed for vpls_pwe_outlif_statistics_properties");

    /** mpls outlif statistics configuration */
    BCM_IF_ERROR_RETURN_MSG(cint_stat_pp_config_main(unit, vpls_mpls_outlif_statistics_properties, bcmStatCounterInterfaceEgressTransmitPp, 0, BCM_CORE_ALL, 0),
        "failed for vpls_mpls_outlif_statistics_properties");
    ...
    for(idx = 0; idx < 2; idx++) {
        ...
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_gport_stat_set(unit, ports[idx], BCM_CORE_ALL, bcmStatCounterInterfaceEgressPp, stat_info), "in ETPP");
        } else {
            BCM_IF_ERROR_RETURN_MSG(bcm_gport_stat_set(unit, ports[idx], BCM_CORE_ALL, bcmStatCounterInterfaceEgressTransmitPp, stat_info), "in ETPP");
        }
    }
