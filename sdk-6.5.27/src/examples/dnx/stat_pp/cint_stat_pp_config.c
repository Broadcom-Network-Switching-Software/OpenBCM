   ...
   int core_id)
{
    ...
    case bcmStatCounterInterfaceIngressReceivePp:
        BCM_IF_ERROR_RETURN_MSG(crps_irpp_data_base_create_wrapper(unit,
                                                core_id,
    ...
    case bcmStatCounterInterfaceEgressPp:
        ...
        break;
    default:
        int es = engine_source; /** Bypass cint complaint of printing enum in order to print the engine source ID */
    ...
    if (*dnxc_data_get(unit, "crps", "eviction", "conditional_action_is_supported", NULL))
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_stat_counter_engine_control_set(unit, flags, &engine, bcmStatCounterSequentialSamplingInterval, 1), "");
    }
    ...
    int *pp_profile_id)
{
    ...
    *pp_profile_id = cint_stat_pp_config_info.pp_profile;
    ...
    if (*(dnxc_data_get(unit, "stat", "stat_pp", "protection_direct_stat_only", NULL)))
    {
        if (cint_stat_pp_config_info.is_protection_fec)
        {
            pp_profile->fec_profile = bcmStatFecProfileDirect;
        }
    }
    else
    {
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_stat_pp_profile_create(unit, flags, engine_source, *pp_profile_id, pp_profile), "");
   ...
   int is_lif)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(cint_stat_pp_config_crps(unit, cint_stat_pp_config_info, engine_source, core_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_stat_pp_create_pp_profile(unit, cint_stat_pp_config_info, engine_source, &pp_profile, &pp_profile_id), "");
    ...
    if (is_lif)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_gport_stat_set(unit, gport, core_id, engine_source, stat_info), "");
    }
    return BCM_E_NONE;
}
