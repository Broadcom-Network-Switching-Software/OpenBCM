 ...
 * 
 ...
 Copyright 2007-2022 Broadcom Inc. All rights reserved. File: cint_stat_pp_mpls_fwd.c Purpose: Statistics examples for SDK ingress APIs using mpls fwd.
 */
 ...
 * 3. mpls fwd nop entry
  cint
  cint_stat_pp_mpls_fwd_nop_example(0,200,203);
  exit;
 *
 * Sent packet:
  tx 1 psrc=200 data=0x000c00020000001100000112884701e61140000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20
 *
 * Received packets on unit 0 should be:
 * Source port: 200, Destination port: 201
 * Data: 0x00000000cd1d000c00020f558100002a884700d0513f000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f200000000000000000000000
    ...
    int out_sys_port)
{
    ...
    entry.flags |= BCM_MPLS_SWITCH_REPLACE;
    ...
    int rif)
{
    ...
    if(rif) {
       ...
       if (*dnxc_data_get(unit, "mpls", "general", "per_intf_lookup_supported", NULL))
       {
    ...
    */
    ...
    if (*dnxc_data_get(unit, "mpls", "general", "per_intf_lookup_supported", NULL))
    {
        ...
        l3_ing_if.flags = l3_ing_if.flags |BCM_L3_INGRESS_WITH_ID;
    }
    ...
    int out_sys_port)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(cint_stat_pp_config_main(ingress_unit, mpls_fwd_php_statistics_config, bcmStatCounterInterfaceIngressReceivePp, gport, BCM_CORE_ALL, is_lif),
        "failed for mpls_fwd_php_statistics_config");

    /*
     * step 4. update the mpls php entry with stat pp information
     */
    BCM_IF_ERROR_RETURN_MSG(mpls_lsr_ilm_php_entry_replace_with_stat_pp(ingress_unit, out_sys_port), "");
    ...
    int out_sys_port)
{
     ...
     */
    if (*dnxc_data_get(ingress_unit, "mpls", "general", "per_intf_lookup_supported", NULL))
    {
        BCM_IF_ERROR_RETURN_MSG(multi_dev_mpls_lsr_swap_with_context_example(ingress_unit,egress_unit, in_sys_port,out_sys_port), "");
    } else {
        BCM_IF_ERROR_RETURN_MSG(multi_dev_mpls_lsr_swap_example(ingress_unit,egress_unit, in_sys_port,out_sys_port), "");
    }
    /*
     * step 2. Counfigure statistics for mpls fwd swap
     */
    BCM_IF_ERROR_RETURN_MSG(cint_stat_pp_config_main(ingress_unit, mpls_fwd_swap_statistics_config, bcmStatCounterInterfaceIngressReceivePp, gport, BCM_CORE_ALL, is_lif),
        "failed for mpls_fwd_swap_statistics_config");

    /*
     * step 3. update the mpls swap entry with stat pp information
     */
    BCM_IF_ERROR_RETURN_MSG(mpls_lsr_ilm_swap_entry_replace_with_stat_pp(ingress_unit, out_sys_port, cint_mpls_lsr_info.in_rif), "");
    ...
    int out_port)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(cint_stat_pp_config_main(unit, mpls_fwd_nop_statistics_config, bcmStatCounterInterfaceIngressReceivePp, gport, BCM_CORE_ALL, is_lif),
        "failed for mpls_fwd_nop_statistics_config");

    /*
     * step 2.MPLS LSR NOP
     */
    BCM_IF_ERROR_RETURN_MSG(mpls_encapsulation_basic_main(unit, in_port, out_port, 0), "");
