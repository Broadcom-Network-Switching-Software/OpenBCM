        ...
        int epmf_ctx_id)
{
     ...
     * Create field group
     */
    BCM_IF_ERROR_RETURN_MSG(cint_field_qual_bypass_filter_add_fg(unit, epmf_ctx_id), "");


    /**
     * Create and set 'SA equals DA' trap in ingress with max strength (0xf) to bypass all the filtering in egress pipe
     */
    BCM_IF_ERROR_RETURN_MSG(cint_field_qual_bypass_filter_set_trap(unit), "");
       ...
       int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_create(unit, 0, bcmRxTrapLinkLayerSaEqualsDa, &cint_field_qual_bypass_filter_trap_id), "");

    printf("Trap %d was created. \n", cint_field_qual_bypass_filter_trap_id);

    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_set(unit, cint_field_qual_bypass_filter_trap_id, trap_config), "");
       ...
       int epmf_ctx_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &cint_field_qual_bypass_filter_fg_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_field_qual_bypass_filter_fg_id, epmf_ctx_id, &attach_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, cint_field_qual_bypass_filter_fg_id, &entry_info, cint_field_qual_bypass_filter_entry_id), "");
...
int cint_field_qual_bypass_filter_destroy(
        ...
        int epmf_ctx_id)
{
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_delete(unit, cint_field_qual_bypass_filter_fg_id, NULL, cint_field_qual_bypass_filter_entry_id), "");

    printf("Entry 0x%X (%d) was deleted. \n", cint_field_qual_bypass_filter_entry_id, cint_field_qual_bypass_filter_entry_id);

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_detach(unit, cint_field_qual_bypass_filter_fg_id, epmf_ctx_id), "");

    printf("Field Group %d was detach from context %d. \n", cint_field_qual_bypass_filter_fg_id, epmf_ctx_id);

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_delete(unit, cint_field_qual_bypass_filter_fg_id), "");

    printf("Field Group %d was deleted. \n", cint_field_qual_bypass_filter_fg_id);

    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_destroy(unit, cint_field_qual_bypass_filter_trap_id), "");
