...
int field_oam_add_entries(int unit, int trap_id, int pars_inc, int fwd_plus_one_type, int fwd_plus_one_mask)
{
    ...
    for(ii = 0 ; ii < 8; ii++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, oam_fg_id, &entry_info, &entry_handle[entry_handle_count]), "");
...
int field_oam_layer_index_main(int unit, bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &oam_fg_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, oam_fg_id, context_id, &attach_info), "");
    ...
    for (index = 0; index < 10; index++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_get(unit,trap_code_list[index],&trap_config), "");


        trap_config.flags = BCM_RX_TRAP_TRAP | BCM_RX_TRAP_UPDATE_DEST;
        BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_set(unit,trap_code_list[index],trap_config), "");

    }

    /**Entries for FIELD_OAM_TRAP_ETH_OAM*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_ETH_OAM,1,0,0), "");


    /**Entries for FIELD_OAM_TRAP_ETH_OAM_PERFORMANCE*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_ETH_OAM_PERFORMANCE,1,0,0), "");

    
    /**Entries for FIELD_OAM_TRAP_BFD_O_IPV4*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_BFD_O_IPV4,2,0,0), "");


    /**Entries for FIELD_OAM_TRAP_BFD_O_IPV4*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_BFD_O_IPV6,2,0,0), "");


    /**Entries for FIELD_OAM_TRAP_BFD_O_MPLS*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_BFD_O_MPLS,2,0,0), "");


    /**Entries for FIELD_OAM_TRAP_BFD_O_PWE*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_BFD_O_PWE,2,IPv4_Type,Type_Mask), "");

    /**Entries for FIELD_OAM_TRAP_BFD_O_PWE*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_BFD_O_PWE,2,IPv6_Type,Type_Mask), "");


    /**Entries for FIELD_OAM_TRAP_BFDCC_O_MPLSTP*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_BFDCC_O_MPLSTP,2,IPv4_Type,Type_Mask), "");

    /**Entries for FIELD_OAM_TRAP_BFDCC_O_MPLSTP*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_BFDCC_O_MPLSTP,2,IPv6_Type,Type_Mask), "");


    /**Entries for FIELD_OAM_TRAP_BFDCV_O_MPLSTP*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_BFDCV_O_MPLSTP,2,IPv4_Type,Type_Mask), "");

    /**Entries for FIELD_OAM_TRAP_BFDCV_O_MPLSTP*/
    BCM_IF_ERROR_RETURN_MSG(field_oam_add_entries(unit,FIELD_OAM_TRAP_BFDCV_O_MPLSTP,2,IPv6_Type,Type_Mask), "");
...
int field_oam_layer_index_destroy(int unit, bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_detach(unit,oam_fg_id,context_id), "");

    printf("Entry delete: ");
    for(ii = 0; ii < entry_handle_count; ii++)
    {
        printf("idx(%d) ",ii);
        BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_delete(unit, oam_fg_id, NULL, entry_handle[ii]), "");

    }
    printf("\n");
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_delete(unit, oam_fg_id), "");
