...
int cint_field_ieee1588_eth_update_main(int unit, bcm_field_context_t context_id, int nif_1_port, int nif_t_port)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(in_port_intf_set(unit, nif_1_port, vlan_y), "");
    /*
     * 1.2. Set NIF-T-Port default properties
     */
    BCM_IF_ERROR_RETURN_MSG(out_port_set(unit, nif_t_port), "");
    /*
     * 1.3. Create ETH-RIF and set MY-Mac
     */
    BCM_IF_ERROR_RETURN_MSG(intf_eth_rif_create(unit, vlan_x, nif_t_mac), "");

    /*
     * 1.4. Create ARP and set its properties
     */
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_encap__create(unit, 0, &arp_id_enc, fgpa_mac, vlan_x), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &fec_id_enc), "");

    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create(unit, fec_id_enc, 0, arp_id_enc, gport, 0, fec_flags2), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &cint_ieee1588_eth_update_fg_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_ieee1588_eth_update_fg_id, context_id, &attach_info), "");
    ...
    BCM_L3_ITF_SET(fec_if, BCM_L3_ITF_TYPE_FEC, fec_id_enc);
    ent_info.entry_action[0].value[0] = fec_if;
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, cint_ieee1588_eth_update_fg_id, &ent_info, &cint_ieee1588_eth_update_ent_id), "");
    return BCM_E_NONE;
}
...
{
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_delete(unit, cint_ieee1588_eth_update_fg_id, NULL, cint_ieee1588_eth_update_ent_id), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_detach(unit, cint_ieee1588_eth_update_fg_id, context_id), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_delete(unit, cint_ieee1588_eth_update_fg_id), "");
