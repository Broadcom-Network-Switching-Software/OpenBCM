                    ...
                    int srh_present)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, tunnel_term_set.tunnel_id, pclass,
                            srv6_evpn_global_info.in_lif_acl_profile_esi), "for IPv6 Tunnel Termination LIF");
    ...
    int p2_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(srv6_evpn_profiles_set(unit), "");

    printf("Create VPN (%d)\n", cint_srv6_evpn_info.vsi);
    BCM_IF_ERROR_RETURN_MSG(srv6_evpn_vswitch_create(unit), "");

    vswitch_ports[0] = ce1_port;
    vswitch_ports[1] = ce3_port;
    vswitch_ports[2] = p1_port;
    vswitch_ports[3] = p2_port;
    BCM_IF_ERROR_RETURN_MSG(srv6_evpn_vswitch_add_ports(unit, 4, vswitch_ports), "");


    printf("Init ESI filter FP application\n");
    BCM_IF_ERROR_RETURN_MSG(srv6_evpn_esi_filter_init(unit), "");

    printf("Create ETH segments\n");
    BCM_IF_ERROR_RETURN_MSG(srv6_evpn_eth_segments_create(unit, ce1_port, ce3_port, p1_port, p2_port), "");
    ...
    int unit)
{
    ...
    printf("- Create VSI\n");
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_create_with_id(unit, cint_srv6_evpn_info.vsi), "");

    printf("- Create MC group\n");
    BCM_IF_ERROR_RETURN_MSG(multicast__open_mc_group(unit, &cint_srv6_evpn_info.vsi, 0), "");
    ...
    int *ports)
{
    ...
    while (nof_ports--)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, cint_srv6_evpn_info.vsi, ports[nof_ports], 0), error_msg);
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_field_srv6_evpn_ingress(unit, srv6_evpn_global_info.in_lif_acl_profile_esi), "");

    BCM_IF_ERROR_RETURN_MSG(cint_field_srv6_evpn_egress(unit), "");
    ...
    srv6_evpn_es_info_s * esi)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(srv6_evpn_configure_port(unit, esi->port), "");
    ...
    if (esi->eve_vid)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(vlan_translate_ive_eve_translation_set(unit, vport->vlan_port_id,
                                                    0x8100,                           0,
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_field_srv6_evpn_egress_esi_port_entry_add(unit, egress_context_id, egress_fg_id,
                                                   &esi_port_handle, esi->label, esi->port), "");
    BCM_IF_ERROR_RETURN_MSG(cint_field_srv6_evpn_egress_esi_entry_add(unit, egress_context_id, egress_fg_id,
    ...
    int p2_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(srv6_evpn_es_add(unit, &ce1_esi_200), "ce1_vlan");
    ...
    BCM_IF_ERROR_RETURN_MSG(srv6_evpn_es_add(unit, &ce3_esi_200), "ce3_vlan");
    ...
    BCM_IF_ERROR_RETURN_MSG(srv6_evpn_es_add(unit, &p1_esi_300), "p1_vlan");
    ...
    BCM_IF_ERROR_RETURN_MSG(srv6_evpn_es_add(unit, &p2_esi_300), "p2_vlan");
    ...
    int is_virtual_esi)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_field_srv6_evpn_esi_encap_egress_fg_add(unit, is_virtual_esi, same_es_p_port), "");

    /** Add the ESI encapsulation ePMF entries.*/
    BCM_IF_ERROR_RETURN_MSG(cint_field_srv6_evpn_esi_encap_egress_entry_add(unit, is_virtual_esi, src_vp, dst_vp, tunnel_init_set.tunnel_id), "");
