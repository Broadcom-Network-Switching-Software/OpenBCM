...
int srv6_usp_trap_from_optimized_lpm = 0;
...
srv6_endpoint_tunnel(int unit, uint8 next_protocol_is_l2, uint8 is_termination, uint8 lif_cs_profile, int is_p2p_tunnel)
{
    ...
    if(srv6_usp_trap_from_optimized_lpm == 0)
    {
         if(*dnxc_data_get(unit, "l3", "feature", "separate_fwd_rpf_dbs", NULL))
        {
            srv6_usp_trap_from_optimized_lpm = 1;
        }
    }
    if(!srv6_usp_trap_from_optimized_lpm)
    {
        if (add_trap_as_destination && (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL)))
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapSrv6Usp, &trap_code), "");
    ...
    else if(add_trap_as_destination)
    {
        int fec_as_raw_index = 0;
        bcm_gport_t gport_port;
        uint32 fec_flags2 = 0;
        int encoded_fec_as_raw;
        /** ARP is assumed to be created in basic_example_ipv6 */
        int arp_id = *dnxc_data_get(unit, "l3", "rif", "nof_rifs", NULL) + 0x384; 
        BCM_L3_ITF_SET(arp_id, BCM_L3_ITF_TYPE_LIF, arp_id);

        BCM_IF_ERROR_RETURN_MSG(get_first_fec_in_range_which_not_in_ecmp_range(unit, 0, 0, &fec_as_raw_index), "");
        fec_as_raw_index += 0x267;
        ...
        /** Assuming interface 100 is created by basic_example_ipv6*/
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, fec_as_raw_index, 100 , arp_id, srv6_usp_last_pass_port, 0,
                                               fec_flags2, 0, 0,&encoded_fec_as_raw), "create egress object FEC_as raw only");


        BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_set(unit, bcmSwitchControlRouteSrv6UspTrapSet, fec_as_raw_index),
                                                       "fail in bcm_dnx_switch_control_set");
        ...
        BCM_L3_ITF_SET(l3rt.l3a_intf, BCM_L3_ITF_TYPE_FEC, fec_as_raw_index);

        if ((*dnxc_data_get(unit, "l3", "feature", "separate_fwd_rpf_dbs", NULL)) ||
            (*dnxc_data_get(unit, "elk", "application", "split_rpf", NULL) && *dnxc_data_get(unit, "elk", "general", "device_type", NULL)))
        {
            l3rt.l3a_flags2 |= BCM_L3_FLAGS2_FWD_ONLY;
        }
...
srv6_endpoint_udp_tunnel(int unit, uint8 next_protocol_is_l2, uint8 is_termination)
{
    ...
    if (host_entry_add)
    {
        ...
        {
            snprintf(error_msg, sizeof(error_msg), "vrf = %d, fec = %d", vrf, ip_route_fec);
            BCM_IF_ERROR_RETURN_MSG(add_host_ipv6(unit, ip6_dip, vrf, ip_route_fec), error_msg);
        }
    }
    ...
    {
        snprintf(error_msg, sizeof(error_msg), "vrf = %d, fec = %d", vrf, ip_route_fec);
        BCM_IF_ERROR_RETURN_MSG(add_host_ipv4(unit, ipv4_dip, vrf, ip_route_fec, 0, 0), error_msg);
    }
    return BCM_E_NONE;
}
...
srv6_endpoint_tunnel_prefix_function_classic_sid(int unit)
{
    ...
    BCM_GPORT_TUNNEL_TO_L3_ITF_LIF(ingress_rif.intf_id, tunnel_term_set.tunnel_id);
    BCM_IF_ERROR_RETURN_MSG(intf_ingress_rif_set(unit, &ingress_rif), "");
    BCM_IF_ERROR_RETURN_MSG(add_route_ipv6(unit, ip6_dip_next_endpoint, ip6_mask, 10, fec), "vrf = 10");
    ...
    BCM_GPORT_TUNNEL_TO_L3_ITF_LIF(ingress_rif.intf_id, tunnel_term_set.tunnel_id);
    BCM_IF_ERROR_RETURN_MSG(intf_ingress_rif_set(unit, &ingress_rif), "");
    BCM_IF_ERROR_RETURN_MSG(add_route_ipv6(unit, ip6_dip_next_endpoint, ip6_mask, 20, fec), "vrf = 20");
    ...
    BCM_GPORT_TUNNEL_TO_L3_ITF_LIF(ingress_rif.intf_id, tunnel_term_set.tunnel_id);
    BCM_IF_ERROR_RETURN_MSG(intf_ingress_rif_set(unit, &ingress_rif), "");
    BCM_IF_ERROR_RETURN_MSG(add_route_ipv6(unit, ip6_dip_next_endpoint, ip6_mask, 30, fec), "vrf = 30");
    default_tunnel = tunnel_term_set.tunnel_id;
    /*
    ...
    BCM_GPORT_TUNNEL_TO_L3_ITF_LIF(ingress_rif.intf_id, tunnel_term_set.tunnel_id);
    BCM_IF_ERROR_RETURN_MSG(intf_ingress_rif_set(unit, &ingress_rif), "");
    BCM_IF_ERROR_RETURN_MSG(add_route_ipv6(unit, ip6_dip_next_endpoint, ip6_mask, 40, fec), "vrf = 40");
    return BCM_E_NONE;
}
...
srv6_update_fecs_values(int unit)
{
    ...
    /** Hierarchy 2 FECs */
    BCM_IF_ERROR_RETURN_MSG(get_first_fec_in_range_which_not_in_ecmp_range(unit, 1, 0, &first_fec_in_hier), "");
    ...
    /** Hierarchy 3 FECs */
    BCM_IF_ERROR_RETURN_MSG(get_first_fec_in_range_which_not_in_ecmp_range(unit, 2, 0, &first_fec_in_hier), "");
  ...
  
{
    ...
    {
        /** SRV6 Ingress Tunnel Eth RIF based on Port only */
        BCM_IF_ERROR_RETURN_MSG(in_port_intf_set(unit, in_port_eth, cint_srv6_tunnel_info.eth_rif_intf_in[INGRESS]), "intf_in");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(intf_eth_rif_create(unit, cint_srv6_tunnel_info.tunnel_vid, cint_srv6_tunnel_info.intf_out_mac_address[INGRESS]), "intf_out");
    ...
    BCM_IF_ERROR_RETURN_MSG(intf_ingress_rif_set(unit, &ingress_rif_endpoint), "for ingress_rif_endpoint.");
    ...
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FEC_ID], 0 , srv6_basic_lif_encode, gport, 0,
                                           fec_flags2, 0, 0,&encoded_fec1), "create egress object FEC_1 only");

    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_L2VPN_FEC_ID], 0 , srv6_basic_lif_encode, gport_l2vpn, 0,
                                           fec_flags2, 0, 0,&encoded_fec1_l2vpn), "create egress object FEC_1_L2VPN only");
    ...
    if (!is_t_insert_encap)
    {
        ...
        BCM_GPORT_FORWARD_PORT_SET(gport, cint_srv6_tunnel_info.tunnel_fec_id[ARP_AC_FEC_ID]);
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_FIRST_SID_FEC_ID], 0 , srv6_first_lif_encode, gport, BCM_L3_2ND_HIERARCHY,
                fec_flags2, 0, 0,&encoded_fec2), "create egress object FEC_2 only, fec");
        ...
        BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_FIRST_SID_L2VPN_FEC_ID], 0 , srv6_first_lif_encode, gport, BCM_L3_2ND_HIERARCHY,
                fec_flags2,0, 0,&encoded_fec2_l2vpn), "create egress object FEC_2_L2VPN only, fec");
    ...
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[ARP_AC_FEC_ID], 0, cint_srv6_tunnel_info.tunnel_arp_id[INGRESS], gport, BCM_L3_3RD_HIERARCHY,
                                           fec_flags2, 0, 0, &encoded_fec3_arp_ac), "create egress object FEC of ARP+AC only");
    ...
    BCM_IF_ERROR_RETURN_MSG(add_route_ipv4(unit, cint_srv6_tunnel_info.route_ipv4_dip, cint_srv6_tunnel_info.route_ipv4_mask, 171 /* VRF of endpoint tunnel*/, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FEC_ID]), "VRF:171");
    snprintf(error_msg, sizeof(error_msg), "VRF:%d", cint_srv6_tunnel_info.vrf_in[INGRESS]);
    BCM_IF_ERROR_RETURN_MSG(add_route_ipv6(unit, cint_srv6_tunnel_info.route_ipv6_dip, cint_srv6_tunnel_info.route_ipv6_mask, cint_srv6_tunnel_info.vrf_in[INGRESS], cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FEC_ID]), error_msg);
    BCM_IF_ERROR_RETURN_MSG(add_route_ipv6(unit, cint_srv6_tunnel_info.route_ipv6_dip, cint_srv6_tunnel_info.route_ipv6_mask, 171 /* VRF of endpoint tunnel*/, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FEC_ID]), "VRF:171");
    ...
    if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL) && add_ipv6_host_entry)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(add_host_ipv4(unit, cint_srv6_tunnel_info.route_ipv4_dip, cint_srv6_tunnel_info.vrf_in[INGRESS], 0, 0, gport),
            "for SRV6 Ingress Tunnel");
        BCM_IF_ERROR_RETURN_MSG(add_host_ipv4(unit, cint_srv6_tunnel_info.route_ipv4_dip, 171 /* VRF of endpoint tunnel*/, 0, 0, gport),
            "for SRV6 Ingress Tunnel");
        BCM_IF_ERROR_RETURN_MSG(add_host_ipv6_encap_dest(unit, cint_srv6_tunnel_info.route_ipv6_dip, cint_srv6_tunnel_info.vrf_in[INGRESS], 0, 0, gport),
            "for SRV6 Ingress Tunnel");
        BCM_IF_ERROR_RETURN_MSG(add_host_ipv6_encap_dest(unit, cint_srv6_tunnel_info.route_ipv6_dip, 171 /* VRF of endpoint tunnel*/, 0, 0, gport),
            "for SRV6 Ingress Tunnel");
    }
    ...
    l2addr.port = gport;
    l2addr.encap_id = 0;
    l2addr.flags = l2_flags;
    l2addr.learn_strength = learn_strength;
                            ...
                            int nof_sids)
{
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(srv6_endpoint_tunnel_cross_connect(unit, 0, cint_srv6_tunnel_info.tunnel_fec_id[INGRESS_SRH_FEC_ID], 0),
        "for End.B6");
    ...
    int fec)
{
    ...
    BCM_GPORT_LOCAL_SET(gport, out_port);
    BCM_IF_ERROR_RETURN_MSG(l3__egress_only_fec__create_inner(unit, fec, 0 , encap_id, gport, 0, fec_flags2,
    ...
    if(srh_next_protocol == 4)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(add_route_ipv4(unit, host, cint_srv6_tunnel_info.route_ipv4_mask, vrf, fec), error_msg);
    }
    else if(srh_next_protocol == 41)
    {
        ...
        if (*dnxc_data_get(unit, "l3", "fwd", "host_entry_support", NULL) && add_ipv6_host_entry)
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(add_host_ipv6_encap_dest(unit, route_ipv6_dip, vrf, _l3_itf, intf_out, 0), "");
        }
        else
        {
            ...
            snprintf(error_msg, sizeof(error_msg), "VRF:%d", vrf);
            BCM_IF_ERROR_RETURN_MSG(add_route_ipv6(unit, route_ipv6_dip, cint_srv6_tunnel_info.route_ipv6_mask, vrf, fec), error_msg);
    ...
    int out_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(dnx_basic_example_ipvx_for_srv6_inner(unit, srh_next_protocol,  rch_port, out_port, fec), "");
    printf("%s(): Exit\r\n",proc_name);
    return BCM_E_NONE;
}
    ...
    int out_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(dnx_basic_example_ipvx_cross_connect_for_srv6(unit, rch_port, out_port, 4), "");
    return BCM_E_NONE;
}
    ...
    int out_port)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(dnx_basic_example_ipvx_cross_connect_for_srv6(unit, rch_port, out_port, 41), "");
    return BCM_E_NONE;
}
...
int srv6_endpoint_psp_extended_configuration(int unit, int rch_port, int field_ctx_ipmf2, int field_ctx_ipmf3, int use_rch_qos)
{
    ...
    if (!ibch1_supported)
    {
        ...
        snprintf(error_msg, sizeof(error_msg), "(port %d)", rch_port);
        BCM_IF_ERROR_RETURN_MSG(bcm_switch_control_indexed_port_set(unit, rch_port, key, value), error_msg);
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &qual_info_dest_info, &qual_info_dest), "qual_info_dest_info");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_qualifier_create(unit, 0, &qual_info_outlif_info, &qual_info_outlif), "qual_info_outlif_info");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &fg_id), "Egress PSP ext cut ipmf3_fg");
    ...
    *destination = rch_port | 0xc0000;
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_value_map(unit, bcmFieldStageIngressPMF2, bcmFieldActionForward, destination, hw_value), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, fg_id, field_ctx_ipmf2, &attach_info), "ipmf2");
...
srv6_psp_usp_update_fwd_destination(int unit, int destination)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_info_get(unit,bcmFieldActionForward,bcmFieldStageIngressPMF1,&action_info_get_size),
        "cint_const_action");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_create(unit, 0, &action_info, &cint_const_action_id), "cint_const_action");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &cint_const_fg_id), "");
    id = cint_const_fg_id;
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_const_fg_id, 9, &attach_info), "");
...
int srv6_icmp_trap_overwrite (int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_rx_trap_type_get(unit, 0, bcmRxTrapDefault, &trap_code), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_field_action_info_get(unit,bcmFieldActionTrap,bcmFieldStageIngressPMF1,&action_info_get_size),
        "cint_const_action");
    ...
    printf("Attaching field group\n");
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_const_fg_id, 8, &attach_info), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, cint_const_fg_id, 37, &attach_info), "");
    return BCM_E_NONE;
}
...
int srv6_psp_copy_inlif1_to_inlif0 (int unit, int field_ctx_ipmf3)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info3, &fg_id3), "Egress PSP ext cut ipmf3_fg");
    ...
    /** context id is defined in appl_ref_init */
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, fg_id3, field_ctx_ipmf3, &attach_info3), "ipmf3");
