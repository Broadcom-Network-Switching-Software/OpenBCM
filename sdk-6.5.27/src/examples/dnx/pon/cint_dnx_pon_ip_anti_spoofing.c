...
int pon_service_ip_anti_spoofing_set(int unit,int start_index,int end_index,uint32 add,int is_dhcp,int is_n1)
{
    ...
    for (index = start_index; index <= end_index; index ++)
    {
        ...
        {
            /*ipv4*/
            BCM_IF_ERROR_RETURN_MSG(bcm_l3_source_bind_add(unit,&info), error_msg);
            /*ipv6*/
...
int pon_service_ip_anti_spoofing_enable(int unit, int start_index, int end_index, uint32 enable,int is_n1)
{
    ...
    for(i = start_index; i <= end_index; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_l3_source_bind_enable_set(unit, pon_gport, enable), error_msg);
        if (is_n1)
        {
...
int sav_field_data_clean(int unit, bcm_field_context_t context_id)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_delete(unit, sav_default_fg_id, NULL, default_ent_id), "");

    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_detach(unit, sav_default_fg_id, context_id), "");
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_delete(unit, sav_default_fg_id), "");

    BCM_IF_ERROR_RETURN_MSG(cint_sav_field_inlif_profile_cs_ctx_destroy(unit), "");
    ...
    int profile)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_create(unit, 0, bcmFieldStageIngressPMF1, &context_info, &sav_inlif_profile_cs_context_id), "");
    ...
    int unit)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_context_destroy(unit, bcmFieldStageIngressPMF1, sav_inlif_profile_cs_context_id), "");
...
int cint_sav_field_set_context_ipmf1(int unit, int start_index, int end_index, int in_lif_profile,int is_n_1)
{
    ...
    for (i =start_index; i<=end_index; i++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_port_class_set(unit, pon_gport, bcmPortClassFieldIngressVport, in_lif_profile), "");
    }
    return BCM_E_NONE;
...
int cint_sav_field_config_ipmf1(int unit, int is_n_1, int is_add)
{
    ...
    if(!is_add)
    {
        if (is_sav_pmf_config) {
            BCM_IF_ERROR_RETURN_MSG(sav_field_data_clean(unit, sav_inlif_profile_cs_context_id), "");
        }
        return BCM_E_NONE;
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(cint_sav_field_inlif_profile_cs_ctx_create(unit, in_lif_profile_sav), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_add(unit, 0, &fg_info, &sav_default_fg_id), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_group_context_attach(unit, 0, sav_default_fg_id, sav_inlif_profile_cs_context_id, &attach_info), "");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_field_entry_add(unit, 0, sav_default_fg_id, &entry_info, &default_ent_id), "");
