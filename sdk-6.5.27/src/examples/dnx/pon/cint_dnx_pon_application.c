 ...
 * Test Scenario - start
       ...
       cint
       pon_1_1_service_init(unit, pon_port, nni_port);
       pon_1_1_service(unit);
 * Test Scenario - end
*
*  Service Model:
* 1:1 Service:
 ...
 * Test Scenario - start
      ...
      cint ../../../../src/examples/dnx/pon/cint_dnx_pon_application.c
      cint
      pon_n_1_service_init(unit, pon_port, nni_port);
...
int pon_1_1_service(int unit,int dtc_mode)
{
    ...
    for (index = 0; index < nof_1_1_services; index++) {
        ...
        BCM_IF_ERROR_RETURN_MSG(pon_lif_create(unit, pon_port, pon_lif_type, tunnel_id, tunnel_id, up_ovlan, up_ivlan, up_pcp, up_ethertype, 0, &pon_gport), error_msg);
...
int pon_n_1_service(int unit, int dtc_mode)
{
    ...
    for (index = 0; index < nof_n_1_services; index++) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, n_1_service_vsi, pon_gport), "");

        /*For downstream*/
        BCM_IF_ERROR_RETURN_MSG(multicast_vlan_port_add(unit, n_1_service_vsi+lif_offset, pon_port, pon_gport), error_msg);
        ...
        if (index == 0)
        {
            ...
            BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, pon_n_1_service_info[index].down_ovlan, nni_port, 0), "NNI");

            /*For upstream*/
            BCM_IF_ERROR_RETURN_MSG(multicast_vlan_port_add(unit, n_1_service_vsi, nni_port, nni_gport), error_msg);

            /* add to vswitch */
            BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_add(unit, n_1_service_vsi, nni_gport), "");
            ...
            BCM_GPORT_MCAST_SET(flood_groups.unknown_unicast_group, lif_offset);
            BCM_GPORT_MCAST_SET(flood_groups.broadcast_group, lif_offset);
            BCM_GPORT_MCAST_SET(flood_groups.unknown_multicast_group, lif_offset);
            BCM_IF_ERROR_RETURN_MSG(bcm_port_flood_group_set(unit,nni_gport,0,&flood_groups), "");
        }
    }
...
int pon_n_1_service_cleanup(int unit)
{
    ...
    for (index = 0; index < nof_n_1_services; index++)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_delete(unit, n_1_service_vsi, pon_gport), "");

        /*Only one NNI LIF*/
        if (index == 0)
        {
            BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_port_delete(unit, n_1_service_vsi, nni_gport), "");
        }
    }

    /*1. delete multicast group*/
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_destroy(unit, n_1_service_vsi), "");

    if (lif_offset != 0) {
        BCM_IF_ERROR_RETURN_MSG(bcm_multicast_destroy(unit, n_1_service_vsi+lif_offset), "");
    }

    /*2. destroy vswitch */
    BCM_IF_ERROR_RETURN_MSG(bcm_vswitch_destroy(unit,n_1_service_vsi), "");
    /* clean PON LIF ingress VLAN editor */
    BCM_IF_ERROR_RETURN_MSG(pon_port_egress_vt_destroy(unit), "");
    for (index = 0; index < nof_n_1_services; index++) {
        snprintf(error_msg, sizeof(error_msg), "index:%d", index);
        BCM_IF_ERROR_RETURN_MSG(pon_port_ingress_vt_destroy(unit,pon_n_1_service_info[index].service_mode, pon_n_1_service_info[index].down_ovlan,
                               pon_n_1_service_info[index].down_ivlan, pon_n_1_service_info[index].pon_gport), error_msg);
    }
...
int pon_1_1_service_m_subport_m_mode_set(int unit, uint32 subport_0_mode_1,uint32 subport_0_mode_2, bcm_port_t port_nni)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(pon_1_1_service_m_subport_one_mode_set(unit, pon_mode_1, subport_0_mode_1, 0, port_nni), "mode 1");
    printf("pon_1_1_service_m_subport_one_mode_set mode 1 Succeed!\n");

    /*set mode 2 configurations*/
    BCM_IF_ERROR_RETURN_MSG(pon_1_1_service_m_subport_one_mode_set(unit, pon_mode_2, 0, subport_0_mode_2, port_nni), "mode 2");
...
int pon_1_1_service_m_subport_one_mode_set(int unit, int pon_mode,uint32 subport_0_mode_1,uint32 subport_0_mode_2, bcm_port_t port_nni)
{
    ...
    snprintf(error_msg, sizeof(error_msg), "mode %d", pon_mode);
    BCM_IF_ERROR_RETURN_MSG(pon_1_1_service_m_subport_one_mode_init(unit, pon_mode, subport_0, subport_0+1, subport_0+2, subport_0+3, port_nni), error_msg);
    printf("pon_1_1_service_m_subport_one_mode_init mode %d Succeed!\n",pon_mode);
    BCM_IF_ERROR_RETURN_MSG(pon_1_1_service_m_subport_one_mode_config(unit, pon_mode, port_nni), error_msg);
...
int pon_1_1_service_m_subport_one_mode_config(int unit, int pon_mode, bcm_port_t port_nni)
{
    ...
    {
        snprintf(error_msg, sizeof(error_msg), "Enable COE port[%d] failed!", pon_m_subport_info[index].pon_subport);
        BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit, pon_m_subport_info[index].pon_subport, bcmPortControlPONEnable, pon_mode), error_msg);
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_port_extender_mapping_info_set(unit, flags, bcmPortExtenderMappingTypePonTunnel,&extender_mapping_info), "");
        /* Configure the port as STC/DTC */
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_control_port_set(unit, pon_m_subport_info[index].pon_subport, bcmVlanPortDoubleLookupEnable, 1), "bcmVlanPortDoubleLookupEnable");
    }
    for (index = 0; index < nof_pon_m_subport; index++)
    {
        pon_gport = 0;
        ...
        /* Create PON LIF */
        snprintf(error_msg, sizeof(error_msg), "index:%d", index);
        BCM_IF_ERROR_RETURN_MSG(pon_lif_create(unit, pon_m_subport_info[index].pon_subport, pon_lif_type, pon_m_subport_info[index].tunnel_id,
                          pon_m_subport_info[index].tunnel_id, up_ovlan, up_ivlan, up_pcp, up_ethertype, 0, &pon_gport), error_msg);
        ...
        /* Set PON LIF egress VLAN editor */
        BCM_IF_ERROR_RETURN_MSG(pon_port_egress_vt_set(unit, service_mode, pon_m_subport_info[index].tunnel_id, up_ovlan, up_ivlan, pon_gport), error_msg);
    
        /* Create NNI LIF */
        BCM_IF_ERROR_RETURN_MSG(nni_lif_create(unit, nni_port, nni_lif_type, is_with_id, down_ovlan, down_ivlan, down_pcp, &nni_gport, &encap_id), error_msg);
        pon_m_subport_info[index].nni_gport = nni_gport;
    
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_gport_add(unit, pon_m_subport_info[index].down_ovlan, nni_port, 0), "NNI");
        /* Cross connect the 2 LIFs */
...
int pon_1_1_service_m_subport_m_mode_cleanup(int unit)
{
    ...
    {
        snprintf(error_msg, sizeof(error_msg), "mode %d", i);
        BCM_IF_ERROR_RETURN_MSG(pon_1_1_service_m_subport_one_mode_cleanup(unit, i), error_msg);
    }
    return BCM_E_NONE;
}
...
int pon_1_1_service_m_subport_one_mode_cleanup(int unit, int pon_mode)
{
    ...
    if (pon_mode == pon_mode_1)
    {
        ...
        for (index = 0; index < nof_pon_m_subport_mode_1; index++) {
            snprintf(error_msg, sizeof(error_msg), "index:%d", index);
            BCM_IF_ERROR_RETURN_MSG(pon_port_ingress_vt_destroy(unit,pon_m_subport_info_mode_1[index].service_mode, pon_m_subport_info_mode_1[index].down_ovlan,
                                   pon_m_subport_info_mode_1[index].down_ivlan, pon_m_subport_info_mode_1[index].pon_gport), error_msg);
        }     
    }
    else if (pon_mode == pon_mode_2)
    {
        ...
        for (index = 0; index < nof_pon_m_subport_mode_2; index++) {
            snprintf(error_msg, sizeof(error_msg), "index:%d", index);
            BCM_IF_ERROR_RETURN_MSG(pon_port_ingress_vt_destroy(unit,pon_m_subport_info_mode_2[index].service_mode, pon_m_subport_info_mode_2[index].down_ovlan,
                                   pon_m_subport_info_mode_2[index].down_ivlan, pon_m_subport_info_mode_2[index].pon_gport), error_msg);
        }
    }
...
int pon_1_1_service_m_subport_m_mode_disable(int unit)
{
    ...
    {
        snprintf(error_msg, sizeof(error_msg), "mode %d!", i);
        BCM_IF_ERROR_RETURN_MSG(pon_1_1_service_m_subport_one_mode_disable(unit,i), error_msg);
...
int pon_1_1_service_m_subport_one_mode_disable(int unit,int pon_mode)
{
    ...
    {
        for (index = 0; index < nof_pon_m_subport_mode_1; index++) {
            snprintf(error_msg, sizeof(error_msg), "Disable PON port[%d] failed!", pon_m_subport_info_mode_1[index].pon_subport);
            BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit, pon_m_subport_info_mode_1[index].pon_subport, bcmPortControlPONEnable, 0), error_msg);
        }
    }
    else if (pon_mode == pon_mode_2)
    {        
        /**disable for mode 2*/
        for (index = 0; index < nof_pon_m_subport_mode_2; index++) {
            snprintf(error_msg, sizeof(error_msg), "Disable PON port[%d] failed!", pon_m_subport_info_mode_2[index].pon_subport);
            BCM_IF_ERROR_RETURN_MSG(bcm_port_control_set(unit, pon_m_subport_info_mode_2[index].pon_subport, bcmPortControlPONEnable, 0), error_msg);
        }
    }
    else
    {
...
int pon_1_1_service_m_subport_mirror_create(int unit, bcm_port_t mir_src, bcm_port_t mir_dest)
{
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_mirror_port_dest_add(unit, mir_src, flags, mirror_dest_id), "");
