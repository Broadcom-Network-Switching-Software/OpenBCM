...
int pon_lif_create(int unit, bcm_port_t port_pon, int lif_type, 
    ...
    bcm_vlan_t up_ovlan, bcm_vlan_t up_ivlan, int up_pcp, 
    bcm_port_ethertype_t up_ethertype, int mac_sav, bcm_gport_t *gport)
{
    int index;
    ...
    if (mac_sav) {
        vlan_port.flags2 |= BCM_VLAN_PORT_FLAGS2_MAC_SAV;
    }
...
int open_ingress_mc_group(int unit, int mc_group_id){
    ...
    snprintf(error_msg, sizeof(error_msg), "flags %d mc_group %d", flags, mc_group);
...
int multicast_vlan_port_add(int unit, int mc_group_id, int sys_port, int gport){
    ...
    snprintf(error_msg, sizeof(error_msg), "mc_group_id:  0x%08x  phy_port:  0x%08x  gport:  0x%08x", mc_group_id, sys_port, gport);
    ...
    snprintf(error_msg, sizeof(error_msg), "mc_group_id:  0x%08x  port:  0x%08x  encap_id:  0x%08x",
        mc_group_id, replications.port, replications.encap1);
    BCM_IF_ERROR_RETURN_MSG(bcm_multicast_add(unit, mc_group_id, BCM_MULTICAST_INGRESS_GROUP, 1, &replications), error_msg);
...
int vswitch_create(int unit, bcm_vlan_t  *new_vsi){
     ...
     */
    BCM_IF_ERROR_RETURN_MSG(open_ingress_mc_group(unit, vsi), "");
    if(util_verbose){
     ...
     */
    if (lif_offset != 0) {
        BCM_IF_ERROR_RETURN_MSG(open_ingress_mc_group(unit, vsi+lif_offset), "");
        if(util_verbose){
            printf("created multicast   0x%08x\n\r",vsi+lif_offset);
        }
    }
...
/*
 * Clear the ingress VLAN editor of PON LIF according to service type.
 */
int pon_port_ingress_vt_destroy(int unit, int service_type, bcm_vlan_t down_ovlan, bcm_vlan_t down_ivlan, 
                            bcm_gport_t pon_gport)
{
    ...
    if (ing_tag_format == any_tag) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_class_set( unit,  &action_class), "ing");
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_id_destroy_all(unit, BCM_VLAN_ACTION_SET_INGRESS), "ing");
    return BCM_E_NONE;
}
                            ...
                            bcm_gport_t pon_gport, int qos_map_id, int* action_id)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_id_create(unit, BCM_VLAN_ACTION_SET_INGRESS | BCM_VLAN_ACTION_SET_WITH_ID, &ing_vlan_action_id), "ing");
        
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_id_set(unit, BCM_VLAN_ACTION_SET_INGRESS, ing_vlan_action_id, &action), "ing");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_translation_set(unit, &port_trans), "ing");
...
/*
 * Clear the egress VLAN editor of PON LIF according to service type.
 */
int pon_port_egress_vt_destroy(int unit)
{
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_id_destroy_all(unit, BCM_VLAN_ACTION_SET_EGRESS), "egr");
    return BCM_E_NONE;
}
...
int pon_port_egress_vt_set(int unit, int service_type, bcm_tunnel_id_t tunnel_id, bcm_vlan_t up_ovlan, bcm_vlan_t up_ivlan, bcm_gport_t pon_gport)
{
    ...
    if (rv == BCM_E_NOT_FOUND) {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_id_create(unit, BCM_VLAN_ACTION_SET_EGRESS | BCM_VLAN_ACTION_SET_WITH_ID, &egr_vlan_action_id), "egr");

        BCM_IF_ERROR_RETURN_MSG(bcm_vlan_translate_action_id_set(unit, BCM_VLAN_ACTION_SET_EGRESS, egr_vlan_action_id, &action), "egr");
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_vlan_port_translation_set(unit, &port_trans), "egr");
