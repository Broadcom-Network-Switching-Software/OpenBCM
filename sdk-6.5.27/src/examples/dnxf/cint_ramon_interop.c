    ...
    int nof_legacy_ports)
{
    ...
    for (port_index = 0; port_index < nof_legacy_ports; ++port_index)
    {
        ...
        BCM_IF_ERROR_RETURN_MSG(bcm_port_resource_default_get(unit, port, 0, &port_resource_arr[port_index]), error_msg);
    }
    ...
    BCM_IF_ERROR_RETURN_MSG(bcm_port_resource_multi_set(unit, nof_legacy_ports, port_resource_arr), error_msg);
    ...
    int unit)
{
     ...
     */
    snprintf(error_msg, sizeof(error_msg), "unit %d.", unit);
    BCM_IF_ERROR_RETURN_MSG(bcm_port_config_get(unit, &port_config), error_msg);
     ...
     * Interop
     */
    BCM_IF_ERROR_RETURN_MSG(cint_ramon_interop(unit, legacy_ports, nof_legacy_ports), error_msg);
