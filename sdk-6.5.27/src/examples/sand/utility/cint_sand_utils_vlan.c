...
int vlan__init_vlan(int unit, int vlan) {
    ...
    if (is_device_or_above(unit, JERICHO2)) {
         ...
         */
        rv = bcm_multicast_group_get(unit, vlan, &flags_get);
        if (rv != BCM_E_NONE)
        {
            printf("vlan__init_vlan(): Error, bcm_multicast_group_get, mc_group_id %d\n", vlan);
            return rv;
        }
        ...
        if (flags_get)
        {
            ...
            if (rv != BCM_E_NONE)
            {
                printf("vlan__init_vlan(): Multicast group exists, try to destroy %d.\n",vlan);
                printf("vlan__init_vlan(): Error, bcm_multicast_destroy, flags_get %d mc_group_id %d \n", flags_get,vlan);
                return rv;
            }
        }
            ...
            if (is_mesh && (is_mesh_mc || is_tdm_supported)) {
                ...
                for(core_index = 0; core_index < max_nof_cores; core_index++) {
                    if ( valid_cores_bmp & (1 << (core_index))){
                        mreps[nof_cores]= core_index;
                        nof_cores ++;
                    }
                }
                rv = bcm_fabric_multicast_set(unit, vlan, 0, nof_cores, mreps);
                if (rv != BCM_E_NONE) {
