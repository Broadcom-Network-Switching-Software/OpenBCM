    ...
    int *is_jericho2)
{
    ...
    *is_jericho2 = (current_device_id_masked == 0x8690 || current_device_id_masked_j2c == 0x8800 || current_device_id_masked == 0x8480 || current_device_id_masked == 0x8850 || current_device_id_masked == 0x8280 || current_device_id_masked == 0x8290 || current_device_id_masked == 0x8840) ? 1 : 0;
    return rv;
}
    ...
    int supported_device_id)
{
    ...
    DEVICE_LIST[0] = ARAD_PLUS;
    DEVICE_LIST[1] = JERICHO;
    DEVICE_LIST[2] = QUMRAN_MX;
    DEVICE_LIST[3] = JERICHO_B0;
    DEVICE_LIST[4] = QUMRAN_MX_B0;
    DEVICE_LIST[5] = JERICHO_PLUS;
    DEVICE_LIST[6] = QUMRAN_AX;
    DEVICE_LIST[7] = QUMRAN_AX_B0;
    DEVICE_LIST[8] = QUMRAN_UX;
    DEVICE_LIST[9] = JERICHO2;
    DEVICE_LIST[10] = JERICHO2_B0;
    DEVICE_LIST[11] = JERICHO2C;
    DEVICE_LIST[12] = QUMRAN2A;
    DEVICE_LIST[13] = QUMRAN2A_B0;
    DEVICE_LIST[14] = QUMRAN2_UX;
    DEVICE_LIST[15] = QUMRAN2_N;
    DEVICE_LIST[16] = JERICHO2P;
    DEVICE_LIST[17] = JERICHO2X;
    ...
    if (current_device_id_masked == 0x8690 || current_device_id_masked == 0x8480 || current_device_id_masked == 0x8850 || current_device_id_masked == 0x8830)
    {
    ...
    else if (current_device_id_masked == 0x8290)
    {
        current_device_id = (QUMRAN2_N & 0xffff);
    }
    else if (current_device_id_masked == 0x8840)
    {
        current_device_id = current_device_id_masked | 0x10;
    }
    else
    {
...
int is_jericho2_kbp_ipv4_enabled(int unit) {
    return (is_device_or_above(unit, JERICHO2) && (*dnxc_data_get(unit, "elk", "application", "ipv4", NULL)));
}
...
int is_jericho2_kbp_ipv6_enabled(int unit) {
    return (is_device_or_above(unit, JERICHO2) && (*dnxc_data_get(unit, "elk", "application", "ipv6", NULL)));
}
...
/* indicate if the device of specific device_id
   Get unit - unit id
   device_id - device id
   yesno  -  indicate if the device is device_id */
int is_device(int unit, int device_id, int *yesno) {
    bcm_info_t info;
    ...
    *yesno =  ((info.device & 0xfff0) == (device_id & 0xfff0)) ? 1 : 0;
...
int vlan_create(int unit, int vid) {
    int rv = BCM_E_NONE;
    bcm_vlan_control_vlan_t vlan_control;
    int is_device_or_above_jr2 = is_device_or_above(unit,JERICHO2);

    /* To avoid BSL errorexit on JER2 above cint, use different routine */
    if (is_device_or_above_jr2)
    {
        rv = bcm_vlan_control_vlan_get(unit, vid, &vlan_control);
 ...
 */
int sand_utils_is_jr2_system_header_mode(int unit){
     ...
     return 0; /* In case not Jericho2 and above, then it is jr1 mode */
 }

int is_jr2_system_header_mode(int unit){
    return sand_utils_is_jr2_system_header_mode(unit);
}
