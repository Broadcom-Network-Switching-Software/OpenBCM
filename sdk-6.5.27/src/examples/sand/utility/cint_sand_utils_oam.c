...
/**
 * Set OAM NTP TOD to specificed seconds and nanoseconds
 *
 * @param unit
 * @param seconds
 * @param nanoseconds
 */
int set_oam_ntp_tod(int unit, int seconds, int nanoseconds) {
    uint64 time;
    uint64 frac_seconds;
    int ntp_freq_control = 0;
    int ptp_freq_control = 0;
    bcm_oam_control_type_t control_var = bcmOamControlNtpToD;

    /*
     * convert input nanoseconds to fractional seconds in (Counter26bit, Accum32bit)
     * frequency step = (ptp_freq_control >> 26) & 0x3f. If clock is 256MHz, ptp_freq_control is 0x10000000, frequency step is 4ns
    ...
    if (is_device_or_above(unit, JERICHO2))
    {
        ntp_freq_control = *(dnxc_data_get(unit, "oam", "general", "oam_ntp_time_freq_control", NULL));
        ptp_freq_control = *(dnxc_data_get(unit, "oam", "general", "oam_IEEE_1588_time_freq_control", NULL));
    }
    else
    {
        ntp_freq_control = 0x44b82fa1;
        ptp_freq_control = 0x10000000;
    }
    COMPILER_64_SET(frac_seconds, 0, nanoseconds);
    COMPILER_64_UMUL_32(frac_seconds, ntp_freq_control);
    COMPILER_64_UDIV_32(frac_seconds, ((ptp_freq_control >> 26) & 0x3f));
    COMPILER_64_SET(time, seconds, (COMPILER_64_HI(frac_seconds) << 6 | ((COMPILER_64_LO(frac_seconds) >> 26) & 0x3F)));

    return bcm_oam_control_set(unit,control_var,time );
}

/**
 * Get OAM NTP TOD(64bits) in which the higer 32 bits is seconds and the lower 32 bits is nanoseconds
 *
 * @param unit
 * @param seconds
 * @param nanoseconds
 */
int get_oam_ntp_tod(int unit, uint64 *time) {
    uint64 arg;
    uint64 tmp;
    uint64 frac_seconds;
    int seconds = 0;
    ...
    bcm_oam_control_get(unit, control_var, &arg);

    /*
     * The whole TOD value is Counter31bit, Counter1bit, Counter26bit, Accum32bit[31:26]
     * Get the lower 32 bits frac_seconds and convert it to actual nanoseconds
     * Actual nanoseconds = (frac_seconds << 26) * 1000000000 / (1 << 58) = frac_seconds * 1000000000 / (1 << 32)
     */
    seconds = COMPILER_64_HI(arg);
    COMPILER_64_SET(frac_seconds, 0, COMPILER_64_LO(arg));
    COMPILER_64_UMUL_32(frac_seconds, 1000000000);
    COMPILER_64_SET(tmp, 0x1, 0);
    COMPILER_64_UDIV_64(frac_seconds, tmp);
    nanoseconds = COMPILER_64_LO(frac_seconds);
    COMPILER_64_SET(*time, seconds, nanoseconds);
}
