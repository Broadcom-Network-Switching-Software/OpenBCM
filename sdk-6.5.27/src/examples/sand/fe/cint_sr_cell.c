    ...
    source_routed_cell_datapath_t datapath)
{
    ...
    {
        int is_dnx;
        bcm_device_member_get(dest_unit, 0, bcmDeviceMemberDNX, &is_dnx);
        /*
         */
        if(is_dnx)
        {
            data_valid_offset = *(dnxc_data_get(dest_unit, "fabric", "cell", "sr_cell_data_valid_offset", NULL));
        }
        else
        {
    ...
    int *link_ids)
{
     ...
     */
    rv = cint_sr_cell_system_verify(src_unit, dest_unit, link_ids[0], flags, datapath);
    if (rv != BCM_E_NONE)
    {
        ...
        int * modid_received)
{
    ...
    rv = cint_sr_cell_system_verify(tx_unit, rx_unit, link_ids[0], flags, datapath);
    if (rv != BCM_E_NONE)
    {
        printf("failed in cint_sr_cell_system_verify\n");
        return rv;
