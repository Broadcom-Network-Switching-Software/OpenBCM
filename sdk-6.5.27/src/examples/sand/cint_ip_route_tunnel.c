 ...
 * cint ../../../../src/examples/sand/utility/cint_sand_utils_mpls_port.c
 * cint ../../../../src/examples/sand/cint_ip_route.c
 * cint ../../../../src/examples/sand/cint_mutli_device_utils.c
 * cint ../../../../src/examples/sand/cint_ip_route_tunnel.c
 ...
 * port_to_system_port(units[0], outP, &outSysport);
 * port_to_system_port(units[0], inP, &inSysport);
 * print example_ip_to_tunnel(units[0], units[0], inSysport, outSysport);
 *
 * BCM.0> diag dbal te 0
 *
 ...
 *
 *  tx 1 psrc=13 data=000c00020000000000ab00128100000f08004500002a000000004e11ab1ac0a801017fffff030023029b00163b5b00000000000000000000000000007168b1fb
 *
 * expected:
 *  - out port = out_port
 *  - vlan 100.
 ...
 *
 *  Data: 00000000cd1d000c00020000810000648847000c81144500002a000000004d11ac1ac0a801017fffff030023029b00163b5b0000000000000000000000000000
 *
 * BCM.0> diag dbal lp
 *
 * BCM.0> diag pp encap core=1
 * Core 1:
 * eep[0]: 4096
 ...
 *
 *  tx 1 psrc=13 data=000c00020000000000ab00128100000f08004500002a000000004e11ab1ac0a801017fffff050023029b00163b5b00000000000000000000000000007168b1fb
 *
 * expected:
 *  - out port = out_port
 *  - vlan = 100.
 ...
 *  MPLS label: label 200, exp 2, ttl 20
 *
 *  Data: 00000000cd1d000c00020000810000648847000c81144500002a000000004d11ac1ac0a801017fffff050023029b00163b5b0000000000000000000000000000
 *
  * BCM.0> diag dbal lp
 *
 */
...
int create_C_tunnel(int unit, int encap_mode, int next_pointer)
{
    ...
    mpls_tunnel_properties.label_out = (encap_mode <= 2) ? additional_labels_info.C2_label : 0;
    mpls_tunnel_properties.ttl = additional_labels_info.C1_ttl;
    mpls_tunnel_properties.ext_ttl = (encap_mode <= 2) ? additional_labels_info.C2_ttl : 0;
    mpls_tunnel_properties.exp = additional_labels_info.C1_exp;
    mpls_tunnel_properties.ext_exp = (encap_mode <= 2) ? additional_labels_info.C2_exp : 0;
    ...
    /*set for C3*/
    if ((encap_mode == 1) || (encap_mode == 2) || (encap_mode == 0)) {
...
int create_B_tunnel(int unit, int encap_mode)
{
    ...
    mpls_tunnel_properties.label_in = (encap_mode >= 1) ? additional_labels_info.B1_label : 0;
    mpls_tunnel_properties.label_out = (encap_mode > 1) ? additional_labels_info.B2_label : 0;
    mpls_tunnel_properties.ttl = (encap_mode >= 1) ? additional_labels_info.B1_ttl : 0;
    mpls_tunnel_properties.ext_ttl = (encap_mode > 1) ? additional_labels_info.B2_ttl : 0;
    mpls_tunnel_properties.exp = (encap_mode >= 1) ? additional_labels_info.B1_exp : 0;
    mpls_tunnel_properties.ext_exp = (encap_mode > 1) ? additional_labels_info.B2_exp : 0;
    mpls_tunnel_properties.with_exp = 1;
...
int tunnels_create(int unit, int encap_mode, int next_pointer, int * last_tunnel_id)
{
    ...
    if (!is_device_or_above(unit, JERICHO2)) {
        ...
        *last_tunnel_id = ((encap_mode >= 1) && (encap_mode <= 2)) ? additional_labels_info.B_intf : additional_labels_info.C_intf;
    } else {
        ...
        */
        ...
        if (encap_mode == 3) {
            ...
            label_index = 2;
            nof_labels = 1;
        } else if (encap_mode == 2) {
            /** Create B and C tunnel*/
            label_index = 0;
            nof_labels = 5;
        } else if (encap_mode == 1) {
            /** Create B and C tunnel*/
            label_index = 1;
            nof_labels = 4;
 ...
 * port_to_system_port(units[0], outP, &outSysport);
 * port_to_system_port(units[0], inP, &inSysport);
 * // encapsulation C1+C2+C3
 * print example_ip_to_tunnel_ext_label(units, nof_units, inSysport, outSysport, 0);
 *
 * Packet to send:
 *      tx 1 psrc=13 data=000c000200000000070001008100000f0800458c003d000000008011b11c0a0000057fffff03c351000000290000000102030405060708090a0b0c0d0e0f1011
 *
 * Expected packet:
 *      Data: 00000000cd1d000c000200008100006488470005587f0004487f0003397f458c003d000000007f11b21c0a0000057fffff03c351000000290000000102030405060708090a0b0c0d0e0f1011
 *
 * Packet to send:
 *       tx 1 psrc=13 data=000C000200000000070000018100000F86DD60300000002A3BFFFE80000000000000020007FFFE00000120010000000000000000000000000004000102030405
 *
 * Expected packet:
 *      Data: 00000000cd1d000c00020000810000648847000550fe000440fe000331fe60300000002a3bfefe80000000000000020007fffe00000120010000000000000000000000000004000102030405
 *
 * BCM.0> diag pp encap core=1
 * Core 1:
 * eep[0]: 8192
 ...
 * tm_port: 200
 *
 * // encapsulation B1+C1+C2+C3
 * print example_ip_to_tunnel_ext_label(units, nof_units, inSysport, outSysport, 1);
 *
 * Packet to send:
 *      tx 1 psrc=13 data=000c000200000000070001008100000f0800458c003d000000008011b11c0a0000057fffff03c351000000290000000102030405060708090a0b0c0d0e0f1011
 *
 * Expected packet:
 *      Data: 00000000cd1d000c000200008100006488470005587f0004487f0003387f0001197f458c003d000000007f11b21c0a0000057fffff03c351000000290000000102030405060708090a0b0c0d0e0f1011
 *
 * Packet to send:
 *      tx 1 psrc=13 data=000C000200000000070000018100000F86DD60300000002A3BFFFE80000000000000020007FFFE00000120010000000000000000000000000004000102030405
 *
 * Expected packet:
 *      Data: 00000000cd1d000c00020000810000648847000550fe000440fe000330fe000111fe60300000002a3bfefe80000000000000020007fffe00000120010000000000000000000000000004000102030405
 *
 * BCM.0> diag pp encap core=1
 * Core 1:
 * eep[0]: 16384
 ...
 *
 * eep[1]: 8192
 * encap_info[1]:MPLS Encapsulation:
 * nof_tunnels: 2
 * tunnel 1:
 *   tunnel_label: 51
 *   push_profile: 1
 ...
 * tm_port: 200
 *
 * // encapsulation B1+B2+C1+C2+C3
 * print example_ip_to_tunnel_ext_label(units, nof_units, inSysport, outSysport, 2);
 *
 * Packet to send:
 *      tx 1 psrc=13 data=000c000200000000070001008100000f0800458c003d000000008011b11c0a0000057fffff03c351000000290000000102030405060708090a0b0c0d0e0f1011
 *
 * Expected packet:
 *      Data: 00000000cd1d000c000200008100006488470005587f0004487f0003387f0002287f0001197f458c003d000000007f11b21c0a0000057fffff03c351000000290000000102030405060708090a0b0c0d0e0f1011
 *
 * Packet to send:
 *      tx 1 psrc=13 data=000C000200000000070000018100000F86DD60300000002A3BFFFE80000000000000020007FFFE00000120010000000000000000000000000004000102030405
 *
 * Expected packet:
 *      Data: 00000000cd1d000c00020000810000648847000550fe000440fe000330fe000220fe000111fe60300000002a3bfefe80000000000000020007fffe00000120010000000000000000000000000004000102030405
 *
 * BCM.0> diag pp encap core=1
 * Core 1:
 * eep[0]: 16384
...
int example_ip_to_tunnel_ext_label(int *units_ids, int nof_units, int in_sysport, int out_sysport, int encap_mode){
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        if (is_device_or_above(unit, JERICHO2)) {
            ...
            route_qos_lif = ((encap_mode >= 1)&&(encap_mode <= 2)) ? additional_labels_info.B_intf : additional_labels_info.C_intf;
    ...
    for (i = 0 ; i < nof_units ; i++){
        ...
        fec_structure.tunnel_gport = ((encap_mode >= 1)&&(encap_mode <= 2)) ? additional_labels_info.B_intf : additional_labels_info.C_intf;
