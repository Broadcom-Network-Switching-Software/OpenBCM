 ...
 * 
 ...
 Copyright 2007-2022 Broadcom Inc. All rights reserved. File: cint_sand_rfc6374.c Purpose: basic examples for MplsLmDm.
 */
...
int mpls_lm_dm_endpoint_create(int unit, bcm_oam_endpoint_type_t type, int use_mpls_out_gport, int set_as_accelerated, int count_lm_dm)
{
    ...
    if (device_type >= device_type_jericho2)
    {
        ...
        {
            rv = mpls_oam_init(unit, bcmOAMEndpointTypeBHHMPLS, 0, &gport, &mpls_outlif, &arp_out_lif, &mpls_label, &mpls_out_gport, &tunnel_switch, &mpls_termination_label_index_enable);
            if (rv != BCM_E_NONE) {
        ...
        {
            rv = mpls_oam_init(unit, bcmOAMEndpointTypeBHHPwe, 0, &gport, &mpls_outlif, &arp_out_lif, &mpls_label, &mpls_out_gport, &tunnel_switch, &mpls_termination_label_index_enable);
            if (rv != BCM_E_NONE) {
              printf("Error, in mpls_init\n");
              return rv;
            }
        ...
        {
            rv = mpls_oam_init(unit, bcmOAMEndpointTypeBhhSection, 0, &gport, &mpls_outlif, &arp_out_lif, &mpls_label, &mpls_out_gport, &tunnel_switch, &mpls_termination_label_index_enable);
            if (rv != BCM_E_NONE) {
              printf("Error, in mpls_init\n");
              return rv;
            }
    ...
    if (set_as_accelerated)
    {
        ...
        delay_obj.period = BCM_OAM_ENDPOINT_CCM_PERIOD_100MS;
        if (device_type < device_type_jericho2)
        {
            /**
             * For Jericho and Jericho_plus the default timestamp_format is bcmOAMTimestampFormatNTP,
