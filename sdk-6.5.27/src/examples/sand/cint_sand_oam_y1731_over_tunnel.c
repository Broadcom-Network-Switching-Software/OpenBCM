...
/** used arp outlif when creating ep for oam over mpls */
int use_arp_outlif_in_ep = 0;
 ...
 */
int mpls_oam_init(int unit, bcm_oam_endpoint_type_t type, int use_mpls_out_gport, int *gport, int *outlif, int *arp_out_lif, int *label, bcm_gport_t *out_gport, bcm_mpls_tunnel_switch_t *tunnel_switch, int *mpls_termination_label_index_enable)
{
  bcm_error_t rv = 0;
  ...
  if (type==bcmOAMEndpointTypeBHHMPLS) {
    if ((*out_gport==BCM_GPORT_INVALID) && !is_device_or_above(unit, JERICHO2))  {
      ...
      *outlif = mpls_util_entity[0].mpls_encap_tunnel[0].tunnel_id;
      ...
      /** Arp Out-LIF */
      *arp_out_lif = mpls_util_entity[1].arps.arp;
...
int oam_example_over_tunnel(int unit,  bcm_oam_endpoint_type_t type, int use_mpls_out_gport)
{
    ...
    rv = mpls_oam_init(unit, type, use_mpls_out_gport, &gport, &outlif, &arp_out_lif, &label, &out_gport, &tunnel_switch, &mpls_termination_label_index_enable);
    if (rv != BCM_E_NONE)
    {
        printf("(%s) \n", bcm_errmsg(rv));
        return rv;
    ...
    */
    ...
    mep_acc_info.intf_id = use_arp_outlif_in_ep ? arp_out_lif :  mep_acc_info.intf_id;
