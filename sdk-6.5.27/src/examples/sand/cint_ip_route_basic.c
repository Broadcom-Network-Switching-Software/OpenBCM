 ...
 */
int esem_default_invlaid = -1;
int already_called = 0;
int default_vlan_port_id_out = esem_default_invlaid;
...
cint_ip_route_basic_s cint_ip_route_info = {
/** Ingress interface */
15,
/** Egress interface */
100,
    ...
    int out_port)
{
    ...
    if (default_vlan_port_id_out != esem_default_invlaid)
    {
    ...
    int arp_plus_ac_type)
{
    ...
    rv = l3__egress_only_fec__create(unit, fec, 0, encap_id_itf, gport, 0);
    if (rv != BCM_E_NONE)
    {
        printf
            ("%s(): Error, create egress object FEC only: fec = %d, intf_out = %d, encap_id = %d, out_port = %d, err_id = %d\n",
    ...
    int out_port)
{
    ...
    {
        rv = get_first_fec_in_range_which_not_in_ecmp_range(ingress_unit, 0, &fec);
        if (rv != BCM_E_NONE)
        {
            printf("Error, get_first_fec_in_range_which_not_in_ecmp_range\n");
            return rv;
     ...
     * 5. Create FEC and set its properties
     */
    rv = l3__egress_only_fec__create(ingress_unit, fec, intf_out, encap_id, out_port, 0);
    if (rv != BCM_E_NONE)
    {
        printf
            ("%s(): Error, create egress object FEC only: fec = %d, intf_out = %d, encap_id = %d, out_port = %d, err_id = %d\n",
