...
/*
 * Store the base SAR stream ID offset
 */
static int sar_base_stream_id_offset = -1;
/*
    ...
    int *user_field)
{
    ...
    char *proc_name;
    int is_dnx = 0;
    int stream_id_offset = local_port_id;
    ...
    rv = BCM_E_NONE;
    {
        ...
        /*
         * For SAR port, get the base stream ID offset
         * Store the offset in sar_base_stream_id_offset to avoid the offset
         * is modified by the new SAR ports.
         */
        if (flags & (BCM_PORT_ADD_FLEXE_SAR | BCM_PORT_ADD_OTN_SAR))
        {
            if (sar_base_stream_id_offset == -1)
            {
                rv = bcm_port_config_get(unit, &config);
                if (BCM_FAILURE(rv))
                {
                    printf("Error, in bcm_port_config_get, unit %d.\n", unit);
                    return rv;
                }
                BCM_PBMP_ITER(config.framer_sar, logical_port)
                {
                    sar_base_stream_id_offset = logical_port;
                    break;
                }
            }
            stream_id_offset = sar_base_stream_id_offset;
        }
    }
    /*
     * Set the destination and editing flags (for the Ingress)
    ...
    tdm_interface_config.stream_id_base = 1000 + stream_id_offset;
    ...
    int mc_port2)
{
    ...
    rv = bcm_port_get(src_unit, local_port_id, &flags, &interface_info, &mapping_info);
    if (rv != BCM_E_NONE)
    {
    ...
    /** mark dest port as SAR in case of multicast */
    if (is_mc)
    {
        flags = 0;
        rv = bcm_port_get(dst_unit, dest_gport_logical, &flags, &interface_info, &mapping_info);
        if (rv != BCM_E_NONE)
        {
            printf("\r\n %s(): 1. 'setup_dnx_tdm' has failed with code %d. Quit.\r\n", proc_name, rv);
            return rv;
    ...
    if (src_sar)
    {
        egress_edit_type = bcmTdmEgressEditingNoStamp;
    }
    else if (is_opt)
    {
        /*
    ...
    if (is_mc)
    {
        ...
        rv = multicast__open_egress_mc_group_with_local_ports(dst_unit, mc_group_id, mc_ports, mc_cuds, num_in_mcast_group, flags);
        if (rv != BCM_E_NONE)
        {
    ...
    int max_size)
{
    ...
    rv = cint_dnx_tdm_example_multi_device(src_unit, dst_unit,
                                         local_port_id,
                                         dest_gport,
                                         is_mc,
                                         mc_group_id,
    ...
    int mc_id)
{
    ...
    proc_name = "cint_dnx_tdm_sar_example";
    rv = BCM_E_NONE;
    if (min_size > max_size)
    {
        rv = BCM_E_PARAM;
