...
int field_jr2_comp_evpn_esi_db(int unit, int presel_id) {
    ...
    if(rv == BCM_E_NONE) {
        /** In J2P and above devices, sys_port locates at learn_payload0[26:6][15:0]}*/
        if (field_action_learn_sys_port[0] == bcmFieldActionCount) {
            /** Destination is 21bit = {5'b01100, sys_port(16b)}, we use [26:0] since no way to pad value to UDF action*/
            bcm_field_action_info_t_init(&action_info);
            action_info.action_type = bcmFieldActionLearnPayload1;
            action_info.prefix_size = 5;
            action_info.prefix_value = 0x18;  /** Learn_Payload_Result_Format - DEST__LIF__XL_FGID */
            action_info.size = 27;
            action_info.stage = bcmFieldStageIngressPMF2;
            void_ptr = &(action_info.name[0]);
        ...
        /** In other devices, sys_port locates at {learn_raw_data1[29:9][15:0]}*/
        if (field_action_learn_sys_port[0] == bcmFieldActionCount) {
            /** Destination is 21bit = {5'b01100, sys_port(16b)}, we use [29:0] since no way to pad value to UDF action*/
            bcm_field_action_info_t_init(&action_info);
            action_info.action_type = bcmFieldActionLearnRaw1;
            action_info.prefix_size = 2;
            action_info.prefix_value = 0x0; /** Learn_Payload_Result_Format[1:0]*/
            action_info.size = 30;
            action_info.stage = bcmFieldStageIngressPMF2;
            void_ptr = &(action_info.name[0]);
    ...
    learn_data[0] = dst_sys_port << destination_offset;
...
int field_jr2_comp_evpn_egress_filter_db(int unit, int presel_id) {
    ...
    rv = bcm_field_group_add(unit, 0, &fg_info, &egress_fg_id);
    if (rv != BCM_E_NONE)
    {
        printf("%s Error (%d), in bcm_field_group_add\n", proc_name, rv);
        return rv;
