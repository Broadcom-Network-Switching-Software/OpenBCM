    ...
    int target_size)
{
    ...
    loc_len = sal_strnlen(source, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
...
utilex_str_append(
    ...
    char *add)
{
    if (sal_strnlen(base, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) != 0)
        sal_strncpy_s(base + sal_strnlen(base, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH), " ", 2);
    sal_strncpy_s(base + sal_strnlen(base, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH), add,
                  sal_strnlen(add, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) + 1);
}
    ...
    uint32 *realtokens)
{
    ...
    i = 0;
    while (((next =
             sal_strnstr(string, delim, sal_strnlen(string, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))) != NULL)
           && (i < (maxtokens - 1)))
    {
        if ((tokens[i] =
             sal_alloc(sal_strnlen(string, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) + 1, "token")) == NULL)
            SHR_EXIT();
        sal_memset(tokens[i], 0, sal_strnlen(string, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) + 1);
        /*
         ...
         */
        string = next + sal_strnlen(delim, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    }
    /*
     * Once end of line or maxtokens achieved - string will keep the last part
     */
    if ((tokens[i] = sal_alloc(sal_strnlen(string, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) + 1, "token")) == NULL)
        SHR_EXIT();
    sal_memset(tokens[i], 0, sal_strnlen(string, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) + 1);
    sal_strncpy_s(tokens[i++], string, sal_strnlen(string, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) + 1);
    ...
    char to)
{
    ...
    loc_len = sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    ...
    int token_size)
{
    ...
    actual_size =
        (token_size <
         sal_strnlen(string, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) ? token_size : sal_strnlen(string,
    ...
    char *str)
{
    ...
    int shift = 0;
    char last_char = 0;
    int str_size = sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    /*
     ...
     */
    for (i_char = sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) - 1; i_char > 0; i_char--)
    {
...
{
    char *str = *str_p;
    int current = sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
     ...
     */
    current = 0;
    while (current < sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
    {
        if ((str[current] != ' ') && (str[current] != ',') && (str[current] != '\t') && (str[current] != '\r') &&
            (str[current] != '\n'))
            break;
    ...
    uint32 *value_p)
{
    ...
    if (sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) == 0)
    {
        return 0;
    }
    ...
    while ((*str == '0') && (sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) > 1))
    {
        str++;
    }

    /** Each string char represents a byte, so we want to calculate number of uint32 in the hex string */
    val_size = (sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) + 7) / 8;
     ...
     */
    nibble_count = 8 - sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) % 8;
    ...
    const char *out_sub_str)
{
    ...
    str_len = sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    in_sub_str_location =
        ((int)
         (sal_strnstr(str, in_sub_str, sal_strnlen(str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)) -
          str)) / (sizeof(char));
    ...
    ii += sal_strnlen(out_sub_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
    jj += sal_strnlen(in_sub_str, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
