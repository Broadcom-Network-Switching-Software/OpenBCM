    ...
    uint32 *timers_group_p)
{
    ...
    *timers_group_p = UTILEX_LL_TIMER_NO_GROUP;
    ...
    for (ii = 0; ii < UTILEX_LL_TIMER_MAX_NOF_TIMER_GROUPS; ii++, utilex_ll_timer_group++)
    {
         ...
         */
        sal_strncpy(utilex_ll_timer_group->name, group_name, sizeof(utilex_ll_timer_group->name));
    ...
    uint32 timers_group)
{
    ...
    sal_strncpy_s(utilex_ll_timer_group->name, UTILEX_LL_TIMER_EMPTY_GROUP_NAME, sizeof(utilex_ll_timer_group->name));
    ...
    uint32 timers_group)
{
    ...
    {
        sal_strncpy_s(utilex_ll_timer_function->name, "no_function", sizeof(utilex_ll_timer_function->name));
    ...
    uint32 timer_ndx)
{
    ...
    if (utilex_ll_timer_function->nof_hits == 0)
    {
         ...
         */
        sal_strncpy_s(utilex_ll_timer_function->name, name, sizeof(utilex_ll_timer_function->name));
    ...
    void)
{
    ...
    ret = _SHR_E_NONE;
    {
        ...
        {
            sal_snprintf(group_name[ii], sizeof(group_name[ii]), "%s%d", "Group", (int) ii);
    ...
    }
    {
        ...
        timer_ndx[1] = 1;
        timers_group = 0;
        sal_snprintf(group_name[0], sizeof(group_name[0]), "%s%d", "TEST", (int) timers_group);
        err = utilex_ll_timer_group_allocate(group_name[0], &timers_group);
        if (err != _SHR_E_NONE)
        {
 ...
 * \brief
 * Convert an uint32 array to an uint8 array
 * where copy is started each time from the MSByte, Big Endian style
 *
 * \par DIRECT INPUT
 *   \param [in] *u32_val  - \n
 *       pointer to an array of uint32
    ...
    uint8 *u8_val)
{
    ...
    cur_u8_indx = sizeof(uint32);
    for (cur_u32 = u32_val, u8_indx = 0; u8_indx < nof_bytes; ++u8_indx)
    {
        u8_val[u8_indx] = (uint8)
            UTILEX_GET_BITS_RANGE(*cur_u32, cur_u8_indx * SAL_UINT8_NOF_BITS - 1,
                                  (cur_u8_indx - 1) * SAL_UINT8_NOF_BITS);
 ...
 */
    sal_snprintf(decimal_ip, UTILEX_PP_IP_STR_SIZE, format,
