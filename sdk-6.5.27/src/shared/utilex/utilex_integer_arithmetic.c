    ...
    uint32 *exp)
{
    ...
    COMPILER_UINT64 max_val_mult_u64, abs_val_numerator_u64, tmp_calc;
    ...
    if (abs_val_numerator <= abs_val_denominator)
    {
        *mnt_bin_fraction = (1 << mnt_nof_bits) - 1;
        /*
         * since mnt_bin_fraction is 32 bits, if high 32 bits in max_val are != 0,
         * this means that mnt_bin_fraction is smaller than max_val so we skip the check
         */
        if (COMPILER_64_HI(max_val) == 0)
        {
            if (*mnt_bin_fraction > COMPILER_64_LO(max_val) - 1)
            {
                *mnt_bin_fraction = COMPILER_64_LO(max_val) - 1;
            }
        }
    ...
    COMPILER_64_COPY(max_val_mult_u64, max_val);
    ...
    {
        COMPILER_64_SET(tmp_calc, 0, real_max_val);
        COMPILER_64_UMUL_32(tmp_calc, abs_val_denominator);
        COMPILER_64_UDIV_64(tmp_calc, max_val);
        if (COMPILER_64_HI(tmp_calc))
        {
            /*
             * Enter in case of overflow.
             */
        ...
        tmp_num = COMPILER_64_LO(tmp_calc);
    }
    else
    {
    ...
    uint32 *abs_val_denominator)
{
    ...
    *abs_val_numerator = 0;
    ...
    /*
     * since abs_val_numerator is 32 bits, if high 32 bits in max_val are != 0,
     * this means that abs_val_numerator is smaller than max_val so we skip the check
     */
    if (COMPILER_64_HI(max_val) == 0)
    {
        if ((*abs_val_numerator) / (*abs_val_denominator) > COMPILER_64_LO(max_val))
        {
            (*abs_val_numerator) = COMPILER_64_LO(max_val) * (*abs_val_denominator);
        }
    }
exit:
    return ex;
    ...
    for (max_val_i = 100; max_val_i < 4000; max_val_i += 100)
    {
        for (abs_val_denominator_i = 1; abs_val_denominator_i < 60; abs_val_denominator_i++)
        {
            for (abs_val_numerator_i = abs_val_denominator_i; abs_val_numerator_i < 600; abs_val_numerator_i++)
            {
                ...
                COMPILER_64_SET(tmp_max_val_i, 0, max_val_i);
                utilex_abs_val_to_mnt_binary_fraction_exp(abs_val_numerator_i,
                                                          abs_val_denominator_i, 5, 3, tmp_max_val_i, &mnt, &exp);
                utilex_mnt_binary_fraction_exp_to_abs_val(5, 3, tmp_max_val_i, mnt,
