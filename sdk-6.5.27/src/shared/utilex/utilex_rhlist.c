    ...
    int sanity)
{
    ...
    if (rhlist)
    {
        ...
        if (name != NULL)
        {
            if (sal_strnlen(name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) >= RHNAME_MAX_SIZE)
            {
    ...
    rhhandle_t * rhentry_p)
{
    ...
    {
        /*
         * Don't fail on sanity check if deliberately including adapter unsupported tests 
         */
        if (utilex_rhlist_name_exists(rhlist, name, rhentry_p) == _SHR_E_NONE && !rhlist->include_unsupported)
        {
            LOG_ERROR(BSL_LOG_MODULE, (BSL_META("Sanity check on name:%s failed\n"), name));
            ret = _SHR_E_PARAM;
            goto out;
        }
        else if (sal_strnlen(name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) >= RHNAME_MAX_SIZE)
        {
            ret = _SHR_E_INTERNAL;
            goto out;
        }
    ...
    rhhandle_t * rhentry_p)
{
    ...
    {
        int status = sal_strncasecmp(name, RHNAME(entry_prev), SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH);
        if (status == 0)
        {
            /*
    ...
    char *name)
{
    ...
    for (entry = rhlist->top; entry; entry = entry->next)
    {
        if (!sal_strncasecmp(name, entry->name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            return entry;
    }
    ...
    char *name)
{
    ...
    for (entry = rhlist->top; entry; entry = entry->next)
    {
        if (!sal_strncmp(name, entry->name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
            return entry->id;
    }
    ...
    rhhandle_t * rhentry_p)
{
    ...
    for (entry = rhlist->top; entry; entry = entry->next)
        if (!sal_strncmp(entry->name, name, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH))
        {
            ret = _SHR_E_NONE;
            if (rhentry_p != NULL)
                *rhentry_p = entry;
