...
((seq->multithread_en && !(utilex_seq_is_kbp_step(current_step->step_name))) || (seq->kbp_multithread_en && utilex_seq_is_kbp_step(current_step->step_name)))

#define UTILEX_SEQ_IS_MULTITHREAD_EN_JOIN(seq)\
(seq->multithread_en || seq->kbp_multithread_en)

/*
 *  \brief taken from diag_sand_framework.h - SH_SAND_MAX_TOKEN_SIZE
 *
 */
#define UTILEX_SEQ_MAX_STEP_NAME_SIZE 1024

#define MEASURMENT_HANDLE_SIZE 30
/**
...
/*
 * Unified print function for all 3 error cases:
 *  -> time + UP deviation
 *  -> time + non-perc UP deviation
 *  -> time - Down Deviation
 * The function doesn't return anything. It is used
 * only for printing the error.
 * It can't be shr_error_e as the error is updated
 * via utilex_seq_update_test_info
 *
 */
static void
utilex_seq_init_time_error_print(
    int unit,
    sal_usecs_t measured_step_time,
    sal_usecs_t expected_step_time,
    utilex_seq_step_t * current_step,
    utilex_step_time_dnx_data step_id_dnx_data,
    uint8 flag)
{
    char device_short_name[RHFILE_MAX_SIZE] = { 0 };
    char device_name[RHFILE_MAX_SIZE] = { 0 };
    BSL_LOG(BSL_LOG_MODULE, ("#############################################\n"));
    BSL_LOG(BSL_LOG_MODULE, ("Error!!! Step %s ID=%d Failed!\n", current_step->step_name, current_step->step_id));
    if (measured_step_time > expected_step_time)
    {
        BSL_LOG(BSL_LOG_MODULE,
                ("The test measured %u microseconds but expected to be less than %u microseconds", measured_step_time,
                 expected_step_time));
        if (flag)
        {
            BSL_LOG(BSL_LOG_MODULE,
                    ("(time_value(%u) + non-perc UP deviation (%u))\n", step_id_dnx_data.step_time,
                     step_id_dnx_data.non_perc_up_deviation));
        }
        BSL_LOG(BSL_LOG_MODULE,
                ("\n1) Please check the reason why the time got increased and if it's due to a bug fix it!\n"));
    }
    else
    {
        BSL_LOG(BSL_LOG_MODULE,
                ("The test measured %u microseconds but expected to in range (%u - %u) microseconds",
                 measured_step_time, expected_step_time,
                 (step_id_dnx_data.step_time + ((step_id_dnx_data.step_time * step_id_dnx_data.up_deviation) / 100))));
        BSL_LOG(BSL_LOG_MODULE,
                ("\n1) Please check the reason why the time got decreased and if it's due to a bug fix it!\n"));
    }
    BSL_LOG(BSL_LOG_MODULE, ("2) If the new time is the legit time update the DNX DATA\n"));
    BSL_LOG(BSL_LOG_MODULE, (" - How to update the DNX DATA? \n"));
#if !defined(NO_FILEIO)
    dbx_file_get_db_short_name(unit, device_name, device_short_name);
#endif
    if (!(sal_strncmp(device_short_name, "", sizeof(device_short_name))))
    {
        BSL_LOG(BSL_LOG_MODULE,
                ("   - Open $SDK/tools/autocoder/DeviceData/dnxf/%s_a0/%s_a0_dev_init.xml\n", device_name,
                 device_name));
    }
    else
    {
        BSL_LOG(BSL_LOG_MODULE,
                ("   - Open $SDK/tools/autocoder/DeviceData/dnx/%s/%s_dev_init.xml\n", device_short_name,
                 device_short_name));
    }
    BSL_LOG(BSL_LOG_MODULE, ("   - Under sub_module = time, table = step_time, entries\n"));
    BSL_LOG(BSL_LOG_MODULE,
            ("       - Find step: %s with Enum ID %d with time=%d \n", current_step->step_name,
             current_step->step_id, step_id_dnx_data.step_time));
    BSL_LOG(BSL_LOG_MODULE,
            ("       - Update the time value(%d) with the new measured value(%d)\n",
             step_id_dnx_data.step_time, measured_step_time));
    BSL_LOG(BSL_LOG_MODULE,
            ("       - In case of inconsistent measurements, you can add down_deviation and up_deviation for that particular step_id.\n"));
    BSL_LOG(BSL_LOG_MODULE, ("#############################################\n"));
}
...
utilex_seq_is_kbp_step(
    char *step_name)
{
    if (sal_strnstr(step_name, "KBP", UTILEX_SEQ_MAX_STEP_NAME_SIZE) != NULL)
    {
        return TRUE;
    }
    return FALSE;
    ...
    int forward)
{
     ...
     */
    if (seq->mem_test_en && (seq->first_step != UTILEX_SEQ_STEP_INVALID || seq->last_step != UTILEX_SEQ_STEP_INVALID))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "Running memory test is not supported on partial sequence\n");
    }
...
utilex_seq_run_step_list_forward_in_parallel(
    ...
    void *input)
{
    utilex_seq_parallel_run_input_t *info = NULL;

    SHR_FUNC_INIT_VARS(unit);

    info = (utilex_seq_parallel_run_input_t *) input;

    /** Run init list function */
    SHR_IF_ERR_EXIT_WITH_LOG(utilex_seq_run_step_list_forward
                             (unit, info->seq, info->depth, info->step_list, info->time_overall, info->swstate_overall,
                              info->memory_overall, info->is_first_done, info->is_last_done),
                             "A parallel init step list (%s) failed on external thread, asserting.%s%s",
                             info->step_name, EMPTY, EMPTY);
...
utilex_seq_run_execute_forward_in_parallel(
    ...
    void *input)
{
    utilex_seq_parallel_execute_input_t *info = NULL;
    SHR_FUNC_INIT_VARS(unit);

    info = (utilex_seq_parallel_execute_input_t *) input;

    /** Run init list function */
    SHR_IF_ERR_EXIT_WITH_LOG(info->forward(unit), "A parallel init step (%s) failed on external thread, asserting.%s%s",
                             info->step_name, EMPTY, EMPTY);
...
/*
 * \brief - function to informatively print ignored memory in step.
 */
static inline void
utilex_seq_print_ignored_memory_in_step(
    utilex_seq_t * seq,
    uint32 log_severity,
    unsigned long memory_difference,
    int depth,
    uint8 is_alloc,
    uint8 is_forward)
{
    ...
    if (is_forward)
    {
        BSL_LOG(BSL_LOG_MODULE | log_severity, ("     - "));
        BSL_LOG(BSL_LOG_MODULE | log_severity,
                ("Ignored Memory:  %lu bytes %s\n", memory_difference, is_alloc ? "allocated" : "freed"));
    }
    else
    {
        BSL_LOG(BSL_LOG_MODULE | log_severity,
                ("     - Ignored Memory %s:   %lu bytes\n", is_alloc ? "allocated" : "freed", memory_difference));
    }
}
    ...
    int *is_last_done)
{
    ...
    unsigned long ignore_alloc_start;
    unsigned long ignore_free_start;
    unsigned long ignore_alloc_end;
    unsigned long ignore_free_end;
    unsigned long alloc_bytes_count_offset;
    unsigned long free_bytes_count_offset;
    char measurment_handle[MEASURMENT_HANDLE_SIZE];
    char seq_prefix[20];
    ...
    for (step_index = 0; step_index < nof_steps_in_list; ++step_index)
    {
        ...
        if (current_step->dyn_flags != NULL && ((total_flags & UTILEX_SEQ_STEP_F_SKIP) == 0))
        {
            dynamic_flags = 0;
        ...
        if ((total_flags & UTILEX_SEQ_STEP_F_USE_DMA) && seq->desc_init_step_enabled_counter_inc != NULL)
        {
            SHR_IF_ERR_EXIT(seq->desc_init_step_enabled_counter_inc(unit));
        }
        ...
        if (UTILEX_SEQ_IS_MULTITHREAD_EN_JOIN(seq) && (total_flags & UTILEX_SEQ_STEP_F_JOIN_ALL_THREADS))
        {
            ...
            for (; thread_id < thread_count; thread_id++)
            {
                ...
                {
                    rv = shr_thread_manager_join(&(seq->active_threads[thread_id]), 60 * SECOND_USEC);
                    if (rv == _SHR_E_FAIL)
                    {
                        seq->last_step = current_step->step_id;
                        SHR_IF_ERR_EXIT(rv);
                    }
                    seq->active_threads[thread_id] = NULL;
        ...
        if (seq->mem_test_en || seq->mem_log_en)
        {
            ...
            sal_get_alloc_counters_main_thr_ignore(&(ignore_alloc_start), &(ignore_free_start));
        }
        ...
        if (seq->swstate_log_en)
        {
            ...
            sal_snprintf(measurment_handle, MEASURMENT_HANDLE_SIZE - 1, "Utilex%s%d", seq_prefix,
        ...
        else
        {
            ...
            if (is_enabled == 1)
            {
                ...
                if (current_step->forward != NULL)
                {
                    ...
                    {
                        /**
                         * Run forward function, in case of a failure make sure that
                         * backward sequence will start running from current step
                         */
                        rv = current_step->forward(unit);
                        if (SHR_FAILURE(rv))
                        {
                            seq->last_step = current_step->step_id;

                            /*
                             * join all active threads from parallel steps
                             */
                            thread_id = 0;
                            thread_count = seq->active_threads_nof;
                            if (thread_count > 0)
                            {
                                ...
                                BSL_LOG(BSL_LOG_MODULE | log_severity,
                                        (" -> Due to error in current step, waiting for all active parallel init steps' threads to finish.\n"));

                                for (; thread_id < thread_count; thread_id++)
                                {
                                    if (seq->active_threads[thread_id])
                                    {
                                        SHR_IF_ERR_EXIT(shr_thread_manager_join
                                                        (&(seq->active_threads[thread_id]), 60 * SECOND_USEC));
                                        seq->active_threads[thread_id] = NULL;
                                        seq->active_threads_nof--;
                                    }
                                }
                ...
                if (current_step->step_sub_array != NULL)
                {
                        ...
                        && (total_flags & UTILEX_SEQ_STEP_F_RUN_ON_THREAD))
                    {
                        ...
                        thread_config_info.bsl_module = BSL_LOG_MODULE;
                        thread_config_info.type = SHR_THREAD_MANAGER_TYPE_RUN_TO_COMPLETION;
                        thread_config_info.callback = utilex_seq_run_step_list_forward_in_parallel;
                        thread_config_info.user_data = thread_info_p;
                        thread_config_info.thread_priority = 1;
        ...
        if (seq->mem_test_en || seq->mem_log_en)
        {
            ...
            sal_get_alloc_counters_main_thr_ignore(&(ignore_alloc_end), &(ignore_free_end));

            /** Calculate the total step memory allocation excluding resources allocated for SW state and excluding ignored memory allocations */
            current_step->internal_info.non_swstate_or_ignored_memory_allocation =
                ((alloc_end - alloc_start) - (free_end - free_start)) >
                0 ? (alloc_end - alloc_start) - (free_end - free_start) - (ignore_alloc_end - ignore_alloc_start) +
                (ignore_free_end - ignore_free_start) : 0;

            if (current_step->step_sub_array == NULL)
            {
                /** Add the memory allocated of each step to the overall for the sequence */
                *memory_overall += current_step->internal_info.non_swstate_or_ignored_memory_allocation;
            }
            ...
            if (seq->mem_log_en)
            {
                ...
                /*
                 * If any memory was allocated but ignored, print amount.
                 */
                if ((ignore_alloc_end - ignore_alloc_start) > 0)
                {
                    utilex_seq_print_ignored_memory_in_step(seq, log_severity, ignore_alloc_end - ignore_alloc_start,
                                                            depth, TRUE, TRUE);
                }

                /*
                 * If any memory was freed but ignored, print amount.
                 */
                if ((ignore_free_end - ignore_free_start) > 0)
                {
                    utilex_seq_print_ignored_memory_in_step(seq, log_severity, ignore_free_end - ignore_free_start,
                                                            depth, FALSE, TRUE);
                }
            }
            ...
            {
                alloc_bytes_count_offset = (alloc_end - alloc_start) - (ignore_alloc_end - ignore_alloc_start);
                free_bytes_count_offset = (free_end - free_start) - (ignore_free_end - ignore_free_start);
                sal_inc_alloc_counters_main_thr_ignore(alloc_bytes_count_offset, free_bytes_count_offset);
        ...
        if ((total_flags & UTILEX_SEQ_STEP_F_USE_DMA) && seq->desc_init_step_enabled_counter_dec != NULL)
        {
            SHR_IF_ERR_EXIT(seq->desc_init_step_enabled_counter_dec(unit));
        }
    }
    ...
    int *is_last_done)
{
    ...
    if (*is_first_done == 0 && step->step_sub_array == NULL)
    {
        if (!(step->static_flags & UTILEX_SEQ_STEP_F_NEVER_SKIP_DEINIT))
        {
            /** Skip this step */
            *total_flags |= UTILEX_SEQ_STEP_F_SKIP;
        }
    ...
    int *is_last_done)
{
    int step_index;
    int nof_steps_in_list = 0;
    unsigned long total_memory_alloc_end;
    unsigned long total_memory_alloc_start;
    unsigned long total_memory_free_start;
    unsigned long total_memory_free_end;
    unsigned long ignore_memory_alloc_end;
    unsigned long ignore_memory_alloc_start;
    unsigned long ignore_memory_free_start;
    unsigned long ignore_memory_free_end;
    unsigned long alloc_bytes_count_offset;
    unsigned long free_bytes_count_offset;
    int memory_freed_during_deinit;
    int memory_allocated_during_init;
    utilex_seq_step_t *current_step;
    int depth_index;
    uint32 log_severity;
    int dynamic_flags;
    bsl_severity_t original_severity = bslSeverityNormal;

    SHR_FUNC_INIT_VARS(unit);

    if (seq->mandatory_deinit == 1)
    {
        /*
         * In case of mandatory deinit we are disabling all prints
         */
        SHR_GET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LOG_MODULE, original_severity);
        SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LOG_MODULE, BSL_SEVERITY_ENC(bslSeverityOff));
    }
    ...
    for (step_index = nof_steps_in_list - 1; step_index >= 0; --step_index)
    {
        ...
        /** Init step vars */
        total_memory_alloc_end = 0;
        total_memory_alloc_start = 0;
        ...
        if (seq->mem_test_en || seq->mem_log_en)
        {
            ...
            sal_get_alloc_counters_main_thr(&(total_memory_alloc_end), &(total_memory_free_end));
            sal_get_alloc_counters_main_thr_ignore(&(ignore_memory_alloc_end), &(ignore_memory_free_end));
        }
        ...
        else
        {
            if (current_step->disabled_by_soc_prop == FALSE)
            {
                ...
                if (current_step->backward != NULL)
                {
                    ...
                    /*
                     * Run the backward cb only if we are not during mandatory_deinit,
                     *  or we are doing mandatory_deinit and MANDATORY_DEINIT flag is used.
                     */
                    if (seq->mandatory_deinit == 0
                        || (seq->mandatory_deinit == 1 && (total_flags & UTILEX_SEQ_STEP_F_MANDATORY_DEINIT) != 0))
                    {
                        /** Run deinit function */
        ...
        if (seq->mem_test_en || seq->mem_log_en)
        {
            ...
            sal_get_alloc_counters_main_thr(&(total_memory_alloc_start), &(total_memory_free_start));
            sal_get_alloc_counters_main_thr_ignore(&(ignore_memory_alloc_start), &(ignore_memory_free_start));

            memory_allocated_during_init = current_step->internal_info.non_swstate_or_ignored_memory_allocation;
            /*
             * Exclude memory freed but ignored during the deinit of the current step.
             */
            memory_freed_during_deinit =
                ((total_memory_free_start - total_memory_free_end) -
                 (total_memory_alloc_start - total_memory_alloc_end)) >
                0 ? (total_memory_free_start - total_memory_free_end) - (total_memory_alloc_start -
                                                                         total_memory_alloc_end) -
                (ignore_memory_free_start - ignore_memory_free_end) + (ignore_memory_alloc_start -
                                                                       ignore_memory_alloc_end) : 0;
            ...
            if (seq->mem_log_en)
            {
                 ...
                 * If any memory was allocated but ignored, print amount.
                 */
                if ((ignore_memory_alloc_start - ignore_memory_alloc_end) > 0)
                {
                    utilex_seq_print_ignored_memory_in_step(seq, log_severity,
                                                            ignore_memory_alloc_start - ignore_memory_alloc_end, depth,
                                                            TRUE, FALSE);
                }
                 ...
                 * If any memory was freed but ignored, print amount.
                 */
                if ((ignore_memory_free_start - ignore_memory_free_end) > 0)
                {
                    utilex_seq_print_ignored_memory_in_step(seq, log_severity,
                                                            ignore_memory_alloc_start - ignore_memory_alloc_end, depth,
                                                            FALSE, FALSE);
                ...
                if (memory_freed_during_deinit != memory_allocated_during_init)
                {
                    ...
                    BSL_LOG(BSL_LOG_MODULE | log_severity,
                            ("     - Memory allocated at init: %d bytes\n", memory_allocated_during_init));
                    ...
                    BSL_LOG(BSL_LOG_MODULE | log_severity,
                            ("     - Memory freed at deinit:   %d bytes\n", memory_freed_during_deinit));
                }
            ...
            if ((total_flags & UTILEX_SEQ_STEP_F_MEM_TEST_SKIP) != 0)
            {
                alloc_bytes_count_offset =
                    (total_memory_alloc_start - total_memory_alloc_end) - (ignore_memory_alloc_start -
                                                                           ignore_memory_alloc_end);
                free_bytes_count_offset =
                    (total_memory_free_start - total_memory_free_end) - (ignore_memory_free_start -
                                                                         ignore_memory_free_end);
                sal_inc_alloc_counters_main_thr_ignore(alloc_bytes_count_offset, free_bytes_count_offset);
...
exit:
    ...
    {
        /*
         * Return the BSL log severity to its original value.
         */
        SHR_SET_SEVERITY_FOR_MODULE_SPECIFIC(BSL_LOG_MODULE, original_severity);
    }
    SHR_FUNC_EXIT;
}
    ...
    int *step_found)
{
    ...
    *step_found = 0;
    for (step_index = 0; step_index < nof_steps_in_list; ++step_index)
    {
        ...
        /** MAX is needed to ensure correct matching in case one is substring of other */
        if (!sal_strncmp
            (step_name, current_step->step_name,
             UTILEX_MAX(sal_strnlen(step_name, UTILEX_SEQ_MAX_STEP_NAME_SIZE),
                        sal_strnlen(current_step->step_name, UTILEX_SEQ_MAX_STEP_NAME_SIZE))))
        {
            /** Step is found */
    ...
    utilex_seq_err_recovery_test_t * test_info)
{
    ...
    unsigned long ignore_alloc_start, ignore_free_start, ignore_alloc_end, ignore_free_end;
    ...
    while ((previous_step_id != original_last_step_id) || first_run)
    {
        ...
        /** verify whether deinit freed all allocated memory (not including ignored memory) */
        if (((alloc_start - free_start) - (ignore_alloc_start - ignore_free_start)) !=
            ((alloc_end - free_end) - (ignore_alloc_end - ignore_free_end)))
        {
            BSL_LOG(BSL_LOG_MODULE, ("#############################################\n"));
            ...
            if (test_info->fail_count < UTILEX_SEQ_NOF_FAILED_STEPS)
            {
                ...
                test_info->fail_mem_size[test_info->fail_count] =
                    (alloc_end - free_end) - (alloc_start - free_start) - (ignore_alloc_end - ignore_free_end) +
                    (ignore_alloc_start - ignore_free_start);
    ...
    utilex_step_time_dnx_data step_id_dnx_data)
{
    ...
    test_info->step_thresh[test_info->count] = step_id_dnx_data.step_time;
    if (debug_flag)
    {
        test_info->computed_thresh_down[test_info->count] =
            step_id_dnx_data.step_time - ((step_id_dnx_data.step_time * step_id_dnx_data.down_deviation) / 100);
        if (step_id_dnx_data.step_time == step_id_dnx_data.test_sensitivity)
        {
            test_info->computed_thresh_down[test_info->count] = 0;
        }
        test_info->computed_thresh_up[test_info->count] =
            step_id_dnx_data.step_time + ((step_id_dnx_data.step_time * step_id_dnx_data.up_deviation) / 100);
        test_info->computed_thresh_non_perc_up[test_info->count] =
            step_id_dnx_data.step_time + step_id_dnx_data.non_perc_up_deviation;
    }
    (test_info->count)++;
}
...
utilex_seq_time_test_run(
    ...
    int debug_flag,
    utilex_seq_func_t parse_time_cb,
    utilex_step_time_dnx_data step_id_dnx_data)
{
    int original_last_step_id, previous_step_id, current_step_id;
    ...
    utilex_seq_step_t *sub_step;
    int step_index, nof_steps_in_list;
    int is_found = 0, is_skip = 0;
    int first_run = 1; /** required in order to run first step in the list if it was provided explicitly */
    uint8 below_sensitivity = FALSE;
    SHR_FUNC_INIT_VARS(unit);
    ...
    while ((previous_step_id != original_last_step_id) || first_run)
    {
        ...
        for (int i = 0; i < step_nof_iterations; i++)
        {
            ...
            {
                BSL_LOG(BSL_LOG_MODULE,
                        ("############ Step %s ID=%d with time %u is not tested since it's below test sensitivity (%u) ###########\n",
                         current_step->step_name, current_step->step_id, total_step_time,
                         step_id_dnx_data.test_sensitivity));
                break;
            }
            /** run sequence until previous step (in case this is not the first step) */
            if (previous_step_id != current_step->step_id)
            ...
            if (is_skip == FALSE)
            {
                ...
                if (total_step_time < step_id_dnx_data.test_sensitivity)
                {
                    below_sensitivity = TRUE;
                    is_skip = TRUE;
                }
            }
            else
        ...
        if (is_skip == FALSE)
        {
            ...
            SHR_IF_ERR_EXIT(parse_time_cb(unit, step_time));

            /** Verify if the measured time for the current step is passing the following conditions:
             *  -The measured value should not be greater than the computed up threshold (time value + up deviation in percentages).
             *  -The measured value should not be greater than the computed up threshold (time value + non_percentages up deviation).
             *  -The measured value should not be less than the computed down threshold (time value - down deviation).
             */
            if ((step_time >
                 (step_id_dnx_data.step_time + ((step_id_dnx_data.step_time * step_id_dnx_data.up_deviation) / 100))))
            {
                utilex_seq_init_time_error_print(unit, step_time, (step_id_dnx_data.step_time -
                                                                   ((step_id_dnx_data.step_time *
                                                                     step_id_dnx_data.up_deviation) / 100)),
                                                 current_step, step_id_dnx_data, FALSE);

                if (test_info_fail->count < UTILEX_SEQ_NOF_FAILED_STEPS)
                {
                    utilex_seq_update_test_info(test_info_fail, debug_flag, current_step, step_time, step_id_dnx_data);
                }
            }
            else if (step_time > step_id_dnx_data.step_time + step_id_dnx_data.non_perc_up_deviation)
            {
                utilex_seq_init_time_error_print(unit, step_time,
                                                 (step_id_dnx_data.step_time + step_id_dnx_data.non_perc_up_deviation),
                                                 current_step, step_id_dnx_data, TRUE);
                if (test_info_fail->count < UTILEX_SEQ_NOF_FAILED_STEPS)
                {
                    utilex_seq_update_test_info(test_info_fail, debug_flag, current_step, step_time, step_id_dnx_data);
                }
                      ...
                      (step_id_dnx_data.step_time -
                       ((step_id_dnx_data.step_time * step_id_dnx_data.down_deviation) / 100)))
                     && (step_id_dnx_data.step_time != step_id_dnx_data.test_sensitivity))
            {
                /*
                 * Do not check down deviation for steps which are using default step_id_dnx_data.test_sensitivity
                 */
                utilex_seq_init_time_error_print(unit, step_time, (step_id_dnx_data.step_time -
                                                                   ((step_id_dnx_data.step_time *
                                                                     step_id_dnx_data.down_deviation) / 100)),
                                                 current_step, step_id_dnx_data, FALSE);
            ...
            {
                if (test_info_success->count < UTILEX_SEQ_NOF_FAILED_STEPS)
                {
                    utilex_seq_update_test_info(test_info_success, debug_flag, current_step, step_time,
                                                step_id_dnx_data);
                }
                BSL_LOG(BSL_LOG_MODULE, ("#############################################\n"));
                BSL_LOG(BSL_LOG_MODULE, ("Step %s PASSED!\n", (current_step->step_name)));
                if (step_id_dnx_data.step_time == step_id_dnx_data.test_sensitivity)
                {
                    BSL_LOG(BSL_LOG_MODULE, ("Down deviation disabled!\n"));
                    BSL_LOG(BSL_LOG_MODULE,
                            ("Measured %u expected to be in range 0 - %u \n", step_time,
                             (step_id_dnx_data.step_time +
                              ((step_id_dnx_data.step_time * step_id_dnx_data.up_deviation) / 100))));
                }
                else
                {
                    BSL_LOG(BSL_LOG_MODULE,
                            ("Measured %u expected to be in range %u - %u \n", step_time,
                             (step_id_dnx_data.step_time -
                              ((step_id_dnx_data.step_time * step_id_dnx_data.down_deviation) / 100)),
                             (step_id_dnx_data.step_time +
                              ((step_id_dnx_data.step_time * step_id_dnx_data.up_deviation) / 100))));
