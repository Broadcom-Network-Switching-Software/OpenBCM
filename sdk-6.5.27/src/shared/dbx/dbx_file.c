    ...
    char **files_list)
{
     ...
     */
    if (dbx_file_get_db_directory(RHFILE_MAX_SIZE, db_path) != _SHR_E_NONE)
    {
        return 0;
    }
    ...
    char *filepath)
{
    ...
    if (ISEMPTY(sdk_db_dir))
    {
         ...
         * Get the database root.
         */
        if (dbx_file_get_db_directory(RHFILE_MAX_SIZE, sdk_db_dir) != _SHR_E_NONE)
        {
            LOG_ERROR(BSL_LOG_MODULE, (BSL_META_U(unit, "DB Directory does not exist\n")));
...
dbx_file_find_sdk_path(
    char *sdk_dir)
{
    static char tmp_sdk_dir[RHFILE_MAX_SIZE] = { 0 };
    /*
     * If SDK directory has not been found on the first invocation of this
     * procedure then it is not searched for on following invocations.
     * If it has then the same SDK path is assumed for all following
     * invocations.
     */
    static int tmp_sdk_dir_was_found = FALSE;
    /*
     * This boolean indicates whether an SDK directory has been searched for,
     * by this procedure, in the past.
     */
    static int tmp_sdk_dir_was_attempted = FALSE;
    char *pos;
    int loc;
    shr_error_e ret;
    char pwd[RHFILE_MAX_SIZE];
    /*
     * If any of these directories/files is found along any of the directories, of
     * which 'pwd' is made, then that directory is assumed to be the 'base' for SDK.
     */
    char *relatives_dir[] = { "/regress/bcm/tests/dnx/", "/tests/dnx/", "/include/soc/drv.h" };
    ...
    if (tmp_sdk_dir_was_attempted == FALSE)
    {
        int ii;

        tmp_sdk_dir_was_attempted = TRUE;
        for (ii = 0; ii < COUNTOF(relatives_dir); ii++)
        {
            ...
            while ((pos = sal_strrchr(pwd, '/')) != NULL)
            {
                loc = pos - pwd + 1;
                pwd[loc - 1] = '\0';
                sal_strncpy_s(tmp_sdk_dir, pwd, loc);
                sal_strncat_s(tmp_sdk_dir, relatives_dir[ii], RHFILE_MAX_SIZE);
                if (dbx_file_dir_exists(tmp_sdk_dir) || dbx_file_exists(tmp_sdk_dir))
                {
                    sal_strncpy_s(sdk_dir, pwd, loc);
                    sal_strncpy_s(tmp_sdk_dir, sdk_dir, RHFILE_MAX_SIZE - 1);
                    tmp_sdk_dir_was_found = TRUE;
                    goto exit;
                }
            }
        }
        sdk_dir[0] = tmp_sdk_dir[0] = 0;
        LOG_VERBOSE(BSL_LOG_MODULE, (BSL_META_U(0, "SDK variable not found\n")));
        ret = _SHR_E_NOT_FOUND;
        goto exit;
    }
    if (tmp_sdk_dir_was_found == TRUE)
    {
    ...
    char *device_attribute)
{
    ...
    RHDATA_ITERATOR(cur, curSubTop, "device")
    {
        ...
        if (!sal_strcasecmp(device_n, soc_dev_name(unit)))
        {
            ...
            RHDATA_GET_STR_DEF_NULL(cur, device_attribute, attribute_name);
            SHR_EXIT();
        }
    }
...
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_SET_CURRENT_ERR(internal_dbx_file_get_db(unit, db_name, base_db_name, "base_db_name"));
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
...
shr_error_e
dbx_file_get_db_short_name(
    int unit,
    char *db_name,
    char *short_name)
{
    SHR_FUNC_INIT_VARS(unit);
    SHR_SET_CURRENT_ERR(internal_dbx_file_get_db(unit, db_name, short_name, "short_name"));
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
    ...
    char *filepath)
{
    ...
    if ((SHR_GET_CURRENT_ERR() != _SHR_E_NONE) && !(flags & CONF_OPEN_CURRENT_DEVICE_ONLY))
    {
    ...
    uint8 create_dir)
{
    ...
    sal_strncat_s(buffer, db_name, RHFILE_MAX_SIZE);
    if (create_dir)
    {
    ...
    sal_strncat_s(buffer, "/adapter", RHFILE_MAX_SIZE);
    if (create_dir)
    {
        SHR_IF_ERR_EXIT(dbx_file_dir_create(buffer));
    }
...
shr_error_e
dbx_file_get_db_directory(
    size_t size,
    char *directory)
{
    /**Temp variable, which will be used to call sal_config_get and getenv.*/
    char *db_dir;

    SHR_FUNC_INIT_VARS(NO_UNIT);
    SHR_NULL_CHECK(directory, _SHR_E_PARAM, "directory should not be NULL");

    /*
     * Get db path from soc property or env variable. If doesn't exists - try env variable.
     */
    db_dir = sal_config_get("dpp_db_path");
    if (!ISEMPTY(db_dir) && (dbx_file_dir_exists(db_dir) == TRUE))
    {
        /*
         * Update the directory string.
         */
        sal_strncpy_s(directory, db_dir, size - 1);
    }
    else
    {
        /*
         * Get db path from env variable. If doesn't exists - take the default "./db".
         */
        ...
        {
            char _current_directory[RHFILE_MAX_SIZE] = { 0 };
            /*
             * Temporary container variable used to build the directory path
             */
            char _temp_directory[RHFILE_MAX_SIZE] = { 0 };
            /*
             * Return the default database directory.
             */
            sal_getcwd(_current_directory, RHFILE_MAX_SIZE);
            /*
             * Build the directory path in a local temp variable in order to make it atomic
             */
            sal_strncpy_s(_temp_directory, _current_directory, size - 1);
            sal_strncat_s(_temp_directory, "db", size - 1);
            sal_strncpy_s(directory, _temp_directory, size - 1);
        }
    }

    if (dbx_file_dir_exists(directory) == FALSE)
    {
        /*
         * DB does not exists and cannot be found in any place
         */
        SHR_EXIT_WITH_LOG(_SHR_E_NOT_FOUND, "DB was not found in %s%s%s", directory, EMPTY, EMPTY);
    }
