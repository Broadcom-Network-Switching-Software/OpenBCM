...
#ifndef __KAPS_JR1_PORTABLE_H
#define __KAPS_JR1_PORTABLE_H
...
#if defined(KAPS_JR1_BIG_ENDIAN) && defined(KAPS_JR1_LITTLE_ENDIAN)
#error cannot define both big and little endian
#endif



#ifdef KAPS_JR1_TRACE
#define KAPS_JR1_TRACE_IN(format, ...) \
    do {\
        kaps_jr1_printf("KAPS_JR1_TRACE:> %s " format, __FUNCTION__, __VA_ARGS__); \
        fflush(stdout);\
    } while (0)

#define KAPS_JR1_TRACE_OUT(format, ...) \
        do {\
            kaps_jr1_printf("KAPS_JR1_TRACE:< %s " format, __FUNCTION__, __VA_ARGS__); \
            fflush(stdout);\
        } while (0)

#define KAPS_JR1_TRACE_PRINT(format, ...) \
    do {\
        kaps_jr1_printf(format, __VA_ARGS__); \
        fflush(stdout);\
    } while (0)
...
#define KAPS_JR1_TRACE_PRINT(format, ...)
      ...
      || defined(__arm__) || defined(aarch64_) || defined(__aarch64__))) || defined(KAPS_JR1_LITTLE_ENDIAN)
...
#define __KAPS_JR1_BYTESWAP_32(x) (x)
#define __KAPS_JR1_BYTESWAP_64(x) (x)
#define __KAPS_JR1_INPLACE_SWAP_32(x, n)
#define __KAPS_JR1_SWAP_4BYTES(x, n)
#define __KAPS_JR1_BYTESWAP_TWO_32B(x) (x)

#ifndef KAPS_JR1_LITTLE_ENDIAN
#define KAPS_JR1_LITTLE_ENDIAN
#endif
    ...
    || defined(__mips__) || defined(__AARCH64EB__) || defined(KAPS_JR1_BIG_ENDIAN)
...
#ifndef KAPS_JR1_BIG_ENDIAN
#define KAPS_JR1_BIG_ENDIAN
#endif


#define __KAPS_JR1_BYTESWAP_64(x)\
...
#define __KAPS_JR1_BYTESWAP_TWO_32B(x)\
...
#define __KAPS_JR1_BYTESWAP_32(x)\
   ((((x) & 0xFF000000ul) >> 24) |   \
    (((x) & 0x00FF0000ul) >> 8)  |   \
    (((x) & 0x0000FF00ul) << 8)  |   \
    (((x) & 0x000000FFul) << 24));
...
#define __KAPS_JR1_INPLACE_SWAP_32(x, n)\
    (((uint32_t *) (&x))[n]) = __KAPS_JR1_BYTESWAP_32((((uint32_t *) (&x))[n]))
...
struct kaps_jr1_timeval {
    long tv_sec;                
    long tv_ns;                 
};

typedef long kaps_jr1_time_t;  
...
struct kaps_jr1_mutex {
    char mutex_opaque_type; 
};


int32_t kaps_jr1_assert_detail(const char *msg, const char *file, int32_t line);


int32_t kaps_jr1_assert_detail_or_error(const char *msg, uint32_t return_error, uint32_t error_code, const char *file, int32_t line);


int32_t kaps_jr1_identity(const int32_t b);

#define KAPS_JR1_CAST_PTR_TO_U32(x) ((uint32_t) (uintptr_t) x) 
#define KAPS_JR1_CAST_U32_TO_PTR(x)    (void*) ((uintptr_t) x) 


#define kaps_jr1_assert(cond, msg) ((void) ((cond) || kaps_jr1_assert_detail((msg), __FILE__, __LINE__))) 
#define kaps_jr1_sassert(cond)     ((void) ((cond) || kaps_jr1_assert_detail((" "), __FILE__, __LINE__))) 
#define kaps_jr1_assert_(b, m)     (kaps_jr1_identity(b) ? 0 : kaps_jr1_assert_detail(m, __FILE__, __LINE__)), 

#define KAPS_JR1_ASSERT_OR_ERROR(cond, msg, return_error, error_code)                                           \
    do {                                                                                                   \
        if (!(cond)) {                                                                                     \
            KAPS_JR1_STRY(kaps_jr1_assert_detail_or_error((msg), (return_error), (error_code), __FILE__, __LINE__)); \
        }                                                                                                  \
    } while (0) 
...
#if 1
#define  KAPS_JR1_TX_ERROR_CHECK(dev, status)                                                                        \
         (void) dev;                                                                                             \
         (void) status;
#else
#define  KAPS_JR1_TX_ERROR_CHECK(dev, status)                                                                        \
    ...
    if (dev->fatal_transport_error || dev->check_fatal_dbg_0 || dev->check_fatal_dbg_1)   {                     \
        ...
        if (status)                                                                                             \
            kaps_jr1_printf("\n Status   :  Reason Code: %d  [ %s ]  ", status, kaps_jr1_get_status_string(status));      \
...
#ifdef WIN32
#define kaps_jr1_concat_identity(a)  a
#define kaps_jr1_concat_2(a, b)      kaps_jr1_concat_identity(a) kaps_jr1_concat_identity(b)
#define kaps_jr1_concat_3(a, b, c)   kaps_jr1_concat_identity(a) kaps_jr1_concat_identity(b) kaps_jr1_concat_identity(c)
#else
#define kaps_jr1_concat_2(a, b)      a##b              
#define kaps_jr1_concat_3(a, b, c)   a##b##c           
#endif


#define kaps_jr1_stdout   stdout


#define kaps_jr1_stderr   stderr


#define kaps_jr1_fopen    fopen


#define kaps_jr1_eof      (EOF)



void *kaps_jr1_memset(void *str, int val, uint32_t size);


void *kaps_jr1_memcpy(void *dest, const void *src, uint32_t size);


int kaps_jr1_memcmp(const void *s1, const void *s2, uint32_t size);


void *kaps_jr1_memmove(void *dstp, const void *srcp, uint32_t numOfBytes);


void *kaps_jr1_sysmalloc(uint32_t size);


void *kaps_jr1_syscalloc(uint32_t nmemblk, uint32_t size);


void kaps_jr1_sysfree(void *ptr);


void kaps_jr1_abort(void);


int kaps_jr1_sprintf(char *buff, const char *fmt, ...);


int kaps_jr1_vsnprintf(char *buff, uint32_t buffsize, const char *fmt, va_list ap);


int kaps_jr1_snprintf(char *buff, uint32_t buffsize, const char *fmt, ...);


int kaps_jr1_printf(const char *fmt, ...);


int kaps_jr1_fputs(const char *str, FILE *fp);


int kaps_jr1_vprintf(const char *fmt, va_list arg);


int kaps_jr1_fprintf(FILE * fp, const char *fmt, ...);


int kaps_jr1_fscanf(FILE * fp, const char *fmt, ...);


int kaps_jr1_fseek(FILE * fp, int32_t offset, int whence);



void kaps_jr1_enable_error_box(int32_t enable);


int kaps_jr1_gettimeofday(struct kaps_jr1_timeval *tv);


int kaps_jr1_gettimerval(struct kaps_jr1_timeval * tv);


kaps_jr1_time_t kaps_jr1_time(kaps_jr1_time_t *currtime);


struct kaps_jr1_tm *kaps_jr1_localtime(kaps_jr1_time_t *currtime, struct kaps_jr1_tm *timeinfo);


char *kaps_jr1_getenv(const char *name);


void kaps_jr1_srandom(uint32_t seed);


int kaps_jr1_random(void);


int kaps_jr1_random_r(uint32_t *seedp);
...
char *kaps_jr1_strsep(char **stringp, const char *delim);



int kaps_jr1_fclose(FILE * fp);



void kaps_jr1_usleep(uint32_t usecs);




void *kaps_jr1_mmap(void *addr, uint32_t length, int prot, int flags,
               int fd, uint64_t offset);



int kaps_jr1_munmap(void *addr, uint32_t length);



int kaps_jr1_open(const char *pathname, int flags, uint32_t mode);


int kaps_jr1_vfprintf(FILE * fp, const char *fmt, va_list ap);


int kaps_jr1_mutex_init(struct kaps_jr1_mutex **mutex);


int kaps_jr1_mutex_destroy(struct kaps_jr1_mutex *mutex);


int kaps_jr1_mutex_lock(struct kaps_jr1_mutex *mutex);


int kaps_jr1_mutex_trylock(struct kaps_jr1_mutex *mutex);


int kaps_jr1_mutex_unlock(struct kaps_jr1_mutex *mutex);



#ifdef KAPS_JR1_UNDEF_FUN
