...
int tscbh_sub10g_phy_loopback_set(const phymod_phy_access_t* phy, phymod_loopback_mode_t loopback, uint32_t enable)
{
    ...
    if (enable && (num_lane != TSCBH_SUB10G_NOF_LANES_IN_CORE)) {
...
int tscbh_sub10g_timesync_enable_set(const phymod_phy_access_t* phy, uint32_t flags, uint32_t enable)
{
     ...
     * Enable fclk on PMD, with div_mode = 8T.
     */
                                     ...
                                     const phymod_timesync_adjust_config_info_t* config_info)
{
    ...
    if (PHYMOD_TIMESYNC_F_SOP_GET(flags)) {
        ts_offset = TSCBH_SUB10G_TS_OFFSET_SOP;
    } else {
        if (PHYMOD_TIMESYNC_F_MAC_DA_GET(flags)) {
            ts_offset = TSCBH_SUB10G_TS_OFFSET_MAC_DA;
        } else {
            ts_offset = TSCBH_SUB10G_TS_OFFSET_SFD;
        }
    }

    /*
     * In Reduced Preamble Mode.
     */
    if (PHYMOD_TIMESYNC_F_REDUCED_PREAMBLE_MODE_GET(flags)) {
        rpm_mode = 1;
    }

    /*sop or sfd timestamping, osts_crc_calc_mode*/
    PHYMOD_IF_ERR_RETURN
        (tbhmod_sub10g_1588_ts_config_set(&phy_copy.access, ts_offset, rpm_mode));

    /*set flck*/
    PHYMOD_IF_ERR_RETURN
        (tbhmod_sub10g_set_fclk_period(&phy_copy.access));

    /*set ui values*/
    PHYMOD_IF_ERR_RETURN
        (temod2pll_speed_id_get(&phy->access, &speed_id));
    PHYMOD_IF_ERR_RETURN
        (tbhmod_sub10g_bit_ui_set(&phy_copy.access, speed_id, num_lane));

    /*Set RX/TX pmd  latency*/
    PHYMOD_IF_ERR_RETURN
        (tbhmod_sub10g_tx_pmd_latency_set(&phy_copy.access, speed_id, num_lane));
    PHYMOD_IF_ERR_RETURN
        (tbhmod_sub10g_rx_pmd_latency_set(&phy_copy.access, speed_id, num_lane));

    if (config_info->am_norm_mode != phymodTimesyncCompensationModeNone) {
        PHYMOD_IF_ERR_RETURN
            (tbhmod_sub10g_tx_pmd_latency_update(&phy_copy.access, speed_id, num_lane,
                                                 config_info->am_norm_mode));

        /*Program deskew adjustment for multi-lane ports */
        PHYMOD_IF_ERR_RETURN
            (tbhmod_sub10g_1588_set_rx_deskew(&phy_copy.access, speed_id,
                                              config_info->am_norm_mode));
    }
       ...
       if ((pll_div == TBHMOD_SUB10G_PLL_MODE_DIV_132) || (pll_div == TBHMOD_SUB10G_PLL_MODE_DIV_66)) {
            switch (osr_mode) {
                ...
                case  TSCBH_SUB10G_OS_MODE_4:
                    sdm_val = TSCBH_SUB10G_SYNCE_SDM_DIVISOR_5G;
                    break;
                case TSCBH_SUB10G_OS_MODE_16p5:
                    sdm_val = TSCBH_SUB10G_SYNCE_SDM_DIVISOR_1G;
                    break;
                default:
            ...
            switch (osr_mode) {
                case TSCBH_SUB10G_OS_MODE_8:
                    sdm_val = TSCBH_SUB10G_SYNCE_SDM_DIVISOR_2P5G;
                    break;
                default:
                    PHYMOD_DEBUG_ERROR(("Unsupported osmode\n"));
                    return PHYMOD_E_UNAVAIL;
