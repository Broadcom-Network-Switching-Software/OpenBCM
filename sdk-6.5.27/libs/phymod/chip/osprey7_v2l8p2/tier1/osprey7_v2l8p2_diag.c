...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  osprey7_v2l8p2_diag.c                                             *
...
err_code_t osprey7_v2l8p2_test_uc_timestamp(srds_access_t *sa__) {
    return osprey7_v2l8p2_INTERNAL_test_uc_timestamp_with_print_options(sa__, 0xFF);
}

static void _osprey7_v2l8p2_display_eye_scan_ignored_lane(srds_access_t *sa__, uint8_t eye_slice) {
    if (eye_slice) {
        USR_PRINTF(("         : -"));
    } else {
        USR_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
...
err_code_t osprey7_v2l8p2_display_eye_scan(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    ...
    Exit_Status = osprey7_v2l8p2_INTERNAL_eye_scan_setup(sa__, 0, &y_max, &y_step, &usr_diag_status);
    ...
    CFUN(osprey7_v2l8p2_display_eye_scan_header(sa__, 1, &usr_diag_status));
    ...
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step)) {
        ...
        if(enable_read) {
            Exit_Status = osprey7_v2l8p2_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (Exit_Status) {
                EFUN_PRINTF(("ERROR: osprey7_v2l8p2_read_eye_scan_stripe() returned: %s, (status = 0x%x)\n", osprey7_v2l8p2_INTERNAL_e2s_err_code(Exit_Status), status));
                goto Exit;
            }
        }
        CFUN(osprey7_v2l8p2_display_eye_scan_stripe(sa__, y, &stripe[buffer_idx], usr_diag_status));
        USR_PRINTF(("\n"));
    }
    CFUN(osprey7_v2l8p2_display_eye_scan_footer(sa__, 1, &usr_diag_status));
    /* stop acquisition */
    CFUN(osprey7_v2l8p2_meas_eye_scan_done(sa__));
Exit:
    return(osprey7_v2l8p2_error_report(sa__, Exit_Status));
}

err_code_t osprey7_v2l8p2_display_all_eye_scan(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    ...
    for ( lane=0; lane < num_lanes; lane++) {
        err_code = osprey7_v2l8p2_acc_set_lane(sa__, lane);
        if (err_code) {
            USR_PRINTF(("%s WARN: Unable to switch to lane(%d) and so ignoring lane for for 2D eye scan\n", API_FUNCTION_NAME, lane));
            continue;
        }
        err_code = osprey7_v2l8p2_INTERNAL_eye_scan_setup(sa__, 0, &i_max[lane], &i_step[lane], &usr_diag_status[lane]);
        if (err_code) {
            uint8_t core;
            core = osprey7_v2l8p2_acc_get_core(sa__);
    ...
    if(lane_mask) {
        CFUN(osprey7_v2l8p2_display_eye_scan_header(sa__, num_lanes, &usr_diag_status[0]));
    }
    else {
        goto Exit;
    }
        ...
        for(lane=0; lane < num_lanes; lane++) {
            if(lane_mask & (1<<lane)) {
                ...
                if(i[lane] >= -i_max[lane]) {
                    local_error = osprey7_v2l8p2_acc_set_lane(sa__, lane);
                    if (local_error) {
                        lane_mask &= (uint8_t)~(1<<lane);
                        _osprey7_v2l8p2_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                        continue;
                    }
                    status = 0;
                    if(eye_slice[lane]) {
                    ...
                    if(enable_read) {
                        local_error = osprey7_v2l8p2_read_eye_scan_stripe(sa__, &stripe[0], &status);
                        if (local_error) {
                            lane_mask &= (uint8_t)~(1<<lane);
                            _osprey7_v2l8p2_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                            continue;
                    ...
                    local_error = osprey7_v2l8p2_display_eye_scan_stripe(sa__, i[lane], &stripe[buffer_idx], usr_diag_status[lane]);
                    if (local_error) {
                        lane_mask &= (uint8_t)~(1<<lane);
                        _osprey7_v2l8p2_display_eye_scan_ignored_lane(sa__, eye_slice[lane]);
                        continue;
    ...
    CFUN(osprey7_v2l8p2_display_eye_scan_footer(sa__, num_lanes, &usr_diag_status[0]));
    for(lane=0;lane < num_lanes;lane++) {
        if(lane_mask & (1<<lane)) {
            CFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));
            CFUN(osprey7_v2l8p2_meas_eye_scan_done(sa__));
        }
    }
Exit:

    if (osprey7_v2l8p2_acc_set_lane(sa__, lane_orig)) {
...
err_code_t osprey7_v2l8p2_display_intrusive_eye_scan(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint32_t stripe[EYE_SCAN_DISPLAY_SRIPE_SIZE] = {0};
    uint32_t *data;
    ...
    err_code_t Exit_Status = ERR_CODE_NONE;
    uint16_t usr_diag_status;
    enum osprey7_v2l8p2_rx_pam_mode_enum pam_mode;

    EFUN(osprey7_v2l8p2_INTERNAL_get_rx_pam_mode(sa__, &pam_mode));

    if (pam_mode == NRZ) {
        intrusive_buff_size = (EYE_SCAN_INTRUSIVE_WIDTH*(EYE_SCAN_NRZ_VERTICAL_IDX_MAX+1));
    } else {
        intrusive_buff_size = (EYE_SCAN_INTRUSIVE_WIDTH*(EYE_SCAN_PAM_VERTICAL_IDX_MAX+1));
    }
    data = (uint32_t *)USR_CALLOC(1, intrusive_buff_size*sizeof(uint32_t));
    if(!data) {
        return (ERR_CODE_MEM_ALLOC_FAIL);
    }
    Exit_Status = osprey7_v2l8p2_INTERNAL_get_intrusive_eye_scan(sa__, data, &y_max, &y_step, &lane_mask, 1, &usr_diag_status);
    ...
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step)) {
        ...
        CFUN(osprey7_v2l8p2_display_eye_scan_stripe(sa__, y, &stripe[0], usr_diag_status));
        USR_PRINTF(("\n"));
    }
    CFUN(osprey7_v2l8p2_display_eye_scan_footer(sa__, 1, &usr_diag_status));
...
err_code_t osprey7_v2l8p2_display_all_intrusive_eye_scan(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint8_t num_lanes, lane;
    uint8_t lane_orig = osprey7_v2l8p2_acc_get_lane(sa__);
    int16_t i[NUM_LANES_MAX] = {0};
    ...
    CFUN(osprey7_v2l8p2_INTERNAL_get_intrusive_eye_scan(sa__, data, i_max, i_step, &lane_mask, num_lanes, &usr_diag_status[0]));
    ...
    while(max_num_steps--) {
        ...
        for(lane=0; lane < num_lanes; lane++) {
            if(lane_mask & (1<<lane)) {
                if(i[lane] >= -i_max[lane]) {
                    err = osprey7_v2l8p2_acc_set_lane(sa__, lane);
                    if (err) {
                        lane_mask &=  (uint8_t)~(1<<lane);
                        if(first_err != ERR_CODE_NONE) first_err = err;
                        _osprey7_v2l8p2_display_eye_scan_ignored_lane(sa__, 0);
                        continue;
                    }

                    /* Create horizontal stripe from the 2D data */
                    for(j=0; j<(EYE_SCAN_INTRUSIVE_WIDTH-1); j++) {    /* -31:-1 */
                        stripe[j] = data[(intrusive_buff_size * lane) + ((vertical_idx_max+1)*((EYE_SCAN_INTRUSIVE_WIDTH-1)-j)) + (i_max[lane]-i[lane])/i_step[lane]];
                    }
                    for(j=(EYE_SCAN_INTRUSIVE_WIDTH-1); j<(EYE_SCAN_DISPLAY_SRIPE_SIZE-1); j++) {   /* 0:32 */
                        stripe[j] = data[(intrusive_buff_size * lane) + ((vertical_idx_max+1)*(j-(EYE_SCAN_INTRUSIVE_WIDTH-1))) + (i_max[lane]-i[lane])/i_step[lane]];
                    }
                    err = osprey7_v2l8p2_display_eye_scan_stripe(sa__, i[lane],&stripe[0], usr_diag_status[lane]);
                    if (err) {
                        lane_mask &=  (uint8_t)~(1<<lane);
                        if(first_err != ERR_CODE_NONE) first_err = err;
                        _osprey7_v2l8p2_display_eye_scan_ignored_lane(sa__, 0);
...
Exit:
    ...
    if(Exit_Status != ERR_CODE_NONE) {
        USR_PRINTF(("WARN: Unrecoverable error occurred during osprey7_v2l8p2_display_all_intrusive_eye_scan():\n."));
        return osprey7_v2l8p2_error_report(sa__, Exit_Status);
    }
    return(first_err);
}
...
err_code_t osprey7_v2l8p2_meas_eye_scan_start(srds_access_t *sa__, uint8_t direction) {
    INIT_SRDS_ERR_CODE
    uint8_t lock;
    uint8_t ilb_en;
    ...
    EFUN(osprey7_v2l8p2_pmd_lock_status(sa__,&lock));
    {
        uint8_t sts;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(osprey7_v2l8p2_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_VSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_HORIZ) {
        EFUN(osprey7_v2l8p2_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_HSCAN_EYE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_SLICE) {
        EFUN(osprey7_v2l8p2_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_EYE_SLICE,GRACEFUL_STOP_TIME));
    } else if(direction == EYE_SCAN_INTR) {
        EFUN(osprey7_v2l8p2_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_INTR_EYE,GRACEFUL_STOP_TIME));
    }
...
typedef struct {
    uint32_t *buffer_ptr;
    } osprey7_v2l8p2_read_eye_scan_stripe_callback_arg_t;

static err_code_t osprey7_v2l8p2_read_eye_scan_stripe_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    osprey7_v2l8p2_read_eye_scan_stripe_callback_arg_t * const cast_arg = (osprey7_v2l8p2_read_eye_scan_stripe_callback_arg_t *)arg;
    UNUSED(sa__);
    *(cast_arg->buffer_ptr++) = osprey7_v2l8p2_INTERNAL_float8_to_uint32((float8_t)(data & 0xFF));
    if (byte_count > 1) {
        *(cast_arg->buffer_ptr++) = osprey7_v2l8p2_INTERNAL_float8_to_uint32((float8_t)(data >> 8));
    }
    return (ERR_CODE_NONE);
}

err_code_t osprey7_v2l8p2_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    INIT_SRDS_ERR_CODE
    srds_info_t const * const osprey7_v2l8p2_info_ptr = osprey7_v2l8p2_INTERNAL_get_osprey7_v2l8p2_info_ptr_with_check(sa__);
    osprey7_v2l8p2_read_eye_scan_stripe_callback_arg_t arg;
    uint32_t timeout_ms = DIAG_POLL_TIMEOUT;
    uint32_t lane_diag_base;
    const uint8_t lane = osprey7_v2l8p2_acc_get_lane(sa__);
    uint8_t diag_rd_ptr;
    ...
    arg.buffer_ptr = buffer;
        ...
        lane_diag_base = osprey7_v2l8p2_info_ptr->diag_mem_ram_base + ((lane%osprey7_v2l8p2_info_ptr->lane_count) * osprey7_v2l8p2_info_ptr->diag_mem_ram_size) +
            (uint32_t)(osprey7_v2l8p2_info_ptr->grp_ram_size*osprey7_v2l8p2_INTERNAL_grp_idx_from_lane(osprey7_v2l8p2_acc_get_physical_lane(sa__)));

    EFUN(osprey7_v2l8p2_INTERNAL_poll_diag_data(sa__, osprey7_v2l8p2_info_ptr, status, &diag_rd_ptr, stripe_size, timeout_ms));

    EFUN(osprey7_v2l8p2_INTERNAL_rdblk_uc_generic_ram(sa__,
                                              lane_diag_base,
                                              (uint16_t)osprey7_v2l8p2_info_ptr->diag_mem_ram_size,
                                              diag_rd_ptr,
                                              stripe_size,
                                              &arg,
                                              osprey7_v2l8p2_read_eye_scan_stripe_callback));

    diag_rd_ptr = (uint8_t)(((uint32_t)diag_rd_ptr + stripe_size) % osprey7_v2l8p2_info_ptr->diag_mem_ram_size);
    EFUN(wrv_usr_diag_rd_ptr(diag_rd_ptr));
    ESTM(*status = rdv_usr_diag_status() & 0xFF);
    return(ERR_CODE_NONE);
}

err_code_t osprey7_v2l8p2_display_eye_scan_stripe(srds_access_t *sa__, int16_t y,uint32_t *buffer, uint16_t usr_diag_status) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(osprey7_v2l8p2_INTERNAL_ladder_setting_to_mV(sa__, slicer_range, data_thresh, slicer, &slicer_range_mV));
...
err_code_t osprey7_v2l8p2_log_api_event(srds_access_t *sa__, struct osprey7_v2l8p2_event_st api_event) {
    INIT_SRDS_ERR_CODE
    srds_info_t * const osprey7_v2l8p2_info_ptr = osprey7_v2l8p2_INTERNAL_get_osprey7_v2l8p2_info_ptr_with_check(sa__);

    if(osprey7_v2l8p2_info_ptr->info_table_version > 0x38) {
        uint16_t data_new = 0;
        data_new = (uint16_t)(((uint16_t)api_event.data2<< 8) | api_event.data1);
        EFUN(osprey7_v2l8p2_pmd_uc_cmd_with_data(sa__, CMD_API_EVENT, api_event.event_id, data_new, GRACEFUL_STOP_TIME));
    }
    return(ERR_CODE_NONE);
}
err_code_t osprey7_v2l8p2_display_eye_scan_header(srds_access_t *sa__, uint8_t i, uint16_t *usr_diag_status) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
    ...
    if(i!=1) {ESTM(lane_bak = osprey7_v2l8p2_acc_get_lane(sa__));}
    for(lane=0;lane<i;lane++) {
        if(i!=1) {EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));}
        if(osprey7_v2l8p2_info_ptr->info_table_version > 0x37) {
            if(((*(usr_diag_status+lane))&DIAG_EYE_SCAN_RANGE_MASK) == DIAG_EYE_SCAN_H_HALF) {
                stepsize_var[lane] = 128;
            }
        }
    }
    if(i!=1) {EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane_bak));}
    ...
    for(x=1;x<=i;x++) {
        if(((*(usr_diag_status+(x-1)))&DIAG_EYE_SCAN_RANGE_MASK) == DIAG_EYE_SCAN_SLICE) {
            _osprey7_v2l8p2_display_eye_scan_ignored_lane(sa__, 1);
        } else
        {
            _osprey7_v2l8p2_display_eye_scan_ignored_lane(sa__, 0);
        }
...
err_code_t osprey7_v2l8p2_display_eye_scan_footer(srds_access_t *sa__, uint8_t i, uint16_t *usr_diag_status) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
    uint8_t stepsize_var[NUM_LANES_MAX];
    uint8_t lane, num_lanes;
    ...
    if(i!=1) {lane_bak = osprey7_v2l8p2_acc_get_lane(sa__);}
    for(lane=0;lane<i;lane++) {
        if(i!=1) {EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));}
        if(osprey7_v2l8p2_info_ptr->info_table_version > 0x37) {
            if(((*(usr_diag_status+lane))&DIAG_EYE_SCAN_RANGE_MASK) == DIAG_EYE_SCAN_H_HALF) {
...
err_code_t osprey7_v2l8p2_read_eye_scan_status(srds_access_t *sa__, uint16_t *status) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p2_meas_eye_scan_done(srds_access_t *sa__) {
  INIT_SRDS_ERR_CODE
  EFUN(osprey7_v2l8p2_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_DISABLE,GRACEFUL_STOP_TIME));
  return(ERR_CODE_NONE);
}


err_code_t osprey7_v2l8p2_start_ber_scan_test(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    INIT_SRDS_ERR_CODE
    uint8_t lock,sts;
    uint8_t ffe_enabled;
    EFUN(osprey7_v2l8p2_INTERNAL_get_ffe_enabled(sa__, &ffe_enabled));
    ...
    EFUN(osprey7_v2l8p2_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_START, ber_scan_mode,GRACEFUL_STOP_TIME));
    return(ERR_CODE_NONE);
}

err_code_t osprey7_v2l8p2_read_ber_scan_data(srds_access_t *sa__, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
    INIT_SRDS_ERR_CODE
    ...
    if((sts & 0x8000) == 0) {
        return(osprey7_v2l8p2_error(sa__, ERR_CODE_DATA_NOTAVAIL));
    }
    *cnt = (uint8_t)((sts & 0x00FF)/3);
    for(i=0;i < *cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(osprey7_v2l8p2_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(osprey7_v2l8p2_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, timeout));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors[i] = osprey7_v2l8p2_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values[i] = (osprey7_v2l8p2_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
...
err_code_t osprey7_v2l8p2_eye_margin_proj_with_data(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    INIT_SRDS_ERR_CODE
    int16_t offset_start = 0;

    /* start UC acquisition */
    EFUN(osprey7_v2l8p2_INTERNAL_eye_margin_proj_start(sa__, ber_scan_mode, timer_control, max_error_control, &offset_start));

    EFUN_PRINTF(("Waiting for measurement time approx %d seconds",timer_control+(timer_control>>1)));
    EFUN(osprey7_v2l8p2_INTERNAL_eye_margin_proj_end(sa__, rate, ber_scan_mode, timer_control, offset_start));

    return(ERR_CODE_NONE);
}

err_code_t osprey7_v2l8p2_display_all_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    INIT_SRDS_ERR_CODE
    ...
    for(lane = 0; lane < num_lanes; lane++) {
        ...
        /* start UC acquisition */
        EFUN(osprey7_v2l8p2_INTERNAL_eye_margin_proj_start(sa__, ber_scan_mode, timer_control, max_error_control, &offset_start[lane]));
    }
    EFUN_PRINTF(("Waiting for measurement time approx %d seconds",timer_control+(timer_control>>1)));
    for(lane = 0; lane < num_lanes; lane++) {
        EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));
        ...
        EFUN(osprey7_v2l8p2_INTERNAL_eye_margin_proj_end(sa__, rate, ber_scan_mode, timer_control, offset_start[lane]));
    }
...
err_code_t osprey7_v2l8p2_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    INIT_SRDS_ERR_CODE
    EFUN(osprey7_v2l8p2_eye_margin_proj_with_data(sa__, rate, ber_scan_mode, timer_control, max_error_control));
    return(ERR_CODE_NONE);
}



err_code_t osprey7_v2l8p2_display_ber_scan_data (srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
    ...
    do {
        if(heye == 1) {
            ...
            if(intrusive) {
                slicer = LMS_SLICER;
                CFUN(osprey7_v2l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], 0, slicer, &nlmv_val));
                margins[i] = direction * nlmv_val;
            } else {
                slicer = LMS_SLICER;
                CFUN(osprey7_v2l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)offset[i], range250, slicer, &nlmv_val));
                margins[i] = direction * nlmv_val;
            }
#else
            margins[i] = info_out->margins[i];
...
err_code_t osprey7_v2l8p2_display_lane_state_hdr(srds_access_t *sa__) {
  EFUN_PRINTF(("LN (P RX  , CDRxN , UC_CFG,   UC_STS,  RST, STP,PLL) "));
...
err_code_t osprey7_v2l8p2_display_lane_state_legend(srds_access_t *sa__) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**********************************************************************************************\n"));
  ...
  EFUN_PRINTF(("EYE(L,R,U,D) : Eye margin @ 1e-5 (NRZ) or 1e-3 (PAM4), as seen by internal diagnostic slicer in mUI and mV\n"));
...
err_code_t osprey7_v2l8p2_display_lane_state(srds_access_t *sa__) {
  err_code_t err_code = osprey7_v2l8p2_INTERNAL_display_lane_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t osprey7_v2l8p2_display_core_state_hdr(srds_access_t *sa__) {
    char core_type[25] = "osprey7_v2l8p2";
...
err_code_t osprey7_v2l8p2_display_core_state_line(srds_access_t *sa__) {
  err_code_t err_code = osprey7_v2l8p2_INTERNAL_display_core_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t osprey7_v2l8p2_display_core_state_legend(srds_access_t *sa__) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
...
/**********************************************/
err_code_t osprey7_v2l8p2_display_core_config(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    EFUN_PRINTF(("\n\n***********************************\n"  ));
    EFUN_PRINTF((    "**** SERDES CORE CONFIGURATION ****\n"  ));
    EFUN_PRINTF((    "***********************************\n\n"));
    {
        struct   osprey7_v2l8p2_uc_core_config_st core_cfg = UC_CORE_CONFIG_INIT;
        struct   osprey7_v2l8p2_uc_core_config_st pll1_cfg = UC_CORE_CONFIG_INIT;
        {
            uint8_t pll_orig;
            ESTM(pll_orig = osprey7_v2l8p2_acc_get_pll_idx(sa__));
            EFUN(osprey7_v2l8p2_acc_set_pll_idx(sa__, 0));
            EFUN(osprey7_v2l8p2_get_uc_core_config(sa__, &core_cfg));
            EFUN(osprey7_v2l8p2_acc_set_pll_idx(sa__, 1));
            EFUN(osprey7_v2l8p2_get_uc_core_config(sa__, &pll1_cfg));
    ...
    EFUN_PRINTF((    "Die Temperature Offset %d\n\n"                              , rdcv_die_temperature_offset()));
#endif
...
err_code_t osprey7_v2l8p2_display_lane_config(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    struct osprey7_v2l8p2_uc_lane_config_st lane_cfg;
    ...
    EFUN_PRINTF((    "*************************************\n\n"                                        ));
    EFUN(                                                         osprey7_v2l8p2_get_uc_lane_cfg(sa__, &lane_cfg));
...
err_code_t osprey7_v2l8p2_display_core_state(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    EFUN(osprey7_v2l8p2_display_core_state_hdr(sa__));
    EFUN(osprey7_v2l8p2_display_core_state_line(sa__));
    EFUN(osprey7_v2l8p2_display_core_state_legend(sa__));
    return ERR_CODE_NONE;
}

err_code_t osprey7_v2l8p2_display_all_lane_state(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint8_t lane = 0, num_lanes = 0;
    uint8_t lane_orig = osprey7_v2l8p2_acc_get_lane(sa__);

    ESTM(num_lanes = rdc_revid_multiplicity());
    EFUN(osprey7_v2l8p2_display_lane_state_hdr(sa__));
    for(lane = 0; lane < num_lanes; lane++) {
        EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));
        EFUN(osprey7_v2l8p2_display_lane_state(sa__));
    }
    EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane_orig));
...
err_code_t osprey7_v2l8p2_display_lane_debug_status(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    /* startup */
    struct osprey7_v2l8p2_usr_ctrl_disable_functions_st     ds;
    struct osprey7_v2l8p2_usr_ctrl_disable_dfe_functions_st dsd;
    /* steady state */
    struct osprey7_v2l8p2_usr_ctrl_disable_functions_st     dss;
    struct osprey7_v2l8p2_usr_ctrl_disable_dfe_functions_st dssd;
    uint8_t core;
    uint8_t lane;
    ...
    ESTM(core=osprey7_v2l8p2_acc_get_core(sa__));
    ...
    EFUN(osprey7_v2l8p2_get_usr_ctrl_disable_startup(sa__, &ds));
    ...
    EFUN(                                                            osprey7_v2l8p2_get_usr_ctrl_disable_startup_dfe(sa__, &dsd));
    ...
    EFUN(                                                           osprey7_v2l8p2_get_usr_ctrl_disable_steady_state(sa__, &dss));
    ...
    EFUN(                                                      osprey7_v2l8p2_get_usr_ctrl_disable_steady_state_dfe(sa__, &dssd));
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    struct osprey7_v2l8p2_uc_core_config_st core_config;
    err_code_t Exit_Status = ERR_CODE_NONE;
    ...
    CFUN(osprey7_v2l8p2_get_uc_core_config(sa__, &core_config));
    CFUN(osprey7_v2l8p2_INTERNAL_get_osr_mode(sa__, &osr_mode));

    if(osr_mode.rx > OSPREY7_V2L8P2_OSX4)
    {
    ...
    CFUN(osprey7_v2l8p2_start_ber_scan_test(sa__, ber_scan_mode, timer_control, max_error_control));
    ...
    if(ber_scan_mode & DIAG_BER_HORZ) {
        ...
        slicer = LMS_SLICER;
        CFUN(osprey7_v2l8p2_INTERNAL_ladder_setting_to_mV(sa__, offset_start, range250, slicer, &nlmv_val));
    ...
    CFUN(osprey7_v2l8p2_INTERNAL_poll_diag_done(sa__, &sts,(uint32_t)(timer_control*2000)));
    ...
    if((sts & 0x8000) == 0) {
        ...
        osprey7_v2l8p2_error(sa__, Exit_Status);
        goto Exit;
    }
    cnt = (uint8_t)((sts & 0x00FF)/3);
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        CFUN(osprey7_v2l8p2_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 200));
        CSTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        CFUN(osprey7_v2l8p2_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 200));
        CSTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors = osprey7_v2l8p2_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values = (osprey7_v2l8p2_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
        if(DIAG_VERBOSE < 5) {
            if (!(i % 4))  {
                USR_PRINTF(("\n"));
            }
            if(ber_scan_mode & DIAG_BER_HORZ) {
                ...
                slicer = LMS_SLICER;
                CFUN(osprey7_v2l8p2_INTERNAL_ladder_setting_to_mV(sa__, (int16_t)(SRDS_ABS(offset_start)-i), range250, slicer, &nlmv_val));
    ...
    USR_PRINTF(("\n"));
    CFUN(osprey7_v2l8p2_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,2000));
Exit:
...
err_code_t osprey7_v2l8p2_display_all_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    INIT_SRDS_ERR_CODE
    EFUN(osprey7_v2l8p2_display_diag_data_core(sa__, diag_level, 0xFF));
    return ERR_CODE_NONE;
}
err_code_t osprey7_v2l8p2_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    INIT_SRDS_ERR_CODE
    uint8_t lane = osprey7_v2l8p2_acc_get_lane(sa__), lane_select=0;

    if (lane < 8) {
        lane_select = (uint8_t)(0x1<<lane);
        EFUN(osprey7_v2l8p2_display_diag_data_core(sa__, diag_level, lane_select));
    }
    else {
...
err_code_t osprey7_v2l8p2_display_diag_data_core(srds_access_t *sa__, uint16_t  diag_level, uint8_t lane_select) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(osprey7_v2l8p2_version(sa__, &api_version));
    ...
    EFUN_PRINTF(("\n"));
    EFUN_PRINTF(("SerDes type          = %s\n",core_type));
    if(OSPREY7_V2L8P2_UAPI_SWITCH == OSPREY7_V2L8P2_UAPI_C1_AFE_WITH_REVID2_7) {
        EFUN_PRINTF(("Silicon Version      = %X\n", OSPREY7_V2L8P2_AFE_IPVERSION_ID_DEFAULT_C1));
    }
    else {
    ...
    EFUN_PRINTF(("\n"));
    ESTM(num_lanes = rdc_revid_multiplicity());
    save_lane = osprey7_v2l8p2_acc_get_lane(sa__);
    ...
    for (lane=0; lane<num_lanes; lane++) {
        if (lane_select & (0x1<<lane)) {
            ...
            micro_stop[lane] = osprey7_v2l8p2_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
            if(err_code) USR_PRINTF(("Unable to stop lane %d micro,  following data is suspect\n", lane));
        }
    }
    EFUN(osprey7_v2l8p2_acc_set_lane(sa__, save_lane));
    if((diag_level & SRDS_DIAG_TIMESTAMP) || (diag_level & SRDS_DIAG_CORE)) {
        diag_data_err = osprey7_v2l8p2_test_uc_timestamp(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
            EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_TIMESTAMP returned error %s ****\n", osprey7_v2l8p2_INTERNAL_e2s_err_code(diag_data_err)));
            diag_level_error |= SRDS_DIAG_TIMESTAMP;
        }
    }
    if(diag_level & SRDS_DIAG_CORE) {
        EFUN(osprey7_v2l8p2_display_core_state_hdr(sa__));
        diag_data_err = osprey7_v2l8p2_display_core_state_line(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    for (lane=0; lane<num_lanes; lane++) {
        if (lane_select & (0x1<<lane)) {
            EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));
            diag_data_err = osprey7_v2l8p2_display_lane_state(sa__);
            if(diag_data_err != ERR_CODE_NONE) {
                EFUN_PRINTF(("\n**** WARNING: osprey7_v2l8p2_display_lane_state() returned error %s ****\n", osprey7_v2l8p2_INTERNAL_e2s_err_code(diag_data_err)));
            }
        }
    }
    EFUN(osprey7_v2l8p2_acc_set_lane(sa__, save_lane));
    ...
    if(diag_level & SRDS_DIAG_LANE) {
        struct osprey7_v2l8p2_detailed_lane_status_st *lane_st;
        err_code_t Exit_Status = ERR_CODE_NONE;
        lane_st = (struct osprey7_v2l8p2_detailed_lane_status_st *)USR_CALLOC(1, sizeof(struct osprey7_v2l8p2_detailed_lane_status_st));
        if(!lane_st) {
            return (ERR_CODE_MEM_ALLOC_FAIL);
        }
        save_lane = osprey7_v2l8p2_acc_get_lane(sa__);
        for (lane=0; lane<num_lanes; lane++) {
            if (lane_select & (0x1<<lane)) {
                CFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));
                CFUN(osprey7_v2l8p2_log_full_pmd_state(sa__, lane_st));
                CFUN(osprey7_v2l8p2_disp_full_pmd_state(sa__, lane_st, 1));
            }
        }
        CFUN(osprey7_v2l8p2_acc_set_lane(sa__, save_lane));
Exit:
        USR_FREE(lane_st);
        if(Exit_Status != ERR_CODE_NONE) {
            EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_LANE returned error %s ****\n", osprey7_v2l8p2_INTERNAL_e2s_err_code(Exit_Status)));
            diag_level_error |= SRDS_DIAG_LANE;
            diag_data_err = osprey7_v2l8p2_error_report(sa__, Exit_Status);
        }
    }
    ...
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
        ...
        for (; micro_num<osprey7_v2l8p2_info_ptr->micro_count; ++micro_num)
        {
            ...
            state.zero_cnt = 0;
            save_lane = osprey7_v2l8p2_acc_get_lane(sa__);
            diag_data_err = osprey7_v2l8p2_INTERNAL_read_event_log_with_callback(sa__, micro_num, 1, &state, osprey7_v2l8p2_INTERNAL_event_log_dump_callback);
            EFUN(osprey7_v2l8p2_acc_set_lane(sa__,save_lane));
            if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_EVENT) {
        save_lane = osprey7_v2l8p2_acc_get_lane(sa__);
        diag_data_err = osprey7_v2l8p2_read_event_log(sa__);
        EFUN(osprey7_v2l8p2_acc_set_lane(sa__,save_lane));
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if (diag_level & SRDS_DIAG_REG_CORE) {
        diag_data_err = osprey7_v2l8p2_reg_select_dump(sa__, SRDS_CORE_REG_ONLY, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if (diag_level & SRDS_DIAG_REG_LANE) {
        ...
        for (lane=0; lane<num_lanes; lane++) {
            if (lane_select & (0x1<<lane)) {
                EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));
                diag_data_err = osprey7_v2l8p2_reg_select_dump(sa__, SRDS_LANE_REG_ONLY, NULL);
                if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        data_cnt = (uint16_t)osprey7_v2l8p2_info_ptr->core_var_ram_size;

        diag_data_err = osprey7_v2l8p2_uc_core_var_blk_dump(sa__, core, data_cnt, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
        ...
        save_lane = osprey7_v2l8p2_acc_get_physical_lane(sa__);
        for (lane = 0; lane < num_micros*2; lane =(uint8_t)(lane + 2)) {
            EFUN(osprey7_v2l8p2_acc_set_physical_lane(sa__, lane));

            diag_data_err = osprey7_v2l8p2_uc_micro_var_blk_dump(sa__, core, lane, data_cnt, NULL);
            if(diag_data_err != ERR_CODE_NONE) {
                EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_UC_CORE returned error %s ****\n", osprey7_v2l8p2_INTERNAL_e2s_err_code(diag_data_err)));
                diag_level_error |= SRDS_DIAG_UC_CORE;
            }
        }
        EFUN(osprey7_v2l8p2_acc_set_physical_lane(sa__, save_lane));
        }
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        ...
        for (lane=0; lane<num_lanes; lane++) {
            if (lane_select & (0x1<<lane)) {
                EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));
                diag_data_err = osprey7_v2l8p2_uc_lane_static_var_blk_dump(sa__, core, lane, data_cnt, NULL);
                if(diag_data_err != ERR_CODE_NONE) {
                    EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_UC_LANE_STATIC returned error %s ****\n", osprey7_v2l8p2_INTERNAL_e2s_err_code(diag_data_err)));
                    diag_level_error |= SRDS_DIAG_UC_LANE;
                }
                diag_data_err = osprey7_v2l8p2_uc_lane_var_blk_dump(sa__, core, lane, data_cnt, NULL);
                if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_EYE) {
        ...
        for (lane=0; lane<num_lanes; lane++) {
            if (lane_select & (0x1<<lane)) {
                EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));
                diag_data_err = osprey7_v2l8p2_display_eye_scan(sa__);
                if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        ...
        for (lane=0; lane<num_lanes; lane++) {
            if (lane_select & (0x1<<lane)) {
                EFUN(osprey7_v2l8p2_acc_set_lane(sa__, lane));
                diag_data_err = osprey7_v2l8p2_display_lane_debug_status(sa__);
                if(diag_data_err != ERR_CODE_NONE) {
    ...
    if((diag_level & SRDS_DIAG_PRBS_PROJ)
        ...
        diag_data_err = osprey7_v2l8p2_display_aggregate_prbs_error_analyzer_proj(sa__, 5440, SRDS_OSP_100GE, lane_mask_aggregate, timeout_s, diag_mode);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_INFO_TABLE) {
        diag_data_err = osprey7_v2l8p2_INTERNAL_display_info_table(sa__);
        if(diag_data_err != ERR_CODE_NONE) {
...
} osprey7_v2l8p2_diag_access_callback_state_t;

static err_code_t osprey7_v2l8p2_diag_access_read_byte_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    osprey7_v2l8p2_diag_access_callback_state_t * const state_ptr = (osprey7_v2l8p2_diag_access_callback_state_t *)arg;
    while (byte_count > 0) {
...
static err_code_t osprey7_v2l8p2_diag_access_read_word_callback(srds_access_t *sa__, void *arg, uint8_t byte_count, uint16_t data) {
    osprey7_v2l8p2_diag_access_callback_state_t * const state_ptr = (osprey7_v2l8p2_diag_access_callback_state_t *)arg;
    UNUSED(byte_count);
...
/* Required Diagnostic Functions */
err_code_t osprey7_v2l8p2_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint32_t addr, uint32_t data, uint16_t param) {
    INIT_SRDS_ERR_CODE
    srds_info_t * const osprey7_v2l8p2_info_ptr = osprey7_v2l8p2_INTERNAL_get_osprey7_v2l8p2_info_ptr_with_check(sa__);
    osprey7_v2l8p2_diag_access_callback_state_t state;
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(osprey7_v2l8p2_acc_rdt_reg(sa__, (uint16_t)(i+addr),&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(osprey7_v2l8p2_acc_rdt_reg(sa__, (uint16_t)addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(osprey7_v2l8p2_acc_wr_reg(sa__, (uint16_t)addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(osprey7_v2l8p2_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           addr + osprey7_v2l8p2_info_ptr->core_var_ram_base,
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                                                           osprey7_v2l8p2_diag_access_read_byte_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read byte: x%04x = x%02x\n",(uint8_t)addr,osprey7_v2l8p2_rdbc_uc_var(sa__, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 EFUN_PRINTF(("\n****  SERDES BLK LANE RAM READ BYTE   ****"));
                                 EFUN(osprey7_v2l8p2_INTERNAL_rdblk_uc_generic_ram(sa__, (addr + osprey7_v2l8p2_info_ptr->lane_var_ram_base + (osprey7_v2l8p2_acc_get_physical_lane(sa__) * osprey7_v2l8p2_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read byte: x%04x = x%02x\n",addr,osprey7_v2l8p2_rdbl_uc_var(sa__, __ERR, (uint16_t)addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 EFUN(osprey7_v2l8p2_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           ...
                                                                           ((data+1)>>1)<<1, /* Round up to nearest word count */
                                                                           &state,
                                                                           osprey7_v2l8p2_diag_access_read_word_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 ESTM_PRINTF(("Core RAM Read word: x%04x = x%04x\n",addr,osprey7_v2l8p2_rdwc_uc_var(sa__, __ERR, (uint8_t)addr)));
                             }
                             } break;
    case SRDS_LANE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Lane RAM Read word: x%04x = x%04x\n",addr,osprey7_v2l8p2_rdwl_uc_var(sa__, __ERR, (uint16_t)addr)));
                             }
                             } break;
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = osprey7_v2l8p2_rdbc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(osprey7_v2l8p2_wrbc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = osprey7_v2l8p2_rdbl_uc_var(sa__, __ERR, (uint16_t)addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(osprey7_v2l8p2_wrbl_uc_var(sa__, (uint16_t)addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = osprey7_v2l8p2_rdwc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & (uint16_t)data));
                             EFUN(osprey7_v2l8p2_wrwc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = osprey7_v2l8p2_rdwl_uc_var(sa__, __ERR,(uint16_t)addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & (uint16_t)data));
                             EFUN(osprey7_v2l8p2_wrwl_uc_var(sa__, (uint16_t)addr,tmp2));
    ...
    case SRDS_GLOB_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read byte: x%04x = x%02x\n",addr,osprey7_v2l8p2_INTERNAL_rdb_uc_var(sa__, __ERR, DATA_RAM_BASE | addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = osprey7_v2l8p2_INTERNAL_rdb_uc_var(sa__, __ERR, DATA_RAM_BASE | addr));
                             tmp2 = (uint8_t)((tmp & (uint8_t)~param) | ((uint8_t)param & data));
                             EFUN(osprey7_v2l8p2_INTERNAL_wrb_uc_var(sa__, DATA_RAM_BASE | addr, tmp2));
    ...
    case SRDS_GLOB_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Glob RAM Read word: x%04x = x%04x\n",addr,osprey7_v2l8p2_INTERNAL_rdw_uc_var(sa__, __ERR, DATA_RAM_BASE | addr)));
                             }
                             } break;
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = osprey7_v2l8p2_INTERNAL_rdw_uc_var(sa__, __ERR, DATA_RAM_BASE | addr));
                             tmp2 = (uint16_t)((tmp & ~param) | (param & (uint16_t)data));
                             EFUN(osprey7_v2l8p2_INTERNAL_wrw_uc_var(sa__, DATA_RAM_BASE | addr, tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(osprey7_v2l8p2_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, (uint16_t)data, GRACEFUL_STOP_TIME));
    ...
    case SRDS_PROG_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Prog RAM Read byte: x%04x = x%02x\n",addr,osprey7_v2l8p2_INTERNAL_rdb_uc_var(sa__, __ERR, addr)));
                             }
                             } break;
    case SRDS_PROG_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 ESTM_PRINTF(("Prog RAM Read word: x%04x = x%04x\n",addr,osprey7_v2l8p2_INTERNAL_rdw_uc_var(sa__, __ERR, addr)));
                             }
                             } break;
    case SRDS_EN_BREAKPOINT: {
        EFUN(osprey7_v2l8p2_INTERNAL_en_breakpoint(sa__, (uint8_t)addr));
                             } break;
    case SRDS_GOTO_BREAKPOINT: {
        EFUN(osprey7_v2l8p2_INTERNAL_goto_breakpoint(sa__, (uint8_t)addr));
                             } break;
    case SRDS_RD_BREAKPOINT: {
        EFUN(osprey7_v2l8p2_INTERNAL_rd_breakpoint(sa__));
                             } break;
    case SRDS_DIS_BREAKPOINT: {
        EFUN(osprey7_v2l8p2_INTERNAL_dis_breakpoint(sa__));
                             } break;
    ...
    default: EFUN_PRINTF(("Invalid request type osprey7_v2l8p2_diag_access\n"));
    }

    return(ERR_CODE_NONE);
}

err_code_t osprey7_v2l8p2_display_state (srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    EFUN(osprey7_v2l8p2_display_core_state(sa__));
    EFUN(osprey7_v2l8p2_display_lane_state_hdr(sa__));
    EFUN(osprey7_v2l8p2_display_lane_state(sa__));
    EFUN(osprey7_v2l8p2_display_lane_state_legend(sa__));
    return(ERR_CODE_NONE);
}

err_code_t osprey7_v2l8p2_display_config (srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    EFUN(osprey7_v2l8p2_display_core_config(sa__));
    EFUN(osprey7_v2l8p2_display_lane_config(sa__));
    return(ERR_CODE_NONE);
}
...
err_code_t osprey7_v2l8p2_force_die_temperature (srds_access_t *sa__, int16_t die_temp) {
    INIT_SRDS_ERR_CODE
    /* disable force */
    if(die_temp == -255) {
...
err_code_t osprey7_v2l8p2_read_die_temperature (srds_access_t *sa__, int16_t *die_temp) {
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p2_read_die_temperature_double (srds_access_t *sa__, USR_DOUBLE *die_temp) {
#ifdef SERDES_API_FLOATING_POINT
    uint16_t die_temp_sensor_reading;
    INIT_SRDS_ERR_CODE
...
#else
    *die_temp = 0;
    EFUN_PRINTF((" Function 'osprey7_v2l8p2_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
    return(osprey7_v2l8p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
#endif
}
...
err_code_t osprey7_v2l8p2_display_linktrn_status(srds_access_t *sa__) {
  INIT_SRDS_ERR_CODE
  EFUN_PRINTF(("\n\n***************************\n"                                                                                                                                    ));
