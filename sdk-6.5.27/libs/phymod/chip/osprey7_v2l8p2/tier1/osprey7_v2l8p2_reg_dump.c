/*************************************************************************************
 *                                                                                   *
 * Copyright: (c) 2021 Broadcom.                                                     *
 * Broadcom Proprietary and Confidential. All rights reserved.                       *
 *                                                                                   *
 *************************************************************************************/

/*************************************************************************************
 *************************************************************************************
 *                                                                                   *
 *  Revision      :  Id                                                              *
 *                                                                                   *
 *  Description   :  const array definition used by  osprey7_v2l8p2_reg_dump().              *
 *                   Each row of osprey7_v2l8p2_reg_dump_arr represents a block of 16        *
 *                   addresses. The first value of a row is the starting address of  *
 *                   a register block, and the second value is a valid register      *
 *                   enable bitmap. The bitmap represents which addresses are valid  *
 *                   within the block.                                               *
 *                                                                                   *
 *************************************************************************************
 *************************************************************************************/


/* THIS FILE IS GENERATED USING AN AUTOMATED SCRIPT... PLEASE DO NOT EDIT THIS FILE DIRECTLY !!! */

#include "osprey7_v2l8p2_reg_dump.h"

/*! @file
 *
 */

osprey7_v2l8p2_reg_dump_arr_t *osprey7_v2l8p2_get_reg_dump_arr(void) {
    static const uint16_t osprey7_v2l8p2_reg_dump_arr[OSPREY7_V2L8P2_REG_DUMP_SECTIONS][2] = {
    /* start register block, valid register enable */
    {0x0090, 0x0fc0},
    {0xd080, 0x6fff},
    {0xd090, 0x01ff},
    {0xd0a0, 0x7fff},
    {0xd0b0, 0x5eeb},
    {0xd0c0, 0x7fff},
    {0xd0d0, 0x011f},
    {0xd0e0, 0x1f07},
    {0xd130, 0x7fff},
    {0xd160, 0x7fff},
    {0xd170, 0x4f2f},
    {0xd1a0, 0x71be},
    {0xd1b0, 0x31be},
    {0xd1c0, 0x5eeb},
    {0xd1d0, 0x5eeb},
    {0xd1e0, 0x7fff},
    {0xd1f0, 0x1fff},
    {0xd330, 0x1f1f},
    {0xd340, 0x400f},
    {0xd350, 0x0007},
    {0xd400, 0x3fff},
    {0xd430, 0x4000},
    {0xd470, 0x007f},
    {0xd4d0, 0x7fff},
    {0xd500, 0xffff},
    {0xd510, 0xffff},
    {0xd520, 0xffff},
    {0xd530, 0xffff},
    {0xd540, 0xffff},
    {0xd550, 0xffff},
    {0xd560, 0x3fff},
    {0xd570, 0x7fff},
    {0xd580, 0x3fff},
    {0xd590, 0x7dff},
    {0xd5a0, 0x7fff},
    {0xd5b0, 0x7fff},
    {0xd5c0, 0x3fff},
    {0xd5d0, 0x7fff},
    {0xd5e0, 0x7fff},
    {0xd5f0, 0x7fff},
    {0xd600, 0x7fff},
    {0xd610, 0x7ff0},
    {0xd620, 0x3fff},
    {0xd630, 0x6fff},
    {0xd640, 0x7fff},
    {0xd650, 0x7fff},
    {0xd660, 0x7fff},
    {0xd100, 0x45ff},
    {0xd120, 0x7fff},
    {0xd150, 0x007c},
    {0xd190, 0x78ff},
    {0xd200, 0x7fff},
    {0xd210, 0x7fff},
    {0xd220, 0x7fff},
    {0xd230, 0x400f},
    {0xd300, 0x07ff},
    {0xd310, 0x01ef},
    {0xffd0, 0x7000},
    {0xd110, 0x7fff},
    {0xd140, 0x3f7f},
    {0xd180, 0x6337},
    {0xd320, 0x0fff},
    {0xd240, 0x7fff},
    };
    return osprey7_v2l8p2_reg_dump_arr;
}
