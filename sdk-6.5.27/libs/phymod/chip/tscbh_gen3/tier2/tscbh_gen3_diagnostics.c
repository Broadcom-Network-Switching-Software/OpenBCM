...
int tscbh_gen3_phy_fec_error_bits_counter_get(const phymod_phy_access_t* phy, uint32_t* count)
{
    phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (tbhmod_gen3_fec_error_bits_counter_get(&phy_copy.access, count));
