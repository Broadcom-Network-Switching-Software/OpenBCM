...
int tscbh_flexe_phy_tx_set(const phymod_phy_access_t* phy, const phymod_tx_t* tx)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        if (tx->tap_mode == phymodTxTapMode3Tap) {
            ...
            rv |= blackhawk_tsc_apply_txfir_cfg(&phy_copy.access,
                                                 enable_taps,
                                                 0,
                                                 tx->pre,
                                                 tx->main,
                                        ...
                                        phymod_phy_speed_config_t* speed_config)
{
    ...
    if ((int)ref_clk == REF_CLOCK_312P5) {
        refclk_in_hz = 312500000;
    } else {
        refclk_in_hz = 156250000;
    }
...
int tscbh_flexe_phy_tx_pam4_precoder_enable_set(const phymod_phy_access_t* phy, int enable)
{
    ...
    if (pcs_lane_enable) {
        ...
        PHYMOD_IF_ERR_RETURN
            (tbhmod_flexe_pcs_reset_sw_war(&phy_copy.access));
    }
...
/*Set timesync adjust*/
int tscbh_flexe_timesync_adjust_set(const phymod_phy_access_t* phy,
                                    uint32_t flags,
...
int tscbh_flexe_phy_fec_corruption_period_set(const phymod_phy_access_t* phy, uint32_t period)
{
    ...
    /*The unit is 1ms in API*/
    PHYMOD_IF_ERR_RETURN
        (tbhmod_tick_time_get(&phy_copy.access, &tick_time));
    PHYMOD_IF_ERR_RETURN
        (tbhmod_flexe_fec_corruption_period_set(&phy_copy.access, ((period * 1000) / tick_time)));
...
int tscbh_flexe_phy_fec_corruption_period_get(const phymod_phy_access_t* phy, uint32_t *period)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (tbhmod_tick_time_get(&phy_copy.access, &tick_time));
    PHYMOD_IF_ERR_RETURN
        (tbhmod_flexe_fec_corruption_period_get(&phy_copy.access, &period_time));
    *period = ((period_time * tick_time) % 1000) ? ((period_time * tick_time) / 1000 + 1) : ((period_time * tick_time) / 1000);
