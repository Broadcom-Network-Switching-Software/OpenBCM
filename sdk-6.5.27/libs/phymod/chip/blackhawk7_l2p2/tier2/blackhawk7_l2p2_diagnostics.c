...
#ifdef PHYMOD_LINKCAT_BLACKHAWK7_L2P2_SUPPORT
                           ...
                           const phymod_phy_eyescan_options_t* eyescan_options)
{
    ...
    if (!lane_config.field.force_pam4_mode) {
        if ((osr_mode != PMD_OSR_1) &&
            (osr_mode != PMD_OSR_2) &&
            (osr_mode != PMD_OSR_4)) {
            PHYMOD_RETURN_WITH_ERR(PHYMOD_E_FAIL, (_PHYMOD_MSG("eyescan is supported by OSx1, OSx2 or OSx4 only!\n")));
        }
    }
                                ...
                                phymod_linkcat_config_t config)
{
    ...
    rv = blackhawk7_l2p2_LinkCat(&phy_acc, &linkcat_config, &linkcat_status);
