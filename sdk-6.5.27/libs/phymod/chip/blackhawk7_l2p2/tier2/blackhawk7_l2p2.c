...
{
    info->core_version = phymodCoreVersionBlackhawk7_l2p2;
    info->serdes_id = 0;
    info->phy_id0 = 0;
    info->phy_id1 = 0;
...
int blackhawk7_l2p2_phy_external_los_enable_set(const phymod_phy_access_t* phy, uint32_t enable)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_l2p2_ext_los_en_set(&phy_copy.access, enable));
    }
...
int blackhawk7_l2p2_phy_external_los_enable_get(const phymod_phy_access_t* phy,
                                                uint32_t* enable)
{
    phymod_phy_access_t pm_phy_copy;
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l2p2_ext_los_en_get(&pm_phy_copy.access, enable));

    return PHYMOD_E_NONE;
}

int blackhawk7_l2p2_phy_txpi_config_set(const phymod_phy_access_t* phy,
                                        const phymod_txpi_config_t* config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_l2p2_tx_pi_enable_set(&phy_copy.access, config->enable));

        PHYMOD_IF_ERR_RETURN
            (blackhawk7_l2p2_tx_pi_ext_phase_step_cnt_select_set(&phy_copy.access,
                 config->mode == PHYMOD_TXPI_EXT_PD_MODE ? 1 : 0));
    }
...
int blackhawk7_l2p2_phy_txpi_config_get(const phymod_phy_access_t* phy,
                                        phymod_txpi_config_t* config)
{
    ...
    PHYMOD_IF_ERR_RETURN
         (blackhawk7_l2p2_tx_pi_enable_get(&phy_copy.access, &config->enable));

    PHYMOD_IF_ERR_RETURN
         (blackhawk7_l2p2_tx_pi_ext_phase_step_cnt_select_get(&phy_copy.access, &ext_sel));

    config->mode = ext_sel ? PHYMOD_TXPI_EXT_PD_MODE : PHYMOD_TXPI_NORMAL_MODE;
