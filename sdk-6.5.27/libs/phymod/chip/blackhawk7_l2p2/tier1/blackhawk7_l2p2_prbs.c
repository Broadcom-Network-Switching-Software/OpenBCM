...
#define BLACKHAWK7_L2P2_MAX_LANE 8
...
static err_code_t _blackhawk7_l2p2_get_prbs_error_analyzer_prbs_chk_lock(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t *prbs_chk_lock);
...
err_code_t blackhawk7_l2p2_BER_setup_measurement(srds_access_t *sa__, uint16_t time_ms, struct blackhawk7_l2p2_ber_data_st *ber_data, struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) { 
    ...
    return(ERR_CODE_NONE);
} /* blackhawk7_l2p2_BER_setup_measurement() */
...
err_code_t blackhawk7_l2p2_BER_get_measurement(srds_access_t *sa__, struct blackhawk7_l2p2_ber_data_st *ber_data, struct blackhawk7_l2p2_prbs_chk_hw_timer_ctrl_st *prbs_chk_hw_timer_ctrl) { 
    ...
    return(ERR_CODE_NONE);
} /* blackhawk7_l2p2_BER_get_measurement() */
...
const char* blackhawk7_l2p2_get_e2s_prbs_mode_enum(enum srds_prbs_polynomial_enum prbs_poly_mode) {
    ...
    if(prbs_poly_mode >= sizeof(blackhawk7_l2p2_e2s_prbs_mode_enum)/sizeof(blackhawk7_l2p2_e2s_prbs_mode_enum[0])) {
        return "UNKNOWN";
    }
    return blackhawk7_l2p2_e2s_prbs_mode_enum[prbs_poly_mode];
}
...
err_code_t blackhawk7_l2p2_display_all_detailed_prbs_state(srds_access_t *sa__) {
    ...
    /* Temp fix for num_lanes, offical fix will be added in SDK-307029 */
    num_lanes = BLACKHAWK7_L2P2_MAX_LANE;

    EFUN(blackhawk7_l2p2_display_detailed_prbs_state_hdr(sa__));
    for(lane = 0; lane < num_lanes; lane++) {
        EFUN(blackhawk7_l2p2_acc_set_lane(sa__, lane));
        EFUN(blackhawk7_l2p2_display_detailed_prbs_state(sa__));
    }
...
err_code_t blackhawk7_l2p2_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    if(enabled) {
        ...
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            ...
            prbs_string =  blackhawk7_l2p2_get_e2s_prbs_mode_enum(prbs_poly_mode);
        }
...
err_code_t blackhawk7_l2p2_prbs_error_analyzer_reset(srds_access_t *sa__) {
    ...
    EFUN(blackhawk7_l2p2_clear_dac_pattern_memory(sa__));  
...
err_code_t blackhawk7_l2p2_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    fec_size = (uint8_t)(err_analyzer->prbs_err_fec_size / BLACKHAWK7_L2P2_PRBS_FEC_FRAME_SIZE_MULTIPLIER);
...
err_code_t blackhawk7_l2p2_prbs_error_analyzer_start(srds_access_t *sa__) {
    ...
    EFUN(blackhawk7_l2p2_prbs_err_count_state(sa__, &prbs_bit_errcnt, &lcklost));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l2p2_display_prbs_error_analyzer_config(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer, uint32_t timeout_s) {
    ...
    if (err_analyzer->diag_mode) {
        EFUN_PRINTF(("%u %hu %hhu %u %hhu %hhu %hhu %08x %08x\n",   timeout_s, err_analyzer->prbs_err_fec_size, err_analyzer->fec_size_frac, err_analyzer->prbs_err_aggregate_mode, err_analyzer->lanes_active, err_analyzer->diag_mode, blackhawk7_l2p2_acc_get_lane(sa__), err_analyzer->pp_field2, err_analyzer->pp_field1));
    }
    else {
        EFUN_PRINTF(("\n ****************************************************************************** \n"));
...
err_code_t blackhawk7_l2p2_get_prbs_error_analyzer_err_count(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    EFUN(_blackhawk7_l2p2_get_prbs_error_analyzer_prbs_chk_lock(sa__, err_analyzer, &prbs_lock));
    if (!prbs_lock) {
        return (ERR_CODE_NONE);
    }
...
err_code_t blackhawk7_l2p2_get_prbs_error_analyzer_aggregate_err_count(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    EFUN(blackhawk7_l2p2_get_prbs_error_analyzer_lane_err_count(sa__, err_analyzer, num_lanes)); 
...
err_code_t blackhawk7_l2p2_prbs_error_analyzer_aggregate_config(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer->lane_mask_aggregate >> lane) & 0x1) {
            ...
            EFUN(blackhawk7_l2p2_prbs_error_analyzer_config(sa__, err_analyzer));  
...
err_code_t blackhawk7_l2p2_display_all_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, uint32_t timeout_s) {
    ...
    EFUN(blackhawk7_l2p2_display_aggregate_prbs_error_analyzer_proj(sa__, prbs_err_fec_size, SRDS_IEEE_50GE, 0xFF, timeout_s, 0x0));
...
err_code_t blackhawk7_l2p2_display_aggregate_prbs_error_analyzer_proj(srds_access_t *sa__, uint16_t prbs_err_fec_size, enum srds_prbs_error_analyzer_aggregate_mode_enum prbs_err_aggregate_mode, uint8_t lane_mask_aggregate, uint32_t timeout_s, uint8_t diag_mode) {
    ...
    EFUN(blackhawk7_l2p2_prbs_error_analyzer_reset(sa__));  
    /* Configure PRBS Error Analyzer Aggregation Mode*/
    EFUN(blackhawk7_l2p2_prbs_error_analyzer_aggregate_config(sa__, &err_analyzer));  
    ...
    for(lane = 0;lane<num_lanes;lane++) {
        if ((err_analyzer.lane_mask_aggregate >> lane) & 0x1) {
            ...
            EFUN(blackhawk7_l2p2_prbs_error_analyzer_start(sa__));  
        }
    }
...
USR_DOUBLE _blackhawk7_l2p2_get_ber_from_fec_table(srds_access_t *sa__, USR_DOUBLE);
USR_DOUBLE _blackhawk7_l2p2_get_ber_from_fec_table(srds_access_t *sa__, USR_DOUBLE i_ber) {
#ifdef SERDES_API_FLOATING_POINT
    uint32_t i=0;
...
err_code_t blackhawk7_l2p2_prbs_error_analyzer_report_proj(srds_access_t *sa__, blackhawk7_l2p2_prbs_err_analyzer_config_status_st err_analyzer, uint32_t timeout_s, blackhawk7_l2p2_prbs_err_analyzer_report_st *err_analyzer_report) {
    ...
    if (err_analyzer.prbs_err_fec_size == 10560) {
        if (delta_n >= 2) {
            ...
            proj_ber[7] = pow(proj_ber[7],1.0/BLACKHAWK7_L2P2_PRBS_ERR_ANALYZER_LINEARITY_ADJUST);
    ...
    if (err_analyzer_report->proj_ber == 0) {
        ...
        err_analyzer_report->proj_ber = _blackhawk7_l2p2_get_ber_from_fec_table(sa__, raw_ber);   
    }
