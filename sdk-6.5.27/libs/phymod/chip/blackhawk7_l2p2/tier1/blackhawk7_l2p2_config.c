...
/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  blackhawk7_l2p2_config.c                                           *
...
#include "blackhawk7_l2p2_version.h"
...
err_code_t blackhawk7_l2p2_stop_rx_adaptation(srds_access_t *sa__, uint8_t enable) {
  if (enable) {
      err_code_t err_code;
      err_code = blackhawk7_l2p2_pmd_uc_control(sa__, CMD_UC_CTRL_STOP_GRACEFULLY,GRACEFUL_STOP_TIME);
      if(err_code) {
          uint8_t temp=0;
          USR_PRINTF(("Warning Graceful stop request returned error %d; Requesting a forceful stop\n",err_code));
          temp = blackhawk7_l2p2_INTERNAL_stop_micro(sa__,0,&err_code);
          /* return value from blackhawk7_l2p2_INTERNAL_stop_micro - immediate stop is not required */
    ...
    return(blackhawk7_l2p2_pmd_uc_control(sa__, CMD_UC_CTRL_RESUME,50));
  }
}

err_code_t blackhawk7_l2p2_request_stop_rx_adaptation(srds_access_t *sa__) {

  return(blackhawk7_l2p2_pmd_uc_control_return_immediate(sa__, CMD_UC_CTRL_STOP_GRACEFULLY));
}
...
err_code_t blackhawk7_l2p2_ucode_crc_verify(srds_access_t *sa__, uint32_t ucode_len, uint16_t expected_crc_value) {
    ...
    if(calc_crc != expected_crc_value) {
        ...
        return(blackhawk7_l2p2_error(sa__, ERR_CODE_UC_CRC_NOT_MATCH));
    }
...
err_code_t blackhawk7_l2p2_set_uc_lane_cfg(srds_access_t *sa__, struct blackhawk7_l2p2_uc_lane_config_st struct_val) {
  ...
  if(reset_state < 7) {
      EFUN_PRINTF(("ERROR: blackhawk7_l2p2_set_uc_lane_cfg(..) called without ln_dp_s_rstb=0\n"));
      return (blackhawk7_l2p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
  }
  EFUN(blackhawk7_l2p2_INTERNAL_update_uc_lane_config_word(&struct_val));
  return(reg_wr_RX_CKRST_CTRL_LANE_UC_CONFIG(struct_val.word));
}

#define SRDS_FORCE_OSCDR_BIT    (5)
err_code_t blackhawk7_l2p2_get_force_lane_cdr_mode(srds_access_t *sa__, enum blackhawk7_l2p2_force_cdr_mode_enum *mode) {
    INIT_SRDS_ERR_CODE
    uint8_t os_mode = 0;
    uint8_t br_mode = 0;
    ...
    if(os_mode && br_mode) {
        return blackhawk7_l2p2_error(sa__, ERR_CODE_INVALID_MODE);
    }

    if(os_mode) {
        *mode = BLACKHAWK7_L2P2_OSCDR_FORCE_ENABLE;
    }
    else if(br_mode) {
        *mode = BLACKHAWK7_L2P2_BRCDR_FORCE_ENABLE;
    }
    else {
        *mode = BLACKHAWK7_L2P2_CDR_FORCE_DISABLE;
    }
    return ERR_CODE_NONE;
}
...
err_code_t blackhawk7_l2p2_force_lane_cdr_mode(srds_access_t *sa__, enum blackhawk7_l2p2_force_cdr_mode_enum mode) {
    ...
    switch(mode) {
        case BLACKHAWK7_L2P2_OSCDR_FORCE_ENABLE:
            ESTM(misc_ctrl = rdv_usr_misc_ctrl_word());
            EFUN(wrv_usr_misc_ctrl_word(misc_ctrl | (0x1 << SRDS_FORCE_OSCDR_BIT))); /*  (force_os_on) */
            EFUN(blackhawk7_l2p2_force_lane_cdr_mode(sa__, BLACKHAWK7_L2P2_BRCDR_FORCE_DISABLE));
            break;
        case BLACKHAWK7_L2P2_OSCDR_FORCE_DISABLE:
            ESTM(misc_ctrl = rdv_usr_misc_ctrl_word());
            EFUN(wrv_usr_misc_ctrl_word(misc_ctrl & (uint16_t)~(0x1 << SRDS_FORCE_OSCDR_BIT)));
            break;
        case BLACKHAWK7_L2P2_BRCDR_FORCE_ENABLE:
            EFUN(wr_lane_uc_config_force_brdfe_en(0x1));
            EFUN(blackhawk7_l2p2_force_lane_cdr_mode(sa__, BLACKHAWK7_L2P2_OSCDR_FORCE_DISABLE));
            break;
        case BLACKHAWK7_L2P2_BRCDR_FORCE_DISABLE:
            EFUN(wr_lane_uc_config_force_brdfe_en(0x0));
            break;
        case BLACKHAWK7_L2P2_CDR_FORCE_DISABLE:
            EFUN(blackhawk7_l2p2_force_lane_cdr_mode(sa__, BLACKHAWK7_L2P2_OSCDR_FORCE_DISABLE));
            EFUN(blackhawk7_l2p2_force_lane_cdr_mode(sa__, BLACKHAWK7_L2P2_BRCDR_FORCE_DISABLE));
            break;
        default:
...
err_code_t blackhawk7_l2p2_get_uc_core_config(srds_access_t *sa__, struct blackhawk7_l2p2_uc_core_config_st *get_val) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_update_uc_core_config_st(get_val));
    return ERR_CODE_NONE;
}
err_code_t blackhawk7_l2p2_get_physical_tx_addr(srds_access_t *sa__, uint8_t lane, uint8_t *physical_tx_lane) {
    ...
    const uint8_t num_lanes = 2;
    for(phy_lane = 0; phy_lane < num_lanes; phy_lane++) {
        EFUN(blackhawk7_l2p2_INTERNAL_get_logical_tx_lane_addr(sa__, phy_lane, &logical_tx_lane));
...
err_code_t blackhawk7_l2p2_get_physical_rx_addr(srds_access_t *sa__, uint8_t lane, uint8_t *physical_rx_lane) {
    ...
    const uint8_t num_lanes = 2;
    for(phy_lane = 0; phy_lane < num_lanes; phy_lane++) {
        EFUN(blackhawk7_l2p2_INTERNAL_get_logical_rx_lane_addr(sa__, phy_lane, &logical_rx_lane));
...
#ifndef SMALL_FOOTPRINT
err_code_t blackhawk7_l2p2_get_uc_lane_cfg(srds_access_t *sa__, struct blackhawk7_l2p2_uc_lane_config_st *get_val) {
    INIT_SRDS_ERR_CODE
  ...
  EFUN(blackhawk7_l2p2_INTERNAL_update_uc_lane_config_st(get_val));
  return (ERR_CODE_NONE);
}
#endif /* SMALL_FOOTPRINT */
...
/*---------------------------------*/
static err_code_t _blackhawk7_l2p2_rst_tx_ln_to_dfs(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
...
static err_code_t _blackhawk7_l2p2_rst_rx_ln_to_dfs(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_reset_tx_lane_to_default(srds_access_t *sa__) {
    ...
    BLACKHAWK7_L2P2_UAPI_INIT_AMS_A0      /* if BLACKHAWK7_V1LXP2 */
    EFUN(_blackhawk7_l2p2_rst_tx_ln_to_dfs(sa__));
    BLACKHAWK7_L2P2_UAPI_NOT_AMS_B0_ERR   /* if not BLACKHAWK7_V2LXP2 and not BLACKHAWK7_V3LXP2, print error */
    return ERR_CODE_NONE;
}
...
err_code_t blackhawk7_l2p2_reset_rx_lane_to_default(srds_access_t *sa__) {
    ...
    BLACKHAWK7_L2P2_UAPI_INIT_AMS_A0      /* if BLACKHAWK7_V1L8P2 */
    EFUN(_blackhawk7_l2p2_rst_rx_ln_to_dfs(sa__));
    BLACKHAWK7_L2P2_UAPI_NOT_AMS_B0_ERR   /* if not BLACKHAWK7_V2LXP2 and not BLACKHAWK7_V3LXP2, print error */
    return ERR_CODE_NONE;
}
...
err_code_t blackhawk7_l2p2_set_use_osr_mode_pins_only(srds_access_t *sa__, uint8_t use_pins_only) {
    INIT_SRDS_ERR_CODE

    EFUN(blackhawk7_l2p2_set_use_tx_osr_mode_pins_only(sa__, use_pins_only));
    EFUN(blackhawk7_l2p2_set_use_rx_osr_mode_pins_only(sa__, use_pins_only));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l2p2_get_use_osr_mode_pins_only(srds_access_t *sa__, uint8_t *tx_use_pins_only, uint8_t *rx_use_pins_only) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(blackhawk7_l2p2_get_use_tx_osr_mode_pins_only(sa__, tx_use_pins_only));
    EFUN(blackhawk7_l2p2_get_use_rx_osr_mode_pins_only(sa__, rx_use_pins_only));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l2p2_set_use_rx_osr_mode_pins_only(srds_access_t *sa__, uint8_t rx_use_pins_only) {
    INIT_SRDS_ERR_CODE
    uint8_t reset_state;
    ESTM(reset_state = rd_rx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk7_l2p2_set_use_rx_osr_mode_pins_only(..) called without ln_dp_s_rstb=0\n"));
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
    if(rx_use_pins_only) {
        EFUN(wr_rx_osr_mode_frc(0x0));
        EFUN(wr_rx_osr_mode_frc_val(BLACKHAWK7_L2P2_OSR_UNINITIALIZED));
    }

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l2p2_get_use_rx_osr_mode_pins_only(srds_access_t *sa__, uint8_t *rx_use_pins_only) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_set_use_tx_osr_mode_pins_only(srds_access_t *sa__, uint8_t tx_use_pins_only) {
    INIT_SRDS_ERR_CODE
    uint8_t reset_state;
    ESTM(reset_state = rd_tx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk7_l2p2_set_use_tx_osr_mode_pins_only(..) called without ln_dp_s_rstb=0\n"));
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
    if(tx_use_pins_only) {
        EFUN(wr_tx_osr_mode_frc(0x0));
        EFUN(wr_tx_osr_mode_frc_val(BLACKHAWK7_L2P2_OSR_UNINITIALIZED));
    }

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l2p2_get_use_tx_osr_mode_pins_only(srds_access_t *sa__, uint8_t *tx_use_pins_only) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l2p2_set_osr_mode(srds_access_t *sa__, enum blackhawk7_l2p2_osr_mode_enum osr_mode) {
    INIT_SRDS_ERR_CODE

    EFUN(blackhawk7_l2p2_set_tx_osr_mode(sa__, osr_mode));
    EFUN(blackhawk7_l2p2_set_rx_osr_mode(sa__, osr_mode));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l2p2_get_osr_mode(srds_access_t *sa__, enum blackhawk7_l2p2_osr_mode_enum *tx_osr_mode,  enum blackhawk7_l2p2_osr_mode_enum *rx_osr_mode) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(blackhawk7_l2p2_get_tx_osr_mode(sa__, tx_osr_mode));
    EFUN(blackhawk7_l2p2_get_rx_osr_mode(sa__, rx_osr_mode));

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l2p2_set_rx_osr_mode(srds_access_t *sa__, enum blackhawk7_l2p2_osr_mode_enum rx_osr_mode) {
    INIT_SRDS_ERR_CODE
    uint8_t reset_state;
    ESTM(reset_state = rd_rx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk7_l2p2_set_rx_osr_mode(..) called without ln_dp_s_rstb=0\n"));
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
...
err_code_t blackhawk7_l2p2_get_rx_osr_mode(srds_access_t *sa__, enum blackhawk7_l2p2_osr_mode_enum *rx_osr_mode) {
    INIT_SRDS_ERR_CODE
    uint8_t rx_osr_ctrl;
    ...
    if(rx_osr_ctrl) {
        ESTM(*rx_osr_mode = (enum blackhawk7_l2p2_osr_mode_enum)(rd_rx_osr_mode_frc_val()));
    }
    else {
        ESTM(*rx_osr_mode = (enum blackhawk7_l2p2_osr_mode_enum)rd_rx_osr_mode_pin());
    }

    return(ERR_CODE_NONE);
}

err_code_t blackhawk7_l2p2_set_tx_osr_mode(srds_access_t *sa__, enum blackhawk7_l2p2_osr_mode_enum tx_osr_mode) {
    INIT_SRDS_ERR_CODE
    uint8_t reset_state;
    ESTM(reset_state = rd_tx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk7_l2p2_set_tx_osr_mode(..) called without ln_dp_s_rstb=0\n"));
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
...
err_code_t blackhawk7_l2p2_get_tx_osr_mode(srds_access_t *sa__, enum blackhawk7_l2p2_osr_mode_enum *tx_osr_mode) {
    INIT_SRDS_ERR_CODE
    uint8_t tx_osr_ctrl;
    ...
    if(tx_osr_ctrl) {
        ESTM(*tx_osr_mode = (enum blackhawk7_l2p2_osr_mode_enum)(rd_tx_osr_mode_frc_val()));
    }
    else {
        ESTM(*tx_osr_mode = (enum blackhawk7_l2p2_osr_mode_enum)rd_tx_osr_mode_pin());
    }
...
err_code_t blackhawk7_l2p2_ucode_pram_load(srds_access_t *sa__, char const * ucode_image, uint32_t ucode_len) {
    ...
    EFUN(blackhawk7_l2p2_ucode_load_init(sa__, 1));
    ...
    while (ucode_len > 0) {
        EFUN(blackhawk7_l2p2_acc_wr_pram(sa__, (uint8_t)*ucode_image));
        --ucode_len;
        ++ucode_image;
    }
    ...
    while (ucode_len_remainder > 0) {
        EFUN(blackhawk7_l2p2_acc_wr_pram(sa__, 0));
        --ucode_len_remainder;
    }
    ...
    EFUN(blackhawk7_l2p2_ucode_load_close(sa__, 1));
    return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l2p2_tx_disable(srds_access_t *sa__, uint8_t enable) {
    ...
    if (enable) {
        ...
        {
            uint8_t output_sel = 0;
            ESTM(output_sel = rd_tx_disable_output_sel());
            if(output_sel != 0) {
                EFUN_PRINTF(("WARNING: tx_disable_output_sel == %d; this is not supported! This will be overridden to 0\n ",output_sel));
            }
            EFUN(wr_tx_disable_output_sel(0));
        ...
        BLACKHAWK7_L2P2_UAPI_ELSE_AMS_B0   /* else if BLACKHAWK7_V2LXP2 || BLACKHAWK7_V3LXP2 */
        EFUN(wr_ams_tx_test_data(0x0));
        BLACKHAWK7_L2P2_UAPI_ELSE_ERR
        EFUN(USR_DELAY_US(100));
...
err_code_t blackhawk7_l2p2_write_rx_afe(srds_access_t *sa__, enum srds_rx_afe_settings_enum param, int8_t val) {
  /* Assumes the micro is not actively tuning */
    ...
    switch(param) {
    case RX_AFE_PF:
        return(blackhawk7_l2p2_INTERNAL_set_rx_pf_main(sa__, (uint8_t)val));

    case RX_AFE_PF2:
        return(blackhawk7_l2p2_INTERNAL_set_rx_pf2(sa__, (uint8_t)val));

    case RX_AFE_VGA:
      return(blackhawk7_l2p2_INTERNAL_set_rx_vga(sa__, (uint8_t)val));


    case RX_AFE_DFE1:
        return(blackhawk7_l2p2_INTERNAL_set_rx_dfe1(sa__, val));

    case RX_AFE_DFE2:
        return(blackhawk7_l2p2_INTERNAL_set_rx_dfe2(sa__, val));

    case RX_AFE_DFE3:
        return(blackhawk7_l2p2_INTERNAL_set_rx_dfe3(sa__, val));

    case RX_AFE_DFE4:
        return(blackhawk7_l2p2_INTERNAL_set_rx_dfe4(sa__, val));

    case RX_AFE_DFE5:
        return(blackhawk7_l2p2_INTERNAL_set_rx_dfe5(sa__, val));

    case RX_AFE_DFE6:
        return (blackhawk7_l2p2_INTERNAL_set_rx_dfe6(sa__, val));

    case RX_AFE_DFE7:
        return (blackhawk7_l2p2_INTERNAL_set_rx_dfe7(sa__, val));

    case RX_AFE_DFE8:
        return (blackhawk7_l2p2_INTERNAL_set_rx_dfe8(sa__, val));

    case RX_AFE_DFE9:
        return (blackhawk7_l2p2_INTERNAL_set_rx_dfe9(sa__, val));

    case RX_AFE_DFE10:
        return (blackhawk7_l2p2_INTERNAL_set_rx_dfe10(sa__, val));

    case RX_AFE_DFE11:
        return (blackhawk7_l2p2_INTERNAL_set_rx_dfe11(sa__, val));

    case RX_AFE_DFE12:
        return (blackhawk7_l2p2_INTERNAL_set_rx_dfe12(sa__, val));

    case RX_AFE_DFE13:
        return (blackhawk7_l2p2_INTERNAL_set_rx_dfe13(sa__, val));

    case RX_AFE_DFE14:
        return (blackhawk7_l2p2_INTERNAL_set_rx_dfe14(sa__, val));

    case RX_AFE_PF3:
        return(blackhawk7_l2p2_INTERNAL_set_rx_pf3(sa__, (uint8_t)val));
...
err_code_t blackhawk7_l2p2_read_rx_afe(srds_access_t *sa__, enum srds_rx_afe_settings_enum param, int8_t *val) {
    ...
    switch(param) {
    case RX_AFE_PF:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_pf_main(sa__, &rd_val));
        *val = (int8_t)rd_val;
        break;
    case RX_AFE_PF2:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_pf2(sa__, &rd_val));
        *val = (int8_t)rd_val;
        break;
    case RX_AFE_VGA:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_vga(sa__, &rd_val));
        *val = (int8_t)rd_val;
      break;
    case RX_AFE_DFE1:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe1(sa__, val));
        break;
    case RX_AFE_DFE2:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe2(sa__, val));
        break;
    case RX_AFE_DFE3:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe3(sa__, val));
        break;
    case RX_AFE_DFE4:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe4(sa__, val));
        break;
    case RX_AFE_DFE5:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe5(sa__, val));
        break;
    case RX_AFE_DFE6:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe6(sa__, val));
        break;
    case RX_AFE_DFE7:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe7(sa__, val));
        break;
    case RX_AFE_DFE8:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe8(sa__, val));
        break;
    case RX_AFE_DFE9:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe9(sa__, val));
        break;
    case RX_AFE_DFE10:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe10(sa__, val));
        break;
    case RX_AFE_DFE11:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe11(sa__, val));
        break;
    case RX_AFE_DFE12:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe12(sa__, val));
        break;
    case RX_AFE_DFE13:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe13(sa__, val));
        break;
    case RX_AFE_DFE14:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_dfe14(sa__, val));
        break;
    case RX_AFE_PF3:
        EFUN(blackhawk7_l2p2_INTERNAL_get_rx_pf3(sa__, &rd_val));
        *val = (int8_t)rd_val;
        break;
    default:
        return(blackhawk7_l2p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
...
err_code_t blackhawk7_l2p2_validate_txfir_cfg(srds_access_t *sa__, enum blackhawk7_l2p2_txfir_tap_enable_enum enable_taps, int16_t pre2, int16_t pre1, int16_t main, int16_t post1, int16_t post2, int16_t post3) {
    ...
    ENULL_MEMSET(&txfir, 0, sizeof(blackhawk7_l2p2_txfir_st));
    ...
    if (main < SRDS_ABS(pre1) || main < pre2 || main < SRDS_ABS(post1) || main < post2 || main < post3) {
        EFUN_PRINTF(("ERROR: main TAP is expected to be greater than all other TAPs."));
        return(blackhawk7_l2p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    return blackhawk7_l2p2_INTERNAL_validate_full_txfir_cfg(sa__, enable_taps, &txfir);
}
...
static uint8_t _blackhawk7_l2p2_get_fir_tap_index(enum blackhawk7_l2p2_tx_afe_settings_enum param) {
    switch(param) {
    case BLACKHAWK7_L2P2_TX_AFE_TAP0:  return  0;
    case BLACKHAWK7_L2P2_TX_AFE_TAP1:  return  1;
    case BLACKHAWK7_L2P2_TX_AFE_TAP2:  return  2;
    case BLACKHAWK7_L2P2_TX_AFE_TAP3:  return  3;
    case BLACKHAWK7_L2P2_TX_AFE_TAP4:  return  4;
    case BLACKHAWK7_L2P2_TX_AFE_TAP5:  return  5;
...
err_code_t blackhawk7_l2p2_write_tx_afe(srds_access_t *sa__, enum blackhawk7_l2p2_tx_afe_settings_enum param, int16_t val) {
    ...
    uint8_t const tap_num = _blackhawk7_l2p2_get_fir_tap_index(param);
    ...
    if (((!in_nrz_range) && ((val < TXFIR_PAM4_SW_TAP_MIN) || (val > TXFIR_PAM4_SW_TAP_MAX))) ||
        ...
        return(blackhawk7_l2p2_error(sa__, ERR_CODE_TXFIR_MAIN_INVALID));
    }
    EFUN(blackhawk7_l2p2_INTERNAL_set_tx_tap(sa__, tap_num, val));
...
err_code_t blackhawk7_l2p2_read_tx_afe(srds_access_t *sa__, enum blackhawk7_l2p2_tx_afe_settings_enum param, int16_t *val) {
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_get_tx_tap(sa__, tap_num, val));
...
err_code_t blackhawk7_l2p2_apply_txfir_cfg(srds_access_t *sa__, enum blackhawk7_l2p2_txfir_tap_enable_enum enable_taps, int16_t pre2, int16_t pre1, int16_t main, int16_t post1, int16_t post2, int16_t post3) {
    ...
    blackhawk7_l2p2_txfir_st txfir;
#if !defined(SMALL_FOOTPRINT) 
    struct blackhawk7_l2p2_event_st api_event;
#endif

    EFUN(blackhawk7_l2p2_validate_txfir_cfg(sa__, enable_taps, pre2, pre1, main, post1, post2, post3));
    ...
    api_event.data2 = 0;
    EFUN(blackhawk7_l2p2_log_api_event(sa__, api_event));
#endif

    return blackhawk7_l2p2_INTERNAL_apply_full_txfir_cfg(sa__, enable_taps, &txfir);
}
...
err_code_t blackhawk7_l2p2_rmt_lpbk(srds_access_t *sa__, uint8_t enable) {
    INIT_SRDS_ERR_CODE
    if (enable) {
        EFUN(blackhawk7_l2p2_loop_timing(sa__, enable));
...
err_code_t blackhawk7_l2p2_core_reset(srds_access_t *sa__, ucode_info_t ucode_info) {
    INIT_SRDS_ERR_CODE
    uint8_t i, num_lanes;
    uint8_t orig_pll = blackhawk7_l2p2_acc_get_pll_idx(sa__);
    uint8_t orig_lane = blackhawk7_l2p2_acc_get_lane(sa__);
    EFUN(blackhawk7_l2p2_uc_reset(sa__, 1, ucode_info));   /* Assert uC reset */
    ...
    for(i = 0; i < num_lanes; i++) {
        EFUN(blackhawk7_l2p2_acc_set_lane(sa__, i));
        EFUN(blackhawk7_l2p2_ln_dp_reset(sa__, 1));
    }
    EFUN(USR_DELAY_US(1));
    for (i = 0; i < NUM_PLLS; i++) {
        EFUN(blackhawk7_l2p2_acc_set_pll_idx(sa__, i));
    ...
    EFUN(blackhawk7_l2p2_acc_set_lane(sa__, orig_lane));
    EFUN(blackhawk7_l2p2_acc_set_pll_idx(sa__, orig_pll));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l2p2_uc_reset_with_info(srds_access_t *sa__, uint8_t enable, ucode_info_t ucode_info) {
    return blackhawk7_l2p2_uc_reset(sa__, enable, ucode_info);
}
...
err_code_t blackhawk7_l2p2_uc_reset(srds_access_t *sa__, uint8_t enable, ucode_info_t ucode_info) {
  ...
  if (enable) {
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_poll_micro_ra_initdone(sa__, 5));/* Poll status of data RAM initialization */
    EFUN(wrc_micro_ra_init(0x0));                         /* Clear command for data RAM initialization */
    {
        ...
        for (micro_idx = (int8_t)(num_micros-1); micro_idx >= 0; micro_idx--) {
            EFUN(blackhawk7_l2p2_acc_set_micro_idx(sa__, (uint8_t)micro_idx));
...
err_code_t blackhawk7_l2p2_wait_uc_active(srds_access_t *sa__) {
    ...
    return (blackhawk7_l2p2_error(sa__, ERR_CODE_MICRO_INIT_NOT_DONE));
}

static err_code_t _blackhawk7_l2p2_get_silicon_version(srds_access_t *sa__, uint8_t *silicon_version);
static err_code_t _blackhawk7_l2p2_get_silicon_version(srds_access_t *sa__, uint8_t *silicon_version) {
    INIT_SRDS_ERR_CODE
    uint8_t afe_version;
    uint8_t revid2;
    ...
    *silicon_version = GET_UIP_VERSION(afe_version, revid2);
    return ERR_CODE_NONE;
}

static err_code_t _blackhawk7_l2p2_ucode_core_ip_check(srds_access_t *sa__);
err_code_t _blackhawk7_l2p2_ucode_core_ip_check(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint8_t status_byte;
...
err_code_t blackhawk7_l2p2_init_blackhawk7_l2p2_info(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    srds_info_t * const blackhawk7_l2p2_info_ptr = blackhawk7_l2p2_INTERNAL_get_blackhawk7_l2p2_info_ptr(sa__); /* Never put a check here */
    ...
    if (blackhawk7_l2p2_info_ptr == NULL) {
        ...
        RELEASE_LOCK_AND_RETURN (blackhawk7_l2p2_error(sa__, ERR_CODE_INFO_TABLE_ERROR));
    }
#ifdef PHYMOD_SUPPORT
    if (PHYMOD_ACC_F_PHYSIM_GET(sa__)) {
        blackhawk7_l2p2_info_ptr->signature = SRDS_INFO_SIGNATURE;
        blackhawk7_l2p2_info_ptr->diag_mem_ram_base = 0x200013A8;
        blackhawk7_l2p2_info_ptr->diag_mem_ram_size = 0x100;
        blackhawk7_l2p2_info_ptr->core_var_ram_base = 0x20000080;
        blackhawk7_l2p2_info_ptr->core_var_ram_size = 0x40;
        blackhawk7_l2p2_info_ptr->lane_var_ram_base = 0x200003B4;
        blackhawk7_l2p2_info_ptr->lane_var_ram_size = 0x600;
        blackhawk7_l2p2_info_ptr->trace_mem_ram_base = 0x20000100;
        blackhawk7_l2p2_info_ptr->trace_mem_ram_size = 0x200;
        blackhawk7_l2p2_info_ptr->micro_var_ram_base = 0x20000300;
        blackhawk7_l2p2_info_ptr->lane_count = 0x2;
        blackhawk7_l2p2_info_ptr->trace_memory_descending_writes = 0x1;
        blackhawk7_l2p2_info_ptr->micro_count = 0x4;
        blackhawk7_l2p2_info_ptr->micro_var_ram_size = 0x10;
        blackhawk7_l2p2_info_ptr->grp_ram_size = 0x27C8;
        blackhawk7_l2p2_info_ptr->ucode_version = 0x0;
        blackhawk7_l2p2_info_ptr->silicon_version = GET_AMS_VERSION(rdc_afe_ipversion_id());
    } else {
    ...
    if (blackhawk7_l2p2_info_ptr->signature == SRDS_INFO_SIGNATURE) {
        ...
        if (err_code == ERR_CODE_NONE) {
            ...
            RELEASE_LOCK_AND_RETURN (blackhawk7_l2p2_error(sa__, ERR_CODE_MICRO_INIT_NOT_DONE));
        }
    }
    /*Never initialized so far, continue with initialization */

    err_code = blackhawk7_l2p2_rdblk_uc_prog_ram(sa__, (uint8_t *)info_table, INFO_TABLE_RAM_BASE, INFO_TABLE_END);
    /* Release lock before returning from the call to avoid deadlock */
    if (ERR_CODE_NONE != err_code) {
       RELEASE_LOCK_AND_RETURN(blackhawk7_l2p2_error(sa__, err_code));
    }
    ...
    blackhawk7_l2p2_info_ptr->lane_count = info_table[INFO_TABLE_OFFS_OTHER_SIZE / sizeof(uint32_t)] & 0xFF;
    blackhawk7_l2p2_info_ptr->trace_memory_descending_writes = ((info_table[INFO_TABLE_OFFS_OTHER_SIZE / sizeof(uint32_t)] & 0x1000000) != 0);

    blackhawk7_l2p2_info_ptr->core_var_ram_size = (info_table[INFO_TABLE_OFFS_OTHER_SIZE / sizeof(uint32_t)] >> 8) & 0xFF;

    blackhawk7_l2p2_info_ptr->lane_var_ram_size = (info_table[INFO_TABLE_OFFS_TRACE_LANE_MEM_SIZE / sizeof(uint32_t)] >> 16) & 0xFFFF;

    blackhawk7_l2p2_info_ptr->diag_mem_ram_size = (info_table[INFO_TABLE_OFFS_DIAG_LANE_MEM_SIZE / sizeof(uint32_t)] & 0xFFFF);
    blackhawk7_l2p2_info_ptr->diag_mem_ram_base = info_table[INFO_TABLE_OFFS_DIAG_MEM_PTR / sizeof(uint32_t)];

    blackhawk7_l2p2_info_ptr->trace_mem_ram_size = (info_table[INFO_TABLE_OFFS_TRACE_LANE_MEM_SIZE / sizeof(uint32_t)] & 0xFFFF);
    blackhawk7_l2p2_info_ptr->trace_mem_ram_base = info_table[INFO_TABLE_OFFS_TRACE_MEM_PTR / sizeof(uint32_t)];

    blackhawk7_l2p2_info_ptr->core_var_ram_base = info_table[INFO_TABLE_OFFS_CORE_MEM_PTR / sizeof(uint32_t)];

    blackhawk7_l2p2_info_ptr->micro_var_ram_base = info_table[INFO_TABLE_OFFS_MICRO_MEM_PTR / sizeof(uint32_t)];
    blackhawk7_l2p2_info_ptr->micro_var_ram_size = (info_table[INFO_TABLE_OFFS_OTHER_SIZE_2 / sizeof(uint32_t)] >> 4) & 0xFF;

    blackhawk7_l2p2_info_ptr->lane_var_ram_base = info_table[INFO_TABLE_OFFS_LANE_MEM_PTR / sizeof(uint32_t)];


    blackhawk7_l2p2_info_ptr->micro_count = info_table[INFO_TABLE_OFFS_OTHER_SIZE_2 / sizeof(uint32_t)] & 0xF;
    blackhawk7_l2p2_info_ptr->grp_ram_size = info_table[INFO_TABLE_OFFS_GROUP_RAM_SIZE / sizeof(uint32_t)] & 0xFFFF;
    blackhawk7_l2p2_info_ptr->ucode_version = info_table[INFO_TABLE_OFFS_UC_VERSION /sizeof(uint32_t)];
    blackhawk7_l2p2_info_ptr->info_table_version = info_table_version;
    blackhawk7_l2p2_info_ptr->is_big_endian = blackhawk7_l2p2_INTERNAL_is_big_endian();
    err_code = _blackhawk7_l2p2_get_silicon_version(sa__, &blackhawk7_l2p2_info_ptr->silicon_version);
    /* Release lock before returning from the call to avoid deadlock */
    if (ERR_CODE_NONE != err_code) {
        RELEASE_LOCK_AND_RETURN (blackhawk7_l2p2_error(sa__, err_code));
    }
    ...
    err_code = _blackhawk7_l2p2_ucode_core_ip_check(sa__);
    if (ERR_CODE_NONE != err_code) {
        RELEASE_LOCK_AND_RETURN (blackhawk7_l2p2_error(sa__, err_code));
    }
#ifdef PHYMOD_SUPPORT
...
err_code_t blackhawk7_l2p2_clear_blackhawk7_l2p2_info(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    srds_info_t * const blackhawk7_l2p2_info_ptr = blackhawk7_l2p2_INTERNAL_get_blackhawk7_l2p2_info_ptr(sa__); /* Never put a check here */
    ...
    ENULL_MEMSET(blackhawk7_l2p2_info_ptr, 0, sizeof(srds_info_t));
...
err_code_t blackhawk7_l2p2_configure_pll_powerdown(srds_access_t *sa__) {
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, BLACKHAWK7_L2P2_PLL_REFCLK_UNKNOWN, BLACKHAWK7_L2P2_PLL_DIV_UNKNOWN, 0, BLACKHAWK7_L2P2_PLL_OPTION_POWERDOWN);
}
...
err_code_t blackhawk7_l2p2_configure_pll_refclk_div(srds_access_t *sa__,
    ...
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, refclk, srds_div, 0, BLACKHAWK7_L2P2_PLL_OPTION_NONE);
}

err_code_t blackhawk7_l2p2_configure_pll_refclk_vco(srds_access_t *sa__,
                                           enum blackhawk7_l2p2_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, refclk, BLACKHAWK7_L2P2_PLL_DIV_UNKNOWN, vco_freq_khz, BLACKHAWK7_L2P2_PLL_OPTION_NONE);
}

err_code_t blackhawk7_l2p2_configure_pll_div_vco(srds_access_t *sa__,
                                        enum blackhawk7_l2p2_pll_div_enum srds_div,
                                        uint32_t vco_freq_khz) {
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, BLACKHAWK7_L2P2_PLL_REFCLK_UNKNOWN, srds_div, vco_freq_khz, BLACKHAWK7_L2P2_PLL_OPTION_NONE);
}

/* following routines will set the VCO rate to 20.625 GHz */
err_code_t blackhawk7_l2p2_configure_20P625G_pll_refclk(srds_access_t *sa__,
                                        enum blackhawk7_l2p2_pll_refclk_enum refclk) {
    return blackhawk7_l2p2_INTERNAL_configure_20P625_pll(sa__, refclk, BLACKHAWK7_L2P2_PLL_DIV_UNKNOWN, BLACKHAWK7_L2P2_PLL_OPTION_NONE);
}

err_code_t blackhawk7_l2p2_configure_20P625G_pll_div(srds_access_t *sa__,
                                        enum blackhawk7_l2p2_pll_div_enum srds_div) {
    return blackhawk7_l2p2_INTERNAL_configure_20P625_pll(sa__, BLACKHAWK7_L2P2_PLL_REFCLK_UNKNOWN, srds_div, BLACKHAWK7_L2P2_PLL_OPTION_NONE);
}
...
err_code_t blackhawk7_l2p2_configure_pll_refclk_div_div2refclk(srds_access_t *sa__,
                                           enum blackhawk7_l2p2_pll_refclk_enum refclk,
                                           enum blackhawk7_l2p2_pll_div_enum srds_div) {
   return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, refclk, srds_div, 0, BLACKHAWK7_L2P2_PLL_OPTION_REFCLK_DIV2_EN);
}

err_code_t blackhawk7_l2p2_configure_pll_refclk_vco_div2refclk(srds_access_t *sa__,
                                           enum blackhawk7_l2p2_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, refclk, BLACKHAWK7_L2P2_PLL_DIV_UNKNOWN, vco_freq_khz, BLACKHAWK7_L2P2_PLL_OPTION_REFCLK_DIV2_EN);
}

err_code_t blackhawk7_l2p2_configure_pll_div_vco_div2refclk(srds_access_t *sa__,
                                        enum blackhawk7_l2p2_pll_div_enum srds_div,
                                        uint32_t vco_freq_khz) {
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, BLACKHAWK7_L2P2_PLL_REFCLK_UNKNOWN, srds_div, vco_freq_khz, BLACKHAWK7_L2P2_PLL_OPTION_REFCLK_DIV2_EN);
}
/* following routines divide refclk input by 2 to prevent fracn mode */
err_code_t blackhawk7_l2p2_configure_pll_refclk_div_div4refclk(srds_access_t *sa__,
                                           enum blackhawk7_l2p2_pll_refclk_enum refclk,
                                           enum blackhawk7_l2p2_pll_div_enum srds_div) {
   return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, refclk, srds_div, 0, BLACKHAWK7_L2P2_PLL_OPTION_REFCLK_DIV4_EN);
}

err_code_t blackhawk7_l2p2_configure_pll_refclk_vco_div4refclk(srds_access_t *sa__,
                                           enum blackhawk7_l2p2_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, refclk, BLACKHAWK7_L2P2_PLL_DIV_UNKNOWN, vco_freq_khz, BLACKHAWK7_L2P2_PLL_OPTION_REFCLK_DIV4_EN);
}

err_code_t blackhawk7_l2p2_configure_pll_div_vco_div4refclk(srds_access_t *sa__,
                                        enum blackhawk7_l2p2_pll_div_enum srds_div,
                                        uint32_t vco_freq_khz) {
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, BLACKHAWK7_L2P2_PLL_REFCLK_UNKNOWN, srds_div, vco_freq_khz, BLACKHAWK7_L2P2_PLL_OPTION_REFCLK_DIV4_EN);
}

err_code_t blackhawk7_l2p2_configure_pll_refclk_div_refclk_2x(srds_access_t *sa__,
                                           enum blackhawk7_l2p2_pll_refclk_enum refclk,
                                           enum blackhawk7_l2p2_pll_div_enum srds_div) {
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, refclk, srds_div, 0, BLACKHAWK7_L2P2_PLL_OPTION_REFCLK_DOUBLER_EN);
}

err_code_t blackhawk7_l2p2_configure_pll_refclk_vco_refclk_2x(srds_access_t *sa__,
                                           enum blackhawk7_l2p2_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, refclk, BLACKHAWK7_L2P2_PLL_DIV_UNKNOWN, vco_freq_khz, BLACKHAWK7_L2P2_PLL_OPTION_REFCLK_DOUBLER_EN);
}

err_code_t blackhawk7_l2p2_configure_pll_div_vco_refclk_2x(srds_access_t *sa__,
                                        enum blackhawk7_l2p2_pll_div_enum srds_div,
                                        uint32_t vco_freq_khz) {
    return blackhawk7_l2p2_INTERNAL_configure_pll(sa__, BLACKHAWK7_L2P2_PLL_REFCLK_UNKNOWN, srds_div, vco_freq_khz, BLACKHAWK7_L2P2_PLL_OPTION_REFCLK_DOUBLER_EN);
}
...
err_code_t blackhawk7_l2p2_ucode_load_init(srds_access_t *sa__, uint8_t pram) {
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_poll_micro_ra_initdone(sa__, 5));  /* Poll for micro_ra_initdone = 1 to indicate initialization done */
...
err_code_t blackhawk7_l2p2_ucode_load(srds_access_t *sa__, uint8_t *ucode_image, uint32_t ucode_len) {
    ...
    EFUN(blackhawk7_l2p2_ucode_load_init(sa__, 0));
    EFUN(blackhawk7_l2p2_ucode_load_write(sa__, ucode_image, ucode_len));
    EFUN(blackhawk7_l2p2_ucode_load_close(sa__, 0));
...
err_code_t blackhawk7_l2p2_ucode_load_verify(srds_access_t *sa__, uint8_t *ucode_image, uint32_t ucode_len) {
    ...
    do {                                                  /* ucode_image read 16bits at a time */
        ...
        if (data_lsw != rddata_lsw) {
            ...
            return (blackhawk7_l2p2_error(sa__, ERR_CODE_UCODE_VERIFY_FAIL));             /* Verify uCode FAIL */
        }
...
err_code_t blackhawk7_l2p2_tx_clock_align_enable(srds_access_t *sa__, uint8_t enable, uint8_t lane_mask) {
    ...
    ESTM(current_lane = blackhawk7_l2p2_acc_get_lane(sa__));
    ...
    if (enable)
    {
        ...
        EFUN(blackhawk7_l2p2_acc_set_lane(sa__,current_lane)); /* return lane to previous state */
...
/*  Fast Link Recovery  */
/************************/
err_code_t blackhawk7_l2p2_configure_fast_link_recovery(srds_access_t *sa__, uint8_t enable, uint8_t enable_timeout, enum blackhawk7_l2p2_flr_timeout_enum timeout_time) {
    INIT_SRDS_ERR_CODE
    ...
    ESTM(reset_state = rd_rx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk7_l2p2_configure_fast_link_recovery(..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n",blackhawk7_l2p2_acc_get_lane(sa__),reset_state));
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
    EFUN(blackhawk7_l2p2_INTERNAL_is_flr_supported(sa__, &flr_support));
    if(!flr_support) {
        EFUN_PRINTF(("ERROR: blackhawk7_l2p2_configure_fast_link_recovery(..) unsupported with currently loaded microcode (Core=%d Lane=%d).\n", blackhawk7_l2p2_acc_get_core(sa__), blackhawk7_l2p2_acc_get_lane(sa__)));
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }

    if (timeout_time_val > 5) {
        ESTM_PRINTF(("timeout_time value for blackhawk7_l2p2_configure_fast_link_recovery() is too large:  %d\n", timeout_time_val));
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    {
...
err_code_t blackhawk7_l2p2_get_configure_fast_link_recovery(srds_access_t *sa__, uint8_t *enable, uint8_t *enable_timeout, enum blackhawk7_l2p2_flr_timeout_enum *timeout_time) {
    INIT_SRDS_ERR_CODE
    uint8_t flr_byte = 0;
    uint8_t flr_support = 0;
    ...
    EFUN(blackhawk7_l2p2_INTERNAL_is_flr_supported(sa__, &flr_support));
    if(!flr_support) {
        EFUN_PRINTF(("ERROR: blackhawk7_l2p2_get_configure_fast_link_recovery(..) unsupported with currently loaded microcode (Core=%d Lane=%d).\n", blackhawk7_l2p2_acc_get_core(sa__), blackhawk7_l2p2_acc_get_lane(sa__)));
        return (blackhawk7_l2p2_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    ...
    *timeout_time = (enum blackhawk7_l2p2_flr_timeout_enum)((flr_byte >> 2) & 0x7);
    return (ERR_CODE_NONE);
}
