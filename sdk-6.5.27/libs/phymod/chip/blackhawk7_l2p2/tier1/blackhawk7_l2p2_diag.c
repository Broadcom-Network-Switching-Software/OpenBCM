...
static void _blackhawk7_l2p2_display_eye_scan_ignored_lane(srds_access_t *sa__, uint16_t usr_diag_status) {
        ...
        for(lane=0; lane < num_lanes; lane++) {
            if(lane_mask & (1<<lane)) {
                ...
                if(i[lane] >= -i_max[lane]) {
                    ...
                    if (local_error) {
                        lane_mask &= (uint8_t)~(1<<lane);
                        _blackhawk7_l2p2_display_eye_scan_ignored_lane(sa__, usr_diag_status[lane]);
                        continue;
                    }
                    status = 0;
                    if(eye_slice[lane]) {
...
err_code_t blackhawk7_l2p2_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    ...
    arg.buffer_ptr = buffer;
        ...
        if(!(blackhawk7_l2p2_info_ptr->lane_count) || (!(blackhawk7_l2p2_info_ptr->diag_mem_ram_size))) {
...
err_code_t blackhawk7_l2p2_display_eye_scan_header(srds_access_t *sa__, uint8_t i, uint16_t *usr_diag_status) {
    ...
    EFUN_PRINTF(("\n"));
    for(x=1;x<=i;x++) {
        _blackhawk7_l2p2_display_eye_scan_ignored_lane(sa__, usr_diag_status[x - 1]);
    }
    EFUN_PRINTF(("\n"));
    return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l2p2_display_all_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    for(lane = 0; lane < num_lanes; lane++) {
        CFUN(blackhawk7_l2p2_acc_set_lane(sa__, lane));
        /* start UC acquisition */
        CFUN(blackhawk7_l2p2_INTERNAL_eye_margin_proj_start(sa__, ber_scan_mode, timer_control, max_error_control, &offset_start[lane]));
    }
    EFUN_PRINTF(("Waiting for measurement time approx %d seconds",timer_control+(timer_control>>1)));
    for(lane = 0; lane < num_lanes; lane++) {
        CFUN(blackhawk7_l2p2_acc_set_lane(sa__, lane));
        ...
        CFUN(blackhawk7_l2p2_INTERNAL_eye_margin_proj_end(sa__, rate, ber_scan_mode, timer_control, offset_start[lane]));
    }

    CFUN(blackhawk7_l2p2_acc_set_lane(sa__, lane_orig));
...
err_code_t blackhawk7_l2p2_display_ber_scan_data (srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *measurement_duration, uint8_t max_offset) {
    ...
    const int32_t ARTIFICIAL_MARGIN_V = 280;    /* Used along ARTIFICIAL_BER when not enough points to extrapolate. Unit: mV. Based on the concept of max Vpp of 1 Volt */
...
err_code_t blackhawk7_l2p2_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    num_lanes = 8; /* to avoid overwritten the code changes in blackhawk7_l2p2_display_diag_data_core */
    if (lane < num_lanes)
    {
