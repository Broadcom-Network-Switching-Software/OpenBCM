...
#ifndef BLACKHAWK7_L2P2_API_DIAG_H
#define BLACKHAWK7_L2P2_API_DIAG_H
...
struct blackhawk7_l2p2_eyescan_options_st {
...
/** Column definition header for blackhawk7_l2p2_display_core_state() API output.
 * To be called before blackhawk7_l2p2_display_core_state_line() API.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 ...
 * Call blackhawk7_l2p2_display_core_state_hdr() API before and blackhawk7_l2p2_display_core_state_legend() after
...
/** Detailed description of each column in blackhawk7_l2p2_display_core_state_line() API output.
 * To be called after blackhawk7_l2p2_display_core_state_line() API to display the legends.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors)
 */
err_code_t blackhawk7_l2p2_display_core_state_legend(srds_access_t *sa__);

/** Display lane state for all lanes.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @return Error Code, if generated (returns ERR_CODE_NONE if no errors).
 */
...
/** Log API Event Struct
 *
 */
struct blackhawk7_l2p2_event_st {
    uint8_t event_id; /*!< ID of the event */
    uint8_t data1;    /*!< First byte of supplemental data if required */
    uint8_t data2;    /*!< Second byte of supplemental data if required */
};
 ...
 * @param[in] api_event Contains event id and optional data for logging.
 *
 * @b Example
 * @code
        srds_access_t *sa__ ;
        struct blackhawk7_l2p2_event_st event;
        event.event_id = 2; event.data1 = 5; event.data2 = 6;
        blackhawk7_l2p2_log_api_event(sa__, event);
...
#define DIAG_EYE_ILB_MODE_EN        (32)
 ...
 * This function only initiates the processor actions. User must use blackhawk7_l2p2_read_eye_scan_stripe() function
 ...
 * @param[in] usr_diag_status diagnostic statuses returned by microcode.
 * @return Error Code during data collection (returns ERR_CODE_NONE if no errors).
 */
 ...
 * @param[in] usr_diag_status is an array (indexed for each lane) of diagnostic statuses returned by microcode.
 * @return Error Code during data collection (returns ERR_CODE_NONE if no errors).
 */
 ...
 * @param[in] measurement_duration is pointer to 32 element array of uint32 containing the time data.
 * @param[in] max_offset is the maximum offset setting which is present in data (usually 31).
 * @return Error Code during data collection (returns ERR_CODE_NONE if no errors).
 */
err_code_t blackhawk7_l2p2_display_ber_scan_data (srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *measurement_duration, uint8_t max_offset);

/** Example eye margin projection API which runs on all lanes.
 * This is an example function which uses the following API's to measure and display BER margin projections.
 * blackhawk7_l2p2_start_ber_scan_test(), blackhawk7_l2p2_read_ber_scan_data(), blackhawk7_l2p2_display_ber_scan_data().
 *
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] rate specifies the data rate in Hz.
 * @param[in] ber_scan_mode the type of test used to take the data(use #srds_diag_ber_mode_enum).
 ...
 * \n Note: Call functions blackhawk7_l2p2_display_lane_state_hdr(sa__) before and blackhawk7_l2p2_display_lane_state_legend() after
...
/** Column definition header for blackhawk7_l2p2 display state.
 * To be called before blackhawk7_l2p2_display_lane_state() function.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
err_code_t blackhawk7_l2p2_display_lane_state_hdr(srds_access_t *sa__);

/** Detailed explanation of each column in blackhawk7_l2p2 display state.
 * To be called after blackhawk7_l2p2_display_lane_state() function to display the legends.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
...
/** Parameterized diagnostic function which provides comprehensive diagnostic and debug information for all lanes.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 ...
 * \verbatim
  bit  0 - 1 = display extended lane state.
 ...
 * This function is required to be implemented by upper level software to enable blackhawk7_l2p2 support.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 * @param[in] diag_level is a MASK style parameter to enable variable level of display.
 *     enum srds_diag_level_enum can be used to help set the diag_level. \n
 * \verbatim
 ...
 * \verbatim
  ...
  bit 10 - display data for ber projection horzontal.
  bit 11 - event log (safe), which does not involve micro for read.  \endverbatim
 * @param[in] lane_select is a MASK of lanes selected to log the register and ram data
 * \verbatim
  bit  0 - lane 0
  bit  1 - lane 1
  bit  2 - lane 2
  ...
  bit  5 - lane 5
  bit  6 - lane 6
  bit  7 - lane 7  \endverbatim
 *
 * @return Error Code generated by API (returns ERR_CODE_NONE if no errors).
 */
err_code_t blackhawk7_l2p2_display_diag_data_core(srds_access_t *sa__, uint16_t diag_level, uint8_t lane_select);

/** Parameterized diagnostic function which provides access to all blackhawk7_l2p2 control and status bits
 * This function is required to be implemented by upper level software to enable blackhawk7_l2p2 support.
 * @param[in] sa__ is an opaque state vector passed through to device access functions.
 ...
 * \verbatim
 ...
 type = 17 - Enable Breakpoint (addr becomes breakpoint #).
               Note: This option should only be used under the direction of engineering.
