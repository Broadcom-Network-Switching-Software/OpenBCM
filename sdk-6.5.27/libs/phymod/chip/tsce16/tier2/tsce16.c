...
int tsce16_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (temod16_port_start_lane_get(&phy->access,
                                     &port_starting_lane,
                                     &port_num_lane));
...
int tsce16_phy_rx_set(const phymod_phy_access_t* phy, const phymod_rx_t* rx)
{
    uint32_t i;
    phymod_phy_access_t phy_copy;
    ...
    /*params check*/
    if((rx->num_of_dfe_taps == 0) || (rx->num_of_dfe_taps < TSCE16_NOF_DFES)){
...
int tsce16_phy_rx_get(const phymod_phy_access_t* phy, phymod_rx_t* rx)
{
    int8_t val;
...
/* Take care of HiGig settings for HiGig Speeds, currently just fixed for 5000_XFI and 12.773G */
int tsce16_phy_interface_config_set(const phymod_phy_access_t* phy, uint32_t flags, const phymod_phy_inf_config_t* config)
{
    ...
    uint16_t scramble_or_val = TEMOD16_SCRAMBLER_64B66B;
    ...
    case 5000:
        if(config->pll_divider_req==80) {
            ...
            scramble_or_val = TEMOD16_SCRAMBLER_8B10B;
        } else {
    ...
    case 20000:
        if(config->interface_type == phymodInterfaceKR2 ||
           config->interface_type == phymodInterfaceCR2) {
            ...
            /* For 20G 4-lane port, scrambler depends on bit of PHYMOD_INTF_MODES_SCR
             * Clear PHYMOD_INTF_F_SCRAMBLE_FORCED_ON and PHYMOD_INTF_F_SCRAMBLE_FORCED_OFF in flags
             */
            flags &= ~(PHYMOD_INTF_F_SCRAMBLE_FORCED_ON | PHYMOD_INTF_F_SCRAMBLE_FORCED_OFF);
        }
        break;
    case 21000:
    ...
    if(PHYMOD_INTF_F_SCRAMBLE_FORCED_ON & flags) {
        if (config->data_rate / num_lane < 5000) {
        ...
        PHYMOD_IF_ERR_RETURN
            (temod16_override_set(&phy->access, TEMOD16_OVERRIDE_SCR_MODE , scramble_or_val));
        PHYMOD_IF_ERR_RETURN
            (temod16_override_set(&phy->access, TEMOD16_OVERRIDE_DESCR_MODE, scramble_or_val));
    } else if (PHYMOD_INTF_F_SCRAMBLE_FORCED_OFF & flags) {
        firmware_lane_config.ScramblingDisable = 1 ;

        scramble_or_val = TEMOD16_SCRAMBLER_BYPASS;
        PHYMOD_IF_ERR_RETURN
            (temod16_override_set(&phy->access, TEMOD16_OVERRIDE_SCR_MODE , scramble_or_val));
        PHYMOD_IF_ERR_RETURN
            (temod16_override_set(&phy->access, TEMOD16_OVERRIDE_DESCR_MODE, scramble_or_val));
                                        ...
                                        phymod_firmware_lane_config_t *lane_config)
{
    ...
    current_pll_div = (uint32_t) pll_mode;
    config->pll_divider_req = temod16_pll_div_get(current_pll_div);
...
int tsce16_phy_interface_config_get(const phymod_phy_access_t* phy, uint32_t flags, phymod_ref_clk_t ref_clock, phymod_phy_inf_config_t* config)
{
    int speed_id;
    phymod_firmware_lane_config_t firmware_lane_config;
    temod16_scrambler_type_t scramble_mode;
    phymod_phy_access_t pm_phy_copy;
    int start_lane, num_lane;
    ...
    PHYMOD_IF_ERR_RETURN(temod16_scramble_mode_get(&pm_phy_copy.access, &scramble_mode));
    if (scramble_mode == TEMOD16_SCRAMBLER_BYPASS) {
        PHYMOD_INTF_MODES_SCR_CLR(config);
    } else {
...
int tsce16_core_lane_map_get(const phymod_core_access_t* core, phymod_lane_map_t* lane_map)
{
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(core->device_op_mode)) {
        PHYMOD_IF_ERR_RETURN(temod16_pcs_lane_swap_get(&core->access, &pcs_swap));
    }

    for (lane = 0; lane < TSCE16_NOF_LANES_IN_CORE; lane++) {
        lane_map->lane_map_tx[lane] = (pcs_swap >> (lane * 4)) & 0x3;
        lane_map->lane_map_rx[lane] = (pcs_swap >> ((lane * 4) + 16)) & 0x3;
    }
    lane_map->num_of_lanes = TSCE16_NOF_LANES_IN_CORE;
...
int tsce16_phy_power_set(const phymod_phy_access_t* phy, const phymod_phy_power_t* power)
{
    ...
    PHYMOD_MEMSET(&an_control, 0x0,  sizeof(temod16_an_control_t));
    PHYMOD_IF_ERR_RETURN
        (temod16_autoneg_control_get(&phy->access, &an_control, &an_complete));
    ...
    for (i = 0; i < num_lane; i++) {
         ...
         if ((power->tx == phymodPowerOff) && (power->rx == phymodPowerNoChange)) {
             mode = PWRDN_TX;
             PHYMOD_IF_ERR_RETURN(merlin16_lane_pwrdn(&pm_phy_copy.access, mode));
         }

         /*enable tx on the PMD side */
         if ((power->tx == phymodPowerOn) && (power->rx == phymodPowerNoChange)) {
             PHYMOD_IF_ERR_RETURN(merlin16_pwrdn_set(&pm_phy_copy.access, 1, 0));
         }
         ...
         if ((power->tx == phymodPowerNoChange) && (power->rx == phymodPowerOn)) {
             PHYMOD_IF_ERR_RETURN(merlin16_pwrdn_set(&pm_phy_copy.access, 0, 0));
         }
         ...
         if ((power->tx == phymodPowerOn) && (power->rx == phymodPowerOn)) {
             if (an_control.enable) {
                 ...
                 if (0 == i) {
                     an_control.enable = 0;
                     PHYMOD_IF_ERR_RETURN
                         (temod16_autoneg_control(&pm_phy_copy.access, &an_control));
                     an_control.enable = 1;
                 }
             } else {
                 /* pcs disable */
             ...
             if ((num_lane - 1) == i) {
                 ...
                 if (an_control.enable) {
                     ...
                     /* pcs enable */
                     PHYMOD_IF_ERR_RETURN
                         (temod16_enable_set(&pm_phy_copy.access));
                 }
                 pm_phy_copy.access.lane_mask = 1 << (start_lane + i);
             }
         }
