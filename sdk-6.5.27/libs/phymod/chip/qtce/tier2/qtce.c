...
int _qtce_core_lane_override_set(const phymod_core_access_t* core)
{
   ...
   for(lane = 0 ; lane < QTCE_NOF_LANES_IN_CORE ; lane++){
       ...
       PHYMOD_IF_ERR_RETURN
           (qmod_pmd_lane_override(&phy_copy.access, QMOD_PMD_LANE_SIGNAL_DETECT_OVRD, 1, 1));
       PHYMOD_IF_ERR_RETURN
           (qmod_pmd_lane_override(&phy_copy.access, QMOD_PMD_LANE_RX_LOCK_OVRD, 1, 1));
   }
...
{  
    ...
    if(config->data_rate==10||config->data_rate==100|| config->data_rate==1000 || config->data_rate==2500) {
...
int qtce_core_init(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    /* apply the override only when there is the lane swap.*/
    PHYMOD_IF_ERR_RETURN
        (qmod_pcs_lane_swap_get(&core->access, &pcs_lane_swap));
    if (pcs_lane_swap != 0x3210) {
        PHYMOD_IF_ERR_RETURN
...
int qtce_phy_pmd_override_enable_set(const phymod_phy_access_t* phy,
                                     phymod_override_type_t pmd_override_type,
                                     uint32_t override_enable,
                                     uint32_t override_val)
{
    phymod_phy_access_t phy_copy;
    qmod_pmd_lane_override_t ovrd_type;
    int lane_id, sub_port;
    ...
    switch (pmd_override_type) {
        case phymodPMDRxLock:
            ovrd_type = QMOD_PMD_LANE_RX_LOCK_OVRD;
            break;
        case phymodPMDSignalDetect:
            ovrd_type = QMOD_PMD_LANE_SIGNAL_DETECT_OVRD;
            break;
        default:
            return PHYMOD_E_UNAVAIL;
    }
    ...
    PHYMOD_IF_ERR_RETURN
        (qmod_pmd_lane_override(&phy_copy.access, ovrd_type, override_enable, override_val));
