                      ...
                      uint32_t                  *an_done)
{
    ...
    READ_DIG_CTL1000X1r(pc, &x1reg);

    an->enable = MIICTLr_AUTONEG_ENABLEf_GET(miictl);
    if (an->enable) {
        an->an_mode = DIG_CTL1000X1r_FIBER_MODE_1000Xf_GET(x1reg) ? phymod_AN_MODE_CL37 : phymod_AN_MODE_SGMII;
    } else {
...
static int _xgxs10g_phy_enable_set(const PHYMOD_ST *pa, const phymod_phy_power_t* power, int combo)
{
    LANECTL3r_t ctl3;
    DIG_MISC3r_t misc3;
    LANERESETr_t rst;
    ...
    if ((power->tx == phymodPowerOff) && (power->rx == phymodPowerOff)) {
        ...
        if (combo){
            BLK0_MISCCTL1r_GLOBAL_PMD_TX_DISABLEf_SET(mctrl, 1);
            PHYMOD_IF_ERR_RETURN(WRITE_BLK0_MISCCTL1r(&pa_copy, mctrl));

            RX_AFE_ANARXCTLr_OVERRIDE_SIGDET_ENf_SET(rxctrl, 1);
    ...
    if ((power->tx == phymodPowerOn) && (power->rx == phymodPowerOn)) {
        ...
        PHYMOD_IF_ERR_RETURN(WRITE_DIG_MISC3r(pa, misc3));
        if (combo){
            BLK0_MISCCTL1r_GLOBAL_PMD_TX_DISABLEf_SET(mctrl, 0);
            PHYMOD_IF_ERR_RETURN(WRITE_BLK0_MISCCTL1r(&pa_copy, mctrl));

            RX_AFE_ANARXCTLr_OVERRIDE_SIGDET_ENf_SET(rxctrl, 0);
            PHYMOD_IF_ERR_RETURN(MODIFY_RX_AFE_ANARXCTLr(&pa_copy, rxctrl));
        }
    }
...
static int _xgxs10g_phy_enable_get(const PHYMOD_ST *pa, phymod_phy_power_t* power )
{
    LANECTL3r_t ctl3;
    int tx_value = 0;
    int rx_value = 0;
    ...
    power->rx = rx_value ? phymodPowerOff : phymodPowerOn;
    power->tx = tx_value ? phymodPowerOff : phymodPowerOn;

    return PHYMOD_E_NONE;
}

int viper_phy_enable_set(const PHYMOD_ST *pa, const phymod_phy_power_t* power, int combo)
{
    int enable = 0;

    if(VIPER_CORE_IS_SGMIIPLUS2_X4(pa)) {
        enable = (power->tx || power->rx) ? 1 : 0;
        return (_sgmiip2x4_phy_enable_set(pa, enable));
    }
    else {
        return (_xgxs10g_phy_enable_set(pa, power, combo));
    }
}

int viper_phy_enable_get(const PHYMOD_ST *pa, phymod_phy_power_t* power)
{
    int enable = 0;

    if(VIPER_CORE_IS_SGMIIPLUS2_X4(pa)) {
        PHYMOD_IF_ERR_RETURN(_sgmiip2x4_phy_enable_get(pa, &enable));
        power->rx = enable;
        power->tx = enable;
    } else {
        PHYMOD_IF_ERR_RETURN(_xgxs10g_phy_enable_get(pa, power));
    }
...
int viper_autoneg_remote_ability_get(const PHYMOD_ST *pa, viper_an_adv_ability_t *ability)
{
    ...
    ability->an_mode = sgmii_mode ? VIPER_AN_MODE_SGMII : VIPER_AN_MODE_CL37;
