...
#include "tscbh_fe_gen2/tier1/tbhmod_fe_gen2_1588_lkup_table.h"
...
/*
 * set lane swapping for core PCS
 */

int _tscbh_fe_gen2_core_pcs_lane_map_set(const phymod_core_access_t* core,
...
/*
 * set lane swapping for core PMD
 */

int _tscbh_fe_gen2_core_pmd_lane_map_set(const phymod_core_access_t* core,
                                         const phymod_lane_map_t* lane_map)
{
    phymod_core_access_t  core_copy;
    uint32_t lane;
...
int _tscbh_fe_gen2_core_firmware_load(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config)
{
    ...
    if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
        flr_ucode = 1;
        blackhawk_ucode_len = BLACKHAWK7_L2P2_FLR_UCODE_IMAGE_SIZE;
    } else {
        blackhawk_ucode_len = BLACKHAWK7_L2P2_UCODE_IMAGE_SIZE;
    }

    blackhawk_ucode = blackhawk7_l2p2_ucode_get(flr_ucode);
...
int tscbh_fe_gen2_phy_cl72_set(const phymod_phy_access_t* phy, uint32_t cl72_en)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l2p2_INTERNAL_is_flr_supported(&pm_phy_copy.access, &flr_support));
    if (flr_support) {
...
int _tscbh_fe_gen2_core_init_pass1(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
    ...
    /* Configure PCS lane swap */
    PHYMOD_IF_ERR_RETURN
        (_tscbh_fe_gen2_core_pcs_lane_map_set(&core_copy, &init_config->lane_map));
    ...
    /* Configure PMD lane mapping */
    PHYMOD_IF_ERR_RETURN
        (_tscbh_fe_gen2_core_pmd_lane_map_set(&core_copy, &init_config->lane_map));
    ...
    if (PHYMOD_CORE_INIT_F_FLR_SUPPORT_GET(init_config)) {
        flr_ucode = 1;
        ucode.stack_size = BLACKHAWK7_L2P2_FLR_UCODE_STACK_SIZE;
        ucode.ucode_size = BLACKHAWK7_L2P2_FLR_UCODE_IMAGE_SIZE;
        ucode.crc_value  = BLACKHAWK7_L2P2_FLR_UCODE_IMAGE_CRC;
        ucode.ucode_dr_code_size_kb = BLACKHAWK7_L2P2_FLR_UCODE_DR_CODE_SIZE_KB;
    } else {
        ucode.stack_size = BLACKHAWK7_L2P2_UCODE_STACK_SIZE;
        ucode.ucode_size = BLACKHAWK7_L2P2_UCODE_IMAGE_SIZE;
        ucode.crc_value  = BLACKHAWK7_L2P2_UCODE_IMAGE_CRC;
        ucode.ucode_dr_code_size_kb = BLACKHAWK7_L2P2_UCODE_DR_CODE_SIZE_KB;
    }

    blackhawk7_ucode = blackhawk7_l2p2_ucode_get(flr_ucode);
     ...
     */
    for (i = 0; i < TBHMOD_FE_GEN2_TS_MPP_MEM_SIZE; i++) {
        PHYMOD_IF_ERR_RETURN
...
int _tscbh_fe_gen2_core_init_pass2(const phymod_core_access_t* core, const phymod_core_init_config_t* init_config, const phymod_core_status_t* core_status)
{
   ...
   /* Enables the 32T clock. */
   for (lane = 0; lane < TSCBH_NOF_LANES_IN_CORE; lane++) {
       phy_access_copy.access.lane_mask = 1 << lane;
       PHYMOD_IF_ERR_RETURN
           (tbhmod_fe_gen2_txclk32_en(&phy_access_copy.access, 1));
   }
    ...
    if (!PHYMOD_DEVICE_OP_MODE_FLEXIBLE_PLL_SELECT_GET(phy_access_copy.device_op_mode)) {
        ...
        for (lane = 0; lane < TSCBH_NOF_LANES_IN_CORE; lane++) {
            ...
            /* FCLK use the same PLL. */
            PHYMOD_IF_ERR_RETURN
                (tbhmod_fe_gen2_pcs_tx_fclk_sel_set(&phy_access_copy.access, pll_index));
            PHYMOD_IF_ERR_RETURN
                (tbhmod_fe_gen2_pcs_rx_fclk_sel_set(&phy_access_copy.access, pll_index));
        }
    }
...
int tscbh_fe_gen2_timesync_enable_set(const phymod_phy_access_t* phy, uint32_t flags, uint32_t enable)
{
    ...
    if (PHYMOD_TIMESYNC_ENABLE_F_RX_GET(flags)) {
        PHYMOD_IF_ERR_RETURN
            (tbhmod_fe_gen2_pcs_rx_ts_en(&phy_copy.access, enable));
    }

    /*
     * Set CLK4SYNC Div to 8.
     */
    fclk_div_mode = BLACKHAWK7_L2P2_CLK4SYNC_DIV_8;
    if (PHYMOD_TIMESYNC_ENABLE_F_CORE_GET(flags)) {
        ...
        for (lane_index = 0; lane_index < num_lane; lane_index++) {
            phy_copy.access.lane_mask = 1 << (start_lane + lane_index);
            PHYMOD_IF_ERR_RETURN
                (tbhmod_fe_gen2_pcs_tx_fclk_sel_get(&phy_copy.access, &fclk_pll_index));

            phy_copy.access.pll_idx = fclk_pll_index;
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_l2p2_clk4sync_enable_set(&phy_copy.access, enable, fclk_div_mode));
        }
    }
...
int tscbh_fe_gen2_timesync_enable_get(const phymod_phy_access_t* phy, uint32_t flags, uint32_t* enable)
{
    ...
    if (PHYMOD_TIMESYNC_ENABLE_F_CORE_GET(flags)) {
        ...
        phy_copy.access.pll_idx = fclk_pll_index;
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_l2p2_clk4sync_enable_get(&phy_copy.access, enable, &fclk_div_mode));
    }
                                      ...
                                      const phymod_timesync_adjust_config_info_t *config)
{
    ...
    int ts_table_index;
    int osr_mode;
    tbhmod_fe_gen2_ts_table_entry *ts_tx_entry = NULL, *ts_rx_entry = NULL;
    tbhmod_fe_gen2_ts_table_entry ts_update_table;
    uint32_t psll_entry[TBHMOD_FE_GEN2_TS_TABLE_SIZE * TBHMOD_FE_GEN2_TS_PSLL_BASED_ENTRY_SIZE];
    uint32_t psll_entry_size, ts_table_size;
    uint32_t pll_div, pll_index;
    int fclk_pll_index;
    tbhmod_refclk_t ref_clk;
    uint32_t fclk_vco, current_vco;
    int mem_offset;
    ...
    tbhmod_fe_gen2_ts_table_entry* tbhmod_fe_gen2_ts_table_tx_sop;
    tbhmod_fe_gen2_ts_table_entry* tbhmod_fe_gen2_ts_table_rx_sop;
    tbhmod_fe_gen2_ts_table_entry* tbhmod_fe_gen2_ts_table_tx_sfd;
    tbhmod_fe_gen2_ts_table_entry* tbhmod_fe_gen2_ts_table_rx_sfd;

    tbhmod_fe_gen2_ts_table_tx_sop = tbhmod_fe_gen2_ts_table_tx_sop_get();
    tbhmod_fe_gen2_ts_table_rx_sop = tbhmod_fe_gen2_ts_table_rx_sop_get();
    tbhmod_fe_gen2_ts_table_tx_sfd = tbhmod_fe_gen2_ts_table_tx_sfd_get();
    tbhmod_fe_gen2_ts_table_rx_sfd = tbhmod_fe_gen2_ts_table_rx_sfd_get();

    if (config->am_norm_mode == phymodTimesyncCompensationModeLatestlane) {
        normalize_to_latest = 1;
    }

    ts_use_sfd_table = 0;
    if (PHYMOD_TIMESYNC_F_SOP_GET(flags)) {
        ts_offset = TSCBH_FE_GEN2_TS_OFFSET_SOP;
    } else {
        if (PHYMOD_TIMESYNC_F_MAC_DA_GET(flags)) {
            ts_offset = TSCBH_FE_GEN2_TS_OFFSET_MAC_DA;
        } else {
            ts_offset = TSCBH_FE_GEN2_TS_OFFSET_SFD;
            ts_use_sfd_table = 1;
        }
    }
    ...
    if ((PHYMOD_TIMESYNC_F_REDUCED_PREAMBLE_MODE_GET(flags)) &&
        (ts_offset != TSCBH_FE_GEN2_TS_OFFSET_SOP)){
        ts_offset -= TSCBH_FE_GEN2_TS_RPM_OFFSET;
    }
    ...
    if (start_lane < 4) {
        /* MPP0 */
        mem_offset = start_lane * TBHMOD_FE_GEN2_TS_DEFAULT_TABLE_SIZE;
    } else {
        /* MPP1 */
        mem_offset = (start_lane - 4) * TBHMOD_FE_GEN2_TS_DEFAULT_TABLE_SIZE;
    }
    ...
    if (speed_id <= TSCBH_FE_GEN2_AUTONEG_SPEED_ID_COUNT) {
        /* AN speed IDs */
    ...
    is_pam4 = (speed_config.data_rate / num_lane) > TSCBH_FE_GEN2_NRZ_MAX_PER_LANE_SPEED ? 1: 0;

    /* 1.5 Get the table index and table size  of the 1588 table */
    PHYMOD_IF_ERR_RETURN
       (tbhmod_fe_gen2_1588_table_index_get(mapped_speed_id, speed_config.fec_type, &ts_table_index, &ts_table_size));
    ...
    psll_entry_size = ts_table_size * TBHMOD_FE_GEN2_TS_PSLL_BASED_ENTRY_SIZE;

    /* 1.6 Find the 1588 table */
    if (ts_use_sfd_table) {
        ts_tx_entry = tbhmod_fe_gen2_ts_table_tx_sfd + ts_table_index;
        ts_rx_entry = tbhmod_fe_gen2_ts_table_rx_sfd + ts_table_index;
    } else {
        ts_tx_entry = tbhmod_fe_gen2_ts_table_tx_sop + ts_table_index;
        ts_rx_entry = tbhmod_fe_gen2_ts_table_rx_sop + ts_table_index;
    }
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l2p2_INTERNAL_read_pll_div(&phy_copy.access, &pll_div));
    PHYMOD_IF_ERR_RETURN
        (tbhmod_pll_to_vco_get(ref_clk, pll_div, &fclk_vco));
    ...
    for (i = 0; i < (int)ts_table_size; i++) {
        PHYMOD_IF_ERR_RETURN
            (tbhmod_fe_gen2_tbl_entry_to_psll_entry_map(&(*ts_rx_entry)[i][0], &psll_entry[i * TBHMOD_FE_GEN2_TS_PSLL_BASED_ENTRY_SIZE]));
    }
    /* 3.2 Add PM offset to each PSLL. */
    PHYMOD_IF_ERR_RETURN
        (tbhmod_fe_gen2_ts_offset_rx_set(&phy_copy.access, current_vco, osr_mode, is_pam4, psll_entry_size, psll_entry));
    /* 3.3 Translate PSLL based array back to 1588 table format. */
    for (i = 0; i < (int)ts_table_size; i++) {
        PHYMOD_IF_ERR_RETURN
            (tbhmod_fe_gen2_psll_entry_to_tbl_entry_map(&psll_entry[i * TBHMOD_FE_GEN2_TS_PSLL_BASED_ENTRY_SIZE], &ts_update_table[i][0]));
    }
    ...
    phy_copy.access.pll_idx = fclk_pll_index;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l2p2_clk4sync_enable_get(&phy_copy.access, &clk4sync_en, &clk4sync_div));
    /*
    ...
    */
    phy_copy.access.lane_mask = lane_mask_backup;
    PHYMOD_IF_ERR_RETURN
        (tbhmod_fe_gen2_pcs_set_1588_ui(&phy_copy.access, current_vco, fclk_vco, osr_mode, clk4sync_div, is_pam4));

    /* 5. Program PMD lantency */
    PHYMOD_IF_ERR_RETURN
        (tbhmod_fe_gen2_1588_pmd_latency(&phy_copy.access, an_en, current_vco, osr_mode, is_pam4));
   ...
   /* 6. Update deskew for TX and RX */
    if (config->am_norm_mode) {
        ...
        phy_copy.access.lane_mask = lane_mask_backup;
        PHYMOD_IF_ERR_RETURN
            (tbhmod_fe_gen2_pcs_set_tx_lane_skew_capture(&phy_copy.access, 1));
        PHYMOD_USLEEP(5);
        PHYMOD_IF_ERR_RETURN
            (tbhmod_fe_gen2_pcs_set_tx_lane_skew_capture(&phy_copy.access, 0));
        phy_copy.access.lane_mask = 0x1 << start_lane;
        PHYMOD_IF_ERR_RETURN
        ...
        phy_copy.access.lane_mask = lane_mask_backup;
        PHYMOD_IF_ERR_RETURN
            (tbhmod_fe_gen2_pcs_measure_tx_lane_skew(&phy_copy.access, an_en, current_vco, osr_mode, is_pam4, normalize_to_latest, &tx_max_skew));

        /* 6.3 Check for rx deskew valid */
        for (i = 0; i < 1000; i++) {
            PHYMOD_USLEEP(10);
            PHYMOD_IF_ERR_RETURN
                (tbhmod_fe_gen2_pcs_ts_deskew_valid(&phy_copy.access, speed_config_entry.bit_mux_mode, &is_valid));
            if (is_valid) {
                break;
            }
        }
        ...
        PHYMOD_IF_ERR_RETURN
            (tbhmod_fe_gen2_pcs_mod_rx_1588_tbl_val(&phy_copy.access, an_en, speed_config_entry.bit_mux_mode,
    ...
    phy_copy.access.lane_mask = 0x1 << start_lane;
    PHYMOD_IF_ERR_RETURN
        (tbhmod_fe_gen2_pcs_ts_config(&phy_copy.access, ts_offset));
                                       ...
                                       phymod_phy_pll_state_t* new_pll_state)
{
    ...
    uint32_t *tscbh_fe_gen2_spd_id_entry;
    uint32_t temp_spd_packed_entry[TSCBH_FE_GEN2_SPEED_ID_ENTRY_SIZE] = {0};
    spd_id_tbl_entry_t speed_config_entry;
    uint8_t is_otn_mode;
    uint32_t fclk_pll_index;
    uint32_t trans_am_en = 0;
    ...
    PHYMOD_MEMSET(&speed_config_entry, 0x0, sizeof(speed_config_entry));
    ...
    *new_pll_state = *old_pll_state;
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        /* Check if transparent AM is needed */
        trans_am_en = PHYMOD_SPEED_CONFIG_TRANSPARENT_AM_GET(speed_config) ? 1 : 0;
        PHYMOD_IF_ERR_RETURN
            (tbhmod_fe_gen2_trans_am_set(&pm_phy_copy.access, speed_config->data_rate, trans_am_en));
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        /* If transparent AM is configured, need to bypass scramble/unscramble for 50G, 100G */
        if (PHYMOD_SPEED_CONFIG_TRANSPARENT_AM_GET(speed_config)) {
            if (speed_config->data_rate == 50000 ||
                speed_config->data_rate == 100000) {
                PHYMOD_MEMCPY(temp_spd_packed_entry, (tscbh_fe_gen2_spd_id_entry + mapped_speed_id * TSCBH_FE_GEN2_SPEED_ID_ENTRY_SIZE),
                              sizeof(uint32_t) * TSCBH_FE_GEN2_SPEED_ID_ENTRY_SIZE);
                spd_ctrl_unpack_spd_id_tbl_entry(temp_spd_packed_entry, &speed_config_entry);
                speed_config_entry.scr_mode = 0;
                spd_ctrl_pack_spd_id_tbl_entry(&speed_config_entry, temp_spd_packed_entry);
                PHYMOD_IF_ERR_RETURN
                    (phymod_mem_write(&pm_phy_copy.access, phymodMemSpeedIdTable, TSCBH_FORCED_SPEED_ID_OFFSET + start_lane,
                                      temp_spd_packed_entry));
            }
        } else {
            PHYMOD_IF_ERR_RETURN
                (phymod_mem_write(&pm_phy_copy.access, phymodMemSpeedIdTable, TSCBH_FORCED_SPEED_ID_OFFSET + start_lane,
    ...
    if (PHYMOD_DEVICE_OP_MODE_FLEXIBLE_PLL_SELECT_GET(phy->device_op_mode)) {
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            fclk_pll_index = pll_index;
            PHYMOD_IF_ERR_RETURN
                (tbhmod_fe_gen2_pcs_tx_fclk_sel_set(&pm_phy_copy.access, fclk_pll_index));
            PHYMOD_IF_ERR_RETURN
                (tbhmod_fe_gen2_pcs_rx_fclk_sel_set(&pm_phy_copy.access, fclk_pll_index));
        }
    }
                                       ...
                                       phymod_phy_speed_config_t* speed_config)
{
    ...
    if (!PHYMOD_DEVICE_OP_MODE_PCS_BYPASS_GET(phy->device_op_mode)) {
        ...
        /* Check if transparent AM eanble */
        phy_copy.access.lane_mask = 0x1 << start_lane;
        PHYMOD_IF_ERR_RETURN
            (tbhmod_fe_gen2_trans_am_get(&phy_copy.access, speed_config->data_rate, &trans_am_en));
        if (trans_am_en) {
            PHYMOD_SPEED_CONFIG_TRANSPARENT_AM_SET(speed_config);
        }
                                             ...
                                             phymod_firmware_load_info_t* info)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l2p2_INTERNAL_is_flr_supported(&pm_phy_copy.access, &flr_support));
    if (flr_support) {
        info->ucode_len = BLACKHAWK7_L2P2_FLR_UCODE_IMAGE_SIZE;
    } else {
        info->ucode_len = BLACKHAWK7_L2P2_UCODE_IMAGE_SIZE;
    }
    info->ucode_ptr = blackhawk7_l2p2_ucode_get(flr_support);
...
STATIC
int _tscbh_fe_gen2_flr_timeout_phymod_to_blackhawk(phymod_phy_flr_timeout_t timeout_time,
                                                   enum blackhawk7_l2p2_flr_timeout_enum *bh7_l2p2_timeout_time)
{
    switch (timeout_time) {
    case phymodFlrTimeout50ms:
        *bh7_l2p2_timeout_time = BLACKHAWK7_L2P2_FLR_TIMEOUT_50MS;
        break;
    case phymodFlrTimeout75ms:
        *bh7_l2p2_timeout_time = BLACKHAWK7_L2P2_FLR_TIMEOUT_75MS;
        break;
    case phymodFlrTimeout100ms:
        *bh7_l2p2_timeout_time = BLACKHAWK7_L2P2_FLR_TIMEOUT_100MS;
        break;
    case phymodFlrTimeout150ms:
        *bh7_l2p2_timeout_time = BLACKHAWK7_L2P2_FLR_TIMEOUT_150MS;
        break;
    case phymodFlrTimeout250ms:
        *bh7_l2p2_timeout_time = BLACKHAWK7_L2P2_FLR_TIMEOUT_250MS;
        break;
    case phymodFlrTimeout500ms:
        *bh7_l2p2_timeout_time = BLACKHAWK7_L2P2_FLR_TIMEOUT_500MS;
        break;
    default:
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM, (_PHYMOD_MSG("unsupported timeout time for blackhawk7_l2p2 %u"), timeout_time));
        return PHYMOD_E_PARAM;
    }
    return PHYMOD_E_NONE;
}
...
STATIC
int _tscbh_fe_gen2_flr_timeout_blackhawk_to_phymod(enum blackhawk7_l2p2_flr_timeout_enum bh7_l2p2_timeout_time,
                                                   phymod_phy_flr_timeout_t *timeout_time)
{
    switch (bh7_l2p2_timeout_time) {
    case BLACKHAWK7_L2P2_FLR_TIMEOUT_50MS:
        *timeout_time = phymodFlrTimeout50ms;
        break;
    case BLACKHAWK7_L2P2_FLR_TIMEOUT_75MS:
        *timeout_time = phymodFlrTimeout75ms;
        break;
    case BLACKHAWK7_L2P2_FLR_TIMEOUT_100MS:
        *timeout_time = phymodFlrTimeout100ms;
        break;
    case BLACKHAWK7_L2P2_FLR_TIMEOUT_150MS:
        *timeout_time = phymodFlrTimeout150ms;
        break;
    case BLACKHAWK7_L2P2_FLR_TIMEOUT_250MS:
        *timeout_time = phymodFlrTimeout250ms;
        break;
    case BLACKHAWK7_L2P2_FLR_TIMEOUT_500MS:
        *timeout_time = phymodFlrTimeout500ms;
        break;
    default:
        PHYMOD_RETURN_WITH_ERR(PHYMOD_E_PARAM, (_PHYMOD_MSG("unsupported timeout time for blackhawk7_L2P2 %u"), bh7_l2p2_timeout_time));
        return PHYMOD_E_PARAM;
    }
...
int tscbh_fe_gen2_phy_flr_config_set(const phymod_phy_access_t* phy, uint32_t enable, phymod_phy_flr_timeout_t timeout_time)
{
    int start_lane, num_lane, i;
    phymod_phy_access_t phy_copy;
    uint8_t flr_support = 0;
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l2p2_INTERNAL_is_flr_supported(&phy_copy.access, &flr_support));
    if (!flr_support) {
    ...
    if (enable) {
        PHYMOD_IF_ERR_RETURN
            (_tscbh_fe_gen2_flr_timeout_phymod_to_blackhawk(timeout_time, &bh7_l2p2_timeout_time));
    } else {
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (blackhawk7_l2p2_configure_fast_link_recovery(&phy_copy.access, enable,
                                                          enable, bh7_l2p2_timeout_time));
    }
...
int tscbh_fe_gen2_phy_flr_config_get(const phymod_phy_access_t* phy, uint32_t *enable,
                                     phymod_phy_flr_timeout_t *timeout_time)
{
    ...
    phy_copy.access.lane_mask = 0x1 << start_lane;
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l2p2_get_configure_fast_link_recovery(&phy_copy.access,
                                                          &flr_enable,
                                                          &flr_timeout_en,
                                                          &bh7_l2p2_timeout_time));

    *enable =  flr_enable ? 1 : 0;
    if (flr_timeout_en) {
        PHYMOD_IF_ERR_RETURN
            (_tscbh_fe_gen2_flr_timeout_blackhawk_to_phymod(bh7_l2p2_timeout_time,
                                                         timeout_time));
    } else {
...
int tscbh_fe_gen2_phy_fw_is_flr_supported(const phymod_phy_access_t* phy, uint32_t* flr_support)
{
    uint8_t support = 0;
    phymod_phy_access_t pm_phy_copy;
    PHYMOD_MEMCPY(&pm_phy_copy, phy, sizeof(pm_phy_copy));
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l2p2_INTERNAL_is_flr_supported(&pm_phy_copy.access, &support));
    *flr_support = support;
