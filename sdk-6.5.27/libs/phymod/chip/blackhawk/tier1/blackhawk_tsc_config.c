...
err_code_t blackhawk_tsc_validate_txfir_cfg(srds_access_t *sa__, enum blackhawk_tsc_txfir_tap_enable_enum enable_taps, int16_t pre2, int16_t pre1, int16_t main, int16_t post1, int16_t post2, int16_t post3) {
    ...
    if ((main < 0) || (pre1 > 0) || (post1 > 0)) {
        EFUN_PRINTF(("ERROR: In BLACKHAWK, main TAP is expected to be positive and pre1 and post1 TAPs are expected to be negative for normal use cases.\n"));
        return(blackhawk_tsc_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    if (main < SRDS_ABS(pre1) || main < pre2 || main < SRDS_ABS(post1) || main < post2 || main < post3) {
        EFUN_PRINTF(("ERROR: in BLACKHAWK, main TAP is expected to be greater than all other TAPs."));
        return(blackhawk_tsc_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
...
err_code_t blackhawk_tsc_apply_txfir_cfg(srds_access_t *sa__, enum blackhawk_tsc_txfir_tap_enable_enum enable_taps, int16_t pre2, int16_t pre1, int16_t main, int16_t post1, int16_t post2, int16_t post3) {
    ...
    EFUN(blackhawk_tsc_validate_txfir_cfg(sa__, enable_taps, pre2, pre1, main, post1, post2, post3));
...
err_code_t blackhawk_tsc_tx_clock_align(srds_access_t *sa__, int num_lanes, int enable) {
    ...
    for(lane = 0;lane<num_lanes;++lane) {
        EFUN(blackhawk_tsc_set_lane(sa__,(uint8_t)lane));
        if(enable) {
...
/*  Fast Link Recovery  */
/************************/
err_code_t blackhawk_tsc_configure_fast_link_recovery(srds_access_t *sa__, uint8_t enable, uint8_t enable_timeout, enum blackhawk_tsc_flr_timeout_enum timeout_time) {
    uint8_t const timeout_time_val = (uint8_t)timeout_time;

    uint8_t reset_state;
    ESTM(reset_state = rd_rx_lane_dp_reset_state());
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: blackhawk_tsc_configure_fast_link_recovery(..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n",blackhawk_tsc_get_lane(sa__),reset_state));
        return (blackhawk_tsc_error(sa__, ERR_CODE_LANE_DP_NOT_RESET));
    }
    {
        uint8_t flr_support = 0;
        EFUN(blackhawk_tsc_INTERNAL_is_flr_supported(sa__, &flr_support));
        if(!flr_support) {
            EFUN_PRINTF(("ERROR: blackhawk_tsc_configure_fast_link_recovery(..) unsupported with currently loaded microcode (Core=%d Lane=%d).\n", blackhawk_tsc_get_core(sa__), blackhawk_tsc_get_lane(sa__)));
            return (blackhawk_tsc_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
        }
    }

    if (timeout_time_val > 5) {
        ESTM_PRINTF(("timeout_time value for blackhawk_tsc_configure_fast_link_recovery() is too large:  %d\n", timeout_time_val));
        return (blackhawk_tsc_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    {
...
err_code_t blackhawk_tsc_get_configure_fast_link_recovery(srds_access_t *sa__, uint8_t *enable, uint8_t *enable_timeout, enum blackhawk_tsc_flr_timeout_enum *timeout_time) {
    uint8_t flr_byte = 0;
    uint8_t flr_support = 0;

    EFUN(blackhawk_tsc_INTERNAL_is_flr_supported(sa__, &flr_support));
    if(!flr_support) {
        EFUN_PRINTF(("ERROR: blackhawk_tsc_get_configure_fast_link_recovery(..) unsupported with currently loaded microcode (Core=%d Lane=%d).\n", blackhawk_tsc_get_core(sa__), blackhawk_tsc_get_lane(sa__)));
        return (blackhawk_tsc_error(sa__, ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
    }
    ...
    *timeout_time = (enum blackhawk_tsc_flr_timeout_enum)((flr_byte >> 2) & 0x7);
    return (ERR_CODE_NONE);
}
