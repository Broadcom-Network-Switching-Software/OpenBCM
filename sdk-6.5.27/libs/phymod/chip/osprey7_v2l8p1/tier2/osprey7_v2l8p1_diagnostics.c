...
#ifdef PHYMOD_LINKCAT_OSPREY7_V218P1_SUPPORT
                                          ...
                                          const phymod_pattern_t* pattern)
{
    ...
    PHYMOD_IF_ERR_RETURN
            (osprey7_v2l8p1_config_shared_tx_pattern (&phy_copy.access,
                                          ...
                                          phymod_pattern_t* pattern)
{
   phymod_phy_access_t phy_copy;
    ...
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (osprey7_v2l8p1_config_shared_tx_pattern_idx_get(&phy_copy.access,
                                  &pattern->pattern_len,
                                  pattern->pattern));
                                          ...
                                          const phymod_pattern_t* pattern)
{
   phymod_phy_access_t phy_copy;
    ...
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (osprey7_v2l8p1_tx_shared_patt_gen_en(&phy_copy.access, (uint8_t) enable, (uint8_t)pattern->pattern_len));
...
int osprey7_v2l8p1_phy_pattern_enable_get(const phymod_phy_access_t* phy, uint32_t* enable)
{
    ...
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
       (osprey7_v2l8p1_tx_shared_patt_gen_en_get(&phy_copy.access, &enable_8));
...
STATIC
int _osprey7_v2l8p1_display_rx_dfe_taps(const phymod_phy_access_t* phy)
{
    int i = 0;
    phymod_rx_t rx;
    phymod_phy_access_t phy_copy;
    ...
    /* first check if the PAM4 mode or not */
    PHYMOD_IF_ERR_RETURN
        (osprey7_v2l8p1_signalling_mode_status_get(&phy_copy.access, &signalling_mode));
    PHYMOD_IF_ERR_RETURN (osprey7_v2l8p1_phy_rx_get(&phy_copy, &rx));

    PHYMOD_DIAG_OUT(("RX DFE Range          = %s\n", signalling_mode == phymodSignallingMethodNRZ ? "NRZ" : "PAM4"));
    PHYMOD_DIAG_OUT(("NUM OF DFE Taps       = %d\n", rx.num_of_dfe_taps));
    for (i = 0; i < rx.num_of_dfe_taps; i++) {
         PHYMOD_DIAG_OUT(("RX DEF Tap%2d          = %4d,   enable          = %d\n", i, rx.dfe[i].value, rx.dfe[i].enable));
    }
    ...
    if (cmd_type == phymod_diag_DSC) {
       for (i = start_lane; i < start_lane + num_lane; i++) {
        ...
        for (i = 0; i < num_lane; i++) {
            ...
            case phymod_diag_DFE_TAPS:
                PHYMOD_DIAG_OUT(("\nLane mask = 0x%x, type = DFE\n", phy_copy.access.lane_mask));
                PHYMOD_IF_ERR_RETURN
                    (_osprey7_v2l8p1_display_rx_dfe_taps(&phy_copy));
                break;
...
STATIC int osprey7_v2l8p1_diagnostics_eyescan_run_uc(const phymod_phy_access_t* phy, uint32_t flags, int intrusive_eyescan)
{
    ...
    if(PHYMOD_EYESCAN_F_PROCESS_GET(flags)) {
        for(j=0; j< PHYMOD_CONFIG_MAX_LANES_PER_CORE; j++) { /* Loop for all lanes. */
            ...
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_INTERNAL_get_rx_pam_mode(&(phy_copy.access), &pam_mode));
            if (pam_mode == NRZ) {
                ...
                if (intrusive_eyescan) {
                    ...
                    rc = osprey7_v2l8p1_display_intrusive_eye_scan(&(phy_copy.access));
                } else {
                    rc = osprey7_v2l8p1_display_eye_scan(&(phy_copy.access));
                }
            }
                           ...
                           const phymod_phy_eyescan_options_t* eyescan_options)
{
    ...
    PHYMOD_IF_ERR_RETURN
      (osprey7_v2l8p1_pmd_lock_status(&phy_copy.access, &pmd_rx_lock));
    ...
    switch(mode) {
        case phymodEyescanModeFast:
            return osprey7_v2l8p1_diagnostics_eyescan_run_uc(phy, flags, 0);
        case phymodEyescanModeIntrusive:
            return osprey7_v2l8p1_diagnostics_eyescan_run_uc(phy, flags, 1);
        default:
                                                  ...
                                                  uint32_t enable)
{
    ...
    case phymod_PAM4TxPattern_JP03B:
        PHYMOD_IF_ERR_RETURN
            (osprey7_v2l8p1_config_tx_jp03b_pattern(&phy_copy.access, (uint8_t) enable));
        break;
    case phymod_PAM4TxPattern_Linear:
        PHYMOD_IF_ERR_RETURN
            (osprey7_v2l8p1_config_tx_linearity_pattern(&phy_copy.access, (uint8_t) enable));
        break;
    default:
                                                  ...
                                                  uint32_t* enable)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (osprey7_v2l8p1_pam4_tx_pattern_enable_get(&phy_copy.access, pattern_type, enable));
...
int osprey7_v2l8p1_phy_tx_pattern_set(const phymod_phy_access_t* phy, phymod_phy_tx_pattern_t tx_pattern)
{
    ...
    switch (tx_pattern) {
        case phymodPhyTxPatternOff:
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_config_tx_jp03b_pattern(&phy_copy.access, 0));
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_config_tx_linearity_pattern(&phy_copy.access, 0));
            break;
        case phymodPhyTxPatternPAM4_JP03B:
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_config_tx_linearity_pattern(&phy_copy.access, 0));
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_config_tx_jp03b_pattern(&phy_copy.access, 1));
            break;
        case phymodPhyTxPatternPAM4_Linear:
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_config_tx_jp03b_pattern(&phy_copy.access, 0));
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_config_tx_linearity_pattern(&phy_copy.access, 1));
            break;
        case phymodPhyTxPatternCustom:
            custom_pattern.pattern = pattern_word;
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_phy_pattern_config_get(&phy_copy, &custom_pattern));
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_config_tx_jp03b_pattern(&phy_copy.access, 0));
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_config_tx_linearity_pattern(&phy_copy.access, 0));
            PHYMOD_IF_ERR_RETURN
                (osprey7_v2l8p1_phy_pattern_enable_set(&phy_copy, 1, &custom_pattern));
            break;
        default:
...
int osprey7_v2l8p1_phy_tx_pattern_get(const phymod_phy_access_t* phy, phymod_phy_tx_pattern_t* tx_pattern)
{
    ...
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (osprey7_v2l8p1_pam4_tx_pattern_enable_get(&phy_copy.access,
    ...
    PHYMOD_IF_ERR_RETURN
        (osprey7_v2l8p1_pam4_tx_pattern_enable_get(&phy_copy.access,
                                                    ...
                                                    &linear_enable));
    PHYMOD_IF_ERR_RETURN
        (osprey7_v2l8p1_tx_shared_patt_gen_en_get(&phy_copy.access,
                                                   &pattern_gen_enable));
    /*
    ...
    switch (options->ber_proj_phase) {
        ...
        case PHYMOD_BER_PROJ_PHASE_F_CONFIG:
            ...
            for (i = 0; i < num_lane; i++) {
                ...
                PHYMOD_IF_ERR_RETURN
                    (osprey7_v2l8p1_prbs_error_analyzer_reset(&phy_copy.access));
                ...
                PHYMOD_IF_ERR_RETURN
                    (osprey7_v2l8p1_prbs_error_analyzer_aggregate_config(&phy_copy.access, &err_analyzer));
                PHYMOD_IF_ERR_RETURN
                    (osprey7_v2l8p1_display_prbs_error_analyzer_config(&phy_copy.access,
        ...
        case PHYMOD_BER_PROJ_PHASE_F_START:
            for (i = 0; i < num_lane; i++) {
                phy_copy.access.lane_mask = 1 << (start_lane + i);
                PHYMOD_IF_ERR_RETURN(osprey7_v2l8p1_prbs_error_analyzer_start(&phy_copy.access));
            }
            break;
        ...
        case PHYMOD_BER_PROJ_PHASE_F_CAL:
            for (i = 0; i < num_lane; i++) {
                 ...
                 * Alternative: use err_analyzer->pp_field2 = osprey7_v2l8p1_get_lane(&phy_copy.access).
                 */
                PHYMOD_IF_ERR_RETURN(osprey7_v2l8p1_get_prbs_error_analyzer_err_count(&phy_copy.access, &err_analyzer));
                ...
                PHYMOD_IF_ERR_RETURN
                    (osprey7_v2l8p1_display_prbs_error_analyzer_err_count(&phy_copy.access, &err_analyzer));
                PHYMOD_IF_ERR_RETURN(
...
int osprey7_v2l8p1_phy_pmd_lane_diag_debug_level_set(const phymod_phy_access_t* phy, uint32_t level)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        PHYMOD_IF_ERR_RETURN
            (osprey7_v2l8p1_set_usr_event_log_group_mask(&phy_copy.access, event_group_mask));
    }
...
int osprey7_v2l8p1_phy_pmd_lane_diag_debug_level_get(const phymod_phy_access_t* phy, uint32_t *level)
{
    ...
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (osprey7_v2l8p1_get_usr_event_log_group_mask(&phy_copy.access, &event_group_mask));
...
int osprey7_v2l8p1_phy_linkcat(const phymod_phy_access_t* phy,
                               phymod_linkcat_config_t config)
{
    ...
    rv = osprey7_v2l8p1_LinkCat(&phy_acc, &linkcat_config, &linkcat_status);
