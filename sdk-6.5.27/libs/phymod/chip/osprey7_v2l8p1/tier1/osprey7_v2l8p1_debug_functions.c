...
static err_code_t _osprey7_v2l8p1_reg_print_no_buf(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr);
static err_code_t _osprey7_v2l8p1_reg_print_with_buf(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr, char reg_buffer[SRDS_DUMP_BUF_SIZE]);
static err_code_t _osprey7_v2l8p1_reg_print(srds_access_t *sa__, uint8_t reg_start_index, uint8_t reg_end_index, char reg_buffer[][SRDS_DUMP_BUF_SIZE], uint8_t *buf_index);
...
/*******************************************************************/
err_code_t osprey7_v2l8p1_get_usr_event_log_group_mask(srds_access_t *sa__, uint32_t *event_group_mask) {
...
err_code_t osprey7_v2l8p1_set_usr_event_log_group_mask(srds_access_t *sa__, uint32_t event_group_mask) {
    srds_info_t * const osprey7_v2l8p1_info_ptr = osprey7_v2l8p1_INTERNAL_get_osprey7_v2l8p1_info_ptr(sa__);
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_add_usr_event_log_group_mask(srds_access_t *sa__, uint32_t event_group_mask) {
    uint32_t mask;
    srds_info_t * const osprey7_v2l8p1_info_ptr = osprey7_v2l8p1_INTERNAL_get_osprey7_v2l8p1_info_ptr(sa__);
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_delete_usr_event_log_group_mask(srds_access_t *sa__, uint32_t event_group_mask) {
    uint32_t mask;
    srds_info_t * const osprey7_v2l8p1_info_ptr = osprey7_v2l8p1_INTERNAL_get_osprey7_v2l8p1_info_ptr(sa__);
    INIT_SRDS_ERR_CODE
...
err_code_t osprey7_v2l8p1_set_usr_ctrl_lane_event_log_level(srds_access_t *sa__, uint8_t lane_event_log_level) {
    ...
    if ((osprey7_v2l8p1_info_ptr->signature >> 24) >= 0x42) {
        if (lane_event_log_level > 4 )
            EFUN(osprey7_v2l8p1_set_usr_event_log_group_mask(sa__, EVENT_GROUP_PRIORITY_5));
        else if (lane_event_log_level ==4 )
            EFUN(osprey7_v2l8p1_set_usr_event_log_group_mask(sa__, EVENT_GROUP_PRIORITY_4));
        else if (lane_event_log_level ==3 )
            EFUN(osprey7_v2l8p1_set_usr_event_log_group_mask(sa__, EVENT_GROUP_PRIORITY_3));
        else if (lane_event_log_level ==2 )
            EFUN(osprey7_v2l8p1_set_usr_event_log_group_mask(sa__, EVENT_GROUP_PRIORITY_2));
        else if (lane_event_log_level ==1 )
            EFUN(osprey7_v2l8p1_set_usr_event_log_group_mask(sa__, EVENT_GROUP_PRIORITY_1));
        else /* == 0  */
            EFUN(osprey7_v2l8p1_set_usr_event_log_group_mask(sa__, EVENT_GROUP_PRIORITY_0));
...
static err_code_t _osprey7_v2l8p1_reg_print_no_buf(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr) {
    ...
    for (offset = 0; offset < 0x10; offset++) {
        if(valid_addr & (0x1 << offset)) {
            EFUN(osprey7_v2l8p1_acc_rdt_reg(sa__, (uint16_t)(start_addr + offset), &rddata));
        }
        else {
            rddata = 0x0;
        }
...
static err_code_t _osprey7_v2l8p1_reg_print(srds_access_t *sa__, uint8_t reg_start_index, uint8_t reg_end_index, char reg_buffer[][SRDS_DUMP_BUF_SIZE], uint8_t *buf_index) {
    INIT_SRDS_ERR_CODE
    uint8_t reg_section;
    osprey7_v2l8p1_reg_dump_arr_t *osprey7_v2l8p1_reg_dump_arr = osprey7_v2l8p1_get_reg_dump_arr();
    if(reg_buffer == NULL) {
        for(reg_section = reg_start_index; reg_section < reg_end_index; reg_section++) {
            EFUN(_osprey7_v2l8p1_reg_print_no_buf(sa__, osprey7_v2l8p1_reg_dump_arr[reg_section][0], osprey7_v2l8p1_reg_dump_arr[reg_section][1]));
        }
    }
    else {
        for(reg_section = reg_start_index; reg_section < reg_end_index; reg_section++) {
            EFUN(_osprey7_v2l8p1_reg_print_with_buf(sa__, osprey7_v2l8p1_reg_dump_arr[reg_section][0], osprey7_v2l8p1_reg_dump_arr[reg_section][1], reg_buffer[*buf_index]));
            (*buf_index)++;
        }
    }
    return (ERR_CODE_NONE);
...
err_code_t osprey7_v2l8p1_reg_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char reg_buffer[][SRDS_DUMP_BUF_SIZE]) {
    ...
    UNUSED(core);
    UNUSED(lane);
    EFUN(osprey7_v2l8p1_reg_select_dump(sa__, SRDS_LANE_AND_CORE_REG, reg_buffer));
    return (ERR_CODE_NONE);
}

err_code_t osprey7_v2l8p1_reg_select_dump(srds_access_t *sa__, uint8_t reg_dump_control, char reg_buffer[][SRDS_DUMP_BUF_SIZE]) {
    INIT_SRDS_ERR_CODE
    uint8_t core = osprey7_v2l8p1_acc_get_core(sa__);
    uint8_t lane = osprey7_v2l8p1_acc_get_lane(sa__);
    uint8_t buf_index = 0;
    ...
    if (reg_dump_control & SRDS_LANE_REG_ONLY) {
        /* Lane Registers */
        EFUN(_osprey7_v2l8p1_reg_print(sa__, 0, OSPREY7_V2L8P1_REG_CORE_START_INDEX, reg_buffer, &buf_index));
    }
    if (reg_dump_control & SRDS_CORE_REG_ONLY) {
        /* Core Registers */
        EFUN(_osprey7_v2l8p1_reg_print(sa__, OSPREY7_V2L8P1_REG_CORE_START_INDEX, OSPREY7_V2L8P1_REG_PLL_START_INDEX, reg_buffer, &buf_index));
        ...
        for (pll_idx = 0; pll_idx < NUM_PLLS; pll_idx++) {
            ...
            EFUN(_osprey7_v2l8p1_reg_print(sa__, OSPREY7_V2L8P1_REG_PLL_START_INDEX, OSPREY7_V2L8P1_REG_UC_START_INDEX, reg_buffer, &buf_index));
        }
        ...
        for(micro_idx = 0; micro_idx < num_micros; micro_idx++) {
            ...
            EFUN(_osprey7_v2l8p1_reg_print(sa__, OSPREY7_V2L8P1_REG_UC_START_INDEX, OSPREY7_V2L8P1_REG_DUMP_SECTIONS, reg_buffer, &buf_index));
        }
...
err_code_t osprey7_v2l8p1_uc_core_var_blk_dump(srds_access_t *sa__,uint8_t core, uint16_t data_buff_size,uint8_t *data_buffer) {
      INIT_SRDS_ERR_CODE

      srds_info_t * osprey7_v2l8p1_info_ptr = osprey7_v2l8p1_INTERNAL_get_osprey7_v2l8p1_info_ptr_with_check(sa__);
      osprey7_v2l8p1_INTERNAL_rdblk_ram_arg_t rdblk_state;
      osprey7_v2l8p1_INTERNAL_ram_dump_state_t state;
      uint32_t core_var_ram_size = 0;
      EFUN(osprey7_v2l8p1_INTERNAL_match_ucode_from_info(sa__, osprey7_v2l8p1_info_ptr));
      core_var_ram_size = (uint32_t)osprey7_v2l8p1_info_ptr->core_var_ram_size;
      state.index = 0;
      ...
      EFUN(osprey7_v2l8p1_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                      ...
                                                      osprey7_v2l8p1_info_ptr->core_var_ram_size,
                                                      &rdblk_state,
                                                      osprey7_v2l8p1_INTERNAL_rdblk_ram_read_callback));
...
err_code_t osprey7_v2l8p1_uc_micro_var_blk_dump(srds_access_t *sa__,uint8_t core,uint8_t lane,uint16_t data_buff_size,uint8_t *data_buffer) {
      ...
      srds_info_t * osprey7_v2l8p1_info_ptr = osprey7_v2l8p1_INTERNAL_get_osprey7_v2l8p1_info_ptr_with_check(sa__);
      EFUN(osprey7_v2l8p1_INTERNAL_match_ucode_from_info(sa__, osprey7_v2l8p1_info_ptr));
      micro_var_ram_size = osprey7_v2l8p1_info_ptr->micro_var_ram_size;
      ...
      EFUN_PRINTF(("\n**** SERDES UC CORE %d MICRO %d RAM VARIABLE DECODED ****\n", core,lane/2));
          ...
          uc_addr_offset = _osprey7_v2l8p1_INTERNAL_get_uc_addr_from_lane(sa__, 0,lane);
...
err_code_t osprey7_v2l8p1_uc_lane_static_var_blk_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, uint32_t data_buff_size,uint8_t *data_buffer) {
    INIT_SRDS_ERR_CODE
    ...
    lane = osprey7_v2l8p1_acc_get_physical_lane(sa__);
    lane_addr_offset = _osprey7_v2l8p1_INTERNAL_get_static_addr_from_lane(sa__,0,lane);
...
err_code_t osprey7_v2l8p1_uc_lane_var_blk_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, uint32_t data_buff_size,uint8_t *data_buffer) {
    INIT_SRDS_ERR_CODE
    uint8_t     rx_lock, uc_stopped = 0;
    ...
    lane = osprey7_v2l8p1_acc_get_physical_lane(sa__);
    lane_addr_offset = _osprey7_v2l8p1_INTERNAL_get_addr_from_lane(sa__, 0 ,lane);
...
err_code_t osprey7_v2l8p1_internal_lpbk(srds_access_t *sa__, uint8_t enable) {
    ...
    switch (lane) {
        ...
        default:
            ...
            return(osprey7_v2l8p1_error(sa__, ERR_CODE_BAD_LANE));
    }
    ...
    }
    {
        ...
        EFUN(osprey7_v2l8p1_get_use_rx_osr_mode_pins_only(sa__, &use_rx_osr_pin));
        if(use_rx_osr_pin) {
    ...
    if (osr_mode.rx != OSPREY7_V2L8P1_OSX2) {
    ...
    if(enable) {
        ...
        EFUN(osprey7_v2l8p1_tx_pi_freq_override(sa__, 0, 0));
    }
    else {
        ...
        EFUN(osprey7_v2l8p1_reset_lane_to_default(sa__));                                /* Toggle lane soft reset */
...
err_code_t osprey7_v2l8p1_isolate_lane_ctrl_pins(srds_access_t *sa__, uint8_t enable) {
    ...
    EFUN(osprey7_v2l8p1_isolate_lane_ctrl_tx_pins(sa__, enable));
    EFUN(osprey7_v2l8p1_isolate_lane_ctrl_rx_pins(sa__, enable));
    return (ERR_CODE_NONE);
}
...
err_code_t osprey7_v2l8p1_log_full_pmd_state_noPRBS(srds_access_t *sa__, struct osprey7_v2l8p1_detailed_lane_status_st *lane_st) {
    ...
    EFUN(osprey7_v2l8p1_INTERNAL_pmd_lock_status(sa__,&lane_st->pmd_lock, &lane_st->pmd_lock_chg));
    {
        err_code_t err_code=ERR_CODE_NONE;
    ...
    EFUN(osprey7_v2l8p1_INTERNAL_sigdet_status(sa__, &lane_st->sigdet, &lane_st->sigdet_chg));
    ...
    lane_st->vga = tmp1;
    EFUN(osprey7_v2l8p1_INTERNAL_get_rx_fga(sa__, &tmp1)); lane_st->fga = tmp1;
    ...
    EFUN(osprey7_v2l8p1_INTERNAL_get_ffe_enabled(sa__, &lane_st->ffe_enable));
    ESTM(lane_st->ffe_gain          = osprey7_v2l8p1_INTERNAL_gray_to_uint8(rd_ams_rx_ffe_gain()));
    ...
    {
        int8_t lms_thresh;
        EFUN(osprey7_v2l8p1_INTERNAL_get_lms_thresh_bin(sa__, &lms_thresh)); lane_st->lms_thresh = lms_thresh;
    }
    ...
    {   /* These exist in Blackhawk7 as well but are not in the lane struct. */
        uint8_t i; osprey7_v2l8p1_eye_margin_t eye_margin;
        ...
        for(i = 0; i < NUM_PAM_EYES; i++) {
            ...
            ESTM(lane_st->pam_heye_avg_mUI[i] = osprey7_v2l8p1_INTERNAL_eye_to_mUI(sa__, (uint8_t)((rdv_usr_sts_pam_heye_right(i) + rdv_usr_sts_pam_heye_left(i) + 1) >> 1)));
            ESTM(lane_st->pam_veye_avg_mV[i] = osprey7_v2l8p1_INTERNAL_eye_to_mV(sa__, (uint8_t)((rdv_usr_sts_pam_veye_upper(i) + rdv_usr_sts_pam_veye_lower(i) + 1) >> 1), 0));
        }
    }
...
err_code_t osprey7_v2l8p1_log_full_pmd_state (srds_access_t *sa__, struct osprey7_v2l8p1_detailed_lane_status_st *lane_st) {
    ...
    EFUN(osprey7_v2l8p1_gen_collect_osprey7_v2l8p1_detailed_lane_status_st(sa__, lane_st));
    return(ERR_CODE_NONE);
}
...
void osprey7_v2l8p1_INTERNAL_print_triage_info(srds_access_t *sa__, err_code_t err_code, uint8_t print_header, uint8_t print_data, uint16_t line)
{
    ...
    if (osprey7_v2l8p1_pmd_lock_status(sa__, &info.pmd_lock)) {
        error_seen = 1;
    }
