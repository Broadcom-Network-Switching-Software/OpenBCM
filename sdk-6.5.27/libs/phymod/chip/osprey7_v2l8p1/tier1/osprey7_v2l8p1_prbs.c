...
static err_code_t _osprey7_v2l8p1_get_prbs_error_analyzer_prbs_chk_lock (srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer, uint8_t *prbs_chk_lock);
...
const char* osprey7_v2l8p1_get_e2s_prbs_mode_enum(enum srds_prbs_polynomial_enum prbs_poly_mode) {
    ...
    if(prbs_poly_mode >= sizeof(osprey7_v2l8p1_e2s_prbs_mode_enum)/sizeof(osprey7_v2l8p1_e2s_prbs_mode_enum[0])) {
        return "UNKNOWN";
    }
    return osprey7_v2l8p1_e2s_prbs_mode_enum[prbs_poly_mode];
}
...
err_code_t osprey7_v2l8p1_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    if(enabled) {
        ...
        if(prbs_poly_mode == PRBS_UNKNOWN) {
            ...
            prbs_string = osprey7_v2l8p1_get_e2s_prbs_mode_enum(prbs_poly_mode);
        }
...
err_code_t osprey7_v2l8p1_get_prbs_error_analyzer_err_count(srds_access_t *sa__, osprey7_v2l8p1_prbs_err_analyzer_config_status_st *err_analyzer) {
    ...
    EFUN(_osprey7_v2l8p1_get_prbs_error_analyzer_prbs_chk_lock(sa__, err_analyzer, &prbs_lock));
    if(!prbs_lock) {
        return (ERR_CODE_NONE);
    }
