...
err_code_t osprey7_v2l8p1_INTERNAL_configure_pll(srds_access_t *sa__,
    ...
    if (pll_option & OSPREY7_V2L8P1_PLL_OPTION_POWERDOWN) {
    ...
    if ((osprey7_v2l8p1_info_ptr->ucode_version & 0xFFFFF) < 0x00201) {
        uint8_t core_ctrl = 0;
    ...
    if (pll_option == OSPREY7_V2L8P1_PLL_OPTION_REFCLK_DOUBLER_EN) {
        EFUN(wrc_ams_pll_refclk_freq2x_en(0x1));
        refclk_freq_hz <<= 1;
    } else if (pll_option == OSPREY7_V2L8P1_PLL_OPTION_REFCLK_DIV2_EN) {
        EFUN(wrc_ams_pll_refdiv2(1));
        EFUN(wrc_ams_pll_div4_2_sel(1));
        refclk_freq_hz >>= 1;
    } else if (pll_option == OSPREY7_V2L8P1_PLL_OPTION_REFCLK_DIV4_EN) {
...
err_code_t osprey7_v2l8p1_INTERNAL_configure_lb_53P125_pll(srds_access_t *sa__,
                                                enum osprey7_v2l8p1_pll_refclk_enum refclk,
                                                enum osprey7_v2l8p1_pll_div_enum srds_div,
                                                enum osprey7_v2l8p1_pll_option_enum pll_option) {
    INIT_SRDS_ERR_CODE
    ...
    EFUN(osprey7_v2l8p1_INTERNAL_configure_pll(sa__, refclk, srds_div, VCO_FREQ_53P125G_KHZ, pll_option));
    if(ams_version == 0xa0) {
        uint8_t core_ctrl = 0;
        EFUN(wrc_ams_pll_pwrdn_hbvco(0x1));
        EFUN(wrc_ams_pll_pwrdn_lbvco(0x0)); /* enable LBVCO */
...
err_code_t osprey7_v2l8p1_INTERNAL_force_hb_53P125_pll(srds_access_t *sa__) {
    INIT_SRDS_ERR_CODE
    uint8_t ams_version;
...
err_code_t osprey7_v2l8p1_INTERNAL_pll_lb_range_check(srds_access_t *sa__, uint8_t *pll_out_range) {
    INIT_SRDS_ERR_CODE
    uint8_t pll_range = 0;
...
err_code_t osprey7_v2l8p1_INTERNAL_pll_hb_range_check(srds_access_t *sa__, uint8_t *pll_out_range) {
    INIT_SRDS_ERR_CODE
    uint8_t pll_range = 0;
...
err_code_t osprey7_v2l8p1_INTERNAL_configure_53P125_pll(srds_access_t *sa__,
                                                enum osprey7_v2l8p1_pll_refclk_enum refclk,
                                                enum osprey7_v2l8p1_pll_div_enum srds_div,
                                                enum osprey7_v2l8p1_pll_option_enum pll_option) {
    INIT_SRDS_ERR_CODE
    ...
    ESTM(ams_version = rd_ams_tx_version_id());
    EFUN(osprey7_v2l8p1_INTERNAL_configure_lb_53P125_pll(sa__, refclk, srds_div, pll_option)); /* Enable LBVCO for A0, some A1, C0, C1 AFE versions */
    ...
    if(ams_version > 0xa0) {
        ...
        EFUN(osprey7_v2l8p1_INTERNAL_pll_lb_range_check(sa__, &pll_out_range));
        if((pll_lock==0) || ((pll_lock==1) && (pll_out_range==1))) {
            EFUN(osprey7_v2l8p1_INTERNAL_force_hb_53P125_pll(sa__)); /* Enable HBVCO */
            EFUN(USR_DELAY_MS(50));
            EFUN(osprey7_v2l8p1_INTERNAL_pll_lock_status(sa__,&pll_lock,&pll_lock_chg));
            EFUN(osprey7_v2l8p1_INTERNAL_pll_hb_range_check(sa__, &pll_out_range));
    ...
    ESTM_PRINTF(("ERROR: Reliable PLL Lock not possible; core %d PLL %d\n",osprey7_v2l8p1_acc_get_core(sa__), osprey7_v2l8p1_acc_get_pll_idx(sa__)));
