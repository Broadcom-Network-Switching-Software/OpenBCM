...
err_code_t blackhawk7_l8p2_enable_pass_through_configuration(phymod_access_t *sa__, int enable)
{
    ...
    PHYMOD_EFUN_WR(wrv_blackhawk7_l8p2_usr_ctrl_word(sa__, write_val));
...
err_code_t
blackhawk7_l8p2_tsc_usr_misc_ctrl_set(phymod_access_t *sa__, uint32_t bitmask, uint8_t value) {
    err_code_t srds_err_code = ERR_CODE_NONE;
    uint16_t misc_val = 0;
...
err_code_t
blackhawk7_l8p2_tsc_usr_misc_ctrl_get(phymod_access_t *sa__, uint32_t bitmask, uint8_t *value) {
    err_code_t srds_err_code = ERR_CODE_NONE;
    uint16_t misc_val = 0;
...
err_code_t
blackhawk7_l8p2_rx_pmd_lock_status_get(phymod_access_t *sa__, uint32_t *pmd_lock, uint32_t *pmd_lock_change)
{
...
err_code_t
blackhawk7_l8p2_signal_detect_status_get(phymod_access_t *sa__, uint32_t *sd_bmp, uint32_t *sd_change_bmp)
{
    int start_lane, num_lane, lane_offset;
    phymod_access_t phy_access_copy;
...
err_code_t blackhawk7_l8p2_tx_pcs_intf_dly_mode_en(phymod_access_t *sa__, uint8_t en)
{
    err_code_t srds_err_code = ERR_CODE_NONE;
    uint8_t reset_state;
    reset_state = rd_rx_lane_dp_reset_state();
    if(reset_state < 7) {
        EFUN_PRINTF(("ERROR: enable tx_pcs_intf_dly_mode without ln_dp_s_rstb=0\n"));
        return ERR_CODE_LANE_DP_NOT_RESET;
    }
    PHYMOD_EFUN_WR(wr_tx_pcs_intf_dly_mode(en));
    return ERR_CODE_NONE;
}

err_code_t
blackhawk7_l8p2_rx_dc_offset_get(phymod_access_t *sa__, uint32_t *dc_offset)
{
    err_code_t srds_err_code = ERR_CODE_NONE;
