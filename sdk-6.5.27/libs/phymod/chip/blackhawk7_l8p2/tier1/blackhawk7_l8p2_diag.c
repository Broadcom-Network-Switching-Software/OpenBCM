...
err_code_t blackhawk7_l8p2_test_uc_timestamp(srds_access_t *sa__) {
    return blackhawk7_l8p2_INTERNAL_test_uc_timestamp_with_print_options(sa__, 0xFF);
}
...
err_code_t blackhawk7_l8p2_log_api_event(srds_access_t *sa__, struct blackhawk7_l8p2_event_st api_event) {
    ...
    if(blackhawk7_l8p2_info_ptr->info_table_version > 0x38) {
        ...
        EFUN(blackhawk7_l8p2_pmd_uc_cmd_with_data(sa__, CMD_API_EVENT, api_event.event_id, data_new, GRACEFUL_STOP_TIME));
    }
    return(ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    INIT_SRDS_ERR_CODE
    uint8_t lane = blackhawk7_l8p2_acc_get_lane(sa__), lane_select=0;

    if (lane < 8) {
        lane_select = (uint8_t)(0x1<<lane);
        EFUN(blackhawk7_l8p2_display_diag_data_core(sa__, diag_level, lane_select));
    }
    else {
...
err_code_t blackhawk7_l8p2_display_diag_data_core(srds_access_t *sa__, uint16_t  diag_level, uint8_t lane_select) {
    ...
    EFUN_PRINTF(("\n"));
    ESTM(num_lanes = rdc_revid_multiplicity());
    save_lane = blackhawk7_l8p2_acc_get_lane(sa__);
    ...
    for (lane=0; lane<num_lanes; lane++) {
        if (lane_select & (0x1<<lane)) {
            ...
            micro_stop[lane] = blackhawk7_l8p2_INTERNAL_stop_micro(sa__,rx_lock,&err_code);
            if(err_code) USR_PRINTF(("Unable to stop lane %d micro,  following data is suspect\n", lane));
        }
    }
    EFUN(blackhawk7_l8p2_acc_set_lane(sa__, save_lane));
...
Exit:
    ...
    if (diag_level & SRDS_DIAG_REG_CORE) {
        diag_data_err = blackhawk7_l8p2_reg_select_dump(sa__, SRDS_CORE_REG_ONLY, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if (diag_level & SRDS_DIAG_REG_LANE) {
        ...
        for (lane=0; lane<num_lanes; lane++) {
            if (lane_select & (0x1<<lane)) {
                EFUN(blackhawk7_l8p2_acc_set_lane(sa__, lane));
                diag_data_err = blackhawk7_l8p2_reg_select_dump(sa__, SRDS_LANE_REG_ONLY, NULL);
                if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        data_cnt = (uint16_t)blackhawk7_l8p2_info_ptr->core_var_ram_size;

        diag_data_err = blackhawk7_l8p2_uc_core_var_blk_dump(sa__, core, data_cnt, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
        ...
        save_lane = blackhawk7_l8p2_acc_get_physical_lane(sa__);
        for (lane = 0; lane < num_micros*2; lane =(uint8_t)(lane + 2)) {
            EFUN(blackhawk7_l8p2_acc_set_physical_lane(sa__, lane));

            diag_data_err = blackhawk7_l8p2_uc_micro_var_blk_dump(sa__, core, lane, data_cnt, NULL);
            if(diag_data_err != ERR_CODE_NONE) {
                EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_UC_CORE returned error %s ****\n", blackhawk7_l8p2_INTERNAL_e2s_err_code(diag_data_err)));
                diag_level_error |= SRDS_DIAG_UC_CORE;
            }
        }
        EFUN(blackhawk7_l8p2_acc_set_physical_lane(sa__, save_lane));
        }
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        ...
        for (lane=0; lane<num_lanes; lane++) {
            if (lane_select & (0x1<<lane)) {
                EFUN(blackhawk7_l8p2_acc_set_lane(sa__, lane));
                diag_data_err = blackhawk7_l8p2_uc_lane_var_blk_dump(sa__, core, lane, data_cnt, NULL);
                if(diag_data_err != ERR_CODE_NONE) {
    ...
    if((diag_level & SRDS_DIAG_PRBS_PROJ)
        ...
        diag_data_err = blackhawk7_l8p2_display_aggregate_prbs_error_analyzer_proj(sa__, 5440, SRDS_IEEE_50GE, lane_mask_aggregate, timeout_s, diag_mode);
        if(diag_data_err != ERR_CODE_NONE) {
