...
#include "tscbh_gen2/tier1/tbhmod_gen4_1588_lkup_table.h"
...
#define TSCBH_GEN4_SERDES_ID           0x30
    ...
    if (PHYID2r_REGID1f_GET(id2) == TSCBH_ID0 &&
       (PHYID3r_REGID2f_GET(id3) == TSCBH_ID1)) {
        ...
        if ((MAIN0_SERDESIDr_MODEL_NUMBERf_GET(serdesid)) == TSCBH_GEN2_SERDES_ID ||
            (MAIN0_SERDESIDr_MODEL_NUMBERf_GET(serdesid)) == TSCBH_GEN4_SERDES_ID)  {
            *is_identified = 1;
        }
    }
...
int tscbh_gen2_phy_firmware_lane_config_get(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t* fw_config)
{
    ...
    phymod_phy_access_t pm_phy_copy;
    uint8_t force_nr_in_an = 0;
    uint8_t force_er_in_an = 0;
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_tsc_usr_misc_ctrl_get(&pm_phy_copy.access,
                                                BLACKHAWK7_L8P2_USR_MISC_CTRL_FORCE_NR,
                                                &force_nr_in_an));

    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_tsc_usr_misc_ctrl_get(&pm_phy_copy.access,
                                                BLACKHAWK7_L8P2_USR_MISC_CTRL_FORCE_ER,
                                                &force_er_in_an));
    fw_config->AnForceNormalReach = force_nr_in_an;
    fw_config->AnForceExtendedReach = force_er_in_an;
...
int _tscbh_gen2_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        if (!is_warm_boot) {
            ...
            /* next program the force NR and ER for AN  */
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_l8p2_tsc_usr_misc_ctrl_set(&phy_copy.access,
                                                       BLACKHAWK7_L8P2_USR_MISC_CTRL_FORCE_NR,
                                                       fw_config.AnForceNormalReach));
            PHYMOD_IF_ERR_RETURN
                (blackhawk7_l8p2_tsc_usr_misc_ctrl_set(&phy_copy.access,
                                                       BLACKHAWK7_L8P2_USR_MISC_CTRL_FORCE_ER,
                                                       fw_config.AnForceExtendedReach));
                                   ...
                                   phymod_phy_pll_state_t* new_pll_state)
{
    ...
    *new_pll_state = *old_pll_state;
    ...
    /* next read fw force ER/NR in AN */
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_tsc_usr_misc_ctrl_get(&pm_phy_copy.access,
                                                BLACKHAWK7_L8P2_USR_MISC_CTRL_FORCE_NR,
                                                &force_nr_in_an));
    ...
    PHYMOD_IF_ERR_RETURN
        (blackhawk7_l8p2_tsc_usr_misc_ctrl_get(&pm_phy_copy.access,
                                                BLACKHAWK7_L8P2_USR_MISC_CTRL_FORCE_ER,
                                                &force_er_in_an));
    firmware_lane_config.AnForceExtendedReach = force_nr_in_an;
    firmware_lane_config.AnForceNormalReach = force_er_in_an;
                                   ...
                                   const phymod_timesync_adjust_config_info_t *config)
{
    ...
    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN
        (tbhmod_model_num_get(&phy_copy.access, &model_num));

    if (model_num == TSCBH_GEN4_SERDES_ID) {
        tbhmod_gen2_ts_table_tx_sop = tbhmod_gen4_ts_table_tx_sop_get();
        tbhmod_gen2_ts_table_rx_sop = tbhmod_gen4_ts_table_rx_sop_get();
        tbhmod_gen2_ts_table_tx_sfd = tbhmod_gen4_ts_table_tx_sfd_get();
        tbhmod_gen2_ts_table_rx_sfd = tbhmod_gen4_ts_table_rx_sfd_get();
    } else {
    ...
    if (model_num == TSCBH_GEN4_SERDES_ID) {
        PHYMOD_IF_ERR_RETURN
           (tbhmod_gen4_1588_table_index_get(mapped_speed_id, speed_config.fec_type, &ts_table_index, &ts_table_size));
    } else {
        PHYMOD_IF_ERR_RETURN
