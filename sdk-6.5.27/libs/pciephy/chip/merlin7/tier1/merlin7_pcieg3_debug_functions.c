...
static err_code_t _merlin7_pcieg3_reg_print_no_buf(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr);
static err_code_t _merlin7_pcieg3_reg_print_with_buf(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr, char reg_buffer[SRDS_DUMP_BUF_SIZE]);
static err_code_t _merlin7_pcieg3_reg_print(srds_access_t *sa__, uint8_t reg_start_index, uint8_t reg_end_index, char reg_buffer[][SRDS_DUMP_BUF_SIZE], uint8_t *buf_index);
...
static err_code_t _merlin7_pcieg3_reg_print_no_buf(srds_access_t *sa__, uint16_t start_addr, uint16_t valid_addr) {
    ...
    for (offset = 0; offset < 0x10; offset++) {
        if(valid_addr & (0x1 << offset)) {
            EFUN(merlin7_pcieg3_acc_rdt_reg(sa__, (uint16_t)(start_addr + offset), &rddata));
        }
        else {
            rddata = 0x0;
        }
...
static err_code_t _merlin7_pcieg3_reg_print(srds_access_t *sa__, uint8_t reg_start_index, uint8_t reg_end_index, char reg_buffer[][SRDS_DUMP_BUF_SIZE], uint8_t *buf_index) {
    INIT_SRDS_ERR_CODE
    uint8_t reg_section;
    merlin7_pcieg3_reg_dump_arr_t *merlin7_pcieg3_reg_dump_arr = merlin7_pcieg3_get_reg_dump_arr();
    if(reg_buffer == NULL) {
        for(reg_section = reg_start_index; reg_section < reg_end_index; reg_section++) {
            EFUN(_merlin7_pcieg3_reg_print_no_buf(sa__, merlin7_pcieg3_reg_dump_arr[reg_section][0], merlin7_pcieg3_reg_dump_arr[reg_section][1]));
        }
    }
    else {
        for(reg_section = reg_start_index; reg_section < reg_end_index; reg_section++) {
            EFUN(_merlin7_pcieg3_reg_print_with_buf(sa__, merlin7_pcieg3_reg_dump_arr[reg_section][0], merlin7_pcieg3_reg_dump_arr[reg_section][1], reg_buffer[*buf_index]));
            (*buf_index)++;
        }
    }
    return (ERR_CODE_NONE);
...
err_code_t merlin7_pcieg3_reg_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char reg_buffer[][SRDS_DUMP_BUF_SIZE]) {
    ...
    UNUSED(core);
    UNUSED(lane);
    EFUN(merlin7_pcieg3_reg_select_dump(sa__, SRDS_LANE_AND_CORE_REG, reg_buffer));
    return (ERR_CODE_NONE);
}

err_code_t merlin7_pcieg3_reg_select_dump(srds_access_t *sa__, uint8_t reg_dump_control, char reg_buffer[][SRDS_DUMP_BUF_SIZE]) {
    INIT_SRDS_ERR_CODE
    uint8_t core = merlin7_pcieg3_acc_get_core(sa__);
    uint8_t lane = merlin7_pcieg3_acc_get_lane(sa__);
    uint8_t buf_index = 0;
    ...
    if (reg_dump_control & SRDS_LANE_REG_ONLY) {
        /* Lane Registers */
        EFUN(_merlin7_pcieg3_reg_print(sa__, 0, MERLIN7_PCIEG3_REG_CORE_START_INDEX, reg_buffer, &buf_index));
    }
    if (reg_dump_control & SRDS_CORE_REG_ONLY) {
        /* Core Registers */
        EFUN(_merlin7_pcieg3_reg_print(sa__, MERLIN7_PCIEG3_REG_CORE_START_INDEX, MERLIN7_PCIEG3_REG_PLL_START_INDEX, reg_buffer, &buf_index));

        /* Core PLL Registers */
        ESTM(pll_orig = merlin7_pcieg3_acc_get_pll_idx(sa__));
        for (pll_idx = 0; pll_idx < NUM_PLLS; pll_idx++) {
            EFUN(merlin7_pcieg3_acc_set_pll_idx(sa__,pll_idx));
            EFUN(_merlin7_pcieg3_reg_print(sa__, MERLIN7_PCIEG3_REG_PLL_START_INDEX, MERLIN7_PCIEG3_REG_UC_START_INDEX, reg_buffer, &buf_index));
        }
        ...
        for(micro_idx = 0; micro_idx < num_micros; micro_idx++) {
            ...
            EFUN(_merlin7_pcieg3_reg_print(sa__, MERLIN7_PCIEG3_REG_UC_START_INDEX, MERLIN7_PCIEG3_REG_DUMP_SECTIONS, reg_buffer, &buf_index));
        }
...
err_code_t merlin7_pcieg3_uc_core_var_blk_dump(srds_access_t *sa__,uint8_t core, uint16_t data_buff_size,uint8_t *data_buffer) {
      ...
      merlin7_pcieg3_INTERNAL_rdblk_ram_arg_t rdblk_state;
      merlin7_pcieg3_INTERNAL_ram_dump_state_t state;
      uint32_t core_var_ram_size = 0;
      EFUN(merlin7_pcieg3_INTERNAL_match_ucode_from_info(sa__, merlin7_pcieg3_info_ptr));
      core_var_ram_size = (uint32_t)merlin7_pcieg3_info_ptr->core_var_ram_size;
      state.index = 0;
      ...
      EFUN(merlin7_pcieg3_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                      ...
                                                      merlin7_pcieg3_info_ptr->core_var_ram_size,
                                                      &rdblk_state,
                                                      merlin7_pcieg3_INTERNAL_rdblk_ram_read_callback));
...
err_code_t merlin7_pcieg3_uc_lane_var_blk_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, uint32_t data_buff_size,uint8_t *data_buffer) {
    ...
    lane_addr_offset = _merlin7_pcieg3_INTERNAL_get_addr_from_lane(sa__, 0 ,lane);
...
err_code_t merlin7_pcieg3_isolate_ctrl_pins(srds_access_t *sa__, uint8_t enable) {
    ...
    ESTM(lane_orig = merlin7_pcieg3_acc_get_lane(sa__));
...
err_code_t merlin7_pcieg3_isolate_lane_ctrl_pins(srds_access_t *sa__, uint8_t enable) {
    ...
    EFUN(merlin7_pcieg3_isolate_lane_ctrl_tx_pins(sa__, enable));
    EFUN(merlin7_pcieg3_isolate_lane_ctrl_rx_pins(sa__, enable));
    return (ERR_CODE_NONE);
}
...
void merlin7_pcieg3_INTERNAL_print_triage_info(srds_access_t *sa__, err_code_t err_code, uint8_t print_header, uint8_t print_data, uint16_t line)
{
    ...
    if (merlin7_pcieg3_version(sa__, &info.api_ver)) {  /* Unable to read api version */
    ...
    if (merlin7_pcieg3_pmd_lock_status(sa__, &info.pmd_lock)) {
        error_seen = 1;
    }
    ...
    info.core = merlin7_pcieg3_acc_get_core(sa__);
    info.lane = merlin7_pcieg3_acc_get_lane(sa__);
    if (print_data) {
        if ((err_code == ERR_CODE_UC_CMD_POLLING_TIMEOUT) || (err_code == ERR_CODE_UC_NOT_STOPPED)) {
