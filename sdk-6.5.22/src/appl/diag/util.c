    ...
    for (qual = 0; qual < bcmFieldQualifyCount; qual++) {
        if(BCM_FIELD_QSET_TEST(qset, qual)) {
            if (first_print == 1) {
                ...
                sal_strncat(buf, separator, BCM_FIELD_QSET_WIDTH_MAX - sal_strlen(buf) - 1);
            }
            format_field_qualifier(buf_local, qual, 1);
            sal_strncat(buf, buf_local, BCM_FIELD_QSET_WIDTH_MAX - sal_strlen(buf) - 1);
        }
    }
    ...
    for (action = 0; action < bcmFieldActionCount; action++) {
        if(BCM_FIELD_ASET_TEST(aset, action)) {
            if (first_print == 1) {
                ...
                sal_strncat(buf, separator, BCM_FIELD_ASET_WIDTH_MAX - sal_strlen(buf) - 1);
            }
            format_field_action(buf_local, action, 1);
            sal_strncat(buf, buf_local, BCM_FIELD_ASET_WIDTH_MAX - sal_strlen(buf) - 1);
        }
    }
    sal_strncat(buf, "}", sal_strlen("}")+1);
