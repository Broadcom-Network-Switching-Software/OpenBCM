...
/* For now only TH3 supports Fast Reboot, so only allow TH3 to set flags */
    ...
    /* Store the current version in HA memory, and to be used as the start
     * version in the following ISSU. */
    if ((sysconf_ltsw_ha_curr_ver_store()) < 0) {
        cli_out("ERROR: Failed to set the software version.\n");
    }
#endif
    if (warm_boot) {
#if defined(BCM_LTSW_SUPPORT) || defined(BCM_CTSW_SUPPORT)
