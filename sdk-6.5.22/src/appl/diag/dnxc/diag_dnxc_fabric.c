...
#ifdef BCM_DNX_SUPPORT
        if (SOC_IS_DNX(unit) &&
            dnx_data_fabric.general.feature_get(unit, dnx_data_fabric_general_is_symb_errors_per_frame_supported))
        {
            SHR_IF_ERR_EXIT(dnx_fabric_if_fmac_symb_error_get(unit, logical_port, &symbol_err));
            is_uc_err = SHR_BITGET(&symbol_err, DNX_15T_FEC_UNCORRECTABLE_ERROR_INDICATION_BIT_IN_DBAL);
            SHR_BITCLR(&symbol_err, DNX_15T_FEC_UNCORRECTABLE_ERROR_INDICATION_BIT_IN_DBAL);
            SHR_BITCLR(&symbol_err, 0);
            _SHR_PBMP_CLEAR(symbol_err_pbmp);
            _SHR_PBMP_WORD_SET(symbol_err_pbmp, 0, symbol_err);
            shr_pbmp_range_format(symbol_err_pbmp, range_format_buf, sizeof(range_format_buf));
