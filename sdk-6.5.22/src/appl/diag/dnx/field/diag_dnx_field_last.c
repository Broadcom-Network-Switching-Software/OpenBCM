 ...
 * }
 */
static bcm_field_group_t tcam_fg_id1 = BCM_FIELD_ID_INVALID;
static bcm_field_group_t tcam_fg_id2 = BCM_FIELD_ID_INVALID;
    ...
    int unit)
{
    ...
    sal_strncpy_s(dest_char, "field_last_info_diag_epmf_key_A", sizeof(fg_info.name));
    ...
    SHR_IF_ERR_EXIT(bcm_field_group_add
                    (unit, BCM_FIELD_FLAG_WITH_ID | BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &exem_fg_id));
    ...
    /*
     * Add first TCAM field group to occupy key B
     */
    bcm_field_group_info_t_init(&fg_info);
    dest_char = &(fg_info.name[0]);
    sal_strncpy_s(dest_char, "field_last_info_diag_epmf_key_B", sizeof(fg_info.name));
    fg_info.stage = bcmFieldStageEgress;
    ...
    SHR_IF_ERR_EXIT(bcm_field_group_add
                    (unit, BCM_FIELD_FLAG_WITH_ID | BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &tcam_fg_id1));
    bcm_field_group_attach_info_t_init(&attach_info);
    attach_info.key_info.nof_quals = fg_info.nof_quals;
    attach_info.key_info.qual_types[0] = fg_info.qual_types[0];
    attach_info.payload_info.nof_actions = fg_info.nof_actions;
    ...
    attach_info.key_info.qual_info[0].input_arg = 1;
    attach_info.key_info.qual_info[0].offset = 0;
    SHR_IF_ERR_EXIT(bcm_field_group_context_attach(unit, 0, tcam_fg_id1, 0 /* default context */ , &attach_info));
    ...
    bcm_field_group_info_t_init(&fg_info);
    dest_char = &(fg_info.name[0]);
    sal_strncpy_s(dest_char, "field_last_info_diag_epmf_key_C", sizeof(fg_info.name));
    fg_info.stage = bcmFieldStageEgress;
    fg_info.fg_type = bcmFieldGroupTypeTcam;
    fg_info.nof_quals = 1;
    ...
    SHR_IF_ERR_EXIT(bcm_field_group_add
                    (unit, BCM_FIELD_FLAG_WITH_ID | BCM_FIELD_FLAG_MSB_RESULT_ALIGN, &fg_info, &tcam_fg_id2));
    bcm_field_group_attach_info_t_init(&attach_info);
    attach_info.key_info.nof_quals = fg_info.nof_quals;
    attach_info.key_info.qual_types[0] = fg_info.qual_types[0];
    attach_info.payload_info.nof_actions = fg_info.nof_actions;
    ...
    attach_info.key_info.qual_info[0].input_arg = 1;
    attach_info.key_info.qual_info[0].offset = 0;
    SHR_IF_ERR_EXIT(bcm_field_group_context_attach(unit, 0, tcam_fg_id2, 0 /* default context */ , &attach_info));

    /*
     * Detach first TCAM field group to free key B
     */
    SHR_IF_ERR_EXIT(bcm_field_group_context_detach(unit, tcam_fg_id1, 0 /* default context */ ));
exit:
    SHR_FUNC_EXIT;
}
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(bcm_field_group_context_detach(unit, tcam_fg_id2, 0 /* default context */ ));
    SHR_IF_ERR_EXIT(bcm_field_group_context_detach(unit, exem_fg_id, 0 /* default context */ ));

    SHR_IF_ERR_EXIT(bcm_field_group_delete(unit, tcam_fg_id1));
    SHR_IF_ERR_EXIT(bcm_field_group_delete(unit, tcam_fg_id2));
    SHR_IF_ERR_EXIT(bcm_field_group_delete(unit, exem_fg_id));

    tcam_fg_id1 = BCM_FIELD_ID_INVALID;
    tcam_fg_id2 = BCM_FIELD_ID_INVALID;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    *context_id_p = (dnx_field_context_t) context_id;
    ...
    if (field_stage == DNX_FIELD_STAGE_EXTERNAL)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_opcode_to_apptype_dnx_to_bcm(unit, opcode_id_uint8, &apptype));
    ...
    if (DIAG_DNX_FIELD_LAST_TCAM_HIT_INDICATION_ENABLE)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, field_stage, &field_map_stage_info));

        /** Get the presel ID which was hit. */
        SHR_IF_ERR_EXIT(diag_dnx_field_last_cs_hit_info_get
                        (unit, field_map_stage_info.cs_hit_indication_table_id, &hit_presel_id));
    ...
    */
    ...
    if (field_stage == DNX_FIELD_STAGE_IPMF1)
    {
            ...
            context_mode.context_ipmf1_mode.hash_mode != DNX_FIELD_CONTEXT_HASH_MODE_DISABLED)
        {
             ...
             */
            mode_string[sal_strnlen(mode_string, sizeof(mode_string)) - 2] = '\0';
    ...
    uint8 *is_entry_found_p)
{
    ...
    *is_entry_found_p = FALSE;
     ...
     */
    entry_handle = DNX_FIELD_ENTRY_TCAM_DT(last_diag_fg_info_p[fg_id_index].key_values[0][0], fg_id, core_id);
    ...
    prt_control_t * prt_ctr)
{
    ...
    for (dnx_fem_id = *fem_start_index_p; dnx_fem_id < max_nof_fems; dnx_fem_id++)
    {
        ...
        for (fem_program_index = 0; fem_program_index < fem_program_max; fem_program_index++)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, fg_stage, &field_map_stage_info));
            ...
            for (sig_iter = 0; sig_iter < DNX_DATA_MAX_FIELD_DIAG_NOF_SIGNALS_PER_ACTION; sig_iter++)
            {
                ...
                if (field_map_stage_info.static_action_info[dnx_action_id].signal_name[sig_iter] == 0)
                {
                    break;
                }
                ...
                rv = sand_signal_output_find
                    (unit, core_id, 0, block_p, from_p, to_p,
                     field_map_stage_info.static_action_info[dnx_action_id].signal_name[sig_iter], &signal_output);

                if (rv == _SHR_E_NOT_FOUND)
                {
                    DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(NULL, "%s: %s\n", field_map_stage_info.static_action_info
                                                          [dnx_action_id].signal_name[sig_iter], "Signal not found");
                    sal_snprintf(act_val_buff, sizeof(act_val_buff), "%s: %s\n",
                                 field_map_stage_info.static_action_info[dnx_action_id].signal_name[sig_iter],
                                 "Signal not found");
                    DIAG_DNX_FIELD_UTILS_STR_INPUT_VERIFY(action_val_string, "%s", act_val_buff);
                    sal_strncat_s(action_val_string, act_val_buff, sizeof(action_val_string));
                }
