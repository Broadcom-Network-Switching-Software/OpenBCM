    ...
    xd_t * xd)
{
    ...
    if (xd->xd_ptch_src_port != BCM_PORT_INVALID)
    {
        ...
        if (DNX_PORT_IS_COE_PORT(flags))
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_algo_port_channel_get(xd->xd_tx_unit, port_ndx, &cpu_channel));
                if (cpu_channel == 0)
                {
        ...
        else if (DNX_PORT_IS_PON_PORT(flags))
        {
            /*
             * In the case of PON port
             * 1. inject packet without PTCH and following headers
             */
            is_no_ptch = TRUE;
        }
    ...
    if ((header_type_incoming_valid) || (ibch1_enable))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_channel_get(xd->xd_tx_unit, xd->xd_in_tm_port, &cpu_channel));
