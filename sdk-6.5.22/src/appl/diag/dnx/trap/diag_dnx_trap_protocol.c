    ...
    int unit)
{
    ...
    /** Configure ICMP profile for RIF */
    SHR_IF_ERR_EXIT(diag_dnx_rx_trap_eth_inrif_create_example(unit, &intf));

    protocol_profiles.icmpv4_trap_profile = 3;

    SHR_IF_ERR_EXIT(bcm_rx_trap_protocol_interface_profiles_set(unit, intf, &protocol_profiles));
    ...
    prt_control_t * prt_ctr)
{
    ...
    bcm_rx_trap_t protocol_types[] = { bcmRxTrapL2cpPeer, bcmRxTrapIcmpRedirect, bcmRxTrap8021xFail,
        ...
        bcmRxTrapDhcpv4Client, bcmRxTrapDhcpv4Server, bcmRxTrapDhcpv6Client, bcmRxTrapDhcpv6Server,
        bcmRxTrapArp, bcmRxTrapArpMyIp, bcmRxTrapCount
    };
    ...
    for (protocol_index = 0; protocol_types[protocol_index] != bcmRxTrapCount; protocol_index++)
    {
        ...
        for (trap_arg_index = 0; trap_arg_index < nof_trap_args[protocol_index]; trap_arg_index++)
        {
            ...
            switch (protocol_types[protocol_index])
            {
                ...
                {
                    protocol_name = "IGMP Membership Query";
                    key.protocol_trap_profile = protocol_profiles.igmp_trap_profile;
                    break;
                }
                case bcmRxTrapIgmpReportLeaveMsg:
                {
                    protocol_name = "IGMP Report Leave Msg";
                    key.protocol_trap_profile = protocol_profiles.igmp_trap_profile;
                    break;
                }
                case bcmRxTrapIgmpUndefined:
                {
                    protocol_name = "IGMP Undefined";
                    key.protocol_trap_profile = protocol_profiles.igmp_trap_profile;
                    break;
                }
                case bcmRxTrapDhcpv4Client:
                {
                    protocol_name = "DHCP IPv4 Client";
                    key.protocol_trap_profile = protocol_profiles.dhcp_trap_profile;
                    break;
                }
                case bcmRxTrapDhcpv4Server:
                {
                    protocol_name = "DHCP IPv4 Server";
                    key.protocol_trap_profile = protocol_profiles.dhcp_trap_profile;
                    break;
                }
                case bcmRxTrapDhcpv6Client:
                {
                    protocol_name = "DHCP IPv6 Client";
                    key.protocol_trap_profile = protocol_profiles.dhcp_trap_profile;
                    break;
                }
                case bcmRxTrapDhcpv6Server:
                {
                    protocol_name = "DHCP IPv6 Server";
                    key.protocol_trap_profile = protocol_profiles.dhcp_trap_profile;
                    break;
                }
                case bcmRxTrapArp:
                {
                    protocol_name = "My ARP";
            ...
            if (BCM_GPORT_TRAP_GET_STRENGTH(trap_gport) != 0 || BCM_GPORT_TRAP_GET_SNOOP_STRENGTH(trap_gport) != 0)
            {
                ...
                if (add_new_protocol)
                {
                    PRT_CELL_SET("%s", protocol_name);
                    PRT_CELL_SET("%d", key.protocol_trap_profile);
    ...
    sh_sand_control_t * sand_control)
{
    ...
    bcm_rx_trap_protocol_profiles_t protocol_profiles;
    uint32 entry_handle_id = 0;
    uint32 dbal_tables[] = { DBAL_TABLE_IN_AC_INFO_DB, DBAL_TABLE_INNER_ETH_VLAN_EDIT_CLASSIFICATION,
