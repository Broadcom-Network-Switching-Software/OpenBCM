...
kleap_stages_e
dnx_ikleap_adapter_stage_to_ikleap_stage(
    int unit,
    int adapter_stage_index)
{
    return adapter_stage_to_ikleap_stage[adapter_stage_index];
}
    ...
    char *res)
{
    ...
    ikleap_stage = dnx_ikleap_adapter_stage_to_ikleap_stage(unit, adapter_stage_index);

    if (kbr_index < 0 || kbr_index > MAX_NUM_KBR || kbr_index >= dnx_ikleap_stages_get(unit, ikleap_stage).nof_kbrs)
    {
        /*
         * Illegal Kbr
         */
    ...
    /*
     * See "fixed_key_lookup" array defined previously in this file
     */
    if (adapter_stage_index == 6 && kbr_index == 6)
    {
        sal_sprintf(res, "%s_%d", "Learning", interface_index);
        SHR_EXIT();
    }
    else if (adapter_stage_index == 6 && (kbr_index == 12 || kbr_index == 13))
    {
