    ...
    sh_sand_control_t * sand_control)
{
    ...
    {
        int ii;
        char access_type_str[DBAL_MAX_STRING_LENGTH];
        LOG_CLI((BSL_META("PEMLA Mapping:\n")));
        if (table->table_type == DBAL_TABLE_TYPE_DIRECT)
        {
            char field_str[DBAL_MAX_STRING_LENGTH];
            dbal_table_field_info_t *field_info;
            if (table->pemla_mapping.access_type == DBAL_HL_ACCESS_REGISTER)
            {
                sal_strncpy(access_type_str, "Register", DBAL_MAX_STRING_LENGTH);
            }
            else
            {
                sal_strncpy(access_type_str, "Memory", DBAL_MAX_STRING_LENGTH);
            }
            LOG_CLI((BSL_META("  Access Type: %s\n"), access_type_str));
            LOG_CLI((BSL_META("  %s ID %d\n"), access_type_str, table->app_id));
