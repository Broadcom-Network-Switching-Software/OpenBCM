    ...
    int is_oh)
{
    ...
    for (ii=0; ii<disp_num; ii++)
    {
        ...
        {
            sal_snprintf(time_slot_string + sal_strnlen(time_slot_string,SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH),
                        string_size - sal_strnlen(time_slot_string,SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH), "%-4d ",
                        time_slot[disp_start+ii]);
        }
    }
    ...
    int *tx_speed)
{
    ...
    bcm_port_flexe_group_cal_t active_cal;
    int cal_slots[bcmPortFlexeGroupCalCount][DNX_DATA_MAX_NIF_FLEXE_MAX_NOF_SLOTS];
    ...
    *tx_speed = 0;
    ...
    for (i = 0; i < sizeof(speed_direction) / sizeof(int); ++i)
    {
        for (group_index = 0; group_index < dnx_data_nif.flexe.nof_groups_get(unit); ++group_index)
        {
            ...
            {
                SHR_IF_ERR_EXIT(bcm_port_flexe_group_cal_slots_get(unit, group_gport, speed_direction[i], cal_index, dnx_data_nif.flexe.max_nof_slots_get(unit), cal_slots[cal_index], &actual_nof_slots));
                for (slot_index = 0; slot_index < actual_nof_slots; ++slot_index)
                {
                    if (cal_slots[cal_index][slot_index] == port)
                    {
            ...
            if ((nof_slots_in_cal[bcmPortFlexeGroupCalA] != 0) || (nof_slots_in_cal[bcmPortFlexeGroupCalB] != 0))
            {
                ...
                {
                    *rx_speed = nof_slots_in_cal[active_cal] * dnx_data_nif.flexe.client_speed_granularity_get(unit);
                }
                else
                {
                    *tx_speed = nof_slots_in_cal[active_cal] * dnx_data_nif.flexe.client_speed_granularity_get(unit);
                }
                break;
            }
        }
...
dnx_diag_flexe_client_dump_cmd(
    ...
    sh_sand_control_t * sand_control)
{
    int port, enable, status, rx_speed, tx_speed, i;
    ...
    sh_sand_control_t * sand_control)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_core_port_get(unit, flexe_port, &phy_instance));
...
sh_sand_cmd_t sh_dnx_diag_flexe_cmds[] = {
    ...
    {"PTPDelay", dnx_diag_flexe_ptp_cmd,              NULL,                         dnx_diag_flexe_ptp_options,     &dnx_diag_flexe_ptp_man,      NULL, NULL, SH_CMD_CONDITIONAL | SH_CMD_NOT_RELEASE, sh_cmd_is_flexe_enable},
