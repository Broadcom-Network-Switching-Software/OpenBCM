...
dnx_kbp_diag_sdk_ver_cmd(
    ...
    sh_sand_control_t * sand_control)
{
    const char *ver_str_expected = "KBP SDK 1.5.15";
    ...
    {
        if (sal_strncmp
            (ver_str_expected, ver_str, sal_strnlen(ver_str_expected, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH)))
        {
    ...
    sh_sand_control_t * sand_control)
{
    ...
    LOG_CLI((BSL_META("kbp search result for opcode %s is HITBIT[%d,%d,%d,%d,%d,%d,%d,%d]\n"), opcode_name,
             search_rslt.hit_or_miss[0], search_rslt.hit_or_miss[1], search_rslt.hit_or_miss[2],
