    ...
    int *base_voq_connector)
{
    ...
    int voq_conn_id, num_of_512, voq_conn_internal_id, port_index;

    SHR_FUNC_INIT_VARS(unit);

    /** Calculate a unique index for the given dst_port info (modid_index and ftmh_pp_dsp) */
    port_index = dst_port_internal_modid_index*APPL_SAND_NOF_PORTS_PER_MODID + dst_port_ftmh_pp_dsp;
    ...
    *base_voq_connector = (num_of_512 * 1024) + (APPL_SAND_E2E_SCHEME_CORE_ID2INDEX(ingress_core_id) * 512) +
