...
/**
* \brief
* Field App for update egress estimated byte to remove when the forward layer is ethernet
* \param [in] unit        - Device id
* \return
*   int - Error Type
* \remark
...
*   * None
*/
shr_error_e appl_dnx_field_eth_fwd_layer_ebtr(
    int unit);
...
/**
 * \brief
 *   This functions determines if the app should be skipped
 * \param [in] unit - The unit number.
 * \param [out] dynamic_flags - indicate if the application should be skipped or not
 * \return
 *   Error code (as per 'shr_error_e').
 ...
 *   shr_error_e
 */
shr_error_e appl_dnx_field_ebtr_eth_layer_cb(
    int unit,
    int *dynamic_flags);
