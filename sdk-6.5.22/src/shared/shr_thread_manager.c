...
static void shr_thread_manager_thread(shr_thread_manager_object_t* obj)
{
    ...
    else
    {
        ...
        /* we need this seperate indicator as a volatile flag (it is marked with the volatile keyword), volatile hints the compiler that this variable may unexpectedly be changed by a different thread so it must be read from memory and not from cache*/
        obj->is_active = 0;
    }
...
static int is_thread_created(shr_thread_manager_handler_t h) 
{
    ...
    /* The obj->is_active is marked as vplatile and is used instead of relying on obj->tid in order to avoid compiler's optimizations that may cause obj->tid
       to have a non up to date value if it was set by a different thread than the current thread */
    if(obj == NULL || obj->is_active == 0) {
