...
int sat_gtf_setup(int unit, int inP, int outP, bcm_sat_gtf_t *gtf_id) {
    ...
    for (is_eir=0; is_eir<=sat_info.is_paired; ++is_eir)
    {
         ...
         *     a. gtf_id=even GTF ID, priority=bcmSatGtfPriEir
         *     b. gtf_id=odd GTF ID, priority=bcmSatGtfPriCir
         */
...
int sat_ctf_setup(int unit, int rx_port, int sat_port, bcm_sat_ctf_t *ctf_id, int *trap_id) {
    ...
    rv = sat_ctf_general_config(unit, ctf_id, sat_info.pri[0], sat_info.dp, sat_info.trap_index, sat_info.trap_qualifier, sat_info.session_id);
    if (BCM_E_NONE != rv) {
...
int sat_setup(int unit, int inP, int outP) {
    ...
    if (sat_info.is_paired)
    {
         ...
         *     TC = sat_info.pri[0]+1
         */
        rv = sat_ctf_general_config(unit, &sat_info.ctf_id[1], sat_info.pri[0]+1, sat_info.dp, sat_info.trap_index, sat_info.trap_qualifier, sat_info.session_id);
        if (BCM_E_NONE != rv) {
            printf("Error in sat_ctf_general_config: %d\n", rv);
            return rv;
        }
