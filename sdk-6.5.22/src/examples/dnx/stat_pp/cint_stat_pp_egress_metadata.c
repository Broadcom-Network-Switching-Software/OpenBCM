 ...
 * 
 ...
 Copyright 2007-2021 Broadcom Inc. All rights reserved. File: cint_stat_pp_egress_metadata.c Purpose: Statistics examples for SDK egress statistics and metadata APIs using an L2 application.
 */
    ...
    int mpls_label)
{
    ...
    if (!sand_utils_is_jr2_system_header_mode(unit))
    {
        ...
        rv = bcm_ipmc_find(unit, &ipmc_addr);
        if (rv != BCM_E_NONE)
        {
            printf("Failed to find IPv4 MC entry (%08X) in routing table\n", ipv4_mc_addr);
            return rv;
        }

        bcm_l3_egress_t egr;
        rv = bcm_l3_egress_get(unit, ipmc_addr.l3a_intf, &egr);
        if (rv != BCM_E_NONE)
        {
            printf("Failed to find FEC entry %d\n", ipmc_addr.l3a_intf);
            return rv;
        }

        bcm_multicast_replication_t replications;

        rv = bcm_multicast_create(unit, BCM_MULTICAST_WITH_ID | BCM_MULTICAST_INGRESS_GROUP, mc_id);
        if (rv != BCM_E_NONE)
        {
            printf("Failed to create multicast group %d\n", mc_id);
            return rv;
        }
        ...
        rv = bcm_multicast_add(unit, mc_id, BCM_MULTICAST_INGRESS_GROUP, 1, &replications);
        if (rv != BCM_E_NONE)
        {
            printf("Failed to add replications to multicast group %d\n", mc_id);
            return rv;
        }
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Failed to replace existing IPMC entry 0x%08X\n", ipv4_mc_addr);
            return rv;
        }
        ...
        if (rv != BCM_E_NONE)
        {
            printf("Failed to replace existing IPv6 multicast entry\n");
            return rv;
        }
    }
