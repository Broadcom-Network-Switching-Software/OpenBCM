...
{
    int rv = BCM_E_NONE;
    uint32 ipv6_encap_size = *dnxc_data_get(unit, "lif", "out_lif", "global_ipv6_outlif_bta_sop", NULL);
    uint32 sid_encap_size = *dnxc_data_get(unit, "lif", "out_lif", "global_srv6_outlif_bta_sop", NULL);
    ...
    int skip_rif)
{
    ...
    if(skip_rif)
    {
        rv = l3__egress_only_fec__create_inner(unit, fec, 0 , encap_id_itf, gport, 0,
                                               0, 0,&encoded_fec);
        if (rv != BCM_E_NONE)
        {
 ...
 */
int
srv6_endpoint_tunnel_cross_connect_prefix_function(int unit, int next_protocol_is_l2, int is_termination)
{
    /* In this example, use a prefix less than 96b or 64b */
    bcm_ip6_t ip6_mask = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
    bcm_ip6_t ip6_mask_96_loc = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
    bcm_ip6_t ip6_mask_96_func = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0x00,0x00};
    bcm_ip6_t ip6_mask_64_loc = {0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
     ...
     */
    bcm_tunnel_terminator_t_init(&tunnel_term_set);
    tunnel_term_set.flags = BCM_TUNNEL_TERM_UP_TO_96_LOCATOR_SEGMENT_ID | BCM_TUNNEL_TERM_CROSS_CONNECT;
    tunnel_term_set.type = bcmTunnelTypeSR6;
     ...
     */
    bcm_tunnel_terminator_t_init(&tunnel_term_set);
    tunnel_term_set.flags = BCM_TUNNEL_TERM_UP_TO_64_LOCATOR_SEGMENT_ID | BCM_TUNNEL_TERM_CROSS_CONNECT;
    tunnel_term_set.type = bcmTunnelTypeSR6;
...
{
    int rv;
    char *proc_name = "srv6_gsid_endpoint_example";
    ...
    /** End-Point-1 FEC Entry - tying end-point-1 OUT-RIF with its OUT_Port, (ARP - DA and VLAN ID) */
    BCM_GPORT_LOCAL_SET(gport, out_port);
    rv = l3__egress_only_fec__create(unit, cint_srv6_tunnel_info.tunnel_fec_id[END_POINT_1_FEC_ID], cint_srv6_tunnel_info.eth_rif_intf_out[ENDPOINT_GSID1], cint_srv6_tunnel_info.tunnel_arp_id[ENDPOINT_GSID1], gport, 0);
    if (rv != BCM_E_NONE)
    {
        printf
    ...
    for (i=0; i < 4 ; i++) {
        sal_memcpy(tunnel_term_set.dip6, gsid[i], 16);
        rv = bcm_tunnel_terminator_create(unit, &tunnel_term_set);
        if(rv != BCM_E_NONE)
        {
            printf("Error bcm_tunnel_terminator_create. rv = %d, end-point-%d\n", rv, end_point_id);
                    ...
                    int next_header_is_eth)
{
    ...
    BCM_GPORT_LOCAL_SET(gport, out_port);
    rv = l3__egress_only_fec__create_inner(unit, cint_srv6_tunnel_info.tunnel_fec_id[EGRESS_FEC_ID], cint_srv6_tunnel_info.eth_rif_intf_out[EGRESS_OUT_VID], cint_srv6_tunnel_info.tunnel_arp_id[EGRESS], gport, 0, 0, 0,&egress_encoded_fec);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, create egress object FEC only\n",proc_name);
        return rv;
    ...
    srv6_gsid_tunnel_termination(unit, DIP_EGRESS, 1, cint_srv6_tunnel_info.vrf_in[EGRESS], next_header_is_eth, is_p2p, 0);
    ...
    srv6_gsid_tunnel_termination(unit, DIP_EGRESS_EOC, 1, cint_srv6_tunnel_info.vrf_in[EGRESS], next_header_is_eth, is_p2p, 1);
    if (is_p2p) {
        if (next_header_is_eth) {
            BCM_GPORT_LOCAL_SET(cc_gports.port2, out_port);
        } else {
    ...
    if (!is_p2p) {
        ...
        rv = add_host_ipv4(unit, cint_srv6_tunnel_info.route_ipv4_dip, cint_srv6_tunnel_info.vrf_in[RCH], egress_encoded_fec, cint_srv6_tunnel_info.eth_rif_intf_out[EGRESS], 0);
        if (rv != BCM_E_NONE)
        {
            printf("%s(): Error, in function add_host_ipv4(), \n",proc_name);
            return rv;
        ...
        rv = add_host_ipv6_encap_dest(unit, cint_srv6_tunnel_info.route_ipv6_dip, cint_srv6_tunnel_info.vrf_in[RCH], egress_encoded_fec, cint_srv6_tunnel_info.eth_rif_intf_out[EGRESS], 0);
        if (rv != BCM_E_NONE)
        {
        ...
        rv = add_route_ipv6(unit, cint_srv6_tunnel_info.route_ipv6_dip, cint_ip_route_basic_ipv6_mask, cint_srv6_tunnel_info.vrf_in[RCH], egress_encoded_fec);
        if (rv != BCM_E_NONE)
        {
