...
bcm_vrf_t vrf = 10;
bcm_vrf_t vrf_ta = 20;
bcm_port_t port_ta = 70;   /* loopback port on firebolt6*/
...
/* UDF */
bcm_error_t udf_create(int unit, int offset_bytes, int width_bytes, uint32 chunk_bmap, bcm_udf_abstract_pkt_format_t pkt_fmt, bcm_udf_id_t *udf_id)
{
    int rv;
...
bcm_error_t fp_udf_qset_set(int unit, bcm_udf_id_t *udf_id_arr, int num_udfs, 
...
{ 
  ...
  {
     printf("FB6 INT needs flow_init_mode=1 configuration (config.bcm). Please check this and re-run!!\n");
     return BCM_E_CONFIG;
  }
  
  uint32 flow_handle;
  uint32 encap_flow_option;
  BCM_IF_ERROR_RETURN(bcm_flow_handle_get(0,"INT",&flow_handle));
  BCM_IF_ERROR_RETURN(bcm_flow_option_id_get(0,flow_handle,
...
bcm_error_t test_setup(int unit)
{
  ...
  BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, ingPort, &gingPort));
  BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, egrPort, &gegrPort));
  BCM_IF_ERROR_RETURN(bcm_port_gport_get(unit, port_ta, &gport_ta));
