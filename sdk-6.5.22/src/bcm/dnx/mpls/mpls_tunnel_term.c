    ...
    int qos_profile)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                 DBAL_RESULT_TYPE_IN_LIF_FORMAT_BIER_IN_LIF_BIER_BFER);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GLOB_IN_LIF, INST_SINGLE, global_in_lif);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_TERMINATION_TYPE, INST_SINGLE, term_profile);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF_PROFILE, INST_SINGLE, in_lif_profile);
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_add(unit, global_lif_info.global_lif, local_inlif));
    }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_get(unit, in_global_lif, &ext_term_found));
    if (ext_term_found)
    {
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    if (_SHR_IS_FLAG_SET(info->flags2, BCM_MPLS_SWITCH2_EXTENDED_TERMINATION))
    {
        SHR_IF_ERR_EXIT(dnx_global_lif_reclassify_delete(unit, in_global_lif));
    }
