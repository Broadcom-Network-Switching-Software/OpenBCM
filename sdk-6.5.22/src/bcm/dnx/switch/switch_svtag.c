    ...
    bcm_switch_svtag_egress_info_t * svtag_info)
{
...

    {
        ...
        SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_SEXEM_2, &table_capacity));
...
shr_error_e
dnx_switch_svtag_egress_raw_entry_get(
    int unit,
    uint32 flags,
    bcm_gport_t gport,
    uint32 *svtag,
    uint32 *accumulation)
{
    uint32 svtag_hw_entry[SWITCH_SVTAG_HW_FIELD_SIZE_IN_UINT32] = { 0 };
    ...
    SHR_IF_ERR_EXIT(dnx_switch_svtag_hw_entry_get(unit, flags, gport, svtag_hw_entry));
