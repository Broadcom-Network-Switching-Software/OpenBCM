    ...
    bcm_gport_t gport)
{
    ...
    _SHR_PBMP_ITER(phy_ports, logical_port)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_algo_port_flexe_logical_phy_id_set(unit, logical_port, -1));
        /*
    ...
    bcm_port_flexe_group_phy_info_t * phy_info)
{
     ...
     */
    SHR_IF_ERR_EXIT(dnx_algo_port_flexe_logical_phy_id_set(unit, phy_info->port, -1));
    /*
     * Get FlexE core port
     */
    BCM_PBMP_CLEAR(flexe_core_port_bmp);
    ...
    else if (type == bcmPortFlexeOhLogicalPhyID)
    {
        ...
        if (is_set)
        {
            if (*val == 0)
            {
                ...
                if (_SHR_PBMP_MEMBER(logical_ports, gport))
                {
                    SHR_ERR_EXIT(_SHR_E_PARAM,
                                 "Cannot set the logical PHY ID to 0 if it belongs to one FlexE group.\r\n");
                }
            }
            else
            {
...
/**
 * \brief - Verify Flexe 1588 MAC set/get APIs.
 *
 * \param [in] unit - chip unit id
    ...
    int val)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_flexe_1588_mac_verify(unit, &val, 1));
    ...
    int *val)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_flexe_1588_mac_verify(unit, val, 0));
    ...
    int enable)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_flexe_bas_oam_to_ext_processor_verify(unit, &enable, 1));
    ...
    int *enable)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_flexe_bas_oam_to_ext_processor_verify(unit, enable, 0));
