    ...
    bcm_port_mapping_info_t * mapping_info)
{
...

    {
        if (DNX_ALGO_PORT_TYPE_IS_IMB(unit, port_info, FALSE))
        {
            ...
            if (interface_info->interface == BCM_PORT_IF_ILKN)
            {
                ...
                {
                    supported_phys =
                        &dnx_data_nif.ilkn.supported_phys_get(unit, interface_info->interface_id)->fabric_phys;
                }
                else
                {
                    supported_phys =
                        &dnx_data_nif.ilkn.supported_phys_get(unit, interface_info->interface_id)->nif_phys;
                }
                BCM_PBMP_ASSIGN(phy_pbmp, interface_info->phy_pbmp);
                ...
                if (dnx_data_nif.ilkn.max_nof_ifs_get(unit) != -1
                    /** elk: 0, L1: 1 */
                    && DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, 0 /* elk */ , 1 /* L1 */ ))
                {
    ...
    {
        if (mapping_info->tm_port != -1 && (mapping_info->in_tm_port != -1 || mapping_info->pp_dsp != -1 ||
                                            mapping_info->out_tm_port != -1))
        {
            SHR_ERR_EXIT(_SHR_E_PARAM,
    ...
    if ((interface_info->interface == BCM_PORT_IF_CRPS) &&
        !dnx_data_crps.engine.feature_get(unit, dnx_data_crps_engine_slim_counters_is_supported))
    {
    ...
    bcm_port_mapping_info_t * mapping_info)
{
    ...
    common_info->channel = common_info->is_channelized ? mapping_info->channel : 0;
    ...
    if (DNX_ALGO_PORT_TYPE_IS_TM(unit, port_info))
    {
        ...
        {
            SHR_INVOKE_VERIFY_DNX(dnx_algo_port_out_tm_add_verify(unit, port_info,
...
static const dnx_port_add_remove_step_t port_add_remove_sequence[] = {
    ...
    {"PortPP",            dnx_port_add_pp_init,                     dnx_port_remove_pp_init,                     NULL,    device_specific_skip},
    ...
    {"EgqQueuing",        dnx_egr_queuing_port_add_handle,          dnx_egr_queuing_port_remove_handle,          NULL,    device_specific_skip},
    ...
    bcm_port_mapping_info_t * mapping_info)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, TRUE /* elk */ , TRUE /* stif */ ,
                                  TRUE /* L1 */ , TRUE /* FLEXE */ )
    ...
    bcm_port_t port)
{
                                                                                                                 ...
                                                                                                                 */ ))
    {
        ...
        lane_rate = DNX_ALGO_PORT_TYPE_IS_NIF_ILKN(unit, port_info, 1, 1) ? speed : (speed / num_lane);
    }
