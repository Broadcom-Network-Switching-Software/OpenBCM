...
#if 1
#define DBAL_FIELD_PTC_ROUTING_ENABLE_PROFILE_GET(_unit) (DBAL_FIELD_PTC_ROUTING_ENABLE_PROFILE_VTT1)
#endif
/*
 * }
 */
 ...
 */
shr_error_e
dnx_port_pp_lif_speculative_parsing_set(
    int unit,
    bcm_port_t gport,
    int enable)
{
    uint32 generic_data_0 = 0;
    uint32 in_lif_speculative_parsing_bit = 0;
    uint32 entry_handle_id;
    dnx_algo_gpm_gport_hw_resources_t gport_hw_resources;
    ...
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_DIS_SPECULATIVE_PARSING_BIT_NUM_IN_GENERAL_DATA, INST_SINGLE,
                     &in_lif_speculative_parsing_bit));
    ...
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_0, INST_SINGLE, (uint32 *) &generic_data_0));
    if (enable == 1)
    {
        SHR_BITCLR(&generic_data_0, in_lif_speculative_parsing_bit);
    }
    else
    {
        SHR_BITSET(&generic_data_0, in_lif_speculative_parsing_bit);
    }

    /** Update generic_data_0*/
    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_IN_LIF, gport_hw_resources.local_in_lif);
    dbal_entry_value_field16_set(unit, entry_handle_id, DBAL_FIELD_LIF_GENERIC_DATA_0, INST_SINGLE, generic_data_0);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
 ...
 */
shr_error_e
dnx_port_pp_lif_speculative_parsing_get(
    int unit,
    bcm_port_t gport,
    int *enable)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_RESULT_TYPE, INST_SINGLE,
                                 gport_hw_resources.inlif_dbal_result_type);
    SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_GET_ALL_FIELDS));
    rv = dbal_entry_handle_value_field32_get(unit, entry_handle_id,
                                             DBAL_FIELD_LIF_GENERIC_DATA_0, INST_SINGLE, (uint32 *) &generic_data_0);

    if (rv == _SHR_E_NONE)
    {
        if (generic_data_0 & (1 << in_lif_speculative_parsing_bit))
        {
            *enable = 0;
        }
        else
    ...
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "The operation failed with code %d!\n", rv);
    }
    ...
    int oam_default_profile)
{
    ...
    DBAL_FUNC_INIT_VARS(unit);
    dnx_in_lif_profile_last_info_t_init(&last_profile);
    SHR_INVOKE_VERIFY_DNX(dnx_port_pp_oam_default_profile_set_verify(unit, oam_default_profile));
    ...
    uint8 vtt1_routing_enablers_valid)
{
    ...
    BCM_PBMP_ITER(gport_info.local_port_bmp, local_port)
    {
        ...
        {
            dbal_entry_value_field8_set(unit, entry_handle_id, DBAL_FIELD_PTC_ROUTING_ENABLE_PROFILE_GET(unit),
    ...
    int recycle_app_type)
{
    ...
    switch (recycle_app_type)
    {
        ...
        {
            vtt1_ptc_routing_enable_valid = TRUE;
    ...
    dnx_port_pp_egress_port_info_t * egress_port_info)
{
    ...
    if (dnx_data_port_pp.
        filters.feature_get(unit, dnx_data_port_pp_filters_rqp_discard_and_invalid_otm_filters_support))
    {
    ...
    uint32 header_type)
{
    ...
    BCM_PBMP_ITER(gport_info->local_port_bmp, local_port)
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_INGRESS_PTC_PORT, &entry_handle_id));
        SHR_IF_ERR_EXIT(dnx_algo_port_in_tm_port_get(unit, local_port, (bcm_core_t *) & tm_core_id, &ptc));
        ...
        if (header_type == BCM_SWITCH_PORT_HEADER_TYPE_MPLS_RAW)
        {
             ...
             */
            dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PTC_ROUTING_ENABLE_PROFILE_GET(unit),
                                         INST_SINGLE, DBAL_ENUM_FVAL_PTC_ROUTING_ENABLE_PROFILE_VTT1_ALWAYSTERMINATE);
        }
        else if ((header_type == BCM_SWITCH_PORT_HEADER_TYPE_RCH_0)
                 || (header_type == BCM_SWITCH_PORT_HEADER_TYPE_RCH_1)
                 || (header_type == BCM_SWITCH_PORT_HEADER_TYPE_RCH_SRV6_USP_PSP))
