    ...
    uint16 chan_id)
{
    ...
    if (dnx_data_fc.coe.calendar_mode_get(unit)->mode == DNX_FC_COE_MODE_PFC)
    {
        ...
        if (chan_id >= 256)
        {
            SHR_ERR_EXIT(_SHR_E_CONFIG, "Channel ID too large under PFC mode!\n");
        }
    }
    ...
    int value)
{
    ...
    SHR_IF_ERR_EXIT(dnx_port_sit_tag_swap_set
                    (unit, master_port, enable,
                     (enable ? DBAL_ENUM_FVAL_SIT_TAG_SWAP_MODE_TPID1_TRIGGER :
    ...
    BCM_PBMP_ITER(gport_info.local_port_bmp, local_port)
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_PRT_QUALIFIER_RAW, INST_SINGLE,
                                     enable ? virtual_tcam_app_db_id : 0);
        SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
    }
    ...
    /*
     * Allow COE enable on rcy_mirror port.
    ...
    if (!enable)
    {
        ...
        /**clear All FC channel IDs mapping on this PTC*/
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_coe_vid_mapping))
        {
            if (port_info.port_type != DNX_ALGO_PORT_TYPE_RCY_MIRROR)
            {
                for (i = 0; i <= DNX_MAX_COE_VID; i++)
                {
                    SHR_IF_ERR_EXIT(dnx_port_sit_coe_fc_chan_id_set(unit, (uint8) core_id, ptc, i, 0));
                }
            }
        }
    }
        ...
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_coe_vid_mapping))
        {
            if (port_info.port_type != DNX_ALGO_PORT_TYPE_RCY_MIRROR)
            {
                ...
                if (mapping_info->fc_channel_id >= 0)
                {
                    ...
                    if (fc_chan_id >= 256)
                    {
                        SHR_ERR_EXIT(_SHR_E_CONFIG, "Input fc_channel_id[%u] wrong!\n", fc_chan_id);
                    }
                    /** set channel_id */
    ...
    bcm_port_extender_mapping_info_t * mapping_info)
{
    ...
    else if (type == bcmPortExtenderMappingTypePortVlan)
    {
        ...
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_coe_vid_mapping))
        {
            ...
            mapping_info->fc_channel_id = (fc_chan_id == 0) ? -1 : fc_chan_id;
        }
    }
