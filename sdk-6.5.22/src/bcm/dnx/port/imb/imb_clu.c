    ...
    const bcm_port_speed_ability_t * abilities)
{
    ...
    for (i = 0; i < num_ability; ++i)
    {
        ...
        is_ability_valid = 0;
        for (ii = 0; ii < dnx_data_nif.eth.supported_clu_an_abilities_info_get(unit)->key_size[0]; ++ii)
        {
            if (dnx_data_nif.eth.supported_clu_an_abilities_get(unit, ii)->is_valid)
            {
                if (abilities[i].an_mode == dnx_data_nif.eth.supported_clu_an_abilities_get(unit, ii)->an_mode)
                {
                    if (abilities[i].speed == dnx_data_nif.eth.supported_clu_an_abilities_get(unit, ii)->speed)
                    {
                        if (abilities[i].resolved_num_lanes ==
                            dnx_data_nif.eth.supported_clu_an_abilities_get(unit, ii)->nof_lanes)
                        {
                            if (abilities[i].fec_type ==
                                dnx_data_nif.eth.supported_clu_an_abilities_get(unit, ii)->fec_type)
                            {
                                if (abilities[i].medium ==
                                    dnx_data_nif.eth.supported_clu_an_abilities_get(unit, ii)->medium)
                                {
