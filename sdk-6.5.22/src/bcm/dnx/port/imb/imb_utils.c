    ...
    bcm_port_speed_ability_t * bcm_abilities)
{
    ...
    for (i = 0; i < dnx_data_nif.eth.supported_clu_an_abilities_info_get(unit)->key_size[0]; ++i)
    {
        if (dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->is_valid)
        {
            if ((an_mode == dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->an_mode)
                && (nof_lanes == dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->nof_lanes)
                && (medium == dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->medium))
            {
                fec = dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->fec_type;
                ...
                {
                    speed = dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->speed;
    ...
    bcm_port_speed_ability_t * bcm_abilities)
{
    ...
    for (i = 0; i < dnx_data_nif.eth.supported_clu_an_abilities_info_get(unit)->key_size[0]; ++i)
    {
        if (dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->is_valid)
        {
            if (nof_lanes == dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->nof_lanes)
            {
                speed = dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->speed;
                if (((speed == 10000) && (portmod_abilities->speed_full_duplex & _SHR_PA_SPEED_10GB))
                    || ((speed == 20000) && (portmod_abilities->speed_full_duplex & _SHR_PA_SPEED_20GB))
                    ...
                    if (cnt_abilities < max_num_abilities)
                    {
                        if (cnt_abilities > 0)
                        {
                            if ((bcm_abilities[cnt_abilities - 1].an_mode ==
                                 dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->an_mode)
                                ...
                                && (bcm_abilities[cnt_abilities - 1].fec_type ==
                                    dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->fec_type))
                            {
                                continue;
                            }
                        }
                        ...
                        if (speed == 10000)
                        {
                            bcm_abilities[cnt_abilities].medium =
                                dnx_data_nif.eth.supported_clu_an_abilities_get(unit, i)->medium;
                        }
                        else
                        {
...
/**
 * \brief
 *  Logs port mode "tx-threshold set" operation in the generic journal
 * \param [in] unit - Device Id
 * \param [in] port - port
 * \param [in] value_orig - original tx-threshold value
 * \return
 *   int - Error Type
 * \remark
 *   * None
 ...
 */
static shr_error_e
imb_generic_portmod_tx_threshold_journal_set(
    int unit,
    int port,
    int value_orig)
{
    ...
    int value)
{
    ...
    SHR_IF_ERR_EXIT(portmod_port_tx_threshold_get(unit, port, &value_orig));

    SHR_IF_ERR_EXIT(portmod_port_tx_threshold_set(unit, port, value));
    DNX_ERR_RECOVERY_UNSUPPRESS(unit);

    SHR_IF_ERR_EXIT(imb_generic_portmod_tx_threshold_journal_set(unit, port, value_orig));
