    ...
    uint32 in_reset)
{
    ...
    if (imb_info->type == imbDispatchTypeImb_nbu)
    {
        core = dnx_data_nif.eth.ethu_per_core_get(unit, imb_info->inst_id)->core_id;
        ethu_id_in_core = imb_info->inst_id - dnx_data_nif.eth.nif_cores_ethus_get(unit, core)->first_ethu;
    }
    else
    {
