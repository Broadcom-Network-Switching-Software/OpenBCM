    ...
    const bcm_port_resource_t * resource_arr)
{
    ...
    for (resource_index = 0; resource_index < nof_ports; ++resource_index)
    {
        ...
        if (dnx_data_macsec.general.feature_get(unit, dnx_data_macsec_general_macsec_block_exists))
        {
            ...
            if (DNX_ALGO_PORT_TYPE_IS_NIF_ETH(unit, port_info, FALSE /* include_stif */ , FALSE /* include_l1 */ ) ==
                TRUE)
            {
                /** extract instance_id based on port */
                SHR_IF_ERR_EXIT(dnx_xflow_macsec_lport_to_macsec_get
    ...
    int link)
{
    ...
    if (ethu_info.imb_type == imbDispatchTypeImb_cdu)
    {
        ...
        /*
         * toggle portmod soft reset in and out, no need for 400G AN since
         * 400G WAR will reset the mac.
         */
        if (an_status.enabled && an_status.locked && link &&
            (an_status.resolved_num_lane != DNX_DATA_MAX_NIF_ETH_NOF_LANES_IN_CDU))
        {
