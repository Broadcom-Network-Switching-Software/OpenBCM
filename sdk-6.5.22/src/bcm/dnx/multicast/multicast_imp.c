        ...
        while (cur_entry_id != DNX_MULTICAST_END_OF_LIST_PTR)
        {
            ...
            switch (action)
            {
                ...
                case DNX_MULTICAST_ENTRY_ITER_REMOVE_ALL:
                    ...
                    {
                        /** Check the destinations - if there is a Trunk destination, the Egress profile also needs to be cleared. */
                    ...
                    /** Clear the entry from HW in order to keep things clean.
                      * The first entry is not freed from SW State data base, it just needs to be cleared. */
