    ...
    int unit)
{
    ...
    if (is_std_1)
    {
        ...
        sal_strncpy(ace_format_info.name, DNX_MULTICAST_PPMC_SINGLE_OUTLIF_ACE_FORMAT_NAME,
        ...
        sal_strncpy(ace_format_info.name, DNX_MULTICAST_PPMC_DOUBLE_OUTLIF_ACE_FORMAT_NAME,
                    sizeof(ace_format_info.name));
        SHR_IF_ERR_EXIT(dnx_field_ace_format_add(unit, ace_format_flags, &ace_format_info, &ace_id));
        /*
        ...
        sal_strncpy(ace_format_info.name, DNX_MULTICAST_PPMC_TRIPLE_OUTLIF_ACE_FORMAT_NAME,
                    sizeof(ace_format_info.name));
        SHR_IF_ERR_EXIT(dnx_field_ace_format_add(unit, ace_format_flags, &ace_format_info, &ace_id));
    }
    ...
    bcm_if_t * encap_id)
{
    ...
    SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_to_forward_information
                    (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, vxlan_port_id, BCM_FORWARD_ENCAP_ID_INVALID,
                     &forward_info));
