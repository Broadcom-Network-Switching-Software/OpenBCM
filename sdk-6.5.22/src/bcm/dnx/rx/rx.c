    ...
    int header_type_out)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_system_header_parse_ftmh(unit, pkt, pkt_len, packet_info, &is_inter_hdr_en));
    ...
    if (packet_info->is_trapped && packet_info->cpu_trap_qualifier == 0)
    {
        ...
        switch (packet_info->cpu_trap_code)
        {
            ...
            case DBAL_ENUM_FVAL_INGRESS_TRAP_ID_TRAP_OAM_PASSIVE:
                ...
                break;
            default:
                /** Get ingress cpu trap id for oam up mep destination 1 */
                SHR_IF_ERR_EXIT(dnx_rx_trap_type_map_info_get(unit, bcmRxTrapEgTxOamUpMEPDest1, &trap_type_info));
                trap_type_info.trap_id =
                    DNX_RX_TRAP_ID_SET(DNX_RX_TRAP_BLOCK_ETPP, DNX_RX_TRAP_CLASS_OAM, trap_type_info.trap_id);
                if (_SHR_E_NONE == bcm_dnx_rx_trap_get(unit, trap_type_info.trap_id, &trap_config))
                {
                    up_mep_ingress_cpu_trap_id = BCM_GPORT_TRAP_GET_ID(trap_config.cpu_trap_gport);
                    if (packet_info->cpu_trap_code == up_mep_ingress_cpu_trap_id)
                    {
                        is_2_system_header_from_trap = TRUE;
                        break;
                    }
                }
                /** Get ingress cpu trap id for oam up mep destination 2 */
                SHR_IF_ERR_EXIT(dnx_rx_trap_type_map_info_get(unit, bcmRxTrapEgTxOamUpMEPDest2, &trap_type_info));
                trap_type_info.trap_id =
                    DNX_RX_TRAP_ID_SET(DNX_RX_TRAP_BLOCK_ETPP, DNX_RX_TRAP_CLASS_OAM, trap_type_info.trap_id);
                if (_SHR_E_NONE == bcm_dnx_rx_trap_get(unit, trap_type_info.trap_id, &trap_config))
                {
    ...
    if (packet_info->is_trapped && packet_info->cpu_trap_qualifier == 0)
    {
        ...
        switch (packet_info->cpu_trap_code)
        {
            ...
            default:
                ...
                trap_type_info.trap_id =
                    DNX_RX_TRAP_ID_SET(DNX_RX_TRAP_BLOCK_ETPP, DNX_RX_TRAP_CLASS_OAM, trap_type_info.trap_id);
                SHR_IF_ERR_EXIT(bcm_dnx_rx_trap_get(unit, trap_type_info.trap_id, &trap_config));
                up_mep_ingress_cpu_trap_id = BCM_GPORT_TRAP_GET_ID(trap_config.cpu_trap_gport);
                is_2_system_header_from_trap = packet_info->cpu_trap_code == up_mep_ingress_cpu_trap_id ? TRUE : FALSE;
        }
    }
    ...
    if (DNX_RX_PARSE_SYSTEM_HEADERS_MODE_JERICHO2(unit))
    {
        ...
        if (is_recycle_append_ftmh == TRUE)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_rx_system_header_parse_ftmh
                            (unit, header_buffer, pkt->tot_len, &packet_info, &is_inter_hdr_en));
    ...
    void *param)
{
    ...
    while (rx_control.thread_running)
    {
        ...
        rv = adapter_read_buffer(unit, 0, &ms_id, &nof_signals, &ifc_type, &port_channel, &port_first_lane,
                                 &len, &(buf[0]));
        /*
        ...
        /** sub_unit_id = (sub_unit_id + 1) % (dnx_data_device.general.nof_sub_units_get(u_number)); */
        unit = (unit + 1) % soc_cm_get_num_devices();
    }
    /*
