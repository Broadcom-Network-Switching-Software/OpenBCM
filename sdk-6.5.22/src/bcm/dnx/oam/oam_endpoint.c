    ...
    const bcm_oam_endpoint_info_t * endpoint_info)
{
    ...
    if (DNX_OAM_DISSECT_IS_ENDPOINT_ACCELERATED(endpoint_info))
    {
        ...
        if (!DNX_OAMP_IS_V2(unit))
        {
            SHR_IF_ERR_EXIT(dnx_oam_oamp_v1_endpoint_remove_from_mep_db
                            (unit, endpoint_info, mep_db_entry,
                             mep_db_extra_data_length, is_mpls_lm_dm_without_bfd_endpoint));
        }
        else
#endif
        {
    ...
    if (DNX_OAM_DISSECT_IS_ENDPOINT_ACCELERATED(endpoint_info) || IS_OAM_CLIENT(endpoint_info))
    {
        SHR_IF_ERR_EXIT(dnx_oam_accelerated_endpoint_destroy_delete_profiles
                        (unit, endpoint_info, mep_hw_profiles_write_data, is_mpls_lm_dm_without_bfd_endpoint));
