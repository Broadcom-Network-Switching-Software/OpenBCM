...
#ifdef BCM_DNX2_SUPPORT
    const dnx_oam_oamp_ccm_endpoint_t *mep_db_entry = (const dnx_oam_oamp_ccm_endpoint_t *) hw_entry;
#endif
    ...
    bcm_oam_endpoint_info_t * endpoint_info)
{
    ...
    else
    {
        endpoint_info->tx_gport = BCM_GPORT_INVALID;
#ifdef BCM_DNX2_SUPPORT
        if (DNX_OAMP_IS_V1(unit) && (mep_db_entry->server_destination != 0))
        {
            if (system_headers_mode == DBAL_ENUM_FVAL_SYSTEM_HEADERS_MODE_JERICHO_MODE)
            {
    ...
    if (is_eth)
    {
        SHR_IF_ERR_EXIT(dnx_oam_oamp_ccm_endpoint_eth_oam_get(unit, hw_entry, endpoint_info));
    }
    else
    {
#ifdef BCM_DNX2_SUPPORT
