...
#define ALGO_L3_ECMP_EXTENDED_GROUP_GET_FIRST_ELEMENT_IN_BANK(unit, bank)\
    (dnx_data_l3.ecmp.nof_ecmp_basic_mode_get(unit) + (bank * dnx_data_l3.ecmp.ecmp_stage_map_size_get(unit)))
/*
    ...
    int *capacity)
{
    ...
    {
        int capacity_exem1, capacity_exem3;
        SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_SEXEM_1, &capacity_exem1));
        SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_SEXEM_3, &capacity_exem3));
        /*
         * In case EXEM1 or EXEM3 has 0 capacity they can't be used and only the EXEM with any capacity will taken for the lookup.
         * If both have some capacity we can fill them till the smallest one will get full so we're taking the MIN value of the capacities.
         */
        if ((capacity_exem1 == 0) || (capacity_exem3 == 0))
        {
            *capacity = UTILEX_MAX(capacity_exem1, capacity_exem3);
        }
        else
        {
            *capacity = UTILEX_MIN(capacity_exem1, capacity_exem3);
        }
    }
#else
    *capacity = 0;
    ...
    int unit)
{
     ...
     */
    {
        ...
        extra_create_info.grain_size = dnx_data_l3.ecmp.ecmp_stage_map_size_get(unit);
    ...
    else
    {
        ...
        range_start =
            UTILEX_MAX(range_start / dnx_data_l3.fec.bank_size_get(unit),
                       dnx_data_l3.fec.first_bank_without_id_alloc_get(unit) - 1);
        /*
    ...
    mdb_cluster_alloc_info_t * clusters)
{
    ...
    dbal_physical_tables_e dbal_physical_fec_table_id = DBAL_NOF_PHYSICAL_TABLES;
    int idx;
    int cl_id = 0;
    SHR_FUNC_INIT_VARS(unit);
