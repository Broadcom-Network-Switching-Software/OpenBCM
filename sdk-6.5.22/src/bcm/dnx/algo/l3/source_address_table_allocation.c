    ...
    if (allocation_needed)
    {
            ...
            (entry_ptr->address_type == source_address_type_ipv6))
        {
                  ...
                  && (*profile < dnx_data_l3.source_address.source_address_table_size_get(unit) / 2)))
            {
                ...
                SHR_IF_ERR_EXIT(sw_state_resource_nof_allocated_elements_in_range_get(unit, module_id,
                                                                                      multi_set_template->allocation_bitmap,
                                                                                      tag * grain_size, grain_size,
                                                                                      &nof_used_elements_per_grain));
                if (nof_used_elements_per_grain < multi_set_template->allocation_bitmap->grainSize)
                {
