    ...
    uint8 increment)
{
    ...
    if (rv != _SHR_E_NOT_FOUND)
    {
        ...
        tag_set_info.tag = (increment) ? tag_get_info.tag + 1 : ((tag_get_info.tag > 0) ? tag_get_info.tag - 1 : 0);
        /*
    ...
    uint8 *first_reference)
{
    ...
    /**
     *  In case of allocation out of range:
     *  The profile may not be in range if the data is compatible with a different profile that is outside range
     *  Possible fix suggestion: add prefix to the data, before allocating using smart template check if exists and in range
     */
    if (extra_arguments != NULL)
    {
