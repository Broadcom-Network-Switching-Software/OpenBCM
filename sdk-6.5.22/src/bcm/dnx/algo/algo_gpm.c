    ...
    uint32 *local_in_lif)
{
    ...
    if (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_in_lif_phy_db_dpc))
    {
        ...
        if (local_in_lif && local_in_lif_per_core)
        {
            ...
            else if (global_in_lif_dummy != DNX_ALGO_GPM_LIF_INVALID)
            {
                DNXCMN_CORES_ITER(unit, core_index_iterate, core_idx)
                {
                    ...
                    local_in_lif_per_core->local_in_lif[core_idx] = local_lif_info.local_lif;

                    if (local_in_lif_per_core->local_in_lif[core_idx] != DNX_ALGO_GPM_LIF_INVALID)
                    {
                        if (local_in_lif_per_core->nof_in_lifs == 0)
                        {
                            SHR_IF_ERR_EXIT(dnx_lif_mngr_inlif_sw_info_table_id_and_result_type_get
                                            (unit, local_in_lif_per_core->local_in_lif[core_idx], core_idx,
            ...
            /** if local lif found on all cores, and the values are the same, update just the first index */
            /** and set core=SHR_CORE_ALL. */
            /** This way, legacy users, which by definition used the IN_LIF in SBC mode, can use legacy parameter local_in_lif */
            if ((*local_in_lif != DNX_ALGO_GPM_LIF_INVALID) && (local_in_lif_per_core->nof_in_lifs == nof_cores))
            {
                local_in_lif_per_core->nof_in_lifs = 1;
    ...
    bcm_gport_t * gport)
{
    ...
    else if (_SHR_IS_FLAG_SET(flags, DNX_ALGO_GPM_GPORT_HW_RESOURCES_GLOBAL_LIF_EGRESS))
    {
        LIF_MNGR_GLOBAL_OUT_LIF_VERIFY(unit, lif);
             ...
             (dnx_data_lif.in_lif.feature_get(unit, dnx_data_lif_in_lif_in_lif_phy_db_dpc)))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
