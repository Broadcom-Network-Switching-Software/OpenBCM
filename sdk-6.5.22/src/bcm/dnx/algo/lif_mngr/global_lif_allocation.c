    ...
    int global_lif)
{
     ...
     *  2. Deallocate in the ingress and egress pools.
     */
    SHR_IF_ERR_EXIT(lif_mngr_db.global_lif_allocation.egress_global_lif.free_single(unit, global_lif, NULL));
...
shr_error_e
dnx_algo_global_out_lif_bta_get(
    int unit,
    int global_lif,
    int *bta)
{
    ...
    int *global_lif)
{
    ...
    if (is_egress && alloc_info && (alloc_info->bta != GLOBAL_LIF_BTA_DEFAULT)
