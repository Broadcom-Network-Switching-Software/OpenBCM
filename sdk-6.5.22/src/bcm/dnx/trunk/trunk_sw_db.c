    ...
    int max_nof_members_per_group)
{
    ...
    sal_snprintf(data.name, DNX_ALGO_RES_MNGR_MAX_NAME_LENGTH, "spa_member_id_manager_pool_%d_group_%d", pool_index,
                 group_id);
    SHR_IF_ERR_EXIT(algo_trunk_db.pools.groups.spa_id_manager.create(unit, pool_index, group_id, &data, NULL));
    ...
    int unit)
{
    ...
    profile_info_t invalid_entry;
    SHR_FUNC_INIT_VARS(unit);

    /** allocate profile info array and init it to invalid values */
    ...
    profile_info_t_init(&invalid_entry);
    SHR_IF_ERR_EXIT(trunk_db.psc_general_info.profile_info.range_fill(unit, 0, max_nof_profiles, invalid_entry));
    ...
    int *member_index)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_EXIT(algo_trunk_db.pools.groups.spa_id_manager.allocate_single(unit, pool, group, flags, NULL,
    ...
    int is_last_replication)
{
    ...
    /** if its invalid, it was already removed */
    if (removed_member_info.index != INVALID_TM_INDEX)
    {
        SHR_IF_ERR_EXIT(algo_trunk_db.pools.groups.
    ...
    if (is_last_replication)
    {
        ...
        /** if the last replication of the member was removed, need to clear it from the system port db */
    ...
    int member)
{
    ...
    {
        /** if PSC is C-Lag it is forbidden to change TM index of used member. */
