        ...
        case BCM_TRUNK_PSC_C_LAG:
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_consistent_hashing_manager_profile_alloc
                            (unit, chm_handle, trunk_id, HIRARCHY0, *psc_profile_type, SINGLE_PROFILE, FIRST_INDEX,
                             psc_info->max_nof_members_in_profile, nof_enabled_members, NULL, NULL,
                             (void *) &user_info));
...
/**
 * \brief - in case BCM_TRUNK_MEMBER_SPA_WITH_ID was provided,
 * verify validity of spa_id :
 *      1. it is not allocated OR the system port is allocated to this spa_id.
 *      this is to answer the case where the same port is added more than once to the same trunk
 *      2. it is within valid range
 *
 * \param [in] unit - unit number
 * \param [in] member - member provided by user
 * \param [in] pool - pool index
 * \param [in] group - group index
 *
 * \return
    ...
    int group)
{
    ...
    if (!gport_exists_in_trunk && provided_spa_id_is_allocated)
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG,
                     ...
                     " but the spa_id is allready allocated in this trunk, and not to the provided system port %d",
    ...
    if (gport_exists_in_trunk && (provided_spa_id != existing_spa_id))
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG,
                     ...
                     " but the system_port is allready allocated with spa_id = %d", provided_spa_id, existing_spa_id);
    }
    ...
    bcm_trunk_member_t * member_array)
{
    ...
    for (member_index = 0; (member_index < max_nof_members_in_group) && (remaining_members); ++member_index)
    {
            ...
            ((trunk_flags & BCM_TRUNK_FLAG_VP) == 0))
        {
            ...
            /** get core from first local port ( there is only one local port on system gport) */
    ...
    for (nof_enabled_members = 0, member_index = 0; member_index < member_count; ++member_index)
    {
            ...
            ((trunk_flags & BCM_TRUNK_FLAG_VP) == 0))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_port_pp_mapping_set(unit, phy_gport_info.local_port, pp_ports[core], spa, IS_LAG));
            ...
            SHR_IF_ERR_EXIT(dnx_trunk_features_cbs_run(unit, DNX_TRUNK_FEATURE_UPDATE_CB_ADD, trunk_id,
                                                       phy_gport_info.local_port));
        }
    ...
    SHR_IF_ERR_EXIT(dnx_trunk_psc_profile_alloc(unit, trunk_id, trunk_info->psc, &trunk_info->psc_info,
                                                nof_enabled_members, &new_psc_profile, &new_psc_profile_type));
    ...
    bcm_trunk_member_t * member)
{
    ...
    else if (psc == BCM_TRUNK_PSC_C_LAG)
    {
        /** for C LAG PSC, need alloc manager to find an empty index and use it */
    ...
    bcm_flow_agg_info_t * flow_agg_info)
{
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    /** Flow aggregate is not implemented and currently not planned to be implemented */
    SHR_ERR_EXIT(_SHR_E_UNAVAIL, "%s is not implemented.", FUNCTION_NAME());
    ...
    bcm_trunk_member_range_t range)
{
    ...
    SHR_INVOKE_VERIFY_DNX(dnx_trunk_member_range_set_get_verify(unit, flags, trunk_id, member, &range, TRUE));
    ...
    bcm_trunk_member_range_t * range)
{
    ...
    /** Verify input parameters */
    SHR_INVOKE_VERIFY_DNX(dnx_trunk_member_range_set_get_verify(unit, flags, trunk_id, member, range, FALSE));
