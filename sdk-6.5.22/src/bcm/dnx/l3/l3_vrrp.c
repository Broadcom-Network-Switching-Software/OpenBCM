...
/*
 * Holds the nof blocks that holds the exem vrrp tables;
 * vrrp exem tables are duplicated between sexem1 and sexe3
 */
#define DNX_L3_VRRP_NOF_MEMORY_BLOCKS_HOLDING_EXEM_DBAL_TABLES 2

#define DNX_L3_VRRP_NOF_ITER_GET(capacity_exem3) (capacity_exem3 == 1 ? 2 : 1)

#define DNX_L3_VRRP_START_ITER_GET(capacity_exem1) (capacity_exem1 == 1 ? 0 : 1)
/*
 * }
 */
/*
 ...
 */
/**
 * \brief enum to hold the indx of each exem table to be used to access the mapping in dnx_l3_vrrp_dbal_table_exem_mapping
 */
typedef enum l3_vrrp_exem_dbal_table_indx_e
{
    L3_VRRP_EXEM_TABLE_PER_PORT_MY_MAC,
    L3_VRRP_EXEM_TABLE_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_0,
    L3_VRRP_EXEM_TABLE_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_1,
    L3_VRRP_EXEM_TABLE_VRID_MY_MAC_WITH_PROTOCOL,
    L3_VRRP_EXEM_TABLE_PER_PORT_IGNORE_MAC,
    /** Number of vrrp exem dbal tables  */
    L3_VRRP_EXEM_TABLE_NOF_TABLES,
...
/**
 * brief - dbal table mapping for exem1 and exem3
 */

static const int
    dnx_l3_vrrp_dbal_table_exem_mapping[L3_VRRP_EXEM_TABLE_NOF_TABLES]
    [DNX_L3_VRRP_NOF_MEMORY_BLOCKS_HOLDING_EXEM_DBAL_TABLES] = {
    {DBAL_TABLE_EXEM_PER_PORT_MY_MAC, DBAL_TABLE_EXEM3_PER_PORT_MY_MAC},
    {DBAL_TABLE_EXEM_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_0, DBAL_TABLE_EXEM3_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_0},
    {DBAL_TABLE_EXEM_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_1, DBAL_TABLE_EXEM3_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_1},
    {DBAL_TABLE_EXEM_VRID_MY_MAC_WITH_PROTOCOL, DBAL_TABLE_EXEM3_VRID_MY_MAC_WITH_PROTOCOL},
    {DBAL_TABLE_EXEM_PER_PORT_IGNORE_MAC, DBAL_TABLE_EXEM3_PER_PORT_IGNORE_MAC}
};
/*
 * }
 */
...
dnx_l3_vrrp_init(
    int unit)
{
    int capacity_sexem_1 = 0, capacity_sexem_3 = 0;
    uint8 exem1_en = 0, exem3_en = 0;
    SHR_FUNC_INIT_VARS(unit);
    ...
    SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_SEXEM_1, &capacity_sexem_1));
    SHR_IF_ERR_EXIT(mdb_db_infos.capacity.get(unit, DBAL_PHYSICAL_TABLE_SEXEM_3, &capacity_sexem_3));
#endif

    if ((capacity_sexem_1 > 0) || (capacity_sexem_3 > 0))
    {
        /** Initialize EXEM based multiple my mac HW defaults */
        SHR_IF_ERR_EXIT(dnx_l3_vrrp_exem_based_multi_my_mac_init(unit));
    }

    (capacity_sexem_1 > 0) ? (exem1_en = 1) : (exem1_en = 0);
    (capacity_sexem_3 > 0) ? (exem3_en = 1) : (exem3_en = 0);

    SHR_IF_ERR_EXIT(algo_l3_db.vrrp_exem.exem1_en.set(unit, exem1_en));
    SHR_IF_ERR_EXIT(algo_l3_db.vrrp_exem.exem3_en.set(unit, exem3_en));
    ...
    dbal_enum_value_field_layer_types_e protocol)
{
    uint32 entry_handle_id;
    int table_iter;
    uint8 exem_1_en, exem_3_en;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FUNC_INIT_VARS(unit);

    SHR_IF_ERR_EXIT(algo_l3_db.vrrp_exem.exem1_en.get(unit, &exem_1_en));
    SHR_IF_ERR_EXIT(algo_l3_db.vrrp_exem.exem3_en.get(unit, &exem_3_en));

    for (table_iter = DNX_L3_VRRP_START_ITER_GET(exem_1_en); table_iter < DNX_L3_VRRP_NOF_ITER_GET(exem_3_en);
         table_iter++)
    {
        /** create handle */
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit,
                         dnx_l3_vrrp_dbal_table_exem_mapping[L3_VRRP_EXEM_TABLE_VRID_MY_MAC_WITH_PROTOCOL][table_iter],
                         &entry_handle_id));
    ...
    int *index)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
             ...
             table_iter++)
        {
            ...
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                            (unit,
                             dnx_l3_vrrp_dbal_table_exem_mapping[L3_VRRP_EXEM_TABLE_PER_PORT_MY_MAC][table_iter],
                             &entry_handle_id));
    ...
    int *index)
{
    ...
    for (pp_port_index = 0; pp_port_index < gport_info.internal_port_pp_info.nof_pp_ports; pp_port_index++)
    {
             ...
             table_iter++)
        {
            ...
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                            (unit,
                             dnx_l3_vrrp_dbal_table_exem_mapping[L3_VRRP_EXEM_TABLE_PER_PORT_IGNORE_MAC]
                             [table_iter], &entry_handle_id));
    ...
    int *index)
{
         ...
         table_iter++)
    {
        ...
        if (DNX_L3_VRRP_IS_MAC_ADDRESS_MSB_1(mac_da))
        {
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                            (unit,
                             dnx_l3_vrrp_dbal_table_exem_mapping
                             [L3_VRRP_EXEM_TABLE_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_1][table_iter], &entry_handle_id));
        }
        else
        {
            ...
            SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                            (unit,
                             dnx_l3_vrrp_dbal_table_exem_mapping
                             [L3_VRRP_EXEM_TABLE_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_0][table_iter], &entry_handle_id));
        }
            ...
            if (result_type == DBAL_RESULT_TYPE_L3_VRRP_EXEM_DB_SW_VSI)
            {
                ...
                if (DNX_L3_VRRP_IS_MAC_ADDRESS_MSB_1(mac_da))
                {
                    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                                    (unit,
                                     dnx_l3_vrrp_dbal_table_exem_mapping
                                     [L3_VRRP_EXEM_TABLE_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_1][table_iter],
                                     &entry_handle_id));
                }
                else
                {
                    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                                    (unit,
                                     dnx_l3_vrrp_dbal_table_exem_mapping
                                     [L3_VRRP_EXEM_TABLE_PER_VSI_MULTIPLE_MY_MAC_DA_MSB_0][table_iter],
                                     &entry_handle_id));
                }
    ...
    bcm_vlan_t vlan)
{
         ...
         table_iter++)
    {
        ...
        SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                        (unit,
                         dnx_l3_vrrp_dbal_table_exem_mapping[L3_VRRP_EXEM_TABLE_VRID_MY_MAC_WITH_PROTOCOL]
                         [table_iter], &entry_handle_id));
