    ...
    if (_SHR_IS_FLAG_SET(ecmp->flags, BCM_L3_WITH_ID))
    {
        ...
        if (_SHR_IS_FLAG_SET(ecmp->flags, BCM_L3_REPLACE))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_l3_ecmp_hierarchy_get
                            (unit, BCM_L3_ITF_VAL_GET(ecmp->ecmp_intf), (uint32 *) &current_ecmp_hierarchy));
        ...
        if (_SHR_IS_FLAG_SET(ecmp->ecmp_group_flags, BCM_L3_ECMP_EXTENDED) && !is_allocated)
        {
            ...
            SHR_IF_ERR_EXIT(DNX_L3_ECMP_GROUP_RESOURCE_CB(unit, BCM_L3_ITF_VAL_GET(ecmp->ecmp_intf),
                                                          is_allocated(unit, BCM_L3_ITF_VAL_GET(ecmp->ecmp_intf),
                                                                       &res_mngr_is_allocated)));
            if (res_mngr_is_allocated)
            {
                SHR_ERR_EXIT(_SHR_E_FULL,
                             "The bank to which the ECMP ID %d belongs to is unavailable \n",
                             BCM_L3_ITF_VAL_GET(ecmp->ecmp_intf));
            }
        }
    }
