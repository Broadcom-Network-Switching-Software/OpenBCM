    ...
    int *exists_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, stage, &field_map_stage_info));
    cs_qual_map_p = &field_map_stage_info.cs_qual_map[bcm_qual];
    ...
    dnx_field_presel_qual_data_t * dnx_cs_qual_info)
{
    ...
    cs_qual_map_p = &field_map_stage_info.cs_qual_map[bcm_cs_qual_info->qual_type];
    /*
    ...
    rhlist_t ** cs_dnx_dump_list_p)
{
    ...
    for (bcm_qual = 0; bcm_qual < bcmFieldQualifyCount; bcm_qual++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, stage, &field_map_stage_info));
            if (field_map_stage_info.cs_qual_map[bcm_qual].nof != 0)
            {
                if (cs_dnx_dump == NULL)
                {
                ...
                if (field_map_stage_info.cs_qual_map[bcm_qual].nof == 1)
                {
                    cs_dnx_dump->field_id[stage] = field_map_stage_info.cs_qual_map[bcm_qual].field_id;
                }
                else
                {
                    cs_dnx_dump->field_id[stage] = field_map_stage_info.cs_qual_map[bcm_qual].field_id_array[0];
                }
            }
        }
    }
    ...
    int *dnx_cs_qual_nof_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, stage, &field_map_stage_info));
    *dnx_cs_qual_info_p = field_map_stage_info.cs_qual_info;
    *dnx_cs_qual_nof_p = field_map_stage_info.cs_qual_nof;
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
        if (field_map_stage_info.cs_qual_nof > DNX_FIELD_CS_QUAL_NOF_MAX)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Stage:\"%s\" has %d CS qualifiers. Please update the maximum of all stages "
                         "which is currently %d.\n",
                         dnx_field_stage_text(unit, dnx_stage),
                         field_map_stage_info.cs_qual_nof, DNX_FIELD_CS_QUAL_NOF_MAX);
        }
    }
    ...
    for (i_bcm_cs_qual = 0; i_bcm_cs_qual < bcmFieldQualifyCount; i_bcm_cs_qual++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
            cs_qual_map = &field_map_stage_info.cs_qual_map[i_bcm_cs_qual];
