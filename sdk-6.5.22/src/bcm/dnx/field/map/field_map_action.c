    ...
    dnx_field_map_action_info_t * dnx_action_info_p)
{
    ...
    switch (dnx_action_class)
    {
        ...
        case DNX_FIELD_ACTION_CLASS_STATIC:
        {
            ...
            if ((action_id < 0) || (action_id >= field_map_stage_info.static_action_nof) ||
                (field_map_stage_info.static_action_info[action_id].field_id == DBAL_FIELD_EMPTY))
            {
                SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "The action(%d) in stage %s does not exist!",
                             action_id, dnx_field_stage_text(unit, dnx_stage));
            }
            dnx_action_info_p->field_id = field_map_stage_info.static_action_info[action_id].field_id;
            dnx_action_info_p->bcm_action = field_map_stage_info.static_action_info[action_id].bcm_action;
            /**
            ...
            for (signal_iter = 0; signal_iter < DNX_DATA_MAX_FIELD_DIAG_NOF_SIGNALS_PER_ACTION; signal_iter++)
            {
                ...
                if (field_map_stage_info.static_action_info[action_id].signal_name[signal_iter] == 0)
                {
                    break;
                }
                dnx_action_info_p->signal_name[signal_iter] =
                    field_map_stage_info.static_action_info[action_id].signal_name[signal_iter];
            }
            break;
        }
    ...
    const dnx_field_action_map_t ** action_map_entry_p)
{
    ...
    else if ((bcm_action >= 0) && (bcm_action < bcmFieldActionCount))
    {
        ...
        else
        {
             ...
             */
            if ((field_map_stage_info.static_action_id_map == NULL) ||
                (field_map_stage_info.static_action_id_map[bcm_action].dnx_action == 0))
            {
                if (print_errors)
                {
                    SHR_ERR_EXIT(_SHR_E_NOT_FOUND,
             ...
             */
            action_map_entry = &field_map_stage_info.static_action_id_map[bcm_action];

            SHR_IF_ERR_EXIT(dnx_pp_prgm_default_image_check(unit, &is_std_1));
            if (!is_std_1 && (field_map_stage_info.static_action_id_map[bcm_action].conversion_cb != NULL))
            {
                SHR_IF_ERR_EXIT(dnx_field_bare_metal_support_check(unit,
                                                                   field_map_stage_info.static_action_id_map
                                                                   [bcm_action].conversion_cb, &bare_metal_support));
            ...
            dnx_action =
                DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, dnx_stage,
                           field_map_stage_info.static_action_id_map[bcm_action].dnx_action);
        }
    }
    ...
    int *dnx_action_nof_p)
{
    ...
    {
        if (field_map_stage_info.static_action_info[i_info].field_id != DBAL_FIELD_EMPTY)
        {
    ...
    dnx_field_action_t * dnx_action_p)
{
    ...
    if ((action_id >= field_map_stage_info.static_action_nof)
        || (field_map_stage_info.static_action_info[action_id].field_id == DBAL_FIELD_EMPTY))
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "The action(%d) in stage %s does not exist!",
                     action_id, dnx_field_stage_text(unit, dnx_stage));
    ...
    dnx_field_action_t * dnx_action_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
    *dnx_action_p = DNX_ACTION(DNX_FIELD_ACTION_CLASS_STATIC, dnx_stage, field_map_stage_info.static_action_nof);
    ...
    dnx_field_action_t * dnx_action_p)
{
    ...
    action_found = FALSE;
    for (action_ndx = 0; action_ndx < field_map_stage_info.static_action_nof; action_ndx++)
    {
        if (field_map_stage_info.static_action_info[action_ndx].field_id == field_id)
        {
            if (action_found)
            {
    ...
    int unit)
{
    ...
    DNX_FIELD_STAGE_ACTION_ITERATOR(stage)
    {
        ...
        for (i_info = 0; i_info < field_map_stage_info.static_action_nof; i_info++)
        {
            uint32 size;
            uint8 dbal_enum_val_illegal;
            dnx_field_base_action_info_t *base_action_info = &field_map_stage_info.static_action_info[i_info];
            ...
            SHR_IF_ERR_EXIT(dbal_fields_is_illegal_value
                            (unit, field_map_stage_info.action_field_id, base_action_info->field_id,
    ...
    for (i_bcm_act = 0; i_bcm_act < bcmFieldActionCount; i_bcm_act++)
    {
        ...
        else
        {
            ...
            DNX_FIELD_STAGE_ACTION_ITERATOR(dnx_stage)
            {
                uint8 dbal_enum_val_illegal;
                SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
                base_action_map = (dnx_field_action_map_t *) & field_map_stage_info.static_action_id_map[i_bcm_act];
                ...
                base_action_info = &field_map_stage_info.static_action_info[action_id];
                if (base_action_info->field_id != DBAL_FIELD_EMPTY)
                {
                    /*
    ...
    for (i_bcm_act = 0; i_bcm_act < bcmFieldActionCount; i_bcm_act++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
            if (field_map_stage_info.static_action_id_map[i_bcm_act].conversion_cb != NULL)
            {
                SHR_IF_ERR_EXIT(dnx_field_bare_metal_support_check(unit,
                                                                   field_map_stage_info.static_action_id_map[i_bcm_act].
