    ...
    dnx_field_stage_e stage)
{
    ...
    else
    {
        ...
        rv = dnx_field_map_stage_info_get(unit, stage, &field_map_stage_info);
        if (rv != _SHR_E_NONE || (pp_stage = field_map_stage_info.pp_stage) == DNX_PP_STAGE_INVALID)
        {
    ...
    bcm_field_AppType_t bcm_apptype)
{
    ...
    else
    {
        ...
        rv = dbal_fields_enum_value_get(unit, DBAL_FIELD_KBP_FORWARD_APP_TYPES, bcm_apptype, &opcode_enum_val);
        if (rv != _SHR_E_NONE)
        {
            apptype_name = "Invalid Apptype";
            return apptype_name;
        }
        apptype_name = dbal_enum_id_type_to_string(unit, DBAL_FIELD_TYPE_DEF_KBP_FORWARD_APP_TYPES, opcode_enum_val);
    }
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stages[stage_idx], &field_map_stage_info));
...
* \param [in] unit - Device ID.
* \param [out] static_opcode_is_valid - Array with DNX_KBP_MAX_STATIC_OPCODES_NOF elements indicating what opcodes are 
*                                       found in the XML.
* \return
*   shr_error_e - Error Type
* \remark
...
dnx_field_map_apptype_to_context_parse_init(
    int unit,
    uint8 static_opcode_is_valid[DNX_KBP_MAX_STATIC_OPCODES_NOF])
{
    void *catalogue = NULL;
    void *opcode_node;
    char *image_name;
    ...
    for (opcode_index = 0; opcode_index < DNX_KBP_MAX_STATIC_OPCODES_NOF; opcode_index++)
    {
    ...
    RHDATA_ITERATOR(opcode_node, catalogue, "AppType")
    {
        ...
        RHDATA_ITERATOR(context_iter, opcode_node, "FwdContext")
        {
            ...
            if (sw_state_is_warm_boot(unit))
            {
                ...
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.predef_info.
                                context_ids.get(unit, apptype, context_index, &fwd_context_uint32_get));
                if (fwd_context_uint32_get != fwd_context_uint32)
                {
                    SHR_ERR_EXIT(_SHR_E_CONFIG, "Mismatch XML and SW state. For Apptype %d, context number %d is %d "
                                 "in XML and %d in SW state.\n",
                                 apptype, context_index, fwd_context_uint32, fwd_context_uint32_get);
                }
            }
            else
            {
        ...
        if (sw_state_is_warm_boot(unit))
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_apptype_sw.predef_info.nof_context_ids.get(unit, apptype, &nof_context_get));
        ...
        else
        {
             ...
             * If not warmboot save to SW state
             */
            SHR_IF_ERR_EXIT(dnx_field_apptype_sw.predef_info.nof_context_ids.set(unit, apptype, nof_contexts));
        }
    ...
    int unit)
{
    ...
    if (nof_static_opcodes != DNX_KBP_MAX_STATIC_OPCODES_NOF)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Unexpected number of elements in dnx_field_static_opcode_info %d. Expected %d.\n",
                     nof_static_opcodes, DNX_KBP_MAX_STATIC_OPCODES_NOF);
    }
    if (nof_static_opcodes < DBAL_NOF_ENUM_KBP_FORWARD_APP_TYPES_VALUES)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Numer of static opcodes (%d) smaller then number of opcode in standard_1 %d.\n",
                     nof_static_opcodes, DBAL_NOF_ENUM_KBP_FORWARD_APP_TYPES_VALUES);
    }
    ...
    if (DNX_KBP_MAX_STATIC_OPCODES_NOF >= first_user_defined_apptype)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     ...
                     DNX_KBP_MAX_STATIC_OPCODES_NOF, first_user_defined_apptype);
    }

    if (DBAL_ENUM_FVAL_KBP_FORWARD_APP_TYPES_INVALID != 0)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "DBAL_ENUM_FVAL_KBP_FORWARD_APP_TYPES_INVALID isn't zero, leading to imporoper ititialization of "
    ...
    if (static_opcode_is_valid[DBAL_ENUM_FVAL_KBP_FORWARD_APP_TYPES_INVALID])
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG, "opcode %d was found, illegal.\r\n", DBAL_ENUM_FVAL_KBP_FORWARD_APP_TYPES_INVALID);
    }
    ...
    for (i_apptype = 0; i_apptype < nof_static_apptypes; i_apptype++)
    {
        ...
        if (is_standard_image)
        {
            ...
            if (mapped_opcode == DBAL_ENUM_FVAL_KBP_FORWARD_APP_TYPES_INVALID)
            {
                continue;
            }
        }
        ...
        if (mapped_opcode >= DNX_KBP_MAX_STATIC_OPCODES_NOF)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         "Apptype %d mapped to opcode %d with HW value %d, above dynamic range (%d).\n",
                         i_apptype, mapped_opcode, mapped_opcode, DNX_KBP_MAX_STATIC_OPCODES_NOF);
        }
    ...
    for (i_opcode = 0; i_opcode < nof_static_opcodes; i_opcode++)
    {
        if (dnx_field_static_opcode_info[i_opcode].is_valid)
        {
            if (!sw_state_is_warm_boot(unit))
            {
                uint8 nof_contexts;
                SHR_IF_ERR_EXIT(dnx_field_apptype_sw.predef_info.
                                nof_context_ids.get(unit, dnx_field_static_opcode_info[i_opcode].app_type,
                                                    &nof_contexts));
                if (nof_contexts > 0)
                {
    ...
    if (is_standard_image)
    {
        for (i_apptype = 0; i_apptype < nof_static_apptypes; i_apptype++)
        {
            ...
            dnx_field_context_t
                mapped_acl_contexts[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE];
            ...
            if (rv != _SHR_E_NOT_FOUND)
            {
                if (dnx_field_standard_1_static_apptype_map[i_apptype].opcode_id != mapped_opcode)
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Apptype %d mapped alternatively to opcodes %d and %d.\n",
                                 i_apptype, dnx_field_standard_1_static_apptype_map[i_apptype].opcode_id,
                                 mapped_opcode);
                }
                ...
                if ((!sw_state_is_warm_boot(unit)) && dnx_kbp_device_enabled(unit))
                {
                    ...
                    if (fwd_nof_contexts > DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_ACL_CONTEXT_NUM)
                    {
                        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Apptype %d mapped to %d FWD contexts, maximum is %d.\n",
                                     i_apptype, fwd_nof_contexts, DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_ACL_CONTEXT_NUM);
                    }
                    for (fwd_context_idx = 0; fwd_context_idx < fwd_nof_contexts; fwd_context_idx++)
                    {
                        ...
                        if (mapped_fwd_contexts[fwd_context_idx] != fwd_acl_ctx_mapping[fwd_context_idx].fwd_context)
                        {
                            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                                         ...
                                         mapped_fwd_contexts[fwd_context_idx],
                                         fwd_acl_ctx_mapping[fwd_context_idx].fwd_context);
                        }
                        if (mapped_acl_contexts[fwd_context_idx] !=
    ...
    int unit)
{
    ...
    if (dnx_data_field.general_data_qualifiers.vw_supported_get(unit))
    {
    ...
    dbal_fields_e * ffc_type_field_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, stage, &field_map_stage_info));
    *ffc_type_field_p = field_map_stage_info.ffc_type_field_a[ffc_type];
    ...
    dbal_tables_e * ffc_table_name_p)
{
     ...
     */
    *ffc_table_name_p = field_map_stage_info.ffc_table;
    ...
    dbal_fields_e * ffc_instruction_name_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, stage, &field_map_stage_info));
    *ffc_instruction_name_p = field_map_stage_info.ffc_instruction;
    ...
    dnx_field_stage_e * dnx_stage_p)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
        if (field_map_stage_info.bcm_stage == bcm_stage)
        {
    ...
    dbal_fields_e * dbal_ctx_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, stage, &field_map_stage_info));
    *dbal_ctx_id = field_map_stage_info.ctx_id;
    ...
    dbal_fields_e * dbal_key)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, stage, &field_map_stage_info));
    *dbal_key = field_map_stage_info.key_field;
    ...
    bcm_field_stage_t * bcm_stage_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
    bcm_stage = field_map_stage_info.bcm_stage;
    ...
    dbal_enum_value_field_layer_types_e * dnx_layer_type)
{
    ...
    switch (bcm_layer_type)
    {
        ...
        case bcmFieldLayerTypeMplsIfit:
            *dnx_layer_type = DBAL_ENUM_FVAL_LAYER_TYPES_MPLS_IFIT;
            break;
        ...
        case bcmFieldLayerTypeBierTi:
            *dnx_layer_type = DBAL_ENUM_FVAL_LAYER_TYPES_BIER_NON_MPLS;
            break;
        case bcmFieldLayerTypeBierMpls:
            *dnx_layer_type = DBAL_ENUM_FVAL_LAYER_TYPES_IN_BIER_BASE_EG_BIER_MPLS;
            break;
        ...
        case bcmFieldLayerTypeLt2pv3:
            *dnx_layer_type = DBAL_ENUM_FVAL_LAYER_TYPES_L2TP;
            break;
    ...
    dbal_tables_e * table_id_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
    *table_id_p = field_map_stage_info.lookup_enabler_table_id;
exit:
    SHR_FUNC_EXIT;
}
    ...
    uint32 *opcode_id_p)
{
    ...
    else
    {
        ...
        {
            if (app_type >= (sizeof(dnx_field_static_opcode_info) / sizeof(dnx_field_static_opcode_info[0])))
            {
                SHR_ERR_EXIT(_SHR_E_PARAM, "Apptype %d is out of range. ranges are %d-%d for static Apptypes, and "
                             "%d-%d for dynamic.\n",
                             app_type, 0,
                             ((int)
                              (sizeof(dnx_field_static_opcode_info) / sizeof(dnx_field_static_opcode_info[0]))) - 1,
                             dnx_data_field.external_tcam.apptype_user_1st_get(unit),
                             dnx_data_field.external_tcam.apptype_user_1st_get(unit) +
                             dnx_data_field.external_tcam.apptype_user_nof_get(unit) - 1);
            }
        ...
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.predef_info.nof_context_ids.get(unit, app_type, &nof_contexts));
        if (nof_contexts <= 0)
        {
            SHR_SET_CURRENT_ERR(_SHR_E_NOT_FOUND);
            SHR_EXIT();
    ...
    bcm_field_AppType_t * app_type_p)
{
    ...
    else
    {
        ...
        app_type = dnx_field_static_opcode_info[opcode_id].app_type;
        SHR_IF_ERR_EXIT(dnx_field_apptype_sw.predef_info.nof_context_ids.get(unit, app_type, &nof_contexts));
        if (nof_contexts <= 0)
        {
    ...
    uint32 *cs_profile_id_p)
{
    ...
    if ((int) opcode_id < 0
        || opcode_id >= (sizeof(dnx_field_static_opcode_info) / sizeof(dnx_field_static_opcode_info[0])))
    {
        SHR_ERR_EXIT(_SHR_E_PARAM, "opcode_id %d is out of range. Maximum %d. User defined apptypes no supported.\n",
                     opcode_id,
                     (int) ((sizeof(dnx_field_static_opcode_info) / sizeof(dnx_field_static_opcode_info[0])) - 1));
    }
    ...
    SHR_IF_ERR_EXIT(dnx_field_apptype_sw.predef_info.
                    nof_context_ids.get(unit, dnx_field_static_opcode_info[opcode_id].app_type, &nof_contexts));
    if (nof_contexts <= 0)
    {
    ...
    dnx_field_context_t acl_contexts[DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE])
{
    ...
    if (apptype >= 0 && apptype < bcmFieldAppTypeCount)
    {
        ...
        if (nof_contexts > DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE)
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Number of FWD contexts for apptype %d (\"%s\") is %d, maximum is %d.\n",
                         apptype, dnx_field_bcm_apptype_text(unit, apptype), nof_contexts,
                         DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE);
        }
    ...
    else
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "Number of FWD contexts for apptype %d (\"%s\") is %d, maximum is %d.\n",
                         apptype, dnx_field_bcm_apptype_text(unit, apptype), fwd_nof_contexts,
                         DNX_DATA_MAX_FIELD_EXTERNAL_TCAM_MAX_FWD_CONTEXT_NUM_FOR_ONE_APPTYPE);
        }
        for (fwd_context_idx = 0; fwd_context_idx < fwd_nof_contexts; fwd_context_idx++)
        {
...
#else
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Reached user defined apptype but KBP not compiled.\r\n");
#endif
    }
