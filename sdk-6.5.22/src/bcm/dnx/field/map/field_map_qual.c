    ...
    dnx_field_qual_info_t * dnx_qual_info_p)
{
    ...
    switch (dnx_qual_class)
    {
        ...
        case DNX_FIELD_QUAL_CLASS_META2:
        {
            ...
            meta_qual_info = &(field_map_stage_info.meta_qual_info[dnx_qual_id]);
        ...
        case DNX_FIELD_QUAL_CLASS_LAYER_RECORD:
        {
            ...
            layer_qual_map_info = &(field_map_stage_info.layer_qual_info[dnx_qual_id]);
    ...
    const dnx_field_qual_map_t ** qual_map_entry_p)
{
    ...
    else if ((bcm_qual >= 0) && (bcm_qual < bcmFieldQualifyCount))
    {
        ...
        if ((dnx_qual == 0) || (dnx_qual == DNX_FIELD_QUAL_TYPE_INVALID))
        {
             ...
             */
            if (field_map_stage_info.meta_qual_map)
            {
                dnx_qual = field_map_stage_info.meta_qual_map[bcm_qual].dnx_qual;
                if ((dnx_qual == 0) || (dnx_qual == DNX_FIELD_QUAL_TYPE_INVALID))
                {
                    /*
                    ...
                    {
                        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get
                                        (unit, DNX_FIELD_STAGE_IPMF1, &field_map_stage_info));
                        dnx_qual = field_map_stage_info.meta_qual_map[bcm_qual].dnx_qual;
                        if ((dnx_qual == 0) || (dnx_qual == DNX_FIELD_QUAL_TYPE_INVALID))
                        {
                            if (print_errors)
                        ...
                        qual_map_entry = &field_map_stage_info.meta_qual_map[bcm_qual];

                        conversion_cb = field_map_stage_info.meta_qual_map[bcm_qual].conversion_cb;
                    }
                    else
                    {
                        if (print_errors)
    ...
    int *dnx_qual_num_p)
{
    ...
    if (stage == DNX_FIELD_STAGE_IPMF2)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, stage, &field_map_stage_ipmf1_info));
 ...
 */
    for (i_info = DNX_FIELD_IPMF1_QUAL_FIRST; i_info < field_map_stage_info.meta_qual_nof; i_info++)
    {
        if (field_map_stage_info.meta_qual_info[i_info].field_id != 0)
        {
 ...
 */
    if (stage == DNX_FIELD_STAGE_IPMF2)
    {
        dnx_field_map_stage_info_t field_map_stage_ipmf1_info;
        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, stage, &field_map_stage_ipmf1_info));
        for (i_info = DNX_FIELD_IPMF1_QUAL_FIRST; i_info < field_map_stage_ipmf1_info.meta_qual_nof; i_info++)
        {
            if (field_map_stage_ipmf1_info.meta_qual_info[i_info].field_id != 0)
            {
                qual_offset = dnx_data_field.qual.params_get(unit, stage, i_info)->offset;
                if (qual_offset < 0)
                {
    ...
    dnx_field_qual_t * dnx_qual_p)
{
    ...
    qual_found = FALSE;
    SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
    for (qual_ndx = 0; qual_ndx < field_map_stage_info.meta_qual_nof; qual_ndx++)
    {
        if (field_map_stage_info.meta_qual_info[qual_ndx].field_id == field_id)
        {
            if (qual_found)
            {
    ...
    if (dnx_stage == DNX_FIELD_STAGE_IPMF2)
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, DNX_FIELD_STAGE_IPMF1, &field_map_stage_info));
        for (qual_ndx = 0; qual_ndx < field_map_stage_info.meta_qual_nof; qual_ndx++)
        {
            if (field_map_stage_info.meta_qual_info[qual_ndx].field_id == field_id)
            {
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
        SHR_IF_ERR_EXIT(dnx_field_map_qual_has_raw_validate(unit, field_map_stage_info.meta_qual_map));
    }
    ...
    int unit)
{
    ...
    DNX_FIELD_STAGE_QUAL_ITERATOR(dnx_stage)
    {
        int i_info;
        SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
        for (i_info = 1; i_info < field_map_stage_info.meta_qual_nof; i_info++)
        {
            dnx_field_meta_qual_info_t *meta_qual_info = &field_map_stage_info.meta_qual_info[i_info];
            ...
            if (meta_qual_info->field_id == DBAL_FIELD_EMPTY)
            {
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "No DBAL field ID for metadata qualifier: %d stage %s\n",
                             i_info, dnx_field_bcm_stage_text(dnx_stage));
            }
        }
    }
    ...
    for (i_bcm_qual = 0; i_bcm_qual < bcmFieldQualifyCount; i_bcm_qual++)
    {
        ...
        else
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
                meta_qual_map = &field_map_stage_info.meta_qual_map[i_bcm_qual];
                /*
                ...
                meta_qual_info = &field_map_stage_info.meta_qual_info[qual_id];
 ...
 */
    for (i_bcm_qual = 0; i_bcm_qual < bcmFieldQualifyCount; i_bcm_qual++)
    {
        ...
        {
            SHR_IF_ERR_EXIT(dnx_field_map_stage_info_get(unit, dnx_stage, &field_map_stage_info));
            if (field_map_stage_info.meta_qual_map != NULL
                && field_map_stage_info.meta_qual_map[i_bcm_qual].conversion_cb != NULL)
            {
                SHR_IF_ERR_EXIT(dnx_field_bare_metal_support_check(unit, field_map_stage_info.meta_qual_map
