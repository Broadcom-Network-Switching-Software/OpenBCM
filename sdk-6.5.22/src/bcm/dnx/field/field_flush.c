...
/**
 * \brief - Sets a flush profile SW DB
 */
static shr_error_e
dnx_field_flush_profile_sw_db_set(
    int unit,
    uint32 flags,
    uint32 flush_profile_id,
    uint32 is_mapped,
    uint32 fg_id)
{
    ...
    /** allocate table handle */
    SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC(unit, DBAL_TABLE_FIELD_FLUSH_PROFILE_SW_DB, &entry_handle_id));

    dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_FLUSH_PROFILE_ID, flush_profile_id);

    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_MAP_VALID, INST_SINGLE, is_mapped);
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FG_ID, INST_SINGLE, fg_id);
...
/**
 * \brief - Gets a flush profile SW DB
 */
static shr_error_e
dnx_field_flush_profile_sw_db_get(
    int unit,
    uint32 flags,
    uint32 flush_profile_id,
    uint32 *is_mapped,
    uint32 *fg_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get
                    (unit, entry_handle_id, DBAL_FIELD_MAP_VALID, INST_SINGLE, is_mapped));
    SHR_IF_ERR_EXIT(dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_FG_ID, INST_SINGLE, fg_id));
    ...
    uint32 *flush_profile_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_flush_profile_sw_db_set(unit, flags, *flush_profile_id, FALSE, 0));
    ...
    uint32 flush_profile_id)
{
    ...
    if (lexem_used_by_stage == TRUE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error! fg_id (=%d) - stage = %d, uses_large_exem = TRUE! EXEM-4 is currently not supported! \n",
                     fg_id, field_stage);
    }

    if (sexem_used_by_stage == FALSE)
    {
        SHR_ERR_EXIT(_SHR_E_PARAM,
                     "Error! fg_id (=%d) - stage = %d, sexem_used_by_stage = FALSE! Only EXEM-3 is supported! \n",
                     fg_id, field_stage);
    }
    ...
    /*
     * Flush Profile can be mapped to only on FG!
     * Verify that the flush profile is not already attached:
     */
    SHR_IF_ERR_EXIT(dnx_field_flush_profile_sw_db_get(unit, flags, flush_profile_id, &is_mapped, &mapped_fg_id));
    if (is_mapped == TRUE)
    {
    ...
    uint32 flush_profile_id)
{
    ...
    /** Set flush-profile SW DB */
    SHR_IF_ERR_EXIT(dnx_field_flush_profile_sw_db_set(unit, flags, flush_profile_id, TRUE, fg_id));
