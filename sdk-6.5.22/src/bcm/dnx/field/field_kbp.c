...
/** See field_kbp.h */
shr_error_e
dnx_field_kbp_key_qual_info_get_kbp_verify(
    int unit,
    ...
    uint32 key_size)
{
    ...
    SHR_IF_ERR_EXIT(dnx_field_kbp_key_fwd2_to_acl_key_id_convert(unit, key_id, &kbp_kbr_id));
    ...
    SHR_IF_ERR_EXIT(dnx_field_kbp_key_aligner_mapping_mem_hw_set(unit, context_id, kbp_kbr_id, BITS2BYTES(key_size)));
...
/**
 * \brief
 *  Obtains the KBP keys available for ACL in an ACL context based on a FWD context.
 * \param [in] unit        - Device Id
    ...
    int *size_on_key_p)
{
    ...
    kbp_key_bmp = dnx_data_field.external_tcam.key_bmp_get(unit);
    ...
    int *offset_on_last_key_p)
{
    ...
    for (kbp_key_index = 0; kbp_key_index < nof_kbp_keys; kbp_key_index++)
    {
        ...
        for (context_index = 0; context_index < nof_fwd_contexts; context_index++)
        {
            ...
            /*
             * Verify that the result if the same for al contexts.
             */
            if (context_index > 0)
            {
    ...
    dnx_field_group_kbp_info_t * kbp_opcode_info_p)
{
    ...
    for (seg_idx = (DNX_KBP_MAX_NOF_SEGMENTS_PER_MASTER_KEY - 1); seg_idx >= nof_fwd_seg; seg_idx--)
    {
        ...
        /*
         * Initializng qual_type_last_packed to silence the compiler
         */
...
/** See field_kbp.h */
shr_error_e
dnx_field_kbp_key_kbp_ffc_info_get(
    int unit,
    dnx_field_group_t fg_id,
    int opcode_id,
    uint8 fwd_context,
    ...
    int unit)
{
    ...
    if (dnx_data_field.external_tcam.nof_acl_keys_master_max_get(unit) > DBAL_NOF_ENUM_KBP_KBR_IDX_VALUES)
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL, "Maximum number of ACL keys in KBP %d is bigger than the maximum number "
                     "of KBP keys in DBAL %d.\n",
                     dnx_data_field.external_tcam.nof_acl_keys_master_max_get(unit), DBAL_NOF_ENUM_KBP_KBR_IDX_VALUES);
    }
    ...
    bcm_field_AppType_t * apptype_p)
{
    ...
    /** Verify input params*/
    SHR_INVOKE_VERIFY_DNX(dnx_field_kbp_context_apptype_create_verify(unit, flags, apptype_info_p, apptype_p));
