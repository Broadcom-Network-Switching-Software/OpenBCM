    ...
    uint32 *local_outlif)
{
    ...
    if ((tunnel->type == bcmTunnelTypeSR6) || (tunnel->type == bcmTunnelTypeEthSR6))
    {
        ...
        dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_GLOB_LIF_FOR_RCY, INST_SINGLE,
                                     lif_table_mngr_outlif_info.global_lif);
    }

    /*
     *  For vxlan, set esem command: indicate that a ESEM
    ...
    *is_qos_model_pipe_mode = 0;
    ...
    if (!is_field_valid)
    {
        *esem_cmd_profile = dnx_data_esem.access_cmd.no_action_get(unit);
             ...
             ((*esem_cmd_profile) == dnx_data_esem.access_cmd.default_native_get(unit)) &&
             ((*tunnel_type) == bcmTunnelTypeSR6))
    {
    ...
    bcm_tunnel_initiator_t * tunnel)
{
    ...
    {
        if (dnx_data_flow.ipv4.use_flow_lif_init_get(unit))
        {
    ...
    bcm_instru_trajectory_trace_t * trajectory_trace)
{
    ...
    {
        /** Add alloc_with_id flag */
        global_lif_info.global_lif = BCM_GPORT_TUNNEL_ID_GET(trajectory_trace->trajectory_trace_id);
        lif_alloc_flags |= LIF_MNGR_GLOBAL_LIF_WITH_ID;
    }
     /** Allocate new tunnel out-LIF - REPLACE flag is not set */
    ...
    /** update returned Tunnel ID's */
    BCM_GPORT_TUNNEL_ID_SET(trajectory_trace->trajectory_trace_id, global_lif_info.global_lif);
exit:
    SHR_FUNC_EXIT;
}
    ...
    bcm_tunnel_initiator_t * tunnel)
{
    ...
    /*
     * Note - only ipv4 gre is supported via flow lif
     */
    if (gport_hw_resources.outlif_dbal_table_id == DBAL_TABLE_EEDB_IPV4_TUNNEL)
    {
        if (dnx_data_flow.ipv4.use_flow_lif_init_get(unit))
        {
            SHR_IF_ERR_EXIT(dnx_tunnel_initiator_get_flow(unit, tunnel));
            SHR_EXIT();
        }
    }
    ...
    SHR_IF_ERR_EXIT(dnx_algo_global_out_lif_bta_get
                    (unit, gport_hw_resources.global_out_lif, &tunnel->estimated_encap_size));
    ...
    bcm_l3_intf_t * intf)
{
    ...
    if (gport_hw_resources.outlif_dbal_table_id == DBAL_TABLE_EEDB_IPV4_TUNNEL)
    {
        if (dnx_data_flow.ipv4.use_flow_lif_init_get(unit))
        {
            SHR_IF_ERR_EXIT(dnx_tunnel_initiator_clear_flow(unit, tunnel_gport));
            SHR_EXIT();
        }
    }
