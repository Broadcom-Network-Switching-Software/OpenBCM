    ...
    uint32 *local_outlif)
{
    ...
    /** Get ESEM cmd profile if replace */
    if (_SHR_IS_FLAG_SET(tunnel->flags, BCM_TUNNEL_REPLACE))
    {
        SHR_IF_ERR_EXIT(dnx_lif_table_mngr_get_outlif_info(unit, *local_outlif, entry_handle_id, &lif_table_mngr_outlif_info));
        lif_table_mngr_outlif_info.flags |= LIF_TABLE_MNGR_LIF_INFO_REPLACE;

        /** Get ESEM cmd */
        dbal_entry_handle_value_field32_get(unit, entry_handle_id, DBAL_FIELD_ESEM_COMMAND, INST_SINGLE, &new_esem_cmd_profile);
    }
    else
    {
        /** set outlif phase */
        lif_table_mngr_outlif_info.logical_phase = DNX_ALGO_LOCAL_OUTLIF_LOGICAL_PHASE_1;
    ...
    bcm_tunnel_initiator_t * tunnel)
{
    ...
    if (tunnel != NULL)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_lif_lib_logical_phase_to_encap_access_convert
                        (unit, outlif_info.logical_phase, &(tunnel->encap_access)));
        /*
         * Whenever the returned value is the same as the Invalid, the Invalid value is returned
         */
        if (tunnel->encap_access == bcmEncapAccessTunnel1)
        {
            tunnel->encap_access = bcmEncapAccessInvalid;
        }
    }
    ...
    uint32 arp_out_lif)
{
    ...
    else
    {
        ...
        if (((tunnel->type == bcmTunnelTypeErspan)
            ...
            ((tunnel->type == bcmTunnelTypeRspanAdvanced)
            && (gport_hw_resources.outlif_dbal_table_id != DBAL_TABLE_EEDB_RSPAN)))
        {
            SHR_ERR_EXIT(_SHR_E_NOT_FOUND, "Illegal to replace non mirror OutLIF to mirror OutLIF");
        }
