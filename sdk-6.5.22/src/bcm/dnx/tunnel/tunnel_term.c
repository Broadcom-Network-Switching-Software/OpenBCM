    ...
    dbal_fields_e * result_dbal_field)
{
    ...
    {
        dnx_tunnel_terminator_srv6_to_ipv6_tcam_table_result_dbal_field(info, result_dbal_field);
    }
    else
    {
 ...
 */
static shr_error_e
dnx_tunnel_terminator_ipv6_tcam_get(
    int unit,
    uint32 entry_handle_id,
    bcm_ip6_t * dip6,
    bcm_ip6_t * dip6_mask,
    int *vrf,
    uint32 *cross_stage_var_2,
    bcm_tunnel_type_t * tunnel_type)
{
    ...
    *cross_stage_var_2 = field_value2[0];
 ...
 *   \param [in] default_vsi - default vsi
 *   \param [out] vrf - vrf
 *   \param [out] fodo_assignment_mode - fodo_assignment_mode
 */
static shr_error_e
    ...
    SHR_IF_ERR_EXIT(dnx_lif_table_mngr_get_inlif_info(unit, _SHR_CORE_ALL, local_inlif, entry_handle_id, &inlif_info));
    {
        ...
        *ecn_mapping_profile = dbal_field[0];
        ...
        if (is_field_valid == TRUE)
        {
            ...
            dnx_tunnel_terminator_srv6_optional_micro_segment_id_flag_set(service_per_flow, flags);
        }
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    {
        dnx_algo_gpm_gport_hw_resources_t gport_hw_resources;
        uint32 fodo, current_fodo_assignment_mode;
        bcm_vlan_t default_vsi;
        uint32 old_ecn_mapping_profile = DNX_QOS_ECN_MAPPING_PROFILE_DEFAULT;
        ...
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_get
                        (unit, gport_hw_resources.local_in_lif,
                         (uint32 *) &global_lif, &propag_profile, (uint32 *) &old_in_lif_profile,
                         &old_ecn_mapping_profile, &flags, &fodo, &default_vsi, &vrf, &current_fodo_assignment_mode));

        /** verify that tunnel type does not change from vxlan type to another or vice-versa */
        SHR_IF_ERR_EXIT(dnx_tunnel_terminator_tunnel_vxlan_type_verify(unit, info, current_fodo_assignment_mode));
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    uint32 vrf;
    bcm_vlan_t default_vsi;
    uint32 dummy_fodo_assignment_mode;
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_get
                    (unit, local_inlif, &global_inlif, &propagation_prof, &in_lif_profile, &ecn_mapping_profile,
                     &(info->flags), &fodo, &default_vsi, &vrf, &dummy_fodo_assignment_mode));
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_optional_flags_get
                    (unit, info, global_inlif, in_lif_profile, &(info->flags)));
    ...
    bcm_tunnel_terminator_t * info)
{
    ...
    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_get
                    (unit, local_inlif, &global_inlif, &propagation_prof, &in_lif_profile, &ecn_mapping_profile,
                     &flags, &fodo, &default_vsi, &vrf, &dummy_fodo_assignment_mode));
    if (global_inlif == 0)
    {
        bcm_gport_t gport;
    ...
    uint32 *local_inlif)
{
    ...
    SHR_IF_ERR_EXIT(dbal_fields_uint32_sub_field_info_get
                    (unit, DBAL_FIELD_IRPP_CROSS_STAGE_VAR_2, cross_stage_var_2, &sub_field_id, local_inlif));
        ...
        while (!is_end)
        {
            ...
            if (dbal_tables[table] == DBAL_TABLE_IPV6_TT_P2P_EM_BASIC)
            {
                ...
                while (!is_end_ipv6)
                {
                     ...
                     */
                    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_ipv6_tcam_get
                                    (unit, entry_handle_id_internal, &dip6, &dip6_mask, &vrf, &cross_stage_var_2,
                                     &type));

                    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_ipv6_tcam_result_optional_lif
                                    (unit, cross_stage_var_2, &result_is_local_inlif, &local_inlif));

                    if ((local_inlif == tunnel_class) && (result_is_local_inlif))
                    {
                        SHR_IF_ERR_EXIT(dnx_algo_gpm_gport_from_lif
                     ...
                     || (dbal_tables[table] == DBAL_TABLE_IPV6_MP_TT_TCAM_BASIC))
            {
                ...
                uint8 result_is_local_inlif = FALSE;
                SHR_IF_ERR_EXIT(dnx_tunnel_terminator_ipv6_tcam_get
                                (unit, entry_handle_id, &info.dip6, &info.dip6_mask, &info.vrf, &cross_stage_var_2,
                                 &info.type));
                SHR_IF_ERR_EXIT(dnx_tunnel_terminator_ipv6_tcam_result_optional_lif
                                (unit, cross_stage_var_2, &result_is_local_inlif, &local_inlif));
                if (result_is_local_inlif)
                {
                    ...
                    SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_get
                                    (unit, local_inlif, &global_inlif, &propagation_prof, &in_lif_profile,
                                     &ecn_mapping_profile, &flags, &fodo, &dummy_vsi, &dummy_vrf,
                                     &dummy_fodo_assignment_mode));
                    /**If the tunnel type has vsi set on it, then its either bcmTunnelTypeEthIn6, bcmTunnelTypeEthSR6 or bcmTunnelTypeCascadedFunct*/
            ...
            else if (dbal_tables[table] == DBAL_TABLE_IPVX_TT_IPSEC)
            {
                ...
                if (in_lif_profile_info.ingress_fields.cs_in_lif_profile_flags ==
                    DNX_IN_LIF_PROFILE_CS_PROFILE_LIF_VXLAN_OVER_ESP)
                {
                    info.type = bcmTunnelTypeVxlanEsp;
                }
                else
                {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_traverse_funct_key_fields
                                (unit, entry_handle_id, &info, &local_inlif));
                ...
                SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_get
                                (unit, local_inlif, &global_inlif, &propagation_prof, &in_lif_profile,
                                 ...
                                 &dummy_fodo_assignment_mode));
                /**If we have set FODO VSI on the tunnel, update the type*/
                SHR_IF_ERR_EXIT(dnx_tunnel_terminator_srv6_l2vpn_type_get(unit, &info, fodo, &type));
                SHR_IF_ERR_EXIT(dnx_tunnel_terminator_in_lif_table_optional_vsi_get(unit, fodo, &info.vlan));
                info.type = type;
            }
