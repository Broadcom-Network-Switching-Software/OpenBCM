    ...
    bcm_l2_addr_t * l2addr)
{
    ...
    if ((dnx_data_l2.general.feature_get(unit, dnx_data_l2_general_ivl_feature_support) == 0) && (l2addr->modid != 0))
    {
    ...
    *eei = 0;
    ...
    if (_SHR_IS_FLAG_SET(l2addr->flags, BCM_L2_TRUNK_MEMBER))
    {
        ...
        SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_to_forward_information
                        (unit, ALGO_GPM_ENCODE_DESTINATION_FLAGS_NONE, trunk_gport, l2addr->encap_id, &forward_info));
    }
    else
    {
        SHR_IF_ERR_EXIT(algo_gpm_gport_and_encap_to_forward_information
                        (unit, ALGO_GPM_ENCODE_DESTINATION_ASYM_VOQ, l2addr->port, l2addr->encap_id, &forward_info));
    }
...
/**
 * \brief
 * Implement the bcm_l2_addr_add in the case it adds to the FWD_MACT or FWD_MACT_IVL logical table
 */
static int
    ...
    bcm_l2_addr_t * l2addr)
{
     ...
     */
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT_FORCE | dbal_commit_flags));
