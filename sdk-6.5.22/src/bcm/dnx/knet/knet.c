...
exit:
 ...
 /**
  * \brief - Justify whether local port is macsec enabled or not.
  *
  * \param [in] unit - Unit id
  ...
  */
int
dnx_knet_local_port_is_macsec_enabled(
    int unit,
    bcm_gport_t port)
{
    ...
    rv = bcm_dnx_xflow_macsec_port_control_get(unit,
                                               BCM_XFLOW_MACSEC_ENCRYPT_DECRYPT_NONE,
                                               port, bcmXflowMacsecPortEnable, &port_info);
    if (SHR_SUCCESS(rv))
    {
    ...
    bcm_knet_netif_t * netif)
{
    ...
    /**
     * If this netif is local port awared and MACSec is enabled on this local port, add SVTAG.
     */
    if (netif->type == BCM_KNET_NETIF_T_TX_LOCAL_PORT && dnx_knet_local_port_is_macsec_enabled(unit, netif->port))
    {
        uint32 svtag, *svtagp;
        uint32 accumulation = 0;

        /**
         * If BCM_KNET_NETIF_F_ADD_SVTAG_NONE is set, Add a NULL SVTAG.
         * Otherwise get the SVTAG from port-based configuration.
         */
        if (netif->flags & BCM_KNET_NETIF_F_ADD_SVTAG_NONE)
        {
            svtag = 0;
        }
        else
        {
            SHR_IF_ERR_EXIT(dnx_switch_svtag_egress_raw_entry_get(unit, 0, netif->port, &svtag, &accumulation));
        }

        svtagp = (uint32 *) &netif_create.netif.svtag[0];
        *svtagp = bcm_htonl(svtag);
        netif_create.netif.flags |= KCOM_NETIF_F_ADD_SVTAG;
    }
    ...
    bcm_knet_netif_t * netif)
{
    ...
    if (netif_get.netif.flags & KCOM_NETIF_F_ADD_SVTAG)
    {
        svtag = bcm_ntohl(*(uint32 *) &netif_get.netif.svtag[0]);
        if (0 == svtag)
        {
            netif->flags |= BCM_KNET_NETIF_F_ADD_SVTAG_NONE;
        }
    }
