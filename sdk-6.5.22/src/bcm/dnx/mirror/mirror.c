        ...
        if (mirror_dest->drop_group_bmp != BCM_MIRROR_DROP_GROUP_BMP_ALL)
        {
            ...
            else
            {
                ...
                SHR_BITSET_RANGE(&drop_group_bmp_mask, dnx_data_snif.ingress.nof_mirror_on_drop_profiles_get(unit),
                                 (SAL_UINT32_NOF_BITS - dnx_data_snif.ingress.nof_mirror_on_drop_profiles_get(unit)));
                if ((drop_group_bmp_mask & mirror_dest->drop_group_bmp) != 0)
                {
    ...
    uint32 current_profile_id)
{
    ...
    SHR_IF_ERR_EXIT(dnx_mirror_rule_get(unit, port, BCM_VLAN_INVALID, flags, &received_profile_id));
    if ((current_profile_id == BCM_MIRROR_INVALID_PROFILE)
        || ((current_profile_id == received_profile_id) && (vlan != BCM_VLAN_INVALID)))
    {
    ...
    bcm_mirror_header_info_t * mirror_header_info)
{
    ...
    {
        if (mirror_header_info->udh[ii].size > dnx_data_field.udh.type_3_length_get(unit))
        {
    ...
    int unit)
{
    ...
    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_EGRESS_MIRROR_ACTION_NULL_PROFILE, INST_SINGLE,
                                 DNX_MIRROR_EGRESS_DISABLE_PROFILE_IDX);
    SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
