    ...
    bcm_vxlan_vpn_config_t * info)
{
    ...
    if (update)
    {
        ...
        if (info->flags & BCM_VXLAN_VPN_REPLACE)
        {
            ...
            rv_vsi2vni =
                dnx_vxlan_vpn_vsi_to_vni_get(unit, info->vpn, info->match_port_class, &old_vnid, &stat_id,
                                             &stat_pp_profile, &vsi_to_vni_result_type, FALSE);
            SHR_IF_ERR_EXIT_EXCEPT_IF(rv_vsi2vni, _SHR_E_NOT_FOUND);
    ...
    bcm_vpn_t l2vpn)
{
    ...
    {
        rv_vsi2vni =
            dnx_vxlan_vpn_vsi_to_vni_get(unit, l2vpn, network_domain, &vnid, &stat_id, &stat_pp_profile, &result_type,
                                         FALSE);
        SHR_IF_ERR_EXIT_EXCEPT_IF(rv_vsi2vni, _SHR_E_NOT_FOUND);
...
*/
static shr_error_e
dnx_vxlan_network_domain_config_virtual_gport_is_deleted_verify(
    int unit,
    bcm_vxlan_network_domain_config_t * config)
{
    int rv;
    ...
    vlan_port.vsi = config->vsi;
    vlan_port.match_class_id = config->network_domain;
    ...
    bcm_vxlan_network_domain_config_t * config)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_vxlan_network_domain_config_virtual_gport_is_deleted_verify(unit, config));
    }
    ...
    bcm_vxlan_network_domain_config_t * config)
{
    ...
    SHR_FUNC_INIT_VARS(unit);
    DNX_ERR_RECOVERY_NOT_NEEDED(unit);
    SHR_INVOKE_VERIFY_DNX(dnx_vxlan_network_domain_config_remove_verify(unit, config));
