    ...
    uint32 round_up)
{
    ...
    else
    {
        ...
        UTILEX_64_UDIV_32(intermediate_numerator, rate_in_mega_bits_per_sec);
        ...
        while (peak_rate_exp > (nof_flipflops - mantissa_field_size) &&
               (SHR_BITTEST_RANGE(peak_rate_man_arr, 0, peak_rate_exp - (nof_flipflops - mantissa_field_size),
                                  set_bits)))
        {
            peak_rate_man_arr[0] = UTILEX_DIV_ROUND_UP(peak_rate_man_arr[0], 2);
            peak_rate_exp--;
        }
    ...
    bcm_cos_queue_t cosq)
{
   ...
   /** retrieve egq port */
    ...
    SHR_IF_ERR_EXIT(dnx_sch_dbal_lag_port_member_set(unit, egq_core, pp_dsp, TRUE));

    /** mark the port is on different core than its scheduler */
    SHR_IF_ERR_EXIT(dnx_sch_dbal_lag_port_core_flip_set(unit, egq_core, pp_dsp, (egq_core == sch_core ? FALSE : TRUE)));
    ...
    bcm_cos_queue_t cosq)
{
    ...
    SHR_IF_ERR_EXIT(dnx_sch_dbal_lag_port_member_set(unit, egq_core, pp_dsp, FALSE));

    /** mark the port is not on different core than its scheduler */
    SHR_IF_ERR_EXIT(dnx_sch_dbal_lag_port_core_flip_set(unit, egq_core, pp_dsp, FALSE));
