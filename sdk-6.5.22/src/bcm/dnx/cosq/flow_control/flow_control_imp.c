    ...
    int *nif_in_core)
{
    ...
    /** Devices with NBU PMs have assimetrical NIF */
    if (dnx_data_nif.eth.nbu_nof_get(unit) != 0)
    {
        nof_lanes_to_remove =
            (dnx_data_nif.eth.nif_cores_ethus_get(unit, core)->first_ethu *
             dnx_data_nif.eth.nof_lanes_in_nbu_get(unit));
    }
    else
    {
        nof_lanes_to_remove = core * dnx_data_nif.phys.nof_phys_per_core_get(unit);
    }
    *nif_in_core = nif_port - nof_lanes_to_remove;
    ...
    int *global_nif)
{
    ...
    {
        nof_lanes_to_add = core * dnx_data_nif.phys.nof_phys_per_core_get(unit);
    }
    *global_nif = nif_port + nof_lanes_to_add;
...
dnx_fc_nof_nif_for_core_get(
    ...
    int *nof_nif)
{
    int nof_lanes_for_current_core = 0;
    SHR_FUNC_INIT_VARS(unit);

    /** Devices with NBU PMs have assimetrical NIF */
    if (dnx_data_nif.eth.nbu_nof_get(unit) != 0)
    {
        nof_lanes_for_current_core =
            (dnx_data_nif.eth.nif_cores_ethus_get(unit, core)->nof_ethus * dnx_data_nif.eth.nof_lanes_in_nbu_get(unit));
    }
    else
    {
        nof_lanes_for_current_core = dnx_data_nif.phys.nof_phys_per_core_get(unit);
    }
    *nof_nif = nof_lanes_for_current_core;
        ...
        if (dnx_data_fc.general.feature_get(unit, dnx_data_fc_general_pfc_rec_cdu_shift))
        {
            ...
            if ((nif_port >= first_affected_nif_port) && (source->cosq == 0))
            {
                dbal_entry_key_field32_set(unit, entry_handle_id, DBAL_FIELD_NIF_PORT,
                                           ((nif_port ==
                                             (dnx_data_nif.phys.nof_phys_per_core_get(unit) -
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, src_port, &port_info));
    if (!DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, 0, 1, 0, 0))
    {
    ...
    bcm_cosq_fc_endpoint_t * target)
{
    ...
    port =
        BCM_GPORT_IS_LOCAL(source->port) ? BCM_GPORT_LOCAL_GET(source->
                                                               port) : BCM_GPORT_LOCAL_INTERFACE_GET(source->port);
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    if (!DNX_ALGO_PORT_TYPE_IS_NIF(unit, port_info, 0, 0, 1, 0))
    {
        SHR_ERR_EXIT(_SHR_E_PORT, "Unsupported port (0x%x), must be NIF", source->port);
    }
