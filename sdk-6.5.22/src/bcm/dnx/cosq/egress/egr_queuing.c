    ...
    int speed)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info))
    {
         ...
         */
        SHR_IF_ERR_EXIT(dnx_egr_queuing_rcy_total_speed_get(unit, core, &prev_if_speed));
    }
    else
    {
    ...
    if (total_egr_if_credits > total_egr_if_credits_limit)
    {
    ...
    bcm_port_t port)
{
    ...
    SHR_IF_ERR_EXIT(dnx_algo_port_info_get(unit, port, &port_info));
    SHR_IF_ERR_EXIT(dnx_algo_port_interface_offset_get(unit, port, &interface_offset));
    if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info) && interface_offset)
    {
    ...
    bcm_port_t port)
{
    ...
    if (dnx_data_egr_queuing.params.feature_get(unit, dnx_data_egr_queuing_params_sub_calendar) == TRUE)
    {
        ...
        if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info))
        {
             ...
             */
            SHR_IF_ERR_EXIT(dnx_egr_queuing_rcy_total_speed_get(unit, core, &rcy_total_speed));
            ...
            */
            SHR_IF_ERR_EXIT(dnx_algo_port_interface_offset_get(unit, port, &interface_offset));
            fqp_egr_if = (interface_offset != 0) ? dnx_data_egr_queuing.params.egr_if_txi_rcy_get(unit) : eps_egr_if;
        }
        else
        {
            /*
    ...
    DNXCMN_CORES_ITER(unit, core, core_id)
    {
        ...
        for (cal_type = 0; cal_type < NUM_DNX_EGR_CAL_TYPE; cal_type++)
        {
            ...
            BCM_PBMP_ITER(port_bm, port)
            {
                ...
                {
                    SHR_IF_ERR_EXIT(dnx_egr_queuing_rcy_total_speed_get(unit, core_id, &if_rate_mbps));
                }
    ...
    bcm_port_t logical_port)
{
    ...
    if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info) && interface_offset)
    {
        egr_if_for_fqp_profile = dnx_data_egr_queuing.params.egr_if_txi_rcy_get(unit);
    }
    else
    {
    ...
    int unit)
{
    ...
    else
    {
        ...
        BCM_PBMP_ITER(logical_port_channels, port_i)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_algo_port_channel_get(unit, port_i, &channel));
            break;
        }
    ...
    if (next_master_port == DNX_ALGO_PORT_INVALID)
    {
        if (DNX_ALGO_PORT_TYPE_IS_RCY(unit, port_info))
        {
            ...
            if (dnx_data_esb.general.feature_get(unit, dnx_data_esb_general_esb_support) == 1)
            {
                ...
                /** Remove the speed of the current port of the total */
                SHR_IF_ERR_EXIT(dnx_algo_port_has_speed(unit, port, DNX_ALGO_PORT_SPEED_F_TX, &has_speed));
                if ((has_speed == TRUE))
                {
                    SHR_IF_ERR_EXIT(dnx_algo_port_interface_rate_get
                                    (unit, port, DNX_ALGO_PORT_SPEED_F_TX, &if_speed_i));
                    rcy_total_speed -= if_speed_i;
                }
             ...
             */
            SHR_IF_ERR_EXIT(dnx_algo_port_interface_offset_get(unit, port, &interface_offset));
            egr_if_for_fqp_profile =
                (interface_offset == 0) ? egr_if : dnx_data_egr_queuing.params.egr_if_txi_rcy_get(unit);
        }
        else
        {
            /**
