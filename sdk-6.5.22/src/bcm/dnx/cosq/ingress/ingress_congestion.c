    ...
    int *vsq_e_id)
{
    ...
    else
    {
        ...
        else
        {
            ...
            alloc_info.range_start = dnx_data_ingr_congestion.vsq.nif_vsq_e_nof_get(unit);
    ...
    int *vsq_f_id)
{
    ...
    else
    {
        ...
        else
        {
            /** When we allocate without id we want to allocate from the extra VSQs */
            alloc_flags |= RESOURCE_TAG_BITMAP_ALLOC_IN_RANGE;
            alloc_info.range_start = (dnx_data_nif.phys.nof_phys_per_core_get(unit) * BCM_COSQ_VSQ_NOF_PG);
    ...
    bcm_cosq_static_threshold_t * threshold)
{
    ...
    /** verify */
    SHR_INVOKE_VERIFY_DNX(dnx_ingress_congestion_voq_gl_ecn_marking_threshold_arg_verify
