    ...
    if (SOC_IS_FBX(unit)) {
        ...
        case bcmVlanPortUseInnerPri:
            ...
            if (BCM_GPORT_IS_MIM_PORT(port) || BCM_GPORT_IS_MPLS_PORT(port) ||
                BCM_GPORT_IS_VXLAN_PORT(port) || BCM_GPORT_IS_FLOW_PORT(port)) {
                              ...
                              bcm_vlan_control_port_t type, int arg)
{
    ...
    if (port == -1) {
        ...
        /* PBMP_E_ALL does not already include LB ports */
        if (soc_feature(unit, soc_feature_configure_loopback_port)) {
            BCM_PBMP_OR(temp_pbmp, PBMP_LB(unit));
        }
...
STATIC int
_bcm_vlan_gport_local_port_get_all(int unit, bcm_vlan_t vlan, int array_max, 
                           ...
                           int* array_size)
{
    ...
    if(!is_vfi) {
        ...
        BCM_IF_ERROR_RETURN(_bcm_vlan_gport_local_port_get_all(unit, vlan,
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if (SOC_IS_TRIDENT3X(unit)) {
            return _bcm_td3_vlan_match_action_multi_get(unit, match,
