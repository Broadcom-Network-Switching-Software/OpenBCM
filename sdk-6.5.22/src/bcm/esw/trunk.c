...
/* Added version for DLB hgt/lag to scache upgrade/downgrade. */
 ...
 */
            ...
            /* Allocate scache space for BCM_TRUNK_FLAG_IPMC_CLEAVE,
             * BCM_TRUNK_FLAG_NONUCAST_TRUNK_BLOCK_MASK_DISABLE flags
             */
        ...
        bcm_trunk_member_t *member)
{
    ...
    if (!SOC_IS_TRIDENT3X(unit) &&
       (trunk_info->flags & BCM_TRUNK_FLAG_NONUCAST_TRUNK_BLOCK_MASK_DISABLE)) {
        return BCM_E_UNAVAIL;
    }
...
#if defined(BCM_XGS3_SWITCH_SUPPORT) && defined(BCM_TRIDENT3_SUPPORT)
    /* reconcile Mirror to Port programming with trunk member ports. */
    if (SOC_IS_TRIDENT3X(unit) && BCM_SUCCESS(rv)) {
        rv = _bcm_xgs3_mirror_trunk_update(unit, tid);
    }
#endif  /* BCM_XGS3_SWITCH_SUPPORT && BCM_TRIDENT3_SUPPORT */
...
_bcm_trunk_reinit(int unit)
{
    ...
    int dlb_recover = 0;
#endif /* BCM_TRIDENT3_SUPPORT || BCM_TOMAHAWK2_SUPPORT */
    ...
    if (BCM_E_NOT_FOUND == rv) {
        ...
        if (recovered_ver >= BCM_WB_VERSION_1_1) {
            /* Recover BCM_TRUNK_FLAG_IPMC_CLEAVE,
             * BCM_TRUNK_FLAG_NONUCAST_TRUNK_BLOCK_MASK_DISABLE flags
             */
            for (tid = 0; tid < TRUNK_CNTL(unit).ngroups_fp; tid++) {
                ...
                if (flags & 0x2) {
                    t_info = &TRUNK_INFO(unit, tid);
                    t_info->flags |= BCM_TRUNK_FLAG_NONUCAST_TRUNK_BLOCK_MASK_DISABLE;
                }
            }
#if defined(BCM_SABER2_SUPPORT)
            if (SOC_IS_SABER2(unit) && soc_sb2_5626x_devtype(unit) &&
...
#if defined(BCM_TOMAHAWK2_SUPPORT) || defined(BCM_TRIDENT3_SUPPORT)
            if (soc_feature(unit, soc_feature_hgt_lag_dlb_optimized)) {
                dlb_recover  = SOC_IS_TRIDENT3X(unit) ?
                            (recovered_ver >= BCM_WB_VERSION_1_9) : 1;

                /* Recover HGT_LAG dynamic load balancing info */
                if (dlb_recover == 1) {
                    BCM_IF_ERROR_RETURN
                        (bcm_th2_hgt_lag_dlb_scache_recover(unit,
                             &trunk_scache));
                }
            }
...
_bcm_esw_trunk_sync(int unit)
{
    ...
    /* Store BCM_TRUNK_FLAG_IPMC_CLEAVE,
     * BCM_TRUNK_FLAG_NONUCAST_TRUNK_BLOCK_MASK_DISABLE flags
     */
    for (tid = 0; tid < TRUNK_CNTL(unit).ngroups_fp; tid++) {
        t_info = &TRUNK_INFO(unit, tid);
        ...
        if (t_info->flags & BCM_TRUNK_FLAG_NONUCAST_TRUNK_BLOCK_MASK_DISABLE) {
            flags |= 0x2;
        }
