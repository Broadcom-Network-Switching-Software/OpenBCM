    ...
    defined(BCM_SABER2_SUPPORT)
            ...
            if (IS_HG_PORT(unit, port)) {
                if (SOC_IS_TD2_TT2(unit)) {
                    soc_mem_lock(unit, LPORT_TABm);
                    rv = soc_mem_field32_modify(unit, LPORT_TABm, port,
                                                      IEEE_802_1AS_ENABLEf, 1);
                    if (BCM_FAILURE(rv)) {
                        soc_mem_unlock(unit, LPORT_TABm);
                        return BCM_E_FAIL;
                    }
                    soc_mem_unlock(unit, LPORT_TABm);
                }
                ...
                if (val == SOC_BLK_XLPORT) {
                    if (SOC_IS_HURRICANE2(unit) || SOC_IS_TD2P_TT2P(unit)) {
                        mode = (timestamping_mode ==
                               ...
                               SOC_IS_GREYHOUND2(unit) || SOC_IS_TRIDENT3X(unit) ||
                               SOC_IS_SABER2(unit) || SOC_IS_APACHE(unit) ||
                               SOC_IS_TOMAHAWKX(unit) || SOC_IS_FIRELIGHT(unit)) {
                        mode = (timestamping_mode ==
                                bcmTimesyncTimestampingMode48bit) ? 1 : 0;
                    } else {
                        continue;
                    ...
                    if (SOC_IS_HURRICANE2(unit) || SOC_IS_TD2P_TT2P(unit)) {
                        ...
                        *timestamping_mode =
                            ...
                            !soc_reg_field_get(unit, XLPORT_MODE_REGr, rval,
                               ...
                               SOC_IS_TRIDENT3X(unit) || SOC_IS_TOMAHAWKX(unit) ||
                               SOC_IS_SABER2(unit) || SOC_IS_APACHE(unit)) {
                        SOC_IF_ERROR_RETURN(
                            READ_XLPORT_MODE_REGr(unit, port,&rval));
                        *timestamping_mode =
