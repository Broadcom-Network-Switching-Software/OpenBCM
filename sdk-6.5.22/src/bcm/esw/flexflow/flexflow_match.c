        ...
        if (info->valid_elements & BCM_FLOW_MATCH_FLEX_DATA_VALID) {
            if (SOC_MEM_FIELD_VALID(unit, sgpp_view_id, IDf)) {
                ...
                if (SOC_IS_TRIDENT3(unit)) {
                    rv = bcm_esw_flow_handle_get(unit,"GBP",&gbp_handle);
                    if (BCM_SUCCESS(rv)) {
                    ...
                    if ((gbp_handle == info->flow_handle) &&
                        (gbp_option == info->flow_option)) {
...
{
    int rv = 0, jx = 0, num_views = 3;
    soc_mem_t view_id_array[3];
