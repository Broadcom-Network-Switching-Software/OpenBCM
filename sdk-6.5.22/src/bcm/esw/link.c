...
#define _BCM_LINK_PBMP_SCACHE_COPY(_pbmp_) \
    do { \
        sal_memcpy(link_scache_ptr, &_pbmp_, sizeof(bcm_pbmp_t)); \
        link_scache_ptr += sizeof(bcm_pbmp_t); \
    } while (0)
#define _BCM_LINK_PBMP_SCACHE_RECOVER(_pbmp_) \
    do { \
        sal_memcpy(&_pbmp_, link_scache_ptr, \
                   SOC_SCACHE_SIZE_MIN(pbmp_scache_sz, sizeof(bcm_pbmp_t))); \
        link_scache_ptr += pbmp_scache_sz; \
    } while (0)
...
_bcm_esw_link_sync(int unit)
{
    ...
    alloc_size = BCM_LINK_PBMP_SCACHE_NUM * sizeof(bcm_pbmp_t);
