...
/*
 * Function:
 *    _bcm_trident_trunk_dfg_blk_arr_print
 *
 * Purpose:
 *    Print the list of blocks in a defragmentation block array.
 *
 * Parameters:
 *    unit        - (IN) Unit number.
 *    minfo       - (IN) Group and Member Memory/Table details.
 *    inuse_count - (IN) Number of blocks in the blk_arr.
 *    inuse_blk   - (IN) Pointer to the defragmentation block array.
 *
 * Returns:
 *    BCM_E_NONE    - Success
 */
STATIC int
_bcm_trident_trunk_dfg_blk_arr_print(int unit,
                                     soc_defragment_mem_info_t minfo,
                                     uint32 inuse_count,
                                     soc_defragment_block_t *inuse_blk)
{
    uint32 b = 0; /* Index iterator. */
    shr_pb_t *pb = NULL; /* Debug print buffer. */

    if (inuse_count && inuse_blk) {
        pb = shr_pb_create();
        shr_pb_printf(pb,
                      "In-use Groups from %s tbl and its members in %s tbl:\n"
                      "----------------------------------------------------\n",
                      SOC_MEM_NAME(unit, minfo.base_mem),
                      SOC_MEM_NAME(unit, minfo.dfg_mem));
        for (b = 0; b < inuse_count; b++) {
            shr_pb_printf(pb,
                          "%4d. tid=%-4d base_ptr=%-5d size=%-4d\n",
                          b,
                          inuse_blk[b].group,
                          inuse_blk[b].base_ptr,
                          inuse_blk[b].size);
        }
...
/*
 * Function:
 *    _bcm_trident_trunk_inuse_blks_get
 *
 * Purpose:
 *    Get the list of blocks in use by the HiGig/Front-Panel Trunk Groups.
 *
 * Parameters:
 *    unit         - (IN)  Unit number.
 *    fabric_trunk - (IN)  Get the in use blocks of Fabric/HiGig Trunks when
 *                         enabled.
 *    minfo        - (IN)  Group and Member Memory/Table details.
 *    inuse_count  - (OUT) Size of inuse_blk array.
 *    inuse_blk    - (OUT) Pointer to the in use blocks array.
 *
 * Returns:
 *    BCM_E_UNIT       - Invalid BCM unit number.
 *    BCM_E_UNAVAIL    - Feature not supported.
 *    BCM_E_PARAM      - Invalid input parameter.
 *    BCM_E_MEMORY     - Memory error.
 *    BCM_E_NOT_FOUND  - No in-use blocks found.
 *    BCM_E_NONE       - Success
 */
STATIC int
_bcm_trident_trunk_inuse_blks_get(int unit,
                                  uint8 fabric_trunk,
                                  soc_defragment_mem_info_t minfo,
                                  uint32 *inuse_count,
                                  soc_defragment_block_t **inuse_blk)
{
    int grp_count = 0; /* Total number of groups supported by the device. */
    int sw_tid_offset = 0; /* SW data structure Trunk ID offset index value. */
    int rv = BCM_E_INTERNAL; /* Return value. */
    uint32 count = 0;        /* In-use blocks count. */
    bcm_trunk_chip_info_t chip_info; /* Trunk support of this device. */
    trunk_private_t *tinfo = NULL;   /* Trunk information. */
    bcm_trunk_t tid_min = BCM_TRUNK_INVALID; /* Min Trunk ID value. */
    bcm_trunk_t tid_max = BCM_TRUNK_INVALID; /* Max Trunk ID value. */
    bcm_trunk_t tid = BCM_TRUNK_INVALID;     /* TGID iterator. */
    soc_defragment_block_t *blk_arr = NULL;  /* blocks array. */
    uint32 edata[SOC_MAX_MEM_FIELD_WORDS] = {0}; /* Entry data buffer. */

    if (!SOC_IS_TRIDENT3X(unit)) {
        /*
         * Trunk defragmentation is currently supported and verified only for
         * the TD3 device.
         */
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: Supported only for TD3 device - rv=%s."
                              "\n"),
                   bcm_errmsg(BCM_E_UNAVAIL)));
        return (BCM_E_UNAVAIL);
    }
    /* Null pointer validation. */
    if (inuse_count == NULL || inuse_blk == NULL) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: Null Ptr: inuse_count or inuse_blk - "
                              "rv=%s.\n"),
                   bcm_errmsg(BCM_E_PARAM)));
        return (BCM_E_PARAM);
    }
    /* Validate defragmentation memory and field values. */
    if (minfo.base_mem == INVALIDm || minfo.dfg_mem == INVALIDm
        || minfo.base_ptr_field == INVALIDf || minfo.size_field == INVALIDf) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: Invalid soc_defragment_mem_info_t struct "
                              "members - rv=%s.\n"),
                   bcm_errmsg(BCM_E_PARAM)));
        return (BCM_E_PARAM);
    }

    /* Get the trunk support of this device. */
    bcm_trunk_chip_info_t_init(&chip_info);
    BCM_IF_ERROR_RETURN
        (bcm_esw_trunk_chip_info_get(unit, &chip_info));

    /*
     * Get the Trunk Group ID index range and the number of Trunk groups
     * supported by this device.
     */
    if (fabric_trunk) {
        /* Fabric/HG Trunk table params. */
        tid_min = chip_info.trunk_fabric_id_min - chip_info.trunk_fabric_id_min;
        tid_max = chip_info.trunk_fabric_id_max - chip_info.trunk_fabric_id_min;
        /*
         * Get the number of HG trunks by deducting front-panel trunk groups
         * count from the total trunk groups on this device.
         */
        grp_count = (tid_max - tid_min) + 1;
        sw_tid_offset = chip_info.trunk_fabric_id_min;
    } else {
        /* Front-panel trunk table params. */
        tid_min = chip_info.trunk_id_min;
        tid_max = chip_info.trunk_id_max;
        grp_count = chip_info.trunk_group_count;
        sw_tid_offset = chip_info.trunk_id_min;
    }
    /* Iterate over the groups and check for in-use status. */
    for (tid = tid_min; tid <= tid_max && count < grp_count; tid++) {
        tinfo = _bcm_esw_trunk_info_get(unit,
                                        (tid + sw_tid_offset));
        if (tinfo == NULL) {
            LOG_ERROR(BSL_LS_BCM_TRUNK,
                      (BSL_META_U(unit,
                                  "Error: Null Ptr(%s): tid=%d + "
                                  "sw_tid_offset=%d - rv=%s.\n"),
                       SOC_MEM_NAME(unit, minfo.base_mem),
                       tid,
                       sw_tid_offset,
            ...
            /*
             * The Trunk Init routine must have allocated resources to maintain
             * trunk information for all groups supported by this device. So,
             * we should't have hit this condition. Check the trunk
             * initialization routine for proper initialization.
             */
            return (BCM_E_INTERNAL);
        }
        if (!tinfo->in_use) {
            /* Skip the trunk groups which are not in-use. */
            continue;
        }

        sal_memset(edata, 0, SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32));
        rv = soc_mem_read(unit, minfo.base_mem, MEM_BLOCK_ANY, tid, edata);
        if (SOC_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_TRUNK,
                      (BSL_META_U(unit,
                                  "Error: soc_mem_read(%s): tid=%d - rv=%s.\n"),
                       SOC_MEM_NAME(unit, minfo.base_mem),
                       tid,
                       bcm_errmsg(rv)));
            if (blk_arr) {
        ...
        /*
         * Allocate memory to store in use group block details, if memory
         * has not been allocated yet.
         */
        if (blk_arr == NULL && count == 0) {
            blk_arr = sal_alloc(grp_count * sizeof(*blk_arr),
                                "bcmtrunkDfgBlkArr");
            if (blk_arr == NULL) {
                LOG_ERROR(BSL_LS_BCM_TRUNK,
                          (BSL_META_U(unit,
                                      "Error: blk_arr memory alloc failed - "
                                      "rv=%s.\n"),
                           bcm_errmsg(BCM_E_MEMORY)));
                return (BCM_E_MEMORY);
            }
            sal_memset(blk_arr, 0, grp_count * sizeof(*blk_arr));
        }
        /* Get the BASE_PTRf value. */
        blk_arr[count].base_ptr = soc_mem_field32_get(unit,
                                                      minfo.base_mem,
                                                      edata,
                                                      minfo.base_ptr_field);
        /*
         * The (group size - 1) i.e. TG_SIZEf field value from HW entry, for
         * actual group size add 1 to this field value.
         */
        blk_arr[count].size = (soc_mem_field32_get(unit,
                                                  minfo.base_mem,
                                                  edata,
                                                  minfo.size_field) + 1);
        blk_arr[count].group = tid;
        count++;
    }

    /* Check and return no in-use blocks found error. */
    if (count == 0 || blk_arr == NULL) {
        LOG_DEBUG(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Debug: %s.%s No in-use blks count=%u "
                              "tid_min=%d tid_max=%d grp_count=%d "
                              "sw_tid_offset=%d - rv=%s\n"),
                   SOC_MEM_NAME(unit, minfo.base_mem),
                   SOC_MEM_NAME(unit, minfo.dfg_mem),
                   count,
    ...
    *inuse_blk = blk_arr;
    ...
    /* Print the sorted in-use blocks array when debugging is enabled. */
    LOG_DEBUG(BSL_LS_BCM_TRUNK,
              (BSL_META_U(unit,
                          "Debug: %s.%s total in-use blks count=%u - "
                          "rv=%d\n"),
               SOC_MEM_NAME(unit, minfo.base_mem),
               SOC_MEM_NAME(unit, minfo.dfg_mem),
               count,
               _bcm_trident_trunk_dfg_blk_arr_print(unit,
                                                    minfo,
                                                    *inuse_count,
                                                    *inuse_blk)));
    return rv;
...
/*
 * Function:
 *    _bcm_trident_trunk_dfg_cleanup
 *
 * Purpose:
 *    Cleanup routine to free any memory allocated for defragmentation.
 *
 * Parameters:
 *    inuse_blk     - (IN) Pointer to in use blocks array memory.
 *    mblk_count    - (IN) Total number of blocks in bcur_loc and bmov_seq.
 *    bcur_loc      - (IN) Pointer to blocks current location array memory.
 *    bmov_seq      - (IN) Pointer to blocks move sequence array memory.
 *    free_blk      - (IN) Pointer to free blocks array memory.
 *
 * Returns:
 *     None.
 */
STATIC void
_bcm_trident_trunk_dfg_cleanup(soc_defragment_block_t *inuse_blk,
                               uint32 mblk_count,
                               ...
                               soc_defragment_block_t *free_blk)
{
    ...
    if (mblk_count > 0 && bmov_seq) {
        /*
         * Iterate over the blocks in the move sequence array and free memory
         * allocated to store the list of base pointer values for blocks that
         * have a non-zero move count (mov_count) value.
         */
        for (b = 0; b < mblk_count; b++) {
...
/*
 * Function:
 *    _bcm_trident_trunk_dfg_blk_mov
 *
 * Purpose:
 *    Move a block of entries from the source base_ptr location to the
 *    destination base_ptr location in the table, also update the group
 *    base_ptr for the group pointing to this block of entries. This routine
 *    follows make-before-break sequence while moving this block and ensure
 *    the group is pointing to its corresponding set of member table entries.
 *
 * Parameters:
 *    unit         - (IN) Unit number.
 *    fabric_trunk - (IN) If enabled the block move is for a fabric/HiGig Trunk.
 *    minfo        - (IN) Group and Member memory/table details.
 *    pb           - (IN) Debug print buffer.
 *    src          - (IN) Block move source location.
 *    dst          - (IN) Block move destination location.
 *
 * Returns:
 *    BCM_E_UNIT       - Invalid BCM unit number.
 *    BCM_E_PARAM      - Invalid input parameter.
 ...
 */
STATIC int
_bcm_trident_trunk_dfg_blk_mov(int unit,
                               uint8 fabric_trunk,
                               soc_defragment_mem_info_t minfo,
                               shr_pb_t *pb,
                               soc_defragment_block_t src,
                               soc_defragment_block_t dst)
{
    int rv = BCM_E_INTERNAL; /* Return value. */
    int alloc_size = 0; /* Allocated memory size in bytes. */
    uint8 *dmabuf = NULL; /* Member table entries DMA buffer. */
    uint32 edata[SOC_MAX_MEM_FIELD_WORDS] = {0}; /* Entry data buffer. */
    void *null_entry = NULL; /* Null entry pointer. */

    /* Validate input parameters. */
    if (src.size == 0 || dst.size == 0) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: src.size=%d dst.size=%d - rv=%s\n"),
                   src.size,
                   dst.size,
                   bcm_errmsg(BCM_E_PARAM)));
        return (BCM_E_PARAM);
    ...
    if (pb == NULL) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: Null print buffer (pb) ptr - rv=%s\n"),
                   bcm_errmsg(BCM_E_PARAM)));
        return (BCM_E_PARAM);
    }

    /*
     * Determine the alloc size in bytes, each entry in the table rounded up
     * to nearest word size and then converted to bytes.
     */
    alloc_size = (soc_mem_entry_words(unit, minfo.dfg_mem) * sizeof(uint32))
                    * src.size;
    /* Allocate DMAable memory for all the entries in this block. */
    dmabuf = soc_cm_salloc(unit, alloc_size, "bcmtrunkDfgDmaMbuf");
    if (NULL == dmabuf) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: soc_cm_salloc(dmabuf) - rv=%s\n"),
                   bcm_errmsg(BCM_E_MEMORY)));
        return (BCM_E_MEMORY);
    }
    /*
     * Copy HW table entries from the source location to the destination
     * location and update the group table base_ptr field to point to the new
     * block of entries@dst location.
     */
    rv = soc_mem_read_range(unit,
                            minfo.dfg_mem,
                            MEM_BLOCK_ANY,
                            src.base_ptr,
                            (src.base_ptr + src.size) - 1,
                            dmabuf);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: soc_mem_read_range() - rv=%s\n"),
                   soc_errmsg(BCM_E_MEMORY)));
    ...
    rv = soc_mem_write_range(unit,
                             ...
                             MEM_BLOCK_ANY,
                             dst.base_ptr,
                             (dst.base_ptr + dst.size) - 1,
                             dmabuf);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: soc_mem_write_range() - rv=%s\n"),
                   soc_errmsg(BCM_E_MEMORY)));
        soc_cm_sfree(unit, dmabuf);
        dmabuf = NULL;
        return rv;
    ...
    /* Update the base_ptr in the Group table */
    sal_memset(edata, 0, SOC_MAX_MEM_FIELD_WORDS * sizeof(uint32));
    ...
    /* Set the new base_ptr index value. */
    soc_mem_field32_set(unit,
                        minfo.base_mem,
                        edata,
                        minfo.base_ptr_field,
                        dst.base_ptr);
    /* Update the base_ptr field in the group table. */
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: soc_mem_write(%s): tid=%d - rv=%s.\n"),
                   SOC_MEM_NAME(unit, minfo.base_mem),
                   src.group,
                   soc_errmsg(rv)));
        return rv;
    ...
    /*
     * Update the SW state of the member table entries in the destination
     * index range.
     */
    if (fabric_trunk) {
        _BCM_HG_TRUNK_MEMBER_USED_SET_RANGE(unit, dst.base_ptr, dst.size);
        shr_pb_printf(pb,
                      "HG Set: tid/src.group=%-4d dst.base_ptr=%-5d "
                      "size=%-4d\n",
                      src.group,
                      dst.base_ptr,
                      dst.size);
    } else {
        _BCM_FP_TRUNK_MEMBER_USED_SET_RANGE(unit, dst.base_ptr, dst.size);
        shr_pb_printf(pb,
                      "FP Set: tid/src.group=%-4d dst.base_ptr=%-5d "
                      "size=%-4d\n",
                      src.group,
                      dst.base_ptr,
                      dst.size);
    ...
    /* Determine null_entry buffer size and allocate DMAable buffer. */
    alloc_size = soc_mem_entry_words(unit,
                                      minfo.dfg_mem) * sizeof(uint32);
    null_entry = soc_cm_salloc(unit,
                               alloc_size,
                               "bcmtrunkDfgNullEntry");
    if (null_entry == NULL) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: soc_cm_salloc(null_entry) - rv=%s\n"),
                   bcm_errmsg(BCM_E_MEMORY)));
        return (BCM_E_MEMORY);
    }
    /* Copy null entry value for this memory. */
    sal_memcpy(null_entry,
               soc_mem_entry_null(unit, minfo.dfg_mem),
               alloc_size);
    /*
     * Clear the HW table entries at the source location. This function
     * writes the null_entry value in the range of indices supplied to this
     * function.
     */
    ...
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: soc_mem_write_range(null_entry) - "
                              "rv=%s\n"),
                   soc_errmsg(BCM_E_MEMORY)));
    ...
    /*
     * Clear the SW state of the member table entries in the source indexx
     * range.
     */
    if (fabric_trunk) {
        _BCM_HG_TRUNK_MEMBER_USED_CLR_RANGE(unit, src.base_ptr, src.size);
        shr_pb_printf(pb,
                      "HG Clr: tid/src.group=%-4d src.base_ptr=%-5d "
                      "size=%-4d\n",
                      src.group,
                      src.base_ptr,
                      src.size);
    } else {
        _BCM_FP_TRUNK_MEMBER_USED_CLR_RANGE(unit, src.base_ptr, src.size);
        shr_pb_printf(pb,
                      "FP Clr: tid/src.group=%-4d src.base_ptr=%-5d "
                      "size=%-4d\n",
                      src.group,
                      src.base_ptr,
                      src.size);
...
/*
 * Function:
 *    _bcm_trident_trunk_dfg_seq_exec
 *
 * Purpose:
 *    Execute the defragmentation blocks move sequence and apply changes to
 *    the hardware tables/memories and also update the software state of the
 *    Front-Panel or HiGigi/Fabric Trunk member table.
 *
 * Parameters:
 *    unit         - (IN) Unit number.
 *    fabric_trunk - (IN) If enabled the defragmentation sequence is for a
 *                        fabric/HiGig trunks else it is for front panel
 *                        trunks.
 *    minfo        - (IN) Group and Member memory/table details.
 *    mblk_count   - (IN) Total number of blocks in bcur_loc and bmov_seq array.
 *    bcur_loc     - (IN) Pointer to blocks current location array.
 *    bmov_seq     - (IN) Pointer to blocks move sequence array.
 *
 * Returns:
 *    BCM_E_UNIT       - Invalid BCM unit number.
 *    BCM_E_PARAM      - Invalid input parameter.
 ...
 */
STATIC int
_bcm_trident_trunk_dfg_seq_exec(int unit,
                                uint8 fabric_trunk,
                                soc_defragment_mem_info_t minfo,
                                uint32 mblk_count,
                                soc_defragment_block_t *bcur_loc,
...
{
    int rv = BCM_E_INTERNAL; /* Return value. */
    uint32 b = 0; /* Block current location array index iterator. */
    uint32 m = 0; /* Block move sequence array index. */
    soc_defragment_block_t src; /* Block source location. */
    soc_defragment_block_t dst; /* Block destination location. */
    shr_pb_t *pb = NULL; /* Debug print buffer. */

    /* Validate input parameters. */
    if (mblk_count == 0 || bcur_loc == NULL || bmov_seq == NULL) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: mblk_count=%u - rv=%s\n"),
                   mblk_count,
                   bcm_errmsg(rv)));
        return (BCM_E_PARAM);
    }
    /* Validate base memory value. */
    if ((fabric_trunk == 0 && minfo.base_mem != TRUNK_GROUPm)
        || (fabric_trunk && minfo.base_mem != HG_TRUNK_GROUPm)) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: Invalid base_mem=%s - rv=%s\n"),
                   SOC_MEM_NAME(unit, minfo.base_mem),
                   bcm_errmsg(BCM_E_PARAM)));
        return (BCM_E_PARAM);
    }
    /* Validate defragmentation memory value. */
    if ((fabric_trunk == 0 && minfo.dfg_mem != TRUNK_MEMBERm)
        || (fabric_trunk && minfo.dfg_mem != HG_TRUNK_MEMBERm)) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: Invalid dfg_mem=%s - rv=%s\n"),
                   SOC_MEM_NAME(unit, minfo.dfg_mem),
                   bcm_errmsg(BCM_E_PARAM)));
        return (BCM_E_PARAM);
    }
    ...
    pb = shr_pb_create();
    /* Execute the defragmentation blocks move sequence. */
    for (b = 0; b < mblk_count; b++) {
        soc_defragment_block_t_init(&src);
        soc_defragment_block_t_init(&dst);

        /* Initialize the block source location. */
        src.base_ptr = bcur_loc[b].base_ptr;
        src.size = bcur_loc[b].size;
        src.group = bcur_loc[b].group;

        for (m = 0; m < bmov_seq[b].mov_count; m++) {
            dst.base_ptr = bmov_seq[b].base_ptr[m];
            dst.size = bmov_seq[b].size;
            dst.group = bcur_loc[b].group;
            rv = _bcm_trident_trunk_dfg_blk_mov(unit,
                                                fabric_trunk,
                                                minfo,
                                                pb,
                                                src,
            ...
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_TRUNK,
                          (BSL_META_U(unit,
                                      "Error: _bcm_trident_trunk_dfg_blk_mov()"
                                      " b=%u m=%u tid=%d base_ptr=%d size=%d "
                                      "- rv=%s\n"),
                           b, m, dst.group, dst.base_ptr, dst.size,
                           bcm_errmsg(rv)));
                shr_pb_destroy(pb);
                return rv;
            }
            /*
             * Current destination location becomes the new source location for
             * the next move of this block.
             */
            src = dst;
        }
    }
...
/*
 * Function:
 *    _bcm_trident_trunk_defragment
 *
 * Purpose:
 *    Routine to defragment HiGig and Front Panel Trunk Memories/Tables.
 *
 * Parameters:
 *    unit            - (IN) Unit number.
 *    fabric_trunk    - (IN) If enabled the defragmentation sequence is for a
 *                           fabric/HiGig trunks else it is for front panel
 *                           trunks.
 *    t_info          - (IN) Pointer to trunk information structure.
 *    req_num_entries - (IN) Number of member table entries requested in the
 *                           new alloc/set call.
 *
 * Returns:
 *    BCM_E_UNIT     - Invalid BCM unit number.
 *    BCM_E_PARAM    - Invalid input parameter.
 *    BCM_E_MEMORY   - Memory error.
 *    BCM_E_INTERNAL - Unexpected internal error.
 *    BCM_E_RESOURCE - Not enough free entries to complete the new alloc
 *                     request.
 *    BCM_E_NONE     - Success
 */
STATIC int
_bcm_trident_trunk_defragment(int unit,
                              uint8 fabric_trunk,
                              trunk_private_t *t_info,
                              int req_num_entries)
{
    int rv = BCM_E_INTERNAL; /* Return value. */
    soc_defragment_mem_info_t minfo; /* Defragmentation memories info. */
    uint32 inuse_count = 0;                   /* Inuse blocks count. */
    soc_defragment_block_t *inuse_blk = NULL; /* Inuse blocks array. */
    uint32 mblk_count;                        /* Inuse blocks moved count. */
    soc_defragment_block_t *bcur_loc = NULL;  /* Moved inuse block cur loc. */
    soc_defragment_blk_mov_seq_t *bmov_seq = NULL; /* Blk move sequence. */
    uint32 free_count = 0;                    /* Free blocks count. */
    soc_defragment_block_t *free_blk = NULL;  /* Free blocks array. */
    soc_defragment_block_t lfree_blk;         /* Largest free block. */

    /* Validate input parameters. */
    if (t_info == NULL) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: t_info ptr is null - rv=%s\n"),
                   bcm_errmsg(BCM_E_PARAM)));
        return (BCM_E_PARAM);
    }
    if (req_num_entries == 0) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: req_num_entries=%d is invalid - rv=%s\n"),
                   req_num_entries,
                   bcm_errmsg(BCM_E_PARAM)));
        return (BCM_E_PARAM);
    }
    ...
    soc_defragment_mem_info_t_init(&minfo);

    if (fabric_trunk) {
        /*
         * Setup the HiGig trunk group and member table details
         * required for defragmentation.
         */
        minfo.base_mem = HG_TRUNK_GROUPm;
        minfo.base_ptr_field = BASE_PTRf;
        minfo.size_field = TG_SIZEf;
        minfo.dfg_mem = HG_TRUNK_MEMBERm;
        /* The entry@index=0 is reserved for non-Katana devices. */
        minfo.first_entry_index = !SOC_IS_KATANAX(unit) ? 1 : 0;
        minfo.last_entry_index = soc_mem_index_max(unit, HG_TRUNK_MEMBERm);
    } else {
        /*
         * Setup the front-panel trunk group and member table details
         * required for defragmentation.
         */
        minfo.base_mem = TRUNK_GROUPm;
        minfo.base_ptr_field = BASE_PTRf;
        minfo.size_field = TG_SIZEf;
        minfo.dfg_mem = TRUNK_MEMBERm;
        /* Check and skip entry@index=0 if it is reserved. */
        minfo.first_entry_index = bcm_trident_trunk_mbr_zero_rsvd[unit] ? 1 : 0;
        minfo.last_entry_index = soc_mem_index_max(unit, TRUNK_MEMBERm);
    }

    /* Get the list of blocks linked to in-use trunk groups. */
    rv = _bcm_trident_trunk_inuse_blks_get(unit,
                                           fabric_trunk,
                                           minfo,
                                           &inuse_count,
                                           &inuse_blk);
    ...
    /*
     * Run the defragmentation algorithm and get the blk move sequence along
     * with the free block/s that are expected post defragmentation.
     */
    soc_defragment_block_t_init(&lfree_blk);
    rv = soc_defragment_sequence_get(unit,
                                     minfo,
                                     inuse_count,
                                     inuse_blk,
                                     &mblk_count,
    ...
    if (BCM_FAILURE(rv)) {
        LOG_DEBUG(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "soc_defragment_sequence_get() - rv=%s\n"),
                   bcm_errmsg(rv)));
        if (inuse_blk) {
            /* Free in-use blocks memory. */
            sal_free(inuse_blk);
            inuse_blk = NULL;
        }
        return rv;
    ...
    /*
     * Validate defragmentation output parameters and return E_INTERNAL error
     * if there are any unexpected output parameter values.
     */
    if ((mblk_count > 0 && (bcur_loc == NULL || bmov_seq == NULL))
        || (free_count > 0 && free_blk == NULL)) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: Null ptr bcur_loc / bmov_seq / free_blk "
                              "mblk_count=%d free_count=%d - rv=%s\n"),
                   mblk_count,
                   free_count,
                   bcm_errmsg(BCM_E_INTERNAL)));

        /* Cleanup memory allocated for block move sequence. */
        _bcm_trident_trunk_dfg_cleanup(inuse_blk,
                                       mblk_count,
                                       bcur_loc,
                                       bmov_seq,
                                       free_blk);
        /* Unexpected exception condition. */
        return BCM_E_INTERNAL;
    }
    /*
     * Check the number of free entries in the largest free block post
     * defragmentation, check if it can satisfy the new alloc request. If it
     * cannot be satisfied even after defragmentation of the table, then return
     * E_RESOURCE error.
     */
    if (req_num_entries > lfree_blk.size) {
        LOG_DEBUG(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Debug: ReqBlk.Enries=%d > Defrag."
                              "LargestFreeBlk.size=%d - rv=%s\n"),
                   req_num_entries,
                   lfree_blk.size,
                   bcm_errmsg(BCM_E_RESOURCE)));
        ...
        _bcm_trident_trunk_dfg_cleanup(inuse_blk,
                                       ...
                                       bmov_seq,
                                       free_blk);
        /* Not enough entries even after defragmentation. */
        return BCM_E_RESOURCE;
    }

    /* Apply the defragmentation sequence to the base_mem and dfg_mem tables. */
    rv = _bcm_trident_trunk_dfg_seq_exec(unit,
                                         fabric_trunk,
                                         minfo,
                                         mblk_count,
                                         bcur_loc,
    ...
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_TRUNK,
                  (BSL_META_U(unit,
                              "Error: _bcm_trident_trunk_dfg_seq_exec() "
                              "- rv=%s\n"),
                   bcm_errmsg(rv)));
        /* Cleanup memory allocated for block move sequence. */
        _bcm_trident_trunk_dfg_cleanup(inuse_blk,
    ...
    /* Cleanup defragmentation resources. */
    _bcm_trident_trunk_dfg_cleanup(inuse_blk,
                                   mblk_count,
                                   bcur_loc,
                                   bmov_seq,
...
_bcm_trident_lag_slb_free_resource(int unit, int tid)
{
    ...
    if ((bcm_trident_trunk_mbr_zero_rsvd[unit] && base_ptr != 0) ||
        (!bcm_trident_trunk_mbr_zero_rsvd[unit])) {
        num_entries = tg_size;
        if (rtag >= 1 && rtag <= 6) {
#ifdef BCM_KATANA_SUPPORT
            if (SOC_IS_KATANAX(unit)) {
...
/*
 * Function:
 *      _bcm_trident_lag_slb_restore_resource
 * Purpose:
 *      Restore resources for a LAG static load balancing group.
 * Parameters:
 *      unit  - (IN) SOC unit number.
 *      tid - (IN) Trunk group ID.
 * Returns:
 ...
 */
STATIC int
_bcm_trident_lag_slb_restore_resource(int unit, int tid)
{
    trunk_group_entry_t tg_entry;
    int base_ptr;
    int tg_size;
...
#if defined(BCM_TRIDENT3_SUPPORT)
        if (SOC_IS_TRIDENT3X(unit)) {
            rv = _bcm_trident_trunk_defragment(unit,
                                               0,
                                               t_info,
                                               num_entries);
            if (BCM_FAILURE(rv)) {
                LOG_DEBUG(BSL_LS_BCM_TRUNK,
                          (BSL_META_U(unit,
                                      "_bcm_trident_trunk_defragment(FP)"
                                      " - rv=%s\n"),
                           bcm_errmsg(rv)));
                return (rv);
            }
            /*
             * Retry allocating the block of entries now, as the table has been
             * successfully defragmented and required number of free entries are
             * available.
             */
            for (base_ptr = 0; base_ptr <= max_base_ptr; base_ptr++) {
                _BCM_FP_TRUNK_MEMBER_TEST_RANGE
                    (unit, base_ptr, num_entries, occupied);
                if (!occupied) {
                    break;
                }
            }
            if (base_ptr > max_base_ptr) {
                /* Unlikely as defragmentation was successful. */
                LOG_ERROR(BSL_LS_BCM_TRUNK,
                          (BSL_META_U(unit,
                                      "Error: base_ptr(%d) > max_base_ptr(%d)"
        ...
        uint32 flags)
{
    ...
    if  (flags & BCM_TRUNK_FLAG_NONUCAST_TRUNK_BLOCK_MASK_DISABLE) {
        return rv;
    }
        ...
        bcm_trunk_member_t *member)
{
    ...
    if (BCM_FAILURE(rv)) {
        _bcm_trident_lag_slb_restore_resource(unit, tid);
        _BCM_TRIDENT_TRUNK_FP_SET_FREE_MEM_RETURN(rv);
    }
        ...
        trunk_private_t *t_info)
{
    ...
    if ((bcm_trident_trunk_mbr_zero_rsvd[unit] && old_base_ptr != 0) ||
        (!bcm_trident_trunk_mbr_zero_rsvd[unit])) {
        old_num_entries = old_tg_size;
        if (old_rtag >= 1 && old_rtag <= 6) {
#ifdef BCM_KATANA_SUPPORT
...
/*
 * Function:
 *      _bcm_trident_hg_slb_restore_resource
 * Purpose:
 *      Restore resources for a Higig static load balancing group.
 * Parameters:
 *      unit  - (IN) SOC unit number.
 *      hgtid - (IN) Higig trunk group ID.
 * Returns:
 ...
 */
STATIC int
_bcm_trident_hg_slb_restore_resource(int unit, int hgtid)
{
    hg_trunk_group_entry_t hg_trunk_group_entry;
    int base_ptr;
    int tg_size;
    ...
    if (base_ptr > max_base_ptr) {
        ...
        if (SOC_IS_TRIDENT3X(unit)
            ...
            if (BCM_FAILURE(rv)) {
                LOG_DEBUG(BSL_LS_BCM_TRUNK,
                          (BSL_META_U(unit,
                                      "_bcm_trident_trunk_defragment(HG)"
                                      " - rv=%s\n"),
                           bcm_errmsg(rv)));
                return (rv);
            }
            /*
             * Retry allocating the block of entries now, as the table has been
             * successfully defragmented and required number of free entries
             * are available.
             */
            for (base_ptr = 0; base_ptr <= max_base_ptr; base_ptr++) {
                /*
                 ...
                 * index base_ptr to (base_ptr + num_entries - 1) is free.
                 */
                _BCM_HG_TRUNK_MEMBER_TEST_RANGE
                        (unit, base_ptr, num_entries, occupied);
                if (!occupied) {
                    break;
                }
        ...
        bcm_trunk_member_t *member)
{
    ...
    rv = _bcm_trident_hg_slb_set(unit, hgtid, add_info,
                                 t_info, trunk_flag);
    if (BCM_FAILURE(rv)) {
        _bcm_trident_hg_slb_restore_resource(unit, hgtid);
        return rv;
    }
