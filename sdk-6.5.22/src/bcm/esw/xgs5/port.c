                                 ...
                                 uint32 mask)
{
    ...
    int logic_port, phy_port, blk, bindex, btype_idx, first_p, blktype, check_lane_1_active;
    int speed, first_speed, num_lanes, encap, first_encap, ovs, first_ovs, check_lr_mix_speed;
    int first_port_null = 0;
    ...
    check_lane_1_active =
        mask & BCMI_XGS5_FLEXPORT_RESTRICTION_PM_FIRST_LANE_ACTIVE ? 1 : 0;
    ...
    SHR_BIT_ITER(blk_bmp, SOC_MAX_NUM_BLKS, blk) {
        ...
        for (i = 0; i < BCMI_XGS5_PORTS_PER_PBLK; i++) {
           ...
           /* 1st lane of PM must be active (speed > 0). */
            if ((check_lane_1_active) && (first_p == 1) && (logic_port == -1)) {
                first_port_null = 1;
            }
        ...
        /* First lane of PM with active ports must be active (speed > 0). */
         if ((check_lane_1_active) && (first_port_null == 1) && (speed != 0)) {
             LOG_ERROR(BSL_LS_BCMAPI_PORT,
                   (BSL_META_U(unit,
                                "-- Restriction: 1st lane of PM cannot be inactive.\n")));
             sal_free(sister_ports);
             return BCM_E_PARAM;
         }
    }
                       ...
                       bcmi_xgs5_port_resource_bmp_t *pr_bmp)
{
    ...
    if (SOC_IS_HELIX5(unit)) {
        SHR_BIT_ITER(pr_bmp->add_bmp_pm, SOC_MAX_NUM_BLKS, blk) {
            soc_helix5_port_blk_reset(unit, blk);
        }
    }
#endif
#ifdef BCM_HURRICANE4_SUPPORT
    if (SOC_IS_HURRICANE4(unit)) {
        SHR_BIT_ITER(pr_bmp->add_bmp_pm, SOC_MAX_NUM_BLKS, blk) {
            soc_hurricane4_port_blk_reset(unit, blk);
        }
    }
#endif
