...
_higig_match_set(bcm_knet_filter_t *filter, uint32 *mh_data, uint32 *mh_mask)
{
    ...
    if (filter->match_flags & BCM_KNET_FILTER_M_MIRRORED) {
        hg_data->overlay1.mirror = filter->m_mirrored;
        hg_mask->overlay1.mirror = 0;
    }
...
_higig2_match_set(bcm_knet_filter_t *filter, uint32 *mh_data, uint32 *mh_mask)
{
    ...
    if (filter->match_flags & BCM_KNET_FILTER_M_MIRRORED) {
        hg_data->ppd_overlay1.mirror = filter->m_mirrored;
        hg_mask->ppd_overlay1.mirror = 0;
    }
...
STATIC void
_mirrored_match_get(int unit, bcm_knet_filter_t *filter,
                  dcb_t *dcb_data, dcb_t *dcb_mask)
{
#if defined(BCM_HIGIG2_SUPPORT)
    uint32 d[4], m[4];
    soc_higig2_hdr_t *hg_data = (soc_higig2_hdr_t *)&d[0];
    soc_higig2_hdr_t *hg_mask = (soc_higig2_hdr_t *)&m[0];
    ...
    if (SOC_DCB_TYPE(unit) == 38) {
        ep_to_cpu_hdr38_t *data = (ep_to_cpu_hdr38_t *)dcb_data;
        ep_to_cpu_hdr38_t *mask = (ep_to_cpu_hdr38_t *)dcb_mask;

        d[0] = soc_htonl(data->mh0);
        d[1] = soc_htonl(data->mh1);
        d[2] = soc_htonl(data->mh2);
        d[3] = soc_htonl(data->mh3);
        m[0] = soc_htonl(mask->mh0);
        m[1] = soc_htonl(mask->mh1);
        m[2] = soc_htonl(mask->mh2);
        m[3] = soc_htonl(mask->mh3);
    } else
#endif
#if defined(BCM_TRIDENT3_SUPPORT)
    if (SOC_DCB_TYPE(unit) == 36) {
        ep_to_cpu_hdr36_t *data = (ep_to_cpu_hdr36_t *)dcb_data;
        ep_to_cpu_hdr36_t *mask = (ep_to_cpu_hdr36_t *)dcb_mask;
    ...
    if (SOC_DCB_TYPE(unit) == 40) {
        ep_to_cpu_hdr40_t *data = (ep_to_cpu_hdr40_t *)dcb_data;
        ep_to_cpu_hdr40_t *mask = (ep_to_cpu_hdr40_t *)dcb_mask;
    ...
    if (SOC_DCB_TYPE(unit) == 23 ||
        SOC_DCB_TYPE(unit) == 26 ||
        SOC_DCB_TYPE(unit) == 29 ||
        SOC_DCB_TYPE(unit) == 31 ||
        SOC_DCB_TYPE(unit) == 32 ||
        SOC_DCB_TYPE(unit) == 33 ||
        SOC_DCB_TYPE(unit) == 34 ||
        SOC_DCB_TYPE(unit) == 35 ||
        SOC_DCB_TYPE(unit) == 37)
    {
        _knet_dcb_type2_t *data = (_knet_dcb_type2_t *)dcb_data;
        _knet_dcb_type2_t *mask = (_knet_dcb_type2_t *)dcb_mask;
        ...
        _knet_dcb_type1_t *data = (_knet_dcb_type1_t *)dcb_data;
        _knet_dcb_type1_t *mask = (_knet_dcb_type1_t *)dcb_mask;
    ...
    if (hg_mask->ppd_overlay1.mirror) {
        filter->match_flags |= BCM_KNET_FILTER_M_MIRRORED;
        filter->m_mirrored = hg_data->ppd_overlay1.mirror;
    }
#endif
}
...
bcm_esw_knet_filter_create(int unit, bcm_knet_filter_t *filter)
{
    ...
    if (SOC_DCB_TYPE(unit) == 11) {
        ...
        if (filter->match_flags & BCM_KNET_FILTER_M_CPU_COS) {
            dcb_data->cpu_cos = filter->m_cpu_cos;
            dcb_mask->cpu_cos = 0;
        }
    } else
#endif
    ...
    if (SOC_DCB_TYPE(unit) == 21) {
        ...
        if (filter->match_flags & BCM_KNET_FILTER_M_MIRRORED) {
            LOG_WARN(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                                 "KNET: Unsupported flag "
                                 "BCM_KNET_FILTER_M_MIRRORED."
                                 "Please use BCM_KNET_FILTER_M_REASON for "
                                 "mirror filter\n")));
            return BCM_E_PARAM;
        }
        /* Set module header match fields */
    ...
    if (SOC_DCB_TYPE(unit) == 23) {
        ...
        if (filter->match_flags & BCM_KNET_FILTER_M_CPU_COS) {
            dcb_data->word4.overlay2.cpu_cos = filter->m_cpu_cos;
            dcb_mask->word4.overlay2.cpu_cos = 0;
        }
    } else
#endif
    ...
    if (SOC_DCB_TYPE(unit) == 24) {
        ...
        if (filter->match_flags & BCM_KNET_FILTER_M_CPU_COS) {
            dcb_data->word4.overlay1.queue_num = filter->m_cpu_cos;
            dcb_mask->word4.overlay1.queue_num = 0;
        }
    } else
#endif
    ...
    if (SOC_DCB_TYPE(unit) == 36) {
        ...
        if (filter->match_flags & BCM_KNET_FILTER_M_CPU_COS) {
            hdr_data->word8.overlay1.cpu_cos = filter->m_cpu_cos;
            hdr_mask->word8.overlay1.cpu_cos = 0;
        }
    } else
#endif
        ...
        if (SOC_DCB_TYPE(unit) == 40) {
            ...
            if (filter->match_flags & BCM_KNET_FILTER_M_CPU_COS) {
                hdr_data->word6.overlay2.cpu_cos = filter->m_cpu_cos;
                hdr_mask->word6.overlay2.cpu_cos = 0;
            }
        } else
#endif
    ...
    if (SOC_DCB_TYPE(unit) == 38) {
        ...
        if (filter->match_flags & BCM_KNET_FILTER_M_CPU_COS) {
            hdr_data->word0.overlay2.cpu_cos = filter->m_cpu_cos;
            hdr_mask->word0.overlay2.cpu_cos = 0;
        }
    } else
#endif
...
bcm_esw_knet_filter_get(int unit, int filter_id, bcm_knet_filter_t *filter)
{
    ...
    if (BCM_SUCCESS(rv)) {
        ...
        if (SOC_DCB_RX_COS_GET(unit, dcb_mask)) {
            filter->match_flags |= BCM_KNET_FILTER_M_CPU_COS;
            filter->m_cpu_cos = SOC_DCB_RX_COS_GET(unit, dcb_data);
        }
        /* Get module header match fields */
        _mirrored_match_get(unit, filter, dcb_data, dcb_mask);
