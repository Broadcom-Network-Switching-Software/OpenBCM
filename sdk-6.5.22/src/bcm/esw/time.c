...
typedef enum bcmi_time_port_mode_e {
    ...
    defined(BCM_GREYHOUND2_SUPPORT) || defined(BCM_FIRELIGHT_SUPPORT)
#ifdef PORTMOD_SUPPORT
...
#endif /* all chips that need this function */
...
#if defined(BCM_TOMAHAWKX_SUPPORT)
    if (SOC_IS_TOMAHAWKX(unit) ) {
        soc_reg_t synce_ctrl = TOP_MISC_CONTROLr;
                             ...
                             int *value)
{
    ...
    case bcmTimeSynceClockSourceControlFrequency:
        rv = _bcm_esw_time_synce_clock_source_frequency_get(unit, &local_clk_src_conf, value);
        break;
    default:
        return BCM_E_PARAM;
    }
...
typedef enum bcmi_time_tod_stage_e {
    ...
    bcmi_time_tod_stage_bte = 4,
    bcmi_time_tod_stage_count = 5
} bcmi_time_tod_stage_t;

#if defined(BCM_CMICM_SUPPORT) || defined(BCM_IPROC_SUPPORT)
/*
 * Function:
 *      bcmi_time_tod_stage_bte_set
 * Purpose:
 *      Internal function to set tod value in BTE or FW.
 * Parameters:
 *      unit    - (IN)  Unit number.
    ...
    bcm_time_tod_t *tod)
{
    int rv = BCM_E_NONE;
    bcm_time_ts_counter_t counter;
    uint64 tod_ns, secs, nano_secs, ts_counter_ns;
    int uc = 0, status;
    int to = 100000; /* Timeout in usec */

    /* Get HW TS counter */
    bcm_time_ts_counter_t_init(&counter);
#if defined(BCM_TIME_V3_SUPPORT)
    if (soc_feature(unit, soc_feature_time_v3)) {
...
#endif
    /* Get timeofday in nanoseconds */
    secs = tod->ts.seconds;
    nano_secs = tod->ts_adjustment_counter_ns;

    COMPILER_64_UMUL_32(secs, 1000000000);
    COMPILER_64_ADD_64(nano_secs, secs);
    COMPILER_64_COPY(tod_ns, nano_secs);

    ts_counter_ns = counter.ts_counter_ns;

    for (uc = 0; uc < SOC_INFO(unit).num_ucs; uc++) {
        rv = soc_uc_status(unit, uc, &status);
        if ((rv == SOC_E_NONE) && (status != 0)) {
            /* Set TOD counter in BTE */
            rv = soc_uc_timeofday_set(unit, uc,  tod_ns, ts_counter_ns, (sal_usecs_t)to);
            if (rv != BCM_E_NONE) {
                LOG_VERBOSE(BSL_LS_BCM_TIME,
                         (BSL_META_U(0, "Failed to set tod in uC %d \n"), uc));
            }
        }
    }
    if (uc >= SOC_INFO(unit).num_ucs) {
        /* Could not find FW load on any of uC Core */
        LOG_VERBOSE(BSL_LS_BCM_TIME,
                    (BSL_META_U(unit,
                                "FW not loaded on any of uC core \n")));
        return BCM_E_NONE;
    }
    return rv;
}
...
/*
 * Function:
 *      bcmi_time_tod_stage_bte_get
 * Purpose:
 *      Internal function to get tod value in BTE or FW.
 * Parameters:
 *      unit    - (IN)  Unit number.
 *      tod     - (OUT) Time of the day values.
 * Returns:
    ...
    bcm_time_tod_t *tod)
{
    int rv = BCM_E_NONE;
    bcm_time_ts_counter_t counter;
    uint64 tod_ns, secs, nano_secs, div;
    int uc = 0;
    int status;
    int to = 100000; /* Timeout in usec */

    /* Get TOD counter in BTE tod */
    for (uc = 0; uc < SOC_INFO(unit).num_ucs; uc++) {
        rv = soc_uc_status(unit, uc, &status);
        if ((rv == SOC_E_NONE) && (status != 0)) {
            if (SOC_E_NONE == rv) {
    ...
    rv = soc_uc_timeofday_get(unit, uc, &tod_ns, (sal_usecs_t)to);
    if (rv != BCM_E_NONE) {
        LOG_VERBOSE(BSL_LS_BCM_TIME,
                  (BSL_META_U(0, "Error in tod get uC %d \n"), uc));
        return rv;
    }
...
#if defined(BCM_CMICM_SUPPORT) || defined(BCM_IPROC_SUPPORT)
    if ((stage == bcmi_time_tod_stage_bte) &&
        (soc_feature(unit, soc_feature_cmicm) || soc_feature(unit, soc_feature_iproc))) {
        rv = bcmi_time_tod_stage_bte_set(unit, tod);
        return rv;
    } else
#endif
#if defined(BCM_TIMESYNC_LIVE_WIRE_TOD_SUPPORT)
...

{
    ...
    if ((stage == bcmi_time_tod_stage_bte) &&
        (soc_feature(unit, soc_feature_cmicm) || soc_feature(unit, soc_feature_iproc))) {
        return bcmi_time_tod_stage_bte_get(unit, tod);
    } else
#endif
#if defined(BCM_TIMESYNC_LIVE_WIRE_TOD_SUPPORT)
    if (soc_feature(unit, soc_feature_timesync_live_wire_tod) &&
        ...
        bcm_time_tod_t *tod)
{
    ...
    } else if (stages & BCM_TIME_STAGE_BTE) {
        rv = bcmi_time_tod_set(unit, tod, bcmi_time_tod_stage_bte);
    } else {
        return BCM_E_PARAM;
    }
#endif
        ...
        bcm_time_tod_t *tod)
{
    ...
    if (stages & BCM_TIME_STAGE_ALL) {
        ...
        rv = bcmi_time_tod_get(unit, tod, bcmi_time_tod_stage_bte);
    } else {
        return BCM_E_PARAM;
    }
#endif
                            ...
                            int enable)
{
    ...
    if (SOC_REG_IS_VALID(unit, NS_TIMESYNC_TS_EVENT_FWD_CFGr)) {
        ...
        if (SOC_IS_HURRICANE4(unit)) {
            /* Override not required for HR4. */
        } else if (SOC_REG_FIELD_VALID(unit, NS_TIMESYNC_TS_EVENT_FWD_CFGr,
                                DESTf)) {
            prev_dest = soc_reg_field_get(unit, NS_TIMESYNC_TS_EVENT_FWD_CFGr, regVal,
                                          DESTf);
            if (enable) {
                if (prev_dest == _bcmTimeCaptureDestCPU) {
                    dest = 0;
                } else {
                    dest = 2;
                }
...
STATIC int
_bcm_esw_time_event_config_set(int unit, _bcm_time_capture_input_t *pTimeCapture, uint32 eventId, int enable)
{
    int rv = BCM_E_NONE;
