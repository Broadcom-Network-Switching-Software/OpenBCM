 ...
 */
int
_bcm_th3_tunnel_reinit(int unit, uint8 **tnl_scache, int offset)
{
    uint16      dev_id;
    uint8       rev_id;
    ...
    if (((dev_id & BCM56980_DEVICE_ID_MASK) == BCM56980_DEVICE_ID)
                    && (rev_id == BCM56980_B0_REV_ID))
    {
        SOC_SCACHE_DUMP_DATA_BEGIN("last_global_cfg_tnl_type1");
        SOC_SCACHE_DUMP_DATA_UINT8d_V((*tnl_scache));
        SOC_SCACHE_DUMP_DATA_END();
        last_global_cfg_tnl_type1[unit] = *(*tnl_scache)++;

        SOC_SCACHE_DUMP_DATA_BEGIN("last_global_cfg_tnl_type2");
        SOC_SCACHE_DUMP_DATA_UINT8d_V((*tnl_scache));
        SOC_SCACHE_DUMP_DATA_END();
 ...
 */
int
_bcm_th3_tunnel_sync(int unit, uint8 **tnl_scache, int offset)
{
    uint16      dev_id;
    uint8       rev_id;
    uint8      *scache_ptr_tmp;
    SOC_SCACHE_DUMP_DECL;
    ...
    if (((dev_id & BCM56980_DEVICE_ID_MASK) == BCM56980_DEVICE_ID)
                    && (rev_id == BCM56980_B0_REV_ID))
    {
        scache_ptr_tmp = *(tnl_scache);
        *(*tnl_scache)++ = last_global_cfg_tnl_type1[unit];
        SOC_SCACHE_DUMP_DATA_BEGIN("last_global_cfg_tnl_type1");
        SOC_SCACHE_DUMP_DATA_UINT8d_V(scache_ptr_tmp);
        SOC_SCACHE_DUMP_DATA_END();
