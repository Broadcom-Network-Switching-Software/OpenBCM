...
/*
 *  Function:
 *  _bcm_th3_mpath_validate
 *  Purpose:
 *        Validate group ID.
 *  Parameters:
 *      unit            - (IN) SOC unit number.
 *      grp_id         - (IN) ECMP group ID.
 *  Returns:
 *      BCM_X_XXX
 */
int
_bcm_th3_mpath_validate(int unit, int ecmp_grp_if) {
    int grp_id = ecmp_grp_if - BCM_XGS3_MPATH_EGRESS_IDX_MIN(unit);
    if ((grp_id >= BCM_TH3_UNDERLAY_ONLY_OFFSET(unit)) &&
