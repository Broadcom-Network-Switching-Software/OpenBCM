...
#define BCM_HR4_PFC_DEADLOCK_CHIP_CONFIG_0_MAX_COS 4
#define BCM_HR4_PFC_DEADLOCK_CHIP_CONFIG_1_MAX_COS 9


_bcm_hr4_pfc_deadlock_control_t *_bcm_hr4_pfc_deadlock_control[BCM_MAX_NUM_UNITS];

STATIC sal_mutex_t _bcm_hr4_pfc_lock[SOC_MAX_NUM_DEVICES] = {NULL};
#define HR4_PFC_LOCK(unit) sal_mutex_take(_bcm_hr4_pfc_lock[unit], sal_mutex_FOREVER)
#define HR4_PFC_UNLOCK(unit) sal_mutex_give(_bcm_hr4_pfc_lock[unit])

int
_bcm_hr4_pfc_reg_index_get(int unit, bcm_port_t port, bcm_port_t *mport, int *index)
{
   soc_info_t *si;
...
int
_bcm_hr4_pfc_deadlock_ignore_pfc_xoff_clear(int unit, int cos, bcm_port_t port)
{
                  ...
                  bcm_cos_t priority, int *hw_cos_index)
{
    ...
    rv = _bcm_hr4_pfc_deadlock_hw_cos_index_get(unit, priority, &temp_hw_index);
    if (rv != BCM_E_NONE) {
    ...
    pfc_deadlock_config = _BCM_HR4_PFC_DEADLOCK_CONFIG(unit, priority);
 ...
 _bcm_hr4_pfc_deadlock_recovery_end(int unit, int cos, bcm_port_t port)
{
    ...
    BCM_IF_ERROR_RETURN(
        _bcm_hr4_pfc_reg_index_get(unit, port, &mmu_port, &index));
 ...
 */
STATIC int
_bcm_hr4_pfc_deadlock_recovery_begin(int unit, int cos, bcm_port_t port)
{
    ...
    pfc_deadlock_pri_config->port_recovery_count[port] = 1 +
        ((pfc_deadlock_pri_config->recovery_timer * 1000) /
         _BCM_HR4_PFC_DEADLOCK_CB_INTERVAL(0));
 ...
 */
int
_bcm_hr4_pfc_deadlock_recovery_update(int unit, int cos)
{
    _bcm_pfc_deadlock_config_t *pfc_deadlock_pri_config = NULL;
    _bcm_hr4_pfc_deadlock_control_t *pfc_deadlock_control = NULL;
    bcm_port_t port;
    ...
    BCM_PBMP_ITER(pfc_deadlock_pri_config->deadlock_ports, port) {
        ...
        if (pfc_deadlock_pri_config->port_recovery_count[port]) {
            ...
            BCM_IF_ERROR_RETURN(
                    _bcm_hr4_pfc_deadlock_recovery_end(unit, cos, port));
        }
    }
    return BCM_E_NONE;
}
    ...
    for (cos = 0; cos < pfc_deadlock_control->pfc_deadlock_cos_max; cos++) {
        ...
        if (pfc_deadlock_control->hw_cos_idx_inuse[cos] == TRUE) {
            ...
            PBMP_ALL_ITER(unit, port) {
                ...
                rv = _bcm_hr4_pfc_reg_index_get(unit, port, &mmu_port, &index);
                if (BCM_FAILURE(rv)) {
                    HR4_PFC_UNLOCK(unit);
                    return rv;
                }
                ...
                if (status &(1 << cos)) {
                    rv = _bcm_hr4_pfc_deadlock_recovery_begin(unit, cos, port);
                    if (BCM_FAILURE(rv)) {
                        HR4_PFC_UNLOCK(unit);
                        return rv;
                    }
            ...
            if (!user_control) {
                ...
                if (SOC_PBMP_NOT_NULL(pfc_deadlock_pri_config->deadlock_ports)) { 
                    rv =_bcm_hr4_pfc_deadlock_recovery_update(unit, cos);
                    if (BCM_FAILURE(rv)) {
                        HR4_PFC_UNLOCK(unit);
                        return rv;
                    }
...
{
    if (priority <= BCM_HR4_PFC_DEADLOCK_CHIP_CONFIG_0_MAX_COS) {
        *chip_config = Q_SCHED_DD_CHIP_CONFIG0r;
    } else if ( priority > BCM_HR4_PFC_DEADLOCK_CHIP_CONFIG_0_MAX_COS && priority <= BCM_HR4_PFC_DEADLOCK_CHIP_CONFIG_1_MAX_COS) {
                              ...
                              _bcm_pfc_deadlock_config_t *config)
{
    ...
    BCM_IF_ERROR_RETURN(
        _bcm_hr4_pfc_deadlock_chip_config_get(unit, priority, &chip_config));
    ...
    if (operation == _bcmPfcDeadlockOperGet) {
        ...
        rv = _bcm_hr4_pfc_deadlock_hw_cos_index_set(unit, priority, &hw_cos_index);
        if (BCM_FAILURE(rv)) {
            HR4_PFC_UNLOCK(unit);
            return rv;
        }
                                      ...
                                      uint8 *enable_status)
{
    ...
    BCM_IF_ERROR_RETURN(
        bcm_hr4_pfc_deadlock_queue_info_get(unit, gport, &mmu_port, &local_port, &type, &split));
    priority += (type * 8);
    HR4_PFC_LOCK(unit);
    pfc_deadlock_pri_config = _BCM_HR4_PFC_DEADLOCK_CONFIG(unit, priority);


    rv = _bcm_hr4_pfc_deadlock_hw_cos_index_get(unit, priority, &hw_cos_index);
    if (BCM_FAILURE(rv)) {
        HR4_PFC_UNLOCK(unit);
        return rv;
    }
    ...
    if (operation == _bcmPfcDeadlockOperGet) { /* GET */
        ...
        if (SOC_PBMP_IS_NULL(pfc_deadlock_pri_config->enabled_ports)) {
            rv =_bcm_hr4_pfc_deadlock_ignore_pfc_xoff_clear(unit, hw_cos_index, local_port);
            if (BCM_FAILURE(rv)) {
                HR4_PFC_UNLOCK(unit);
                return rv;
            }
    ...
    HR4_PFC_UNLOCK(unit);
    BCM_IF_ERROR_RETURN(_bcm_hr4_pfc_deadlock_update_cos_used(unit));
    return BCM_E_NONE;
}
 ...
 */
int
_bcm_hr4_pfc_deadlock_recovery_reset(int unit)
{
    _bcm_hr4_pfc_deadlock_control_t *pfc_deadlock_control = NULL;
    _bcm_pfc_deadlock_config_t *pfc_deadlock_pri_config = NULL;
    int priority = 0, i;
    ...
    for (i = 0; i < pfc_deadlock_control->pfc_deadlock_cos_max; i++) {
        ...
        BCM_PBMP_ITER(pfc_deadlock_pri_config->deadlock_ports, port) {
            ...
            BCM_IF_ERROR_RETURN(
                    _bcm_hr4_pfc_deadlock_recovery_end(unit, i, port));
        }
    }
...
/*
 * Function:
 *      _bcm_hr4_pfc_deadlock_control_get_hw
 * Description:
 ...
 */
int
_bcm_hr4_pfc_deadlock_control_get_hw(int unit, bcm_switch_control_t type, int *arg)
{
    _bcm_hr4_pfc_deadlock_control_t *pfc_deadlock_control = NULL;
    _bcm_hr4_pfc_hw_resorces_t *hw_res = NULL;
    uint32 rval = 0, field_val = 0;
...
STATIC int
_bcm_hr4_pfc_deadlock_default(int unit)
{
    _bcm_hr4_pfc_deadlock_control_t *pfc_deadlock_control = NULL;
    int i = 0;
    ...
    _BCM_HR4_PFC_DEADLOCK_HW_RES_INIT(&pfc_deadlock_control->hw_regs_fields);
    ...
    sal_memset(pfc_deadlock_control->pfc_cos2pri, -1,
            (HR4_PFC_DEADLOCK_MAX_COS * sizeof(bcm_cos_t)));
...
/* HW res init */
int
_bcm_hr4_pfc_deadlock_hw_res_init(int unit)
{
     int i = 0;
    _bcm_hr4_pfc_hw_resorces_t *hw_res = NULL;
    _bcm_hr4_pfc_deadlock_control_t *pfc_deadlock_control = NULL;
...
int
_bcm_hr4_pfc_deadlock_init(int unit)
{
    _bcm_hr4_pfc_deadlock_control_t *pfc_deadlock_control = NULL;

    BCM_IF_ERROR_RETURN(_bcm_hr4_pfc_deadlock_default(unit));
    BCM_IF_ERROR_RETURN(_bcm_hr4_pfc_deadlock_hw_res_init(unit));

    pfc_deadlock_control = _BCM_HR4_UNIT_PFC_DEADLOCK_CONTROL(unit);
    pfc_deadlock_control->pfc_deadlock_cos_max = HR4_PFC_DEADLOCK_MAX_COS;
    pfc_deadlock_control->cosq_inv_mapping_get = _bcm_hr4_cosq_inv_mapping_get;

    _bcm_hr4_pfc_lock[unit] = sal_mutex_create("pfc lock");
    if (_bcm_hr4_pfc_lock[unit] == NULL) {
        return BCM_E_MEMORY;
    }
 ...
 */
int
_bcm_hr4_pfc_deadlock_deinit(int unit)
{
    _bcm_hr4_pfc_deadlock_control_t *pfc_deadlock_control = NULL;
    ...
    if (pfc_deadlock_control != NULL) {
        ...
        _BCM_HR4_UNIT_PFC_DEADLOCK_CONTROL(unit) = NULL;
    }
    if (_bcm_hr4_pfc_lock[unit]) {
        sal_mutex_destroy(_bcm_hr4_pfc_lock[unit]);
        _bcm_hr4_pfc_lock[unit] = NULL;
    }    
 ...
 */
int
_bcm_hr4_pfc_deadlock_reinit(int unit)
{
    _bcm_hr4_pfc_deadlock_control_t *pfc_deadlock_control = NULL;
    _bcm_pfc_deadlock_config_t *pfc_deadlock_pri_config = NULL;
    _bcm_hr4_pfc_hw_resorces_t *hw_res = NULL;
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_hr4_pfc_deadlock_control_get_hw(unit,
    ...
    for (i = 0; i < pfc_deadlock_control->pfc_deadlock_cos_max; i++) {
        ...
        rv = _bcm_hr4_pfc_deadlock_hw_cos_index_get(unit, priority,
                                                    &temp_hw_index);
        if (BCM_SUCCESS(rv)) {
            pfc_deadlock_pri_config->flags   |= _BCM_PFC_DEADLOCK_F_ENABLE;
            ...
            /* coverity[overrun-call] */
            BCM_IF_ERROR_RETURN(
                _bcm_hr4_pfc_deadlock_hw_oper(unit, _bcmPfcDeadlockOperGet,
