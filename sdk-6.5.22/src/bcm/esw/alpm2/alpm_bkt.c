                           ...
                           int start_bnk, int alloc_sz, int step)
{
    ...
    if ((check_avl == 0) && !(start_bnk == 0 && step == bnk_pbk)) {
        ALPM_ERR(("alpm_bkt_sharing_bnk_alloc: check_avl=%d, \
                   start_bnk=%d, step=%d, bnk_pbk=%d\n",
                   check_avl, start_bnk, step, bnk_pbk));
    }
                  ...
                  _alpm_pvt_node_t *dst_pvt, int merge_dir)
{
    ...
    if (merge_dir == ACB_MERGE_PARENT_TO_CHILD) {
        ...
        /* Child to parent merge, child pvt needs to be invalidated first */
