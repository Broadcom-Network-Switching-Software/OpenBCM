...
/* Variable FMT even bucket split */
int
alpm_lib_trie_splitable(alpm_lib_trie_node_t *trie, alpm_lib_trie_node_t *child,
...
/* FIXED_FMT bucket split (uneven) */
int
alpm_lib_trie_splitable2(alpm_lib_trie_node_t *trie, alpm_lib_trie_node_t *child,
                            alpm_lib_trie_callback_ext_f cb, void *user_data,
                            int max_count, int max_split_count)
{
    /*
...
static int _trie_split(alpm_lib_trie_node_t  *trie, uint32 *pivot,
               ...
               alpm_lib_trie_splitable_f spcb,
               void *user_data,
    ...
    if ((*length > max_split_len) && (trie->count != max_count)) {
        ...
        if ((trieSplitStatePayloadSplit == *state) &&
            ...
            *state = trieSplitStatePruneNodes;
            return rv;
                  ...
                  spcb(trie, trie->child[bit], cb, user_data, max_count, max_split_count)) {
        /*
        ...
        *split_count = trie->count;
        ...
        rv = _trie_split(trie->child[bit],
                 ...
                 state, cb, spcb, user_data, max_split_count);
    }
    }
 ...
 *     spcb is the trie_splitable function pointer.
 */
                        ...
                        const int max_split_count)
{
    ...
    if (spcb == NULL) {
        spcb = alpm_lib_trie_splitable; /* default is even splitable function */
    }
    ...
    if (trie->trie) {
        ...
        if (trie->v6_key) {
            ...
            rv = _alpm_lib_trie_v6_split(trie->trie, pivot, length, &split_count, split_trie_root,
                     &child, max_count, max_split_len, &state, cb, spcb, user_data, max_split_count);
        } else {
