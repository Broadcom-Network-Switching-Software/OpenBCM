    ...
    int *member_count)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_trunk_get(unit, tid, t_data, member_max, new_member_array, member_count);
    ...
    bcm_compat6521_trunk_member_t *member_array)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_trunk_set(unit, tid, trunk_info, member_count, new_member_array);
...
/*
 * Function:
 *      _bcm_compat6521in_trunk_resilient_entry_t
 * Purpose:
 *      Convert the bcm_trunk_resilient_entry_t datatype from <=6.5.21 to
 *      SDK 6.5.22+
 * Parameters:
 *      from        - (IN) The <=6.5.21 version of the datatype
 *      to          - (OUT) The SDK 6.5.22+ version of the datatype
...
STATIC void
_bcm_compat6521in_trunk_resilient_entry_t(
    bcm_compat6521_trunk_resilient_entry_t *from,
    bcm_trunk_resilient_entry_t *to)
{
    to->hash_key = from->hash_key;
    to->tid = from->tid;
    _bcm_compat6521in_trunk_member_t(from->member, to->member);
}

/*
 * Function:
 *      bcm_compat6521_trunk_resilient_replace
 * Purpose:
 *      Compatibility function for RPC call to bcm_trunk_resilient_replace.
 * Parameters:
 *      unit - (IN) Unit number.
 *      flags - (IN) BCM_TRUNK_RESILIENT flags.
 *      match_entry - (IN) Matching criteria
 ...
 *      BCM_E_XXX
 */
int bcm_compat6521_trunk_resilient_replace(
    int unit,
    uint32 flags,
    bcm_compat6521_trunk_resilient_entry_t *match_entry,
    int *num_entries,
    bcm_compat6521_trunk_resilient_entry_t *replace_entry)
{
    int rv = BCM_E_NONE;
    bcm_trunk_resilient_entry_t *new_match_entry = NULL;
    bcm_trunk_resilient_entry_t *new_replace_entry = NULL;

    if (match_entry != NULL && replace_entry != NULL) {
        new_match_entry = (bcm_trunk_resilient_entry_t *)
                     sal_alloc(sizeof(bcm_trunk_resilient_entry_t),
                     "New match_entry");
        if (new_match_entry == NULL) {
            return BCM_E_MEMORY;
        }
        /* Transform the entry from the old format to new one */
        _bcm_compat6521in_trunk_resilient_entry_t(match_entry, new_match_entry);
        new_replace_entry = (bcm_trunk_resilient_entry_t *)
                     sal_alloc(sizeof(bcm_trunk_resilient_entry_t),
                     "New replace_entry");
        if (new_replace_entry == NULL) {
            sal_free(new_match_entry);
        ...
        /* Transform the entry from the old format to new one */
        _bcm_compat6521in_trunk_resilient_entry_t(replace_entry, new_replace_entry);
    } else {
        return BCM_E_PARAM;
    }

    /* Call the BCM API with new format */
    rv = bcm_trunk_resilient_replace(unit, flags, new_match_entry, num_entries, new_replace_entry);
    ...
    bcm_compat6521_vlan_translate_egress_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_vlan_translate_egress_action_extended_add(unit, new_key_config, new_action);
    ...
    bcm_compat6521_vlan_translate_egress_action_set_t *action)
{
    ...
    /* Call the BCM API with new format */
    rv = bcm_vlan_translate_egress_action_extended_get(unit, new_key_config, new_action);
