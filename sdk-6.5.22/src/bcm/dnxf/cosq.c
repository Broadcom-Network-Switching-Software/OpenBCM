    ...
    bcm_cosq_gport_info_t * gport_info)
{
    ...
    switch (gport_type)
    {
        ...
        case bcmCosqGportTypeFabricPipeMiddle:
            ...
            break;
        default:
            LOG_ERROR(BSL_LS_BCM_COSQ, (BSL_META_U(unit, "Invalid Handle type (%d) requested\n"), gport_type));
            rc = _SHR_E_PARAM;
            break;
    }
    ...
    int weight)
{
    ...
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cosq_pipe_weight_set,
                                   (unit, soc_dnxf_cosq_pipe_type_ingress, pipe, port, weight_mode, weight));
        SHR_IF_ERR_EXIT(rc);
    ...
    int *weight)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport))   /* ingress */
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_cosq_pipe_weight_get,
                                   (unit, soc_dnxf_cosq_pipe_type_ingress, pipe, port, weight_mode, weight));
        SHR_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport))       /* middle */
