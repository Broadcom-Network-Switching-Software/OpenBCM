    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL_NO_ARGS(unit, mbcm_dnxf_stat_init));
    ...
    SHR_IF_ERR_EXIT(soc_counter_start(unit, flags, interval, SOC_CONTROL(unit)->info.sfi.bitmap));
    ...
    uint64 *value)
{
    ...
    SHR_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_stat_get, (unit, port, value, counters, array_size)));
