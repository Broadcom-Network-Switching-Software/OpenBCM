    ...
    int dev_id)
{
    ...
    for (blk = 0; SOC_BLOCK_INFO(unit, blk).type >= 0; blk++)
    {
        ...
        SHR_IF_ERR_EXIT(soc_dnxf_drv_block_valid_get
        ...
        switch (blktype)
        {
            ...
            case SOC_BLK_RTP:
                ...
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: too much RTP blocks");
                }
                break;
            case SOC_BLK_OCCG:
                si->occg_block = blk;
            ...
            case SOC_BLK_DFL_FBC:
                if (SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_DFL_FBC_BLKS)
                {
                    si->dfl_fbc_block[SOC_BLOCK_INFO(unit, blk).number] = blk;
                    si->block_port[blk] = SOC_BLOCK_INFO(unit, blk).number | SOC_REG_ADDR_INSTANCE_MASK;
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: too much DFL_FBC blocks");
                }
                break;
            case SOC_BLK_VTMON:
                if (SOC_BLOCK_INFO(unit, blk).number < SOC_MAX_NUM_VTMON_BLKS)
                {
                    si->vtmon_block[SOC_BLOCK_INFO(unit, blk).number] = blk;
                    si->block_port[blk] = SOC_BLOCK_INFO(unit, blk).number | SOC_REG_ADDR_INSTANCE_MASK;
                }
                else
                {
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: too much VTMON blocks");
                }
                break;
            ...
            case SOC_BLK_IPW:
                break;
            default:
                SHR_ERR_EXIT(_SHR_E_INTERNAL, "soc_dnxf_info_config: unknown block type %d ", blktype);
                break;
        }
