...
smbus_start_wait(int unit)
{
    ...
    soc_timeout_init(&to, 25000, 1000);
...
smbus_read_with_status_check(int unit, uint8 *data)
{
    ...
    if (soc_reg_field_get(unit, CMIC_I2CM_SMBUS_MASTER_DATA_READr, rval, MASTER_RD_STATUSf) == 3) {
...
cmicx_smbus_read_with_status_check(int unit, uint8 *data)
{
    ...
    if (soc_reg_field_get(unit, IPROCPERIPH_SMBUS1_SMBUS_MASTER_DATA_READr, rval, MASTER_RD_STATUSf) == 3) {
        *data = (uint8) (rval & 0xff);
    } else {
        rv = SOC_E_FAIL;
    }
