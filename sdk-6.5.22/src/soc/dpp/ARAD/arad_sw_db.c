...
STATIC uint32 arad_sw_db_modport2master_lagsysport_init(
    SOC_SAND_IN int unit
  )
{
  soc_error_t rv;
  ...
  rv = sw_state_access[unit].dpp.soc.arad.tm.modport2master_lagsysport.is_allocated(unit, &is_allocated);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 10, exit);

  if(!is_allocated) {
      rv = sw_state_access[unit].dpp.soc.arad.tm.modport2master_lagsysport.alloc(unit, ARAD_NOF_MODPORT);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 10, exit);
  }

  for(modport_idx = 0; modport_idx < ARAD_NOF_MODPORT; ++modport_idx) {
      rv = sw_state_access[unit].dpp.soc.arad.tm.modport2master_lagsysport.set(unit, modport_idx, ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT);
      SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, ARAD_GEN_ERR_NUM_CLEAR, exit);
  }
exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sw_db_modport2master_lagsysport_init()", 0, 0);
}
  ...
  )
{
    ...
    res = arad_sw_db_modport2master_lagsysport_init(unit);
    SOCDNX_SAND_IF_ERR_EXIT(res);    
  ...
  )
{
  ...
  rv = sw_state_access[unit].dpp.soc.arad.tm.modport2master_lagsysport.set(unit, modport, sysport);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 30, exit);


exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sw_db_modport2master_lagsysport_set()", fap_id, fap_port_id);
}
  ...
  )
{
  ...
  SOC_SAND_INIT_ERROR_DEFINITIONS(0);
  res = arad_sw_db_modport2master_lagsysport_set(unit,fap_id, fap_port_id, ARAD_SW_DB_MODPORT2SYSPORT_INVALID_SYSPORT);
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sw_db_modport2master_lagsysport_remove()", fap_id, fap_port_id);
}
  ...
  )
{
  ...
  modport = fap_id | (fap_port_id << ARAD_DEVICE_NUMBER_BITS);
  SOC_SAND_ERR_IF_ABOVE_NOF(modport, ARAD_NOF_MODPORT, SOC_SAND_VALUE_ABOVE_MAX_ERR, 20, exit);
  rv = sw_state_access[unit].dpp.soc.arad.tm.modport2master_lagsysport.get(unit, modport, sysport);
  SOC_SAND_SOC_CHECK_FUNC_RESULT(rv, 30, exit);  

exit:
  SOC_SAND_EXIT_AND_SEND_ERROR( "error in arad_sw_db_modport2master_lagsysport_get()", fap_id, fap_port_id);
}
