...
int arad_kbp_recover_txpcs_soft_reset_toggle(int unit, int core, int mdio_id)
{
    ...
    int kbp_port_id = 0, kbp_port_id_base = 0;
    SOCDNX_INIT_FUNC_DEFS;

    if (ARAD_KBP_OP_IS_DUAL_MODE) {
        if (elk->kbp_port_cross_connectivity)
        {
            kbp_port_id_base = unit ? 0 : 2;
        } else {
            kbp_port_id_base = unit ? 2 : 0;
        }
        kbp_port_id = (elk->kbp_port_core_mapping[kbp_port_id_base] == core) ? kbp_port_id_base : kbp_port_id_base + 1;
     } else {
