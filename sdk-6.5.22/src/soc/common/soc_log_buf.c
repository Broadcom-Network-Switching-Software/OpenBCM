...
_soc_log_buf_read(void *location, int offset, void *data, int size)
{
    ...
    if (part_one >= size) { /* There is no overrun */
        sal_memcpy(data, cur_log, size);
    } else { /* The read must be broken into two parts */
        int part_two = size - part_one;
        char *cur_dst = data;
        sal_memcpy(cur_dst, cur_log, part_one);
        cur_dst += part_one;
        cur_log = buf;
        sal_memcpy(cur_dst, cur_log, part_two);
    }
...
_soc_log_buf_write(void *location, int offset, void *data, int size)
{
    ...
    if (part_one >= size) { /* There is no overrun */
        sal_memcpy(cur_log, data, size);
    } else { /* The write must be split into two parts */
        int part_two = size - part_one;
        char *cur_dst = data;
        sal_memcpy(cur_log, cur_dst, part_one);
        cur_dst += part_one;
        cur_log = buf;
        sal_memcpy(cur_log, cur_dst, part_two);
    }
