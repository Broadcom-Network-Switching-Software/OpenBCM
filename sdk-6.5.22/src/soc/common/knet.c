...
soc_knet_dune_info_init(int unit, kcom_msg_hw_init_t *kmsg)
{
    ...
    if (SOC_IS_DNX(unit))
    {
        ...
        for (logical_port = 0; logical_port < max_num_ports; logical_port++)
        {
            ...
            if (ucode_port->interface == BCM_PORT_IF_OAMP)
            {
                kmsg->oamp_local_ports[oamp_port_count] = logical_port;
                oamp_port_count ++;
                if (oamp_port_count >= KCOM_HW_INIT_OAMP_LOCAL_PORTS_MAX)
                {
                    break;
                }
                /* Record number of OAMP ports */
                kmsg->oamp_local_ports_number = oamp_port_count;
            }
        }
