...
_soc_ser_bus_buffer_log_match(void *criteria, void *entry)
{
    ...
    sal_memset(&entry_data, 0, sizeof(soc_ser_log_tlv_memory_t));
...
_soc_ser_log_match_entry(void *criteria, void *entry)
{
    ...
    while ((tlv_hdr->type != 0) && !(found_gen && found_mem)) {
        if (tlv_hdr->type == SOC_SER_LOG_TLV_GENERIC) {
            ...
            found_mem = 1;
        } else if (tlv_hdr->type == SOC_SER_LOG_TLV_REGISTER) {
            tlv_reg = (soc_ser_log_tlv_register_t*)(((char*)tlv_hdr) +
                sizeof(soc_ser_log_tlv_hdr_t));
            entry_data.reg = tlv_reg->reg;
            entry_data.index = tlv_reg->index;
            entry_data.port = tlv_reg->port;
            found_reg = 1;
        }
        tlv_hdr = (soc_ser_log_tlv_hdr_t*)((char*)tlv_hdr +
            tlv_hdr->length + sizeof(soc_ser_log_tlv_hdr_t));
    ...
    if ((found_gen && found_mem) &&
        ...
        return 1;
    } else if((found_gen && found_reg) &&
        (crit->reg == entry_data.reg) &&
        (crit->port == entry_data.port) &&
        (crit->index == entry_data.index) &&
        (SAL_USECS_SUB(crit->time, entry_data.time) <= 0)) {
        return 1;
    }
...
int
soc_ser_reg_log_find_recent(int unit, soc_reg_t reg, int port, int index,
                            sal_usecs_t time)
{
    int rv = SOC_E_NONE;
    char *buffer;
...
soc_ser_log_print_tlv(int unit, void *buffer)
{
    ...
    case SOC_SER_LOG_TLV_CONTENTS:
        ...
        for (index = 0; index < tlv_hdr->length; index++) {
            ...
            LOG_CLI((BSL_META_U(unit, "%02x "), *((uint8*)(data + index))));
            if ((index%16)==15) {
                LOG_CLI((BSL_META_U(unit, "\n")));
            }
        }
    ...
    case SOC_SER_LOG_TLV_GENERIC:
        ...
        LOG_CLI((BSL_META_U(unit, "\t\thard_fault: %d\n"),
                   (int)(((soc_ser_log_tlv_generic_t*)(data))->hard_fault)));
        break;
    default:
