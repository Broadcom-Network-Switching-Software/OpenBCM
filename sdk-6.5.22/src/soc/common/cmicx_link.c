...
static soc_cmicx_pport_to_lsbit_t bcm56870_a0_pport_to_lsbit[] = {
    /* pport,  lsbit */
    {  128,    129 }, /* 1st Management port. */
    {  129,    131 }  /* 2nd Management port. */
};
#endif
 ...
 */
STATIC
void _soc_pport_lsbit_remap(int unit, ls_phy_pbmp_t *pbm)
{
    int i;
    ...
    for (i = (COUNTOF(bcm56870_a0_pport_to_lsbit) - 1); i >= 0; i--) {
        if (LS_PHY_PBMP_MEMBER(*pbm, map[i].pport)) {
            LS_PHY_PBMP_PORT_REMOVE(*pbm, map[i].pport);
            LS_PHY_PBMP_PORT_ADD(*pbm, map[i].lsbit);
        }
    }
 ...
 */
STATIC
void _soc_lsbit_pport_remap(int unit, soc_pbmp_t *pbm)
{
    int i;
    soc_cmicx_pport_to_lsbit_t *map;
    ...
    for (i = 0; i < COUNTOF(bcm56870_a0_pport_to_lsbit); i++) {
        if (SOC_PBMP_MEMBER(*pbm, map[i].lsbit)) {
            SOC_PBMP_PORT_REMOVE(*pbm, map[i].lsbit);
            SOC_PBMP_PORT_ADD(*pbm, map[i].pport);
        }
    }
...
int soc_iproc_link_status_update(int unit, soc_iproc_mbox_msg_t *msg)
{
    ...
    if (SOC_IS_ESW(unit)) {
        ...
        if (SOC_IS_TRIDENT3(unit)) {
            CMICX_LINK_LOCK(unit);
            _soc_lsbit_pport_remap(unit, &soc->cmicx_link_stat);
            CMICX_LINK_UNLOCK(unit);
        }
...
int soc_cmicx_linkscan_hw_init(int unit)
{
    ...
    if (!soc->iproc_m0ls_init_done) {
        ...
        hw_cfg_async = soc_property_get(unit,
                                        spn_CMICX_LS_HW_CFG_ASYNC_ENABLE,
                                        0);
...
int soc_cmicx_linkscan_config(int unit, soc_pbmp_t *hw_mii_pbm)
{
    ...
    if (SOC_IS_TRIDENT3(unit)) {
        _soc_pport_lsbit_remap(unit, &phy_mii_pbm);
    }
#endif
    ...
    if (soc_property_get(unit, spn_CMICX_LS_HW_CFG_ASYNC_ENABLE, 0)) {
