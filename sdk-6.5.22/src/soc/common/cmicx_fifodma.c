...
cmicx_dnx_fifodma_done(int unit, void *data)
{
    ...
    SHR_IF_ERR_EXIT_NO_MSG(soc_dnx_fifodma_channel_get(unit, soc_dnx_fifodma_src_olp, &olp_channel));
    
    dma_event_interface_enable = dnx_data_oam.oamp.oamp_fifo_dma_event_interface_enable_get(unit);
    if (dma_event_interface_enable)
    {
        SHR_IF_ERR_EXIT_NO_MSG(soc_dnx_fifodma_channel_get(unit, soc_dnx_fifodma_src_oam_event, &oam_oamp_event_channel));
