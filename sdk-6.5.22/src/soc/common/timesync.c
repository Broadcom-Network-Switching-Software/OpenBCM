 ...
 */
int
soc_port_phy_timesync_tx_info_get(int unit, soc_port_t port, soc_port_timesync_tx_info_t *info)
{
    int         rv;
    phy_ctrl_t   *int_pc;
    phy_ctrl_t   *ext_pc;
    ...
    INT_MCU_LOCK(unit);
    rv = PHY_TIMESYNC_TX_INFO__GET(pd, unit, port, info);
    INT_MCU_UNLOCK(unit);

    if (SOC_FAILURE(rv)) {
        LOG_WARN(BSL_LS_SOC_COMMON,
                 (BSL_META_U(unit,
                             "soc_port_phy_timesync_tx_info_get failed %d\n"), rv));
    }
    return rv;
}
