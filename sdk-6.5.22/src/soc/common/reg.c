...
#define SOC_REG_ADDR_INVALID_PORT 0 /* for asserts */
    ...
    if ((!block_id_mask) && (!schan_id_mask) && (REG_PORT_ANY == port || instance_mask || !SOC_BLOCK_IN_LIST(unit, regblktype, portblktype))) {
        ...
        case SOC_BLK_CCML:
            ...
            break;
        case SOC_BLK_GCK_WRAP:
            block=GCK_WRAP_BLOCK(unit, blkport);
            break;
        case SOC_BLK_GCK:
            block=GCK_BLOCK(unit, blkport);
            break;
        case SOC_BLK_DTM:
        ...
        case SOC_BLK_BRDC_DTL:
            ...
            break;
        case SOC_BLK_BRDC_GCK_WRAP:
            block=BRDC_GCK_WRAP_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_RTP:
            block=BRDC_RTP_BLOCK(unit);
            break;
        case SOC_BLK_BRDC_GCK:
            block=BRDC_GCK_BLOCK(unit);
            break;
