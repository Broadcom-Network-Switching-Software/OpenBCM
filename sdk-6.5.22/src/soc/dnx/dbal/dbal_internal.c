    ...
    for (ii = 0; ii < DBAL_RES_INFO.nof_result_fields; ii++)
    {
        ...
        if (entry_handle->error_info.error_exists)
        {
            ...
            SHR_ERR_EXIT(_SHR_E_INTERNAL,
                         ...
                         dbal_field_with_inst_to_string(unit,
                                                        entry_handle->error_info.field_id,
                                                        entry_handle->error_info.inst_idx,
    ...
    dbal_pointer_type_e pointer_type)
{
        ...
        && !SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ADDRESS))
    {
        ...
        if (!(SHR_BITGET(table_field->field_indication_bm, DBAL_FIELD_IND_IS_FIELD_ENUM)))
        {
            ...
            DBAL_FIELD_ERR_HANDLE((is_illegal || ret_val), "Illegal value for field, in illegals list", 0);
        }
    }
    ...
    uint32 *attr_val)
{
    ...
    else if (attr_type == DBAL_ENTRY_ATTR_CACHED)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "cached set supported only for KBP TCAM table\n");
        }
#endif
    }
    else
    ...
    char *top_node_name)
{
    ...
    while ((dirp = sal_readdir(dir)) != NULL)
    {
        ...
        path =
            sal_alloc(sal_strnlen(parent_dir, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) +
                      sal_strnlen("/", SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) + sal_strnlen(dirp->d_name,
                                                                                                   sizeof(dirp->d_name))
                      + 100, "dir path");
        ...
        if (sal_strcmp(path + (sal_strnlen(path, SHR_SAND_DEFAULT_MAX_STRING_OPERATION_LENGTH) - 4), ".xml"))
        {
            continue;
        }
