    ...
    uint32 *attr_val)
{
    ...
    if (attr_type == DBAL_ENTRY_ATTR_PRIORITY)
    {
        ...
        {
            SHR_ERR_EXIT(_SHR_E_INTERNAL, "priority get supported only for KBP TCAM table\n");
        }
#endif
    }
            ...
            if (update_entry_handle->error_info.error_exists)
            {
                ...
                SHR_ERR_EXIT(_SHR_E_PARAM, "Failed updating field %s\n",
                             dbal_field_with_inst_to_string(unit,
                                                            update_entry_handle->error_info.field_id,
                                                            update_entry_handle->error_info.inst_idx, string_field));
            }
        }
        else
        {
