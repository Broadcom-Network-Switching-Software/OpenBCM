    ...
    uint32 *num_of_entries)
{
    ...
    else
    {
        if (table->max_capacity == 0)
        {
            ...
            {
                uint32 max_capacity_uint32;
                if (table->sw_access_info.table_size_formula.cb)
                {

                    SHR_IF_ERR_EXIT(table->sw_access_info.
                                    table_size_formula.cb(unit, NULL, DBAL_FIELD_EMPTY, &max_capacity_uint32));
                }
                else
                {
                    max_capacity_uint32 = table->sw_access_info.table_size_formula.val;
                }
                (*num_of_entries) = max_capacity_uint32;
                table->max_capacity = max_capacity_uint32;
            }
        }
        else
        {
