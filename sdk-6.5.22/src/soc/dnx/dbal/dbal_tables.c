    ...
    dbal_tables_e * table_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_db_init_table_interface_post_process(unit, *table_id, table));

    if (access_method == DBAL_ACCESS_METHOD_MDB)
    {
        SHR_IF_ERR_EXIT(dbal_db_init_table_mdb_post_process(unit, *table_id, table));
    }
    else if (access_method == DBAL_ACCESS_METHOD_KBP)
    {
        SHR_IF_ERR_EXIT(dbal_db_init_table_kbp_post_process(unit, *table_id, table));
    }
    ...
    RHDATA_ITERATOR(appdb_node, top_node, "AppDB")
    {
        ...
        RHDATA_ITERATOR(results_node, multiple_results_node, "Result")
        {
            ...
            SHR_IF_ERR_EXIT_EXCEPT_IF(dbx_xml_property_get_str
                                      ...
                                      _SHR_E_NOT_FOUND);
            if (sal_strcmp(valid_result_type_str, EMPTY))
            {
    ...
    dbal_tables_e * table_id)
{
    ...
    SHR_IF_ERR_EXIT(dbal_tables_table_create_internal
                    (unit, access_method, table_type, core_mode, nof_key_fields, keys_info, &nof_result_fields,
                     &internal_results_info[0][0], 1, NULL, table_name, FALSE, table_id));
