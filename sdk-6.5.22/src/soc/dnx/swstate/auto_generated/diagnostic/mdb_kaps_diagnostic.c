...
mdb_kaps_db_db_info_is_valid_dump(int  unit,  int  db_info_idx_0,  dnx_sw_state_dump_filters_t  filters)
{
    ...
    else
    {
        ...
        if(i0 >= dnx_sw_state_get_nof_elements(unit, MDB_KAPS_MODULE_ID, ((mdb_kaps_sw_state_t*)sw_state_roots_array[unit][MDB_KAPS_MODULE_ID])->db_info))
        {
        ...
        if(dnx_sw_state_get_nof_elements(unit, MDB_KAPS_MODULE_ID, ((mdb_kaps_sw_state_t*)sw_state_roots_array[unit][MDB_KAPS_MODULE_ID])->db_info) == 0)
        {
            SHR_EXIT();
        }
