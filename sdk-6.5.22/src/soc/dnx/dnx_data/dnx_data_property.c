    ...
    dnx_data_sch_flow_quartet_type_t * quartet_type)
{
    ...
    if (dnx_data_sch.flow.feature_get(unit, dnx_data_sch_flow_flex_quartet_supported))
    {
        ...
        if (region >= dnx_data_sch.flow.nof_connectors_only_regions_get(unit))
        {
            ...
            {
                propval =
                    dnx_drv_soc_property_suffix_num_only_suffix_str_get(unit, user_region, propkey,
                                                                        "base_region_core0_");
                if (propval == NULL)
                {
                    propval = dnx_drv_soc_property_suffix_num_only_suffix_str_get(unit, user_region, propkey, "core0");
                }
            }
            else if (core == 1)
            {
                ...
                if (propval == NULL)
                {
                    propval = dnx_drv_soc_property_suffix_num_only_suffix_str_get(unit, user_region, propkey, "core1");
                }
            }
    ...
    uint32 *is_macsec_enabled)
{
    ...
    *is_macsec_enabled =
        (dnx_data_macsec.
         general.feature_get(unit, dnx_data_macsec_general_macsec_block_exists)) & dnx_drv_soc_property_get(unit,
                                                                                                            spn_MACSEC_ENABLED,
                                                                                                            *is_macsec_enabled);

    if (*is_macsec_enabled > 1)
    {
        SHR_ERR_EXIT(_SHR_E_CONFIG,
