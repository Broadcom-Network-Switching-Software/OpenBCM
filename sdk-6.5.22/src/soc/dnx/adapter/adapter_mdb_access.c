...
#define ADAPTER_MDB_MEMORY_ID_INVALID (255)
    ...
    uint8 buffer[ADAPTER_MDB_ACCESS_MAX_BUFFER_SIZE_U8])
{
    ...
    table_payload_size = entry->payload_size;
    ...
    if (command == UPDATE_FRM_SOC_LOGICAL_ACCESS_WRITE)
    {
        ...
        SHR_BITCOPY_RANGE(entry_payload, 0, entry->payload, 0, table_payload_size);
    ...
    dbal_physical_entry_t * entry)
{
    ...
    if (FD_ISSET(adapter_mem_reg_fd, &read_vect))
    {
        ...
        SHR_BITCOPY_RANGE(entry->payload, 0, entry_payload, 0, ntohl(nw_order_payload_bit_size));
    }
    else
    {
        SHR_SET_CURRENT_ERR(_SHR_E_TIMEOUT);
