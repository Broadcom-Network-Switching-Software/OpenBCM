...
{
    uint8 entry_hit;
    dnx_field_tcam_core_e core = DNX_FIELD_ENTRY_TCAM_INTERNAL_DT_CORE_ID(entry_handle);
    ...
    uint32 flags)
{
    ...
    core = DNX_FIELD_ENTRY_TCAM_INTERNAL_DT_CORE_ID(entry->entry_hw_id);
    ...
    uint8 is_update)
{
    ...
    if (db_info.key_size == dnx_data_field.tcam.key_size_double_get(unit))
    {
        ...
        if (is_update)
        {
            ...
            SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_invalidate(unit, core, position, part, 0));
        }
    ...
    uint32 *access_id)
{
    ...
    for (bank_id = 0; bank_id < dnx_data_field.tcam.nof_banks_get(unit); bank_id++)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_tcam_bank_is_handler_on_bank(unit, bank_id, app_id, &handler_on_bank));
        if (handler_on_bank)
        {
    ...
    uint32 *access_id)
{
    ...
    else
    {
        SHR_IF_ERR_EXIT_NO_MSG(dnx_field_tcam_access_key_to_access_id_sw
                               (unit, core, app_id, key, key_mask, db_info.key_size, access_id));
    }
    ...
    uint8 is_move_operation)
{
    ...
    if (db_info.key_size == dnx_data_field.tcam.key_size_double_get(unit))
    {
        ...
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_invalidate(unit, core, second_part_double_position, part, 0));
    }
