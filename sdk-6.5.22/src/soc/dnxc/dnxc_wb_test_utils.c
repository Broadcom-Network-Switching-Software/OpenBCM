...
int dnxc_wb_warmboot_test_mode[SOC_MAX_NUM_DEVICES] = { 0 };

int dnxc_wb_override_wb_test[SOC_MAX_NUM_DEVICES] = { 0 };

int dnxc_wb_disable_once_wb_test[SOC_MAX_NUM_DEVICES] = { 0 };

int dnxc_wb_warmboot_test_counter[SOC_MAX_NUM_DEVICES] = { 0 };

int dnxc_warm_boot_exp_param[SOC_MAX_NUM_DEVICES] = { 0 };

int dnxc_nof_guaranteed_reboots[SOC_MAX_NUM_DEVICES] = { 0 };

int dnxc_wb_nested_api_counter[SOC_MAX_NUM_DEVICES] = { 0 };

int dnxc_wb_counter_filter[SOC_MAX_NUM_DEVICES][2] = { {0} };

int dnxc_wb_deinit_init_during_wb_test[SOC_MAX_NUM_DEVICES] = { 0 };
...
static void dnxc_wb_disable_once_get(
    int unit,
    int *wb_flag);
static void dnxc_wb_set_backoff_rate(
    int unit);
static void dnxc_wb_set_nof_guaranteed_reboots(
    int unit);
    ...
    int unit)
{
    ...
    if (!dnxc_wb_test_exp_backoff(unit, test_counter, &nof_skip))
    {
        if (nof_skip)
        {
...
void
dnxc_wb_set_backoff_rate(
    int unit)
{
    int warm_boot_exp_param = 10;

    if (soc_is(unit, DNX_DEVICE))
    {
#ifdef BCM_DNX_SUPPORT
        warm_boot_exp_param = dnx_data_dev_init.ha.warmboot_backoff_rate_get(unit);
#endif
    }
    else if (soc_is(unit, DNXF_DEVICE))
    {
#ifdef BCM_DNXF_SUPPORT
        warm_boot_exp_param = dnxf_data_dev_init.ha.warmboot_backoff_rate_get(unit);
#endif
    }

    dnxc_warm_boot_exp_param[unit] = warm_boot_exp_param;
}

void
dnxc_wb_set_nof_guaranteed_reboots(
    int unit)
{
    int nof_guaranteed_reboots = 5;

    if (soc_is(unit, DNX_DEVICE))
    {
#ifdef BCM_DNX_SUPPORT
        nof_guaranteed_reboots = dnx_data_dev_init.ha.nof_guaranteed_reboots_get(unit);
#endif
    }
    else if (soc_is(unit, DNXF_DEVICE))
    {
#ifdef BCM_DNXF_SUPPORT
        nof_guaranteed_reboots = dnxf_data_dev_init.ha.nof_guaranteed_reboots_get(unit);
#endif
    }

    dnxc_nof_guaranteed_reboots[unit] = nof_guaranteed_reboots;
}
    ...
    int *nof_skip)
{
    int skip = 0;
    int skip_determiner;
    int warm_boot_exp_param;
    int nof_guaranteed_reboots;
    int run_reboot = 0;

    if (test_counter == 1)
    {
        dnxc_wb_set_backoff_rate(unit);
        dnxc_wb_set_nof_guaranteed_reboots(unit);
    }
    warm_boot_exp_param = dnxc_warm_boot_exp_param[unit];
    nof_guaranteed_reboots = dnxc_nof_guaranteed_reboots[unit];
    skip_determiner = warm_boot_exp_param;

    while (skip_determiner <= test_counter / warm_boot_exp_param)
    {
        skip_determiner *= warm_boot_exp_param;
    }

    if ((test_counter <= nof_guaranteed_reboots) || ((test_counter % skip_determiner) == 0))
    {
        *nof_skip = 0;
        run_reboot = 1;
    }
    ...
    {
        skip = skip_determiner - (test_counter % skip_determiner);

        if (test_counter == (nof_guaranteed_reboots + 1))
        {
            *nof_skip = warm_boot_exp_param - nof_guaranteed_reboots;
        }
        else if ((nof_skip != NULL) && ((skip + 1) == skip_determiner))
        {
            *nof_skip = skip;
        }
