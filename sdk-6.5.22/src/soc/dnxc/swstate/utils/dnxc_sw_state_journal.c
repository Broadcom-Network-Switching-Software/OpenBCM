    ...
    char *start_msg)
{
    ...
    for (idx = 0; idx < print_upper_bound; idx++)
    {
        ...
        sal_strncat(dest, str_hex_value, sal_strnlen(str_hex_value,sizeof(str_hex_value)));
    }
    ...
    if (print_upper_bound != size)
    {
        ...
        sal_strncat(dest, str_repeats, sal_strnlen(str_repeats,sizeof(str_repeats)));
    }
    ...
    uint8 is_comparison)
{
    ...
    if(is_comparison)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_rollback_journal_distinct(unit, er_thread_id, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE, &dnxc_sw_state_journal_entries_are_distinct));
        SHR_IF_ERR_EXIT_NO_MSG(dnx_rollback_journal_rollback(unit, er_thread_id, FALSE, DNXC_SW_STATE_COMPARISON_JOURNAL_HANDLE));
    }
    else
    {
