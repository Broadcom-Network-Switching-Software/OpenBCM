...
STATIC void
_soc_td3_ser_reg_control_get(int unit,
                             const _soc_reg_ser_en_info_t *ser_en_info_list,
                             soc_reg_t reg, soc_reg_t   *ser_control_reg,
                             soc_mem_t *ser_control_mem,
                             soc_field_t *ser_enable_field)
{
    int i;
...
STATIC int
_soc_td3_reg_is_dyn(int unit, soc_reg_t reg)
{
...
STATIC int
_soc_td3_populate_ser_log_for_reg(int unit, soc_reg_t parity_enable_reg,
                                  soc_mem_t parity_enable_mem,
                                  soc_field_t parity_enable_field,
                                  soc_reg_t reg,
                                  int index,
                                  ...
                                  sal_usecs_t detect_time)
{
    ...
    if (reg != INVALIDr) {
        skip_pop = _soc_td3_reg_is_dyn(unit, reg) ||
                   ((parity_enable_reg == INVALIDr) &&
                   (parity_enable_mem == INVALIDm)) ||
                   (parity_enable_field == INVALIDf);
         ...
         */
        if (!skip_pop) {
            if (soc_td3_parity_bit_enable(unit, parity_enable_reg,
                                        parity_enable_mem,
                                        parity_enable_field,
                                        -1, 0) < 0) {
                return 0;
    ...
    if (NUM_PIPE(unit) == 1) {
        ...
        switch (*mem) {
            ...
            case L3_DEFIP_DATA_ONLYm:
                ...
                *mem = (*mem == L3_DEFIP_DATA_ONLYm ? (report_mem == L3_DEFIPm ?
                                                       L3_DEFIP_DATA_ONLYm :
 ...
 *      and so on...
 */
                        spci.parity_type = (ecc_parity == 0) ? SOC_PARITY_TYPE_PARITY :
                                            SOC_PARITY_TYPE_ECC;
                    } else {
                        spci.flags |= SOC_SER_REG_MEM_UNKNOWN;
                    }
                    ...
                    if (!disable_ser_correction && !l2_mgmt_ser_fifo) {
                        LOG_VERBOSE(BSL_LS_SOC_SER, (BSL_META_U(unit,
                            ...
                            unit, (spci.mem != INVALIDm) ? SOC_MEM_NAME(unit, spci.mem) :
                            "INVALIDm", spci.pipe_num));
#ifdef BCM_FLOWTRACKER_V2_SUPPORT
                        if ((soc_feature(unit,
                ...
                if (l2_mgmt_ser_fifo) {
                    ...
                    spci.reg = ainfo.reg;
                    _soc_td3_ser_reg_control_get(unit, ser_reg_info_table,
                                                 spci.reg,
                                                 &parity_enable_reg,
                                                 &parity_enable_mem,
                                                 &parity_enable_field);
                    spci.log_id = _soc_td3_populate_ser_log_for_reg(unit,
                                                                    parity_enable_reg,
                                                                    parity_enable_mem,
                                                                    parity_enable_field,
                                                                    spci.reg,
...
int soc_td3_ser_single_bit_report_for_bus_buf_get(int unit, int *enable)
{
    int i;
    soc_reg_t ecc_reg;
    soc_field_t ecc_fld;
    ...
    bus_ser_info = SOC_IP_BUS_SER_INFO(unit);

    if (NULL == bus_ser_info) {
        return SOC_E_UNAVAIL;
    }
