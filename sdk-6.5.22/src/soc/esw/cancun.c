...
int soc_cancun_chip_rev_validate(const uint32 chip_rev_id,
    ...
    if ((cancun_dev_id == BCM56275_DEVICE_ID) &&
        ...
        if (((dev_id == BCM56273_DEVICE_ID) && (rev_id == BCM56273_A0_REV_ID)) ||
            ...
            ((dev_id == BCM56175_DEVICE_ID) && (rev_id == BCM56175_A1_REV_ID)) ||
            ((dev_id == BCM56176_DEVICE_ID) && (rev_id == BCM56176_A1_REV_ID))) {
            return TRUE;
        }
    }
    ...
    if ((cancun_dev_id == BCM56470_DEVICE_ID) &&
        ...
        if (((dev_id == BCM56471_DEVICE_ID) && (rev_id == BCM56471_A0_REV_ID)) ||
            ...
            ((dev_id == BCM56474_DEVICE_ID) && (rev_id == BCM56474_A0_REV_ID))) {
            return TRUE;
        }
    }
#endif
...
int soc_cancun_file_load(int unit, uint8* buf, long buf_bytes, uint32* type,
    ...
    if(*type == CANCUN_SOC_FILE_TYPE_CIH) {
        ...
        cc->cch->status = status;
        /* CCH file load happens before init misc, hence setting HG3 feature here */
        rv = soc_cancun_branch_id_get(unit,
                       CANCUN_SOC_FILE_TYPE_CMH, &branch_id);
        if (branch_id == CANCUN_FILE_BRANCH_ID_HG3L) {
            SOC_FEATURE_SET(unit, soc_feature_higig3);
        }
...
/* Function to bind soc_feature to a
 * given CANCUN version and branch ID
 */
int _soc_cancun_bind_soc_features(int unit, uint32 running_cancun_ver,
                                 soc_cancun_file_branch_id_e running_branch_id) {
    int entry = 0;
    uint32 cancun_ver_low, cancun_ver_high;
    ...
    for (entry = 0; entry < COUNTOF(soc_feature_cancun_version_map); entry++) {
        ...
        branch_id = map_entry.branch_id;

        /* By default, the CANCUN version coming from the mapping assumes branch is always
         * default. For other branches like HGoE we need to update the branch ID information
         * in the CANCUN version passed */
        if (branch_id > CANCUN_FILE_BRANCH_ID_DEF) {
            if (cancun_ver_low != SOC_CANCUN_VERSION_DEF_INVALID) {
                cancun_ver_low = ((cancun_ver_low & CANCUN_VERSION_EXCLUDE_BRANCH_ID_MASK)
                                 | (branch_id << CANCUN_VERSION_OFFSET_BRANCH_ID));
            }
            if (cancun_ver_high != SOC_CANCUN_VERSION_DEF_INVALID) {
                cancun_ver_high = ((cancun_ver_high & CANCUN_VERSION_EXCLUDE_BRANCH_ID_MASK)
                                  | (branch_id << CANCUN_VERSION_OFFSET_BRANCH_ID));
            }
        } else if (branch_id == 0) {
             /* Default to default branch ID if not specified */
             branch_id = CANCUN_FILE_BRANCH_ID_DEF;
        }
        ...
        /* Ensure features are being set only on matching branches */
        if (running_branch_id == branch_id) {
            /* Sanity checks */
            ...
            if (cancun_ver_high != SOC_CANCUN_VERSION_DEF_INVALID) {
                if (cancun_ver_low != cancun_ver_high) {
...
/* For a given CANCUN branch version,
 * return the base CANCUN version
 */

int _soc_cancun_get_base_ver(int unit, uint32 branched_ver,
         uint32* base_ver, int* base_valid) {
    int entry = 0;

    *base_ver = SOC_CANCUN_VERSION_DEF_INVALID;
    *base_valid = 0;
    for (entry = 0; entry < COUNTOF(cancun_branch_version_map); entry++) {
        uint32 branch_cancun_ver, mapped_cancun_ver;
        cancun_branch_version_map_t cancun_map_entry;
        cancun_map_entry = cancun_branch_version_map[entry];
        branch_cancun_ver = cancun_map_entry.branch_cancun_ver;
        mapped_cancun_ver = cancun_map_entry.base_cancun_ver;

        if (SOC_INFO(unit).chip_type != cancun_map_entry.chip_type) {
            continue;
        }

        /* Sanity checks */
        if ((branch_cancun_ver == SOC_CANCUN_VERSION_DEF_INVALID) ||
            (mapped_cancun_ver == SOC_CANCUN_VERSION_DEF_INVALID)) {
                return SOC_E_INTERNAL;
        }

        if (branched_ver == branch_cancun_ver) {
            *base_ver = mapped_cancun_ver;
            *base_valid = 1;
            break;
        }
    }
...
int soc_cancun_version_soc_feature_bind(int unit) {
    int entry = 0;
    uint32 running_cancun_ver;
    soc_cancun_file_branch_id_e running_branch_id = 0;

    /* Get running cancun version */
    SOC_IF_ERROR_RETURN(soc_cancun_version_get(unit, &running_cancun_ver));
    /* Get branch ID of the CANCUN fetched */
    SOC_IF_ERROR_RETURN(soc_cancun_branch_id_get(unit,
                        CANCUN_SOC_FILE_TYPE_CIH, &running_branch_id));

    SOC_IF_ERROR_RETURN(_soc_cancun_bind_soc_features
                        (unit, running_cancun_ver, running_branch_id));

    /* Enable/Disable soc_features on the branch CANCUN identical
     * to the base CANCUN version
     */
    for (entry = 0; entry < COUNTOF(cancun_branch_version_map); entry++) {
        int base_valid = 0;
        uint32 branch_cancun_ver, base_cancun_ver;
        cancun_branch_version_map_t map_entry;

        map_entry = cancun_branch_version_map[entry];

        branch_cancun_ver = map_entry.branch_cancun_ver;

        /* Get the mapped base cancun for the present branch CANCUN */
        SOC_IF_ERROR_RETURN(_soc_cancun_get_base_ver(unit, branch_cancun_ver,
                            &base_cancun_ver, &base_valid));

        if (running_cancun_ver == branch_cancun_ver) {
            int base_branch_id = 0;
            uint32 mapped_cancun_ver = SOC_CANCUN_VERSION_DEF_INVALID;

            mapped_cancun_ver = base_cancun_ver;

            /*Keep binding soc_features with the base CANCUN
             * as long as mapping exists
             */
            while (base_valid) {
                base_branch_id = (mapped_cancun_ver >> CANCUN_VERSION_OFFSET_BRANCH_ID) & 0xF;
                /* Bind soc features based on the base CANCUN version */
                SOC_IF_ERROR_RETURN(_soc_cancun_bind_soc_features(unit,
                                    mapped_cancun_ver, base_branch_id));
                SOC_IF_ERROR_RETURN(_soc_cancun_get_base_ver(unit,
                                    mapped_cancun_ver, &mapped_cancun_ver, &base_valid));
            }
        }
    }
