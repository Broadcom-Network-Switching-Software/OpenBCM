...
/**
@name: tdm_mv2_ovsb_pkt_shaper_cpy_prev_pm_indx
@param:

Copy pkt_shaper pm_indx from previous pkt_shaper before flexing.
 */
static int
tdm_mv2_ovsb_pkt_shaper_cpy_prev_pm_indx(tdm_mod_t *_tdm)
{
    int hpipe, phy_port, grp_idx, i, phy_lo, phy_hi;
    int *cal;
    int *cal_prev;
    ...
    tdm_mv2_cmn_get_pipe_port_range(param_cal_id, &phy_lo, &phy_hi);

    for (hpipe = 0; hpipe < 2; hpipe ++) {
        grp_idx = MV2_SHAPING_GRP_IDX_0 + hpipe;
        /* Get the right pkt scheduler calendar*/
        switch (param_cal_id) {
            case 0:	cal = _tdm->_chip_data.cal_0.cal_grp[grp_idx]; break;
            case 1:	cal = _tdm->_chip_data.cal_1.cal_grp[grp_idx]; break;
            case 2:	cal = _tdm->_chip_data.cal_2.cal_grp[grp_idx]; break;
            case 3:	cal = _tdm->_chip_data.cal_3.cal_grp[grp_idx]; break;
            default:
                TDM_PRINT1("Invalid calendar ID - %0d\n", param_cal_id);
                return FAIL;
        }
        switch (param_cal_id) {
            case 0:	cal_prev = _tdm->_prev_chip_data.cal_0.cal_grp[grp_idx]; break;
            case 1:	cal_prev = _tdm->_prev_chip_data.cal_1.cal_grp[grp_idx]; break;
            case 2:	cal_prev = _tdm->_prev_chip_data.cal_2.cal_grp[grp_idx]; break;
            case 3:	cal_prev = _tdm->_prev_chip_data.cal_3.cal_grp[grp_idx]; break;
            default:
                TDM_PRINT1("Invalid calendar ID - %0d\n", param_cal_id);
                return FAIL;
        }
        ...
        /* copy */
        TDM_PRINT2("pkt_shaper_cal: [COPY] cal_id  %0d, hpipe %0d\n",
                   param_cal_id, hpipe);
        sal_memcpy(cal, cal_prev, sizeof(int) * MV2_SHAPING_GRP_LEN);

        /* update slots for down ports */
        for (phy_port = phy_lo; phy_port <= phy_hi; phy_port++) {
            if (param_states_prev[phy_port-1] ==
                PORT_STATE__FLEX_OVERSUB_DN) {
                for (i = 0; i < MV2_SHAPING_GRP_LEN; i++) {
                    if (cal[i] == phy_port) {
                        cal[i] = param_token_empty;
                        TDM_PRINT4("%s phy_port=%0d, slot=%0d, token=%0d\n",
                                   "pkt_shaper_cal: [DOWN] ",
                                   phy_port, i, param_token_empty);
                    }
                }
            }
        }
...
tdm_mv2_ovsb_pkt_shaper(tdm_mod_t *_tdm)
{
    ...
    if (_tdm->_chip_data.soc_pkg.flex_port_en == 1) {
        tdm_mv2_ovsb_pkt_shaper_cpy_prev_pm_indx(_tdm);
    }
