...
sand_adapter_signal_skip_check(
       int unit,
       int mdb_type,
       int stage_and_interf,
       dnx_pp_stage_e adapter_stage_index)
{
    kleap_stages_e ikleap_stage;
    dbal_physical_tables_e phy_db_table;
    ...
    if ( mdb_type == TCAM_MDB_TYPE )
    {
        ...
        if ( adapter_stage_index-DNX_PP_STAGE_LLR != stage_and_interf/1000 ||
             adapter_stage_index < DNX_PP_STAGE_VTT1 ||
             adapter_stage_index > DNX_PP_STAGE_IFWD2)
        {
            return 1;
        }
        ikleap_stage = dnx_ikleap_adapter_stage_to_ikleap_stage(unit, stage_and_interf/1000);

        
        if (stage_and_interf%1000 >= dnx_ikleap_stages_get(unit, ikleap_stage).nof_kbrs)
        {
            return 1;
        }
        phy_db_table = dnx_ikleap_stages_get(unit, ikleap_stage).kbr2physical[stage_and_interf%1000];

        
        if ( phy_db_table != DBAL_PHYSICAL_TABLE_TCAM )
        {
            return 1;
        }
    }
    ...
    pp_stage_t *cur_pp_stage)
{
    ...
    while ( data_index > 0)
    {
        ...
        skip_signal_flag = 0;
        if ( mdb_type == DIRECT_MDB_TYPE || mdb_type == EEDB_MDB_TYPE )
        {
            sal_snprintf(interface_index, RHSTRING_MAX_SIZE - 1, "%s", "");
        }
        else if ( dnx_ikleap_stages_get_interface_index(unit,
                stage_and_intrf/1000, stage_and_intrf%1000, interface_index) != _SHR_E_NONE )
        {
            skip_signal_flag = 1;
        }
        ...
        if ( sand_adapter_memory_id_and_type_2_db_id(unit, mdb_type, memory_id, &db_id) != _SHR_E_NONE ||
             sand_adapter_signal_skip_check(unit, mdb_type, stage_and_intrf, cur_pp_stage->pp_id) == 1)
        {
            skip_signal_flag = 1;
        }
