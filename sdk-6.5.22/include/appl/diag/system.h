...
#endif
/*
 * If this is any of DNXC compilations but DFE is also included then do
 * include drv.h
 */
 ...
 */
 ...
 extern void    diag_sdk_cleanup(void);
...
extern int ltsw_l2_learn_set(int unit, int enable);
...
extern int ctsw_pw_running(int unit);
extern int ctsw_pw_clean(int unit);
...
#if defined(BCM_CTSW_SUPPORT)
extern int
ctsw_show_counters_proc(int unit, bcm_pbmp_t pbmp, int flags);

extern int
ctsw_clear_counters_proc(int unit, bcm_pbmp_t pbmp);

extern int
ctsw_do_resync_counters(int unit, bcm_pbmp_t pbmp);
#endif /* BCM_CTSW_SUPPORT */
