 ...
 * \param [in,out] global_lif_info - global lif allocation info such as BTA and pointer to memory to write output into. \n
 ...
 *   \param [in] is_validate - Indication whether to perform
 *          validations.
 * \par INDIRECT INPUT
 *   None
 * \par DIRECT OUTPUT
 ...
 */
shr_error_e dnx_global_lif_reclassify_add(
    int unit,
    uint32 global_inlif,
    uint32 local_inlif);
 ...
 */
shr_error_e dnx_global_lif_reclassify_get(
    int unit,
    uint32 global_inlif,
    uint8 *found);
 ...
 *     Allocated/Given Global-In-LIF.
 */
shr_error_e dnx_global_lif_reclassify_delete(
    int unit,
    uint32 global_inlif);
