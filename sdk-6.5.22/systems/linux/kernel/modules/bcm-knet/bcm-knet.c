...
typedef struct bkn_switch_info_s {
    ...
    uint8_t oamp_local_ports_number; /* Indicates number of OAMP local ports */
    uint32_t oamp_local_ports[KCOM_HW_INIT_OAMP_LOCAL_PORTS_MAX];   /* OAMP local ports array */
    ...
    bkn_dune_system_header_info_t *packet_info)
{
    ...
    for (idx = 0; idx < sinfo->oamp_local_ports_number; idx++)
    {
        /*
         * Convert system port id to local port
         * Source_sys_port_aggregate = device_index * 1024 + local port
         */
        if ((packet_info->ftmh.source_sys_port_aggregate & BKN_DNX_SYSPORTS_PER_DEVICE_MASK) == sinfo->oamp_local_ports[idx])
        {
            is_oamp_punted = TRUE;
        }
     }
        ...
        if (pktlen > pkt_hdr_size + 18) {
            if (device_is_sand(sinfo)) {
                ...
                memcpy(sand_system_headers, pkt,
                       ((pkt_hdr_size > RCPU_RX_META_SIZE) ?
                        RCPU_RX_META_SIZE : pkt_hdr_size));
            ...
            filter = bkn_match_rx_pkt(sinfo, skb->data + pkt_hdr_size,
                                      pktlen - pkt_hdr_size, match_data,
                                      chan, &cbf);
    ...
    if (sinfo->tx.free > 1) {
        ...
        if (priv->flags & KCOM_NETIF_F_RCPU_ENCAP) {
            ...
            if (device_is_sand(sinfo)) {
                ...
                cpu_channel = (skb->data[28] << 24) | (skb->data[29] << 16) |
                              (skb->data[30] << 8) | (skb->data[31]);
            ...
            if (device_is_sand(sinfo) && priv->flags & KCOM_NETIF_F_ADD_SVTAG) {
                DBG_DUNE(("ADD SVTAG TAG\n"));
                if (skb_header_cloned(skb) || skb_headroom(skb) < 4) {
                    /* Current SKB cannot be modified */
                    DBG_SKB(("Realloc Tx SKB\n"));
                    headroom = KCOM_NETIF_SVTAG_SIZE;
                    tailroom = FCS_SZ;
                    new_skb = skb_copy_expand(skb,
                                              headroom + skb_headroom(skb),
                                              tailroom + skb_tailroom(skb),
                    ...
                    skb_push(new_skb, KCOM_NETIF_SVTAG_SIZE);
                    memcpy(new_skb->data, pktdata, hdrlen + 12);
                    bkn_skb_tstamp_copy(new_skb, skb);
                    dev_kfree_skb_any(skb);
                    skb = new_skb;
                } else {
                    /* Add svtag to existing buffer */
                    DBG_SKB(("Expand Tx SKB\n"));
                    skb_push(skb, KCOM_NETIF_SVTAG_SIZE);
                    for (idx = 0; idx < hdrlen + 12; idx++) {
                        skb->data[idx] = skb->data[idx + KCOM_NETIF_SVTAG_SIZE];
                    }
                }
                pktdata = skb->data;
                for (idx = 0; idx < KCOM_NETIF_SVTAG_SIZE; idx++) {
                    pktdata[hdrlen + 12 + idx] = priv->svtag[idx];
                }
                pktlen += KCOM_NETIF_SVTAG_SIZE;
            }
        }
...
bkn_knet_hw_init(kcom_msg_hw_init_t *kmsg, int len)
{
    ...
    if (device_is_sand(sinfo)) {
        ...
        if (kmsg->oamp_local_ports_number > KCOM_HW_INIT_OAMP_LOCAL_PORTS_MAX)
        {
            cfg_api_unlock(sinfo, &flags);
            gprintk("Number of OAMP ports(%d) is greater than the maximum value allowed(%d)\n",
                        kmsg->oamp_local_ports_number,
                        KCOM_HW_INIT_OAMP_LOCAL_PORTS_MAX);
            kmsg->hdr.status = KCOM_E_PARAM;
            return sizeof(kcom_msg_hdr_t);
        }
        sinfo->oamp_local_ports_number = kmsg->oamp_local_ports_number;
        for (idx = 0; idx < KCOM_HW_INIT_OAMP_LOCAL_PORTS_MAX; idx ++)
        {
            sinfo->oamp_local_ports[idx] = kmsg->oamp_local_ports[idx];
        }
    }
...
bkn_knet_netif_create(kcom_msg_netif_create_t *kmsg, int len)
{
    ...
    if (device_is_sand(sinfo)) {
        ...
        for (idx = 0; idx < KCOM_NETIF_SVTAG_SIZE; idx++)
        {
            priv->svtag[idx] = kmsg->netif.svtag[idx];
        }
    }
...
bkn_knet_netif_get(kcom_msg_netif_get_t *kmsg, int len)
{
    ...
    for (idx = 0; idx < KCOM_NETIF_SVTAG_SIZE; idx++) {
        kmsg->netif.svtag[idx] = priv->svtag[idx];
    }
    ...
    if (device_is_sand(sinfo)) {
        ...
        for (idx = 0; idx < sinfo->oamp_local_ports_number; idx++)
        {
            DBG_DUNE(("DNX OAMP ports [%d/%d]: %d\n",
                idx,
                sinfo->oamp_local_ports_number,
                sinfo->oamp_local_ports[idx]));
        }
    }
    return len;
}
